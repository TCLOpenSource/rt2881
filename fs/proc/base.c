...
#ifdef CONFIG_RTK_KDRV_KILLER
	trace_android_vh_write_oom_score_adj(task, oom_adj);
#endif /* CONFIG_RTK_KDRV_KILLER */
...
#ifdef CONFIG_RTK_KDRV_ADJUSTER
	trace_android_vh_update_oom_score_adj(task);
#endif
...
static int rtk_kill_tasks(struct file *file, int oom_score_adj)
{
	struct task_struct *task;
	...
	if(strstr(task->comm, "launcherx")) {
		pr_emerg("%s : Avoid to kill launcherX\n", __func__);
		return 0;
	}

	if (task->tgid != task->pid) {
		pr_emerg("%s : Only kill Thead Group ID\n", __func__);
		return -ESRCH;
	} else {
		pr_emerg("%s : Try to kill %s, oom score adj %d\n", __func__, task->comm, oom_score_adj);
	}

	return kill_something_info(SIGKILL, SEND_SIG_PRIV, task->pid);
}
#endif
					...
					size_t count, loff_t *ppos)
{
	...
	if (oom_score_adj >= 700) {
		err = rtk_kill_tasks(file, oom_score_adj);
	} 
#endif
