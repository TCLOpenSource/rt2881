...
static bool __maybe_unused is_unbalance_ddr_system(void)
{
						...
						const struct alloc_context *ac)
{
	...
	int nr_rescan = 0;
#ifdef CONFIG_ARM64
 #define ZONE_BW_LOWBIT  ZONE_DMA32
 #define ZONE_BW_HIGHBIT ZONE_NORMAL
#else
 #define ZONE_BW_LOWBIT  ZONE_NORMAL
 #define ZONE_BW_HIGHBIT ZONE_HIGHMEM
#endif
#endif
...
#if defined(CONFIG_ARCH_RTK6748)
	if (is_unbalance_ddr_system() && 
		(ac->highest_zoneidx == ZONE_BW_HIGHBIT) && 
		((gfp_mask & __GFP_MOVABLE) == 0) &&
		(zonelist_rescan == false) &&
		(nr_rescan == 0)) {
		zonelist_rescan = true;
//		pr_info("normal-rescan(%d), gfp=%lx, no_fallback=%d, alloc_flags=%lx, highest=%d, nr_rescan=%d\n", 
//				zone_idx(last_zone), gfp_mask, no_fallback, alloc_flags, ac->highest_zoneidx, nr_rescan);
	}
...
void __init set_movablecore(unsigned long pages)
{
	required_movablecore = pages;
}
