...
#ifdef CONFIG_RTK_KDRV_KILLER
DECLARE_HOOK(android_vh_write_oom_score_adj,
	TP_PROTO(struct task_struct *task, int oom_adj),
	TP_ARGS(task, oom_adj));
DECLARE_HOOK(android_vh_record_mlock_files,
	TP_PROTO(unsigned long start, size_t len, bool *killer_allow_lock),
	TP_ARGS(start, len, killer_allow_lock));
DECLARE_HOOK(android_vh_record_munlock_files,
	TP_PROTO(unsigned long start, size_t len),
	TP_ARGS(start, len));
#endif /* CONFIG_RTK_KDRV_KILLER */
#ifdef CONFIG_RTK_KDRV_ADJUSTER
DECLARE_HOOK(android_vh_update_oom_score_adj,
        TP_PROTO(struct task_struct *task),
        TP_ARGS(task));
#endif /* CONFIG_RTK_KDRV_ADJUSTER */
