	...
	if (PageHighMem(page)) {
		...
		while (size > 0) {
			...
			if (coherent_flag != COHERENT)
				__dmac_flush_range(ptr, ptr + PAGE_SIZE);
			kunmap_atomic(ptr);
			page++;
			size -= PAGE_SIZE;
		}
		...
		memset(ptr, 0, size);
		if (coherent_flag != COHERENT) {
			__dmac_flush_range(ptr, ptr + size);
...
#ifdef CONFIG_REALTEK_MEMORY_MANAGEMENT
	if (rtk_record_insert(page_to_pfn(page) << PAGE_SHIFT, RTK_SIGNATURE | DRIVER_ID | count_page, 0, __builtin_return_address(0), DVR_OWNER_NOID)) {
