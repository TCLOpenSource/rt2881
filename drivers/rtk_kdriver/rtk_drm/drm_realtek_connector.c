...
static int rtk_drm_add_extra_modes(struct drm_connector *connector)
{
	...
	struct rtk_drm_mode_data probed_data;

	rtk_drm_get_screen_size(&probed_data.hdisplay, &probed_data.vdisplay);
	probed_data.vrefresh = Get_DISPLAY_REFRESH_RATE();
	probed_data.reduced_blanking = false;
	probed_data.interlaced = false;
	probed_data.margins = false;

	mode = drm_cvt_mode(connector->dev, probed_data.hdisplay, probed_data.vdisplay, probed_data.vrefresh, 
		                probed_data.reduced_blanking, probed_data.interlaced, probed_data.margins);

	if (mode) {
		drm_mode_probed_add(connector, mode);
		DRM_ERROR("[rtk][drm] probed mode:%dx%d@%dHz\n",probed_data.hdisplay, probed_data.vdisplay, probed_data.vrefresh);
		num_modes++;
	}
#endif
	for (i = 0; i < ARRAY_SIZE(rtk_drm_extra_modes); i++) {
