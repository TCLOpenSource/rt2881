...
#if defined(ARM_AFBC)   //ARM AFBC
const uint64_t gdma_format_modifiers[]={
 DRM_FORMAT_MOD_LINEAR,
 DRM_FORMAT_MOD_ARM_AFBC(AFBC_FORMAT_MOD_BLOCK_SIZE_32x8 | AFBC_FORMAT_MOD_YTR | AFBC_FORMAT_MOD_SPARSE | AFBC_FORMAT_MOD_SPLIT),
 DRM_FORMAT_MOD_INVALID
};

#elif defined(IMG_TFBC) //IMG TFBC
const uint64_t gdma_format_modifiers[]={
 DRM_FORMAT_MOD_LINEAR,
...
#if defined(RTK_DRM_USE_ATOMIC)
static bool rtk_drm_format_mod_supported(struct drm_plane *plane, uint32_t format,
				     uint64_t modifier)
{
	bool ret = false;
	GDMA_DISPLAY_PLANE gdma_plane = rtk_drm_get_gdma_plane_index(drm_plane_index(plane));
	if(gdma_plane != GDMA_PLANE_OSD1 && gdma_plane != GDMA_PLANE_OSD2 && gdma_plane != GDMA_PLANE_OSD3)
		return false;

#if defined(ARM_AFBC)
	if((format == DRM_FORMAT_ABGR8888) && modifier == DRM_FORMAT_MOD_ARM_AFBC(AFBC_FORMAT_MOD_BLOCK_SIZE_32x8 |
		AFBC_FORMAT_MOD_YTR |
...
static void rtk_drm_plane_helper_atomic_disable(struct drm_plane *plane,
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 15, 0))
...
{
	rtk_drm_gdma_set_plane_enabled(plane->dev->dev, drm_plane_index(plane), false);
}
...

{
	...
	if (fb) {
		rtk_drm_gdma_set_plane_enabled(plane->dev->dev, drm_plane_index(plane), true);
...
#endif
	.atomic_disable = rtk_drm_plane_helper_atomic_disable,
...
static const struct drm_plane_funcs rtk_drm_plane_funcs = {
	...
	.format_mod_supported = rtk_drm_format_mod_supported,
};
#else
			   ...
			   const uint32_t src_x, const uint32_t src_y)
{
	...
	drm_mode_get_hv_timing(&new_crtc_state->mode, &hdisplay, &vdisplay);
	screen_w = (uint32_t) plane->dev->mode_config.max_width;
	screen_h = (uint32_t) plane->dev->mode_config.max_height;
	...
	case RTK_DRM_VERSION_GDMA_ADAPTER:
		if(drm_drv_uses_atomic_modeset(crtc->dev))
			rtk_drm_gdma_set_surface_atomic(crtc->dev->dev,
					      rtk_drm_get_gdma_plane_index(drm_plane_index(plane)), address, 0,
					      ...
					      screen_w, screen_h, fb->hot_x, fb->hot_y, pitch,
					      format, modifier,
					      255,
					      false);
		else
