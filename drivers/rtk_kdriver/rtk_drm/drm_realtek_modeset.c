...
#elif defined(ARM_AFBC)
		switch (mode_cmd->modifier[0]) {
			case DRM_FORMAT_MOD_ARM_AFBC(AFBC_FORMAT_MOD_BLOCK_SIZE_32x8 | AFBC_FORMAT_MOD_YTR | AFBC_FORMAT_MOD_SPARSE | AFBC_FORMAT_MOD_SPLIT):
			case DRM_FORMAT_MOD_LINEAR:
			case DRM_FORMAT_MOD_INVALID:
				break;
			default:
...
int rtk_drm_modeset_early_init(struct rtk_drm_private *dev_priv)
{
	...
	dev_priv->plane = rtk_drm_plane_create(dev, DRM_PLANE_TYPE_PRIMARY); //index 0 OSD1
	...
	dev_priv->cursor_plane = rtk_drm_plane_create(dev, DRM_PLANE_TYPE_CURSOR); //index 1 OSD3
	if (IS_ERR(dev_priv->cursor_plane)) {
		DRM_ERROR("failed to create a cursor plane\n");
		err = PTR_ERR(dev_priv->cursor_plane);
		goto err_config_cleanup;
	}

	dev_priv->crtc = rtk_drm_crtc_create(dev, 0, dev_priv->plane, dev_priv->cursor_plane);
	...
	dev_priv->overlay_plane = rtk_drm_plane_create(dev, DRM_PLANE_TYPE_OVERLAY); //index 2 OSD2
