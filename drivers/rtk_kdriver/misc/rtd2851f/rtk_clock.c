...
int rtk_cpufreq_get_PCB_ver(void)
{
	unsigned char lsadc_val = 0;
	int lsadc_return = -1;
	static int rtk_cpufreq_pcb_ver = 0;

	if(rtk_cpufreq_pcb_ver == 0)
	{
		//PCB ver define on LSADC index 5
		lsadc_return = rtk_lsadc_get_adcValue(5, &lsadc_val);
		rtd_pr_cpu_freq_info("%s: PCB lsadc(0x%x)\n", __func__, lsadc_val);
		if(lsadc_return == 0)
		{
			//mapping table from HW SD Jason_0128
			unsigned int lsadc_mapping[10] = {24, 51, 76, 101, 129, 150, 181, 202, 232, 255};
			unsigned int pcb_mapping[10] = {2,  3,  4,   5,   6,   1,  7,   8,   9,  10};
			unsigned int i;
			for(i = 0; i< 10; i++)
			{
				if(lsadc_val <= lsadc_mapping[i])
				{
					rtk_cpufreq_pcb_ver = pcb_mapping[i];
					break;
				}
			}
			rtd_pr_cpu_freq_info("%s: PCB ver(%d)\n", __func__, rtk_cpufreq_pcb_ver);
		}
		else //default set V1
		{
			rtd_pr_cpu_freq_info("%s: Error: read lsadc 5 fail(%x/%x)\n",__func__, lsadc_val, lsadc_return);
			rtk_cpufreq_pcb_ver = 1;
		}
	}
	return rtk_cpufreq_pcb_ver;
}
int rtk_set_voltage_select_pin(void)
{
	static int rtk_cpufreq_select_pin = -1;

	if(rtk_cpufreq_select_pin == -1)
	{
		if(get_product_type() == PRODUCT_TYPE_DIAS)
		{
			if(rtk_cpufreq_get_PCB_ver() == 1)
				rtk_cpufreq_select_pin = 0; //old pin //GPIO86
			else
				rtk_cpufreq_select_pin = 1; //new pin //GPIO115
		}
		else //PRODUCT_TYPE_DEMETER
		{
			if((rtk_cpufreq_get_PCB_ver() == 1) || (rtk_cpufreq_get_PCB_ver() == 2))
				rtk_cpufreq_select_pin = 0; //old pin //GPIO86
			else
				rtk_cpufreq_select_pin = 1; //new pin //GPIO115
		}
	}
	return rtk_cpufreq_select_pin;
}
void rtk_set_voltage_pin(int voltage_level)
{
	...
	{
		case SCPU_L:
		if(rtk_set_voltage_select_pin() == 0)
			rtk_set_io_direction("PIN_SCPU_VID_0",0);	//GPIO86
		else
			rtk_set_io_direction("PIN_SCPU_VID_1",0);	//GPIO115
		break;

		case SCPU_H:
		if(rtk_set_voltage_select_pin() == 0)
			rtk_set_io_direction("PIN_SCPU_VID_0",1);	//GPIO86
		else
			rtk_set_io_direction("PIN_SCPU_VID_1",1);	//GPIO115
		break;
