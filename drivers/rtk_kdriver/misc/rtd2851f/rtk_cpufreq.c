...
#define RTK_BOOTING_BOOSTMODE_ENABLE	(1)
#define RTK_BOOTING_BOOSTMODE_TIMEOUT_SEC (60)
...
#ifdef CONFIG_RTK_SUPPORT_BOOST_MODE
	if (policy_has_boost_freq(policy) && rtk_boost_board_support() == 1)
...
// check thermal degree for enable or not
// return 1 if support
extern int rtk_cpufreq_get_PCB_ver(void);
int rtk_boost_thermal_protect_disable = 0;
static int rtk_boost_board_support(void)
{
	//for demo board, depends on OTP value RL6866-4601
#ifdef CONFIG_REALTEK_VOLTAGE_CTRL
	static int rtk_boost_board_support_val = -1;
	int otp_value_mp_status_scpu_over_freq;

	//check OTP and PCB ver to determine support or not
	if(rtk_boost_board_support_val == -1)
	{
		//read 3049 will first access done bit (3050), if done bit is burned then read 3049 value
		otp_value_mp_status_scpu_over_freq = rtk_otp_field_read_int_by_name("mp_status_scpu_over_freq");
		rtd_pr_cpu_freq_info("rtk_boost_board_support OTP new(%d/%d)\n",
								otp_value_mp_status_scpu_over_freq, rtk_cpufreq_get_PCB_ver());

		if(otp_value_mp_status_scpu_over_freq == 1)
		{
#if 1 //also need to check PCB ver
			if(get_product_type() == PRODUCT_TYPE_DIAS)
			{
				if((rtk_cpufreq_get_PCB_ver() != 2) && (rtk_cpufreq_get_PCB_ver() != 3))
					rtk_boost_board_support_val = 1;
				else
					rtk_boost_board_support_val = 0;
			}
			else//PRODUCT_TYPE_DEMETER
			{
				if((rtk_cpufreq_get_PCB_ver() != 4) && (rtk_cpufreq_get_PCB_ver() != 5))
					rtk_boost_board_support_val = 1;
				else
					rtk_boost_board_support_val = 0;
			}
	...
	//check thermal protect trigger for boost or not ?
	if(rtk_boost_thermal_protect_disable == 1)
		return 0;
	else
...
static void rtk_cpufreq_set_boost_directly(bool state) //thermal protect/supsend/timeout
{
	...
	//if enable, need check support or not?
	if(state == 1)
		if( rtk_boost_board_support() == 0)
			return;
...
#ifdef CONFIG_RTK_SUPPORT_BOOST_MODE_TIMEOUT
int rtk_boost_booting_enable(void)
{
#ifdef RTK_BOOTING_BOOSTMODE_ENABLE
	rtk_boost_reset_timeout_value(RTK_BOOTING_BOOSTMODE_TIMEOUT_SEC);
	rtk_cpufreq_set_boost_directly(1);
	return 1;
#else
	return 0;
#endif
...
#ifdef CONFIG_RTK_SUPPORT_BOOST_MODE
	//if boost mode not support, then remove the cpufreq_driver.set_boost to NOT create the node
	if( rtk_boost_board_support() == 0)
	{
		rtd_pr_cpu_freq_info("%s: remove boost mode node\n", __func__);
		rtk_cpufreq_driver.set_boost = 0;
	}
#endif
...
#endif
#ifdef CONFIG_RTK_SUPPORT_BOOST_MODE_TIMEOUT
	rtd_pr_cpu_freq_info("%s : set boot time boost(%d)\n", __func__, rtk_boost_booting_enable());
#endif
	return 0;
register_fail:
	return ret;
...
#define CPUFREQ_LEVEL_DISABLE_BOOST_PARAMETER (5)
#define CPUFREQ_LEVEL_DISABLE_BOOST_UP (CPUFREQ_LEVEL_0_UP_DEGREE - CPUFREQ_LEVEL_DISABLE_BOOST_PARAMETER)
#define CPUFREQ_LEVEL_DISABLE_BOOST_DOWN       (CPUFREQ_LEVEL_DISABLE_BOOST_UP - 5)
...
void cpu_thermal_callback(void *data, int cur_degree , int reg_degree, char *module_name)
{
	...
	if(cur_degree >= CPUFREQ_LEVEL_DISABLE_BOOST_UP)
	{
		//turn off boost mode support
		rtk_cpufreq_set_boost_directly(0);
		rtk_boost_thermal_protect_disable = 1;
	}

	if((rtk_boost_thermal_protect_disable == 1) && (cur_degree < CPUFREQ_LEVEL_DISABLE_BOOST_DOWN))
	{
		//trun on boost mode support
		rtk_boost_thermal_protect_disable = 0;
	}
...
int register_cpu_thermal_handler(register_temp_fun_ptr opt)
{
	...
	if ((ret = opt(CPUFREQ_LEVEL_DISABLE_BOOST_DOWN  - 10, cpu_thermal_callback, &temp, "cpufreq")) < 0)
