...
extern unsigned char judge_source_support_video_delay(VSC_INPUT_TYPE_T srctype);
...
#endif
{
	if(I2R_PVR_MAIN_flag){
		...
		if ( Scaler_DispGetInfoPtr() != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE ){
			unsigned char isEnter_Hist = Enter_Hist;
			if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 1500)
			{
				static unsigned int f_cnt = 0;
				if (f_cnt++%3 != 0)
					isEnter_Hist = FALSE;
			}
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	{//seamless source (hdmi or vdec) record capture block and clear m capture done status for freeze
		if(get_vsc_src_is_hdmi_or_dp() && hdmi_position_seamless && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && get_mdomain_driver_status())
	...
	if(handle_ch1_end_flag && hdmi_finish_seamless)
	{
		...
		spin_unlock(get_hdr_seamless_change_spinlock());//unlock i3ddma change  spinlock
		seamless_info_sharememory = (seamless_change_sync_info *)Scaler_GetShareMemVirAddr(SCALERIOC_SEAMLESS_CHANGE_SHAREMEMORY_INFO_SYNC_FLAG);
        if(get_vsc_src_is_hdmi_or_dp() && (get_hdr_semaless_active()))
        {
        	if(seamless_info_sharememory && seamless_info_sharememory->hdmi_not_ready) {
	           	if (RPC_SysInfo) {
	...
	if (handle_ch1_sta_flag == 1 && SysInfo != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) {
		unsigned char isEnter_Hist = TRUE;
		if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 1500)
		{
				static unsigned int f_cnt = 0;
				if (f_cnt++%3 != 0)
