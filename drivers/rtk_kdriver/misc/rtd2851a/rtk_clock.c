...
{
	//dvfs_func[0].func=fn;
	rtd_pr_cpu_freq_err("DDK call register_gpu_dvfs_callback, but will not apply it due to no dvs\n");
	return 0;
}
EXPORT_SYMBOL(register_gpu_dvfs_callback);
...
int __rtk_clk_set_rate_ex(struct clk *clk, unsigned int rate, int algo_id, int cpu, int idx)
{
	...
	if(new_state == pre_state) // R state switch
	{
		rtk_clk_set_pll(info, rate, rate);
#if 0 //no used on 2851A, only support one P freq
	...
	else
	{
		...
		//step1: set pll directly
		rtk_clk_set_pll(info, pll, pll);
