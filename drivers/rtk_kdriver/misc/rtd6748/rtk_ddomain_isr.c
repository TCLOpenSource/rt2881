...
#endif
/*LD DEMO*/
extern unsigned char ucLD_Init_Done;
...
extern void Scaler_Set_Reset_VODMA_Measure_Flag(unsigned int flag);
extern unsigned int Scaler_Get_Reset_VODMA_Measure_Flag(void);
extern void Scaler_Set_Reset_VODMA_UzuLc(unsigned int uzuLc);
extern unsigned int Scaler_Get_Reset_VODMA_UzuLc(void);
#endif

#define RESET_VODMA_INTERRUPT_PERIOD_THRESHOLD_24HZ 41*90 //8ms
#define RESET_VODMA_INTERRUPT_PERIOD_THRESHOLD_25HZ 40*90 //8mfs
...
irqreturn_t ddomain_isr(int irq, void *dev_id)
{
	...
	UINT8 handle_dtg_vlc_status_flag = 0;
...
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
	static UINT8 b_resetvodma_measure = 0;
	static UINT8 b_pre_resetvodma_measure = 0;
	static ppoverlay_dtg_ie_2_RBUS resv_ppoverlay_dtg_ie_2_reg;
#endif
	...
	handle_dtg_vlc_status_flag = dtg_pending_status_2_reg.dtg_vlc_status_2;
	...
	if (dtg_ie_2_reg.dtg_vlc_ie_2 && handle_dtg_vlc_status_flag) {// vlc isr

		fwif_color_GLC_VDF_ISR();

		//clear status
		dtg_pending_status_2_reg.regValue=0;
		dtg_pending_status_2_reg.dtg_vlc_status_2 = 1;//write clear status
...
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
						|| (Scaler_Get_Reset_VODMA_Measure_Flag() == 1 && !b_resetvodma_measure)
#endif
					)
#endif
				{
...
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
					if(Scaler_Get_Reset_VODMA_Measure_Flag() == 1)
					{
						ppoverlay_dtg_ie_2_RBUS temp_dtg_ie_2_reg;
						resv_ppoverlay_dtg_ie_2_reg.regValue = IoReg_Read32(PPOVERLAY_DTG_ie_2_reg); // reserve origin ie reg val
						temp_dtg_ie_2_reg.dtg_ie_2 = 1;
						temp_dtg_ie_2_reg.ivs_rm_rising_ie_2 = 1; // enable only ivs_rm avoid other interrupt interfere
						IoReg_Write32(PPOVERLAY_DTG_ie_2_reg, temp_dtg_ie_2_reg.regValue);
						rtd_pr_ddomain_isr_notice("[voreset]DTG_ie set (%x)", temp_dtg_ie_2_reg.regValue);
						b_resetvodma_measure = TRUE;
					}
#endif
	...
	if (dtg_ie_2_reg.mv_den_sta_event_ie_2 && handle_mv_den_sta_flag) {//data start isr
		...
		if((Scaler_Get_Reset_VODMA_Flag()  && b_resetvodma) || (!Scaler_Get_Reset_VODMA_Flag() && Scaler_Get_Reset_VODMA_DisableEnable_Cancel_Flag() && b_resetvodma)){
			...
			{
				Scaler_Set_Reset_VODMA_Flag(_DISABLE);
				b_pre_resetvodma_measure = false;
			}
#endif
		}
	}
	...
	{
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
#define RESETVODMA_UZULC_THRESHOLD 10
#define RESETVODMA_MEASURE_THRESHOLD 5
		if(b_resetvodma_measure)
		{
			static int resetvodma_measure_ct = 0;
			unsigned int resetvodma_uzuLc = PPOVERLAY_new_meas2_linecnt_real_get_uzudtg_dly_line_cnt_rt(IoReg_Read32(PPOVERLAY_new_meas2_linecnt_real_reg));
			if(resetvodma_uzuLc < RESETVODMA_UZULC_THRESHOLD)
			{
				Scaler_Set_Reset_VODMA_UzuLc(resetvodma_uzuLc);
				rtd_pr_ddomain_isr_notice("[voreset]ivs resetvodma_uzuLc = %d", resetvodma_uzuLc);
				b_resetvodma_measure = FALSE;
				Scaler_Set_Reset_VODMA_Measure_Flag(0);
				b_pre_resetvodma_measure = TRUE;
				IoReg_Write32(PPOVERLAY_DTG_ie_2_reg, resv_ppoverlay_dtg_ie_2_reg.regValue);
				rtd_pr_ddomain_isr_notice("[voreset]DTG_ie restore (%x)", resv_ppoverlay_dtg_ie_2_reg.regValue);
				resetvodma_measure_ct = 0;
			}
			else
			{
				++resetvodma_measure_ct;
				if(resetvodma_measure_ct >= RESETVODMA_MEASURE_THRESHOLD)
				{
					rtd_pr_ddomain_isr_notice("[voreset]RESET_VODMA_BY_MEASURE Failed");
					Scaler_Set_Reset_VODMA_UzuLc(0);
					b_resetvodma_measure = FALSE;
					Scaler_Set_Reset_VODMA_Measure_Flag(2);
					b_pre_resetvodma_measure = TRUE;
					IoReg_Write32(PPOVERLAY_DTG_ie_2_reg, resv_ppoverlay_dtg_ie_2_reg.regValue);
					rtd_pr_ddomain_isr_notice("[voreset]DTG_ie restore (%x)", resv_ppoverlay_dtg_ie_2_reg.regValue);
					resetvodma_measure_ct = 0;
				...
				{
					rtd_pr_ddomain_isr_notice("[voreset]RESET_VODMA_BY_MEASURE ct=%d uzuLc=%d", resetvodma_measure_ct, resetvodma_uzuLc);
				}
			}
		}
#endif
	...
	if (dtg_ie_2_reg.memcdtgv_den_sta_event_ie_2 && handle_memcdtgv_den_sta_flag) {//memc data start isr
		if (!LDDemoMode_init_done && ucLD_Init_Done)			
	...
	if (ldspi_rx_data_ctrl_reg.tx_done_int_en && ldspi_wclr_ctrl_reg.tx_done && ldspi_rx_data_ctrl_reg.ld_spi_int2_en){ // tx_done isr

		if (gLDSetLUT_flag==1) {
			drvif_color_set_LDSPI_DataSRAM_Data_Continuous(&tLD_LDSPI_DataSRAM, 0);
			gLDSetLUT_flag = 0;
		}
		// Hack Rhal behavior -- true switch: vpq_led_LocalDimmingDemoCtrlMode
		if (drvif_color_get_Temp() ){
			if (!LDDemoMode_init_done){
				fwif_color_set_LED_LDCtrlDemoMode(1);
				LD_tx_done_cnt = 0;
				rtd_printk(KERN_EMERG, "VPQ_ISR", "set LDDemoMode_init_done=%d\n", LDDemoMode_init_done);
			}
	...
	if (ldspi_rx_data_ctrl_reg.tx_done_int_en && ldspi_wclr_ctrl_reg.tx_done_2 && ldspi_rx_data_ctrl_reg.ld_spi_int2_en){ // tx_done_1 isr
	...
	if (ldspi_rx_data_ctrl_reg.tx_done_int_en && ldspi_wclr_ctrl_reg.tx_done_3 && ldspi_rx_data_ctrl_reg.ld_spi_int2_en){ // tx_done_1 isr
	...
	if (ldspi_rx_data_ctrl_reg.sram_hw_write_done_int_en && ldspi_wclr_ctrl_reg.sram_hw_write_done && ldspi_rx_data_ctrl_reg.ld_spi_int2_en){ // sram_hw_write_done_int_en isr
