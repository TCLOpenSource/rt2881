...
void rtk_set_voltage_pin_two_pin(int voltage_level)
{
	//contrl PIN_SCPU_VID_0/PIN_SCPU_VID_1
...
void rtk_set_voltage_pin_one_pin(int voltage_level)
{
	//contrl PIN_SCPU_VID_0 only, PIN_SCPU_VID_1 is control by bootcode
	switch(voltage_level)
	{
		case SCPU_L_L:
		case SCPU_L_H:
...
int rtk_cpu_use_one_pin = 0;
void rtk_set_voltage_pin(int voltage_level)
{
	if(rtk_cpu_use_one_pin == 1)
		rtk_set_voltage_pin_one_pin(voltage_level);
	else
		rtk_set_voltage_pin_two_pin(voltage_level);
}
...
void rtk_set_voltage(struct rtk_cpufreq_info *info, unsigned int pll, unsigned int rate)
{
	...
	rtd_pr_cpu_freq_info("rtk_set_voltage: pll(%d), rate(%d), vol(%d/%d)\n",pll, rate, voltage_level, old_voltage_level);
...
tscpu_freq_map rtk_scpu_freq_map[]= {
	...
	{ 0x071D2D00, 1320000, 1173000 }, //1.32
