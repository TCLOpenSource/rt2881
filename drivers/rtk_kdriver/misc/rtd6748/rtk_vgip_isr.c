...
static int vgip_isr_resume(struct platform_device *dev)
{
	...
	rtd_pr_vgip_isr_notice("[VGIP_ISR] nnip int=0x%x\n",rtd_inl(SYS_REG_INT_CTRL_SCPU_reg));
...
unsigned char Get_Val_hdmi_position_seamless(void)
{
	unsigned char result = FALSE;

	spin_lock(get_i3ddma_change_spinlock());
	result = hdmi_position_seamless;
	spin_unlock(get_i3ddma_change_spinlock());
...
{//call by dma vgip interrupt
	...
	extern unsigned char Get_Val_latency_pattern_change(void);
	...
	if(Get_Val_latency_pattern_change())
...
extern VIP_AUTOMA_Time_CTRL gVIP_AUTOMA_Time_Profile;
...
#endif
{
	unsigned int *pTime_Profile = &gVIP_AUTOMA_Time_Profile.pq_automa_task_profile[0];
	if(I2R_PVR_MAIN_flag){
		if ( Scaler_DispGetInfoPtr() != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE ){
			...
			if(Enter_Hist && !scalerVIP_is_karaoke_4k60()){
				pTime_Profile[0] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
				scalerVIP_colorHistISR(_CHANNEL1);  // Erin
				pTime_Profile[0] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)) - pTime_Profile[0];
				if(fwif_color_GetAutoMAFlag()) {
					pTime_Profile[1] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
					scalerVIP_colorMaAutoISR(_CHANNEL1); //Thur 20070920
					pTime_Profile[1] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)) - pTime_Profile[1];
					if( g_IsNot4KVO && Scaler_Get3DEnable() && scalerVIP_check_vdc_hw_pwr() && AVD_Clock_Flag_detection()) {
						pTime_Profile[2] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
						scalerVideo_adaptive3DAdjust();
						pTime_Profile[2] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)) - pTime_Profile[2];
						/*
	...
	// AI application
		if(bAIInited)
		{
			...
			pTime_Profile[3] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
			scalerAI_postprocessing();
			pTime_Profile[3] = TIMER_SCPU_CLK90K_LO_get_val(IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)) - pTime_Profile[3];
			// end AI
...
#endif
		if (gVIP_AUTOMA_Time_Profile.pq_automa_task_log_en == 1)
			rtd_pr_vgip_isr_info("pq_automa,HistISR=%d,MaAutoISR=%d,3DAdjust=%d,AI=%d,\n", 
				pTime_Profile[0], pTime_Profile[1], pTime_Profile[2], pTime_Profile[3]);
...
extern unsigned int Scaler_Get_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(void);
extern void Scaler_Set_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(unsigned int ucEnable);
extern void Scaler_Set_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv(unsigned int iv2dv);
extern unsigned int Scaler_Get_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv(void);
extern void framesync_set_iv2dv_delay_disable_db(unsigned int iv2dv);

extern void set_enable_ivs_cnt_ori_uninterrupted_2_frame(unsigned char enable);
extern unsigned char get_enable_ivs_cnt_ori_uninterrupted_2_frame(void);
extern void set_uninterrupted_ivs_cnt_ori_for_2_frame(unsigned char index, unsigned int cnt);
...
extern void dynamic_change_hdr_setting_hdmi_finish(unsigned char hdr_enable, I3DDMA_COLOR_SPACE_T color_format, unsigned char hdr_type);
    ...
    static unsigned char ivs_ori_index = 0;
    ...
    //read 2 frame ivs ori
    if(handle_ch1_sta_flag)
    {
        if(get_enable_ivs_cnt_ori_uninterrupted_2_frame())
        {
            ppoverlay_ivs_cnt_3_reg.regValue = IoReg_Read32(PPOVERLAY_IVS_cnt_3_reg);
            set_uninterrupted_ivs_cnt_ori_for_2_frame(ivs_ori_index, ppoverlay_ivs_cnt_3_reg.ivs_cnt_ori);
            ivs_ori_index++;
        }
        else
        {
        ...
        if(ivs_ori_index > 1)
        {
            set_enable_ivs_cnt_ori_uninterrupted_2_frame(0);
            ivs_ori_index = 0;
        }
    }
	/*Handle SMF BBD*/
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if(handle_ch1_sta_flag && Get_Val_hdmi_position_seamless())
	{
		spin_lock(get_i3ddma_change_spinlock());//lock i3ddma change spinlock
...
if((reg_value & BIT(29))){
    ...
    if(handle_ch1_end_flag)
    {
        if(Scaler_Get_Reset_VODMA_Vgip_Hs_Iv2dv_Flag())
        {
            unsigned int iv2dv_delay = Scaler_Get_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv();
            vgip_vgip_chn1_ctrl_RBUS vgip_chn1_ctrl_reg;
            ...
            rtd_pr_vgip_isr_info("Set ch1_vs_by_hs_en_n %d (%x) @ vgipLc(%d)", vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n, IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg)
                , VGIP_VGIP_CHN1_LC_get_ch1_line_cnt(IoReg_Read32(VGIP_VGIP_CHN1_LC_reg)));

            framesync_set_iv2dv_delay_disable_db(iv2dv_delay);
