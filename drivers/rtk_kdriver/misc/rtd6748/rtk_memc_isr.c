...
unsigned int g_memc_input_vsync_cnt = 0;
unsigned int g_memc_outpur_vsync_cnt = 0;
unsigned int g_memc_outpur_vsync_cnt_2 = 0;
...
static irqreturn_t memc_isr(int irq, void *dev_id)
{
	...
	if(Mid_MISC_GetOutINTStatus(INT_TYPE_VSYNC))
	{
		...
		if(Mid_MISC_GetOutINTEnable(INT_TYPE_VSYNC)){
			...
			g_memc_outpur_vsync_cnt++;
			g_memc_outpur_vsync_cnt_2++;
		}
		#ifdef MEMC_ISR_Times_debug		
		sEnterCnt_out_V++;
		/*if(currentPts - sLastPts_1s > 90090){			
...
static int memc_isr_irq_number = 0;
static int memc_isr_set_affinity_enable = 0;
void set_memc_isr_affinity(void)
{
    struct cpumask mask;
	// if (memc_isr_irq_number != 0 && memc_isr_set_affinity_enable == 0) {
	cpumask_clear(&mask);
	cpumask_set_cpu(0, &mask);
	cpumask_set_cpu(1, &mask);
	irq_set_affinity(memc_isr_irq_number, &mask);
	memc_isr_set_affinity_enable = 1;
	rtd_pr_memc_notice("[0314] MEMC ISR WORKAROUND Done!");
	// }
}

void reset_memc_isr_affinity(void)
{
    struct cpumask mask;
	if (memc_isr_irq_number != 0 && memc_isr_set_affinity_enable == 1) {
		cpumask_clear(&mask);
		cpumask_set_cpu(0, &mask);
		cpumask_set_cpu(1, &mask);
		...
		irq_set_affinity(memc_isr_irq_number, &mask);
		memc_isr_set_affinity_enable = 0;
	}
}
...
static int __init memc_isr_init_irq(struct platform_device *pdev)
{
	...
	rtd_pr_memc_notice("[0314] MEMC Workaround Get IRQ!");

	memc_isr_irq_number = irq;
	...
	rtd_pr_memc_notice("[0314] MEMC Workaround Start setup!");
	...
	rtd_pr_memc_notice("[0314] MEMC Workaround Start!");
	set_memc_isr_affinity();
	return 0;
}
