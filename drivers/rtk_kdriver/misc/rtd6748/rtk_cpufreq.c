...
#define PLL_STATUS_REG	(0xB80003A0)
#define PLL_STATUS_MASK	(0x0007FFFF)
#define PLL_STATUS_SHIFT (0)
#define PLL_STATUS_STABLE_UPPER_BOUND	0x40000
#define PLL_STATUS_STABLE_LOWER_BOUND	0xB400
#define PLL_STATUS_RESULT()	((rtd_inl(PLL_STATUS_REG) & PLL_STATUS_MASK) >>  PLL_STATUS_SHIFT)

//check PLL stable, the PLL status value match lower and upper bound
__maybe_unused static int rtk_boost_check_PLL_stable(void)
{
	unsigned int pll_stable_result = PLL_STATUS_RESULT();
	if((pll_stable_result < PLL_STATUS_STABLE_UPPER_BOUND) && (pll_stable_result > PLL_STATUS_STABLE_LOWER_BOUND))
		return 1;
	else
		return 0;
}
...
// check PLL stable
// return 1 if support
static int rtk_boost_board_support(int check_PLL)
{
	//for demo board, depends on rtk_cpu_dvs_enable(hw_cpu_package_size OTP) and mp_scpu_over_freq(OTP)
#ifdef CONFIG_REALTEK_VOLTAGE_CTRL
	...
	if(rtk_boost_board_support_val == -1)
	{
		...
		if((otp_value_scpu_over_freq == 0) && (rtk_cpu_dvs_enable == 1))
			rtk_boost_board_support_val = 1;
		else
			rtk_boost_board_support_val = 0;
	}

	//need to consider the PLL stable status
	if(check_PLL == 1)
	{
		if(rtk_boost_check_PLL_stable() == 1)
...
#ifdef CONFIG_RTK_SUPPORT_BOOST_MODE
	if (policy_has_boost_freq(policy) && rtk_boost_board_support(0) == 1)
...
#endif
{
	...
	if( rtk_boost_board_support(1) == 0)
		return -EINVAL;
...
#ifdef CONFIG_REALTEK_VOLTAGE_CTRL
extern unsigned int rtk_cpu_use_one_pin;
#endif
...
static int rtk_cpufreq_driver_init(void)
{
	...
	if(rtk_cpu_dvfs_disable == 0)
	{
		...
		if((package_value == PACKAGE_BGA25_22_7) || (package_value == PACKAGE_BGA22_7_CN2) || (package_value == PACKAGE_BGA22_7_CN3))
			rtk_cpu_dvs_enable = 1;

		//check CN2 board
		//read OTP check IDDQ done bit (both SCPU/CORE share same)
		//read lsadc to make sure is R+ ver.2 board (value > 0xE7)

		if(package_value == PACKAGE_BGA22_7_CN2)
		{
			int iddq_check = -1;
			unsigned char lsadc_val = 0;
			int lsadc_return = -1;
			int isPCB_ver_1 = 0;

			lsadc_return = rtk_lsadc_get_adcValue(1, &lsadc_val);
			iddq_check = rtk_get_cp_iddq_scpu_power();
			if((lsadc_val >= 0x45) && (lsadc_val <= 0x64))
				isPCB_ver_1 = 1;

			if((lsadc_return == 0) && (isPCB_ver_1 == 0) && (iddq_check != -1))
				rtk_cpu_use_one_pin = 1;

			rtd_pr_cpu_freq_info("%s: lsadc(%x/%x), iddq_check(%x), isPCB_ver_1(%x)\n",__func__, lsadc_val, lsadc_return, iddq_check, isPCB_ver_1);
		}
		else if(package_value == PACKAGE_BGA22_7_CN3)
		{
			// no need to check PCB version
			rtk_cpu_use_one_pin = 1;
		}
		rtd_pr_cpu_freq_info("%s: rtk_cpu_use_one_pin(%d)\n", __func__, rtk_cpu_use_one_pin);
#endif
	...
	if( rtk_boost_board_support(0) == 0)
	{
