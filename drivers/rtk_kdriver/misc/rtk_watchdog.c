	...
	if(enable_check_av_watchgod)
	{
		...
		if (watchdog_task == 0)
		{
			...
			break;
                ...
                if((rtd_inl(0xb80ca4d0) & 0x3ffff) != 0 || (rtd_inl(0xb80ca4d4) & 0x3ffff) != 0   || (rtd_inl(0xb80ca6d0) & 0x3ffff) != 0  || (rtd_inl(0xb80ca6d4) & 0x3ffff) != 0  || (rtd_inl(0xb80c98d0) & 0x3ffff) != 0  || (rtd_inl(0xb80c98d4) & 0x3ffff) != 0  || (rtd_inl(0xb80c9ad0) & 0x3ffff) != 0 || (rtd_inl(0xb80c9ad4) & 0x3ffff) != 0 )
                {
                    rtd_pr_misc_err("******DDR RX FIFO mismatch !!!!!******\n");
		    rtd_pr_misc_err("[0xb80ca4d0] = %x\n", rtd_inl(0xb80ca4d0));
	            rtd_pr_misc_err("[0xb80ca4d4] = %x\n", rtd_inl(0xb80ca4d4));
	            rtd_pr_misc_err("[0xb80ca6d0] = %x\n", rtd_inl(0xb80ca6d0));
	            rtd_pr_misc_err("[0xb80ca6d4] = %x\n", rtd_inl(0xb80ca6d4));
	            rtd_pr_misc_err("[0xb80c98d0] = %x\n", rtd_inl(0xb80c98d0));
	            rtd_pr_misc_err("[0xb80c98d4] = %x\n", rtd_inl(0xb80c98d4));
	            rtd_pr_misc_err("[0xb80c9ad0] = %x\n", rtd_inl(0xb80c9ad0));
	            rtd_pr_misc_err("[0xb80c9ad4] = %x\n", rtd_inl(0xb80c9ad4));
		}
...
#elif defined (CONFIG_ARCH_RTK2819A)
        iRegVal = rtd_inl(ISO_GPIO_GPDATO_1_reg);
        iRegVal |= (1<<ISO_GPIO_GPDATO_1_gpdato43_shift);//pull low to POWER_EN pin
        rtd_outl(ISO_GPIO_GPDATO_1_reg, iRegVal);
#else		
...
static long rtk_wdog_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret;
...
#if defined(CONFIG_ARM64) && defined(CONFIG_COMPAT)
	.compat_ioctl = rtk_wdog_compat_ioctl,
#endif
};
...
static void watchdog_shutdown(struct platform_device *pdev)
{
#if  (IS_ENABLED(CONFIG_ARCH_RTK6748) || IS_ENABLED(CONFIG_ARCH_RTK2885P))
        rtd_setbits(WDOG_FCCTRL_reg, 0x02);//cnt clear
#endif
        rtd_pr_misc_err( "do watchdog_shutdown\n");
}
