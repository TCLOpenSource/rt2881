...
extern unsigned char newbase_hdmi_get_vsem_is_dolby(unsigned char port);
extern unsigned int Get_fake_scaler_connect_impl(void);

/* Variables */

DOLBY_HDMI_VSIF_T pre_dolby_hdmi_vsif_mode;
unsigned char pre_hdmi_vsem_is_dolby;
...
static long pre_dolby_hdmi_drm_mode;
static unsigned char dv_vrr_status;

#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DP)
static DOLBY_DP_TYPE pre_dolby_vision_dp_mode;
void reset_pre_dolby_vision_dp_mode(void)
{
	pre_dolby_vision_dp_mode = INIT_DOLBY_MODE;
}
#endif
static const int threashold_of_dolby_sdp_mode_change = 1;
...
#endif
    pre_hdmi_vsem_is_dolby = INIT_DOLBY_MODE;

    pre_dolby_hdmi_drm_mode = INIT_DOLBY_MODE;
    dv_vrr_status = INIT_DOLBY_MODE;
}
...
extern void I3DDMA_SetCap0LastWriteFlag(unsigned int flag);
extern unsigned int I3DDMA_Get_VO_SWBufIndex(void);
extern unsigned int get_i3ddma_video_delay_total_bufnum(void);
extern unsigned char I3DDMA_SwGetBufNum(void);
UINT32 g_ulI3SwModeIdx = 0;
extern I3DDMA_CAPTURE_BUFFER_T i3ddma_vdelay_cap_buffer[SW_I3DDMA_CAP_NUM];
...
void memc_force_unfreeze_check(void)
{
	...
	if(RPC_SysInfo)
	{
		if(RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status])
		{
			...
			{
				rtd_pr_vgip_isr_info("DM_freeze, un-freeze check\n");
				MEMC_Lib_Freeze(0);
				RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 0;
				rtd_pr_vgip_isr_err(" ###[ERR] func:%s timeout####\r\n",__FUNCTION__);
...
void forcebg_memc_force_unfreeze(void)
{
	...
	if(RPC_SysInfo)
	{
		if(RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status])
		{
			rtd_pr_vgip_isr_info("DM_freeze, unfreeze force,\n");
			MEMC_Lib_Freeze(0);
			RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 0;
			rtd_pr_vgip_isr_err(" ### func:%s enable forcebg unfreeze memc####\r\n", __FUNCTION__);
		}
	}
...
DECLARE_TASKLET_OLD(pq_automa_tasklet, pq_automa_task);
#else
DECLARE_TASKLET(pq_automa_tasklet, pq_automa_task, 0);
#endif
// for ml8 dma recovery flow
unsigned int blk_decont_dma_vs_cnt = 0, blk_decont_dma_sdnr_cnt = 0;
unsigned char blk_decont_dma_errFlow = 0;
// end for ml8 dma recovery flow
extern unsigned char DI_F2D_CTRL_DB_Apply_Wait;
...
//AT V6.2 Patch 2022/03/31 - END
#endif //#ifdef CONFIG_RTK_SRE_AATS_UCT_AT
...
#endif //#ifdef SDNR_LINECNT_DEBUG
...
#endif //#ifdef CONFIG_HDR_SDR_SEAMLESS

extern void *dolby_facade_get_hdmi_dolby_vsi_content(void);
	...
	unsigned char handle_ch1_vlc_flag = 0;
	...
	if (vgip_chn1_ctrl_reg.ch1_vact_end_ie && vgip_chn1_status_reg.ch1_vact_end && int_ctl_reg.vgip_int_ie){	// _BIT25
		...
		{
			// for ml8 dma recovery flow
			blk_decont_dma_vs_cnt++;
			if(drvif_color_Block_DeContour_run_recover_flow())
			{
				bool enable_log = drvif_color_Block_DeContour_debug_recover_flow();
				
				if(blk_decont_dma_errFlow==0 && drvif_color_Block_DeContour_check_dma_errFlag()==1)
				{
					if(enable_log) rtd_pr_vgip_isr_info("lsy %s(%d) start  errFlow %d cnt(%d, %d)\n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
					blk_decont_dma_errFlow = 1;
				}			
				else if(blk_decont_dma_errFlow==3)
				{
					if(enable_log) rtd_pr_vgip_isr_info("lsy %s(%d) start  errFlow %d cnt(%d, %d) \n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
					
					drvif_color_Block_DeContour_clear_wr_force_all_rst(0);
					drvif_color_Block_DeContour_clear_wr_data_fifo_underflow_err();
					drvif_color_Block_DeContour_clear_wdma_fifo_ovfl_clr(1);
					blk_decont_dma_errFlow = 4;
				}
				else if(blk_decont_dma_errFlow==4)
				{
					if(enable_log) rtd_pr_vgip_isr_info("lsy %s(%d) start  errFlow %d cnt(%d, %d) \n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
					
					drvif_color_Block_DeContour_clear_wdma_fifo_ovfl_clr(0);
					blk_decont_dma_errFlow = 0;
				}			
			}
			// end for ml8 dma recovery flow
		}
	...
	// vertical line compare(vlc) interrupt
	if (vgip_vgip_chn1_wde_reg.ch1_vlc_ie && vgip_chn1_status_reg.ch1_vlc_status ) {
		
		handle_ch1_vlc_flag = 1;

		if( SysInfo != NULL && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE ) {
			// clr DMA Err status
			...
			fwif_color_HDR_VIVID_frameSync_DMA_Apply_ISR();
		}
	...
	if(vgip_vgip_chn1_wde_reg.ch1_dispi_vact_end_ie && vgip_chn1_status_reg.ch1_sdnr_vact_end) {
		...
		{
			// for ml8 dma recovery flow
			blk_decont_dma_sdnr_cnt++;
			if(drvif_color_Block_DeContour_run_recover_flow())	
			{		
				bool enable_log = drvif_color_Block_DeContour_debug_recover_flow();

				if(blk_decont_dma_errFlow==1)
				{
					if(enable_log) rtd_pr_vgip_isr_info("lsy %s(%d) start  errFlow %d cnt(%d, %d)\n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
					
					drvif_color_Block_DeContour_set_snr_ssb_arb_en(0);
					drvif_color_Block_DeContour_set_dma_config(1);	
					drvif_color_Block_DeContour_clear_wr_force_all_rst(1);
					blk_decont_dma_errFlow = 2;
				}
				else if(blk_decont_dma_errFlow==2)
				{
					if(enable_log) rtd_pr_vgip_isr_info("lsy %s(%d) start  errFlow %d cnt(%d, %d)\n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
					
					drvif_color_Block_DeContour_set_snr_ssb_arb_en(1);
					drvif_color_Block_DeContour_set_dma_config(0);	
					blk_decont_dma_errFlow = 3;
				}			
			}
			// end for ml8 dma recovery flow
		}
...
//#ifdef CONFIG_REALTEK_2K_MODEL_ENABLED
	if((get_panel_res() == PANEL_RES_FHD)||(get_panel_res() == PANEL_RES_HD)){
		...
		if(handle_ch1_sta_flag && (get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_Get_multibuffer_flag()==1) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE))
		{
			...
			if(get_mach_type()==MACH_ARCH_RTK2885P) {
				...
				if (nonlibdma_drv_memory_Get_capture_multibuffer_flag() == 0) {
					//drv_memory_Set_multibuffer_number_pre(drv_memory_Get_multibuffer_number());
					nonlibdma_drv_memory_Set_multibuffer_number_pre(nonlibdma_drv_memory_Get_multibuffer_number());
					...
					if((nonlibdma_drv_memory_Get_multibuffer_number_pre()-1) != nonlibdma_get_cap_ddr_inctrl_reg_bufnum(0))
						nonlibdma_drv_memory_capture_multibuffer_switch_buffer_number();
				}
			}
		}
		...
		if(handle_ch1_end_flag && (get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_Get_multibuffer_flag()==1) &&(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_Get_capture_multibuffer_flag()==1))
		{

			//drv_memory_display_multibuffer_switch_buffer_number();
			nonlibdma_drv_memory_display_multibuffer_switch_buffer_number();
		}
	}
//#endif
	/*********sub channel*******/
...
#ifdef VGIP_ISR_CLEAR_NEW
	// Check if handled IRQ
	if (handle_ch1_sta_flag || handle_ch1_end_flag || handle_ch1_sdnr_act_end_flag || handle_ch1_vlc_flag)
	{
		vgip_chn1_status_reg.regValue = 0;

		if(handle_ch1_sta_flag)
			vgip_chn1_status_reg.ch1_vact_start = 1;

		if(handle_ch1_end_flag)
			vgip_chn1_status_reg.ch1_vact_end = 1;

		if(handle_ch1_sdnr_act_end_flag)
			vgip_chn1_status_reg.ch1_sdnr_vact_end = 1;

		if(handle_ch1_vlc_flag)
			vgip_chn1_status_reg.ch1_vlc_status = 1;
	...
	{
		dma_vgip_chn1_status_reg.regValue = 0;
	...
	{
		h3ddma_cap0_cap_status_reg.regValue = 0;
	...
	{
		h3ddma_cap3_cap_status_reg.regValue = 0;
	...
	{
		vgip_chn2_status_reg.regValue = 0;
...
#endif //#if 1
#endif //#ifdef VGIP_ISR_CLEAR_NEW
...
#ifdef CONFIG_SUPPORT_DOLBY_VSIF
	if ((get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && handle_dma_ch3_sta_flag)
			|| (!get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && handle_ch1_sta_flag)) {
		...
		if (drvif_Hdmi_get_infoframe_thread_stop()==_TRUE) {
			...
			if(VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
			{
				...
				check_dolby_vision_drm_packet();
#endif //#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DRM)
#ifdef CONFIG_SUPPORT_DOLBY_VSIF
				...
				if (pre_dolby_hdmi_vsif_mode != get_HDMI_Dolby_VSIF_mode())
				{
					if (pre_dolby_hdmi_vsif_mode == INIT_DOLBY_MODE) {
						...
						if (dolby_vsif_mode_change_count > threashold_of_dolby_vsif_mode_change) {
							if((get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_LL && pre_dolby_hdmi_vsif_mode == DOLBY_HDMI_VSIF_STD)
								|| (pre_dolby_hdmi_vsif_mode == DOLBY_HDMI_VSIF_LL && get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_STD))
...
#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DRM)
				if(pre_dolby_hdmi_drm_mode != get_hdmi_dolby_drm_mode()) {
					if (pre_dolby_hdmi_drm_mode == INIT_DOLBY_MODE) {
						pre_dolby_hdmi_drm_mode = INIT2_DOLBY_MODE;
					} else if (pre_dolby_hdmi_drm_mode == INIT2_DOLBY_MODE) {
						pre_dolby_hdmi_drm_mode = get_hdmi_dolby_drm_mode();
					} else {
						flag_dolby_vsif_change = 1;
						pre_dolby_hdmi_drm_mode = get_hdmi_dolby_drm_mode();
						dolby_mode_change_triger_mute = TRUE;
				...
				if(pre_hdmi_vsem_is_dolby != newbase_hdmi_get_vsem_is_dolby(port))
				{
					if (pre_dolby_vsem_type == INIT_DOLBY_MODE) {
						pre_hdmi_vsem_is_dolby = INIT2_DOLBY_MODE;
					} else if (pre_dolby_vsem_type == INIT2_DOLBY_MODE) {
						pre_hdmi_vsem_is_dolby = newbase_hdmi_get_vsem_is_dolby(port);
					} else {
						if(pre_hdmi_vsem_is_dolby == 0 && newbase_hdmi_get_vsem_is_dolby(port) == 1) {
							rtd_pr_vgip_isr_info("[%s:%d][Dolby] VSEM is dolby %d -> %d\n", __func__, __LINE__,pre_hdmi_vsem_is_dolby,newbase_hdmi_get_vsem_is_dolby(port));
							dolby_mode_change_triger_mute = TRUE;
						}
						pre_hdmi_vsem_is_dolby = newbase_hdmi_get_vsem_is_dolby(port);
					}
		...
		if (get_vsc_src_is_hdmi_or_dp() && dolby_mode_change_triger_mute) {
			...
			if(VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
			{
				...
				if (pre_dolby_hdmi_vsif_mode == get_HDMI_Dolby_VSIF_mode())
				{
					...
					if ((get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_LL) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI)) {
						vfe_hdmi_vsi_t *vsi_data = dolby_facade_get_hdmi_dolby_vsi_content();
						if ((vsi_data != NULL) && (0 != drvif_Hdmi_GetDolbyVsInfoFrame(vsi_data))) {
							rtd_pr_vgip_isr_debug("vfe_hdmi_drv_get_port_vsi_info error :%x\n", vfe_hdmi_drv_get_port_vsi_info(vsi_data));
							rtd_pr_vgip_isr_debug("VSI=%x, %x,%x,%x,%x,%x,%x\n",
							vsi_data->header.type ,
							vsi_data->header.version,
							vsi_data->header.length,
							vsi_data->checksum,
							vsi_data->ieee_reg_id[0],
							vsi_data->ieee_reg_id[1],
							vsi_data->ieee_reg_id[2]);
							rtd_pr_vgip_isr_debug("payload[0]=%x\n", vsi_data->payload[0]);

							dolby_adapter_dolby_trigger_timer6();
						}
					}
#endif //#ifdef CONFIG_RTK_KDRV_DV
				}

#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DRM)
				if(pre_dolby_hdmi_drm_mode == get_hdmi_dolby_drm_mode()) {
					if ((get_hdmi_dolby_drm_mode() == DOLBY_HDMI_DRM) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI)) {
						void *drm_data = dolby_facade_get_hdmi_dolby_vsi_content();
						if ((drm_data != NULL) && (0 != drvif_Hdmi_GetDrmInfoFrame(drm_data))) {
							dolby_adapter_dolby_trigger_timer6();
						}
					}
				}
...
#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DP)
		if(get_dolby_dp_load_at_isr() == TRUE) {
			if(VSC_INPUTSRC_DP == Get_DisplayMode_Src(SLR_MAIN_DISPLAY)) {
				check_dp_dolby_vision();
				if (pre_dolby_vision_dp_mode != dolby_adapter_get_dolby_vision_dp_mode()) {
					if (pre_dolby_vision_dp_mode == INIT_DOLBY_MODE) {
						pre_dolby_vision_dp_mode = INIT2_DOLBY_MODE;
					} else if (pre_dolby_vision_dp_mode == INIT2_DOLBY_MODE) {
						pre_dolby_vision_dp_mode = dolby_adapter_get_dolby_vision_dp_mode();
					} else {
						rtd_pr_vgip_isr_info("[%s:%d][DV] SDP %ld -> %ld\n", __func__, __LINE__,
								pre_dolby_vision_dp_mode, 
								dolby_adapter_get_dolby_vision_dp_mode());
						pre_dolby_vision_dp_mode = dolby_adapter_get_dolby_vision_dp_mode();
						dolby_mode_change_triger_mute = TRUE;
						if(dolby_adapter_get_dolby_vision_dp_mode() == DOLBY_DP_SDP_DISABLE)
							set_dolby_dp_load_at_isr_nonsemaphore(FALSE);
					}
				} else {
					DOLBY_DP_TYPE type = dolby_adapter_get_dolby_vision_dp_mode();
					if ((type != DOLBY_DP_NONE) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI)) {
						extern void *dolby_facade_get_hdmi_dolby_vsi_content(void);

						DP_SDP_DATA_T *sdp_data = (DP_SDP_DATA_T *)dolby_facade_get_hdmi_dolby_vsi_content();
						if ((sdp_data != NULL) && (false != get_dolby_vision_sdp_data(type, sdp_data))) {
							dolby_adapter_dolby_trigger_timer6();
						}
					}
				}
...
#endif //#if IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DP)
	}
#endif //#ifdef CONFIG_SUPPORT_DOLBY_VSIF
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				...
				if(seamless_info_sharememory)
				{
					...
					)
					{
						...
						{//memc freeze first
							...
							{
								rtd_pr_vgip_isr_info("DM_freeze, freeze hdmi seamless,\n");
								MEMC_Lib_Freeze(1);
							}
							else
							{
								rtd_pr_vgip_isr_info("DM_freeze, no need freeze hdmi seamless not active\n");
							}
	...
	{//seamless source (hdmi or vdec) record capture block and clear m capture done status for freeze
		...
		if(VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY) && hdmi_position_seamless && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && (get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_get_mdomain_driver_status()))
		{//currently only hdmi need to clear m domain status for game mode seamless freeze
			...
			nonlibdma_clear_m_cap_done_status(SLR_MAIN_DISPLAY);//clear status for freeze check cap status
		}
		//merlin8/8p mdomain one-bin
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			//record_current_m_cap_block(SLR_MAIN_DISPLAY);//record m domain cpature block status for freeze use
			nonlibdma_record_current_m_cap_block(SLR_MAIN_DISPLAY);//record m domain cpature block status for freeze use
		}
	}
               		...
               		if (RPC_SysInfo) {
						...
						{//need freeze
							if(decide_seamless_freeze_condition())
							{
								if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
								{
									rtd_pr_vgip_isr_info("DM_freeze, freeze VDEC seamless,\n");
									MEMC_Lib_Freeze(1);
								}
								else
								{
									rtd_pr_vgip_isr_info("DM_freeze, no need freeze VDEC seamless not active\n");
								}
							}
						}
						else
						...
						memc_freeze_record_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
						RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 1; /* enable*/
                        if(Get_fake_scaler_connect_impl()) {
                            set_display_forcebg_mask_by_isr(SLR_MAIN_DISPLAY, SLR_FORCE_BG_TYPE_DTV_CHANNEL_CHANGE_HDR_SEAMLESS, 1);//set SLR_FORCE_BG_TYPE_DTV_CHANNEL_CHANGE_HDR_SEAMLESS mask
						    RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = Seamless_Freeze_Frames_Max;
                            rtd_pr_vgip_isr_info("fake flow seamless freeze cnt %d\n", RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt]);
                        } else {
						    RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = Seamless_Freeze_Frames_Max;
                        }
                	}
	...
	if (RPC_SysInfo && handle_ch1_end_flag) {
		if (RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] == 1) {
			if (RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] != 0) {	/* cnt sattus*/
				unsigned char* hdr_autorun_tbl_flag = Scaler_color_Access_HDR_AutoRun_TBL_Flag(0, 0); 
            ...
            if (Get_fake_scaler_connect_impl() && (hdr_autorun_tbl_flag == NULL)) {
                    rtd_pr_vgip_isr_info("[eli]cnt= %d, t= %d\n", RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt], scaler_rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
                    RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = 0;
            }
            rtd_pr_vgip_isr_info("[eli]cnt= %d,  fake_scaler = %d hdr_autorun_tbl_flag = %p t= %08x\n",
                        RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt],
                        Get_fake_scaler_connect_impl(),
                        hdr_autorun_tbl_flag,
                        scaler_rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
			} else {	/* un-freeze*/
				//rtd_pr_vgip_isr_debug("[eli]un-freeze, t= %d\n", scaler_rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
				rtd_pr_vgip_isr_info("DM_freeze, un-freeze,\n");
				MEMC_Lib_Freeze(0);
				RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 0;
				RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = 0;
    ...
    if(handle_h3ddma_Cap_last_wr_flag && (get_video_delay_type() == I3DDMA_VIDEO_DELAY))
    {
        ...
        if(judge_source_support_video_delay(Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
             && (true == get_force_i3ddma_enable(SLR_MAIN_DISPLAY)))
        {
            if(vodma_vodma_i2rnd_fifo_th_reg.flag_fifo_style_i3ddma)
            { // input fast
                ulNewI3Idx = (I3DDMA_Get_VO_SWBufIndex() + get_i3ddma_video_delay_total_bufnum() - I3DDMA_SwGetBufNum() + 1);
                rtd_outl(H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_0_reg, i3ddma_vdelay_cap_buffer[ulNewI3Idx % get_i3ddma_video_delay_total_bufnum()].phyaddr);
            }
            else { //input slow
                ulNewI3Idx = (g_ulI3SwModeIdx + 1) % get_i3ddma_video_delay_total_bufnum();
                rtd_outl(H3DDMA_CAP0_WR_DMA_num_bl_wrap_addr_0_reg, i3ddma_vdelay_cap_buffer[ulNewI3Idx].phyaddr);

                I3DDMA_SetCap0LastWriteFlag(g_ulI3SwModeIdx);

                g_ulI3SwModeIdx = ulNewI3Idx;
            }
        }
    }
	...
	if(handle_ch2_end_flag && scaler_scart_out_isr_get_enable())
	{
		...
		if(get_mach_type()==MACH_ARCH_RTK2885P) {
			...
			{
				if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 0)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg();
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg();
					}
				}
				else if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 1)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2addr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg();
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2addr_reg();
					}

				}
				else if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 2)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2addr_reg()+TVE_VFLIP_SIZE_OFFSET;
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_even_addr = nonlibdma_get_mdomain_cap_ddr_in2addr_reg();
						buf_odd_addr = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg();
					}
			...
			else
			{
				if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 0)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2_3rdaddr_reg();
					}
				}
				else if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 1)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2addr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2addr_reg();
					}
				...
				else if(nonlibdma_get_mdomain_cap_ddr_in2status_reg_in2_block_select() == 2)
				{
					if(scaler_get_tve_vflip_enable())
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg()+TVE_VFLIP_SIZE_OFFSET;
					}
					else
					{
						buf_prog_mode = nonlibdma_get_mdomain_cap_ddr_in2_2ndaddr_reg();
					}
	...
	if(handle_ch1_sta_flag) {
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI && get_HDMI_HDR_mode() == HDR_DOLBY_HDMI) {
			...
			if(dv_mute) {
				rtd_pr_vgip_isr_info("[%s:%d][Dolby] enbale forcebg by dolby internal status change\n", __func__ ,__LINE__);
	...
	if(rtd_inl(VGIP_VGIP_VBISLI_reg)&_BIT24){
		if(handle_ch1_sta_flag==1) {
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_drv_memory_debug_print_buf_idx();
			}
		}
	}
...
if((reg_value & BIT(30))){
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) ||(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA)|| ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1))) {
		if(handle_ch1_sta_flag)
		{
