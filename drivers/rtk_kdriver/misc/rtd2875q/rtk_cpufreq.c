...
int rtk_cpufreq_SCPU_can_overfreq(void)
{
	static int otp_value_mp_status_scpu_over_freq = -1;
	if(otp_value_mp_status_scpu_over_freq == -1)
	{
		...
		rtd_pr_cpu_freq_err("%s: (%d)\n",__func__, otp_value_mp_status_scpu_over_freq);
	}

  	//default 1.3G, only when OTP burn with 0x1 down to 1.2G
	if(otp_value_mp_status_scpu_over_freq == 1)
		return 0;
	else
		return 1;
...
static int rtk_cpufreq_init(struct cpufreq_policy *policy)
{
...

	{
		...
		for (i = 0; i < max_opps; i++)
		{
			...
			//overwirte with new one if not support overclock
#ifdef CONFIG_RTK_SUPPORT_CPUFREQ_OTP
			if(rtk_cpufreq_SCPU_can_overfreq() == 0)
			{
				if(i == (max_opps-2))
					freq_table[i].frequency = rtk_cpufreq_get_pll(2);
				if(i == (max_opps-1))
					freq_table[i].frequency = rtk_cpufreq_get_pll(3);
			}
#endif
...
#ifdef CONFIG_RTK_SUPPORT_CPUFREQ_OTP
		if(rtk_cpufreq_SCPU_can_overfreq() == 0)
			info->Rpll = info->Ppll = rtk_cpufreq_get_pll(2);
		else
#endif
			info->Rpll = info->Ppll = rtk_cpufreq_get_pll(0);
		...
		rtd_pr_cpu_freq_info("(%d)set PLL(%d/%d/%d/%d/%d/%d/%d/%d/%d)\n",
			policy->cpu, info->Rpll, info->Ppll, info->Apll, info->Xpll,
			rtk_cpufreq_get_pll(0), rtk_cpufreq_get_pll(1), rtk_cpufreq_get_pll(2), rtk_cpufreq_get_pll(3), max_opps);
	}
