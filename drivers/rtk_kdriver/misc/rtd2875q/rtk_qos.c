...
#include "rbus/tvsb2_reg.h" //for latency_threshold
...
///////////////////////////////////////
//dc_mc QoS
// CH0 - SCPU express lane Read
// CH1 - DC_SYS
// CH2 - DC_SYS2 Read
// CH3 - DC_SYS2 Write
// CH4 - DC_SYS3 Read
// CH5 - DC_SYS3 Write
// CH6 - SCPU express lane Write
// CH7 - SCPU express lane2 Read
// CH8 - SCPU express lane2 Write
...
//dc_mc
// MC_Qos_ctl_0  C2A00
// MC_Qos_ctl_1  C2A10 ~ MC_Qos_ctl_9 C2A30
// MC_Qos_ctr_11 C2A34, MC_Qos_ctl_12 C2A3C
#define QOS_CTL_NUM             12              // 0, 1~9, 11~12

// MC_Qos_ch0_ctl_0, C2A60 ~ MC_Qos_ch0_ctl_5, C2A74
// MC_Qos_ch1_ctl_0, C2A80 ~ ...
// ...
// MC_Qos_ch8_ctl_0, C2B60 ~ MC_Qos_ch8_ctl_5, C2B74
#define QOS_CH_NUM              9
#define QOS_CHCTL_NUM           6               // ctrl per channel
#define QOS_CH_REG_OFFSET       (0x20)
#define QOC_CHCTL_REG_OFFSET    (0x4)

//dc_mc2 : simliar as dc_mc
// MC_Qos_ctl_0  C3A00
// MC_Qos_ctl_1  C3A10 ~ MC_Qos_ctl_9  C3A30
// MC_Qos_ctr_11 C3A34,  MC_Qos_ctl_12 C3A3C
// MC_Qos_ch0_ctl_0 C3A60 ~ MC_Qos_ch0_ctl_5, C3A74
// ...
// MC_Qos_ch8_ctl_0 C3B60 ~ MC_Qos_ch8_ctl_5, C3B74



unsigned char g_rtk_qos_force_setting = 0;
unsigned char g_rtk_qos_current_setting = 0;
unsigned int g_rtk_qos_parameter[(QOS_CTL_NUM + (QOS_CH_NUM * QOS_CHCTL_NUM)) * MC_NUM];
...
static int rtk_qos_save_mc_normal_setting(void)
{
	...
	for (mc_idx = 0; mc_idx < MC_NUM; mc_idx++) {
		...
		// QOS_CH_NUM * QOS_CHCTL_NUM
		for (ch_idx = 0; ch_idx < QOS_CH_NUM; ch_idx++) {
			for (ctl_idx = 0; ctl_idx < QOS_CHCTL_NUM; ctl_idx++) {

				g_rtk_qos_parameter[idx++] = rtd_inl(DC_MC_MC_Qos_ch0_ctl_0_reg + ch_idx*QOS_CH_REG_OFFSET + ctl_idx*QOC_CHCTL_REG_OFFSET);

			}
		}
	}

	rtd_pr_qos_debug("[QOS] save %d parameters\n", idx);

	return 0;
}


static int rtk_qos_restore_mc_normal_setting(void)
{
	int idx = 0;
	int mc_idx, ch_idx, ctl_idx;
	...
	for (mc_idx = 0; mc_idx < MC_NUM; mc_idx++) {
		...
		for (ch_idx = 0; ch_idx < QOS_CH_NUM; ch_idx++) {
			for (ctl_idx = 0; ctl_idx < QOS_CHCTL_NUM; ctl_idx++) {
				...
				rtd_outl(DC_MC_MC_Qos_ch0_ctl_0_reg + ch_idx*QOS_CH_REG_OFFSET + ctl_idx*QOC_CHCTL_REG_OFFSET, g_rtk_qos_parameter[idx++]);

			}
		}
	}

	rtd_pr_qos_debug("[QOS] load %d parameters\n", idx);
...
static int __qos_set_mode(RTK_QOS_MODE_T mode)
{
	...
	case RTK_QOS_NORMAL_MODE:
		rtk_qos_restore_mc_normal_setting();
		break;
...
extern int rtk_aicam_is_running(void);
		if(rtk_aicam_is_running())
...
#define TVSB2_ID_DI_R       1
#define TVSB2_ID_MAIN_DISP  3
#define TVSB2_ID_VO1_Y      6

#define TVSB4_ID_OD_R       1
//#define TVSB4_ID_TRAFFIC    3 //there is no TRAFFIC nor SUBTITLE arb_en ID in 2875q
#define TVSB4_ID_OSD_1      4
#define TVSB4_ID_OSD_2      5

#define MEMC_ID_PROG0       0 // MEMC_ID_KME_W
#define MEMC_ID_PROG1       1 // MEMC_ID_KME_R
#define MEMC_ID_PROG2       4 // MEMC_KMC_R0

__maybe_unused static irqreturn_t memc_pc_intr(int irq, void *dev_id)
{
	unsigned long memc_pc_lat_int;
	unsigned long memc_pc_lat_int_mask =
		DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl2_pc_prog0_lat_interrupt_mask |
		DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl2_pc_prog1_lat_interrupt_mask |
		DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl2_pc_prog2_lat_interrupt_mask;

	memc_pc_lat_int = rtd_inl(DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl2_reg);
	if (memc_pc_lat_int & memc_pc_lat_int_mask) {

		rtd_pr_qos_info("[PC_INT] memc %lx\n", (memc_pc_lat_int & memc_pc_lat_int_mask));

		// handling interrupt or event to defer serivce
		rtd_setbits(DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl2_reg, memc_pc_lat_int_mask);

		// disable & stop performance counter
		rtd_outl(DBUS_WRAPPER_MEMC_DCU1_PC_CTRL_reg, 0x0);
#if 0
		// enable & start performance counter
		rtd_outl(DBUS_WRAPPER_MEMC_DCU1_PC_CTRL_reg, DBUS_WRAPPER_MEMC_DCU1_PC_CTRL_pc_lat_interrupt_en(0x1) |
			DBUS_WRAPPER_MEMC_DCU1_PC_CTRL_pc_rd_only(0x1) | DBUS_WRAPPER_MEMC_DCU1_PC_CTRL_pc_go(0x1));
...
__maybe_unused static void memc_pc_intr_setup(void)
{
	unsigned int idx;

#if 1
	// memc_isr_init_irq() to init
	// memc_isr() to call memc_pc_intr
#else
	struct device_node *np = NULL;
	int irq_num;

	np = of_find_node_by_name(NULL, "memc_isr");
	if (!np) {
		rtd_pr_qos_info("[PC_INT] there is no memc device node\n");
		return;
	}
	irq_num = irq_of_parse_and_map(np, 0);
	rtd_pr_qos_info("[PC_INT] memc irq_num = %d\n", irq_num);
	...
	if (request_irq(
		...
		"MEMC",
		memc_pc_intr)) {
		rtd_pr_qos_err("[PC_INT] MEMC: cannot register IRQ \n");
		return ;
	}

	/* memc INT rounting to SCPU */
	...
	rtd_outl(DBUS_WRAPPER_MEMC_DCU1_PC_prog_ctrl1_reg, (MEMC_ID_PROG0) << 0 |
		(MEMC_ID_PROG1) << 8 | (MEMC_ID_PROG2) << 16);

	// set lat threshold for each prog
	for (idx = 0; idx < PC_PROG_NUM; idx++)
		rtd_outl(DBUS_WRAPPER_MEMC_DCU1_PC_prog_0_latency_threshold_reg - 0x4*idx, PC_LATENCY_THRESHOLD);
...
__maybe_unused static void tvsb4_pc_intr_setup(void)
{
	...
	rtd_outl(TVSB4_TV_SB4_DCU1_PC_prog_ctrl1_reg, (TVSB4_ID_OD_R) << 0 |
		(TVSB4_ID_OSD_1) << 8 | (TVSB4_ID_OSD_2) << 16);
	// set lat threshold for each prog
	for (idx = 0; idx < PC_PROG_NUM; idx++)
