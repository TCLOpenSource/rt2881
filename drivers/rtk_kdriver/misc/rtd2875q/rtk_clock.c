...
*/
#define VOLTAGE_LEVEL_DEDAULT	SCPU_H_H
...
//retunr 1 if using VID new flow (3 level VID control)
//return 0 means original flow (4 level depends on DTS opp-level value)
int rtk_is_VID_new_flow(void)
{
#if 0
	static int scpu_vid_new_flow = -1;
	if(scpu_vid_new_flow == -1)
	{
		int scpu_ft_iddq_done;
		scpu_ft_iddq_done = rtk_otp_field_read_int_by_name("hw_mp_status_ft_iddq_done");
		if(scpu_ft_iddq_done == -1)
			panic("%s: cant't get from OTP\n");

		if(scpu_ft_iddq_done == 1)
			scpu_vid_new_flow = 1;
		else
			scpu_vid_new_flow = 0;
		rtd_pr_cpu_freq_info("%s: scpu_ft_iddq_done(%d), scpu_vid_new_flow(%d)\n", __func__, scpu_ft_iddq_done, scpu_vid_new_flow);
	}
	return scpu_vid_new_flow;
#else
	//original flow (4 level depends on DTS opp-level value)
	return 0;
#endif
}


#define SCPU_IDDQ_THRESHOLD_H (28)
#define SCPU_IDDQ_THRESHOLD_M (14)
//get the VID level from by OTP SCPU IDDQ value couting
//reference this when using the new VID new flow (3 level VID control)
int rtk_get_VID_levle(void)
{
	static int scpu_vid_level = SCPU_NONE;
	if(scpu_vid_level == SCPU_NONE)
	{
		int scpu_ft_iddq_value;
		scpu_ft_iddq_value = rtk_otp_field_read_int_by_name("hw_iddq_scpu_power");
                if(scpu_ft_iddq_value == -1)
                        panic("%s: cant't get from OTP\n");

		if(scpu_ft_iddq_value > SCPU_IDDQ_THRESHOLD_H)
			scpu_vid_level = SCPU_L_L;
		else if(scpu_ft_iddq_value > SCPU_IDDQ_THRESHOLD_M)
			scpu_vid_level = SCPU_L_H;
		else
			scpu_vid_level = SCPU_H_L;
		rtd_pr_cpu_freq_info("%s: scpu_ft_iddq_value(%d), scpu_vid_level(%d)\n", __func__, scpu_ft_iddq_value, scpu_vid_level);
	}
	return scpu_vid_level;
}
...
int rtk_get_voltage_level(struct rtk_cpufreq_info *info, unsigned int pll, unsigned int rate)
{
	...
	if(rtk_cpufreq_SCPU_can_overfreq() == 0)
	{
		//normal
		if(rate == rtk_cpufreq_get_pll(2))
			opp = dev_pm_opp_find_freq_exact(cpu_dev, rtk_cpufreq_get_pll(0) * 1000, 1);
		else if(rate == rtk_cpufreq_get_pll(3)) //boost
			opp = dev_pm_opp_find_freq_exact(cpu_dev, rtk_cpufreq_get_pll(1) * 1000, 1);
	}
	else
#endif
	...
	//rtd_pr_cpu_freq_info("%s: pll/rate/level %d/%d/%d\n", pll, rate, level);

	if(rtk_is_VID_new_flow() && (level!= SCPU_H_H))
		return rtk_get_VID_levle();
	else //original
		return level;
}
...
{
	//GPO_20/GPO_21: PIN_SCPU_VID_0/PIN_SCPU_VID_1
...
void rtk_set_voltage(struct rtk_cpufreq_info *info, unsigned int pll, unsigned int rate)
{
	...
	/* [ML9BU-226]
		2885p use two pin GPIO20/GPIO21 as VID_CPU/VID_CPU
		(bootcode define with PIN_SCPU_VID_0/PIN_SCPU_VID_1)
		The DTS use four level {1/2/3/4} as original VID_CPU/VID_CPU two pin mapping.
	*/
	voltage_level = rtk_get_voltage_level(info, pll, rate);
	if(voltage_level == old_voltage_level)
		return;

	rtd_pr_cpu_freq_err("rtk_set_voltage org(%d): GPO(%x/%x)\n", old_voltage_level, rtd_inl(MIS_GPIO_GP0DATO_reg), rtd_inl(MIS_GPIO_GP0DIR_reg));

	rtk_set_voltage_pin(voltage_level);
	rtd_pr_cpu_freq_err("rtk_set_voltage: pll(%d), rate(%d), vol(%d/%d), GPO(%x/%x)\n",
		pll, rate, voltage_level, old_voltage_level, rtd_inl(MIS_GPIO_GP0DATO_reg), rtd_inl(MIS_GPIO_GP0DIR_reg));
...
void rtk_set_default_voltage(void)
{
	...
	if(rtk_is_VID_new_flow())
		voltage_level = rtk_get_VID_levle();
	...
	old_voltage_level = voltage_level;
	udelay(50);
	rtd_pr_cpu_freq_err("rtk_set_default_voltage:(%d), GPO(%x/%x)\n", voltage_level, rtd_inl(MIS_GPIO_GP0DATO_reg), rtd_inl(MIS_GPIO_GP0DIR_reg));
}
...
unsigned int rtk_get_boot_freq(int cluster_id)
{
	...
	//Merlin9(T22): 27*(Ncode+(3) + Fcode/2048)/(pllscpu_prediv + 1);
	...
	rate = (FREQ_XTAL_MHZ * 2048 * (n_code + (3)) + FREQ_XTAL_MHZ * f_code) >> (11 + pllscpu_prediv);
#endif
...
#ifdef CONFIG_RTK_SUPPORT_CPUFREQ_OTP
extern int rtk_cpufreq_SCPU_can_overfreq(void);
#endif
...
#ifdef CONFIG_ARCH_RTK2875Q
	//Merlin9 SCPU ratio to PLL
#ifdef CONFIG_RTK_SUPPORT_CPUFREQ_OTP
//	if(rtk_cpufreq_SCPU_can_overfreq() == 0)
//		rate = (rate * RTK_CPUFREQ_CPU_RATE_TO_PLL_MUL_OTP) >> (RTK_CPUFREQ_CPU_RATE_TO_PLL_SHIFT_OTP);
//	else
		rate = (rate * RTK_CPUFREQ_CPU_RATE_TO_PLL_MUL) >> (RTK_CPUFREQ_CPU_RATE_TO_PLL_SHIFT);
#else
	rate = (rate * RTK_CPUFREQ_CPU_RATE_TO_PLL_MUL) >> (RTK_CPUFREQ_CPU_RATE_TO_PLL_SHIFT);
#endif //CONFIG_RTK_SUPPORT_CPUFREQ_OTP

#else
#error	"CPUFREQ: Should define platform specific MUL and SHIFT !!"
#endif
	...
	n_code= ((rate <<pllscpu_prediv)/FREQ_XTAL) -(3);
	f_code= (unsigned int )(((rate <<pllscpu_prediv) - (n_code +(3))*FREQ_XTAL)*2048/FREQ_XTAL)+1;
