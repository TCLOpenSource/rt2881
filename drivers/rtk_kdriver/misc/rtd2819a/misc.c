...
#define RTD_SHARE_MEM_BASE      0xb8060500
                ...
                case MISC_SET_DEBUG_LOCK: {
                        ...
                        if(nLock) {
                                ...
                                rtd_part_outl(PINMUX_ST_Pin_Mux_Ctrl0_reg, 14, 12, 0x2);// switch JTAG tms to src 2
                ...
                case MISC_SET_MEMTESTER_RESULT: {
                        ...
                        MISC_INFO("Set dummy register 0x%x : 0x%x", REG_SC_DUMMY21, val);
                        if (val & 0x2)
                            panic("Memory Tester Fail.");
                }
                break;
...
#if 1 //FIXME: BRINGUP need add
                        switch(param)
                        {
                                case UART0_PINMUX_ENABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_0_reg, 0xFFFF0F0F, 0x00004040); //switch uart0 pinmux
                                        rtd_maskl(PINMUX_ST_Pin_Mux_Ctrl1_reg, 0xFFFFFF8F, 0x00000000); //switch uart rx src0 6:0x18060244[3:0] = 0x0
                                        break;
                                case UART1_PINMUX_ENABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_0_reg, 0xFFFF0F0F, 0x00005050); //switch uart1 pinmux
                                        rtd_maskl(PINMUX_Pin_Mux_Ctrl1_reg, 0xFFFFFFF8, 0x00000004); //switch uart rx src1 7:0x18060248[31:28] = 0x1
                                        break;
                                case UART0_UART1_PINMUX_DISABLE:
                                        rtd_maskl(PINMUX_ST_GPIO_ST_CFG_0_reg, 0xFFFF0F0F, 0x0000F0F0); //switch to GPIO pin
                                        break;
                                default:
...
void dump_tvsb1_error_status(unsigned int val, char * msg)
{
        ...
        MISC_ERR("[%s]reg val:0x%08x\n", msg, val);
        if(TVSB1_TV_SB1_DCU1_error_status_get_error_read_overflow(val))
        {
                MISC_ERR("%s - IP(%s) Error in read fifo\n", msg,Error_Client_ID[TVSB1_TV_SB1_DCU1_error_status_get_error_zero_length_id(val)]);
        }

        if(TVSB1_TV_SB1_DCU1_error_status_get_error_write_overflow(val))
        {
...
void dump_tvsb2_error_status(unsigned int val, char * msg)
{
        ...
        if(TVSB2_TV_SB2_DCU1_error_status_get_error_read_overflow(val)) {
                MISC_ERR("%s - Error_read_full\n", msg);
        }

        if(TVSB2_TV_SB2_DCU1_error_status_get_error_write_overflow(val)) {
...
void dump_tvsb3_error_status(unsigned int val, char * msg)
{
        ...
        if(TVSB3_TV_SB3_DCU1_error_status_get_error_read_overflow(val)) {
                MISC_ERR("%s - Error_read_full\n", msg);
        }

        if(TVSB3_TV_SB3_DCU1_error_status_get_error_write_overflow(val)) {
                MISC_ERR("%s - Error_write_full\n", msg);
        }

        if(TVSB3_TV_SB3_DCU1_error_status_get_error_wdone_overflow(val)) {
...
void dump_tvsb4_error_status(unsigned int val, char * msg)
{
        ...
        if(TVSB4_TV_SB4_DCU1_error_status_get_error_read_overflow(val)) {
                MISC_ERR("%s - Error_read_full\n", msg);
        }

        if(TVSB4_TV_SB4_DCU1_error_status_get_error_write_overflow(val)) {
                MISC_ERR("%s - Error_write_full\n", msg);
        }
...
void dump_tvsb5_error_status(unsigned int val, char * msg)
{
        ...
        if(TVSB5_TV_SB5_DCU1_error_status_get_error_wdone_overflow(val)) {
                MISC_ERR("%s - Error_wdone_fifo\n", msg);
        }

        if(TVSB5_TV_SB5_DCU1_error_status_get_error_read_overflow(val)) {
                MISC_ERR("%s - Error_read_full\n", msg);
        }

        if(TVSB5_TV_SB5_DCU1_error_status_get_error_write_overflow(val)) {
                MISC_ERR("%s - Error_write_full\n", msg);
        }
...
void dump_tvsb7_error_status(unsigned int val, char * msg)
{
        ...
        if(TVSB7_TV_SB7_DCU1_error_status_get_error_read_overflow(val)) {
                MISC_ERR("%s - Error_read_full\n", msg);
        }
        if(TVSB7_TV_SB7_DCU1_error_status_get_error_write_overflow(val)) {
                MISC_ERR("%s - Error_write_full\n", msg);
        }
        if(TVSB7_TV_SB7_DCU1_error_status_get_error_wdone_overflow(val)) {
                MISC_ERR("%s - Error_wdone_fifo\n", msg);
        }
}
