...
static UINT8 handle_dtg_vlc2_flag = 0;
static UINT8 handle_osd123dtg_frc2fsync_flag = 0;
unsigned int scaler_ddomain_isr(void)
{
	...
	extern int scaler_dtg_set_frc2fsync_interrupt(DTG_TYPE type, unsigned char enable);
	extern int drvif_scaler_enable_frc2fsync_HW_speedup_vtotal(DTG_TYPE dtg_type, unsigned char bEnable);
...
#endif
			scaler_dtg_set_frc2fsync_interrupt(DTG_MASTER, 0);
			drvif_scaler_enable_frc2fsync_HW_speedup_vtotal(DTG_MASTER, 0);
					ret = IRQ_HANDLED;
	}
    ...
    if((dtg_ie_2_reg.osddtg_frc2fsync_event_ie_2 == 1) && (handle_osd123dtg_frc2fsync_flag == 1))
    {
        rtd_pr_ddomain_isr_notice("osd123 fsync ok\n");
        drvif_scaler_enable_frc2fsync_HW_speedup_vtotal(DTG_OSD123, 0);
        scaler_dtg_set_frc2fsync_interrupt(DTG_OSD123, 0);
        ret = IRQ_HANDLED;
    }
    ...
    if( dtg_ie_2_reg.dtg_vlc2_ie_2 && handle_dtg_vlc2_flag )    // for mipi command mode
    {
        vbe_disp_en_control_mipi(_DISABLE); // To prevent Master DTG line count over 16383
        ret = IRQ_HANDLED;
    }
...
unsigned int pq_ddomain_isr(void)
{
	...
	if (dtg_ie_2_reg.mv_den_end_event_ie_2 && handle_mv_den_end_flag) {
		...
		if(g_Writ_Gamma_disr==1){
			//rtd_pr_ddomain_isr_emerg( "gamma ddomain isr update GAMMA to driver l !!!\n");
	...
	if(blu_ld_smartld_int_reg.hw_blight_end_ie2 && blu_ld_smartld_pending_status_reg.hw_blight_end_int  && ldspi_rx_data_ctrl_reg.ld_spi_int2_en){ // LD data end isr
	
		blu_ld_smartld_pending_status_reg.regValue = 0;
		blu_ld_smartld_pending_status_reg.hw_blight_end_int = 1;
...
void ddomain_isr_clear_events(void)
{
    ...
    if( dtg_ie_2_reg.dtg_vlc2_ie_2 && handle_dtg_vlc2_flag )
    {
        dtg_pending_status_2_reg.regValue = 0;
        dtg_pending_status_2_reg.dtg_vlc2_status_2 = 1;        // write clear status
        IoReg_Write32(PPOVERLAY_DTG_pending_status_2_reg, dtg_pending_status_2_reg.regValue);
    }
}
...
irqreturn_t ddomain_isr(int irq, void *dev_id)
{
    ...
    handle_dtg_vlc2_flag = dtg_pending_status_2_reg.dtg_vlc2_status_2;
    handle_osd123dtg_frc2fsync_flag = dtg_pending_status_2_reg.osddtg_frc2fsync_event_2;
...
//    ppoverlay_dtg_ie_2_RBUS ppoverlay_dtg_ie_2_reg;
    ...
    /* Request IRQ */
    if(request_irq(irq, ddomain_isr, IRQF_SHARED, "DDOMAIN ISR", (void *)pdev))
    {
