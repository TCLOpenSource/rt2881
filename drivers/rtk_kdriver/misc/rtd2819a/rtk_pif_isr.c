...
#if defined(CONFIG_ENABLE_MIPITX)
...
irqreturn_t mipitx_ISR(int irq, void *dev_id)
{
    return mipitx_lprx_ISR();
}
#endif
...
#if defined(CONFIG_ENABLE_MIPITX)
	/* Request MIPI IRQ */
	ret = request_irq(irq, mipitx_ISR, IRQF_SHARED, "MIPI ISR", (void *)pdev);
	if (ret) {
		rtd_pr_pif_isr_err("mipitx_ISR: cannot register IRQ %d. ret=%d\n", irq, ret);
		return ret;
	}
#endif
...
#if defined(CONFIG_ENABLE_MIPITX)
    mipi_dsi_mipi_ctrl_0_reg.regValue = IoReg_Read32(MIPI_DSI_MIPI_CTRL_0_reg);
	mipi_dsi_mipi_ctrl_0_reg.mipi_irq_en = 0;
	IoReg_Write32(MIPI_DSI_MIPI_CTRL_0_reg, mipi_dsi_mipi_ctrl_0_reg.regValue);
#endif
