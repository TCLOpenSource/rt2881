...
#endif
#if IS_ENABLED(CONFIG_ENABLE_HDMITX)
extern void hdmitx_sys_clock_reset(void);
#endif
...
static int vgip_isr_resume(struct platform_device *dev)
{
	...
	hdmitx_sys_clock_reset(); // need refine
...
irqreturn_t OnlineMeasureISR(int irq, void *dev_id)
{
	...
	unsigned char online3_flag = 0, online4_flag = 0;
	...
	if ( int_ctl_reg.onms1_int_ie == 0 && int_ctl_reg.onms2_int_ie == 0
		&& int_ctl_reg.onms3_int_ie == 0 && int_ctl_reg.onms4_int_ie ==0)
		return IRQ_NONE;
	...
	onms_onms4_status_reg.regValue = scaler_rtd_inl(ONMS_onms4_status_reg);
	onms_onms4_interrupt_en_reg.regValue = scaler_rtd_inl(ONMS_onms4_interrupt_en_reg);
	if((onms_onms4_status_reg.regValue & onms_onms4_interrupt_en_reg.regValue)>0)
		online4_flag = 1;
	else
		online4_flag = 0;

	if(online1_flag || online2_flag || online3_flag || online4_flag)
	{//process online measure interrupt
		...
		if (online4_flag) {
			...
			//scaler_rtd_outl(ONMS_onms1_status_reg, onms_onms1_status_reg.regValue);
			//step 2 disable onlinemeasure interrupt
			scaler_rtd_outl(ONMS_onms4_interrupt_en_reg, 0);
			int_ctl_reg.onms4_int_ie = 0;
			scaler_rtd_outl(VGIP_INT_CTL_reg, int_ctl_reg.regValue);
			//Syep3 pass RPC to Notify kernel scaler code to process signel not stable issue
			triggle_onms_handler(SLR_MAIN_DISPLAY);
		}

		if(online3_flag || online1_flag || online4_flag)
		{
...
#if IS_ENABLED(CONFIG_ENABLE_HDMITX)
irqreturn_t TXPreOnlineMeasureISR(int irq, void *dev_id)
{
	extern void txpre_trigger_onms_handler(void);
	unsigned char online5_flag = 0;
	...
	if(0 == SYS_REG_SYS_CLKEN6_get_clken_hdmitx(IoReg_Read32( SYS_REG_SYS_CLKEN6_reg )))
		return IRQ_NONE;

	hdmitx_vgip_hdmitx_vgip_int_ctl_reg.regValue = IoReg_Read32( HDMITX_VGIP_HDMITX_VGIP_INT_CTL_reg );
	if (0 == hdmitx_vgip_hdmitx_vgip_int_ctl_reg.onms1_int_ie)
		return IRQ_NONE;

	onms5_onmstx_status_reg.regValue = IoReg_Read32(ONMS5_onmstx_status_reg);
	onms5_onmstx_interrupt_en_reg.regValue = IoReg_Read32(ONMS5_onmstx_interrupt_en_reg);
	if(onms5_onmstx_status_reg.regValue & onms5_onmstx_interrupt_en_reg.regValue)
		online5_flag = 1;
	else
		online5_flag = 0;

	if(online5_flag) {//process online measure interrupt
		//step 1 clean onlinemeasure interrupt pending
		//IoReg_Write32(ONMS_onms1_status_reg, onms_onms1_status_reg.regValue);
		//step 2 disable onlinemeasure interrupt
		IoReg_Write32(ONMS5_onmstx_interrupt_en_reg, 0);
		hdmitx_vgip_hdmitx_vgip_int_ctl_reg.onms1_int_ie = 0;
		IoReg_Write32(ONMS5_onmstx_interrupt_en_reg, hdmitx_vgip_hdmitx_vgip_int_ctl_reg.regValue);
		//Syep3 pass RPC to Notify kernel scaler code to process signel not stable issue
		txpre_trigger_onms_handler();
	...
	{//letency parameter change
		...
		if(video_latency_pattern.bOnOff)
		{
				...
				{//PTG disable -> enable
					...
					*/
					IoReg_SetBits(PINMUX_ST_GPIO_ST_CFG_3_reg, _BIT15|_BIT14|_BIT13|_BIT12);
...
extern void dynamic_change_hdr_setting_hdmi_finish(unsigned char hdr_enable, I3DDMA_COLOR_SPACE_T color_format, unsigned char hdr_type);
	...
	unsigned char handle_h3ddma_Cap1_last_wr_flag = 0;
	...
	h3ddma_cap1_cap_status_reg.regValue = scaler_rtd_inl(H3DDMA_CAP1_Cap_Status_reg);
	...
	if(h3ddma_interrupt_enable_reg.cap1_last_wr_ie && h3ddma_cap1_cap_status_reg.cap1_cap_last_wr_flag) {
		handle_h3ddma_Cap1_last_wr_flag = 1;
	}
	...
	{	// _BIT24
		...
		if(drvif_Hdmi_get_infoframe_thread_stop()==_TRUE) {
				if(VSC_INPUTSRC_HDMI == Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
				{
					...
					if (pre_dolby_hdmi_vsif_mode != get_HDMI_Dolby_VSIF_mode_by_port(port))
					{
							...
							pre_dolby_hdmi_vsif_mode = get_HDMI_Dolby_VSIF_mode_by_port(port);
						} else {
							rtd_pr_vgip_isr_info("[%s:%d][Dolby] VSIF %d -> %d\n", __func__, __LINE__, pre_dolby_hdmi_vsif_mode,  get_HDMI_Dolby_VSIF_mode_by_port(port));
							...
							if (dolby_vsif_mode_change_count > threashold_of_dolby_vsif_mode_change) {
								if((get_HDMI_Dolby_VSIF_mode_by_port(port) == DOLBY_HDMI_VSIF_LL) || (pre_dolby_hdmi_vsif_mode == DOLBY_HDMI_VSIF_LL))
...
#ifdef CONFIG_RTK_KDRV_DV
						if ((get_HDMI_Dolby_VSIF_mode_by_port(port) == DOLBY_HDMI_VSIF_LL) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI)) {
		...
		if(get_dolby_dp_load_at_isr() == TRUE) {
			...
			if(VSC_INPUTSRC_DP == Get_DisplayMode_Src(SLR_MAIN_DISPLAY)) {
				...
				if (pre_dolby_vision_dp_mode != dolby_adapter_get_dolby_vision_dp_mode_by_port(port)) {
					if (pre_dolby_vision_dp_mode == INIT_DOLBY_MODE) {
						...
						pre_dolby_vision_dp_mode = dolby_adapter_get_dolby_vision_dp_mode_by_port(port);
					} else {
						rtd_pr_vgip_isr_info("[%s:%d][DV] SDP %ld -> %ld\n", __func__, __LINE__,
								pre_dolby_vision_dp_mode, 
								dolby_adapter_get_dolby_vision_dp_mode_by_port(port));
						pre_dolby_vision_dp_mode = dolby_adapter_get_dolby_vision_dp_mode_by_port(port);
						dolby_mode_change_triger_mute = TRUE;
						if(dolby_adapter_get_dolby_vision_dp_mode_by_port(port) == DOLBY_DP_SDP_DISABLE)
							set_dolby_dp_load_at_isr_nonsemaphore(FALSE);
					}
				} else {
					DOLBY_DP_TYPE type = dolby_adapter_get_dolby_vision_dp_mode_by_port(port);
					if ((type != DOLBY_DP_NONE) && (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI)) {
						...
						if ((sdp_data != NULL) && (false != get_dolby_vision_sdp_data_by_port(port, type, sdp_data))) {
							dolby_adapter_dolby_trigger_timer6();
						}
					}
				}
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		...
		if(handle_ch1_sta_flag)
		{
		    ...
		    if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI))
			{
				if(seamless_info_sharememory)
				{
					...
					{
						if(decide_seamless_freeze_condition() && RPC_SysInfo && (RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] == 0) && (get_hdmi_4k_hfr_mode_with_display(SLR_MAIN_DISPLAY) == HDMI_NON_4K120))
						{//memc freeze first
            ...
            {//run hdr change
                if(get_hdr_semaless_active())
                {
               		if (RPC_SysInfo) {
						...
						memc_freeze_record_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
						RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Status] = 1; /* enable*/
                        if(Get_fake_scaler_connect_impl() && (SEAMLESS_WITHOUT_FREEZE == Scaler_ChangeUINT32Endian(seamless_info_sharememory->vdec_change_flag))) {
						    RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = 0;
                            rtd_pr_vgip_isr_info("fake flow seamless freeze cnt 0\n");
                        } else {
						    RPC_SysInfo->HDR_info.Ctrl_Item[TV006_HDR_Seamless_Freeze_Cnt] = Seamless_Freeze_Frames_Max;
                        }
                	}
	...
	if(handle_ch1_sta_flag) {
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI && get_HDMI_HDR_mode() == HDR_DOLBY_HDMI) {
			...
			unsigned char port = Get_DisplayMode_Port(SLR_MAIN_DISPLAY);
			unsigned char cur_vrr_status = drvif_Hdmi_Multi_port_GetVRREnable(port) || drvif_Hdmi_Multi_port_GetAMDFreeSyncEnable(port);

			if(dv_vrr_status != cur_vrr_status) {
				if (dv_vrr_status == INIT_DOLBY_MODE) {
					dv_vrr_status = INIT2_DOLBY_MODE;
				} else if (dv_vrr_status == INIT2_DOLBY_MODE) {
					dv_vrr_status = cur_vrr_status;
				}  else {
...
#ifdef CONFIG_ENABLE_DOLBY_VISION_HDMI_AUTO_DETECT
		if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
				&& ((get_HDMI_Dolby_VSIF_mode_by_port(port) == DOLBY_HDMI_VSIF_STD)
					|| (get_HDMI_Dolby_VSIF_mode_by_port(port) == DOLBY_HDMI_h14B_VSIF))) {
...
if((reg_value & BIT(30))){
	...
	if(handle_h3ddma_Cap1_last_wr_flag)
	{//need check
		extern unsigned char IDDMA_Set_CapStartAddr(unsigned char dev);
		extern unsigned char IDDMA_Check_IsI3Occpuy(void);
		if(IDDMA_Check_IsI3Occpuy() == FALSE)
		{
			scaler_I3ddma_ISR();
		}
		else
			IDDMA_Set_CapStartAddr(I3DDMA_DEV);
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) ||(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA)|| ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdmi_4k_hfr_mode_with_display(SLR_MAIN_DISPLAY) == HDMI_4K120_2_1))) {
		if(handle_ch1_sta_flag)
		{
	...
	if(handle_h3ddma_Cap1_last_wr_flag)
		h3ddma_cap1_cap_status_reg.cap1_cap_last_wr_flag = 1;
...
/*********sub channel************/
	...
	if (handle_h3ddma_Cap1_last_wr_flag)
		scaler_rtd_outl(H3DDMA_CAP1_Cap_Status_reg, h3ddma_cap1_cap_status_reg.regValue);
	...
	if(handle_ch1_sta_flag
		 ...
		 || handle_h3ddma_Cap1_last_wr_flag
...
static int __init vgip_isr_init_irq(struct platform_device *pdev)
{
...
#if IS_ENABLED(CONFIG_ENABLE_HDMITX)
	ret = request_irq(irq, TXPreOnlineMeasureISR, IRQF_SHARED,
					"TXPreOnlineMeasure ISR", (void *)"TXPreONMS_ISR");
	if (ret)
	{
		rtd_pr_vgip_isr_err("txpre_online_isr_platform_driver_isr: cannot register IRQ .ret=%d\n", ret);
		free_irq(irq, (void *)pdev);
		return ret;
	}
#endif
	...
	// enable TXPRE interrupt
	rtd_outl(HDMITX_VGIP_HDMITX_VGIP_INT_CTL_reg, _BIT4|_BIT0);
