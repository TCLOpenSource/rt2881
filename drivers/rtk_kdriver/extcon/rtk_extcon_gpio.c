...
struct extcon_gpio_name{
    char gpio_name[32];
};

struct extcon_gpio_name g_extcon_gpio_name_list[] = {
    {.gpio_name = "PIN_HP_OUT_JD"},
    {.gpio_name = "PIN_SWITCH_WOV"},
};
...
extern void rtk_set_hp_value_count(unsigned int val);
...
static void rtk_extcon_gpio_sync_state(struct gpio_rtkextcon_data *p_extcon_data)
{
    ...
    if(p_extcon_data->type == HP_OUT_JD){
        ...
        if (rtk_gpio_get_pcb_info("PIN_HP_MUTE", &pInfo) == 0){
            RTK_EXTCON_INFO("extcon Hp_status = %d",Hp_status);
            IO_Direct_Set("PIN_HP_MUTE",Hp_status);
        }
...
static int rtk_extcon_gpio_parse_and_register_dev(char *name, struct platform_device *pdev)
{
    ...
    if (GET_PIN_TYPE(ullPcbMgrValue) == PCB_PIN_TYPE_GPIO) {
        ...
        gpio_group = ISO_GPIO;
    } else if(GET_PIN_TYPE(ullPcbMgrValue) == PCB_PIN_TYPE_MIO_GPIO) {
        gpio_group = MIO_GPIO;
    } else if(GET_PIN_TYPE(ullPcbMgrValue) == PCB_PIN_TYPE_ISO_GPIO_MIO){
        gpio_group = ISO_GPIO_MIO;
    } else{
        return -1;
    }
...
static int rtk_extcon_gpio_probe(struct platform_device *pdev)
{
    ...
    while (index < EXTCON_ARRAY_SIZE(g_extcon_gpio_name_list)) {
        rtk_extcon_gpio_parse_and_register_dev(g_extcon_gpio_name_list[index].gpio_name, pdev);
        index++;
    }
