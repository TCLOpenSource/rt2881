...
typedef struct _PANEL_FILE_HEADER_LD {
	// table offset of each table
	int	iSize;
	int	iVipTableLDOffset;
	int	iVipTableLDCOMPOffset;	
	int   iVipTableLDSPIOffset;
	int   iVipTableLDINDEXMAPOffset;
	int	iVipLDVerMiscOffset;
	int	iVersion;
	unsigned int iCRCValue;		// check value
} PANEL_FILE_HEADER_LD;

typedef struct _PANEL_TABLE_STRUCT_HEADER_LD {
    int              iSize;
    int              iTableArraySize;  // memory size
    int              iIndexTableSize;
    unsigned char    iTableType;        //PANEL_TABLE_TYPE
    ...
    unsigned char    iIndexTableLen;
    unsigned char    reserved;    // reserved to keep picmode tableLen
} PANEL_TABLE_STRUCT_HEADER_LD;

typedef struct _PANEL_TABLE_STRUCT_LD {
    PANEL_TABLE_STRUCT_HEADER_LD    header;
    void*    pTableArray;
} PANEL_TABLE_STRUCT_LD;

typedef struct _PANEL_TABLE_STRUCT_LDCOMP {
    PANEL_TABLE_STRUCT_HEADER_LD    header;
    void*    pTableArray;
} PANEL_TABLE_STRUCT_LDCOMP;

typedef struct _PANEL_TABLE_STRUCT_LDSPI {
    PANEL_TABLE_STRUCT_HEADER_LD    header;
    void*    pTableArray;
} PANEL_TABLE_STRUCT_LDSPI;

typedef struct _PANEL_TABLE_STRUCT_LDINDEXMAP {
    PANEL_TABLE_STRUCT_HEADER_LD    header;
    void*    pTableArray;
} PANEL_TABLE_STRUCT_LDINDEXMAP;

typedef struct _VIP_LD_VER_MISC_INFO { //512 bytes
	char vipver[64];
	char reserved[448];
} VIP_LD_VER_MISC_INFO;

typedef struct _PANEL_FILE_HEADER_BLPF {	
// table offset of each table	
int	iSize;	
int	iVipTableBLPFOffset;	
int	iVipBLPFVerMiscOffset;	
int	iVersion;	
unsigned int iCRCValue;		// check value
} PANEL_FILE_HEADER_BLPF; 

typedef struct _PANEL_TABLE_STRUCT_HEADER_BLPF {
    int              iSize;
    int              iTableArraySize;  // memory size
    int              iIndexTableSize;
    unsigned char    iTableType;        //PANEL_TABLE_TYPE
    ...
    unsigned char    iIndexTableLen;
    unsigned char    reserved;    // reserved to keep picmode tableLen
} PANEL_TABLE_STRUCT_HEADER_BLPF;

typedef struct _PANEL_TABLE_STRUCT_BLPF {
    PANEL_TABLE_STRUCT_HEADER_BLPF    header;
    void*    pTableArray;
} PANEL_TABLE_STRUCT_BLPF;

typedef struct _VIP_BLPF_VER_MISC_INFO { //512 bytes
	char vipver[64];
	char reserved[448];
} VIP_BLPF_VER_MISC_INFO;
