...
#define VIP_DMAto3DTABLE_INV_OUT_GAMMA_SIZE (8 * 1024)
#define VIP_DMAto3DTABLE_OUT_GAMMA_SIZE (8 * 1024)
#define VIP_DMAto3DTABLE_GAMMA_SIZE (8 * 1024)
...
#define VIP_DMAto3DTABLE_Total (VIP_DMAto3DTable_HDR_3D_LUT_SIZE+VIP_DMAto3DTable_HDR_EOTF_SIZE+VIP_DMAto3DTable_HDR_OETF_SIZE+\
	...
	VIP_DMAto3DTABLE_INV_OUT_GAMMA_SIZE+VIP_DMAto3DTABLE_OUT_GAMMA_SIZE+VIP_DMAto3DTABLE_GAMMA_SIZE)
...
/* ========= HDR VIVID (CUVA) =============*/
#define support_hdr10_vivi_mode
/*=========================*/
...
typedef enum _VIP_DMAto3DTABLE_INDEX {
	...
	DMAto3DTABLE_SRNN,
	DMAto3DTABLE_OD,
	DMAto3DTABLE_INV_OUT_GAMMA,
	DMAto3DTABLE_OUT_GAMMA,
	DMAto3DTABLE_GAMMA,
...
typedef struct _VIP_ODPH_LUT_Gain_Data {
	...
	short ODPH_Gain_byLUT[6];	// RGB, recursive/non-recursive
...
enum {
	LD_SPI_2port_top_buttom = 0,
	LD_SPI_2port_left_right,
	LD_SPI_4port_top_buttom,
	LD_SPI_4port_left_right,
	LD_SPI_4port_hv1,	//left-top, right-top, left-but, right-but
	LD_SPI_4port_hv2,	//left-top, left-but, right-top, right-but
	LD_SPI_max,
};
...
typedef struct _LEDdriver_Arg {
	...
	unsigned short  leddriver_InitCurrent;
	unsigned short  leddriver_BlockNumber;
	unsigned short  leddriver_BoostCurrent;
	unsigned short  leddriver_CurrentStep;
} LEDdriver_Arg;

typedef struct _QS_LD_Ctrl {
	unsigned char  QS_LD_INIT;
	unsigned char  QS_LD_Level;
	unsigned char  QS_Backlight;
} QS_LD_Ctrl;
...
void fwif_color_set_rgb_sharpness_level(unsigned char value);
...
void fwif_color_Set_LD_lutTableIndex(unsigned char ld_level);
unsigned char  fwif_color_Get_LD_lutTableIndex(void);
// m8 add
...
void fwif_color_LD_SW_PowerProtection(void);
void fwif_color_set_LD_SPI_BoostEnable(unsigned char Enable);
unsigned char fwif_color_get_LD_SPI_BoostEnable(void);
void fwif_color_LD_SW_Boost(unsigned short *DataSRAM);
void fwif_color_LD_SW_Temporal_Filter(unsigned short* current_value, unsigned short *target_value, unsigned short step, unsigned short step_diff, unsigned short cnt);
...
char fwif_color_LD_init_quick_show(void);
char fwif_color_BLPF_init_quick_show(void);
void vpq_do_LD_init_from_tbl(void);
#if 0	// old qs, no need	
...
void fwif_color_set_LD_IndexSRAM_Mapping(unsigned char src_idx, unsigned char TableIdx);
...
void fwif_color_set_LEDdriver_Init (void);
void fwif_color_set_LEDdriver_CurrentSwitch(void);
void fwif_color_Get_LEDdriver_Args (LEDdriver_Arg* arg);
void fwif_color_Set_LEDdriver_Args (LEDdriver_Arg* arg);
void fwif_color_Set_LD_LEDDriverArg_Done(unsigned char status);
unsigned char  fwif_color_Get_LD_LEDDriverArg_Done(void);
...
char fwif_color_set_Gamma_InvGamma_Ctrl(unsigned char display, unsigned char tblSelect, unsigned short *invGamma_R, unsigned short *invGamma_G,  unsigned short *invGamma_B, unsigned char useISR, unsigned char ucDoCtrlBack);
...
char fwif_color_HDR_VIVID_frameSync_DMA_Apply_ISR(void);
...
void fwif_color_set_MEMC_internal_dither(unsigned char enable,VIP_PANEL_BIT default_vip_panel_bit,unsigned char nTableType);
...
void fwif_color_Set_NN_SRCtrl(unsigned char srnn_ctrl_idx, unsigned char shpLv);
...
void fwif_color_update_invgamma_gamma_table_TASK(void);
...
unsigned char fwif_color_get_demo_text_en(void);
void fwif_color_set_demo_text_en(void);
void fwif_color_clear_demo_text_en(void);
unsigned char fwif_color_get_game_demo_text_mode(void);
unsigned char fwif_color_req_demo_text_mode_changed(void);
...
unsigned short fwif_color_req_demo_text_fade_out_cnt(void);
...
void fwif_color_WB_Pattern_ctrl_osd_on_off(unsigned char enPattern);
unsigned char fwif_color_main_d1_is_2p_mode(void);

int fwif_color_set_Gamma_By_DMA(unsigned short *pDecode_TBL_R, unsigned short *pDecode_TBL_G, unsigned short *pDecode_TBL_B);
int fwif_color_set_InvOutGamma_By_DMA(unsigned short *pDecode_TBL_R, unsigned short *pDecode_TBL_G, unsigned short *pDecode_TBL_B);
int fwif_color_set_OutGamma_By_DMA(unsigned short *pDecode_TBL_R, unsigned short *pDecode_TBL_G, unsigned short *pDecode_TBL_B);
bool fwif_color_od_BND_early_init(void);
#if 0 //H Rim
