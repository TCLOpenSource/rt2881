...
enum {
	...
	/* 4 */ PQMASK_COLORSTRUCT_DYNAMIC,
	/* 5 */ PQMASK_COLORSTRUCT_DEMO,
	/* 6 */ PQMASK_COLORSTRUCT_NUM,
};
...
typedef struct PQMASK_COLOR_STATUS {
	...
	unsigned char AiGenre_CG_Enter_count;
	unsigned char AiGenre_CG_Exit_count;
	unsigned char MotionFlag;
	unsigned char Motion_CurrentGMV;
	unsigned char Motion_Enter_count;
	unsigned char Motion_Exit_count;
...
typedef struct PQMASK_COLOR_DEMO_CTRL {
	unsigned char bSemanticDemoEn;
	//unsigned char bDeepnessDemoEn;
} PQMASK_COLOR_DEMO_CTRL_T;
...
void scalerPQMaskColor_GetMotionFlag(void);
