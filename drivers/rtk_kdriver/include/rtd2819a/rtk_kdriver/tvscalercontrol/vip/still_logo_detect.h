...
#define SLD_Tbl_Block_NUM (40*24)
...
// Ring 20240528 for SLD
// Mac9q Modified
typedef struct {
    unsigned short SLD_local_gain_table[SLD_Tbl_Block_NUM]; // size increase from 576 = 32*18 to 960 = 40*24
	unsigned int SLD_tbl_dir_sel;
...
// Mac9q added
typedef struct {
    int SLD_tbl_fac_sel_en;
	int SLD_tbl_fac_sel;
} DRV_SLD_OutY_tbl_Fac_sel;
...
char drvif_color_set_SLD_CTRL(unsigned int phy_addr, unsigned int add_size, unsigned char scale_mode, unsigned int sld_width, unsigned int sld_height);
void drif_color_SLD_apply_normal_setting(void);

unsigned char drvif_color_set_SLD_LCblk_inform(void);
/*K8 Add CY 0510*/
// Mac9q Modified by Ring 20240528 
