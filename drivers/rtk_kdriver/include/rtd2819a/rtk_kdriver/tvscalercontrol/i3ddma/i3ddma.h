...
#else
    #define _4K_2K_36BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_4K_WID, 36) * _2K_LEN * 16)
    #define _4K_2K_10BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_4K_WID, 24) * _2K_LEN * 16)
    #define _4K_2K_12BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_4K_WID, 12) * _2K_LEN * 16)
    #define _4K_2K_20BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_4K_WID, 20) * _2K_LEN * 16)
    #define _2K_1K_36BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_2K_WID, 36) * _1K_LEN * 16)
    #define _2K_1K_10BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_2K_WID, 24) * _1K_LEN * 16)
    #define _2K_1K_12BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_2K_WID, 12) * _1K_LEN * 16)
    #define _2K_1K_20BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_2K_WID, 20) * _1K_LEN * 16)
    #define _2K_1K_24BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_2K_WID, 24) * _1K_LEN * 16)
    #define _3K_1K_12BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_3K_1K_WID, 12) * _3K_1K_LEN * 16)
    #define _3K_1K_20BIT_SIZE (calc_i3ddma_comp_line_sum_bit(_3K_1K_WID, 20) * _3K_1K_LEN * 16)
...
typedef enum {
	IDDMA_CAPTURE_BUFFER_FLAG_RDY,      //buffer allocate or mmap done
	IDDMA_CAPTURE_BUFFER_FLAG_IDLE,
	IDDMA_CAPTURE_BUFFER_FLAG_WD,
	IDDMA_CAPTURE_BUFFER_FLAG_QUEUD,
}IDDMA_CAPTURE_BUFFER_FLAGE;
...
typedef struct {
	...
	unsigned int flag;  //bit mean IDDMA_CAPTURE_BUFFER_FLAGE
	int fd; //for dma buffer
	unsigned int bufferId;
	unsigned int crc;
	unsigned long long timestamp;
	unsigned long long weight; //write once to increase 1
...
typedef enum {
	...
	I3DDMA_COLOR_YUV444,
	I3DDMA_COLOR_YUV420,
	I3DDMA_COLOR_NV8BITS, //nv12 8bits
	I3DDMA_COLOR_NV10BITS, //nv12 10bits
	I3DDMA_COLOR_YV12, //yv12 8bits
	I3DDMA_COLOR_I420, //I420 8bits
	I3DDMA_COLOR_P010, //P010 10bits
	I3DDMA_COLOR_UNKNOW
...
typedef enum _IDDMA_WORK_MODE_E_{
	IDDMA_DISPLAY_MODE,
	IDDMA_NN_MODE,
	IDDMA_CAP_MODE,
}IDDMA_WORK_MODE_E;

typedef enum _IDDMA_PORT_STATE_E_{
	IDDMA_PORT_IDLE,
	IDDMA_PORT_OCCUPY,
}IDDMA_PORT_STATE_E;

typedef enum _IDDMA_MEMORY_TYPE_E_{
	IDDMA_MEMORY_MMAP,
	IDDMA_MEMORY_DMA,
	IDDMA_MEMORY_USERPTR,
}IDDMA_MEMORY_TYPE_E;

#define IDDMA_DEFAULT_BUFFER_NUM (4)
#define IDDMA_MAX_BUFFER_NUM (32)
typedef struct _IDDMA_PORT_ATTR_T_{
	IDDMA_PORT_STATE_E occupy;
	unsigned char forcebg_en; //true ap will draw black picture
	unsigned char source_type;
	unsigned char hdmi_port; //hdmi or vo port
	unsigned char cap_format;
	unsigned char cap_depth;
	unsigned char format_is_3planes;
	IDDMA_MEMORY_TYPE_E memory_type;
	unsigned char buffer_num;
	unsigned int pitch;
	unsigned int image_size; //one buffer size  
	unsigned int total_buffer_size;
	unsigned long start_buffer_addr;
	StructSrcRect input_region;
	StructSrcRect output_region;
	unsigned int read_point;
	unsigned int write_point;
	I3DDMA_CAPTURE_BUFFER_T iddma_buffer[IDDMA_MAX_BUFFER_NUM];
#ifndef BUILD_QUICK_SHOW
	spinlock_t iddma_spinlock;
#endif
}IDDMA_DEV_ATTR_T;
...
extern void I3DDMA_Get3DGenTiming(unsigned char dev, I3DDMA_TIMING_T *tx_timing, I3DDMA_TIMING_T *gen_timing);
extern void I3DDMA_BandWidth_Control(unsigned char dev, I3DDMA_TIMING_T* tx_timing, I3DDMA_TIMING_T*gen_timing);
...
#endif
void IDDMA_Mapping_Port_BySourceAndIdx(unsigned char source_type, unsigned char idx);
