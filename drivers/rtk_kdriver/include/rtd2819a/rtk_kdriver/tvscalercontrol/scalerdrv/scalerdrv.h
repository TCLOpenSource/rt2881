...
//#define CONFIG_MDOMAIN_FORCE_CARVEOUT 1

#ifdef CONFIG_SCALER_BRING_UP
// #define CONFIG_RUN_DDTS
#ifndef CONFIG_RUN_DDTS
#define AUTO_BRINGUP_SOURCE _SRC_HDMI //_SRC_TV  _SRC_CVBS  _SRC_HDMI _SRC_MINI_DP
#define AUTO_BRINGUP_SOURCE_SUB _SRC_MAX
#define BRING_UP_MULTIPORT_NUM 3 // This is current multiport number
...
#define IDDMA_MAX_OUTPUT_WIDTH (2048)
#define IDDMA_MAX_OUTPUT_HEIGHT (1080)
#define IDDMA_DEFAULT_OUTPUT_WIDTH (1920)
#define IDDMA_DEFAULT_OUTPUT_HEIGHT (1080)
...
typedef enum {
    VGIP_SRC_HDMI1 = 0, // 000: HDMI1 (2p)
    VGIP_SRC_HDMI2,     // 001: HDMI2 (2p)
    VGIP_SRC_ATV,       // 010: VD/ATV
    VGIP_SRC_VODMA,     // 011: VODMA
    VGIP_SRC_HDR1,      // 100: HDR1 (2p)
    VGIP_SRC_HDR2,      // 101: HDR2 (2p)
    VGIP_SRC_HDMI3,     // 110: HDMI3 (2p)
    VGIP_SRC_HDMI4,     // 111: HDMI4 (2p)
} VGIP_SRC_TYPE;

typedef enum _VGIP_PATH
{
    VGIP_PATH_I1 = 1,   // MAIN path
    VGIP_PATH_I2 = 2,   // SUB path
    VGIP_PATH_I3 = 3,   // DMA path
    VGIP_PATH_I4 = 4,
    VGIP_PATH_I5 = 5,
    VGIP_PATH_I6 = 6,
} VGIP_PATH;
...
//BBD_FUNCTION_DONE ->BBD_FUNCTION_REQ_ENABLE -> BBD_FUNCTION_REQ_VGIP_DRIVER->BBD_FUNCTION_DONE
    ...
    BBD_FUNCTION_REQ_VGIP_DRIVER = 1,//set vgip driver for bbd detect
    BBD_FUNCTION_REQ_ENABLE = 2,//ioctrl to set this . request to do BBD detect. set vgip driver request
}_BBD_FUNCTION_STAGE;
...
void drvif_scaler_set_display_mode(unsigned char timing_mode, DTG_APP_TYPE dtg_app_type);
...
void Set_Val_DP_Vsync_Regen_Pos(unsigned char dev, unsigned char src);
...
extern void (*drivf_scaler_reset_freerun)(DTG_TYPE);
...
void modestate_set_fractional_framesyncByDisp(unsigned char _channel, DTG_APP_TYPE dtgAppType);
...
unsigned char get_hdmi_4k_hfr_mode_with_display(unsigned char display);//two pixel mode enable
...
void decide_scaler_hdmi_hfr_mode(unsigned char display);//save scaler need to run 4k120 or not. decide at scaler start
...
typedef enum
{
	LINERATE_UPDATE_S1_F2P_GATE_SEL,
	LINERATE_UPDATE_UZUDTG_DH_TOTAL,
	LINERATE_UPDATE_DCLK_FRACT
}LINERATE_UPDATE_INFO;
...
void bbd_info_update_GAVmode(unsigned char display, StructRect active_size, StructRect ori_size);//active win change event
...
void Set_Val_scaler_vdec_source_type(unsigned char type, unsigned char display);//update the vdec source type for scaler
unsigned char Get_Val_scaler_vdec_source_type(unsigned char display);//get the vdec source type for scaler
...
int modestate_set_live_stream_iv2dv_ctrl(void);
