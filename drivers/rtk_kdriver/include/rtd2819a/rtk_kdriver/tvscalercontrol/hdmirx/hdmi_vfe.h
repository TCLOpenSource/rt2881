...
typedef enum {
    VFE_HDMI_REPORT_ZERO_TIMING = _BIT0,
    VFE_HDMI_SOURCE_GOOD_TIMING_READY = _BIT1,
    VFE_HDMI_CLEAR_ZERO_FLAG = 0,
    VFE_HDMI_ZERO_TIMING_FLAG_NON = _BIT2,
}VFE_HDMI_ZERO_TIMING_CHECK;
...
/**
 *  * int vfe_hdmi_drv_connect_h5x(unsigned char port)
 *  @param [IN] port, hdmi port
 *  @return 0 : ok; other not ok
 *  it connects hdmi to detect timing for selected port
 */
...
/**
 *  * int vfe_hdmi_drv_get_port_timing_info_h5x(vfe_hdmi_timing_info_t *timing_info)
 *   * @param [IN] timing_info, refer to vfe_hdmi_timing_info_t
 *    * @return 0 : ok; other not ok
 *     *
 *      * it gets timing info for selected port
...
*/
void vfe_hdmi_drv_handle_on_line_measure_error(unsigned char ch, unsigned char vfe_call);
...
unsigned char vfe_hdmi_drv_timing_ready(unsigned char ch);//for k4lp new scaler flow use. By will
void vfe_hdmi_drv_reset_hdmi_timing_ready(unsigned char ch);//for k4lp new scaler flow use. By will
...
extern void vfe_hdmi_bypass_mode_set(unsigned char ch, unsigned char bypass_mode);

/*------------------------------------------------------------------
 * Func : vfe_hdmi_set_zero_timing_flag
 *
 * Desc : api that used for setting hdmi zero timing status
 *
 * Para : [IN] ch     :  channel number
          [IN]  zero_timing_status: 0->CLEAR_ZERO_FLAG, 1->REPORT_ZERO_TIMING, 2->SOURCE_GOOD_TIMING_READY
 *
 * Retn :   Non
 *------------------------------------------------------------------*/
extern void vfe_hdmi_set_zero_timing_flag(unsigned char ch, VFE_HDMI_ZERO_TIMING_CHECK zero_timing_status);
/*------------------------------------------------------------------
 * Func : vfe_hdmi_get_zero_timing_flag
 *
 * Desc : api that used for getting hdmi zero timing status
 *
 * Para : [IN] ch     :  channel number
 *
 * Retn :  VFE_HDMI_ZERO_TIMING_CHECK enum
 *------------------------------------------------------------------*/
extern VFE_HDMI_ZERO_TIMING_CHECK vfe_hdmi_get_zero_timing_flag(unsigned char ch);
