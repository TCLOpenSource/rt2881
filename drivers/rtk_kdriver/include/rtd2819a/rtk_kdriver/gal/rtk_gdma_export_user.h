...
#define GDMA_CONFIG_ANDROID_LAYERS_EXP 	_IOR(GDMA_IOC_MAGIC, 76, int)
#define GDMA_CONFIG_ANDROID_LAYERS_V3 	_IOR(GDMA_IOC_MAGIC, 77, int)

#define GDMA_CONFIG_GDMAEXT_TEST_CMD 	_IOR(GDMA_IOC_MAGIC, 78, int)
#define GDMA_SET_MIXER_VIDEO_COLOR_RANGE 	_IOR(GDMA_IOC_MAGIC, 79, int)
#define GDMA_GET_OSD_PANEL_CONFIG                      _IOR(GDMA_IOC_MAGIC, 80, int)
#define GDMA_GET_OSD_RUNTIME_STATUS                    _IOR(GDMA_IOC_MAGIC, 81, int)

#define GDMA_IOC_MAXNR                 82
...
typedef enum {
	...
	VO_OSD_COLOR_FORMAT_ARGB2101010,	// 0x8 A2R10G10B10
	...
	VO_OSD_COLOR_FORMAT_RGB888,     /* 22(tfbc).  24-bit RGB    (888)  with constant alpha */

	VO_OSD_COLOR_FORMAT_RGB101010_TFBC=23 ,	// 0x17  TFBC/o5 pqdc
	VO_OSD_COLOR_FORMAT_PQDC=23 ,		// 0x17  TFBC/o5 pqdc

	// > 0x1F (31) is little endian 
	...
	VO_OSD_COLOR_FORMAT_ARGB8888_LITTLE   = 39,  // 0x27
	VO_OSD_COLOR_FORMAT_ARGB2101010_LITTLE= 40,  // 0x28
	...
	//for least 5 bits are 0x0 to avoid conflict
	//VO_OSD_COLOR_FORMAT_PQC_LITTLE     = (0x01<<5),
...
typedef struct {
	...
	unsigned int wb_img_addr;		/*  physical address for OSD compress output address 0: not write-back, otherwise do write
-back function */
...
#define GDMA_GRAPHIC_LAYERS_OBJECT_V2_VER (0xFEA00002)
typedef struct {
	int version;
	int size;
	
	// start of GRAPHIC_LAYERS_OBJECT{}
	int layer_num;
    GDMA_LAYER_OBJECT layer[GDMA_LAYER_NUM];
	int cookie;// 1:blocking mode 2:nonblocking mode
...
} GRAPHIC_LAYERS_OBJECT_V2;

/*
ChangeLog:
  1. for multi seg, add GRAPHIC_LAYERS_OBJECT_V3


*/

typedef struct __attribute__((packed)) {
    int osd_plane:4;	//type VO_GRAPHIC_OSD1 xx
    int reserved1:12;
    int reserved2:16;
} LAYER_OBJ_flag_V1;
...
} GDMA_LAYER_OBJECT_V3;


#define GDMA_GRAPHIC_LAYERS_OBJECT_V3_VER (0xFEA00003)
typedef struct {
	int version;  
	int size;// first /second fields must fit
	
	// start of GRAPHIC_LAYERS_OBJECT{}
	int layer_num;
    GDMA_LAYER_OBJECT_V3 layer[GDMA_LAYER_NUM];
	int cookie;// 1:blocking mode 2:nonblocking mode
...
typedef struct {
    //must specify
    VO_GRAPHIC_PLANE    plane;      //user mode plane
    int                 enable;

    //plane alpha ,... 
    

} GDMA_MODIFY_MIXER_CMD;

typedef struct {
	unsigned int refresh_interval;
	unsigned int sw_vsync_ns;
	unsigned int vsync_ns;
	int is_osd_split;
	int resver1;
	int resver2;
	int resver3;
	int resver4;
} GDMA_OSD_RUNTIME_STATUS;



int GDMA_ReceiveGraphicLayers(GRAPHIC_LAYERS_OBJECT *data);
int GDMA_ReceiveGraphicLayers_exp(GRAPHIC_LAYERS_OBJECT_V2 *data);

int GDMA_ReceiveGraphicLayers_V3_in(GRAPHIC_LAYERS_OBJECT_V3 *data);
