...
#include "osdExt/rtk_osdExt_rbus.h"
...
#if defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6748) || defined(CONFIG_ARCH_RTK2875Q) || defined(CONFIG_ARCH_RTK2819A)
...
#define GDMA_LOG_ALL (9)
#define GDMA_LOG_VERBOSE (7)
#define GDMA_LOG_DEBUG  (5)
#define GDMA_LOG_WARNING (3)
#define GDMA_LOG_ERROR (2)
#define GDMA_LOG_FATAL (1)
#define GDMA_LOG_DUMP (GDMA_LOG_FATAL)
#ifdef GDMA_DEBUG
...
#define GDMA_ERR(fmt, args...) do { \
   rtd_pr_gdma_err("[GDMA] ERR:" fmt, __FUNCTION__, __LINE__, ## args); \
  } while(0)

#define GDMA_PRINT(level,fmt, arg...) do { \
	if( unlikely(gDebugGDMA_loglevel >= level) ) { rtd_pr_gdma_crit("gdma "  fmt, ##arg ); } \
   } while(0) 
...
typedef struct _GDMA_WIN_KEY_MIN {
    unsigned int key_min:24;
    unsigned int keyEn:1;
    unsigned int keymin_29_24:6;
	unsigned int color_range_enable:1;
} GDMA_WIN_KEY_MIN;

typedef struct _GDMA_WIN_KEY_MAX {
    unsigned int key_max:30;
...
typedef struct {
	...
	union {
		...
		GDMA_WIN_KEY_MIN  colorRange_min;
	};
    union {
...
#ifndef BUILD_QUICK_SHOW
//#if defined(CONFIG_ARCH_RTK287O) || defined(CONFIG_ARCH_RTK288O) || defined(CONFIG_ARCH_RTK2875) || defined(CONFIG_ARCH_RTK2885M) || defined(CONFIG_ARCH_RTK6748) || defined(CONFIG_ARCH_RTK2875Q) || defined(CONFIG_ARCH_RTK2819A)
    unsigned int      fbdc3_addr;
    unsigned int      fbdc4_addr;

    TFBC_FORMAT             tfbc_format;
    AFBC_TOTAL_TILE         afbc_totaltile;
    AFBC_PIC_PIXEL_XY       afbc_xy_pixel;
    AFBC_START_XY           afbc_xy_sta;
	GDMA_WIN_KEY_MAX		colorRange_max;
//#endif
#endif
...
typedef struct {
    ...
    GDMA_REG_CONTENT reg_content;   /*  pre-calculate by workqueue and store */
        GDMA_4_REG_CONTENT reg_4_content;
        GDMA_5_REG_CONTENT reg_5_content;
    };
    #else
        GDMA_REG_CONTENT reg_content;   /*  pre-calculate by workqueue and store */
    #endif//
...
typedef struct {
    ...
    int osd4_init_on;
    int osd5_init_on;
    OSD_FLOW_STATE osd4_status;
    OSD_FLOW_STATE osd5_status;

    struct semaphore sem_receiveExt;
    
    int osd45_sync;
    int enable_identity[GDMA_PLANE_MAXNUM];
...
/* @brief Notify gdma should enable/disable osdSPLIT mode 
 * 
 * @param[in] enable  0: disable osd Split
                      1: enable os split
         [in] param2 not used currently
 */
