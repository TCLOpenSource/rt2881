...
#define GPU_decmp_lu_VEdataStart_reg (GPU_decmp_lu_GPUdataStart_reg)
...
#define GPU_decmp_ch_VEdataStart_reg (GPU_decmp_ch_GPUdataStart_reg)
...
//#ifdef DECMP_PRE_SET_VALUE
 ...
 #define DECMP_CHECK_VALUE(addr, val) \
    do { \
		rtd_pr_gdecmp_err("hope rtd_outl(0x%08x, 0x%08x);\n", addr-0xA0000000, val); \
		if( rtd_inl(addr) != val ) { \
			rtd_pr_gdecmp_err("kkk addr: 0x%08x value not same 0x%08x != 0x%08x\n", addr, rtd_inl(addr), val); \
		} \
    } while(0)
...
#define DECMP_RTDOUTL_GPU(addr, val) \
    do { \
        rtd_pr_gdecmp_err("rtd_outl(0x%08x, 0x%08x);\n", addr-0xA0000000, val); \
        rtd_outl(addr, val); \
        rtd_pr_gdecmp_err("\t addr: 0x%08x, got 0x%08x   %s %d\n", addr-0xA0000000, rtd_inl(addr), __FUNCTION__, __LINE__ ); \
    } while(0)
...
#define DECMP_PRINT(level,fmt, arg...) do { \
       if( unlikely(gDebugDecmp_loglevel >= level) ) { rtd_pr_gdecmp_crit("gDecmp TT:%lld "  fmt, ktime_to_us(ktime_get()), ##arg ); } \
   } while(0)



#define DECMP_LOG_DEBUG rtd_pr_gdecmp_err

#if 1
#define DECMP_LOG_MEM rtd_pr_gdecmp_err
#else
