...
# check tag name
#####################################
TAG_NAME_PRESENT=$(grep -w TAG_NAME_$TAG_NAME $MODULE_TAG_FILE)
...
# check log api
#####################################
API_PRESENT=$(grep -w "print api for $TAG_NAME" $MODULE_LOG_FILE)


if [ -z "$API_PRESENT" ]
then
    echo "API_PRESENT absent, force add it"
    cat $MODULE_TAG_FILE | grep -w $TAG_NAME > $MODULE_TAG_FILE_TMP
