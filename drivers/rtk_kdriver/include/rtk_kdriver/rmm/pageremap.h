...
// rtkmem_query_address_V1, version = (0xEF000001)
#define RTKMEM_QUERY_ADDR_V1_VERSION (0xEF000001)
typedef struct  {
	__u32 version;
    __u32 fd;
    __u64 stamp_id;
	__u32 multi_seg_count; //#chunks
    __u64 address;
    __u32 address_size;
    __u64 addr_2;
	__u32 addr_size_2;
...
#define DVR_OWNER_VE_FLUSH    0xFF0A00FC //FIXME non-dcmt tag for ve memory flush

// The following functions are used by user to allocate continuous memory...
#if 1 // va_arg wrapper, TODO: fixed args with owner id
#define pli_malloc(SIZE, FLAGS, ...) va_pli_malloc(SIZE, FLAGS, ##__VA_ARGS__, DVR_OWNER_NOID)
unsigned long va_pli_malloc(size_t size, int flags, ...); // 3rd arg: DCMT MODULE_INFO.id
#endif
...
static inline void cma_unset_bp(int dev_idx) {}
...
static inline unsigned long get_memory_size_total(void) { return 0; }
...
static inline unsigned long get_cma_avail_size_specific(int flags, size_t size, unsigned long *total) { return 0; }
