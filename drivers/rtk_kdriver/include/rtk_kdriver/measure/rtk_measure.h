...
typedef struct {
    ...
    unsigned int single_p_mode_thd;
    unsigned char is_v_drop; // 1: v drop, 0:not v drop
    //---- info for DSC ----
    unsigned char dsc_version_minor;    // for DSC mode
    unsigned char convert_rgb : 1;    // Indicates whether DSC color space, 0 = Color space is YCbCr, 1 = Encoder converts RGB to YCoCg-R,
    unsigned char simple_422 : 1;    // for DSC mode
    unsigned char native_420 : 1;    // for DSC mode
    unsigned char native_422 : 1;    // for DSC mode
    unsigned char is_dsc_to_tx : 1;  // 1:timing is dsc, 0:timing is not dsc
    unsigned short cvtem_h_front;    // for HDMI TX CVTEM
    unsigned short cvtem_h_sync;    // for HDMI TX CVTEM
    unsigned short cvtem_h_back;    // for HDMI TX CVTEM
    unsigned short cvtem_hcative_bytes;    // for HDMI TX CVTEM
    //---------------------
    unsigned int measure_wait_ms;
} MEASURE_TIMING_T;
