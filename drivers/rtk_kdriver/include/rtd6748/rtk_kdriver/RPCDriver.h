...
#define RPC_DTS_MAP_PHYS		0x1b100000
#ifdef CONFIG_ARM64
#define CONFIG_ARM64_RPC
extern void __iomem *rpc_base;
#define get_rpc_addr(x)     (rpc_base + ((long)(x) - RPC_DTS_MAP_PHYS))
#define rpc_pa(x)           ((u64)x-(u64)(rpc_base-RPC_DTS_MAP_PHYS))
...
#else
extern void __iomem *rpc_base;
extern unsigned long rpc_phy_base;
#define get_rpc_addr(x)     (rpc_base + ((long)(x) - rpc_phy_base))
#define rpc_pa(x)           (x-(char *)(rpc_base-rpc_phy_base))
...
{
	int size = get_ring_data_size(get_rpc_addr(dev->ringStart), get_rpc_addr(dev->ringEnd), get_rpc_addr(dev->ringIn), out);
	int tail = (int)get_rpc_addr(dev->ringEnd) - (int)out;
	...
	if(tail >= datasize){
		...
		my_memcpy((int *)buf, (int *)out, tail);
		my_memcpy((int *)(buf + tail), (int *)get_rpc_addr(dev->ringStart), datasize - tail);
		out = get_rpc_addr(dev->ringStart) + datasize - tail;
	}
...
static inline void update_nextRpc(RPC_DEV_EXTRA *extra, char *nextRpc)
{
	...
	if (tmp == (int)get_rpc_addr(extra->dev->ringEnd))
		extra->nextRpc = get_rpc_addr(extra->dev->ringStart);
	else
		extra->nextRpc = (char *)tmp;
}
...
{
	RPC_COMMON_Dev *dev = extra->dev;
	return get_rpc_addr(dev->ringOut) == extra->nextRpc;
}

static inline int ring_empty(RPC_COMMON_Dev *dev)
{
	return get_rpc_addr(dev->ringOut) ==get_rpc_addr(dev->ringIn);
}
#endif
