...
typedef struct
{
	...
	unsigned char  u8_force_repeat_level_flag;
	unsigned char  u8_force_caddpnt;
...
typedef struct {
	unsigned char kphase_me_work_mode;
	unsigned char kme_me_work_mode;
	unsigned char me_work_mode_multi_scan;
	unsigned char kme_me2_vbuf_doublescan_en;
}_str_ME_Scan_table;
...
typedef struct {
	unsigned char	mc_var_fir_slope;
	unsigned char	mc_var_fir_y_max;
	unsigned char	mc_var_fir_y_min;
}_str_MC_VarLpf_table;
...
VOID Wrt_ComReg_Manual_colormatrix(_OUTPUT_ME_SCENE_ANALYSIS *pSceneInfo);
...
void FixSubtitleBroken_Set(void);
BOOL MEMC_HSR_ON_OFF_ChangeStatus(void);
BOOL Check_MEMC_DMA_Error(void);
void FixSubtitleBroken_Set(void);
unsigned int MEMC_Lib_Get_s16_speed(void);
