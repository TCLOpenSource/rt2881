...
typedef struct {
    ...
    unsigned char convert_rgb;    // Indicates whether DSC color space, 0 = Color space is YCbCr, 1 = Encoder converts RGB to YCoCg-R,
    unsigned char simple_422;    // for DSC mode
    unsigned char native_420;    // for DSC mode
    unsigned char native_422;    // for DSC mode
} HDMI_TIMING_T;
...
extern unsigned char drvif_Hdmi_GetVsemIsDolby(unsigned char port);
extern unsigned char drvif_Hdmi_get_dolby_vsem_infoframe(unsigned char port, unsigned char *p_vsem);
extern unsigned int drvif_hdmi_get_no_dolby_vsem_emp_cnt(unsigned char port);
