...
unsigned char g_is_run_m8p_flow = 0;
//------------------------------------------------------------------
// CRT
//------------------------------------------------------------------
...
void lib_hdmi_stb_wakeup_by_tmds_init(unsigned char nport, unsigned char en)
{
    if(en)
    {
        ...
        hdmi_mask(HDMI_STB_GDI_TMDSCLK_SETTING_02_reg, \
            ~HDMI_STB_P0_GDI_TMDSCLK_SETTING_02_clk_counter_debounce_mask, 0x10);  // stable for X times
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_clear_avmute(nport);
    }else{
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        return m8p_lib_hdmi_get_avmute(nport);
    }
...
{
    if (lib_hdmi_get_run_m8p_flag())
        return COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_get_yuv420_in_sel(hdmi_in(M8P_HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_reg));
    else
        return COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_get_yuv420_in_sel(hdmi_in(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg));
}
...
void lib_hdmi_set_yuv420_in_sel(unsigned char nport, COMMON_420_REG_YUV420_IN_SEL_T source_in)
{
    ...
    {
        if (lib_hdmi_get_run_m8p_flag())
            rtd_maskl(M8P_HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask), COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(source_in));
        else
            rtd_maskl(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask), COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(source_in));
    }
    else
    {
...
void lib_hdmi_video_pll_init(unsigned char nport)
{
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
        ...
        hdmi_mask(addr, ~M8P_HDMI_DP_COMMON_HDMIRX_YUV420_CTRL_1_yuv420_cr1_mode_mask, 0);
        //hdmi_mask(HDMI_YUV420_CR1_reg, ~HDMI_YUV420_CR1_vactive_mask, HDMI_P0_YUV420_CR1_vactive(0));
        hdmi_mask(addr, ~M8P_HDMI_DP_COMMON_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive_mask, M8P_HDMI_DP_COMMON_HDMIRX_YUV420_CTRL_1_yuv420_cr1_vactive(0));
    }else{
        //hdmi_mask(HDMI_YUV420_CR1_reg, ~HDMI_YUV420_CR1_mode_mask, 0);// no use
    ...
    )
{
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
        if(nport < 2){
            return m8p_lib_hdmi_set_video_dprxpll(nport, b_clk, frl_mode, cd, is_interlace, is_420, pll_pixelclockx1024);
        }
    }
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_crc_continue_mode_en(nport, enable);
    }else{
        if(enable)
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        return M8P_HDMI_CRC_Ctrl_get_crc_conti(hdmi_in(M8P_HDMI_CRC_Ctrl_reg));
    }else{
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        hdmi_out(M8P_HDMI_DES_CRC_reg, common_crc);
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return M8P_HDMI_DES_CRC_get_crc_des(hdmi_in(M8P_HDMI_DES_CRC_reg));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return M8P_HDMI_CRC_ERR_CNT0_get_crc_err_cnt(hdmi_in(M8P_HDMI_CRC_ERR_CNT0_reg));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_crc_auto_cmp_en(nport, enable);
    }else{
        if(enable){
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return m8p_lib_hdmi_get_crc_auto_cmp_status(nport);
    else
...
{// Common crc
    if(lib_hdmi_get_run_m8p_flag() == 1){
        return m8p_lib_hdmi_read_video_common_crc(nport, res_sel, p_crc, is_continue_mode);
    }else{
        if(is_continue_mode)
        {
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        hdmi_mask(M8P_HDMI_YUV422_CD_reg, ~(M8P_HDMI_YUV422_CD_det_en_mask), enable ? 1 : 0);
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return M8P_HDMI_YUV422_CD_get_det_en(hdmi_in(M8P_HDMI_YUV422_CD_reg));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        hdmi_mask(M8P_HDMI_YUV422_CD_reg, ~(M8P_HDMI_YUV422_CD_frame_cnt_thr_mask), M8P_HDMI_YUV422_CD_frame_cnt_thr(g_yun422_cd_frame_cnt_thd));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return M8P_HDMI_YUV422_CD_get_frame_cnt_thr(hdmi_in(M8P_HDMI_YUV422_CD_reg));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        hdmi_mask(M8P_HDMI_YUV422_CD_reg, ~(M8P_HDMI_YUV422_CD_pixel_cnt_thr_mask), M8P_HDMI_YUV422_CD_frame_cnt_thr(g_yuv422_cd_pixel_cnt_thd));
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return M8P_HDMI_YUV422_CD_get_pixel_cnt_thr(hdmi_in(M8P_HDMI_YUV422_CD_reg));
    else
...
YUV422_CD_STATUS_T lib_hdmi_yuv422_get_color_depth(unsigned char nport)
{
    if(lib_hdmi_get_yuv422_cd_det_en_status(nport) == 1)
    {
        ...
        if(lib_hdmi_get_run_m8p_flag() == 1){
            det_12b_flag = HDMI_YUV422_CD_get_det_12b_flag(hdmi_in(M8P_HDMI_YUV422_CD_reg));
            det_10b_flag = HDMI_YUV422_CD_get_det_10b_flag(hdmi_in(M8P_HDMI_YUV422_CD_reg));
        }else{
...
{//at video common, p0=p1=p2=p3 
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_set_hd20_21_ctrl_pixel_mode(nport, pixel_mode);
    }else{
        switch(pixel_mode)
        {
        case 1: // 1 pixel mode
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_dsc_clk_div_sel_by_pixel_mode(nport, pixel_mode);
    }else{
        //no need
        return;
    }
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        return m8p_lib_hdmi_get_fw_debug_bit(nport, debug_bit);
    else
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1)
        m8p_lib_hdmi_set_fw_debug_bit(nport, debug_bit, on);
    else
...
void lib_hdmi_mac_init(unsigned char nport, unsigned char br_swap)
{
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
     // 1. ex phy fifo ck en
        hdmi_mask(M8P_HDMI_EX_PHY_FIFO_CR0_reg,
            ~(M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_b_afifo_en_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_g_afifo_en_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_r_afifo_en_mask),
            (M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_b_afifo_en_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_g_afifo_en_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_r_afifo_en_mask));    // Port0

        // 2. ex phy afifo en
        hdmi_mask(M8P_HDMI_EX_PHY_FIFO_CR2_reg,
            ~(M8P_HDMI_EX_P0_EX_PHY_FIFO_CR2_port0_c_afifo_en_mask), M8P_HDMI_EX_P0_EX_PHY_FIFO_CR2_port0_c_afifo_en_mask);    // Port0_c_afifo_en & Port1_c_afifo_en = 

        // 3. ex mac  clk invert
        hdmi_mask(M8P_HDMI_EX_PHY_FIFO_CR1_reg,
            ~(M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_rclk_inv_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_gclk_inv_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_bclk_inv_mask),
           (M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_rclk_inv_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_gclk_inv_mask|M8P_HDMI_EX_P0_EX_PHY_FIFO_CR1_port0_bclk_inv_mask));    // port0

        // 4. ex phy fifo c clk inv
        hdmi_mask(M8P_HDMI_EX_PHY_FIFO_CR3_reg,
            ~(M8P_HDMI_EX_P0_EX_PHY_FIFO_CR3_port0_cclk_inv_mask),M8P_HDMI_EX_P0_EX_PHY_FIFO_CR3_port0_cclk_inv_mask);    //Port0_cclk_inv & Port1_cclk_inv = 1

        // 5. PHY FIFO R/G/B ch async fifo enable
        hdmi_mask(M8P_HDMI_PHY_FIFO_CR0_reg,
            ~(M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_b_afifo_en_mask|M8P_HDMI_P0_PHY_FIFO_CR0_port0_g_afifo_en_mask|M8P_HDMI_P0_PHY_FIFO_CR0_port0_r_afifo_en_mask),
            (M8P_HDMI_EX_P0_EX_PHY_FIFO_CR0_port0_b_afifo_en_mask|M8P_HDMI_P0_PHY_FIFO_CR0_port0_g_afifo_en_mask|M8P_HDMI_P0_PHY_FIFO_CR0_port0_r_afifo_en_mask));    // Port0

        // 6. PHY FIFO R/G/B PHY input clk invert
        hdmi_mask(M8P_HDMI_PHY_FIFO_CR1_reg,
            ~(M8P_HDMI_P0_PHY_FIFO_CR1_port0_bclk_inv_mask|M8P_HDMI_P0_PHY_FIFO_CR1_port0_gclk_inv_mask|M8P_HDMI_P0_PHY_FIFO_CR1_port0_rclk_inv_mask),
            0x0);    // Port0
    }else{
        // 1. ex phy fifo ck en
        hdmi_mask(HDMI_EX_P0_EX_PHY_FIFO_CR0_reg,
...
//    lib_hdmi_mac_afifo_enable(nport,HDMI_4LANE);
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
        ...
        m8p_lib_hdmi_drop_pkt_enable(nport, DROP_PKT_ALL, 1);
        m8p_lib_hdmi_pkt_to_hdmitx_output_enable(nport, 0);
        // support hd20 mac measure
        SET_FLOW_CFG(HDMI_FLOW_CFG_TIMING_MEASURE, HDMI_FLOW_CFG7_HD20_TIMING_MEASURE_EN, 1);
    }
}
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd20_fec_measure_restart(nport);
    }else{
        ;//not support
    }
}
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hd20_measure(port, update_timing);
    }else{
        ;//not support
    }
}
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd20_clr_measure_last_vs_cnt(nport);
    }else{
        ;//not support
    }
}
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd20_meas_set_frame_pop_mode(nport, mode);
    }else{
        ;//not support
    }
}
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        return m8p_lib_hdmi_hd20_meas_get_frame_pop_mode(nport);
    }
...
void lib_hdmi_output_disable(unsigned char port)
{
    ...
    for(i=0; i<HDMI_PORT_TOTAL_NUM; i++)
    {
        ...
        lib_hdmi_video_output(i, 0);     // disable video output
        lib_hdmi_audio_output(i, 0);     // close audio
        lib_hdmi_ofms_clock_sel(i, 0);   // switch back to tmds clock
        lib_hdmi_set_dpc_enable(i, 0);   // disable dpc
    }
}

void lib_hdmi_set_run_m8p_flag(void)
{
    if(get_mach_type()==MACH_ARCH_RTK2885P){
        HDMI_INFO("%s: m8 run m8\n", __func__);
        g_is_run_m8p_flow = 0;
    }else if(get_mach_type()==MACH_ARCH_RTK2885P2){
       HDMI_INFO("%s: m8p run m8\n", __func__);
       g_is_run_m8p_flow = 1;
    }else if(get_mach_type()==MACH_ARCH_RTK2885PP){
        HDMI_INFO("%s: m8p run m8p\n", __func__);
        g_is_run_m8p_flow = 1;
    }
}
unsigned char lib_hdmi_get_run_m8p_flag(void)
{
    return g_is_run_m8p_flow;
}
