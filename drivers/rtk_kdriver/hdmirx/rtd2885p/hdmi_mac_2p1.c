...
{//Alwasy use physical port 0 address
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_mac_crt_perport_on(nport);
    }else{
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_mac_crt_common_reset(nport);
    }else{
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_timing_gen_en_vs_align(nport, delay_ms);
    }else{
...
void lib_hdmi_hd21_clk_div_sel(unsigned char nport, unsigned char pixel_mode)
{
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_clk_div_sel(nport, pixel_mode);
    }else{
        switch(pixel_mode)
        {
        case 1: // 1 pixel mode
            ...
            hdmi_mask(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel_mask),
                    COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel(0));
            break;
...
{
	;// ml8 not support 1frame pop mode, do nothing
}

// 0: 4 frame pop  1: 1 frame pop
unsigned char lib_hdmi_hd21_meas_get_frame_pop_mode(unsigned char nport)
{
    return 0;//ml8 not support 1frame pop mode,return 4 frame pop
}
...
HDMI21_MEASURE_RESULT lib_hdmi21_hd21_measure(unsigned char nport, unsigned char is_dsc_mode, MEASURE_TIMING_T* update_timing)
{
    ...
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_set_polarity_meas_mode(nport, HD21_POLARITY_MANUAL_MEAS);//invert
    }
...
{
    if(lib_hdmi_get_run_m8p_flag() == 1){
        m8p_lib_hdmi_hd21_vdrop_set(nport, enable, htotal);
    }else{
        if(enable){
            ...
            hdmi_mask(COMMON_RX_420_LINE_DROP_CTRL_0_reg, ~(COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_valid_delay_mask),
                        COMMON_RX_420_LINE_DROP_CTRL_0_line_drop_valid_delay(htotal/4/2));
