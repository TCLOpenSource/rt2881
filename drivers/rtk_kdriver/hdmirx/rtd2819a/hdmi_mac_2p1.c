...
#define HS_POLARITY_CHECK_DEBOUNCE_THD    2

#define LTPASS_WAIT_FLTSTART_CLR_TIMEOUT_THD   1000
...
{//Alwasy use physical port 0 address
    unsigned int clken_pre_mask = HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre0_mask << nport;
    unsigned int clken_hdcp_pre_mask = HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre0_mask << nport;
    unsigned int rstn_pre_mask = HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre0_mask << nport;
    unsigned int rstn_hdcp_pre_addr = HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre0_mask << nport;

    if(!lib_hdmi_is_hdmi_21_available(nport))
        return;

    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg,~(clken_pre_mask|clken_hdcp_pre_mask), 0x0);
    udelay(5);
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg,~(rstn_pre_mask|rstn_hdcp_pre_addr), 0x0);
}
...
{//Alwasy use physical port 0 address
    ...
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg,~(rstn_pre_mask|rstn_hdcp_pre_addr), (clken_pre_mask|clken_hdcp_pre_mask));
    udelay(5);
    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg,~(clken_pre_mask|clken_hdcp_pre_mask), (clken_pre_mask|clken_hdcp_pre_mask));
}
...
void lib_hdmi_hd21_mac_crt_common_reset(unsigned char nport)
{
    UINT32 hd21_common_clk_mask[HDMI_PORT_TOTAL_NUM] = {
            HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common0_mask,
            HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common1_mask,
            HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common2_mask,
            HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common3_mask};
    UINT32 hd21_common_rst_mask[HDMI_PORT_TOTAL_NUM] = {
            HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common0_mask,
            HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common1_mask,
            HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common2_mask,
            HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common3_mask};
    ...
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return ;
    // clk en disable
    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg, ~(hd21_common_clk_mask[nport]), 0x0);
    udelay(5);
    // reset 0
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg, ~(hd21_common_rst_mask[nport]), 0x0);
    udelay(5);
     // reset 1
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg, ~(hd21_common_rst_mask[nport]), hd21_common_rst_mask[nport]);
    udelay(5);
    // clk en enable
    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg, ~(hd21_common_clk_mask[nport]), hd21_common_clk_mask[nport]);
...
{//Alwasy use physical port 0 address
    // clk en disable
    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg, ~(HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre1_mask), 0x0);
    udelay(5);

    // reset 0
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg, ~(HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre1_mask), 0x0);
}
...
{//Alwasy use physical port 0 address
    ...
    hdmi_mask(HDMI_TOP_COMMON_hdmi_rst3_reg, ~(HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre1_mask),
        (HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_rst3_rstn_hd21_hdcp_pre1_mask));
    udelay(5);
    ...
    hdmi_mask(HDMI_TOP_COMMON_hdmi_clken3_reg, ~(HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre1_mask),
        (HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_common0_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_pre1_mask|
        HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre0_mask|HDMI_TOP_COMMON_hdmi_clken3_clken_hd21_hdcp_pre1_mask));
}
...
void lib_hdmi_hd21_timing_gen_en_vs_align(unsigned char nport,unsigned int delay_ms)
{
    // RL6951-237 : tg en check time to enable
    HDMI_INFO("timing gen vs align, %d ms in one frame, setting=%08x\n", delay_ms, hdmi_in(HD21_TIMING_GEN_CR_reg));

    if(delay_ms < HD21_TG_VS_ALIGN_THD_MS)
    {// 48~144hz need
        hdmi_mask(HD21_TIMING_GEN_CR_reg, ~HD21_TIMING_GEN_CR_tg_en_align_sel_mask, HD21_TIMING_GEN_CR_tg_en_align_sel_mask);
        mdelay(delay_ms);
    }
    else
    {// 24~47hz
        hdmi_mask(HD21_TIMING_GEN_CR_reg, ~HD21_TIMING_GEN_CR_tg_en_align_sel_mask, 0);
    }
}
...
{
    unsigned int addr = HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_reg + nport * 0x1000;
    ...
    case 1: // 1 pixel mode
        ...
        hdmi_mask(addr, ~(HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel_mask),
                HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_pixel_transform_mode_sel(1));
        break;
...
void lib_hdmi_hd21_clr_fec_frl_hs(unsigned char nport)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return;
 ...
 * Retn : N/A
 *--------------------------------------------------*/
void lib_hdmi_hd21_fec_set_hs_polarity(unsigned char nport, unsigned char hs_inv)
{
    if (lib_hdmi_is_hdmi_21_available(nport))
    {
        HDMI_INFO("Port = %d, set_hs_polarity (hs_inv= %d, before h pol=%d)\n",
            nport, hs_inv, lib_hdmi_hd21_meas_get_hs_polarity(nport));

        hdmi_mask(HD21_FEC_FEC_HDMI21_DI_0_reg,
            ~(HD21_FEC_FEC_HDMI21_DI_0_hs_polar_invert_mask),
            HD21_FEC_FEC_HDMI21_DI_0_hs_polar_invert((hs_inv) ? 1 : 0));
    }
}
...
unsigned char lib_hdmi_hd21_fec_get_hs_polar_invert(unsigned char nport)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return 0;

    return HD21_FEC_FEC_HDMI21_DI_0_get_hs_polar_invert(hdmi_in(HD21_FEC_FEC_HDMI21_DI_0_reg));
}
...
void lib_hdmi_hd21_meas_set_frame_pop_mode(unsigned char nport, unsigned char mode)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return;

    hdmi_mask(HD21_MEAS_FRL_MEASURE_A_reg, ~HD21_MEAS_FRL_MEASURE_A_frame_pop_mode_mask, HD21_MEAS_FRL_MEASURE_A_frame_pop_mode(mode?1:0));
}

// 0: 4 frame pop  1: 1 frame pop
unsigned char lib_hdmi_hd21_meas_get_frame_pop_mode(unsigned char nport)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return 0;

    return HD21_MEAS_FRL_MEASURE_A_get_frame_pop_mode(hdmi_in(HD21_MEAS_FRL_MEASURE_A_reg));
}
...
/*---------------------------------------------------
 * Func : lib_hdmi_hd21_meas_get_hs_polarity
 *
 * Desc : get hdcp vs polarity
 *
 * Para : nport : hdmi port id
 ...
 * Retn : 0 : Positive VS, 1 : Negative VS
 *--------------------------------------------------*/
unsigned char lib_hdmi_hd21_meas_get_hs_polarity(unsigned char nport)
{
    if (lib_hdmi_is_hdmi_21_available(nport))
        return HD21_MEAS_FRL_MEASURE_9_get_hs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg));
    return 0;
}

/*---------------------------------------------------
 * Func : lib_hdmi_hd21_meas_get_current_polarity
 *
 * Desc : get v and h polarity
 *
 * Para : nport : hdmi port id
 *
 * Retn : (0:h/v-, 1:h+v-, 2:h-v+, 3:h/v +)
 *--------------------------------------------------*/
unsigned char lib_hdmi_hd21_meas_get_current_polarity(unsigned char nport)
{
    unsigned int current_pol = 0;

    if (lib_hdmi_is_hdmi_21_available(nport))
    {
        if(HD21_MEAS_FRL_MEASURE_9_get_vs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 0)
           current_pol |= _BIT1;

        if(HD21_MEAS_FRL_MEASURE_9_get_hs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 0)
            current_pol |= _BIT0;
...
/*---------------------------------------------------
 * Func : lib_hdmi_hd21_meas_get_original_polarity
 *
 * Desc : get v and h polarity
 *
 * Para : nport : hdmi port id
 ...
 * Retn : (0:h/v-, 1:h+v-, 2:h-v+, 3:h/v +)
 *--------------------------------------------------*/
unsigned char lib_hdmi_hd21_meas_get_original_polarity(unsigned char nport)
{
    unsigned int original_pol = 0;

    if (lib_hdmi_is_hdmi_21_available(nport))
    {
        if(lib_hdmi_hd21_fec_get_vs_polar_invert(nport) == 0)
        {
            if(HD21_MEAS_FRL_MEASURE_9_get_vs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 0)
               original_pol |= _BIT1;
        } else {
            if(HD21_MEAS_FRL_MEASURE_9_get_vs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 1)
               original_pol |= _BIT1;
        }

        if(lib_hdmi_hd21_fec_get_hs_polar_invert(nport) == 0)
        {
            if(HD21_MEAS_FRL_MEASURE_9_get_hs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 0)
                original_pol |= _BIT0;
        } else {
            if(HD21_MEAS_FRL_MEASURE_9_get_hs_polarity(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg)) == 1)
                original_pol |= _BIT0;
        }
...
void lib_hdmi_hd21_set_polarity_meas_mode(unsigned char nport, HD21_MEASURE_POLARITY_MEAS_MODE mode)
{
    hdmi_mask(HD21_MEAS_FRL_MEASURE_9_reg, ~(HDMI21_P0_MEAS_FRL_MEASURE_9_polarity_meas_mask), HDMI21_P0_MEAS_FRL_MEASURE_9_polarity_meas(mode));
}

HD21_MEASURE_POLARITY_MEAS_MODE lib_hdmi_hd21_get_polarity_meas_mode(unsigned char nport)
{
    return HDMI21_P0_MEAS_FRL_MEASURE_9_get_polarity_meas(hdmi_in(HD21_MEAS_FRL_MEASURE_9_reg));
}
...
HDMI21_MEASURE_RESULT lib_hdmi21_hd21_measure(unsigned char nport, unsigned char is_dsc_mode, MEASURE_TIMING_T* update_timing)
{
    ...
    if (GET_H_VIDEO_FSM(nport) == MAIN_FSM_HDMI_DISPLAY_ON && is_fec_measure_complete == 0)
    {
        int index = 0;
        for (index = 0; index < 20 ; index++)
        {
            HDMI_EMG("[HD21MS]is_vsync_get=%d, is_ps_measure_complete=%d,is_fec_measure_complete=%d ,hdmi_rx[nport].video_t.vs_valid=%d,video_t=%d,m_hd21_measure_last_vs_cnt=%d, index=%d\n",
                is_vsync_get, is_ps_measure_complete, is_fec_measure_complete,hdmi_rx[nport].video_t.vs_valid, hdmi_rx[nport].video_t.vs_cnt, m_hd21_measure_last_vs_cnt[nport], index);

            msleep(20);
            is_fec_measure_complete = lib_hdmi_hd21_fec_is_measure_completed(nport);
            if (is_fec_measure_complete ==1)
                break;
        }
    }

    lib_hdmi_hd21_set_polarity_meas_mode(nport, HD21_POLARITY_MANUAL_MEAS);//invert
    ...
    update_timing->h_total = HD21_MEAS_FRL_MEASURE_11_get_htotal_cnt(hdmi_in(HD21_MEAS_FRL_MEASURE_11_reg));
    ...
    update_timing->v_act_len = HD21_MEAS_FRL_MEASURE_16_get_vact_cnt(hdmi_in(HD21_MEAS_FRL_MEASURE_16_reg));
    ...
    update_timing->h_act_sta = update_timing->IHSyncPulseCount + HD21_MEAS_FRL_MEASURE_D_get_hback_cnt(hdmi_in(HD21_MEAS_FRL_MEASURE_D_reg)); // hact_sta = hs_width + h_back
    update_timing->v_act_sta = update_timing->IVSyncPulseCount + HD21_MEAS_FRL_MEASURE_14_get_vback_cnt(hdmi_in(HD21_MEAS_FRL_MEASURE_14_reg)); // vact_sta = vs_width + v_back
...
#if 0 // mac measure after dpc, sw no need to cal
    ...
    if(update_timing->is_interlace){
        if(update_timing->v_total%2){
            HDMI_WARN("[HD21MS] interlace Corrected v total from %d to event num\n", update_timing->v_total);
            update_timing->v_total -= 1;
        }
        if(update_timing->v_act_sta%2){
            HDMI_WARN("[HD21MS] interlace Corrected v star from %d to event num\n", update_timing->v_act_sta);
    ...
    if(GET_H_VIDEO_FSM(nport) < MAIN_FSM_HDMI_VIDEO_READY){
        ...
        HDMI_WARN("[HD21MS] Polarity: %d(0:h/v-, 1:h+v-, 2:h-v+, 3:h/v +)\n", update_timing->polarity);
        HDMI_WARN("[HD21MS] Hstar: %d\n", update_timing->h_act_sta);
        HDMI_WARN("[HD21MS] Vstar: %d\n", update_timing->v_act_sta);
...
void lib_hdmi_hd21_clr_measure_last_vs_cnt(unsigned char nport)
{
    if (!lib_hdmi_is_hdmi_21_available(nport))
        return;

    m_hd21_measure_last_vs_cnt[nport] = 0;
}
...
void newbase_hdmi_hd21_init_fw_vs_polarity(unsigned char port)
{
    ...
    hdmi_rx[port].hdmi_2p1_hs_pol = 0xF;
    hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt = 0;
    lib_hdmi_hd21_fec_set_hs_polarity(port, 0); // reset invert to zero
    lib_hdmi_hd21_fec_set_fw_vs_polarity(port, 1, 0); // reset invert to zero
}

void newbase_hdmi_hd21_qs_init_fw_polarity(unsigned char port)
{
    if (port >= HDMI_PORT_TOTAL_NUM)
        return;
    hdmi_rx[port].hdmi_2p1_fw_pol_check_cnt = FW_POLARITY_CHECK_DEBOUNCE_THD;
    hdmi_rx[port].hdmi_2p1_vs_pol = lib_hdmi_hd21_meas_get_vs_polarity(port);
    hdmi_rx[port].hdmi_2p1_hs_pol = lib_hdmi_hd21_meas_get_hs_polarity(port);
    hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt = HS_POLARITY_CHECK_DEBOUNCE_THD;
}
...
void newbase_hdmi_hd21_link_training_fsm(unsigned char port)
{
    ...
    {
		unsigned int wait_timeout = LTPASS_WAIT_FLTSTART_CLR_TIMEOUT_THD;
        ...
        {//wait FRL start clear, follow HDMI2.1a standard
            msleep(10);
            if(lib_hdmi_scdc_get_frl_start(port) == 1)
            {
    ...
    case LT_FSM_LTSP_CHECK:
    {
        ...
        lib_hdmi_hd21_hs_pol_check(port);
        break;
    }
    default:
        break;
...
void newbase_hdmi_hd21_clk_gen(unsigned char nport, unsigned char dsc_enable, unsigned char pixel_mode)
{
    ...
    if ( dsc_enable)
    {
        hdmi_mask(addr, ~HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask, HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(2));
        newbase_hdmi_hd21_dsc_enanble(nport, 1);
    }
    else
    {
        hdmi_mask(addr, ~HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask, HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(0)); 
    }
...
void lib_hd21ms_timing_check_v(UINT8 port, MEASURE_TIMING_T* update_timing)
{
    ...
    if( update_timing->v_total != hdmi_rx[port].timing_t.v_total){
        HDMI_WARN("[HD21MS]vtotal: offms %d -> hd21ms %d\n", (UINT32)hdmi_rx[port].timing_t.v_total, update_timing->v_total);
        hdmi_rx[port].timing_t.v_total = update_timing->v_total;
    }
    if( update_timing->v_freq != hdmi_rx[port].timing_t.v_freq){
        HDMI_WARN("[HD21MS]vfreq: offms %d -> hd21ms %d\n", (UINT32)hdmi_rx[port].timing_t.v_freq, (UINT32)update_timing->v_freq);
        hdmi_rx[port].timing_t.v_freq = update_timing->v_freq;
    }
}


void lib_hd21_measure(unsigned char port, unsigned char is_dsc_mode, MEASURE_TIMING_T* update_timing)
{
    HDMI21_MEASURE_RESULT result = HDMI21_MEASURE_SUCCESS;
    result = lib_hdmi21_hd21_measure(port, is_dsc_mode, update_timing);
    if(result == HDMI21_MEASURE_SUCCESS){
        lib_hd21ms_timing_check_h(port, update_timing);
        lib_hd21ms_timing_check_v(port, update_timing);
        if( update_timing->polarity != hdmi_rx[port].timing_t.polarity){
            HDMI_WARN("[HD21MS]polarity: offms %d -> hd21ms %d\n", hdmi_rx[port].timing_t.polarity, update_timing->polarity);
            hdmi_rx[port].timing_t.polarity = update_timing->polarity;// hdmi2.1 offms can't get neg pol
        }
    }else{
        HDMI_WARN("[HD21MS]hd21 measure failed result = %d\n", result);
    }
}

void lib_hdmi_hd21_hs_pol_check(unsigned char port)
{
    unsigned char hs_pol = 0xF;
    if(hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt < HS_POLARITY_CHECK_DEBOUNCE_THD){
        unsigned char frl_hs = lib_hdmi_hd21_get_fec_frl_hs(port);
        unsigned char v_pkt_det = lib_hdmi_hd21_get_fec_v_pkt_det(port);
        if ((frl_hs == TRUE) && (v_pkt_det == TRUE)) {
            lib_hdmi_hd21_clr_fec_frl_hs(port);
            lib_hdmi_hd21_clr_v_pkt_det(port);
        }
        else {
            return;
        }
        hs_pol = lib_hdmi_hd21_meas_get_hs_polarity(port);

        if (hdmi_rx[port].hdmi_2p1_hs_pol != hs_pol && hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt) {
            hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt = 0;
            HDMI_INFO("HS Polarity unstableStable (Polarity=%d->%d, Cnt=%d, is_invert=%d)\n",
                hdmi_rx[port].hdmi_2p1_hs_pol, hs_pol,
                hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt,
                lib_hdmi_hd21_fec_get_hs_polar_invert(port));
        }
        else {
            HDMI_INFO("HS Polarity Stable (Polarity=%d, Cnt=%d, is_invert=%d)\n",
                hs_pol,
                hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt,
                lib_hdmi_hd21_fec_get_hs_polar_invert(port));
            hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt++;
        ...
        if (hdmi_rx[port].hdmi_2p1_hs_pol_check_cnt == HS_POLARITY_CHECK_DEBOUNCE_THD) {
            HDMI_INFO("HS Polarity Stable (Polarity=%d)\n", hdmi_rx[port].hdmi_2p1_hs_pol);
            lib_hdmi_hd21_fec_set_hs_polarity(port, hdmi_rx[port].hdmi_2p1_hs_pol);
        }
    }
}

void newbase_hdmi_hd21_stb_dpm_init(unsigned char nport)
{
    lib_hdmi_scdc_set_flt_ready(nport);
    lib_hdmi_scdc_clr_frl_start(nport);
}

unsigned char lib_hdmi_hd21_is_irq_pending(unsigned char nport)
{
    if(!lib_hdmi_is_hdmi_21_available(nport))
        return 0;
...
void lib_hdmi_hd21_clr_irq_pending(unsigned char nport)
{
    if(!lib_hdmi_is_hdmi_21_available(nport))
        return;
...
void lib_hdmi_hd21_vdrop_set(unsigned char nport, unsigned char enable, unsigned int htotal)
{
    ;// not support
}
