...
void hdcp_swap_ll(unsigned long long * in)
{
	u8 *ptr = (uint8_t*)in;
	u8 tmp;
	u8 i = 0;
...
u32 hdcp_do_shuffle(u32 *sh, u32 in, u32 sel)
{
	int A = GETBIT(*sh, 1);
	int B = GETBIT(*sh, 0);
	if (sel == 0) {
...
u32 hdcp_do_lfsr(LFSR* lfsr_handle)
{
	...
	//hdcp_do_lfsr0(&lfsr_handle->lfsr0);
	//hdcp_do_lfsr1(&lfsr_handle->lfsr1);
	//hdcp_do_lfsr2(&lfsr_handle->lfsr2);
	//hdcp_do_lfsr3(&lfsr_handle->lfsr3);
	...
	sh_out = hdcp_do_shuffle(&lfsr_handle->sh0, sh_in, LFSR_TAP[0][1]);
	sh_out = hdcp_do_shuffle(&lfsr_handle->sh1, sh_out, LFSR_TAP[1][1]);
	sh_out = hdcp_do_shuffle(&lfsr_handle->sh2, sh_out, LFSR_TAP[2][1]);
	sh_out = hdcp_do_shuffle(&lfsr_handle->sh3, sh_out, LFSR_TAP[3][1]);
