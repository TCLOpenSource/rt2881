...
*   
*/
void hdcp14_dump_char_array(char *name, unsigned char *data, unsigned int len)
{
	int i = 0;
	int buf_len = len * 6;
...
bool hdcp_14_compute_km(u8 *keys, u8 *ksv, u8 *km)
{
	u64 km_value = 0;
	int i = 0;
	int j = 0;
...
{
	SHA1Context sha1ctx;
	hdcp_SHA1Reset(&sha1ctx);
	hdcp_SHA1Input(&sha1ctx, input, len);
	hdcp_SHA1Result(&sha1ctx, hash);
	return true;
}

bool hdcp_14_compute_v(u8 *ksv_list, u32 list_len, 
				u8 *bstatus, u8 *m0, u8 *v)
{
	...
	hdcp14_dump_char_array("[HDCP]hdcp_1_4_compute_v sha1 input", msg, index);
	
	ret = do_sha1(msg, msg_len, v);
	hdcp14_dump_char_array("[HDCP] v", v, 20);
	kfree(msg);
	return ret;
}


bool hdcp_14_block_cipher(u8 *km, u8 repeater, u8 *an, 
						u8 *ks, u8 *m, u8 *r)
{
	int i = 0;
	LFSR* lfsr_ctx = NULL; 
	...
	for(i = 0; i < 48; i++) {
		...
		hdcp_14_round_function_b(bx, by, bz, ky, &bxo, &byo, &bzo);
		hdcp_14_round_function_k(kx, ky, kz, &kxo, &kyo, &kzo);
		bx = bxo;
		by = byo;
		bz = bzo;
		kx = kxo;
	...
	//hdcp14_dump_char_array("ks", ks, 7);
	...
	lfsr_ctx = hdcp_lfsr_init(ks);
	if(!lfsr_ctx)
		return false;
	//lfsr_print(lfsr_ctx);
	for(i  = 0; i < 56; i++) {
		u32 lfsr_v = hdcp_do_lfsr(lfsr_ctx);
		u32 output = 0;	
		output = hdcp_14_output_function(ky, kz, by, bz);	
	...
	hdcp_lfsr_uninit(lfsr_ctx);
	//hdcp14_dump_char_array("m", m, 8);
	//hdcp14_dump_char_array("m", r, 2);	
	return true;				
}
