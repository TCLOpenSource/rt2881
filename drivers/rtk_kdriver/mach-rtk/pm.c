...
static int notrace rtk_enter_suspend(unsigned long param)
{
...
#if !(defined(CONFIG_RTK_KDRV_OPENBSD) || IS_ENABLED(CONFIG_RTK_KDRV_TEE))
	...
	while(1)
	{
		...
		{
			//rtd_pr_mach_rtk_err(KERN_ERR "KCPU STR flow not finish\n");
			if(count++ > 200) // wait 2s
				return 0;
		}
		else
		{
			rtd_pr_mach_rtk_err( "KCPU STR resume point(%x)\n", kcpu_resume_address);
			break;
		}
		mdelay(10);
	}
#endif
#endif
	rtd_pr_mach_rtk_err("<< %s >>: Ready to load 8051 (%lu)...\n", __FUNCTION__, param);
	rtk_pm_load_8051(param);

#if IS_ENABLED(CONFIG_RTK_KDRV_TEE)
//        asm volatile ("1: b 1b \n");  //debug of EMCU
	rtd_pr_mach_rtk_err("<< %s >>: platform_cpu_die() have CPU itself died. (%lu)...\n", __FUNCTION__, param);
        //platform_cpu_die(0);  //cpu 0, will call level-2 suspend to secure world.
	if (psci_ops.cpu_suspend){
		psci_ops.cpu_suspend(PSCI_SUSPEND_PARAM,virt_to_phys(cpu_resume));
	}
#else
...
//reload the openBSD version kcpu fw
        rtd_pr_mach_rtk_notice( "run suspend reload openBSD flow\n");
	...
	// do nothing. bootcode will (and should) start OPTEE from original physical address
		...
		*((volatile int *)phys_to_virt(SCPU_STR_STATUS_FLAG_ADDR)) = SCPU_STR_STATUS_SUSPEND_BEGIN;
		...
		rtd_pr_mach_rtk_err("rtk_pm_begin: state:%d, fixit!?  \n", state);
		...
		//while(1); //while loop has empty body Wempty-body
		asm ("1: b 1b \n");
#endif
            break;
        default:
...
static int rtk_pm_enter(suspend_state_t state)
{
    ...
    switch (state)
    {
		...
		if (!cpu_suspend(SUSPEND_RAM, rtk_enter_suspend)) {
			...
			rtd_pr_mach_rtk_err("<< %s >>: Error in cpu_suspend...\n", __FUNCTION__);
		//rtd_pr_mach_rtk_err("<< %s >>: Ready to resume from suspend 1...\n", __FUNCTION__);
#endif // CONFIG_RTK_KDRV_EMCU
		return 0;
        case PM_SUSPEND_ON:
		rtd_pr_mach_rtk_err("<< %s >>: FOUND PM_SUSPEND_ON, system will shutdown....\n", __FUNCTION__);
#ifdef CONFIG_RTK_KDRV_EMCU
		if (!cpu_suspend(SUSPEND_WAKEUP, rtk_enter_suspend)) {
			...
			rtd_pr_mach_rtk_err("<< %s >>: Error in cpu_suspend...\n", __FUNCTION__);
		rtd_pr_mach_rtk_err("<< %s >>: Ready to resume from suspend 2...\n", __FUNCTION__);
#endif // CONFIG_RTK_KDRV_EMCU
		return 0;
        case PM_SUSPEND_STANDBY:
        break;
...
static void rtk_pm_end(void)
{
	...
	if(rtd_inl(STB_WDOG_DATA6_reg) != 0xcafecafe)
		...
		rtd_pr_mach_rtk_notice( "KCPU STR resume finish\n");
#endif
#endif
#ifdef CONFIG_RTK_KDRV_EMCU
	...
	if((wakeup_src != WKSOR_UNDEF) && ((wakeup_src != WKSOR_RTC) && (wakeup_src != WKSOR_WIFI) && (wakeup_src != WKSOR_WOW)))
		...
		rtd_pr_mach_rtk_err("No send power key(%x)\n", wakeup_src);
#endif
}
...
static void rtk_pm_recover(void)
{
	...
	rtd_pr_mach_rtk_notice( "Before notify KCPU STR resume under platform recover stage(%x)\n", rtd_inl(STB_WDOG_DATA6_reg));
	...
	while(1)
	{
		...
		{
			rtd_pr_mach_rtk_err( "Get KCPU STR resume point(%x), then start to notify KCPU resume\n", kcpu_resume_address);
...
static int rtk_hibernation_begin(pm_message_t stage)
#else
...
#endif
{
	...
	rtd_pr_mach_rtk_notice( "Notify KCPU STD suspend flow\n");
	rtd_outl(STB_WDOG_DATA6_reg, 0x23792379);
#endif
	return 0;
}
...
static void rtk_hibernation_end(void)
{
	...
	if(rtd_inl(STB_WDOG_DATA6_reg) != 0xcafecafe)
		...
		rtd_pr_mach_rtk_notice( "KCPU STD resume finish\n");
#endif
}
...
static int rtk_hibernation_pre_snapshot(void)
{
	...
	else
	{
		...
		*((volatile int *)phys_to_virt(KCPU_STD_RESUME_ADDR)) = kcpu_resume_address;
		rtd_pr_mach_rtk_notice( "KCPU STD resume point(%x)\n", kcpu_resume_address);
	}
#endif
	return ret;
}
...
static void rtk_hibernation_finish(void)
{
	...
	rtd_pr_mach_rtk_notice( "Notify KCPU STD make image resume\n");
	rtd_outl(STB_WDOG_DATA6_reg, 0x23992399);
#endif
...
{
    rtd_pr_mach_rtk_err("Realtek Power Management 2015/08/05 15:00\n");
