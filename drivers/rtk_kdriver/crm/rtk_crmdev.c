...
typedef enum _resource_client_state {
  ...
  RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE,
  RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE_NOTIFIED,
...
struct _resource_client_t {
  ...
  /* record acquire codec info */
  int32_t codec_info_count;
  rtk_crmdev_codec_info_t codec_info[MAX_RECORDS_COUNT];
  ...
  /* monitor all resource change happen */
  bool monitor_resource_change;
  rtk_crmdev_resource_change_t resource_change;
};
...
static const char *_codec_type_to_name(int32_t codec_type) {
  switch (codec_type) {
    ...
    case RTK_CRMDEV_CODEC_TYPE_VSC_WIN1:
      return "vsc_win1";
    case RTK_CRMDEV_CODEC_TYPE_VT_CAPTURE:
      return "vt";
    case RTK_CRMDEV_CODEC_TYPE_VT_CAMERA:
      return "vt_camera";
    case RTK_CRMDEV_CODEC_TYPE_VT_ACR:
      return "vt_acr";
    case RTK_CRMDEV_CODEC_TYPE_VT_SCREENCAPTURE:
      return "vt_screencapture";
    case RTK_CRMDEV_CODEC_TYPE_VT_MEDIA:
      return "vt_media";
    default:
      return "none";
  }
...
static inline bool _codec_info_is_vt(const rtk_crmdev_codec_info_t *info) {
    return ((info->codec & RTK_CRMDEV_CODEC_TYPE_VT_CAPTURE) ? true : false);
}
...
static const char *_codec_info_to_string(const rtk_crmdev_codec_info_t *info) {
  ...
  if (_codec_info_is_video(info)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             ...
             _codec_info_is_display(info) ||
             _codec_info_is_vsc(info)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "codec-info: %s %s", codec_name, encoder);
  } else if (_codec_info_is_vt(info)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "codec-info: %s", codec_name);
  } else {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "codec-info: unknown");
  }
...
static void _codec_resource_set_from_providers(
  ...
  for (i = 0; i < provider_count; i++) {
    ...
    } else if (!strncmp(provider->name, "adp2", sizeof(provider->name))) {
      resource->audio_port = RTK_CRMDEV_AUDIO_PORT_2;
    } else if (!strncmp(provider->name, "adp3", sizeof(provider->name))) {
      resource->audio_port = RTK_CRMDEV_AUDIO_PORT_3;
...
static void _codec_resource_get_providers(
  ...
  for (i = 0; i < _device_table->provider_count; i++) {
    ...
    if (all_client  || _resource_provider_contain_unique_id(provider, unique_id, NULL, NULL)) {
      ...
      } else if (_resource_provider_is_name(provider, "adp2")) {
        if (resource->audio_port & RTK_CRMDEV_AUDIO_PORT_2) {
          providers[(*provider_count)++] = provider;
        }
      } else if (_resource_provider_is_name(provider, "adp3")) {
        if (resource->audio_port & RTK_CRMDEV_AUDIO_PORT_3) {
          providers[(*provider_count)++] = provider;
        }
...
static const char *_codec_resource_to_string(
  ...
  snprintf(
      ...
      resource->max_width, resource->max_height, resource->compress);
  return _device_table->buffer;
}
...
static inline const char *_resource_client_get_state_name(
  ...
  } else if (test_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE,
                      &client->state)) {
    return "resource-change";
  } else if (test_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE_NOTIFIED,
                      &client->state)) {
    return "resource-change_notified";
  } else {
    return "unknown";
  }
}
...
static bool _resource_client_create(struct file *filp) {
  ...
  if (client) {
    ...
    client->monitor_resource_change = 0;
    client->codec_info_count = 0;
    memset(client->codec_info, 0, MAX_RECORDS_COUNT * sizeof(rtk_crmdev_codec_info_t));
...
static void _resource_client_notify_resource_change(resource_client_t *client,
                                                    rtk_crmdev_codec_info_t* info, bool acquired) {
  int ret = 0, i = 0;
  resource_client_t *curr = NULL;
  rtk_crmdev_resource_change_t* data = NULL;

  // notify all monitor which monitor_resource_change = 1
  list_for_each_entry(curr, &_client_list, list) {
    if(curr->monitor_resource_change == 0)
      continue;

    data = &curr->resource_change;
    memset(data, 0, sizeof(rtk_crmdev_resource_change_t));

    if(acquired == false) {
      for(i = 0; i < client->codec_info_count; i++) {
        data->resList[data->count].acquired = 0;
        strncpy(data->resList[data->count].clientName,
          client->name, sizeof(client->name));
        memcpy(&data->resList[data->count].info,
          &client->codec_info[i], sizeof(rtk_crmdev_codec_info_t));
        data->count++;
      }
    }else {
      data->resList[data->count].acquired = 1;
      strncpy(data->resList[data->count].clientName,
        client->name, sizeof(client->name));
      memcpy(&data->resList[data->count].info, info, sizeof(rtk_crmdev_codec_info_t));
      data->count++;
    }

    _resource_client_wake_event(curr, RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE);

    ret = wait_event_freezable(
        curr->event_wait,
            test_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE_NOTIFIED, &curr->state));

    test_and_clear_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE_NOTIFIED,
                             &curr->state);
  }
...
static int _resource_provider_acquire_priority_compare(const void *lhs,
  ...
  if (lsize == 0 && rsize != 0) {
    ...
    return -1;
  } else if (strcmp(l->name, r->name) < 0) {
    return -1;
  } else if (strcmp(l->name, r->name) > 0) {
    return 1;
  } else {
...
static inline bool _resource_format_is_vt(const resource_format_t *format) {
    return ((format->codec & RTK_CRMDEV_CODEC_TYPE_VT_CAPTURE) ? true : false);
}
...
static const char *_resource_format_to_string(const resource_format_t *format) {
  if (_resource_format_is_video(format)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             ...
             _resource_format_is_display(format) ||
             _resource_format_is_vsc(format)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "%s (%04x) %s", _codec_type_to_name(format->codec), format->codec,
             format->encoder ? "encoder" : "decoder");
  } else if (_resource_format_is_vt(format)) {
    snprintf(_device_table->buffer, sizeof(_device_table->buffer),
             "%s (%04x)", _codec_type_to_name(format->codec), format->codec);
  } else {
...
static bool _resource_format_acquire(resource_format_t *format,
  ...
  for (i = 0; i < provider_count; i++) {
      ...
      if (!strncmp(provider->name, "vbm", sizeof(provider->name)) &&
          ...
          all_vbm_used = true;
      } else if (!strncmp(provider->name, "compress", sizeof(provider->name))) {
          compress = true;
      }
  }
...
/* RHAL_CRM_STATUS RHAL_CRM_MonitorResourceChange(RHAL_CRM_MONITOR pHandle,
    RHAL_CRM_RESOURCE_CHANGE_CALLBACK pfnResChangeCallback);*/
static void _resource_service_monitor_resource_change_lock_table(
    resource_client_t *client, rtk_crmdev_request *req) {
  resource_client_t *curr = NULL;
  rtk_crmdev_resource_change_t *data = &req->data.resource_change;
  client->monitor_resource_change = 1;

  list_for_each_entry(curr, &_client_list, list) {
    size_t i;
    for(i = 0; i < curr->codec_info_count; i++) {
      data->resList[data->count].acquired = 1;
      strncpy(data->resList[data->count].clientName,
        curr->name,sizeof(curr->name));
      memcpy(&data->resList[data->count].info,
        &curr->codec_info[i], sizeof(rtk_crmdev_codec_info_t));
      data->count++;
    }
  }
...
static void _resource_service_release_lock_table(resource_client_t *client,
  ...
  // clear all client recorded codec info
  if(client->codec_info_count) {
    _resource_client_notify_resource_change(client, NULL, false);
    client->codec_info_count = 0;
    memset(client->codec_info, 0,
        MAX_RECORDS_COUNT * sizeof(rtk_crmdev_codec_info_t));
  }
...
static int _resource_service__acquire_lock_acquire(resource_client_t *client,
  ...
  // record codec info to client struct
  if(req->status == RTK_CRMDEV_STATUS_OK) {
    memcpy(&client->codec_info[client->codec_info_count],
        &req->data.acquire_resource.info, sizeof(rtk_crmdev_codec_info_t));
    client->codec_info_count++;

    _resource_client_notify_resource_change(client, &req->data.acquire_resource.info, true);
  }
  mutex_unlock(&_table_lock);
  return ret;
}
...
static long _rtk_crmmgr_ioctl_wait_event(resource_client_t *client,
  ...
  ret = wait_event_freezable(
      ...
      test_bit(RESOURCE_CLIENT_STATUS_CANCEL, &client->state) ||
          ...
          test_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE, &client->state));
  if (ret) {
    if (signal_pending(current)) {
  ...
  } else if (test_and_clear_bit(RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE,
                                &client->state)) {
    req->status = RTK_CRMDEV_STATUS_RESOURCE_CHANGE;
    req->data.resource_change = client->resource_change;
  } else {
    BUG();
  }
...
static long _rtk_crmmgr_ioctl_monitor_resource_change(resource_client_t *client,
                                                      rtk_crmdev_request *req){
  rtd_pr_crm_err("[crmmgr] monitor resource change  : %s\n", _resource_client_get_name(client));

  mutex_lock(&_table_lock);
  _resource_service_monitor_resource_change_lock_table(client, req);
  mutex_unlock(&_table_lock);
...
static long _rtk_crmmgr_ioctl_acquire_resource(resource_client_t *client,
  ...
  if (0 == mutex_trylock(&_acquire_lock)) {
    rtd_pr_crm_err("[crmmgr] acquire  : %s lock fail\n", _resource_client_get_name(client));
    req->status = RTK_CRMDEV_STATUS_BUSY;
    return 0;
  }
...
static long _rtk_crmmgr_ioctl_response_resource_change(resource_client_t *client,
                                                rtk_crmdev_request *req) {
  rtd_pr_crm_debug("[crmmgr] events   : %s response resource change notify done\n",
          _resource_client_get_name(client));
  _resource_client_wake_event(client,
                              RESOURCE_CLIENT_STATUS_RESOURCE_CHANGE_NOTIFIED);
  req->status = RTK_CRMDEV_STATUS_OK;
...
static long _rtk_crmmgr_unlocked_ioctl(struct file *filp, unsigned int cmd,
  ...
  long ret = 0;
  resource_client_t *client = (resource_client_t *)filp->private_data;
  rtk_crmdev_request* req = kmalloc(sizeof(rtk_crmdev_request), GFP_KERNEL);;
  if(req == NULL) {
    rtd_pr_crm_err("[crmmgr] ioctl    : kmalloc rtk_crmdev_request failed\n");
    kfree(req);
    return -EFAULT;
  }

  if (cmd == RTK_CRMMGR_IOC_REQUEST) {
    if (copy_from_user(req, (void __user *)arg, sizeof(rtk_crmdev_request))) {
      kfree(req);
      return -EFAULT;
    }
    switch (req->cmd) {
      ...
      case RTK_CRMDEV_CMD_CLEAR_RESOURCE:
        ...
        break;
      case RTK_CRMDEV_CMD_MONITOR_RESOURCE_CHANGE:
        ret = _rtk_crmmgr_ioctl_monitor_resource_change(client, req);
        break;
      ...
      case RTK_CRMDEV_CMD_RESPONSE_PRIORITY:
        ...
        break;
      case RTK_CRMDEV_CMD_RESPONSE_RESOURCE_CHANGE:
        ret = _rtk_crmmgr_ioctl_response_resource_change(client, req);
        break;
      default:
    ...
    if (!ret) {
      if (copy_to_user((void __user *)arg, req, sizeof(rtk_crmdev_request))) {
        kfree(req);
        return -EFAULT;
      }
    } else {
