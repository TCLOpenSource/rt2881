...
void lib_offms_set_de_offset_mode(unsigned char  en)
{
#ifdef OFFMS_SP_MS3Ctrl_off_de_offset_mode_mask
    rtd_maskl(OFFMS_SP_MS3Ctrl_reg, ~(OFFMS_SP_MS3Ctrl_off_de_offset_mode_mask),  OFFMS_SP_MS3Ctrl_off_de_offset_mode(en?1:0)); //de offset mode
#endif
}

void lib_offms_set_pixel_mode(OFFMS_PIXEL_MODE_SEL pixel_mode)
{
#ifdef OFFMS_SP_MS3Ctrl_pixel_mode_mask
    rtd_maskl(OFFMS_SP_MS3Ctrl_reg, ~(OFFMS_SP_MS3Ctrl_pixel_mode_mask),  OFFMS_SP_MS3Ctrl_pixel_mode(pixel_mode));
#endif
}

OFFMS_PIXEL_MODE_SEL lib_offms_get_pixel_mode(void)
{
#ifdef OFFMS_SP_MS3Ctrl_pixel_mode_mask
    return OFFMS_SP_MS3Ctrl_get_pixel_mode(rtd_inl(OFFMS_SP_MS3Ctrl_reg));
#else
    return OFFLINE_MS_1_PIXEL_MODE;
#endif
}
...
unsigned char lib_offline_measure_ex(OFFMS_SOURCE_SEL sel, MEASURE_TIMING_T *tm, unsigned char frl_mode, OFFMS_MSMODE ms_mode)
{
    ...
    unsigned char  htotal_f = 0;
    unsigned char  hsync_f = 0;
    ...
    if(sel == OFFMS_SOURCE_HDMI21 || sel == OFFMS_SOURCE_DSCD){
        lib_offms_set_de_offset_mode(1);
        lib_offms_set_pixel_mode(OFFLINE_MS_4_PIXEL_MODE);
    }else{
        lib_offms_set_de_offset_mode(0);
        lib_offms_set_pixel_mode(OFFLINE_MS_1_PIXEL_MODE);
    }
    ...
    tm->v_act_len = OFFMS_MES_RESULT_VSTA_END_get_off_vx_end(rtd_inl(OFFMS_MES_RESULT_VSTA_END_reg)) - tm->v_act_sta;
    tm->h_act_len = OFFMS_MES_RESULT_HSTA_END_get_off_hx_end(rtd_inl(OFFMS_MES_RESULT_HSTA_END_reg)) - tm->h_act_sta;

    tm->h_total = OFFMS_SP_MS3Rst1_get_off_hs_period_out_h12b(rtd_inl(OFFMS_SP_MS3Rst1_reg));
    tm->v_total = OFFMS_SP_MS3Rst0_get_off_vs_period_out(rtd_inl(OFFMS_SP_MS3Rst0_reg));
    htotal_f = OFFMS_SP_MS3Rst1_get_off_hs_period_out_f2b(rtd_inl(OFFMS_SP_MS3Rst1_reg));
    hsync_f = OFFMS_SP_MS3Rst2_get_off_hs_high_out_f2b(rtd_inl(OFFMS_SP_MS3Rst2_reg));
    raw_h_sync_width = OFFMS_SP_MS3Rst2_get_off_hs_high_out(rtd_inl(OFFMS_SP_MS3Rst2_reg));
    raw_v_sync_width = OFFMS_SP_MS3Rst2_get_off_vs_high_out(rtd_inl(OFFMS_SP_MS3Rst2_reg));

    if(0) // for debug
    {
        unsigned char i = 0;
        unsigned int base_addr = OFFMS_SP_MS3Ctrl_reg;
        for(i = 0; i < 8; i++){
            MEASURE_WARN("[OFMS] 0x%x =  0x%x\n",base_addr, rtd_inl(base_addr));
            base_addr += 4;
        }
    }
#else
    ...
    case OFFMS_SOURCE_DSCD:
        ...
        if (frl_mode != MODE_TMDS && lib_offms_get_pixel_mode() == OFFLINE_MS_1_PIXEL_MODE) {
            //H5X 4 pixel mode
            MEASURE_INFO("[OFMS]H5X 4 Pixel mode!Hx4\n");
            tm->h_act_sta <<= 2;
            tm->h_total <<= 2;
            tm->h_act_len <<= 2;
            tm->IHSyncPulseCount <<= 2;
            htotal_f <<= 2;
            hsync_f <<= 2;
            if (tm->pixel_repeat > 0)
            {
                ...
                tm->h_total /= (tm->pixel_repeat+1);
                tm->h_act_len /= (tm->pixel_repeat+1);
                tm->IHSyncPulseCount /= (tm->pixel_repeat+1);
                htotal_f /= (tm->pixel_repeat+1);
                hsync_f /= (tm->pixel_repeat+1);
            }
        }
        ...
        if(COLOR_YUV420 == tm->colorspace)
        {
            ...
            else if (tm->pll_pixelclockx1024 > (tm->single_p_mode_thd << 9)) // max pll freq/2*1024 from g_panel_vfreq_hd21_pll_mapping_table
            {//For TMDS YUV420
                MEASURE_WARN("HDMI2.0 TMDS YUV420 over THD, Hx2!,  pll_pixelclockx1024=%ld > %d\n",  tm->pll_pixelclockx1024, tm->single_p_mode_thd << 9);
                tm->h_act_sta <<= 1;
                tm->h_total <<= 1;
                tm->h_act_len <<= 1;
                tm->IHSyncPulseCount <<= 1;
    ...
    if(tm->is_interlace){
        if(tm->v_total%2){
            MEASURE_WARN("[OFMS]interlace Corrected v total from %d to event num\n", tm->v_total);
            tm->v_total -= 1;
        }
        if(tm->v_act_sta%2){
            MEASURE_WARN("[OFMS]interlace Corrected v star from %d to event num\n", tm->v_act_sta);
            tm->v_act_sta -= 1;
        }
    }

    lib_measure_active_correction(&(tm->h_act_len), &(tm->v_act_len));
    if(lib_offms_get_pixel_mode() == OFFLINE_MS_1_PIXEL_MODE){
        htotal_f /= 4;
        hsync_f /= 4;
    }
    tm->h_total = tm->h_total + htotal_f;
    tm->IHSyncPulseCount = tm->IHSyncPulseCount + hsync_f;
...
unsigned char lib_offline_measure_demode(OFFMS_SOURCE_SEL sel, MEASURE_TIMING_T *tm, unsigned char frl_mode)
{
    ...
    // for 32bit
    pix_clk = (unsigned long long)tm->h_total * tm->v_act_len *27*1000000;
    do_div(pix_clk, (unsigned long long)(de_vs_end - tm->v_act_sta));
    vfreq = lib_measure_calc_vfreq(lib_measure_get_vs_period());
    if(vfreq != 0 && tm->h_total != 0){
        do_div(pix_clk, (unsigned long long)tm->h_total);
        tm->v_total = (unsigned int)pix_clk*1000/vfreq;
    }


    // vstar = (reg_vstar*pix_clk/27)/htotal
    // eg: 4k2k120  pix_clk=1188Mhz
    if(tm->h_total != 0){
        temp_val = (unsigned long long)tm->v_act_sta*pix_clk;
        do_div(temp_val, 27000000);
        tm->v_act_sta = (unsigned int)temp_val/tm->h_total;
    }
...
unsigned char lib_offline_measure_native(OFFMS_SOURCE_SEL sel, MEASURE_TIMING_T *tm, unsigned char frl_mode)
{
    ...
    tm->h_total = tm->h_total + htotal_f/4;
    tm->IHSyncPulseCount = tm->IHSyncPulseCount + hsync_f/4;
#else
    htotal_f = 0; //no use
    hsync_f = 0;//no use
    tm->v_act_len = OFFMS_MES_RESULT_VSTA_END_get_off_vx_end(rtd_inl(OFFMS_MES_RESULT_VSTA_END_reg)) - tm->v_act_sta + 1;
...
void lib_hdmi_dp_measure_common_port_setting(unsigned char port, OFFMS_SOURCE_SEL sel, unsigned char frl_mode)
{

#if defined(CONFIG_ARCH_RTK2819A)
    unsigned int addr = HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_reg + port * 0x1000;

    rtd_maskl(HDMI_DP_COMMON_MISC_COMMON_PORT_SEL_reg, ~(HDMI_DP_COMMON_MISC_COMMON_PORT_SEL_common_offms_sel_mask),
        HDMI_DP_COMMON_MISC_COMMON_PORT_SEL_common_offms_sel(port));


    MEASURE_WARN("[lib_hdmi_dp_measure_common_port_setting] port:%d, offms out sel = %d\n", port, sel);

    rtd_maskl(addr, ~(HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_offms_out_sel_mask), HDMI_DP_COMMON_VIDEO_AND_OFFMS_PATH_SEL_offms_out_sel(sel));
    switch(sel)
    {
    case OFFMS_SOURCE_HDMI:
    case OFFMS_SOURCE_HDMI21:
        rtd_maskl(HDMI_DP_COMMON_MISC_OFFMS_CTS_FIFO_CTRL_reg, ~(HDMI_DP_COMMON_MISC_OFFMS_CTS_FIFO_CTRL_offms_en_bypass_mask), HDMI_DP_COMMON_MISC_OFFMS_CTS_FIFO_CTRL_offms_en_bypass(0));
        break;
    case OFFMS_SOURCE_DPRX:
        //ms_src_clock_path = MEASURE_CLOCK_PATH_DP;
        break;
    default:
        MEASURE_WARN("[lib_hdmi_dp_measure_common_port_setting] ERROR, undefined input measure src=%d,",sel);
        break;
    }
    ...
    case OFFMS_SOURCE_HDMI21:
        ...
        rtd_maskl(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask), COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(0)); // 0:hdmi
        break;
    case OFFMS_SOURCE_DPRX:
        //ms_src_clock_path = MEASURE_CLOCK_PATH_DP;
        break;
    case OFFMS_SOURCE_DSCD:
        rtd_maskl(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_reg, ~(COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel_mask), COMMON_RX_420_VIDEO_AND_OFFMS_PATH_SEL_yuv420_in_sel(2)); // 2:hdmi + dscd
        break;
    default:
        MEASURE_WARN("[lib_hdmi_dp_measure_common_port_setting] ERROR, undefined input measure src=%d,",sel);
        break;
...
#else
    // do nothing if no 420_common ip
    port = 0;//no use
    sel = 0;//no use
...
unsigned char newbase_hdmi_dp_measure_video_ex(OFFMS_SOURCE_SEL sel, unsigned char port, MEASURE_TIMING_T *tm, unsigned char frl_mode,unsigned char is_HV_Mode)
{
    ...
    lib_hdmi_dp_measure_common_port_setting(port, sel, frl_mode);
    ...
    if (!ms_done) { return MEASURE_FAIL_OFMS_NOT_FINISH; }
    if (tm->h_act_len > tm->h_total) { return MEASURE_FAIL_HACT_OVER_HTOTAL; }
    if (tm->v_act_len > tm->v_total) { return MEASURE_FAIL_VACT_OVER_VTOTAL; }
    if (tm->h_act_len < MEASURE_HACT_MIN) { return MEASURE_FAIL_HACT_UNDER_MIN; }
    if (tm->v_act_len < MEASURE_VACT_MIN) { return MEASURE_FAIL_VACT_UNDER_MIN; }
#if 0// hdmrx
    ...
    if (!ms_done) { return MEASURE_FAIL_OFMS_NOT_FINISH; }
    if(tm->run_vrr == 0){
        if (MEASURE_ABS(tm->h_total, tm_cmp.h_total) > MEASURE_HTOTAL_DIFF_THD) { return MEASURE_FAIL_HTOTAL_DIFF; }
        if (MEASURE_ABS(tm->v_total, tm_cmp.v_total) > MEASURE_VTOTAL_DIFF_THD) { return MEASURE_FAIL_VTOTAL_DIFF; }
        if (tm->h_act_len != tm_cmp.h_act_len) { return MEASURE_FAIL_HACT_DIFF; }
        if (tm->v_act_len != tm_cmp.v_act_len) { return MEASURE_FAIL_VACT_DIFF; }
        if (MEASURE_ABS(tm->h_freq, tm_cmp.h_freq) > MEASURE_HFREQ_DIFF_THD) { return MEASURE_FAIL_HFREQ_DIFF; }
        if (MEASURE_ABS(tm->v_freq, tm_cmp.v_freq) > MEASURE_VFREQ_DIFF_THD) { return MEASURE_FAIL_VFREQ_DIFF; }
    }
    return MEASURE_SUCCESS_NORMAL_BY_HV;
}
...
void newbase_measure_init(void)
{
    ...
    lib_measure_offms_crt_reset();  // crt reset 
}
...
unsigned char lib_measure_offms_start(void)
{
    ...
    unsigned int t2 = OFFMS_WAIT_TIME_OUT_CNT;
    unsigned int offms_chg_status = 0;

    if(OFFMS_SP_MS3Ctrl_get_off_online_en(rtd_inl(OFFMS_SP_MS3Ctrl_reg)) == 0)
    {// one shot mode
        _dump_measure_status("start measure, ONE_SHOT");
        //Clear OFFMS timeout status
        rtd_maskl(OFFMS_SP_MS3Stus_reg, ~(OFFMS_SP_MS3Stus_off_hs_pol_chg_mask|OFFMS_SP_MS3Stus_off_hs_over_range_mask
        ...
        do {
            unsigned int off_start_ms = OFFMS_SP_MS3Ctrl_get_off_start_ms(rtd_inl(OFFMS_SP_MS3Ctrl_reg));
            unsigned int ms3status_reg = rtd_inl(OFFMS_SP_MS3Stus_reg);

            offms_chg_status = OFFMS_SP_MS3Stus_get_off_vs_per_to_long(ms3status_reg);

            MEASURE_NOTICE(" Check off_start_ms, t1 =%d, off_start_ms=%d, offms_chg_status=%d, off_ms_now=%d, OFFMS_SP_MS3Ctrl_reg=%x,OFFMS_SP_MS3Stus_reg=%x\n",
                t1, off_start_ms, offms_chg_status, OFFMS_SP_MS3Stus_get_off_ms_now(ms3status_reg), rtd_inl(OFFMS_SP_MS3Ctrl_reg), ms3status_reg);

            if ( !off_start_ms)  break;

            if(offms_chg_status)
            {
                MEASURE_WARN("Check msdone abort, signal unstable!!vs_per_to_long=%d, vs_per_to=%d, vs_high_to=%d\n",
            ...
            mdelay(OFFMS_CHECK_MSDONE_EVERY_MS);
        } while ((t1 > 0) && (offms_chg_status ==0));

#if defined(CONFIG_ARCH_RTK2851C) || defined(CONFIG_ARCH_RTK2851F)
        rtd_maskl(OFFMS_SP_MS3Ctrl_reg, ~OFFMS_SP_MS3Ctrl_off_popup_meas_mask, OFFMS_SP_MS3Ctrl_off_popup_meas_mask); // set pop measure

        do {
            unsigned int is_msdone = (OFFMS_SP_MS3Ctrl_get_off_popup_meas(rtd_inl(OFFMS_SP_MS3Ctrl_reg))==0) ? 1: 0;
            unsigned int ms3status_reg = rtd_inl(OFFMS_SP_MS3Stus_reg);

            offms_chg_status = OFFMS_SP_MS3Stus_get_off_vs_per_to_long(ms3status_reg);

            MEASURE_NOTICE(" Check msdone, t2 =%d, is_msdone=%d, offms_chg_status=%d, off_ms_now=%d, OFFMS_SP_MS3Ctrl_reg=%x,OFFMS_SP_MS3Stus_reg=%x\n",
                t2, is_msdone, offms_chg_status, OFFMS_SP_MS3Stus_get_off_ms_now(ms3status_reg), rtd_inl(OFFMS_SP_MS3Ctrl_reg), ms3status_reg);
            ...
            msleep(OFFMS_CHECK_MSDONE_EVERY_MS);
        } while ((t2 > 0) && (offms_chg_status ==0));
#endif
    }
    else
    {//continuous mode
        _dump_measure_status("start measure, CONTINUOUS_MODE");
        ...
        do {
            ...
            unsigned int is_msdone = (OFFMS_SP_MS3Ctrl_get_off_popup_meas(rtd_inl(OFFMS_SP_MS3Ctrl_reg))==0) ? 1: 0;
#else
            unsigned int is_msdone = (OFFMS_SP_MS3Stus_get_off_msdone(rtd_inl(OFFMS_SP_MS3Stus_reg))==1) ? 1: 0;
#endif
            unsigned int ms3status_reg = rtd_inl(OFFMS_SP_MS3Stus_reg);

            offms_chg_status = OFFMS_SP_MS3Stus_get_off_vs_per_to_long(ms3status_reg);

            MEASURE_NOTICE(" Check msdone, t1 =%d, is_msdone=%d, offms_chg_status=%d, off_ms_now=%d, OFFMS_SP_MS3Ctrl_reg=%x,OFFMS_SP_MS3Stus_reg=%x\n",
                t1, is_msdone, offms_chg_status, OFFMS_SP_MS3Stus_get_off_ms_now(ms3status_reg), rtd_inl(OFFMS_SP_MS3Ctrl_reg), ms3status_reg);
...
#if !(defined(CONFIG_ARCH_RTK2851C) || defined(CONFIG_ARCH_RTK2851F))
        rtd_maskl(OFFMS_SP_MS3Stus_reg, ~OFFMS_SP_MS3Stus_off_msdone_mask, OFFMS_SP_MS3Stus_off_msdone_mask); // write 1 clear
#endif

    }

    if (t1 == 0 || t2 == 0 ||  (offms_chg_status != 0))
    {
        _dump_measure_status("measure is not completed");
        return FALSE;
    }
    else
    {
