...
unsigned char newbase_hdmi_setup_video_pll(unsigned char port, unsigned char frl_mode)
{
	...
	SET_HDMI_MODE_TYPE((unsigned char)GET_H_MODE(port)); //set HDMI mode to hdcptx for repeater
#endif
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        HDCP_INFO("HDMI[p%d][BCH] HDCP mode:%s, RxStatus(%02x, %02x), hdcp_enc=%d, bch err 1bit=%d, 2bit err (detect,duration,cnt)=(%d, %d, %d), bch_pkt_value=%d, raw_tmds_phy_md_clk=%d, hw_reauth=%d \n", 
