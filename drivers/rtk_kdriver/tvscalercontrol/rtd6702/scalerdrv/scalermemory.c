...
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
		...
		if(allocate_memory_for_8k_platform()) {
			...
			//16 bytes alignment
			MemTag[MEMIDX_SUB_THIRD].StartAddr = align_addr& 0xfffffff0;
			MemTag[MEMIDX_SUB_SEC].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size, DMA_SPEEDUP_ALIGN_VALUE) & 0xfffffff0;
			MemTag[MEMIDX_SUB].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size, DMA_SPEEDUP_ALIGN_VALUE) & 0xfffffff0;
