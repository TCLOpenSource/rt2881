...
void Set_Val_i3ddma_buffer_size(unsigned char ucIndex, unsigned int size)
{
    if(ucIndex < CAP_BUF_NUM_MAX)
    {
        i3ddmaCtrl.cap_buffer[ucIndex].size = size;
    }
}
...
static unsigned int s_i3ddma_sw_delay_msec = 0;
static unsigned int s_i3ddma_sw_delay_msec_apply = 0;
static unsigned char s_alloced_buffer_num = 0;
static unsigned int s_i3ddma_vdelay_numbers = 0;
#define I3DDMA_SW_VEDIO_DELAY_TIME   (200)  //msec
...
unsigned int I3DDMA_SwGetVideoDelayApply(void);
unsigned int I3DDMA_SwGetVideoDelay(void);
unsigned char I3DDMA_SwSetVideoDelay(unsigned int unMsecs);
unsigned char I3DDMA_SwGetBufNum(void);
void I3DDMA_SwSetBufNum(unsigned char ucNum);
unsigned char I3DDMA_TransferDelayMsecToBufNum(unsigned int unMsecs);
unsigned char I3DDMA_SwAllocMemory(void);
void I3DDMA_SwSetBufBoundary(void);
void I3DDMA_Send_Vo_SwBufNum_share_memory(unsigned char ucNum);

extern void video_delay_free_memory(void);
...
#if 0 //dump i3ddma buf for debug
...
{//disable capture 0 for 4k120 borrow i3ddma memory
	...
	pr_notice("### %s i3ddma capture disable status (%d %d), timeoutcnt=%d db=%x", __FUNCTION__, (H3DDMA_CAP0_CTI_DMA_WR_Ctrl_get_cap0_dma_enable(IoReg_Read32(H3DDMA_CAP0_CTI_DMA_WR_Ctrl_reg))),
...
//#ifndef BUILD_QUICK_SHOW
	...
	*/
    ...
    if(bAlloc && (get_video_delay_type() == UNKNOW_VIDEO_DELAY_TYPE))
    {
#ifdef CONFIG_VBM_HEAP
    ...
    malloccacheaddr_video_phyaddr = get_vbm_heap_qshow_scaler_address(&alloc_size);
    if(0 == malloccacheaddr_video_phyaddr)
    {
        printf2("qs get memory fail\n");
    }
#endif
#endif
...
unsigned int I3DDMA_CalVideoSize(void)
{
    unsigned short usActW = 0;
    unsigned short usActH = 0;
    unsigned int unVideoSize = 0;
    unsigned char  unPixelBits=0;

    usActW = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE);
    usActH = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE);

    rtd_pr_i3ddma_info("[I3DDMA][%s, %d]usActW :%d, usActH:%d\n", __FUNCTION__, __LINE__, usActW, usActH);

    if(dvrif_i3ddma_compression_get_enable())
    {
        if(dvrif_i3ddma_get_compression_bits() == COMPRESSION_12_BITS)
            unPixelBits = 12;
        else if(dvrif_i3ddma_get_compression_bits() == COMPRESSION_16_BITS)
                unPixelBits = 16;
        else if(dvrif_i3ddma_get_compression_bits() == COMPRESSION_20_BITS)
            unPixelBits = 20;
        else
            unPixelBits = 24;
    }
    ...
    {
        //worst case
        if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_COLOR_SPACE) == I3DDMA_COLOR_YUV422)
            unPixelBits = 20;//10*2
        else
            unPixelBits = 30;
    }
    usActW = drvif_memory_get_data_align(usActW,32);
#ifdef CONFIG_BW_96B_ALIGNED
    unVideoSize = drvif_memory_get_data_align((usActW * unPixelBits + 256) / 8, DMA_SPEEDUP_ALIGN_VALUE) * usActH;
#else
    unVideoSize = ((usActW * unPixelBits + 256) / 8 * usActH);
#endif
...
unsigned char video_delay_alloc_memory(unsigned int video_size)
{
    ...
    unNum = I3DDMA_VIDEO_DELAY_BUFFER_NUMBER;
    ...
    s_alloced_buffer_num = unNum;
    rtd_pr_i3ddma_info("[VideoDelay] s_alloced_buffer_num=%d\n", s_alloced_buffer_num);
...
unsigned int I3DDMA_SwGetVideoDelayApply(void)
{
    return s_i3ddma_sw_delay_msec_apply;
}

unsigned int I3DDMA_SwGetVideoDelay(void)
{
    return s_i3ddma_sw_delay_msec;
}

unsigned char I3DDMA_SwSetVideoDelay(unsigned int unMsecs)
{
    if(unMsecs > I3DDMA_SW_VEDIO_DELAY_TIME)
    {
        rtd_pr_i3ddma_err("[I3DDMA][%s, %d]i3ddma sw video delay time more than max(%d)\n", __FUNCTION__, __LINE__, I3DDMA_SW_VEDIO_DELAY_TIME);
        s_i3ddma_sw_delay_msec = I3DDMA_SW_VEDIO_DELAY_TIME;
        return FALSE;
    }
    else
    {
        s_i3ddma_sw_delay_msec = unMsecs;
    }

    rtd_pr_i3ddma_info("[I3DDMA][%s, %d]i3ddma sw video delay time s_set_i3ddma_sw_delay_msec(%d)\n", __FUNCTION__, __LINE__, s_i3ddma_sw_delay_msec);
...
unsigned char I3DDMA_GetAllocedBufNum(void)
{
    return s_alloced_buffer_num;
}

void I3DDMA_SetAllocedBufNum(unsigned char num)
{
    s_alloced_buffer_num = num;
}

unsigned char I3DDMA_TransferDelayMsecToBufNum(unsigned int unMsecs)
{
    unsigned char ucBufNum = 0;
    unsigned short usFreq = 0;
    unsigned int unBufTime = 0;

    if(unMsecs != 0)
    {
        usFreq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ);

        unBufTime = 10000000 / usFreq;

        if (unBufTime > 0)
        {
            ucBufNum = ((unMsecs * 1000) / unBufTime) + 2;
        }
        rtd_pr_i3ddma_info("[I3DDMA][%s, %d]unMsecs:%d, usFreq:%d, unBufTime:%d, ucBufNum:%d \n", __FUNCTION__, __LINE__,unMsecs,usFreq,unBufTime,ucBufNum);
    }
...
void I3DDMA_3DDMADeInit(void) {
	...
	i3ddmaCtrl.i3ddma_3d_capability= 0;
	rtd_pr_i3ddma_info("[HDMI-3D] Deinit done!\n");
}
#endif
//[ECN][RL6543-17]
