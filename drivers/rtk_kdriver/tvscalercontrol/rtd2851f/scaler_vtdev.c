...
#ifndef UT_flag
#include <hw_monitor/arm_wrapper.h>
#endif
...
#define _RGB2YUV_set_Rin_offset		0x0d
...
void init_vt_write_read_index(void)
{
    ...
    {
        pVtBufStatus->write = Scaler_ChangeUINT32Endian(init_idx);
        pVtBufStatus->read = Scaler_ChangeUINT32Endian(init_idx);
    }
}
...
void drvif_color_colorspaceyuv2rgbtransfer(DC2H_IN_SEL dc2h_in_sel)
{
	...
	dc2h_rgb2yuv_ctrl_reg.set_r_in_offset= table_index [_RGB2YUV_set_Rin_offset];
...
//	unsigned int width96align;
    ...
    if (get_vt_VtSwBufferMode()) {
        dc2h_dma_dc2h_seq_interrupt_enable_Reg.regValue = IoReg_Read32(DC2H_DMA_dc2h_seq_Interrupt_Enable_reg);
        dc2h_dma_dc2h_seq_interrupt_enable_Reg.vs_rising_ie = 1;

        dc2h_vi_dc2h_inten_Reg.regValue = IoReg_Read32(DC2H_VI_DC2H_INTEN_reg);
        //dc2h_vi_dc2h_inten_Reg.video_in_last_wr_y_flag_ie = 1;
        dc2h_vi_dc2h_inten_Reg.video_in_last_wr_c_flag_ie = 1;
        //dc2h_vi_dc2h_inten_Reg.vsinte1 = 1;
    }
...
//before scaler input have hsd,so in_size not morethan 3840
    if(in_size->nWidth > VT_CAP_FRAME_WIDTH_4K2K || in_size->nLength > VT_CAP_FRAME_HEIGHT_4K2K){
        ...
        rtd_pr_vt_notice("[warning] Input Source Hact over 3840 or 2160!!!!\n");
    }
}
...
unsigned char HAL_VT_SetVideoFrameOutputDeviceOutputRegion(KADP_VT_VIDEO_WINDOW_TYPE_T videoWindowID, KADP_VT_DUMP_LOCATION_TYPE_T dumpLocation, KADP_VT_RECT_T *pOutputRegion)
{
    ...
    if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_STATE) == _MODE_STATE_ACTIVE){
        if(get_drm_case_securestatus()==TRUE){
            rtd_pr_vt_emerg("[VT]%s=%d DRM CASE source security is TRUE,could not do vt capture!\n", __FUNCTION__, __LINE__);
            up(&VT_Semaphore);
            return FALSE;
        }
    }
...
unsigned int VT_Output_Colorspace(void)
{
    ...
    if(curCapInfo.enable == TRUE){
        ...
        if(DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_get_en_rgb2yuv(IoReg_Read32(DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_reg))==0)
        {
            if((get_VT_Pixel_Format() == VT_CAP_NV12)||(get_VT_Pixel_Format() == VT_CAP_NV16)||(get_VT_Pixel_Format() == VT_CAP_NV21))
            {
        ...
        else if((DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_get_en_rgb2yuv(IoReg_Read32(DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_reg))==1) || (DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_get_en_rgb2yuv(IoReg_Read32(DC2H_RGB2YUV_DC2H_RGB2YUV_CTRL_reg))==2))
        {
            vtoutput_clorspace_state = KADP_VT_VIDEO_COLOR_SPACE_BT709;
        }
        else
