...
static struct semaphore Main_Mdomain_ctrl_Semaphore;//protect m domain capture enable memory trash
...
#endif
extern void set_video_delay_type(VIDEO_DELAY_TYPE res);
...
unsigned char CMA_alloc_m_domain_original_buffer(void)
{
	...
	for(CMA_buffer_index = 0; CMA_buffer_index < CMA_buffer_number; CMA_buffer_index++)
	{
		...
		if (CMA_m_domain_buffer == 0)
		{
			...
			CMA_m_domain_buffer = pli_malloc(CMA_buffer_size, GFP_DCU3);
...
void CMA_use_i3ddma_buffer_reset_DCMT_impl(unsigned long i3ddma_start_addr, unsigned int i3ddma_size)
{// DCMT add i3ddma buffer, borrowed from VMB.
#ifndef UT_flag
	dcmt_cma_desc_t desc = {0};
	unsigned char CMA_buffer_index = 0;

	if(i3ddma_start_addr && i3ddma_size)
	{
		rtd_pr_scaler_memory_info("[%s] Not use CMA alloc buffer(0x%px). Use i3ddma buffer borrowed from VBM. i3ddma_start_addr=0x%px, i3ddma_size=0x%x\n", __func__, CMA_get_m_domain_original_buffer(CMA_buffer_index), i3ddma_start_addr, i3ddma_size);
	}
	else
	{
		rtd_pr_scaler_memory_err("###[Bug] func:%s i3ddma_start_addr=0x%px, i3ddma_size=0x%x ##\n", __func__, i3ddma_start_addr, i3ddma_size);
		return;
	}

	dcmt_cma_mdomain_unset(&desc);  // Do unset DCMT before set DCMT

	// DCMT setting about the buffer borrowed from CMA originally
	for(CMA_buffer_index = 0; CMA_buffer_index < CMA_buffer_number; CMA_buffer_index++)
	{
		desc.pa_start[CMA_buffer_index] = CMA_get_m_domain_original_buffer(CMA_buffer_index);
		desc.pa_size[CMA_buffer_index] = CMA_get_m_domain_original_size(CMA_buffer_index);
	...
	// DCMT setting about the buffer borrowed from i3ddma
	desc.pa_start[CMA_buffer_index] = i3ddma_start_addr;
	desc.pa_size[CMA_buffer_index] = i3ddma_size;
	desc.region_cnt++;

	dcmt_cma_mdomain_set(&desc);  // Set DCMT
#endif
}
void (*CMA_use_i3ddma_buffer_reset_DCMT)(unsigned long i3ddma_start_addr, unsigned int i3ddma_size) = CMA_use_i3ddma_buffer_reset_DCMT_impl;
/*
	API releated to CMA malloc, doesn't use carvedout memory.
	End
*/
...
unsigned int get_query_start_address(unsigned char idx){
	...
	unsigned long buffer_size_main = 0;
#ifdef CONFIG_I2RND_ENABLE
	...
	if (idx != QUERY_IDX_VIP_DMAto3DTABLE)
		rtd_pr_scaler_memory_info("[%s]idx = %d, addr = 0x%lx, size = 0x%lx\n", __FUNCTION__, idx, addr, size);
	return addr;
}
...
#else //#ifdef CONFIG_DYNAMIC_ALLOCATION_SCALER_MEMORY
...
static void MemSetDisplayAddr(unsigned char display, unsigned int Addr1, unsigned int Addr2)
{
	...
	if(display == SLR_MAIN_DISPLAY){
        ...
        if(!drv_memory_Get_multibuffer_flag()){	//multi buffer case do not set m_disp boundary the same as m_cap boundary
		...
		/*Remove it becasue drv_memory_cap_limit_boundary_setting already set this
		//update m_disp boundary the same as m_cap boundary @Crixus 20171227
...
void memory_set_input_format(unsigned char display, unsigned char bDouble)
{
	...
	else
	{
		...
		if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > (Get_DISPLAY_REFRESH_RATE()*10+50)) &&(display == SLR_MAIN_DISPLAY))
		{
	...
	else
	{
...
#else
                    printf2("qs memory is not enough\n");
#endif
                }
		}
		...
		query_addr = get_query_start_address(QUERY_IDX_MDOMAIN_MAIN);
			...
			if(get_platform_model() == PLATFORM_MODEL_2K) {
				if(Get_rotate_function(SLR_MAIN_DISPLAY)) {	//2k mode rotate borrow 1 CMA address,8M
					...
					if(temp_addr == INVALID_VAL)
					{
						...
						set_m_domain_setting_err_status(display, MDOMAIN_SETTING_ERR_MAIN_2K_ROTATE_NO_MEM); // set error status and don't enable Mcap_en later
						return;
					}
					...
					if(mdomain_2k_malloc_cma_memory(buffer_size)) {
						...
						set_m_domain_setting_err_status(display, MDOMAIN_SETTING_ERR_MAIN_2K_NO_MEM); // set error status and don't enable Mcap_en later
						return;
					}
            ...
            #endif
			{
                ...
                } else if(!is_2k_memory_buffer_source(SLR_MAIN_DISPLAY))
				{//4k
					...
					{
                         {
                            ...
                            MemTag[MEMIDX_MAIN_SEC].StartAddr = drvif_memory_get_data_align(i3ddmaCtrl.cap_buffer[0].phyaddr + i3ddmaCtrl.cap_buffer[0].size*1, DMA_SPEEDUP_ALIGN_VALUE);
                            MemTag[MEMIDX_MAIN].StartAddr = drvif_memory_get_data_align(i3ddmaCtrl.cap_buffer[0].phyaddr + i3ddmaCtrl.cap_buffer[0].size*2, DMA_SPEEDUP_ALIGN_VALUE);

                            if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
                                CMA_use_i3ddma_buffer_reset_DCMT(MemTag[MEMIDX_MAIN_THIRD].StartAddr, (MemTag[MEMIDX_MAIN].StartAddr + i3ddmaCtrl.cap_buffer[0].size - MemTag[MEMIDX_MAIN_THIRD].StartAddr));
                        }
					}
					else
					{
...
#else
                        printf2("main modomain no memory\n");
#endif
			...
			if(get_platform_model() == PLATFORM_MODEL_2K) {
				...
				{//4k
					if(i3ddmaCtrl.cap_buffer[0].phyaddr)
					{
						...
						if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
							CMA_use_i3ddma_buffer_reset_DCMT(MemTag[MEMIDX_MAIN_THIRD].StartAddr, (MemTag[MEMIDX_MAIN].StartAddr + buffer_size - MemTag[MEMIDX_MAIN_THIRD].StartAddr));
					}
					else
					{
...
void memory_set_sub_displaywindow_change(unsigned char changetomain)
{
	...
	mdomain_disp_ddr_subctrl_Reg.sub_cap2disp_distance = 1;
	IoReg_Write32(MDOMAIN_DISP_DDR_SubCtrl_reg, mdomain_disp_ddr_subctrl_Reg.regValue);
	/*IoReg_Write32(MDOMAIN_DISP_Disp_sub_enable_reg, MDOMAIN_DISP_Disp_sub_enable_sub_disp_en_mask);*/
...
void dvrif_memory_handler(void)
{
    ...
    if(display == SLR_MAIN_DISPLAY)
    {
        ...
        if(!compare_rerify_vsc_source_num_result(display))
        {
            ...
            rtd_pr_scaler_memory_info("### [%d] vsc source change return ###\r\n", __FUNCTION__);
            return;
        }
#endif // #ifndef UT_flag
...
void mdomain_handler_onlycap(void)
{
	...
	set_m_domain_setting_err_status(display, MDOMAIN_SETTING_NO_ERR); // reset M domain setting error status
	memory_set_capture(TRUE, MEMCAPTYPE_FRAME, display);
	drv_memory_limit_boundary_enable(display, drv_memory_get_limit_boundary_mode(display));//for boundary mode setting
	if(get_m_domain_setting_err_status(display) == MDOMAIN_SETTING_NO_ERR)
		drvif_memory_EnableMCap(display);
	else
 ...
 * @return { none }
 * @note
 * Memory main/sub display are double buffered, those registers will be updated
...
//	cmp_alpha_en = drv_memory_di_Decide_PQC_alpha_En();
	...
	if (PQC_Mode == PQC_Frame_Mode) {
		...
		di_di_pq_cmp_bit_reg.frame_limit_bit = PQC_Bit_Limit;
		di_di_pq_cmp_bit_reg.line_limit_bit = PQC_Bit_Limit;
		di_di_pq_cmp_bit_reg.block_limit_bit = 0x3f;	/*recommand by skyred for mer3*/
	}
#else
	di_di_pq_cmp_bit_reg.frame_limit_bit = PQC_Bit_Limit;
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		if(Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_FRAMESYNC) == FALSE){
			if(Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE){
				drv_memory_display_set_check_frc_style();
...
bool get_mdomain_input_fast_status(unsigned char display)
{
	...
	rtd_pr_scaler_memory_notice("[%s] %d display=%d, input_fast=%d\n", __FUNCTION__, __LINE__, display, input_fast_flag);
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		do
		{
			...
			if(cur_disp_idx == pre_disp_idx)
			{
				...
				cur_disp_idx = MDOMAIN_DISP_DDR_MainStatus_get_main_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_MainStatus_reg));
				rtd_pr_scaler_memory_notice("[%s] %d M domain main done-2(check again), disp new_idx=%d, count=%d\n", __FUNCTION__, __LINE__, cur_disp_idx, count);
				if(count == 0)
				{
#ifdef BUILD_QUICK_SHOW
	...
	else
	{
		do
		{
			...
			if(cur_disp_idx == pre_disp_idx)
			{
				...
				cur_disp_idx = MDOMAIN_DISP_DDR_SubStatus_get_sub_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_SubStatus_reg));
				rtd_pr_scaler_memory_notice("[%s] %d M domain sub done-2(check again), disp new_idx=%d, count=%d\n", __FUNCTION__, __LINE__, cur_disp_idx, count);
...
void drv_memory_wait_cap_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case)
{
	...
	if(get_mdomain_input_fast_status(display) == TRUE)
	{
		...
		unsigned char original_mdisp_db_en = MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_enable(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
		unsigned char original_pqdc_en = MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_get_decmp_en(IoReg_Read32(MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg));
		...
		if((original_pqdc_en) && (display == SLR_MAIN_DISPLAY))
		{
			//Disable de-compression
			IoReg_ClearBits(MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg, MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_decmp_en_mask);
		...
		{
			//Enable de-compression
			IoReg_SetBits(MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg, MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_decmp_en_mask);
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		do
		{
			...
			rtd_pr_scaler_memory_notice("[%s] %d main wait first write finish count=%d, pre_cap_idx=%d \n", __FUNCTION__, __LINE__, count, pre_cap_idx);

			count = 40;
			//rtd_pr_scaler_memory_notice("[%s] %d M domain main done-0, cap pre idx = %d\n", __FUNCTION__, __LINE__, pre_cap_idx);
			...
			while((MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))) && --count)
			{
                if (MDOMAIN_CAP_Cap_In1_enable_get_in1_cap_enable(IoReg_Read32(MDOMAIN_CAP_Cap_In1_enable_reg)) == 0)
                {
			...
			if(cur_cap_idx == pre_cap_idx)
			{
				...
				cur_cap_idx = MDOMAIN_CAP_DDR_In1Status_get_in1_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In1Status_reg));
				rtd_pr_scaler_memory_notice("[%s] %d M domain main done-2(check again), cap new_idx=%d, count=%d\n", __FUNCTION__, __LINE__, cur_cap_idx, count);
				if(count == 0)
				{
#ifdef BUILD_QUICK_SHOW
					printf2("M domain main timeout-2 !\n");
	...
	else
	{
		do
		{
			...
			rtd_pr_scaler_memory_notice("[%s] %d sub wait first write finish count=%d, pre_cap_idx=%d \n", __FUNCTION__, __LINE__, count, pre_cap_idx);
			...
			if(cur_cap_idx == pre_cap_idx)
			{
				...
				cur_cap_idx = MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg));
				rtd_pr_scaler_memory_notice("[%s] %d M domain sub done-2(check again), cap new_idx=%d, count=%d\n", __FUNCTION__, __LINE__, cur_cap_idx, count);
...
void drv_memory_set_game_mode_dynamic(unsigned char enable){
    if(enable_M_GameMode_Dynamic != enable)
    {
        rtd_pr_new_game_mode_emerg("[%s] %d->%d", __FUNCTION__, enable_M_GameMode_Dynamic, enable);
    }
...
//  ========================================================================
    ...
    unsigned int vfreq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ);
...
{
    unsigned int iv2dv_delay = drv_GameMode_calculate_iv2dv_delay();
...
{
    unsigned long mallocAddr1_from_VBM = 0; //address allocated from VBM
    unsigned long getSize = 0; //VBM return memory size
    UINT32  reqSize = 0; //request size for allocate memory
    unsigned int buffer_size = scaler_memory_cal_multibuf_size();

    rtd_pr_scaler_memory_notice("[%s] buffer_size=0x%x\n", __FUNCTION__,buffer_size);

    reqSize = buffer_size*2; //VBM chunks 64M + 42.5 MB
#ifdef CONFIG_VBM_HEAP
    if (VBM_BorrowMemChunk(&memory_multibuf_dmabuf_hndl, (unsigned long *)(&mallocAddr1_from_VBM), (unsigned long *)(&getSize), reqSize,0) != 0)
#else
    if (VBM_BorrowMemChunk((unsigned long *)(&mallocAddr1_from_VBM), (unsigned long *)(&getSize), reqSize,0) != 0)
#endif
    {
        rtd_pr_scaler_memory_emerg("[videoDelay] alloc vbm memory failed[%lx]\n",  mallocAddr1_from_VBM);
        return 1; //malloc memory fail
    }
    if(getSize < reqSize)
        rtd_pr_scaler_memory_emerg("[videoDelay][%s] Error: vbm mem size(getSize:%ld) is not 62M \n", __FUNCTION__ ,getSize);

    if((unsigned long)mallocAddr1_from_VBM == (unsigned long)NULL)
    {
        rtd_pr_scaler_memory_emerg("[videoDelay] alloc vbm memory NULL\n");
        return 1;
    }

    rtd_pr_scaler_memory_notice("[VBM] reqSize:%d, getSize:%ld\n", reqSize, getSize);
    get_borrowed_VBM_Size = getSize;

    MemTag[MEMIDX_MAIN_FIFTH].Allocate_VirAddr = 0;
    MemTag[MEMIDX_MAIN_FIFTH].StartAddr = mallocAddr1_from_VBM;
    MemTag[MEMIDX_MAIN_FIFTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_FIFTH].StartAddr;
    MemTag[MEMIDX_MAIN_FIFTH].Size = buffer_size;
    MemTag[MEMIDX_MAIN_FIFTH].Status = ALLOCATED_FROM_LOW;
    //update start address
    IoReg_Write32(MDOMAIN_CAP_capm_fifth_buf_addr_reg, MemTag[MEMIDX_MAIN_FIFTH].StartAddr);
    IoReg_Write32(MDOMAIN_DISP_main_fifth_buf_addr_reg, MemTag[MEMIDX_MAIN_FIFTH].StartAddr);

    MemTag[MEMIDX_MAIN_SIXTH].Allocate_VirAddr = 0;
    MemTag[MEMIDX_MAIN_SIXTH].StartAddr = mallocAddr1_from_VBM + buffer_size;
    MemTag[MEMIDX_MAIN_SIXTH].Allocate_StartAddr = MemTag[MEMIDX_MAIN_SIXTH].StartAddr;
    MemTag[MEMIDX_MAIN_SIXTH].Size = buffer_size;
    MemTag[MEMIDX_MAIN_SIXTH].Status = ALLOCATED_FROM_LOW;
    //update start address
    IoReg_Write32(MDOMAIN_CAP_capm_sixth_buf_addr_reg, MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
    IoReg_Write32(MDOMAIN_DISP_main_sixth_buf_addr_reg, MemTag[MEMIDX_MAIN_SIXTH].StartAddr);
...
unsigned char scaler_memory_free_multibuf(void)
{
    ...
    if (MemTag[MEMIDX_MAIN_FIFTH].StartAddr)
    {
        rtd_pr_scaler_memory_emerg("[videodelay]%s call VBM_ReturnMemChunk2\n",__FUNCTION__);
#ifdef CONFIG_VBM_HEAP
        VBM_ReturnMemChunk(memory_multibuf_dmabuf_hndl, MemTag[MEMIDX_MAIN_FIFTH].StartAddr, get_borrowed_VBM_Size);
#else
        VBM_ReturnMemChunk(MemTag[MEMIDX_MAIN_FIFTH].StartAddr, get_borrowed_VBM_Size);
#endif
    }

    for (i = MEMIDX_MAIN_FIFTH; i <= MEMIDX_MAIN_EIGHTH; i++)
    {
        MemTag[i].Allocate_VirAddr = 0;
...
void drv_memory_display_set_input_fast_impl(unsigned char display, unsigned char enable){
	mdomain_disp_ddr_mainctrl_RBUS mdomain_disp_ddr_mainctrl_reg;
...
void (*drv_memory_display_set_input_fast)(unsigned char display, unsigned char enable) = drv_memory_display_set_input_fast_impl;
...
void drv_memory_cap_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	...
	if(display == SLR_MAIN_DISPLAY){
		// Set boundary for each block
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_CTRL2_reg);
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk0_sel = 0; //use 0xb8027754 boundary
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk1_sel = 1; //use 0xb802775C boundary
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk2_sel = 2; //use 0xb8027764 boundary
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_CTRL2_reg, mdomain_cap_capmain_boundaryaddr_ctrl2_reg.regValue);

		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(MemTag[MEMIDX_MAIN].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(MemTag[MEMIDX_MAIN].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
	}
	else if(display == SLR_SUB_DISPLAY){
		// Set boundary for each block
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr_CTRL2_reg);
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk0_sel = 0; //use 0xb8027774 boundary
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk1_sel = 1; //use 0xb802777c boundary
		mdomain_cap_capsub_boundaryaddr_ctrl2_reg.in2_blk2_sel = 2; //use 0xb8027784 boundary
		...
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg, (MemTag[MEMIDX_SUB].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size - FINAL_BURST_LENTH) & 0xfffffff0);//need cut burstLen
...
void drv_memory_disp_limit_boundary_setting(unsigned char display){
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DispMain_BoundaryAddr_CTRL2_reg);
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk0_sel = 0; //use 0xb8027754 boundary
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk1_sel = 1; //use 0xb802775C boundary
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk2_sel = 2; //use 0xb8027764 boundary
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_CTRL2_reg, mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.regValue);
		...
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk0_sel = 0; //use 0xb8027284-0xb8027288 boundary
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk1_sel = 1; //use 0xb8027774-0xb8027778 boundary
		mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk2_sel = 2; //use 0xb802777c-0xb8027780 boundary
		...
		if(get_hdmi_vrr_4k60_mode()){
			//4K60Hz VRR flow
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg) & 0xfffffff0));//boundary 0
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg) & 0xfffffff0));//boundary 0
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg) & 0xfffffff0));//boundary 1
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg) & 0xfffffff0));//boundary 1
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg) & 0xfffffff0));//boundary 2
			IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg) & 0xfffffff0));//boundary 2
		}
		else{
...
unsigned int calc_compress_memory_size(SCALER_DISP_CHANNEL display, MemCompression_Bits eCompressBit)
{
    ...
    ulCompressMemorySize = ulCapLen * (ulLineCompressSize / 8) + FINAL_BURST_LENTH; //for boundary use. last burst avoid trash
	...
	else
	{
		if(display == SLR_MAIN_DISPLAY)
		{
			...
			if(use_db_disable)
			{
				...
				drvif_scaler_mdisp_set_disp1_double_enable(_ENABLE);
			}
			...
			if(use_db_disable)
			{
				while(apply_timeout_cnt-- && (drvif_scaler_mdisp_get_disp1_double_apply()))
				{
					msleep(10);
				}
				if(!apply_timeout_cnt)
					printk(KERN_ERR"[%s] %d using db to disable wait timeout!!\n", __FUNCTION__, __LINE__);
				else
					printk(KERN_INFO "[%s] %d using db to disable\n", __FUNCTION__, __LINE__);
			}

			drvif_scaler_mdisp_set_disp1_double_enable(_DISABLE);
		}
		else
		{
			//disable sub disp
