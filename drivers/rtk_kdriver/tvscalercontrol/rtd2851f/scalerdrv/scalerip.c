...
//					if(Scaler_DispGetInputInfo(SLR_INPUT_CAP_WID) >= 960)
					...
					{	// 5A
						...
						DI_SW_Detection_in_VPQ_TSK_Stable_CNT = 5;
		...
		if(display == _CHANNEL1)
		{
			...
			if(vgip_chn1_ctrl_reg.ch1_digital_mode) //digital mode need to set porch
			{
				...
				else//hdmi case
				{
					...
					{// For h_total_porch > VGIP_H_PORCH_MARGIN case.
						...
						{// It's equal to h_front_porch > VGIP_H_PORCH_MARGIN case. It means there has enough h_front_porch for h_back_porch(hporch_num) using.
							vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE) + 10;
						}
						else
						{// For "h_total_porch > VGIP_H_PORCH_MARGIN" and "h_front_porch < VGIP_H_PORCH_MARGIN" case.
							vgip_chn1_delay_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_DELAY_reg);
							vgip_chn1_delay_reg.ch1_ihs_dly = 10; // h delay
							IoReg_Write32(VGIP_VGIP_CHN1_DELAY_reg, vgip_chn1_delay_reg.regValue);

							vgip_chn1_misc_reg.ch1_hporch_num = Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_STA_PRE);
						}

						rtd_pr_vbe_info("###(%d) ch1_hporch_num =%d, delay=0x%08x\n", __LINE__, vgip_chn1_misc_reg.ch1_hporch_num, IoReg_Read32(VGIP_VGIP_CHN1_DELAY_reg));
					}
					else
					{// For h_total_porch < VGIP_H_PORCH_MARGIN case. It means that h_total_porch may not be enough, but it almost never happens in real HDMI timing info.
						vgip_chn1_delay_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_DELAY_reg);
						vgip_chn1_delay_reg.ch1_ihs_dly = 10; // h delay
						IoReg_Write32(VGIP_VGIP_CHN1_DELAY_reg, vgip_chn1_delay_reg.regValue);
