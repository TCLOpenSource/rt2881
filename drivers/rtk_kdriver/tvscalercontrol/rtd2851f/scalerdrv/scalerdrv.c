...
extern unsigned char I3DDMA_SwGetBufNum(void);
extern VIDEO_DELAY_TYPE get_video_delay_type(void);
#ifndef BUILD_QUICK_SHOW
...
#if !defined (BUILD_QUICK_SHOW) && defined (CONFIG_RTK_KDRV_DV)
...
{/*TRUE: memc bypass; FALSE: memc on */
	...
	}else if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI)||(Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)){
...
void scaler_set_main_vgip_capture(unsigned char ucIVS2DVSDelay, unsigned short ucIHSDelay)
{
    ...
    if((Scaler_InputSrcGetMainChType() == _SRC_HDMI) || (Scaler_InputSrcGetMainChType() == _SRC_DISPLAYPORT)) {
		...
		if (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_STA) > (VGIP_Delay_Margin+VGIP_Vdelay_Min)) {
...
//		unsigned int vFreq = 0;
		...
		drvif_scaler_wclr_abnormal_dvs_long_short();
			...
			if(drv_memory_get_game_mode() || (judge_vrr_freesync())
                || drv_memory_get_vdec_direct_low_latency_mode()){//if game mode case, need to use calculation result @Crixus 20170712
                ...
                {
                    //avoid fake scaler framesync error
                    fs_iv_dv_fine_tuning5_reg.iv2dv_line = _VO_GATTING_OFFSET;
                    fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = _VO_GATTING_OFFSET;
                    rtd_pr_vbe_emerg("[%s][%d]fake case set iv2dv delay = %d\n", __FUNCTION__, __LINE__, fs_iv_dv_fine_tuning5_reg.iv2dv_line );
                }
#endif
			}
			else{
        ...
        //if den target + 20 over vtotal, need protect it
        if(ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target > Get_DISP_VERTICAL_TOTAL())
        {
            ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target = Get_DISP_DEN_END_VPOS() + (Get_DISP_VERTICAL_TOTAL() - Get_DISP_DEN_END_VPOS()) / 2;
        }

        //if den target less than min vtotal, need protect it
    ...
    else
    {
        switch(get_panel_res())
        {
            ...
            default:
                ...
                if((Get_DISPLAY_PANEL_OLED_TYPE() == FALSE) && (Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN() > DVS_4K_SHORT_LINE_THRESHOLD))
                {
                    *dvs_short = DVS_4K_SHORT_LINE_THRESHOLD - 1;
                }
                else
                {
                    *dvs_short = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN() - 1;
...
void drvif_scaler_enable_abnormal_dvs_protect(unsigned char bEnable)
{
    ...
    ppoverlay_dvs_abnormal_control_reg.dvs_short_line_th = ppoverlay_dvs_abnormal_control_reg.dvs_short_line_th - 1;
	...
	rtd_pr_vbe_notice("[%s] enable = %d, abnormal setting:%x \n", __FUNCTION__, bEnable, ppoverlay_dvs_abnormal_control_reg.regValue);
}
...
//		Scaler_DispSetInputInfo(SLR_INPUT_DI_WID,Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID));
        ...
        if (Get_rotate_function(SLR_MAIN_DISPLAY) && (get_rotate_curState(SLR_MAIN_DISPLAY) == MAIN_ROTATE_INIT_UZD))
        {
            ...
            if ((_DISP_WID <= 1920) || (_DISP_LEN <= 1080))
            {
                ...
                rotate_wid = ROTATE_MAX_CAP_WIDTH;
                rotate_len = ROTATE_MAX_CAP_LENGTH;
            }
			...
			if (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC) == FALSE) {
				if ((Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_DISP_WID)> max_width)
						...
						&& !(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) || ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_DP)))
							&& (Get_DISPLAY_REFRESH_RATE() == 120) && (judge_vrr_freesync()))
    ...
    printf2("Game=%d,PC=%d, 422=%d, vflip=%d\n", is_QS_game_mode(), Get_Val_vsc_run_pc_mode(), Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422),Get_PANEL_VFLIP_ENABLE());
	printf2("Panel_type=%d,Panel_rate=%d,VRR=%d\n", Get_DISPLAY_PANEL_TYPE(), Get_DISPLAY_REFRESH_RATE(), ((vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())));
#else
...
*/
	if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 235) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 255)){
		...
		result = FALSE;
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 844) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 855)){
		if(Get_DISPLAY_REFRESH_RATE()>60)
			result = FALSE;        
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 994) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1005)){
		if(Get_DISPLAY_REFRESH_RATE()>=100)
			result = FALSE;
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 1191) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1206)){
		if(Get_DISPLAY_REFRESH_RATE()>=120)
			result = FALSE;
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 1434) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1445)){
		if(Get_DISPLAY_REFRESH_RATE()>=144)
			result = FALSE;
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 1644) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1655)){
		if(Get_DISPLAY_REFRESH_RATE()>=165)
			result = FALSE;
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= 1794) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) <= 1805)){
		if(Get_DISPLAY_REFRESH_RATE()>=180)
			result = FALSE;
	}
	/*
	if(result == TRUE)
...
#if IS_ENABLED(CONFIG_RTK_KDRV_RPC)
#ifndef CONFIG_SCALER_BRING_UP
	if (RPC_FAIL==send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_ConfigVodmaVFlip, VO_VIDEO_PLANE_V1, enable, &result))
	{
		rtd_pr_vbe_debug("RPC VO1 Rotate VFlip fail!!\n");
		return FALSE;
	}
#endif
#endif
...
void modestate_decide_dtg_m_mode(void)
{
		...
		(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
        ...
        if(Get_DISPLAY_REFRESH_RATE() == 120)
        {
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
				...
				Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 175);
		}else{
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 50);
		}
	}else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 295) &&
	...
	{
		if(vbe_disp_get_game_mode_dynamic_change_clk_flag() || vbe_disp_get_game_framerate_1to1_change_flag()){
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 48);
			...
			(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 605))	//60hz
	{
		...
		if(vbe_disp_get_game_mode_dynamic_change_clk_flag() || vbe_disp_get_game_framerate_1to1_change_flag()){
			if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 845) &&
				(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 855)){	//85Hz
				Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 85);
			}
			...
			else if((Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 1795) &&
					(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 1805)){	//180Hz
				Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 180);
			}
			else
				Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ)/10);                      
		}
		else if(Get_DISPLAY_REFRESH_RATE() == 120){
...
void modestate_decide_dtg_m_modeByDisp(unsigned char _channel)
{
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
        ...
        if(Get_DISPLAY_REFRESH_RATE() == 120)
        {
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
				...
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, 175);
		}else{
			Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, 50);
		}
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 605))	//60hz
	{
		...
		if(vbe_disp_get_game_mode_dynamic_change_clk_flag() || vbe_disp_get_game_framerate_1to1_change_flag()){
			if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 845) &&
				(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 855)){	//85Hz
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, 85);
			}
			else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 995) &&
			...
			else if((Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) > 1795) &&
					(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 1805)){	//180Hz
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, 180);
			}
			else
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ, Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ)/10);                
		}else if(Get_DISPLAY_REFRESH_RATE() == 120){
...
void modestate_decide_fractional_framesync(void)
{
		...
		(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() == 120){
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC){
				...
				rtd_pr_vbe_debug("15/2, 2pixel \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 2);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 15);
			}
		}else{
		...
		(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() == 120){
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC){
				...
				rtd_pr_vbe_debug("7/1, 2pixel \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 7);
			}
		}else{
			if(Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ) == 60){
	...
	{
		if(vbe_disp_get_game_mode_dynamic_change_clk_flag() || vbe_disp_get_game_framerate_1to1_change_flag()){
			rtd_pr_vbe_debug("1/1, 1pixel (input > 60Hz) \n");
			Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
			Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 1);
		}else if(Get_DISPLAY_REFRESH_RATE() == 120){
			rtd_pr_vbe_debug("2/1, 2pixel \n");
			...
			(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() == 120){
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC){
				...
				rtd_pr_vbe_debug("15/2, 2pixel \n");
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO, 2);
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 15);
			}
		}else{
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
		if(Get_DISPLAY_REFRESH_RATE() == 120){
			...
			if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC){
				...
				rtd_pr_vbe_debug("7/1, 2pixel \n");
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
				Scaler_DispSetInputInfoByDisp(channel,SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 7);
			}
		}else{
			if(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ) == 60){
				rtd_pr_vbe_debug("12/5, 1pixel \n");
...
//	ppoverlay_display_timing_ctrl2_RBUS display_timing_ctrl2_reg;
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		vertical_total = Get_DISP_VERTICAL_TOTAL();
		rtd_pr_vbe_notice("######## Convertor board set panel vtotal:%d ##########\r\n", vertical_total);
	}
...
void hdmi_vtop_hfr_control(void)
{
	...
	if(src == VSC_INPUTSRC_HDMI || (src == VSC_INPUTSRC_DP))
	{
		...
		{//4k2k input
			HDMI_COLOR_SPACE_T hdmi_color_space;
			DP_COLOR_SPACE_E   dp_color_space;
			...
			hdmi_color_space = drvif_Hdmi_GetColorSpace();
#if IS_ENABLED(CONFIG_RTK_DPRX)
			dp_color_space = drvif_Dprx_GetColorSpace();
#else
			dp_color_space = DP_COLOR_SPACE_UNKNOWN;
#endif
			if ((src == VSC_INPUTSRC_HDMI && hdmi_color_space == COLOR_YUV422)
					|| (src == VSC_INPUTSRC_DP && dp_color_space == DP_COLOR_SPACE_YUV422)
			   )
			{
				dm_input_format_reg.dm_in_sel = 6;// hdmi/dp 422
			}
			else
			{
				dm_input_format_reg.dm_in_sel = 7;// hdmi/dp 444
			}
...
VSC_DRIVER_PATTERN vsc_driver_pattern_table[] ={
	...
	{HDMI_2K1K_120,	 	        0, 0, 1, 0, 0, 1, 0},
	{HDMI_2K1K_240,	 	        0, 0, 1, 0, 0, 1, 0},
	{HDMI_25K1K_30, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_25K1K_60, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_25K1K_120, 	        0, 0, 0, 1, 0, 0, 0},
	{HDMI_25K1K_240, 	        0, 0, 0, 1, 0, 0, 0},
	{HDMI_3K1K_30, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_3K1K_60, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_3K1K_120, 	               0, 0, 0, 1, 0, 0, 0},
	{HDMI_3K1K_240,     	        0, 0, 0, 1, 0, 0, 0},
	{HDMI_4K2K_30, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_4K2K_60, 		        3, 0, 0, 1, 0, 0, 0},
	{HDMI_4K2K_120, 		        0, 0, 0, 1, 0, 0, 0},
	{HDMI_4K2K_240, 		        0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_2K1K_30,		 0, 0, 1, 0, 0, 1, 0},
	{HDMI_RGB_2K1K_60, 		 0, 0, 1, 0, 0, 1, 0},
	{HDMI_RGB_2K1K_120,	 	 0, 0, 1, 0, 0, 1, 0},
	{HDMI_RGB_2K1K_240,	 	 0, 0, 1, 0, 0, 1, 0},
	{HDMI_RGB_25K1K_30, 	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_25K1K_60, 	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_25K1K_120, 	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_25K1K_240, 	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_3K1K_30, 		 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_3K1K_60, 		 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_3K1K_120, 	 0, 0, 1, 0, 0, 0, 0},
	{HDMI_RGB_3K1K_240,     	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_4K2K_30, 		 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_4K2K_60, 		 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_4K2K_120, 	 0, 0, 0, 1, 0, 0, 0},
	{HDMI_RGB_4K2K_240, 	 0, 0, 0, 1, 0, 0, 0},
...
#endif
	{
		...
		else if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_DP)
		{
            ...
            if((cur_act_w <= 1920)||(cur_act_h <= 1080)) {
                ...
                } else if(cur_freq >= 310 && cur_freq <= 990){
                    if(Get_Val_vsc_run_pc_mode() == TRUE) {
                        index = HDMI_RGB_2K1K_60;
                    } else{
                        index = HDMI_2K1K_60;
                    }
                } else if((cur_freq > 990) && (cur_freq < 1210)) {
                    if(Get_Val_vsc_run_pc_mode() == TRUE) {
...
void print_current_driver_pattern(void)
{
	...
	char *timing_string[] = {
        ...
        "HDMI_2K1K_240",         //hdmi timimng<=2K1K  framerate is over 120
        ...
        "HDMI_4K2K_240",        //hdmi timimng<=2K1K  framerate is over 120
		...
		do{
			...
			do{
				if(IoReg_Read32(VODMA_VODMA_CLKGEN_reg) & VODMA_VODMA_CLKGEN_en_fifo_full_gate_mask)
					...
					usleep_range(5000, 5000); /* delay 10 ms*/
