...
void drvif_get_LdInterface_BasicCtrl(DRV_LDINTERFACE_Basic_Ctrl *ptr)
{
	...
	ptr->in_data_align = ldspi_ld_ctrl_reg.in_data_align;
	ptr->out_data_align = ldspi_ld_ctrl_reg.out_data_align;
	ptr->ld_mode    = ldspi_ld_ctrl_reg.ld_mode;
	if(ldspi_ld_ctrl_reg.send_follow_vsync)
	    ptr->send_mode = VSYNC_MODE;
	else
	    ptr->send_mode = TRIGGER_MODE;    
	ptr->separate_two_block = ldspi_ld_ctrl_reg.separate_two_block;
	ptr->w_package_repeater = ldspi_as_wr_ctrl_reg.w_package_repeater;
	ptr->ld_backlight_timing_delay = ldspi_ld_spi_ld_timing_fune_tune_reg.ld_backlight_timing_delay;
	ptr->vsync_delay = ld_spi_v_sync_delay_reg.vsync_delay;
...
bool check_LDSPI_IndexSRAM_status_NULL(DRV_LD_LDSPI_INDEXSRAM_TYPE *LDSPI_IndexSRAM){
	...
	//rtd_pr_vpq_emerg("[VPQ_LED] check_LDSPI_IndexSRAM_status_NULL done\n");
	return false;
