...
FILM_FWMode DETECTION_scalerVIP_film_FWmode(void)
{
	...
	if(fw_di_film_sawtooth_comparePrevNext){
		...
		for(i=0;i<8;i++)
		{
			if( p22_StatusNextDiffSum[i][0]*48 < p22_StatusPrevDiffSum[i][0] )
			{
				StatusNextStaticPattern_film22_PTR[i][0] = StatusNextStaticPattern_film22_PTR[i][0] | 1;
				hw_StatusNextFieldMotion_film22_All = hw_StatusNextFieldMotion_film22_All & (~(1<<(7^i)));
				hw_StatusPrevFieldMotion_film22_All = hw_StatusPrevFieldMotion_film22_All | (1<<(7^i));
			}
			else
			if( p22_StatusNextDiffSum[i][0] > p22_StatusPrevDiffSum[i][0]*48 )
			{
				StatusNextStaticPattern_film22_PTR[i][0] = StatusNextStaticPattern_film22_PTR[i][0] & -2;
				hw_StatusNextFieldMotion_film22_All = hw_StatusNextFieldMotion_film22_All | (1<<(7^i));
				hw_StatusPrevFieldMotion_film22_All = hw_StatusPrevFieldMotion_film22_All & (~(1<<(7^i)));
			}
		}
...
*/
	...
	a_film_log5[a_film_log5_pointer*5  ] = FrameDiff;
	a_film_log5[a_film_log5_pointer*5+1] = PrevDiff;
	a_film_log5[a_film_log5_pointer*5+2] = NextDiff;
	a_film_log5[a_film_log5_pointer*5+3] = Status22Detected_cnt;
	a_film_log5[a_film_log5_pointer*5+4] = Status22Sequence;
	if(Status22Detected){a_film_log5[a_film_log5_pointer*5+3]+=100;}
	if(Status32Detected1){a_film_log5[a_film_log5_pointer*5+3]+=400;}
	if(Status32Detected2){a_film_log5[a_film_log5_pointer*5+3]+=200;}
	if(hw_film_Status22Detected){a_film_log5[a_film_log5_pointer*5+3]+=1000;}
	if(hw_film_Status32Detected1){a_film_log5[a_film_log5_pointer*5+3]+=4000;}
	if(hw_film_Status32Detected2){a_film_log5[a_film_log5_pointer*5+3]+=2000;}
	if(filmQuick_en){
...
#define bad_edit_test 0
	...
	*/
	...
	if(fw_MixedFilmConfuse){a_film_log5[a_film_log5_pointer*5+3]+=10000;}
	if(fw_StatusFilmDetected){a_film_log5[a_film_log5_pointer*5+3]+=40000;}
	if(fw_StatusMixedFilmDetected){a_film_log5[a_film_log5_pointer*5+3]+=20000;}
...
*/
	...
	if( hw_film_Status22Detected )
	{
		...
		//*/
		im_di_film_new_function_main_reg.fw_film_en = 0;
		// only known ujin 22 patterns need fw
			///reg_mpegnr_ich1_accessdata_posctrl_pk_reg.startx=0x140;
	}
	else
	// no teethth , need fw 22
	{
		im_di_film_new_function_main_reg.fw_film_en = 1;
			///reg_mpegnr_ich1_accessdata_posctrl_pk_reg.startx=0x140;
	}
	...
	*/
	if( im_di_film_new_function_main_reg.fw_film_doublebuffer_en )
	{
		...
		if( /*bak_fw_film_filmdetected && */!im_di_film_new_function_main_reg.fw_film_filmdetected )
		{
			im_di_film_new_function_main_reg.fw_film_doublebuffer_en = 0;
	...
	if( Status32Detected1_cnt>0||Status32Detected2_cnt>0||Status22Detected_cnt>0 )
	{
	...
	//if( im_di_film_new_function_main_reg.fw_film_en )
	{
		if( im_di_film_new_function_main_reg.fw_film_filmdetected )
		{
			fw_film_sugg_smd_off = 1;
		}
	}
	//else
	{
		if( hw_film_Status32Detected1||hw_film_Status32Detected2||hw_film_Status22Detected )
		{
			fw_film_sugg_smd_off = 1;
		}
	}
	if(fw_film_sugg_smd_off){a_film_log5[a_film_log5_pointer*5+4]+=10000;}
