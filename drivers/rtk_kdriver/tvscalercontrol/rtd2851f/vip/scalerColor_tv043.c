...
static void pq_rtd_part_outl(unsigned int reg_addr, unsigned int endBit, unsigned int startBit, unsigned int value)
{
...
extern unsigned char vpqled_Get_LD_lutTableIndex(void);
...
void fwif_color_set_LD_Boost_TV043(unsigned short *DutyLimit)
{
	...
	if (DutyLimit == NULL)
		return;
	LDTableIdx = vpqled_Get_LD_lutTableIndex();
...
void fwif_color_set_LD_To_LDSPI_Method_tv043(MOVE_LD_TO_LDSPI method)
{
	...
	if (method == MOVE_LD_TO_LDSPI_BYSW_120HZ) {
		...
		pq_rtd_part_outl(PINMUX_MAIN_GPIO_TLEFT_CFG_6_reg,31,28,0xD);
		ld_ctrl.send_follow_vsync = 0;
		IoReg_Write32(LDSPI_LD_CTRL_reg, ld_ctrl.regValue);

	} else if (method == MOVE_LD_TO_LDSPI_BYSW_60HZ){
		LD_Backlight_Final_Decision.ld_blu_spien = 0;	
		pq_rtd_part_outl(PINMUX_MAIN_GPIO_TLEFT_CFG_6_reg,31,28,0xF);
		ld_ctrl.send_follow_vsync = 0;
		IoReg_Write32(LDSPI_LD_CTRL_reg, ld_ctrl.regValue);

	} else {
		LD_Backlight_Final_Decision.ld_blu_spien = 1;
		pq_rtd_part_outl(PINMUX_MAIN_GPIO_TLEFT_CFG_6_reg,31,28,0x6);
