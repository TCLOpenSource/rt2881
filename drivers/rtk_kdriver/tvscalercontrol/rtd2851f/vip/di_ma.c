...
unsigned char DI_F2D_CTRL_DB_Apply_Wait = 0;
char drvif_color_DI_F2D_CTRL(unsigned char F2D_en)
{
	char ret = 0;
	di_db_reg_ctl_RBUS  db_reg_ctl_reg;
	...
	di_im_di_weave_window_control_reg.force2d = F2D_en;
	IoReg_Write32(DI_IM_DI_WEAVE_WINDOW_CONTROL_reg, di_im_di_weave_window_control_reg.regValue);

	IoReg_SetBits(DI_db_reg_ctl_reg, DI_db_reg_ctl_db_apply_mask);
	DI_F2D_CTRL_DB_Apply_Wait = 1;
	do {		
		usleep_range(20, 30);
		db_reg_ctl_reg.regValue = IoReg_Read32(DI_db_reg_ctl_reg);
		timeout--;
		//msleep(1);
	} while ((db_reg_ctl_reg.db_apply == 1) /*&& (DI_F2D_CTRL_DB_Apply_Wait == 1)*/ && (timeout > 0));

	if (timeout <= 0)
	{
		di_im_di_weave_window_control_reg.regValue = IoReg_Read32(DI_IM_DI_WEAVE_WINDOW_CONTROL_reg);
		di_im_di_weave_window_control_reg.force2d = 0;	// can't force2D in porch, skip force 2D
		IoReg_Write32(DI_IM_DI_WEAVE_WINDOW_CONTROL_reg, di_im_di_weave_window_control_reg.regValue);
		
		rtd_pr_vpq_emerg("DI_Enable, f2d_ctrl without isr flag, timeout=%d, apply=%d,DI_DMA_Reset_DB_Apply_Wait=%d,F2D_en=%d,\n", 
			timeout, db_reg_ctl_reg.db_apply, DI_F2D_CTRL_DB_Apply_Wait, F2D_en);
	...
	usleep_range(1000, 1200);	// skip front porch for DI_F2D_CTRL_DB_Apply_Wait, DI_F2D_CTRL_DB_Apply_Wait is clear on data end
	return ret;
}
...
void drvif_color_DI_Addr_Boundary_ini(unsigned int DI_addr_sta, unsigned int DI_addr_end, unsigned int DI_MASNR_addr_sta, unsigned int DI_MASNR_addr_end)
{
	...
	IoReg_SetBits(DI_db_reg_ctl_reg, DI_db_reg_ctl_db_apply_mask);	
	IoReg_SetBits(DI_db_reg_ctl_reg, DI_db_reg_ctl_db_apply_1_mask);
}
...
void drvif_color_set_Hor_Act_Start_time(unsigned short value)
{
	di_tr2to1_RBUS di_tr2to1_reg; //

	//set di back porch

	di_tr2to1_reg.regValue =rtd_inl(DI_tr2to1_reg);
	di_tr2to1_reg.tr2to1_dist = value;

	rtd_outl(DI_tr2to1_reg, di_tr2to1_reg.regValue);
