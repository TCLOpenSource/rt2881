...
//#ifdef CONFIG_DUAL_CHENNEL
	    ...
	    IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_read_mask); // db_read=1 => rbus buffer  // mac7p HW bug, set db_read=1 before sub db_en R/W
	    IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_en_mask); // sub DB en
		...
		IoReg_Write32(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, ich2_RGB2YUV_CTRL_REG.regValue);
	    IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_read_mask); // db_read=1 => rbus buffer  // mac7p HW bug, set db_read=1 before sub db_en R/W
	    IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_apply_mask); // sub DB apply
...
void drvif_color_inv_output_gamma_control_front(unsigned char display, unsigned char is_online)
{
...
{
#if 0	// use drvif_color_colorwrite_outGamma_enhance
	unsigned short ii, cnt = 10000;
...
char drvif_color_colorwrite_outGamma_enhance(unsigned int *pArray, unsigned int *pArray_index)
{
	...
	for (ii = 0; ii < (Bin_Num_Gamma>>1); ii++) {
		...
		if (cnt == 0) 
		{
			rtd_pr_vpq_emerg("drvif_color_colorwrite_outGamma_enhance ii=%d, time out\n", ii);
			ret = -1;
			break;
		}
	}
...
char drvif_color_Out_Gamma_Curve_Write_Enhance(unsigned int *pArray, unsigned int *pArray_index, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
	outputgamma_out_gamma_ctrl_1_RBUS out_gamma_ctrl_1_REG;
	out_gamma_ctrl_1_REG.regValue = IoReg_Read32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg);
	out_gamma_ctrl_1_REG.out_gamma_ax_ch_sel = RGB_chanel_idx;
	IoReg_Write32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg, out_gamma_ctrl_1_REG.regValue);
	return drvif_color_colorwrite_outGamma_enhance(pArray, pArray_index);
}

void drvif_color_Out_Gamma_Curve_Write(unsigned int *Gamma_Encode, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
#if 0	// use drvif_color_Out_Gamma_Curve_Write_Enhance
	outputgamma_out_gamma_ctrl_1_RBUS out_gamma_ctrl_1_REG;
	out_gamma_ctrl_1_REG.regValue = IoReg_Read32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg);
	out_gamma_ctrl_1_REG.out_gamma_ax_ch_sel = RGB_chanel_idx;
	IoReg_Write32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg, out_gamma_ctrl_1_REG.regValue);
...
char drvif_DM_HDR_3dLUT_16(unsigned char enable, unsigned short *pArray)/*flora@20150617, modify by juwen*/
{
	...
	threedlut_minmaxc2_reg.regValue = IoReg_Read32(DM_ThreeDLUT_MinMaxC2_reg);
	threedlut_minmaxc3_reg.regValue = IoReg_Read32(DM_ThreeDLUT_MinMaxC3_reg);
	//threedlut_minmaxinv1_reg.regValue = IoReg_Read32(DM_ThreeDLUT_MinMaxInv1_reg); // CInv set by drvif_color_Set_ST2094_3Dlut_CInv
	//threedlut_minmaxinv2_reg.regValue = IoReg_Read32(DM_ThreeDLUT_MinMaxInv2_reg);
	//threedlut_minmaxinv3_reg.regValue = IoReg_Read32(DM_ThreeDLUT_MinMaxInv3_reg);
	threedlut_minmaxc1_reg.regValue = 0x7fff0000;//0xffff0000;//MaxC1/MinC1
	if (VIP_system_info_structure_table ->HDR_flag == HAL_VPQ_HDR_MODE_HDR10_VIVID)
...
unsigned char drvif_color_set_HDR10_Enable_Seamless(unsigned char* enable)
{
	...
	if (enable[HDR_EN])
	{
		...
		dm_submodule_temp.b03_enable = 0;
		dm_submodule_temp.b04_enable = 0;
#if 1		/* FIXME: just for pass compiler */
		dm_submodule_temp.b01_06_enable = 0;
#else
		if (VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10_VIVID)
					dm_submodule_temp.b01_06_enable = 1;
...
void drvif_color_set_HDR_RGB_swap(unsigned char swap)
{
	...
	spin_lock_irqsave_vgip_reg(&flags);
	...
	spin_unlock_irqrestore_vgip_req(&flags);
}
...
*/
		...
		case PQ_ByPass_D_RGB_SHP:
			IoReg_ClearBits(COLOR_DLCTI_RGB_SHPNR_CTRL_RGB2Y_2_reg, COLOR_DLCTI_RGB_SHPNR_CTRL_RGB2Y_2_rgb_sharp_enable_mask);
			IoReg_SetBits(COLOR_D_VC_Global_CTRL_DB_reg, COLOR_D_VC_Global_CTRL_DB_db_load_mask);//_BIT0
		break;

		case PQ_ByPass_I_LineBuff:			
			IoReg_SetBits(IEDGE_SMOOTH_EdgeSmooth_EX_CTRL_reg, IEDGE_SMOOTH_EdgeSmooth_EX_CTRL_lbuf_disable_mask);			
