...
void drvif_color_set_od_Ymode(unsigned char Y_mode)
{
	...
	od_ymode_ctrl_reg.od_ymode_en = Y_mode;
...
char drvif_set_color_od_BND(unsigned long od_size, unsigned long od_addr, unsigned long vip_size, unsigned long vip_addr)
{
	...
	if (od_addr<vip_addr) {
		IoReg_Write32(OD_DMA_ODDMA_RD_Rule_check_up_reg, ((unsigned int)vip_addr) + vip_size - burst_length);
		...
		IoReg_Write32(OD_DMA_ODDMA_RD_Rule_check_low_reg, (unsigned int)vip_addr);
	}

	//IoReg_Write32(OD_DMA_ODDMA_RD_Rule_check_up_reg, ((unsigned int)od_addr) + od_size - 1024);
	//IoReg_Write32(OD_DMA_ODDMA_RD_Rule_check_low_reg, (unsigned int)od_addr);
	return 0;
}
	...
	UINT8 FrameLimitBit, int dataColor, int dataFormat, unsigned long od_size, unsigned long od_addr, unsigned char qp_mode)
{
	...
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_disp_db_en_mask);	// db enable;
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_cap_db_en_mask);	// db enable;

	VIPprintf("[OD] Gibi_od.cpp::drvif_color_od_pqc() calling...\n");

	if (od_addr == 0 || od_size == 0) {
		rtd_pr_vpq_emerg("[%s %d] ERR : %lx %lx\n", __func__, __LINE__, od_addr, od_size);
		return false;
	}
	...
	{
		rtd_pr_vpq_emerg("[OD] Gibi_od.cpp::input value conflict...\n");
		rtd_pr_vpq_emerg("420 or 400 doesn't support RGB compression\n");
		return false;
	}
	...
	*/
	...
	oddma_pq_cmp_reg.cmp_width_div32 = (width + 31) >> 5;
	...
	oddma_pq_decmp_reg.decmp_width_div32 = (width + 31) >> 5;
	...
	oddma_pq_cmp_pair_reg.cmp_qp_mode = qp_mode;
	oddma_pq_decmp_pair_reg.decmp_qp_mode = qp_mode;

    //both 120hz and 60hz panel use same setting
	oddma_pq_cmp_pair_reg.cmp_data_bit_width = (bit_sel>1)?(1):(0);	// 0;
	oddma_pq_decmp_pair_reg.decmp_data_bit_width = (bit_sel>1)?(1):(0);	// 0;
	...
	od_dma_oddma_wr_ctrl_reg.cap_en = (bODInited==true)?(1):(0);
	...
	od_dma_oddma_rd_ctrl_reg.disp_en = (bODInited==true)?(1):(0);
	od_dma_oddma_rd_ctrl_reg.disp_max_outstanding = 3;
	...
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_disp_db_apply_mask);	// db apply;
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_cap_db_apply_mask);	// db apply;
...
void drvif_set_color_od_NR(DRV_OD_NR_Mode_EN_Ctrl *ptr, DRV_OD_NR_Mode_Ctrl *ptrTH)
{
	...
	if (ptr == NULL || ptrTH == NULL) {
		rtd_pr_vpq_emerg("%s, null return.\n", __func__);
		return;
	}
	
	noise_reduction_ctrl_reg.regValue = IoReg_Read32(OD_od_Noise_reduction_ctrl_reg);
	noise_reduction_ctrl_reg.od_nr_mode = ptr->NR_mode_en;
	noise_reduction_ctrl_reg.od_b_nr_en = ptr->NR_B_en;
	noise_reduction_ctrl_reg.od_g_nr_en = ptr->NR_G_en;
	noise_reduction_ctrl_reg.od_r_nr_en = ptr->NR_R_en;
	noise_reduction_ctrl_reg.od_b_nr_thd = ptrTH->NR_B_th;
	noise_reduction_ctrl_reg.od_g_nr_thd = ptrTH->NR_G_th;
	noise_reduction_ctrl_reg.od_r_nr_thd = ptrTH->NR_R_th;
...
void drvif_set_color_od_DownSample(DRV_OD_down_sample_Ctrl *ptr)
{
	...
	od_od_hhf_reg.regValue = IoReg_Read32(OD_OD_HHF_reg);

	od_od_hhf_reg.od_hhf_en = ptr->OD_down_sample.hhf_en;	
	od_od_hhf_reg.od_hhf_max = ptr->OD_down_sample_LUT.hhf_max;
	od_od_hhf_reg.od_hhf_gain = ptr->OD_down_sample_LUT.hhf_gain;
	od_od_hhf_reg.od_hhf_th1 = ptr->OD_down_sample_LUT.hhf_th1;
	od_od_hhf_reg.od_hhf_th2 = ptr->OD_down_sample_LUT.hhf_th2;
	od_od_hhf_reg.od_hhf_w1 = ptr->OD_down_sample_LUT.hhf_w1;
	od_od_hhf_reg.od_hhf_w2 = ptr->OD_down_sample_LUT.hhf_w2;

	od_ctrl_reg.od_scaling_en = ptr->OD_down_sample.scaling_en;
	od_ctrl_reg.od_scaling_filter = ptr->OD_down_sample.scale_filter;

	IoReg_Write32(OD_OD_HHF_reg, od_od_hhf_reg.regValue);
...
void drvif_set_color_od_Y_mode_Bias_Ctrl(DRV_OD_Y_mode_Bias_Ctrl *ptr)
{
	...
	od_ymode_ctrl_reg.od_ymode_sat_en = ptr->OD_Y_mode_Bias_En.bias_en_Y_sat_en;
	od_ymode_sat_reg.sat_gray2rgb_en = ptr->OD_Y_mode_Bias_En.gray2rgb_en;
	od_ymode_sat_reg.sat_gray2cmy_en = ptr->OD_Y_mode_Bias_En.gray2cmy_en;
	od_ymode_sat_reg.sat_color2gray_en = ptr->OD_Y_mode_Bias_En.color2gray_en;

	od_ymode_sat_reg.sat_r_en = ptr->OD_Y_mode_Bias_En2.bias_R_en;
	od_ymode_sat_reg.sat_g_en = ptr->OD_Y_mode_Bias_En2.bias_G_en;
	od_ymode_sat_reg.sat_b_en = ptr->OD_Y_mode_Bias_En2.bias_B_en;
	od_ymode_sat_reg.sat_c_en = ptr->OD_Y_mode_Bias_En2.bias_C_en;
	od_ymode_sat_reg.sat_m_en = ptr->OD_Y_mode_Bias_En2.bias_M_en;
	od_ymode_sat_reg.sat_y_en = ptr->OD_Y_mode_Bias_En2.bias_Y_en;

	od_ymode_sat_reg.sat_y_diff_neg_en = ptr->OD_Y_mode_Bias_En3.Y_diff_neg_en;
	od_ymode_sat_reg.sat_y_diff_pos_en = ptr->OD_Y_mode_Bias_En3.Y_diff_pos_en; 
	od_ymode_sat_reg.sat_offset_clamp_en = ptr->OD_Y_mode_Bias_En3.offset_clamp_en; 
...
void drvif_set_color_od_Y_mode_Bias_Settings(DRV_OD_Y_mode_Bias_Settings *ptr)
{
	...
	//od_ymode_lpf_RBUS od_ymode_lpf_reg;
	...
	//od_ymode_lpf_reg.regValue = IoReg_Read32(OD_YMODE_LPF_reg);

	od_ymode_sat_reg.sat_r_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.R_diff_th;
	od_ymode_sat_reg.sat_g_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.G_diff_th;
	od_ymode_sat_reg.sat_b_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.B_diff_th;
	od_ymode_sat_th_reg.sat_rg_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.RG_diff_th;
	od_ymode_sat_th_reg.sat_gb_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.GB_diff_th;
	od_ymode_sat_th_reg.sat_br_diff_th = ptr->Y_mode_Bias_Settings_Diff_Th.BR_diff_th;

	od_ymode_sat_offset_gain_reg.sat_gray2r_offset_gain = ptr->Y_mode_Bias_Settings_Offset.R_offset;
	od_ymode_sat_offset_gain_reg.sat_gray2g_offset_gain = ptr->Y_mode_Bias_Settings_Offset.G_offset;
	od_ymode_sat_offset_gain_reg.sat_gray2b_offset_gain = ptr->Y_mode_Bias_Settings_Offset.B_offset;
	od_ymode_sat_offset_gain_reg.sat_gray2c_offset_gain = ptr->Y_mode_Bias_Settings_Offset.C_offset;
	od_ymode_sat_offset_gain_reg.sat_gray2m_offset_gain = ptr->Y_mode_Bias_Settings_Offset.M_offset;
	od_ymode_sat_offset_gain_reg.sat_gray2y_offset_gain = ptr->Y_mode_Bias_Settings_Offset.Y_offset;

	od_ymode_sat_th_reg.sat_diff_th_factor  = ptr->Y_mode_Bias_Settings_Sat.sat_diff_factor;
	od_ymode_sat_th_reg.sat_color2gray_th  = ptr->Y_mode_Bias_Settings_Sat.sat_color2gray_th;
	od_ymode_sat_offset_gain_reg.sat_color2gray_offset_gain  = ptr->Y_mode_Bias_Settings_Sat.sat_color2gray_offset_gain;
	
	od_ymode_sat_y_reg.y_diff_lo_th = ptr->Y_mode_Bias_Settings_Low.ydiff_low_th;
	od_ymode_sat_y_reg.y_diff_hi_th = ptr->Y_mode_Bias_Settings_Low.ydiff_high_th;
	od_ymode_sat_y_reg.offset_lo_th = ptr->Y_mode_Bias_Settings_Low.offset_low_th;
	od_ymode_sat_y_reg.offset_hi_th = ptr->Y_mode_Bias_Settings_Low.offset_high_th;
	od_ymode_sat_y_slope_reg.offset_slope = ptr->Y_mode_Bias_Settings_Low.offset_slope;


	od_ymode_sat_y_color2gray_reg.y_diff_lo_th_color2gray = ptr->Y_mode_Bias_Settings_Low_gray.ydiff_low_th_gray;
	od_ymode_sat_y_color2gray_reg.y_diff_hi_th_color2gray = ptr->Y_mode_Bias_Settings_Low_gray.ydiff_high_th_gray;
	od_ymode_sat_y_color2gray_reg.offset_lo_th_color2gray = ptr->Y_mode_Bias_Settings_Low_gray.offset_low_th_gray;
	od_ymode_sat_y_color2gray_reg.offset_hi_th_color2gray = ptr->Y_mode_Bias_Settings_Low_gray.offset_high_th_gray;
	od_ymode_sat_y_slope_reg.offset_slope_color2gray = ptr->Y_mode_Bias_Settings_Low_gray.offset_slop_gray;
	...
	//IoReg_Write32(OD_YMODE_LPF_reg, od_ymode_lpf_reg.regValue);
		
}

void drvif_set_color_od_Vertical_Gain_ctrl(DRV_OD_Vertical_Gain_ctrl *ptr)
{
	od_od_vertical_gain_setting_RBUS od_od_vertical_gain_setting_reg;
	od_od_vertical_gain_table0_RBUS od_od_vertical_gain_table0_reg;
	od_od_vertical_gain_table1_RBUS od_od_vertical_gain_table1_reg;
	od_od_vertical_gain_table2_RBUS od_od_vertical_gain_table2_reg;
	od_od_vertical_gain_table3_RBUS od_od_vertical_gain_table3_reg;
	od_od_vertical_gain_table4_RBUS od_od_vertical_gain_table4_reg;
	od_od_vertical_gain_table5_RBUS od_od_vertical_gain_table5_reg;
	od_od_vertical_gain_table6_RBUS od_od_vertical_gain_table6_reg;
	od_od_vertical_gain_table7_RBUS od_od_vertical_gain_table7_reg;
	od_od_vertical_gain_debug_mode_RBUS od_od_vertical_gain_debug_mode_reg;
	...
	od_od_vertical_gain_setting_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_SETTING_reg);
	od_od_vertical_gain_table0_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table0_reg);
	od_od_vertical_gain_table1_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table1_reg);
	od_od_vertical_gain_table2_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table2_reg);
	od_od_vertical_gain_table3_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table3_reg);
	od_od_vertical_gain_table4_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table4_reg);
	od_od_vertical_gain_table5_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table5_reg);
	od_od_vertical_gain_table6_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table6_reg);
	od_od_vertical_gain_table7_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_table7_reg);
	od_od_vertical_gain_debug_mode_reg.regValue = IoReg_Read32(OD_OD_VERTICAL_GAIN_DEBUG_MODE_reg);
	
	od_od_vertical_gain_setting_reg.vergain_en = ptr->Vertical_Gain_General.V_en;
	od_od_vertical_gain_setting_reg.vergain_factor = ptr->Vertical_Gain_General.V_factor;
	od_od_vertical_gain_debug_mode_reg.vergain_offset = ptr->Vertical_Gain_General.V_offset;

	od_od_vertical_gain_table0_reg.vergain_table0 = ptr->Vertical_Gain_V0_15.V_gain0;
	od_od_vertical_gain_table0_reg.vergain_table1 = ptr->Vertical_Gain_V0_15.V_gain1;
	od_od_vertical_gain_table0_reg.vergain_table2 = ptr->Vertical_Gain_V0_15.V_gain2;
	od_od_vertical_gain_table0_reg.vergain_table3 = ptr->Vertical_Gain_V0_15.V_gain3;
	od_od_vertical_gain_table1_reg.vergain_table4 = ptr->Vertical_Gain_V0_15.V_gain4;
	od_od_vertical_gain_table1_reg.vergain_table5 = ptr->Vertical_Gain_V0_15.V_gain5;
	od_od_vertical_gain_table1_reg.vergain_table6 = ptr->Vertical_Gain_V0_15.V_gain6;
	od_od_vertical_gain_table1_reg.vergain_table7 = ptr->Vertical_Gain_V0_15.V_gain7;
	od_od_vertical_gain_table2_reg.vergain_table8 = ptr->Vertical_Gain_V0_15.V_gain8;
	od_od_vertical_gain_table2_reg.vergain_table9 = ptr->Vertical_Gain_V0_15.V_gain9;
	od_od_vertical_gain_table2_reg.vergain_table10 = ptr->Vertical_Gain_V0_15.V_gain10;
	od_od_vertical_gain_table2_reg.vergain_table11 = ptr->Vertical_Gain_V0_15.V_gain11;
	od_od_vertical_gain_table3_reg.vergain_table12 = ptr->Vertical_Gain_V0_15.V_gain12;
	od_od_vertical_gain_table3_reg.vergain_table13 = ptr->Vertical_Gain_V0_15.V_gain13;
	od_od_vertical_gain_table3_reg.vergain_table14 = ptr->Vertical_Gain_V0_15.V_gain14;
	od_od_vertical_gain_table3_reg.vergain_table15 = ptr->Vertical_Gain_V0_15.V_gain15;
	
	od_od_vertical_gain_table4_reg.vergain_table16 = ptr->Vertical_Gain_V16_31.V_gain16;
	od_od_vertical_gain_table4_reg.vergain_table17 = ptr->Vertical_Gain_V16_31.V_gain17;
	od_od_vertical_gain_table4_reg.vergain_table18 = ptr->Vertical_Gain_V16_31.V_gain18;
	od_od_vertical_gain_table4_reg.vergain_table19 = ptr->Vertical_Gain_V16_31.V_gain19;
	od_od_vertical_gain_table5_reg.vergain_table20 = ptr->Vertical_Gain_V16_31.V_gain20;
	od_od_vertical_gain_table5_reg.vergain_table21 = ptr->Vertical_Gain_V16_31.V_gain21;
	od_od_vertical_gain_table5_reg.vergain_table22 = ptr->Vertical_Gain_V16_31.V_gain22;
	od_od_vertical_gain_table5_reg.vergain_table23 = ptr->Vertical_Gain_V16_31.V_gain23;
	od_od_vertical_gain_table6_reg.vergain_table24 = ptr->Vertical_Gain_V16_31.V_gain24;
	od_od_vertical_gain_table6_reg.vergain_table25 = ptr->Vertical_Gain_V16_31.V_gain25;
	od_od_vertical_gain_table6_reg.vergain_table26 = ptr->Vertical_Gain_V16_31.V_gain26;
	od_od_vertical_gain_table6_reg.vergain_table27 = ptr->Vertical_Gain_V16_31.V_gain27;
	od_od_vertical_gain_table7_reg.vergain_table28 = ptr->Vertical_Gain_V16_31.V_gain28;
	od_od_vertical_gain_table7_reg.vergain_table29 = ptr->Vertical_Gain_V16_31.V_gain29;
	od_od_vertical_gain_table7_reg.vergain_table30 = ptr->Vertical_Gain_V16_31.V_gain30;
	od_od_vertical_gain_table7_reg.vergain_table31 = ptr->Vertical_Gain_V16_31.V_gain31;
	
	IoReg_Write32(OD_OD_VERTICAL_GAIN_SETTING_reg, od_od_vertical_gain_setting_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table0_reg, od_od_vertical_gain_table0_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table1_reg, od_od_vertical_gain_table1_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table2_reg, od_od_vertical_gain_table2_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table3_reg, od_od_vertical_gain_table3_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table4_reg, od_od_vertical_gain_table4_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table5_reg, od_od_vertical_gain_table5_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table6_reg, od_od_vertical_gain_table6_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_table7_reg, od_od_vertical_gain_table7_reg.regValue);
	IoReg_Write32(OD_OD_VERTICAL_GAIN_DEBUG_MODE_reg, od_od_vertical_gain_debug_mode_reg.regValue);
...
void drvif_color_od(unsigned char bOD)
{
	...
	IoReg_SetBits(OD_OD_CTRL1_reg, OD_OD_CTRL1_od_en_dbf_en_mask);	// db enable;
	...
	if (bOD == 0) {
		...
		od_ctrl_reg.od_en = (bODInited==true)?(1):(0);
	}
	IoReg_Write32(OD_OD_CTRL_reg, od_ctrl_reg.regValue);
	...
	IoReg_SetBits(OD_OD_CTRL1_reg, OD_OD_CTRL1_db_apply_mask);	// db apply;
...
void drvif_color_od_table(unsigned int *pODtable, unsigned char targetmode)
{
	...
	*/
	lut_addr_reg.regValue = IoReg_Read32(OD_OD_LUT_ADDR_reg);	
	lut_addr_reg.od_lut_dma_read = 0; // 0: rbus, 1:dbus
	...
	/* disable table access*/
	IoReg_ClearBits(OD_OD_LUT_ADDR_reg, OD_OD_LUT_ADDR_od_lut_ax_en_mask);
}
...
int drvif_color_wait_od_dma_lut_set_done(unsigned int WaitCnt){
	unsigned char od_dma_lut_done_flag;
	unsigned int Counter = WaitCnt;
	int Ret=0;
#ifndef BUILD_QUICK_SHOW	// lut dma is no need on qs
	do {
		usleep_range(500, 1000);
		od_dma_lut_done_flag = OD_OD_status_get_dma_read_table(IoReg_Read32(OD_OD_status_reg));		
		Counter--;

	} while ((od_dma_lut_done_flag != 0) && (Counter>0));

	if(Counter==0 && od_dma_lut_done_flag==1)
		Ret=-1;
#endif	
	return Ret;		
...
{	
	IoReg_SetBits(OD_OD_status_reg, OD_OD_status_dma_read_table_mask);
}
...
void drvif_color_od_table_17x17_set_dbus(unsigned char tableType, unsigned int lut_addr, unsigned int lut_size)
{
	...
	lut_addr_reg.od_lut_dma_read = 1; // 0: rbus, 1:dbus
	lut_addr_reg.table_type = tableType; // 0: delta, 1: target
	lut_addr_reg.od_lut_dma_sel = 0; // 0: sram0~2
	lut_addr_reg.od_lut_ax_en = 1;
	lut_addr_reg.od_lut_ax_mode = 0;
	lut_addr_reg.od_lut_adr_mode = 0; // col(cur) indexfirst
	lut_addr_reg.od_lut_column_addr = 0;
...
unsigned char drvif_color_set_od_dma_enable(unsigned char enable)
{
	...
	// b802ce08 cap enable
	od_dma_oddma_wr_ctrl_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_WR_Ctrl_reg);
	od_dma_oddma_wr_ctrl_reg.cap_en = (bODInited==true)?(enable):(0);
	IoReg_Write32(OD_DMA_ODDMA_WR_Ctrl_reg, od_dma_oddma_wr_ctrl_reg.regValue);

	// b802ce88 disp enable
	od_dma_oddma_rd_ctrl_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_RD_Ctrl_reg);
	od_dma_oddma_rd_ctrl_reg.disp_en = (bODInited==true)?(enable):(0);
	IoReg_Write32(OD_DMA_ODDMA_RD_Ctrl_reg, od_dma_oddma_rd_ctrl_reg.regValue);
