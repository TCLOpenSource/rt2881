...
unsigned char scaler_get_I2D_tracking_in_realcinema(void);
void dma_vgip_pre_setting_for_dma_online_measure(VSC_INPUT_TYPE_T inputSrctype);
...
bool IsPcModeTiming(MEASURE_TIMING_T *info);
...
void set_i3ddma_vdelay_buffer(VIDEO_WID_T wId, UINT8 buffer);
...
unsigned int Get_fake_scaler_lowdelay_impl(void);
...
void spin_lock_irqsave_vgip_reg(unsigned long *flags);
void spin_unlock_irqrestore_vgip_req(unsigned long *flags);
