...
static unsigned char s_enable_print_edid[HDMI_PORT_TOTAL_NUM];

// save standard edid.
HDMI_EDID_STATUS_T m_standard_edid[HDMI_PORT_TOTAL_NUM];
...
{// 1: save success, 0: save fail 
    ...
    else
    {
        ...
        m_edid_status[port].current_edid_length = len;
...
void newbase_hdmi_sync_qs_edid(unsigned char port)
{
    unsigned int edid_len = 0;
    unsigned char edid[MAX_EDID_LENGTH] = {0};
    HDMI_CHANNEL_T* p_ch = newbase_hdmi_get_pcbinfo(port);

    HDMI_EDID_INFO("[%s] Start !!\n", __func__);
    if (p_ch == NULL || p_ch->phy_port == HDMI_PHY_PORT_NOTUSED)
    {
        HDMI_EDID_INFO("%s: sync port %d failed - port not used, skip it\n", __func__, port);
        return;
    }

    newbase_hdmi_read_edid(port, (unsigned char *)edid, MAX_EDID_LENGTH);

    // 7Eh 1 Byte Extension Flag, Number of (optional) 128-byte EDID
    // extension blocks to follow.
    edid_len = (edid[126] + 1)*128;
    newbase_hdmi_save_edid(port, edid, edid_len);
    m_edid_status[port].edid_capability = newbase_hdmi_check_edid_capability(edid, edid_len);
    HDMI_EDID_INFO("%s Port=%d, Checksum=(%02x,%02x,%02x), capability=0x%x, len=%u\n",
        __func__, port, edid[127], edid[255], edid[383], m_edid_status[port].edid_capability, \
        m_edid_status[port].current_edid_length);

    HDMI_EDID_INFO("[%s] End !!\n", __func__);
}
...
void newbase_hdmi_edid_init(void)
{
    ...
    for (j=0; j<HDMI_PORT_TOTAL_NUM; j++)
    {
        ...
        if(!newbase_hdmi_write_edid(j, p_edid_table, m_edid_status[j].current_edid_length))
        {
...
void newbase_hdmi_edid_dump(unsigned char port)
{
    unsigned int i =0;
    unsigned int edid_len = 0;
    unsigned char *p = NULL;

    if ((port >= HDMI_PORT_TOTAL_NUM) || (m_edid_status[port].current_edid_length > MAX_EDID_LENGTH))
        return;

    edid_len = m_edid_status[port].current_edid_length;
    for (i=0;(i+15)<edid_len;i+=16)
    {
        p = m_edid_status[port].current_edid_table + i;
        HDMI_EDID_INFO("0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X\n",
            *p, *(p+1), *(p+2), *(p+3), *(p+4), *(p+5), *(p+6), *(p+7), \
            *(p+8), *(p+9), *(p+10), *(p+11), *(p+12), *(p+13), *(p+14), *(p+15));
    }
}

void newbase_hdmi_edid_enable_print_edid(unsigned char port)
{
    HDMI_INFO("%s, port=%d\n", __func__, port);
    s_enable_print_edid[port] = 1;
}

void newbase_hdmi_edid_reset_print_edid(unsigned char port)
{
    s_enable_print_edid[port] = 0;
}

unsigned char newbase_hdmi_edid_is_print_edid_enabled(unsigned char port)
{
    return s_enable_print_edid[port];
}
