...
{
    unsigned char port = newbase_hdmi_get_current_display_port();
    unsigned char is_avmute = lib_hdmi_get_avmute(port);
    unsigned char bch_err =lib_hdmi_get_bch_2bit_error(port);

    if ((is_avmute == 0) && !newbase_hdmi_is_avmute_time_over_thd(port))
        is_avmute = 1;

    return (is_avmute && !bch_err);
}
...
/*------------------------------------------------------
 * Func : drvif_Hdmi_Set_WatchDog_debug_mode
 *
 * Desc : Enable/Disable HW watchdog debug mode/target
 *
 * Parm : enable=1, Enable WD debug mode and set target; enable=0, Clear WD debug mode and set target
 *
 * Retn : N/A  :
 *------------------------------------------------------*/
void drvif_Hdmi_Set_WatchDog_debug_mode(unsigned char enable)
{
    unsigned char port = newbase_hdmi_get_current_display_port();

    newbase_hdmi_set_wd_debug_mode(port, enable);
}

/*------------------------------------------------------
 * Func : drvif_Hdmi_get_WatchDog_debug_mode
 *
 * Desc : get HW watchdog debug mode/target status
 *
 * Parm : N/A
 *
 * Retn : 1 : debug mode enable
 *        0 : debug mode disable
 *------------------------------------------------------*/
unsigned char drvif_Hdmi_get_WatchDog_debug_mode(void)
{
    unsigned char port = newbase_hdmi_get_current_display_port();

    return newbase_hdmi_get_wd_debug_mode(port);
}
...
unsigned char drvif_Hdmi_GetVsemIsDolby(unsigned char port)
{
    return newbase_hdmi_get_vsem_is_dolby(port);
}
