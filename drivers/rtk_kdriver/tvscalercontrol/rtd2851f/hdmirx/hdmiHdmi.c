...
unsigned long m_avmute_time[HDMI_PORT_TOTAL_NUM] = {HDMI_MAX_SYSTEM_TIME};
...
#define HDMI_DPM_MONITOR_NO_TMDS_CLK_THD        300
#define HDMI_DPM_MONITOR_TMDS_CLK_STABLE_THD    (10)

typedef enum {
	HDMI_DPM_DISABLE = 0,
	HDMI_DPM_ENABLE = 0x1,
	HDMI_DPM_WAKEUP_WHEN_TMDS_CLK_DETECTED = 0x2,
	HDMI_DPM_IGNORE_NO_TMDS_CLK_AT_BEGIN = 0x3,  // After raising HPD, there will be a period of stable signal, during which it cannot be considered
} ENUM_HDMI_DPM_TYPE;

typedef struct {
	unsigned char dpm_detect; // Record the hdmi detection command sent by the AP
	unsigned char record_5v_status;  // record the 5V status when AP start hdmi detection
	unsigned char record_is_valid_tmds_clk;   // record the tmds clock when AP start hdmi detection

	unsigned char no_tmds_clk_cnt;
	unsigned char start_dpm;  // 0 --> DPM disable; 1 -> DPM enabled and wake up when tmd clk is detected. 
	unsigned char is_should_wakeup;
} HDMI_DPM_T;

extern HDMI_EDID_STATUS_T m_standard_edid[HDMI_PORT_TOTAL_NUM];
...
// dpm
unsigned char hdmi_fake_wake_up;
unsigned char hdmi_dpm_start_monitor_signal = 0;
HDMI_DPM_T hdmi_dpm[HDMI_PORT_TOTAL_NUM];
/**********************************************************************************************
*
*	Const Declarations
*
...
static int _hdmi_interrupt_handler(unsigned char port)
{
	...
	{
		if(GET_FLOW_CFG(HDMI_FLOW_CFG_IRQ_HANDLE, HDMI_FLOW_CFG8_HDMI_WD_DEBUG_MODE)){
			if(newbase_hdmi_is_dispsrc(SLR_MAIN_DISPLAY)){
				// when scaler 18028004[14] hdmi_from_main set 1
				// and set wd debug mode 1 will force mute (180280F8[31] raising)
				// set wd debug mode 0 clr 180280F8[31]
				newbase_hdmi_set_wd_debug_mode(port,1);
			}
		}
		...
		if(GET_FLOW_CFG(HDMI_FLOW_CFG_IRQ_HANDLE, HDMI_FLOW_CFG8_HDMI_WD_DEBUG_MODE)){
			if(lib_hdmi_get_wd_debug_mode(port)){
				// when scaler 18028004[14] hdmi_from_main set 1
				// and set wd debug mode 1 will force mute (180280F8[31] raising)
				// set wd debug mode 0 clr 180280F8[31]
				lib_hdmi_set_wd_debug_mode(port,0);
			}
		}
...
#ifndef BUILD_QUICK_SHOW
void newbase_hdmi_qs_set_irq_status(unsigned char port)
{
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_AVMUTE_IRQ_ENABLE))
	{
		if (1)
...
void newbase_hdmi_qs_init(unsigned char port)
{
	...
	hdmi_rx[port].cable_conn = newbase_hdmi_get_5v_state(port) \
	...
	is_interlace = newbase_hdmi_get_is_interlace_reg(port, MODE_TMDS, HDMI_MS_ONESHOT);
	...
	newbase_hdmi_qs_set_irq_status(port);
	_init_hpd_delay_wq();

}
#endif

void newbase_hdmi_spin_lock_init(void)
{
...
void newbase_hdmi_qs_init_source(unsigned char port)
{
	...
	if (is_QS_hdmi_enable() && (port == get_QS_portnum())){
...
void newbase_hdmi_port_var_init(unsigned char port, unsigned char full_reset)
{
	...
	/* reset avmute status */
	m_avmute_time[port] = HDMI_MAX_SYSTEM_TIME;
}
...
unsigned char newbase_hdm_is_timing_change(unsigned char port, MEASURE_TIMING_T* last_timing)
{
    ...
    if(1)
    {
        ...
        if (HDMI_ABS(current_measure_tm.v_freq, last_timing->v_freq) > v_freq_tol)
        {
            ...
            if(hdmi_rx[port].spd_t.type_code !=0 && newbase_hdmi_check_freesync_info_version(&hdmi_rx[port].spd_t) !=  FREESYNC_INFO_INVALID){
                HDMI_WARN("[newbase_hdm_is_timing_change] get freesync version ,do not check vfreq\n");
                result = FALSE;
            }else{
                result |= TRUE;
            }
...
unsigned char video_fsm_hdmi_display_on_proccess(unsigned char port, unsigned char frl_mode)
{
	if (newbase_hdmi_display_on(port, frl_mode)) {
		...
		{	
			...
			Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_HDMI, SOURCE_GOOD_TIMING_READY);
#endif
			newbase_hdmi_enable_char_bit_debug_print_step(port);
			m_avmute_time[port] = HDMI_MAX_SYSTEM_TIME;
			newbase_hdmi_edid_enable_print_edid(port);
		}
...
int newbase_hdmi_enable_hdirq(unsigned char port)
{
	...
	if(p_phy_st==NULL)
	{
		HDMI_EMG("%s, p_phy_st NULL Pointer \n", __func__);
		return -1;
	}
	...
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
	{
		HDMI_INFO("%s[%d] is_fw_mode=%d, current_auto_mode_result=%d， hdirq_en = %d\n", __func__, port, lib_hdmi_is_modedet_fwmode(port), lib_hdmi_get_hdmi_mode_reg(port), lib_hdmi_get_hdirq_en(port));
		if (!hdmi_rx[port].hdmi_2p1_en || !lib_hdmi_get_hdirq_en(port))
		{
			if(lib_hdmi_is_need_toggle_hdirq_en(port) == 1)
			{
				lib_hdmi_set_hdmi_scpu_irq_enable(0);
				lib_hdmi_toggle_hdirq_en(port);
			}

			lib_hdmi_set_hdirq_en(port, 1);
			if(lib_hdmi_is_irq_pending(port))
			{
				HDMI_WARN("HDMI[%d]  FSM=%s(%d), is irq pending ,clr pending bit\n", port, _hdmi_video_fsm_str(GET_H_VIDEO_FSM(port)), GET_H_VIDEO_FSM(port));
				lib_hdmi_clr_irq_pending(port);
			}

			if(lib_hdmi_is_need_toggle_hdirq_en(port) == 1)
				lib_hdmi_set_hdmi_scpu_irq_enable(1);
		}
	}
...
unsigned char newbase_hdmi_detect_mode(unsigned char port)
{
	...
	if((g_detect_fsm_status[port].execute_action&HDMI_EVENT_ACTION_RESET_TO_WAIT_SYNC) == HDMI_EVENT_ACTION_RESET_TO_WAIT_SYNC)
	{
		...
		SET_H_VIDEO_FSM(port, MAIN_FSM_HDMI_WAIT_SYNC);
		newbase_hdmi_reset_char_bit_debug_print_step(port);
		newbase_hdmi_edid_reset_print_edid(port);
...
HDMI_CONDITION_CHG_TYPE newbase_hdmi_check_condition_change(unsigned char port)
{
	...
	if((GET_H_VIDEO_FSM(port) < MAIN_FSM_HDMI_VIDEO_READY) &&  (GET_H_RUN_VRR(port) != (newbase_hdmi_get_vrr_enable(port) |newbase_hdmi_get_freesync_enable(port) |newbase_hdmi_get_qms_enable(port)) ))
...
void newbase_hdmi_set_video_common_sel(unsigned char port)
{
	...
	if(timing_pixel_clock<= single_pixel_mode_clk_thd)
...
unsigned char newbase_hdmi_check_mode(void)
{
	...
	if (newbase_hdmi_edid_is_print_edid_enabled(port))
	{
		newbase_hdmi_edid_dump(port);
		newbase_hdmi_edid_reset_print_edid(port);
	}
...
void newbase_hdmi2p0_check_tmds_config(unsigned char port)
{
	...
	unsigned char tmds_toggle_retry = (hdmi_rx[port].hdmi2p0_tmds_scdc_config_update_count) ? 5 : 1;
...
void newbase_hdmi2p0_inc_scdc_toggle(unsigned char port)
{
	...
	HDMI_INFO("[newbase_hdmi2p0_inc_scdc_toggle][%d] inc tmds toggle=%d\n", port, hdmi_rx[port].hdmi2p0_tmds_toggle_flag);
}
...
unsigned int m_pre_tmds_phy_clk[4] = {0, 0, 0, 0}; // for store previous tmds clk
unsigned int m_print_tmds_phy_clk_value[4] = {0, 0, 0, 0}; // for print frequency
unsigned int m_print_tmds_phy_clk_thd[4] = {0, 0, 0, 0}; // for print frequency

#define TMDS_VALID_CLK_MIN    220 // HDMI MIN 25M, b_clk=255
#define TMDS_VALID_CLK_MAX    6000 //HDMI MAX 594M 
#define TMDS_CLK_PRINT_THD    200
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    tmds_clk = HDMI_ABS(m_pre_tmds_phy_clk[port], hdmi_rx[port].raw_tmds_phy_md_clk);
    if((GET_FLOW_CFG(HDMI_FLOW_CFG_DFE, HDMI_FLOW_CFG5_TMDS_CLK_CHANGE_THD)!=0) &&
        (tmds_clk >=GET_FLOW_CFG(HDMI_FLOW_CFG_DFE, HDMI_FLOW_CFG5_TMDS_CLK_CHANGE_THD)) &&
        (frl_mode == MODE_TMDS))
    {// print TMDS clock change for debugging
        if (tmds_clk >= TMDS_CLK_PRINT_THD)
        {
        ...
        if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_CHRGB_CHECK_EN)&& is_wait_enough_time)
        {
            ...
            *--------------------------------------------------------------*/
            ...
            if (hdmi_rx[port].ch_fifo_error_tomeout > fifo_error_timeout || hdmi_rx[port].rgb_async_fifo_error_timeout > fifo_error_timeout)
            {
                ...
                {
                    rest_phy = 0;
                    ERP_WARN("HDMI[p%d] TMDS Buffer timeout, EARC working, do nothing! (ch_fifo_error_tomeout=%d/rgb_async_fifo_error_timeout=%d, fw_bit_disp_error_overthd_cnt=%d) Reset Phy\n",
                        port, hdmi_rx[port].ch_fifo_error_tomeout, hdmi_rx[port].rgb_async_fifo_error_timeout, hdmi_rx[port].fw_bit_disp_error_overthd_cnt);
                }
                else
                {
                    if((hdmi_rx[port].last_fw_bit_error[0]>HDMI_SCRAMBLE_BIT_ERROR_RECOVER_THD)&&
                        (hdmi_rx[port].last_fw_bit_error[1]>HDMI_SCRAMBLE_BIT_ERROR_RECOVER_THD)&&
                        (hdmi_rx[port].last_fw_bit_error[2]>HDMI_SCRAMBLE_BIT_ERROR_RECOVER_THD)&&
                        (hdmi_rx[port].fw_bit_disp_error_overthd_cnt >=HDMI_SCRAMBLE_BIT_ERROR_DETECT_MIN && hdmi_rx[port].fw_bit_disp_error_overthd_cnt<=HDMI_SCRAMBLE_BIT_ERROR_DETECT_MAX))
                    {// SCDC scramble error detected
                        newbase_hdmi2p0_inc_scdc_toggle(port);
                        rest_phy = 1;
                        ERP_WARN("HDMI[p%d] TMDS Scramble error detected!! (ch_fifo_error_tomeout=%d/rgb_async_fifo_error_timeout=%d, fw_bit_disp_error_overthd_cnt=%d) Reset Phy\n",
                            port, hdmi_rx[port].ch_fifo_error_tomeout, hdmi_rx[port].rgb_async_fifo_error_timeout, hdmi_rx[port].fw_bit_disp_error_overthd_cnt);
                    ...
                    {
                        rest_phy = 1;
                        ERP_WARN("HDMI[p%d] TMDS Buffer timeout (ch_fifo_error_tomeout=%d/rgb_async_fifo_error_timeout=%d, fw_bit_disp_error_overthd_cnt=%d) Reset Phy\n",
                            port, hdmi_rx[port].ch_fifo_error_tomeout, hdmi_rx[port].rgb_async_fifo_error_timeout, hdmi_rx[port].fw_bit_disp_error_overthd_cnt);
                    }
                }
	...
	if (port == newbase_hdmi_get_current_display_port())
	{
		...
		if (hdmi_rx[port].is_avmute  != mute_on)
		{
			...
			if (mute_on || !newbase_hdmi_is_avmute_time_over_thd(port))
			{
				m_avmute_time[port] = hdmi_get_system_time_ms();
				HDMI_INFO("[%s port:%d] Delay %d ms to clear avmute\n", __func__, port, AVMUTE_ON_TO_OFF_TIME_THD);
			}
		}
	}
    }
...
void newbase_hdmi_connect_reset_all(unsigned char port, unsigned char hdmi_connect)
{
	...
	else
	{
		...
		if (newbase_hdmi_ddc_channel_status(port , &ddc_ch) == 0) //always pass, no need to judge in this case
		{
			lib_hdmiddc_enable(ddc_ch, 0);
...
unsigned char newbase_hdmi_is_avmute_time_over_thd(unsigned char port)
{
	const unsigned long kMaxTimeMs = HDMI_MAX_SYSTEM_TIME/90;
	unsigned long time = 0;

	if (m_avmute_time[port] == HDMI_MAX_SYSTEM_TIME)
		return 1;

	time = hdmi_get_system_time_ms();
	if (time >= m_avmute_time[port])
	{
		time -= m_avmute_time[port];
		if (time >= AVMUTE_ON_TO_OFF_TIME_THD)
			return 1;
	}
	else
	{
		time = kMaxTimeMs- m_avmute_time[port];
		if (time >= AVMUTE_ON_TO_OFF_TIME_THD)
			return 1;
		time = AVMUTE_ON_TO_OFF_TIME_THD - time;
		if (hdmi_get_system_time_ms() >= time)
			return 1;
	}
...
void newbase_hdmi_set_wd_debug_mode(unsigned char port, unsigned char enable)
{
	if (GET_H_VIDEO_FSM(port) != MAIN_FSM_HDMI_VIDEO_READY) {
		HDMI_WARN("[p%d] %s video state incorrect (%d)\n", port, __func__, GET_H_VIDEO_FSM(port));
		return;
	...
	lib_hdmi_set_wd_debug_mode(port, enable);
}


unsigned char newbase_hdmi_get_wd_debug_mode(unsigned char port)
{
	return lib_hdmi_get_wd_debug_mode(port);
}
...
unsigned char newbase_hdmi_check_stable_before_display(void)
{
	...
	newbase_hdmi_enable_hdirq(port);
...
#if (!defined UT_flag) && (!defined BUILD_QUICK_SHOW)
unsigned char newbase_hdmi_set_5v_wakeup(unsigned int port, unsigned char enable)
{
	HDMI_CHANNEL_T* p_info = NULL;
	unsigned int val = 0;

	HDMI_INFO("%s [port:%u] enable=%d\n", __func__, port, enable);

	if (port >= HDMI_PORT_TOTAL_NUM)
		return 0;

	p_info = newbase_hdmi_get_pcbinfo(port);
	if (!p_info->valid_port)
		return 0;
	...
	if (enable)
		val |= 0x1;
	port |= 0x100; // Mark as HDMI 5V wakeup.
	emcu_set_hdmi_det(port, val);
	return 1;
}

void newbase_hdmi_dpm_write_fhd_edid(unsigned char port)
{
#define HDMI_FHD_EDID_SIZE			256

	unsigned char kEdid_fhd[HDMI_FHD_EDID_SIZE]= {
		0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x4D,0x67,0x11,0x11,0x01,0x01,0x01,0x01,
		0x20,0x22,0x01,0x03,0x80,0x46,0x27,0x78,0x0A,0xEE,0x91,0xA3,0x54,0x4C,0x99,0x26,
		0x0F,0x50,0x54,0x21,0x08,0x00,0x31,0x40,0x45,0x40,0x61,0x40,0x71,0x40,0x81,0x80,
		...
		,
	};

	if (m_standard_edid[port].edid_initialized)
		newbase_hdmi_write_edid(port, m_standard_edid[port].current_edid_table, m_standard_edid[port].current_edid_length);
	else
		newbase_hdmi_write_edid(port, kEdid_fhd, HDMI_FHD_EDID_SIZE);
}
...
void newbase_hdmi_enable_dpm(void)
{
	...
	HDMI_CHANNEL_T* p_info = NULL;
	unsigned int val = 0;
	ENUM_HDMI_DPM_TYPE dpm_type = HDMI_DPM_DISABLE;
	...
	for(ch = 0 ; ch < HDMI_PORT_TOTAL_NUM; ch++)
	{
		...
		p_info = newbase_hdmi_get_pcbinfo(port);
		val = (unsigned int)p_info->det_5v;
		val <<= 8;
		if (emcu_get_hdmi_det(ch))
		{
			if (hdmi_dpm[port].start_dpm)
				dpm_type = HDMI_DPM_WAKEUP_WHEN_TMDS_CLK_DETECTED;
			else if ((hdmi_dpm_start_monitor_signal == 2) && (hdmi_dpm[port].no_tmds_clk_cnt <= HDMI_DPM_MONITOR_NO_TMDS_CLK_THD))
				dpm_type = HDMI_DPM_IGNORE_NO_TMDS_CLK_AT_BEGIN;  // fake wakeup + less than 3s + dpm=0
			else
				dpm_type = HDMI_DPM_ENABLE;
			val |= (dpm_type&0xff);
			emcu_set_hdmi_det(port, val);
			newbase_hdmi_dpm_write_fhd_edid(port);
		}
		else
			emcu_set_hdmi_det(port, val);

		HDMI_INFO("[DPM][p%d]dpm_type=%d\n", port, dpm_type);
	}

	// after suspend
	if (hdmi_dpm_start_monitor_signal == 1)
		hdmi_dpm_start_monitor_signal = 2;
}

void newbase_hdmi_reset_dpm_variables_by_port(unsigned char port)
{
	if (port >= HDMI_PORT_TOTAL_NUM)
		return;

	hdmi_dpm[port].record_5v_status = 0;
	hdmi_dpm[port].record_is_valid_tmds_clk = 0;
	hdmi_dpm[port].no_tmds_clk_cnt = 0;
	hdmi_dpm[port].start_dpm = 0;
	hdmi_dpm[port].is_should_wakeup = 0;
}

void newbase_hdmi_reset_dpm_variables(void)
{
	unsigned char port;

	for (port = 0; port<HDMI_PORT_TOTAL_NUM; port++)
		newbase_hdmi_reset_dpm_variables_by_port(port);
	hdmi_fake_wake_up = 0;
}

void newbase_hdmi_dpm_update_status(void)
{
	unsigned char nport;

	for (nport=0; nport<HDMI_PORT_TOTAL_NUM; nport++)
	{
		hdmi_dpm[nport].start_dpm = emcu_get_emcu_hdmi_enable_dpm(nport);
		HDMI_INFO("[DPM][p%d]EMCU DPM %s\n", nport, hdmi_dpm[nport].start_dpm ? "enable" : "disabled");
	}
}

unsigned char newbase_hdmi_dpm_is_should_wakeup(unsigned char port)
{
	if (hdmi_dpm[port].is_should_wakeup)
		return 1;
	return 0;
}

void newbase_hdmi_dpm_set_signal_monitor(unsigned char start_signal_monitor)
{
	unsigned char port;
	unsigned char i;
	unsigned char tmp_start_signal_monitor = 0;

	tmp_start_signal_monitor = hdmi_dpm_start_monitor_signal ? 1 : 0;
	if (tmp_start_signal_monitor != start_signal_monitor)
	{
		// start monitoring the signal status
		if (start_signal_monitor == 1)
		{
			for (port=0; port<HDMI_PORT_TOTAL_NUM; port++)
			{
				...
				hdmi_dpm[port].record_5v_status = newbase_hdmi_get_5v_state(port);

				// check if the signal is stable
				if (hdmi_dpm[port].record_5v_status)
				{
					for (i=0;i<HDMI_DPM_MONITOR_TMDS_CLK_STABLE_THD;i++)
					{
						if (!newbase_rxphy_is_clk_stable(port) \
							|| (newbase_rxphy_get_clk(port) < VALID_MIN_CLOCK))
							break;

						if (GET_H_VIDEO_FSM(port) == MAIN_FSM_HDMI_VIDEO_READY)
						{
							i = HDMI_DPM_MONITOR_TMDS_CLK_STABLE_THD;
							break;
						}
						msleep(5);
					}

					if (HDMI_DPM_MONITOR_TMDS_CLK_STABLE_THD == i)
					{
						HDMI_INFO("[DPM] signal is stable\n");
						hdmi_dpm[port].record_is_valid_tmds_clk = 1;
					}
				}
					
				if (!hdmi_dpm[port].record_5v_status || !hdmi_dpm[port].record_is_valid_tmds_clk)
					hdmi_dpm[port].start_dpm = 1;
			}
		}
		else if (start_signal_monitor == 0) // stop monitor signal
		{
			for (port=0; port<HDMI_PORT_TOTAL_NUM; port++)
			{
				if (newbase_hdmi_phy_port_status(port)==HDMI_PHY_PORT_NOTUSED) 
		...
		hdmi_dpm_start_monitor_signal = start_signal_monitor;
	}

}

void newbase_hdmi_dpm_monitor(unsigned char port)
{
	unsigned char emcu_fake_wake_up = 0;
	unsigned char ch;
	unsigned char emcu_hdmi_det = 0;
	
	// do nothing on unused port or DPM is disabled  
	if (hdmi_vfe_get_hdmi_channel(port, &ch)<0)
		return;

	emcu_fake_wake_up = emcu_get_hdmi_det(EMCU_DPM_FAKE_WAKEUP_BIT);
	if (!hdmi_fake_wake_up && emcu_fake_wake_up)
	{
		hdmi_fake_wake_up = 1;
		HDMI_INFO("[DPM]fake wake up\n");
	}
	else if (hdmi_fake_wake_up && !emcu_fake_wake_up)
	{
		hdmi_fake_wake_up = 0;
		HDMI_INFO("[DPM]clr fake wake up status\n");
	}

	emcu_hdmi_det = emcu_get_hdmi_det(ch);
	if (hdmi_dpm[port].dpm_detect != emcu_hdmi_det)
	{
		// stop hdmi detection
		if (!emcu_hdmi_det)
		{
			memset(hdmi_dpm+port, 0, sizeof(HDMI_DPM_T));

			// send message to emcu and stop all hdmi signal detection in emcu.
			emcu_set_hdmi_det(port, 0);
			// stop detect signal
			hdmi_dpm_start_monitor_signal = 0;
			HDMI_INFO("[DPM][p%d]stop hdmi detection\n", port);
		}

		hdmi_dpm[port].dpm_detect = emcu_hdmi_det;
	}

	// AP enable hdmi detection
	if (hdmi_dpm[port].dpm_detect && hdmi_dpm_start_monitor_signal)
	{
		if (hdmi_dpm[port].start_dpm)
		{
			if (!hdmi_dpm[port].is_should_wakeup \
				...
				&& (newbase_rxphy_get_clk(port) >= VALID_MIN_CLOCK))
			{
				hdmi_dpm[port].is_should_wakeup = 1;
				HDMI_INFO("[DPM]is_should_wakeup=1\n");
			}
		}
		else
		{
			...
			{
				hdmi_dpm[port].start_dpm = 1;
				HDMI_INFO("[DPM][p%d] no 5V exists\n", port);
				return;
			}

			// check tmds clock
			if (newbase_rxphy_get_clk_pre(port))
				hdmi_dpm[port].no_tmds_clk_cnt = 0;
			else if (hdmi_dpm[port].no_tmds_clk_cnt < HDMI_DPM_MONITOR_NO_TMDS_CLK_THD)
				hdmi_dpm[port].no_tmds_clk_cnt++;
			else
			{
				hdmi_dpm[port].start_dpm = 1;
				HDMI_INFO("[DPM][p%d] no tmds clk\n", port);
				return;
			}
		}
	}
