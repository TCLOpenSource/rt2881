...
void vfe_hdmi_sync_qs_status(void)
{
	...
	if(is_get_vic){
		newbase_hdmi_set_current_vic(port, select_vic) ;
	}

	// Please note that the EDID is correct when there is only one hdmi port.
	// If there are multiple HDMI ports in quick show project, only the 
	// EDID of the port used during quick show is correct, as the EDIDs of 
	// other ports are be overwritten by the default EDIS. The overwriting 
	// process is shown in HdmiTable_Init() in vfe_hdmi_qs_drv_init().
	newbase_hdmi_sync_qs_edid(port);
}
...
int vfe_hdmi_drv_connect(unsigned char ch)
{
	...
	newbase_hdcp_set_protect(hdmi_port, 1); //enable hdcp protect in hdcp mode
...
int vfe_hdmi_drv_disconnect(unsigned char ch)
{
	...
	newbase_hdcp_set_protect(hdmi_port, 0); //disable hdcp protect in hdcp mode
...
EXPORT_SYMBOL(vfe_hdmi_drv_get_drm_info);
...
void vfe_hdmi_drv_suspend(void)
{
	...
	newbase_hdmi_reset_dpm_variables();
}
...
void vfe_hdmi_drv_resume_init(void)
{
	...
	newbase_hdmi_reset_dpm_variables();
	newbase_hdmi_dpm_update_status();
}
