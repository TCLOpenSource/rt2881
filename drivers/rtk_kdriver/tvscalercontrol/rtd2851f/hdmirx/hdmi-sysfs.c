...
// DPM
extern unsigned char hdmi_dpm_start_monitor_signal;
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_ctrl")==0)
    {
        ...
        else if (sscanf(buf, "5v_wakeup=%2d,%2d", &port, &val)==2)
        {
            newbase_hdmi_set_5v_wakeup((unsigned char)port, (unsigned char)val);
        }
        else
        {
            return -1;
                   ...
                   const char *buf, size_t len)
{
    ...
    if (strcmp(attr->attr.name, "dfe_test")==0)
    {
        ...
        if (sscanf(buf, "dfe_scan_le=%1d,%d,%d,%d,%d", &port, &ch, &min, &max, &period)==5)
        {
            ...
            if ((port <0 || port>=HDMI_RX_PORT_MAX_NUM) || (ch <0 || ch >2) ||
        ...
        else if (sscanf(buf, "dump_dfe_param=%1d", &port)==1)
        {
            ...
            if (port <0 || port>=HDMI_RX_PORT_MAX_NUM)
            {
                HDMI_EMG("invalid parameter!!!\n");
                return -EINVAL;
            }
        ...
        else if (sscanf(buf, "dfe_update_le=%1d,%d,%d", &port, &ch, &le)==3)
        {
            ...
            if ((port <0 || port>=HDMI_RX_PORT_MAX_NUM) || (ch <0 || ch >2) || !IS_VALID_LE(le))
            {
                HDMI_EMG("invalid parameter!!!\n");
                return -EINVAL;
            }
...
//===========================================================================
// Func : hdmi_dpm_info_show
//===========================================================================
static ssize_t hdmi_dpm_info_show(
    struct device*              dev,
    struct device_attribute*    attr,
    char*                       buf
    )
{
    ...
    if (strcmp(attr->attr.name, "dpm")==0)
    {
        n = scnprintf(ptr, count, "start_monitor_signal=%d\n", hdmi_dpm_start_monitor_signal);
        ptr+=n; count-=n;
        for (ch=0; ch<HDMI_PORT_TOTAL_NUM; ch++)
        {
            if (hdmi_vfe_get_hdmi_port(ch, &port)!=0)
                continue;
            n = scnprintf(ptr, count, "ch%d_wakeup=%d\n", ch, newbase_hdmi_dpm_is_should_wakeup(port));
            ptr+=n; count-=n;

            n = scnprintf(ptr, count, "ch%1u_EDID=", ch);
            ptr+=n; count-=n;
            if (m_standard_edid[port].edid_initialized)
            {
                for (i=0; i<m_standard_edid[port].current_edid_length; i++)
                {
                    n = scnprintf(ptr, count, "%02X", m_standard_edid[port].current_edid_table[i]);
                    ptr+=n; count-=n;
                }
            }
            n = scnprintf(ptr, count, "\n");
...
//===========================================================================
// Func : hdmi_dpm_info_store
//===========================================================================
static ssize_t hdmi_dpm_info_store(struct device *dev,
                   struct device_attribute *attr,
                   const char *buf, size_t count)
{
    unsigned int val = 0;
    ...
    if (strcmp(attr->attr.name, "dpm")==0)
    {
        if (sscanf(buf, "start_monitor_signal=%1d",&val)==1)
        {
            HDMI_INFO("[DPM] %s monitor signal\n", val ? "enable" : "disable");
            val = val ? 1 : 0;
            newbase_hdmi_dpm_set_signal_monitor(val);
        }
        else if (sscanf(buf, "ch%1d_EDID=",&val)==1)
        {
            HDMI_INFO("[DPM] hdmi channel %u, count=%u\n", val, count);
            ch = (unsigned char)val;
            if ((hdmi_vfe_get_hdmi_port(ch, &port)<0) || (count < 10))
            {
                HDMI_INFO("[DPM] parameter error,ch=%u, count=%u \n", val, count);
                return count;
            }

            len = count -9;
            len >>= 1;
            p = (unsigned char *)buf+9;
            for (i=0; i<len;i++)
            {
                if (sscanf(p, "%2X",&val)!=1)
                {
                    HDMI_INFO("[DPM] parse edid fail, i=%u, len=%u\n", i, len);
                    return count;
                }
                m_standard_edid[port].current_edid_table[i] = (unsigned char)val;

                p += 2;
            }
            m_standard_edid[port].current_edid_length = len;
            m_standard_edid[port].edid_initialized = 1;
        }
...
static DEVICE_ATTR(dpm, 0644, hdmi_dpm_info_show, hdmi_dpm_info_store);
//////////////////////////////////////////////////////////////////////////////
// File Operations
//////////////////////////////////////////////////////////////////////////////
...
int __init hdmi_module_init(void)
{
    ...
    device_create_file(hdmi_miscdev.this_device, &dev_attr_dpm);
    return 0;
}
...
#endif
{
    ...
    device_remove_file(hdmi_miscdev.this_device, &dev_attr_dpm);
