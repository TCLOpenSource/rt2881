...
unsigned short hdmi_char_bit_debug_print_step[HDMI_PORT_TOTAL_NUM] = {0};
...
{
    hdmi_mask(HDMI_STB_SCDC_CR_reg, ~HDMI_STB_SCDC_CR_scdc_en_mask, (enable) ? HDMI_STB_SCDC_CR_scdc_en(1) : 0);
}
...
{
    hdmi_mask(HDMI_STB_SCDC_CR_reg, ~(HDMI_STB_SCDC_CR_scdc_reset_mask | HDMI_STB_SCDC_CR_hw_mask_en_mask), (rst) ? HDMI_STB_SCDC_CR_scdc_reset(1) : 0);
}
    ...
    )
{
    hdmi_mask(HDMI_STB_SCDC_CR_reg, ~HDMI_STB_SCDC_CR_write_1_clr_mask, 
                                     HDMI_STB_SCDC_CR_write20_det_irq_flag(1));    
}
...
{
    hdmi_mask(HDMI_STB_SCDC_CR_reg, ~HDMI_STB_SCDC_CR_write_1_clr_mask, 
                                 HDMI_STB_SCDC_CR_write31_det_irq_flag(1));    
}
...
void lib_hdmi_scdc_set_src_clr_frl_start(unsigned char nport, unsigned char enable)
{
    hdmi_mask(HDMI_STB_SCDC_CR_reg, ~HDMI_STB_SCDC_CR_frl_start_src_clr_en_mask,
            (enable) ? HDMI_STB_SCDC_CR_frl_start_src_clr_en(1) : 0);
}
...
void newbase_hdmi_scdc_flow_print(unsigned char port)
{
    ...
    if (hdmi_char_bit_debug_print_cnt[port] >= g_char_bit_print_cycle_cnt_thd)
    {
        ...
        else
        {
            ...
            CEDERR_FLOW_PRINTF("Signal STS Port[%d]: PhyState=%d; CHAR(b,g,r)=(%d,%d,%d)); MIN=%d,mac_clk(b,g,r,c,rate)=(%d,%d,%d,%d,%d); BCH_ERR cnt(1,2)=(%d,%d); hw_reauth=%d; AVMUTE=%d; BGRHV=0x%x;5V(%d,%d,%d,%d)\n",
                port,
                phy_proc_state,
                char_b_err,
                char_g_err,
        ...
        if (hdmi_char_bit_debug_print_step[port] > 1)
            hdmi_char_bit_debug_print_step[port] -= 2;
    }
    else
    {
        unsigned short step = 0;

        if (hdmi_char_bit_debug_print_step[port] > 0)
            step += hdmi_char_bit_debug_print_step[port];
        ...
        if((newbase_hdmi_ced_err_thd_check(port, DIAGNOSTIC_CED_ERROR_THD)) ||
            ...
            step +=1;
        hdmi_char_bit_debug_print_cnt[port] += step;
    }
...
void newbase_hdmi_errdet_handler(unsigned char port)
{
    ...
    for(i = 0; i <  HDMI_ERRDET_NUM; i++)
    {
        ...
        case HDMI_HD20_BIT_ERR:
            ...
            else if((GET_H_VIDEO_FSM(port) <= MAIN_FSM_HDMI_MEASURE) && (m_period_errdet_status[i].is_error_overthd== TRUE))
            {// record error if cant not measure timing stable
                hdmi_rx[port].last_fw_bit_error[0] = current_err_ent[i][0];
                hdmi_rx[port].last_fw_bit_error[1] = current_err_ent[i][1];
                hdmi_rx[port].last_fw_bit_error[2] = current_err_ent[i][2];
                hdmi_rx[port].last_fw_bit_error[3] = current_err_ent[i][3];
                if((m_period_errdet_status[i].period_err_correction_cnt[0] ==m_period_errdet_status[i].period_err_correction_cnt[1])&&
                    (m_period_errdet_status[i].period_err_correction_cnt[0] ==m_period_errdet_status[i].period_err_correction_cnt[2])&&
                    (m_period_errdet_status[i].period_err_correction_cnt[1] ==m_period_errdet_status[i].period_err_correction_cnt[2]))
                {// for >340M scramble error
                    hdmi_rx[port].fw_bit_disp_error_overthd_cnt ++;
                }
            }
            else if(GET_H_VIDEO_FSM(port) == MAIN_FSM_HDMI_DISPLAY_ON)
            {//reset to init
                hdmi_rx[port].last_fw_bit_error[0] = 0;
                hdmi_rx[port].last_fw_bit_error[1] = 0;
                hdmi_rx[port].last_fw_bit_error[2] = 0;
                hdmi_rx[port].last_fw_bit_error[3] = 0;
                hdmi_rx[port].fw_bit_disp_error_overthd_cnt = 0;
            }
            break;
        case HDMI_HD20_CHAR_ERR:
        ...
        if((m_period_errdet_status[i].is_error_overthd ==TRUE) && (m_hdmi_errdet_config_table[i].is_polling_print == TRUE))
        {
			...
			if((m_hdmi_errdet_config_table[i].errdet_type==HDMI_HD20_CHAR_ERR)||(m_hdmi_errdet_config_table[i].errdet_type==HDMI_HD20_BIT_ERR))
			{
				if(((m_period_errdet_status[i].last_error_cnt[0] - m_period_errdet_status[i].start_record_error_index[0])>10)||
				 ((m_period_errdet_status[i].last_error_cnt[1] - m_period_errdet_status[i].start_record_error_index[1])>10)||
				((m_period_errdet_status[i].last_error_cnt[2] - m_period_errdet_status[i].start_record_error_index[2])>10))
				{
				       HDMI_WARN("CHAR_ERR or BIT_ERR > 10 in  in %d ms\n",m_hdmi_errdet_config_table[i].every_x_ms);
		 			lib_hdm_set_cdr_kp(port);
				}	
			}
            }
            else
...
void newbase_hdmi_scdc_spin_lock_init(void)
{
...
void newbase_hdmi_enable_char_bit_debug_print_step(unsigned char nport)
{
    HDMI_INFO("%s, port=%d\n", __func__, nport);
    hdmi_char_bit_debug_print_step[nport] = CHAR_BIT_PRINT_CYCLE_CNT_THD/100;
}

void newbase_hdmi_reset_char_bit_debug_print_step(unsigned char nport)
{
    HDMI_INFO("%s, port=%d\n", __func__, nport);
    hdmi_char_bit_debug_print_step[nport] = 0;
}
