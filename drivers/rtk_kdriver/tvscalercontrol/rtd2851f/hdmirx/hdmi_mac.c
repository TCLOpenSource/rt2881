...
void lib_hdmi_set_wd_debug_mode(unsigned char nport, unsigned char en)
{
    hdmi_mask(HDMI_HDMI_WDCR_reg, ~(HDMI_HDMI_WDCR_hdmi_wd_debug_mode_mask|HDMI_HDMI_WDCR_hdmi_wd_debug_target_mask), (en) ? (HDMI_HDMI_WDCR_hdmi_wd_debug_mode_mask|HDMI_HDMI_WDCR_hdmi_wd_debug_target_mask) : 0);
    HDMI_INFO("[p%d] set watchdog debug mode = %d \n", nport, en);
}

unsigned char lib_hdmi_get_wd_debug_mode(unsigned char nport)
{
    return HDMI_HDMI_WDCR_get_hdmi_wd_debug_mode(hdmi_in(HDMI_HDMI_WDCR_reg));
}
...
int newbase_hdmi_get_dpm_enable(unsigned int nport)
{
    ...
    if (hdmi_vfe_get_hdmi_channel(nport, &ch)!=0)
        return 0;
