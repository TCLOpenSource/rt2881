...
void newbase_hdmi_flow_cfg_info_init(void)
{
    ...
    for(i=0; i<g_panel_vfreq_hd21_pll_mapping_table_num; i++)
    {
        if(GET_PANEL_REFRESH_RATE() == g_panel_vfreq_hd21_pll_mapping_table[i].panel_max_v_freq &&
            GET_PANEL_HACTIVE() == g_panel_vfreq_hd21_pll_mapping_table[i].panel_h_active &&
            GET_PANEL_VACTIVE() == g_panel_vfreq_hd21_pll_mapping_table[i].panel_v_active)
        {
            ...
            HDMI_EMG("[newbase_hdmi_flow_cfg_info_init] detect special panel freq=%d, h_act=%d, v_act=%d, use hd21 PLL setting=%d\n", GET_PANEL_REFRESH_RATE(), GET_PANEL_HACTIVE(), GET_PANEL_VACTIVE(), g_panel_vfreq_hd21_pll_mapping_table[i].hd21_max_pll);
            break;
        }
    }
