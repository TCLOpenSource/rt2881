...
#include <rtk_kdriver/hdcp/hdcp2_2/hdcp2_interface.h>
...
unsigned char hdcp_protect_flag= 0;
...
void lib_hdmi_hdcp_set_fifo_mode(unsigned char nport, unsigned char auto_mode,unsigned char fw_fifo_mode_hdcp22)
{
    ...
    hdmi_mask(HDMI_STB_SCDC_PCR_reg, ~(HDMI_STB_SCDC_PCR_fifo_mode_fw_mask | HDMI_STB_SCDC_PCR_fifo_switch_mode_mask), fifo_mode);
}
...
void newbase_hdmi_hdcp_enable(unsigned char port, unsigned char enable)
{
    ...
    else
    {
        ...
        newbase_hdcp_set_protect(port, 0);
...
void newbase_hdcp_handler(unsigned char port)
{
    ...
    if (p_hdcp->hdcp14_aksv_rcv)
    {
        ...
        newbase_hdcp_set_protect(port, 1);
    ...
    else 
    {
        ...
        if ((newbase_hdmi_get_power_saving_state(port) == PS_FSM_POWER_SAVING_OFF) && newbase_rxphy_get_setphy_done(port) && (lib_hdmi_hdcp22_get_enable_reauth(port) == 0))
        {
            HDCP_WARN("HDMI[p%d] hw auto reauth is 0, force to 1\n", port);
            lib_hdmi_hdcp22_enable_reauth(port, 1);
        }
    }
  #endif
...
//QS mode, vcup run hdcp mode, scpu don't konw hdcp status, the api will check hdcp auth status
HDMI_HDCP_E hdmi_hdcp_check_hdcp_stauts(unsigned char port)
{
    HDMI_HDCP_E running_hdcp_mode = NO_HDCP;

    if (lib_hdmi_hdcp22_is_onoff(port)==1 && lib_hdmi_hdcp14_is_onoff(port)==0){
        if(lib_hdmi_hdcp_get_enc_toggle(port))
            running_hdcp_mode = HDCP22;
        else
            running_hdcp_mode = NO_HDCP;
    }
    else if (lib_hdmi_hdcp22_is_onoff(port)==0 && lib_hdmi_hdcp14_is_onoff(port)==1)
        running_hdcp_mode = HDCP14;
    else if (lib_hdmi_hdcp22_is_onoff(port)==1 && lib_hdmi_hdcp14_is_onoff(port)==1)
        running_hdcp_mode = NO_HDCP;
...
HDMI_HDCP_E newbase_hdcp_get_auth_mode(unsigned char port)
{
    ...
    if (lib_hdmi_hdcp22_is_onoff(port)==0 && lib_hdmi_hdcp14_is_onoff(port)==0)
        ...
        running_hdcp_mode = hdmi_hdcp_check_hdcp_stauts(port);
    }
    else
    {
...
int newbase_hdcp_set_protect(unsigned char port, unsigned char enable)
{
#ifdef CONFIG_OPTEE_HDCP2
    unsigned char current_port;
    HDMI_HDCP_E running_hdcp_mode = HDCP_OFF;

    if(hdcp_protect_flag == enable)
        return 0;

    if (hdmi_vfe_get_connected_port(&current_port)<0) {
        HDCP_FLOW_PRINTF("forget to connect hdmi channel\n");
        return -1;
    }

    //current_port = newbase_hdmi_get_current_display_port();
    if (port != current_port){
        HDCP_FLOW_PRINTF("not current port\n");
        return -1;
    }

    if(enable){
        running_hdcp_mode = newbase_hdcp_get_auth_mode(port);
        if((running_hdcp_mode == HDCP22) || (running_hdcp_mode == HDCP14)){
            HDCP_WARN("[p%d] hdcp enable protect\n", port);
            if(optee_hdcp_set_protect(HDMI_HDCP2_MODE, port, 1)==0){
                hdcp_protect_flag = 1;
            }else{
                HDCP_WARN("[p%d] hdcp enable protect failed!\n", port);
            }
            return 0;
        }else{
            HDCP_WARN("[p%d] hdcp mode:%s, disable protect \n", port, _hdmi_hdcp_mode_str(running_hdcp_mode));
        }
    }else{
        HDCP_WARN("[p%d] hdcp disable protect\n", port);
    }
    if(optee_hdcp_set_protect(HDMI_HDCP2_MODE, port, 0)==0){
        hdcp_protect_flag = 0;
    }else{
        HDCP_WARN("[p%d] hdcp disable protect failed!\n", port);
    }
#endif
    return 0;
}
...
void newbase_hdmi_hdcp14_write_ksv_fifo(unsigned char port, unsigned char* p_ksv_fifo, unsigned int len)
{
    ...
    nport = port;
    for(i=0; i<len; i++){
        //follow spec hdcp repeater flow
        hdmi_out(HDMI_STB_HDCP_AP_reg, 0x43 | HDMI_STB_P0_HDCP_AP_dp_ksvfifo_ptr(i));
        hdmi_out(HDMI_STB_HDCP_DP_reg, p_ksv_fifo[i]);
        //HDCP_WARN("%x ", (UINT32)p_ksv_fifo[i]);
    }
#endif
}
