...
#include <rbus/edptx_common_reg.h>
#include <rbus/edptx0_aux_dphy_reg.h>
#include <rbus/edptx0_aux_mac_reg.h>
#include <rbus/edptx0_ml_dphy_reg.h>
#include <rbus/edptx0_ml_hpd_reg.h>
#include <rbus/edptx0_ml_mac_reg.h>
#include <rbus/edptx1_aux_dphy_reg.h>
#include <rbus/edptx1_aux_mac_reg.h>
#include <rbus/edptx1_ml_dphy_reg.h>
#include <rbus/edptx1_ml_hpd_reg.h>
#include <rbus/edptx1_ml_mac_reg.h>
...
unsigned int edp_panel_resum_run=1;//1--new flow(read & write register)  other---old flow(load script)
...
static unsigned char ap_panel_sscg_enable = 0;
static unsigned int ap_panel_sscg_percent = 0;
static unsigned int ap_panel_sscg_period = 0;
...
static BOOLEAN vbe_mprt_enable = 0;
...
{
	if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC) || judge_vrr_freesync())
		return FALSE;
...
#define PMIC_PORT           1
#define PMIC_ADDR           0x36    // default address
#define PMIC_CSPI_NAME      "CS603"

UINT8 g_pmic_i2c_addr = PMIC_ADDR;
UINT8 g_pmic_i2c_port = PMIC_PORT;
UINT8 PMIC_BUFF[FLASH_READ_SIZE]={
    0x48,0x09,0xA4,0x10,0x23,0x84,0xF8,0x46,0x77,0x0B,0x08,0x00,0x10,0x07,0x00,0x00,  //bin file data 42bytes
    0x3E,0xC3,0xD3,0x33,0x82,0xDA,0x2A,0xA2,0x3C,0x21,0xA1,0xC0,0x1A,0x71,0x38,0x10,
    0x20,0xA3,0x01,0x60,0x0D,0x1B,0x20,0x00,
    0x1F,0xEF,        //CRC
};
typedef struct {
    UINT8 u8_reg;
    UINT8 u8_buff[FLASH_READ_SIZE-1];
} I2C_XFER_PACKET;

void vbe_write_PMIC(void)
{
#ifndef UT_flag
    unsigned char sPmicName[PMIC_NAME_SIZE];
    I2C_XFER_PACKET xfer_packet = { 0 };
    int bin_size = sizeof(xfer_packet.u8_buff);
    memcpy(xfer_packet.u8_buff, PMIC_BUFF, bin_size);
    Get_PMIC_Name(sPmicName);
    rtd_pr_vbe_notice("write PMIC name=%s\n",sPmicName);
    if(Get_PMIC_BIN_SIZE() && Get_PMIC_I2c_Addr())
    {
        bin_size = Get_PMIC_BIN_SIZE();
        g_pmic_i2c_port = Get_PMIC_I2C_PORT();
        g_pmic_i2c_addr = Get_PMIC_I2c_Addr();
        rtd_pr_vbe_notice("[PMIC] -> write\n");
        xfer_packet.u8_reg = 0;
        if (i2c_master_send_ex_flag(g_pmic_i2c_port, g_pmic_i2c_addr, (UINT8 *)&xfer_packet, bin_size + 1, I2C_M_FAST_SPEED) < 0) {
            rtd_pr_vbe_err("[I2C] write -> fail !\n");
        }
    }
#endif
}
void vbe_read_PMIC(void)
{
    I2C_XFER_PACKET xfer_packet = { 0 };
    int bin_size = sizeof(xfer_packet.u8_buff);
    ...
    if(Get_PMIC_BIN_SIZE() && Get_PMIC_I2c_Addr())
    {
        ...
        g_pmic_i2c_port = Get_PMIC_I2C_PORT();
        g_pmic_i2c_addr = Get_PMIC_I2c_Addr();
        rtd_pr_vbe_notice("[PMIC] -> read\n");
        xfer_packet.u8_reg = 0;
#ifndef UT_flag
        if (i2c_master_recv_ex_flag(g_pmic_i2c_port, g_pmic_i2c_addr, &xfer_packet.u8_reg, 1, xfer_packet.u8_buff, bin_size, I2C_M_FAST_SPEED) < 0) {
            rtd_pr_vbe_err("[I2C] read -> fail !\n");
        } 
        if((xfer_packet.u8_buff[0] != 0x00) && (xfer_packet.u8_buff[1] != 0x00))
            memcpy(PMIC_BUFF, xfer_packet.u8_buff, bin_size);
        else
            rtd_pr_vbe_err("read error!!!!\n");
#endif
    }
}
unsigned char Panel_SetPanelVCC(unsigned char enable)
{
#ifndef UT_flag
    int pinindex = 0;
    int ret = 0;
    ...
    rtd_pr_vbe_notice("Panel_SetPanelVCC=%d\n",enable);
    if (pcb_mgr_get_enum_info_byname("PIN_LVDS_ON_OFF", &param) == 0)
    {
        if (GET_PIN_TYPE(param) == PCB_PIN_TYPE_ISO_GPIO) {
            ...
            rtd_pr_vbe_notice( "ISO_GPIO gpio_num:%d\n",pinindex);
            rtd_pr_vbe_notice(  "enable:%d\n",enable);
            ...
            rtd_pr_vbe_notice( "MISC_GPIO gpio_num:%d\n",pinindex);
            ret = rtk_gpio_output(s_gpio, enable);
            rtk_gpio_set_dir(s_gpio,1);
            if (ret < 0)
                return 0;
...
unsigned char judge_vrr_freesync(void)
{
    if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() || vbe_disp_get_force_vrr_condition())
    {
        return 1;
    }
    else
...
unsigned char vbe_disp_force_vrr_flag = 1;
unsigned char vbe_disp_get_force_vrr_condition(void)
{
	return (vbe_disp_force_vrr_flag && drv_memory_get_game_mode_flag() && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 475)
#ifndef BUILD_QUICK_SHOW
	&& get_new_game_mode_condition()
#endif
	);
}
unsigned char vbe_disp_get_force_vrr_flag(void)
{
	return vbe_disp_force_vrr_flag;
}
void vbe_disp_set_force_vrr_flag(unsigned char enable)
{
	vbe_disp_force_vrr_flag = enable;
}


unsigned char scaler_check_pre_game_mode_condition_flag = 0;
unsigned char vbe_disp_get_pre_game_mode_condition(void)
{
	return scaler_check_pre_game_mode_condition_flag;
}
void vbe_disp_set_pre_game_mode_condition(unsigned char enable)
{
	scaler_check_pre_game_mode_condition_flag = enable;
}

unsigned char (*Get_Val_pre_game_mode_condition_impl)(void) = vbe_disp_get_pre_game_mode_condition;

void (*Set_Val_pre_game_mode_condition_impl)(unsigned char) = vbe_disp_set_pre_game_mode_condition;


unsigned char scaler_game_mode_framerate_1to1_change_flag = 0;
unsigned char vbe_disp_get_game_framerate_1to1_change_flag(void)
{
	return scaler_game_mode_framerate_1to1_change_flag;
}
void vbe_disp_set_game_mode_framerate_1to1_change_flag(unsigned char enable)
{
	scaler_game_mode_framerate_1to1_change_flag = enable;
}

unsigned char (*Get_Val_game_framerate_1to1_change_flag_impl)(void) = vbe_disp_get_game_framerate_1to1_change_flag;

void (*Set_Val_game_framerate_1to1_change_flag_impl)(unsigned char) = vbe_disp_set_game_mode_framerate_1to1_change_flag;
...
{
#ifdef GAME_MODE_DYNAMIC
	/** Dynamic clk flow use for game mode low delay tuning , normal condition don't use **/
...
#ifndef UT_flag
void check_game_mode_new_low_delay_condition(void)
{
	if(get_product_type() == PRODUCT_TYPE_DIAS){
#ifdef NEW_GAME_MODE_LOW_DELAY
#ifdef GAME_MODE_DYNAMIC
		if((vbe_disp_check_support_game_mode_dynamic_change_clk()) && 
			((drv_memory_get_game_mode() == _ENABLE) || (Get_Val_qs_game_flag())) && 
			(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 475)){
			vbe_disp_set_game_mode_dynamic_change_clk_flag(_ENABLE);
			vbe_disp_set_pre_game_mode_condition(_ENABLE);
		}
		else {
			vbe_disp_set_game_mode_framerate_1to1_change_flag(_DISABLE);
			vbe_disp_set_game_mode_dynamic_change_clk_flag(_DISABLE);
			vbe_disp_set_pre_game_mode_condition(_DISABLE);    
		}
#else
		if(((drv_memory_get_game_mode() == _ENABLE) || (Get_Val_qs_game_flag())) && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ) > 475){
			vbe_disp_set_game_mode_framerate_1to1_change_flag(_ENABLE);
		}    
		else {
			vbe_disp_set_game_mode_framerate_1to1_change_flag(_DISABLE);
			vbe_disp_set_game_mode_dynamic_change_clk_flag(_DISABLE);
...
extern int GDMA_ConfigOSDxMixerEnable(GDMA_DISPLAY_PLANE disPlane, bool flag);
extern int GDMA_ConfigOSDxEnable(GDMA_DISPLAY_PLANE disPlane, bool flag);
...
void vbe_disp_set_game_mode_dynamic_change_settings(unsigned char display)
{
	...
	*/
	...
	if(get_product_type() == PRODUCT_TYPE_DIAS){
		...
		vbe_disp_config_dynamic_edp_interface();
		GDMA_ConfigOSDxEnable(GDMA_PLANE_OSD1, TRUE);
#endif
	}
#endif
...
void vbe_disp_config_dynamic_edp_interface(void)
{
    /*
    Stream Clock = DPLL clock * SFG pixel mode / edP port
    Stream clock = link clock(540M) x Mvid / Nvid
    Stream clock * BPC * 3 /8 = (TU_DATA_SIZE/TU_SIZE) * link clock * LANE_NUM , TU_Size = 0x40
    Hdelay = (Hsta + 96 x 64 / (BPC*3) / 2) * link clock / Stream clock
    */
    unsigned int pixel_mode = 1;
    unsigned int edp_port_number = (rtd_part_inl(SFG_SFG_CTRL_0_reg, 4, 0)+1)/2;
    unsigned int link_clk = 540; //540M
    unsigned int TU_Size = 64; //0x40
    unsigned int LANE_NUM = 4;
    unsigned int Nvid = 8192; //0x2000
    unsigned int BPC = 8; //Panel color Bit
    unsigned int dclk = Get_DISPLAY_CLOCK_TYPICAL();
    unsigned int stream_clk = Get_DISPLAY_CLOCK_TYPICAL();
    unsigned int Mvid = IoReg_Read32(EDPTX0_ML_MAC_MN_M_VID_24_reg);
    unsigned int TU_DATA_SIZE = 29,tu_data_size_temp; //0x1D
    unsigned int Hdelay = IoReg_Read32(EDPTX0_ML_MAC_HDEALY_78_reg);

    if(rtd_part_inl(SFG_SFG_CTRL_0_reg, 17, 16) == 0x0) //1p
        pixel_mode = 1;
    else if(rtd_part_inl(SFG_SFG_CTRL_0_reg, 17, 16) == 0x1) //2p
        pixel_mode = 2;
    else if(rtd_part_inl(SFG_SFG_CTRL_0_reg, 17, 16) == 0x2) //4p
        pixel_mode = 4;
    else if(rtd_part_inl(SFG_SFG_CTRL_0_reg, 17, 16) == 0x3) //8p
        pixel_mode = 8;

    rtd_pr_vbe_notice("pixel_mode = %d, edp_port_number = %d\n", pixel_mode , edp_port_number);

    if(rtd_part_inl(EDPTX0_ML_MAC_MSA_MISC0_3C_reg, 7, 5) == 0x0) //6bit
        BPC = 6;
    else if(rtd_part_inl(EDPTX0_ML_MAC_MSA_MISC0_3C_reg, 7, 5) == 0x1) //8bit
        BPC = 8;
    else if(rtd_part_inl(EDPTX0_ML_MAC_MSA_MISC0_3C_reg, 7, 5) == 0x2) //10bit
        BPC = 10;
    else if(rtd_part_inl(EDPTX0_ML_MAC_MSA_MISC0_3C_reg, 7, 5) == 0x3) //12bit
        BPC = 12;
    else if(rtd_part_inl(EDPTX0_ML_MAC_MSA_MISC0_3C_reg, 7, 5) == 0x4) //16bit
        BPC = 16;

    stream_clk = dclk * pixel_mode / edp_port_number;

    rtd_pr_vbe_notice("dclk = %d, stream_clk = %d, BPC = %d\n", dclk , stream_clk, BPC);

    Mvid = ((stream_clk / (link_clk*100)) * Nvid) / 10000;
    rtd_part_outl(EDPTX0_ML_MAC_MN_M_VID_24_reg, 23, 0, Mvid); //0xb8066324[23:0]
    rtd_part_outl(EDPTX1_ML_MAC_MN_M_VID_24_reg, 23, 0, Mvid); //0xb8067324[23:0]
    IoReg_SetBits(EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_reg, _BIT6); //double buffer apply
    IoReg_SetBits(EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_reg, _BIT6); //double buffer apply

    tu_data_size_temp = (((stream_clk / 8) / LANE_NUM / link_clk) * BPC * 3 * TU_Size) / 100000;
    TU_DATA_SIZE = (tu_data_size_temp + 9) / 10;//>0.1 set 1
    rtd_part_outl(EDPTX0_ML_MAC_TU_DATA_SIZE_74_reg, 9, 3, TU_DATA_SIZE); //0xb8066374[9:3]
    rtd_part_outl(EDPTX1_ML_MAC_TU_DATA_SIZE_74_reg, 9, 3, TU_DATA_SIZE); //0xb8067374[9:3]

    Hdelay = (Get_DISP_DEN_STA_HPOS() + ((96*64)/(BPC*3))) * (link_clk*10000) / stream_clk * 100;
    rtd_part_outl(EDPTX0_ML_MAC_HDEALY_78_reg, 15, 0, Hdelay); //0xb8066378[15:0]
    rtd_part_outl(EDPTX1_ML_MAC_HDEALY_78_reg, 15, 0, Hdelay); //0xb8067378[15:0]

    rtd_pr_vbe_notice("Mvid = %d, TU_DATA_SIZE = %d, Hdelay = %d\n", Mvid , TU_DATA_SIZE, Hdelay);
}
...
#define ECO_FRAMERATE (600)

extern void set_eco_mode_shared_flag(unsigned char flag);
unsigned int vbe_eco_mode_flag = 0;
unsigned int HAL_VBE_GET_ECO_MODE(void)
{
    return vbe_eco_mode_flag;
}
void HAL_VBE_SET_ECO_MODE(unsigned int enable)
{
    vbe_eco_mode_flag = enable;
    rtd_pr_vbe_notice("[%s] vbe_eco_mode_flag=%d", __FUNCTION__, vbe_eco_mode_flag);
}
void (*Set_Val_HAL_VBE_SET_ECO_MODE_impl)(unsigned int) = HAL_VBE_SET_ECO_MODE;
unsigned int (*Get_Val_HAL_VBE_GET_ECO_MODE_impl)(void) = HAL_VBE_GET_ECO_MODE;

unsigned int vbe_eco_mode_status_flag = 0;
unsigned int vbe_get_eco_mode_status_flag(void)
{
    return vbe_eco_mode_status_flag;
}
void vbe_set_eco_mode_status_flag(unsigned int enable)
{
    vbe_eco_mode_status_flag = enable;
    set_eco_mode_shared_flag(enable);
    rtd_pr_vbe_notice("[%s] vbe_eco_mode_status_flag=%d", __FUNCTION__, vbe_eco_mode_status_flag);
}
void (*Set_Val_vbe_set_eco_mode_status_flag_impl)(unsigned int) = vbe_set_eco_mode_status_flag;
unsigned int (*Get_Val_vbe_get_eco_mode_status_flag_impl)(void) = vbe_get_eco_mode_status_flag;

unsigned int get_current_dclk(void)
{
    return (Get_DISPLAY_REFRESH_RATE() * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL());
}

bool Set_Eco_flag_Get_Dclk_Changed(unsigned int hal_vbe_eco_flag)
{
    bool isDclkChanged = false;
    unsigned int preDclk = 0;
    unsigned int targetDclk = 0;
    preDclk = get_current_dclk();
    vbe_set_eco_mode_status_flag(hal_vbe_eco_flag);
    targetDclk = get_current_dclk();
    if(preDclk != targetDclk)
    {
        isDclkChanged = true;
    }
    rtd_pr_vbe_notice("[%s]preDclk=%d targetDclk=%d isDclkChanged=%d", __FUNCTION__, preDclk, targetDclk, isDclkChanged);
    return isDclkChanged;
}

extern void GDMA_Clk_Select(int src_type, bool enable);
extern struct semaphore* get_hdmi_detectsemaphore(void);
extern void HDMI_set_detect_flag(unsigned char enable);
void vbe_disp_check_eco_mode(bool forceRun)
{
    ppoverlay_double_buffer_ctrl_RBUS double_buffer_ctrl_reg;
    ppoverlay_double_buffer_ctrl2_RBUS ppoverlay_double_buffer_ctrl2_reg;
    ppoverlay_dispd_smooth_toggle1_RBUS dispd_smooth_toggle1_reg;
    ...
    unsigned int hal_vbe_eco_flag = 0;
    unsigned int inputFrameRate = 0;
    unsigned int dispFr = 0;
    unsigned int preDispFr = 0;
    unsigned int delay = 0;
    bool isDclkChanged = false;

    hal_vbe_eco_flag = HAL_VBE_GET_ECO_MODE();

    preDispFr = Get_DISPLAY_REFRESH_RATE();

    if(forceRun || (vbe_get_eco_mode_status_flag() != hal_vbe_eco_flag))
    {
        isDclkChanged = Set_Eco_flag_Get_Dclk_Changed(hal_vbe_eco_flag);

        if(isDclkChanged)
        {
            if(preDispFr == 0)
            {
                preDispFr = 60; // exception handle
            }
            delay = ((1000 / preDispFr) + 1); // ms
            delay *= 2; // delay 2 frame time
        }
    }
    else
    {
    ...
    rtd_pr_vbe_notice("[%s] start!! DispFrameRate: %d -> %d \n", __FUNCTION__, preDispFr, Get_DISPLAY_REFRESH_RATE());

    if(isDclkChanged)
    {
        Scaler_ForcePanelBg(TRUE);
    }

    inputFrameRate = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ);
    dispFr = (Get_DISPLAY_REFRESH_RATE() * 10);

    if(vbe_get_eco_mode_status_flag())
    {
        Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_V_FREQ, ECO_FRAMERATE);
    }
    else
    {
        Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_V_FREQ, dispFr);
    }
    ...
    */
    ...
    dtg_frame_cnt1_reg.regValue = rtd_inl(PPOVERLAY_DTG_FRAME_CNT1_reg);
    while(!((dtg_frame_cnt1_reg.uzudtg_output_frame_cnt == uzu_valid_framecnt) &&
        (PPOVERLAY_new_meas2_linecnt_real_get_uzudtg_dly_line_cnt_rt(rtd_inl(PPOVERLAY_new_meas2_linecnt_real_reg)) > Get_DISP_ACT_STA_VPOS()) &&
        (PPOVERLAY_new_meas2_linecnt_real_get_uzudtg_dly_line_cnt_rt(rtd_inl(PPOVERLAY_new_meas2_linecnt_real_reg)) < Get_DISP_ACT_END_VPOS())) && --count){
        dtg_frame_cnt1_reg.regValue = rtd_inl(PPOVERLAY_DTG_FRAME_CNT1_reg);
    }
    if(count == 0)
        rtd_pr_vbe_info("[DTG] Wait to uzu_valid_framecnt timeout!!, uzu_valid_framecnt = %d, uzudtg_output_frame_cnt = %d\n",uzu_valid_framecnt, dtg_frame_cnt1_reg.uzudtg_output_frame_cnt);
    ...
    if(isDclkChanged)
    {
        drvif_clock_dynamic_reset_dclk(); //reset dclk
        GDMA_Clk_Select(0, true); //reset GDMA clock
        vbe_disp_config_eco_dynamic_interface();

        mdelay(delay);
        rtd_pr_vbe_notice("[%s] delay = %d ms", __FUNCTION__, delay);
    }
    ...
    if(inputFrameRate == 0)
    {
        if(vbe_get_eco_mode_status_flag())
        {
            inputFrameRate = ECO_FRAMERATE;
        }
        else
        {
            inputFrameRate = dispFr;
        }
    }
    Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_V_FREQ, inputFrameRate);

#if 1 // scaler rerun
    if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE)
    {
        rtd_pr_vbe_notice("[%s] Eco mode status change to (%d) Need to rerun scaler!!\n", __FUNCTION__, vbe_get_eco_mode_status_flag());
    ...
    if(isDclkChanged)
    {
        Scaler_ForcePanelBg(FALSE);
    }

    rtd_pr_vbe_notice("[%s] finish!!\n", __FUNCTION__);

}

void vbe_disp_config_eco_dynamic_interface(void)
{
    if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K
        || Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K)
    {
        // vby1 mac no change
    }
...
unsigned char Get_Val_ap_panel_sscg_enable(void)
{
    return ap_panel_sscg_enable;
}

unsigned int Get_Val_ap_panel_sscg_percent(void)
{
    return ap_panel_sscg_percent;
}

unsigned int Get_Val_ap_panel_sscg_period(void)
{
    return ap_panel_sscg_period;
}
...
unsigned char modestate_decide_display_timing(void)
{
	if(Disp_Get_isForceFreeRun()==DISP_FREE_RUN){
		...
		return DISPLAY_MODE_FRC;
	}else if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		return DISPLAY_MODE_FRC;
	}else
...
//	ppoverlay_uzudtg_control1_RBUS ppoverlay_uzudtg_control1_reg;
	...
	// debug print for special panel type
	if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI)
		rtd_pr_vbe_notice("[PanelType] VBY1_TO_HDMI.\n");
	else if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)
		rtd_pr_vbe_notice("[PanelType] LVDS_TO_HDMI.\n");
	else if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)
		rtd_pr_vbe_notice("[PanelType] VBY1_TO_LVDS_TO_HDMI.\n");  
	...
	if(get_product_type() == PRODUCT_TYPE_DIAS){
#ifdef GAME_MODE_DYNAMIC
		if(vbe_disp_get_pre_game_mode_condition()){
			GDMA_ConfigOSDxEnable(GDMA_PLANE_OSD1, FALSE);
		}
#endif
	}
...
*/
	...
	//		vbe_dtg_memec_bypass_switch_set(1);
		...
		}else if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (get_hdmi_4k_hfr_mode() != HDMI_NON_4K120))
		{
			...
			else if(judge_vrr_freesync()){	//Freesync need go low delay mode, bypass memc
				//disable MEMC clock
				Scaler_MEMC_MEMC_CLK_OnOff(0, 0, 1);
				Scaler_MEMC_Bypass_On(1);
...
unsigned int scaler_disp_smooth_get_memc_protect_vtotal(PROTECT_VTOTAL_TYPE eType)
{
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
    {
        vFreq = (pVOInfo->voRealFramerate / 10);
    }
...
void scaler_disp_smooth_variable_setting(void)
{
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		rtd_pr_vbe_notice("[%s][%d]Covertor board=FRC\n",__FUNCTION__,__LINE__);
		modestate_set_fll_running_flag(_DISABLE);
		drvif_scaler_set_display_mode(DISPLAY_MODE_FRC);
	}
	#else
...
#if IS_ENABLED(CONFIG_RTK_KDRV_RPC)
#ifndef CONFIG_SCALER_BRING_UP
	if (RPC_FAIL==send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_ConfigVodmaVFlip, VO_VIDEO_PLANE_V1, Get_PANEL_VFLIP_ENABLE(), &result))
	{
		rtd_pr_vbe_debug("RPC init VO1 VFlip fail!!\n");
		ret=0;
	}
	if (RPC_FAIL==send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_ConfigVodmaVFlip, VO_VIDEO_PLANE_V2, Get_PANEL_VFLIP_ENABLE(), &result))
	{
		rtd_pr_vbe_debug("RPC init VO2 VFlip fail!!\n");
		ret=0;
	}
...
EXPORT_SYMBOL(HAL_VBE_DISP_Initialize);
...
*/
	...
	up(&VBE_Semaphore);
#ifndef UT_flag
    handler_ic_support_panel_type_by_opt();
#endif
	return ret;
}
...
void edp_panel_insert_idle_pattern(void)
{
	edptx0_ml_dphy_dptx_ml_pat_sel_04_RBUS edptx0_ml_dphy_dptx_ml_pat_sel_04_reg;
	edptx1_ml_dphy_dptx_ml_pat_sel_04_RBUS edptx1_ml_dphy_dptx_ml_pat_sel_04_reg;

	rtd_pr_vbe_emerg("edp_panel_insert_idle_pattern!!!\n");
    // TX0
	edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.regValue = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
	edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.tx_ml_pat_sel = 0;//idel pattern
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg, edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);
	msleep(25);
	edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.tx_ml_pat_sel = 4;//video pattern
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg, edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);

    // TX1
	edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.regValue = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
	edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.tx_ml_pat_sel = 0;//idel pattern
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg, edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);
	msleep(25);
	edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.tx_ml_pat_sel = 4;//video pattern
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg, edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);
}

#ifdef CONFIG_ENABLE_EDPTX// [eDP]
// eDP init
...
extern void eDpTx_set_edp_tx_timing_info(unsigned char            macCnt, unsigned int pixelFreq, unsigned char linkRate);
extern void eDpTx_set_Force_MSA_Enhance_Framinig(unsigned char enable);
extern void eDpTx_set_Force_MSA_down_Spread(unsigned char enable);
extern void eDpTx_set_Force_Alternate_SR_Cap(unsigned char enable);

void edp_panel_aux_flow_enable(void)
{
	unsigned char mac_num = 0;

	rtd_pr_vbe_notice("edp_panel_aux_flow_enable\n");
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) ||
		...
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_3K1K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)){
		unsigned char outputPort, timeMode;
		outputPort = _DPTX_TX0_PORT;
		timeMode = Get_DISPLAY_PANEL_CUSTOM_INDEX() - P_EDP_P0_4K2K_60_594M;

//		rtd_pr_vbe_notice("[eDP] panel Type/Index/Name=%d/%d/%s\n",
//										Get_DISPLAY_PANEL_TYPE(), Get_DISPLAY_PANEL_CUSTOM_INDEX(), Panel_GetPanelConfigParameter()->sPanelName);
		rtd_pr_vbe_notice("[eDP] TimeMode=%d@Port[%d]\n", timeMode, outputPort);

		// eDP TX status int (port2, timing 4k2kp60@533D28M)
		UserCommonPowerPanel_Init(_PANEL_OFF);
		eDpTx_set_newOutPort(outputPort);
		eDpTx_set_newTimingMode(timeMode);

		// eDP init
		eDpTx_set_currentTimingMode();
		eDpTx_set_outPort();
		eDpTx_set_Force_MSA_Enhance_Framinig(Get_PANEL_EDP_FORCE_MSA_ENHANCE_FRAMING());
		eDpTx_set_Force_MSA_down_Spread(Get_PANEL_EDP_FORCE_MSA_DOWN_SPREAD());
		eDpTx_set_Force_Alternate_SR_Cap(Get_PANEL_EDP_FORCE_ALTERNATE_SR_CAP());
	}

	// Driver Start
	mdelay(Get_PANEL_OFF_TO_ON_ms());
	rtd_pr_vbe_notice("[eDP] LinkRate=%d, MacNum=%d, clock=%d\n", Get_PANEL_EDP_LINK_RATE(), Get_PANEL_EDP_MAC_NUM(), Get_DISPLAY_CLOCK_TYPICAL());
	if (Get_PANEL_EDP_MAC_NUM() == 0) { // from x panel.ini
		mac_num = ((IoReg_Read32(SFG_SFG_CTRL_0_reg) >> 8) & 0x1F) + 1; // bit[12:8]: seg_num
		rtd_pr_vbe_notice("[eDP] SFG_SFG_CTRL_0_reg=%x, MacNum=%d\n", rtd_inl(SFG_SFG_CTRL_0_reg), mac_num);
	}
	// Get color bits
	eDpTx_set_edp_tx_timing_info(mac_num, Get_DISPLAY_CLOCK_TYPICAL()/1000, Get_PANEL_EDP_LINK_RATE());
	eDpTx_set_newOutPort(mac_num == 2 ? _DPTX_TX0_PORT|_DPTX_TX1_PORT : _DPTX_TX0_PORT);
	eDpTx_set_outPort();
	UserCommonPowerPanelAction(mac_num == 2 ? _DPTX_TX0_PORT|_DPTX_TX1_PORT : _DPTX_TX0_PORT, _PANEL_ON);
}
#endif
...
{
	//unsigned int is_lane_power_on = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_reg);
    ...
    rtd_pr_vbe_emerg("HAL_VBE_DISP_SetDisplayOutput. +++++ (enable:%d)\n", bOnOff);
	...
	if(bOnOff)
	{
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP)){
			msleep(25);
		...
		(Get_DISPLAY_PANEL_TYPE() != P_EDP_3K1K) &&
		(Get_DISPLAY_PANEL_TYPE() != P_EDP_4K1K)) {
		IoReg_SetBits(PLL27X_REG_PLL_SSC4_reg,_BIT0); //Enable SSC
        }

		rtd_pr_vbe_notice("[HAL_VBE_DISP_SetDisplayOutput]  Get_DISPLAY_PANEL_TYPE() =%d\n", Get_DISPLAY_PANEL_TYPE());
		//rtd_pr_vbe_notice("[HAL_VBE_DISP_SetDisplayOutput]  is_lane_power_on =%d\n", is_lane_power_on);
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)||(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)
			|| (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K)){
			vbe_disp_lane_power_on();
		}else if((Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K_2PORT) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_3K1K) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)) {
			vbe_disp_lane_power_on();
			//if (!is_lane_power_on)
#ifdef CONFIG_ENABLE_EDPTX
				//edp_panel_aux_flow_enable();
                if(vbe_get_eco_mode_status_flag())
                {
                    vbe_disp_check_eco_mode(true);
                }
#else
				edp_panel_insert_idle_pattern();
#endif
		}else{
			rtd_pr_vbe_debug("lane power control not execute\n");
		}

        //if((Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
        //    vbe_write_PMIC();

        panel_dlg_switch_panel_mode(panel_dlg_get_cur_dlg_mode());
        panel_dlg_pmic_setting(panel_dlg_get_cur_dlg_mode());
	}
	else
	{
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE) ||
            (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP)){
			rtd_pr_vbe_emerg("HAL_VBE_DISP_SetDisplayOutput. TCON++ (%x)\n", IoReg_Read32(TCON_TCON_IP_EN_reg));
		...
		//KTASKWBS-8895/KTASKWBS-9202 : lane power control by this api
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)||(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)
		|| (Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K_2PORT) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_3K1K) || (Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)
		|| (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K)){
    ...
    rtd_pr_vbe_emerg("RESUME_DEBUG2: 0x18000C20=%x\n", rtd_inl(0xb8000C20));
    rtd_pr_vbe_emerg("RESUME_DEBUG2: 0x1802D9B8=%x\n", rtd_inl(0xb802D9B8));
    rtd_pr_vbe_emerg("RESUME_DEBUG2: 0x1802D70C=%x\n", rtd_inl(0xb802D70C));
    rtd_pr_vbe_emerg("RESUME_DEBUG2: 0x1802DC94=%x\n", rtd_inl(0xb802DC94));
    rtd_pr_vbe_emerg("RESUME_DEBUG2: 0x18000CC0=%x\n", rtd_inl(0xb8000CC0));
...
void HAL_VBE_DISP_SetSpreadSpectrum(BOOLEAN bEnable, UINT16 u16Percent, UINT16 u16Period)
{
	...
	else if((u16Period>=10)&&(u16Period<100))
		u16Period = u16Period*1000;
	else
		u16Period = u16Period*10000;

	u16Percent = u16Percent * 20;
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) ||
		...
		if((bEnable == 0) || (u16Percent ==0) || (Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI)
	        || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_eDP)) {
			...
			pll_ssc0_reg.oc_step_set = ((bpsin == 1) ? 0x10 : 0x200);
		...
		target_freq = ulFreq / 1000000 * (10000+dclk_sscg_range/2);
		lowest_freq = ulFreq / 1000000 * (10000-dclk_sscg_range/2);

		temp_range = (10000-dclk_sscg_range/2)*10000/(10000+dclk_sscg_range/2);
		temp_carry = ((10000-dclk_sscg_range/2)*100000/(10000+dclk_sscg_range/2))%10;

		if(temp_carry > 5)
			temp_range += 1;
		
		//rtd_pr_vbe_debug("dclk_sscg_range:%d, target_freq:%d, lowest_freq:%d, temp_range:%d \n", dclk_sscg_range, target_freq, lowest_freq, temp_range);
		
		temp = target_freq/27*(bpsin + (1-bpsin)*(n_code+2));
		m_code = ((temp/10000)-3);
		f_code = ((temp%10000)*2048)/10000;
		temp_ssc = ((target_freq/10000)*(temp_range)/27*(bpsin + (1-bpsin)*(n_code+2))) + ((target_freq%10000)*(temp_range)/27*(bpsin + (1-bpsin)*(n_code+2))/10000);
		...
		temp = (10000*((m_code-n_code_ssc)*2048+f_code-f_code_ssc)/2/2048 + (n_code_ssc+3)*10000 + (f_code_ssc*10000)/2048);
		if(temp != 0) {
...
unsigned char HAL_VBE_DISP_SetVideoMirror(BOOLEAN bIsH,BOOLEAN bIsV)
{
	...
	rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"HFlip=%d, VFlip = %d\n", bIsH, bIsV);

#if IS_ENABLED(CONFIG_RTK_KDRV_RPC)
#ifndef CONFIG_SCALER_BRING_UP
	if (RPC_FAIL==send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_ConfigVodmaVFlip, VO_VIDEO_PLANE_V1, bIsV, &result))
	{
		rtd_pr_vbe_debug("RPC VO1 VFlip fail!!\n");
		return FALSE;
	}
	if (RPC_FAIL==send_rpc_command(RPC_VIDEO, VIDEO_RPC_VOUT_ToAgent_ConfigVodmaVFlip, VO_VIDEO_PLANE_V2, bIsV, &result))
	{
		rtd_printk(KERN_EMERG, TAG_NAME_VBE,"RPC VO2 VFlip fail!!\n");
		return FALSE;
	}
#endif
#endif
...
void HAL_VBE_DISP_Panel2Pout(UINT8* pixelMode)
{
#ifndef UT_flag
    *pixelMode = get_panel_pixel_mode();
#endif
    Set_Val_vbe_ioctl_cmd(VBE_IOC_PANEL_2P_OUT);
}
...
EXPORT_SYMBOL(HAL_VBE_DISP_GetPanelSize);
...
void HAL_VBE_DISP_SetMprtEnable(BOOLEAN bOnOff)
{
#if 0
    //for ut
    Set_Val_vbe_ioctl_cmd(VBE_IOC_SET_MPRT);
    vbe_mprt_enable = bOnOff;
#ifndef BUILD_QUICK_SHOW
#ifndef UT_flag
    if(bOnOff)
    {
        //HKAID-309 more than 75Hz need set pwm
        if(Get_DISPLAY_REFRESH_RATE() > 75)
        {
            rtk_pwm_bl_en_switch(PWM_MODE, Get_DISPLAY_REFRESH_RATE(), 128);
        }
    }
    else
    {
        rtk_pwm_bl_en_switch(GPIO_MODE, Get_DISPLAY_REFRESH_RATE(), 128);
    }
#endif
#endif
#endif
...
BOOLEAN HAL_VBE_DISP_GetMprtEnable(void)
{
    return vbe_mprt_enable;
}
...
void vbe_disp_panel_suspend(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
	{
		...
		vbe_disp_panel_isp_suspend_resume.ISP_ISP_TST_OUT_S_R=		  IoReg_Read32(ISP_ISP_TST_OUT_reg);										   
		vbe_disp_panel_isp_suspend_resume.ISP_ISP_FIFO_BIST_S_R=	  IoReg_Read32(ISP_ISP_FIFO_BIST_reg);
		...
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K))
	{
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DP_PHY_CTRL_00_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DP_PHY_CTRL_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_0_08_S_R = IoReg_Read32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_0_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_1_0C_S_R = IoReg_Read32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_1_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_2_10_S_R = IoReg_Read32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_2_10_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_S_R = IoReg_Read32(EDPTX1_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_CTRL_34_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_PHY_CTRL_34_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_LANE_SWAP_38_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_LANE_SWAP_38_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_8B10B_TST_3C_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_8B10B_TST_3C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SR_CTRL0_40_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_PHY_SR_CTRL0_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DUMMY_2_4C_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DUMMY_2_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DUMMY_3_50_S_R = IoReg_Read32(EDPTX1_ML_DPHY_DUMMY_3_50_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TX_CTRL_40_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_TX_CTRL_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TIMEOUT_44_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_TIMEOUT_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FSM_STATUS_48_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_FSM_STATUS_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_TRAN_CTRL_4C_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUXTX_TRAN_CTRL_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_REQ_CMD_50_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUXTX_REQ_CMD_50_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_REQ_DATA_54_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUXTX_REQ_DATA_54_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_REPLY_CMD_58_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_REPLY_CMD_58_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_REPLY_DATA_5C_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_REPLY_DATA_5C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_CTRL_6C_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_FIFO_CTRL_6C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TX_FIFO_STATUS_70_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_TX_FIFO_STATUS_70_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_RD_PTR_74_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_FIFO_RD_PTR_74_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_WR_PTR_78_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_FIFO_WR_PTR_78_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_RETRY_1_7C_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_RETRY_1_7C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_RETRY_2_80_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_RETRY_2_80_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_IRQ_EVENT_84_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_IRQ_EVENT_84_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_IRQ_EN_88_S_R = IoReg_Read32(EDPTX1_AUX_MAC_AUX_IRQ_EN_88_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_DUMMY_0_8C_S_R = IoReg_Read32(EDPTX1_AUX_MAC_DUMMY_0_8C_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DIG_PHY_00_S_R = IoReg_Read32(EDPTX1_AUX_DPHY_AUX_DIG_PHY_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DIG_PHY_04_S_R = IoReg_Read32(EDPTX1_AUX_DPHY_AUX_DIG_PHY_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_IRQ_EN_08_S_R = IoReg_Read32(EDPTX1_AUX_DPHY_AUX_IRQ_EN_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_IRQ_FLAG_0C_S_R = IoReg_Read32(EDPTX1_AUX_DPHY_AUX_IRQ_FLAG_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DEBUG_10_S_R = IoReg_Read32(EDPTX1_AUX_DPHY_AUX_DEBUG_10_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_MAC_CTRL_00_S_R = IoReg_Read32(EDPTX0_ML_MAC_DP_MAC_CTRL_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_RESET_CTRL_04_S_R = IoReg_Read32(EDPTX0_ML_MAC_DP_RESET_CTRL_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_DEBUG_CTRL_08_S_R = IoReg_Read32(EDPTX0_ML_MAC_DP_DEBUG_CTRL_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_IRQ_CTRL_0C_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_IRQ_CTRL_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_FIFO_CTRL_10_S_R = IoReg_Read32(EDPTX0_ML_MAC_PG_FIFO_CTRL_10_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MAX_WL_14_S_R = IoReg_Read32(EDPTX0_ML_MAC_MAX_WL_14_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_LFIFO_WL_18_S_R = IoReg_Read32(EDPTX0_ML_MAC_LFIFO_WL_18_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_INTERRUPT_CTRL_9C_S_R = IoReg_Read32(EDPTX0_ML_MAC_PG_INTERRUPT_CTRL_9C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_M_VID_24_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_M_VID_24_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_N_VID_28_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_N_VID_28_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MVID_AUTO_2C_S_R = IoReg_Read32(EDPTX0_ML_MAC_MVID_AUTO_2C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_NVID_ASYNC_M_30_S_R = IoReg_Read32(EDPTX0_ML_MAC_NVID_ASYNC_M_30_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_NVID_ASYNC_L_34_S_R = IoReg_Read32(EDPTX0_ML_MAC_NVID_ASYNC_L_34_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MSA_CTRL_38_S_R = IoReg_Read32(EDPTX0_ML_MAC_MSA_CTRL_38_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MSA_MISC0_3C_S_R = IoReg_Read32(EDPTX0_ML_MAC_MSA_MISC0_3C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HTT_40_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_HTT_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HST_44_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_HST_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HWD_48_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_HWD_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HSW_4C_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_HSW_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VTTE_50_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_VTTE_50_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VST_54_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_VST_54_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VHT_58_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_VHT_58_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VSW_5C_S_R = IoReg_Read32(EDPTX0_ML_MAC_MN_STRM_ATTR_VSW_5C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VBID_60_S_R = IoReg_Read32(EDPTX0_ML_MAC_VBID_60_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VBID_FW_CTL_64_S_R = IoReg_Read32(EDPTX0_ML_MAC_VBID_FW_CTL_64_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_CTRL_68_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_CTRL_68_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_V_DATA_PER_LINE0_6C_S_R = IoReg_Read32(EDPTX0_ML_MAC_V_DATA_PER_LINE0_6C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_TU_SIZE_70_S_R = IoReg_Read32(EDPTX0_ML_MAC_TU_SIZE_70_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_TU_DATA_SIZE_74_S_R = IoReg_Read32(EDPTX0_ML_MAC_TU_DATA_SIZE_74_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_HDEALY_78_S_R = IoReg_Read32(EDPTX0_ML_MAC_HDEALY_78_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_SEC_END_CNT_7C_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_SEC_END_CNT_7C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_DEBUG_80_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_DEBUG_80_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_CTL_8C_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_CTL_8C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV1_90_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_RSV1_90_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV2_94_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_RSV2_94_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VESA_FMT_REGEN_A0_S_R = IoReg_Read32(EDPTX0_ML_MAC_VESA_FMT_REGEN_A0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_MBIST_CTRL_A4_S_R = IoReg_Read32(EDPTX0_ML_MAC_PG_MBIST_CTRL_A4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_DRF_MBIST_CTRL_A8_S_R = IoReg_Read32(EDPTX0_ML_MAC_PG_DRF_MBIST_CTRL_A8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_S_R = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV3_B0_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_RSV3_B0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV8_B4_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_RSV8_B4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV9_B8_S_R = IoReg_Read32(EDPTX0_ML_MAC_DPTX_TOP_RSV9_B8_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_CTRL_C0_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_CTRL_C0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_IRQ_C4_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_IRQ_C4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_IRQ_EN_C8_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_IRQ_EN_C8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER1_CC_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER1_CC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER2_D0_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER2_D0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER3_D4_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER3_D4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER4_D8_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER4_D8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER5_DC_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER5_DC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER6_E0_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER6_E0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER7_E4_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_TIMER7_E4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_LONG0_E8_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_LONG0_E8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_LONG1_EC_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_LONG1_EC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_STATE_F0_S_R = IoReg_Read32(EDPTX0_ML_HPD_HPD_STATE_F0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_RESERVED_0_F4_S_R = IoReg_Read32(EDPTX0_ML_HPD_RESERVED_0_F4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_RESERVED_1_F8_S_R = IoReg_Read32(EDPTX0_ML_HPD_RESERVED_1_F8_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DP_PHY_CTRL_00_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DP_PHY_CTRL_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_0_08_S_R = IoReg_Read32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_0_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_1_0C_S_R = IoReg_Read32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_1_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_2_10_S_R = IoReg_Read32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_2_10_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_S_R = IoReg_Read32(EDPTX0_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_CTRL_34_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_PHY_CTRL_34_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_LANE_SWAP_38_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_LANE_SWAP_38_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_8B10B_TST_3C_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_8B10B_TST_3C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SR_CTRL0_40_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_PHY_SR_CTRL0_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DUMMY_2_4C_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DUMMY_2_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DUMMY_3_50_S_R = IoReg_Read32(EDPTX0_ML_DPHY_DUMMY_3_50_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TX_CTRL_40_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_TX_CTRL_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TIMEOUT_44_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_TIMEOUT_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FSM_STATUS_48_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_FSM_STATUS_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_TRAN_CTRL_4C_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUXTX_TRAN_CTRL_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_REQ_CMD_50_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUXTX_REQ_CMD_50_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_REQ_DATA_54_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUXTX_REQ_DATA_54_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_REPLY_CMD_58_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_REPLY_CMD_58_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_REPLY_DATA_5C_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_REPLY_DATA_5C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_CTRL_6C_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_FIFO_CTRL_6C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TX_FIFO_STATUS_70_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_TX_FIFO_STATUS_70_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_RD_PTR_74_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_FIFO_RD_PTR_74_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_WR_PTR_78_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_FIFO_WR_PTR_78_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_RETRY_1_7C_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_RETRY_1_7C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_RETRY_2_80_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_RETRY_2_80_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_IRQ_EVENT_84_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_IRQ_EVENT_84_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_IRQ_EN_88_S_R = IoReg_Read32(EDPTX0_AUX_MAC_AUX_IRQ_EN_88_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_DUMMY_0_8C_S_R = IoReg_Read32(EDPTX0_AUX_MAC_DUMMY_0_8C_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DIG_PHY_00_S_R = IoReg_Read32(EDPTX0_AUX_DPHY_AUX_DIG_PHY_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DIG_PHY_04_S_R = IoReg_Read32(EDPTX0_AUX_DPHY_AUX_DIG_PHY_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_IRQ_EN_08_S_R = IoReg_Read32(EDPTX0_AUX_DPHY_AUX_IRQ_EN_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_IRQ_FLAG_0C_S_R = IoReg_Read32(EDPTX0_AUX_DPHY_AUX_IRQ_FLAG_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DEBUG_10_S_R = IoReg_Read32(EDPTX0_AUX_DPHY_AUX_DEBUG_10_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL0_S_R = IoReg_Read32(EDPTX_COMMON_EDP_LANE_CTRL0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL1_S_R = IoReg_Read32(EDPTX_COMMON_EDP_LANE_CTRL1_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL2_S_R = IoReg_Read32(EDPTX_COMMON_EDP_LANE_CTRL2_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL3_S_R = IoReg_Read32(EDPTX_COMMON_EDP_LANE_CTRL3_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_TST_SEL_S_R = IoReg_Read32(EDPTX_COMMON_EDP_TST_SEL_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_MAC_CTRL_00_S_R = IoReg_Read32(EDPTX1_ML_MAC_DP_MAC_CTRL_00_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_RESET_CTRL_04_S_R = IoReg_Read32(EDPTX1_ML_MAC_DP_RESET_CTRL_04_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_DEBUG_CTRL_08_S_R = IoReg_Read32(EDPTX1_ML_MAC_DP_DEBUG_CTRL_08_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_IRQ_CTRL_0C_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_IRQ_CTRL_0C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_FIFO_CTRL_10_S_R = IoReg_Read32(EDPTX1_ML_MAC_PG_FIFO_CTRL_10_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MAX_WL_14_S_R = IoReg_Read32(EDPTX1_ML_MAC_MAX_WL_14_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_LFIFO_WL_18_S_R = IoReg_Read32(EDPTX1_ML_MAC_LFIFO_WL_18_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_INTERRUPT_CTRL_9C_S_R = IoReg_Read32(EDPTX1_ML_MAC_PG_INTERRUPT_CTRL_9C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_M_VID_24_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_M_VID_24_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_N_VID_28_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_N_VID_28_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MVID_AUTO_2C_S_R = IoReg_Read32(EDPTX1_ML_MAC_MVID_AUTO_2C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_NVID_ASYNC_M_30_S_R = IoReg_Read32(EDPTX1_ML_MAC_NVID_ASYNC_M_30_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_NVID_ASYNC_L_34_S_R = IoReg_Read32(EDPTX1_ML_MAC_NVID_ASYNC_L_34_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MSA_CTRL_38_S_R = IoReg_Read32(EDPTX1_ML_MAC_MSA_CTRL_38_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MSA_MISC0_3C_S_R = IoReg_Read32(EDPTX1_ML_MAC_MSA_MISC0_3C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HTT_40_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_HTT_40_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HST_44_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_HST_44_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HWD_48_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_HWD_48_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HSW_4C_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_HSW_4C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VTTE_50_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_VTTE_50_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VST_54_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_VST_54_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VHT_58_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_VHT_58_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VSW_5C_S_R = IoReg_Read32(EDPTX1_ML_MAC_MN_STRM_ATTR_VSW_5C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VBID_60_S_R = IoReg_Read32(EDPTX1_ML_MAC_VBID_60_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VBID_FW_CTL_64_S_R = IoReg_Read32(EDPTX1_ML_MAC_VBID_FW_CTL_64_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_CTRL_68_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_CTRL_68_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_V_DATA_PER_LINE0_6C_S_R = IoReg_Read32(EDPTX1_ML_MAC_V_DATA_PER_LINE0_6C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_TU_SIZE_70_S_R = IoReg_Read32(EDPTX1_ML_MAC_TU_SIZE_70_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_TU_DATA_SIZE_74_S_R = IoReg_Read32(EDPTX1_ML_MAC_TU_DATA_SIZE_74_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_HDEALY_78_S_R = IoReg_Read32(EDPTX1_ML_MAC_HDEALY_78_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_SEC_END_CNT_7C_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_SEC_END_CNT_7C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_DEBUG_80_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_DEBUG_80_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_CTL_8C_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_CTL_8C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV1_90_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_RSV1_90_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV2_94_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_RSV2_94_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VESA_FMT_REGEN_A0_S_R = IoReg_Read32(EDPTX1_ML_MAC_VESA_FMT_REGEN_A0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_MBIST_CTRL_A4_S_R = IoReg_Read32(EDPTX1_ML_MAC_PG_MBIST_CTRL_A4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_DRF_MBIST_CTRL_A8_S_R = IoReg_Read32(EDPTX1_ML_MAC_PG_DRF_MBIST_CTRL_A8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_S_R = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV3_B0_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_RSV3_B0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV8_B4_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_RSV8_B4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV9_B8_S_R = IoReg_Read32(EDPTX1_ML_MAC_DPTX_TOP_RSV9_B8_reg);

		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_CTRL_C0_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_CTRL_C0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_IRQ_C4_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_IRQ_C4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_IRQ_EN_C8_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_IRQ_EN_C8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER1_CC_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER1_CC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER2_D0_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER2_D0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER3_D4_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER3_D4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER4_D8_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER4_D8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER5_DC_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER5_DC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER6_E0_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER6_E0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER7_E4_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_TIMER7_E4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_LONG0_E8_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_LONG0_E8_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_LONG1_EC_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_LONG1_EC_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_STATE_F0_S_R = IoReg_Read32(EDPTX1_ML_HPD_HPD_STATE_F0_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_RESERVED_0_F4_S_R = IoReg_Read32(EDPTX1_ML_HPD_RESERVED_0_F4_reg);
		vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_RESERVED_1_F8_S_R = IoReg_Read32(EDPTX1_ML_HPD_RESERVED_1_F8_reg);
	}
}

void edp_panel_resume_new_stage1(void)
{
	//---------------------------------------------------------------------------------------------------------------------------------
	//HPTDN Pin share
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX0_ML_HPD_HPD_CTRL_C0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_CTRL_C0_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_IRQ_C4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_IRQ_C4_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_IRQ_EN_C8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_IRQ_EN_C8_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER1_CC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER1_CC_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER2_D0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER2_D0_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER3_D4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER3_D4_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER4_D8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER4_D8_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER5_DC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER5_DC_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER6_E0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER6_E0_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_TIMER7_E4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_TIMER7_E4_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_LONG0_E8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_LONG0_E8_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_LONG1_EC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_LONG1_EC_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_HPD_STATE_F0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_HPD_STATE_F0_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_RESERVED_0_F4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_RESERVED_0_F4_S_R);
	IoReg_Write32(EDPTX0_ML_HPD_RESERVED_1_F8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_HPD_RESERVED_1_F8_S_R);
	
	IoReg_Write32(EDPTX1_ML_HPD_HPD_CTRL_C0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_CTRL_C0_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_IRQ_C4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_IRQ_C4_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_IRQ_EN_C8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_IRQ_EN_C8_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER1_CC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER1_CC_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER2_D0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER2_D0_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER3_D4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER3_D4_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER4_D8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER4_D8_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER5_DC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER5_DC_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER6_E0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER6_E0_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_TIMER7_E4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_TIMER7_E4_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_LONG0_E8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_LONG0_E8_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_LONG1_EC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_LONG1_EC_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_HPD_STATE_F0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_HPD_STATE_F0_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_RESERVED_0_F4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_RESERVED_0_F4_S_R);
	IoReg_Write32(EDPTX1_ML_HPD_RESERVED_1_F8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_HPD_RESERVED_1_F8_S_R);

	//---------------------------------------------------------------------------------------------------------------------------------
	//eDP Dphy
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX_COMMON_EDP_LANE_CTRL0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL0_S_R);
	IoReg_Write32(EDPTX_COMMON_EDP_LANE_CTRL1_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL1_S_R);
	IoReg_Write32(EDPTX_COMMON_EDP_LANE_CTRL2_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL2_S_R);
	IoReg_Write32(EDPTX_COMMON_EDP_LANE_CTRL3_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_LANE_CTRL3_S_R);
	IoReg_Write32(EDPTX_COMMON_EDP_TST_SEL_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX_COMMON_EDP_TST_SEL_S_R);

	//---------------------------------------------------------------------------------------------------------------------------------
	//--------------------------------TX0_Dphy-------------------------------- 
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX0_ML_DPHY_DP_PHY_CTRL_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DP_PHY_CTRL_00_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_0_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_0_08_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_1_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_1_0C_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_CUSTOM_PATTERN_2_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_CUSTOM_PATTERN_2_10_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_PHY_CTRL_34_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_CTRL_34_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_LANE_SWAP_38_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_LANE_SWAP_38_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_8B10B_TST_3C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_8B10B_TST_3C_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_PHY_SR_CTRL0_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SR_CTRL0_40_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DUMMY_2_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DUMMY_2_4C_S_R);
	IoReg_Write32(EDPTX0_ML_DPHY_DUMMY_3_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_DPHY_DUMMY_3_50_S_R);

	//---------------------------------------------------------------------------------------------------------------------------------
	//--------------------------------TX1_Dphy-------------------------------- 
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX1_ML_DPHY_DP_PHY_CTRL_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DP_PHY_CTRL_00_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_0_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_0_08_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_1_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_1_0C_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_CUSTOM_PATTERN_2_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_CUSTOM_PATTERN_2_10_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_COMPLIANCE_EYE_PATTERN_30_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_PHY_CTRL_34_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_CTRL_34_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_LANE_SWAP_38_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_LANE_SWAP_38_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_8B10B_TST_3C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_8B10B_TST_3C_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_PHY_SR_CTRL0_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SR_CTRL0_40_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_0_44_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DPTX_PHY_SKEW_CTRL_1_48_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DUMMY_2_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DUMMY_2_4C_S_R);
	IoReg_Write32(EDPTX1_ML_DPHY_DUMMY_3_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_DPHY_DUMMY_3_50_S_R);
}

void edp_panel_resume_tx0_mac(void)
{
	//---------------------------------------------------------------------------------------------------------------------------------
	////--------------------------------TX0_mac--------------------------------
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX0_AUX_DPHY_AUX_DIG_PHY_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DIG_PHY_00_S_R);
	IoReg_Write32(EDPTX0_AUX_DPHY_AUX_DIG_PHY_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DIG_PHY_04_S_R);
	IoReg_Write32(EDPTX0_AUX_DPHY_AUX_IRQ_EN_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_IRQ_EN_08_S_R);
	IoReg_Write32(EDPTX0_AUX_DPHY_AUX_IRQ_FLAG_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_IRQ_FLAG_0C_S_R);
	IoReg_Write32(EDPTX0_AUX_DPHY_AUX_DEBUG_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_DPHY_AUX_DEBUG_10_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_TX_CTRL_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TX_CTRL_40_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_TIMEOUT_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TIMEOUT_44_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_FSM_STATUS_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FSM_STATUS_48_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUXTX_TRAN_CTRL_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_TRAN_CTRL_4C_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUXTX_REQ_CMD_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_REQ_CMD_50_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUXTX_REQ_DATA_54_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUXTX_REQ_DATA_54_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_REPLY_CMD_58_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_REPLY_CMD_58_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_REPLY_DATA_5C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_REPLY_DATA_5C_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_FIFO_CTRL_6C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_CTRL_6C_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_TX_FIFO_STATUS_70_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_TX_FIFO_STATUS_70_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_FIFO_RD_PTR_74_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_RD_PTR_74_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_FIFO_WR_PTR_78_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_FIFO_WR_PTR_78_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_RETRY_1_7C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_RETRY_1_7C_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_RETRY_2_80_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_RETRY_2_80_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_IRQ_EVENT_84_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_IRQ_EVENT_84_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_AUX_IRQ_EN_88_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_AUX_IRQ_EN_88_S_R);
	IoReg_Write32(EDPTX0_AUX_MAC_DUMMY_0_8C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_AUX_MAC_DUMMY_0_8C_S_R);
	
	IoReg_Write32(EDPTX0_ML_MAC_DP_MAC_CTRL_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_MAC_CTRL_00_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DP_RESET_CTRL_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_RESET_CTRL_04_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DP_DEBUG_CTRL_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DP_DEBUG_CTRL_08_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_IRQ_CTRL_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_IRQ_CTRL_0C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_PG_INTERRUPT_CTRL_9C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_INTERRUPT_CTRL_9C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_M_VID_24_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_M_VID_24_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_N_VID_28_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_N_VID_28_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MVID_AUTO_2C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MVID_AUTO_2C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_NVID_ASYNC_M_30_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_NVID_ASYNC_M_30_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_NVID_ASYNC_L_34_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_NVID_ASYNC_L_34_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_S_R);

	IoReg_Write32(EDPTX0_ML_MAC_MSA_CTRL_38_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MSA_CTRL_38_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MSA_MISC0_3C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MSA_MISC0_3C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_HTT_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HTT_40_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_HST_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HST_44_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_HWD_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HWD_48_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_HSW_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_HSW_4C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_VTTE_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VTTE_50_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_VST_54_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VST_54_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_VHT_58_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VHT_58_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MN_STRM_ATTR_VSW_5C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MN_STRM_ATTR_VSW_5C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_VBID_60_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VBID_60_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_VBID_FW_CTL_64_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VBID_FW_CTL_64_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_CTRL_68_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_CTRL_68_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_V_DATA_PER_LINE0_6C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_V_DATA_PER_LINE0_6C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_TU_SIZE_70_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_TU_SIZE_70_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_TU_DATA_SIZE_74_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_TU_DATA_SIZE_74_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_HDEALY_78_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_HDEALY_78_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_SEC_END_CNT_7C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_SEC_END_CNT_7C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_DEBUG_80_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_DEBUG_80_S_R);
	
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_CTL_8C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_CTL_8C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_RSV1_90_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV1_90_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_RSV2_94_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV2_94_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_VESA_FMT_REGEN_A0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_VESA_FMT_REGEN_A0_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_PG_MBIST_CTRL_A4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_MBIST_CTRL_A4_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_PG_DRF_MBIST_CTRL_A8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_DRF_MBIST_CTRL_A8_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_RSV3_B0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV3_B0_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_RSV8_B4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV8_B4_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_DPTX_TOP_RSV9_B8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_DPTX_TOP_RSV9_B8_S_R);

	IoReg_Write32(EDPTX0_ML_MAC_PG_FIFO_CTRL_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_PG_FIFO_CTRL_10_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_MAX_WL_14_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_MAX_WL_14_S_R);
	IoReg_Write32(EDPTX0_ML_MAC_LFIFO_WL_18_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX0_ML_MAC_LFIFO_WL_18_S_R);
}

void edp_panel_resume_tx1_mac(void)
{
	//---------------------------------------------------------------------------------------------------------------------------------
	////--------------------------------TX1_mac-------------------------------- 
	//---------------------------------------------------------------------------------------------------------------------------------
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_TX_CTRL_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TX_CTRL_40_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_TIMEOUT_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TIMEOUT_44_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_FSM_STATUS_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FSM_STATUS_48_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUXTX_TRAN_CTRL_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_TRAN_CTRL_4C_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUXTX_REQ_CMD_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_REQ_CMD_50_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUXTX_REQ_DATA_54_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUXTX_REQ_DATA_54_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_REPLY_CMD_58_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_REPLY_CMD_58_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_REPLY_DATA_5C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_REPLY_DATA_5C_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_FIFO_CTRL_6C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_CTRL_6C_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_TX_FIFO_STATUS_70_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_TX_FIFO_STATUS_70_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_FIFO_RD_PTR_74_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_RD_PTR_74_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_FIFO_WR_PTR_78_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_FIFO_WR_PTR_78_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_RETRY_1_7C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_RETRY_1_7C_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_RETRY_2_80_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_RETRY_2_80_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_IRQ_EVENT_84_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_IRQ_EVENT_84_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_AUX_IRQ_EN_88_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_AUX_IRQ_EN_88_S_R);
	IoReg_Write32(EDPTX1_AUX_MAC_DUMMY_0_8C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_MAC_DUMMY_0_8C_S_R);
	IoReg_Write32(EDPTX1_AUX_DPHY_AUX_DIG_PHY_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DIG_PHY_00_S_R);
	IoReg_Write32(EDPTX1_AUX_DPHY_AUX_DIG_PHY_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DIG_PHY_04_S_R);
	IoReg_Write32(EDPTX1_AUX_DPHY_AUX_IRQ_EN_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_IRQ_EN_08_S_R);
	IoReg_Write32(EDPTX1_AUX_DPHY_AUX_IRQ_FLAG_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_IRQ_FLAG_0C_S_R);
	IoReg_Write32(EDPTX1_AUX_DPHY_AUX_DEBUG_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_AUX_DPHY_AUX_DEBUG_10_S_R);

	IoReg_Write32(EDPTX1_ML_MAC_DP_MAC_CTRL_00_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_MAC_CTRL_00_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DP_RESET_CTRL_04_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_RESET_CTRL_04_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DP_DEBUG_CTRL_08_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DP_DEBUG_CTRL_08_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_IRQ_CTRL_0C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_IRQ_CTRL_0C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_PG_INTERRUPT_CTRL_9C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_INTERRUPT_CTRL_9C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_M_VID_24_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_M_VID_24_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_N_VID_28_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_N_VID_28_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MVID_AUTO_2C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MVID_AUTO_2C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_NVID_ASYNC_M_30_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_NVID_ASYNC_M_30_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_NVID_ASYNC_L_34_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_NVID_ASYNC_L_34_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_S_R);

	IoReg_Write32(EDPTX1_ML_MAC_MSA_CTRL_38_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MSA_CTRL_38_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MSA_MISC0_3C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MSA_MISC0_3C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_HTT_40_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HTT_40_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_HST_44_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HST_44_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_HWD_48_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HWD_48_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_HSW_4C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_HSW_4C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_VTTE_50_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VTTE_50_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_VST_54_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VST_54_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_VHT_58_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VHT_58_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MN_STRM_ATTR_VSW_5C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MN_STRM_ATTR_VSW_5C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_VBID_60_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VBID_60_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_VBID_FW_CTL_64_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VBID_FW_CTL_64_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_CTRL_68_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_CTRL_68_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_V_DATA_PER_LINE0_6C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_V_DATA_PER_LINE0_6C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_TU_SIZE_70_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_TU_SIZE_70_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_TU_DATA_SIZE_74_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_TU_DATA_SIZE_74_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_HDEALY_78_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_HDEALY_78_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_SEC_END_CNT_7C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_SEC_END_CNT_7C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_DEBUG_80_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_DEBUG_80_S_R);
	
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_CTL_8C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_CTL_8C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_RSV1_90_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV1_90_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_RSV2_94_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV2_94_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_MIN_H_BLANK_WIDTH_98_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_INTERRUPT_CTRL_9C_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_VESA_FMT_REGEN_A0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_VESA_FMT_REGEN_A0_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_PG_MBIST_CTRL_A4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_MBIST_CTRL_A4_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_PG_DRF_MBIST_CTRL_A8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_DRF_MBIST_CTRL_A8_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_ARBITER_SEC_IDLE_END_CNT_AC_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_RSV3_B0_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV3_B0_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_RSV8_B4_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV8_B4_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_DPTX_TOP_RSV9_B8_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_DPTX_TOP_RSV9_B8_S_R);

	IoReg_Write32(EDPTX1_ML_MAC_PG_FIFO_CTRL_10_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_PG_FIFO_CTRL_10_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_MAX_WL_14_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_MAX_WL_14_S_R);
	IoReg_Write32(EDPTX1_ML_MAC_LFIFO_WL_18_reg,     vbe_disp_panel_edp_suspend_resume.EDPTX1_ML_MAC_LFIFO_WL_18_S_R);
}

void edp_panel_resume_new(void)
{
	edptx0_ml_mac_mn_vid_auto_en_1_20_RBUS edptx0_ml_mac_mn_vid_auto_en_1_20_reg;
	edptx0_ml_mac_msa_ctrl_38_RBUS edptx0_ml_mac_msa_ctrl_38_reg;
	edptx0_ml_mac_arbiter_ctrl_68_RBUS edptx0_ml_mac_arbiter_ctrl_68_reg;
	edptx0_ml_mac_arbiter_debug_80_RBUS edptx0_ml_mac_arbiter_debug_80_reg;
	edptx0_ml_dphy_dptx_ml_pat_sel_04_RBUS edptx0_ml_dphy_dptx_ml_pat_sel_04_reg;
	edptx1_ml_mac_mn_vid_auto_en_1_20_RBUS edptx1_ml_mac_mn_vid_auto_en_1_20_reg;
	edptx1_ml_mac_msa_ctrl_38_RBUS edptx1_ml_mac_msa_ctrl_38_reg;
	edptx1_ml_mac_arbiter_ctrl_68_RBUS edptx1_ml_mac_arbiter_ctrl_68_reg;
	edptx1_ml_mac_arbiter_debug_80_RBUS edptx1_ml_mac_arbiter_debug_80_reg;
	edptx1_ml_dphy_dptx_ml_pat_sel_04_RBUS edptx1_ml_dphy_dptx_ml_pat_sel_04_reg;

	edp_panel_resume_new_stage1();
	rtd_pr_vbe_debug("[edp_panel_resume_new] edp_panel_resume_new_stage1 finish!\n");
	
	edp_panel_resume_tx0_mac();
	rtd_pr_vbe_debug("[edp_panel_resume_new] edp_panel_resume_tx0_mac finish!\n");

	edp_panel_resume_tx1_mac();
	rtd_pr_vbe_debug("[edp_panel_resume_new] edp_panel_resume_tx1_mac finish!\n");

	//TX0 mac double buffer
	edptx0_ml_mac_mn_vid_auto_en_1_20_reg.regValue = IoReg_Read32(EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_reg);
	edptx0_ml_mac_mn_vid_auto_en_1_20_reg.mn_vid_db = 1;
	IoReg_Write32(EDPTX0_ML_MAC_MN_VID_AUTO_EN_1_20_reg,     edptx0_ml_mac_mn_vid_auto_en_1_20_reg.regValue);//DB enable
	edptx0_ml_mac_arbiter_debug_80_reg.regValue = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_DEBUG_80_reg);
	edptx0_ml_mac_arbiter_debug_80_reg.sec_end_cnt_db = 1;
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_DEBUG_80_reg,     edptx0_ml_mac_arbiter_debug_80_reg.regValue);//DB enable
	edptx0_ml_mac_msa_ctrl_38_reg.regValue = IoReg_Read32(EDPTX0_ML_MAC_MSA_CTRL_38_reg);
	edptx0_ml_mac_msa_ctrl_38_reg.msa_db = 1;
	IoReg_Write32(EDPTX0_ML_MAC_MSA_CTRL_38_reg,     edptx0_ml_mac_msa_ctrl_38_reg.regValue);
	edptx0_ml_mac_arbiter_ctrl_68_reg.regValue = IoReg_Read32(EDPTX0_ML_MAC_ARBITER_CTRL_68_reg);//DB enable
	edptx0_ml_mac_arbiter_ctrl_68_reg.arbiter_en = 1;
	IoReg_Write32(EDPTX0_ML_MAC_ARBITER_CTRL_68_reg,     edptx0_ml_mac_arbiter_ctrl_68_reg.regValue);//DB enable

	//TX1 mac double buffer
	edptx1_ml_mac_mn_vid_auto_en_1_20_reg.regValue = IoReg_Read32(EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_reg);
	edptx1_ml_mac_mn_vid_auto_en_1_20_reg.mn_vid_db = 1;
	IoReg_Write32(EDPTX1_ML_MAC_MN_VID_AUTO_EN_1_20_reg,	 edptx1_ml_mac_mn_vid_auto_en_1_20_reg.regValue);//DB enable
	edptx1_ml_mac_arbiter_debug_80_reg.regValue = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_DEBUG_80_reg);
	edptx1_ml_mac_arbiter_debug_80_reg.sec_end_cnt_db = 1;
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_DEBUG_80_reg,     edptx1_ml_mac_arbiter_debug_80_reg.regValue);//DB enable
	edptx1_ml_mac_msa_ctrl_38_reg.regValue = IoReg_Read32(EDPTX1_ML_MAC_MSA_CTRL_38_reg);
	edptx1_ml_mac_msa_ctrl_38_reg.msa_db = 1;
	IoReg_Write32(EDPTX1_ML_MAC_MSA_CTRL_38_reg,     edptx1_ml_mac_msa_ctrl_38_reg.regValue);
	edptx1_ml_mac_arbiter_ctrl_68_reg.regValue = IoReg_Read32(EDPTX1_ML_MAC_ARBITER_CTRL_68_reg);//DB enable
	edptx1_ml_mac_arbiter_ctrl_68_reg.arbiter_en = 1;
	IoReg_Write32(EDPTX1_ML_MAC_ARBITER_CTRL_68_reg,     edptx1_ml_mac_arbiter_ctrl_68_reg.regValue);//DB enable

	//TX0 mac pattern double buffer
	edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.regValue = IoReg_Read32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
	edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.pat_sel_dbuf = 1;
	IoReg_Write32(EDPTX0_ML_DPHY_DPTX_ML_PAT_SEL_04_reg,     edptx0_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);//DB enable

	//TX1 mac pattern double buffer
	edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.regValue = IoReg_Read32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg);
	edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.pat_sel_dbuf = 1;
	IoReg_Write32(EDPTX1_ML_DPHY_DPTX_ML_PAT_SEL_04_reg,     edptx1_ml_dphy_dptx_ml_pat_sel_04_reg.regValue);//DB enable
}
...
void vbe_disp_panel_resume(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
	{
		...
		IoReg_Write32(ISP_ISP_TST_OUT_reg ,       vbe_disp_panel_isp_suspend_resume.ISP_ISP_TST_OUT_S_R);     
		IoReg_Write32(ISP_ISP_FIFO_BIST_reg ,     vbe_disp_panel_isp_suspend_resume.ISP_ISP_FIFO_BIST_S_R); 
		...
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K))
	{
		if(edp_panel_resum_run == 1)
		{
			rtd_pr_vbe_debug("[vbe_disp_panel_resume] EDP panel resume new flow!\n");
			edp_panel_resume_new();
			return;
		}
		//eDP Dphy
		...
		//--------------------------------TX1_Dphy--------------------------------
		IoReg_Write32(0xb8067200, 0x0000000C);  //[7:0]: 4 lane
		IoReg_Write32(0xb8067204, 0x00000042);  //[7:0]: Video pat, DB enable
		                                   //[31:8]: Reserved
		IoReg_Write32(0xb8067234, 0x00000015);  //insert_en=1, skew_en, normal scb, scb_en
		IoReg_Write32(0xb8067238, 0x0000001B);  //[7:0] lane swap, lane0=0,lane1=1,lane2=2,lane3=3
		IoReg_Write32(0xb806723C, 0x00000010);  //[7:0] sr_insert_per_MSB(total=512), tst_mode=0;
		IoReg_Write32(0xb8067240, 0x000000FF);  //[7:0] sr_insert_per_LSB(total=512)
		IoReg_Write32(0xb8067244, 0x00000000);  //[7:0] lane1/lane0: skew no delay
		IoReg_Write32(0xb8067248, 0x00000000);  //[7:0] lane3/lane2: skew no delay
		...
		if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_P0_QHD_165_638M) {
			...
			IoReg_Write32(0xb802df04, 0x000000ff); 		// PN Swap
		...
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_4k2k_60_LM270WR3) {
			IoReg_Write32(0xb802df04, 0x000000ff); // PN SWAP
			IoReg_Write32(0xb8066300, 0x000000C7);      // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8066304, 0x00000000);      // [7:0]:Pg_rst=0,msa_rst=0
			IoReg_Write32(0xb8066308, 0x00000000);      // [7:0]:debug sel
		...
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_QHD_165_LM270WQA) {
			IoReg_Write32(0xb802df04, 0x000000ff); // lane assign
			IoReg_Write32(0xb802df10, 0x03020100); // lane assign
			IoReg_Write32(0xb802df14, 0x07060504); // lane assign
			...
			IoReg_Write32(0xb806631C, 0x00000000);      // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);      // [7] MN_vid auto=dis, [6]:db, [0]:popup
			IoReg_Write32(0xb8066324, 0x00001215);      // [23:0]M_vid=0x1F9A
			IoReg_Write32(0xb8066328, 0x00002000);      // [23:0]N_vid=0x2000
			IoReg_Write32(0xb8066330, 0x00000000);      // [7:0] N_mul=0;
			IoReg_Write32(0xb8066320, 0x00000040);      // [7]=0 FW manually set M/N, [6]=1 double buffer apply
			//// MSA
			...
			IoReg_Write32(0xb806633C, 0x00000040);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
							       // [7:5]:color bit=8 bit [4:3]format=RGB,
			IoReg_Write32(0xb8066340, 0x00000580);      // [15:0]:Htot = 4000
			IoReg_Write32(0xb8066344, 0x00000040);      // [15:0]:Hsta = 80
			IoReg_Write32(0xb8066348, 0x00000500);      // [15:0]:Hwidth = 3840
			IoReg_Write32(0xb806634C, 0x0000000f);      // [15]HSP=low, [14:0]:Hsw = 0x20 = 32
			IoReg_Write32(0xb8066350, 0x000005e1);      // [15:0]:Vtot = 2222
			IoReg_Write32(0xb8066354, 0x00000020);      // [15:0]:Vsta = 54
			IoReg_Write32(0xb8066358, 0x000005a0);      // [15:0]:Vhight = 0x0870 = 2160
			IoReg_Write32(0xb806635C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5
			//// VBID
			IoReg_Write32(0xb8066360, 0x00000000);      // [2]audiomute_flag [1]novideo_flag
			IoReg_Write32(0xb8066364, 0x00000000);      // [1]interlace=0 [0]vbid_fw_ctrl=HW mode
			//// ARBITER
			IoReg_Write32(0xb8066368, 0x00000012);      // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806636C, 0x000004b0);      // [15:0]vdata_per_line=0x0B40 (2880)
							       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
			IoReg_Write32(0xb8066370, 0x00000040);      // [7]TU_size<1,[6:0]:tu_size=0x40 (64)
			IoReg_Write32(0xb8066374, 0x00000110);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
							       // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
			IoReg_Write32(0xb8066378, 0x000000ed);      // [15:0]:hdelay
			IoReg_Write32(0xb806637C, 0x00000064);      // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);      // [7:0]:sec_end_cnt MSB[15:8]
			IoReg_Write32(0xb8066380, 0x00000080);      // sec_end db apply

			////--------------------------------TX1_mac--------------------------------
			IoReg_Write32(0xb8067300, 0x000000C7);      // [7:0]:MSO=4 lane, enhance mode=EN, 4-lane
			IoReg_Write32(0xb8067304, 0x00000000);      // [7:0]:Pg_rst=0,msa_rst=0
			IoReg_Write32(0xb8067308, 0x00000000);      // [7:0]:debug sel
			IoReg_Write32(0xb806730C, 0x00000000);      // [7:0]:irq disable
			IoReg_Write32(0xb806731C, 0x00000000);      // IE disable
			IoReg_Write32(0xb8067320, 0x00000000);      // [7] MN_vid auto=dis, [6]:db, [0]:popup
			IoReg_Write32(0xb8067324, 0x00001215);      // [23:0]M_vid=0x001215
			IoReg_Write32(0xb8067328, 0x00002000);      // [23:0]N_vid=0x2000
			IoReg_Write32(0xb8067330, 0x00000000);      // [7:0] N_mul=0;
			IoReg_Write32(0xb8067320, 0x00000040);      // [7]=0 FW manually set M/N, [6]=1 double buffer apply
			//// MSA
			IoReg_Write32(0xb8067338, 0x00000040);      // [7] msa_db=0 [6]=1 just before MSA
			IoReg_Write32(0xb806733C, 0x00000040);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
			                               // [7:5]:color bit=10 bit [4:3]format=RGB,
			IoReg_Write32(0xb8067340, 0x00000580);      // [15:0]:Htot =1408
			IoReg_Write32(0xb8067344, 0x00000040);      // [15:0]:Hsta = 64
			IoReg_Write32(0xb8067348, 0x00000500);      // [[15:0]:Hwidth = 1280
			IoReg_Write32(0xb806734C, 0x0000000F);      // [15]HSP=low, [14:0]:Hsw = 0x15 = 0F
			IoReg_Write32(0xb8067350, 0x000005E1);      // [15:0]:Vtot = 1505
			IoReg_Write32(0xb8067354, 0x00000020);      // [15:0]:Vsta = 32
			IoReg_Write32(0xb8067358, 0x000005A0);      // [15:0]:Vhight = 0x05A0 = 1440
			IoReg_Write32(0xb806735C, 0x00000005);      // [15]VSP=low, [14:0]:Vsw = 5

			//// VBID
			IoReg_Write32(0xb8067360, 0x00000000);      // [2]audiomute_flag [1]novideo_flag
			IoReg_Write32(0xb8067364, 0x00000000);      // [1]interlace=0 [0]vbid_fw_ctrl=HW mode
			//// ARBITER
			IoReg_Write32(0xb8067368, 0x00000012);       // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806736C, 0x000004B0);      // [15:0]vdata_per_line=0x04B0 (1200)
			                               //  vdata_per_line==> Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
			IoReg_Write32(0xb8067370, 0x00000040);      // [7]TU_size<1,[6:0]:tu_size=0x40 (64)
			IoReg_Write32(0xb8067374, 0x00000110);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
			                              // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
			IoReg_Write32(0xb8067378, 0x000000ed);      // [15:0]:hdelay
			IoReg_Write32(0xb806737C, 0x00000064);      // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80673AC, 0x00000046);      // [7:0]:sec_end_cnt MSB[15:8]
			IoReg_Write32(0xb8067380, 0x00000080);      // sec_end db apply

		}
		else if(Get_DISPLAY_PANEL_CUSTOM_INDEX() == P_EDP_QHD_165_PN315EU02) {
			IoReg_Write32(0xb802df04, 0x000000ff); // lane assign
			IoReg_Write32(0xb802df10, 0x03020100); // lane assign
			IoReg_Write32(0xb802df14, 0x03020100); // lane assign
			...
			IoReg_Write32(0xb806631C, 0x00000000);      // IE disable
			IoReg_Write32(0xb8066320, 0x00000000);      // [7] MN_vid auto=dis, [6]:db, [0]:popup
			IoReg_Write32(0xb8066324, 0x00002835);      // [23:0]M_vid=0x1F9A
			IoReg_Write32(0xb8066328, 0x00002000);      // [23:0]N_vid=0x2000
			IoReg_Write32(0xb8066330, 0x00000000);      // [7:0] N_mul=0;
			IoReg_Write32(0xb8066320, 0x00000040);      // [7]=0 FW manually set M/N, [6]=1 double buffer apply
			//// MSA
			...
			IoReg_Write32(0xb806633C, 0x00000020);      // [14]:color format follow misc1.6, [13]ycc_col=601, [12]VESA, [11]Async,[10:9] no stereo, [8]interlace odd num,
							       // [7:5]:color bit=8 bit [4:3]format=RGB,
			IoReg_Write32(0xb8066340, 0x00000ac8);      // [15:0]:Htot = 4000
			IoReg_Write32(0xb8066344, 0x00000064);      // [15:0]:Hsta = 80
			IoReg_Write32(0xb8066348, 0x00000a00);      // [15:0]:Hwidth = 3840
			IoReg_Write32(0xb806634C, 0x00000040);      // [15]HSP=low, [14:0]:Hsw = 0x20 = 32
			IoReg_Write32(0xb8066350, 0x000005d2);      // [15:0]:Vtot = 2222
			IoReg_Write32(0xb8066354, 0x00000019);      // [15:0]:Vsta = 54
			IoReg_Write32(0xb8066358, 0x000005a0);      // [15:0]:Vhight = 0x0870 = 2160
			IoReg_Write32(0xb806635C, 0x00000004);      // [15]VSP=low, [14:0]:Vsw = 5
			//// VBID
			IoReg_Write32(0xb8066360, 0x00000000);      // [2]audiomute_flag [1]novideo_flag
			IoReg_Write32(0xb8066364, 0x00000000);      // [1]interlace=0 [0]vbid_fw_ctrl=HW mode
			//// ARBITER
			IoReg_Write32(0xb8066368, 0x00000012);      // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode [0]arbiter_en=0
			IoReg_Write32(0xb806636C, 0x00000780);      // [15:0]vdata_per_line=0x0B40 (2880)
							       // Hwidth * BPC (bit per component) * 3 / 8 (byte) = lane_num * v_data_per_line (byte)
			IoReg_Write32(0xb8066370, 0x00000040);      // [7]TU_size<1,[6:0]:tu_size=0x40 (64)
			IoReg_Write32(0xb8066374, 0x000001e3);      // [9:3]tu_data_size=0x1D, [2:0]=0.2=0x0
							       // tu_data_size =-> SCLK(freq) * BPC * 3 /8 (byte)= (TU_DATA_SIZE/TU) * LCLK(freq) * LANE_NUM (byte)
			IoReg_Write32(0xb8066378, 0x0000011b);      // [15:0]:hdelay
			IoReg_Write32(0xb806637C, 0x00000064);      // [7:0]:sec_end_cnt LSB[7:0]
			IoReg_Write32(0xb80663AC, 0x00000046);      // [7:0]:sec_end_cnt MSB[15:8]
			IoReg_Write32(0xb8066380, 0x00000080);      // sec_end db apply
		}
		...
		////TX_TOP
		IoReg_Write32(0xb806738C, 0x00000000);      // I2V_first_blank_filter[1], idel2Vdieo switch=cnt to 8191
		IoReg_Write32(0xb8067390, 0x00000000);      // RSV1
		IoReg_Write32(0xb8067394, 0x00000000);      // RSV2

		////Arbiter
		IoReg_Write32(0xb8067398, 0x00008000);      // (Read_only) [15]wclr [14:0]min_h_blank_wid
		IoReg_Write32(0xb806739C, 0x00000000);      // IE disable

		////Vesa
		IoReg_Write32(0xb80673A0, 0x00000000);      // align Vsync and Hsync = disable
		IoReg_Write32(0xb80673B0, 0x00000013);      // [3] FW M/N code [0] using source's field signal
		IoReg_Write32(0xb80673B8, 0x000000FF);      // [0]=1 Ref BE to gen VBID[0]

		////db apply
		IoReg_Write32(0xb8067338, 0x000000C0);      // [7] msa_db=1 [6]=0 just before MSA
		IoReg_Write32(0xb8067368, 0x00000013);      // [6]idel switch by VS, [5]db=0, [4]video_idle_db=vact_edge[3]Video_idle=0, [2] Hwid_div2=0,[1]vactive_md=new mode
						       // [0]arbiter_en=1

		//TX0 mac
		IoReg_Write32(0xb8066204, 0x00000043);  //[7:0]: Video pattern, DB enable
		IoReg_Write32(0xb8066310, 0x000000ff);  //[7:0]:FIFO status wclr
		IoReg_Write32(0xb8066314, 0x0000007F);  // (Read_only wclr)[7:0]:max_vf_water_level
		IoReg_Write32(0xb8066318, 0x0000007F);  // (Read_only wcle)[7:0]:vf_water_level

		//TX1 mac
		IoReg_Write32(0xb8067204, 0x00000043);  //[7:0]: Video pattern, DB enable
		IoReg_Write32(0xb8067310, 0x000000ff);  //[7:0]:FIFO status wclr
		IoReg_Write32(0xb8067314, 0x0000007F);  // (Read_only wclr)[7:0]:max_vf_water_level
		IoReg_Write32(0xb8067318, 0x0000007F);  // (Read_only wcle)[7:0]:vf_water_level
	}

    panel_dlg_tconless_setting(panel_dlg_get_cur_dlg_mode());
...
void vbe_disp_suspend(void)
{
	...
	gdma_suspend_by_vbe();
#endif
    rtd_pr_vbe_emerg("vbe_disp_suspend+++\n");
	...
	vbe_disp_suspend_resume.CRT_PLL_SSC2_S_R = IoReg_Read32(PLL27X_REG_PLL_SSC2_reg);
	...
	vbe_disp_suspend_resume.CRT_SYS_PLL_DISP4_S_R = IoReg_Read32(PLL27X_REG_SYS_PLL_DISP4_reg);
...
{
    rtd_pr_vbe_emerg("[vbe_disp_init_flow_pre_settings] +++++ \n");
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)  ||
		...
		//Disable Async FIFO,	rtd_outl(0xb802DC94, 0x00000000);
		vbe_disp_suspend_resume.SFG_PIF_FIFO_CTRL0_S_R &= ~_BIT0;
        if((Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
        {
            sfg_disp_if_ctrl_02_RBUS sfg_disp_if_ctrl_02_reg;
            sfg_disp_if_ctrl_02_reg.regValue = IoReg_Read32(SFG_DISP_IF_CTRL_02_reg);
            sfg_disp_if_ctrl_02_reg.clken_sfg = 1;
            sfg_disp_if_ctrl_02_reg.rbus_clken_sfg = 1;
            sfg_disp_if_ctrl_02_reg.clken_tcon = 1;
            sfg_disp_if_ctrl_02_reg.rbus_clken_isp = 1;
            sfg_disp_if_ctrl_02_reg.clken_tcon = 1;
            rtd_outl(SFG_DISP_IF_CTRL_02_reg, sfg_disp_if_ctrl_02_reg.regValue);
        }
        else if(Get_DISPLAY_PANEL_TYPE() == P_CMPI)
        {
            sfg_disp_if_ctrl_02_RBUS sfg_disp_if_ctrl_02_reg;
            sfg_disp_if_ctrl_02_reg.regValue = IoReg_Read32(SFG_DISP_IF_CTRL_02_reg);
            sfg_disp_if_ctrl_02_reg.clken_sfg = 1;
            sfg_disp_if_ctrl_02_reg.rbus_clken_sfg = 1;
            sfg_disp_if_ctrl_02_reg.rbus_clken_epi = 1;
            sfg_disp_if_ctrl_02_reg.clken_tcon = 1;
            rtd_outl(SFG_DISP_IF_CTRL_02_reg, sfg_disp_if_ctrl_02_reg.regValue);
        }
		...
		sfg_disp_if_ctrl_02_reg.clken_sfg = 1;
		sfg_disp_if_ctrl_02_reg.rbus_clken_sfg = 1;
		sfg_disp_if_ctrl_02_reg.rbus_clken_edptx = 1;//0x00101001
		rtd_outl(SFG_DISP_IF_CTRL_02_reg, sfg_disp_if_ctrl_02_reg.regValue);
	}
...
{
    rtd_pr_vbe_emerg("[vbe_disp_init_flow_finish] +++++ \n");

    rtd_pr_vbe_emerg("RESUME_DEBUG_SETTING+++: 0x18000C20=%x\n", rtd_inl(0xb8000C20));
    rtd_pr_vbe_emerg("RESUME_DEBUG_SETTING+++: 0x1802D9B8=%x\n", rtd_inl(0xb802D9B8));
    rtd_pr_vbe_emerg("RESUME_DEBUG_SETTING+++: 0x1802D70C=%x\n", rtd_inl(0xb802D70C));
    rtd_pr_vbe_emerg("RESUME_DEBUG_SETTING+++: 0x1802DC94=%x\n", rtd_inl(0xb802DC94));
    rtd_pr_vbe_emerg("RESUME_DEBUG_SETTING+++: 0x18000CC0=%x\n", rtd_inl(0xb8000CC0));
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_VBY1_3K1K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS) ||
		(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) ||
	...
	(Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP)){
        ...
        if((Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
        {
            sfg_pif_fifo_ctrl_08_RBUS sfg_pif_fifo_ctrl_08_reg;
            sfg_pif_fifo_ctrl_08_reg.regValue = IoReg_Read32(SFG_PIF_FIFO_CTRL_08_reg);
            sfg_pif_fifo_ctrl_08_reg.pif_fifo_en = 1;
            rtd_outl(SFG_PIF_FIFO_CTRL_08_reg, sfg_pif_fifo_ctrl_08_reg.regValue);
        }
        if(Get_DISPLAY_PANEL_TYPE() == P_CMPI)
            IoReg_SetBits(SFG_SFG_DUMMY_reg,_BIT29);
	}else if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)||(Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)){
...
//	UINT32 cnt=0;
    ...
    rtd_pr_vbe_emerg("vbe_disp_resume+++\n");
	...
	if(/*(IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg) & _BIT24) &&*/ !(first_disp_resume)) {
		...
		gdma_resume_by_vbe();
#endif
        if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP))
        {// suspend will Tcon off,now resume
            IoReg_Write32(TCON_TCON_IP_EN_reg, (vbe_disp_panel_tcon_suspend_resume.TCON_TCON_IP_EN_S_R)&(~_BIT0));
        }
		return;
	}
	...
	//IoReg_Write32(SFG_SFG_CTRL_0_reg, vbe_disp_suspend_resume.SFG_SFG_CTRL_0_S_R);
	/*Clk*/
	IoReg_Write32(PLL27X_REG_SYS_PLL_DISP3_reg, 0x4);
	IoReg_Write32(PLL27X_REG_PLL_SSC0_reg, vbe_disp_suspend_resume.CRT_PLL_SSC0_S_R&~_BIT0); //Disable OC_EN
	IoReg_Write32(PLL27X_REG_PLL_SSC4_reg, vbe_disp_suspend_resume.CRT_PLL_SSC4_S_R&~_BIT0); //Disable SSC 
	IoReg_Write32(PLL27X_REG_PLL_SSC2_reg, vbe_disp_suspend_resume.CRT_PLL_SSC2_S_R);
	...
	IoReg_Write32(PLL27X_REG_SYS_PLL_DISP3_reg, vbe_disp_suspend_resume.CRT_SYS_PLL_DISP3_S_R&0xFFFFFF04);
	IoReg_Write32(PLL27X_REG_SYS_PLL_DISP4_reg, vbe_disp_suspend_resume.CRT_SYS_PLL_DISP4_S_R);
	IoReg_SetBits(PLL27X_REG_SYS_PLL_DISP3_reg, _BIT0);
	IoReg_SetBits(PLL27X_REG_SYS_PLL_DISP3_reg, _BIT6);

	udelay(100); //150us for dpll hw apply
	...
	IoReg_ClearBits(PLL27X_REG_SYS_PLL_DISP3_reg, _BIT2);
	...
	gdma_resume_by_vbe();
#endif
    rtd_pr_vbe_emerg("RESUME_DEBUG1: 0x18000C20=%x\n", rtd_inl(0xb8000C20));
    rtd_pr_vbe_emerg("RESUME_DEBUG1: 0x1802D9B8=%x\n", rtd_inl(0xb802D9B8));
    rtd_pr_vbe_emerg("RESUME_DEBUG1: 0x1802D70C=%x\n", rtd_inl(0xb802D70C));
    rtd_pr_vbe_emerg("RESUME_DEBUG1: 0x1802DC94=%x\n", rtd_inl(0xb802DC94));
    rtd_pr_vbe_emerg("RESUME_DEBUG1: 0x18000CC0=%x\n", rtd_inl(0xb8000CC0));
...
{//only for runtime PM low power
    ...
    {
        IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_reg ,  vbe_disp_suspend_resume.PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_S_R);
        rtd_pr_vbe_notice("low_power_lane_power_resume, open lane power,  lane_power=%x \n",IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_reg));


    }
    else
    {
        rtd_pr_vbe_notice("low_power_lane_power_resume, do nothing lane_power=%x \n",IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_0_28_reg));
    }
#endif
    return 0;
}
...
void vbe_disp_lane_power_on(void)
{
	if((Get_DISPLAY_PANEL_TYPE() == P_EDP_4K2K) ||
		...
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_3K1K) ||
		(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)){
		rtd_outl(0xb8066204, 0x00000013);  //[7:0]: TPS1 pattern, DB enable
        }
#ifdef CONFIG_PM
	...
	(Get_DISPLAY_PANEL_TYPE() == P_EDP_4K1K)){
		mdelay(200);
		rtd_outl(0xb8066204, 0x00000033);  //[7:0]: TPS3 pattern, DB enable 
		mdelay(200);
		rtd_outl(0xb8066204, 0x00000003);  //[7:0]: Idle pattern, DB enable 
		mdelay(200);
		rtd_outl(0xb8066204, 0x00000043);  //[7:0]: Video pattern, DB enable
	}
}
...
{
    if(Freesync_mode_enable != enable)
    {
        rtd_pr_new_game_mode_emerg("[%s] %d->%d", __FUNCTION__, Freesync_mode_enable, enable);
    }
    Freesync_mode_enable = enable;
#ifndef BUILD_QUICK_SHOW
    panel_dlg_operator_panel_freesync_mode(Freesync_mode_enable);
#endif
}
...
{
    if(scaler_VRR_timingMode_flag != flag)
    {
        rtd_pr_new_game_mode_emerg("[%s] %d->%d", __FUNCTION__, scaler_VRR_timingMode_flag, flag);
    }
    scaler_VRR_timingMode_flag = flag;
#ifndef BUILD_QUICK_SHOW
    panel_dlg_operator_panel_vrr_mode(scaler_VRR_timingMode_flag);
#endif
}
#ifndef UT_flag
...
BOOL end_time_print_en=FALSE;
BOOL vbe_getEndTimePrintEn(void)
{
	return end_time_print_en;
}


#ifndef BUILD_QUICK_SHOW
#ifndef UT_flag
void handler_ic_support_panel_type_by_opt(void)
{
    int otpRet = rtk_otp_field_read_int_by_name("sw_func_screen_freq_selection");
    ...
    if(otpRet == 0)
    {
        if(dclk > 700000000ULL)
        {
            //osd and video all mute
            vbe_disp_sfg_inner_ptg(1, VBE_DISP_INNER_PTG_TYPE_COLOR_BLACK);
            vbe_disp_lane_power_off();
            rtd_pr_vbe_notice("[%s]otp fail!!!\n", __FUNCTION__);
        }
    }
#if 0 // debug msg
    rtd_pr_vbe_notice("[%s]dclk:%lld, otpRet:%d\n", __FUNCTION__, dclk, otpRet);
#endif
}
...
#ifndef BUILD_QUICK_SHOW
#ifndef UT_flag
    // command : echo vbe_eco=0 > /dev/vbedev
    if(strcmp(cmd_buf, "vbe_eco=0\n") == 0)
    {
        rtd_pr_vbe_emerg("vbe_eco=0\n");
        HAL_VBE_SET_ECO_MODE(FALSE);
    }
    else if(strcmp(cmd_buf, "vbe_eco=1\n") == 0)
    {
        rtd_pr_vbe_emerg("vbe_eco=1\n");
        HAL_VBE_SET_ECO_MODE(TRUE);
    }

    // command : echo vbe_forcevrr=0 > /dev/vbedev
    if(strcmp(cmd_buf, "vbe_forcevrr=0\n") == 0)
    {
        rtd_pr_vbe_emerg("vbe_forcevrr=0\n");
        vbe_disp_set_force_vrr_flag(FALSE);
    }
    else if(strcmp(cmd_buf, "vbe_forcevrr=1\n") == 0)
    {
        rtd_pr_vbe_emerg("vbe_forcevrr=1\n");
        vbe_disp_set_force_vrr_flag(TRUE);
    }
#endif
#endif

    //echo vsc_re > /dev/vbedev
    if(strcmp(cmd_buf, "vsc_re\n") == 0)
    {
        rtd_pr_vbe_notice("[%s] force rerun scaler!!\n", __FUNCTION__, vbe_get_eco_mode_status_flag());
        Scaler_DispSetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE, _MODE_STATE_SEARCH);//Let main path to search state. In order to request VO
        DbgSclrFlgTkr.Main_Run_Scaler_flag = TRUE;//trigger run scaler
        DbgSclrFlgTkr.Main_Scaler_Stop_flag = FALSE;//Let scaler can be run
        vsc_force_rerun_main_scaler = TRUE;
...
#endif
	else if(strcmp(cmd_buf, "edp_panel_resum_run=1\n") == 0){
		edp_panel_resum_run = 1;
		rtd_pr_vbe_emerg("enable edp panel resum new flow \n");
	}
	else if(strcmp(cmd_buf, "edp_panel_resum_run=0\n") == 0){
		edp_panel_resum_run = 0;
		rtd_pr_vbe_emerg("disable edp panel resum new flow \n");
	}
	else if(strcmp(cmd_buf, "end_time_print_en=0\n") == 0){
        end_time_print_en = FALSE;
		rtd_pr_vbe_emerg("disable max end time printf! \n");
	}
	else if(strcmp(cmd_buf, "end_time_print_en=1\n") == 0){
        end_time_print_en = TRUE;
		rtd_pr_vbe_emerg("enable max end time printf! \n");
	}

#ifdef CONFIG_DYNAMIC_PANEL_SELECT
    if(strcmp(cmd_buf, "hsr=1\n") == 0){
        HAL_VBE_Panel_Set_DlgEnable(1);
        rtd_pr_vbe_emerg("enable hsr = 1\n");
    }
    else if(strcmp(cmd_buf, "hsr=0\n") == 0){
        HAL_VBE_Panel_Set_DlgEnable(0);
        rtd_pr_vbe_emerg("enable hsr = 0\n");
    }
#endif

    if(strcmp(cmd_buf, "dvs_protect=0\n") == 0)
    {
        rtd_pr_vbe_emerg("[%s %d]dvs_protect=0\n", __FUNCTION__, __LINE__);
        drvif_scaler_set_dvs_protect(0);
    }
    else if(strcmp(cmd_buf, "dvs_protect=1\n") == 0)
    {
        rtd_pr_vbe_emerg("[%s %d]dvs_protect=1\n", __FUNCTION__, __LINE__);
        drvif_scaler_set_dvs_protect(1);
    }
...
long vbe_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case VBE_IOC_DISP_WRITE_SPREADSPECTRUM:
		{
			...
			{   
                ap_panel_sscg_enable = spreadspectrum.bEnable;
                ap_panel_sscg_percent = spreadspectrum.Percenct;
                ap_panel_sscg_period = spreadspectrum.Period;
        ...
        case VBE_IOC_SET_MPRT:
            {
                BOOLEAN mprtEnable = 0;
                if(copy_from_user((void *)&mprtEnable, (const void __user *)arg, sizeof(BOOLEAN)))
                {
                    retval = -EFAULT;
                    rtd_pr_vbe_debug("scaler vbe ioctl code=VBE_IOC_SET_MPRT failed!!!!!!!!!!!!!!!\n");
                }
                else
                {
                    HAL_VBE_DISP_SetMprtEnable(mprtEnable);
                }
            }
            break;
        case VBE_IOC_PANEL_2P_OUT:
        {
            unsigned char pixelMode = 0;
            HAL_VBE_DISP_Panel2Pout(&pixelMode);
            if (copy_to_user((void __user *)arg, (void *)&pixelMode, sizeof(unsigned char)))
            {
                retval = -EFAULT;
                rtd_pr_vbe_debug("scaler vbe ioctl code = VBE_IOC_PANEL_2P_OUT failed!!!!!!!!!!!!!!!\n");
            }
            break;
        }
#ifdef CONFIG_DYNAMIC_PANEL_SELECT
        case VBE_IOC_SET_DLG_MODE:
        {
            ...
            else
            {
                ...
                if(panel_dlg_get_dlg_change_count() == 0xffffffff)
                {
                    panel_dlg_set_dlg_change_count(0);
                }
                else
                {
                    panel_dlg_set_dlg_change_count(panel_dlg_get_dlg_change_count() + 1);
                }

                if(panel_dlg_check_dlg_mode_change())
                {
...
#ifndef UT_flag
#ifndef BUILD_QUICK_SHOW
        case VBE_IOC_SET_ECO_MODE:
        {
            unsigned int enable = 0;

            if (copy_from_user((void *)&enable, (const void __user *)arg, sizeof(unsigned int)))
            {
                retval = -EFAULT;
                rtd_pr_vbe_debug("scaler vbe ioctl code = VBE_IOC_SET_ECO_MODE failed!!!!!!!!!!!!!!!\n");
            }
            else
            {
                HAL_VBE_SET_ECO_MODE(enable);
            }
            break;
        }

        case VBE_IOC_GET_ECO_MODE:
        {
            unsigned int enable = 0;

            enable = HAL_VBE_GET_ECO_MODE();

            if (copy_to_user((void __user *)arg, (void *)&enable, sizeof(unsigned int)))
            {
                retval = -EFAULT;
                rtd_pr_vbe_err("scaler vbe ioctl code = VBE_IOC_GET_ECO_MODE failed!!!!!!!!!!!!!!!\n");
            }
            break;
        }
#endif
...
static void vbe_shutdown(struct platform_device *pdev)
{
    IO_Direct_Set("PIN_LVDS_ON_OFF", 0);
    IO_Direct_Set("PIN_BL_ON_OFF", 0);

    rtd_pr_vbe_notice("[%s] pin off", __FUNCTION__);
}
...
int vbe_module_init(void)
{
	...
	extern void gdma_get_disp_refresh_rate_ptr(void* fun_ptr);
#endif
...
#if IS_ENABLED(CONFIG_RTK_KDRV_GDMA)
	gdma_get_disp_refresh_rate_ptr(Get_DISPLAY_REFRESH_RATE);
#endif
