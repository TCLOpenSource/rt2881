...
/*				down(&VPQ_ld_running_Semaphore);
				...
				rtd_pr_vpq_info("kernel VPQ_LED_LDEnable success\n");
				ret = 0;
			}
		}
		break;
	...
	case VPQ_LED_IOC_SET_LDSPIEnable:
		{
			...
			if (copy_from_user(&bCtrl, (int __user *)arg, sizeof(unsigned char))) {
				...
				if (fwif_color_Get_LD_Init_Done()) {
					...
					rtd_pr_vpq_info("kernel VPQ_LED_IOC_SET_LDSPIEnable(%d)\n", bCtrl);
				}
	...
	case VPQ_LED_IOC_SET_SLR_VIP_TABLE_LD:
		{
			...
			if (fwif_color_Get_LD_Init_Done()==true){
				rtd_pr_vpq_debug( "[VPQ_LED]  TABLE_LD has done\n");
				break;
			}
		...
		{						
			...
			if (copy_from_user(&m_defaultVipTableBLPF,  (int __user *)arg,sizeof(m_defaultVipTableBLPF))) {
				...
				if (tDRV_LD_Global_Ctrl.ld_blk_type == 0)
					...
					/*LD & LDSPI ISR*/
					fwif_color_set_LD_hw_blight_end_ie2_en(true); /*enable after real chip test*/
	...
	case VPQ_LED_IOC_InitLDSPI:
		{
			...
			if (copy_from_user(&bCtrl, (int __user *)arg, sizeof(unsigned char))) {
				...
				up(&VPQ_LED_Semaphore);
				rtd_pr_vpq_info("[VPQ_LED] kernel VPQ_LED_IOC_InitLDSPI done\n");
				ret = 0;
			}
	...
	case VPQ_LED_IOC_SET_MCULESS_LEDdriver_Arg:	/* AC on will call this API once */
		{
			LEDdriver_Arg tleddriver_args;
			rtd_pr_vpq_info("[VPQ_LED] VPQ_LED_IOC_SET_MCULESS_LEDdriver_Arg start\n");
