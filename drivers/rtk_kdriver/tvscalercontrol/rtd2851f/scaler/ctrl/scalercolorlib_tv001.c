...
{
	extern short RADCR_Enable; extern unsigned char RADCR_BL_ready; extern unsigned char RADCR_CT_ready;
    ...
    if(type == MACH_ARCH_RTK2851F){
		...
		if (RADCR_Enable && RADCR_BL_ready && RADCR_CT_ready) //when DCR, colortemp is dynamic, don't set here
			return;
#endif
...
void Scaler_RADCR_ColorTemp_Init(void)
{
#ifndef BUILD_QUICK_SHOW
	extern void scalerVIP_RADCR_save_osd(TV001_COLOR_TEMP_DATA_S *colorTemp);
	TV001_COLOR_TEMP_DATA_S ct = {0};
	...
	ct.redGain = invoutputgamma_iog_color_temp_contrast_0_reg.iog_contrast_r;
	ct.greenGain = invoutputgamma_iog_color_temp_contrast_0_reg.iog_contrast_g;
	ct.blueGain = invoutputgamma_iog_color_temp_contrast_1_reg.iog_contrast_b;
	ct.redOffset = invoutputgamma_iog_color_temp_brightness_0_reg.iog_bright_r;
	ct.greenOffset = invoutputgamma_iog_color_temp_brightness_0_reg.iog_bright_g;
	ct.blueOffset = invoutputgamma_iog_color_temp_brightness_1_reg.iog_bright_b;

	scalerVIP_RADCR_save_osd(&ct);
#endif
}
...
//#ifdef ANDTV_MAC5_COMPILE_FIX
	switch(pqModule){
		...
		if(onOff == 0){
#if 0	// no need to disable PQ for new user menu
		...
		if(onOff == 0){
			...
			drvif_color_set_PQ_ByPass(PQ_ByPass_DCC);
			drvif_color_set_PQ_ByPass(PQ_ByPass_DCTI);
			//drvif_color_set_PQ_ByPass(PQ_ByPass_ICM);	// can't bypass LC, only disable ICM Y2R/R2Y in RGB 444
			drvif_color_set_PQ_ByPass(PQ_ByPass_SHP);
			drvif_color_set_PQ_ByPass(PQ_ByPass_CDS);
			drvif_color_set_PQ_ByPass(PQ_ByPass_UV_Offset);
			drvif_color_set_PQ_ByPass(PQ_ByPass_V_CTI);
			drvif_color_set_PQ_ByPass(PQ_ByPass_De_Contour);
			//drvif_color_set_PQ_ByPass(PQ_ByPass_LC);	// can't bypass LC, only enable LC R2Y in RGB 444
			drvif_color_set_PQ_ByPass(PQ_ByPass_SU_PEAKING);
			drvif_color_set_PQ_ByPass(PQ_ByPass_UV_Delay);
			drvif_color_set_PQ_ByPass(PQ_ByPass_I_LineBuff);
...
unsigned int Scaler_GetHdr10Enable(unsigned char * bEnable)
{
	...
	if(VIP_RPC_system_info_structure_table == NULL){
		rtd_pr_vpq_debug("[%s:%d] Warning here!! RPC_system_info=NULL! \n",__FILE__, __LINE__);
		return _FAIL;
	}
...
void Scaler_SetDemoMode(TV001_DEMO_MODE_E demoMode,unsigned char onOff)
{
	if(!onOff){ //off
		...
		case TV001_DEMO_SDR_TO_HDR:
		{
			_RPC_system_setting_info *VIP_system_RPC_info_structure_table;
			VIP_system_RPC_info_structure_table = (_RPC_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_RPC_system_info_structure);
			if(VIP_system_RPC_info_structure_table && VIP_system_RPC_info_structure_table->HDR_info.Ctrl_Item[TV006_HDR_En])
...
void fwif_color_gamma_encode_TableSize(RTK_TableSize_Gamma *pData)
{
	...
	if (gVip_Table == NULL) {
		rtd_pr_vpq_debug("~get vipTable Error return, %s->%d, %s~\n", __FILE__, __LINE__, __FUNCTION__);
		return;
	}
	...
	Gamma_Mode =1 ;
	//rtd_pr_vpq_debug("yuan,OSD_Color_name=%d,Gamma_Mode=%d\n",OSD_Color_name,Gamma_Mode);
	...
	//rtd_pr_vpq_debug("OSD_Color_temp= %d\n", OSD_Color_temp);
	if (OSD_Color_temp !=  0) {
	...
	//for(i=0;i<3;i++)
		//rtd_pr_vpq_debug("[tim_li]after mapping low:i=%d,r=%d,g=%d,b=%d \n",i,pData->pu16Gamma_r[i],pData->pu16Gamma_g[i],pData->pu16Gamma_b[i]);

	//for(i=253;i<256;i++)
		//rtd_pr_vpq_debug("[tim_li]after mapping high:i=%d,r=%d,g=%d,b=%d \n",i,pData->pu16Gamma_r[i],pData->pu16Gamma_g[i],pData->pu16Gamma_b[i]);

	//rtd_pr_vpq_debug("[tim_li]after mapping max:r=%d,g=%d,b=%d \n",maxvalueR,maxvalueG,maxvalueB);
#endif
	for(i = 0 ; i < 256 ; i++)
	{
		//rtd_pr_vpq_debug("-lhh001- TCL_g[%d] = %d    %d    %d\n", i, pData->pu16Gamma_r[i], pData->pu16Gamma_g[i], pData->pu16Gamma_b[i]);
		if(i < 255)
		{
			for(j = 0 ; j < 4 ; j++)
			{
...
void Scaler_Get_ScreenPixel_info(SCREEN_PIXEL_INFO* sp_info)
{
    ...
    rtd_pr_vpq_debug("scaler_get_screen_pixel_info !!!!h_sta(%d)===h_end(%d)!\n",sp_info->h_sta,sp_info->h_end);
	rtd_pr_vpq_debug("scaler_get_screen_pixel_info !!!!v_sta(%d)===v_end(%d)!\n",sp_info->v_sta,sp_info->v_end);
    ...
    rtd_pr_vpq_debug("The capture window too large, get screen pixel  total_pixel(%d)!!!\n",total_pixel);
	//if(total_pixel > 200)
	//{
	//	rtd_pr_vpq_debug("The capture window too large, get screen pixel fail !!!\n");
	//	return;
	//}
...
void Scaler_Set_hdmi_provider_type(unsigned char mode)
{
	...
	rtd_pr_vpq_debug("yuan,Scaler_Set_PCmode=%d\n",mode);

	if(VIP_RPC_system_info_structure_table == NULL){
		rtd_pr_vpq_debug("[%s:%d] Warning here!! RPC_system_info=NULL!(%p) \n",__FILE__, __LINE__,VIP_RPC_system_info_structure_table);
		return;
	}
	...
	switch(demoMode)
	{
		...
		case PQ_DEMO_HDR_SIDE_BY_SIDE:
			if(onoff)
			{
				...
				//fwif_color_set_Gamma_InvGamma_Ctrl(1, 1, invGammaTBL, invGammaTBL, invGammaTBL);//enable sub inverse gamma
...
void Scaler_SetBrightness_LUT_OETF(unsigned short *Out_T , BRIGHTNESS_LUT_OETF *pData)
{
	...
	#if 0//for debug
			UINT16 m;
			rtd_pr_vpq_debug("1yuan=============\n");
			for (m = 0; m < 256; m++)
				rtd_pr_vpq_debug("pf_LUT[%d] = %d\n", m, pData->pf_LUT[m]);
	#endif
	...
	#if 0//for debug
			UINT16 n;
				rtd_pr_vpq_debug("2yuan=============\n");
			for (n = 0; n < 257; n++)
				rtd_pr_vpq_debug("In_OETF[%d] = %d\n", n, In_OETF[n]);
	#endif
...
#if 0//for debug
				UINT16 k;
					rtd_pr_vpq_debug("3yuan=============\n");
				for (k = 0; k < 1025; k)
					rtd_pr_vpq_debug("Out_T[%d] = %d\n", k, Out_T[k]);
...
void Scaler_SetOETF2Gamma(BRIGHTNESS_LUT_OETF *pData)
{
	...
	static unsigned short intbl[257];
	             #if 0//for debug
			UINT16 m;
			rtd_pr_vpq_debug("1yuan====Scaler_SetOETF2Gamma=========\n");
			for (m = 0; m < 256; m++)
				rtd_pr_vpq_debug("pf_LUT[%d] = %d\n", m, pData->pf_LUT[m]);
	              #endif
	...
	for(i=0;i<256;i++){
		bit_diff = ((intbl[(i+1)]-intbl[i])>>2);
			...
			#if 0//for debug
			UINT16 k;
			rtd_pr_vpq_debug("yuan=============\n");
			for (k = 0; k < 1025; k++)
			rtd_pr_vpq_debug("OEFT2Gamma[%d] = %d\n", k, OEFT2Gamma[k]);
			#endif
...
{
	rtd_pr_vpq_debug("yuan,inv/sRGB=%d\n",onoff);
	if(onoff==1){
...
void Scaler_Set_PictureMode_PQsetting(RTK_VPQ_MODE_TYPE_TV030 mode)
{
	...
	rtd_pr_vpq_debug("yuan,PictureMode=%d\n",mode);
	...
	case PQ_MODE_HDR_ENHANCE_WIDE:
		Scaler_set_ICM_table(9,0);
		rtd_pr_vpq_debug("PictureMode_PQsetting :NOT SUPPORT!!\n");
		break;
	...
	case PQ_MODE_SHOP_DEMO: //SHOP MODE
		rtd_pr_vpq_debug("tim_li,HOME -> SHOP dcc !!! \n");
	...
	if((picturemode !=mode) && (picturemode==PQ_MODE_SHOP_DEMO)){
		rtd_pr_vpq_debug("tim_li,SHOP -> HOME !!! \n");
