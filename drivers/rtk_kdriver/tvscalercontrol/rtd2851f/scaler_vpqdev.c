...
extern UINT32 Identity_Gamma[512];
...
void vpq_init_gamma_invgamma_invoutgamma(void)
{
#ifndef UT_flag
	...
	fwif_color_write_InvOutputGamma_curve(Identity_Gamma, Identity_Gamma, Identity_Gamma, 0, 0);
...
#ifndef UT_flag
	vpq_init_gamma_invgamma_invoutgamma();
#endif
...
{
	if (mVIP_PCIDTable.VIP_PCID_En_Ctrl.LINEOD_EN == 1)
	{
		fwif_color_set_LINEOD_by_pSLRTBL(&mVIP_PCIDTable);	
		rtd_pr_vpq_info("pcid_do_resume, ini from bin flow\n");
	}
	else
	{
...
{
	if (((mVIP_VALC_Table.VIP_VALC_En_Ctrl.PCID_EN == 1) || (mVIP_VALC_Table.VIP_VALC_En_Ctrl.PCID2_EN == 1)) && 
		(mVIP_VALC_Table.VALC_TBL.VALC_CTRL.VALC_En == 1))
	{
		fwif_color_set_VALC_by_pSLRTBL(&mVIP_VALC_Table);	
		rtd_pr_vpq_info("valc_do_resume, ini from bin flow\n");
	}
}
...
void vpq_do_resume(void)
{
	...
	fwif_color_inv_gamma_control_back(SLR_MAIN_DISPLAY, 0);
	//vpq_set_gamma(1, 0);
	//fwif_color_colorwrite_Output_gamma(Output_InvOutput_Gamma_TBL_Reserved04);	// use vpq_do_OUTGamma_init();
	drvif_color_inv_gamma_enable(SLR_MAIN_DISPLAY, 0, 0);
	drvif_color_inv_gamma_enable(SLR_SUB_DISPLAY, 0, 0);
...
unsigned int sem_se_start_wait_t = 0;
...
#else
		sem_se_start_wait_t = drvif_color_get_cur_counter();
...
int vpq_rumtime_suspend(struct device *p_dev)
{
	...
	{//wait vpq freeze
		udelay(50);
		if(drvif_color_wait_timeout_check_by_counter(cur_time1, 5000))
		{
		    rtd_pr_vpq_err("### VPQ_RTPM [err] wait VPQ related task pending fail ###\r\n");
#if IS_ENABLED(CONFIG_RTK_AI_DRV)
			rtd_pr_vpq_err("### VPQ_RTPM [err] tsk status %d %d %d %d %d, sem_se t=%x, c=%x ###\r\n", 
				Get_Var_VPQ_rtpm_tsk_VPQ_freeze_stauts(), Get_Var_VPQ_rtpm_tsk_block_VPQ_done(),
				Get_Var_VPQ_rtpm_tsk_SE_freeze_stauts(), Get_Var_VPQ_rtpm_tsk_block_SE_done(), 
				Get_Var_VPQ_rtpm_tsk_block(), sem_se_start_wait_t, drvif_color_get_cur_counter());
#endif
		    return -1;
		}
	}
...
#ifndef UT_flag    
    if(fwif_color_set_Panel_Dither_By_ini() == 0){
    	if (Get_DISPLAY_PANEL_MPLUS_RGBW() == TRUE || Get_DISPLAY_PANEL_BOW_RGBW() == TRUE)
    		fwif_color_set_Panel_Dither(TRUE,VIP_PANEL_BIT8,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15);
    	else
    		fwif_color_set_Panel_Dither(TRUE,VIP_PANEL_BIT10,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15);
...
{
	unsigned char ret;
	unsigned char vpq_done=0,se_done=0;

	if (Get_Var_VPQ_fw_tsk_running_flag() == 1)
		vpq_done = Get_Var_VPQ_rtpm_tsk_block_VPQ_done() | Get_Var_VPQ_rtpm_tsk_VPQ_freeze_stauts();
	else
		vpq_done = 1;

	if (Get_Var_VPQ_se_tsk_running_flag() == 1)
		se_done = Get_Var_VPQ_rtpm_tsk_block_SE_done() | Get_Var_VPQ_rtpm_tsk_SE_freeze_stauts();
	else
		se_done = 1;
	...
	else if (((vpq_done == 1)&&(se_done == 1)) && (Get_Var_VPQ_rtpm_tsk_block() == 1) ){
		ret = 1;
	}
	else{
		ret = 0;
