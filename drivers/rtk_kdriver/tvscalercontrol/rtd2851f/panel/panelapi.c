...
unsigned int Get_DISPLAY_CLOCK_TYPICAL(void)
{
    ...
    if(vbe_get_eco_mode_status_flag())
    {
        new_dclk = Get_DISPLAY_REFRESH_RATE() * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL();
	...
	new_dclk = (Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ)+1) * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL();
