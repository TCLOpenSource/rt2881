...
void Scaler_SetColorTempData(TV001_COLOR_TEMP_DATA_S *colorTemp)
{
	...
	if ((Scaler_APDEM_Arg_Access(DEM_command_Sent_NUM, 0, 0) == 0) || ((fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030)||(Scaler_color_Get_flow_type()==FLOW_TV30))) {
	...
	(colorTemp->redOffset-512)*4, (colorTemp->greenOffset-512)*4, (colorTemp->blueOffset-512)*4, WaitSync);
		...
		if (RADCR_Enable) //when DCR, colortemp is dynamic, don't set here
			return;
...
{
	rtd_pr_vpq_info("%s demoMode = %d, onOff = %d\n", __FUNCTION__, demoMode, onOff);
	if(!onOff){ //off
		switch(demoMode) {
...
#if 1 //for debug
		rtd_pr_vpq_info("osd,R:R=%d,G=%d,B=%d\n",osd[0][0],osd[0][1],osd[0][2]);
		rtd_pr_vpq_info("osd,G:R=%d,G=%d,B=%d\n",osd[1][0],osd[1][1],osd[1][2]);
		rtd_pr_vpq_info("osd,B:R=%d,G=%d,B=%d\n",osd[2][0],osd[2][1],osd[2][2]);
		rtd_pr_vpq_info("osd,Y:R=%d,G=%d,B=%d\n",osd[3][0],osd[3][1],osd[3][2]);
		rtd_pr_vpq_info("osd,C:R=%d,G=%d,B=%d\n",osd[4][0],osd[4][1],osd[4][2]);
		rtd_pr_vpq_info("osd,M:R=%d,G=%d,B=%d\n",osd[5][0],osd[5][1],osd[5][2]);
#endif	
