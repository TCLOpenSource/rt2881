...
#define MEMC_AUTOREGRESSION_NUM			(5*6)//AUTOREGRESSION_TYPE_NUM & _Self_Check_Result
...
extern StructColorDataFacModeType g_curColorFacTable_airplay_Ex;
extern VIP_GLC_CTRL gVIP_GLC_CTRL;
extern VIP_GLC_INFO gVIP_GLC_INFO;
extern SLR_VIP_TABLE_GLC m_VIP_GLC_TBL;
extern unsigned char VPQEX_i2c_rlink_set_test_for_rtice(unsigned char mode, unsigned char isSetFlag);
...
//		memcpy(buf, (unsigned char *) &(smartPic_clue->Adapt_DCC_Info.S_RPC_DCC_LAYER_Array.Apply_Curve), size);
	...
	case TOOLACCESS_PCID_RgnWeightTbl_8x24: /*1253 get*/
...
*/
	...
	case TOOLACCESS_StructColorDataFacModeType_AIRPLAY:
	{
		if (size > sizeof(StructColorDataFacModeType))
			return -ID_SIZE_ERR;
		
		memcpy(buf, &g_curColorFacTable_airplay_Ex, size);
		*num_type = VIP_UINT8;
...
*/
	...
	case TOOLACCESS_VIP_PQ_AP_DIAS_ENG_MENU_TEST:
		...
		buf[0] = Scaler_APDEM_Arg_Access(rtice_param.mode0, 0, 0);
	...
	{		
		...
		bTemp = drvif_color_get_gamma_enable(SLR_MAIN_DISPLAY);
		drvif_color_set_gamma_enable(SLR_MAIN_DISPLAY, 0);
...
*/
	...
	case TOOLACCESS_VIP_SLD_Global_SLD_TBL:
		...
		*num_type = VIP_INT16;
		...
		break;
#endif
	case TOOLACCESS_VIP_GLC_CTRL:
		if (size+start_addr > sizeof(VIP_GLC_CTRL))
			return -ID_SIZE_ERR;

		pwUINT16Temp = (unsigned short*)&gVIP_GLC_CTRL;
	...
	case TOOLACCESS_VIP_GLC_INFO:
		if (size+start_addr > sizeof(VIP_GLC_INFO))
			return -ID_SIZE_ERR;

		pwUINT16Temp = (unsigned short*)&gVIP_GLC_INFO;
	...
	case TOOLACCESS_VIP_GLC_TBL:
		if (size+start_addr > sizeof(SLR_VIP_TABLE_GLC))
			return -ID_SIZE_ERR;

		pwUINT16Temp = (unsigned short*)&m_VIP_GLC_TBL;
	...
	case TOOLACCESS_gVIP_AUTOMA_Time_Profile:
	{
		if (size+start_addr > sizeof(VIP_AUTOMA_Time_CTRL))
			return -ID_SIZE_ERR;

		pwUINT32Temp = (unsigned int*)(&gVIP_AUTOMA_Time_Profile);
		
		memcpy(buf, pwUINT32Temp+start_addr, size);
...
/************************************************************/
	...
	break;
#ifdef CONFIG_H5CX_SUPPORT
	case TOOLACCESS_Rlink_i2c_DEBUG:
	{
		if (size > sizeof(char))
			return -ID_SIZE_ERR;
		u8_i = VPQEX_i2c_rlink_set_test_for_rtice(0, 0);

		memcpy(buf, &u8_i, size);
	...
	case TOOLACCESS_NNSR_VIP_TABLE:
	{
		...
		if(rtice_param.mode0 == 1) {
			unsigned short *pAddr[] = {
				gVipSRNNTbl.SRNN_Param.Param_SRNN_mode2_2X,
				gVipSRNNTbl.SRNN_Param.Param_SRNN_mode5_3X,
				gVipSRNNTbl.SRNN_Param.Param_SRNN_mode7_4X,
				gVipSRNNTbl.SRNN_Param.Param_SRNN_HSR_mode2_2X,
				gVipSRNNTbl.SRNN_Param.Param_SRNN_HSR_mode5_3X,
				gVipSRNNTbl.SRNN_Param.Param_SRNN_HSR_mode7_4X,
			};
...
//			Scaler_SetMA_Chroma_Error(buf[12]); // merlin4 remove
	...
	case TOOLACCESS_StructColorDataFacModeType_AIRPLAY:
	{
		...
		memcpy(&g_curColorFacTable_airplay_Ex, buf, size);
	...
	case TOOLACCESS_VIP_PQ_Power_Saving:
		...
		case TOOLACCESS_VIP_PQ_AP_DIAS_ENG_MENU_TEST:
			...
			pwCopyTemp = buf[0];
			//fwif_color_ChangeUINT32Endian(pwUINT32Temp, size/sizeof(int), 1);
			Scaler_set_APDEM(rtice_param.mode0, (void*)&pwCopyTemp);
		...
		{		
			...
			fwif_set_gamma_system(0, pwINT16Temp, pwINT16Temp+1025, pwINT16Temp+2050, 1);

			drvif_color_set_gamma_enable(SLR_MAIN_DISPLAY, bTemp);
...
//		vpq_PQ_setting_check();
	...
	case TOOLACCESS_gVIP_AUTOMA_Time_Profile:
	{
		...
		memcpy(pwUINT32Temp+start_addr, buf, size);
	}
	break;
...
/************************************************************/
	...
	break;
		...
		case TOOLACCESS_Rlink_i2c_DEBUG:
		{
			if (size > sizeof(char))
				return -ID_SIZE_ERR;
			memcpy(&bTemp, buf, size);
			VPQEX_i2c_rlink_set_test_for_rtice(bTemp, 1);
		}
		break;
#endif
#ifdef CONFIG_CUSTOMER_TV002
...
int  rtice_SetGet_PQ_Power_Saving(ap_data_param rtice_param, unsigned char *num_type, unsigned char *buf, _system_setting_info *system_setting_info, unsigned char isSet_Flag)
{
	...
	if (isSet_Flag == 1) {
		...
		ret = (unsigned char)Scaler_color_Set_PQ_ByPass_Lv(lv, 0);
