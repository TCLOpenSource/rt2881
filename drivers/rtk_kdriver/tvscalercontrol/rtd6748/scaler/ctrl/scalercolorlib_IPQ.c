...
void Scaler_SetOETF2Gamma(BRIGHTNESS_LUT_OETF *pData)
{
	...
	for(i=0;i<256;i++){
		if(i>0){
			if(pData->pf_LUT[i]<pData->pf_LUT[i-1]){
				rtd_pr_vpq_info("Scaler_SetOETF2Gamma():ERROR!! data invert!!! \n");
				return;
			}
		}
	...
	if (srcType == _SRC_VGA) {
		...
		if (drvif_IsHDMI() && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_MODECURR) != _MODE_NEW)) {
			if (bUse_HDMI_Range_Detect && data_range == HDMI_CSC_DATA_RANGE_MODE_FULL) {
				...
				if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) && (drvif_Hdmi_GetColorSpace() == COLOR_RGB)){
		...
		if (VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_SDR_MAX_RGB) {	// max RGB
			if ((Scaler_DispGetInputInfo(SLR_INPUT_COLOR_SPACE) == COLOR_RGB) || (Scaler_VOFrom(Scaler_Get_CurVoInfo_plane()) == 2)) {
				...
				rtd_pr_vpq_info("[y2r] 222222 colorFMT_info=%d,jpeg=%d \n",colorFMT_info,Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()));
				if (colorFMT_info == isBT2020) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
				} else if (colorFMT_info == is709) {
					if (Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()) == 1)
				...
				rtd_pr_vpq_info("[y2r] 33333 colorFMT_info=%d,jpeg=%d  \n",colorFMT_info,Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()));
				if (colorFMT_info == isBT2020) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
				} else if (colorFMT_info == is709) {
					if (Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()) == 1)
...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table_SDR_MAX_RGB_TV043(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	...
	if (nSrcType == _SRC_HDMI) {
		...
		rtd_pr_vpq_info("[rgb2yuv]HDMI_CSC_DataRange_Mode=%d,IsRGB_YUV_RANGE=%d,data_range=%d \n",HDMI_CSC_DataRange_Mode,IsRGB_YUV_RANGE,data_range);
	} else if (nSrcType == _SRC_VO) {
		rtd_pr_vpq_info("[rgb2yuv]nSrcType == _SRC_VO \n");
		if (pVOInfo->colour_primaries == 1) {
			colorFMT_info = is709;
		} else if (pVOInfo->colour_primaries == 6 || pVOInfo->colour_primaries == 7) {
			colorFMT_info = is601;
	...
	// "Input_Data_Mode" always use limited, D-doamin will always use "limit to full"
		...
		rtd_pr_vpq_info("[rgb2yuv]:maxrgb,nSrcType=%d,video_format=%d,colorFMT_info=%d \n",nSrcType,video_format,colorFMT_info);
		if (nSrcType == _SRC_HDMI) {
			if (video_format !=  0) {/*YUV444 or YUV422*/
				if (colorFMT_info == isBT2020) {
			...
			rtd_pr_vpq_info("[rgb2yuv]other source \n");
			if (colorFMT_info == isBT2020) {
				table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC2020_0_255;
				VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_2020_NonConstantY;
			} else if (colorFMT_info == is709) {
					if(Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane()) == 1 || vdec_src_type == VSC_VDEC_CASE_AIRPLAY){// 0) DTV, 1) JPEG
						table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC709_16_235;
						rtd_pr_vpq_info("[rgb2yuv]maxrgb VO JPEG RGB2YUV:F2L;YUV2RGB :709_F2L\n");
					}else{
						table_num = RGB2YUV_COEF_RGB_16_235_TO_YCC709_16_235;
						rtd_pr_vpq_info("[rgb2yuv]maxrgb VO src RGB2YUV:L2L;YUV2RGB :709_L2L\n");
					}
					VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_709_Limted;
				
			} else if (colorFMT_info == is601) {
				if( vdec_src_type == VSC_VDEC_CASE_AIRPLAY){
					table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC601_16_235;
					VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_601_Limted;
				}else{
					table_num = RGB2YUV_COEF_RGB_16_235_TO_YCC601_16_235;
		...
		if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG) {	// JPEG flag comes from VSC connect, and don't enter DM
			rtd_pr_vpq_info("[r2y] 1111111,colorFMT_info=%d \n",colorFMT_info);
			if (colorFMT_info == isBT2020) {
				table_num = RGB2YUV_COEF_BYPASS_NORMAL_GAIN;
				VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_2020_NonConstantY;
			} else if (colorFMT_info == is709) {
			...
			rtd_pr_vpq_info("[r2y] 22222,colorFMT_info=%d \n",colorFMT_info);
			if (colorFMT_info == isBT2020) {
				table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC2020_16_235;
				VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_2020_NonConstantY;
			} else if (colorFMT_info == is709) {
			...
			rtd_pr_vpq_info("[r2y] 33333,colorFMT_info=%d \n",colorFMT_info);
			if (colorFMT_info == isBT2020) {
				table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC2020_16_235;
				VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_2020_NonConstantY;
			} else if (colorFMT_info == is709) {
			...
			rtd_pr_vpq_info("[r2y] 4444444,colorFMT_info=%d \n",colorFMT_info);
			if (colorFMT_info == isBT2020) {
				table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC2020_16_235;
				VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_2020_NonConstantY;
			} else if (colorFMT_info == is709) {
		...
		if(Scaler_VOFromJPEG(Scaler_Get_CurVoInfo_plane())){
			rtd_pr_vpq_info("[r2y] 555555,is jpeg \n");
			table_num = RGB2YUV_COEF_RGB_0_255_TO_YCC709_16_235;
			VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_709_Limted;
		}
	} else {
	...
	rtd_pr_vpq_info("CSC_SMR,disp=%d,Src=%d,HD=%d,detect=%d,i3d_en()=%d,IsHDMI()=%d,video_format=%d,colorFMT_info=%d,data_range=%d,Dolby=%d,HDR_flag=%d,vdec_src_type=%d\n",
		channel, nSrcType, nSD_HD, bUse_HDMI_Range_Detect, fwif_color_get_force_run_i3ddma_enable(channel), drvif_IsHDMI(), video_format, colorFMT_info, data_range,DolbyHDR_flag,HDR_flag,vdec_src_type);
...
void fwif_color_gamma_encode_TableSize_2(RTK_TableSize_Gamma *pData)
{
	...
	pData->pu16Gamma_r[255]=(maxvalueR+pData->pu16Gamma_r[254])/2;
	pData->pu16Gamma_g[255]=(maxvalueG+pData->pu16Gamma_g[254])/2;
	pData->pu16Gamma_b[255]=(maxvalueB+pData->pu16Gamma_b[254])/2;;
...
void Scaler_Get_ScreenPixel_info(SCREEN_PIXEL_INFO* sp_info)
{
    ...
    rtd_pr_vpq_emerg("scaler_get_screen_pixel_info !!!!h_sta(%d)===h_end(%d)!\n",sp_info->h_sta,sp_info->h_end);
	rtd_pr_vpq_emerg("scaler_get_screen_pixel_info !!!!v_sta(%d)===v_end(%d)!\n",sp_info->v_sta,sp_info->v_end);
    ...
    rtd_pr_vpq_emerg("The capture window too large, get screen pixel  total_pixel(%d)!!!\n",total_pixel);
	if(width % 2 != 0)
...
void Scaler_color_Set_DemoMode(unsigned int demomode,unsigned char onoff)
{
	switch(demomode){
		case 0:// memc demo
			if(onoff)
				Scaler_MEMC_SetTrueMotionDemo(0);
			else
				Scaler_MEMC_SetTrueMotionDemo(1);
			break;
		case 1: // gamut demo
			if(onoff)
