...
//	h3ddma_i3ddma_enable_RBUS i3ddma_i3ddma_enable_reg;
	...
	if(!get_nn_force_i3ddma_enable())
		...
		if((scalerAI_get_AIPQ_mode_enable_flag()) || (get_nn_force_i3ddma_enable()) ||
			((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && 
...
unsigned char video_delay_alloc_memory(unsigned int video_size)
{
    ...
    if ((get_video_delay_type() == UNKNOW_VIDEO_DELAY_TYPE) && (i3ddmaCtrl.cap_buffer[0].phyaddr) && (s_i3ddma_sw_cap_buffer[0].phyaddr == 0))
    {
        disable_i3ddma_cap();
    ...
    {
        /*FIXME: At present, the flow of video playback is to apply for 8 buffers at once, so the following conditions are applicable. 
        If you want to change it to dynamic request buffer in the future, you need to modify the following conditions.
        */
        if (s_i3ddma_sw_cap_buffer[0].size == video_size){
            rtd_pr_i3ddma_info("[VideoDelay] buffer memory alloc is ready!\n");
            return TRUE;
        } else {
            rtd_pr_i3ddma_info("[VideoDelay] size changed! disable_videodelay\n");
            disable_videodelay();
        }
    }
...
//	vodma_vodma_pvs0_gen_RBUS vodma_vodma_pvs0_gen_Reg;
	...
	{//set fixed iv2pv delay for qms.
		...
		id2pv_delay_1 = (((voHtotal * voVactive) / 10) * QMS_IV2PVDELAY_PERCENT) / 10;//one frame active size delay. iv2pv delay use pixel clock to calculate. VO is 1p design, no need to "/ 2".
...
char I3DDMA_SetupVODMA(I3DDMA_TIMING_T *timing,unsigned char display) {
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) && (Get_DISPLAY_PANEL_CUSTOM_INDEX() == VBY_ONE_PANEL_COMMERCIAL_4K2K_600M_1S_8L_DUPLICATE))
		config_data->hw_auto_mode = 1;
#else
	config_data->hw_auto_mode = ((get_HDMI_HDR_mode() == HDR_HDR10_HDMI) ? 0 : 1);
#endif
...
void h3ddma_nn_run_capture_config(I3DDMA_TIMING_T *timing)
{
	...
	memset((unsigned char *)Scaler_GetShareMemVirAddr(SCALERIOC_I3_NN_SHARE_BUF_STATUS), 0, sizeof(I3DDMA_NN_BUFFER_ATTR_T) * NN_CAP_BUFFER_NUM);
...
int h3ddma_get_NN_read_buffer(unsigned int *a_pulYAddr, unsigned int *a_pulCAddr, unsigned long long *uzd_timestamp, unsigned int *a_pulCropYAddr, unsigned int *a_pulCropCAddr, unsigned long long *crop_timestamp)
{
	...
	if((H3DDMA_CAP1_CTI_DMA_WR_Ctrl_get_cap1_dma_enable(IoReg_Read32(H3DDMA_CAP1_CTI_DMA_WR_Ctrl_reg)) == 0) ||
		(H3DDMA_CAP2_CTI_DMA_WR_Ctrl_get_cap2_dma_enable(IoReg_Read32(H3DDMA_CAP2_CTI_DMA_WR_Ctrl_reg)) == 0))
	{
		return -1;
	}
