...
//		g_ulVDCPhyAddr = (unsigned int)dvr_to_phys((void*)g_ulVDCPhyAddr_cache);
		g_ulVDCPhyAddr = (unsigned int)pli_malloc(_SDRAM_3D_COM_USE, GFP_DCU2_FIRST);
	}
...
//		dvr_free((void*)g_ulVDCPhyAddr_cache);
		pli_free((unsigned long)g_ulVDCPhyAddr);
		g_ulVDCPhyAddr = 0;
		...
		case DETECT_STAGE_INIT:
			...
			if (uc625_flag != 0xff)
			{
				...
				case TV_Colombia:	// PAL_NC_358, PAL_M, NTSC
					if (uc625_flag == 0) {
						//rtd_printk(KERN_INFO, TAG_NAME_AVD, "#####[%s(%d)] DETECT_STAGE_INIT, noise_status_check, 903c=%x, Vstate_check=%x\n", __func__, __LINE__, vdcf_inl(VDTOP_NOISE_STATUS_reg), drvif_module_vdc_Read_Vstate());
						if(drvif_video_get_pal_flag() == 1 ||(drvif_module_vdc_Read_Vstate() >= 1 && drvif_module_vdc_NoiseStatus() > 0xa0)){
							...
							ucVdChange = 1;
							rtd_printk(KERN_INFO, TAG_NAME_AVD, "TV_Colombia.......set PAL_M !!!\n");
						}else{
							g_ucVDMode = VDC_MODE_NTSC;
						}
					} else
...
#endif
						rtd_printk(KERN_INFO, TAG_NAME_AVD, "[%d] PAL=>SECAM ? 94a4=%x\n",j,vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
						msleep(10);
					}
					g_ucTimerEnable = FALSE;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#endif
						rtd_printk(KERN_INFO, TAG_NAME_AVD, "[%d] PAL-M=>NTSC ? 94a4=%x\n",j,vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
						msleep(10);
					}
					g_ucTimerEnable = FALSE;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				...
				if (drvif_module_vdc_ReadMode(VDC_SET) == VDC_MODE_NTSC) {
					...
					for (k = 0 ; k <= ucVdCount; ++k) { /* check chroma locked */
						...
						msleep(10);
						rtd_printk(KERN_INFO, TAG_NAME_AVD, "[%d] clock loop 2, 94a4=%x\n",j ,vdcf_inl(VDTOP_MODE_DETECTION_STATUS_reg));
					}
					g_ucTimerEnable = FALSE;
				}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		if ((0 == drvif_module_vdc_GetLockStatus()) && (_SRC_TV == Scaler_AVD_Get_g_ucSrcType())) {
			...
			if (((ptAVDScalerDispInfo->ucMode_Curr) == 0 )|| avd_good_timing_ready)
			{
				if(Scaler_AVD_Get_ChannelChange() || Scaler_AVD_GetIsChannelChange() || !avd_good_timing_ready){
					...
					if(VDTOP_CSTATE_STATUS_get_cpll_cgain_reset_en(vdcf_inl(VDTOP_CSTATE_STATUS_reg))) {
						rtd_pr_avd_info("#####[%s(%d)] cpll_cgain_reset=0x%08x\n",__FUNCTION__,__LINE__,vdcf_inl(0xB8019218));
						IoReg_ClearBits(VDTOP_CSTATE_STATUS_reg, _BIT9); // make sure cpll_cgain_reset_en to 0
					}
