...
static unsigned char s_direct_driving = 0;
...
unsigned char panel_dlg_get_direct_driving_flag(void)
{
    return s_direct_driving;
}

void panel_dlg_set_direct_driving_flag(unsigned char flag)
{
    s_direct_driving = flag;
}
...
void panel_dlg_set_dlg_mode_by_ap(unsigned char *dlgMode)
{
    if(dlgMode != NULL)
    {
        ...
        else if(strcmp(dlgMode, "PANEL_DLG_4k2k60") == 0)
        {
            s_panelDlgModeByAp = PANEL_DLG_4k2k60;
        }
    }
    else
    {
...
PANEL_DLG_MODE panel_dlg_get_panel_dlg_mode(void)
{
    ...
    if(panel_dlg_get_direct_driving_flag())
    {
        //direct driving enable
        return PANEL_DLG_4k2k60;
    }
...
static void panel_dlg_init_parameter_info(PANEL_DLG_MODE panelDlgMode)
{
    ...
    switch(panelDlgMode)
    {
        ...
        case PANEL_DLG_4k2k60:
            panel_dlg_customer_get_4k2k60_dlg_parameter_info(&dlg_panel_parameter_info);
            break;
...
void panel_dlg_handle(PANEL_DLG_MODE dlgMode)
{
    ...
    handler_dlg_call_back(E_DLG_BL_DISABLE);
    ...
    else if(panel_dlg_get_cur_dlg_mode() == PANEL_DLG_DEFAULT)
    {
        handler_dlg_call_back(E_DLG_MEMC);
    }
    else if(panel_dlg_get_cur_dlg_mode() == PANEL_DLG_4k1k120)
    {
        handler_dlg_call_back(E_DLG_MEMC);
    }
    ...
    handler_dlg_call_back(E_DLG_BL_ENABLE);
}
...
int panel_dlg_eye_diagrams_setting_init(unsigned char *src, int len, unsigned int type)
{
    //if(Get_DISPLAY_HSR_MODE() != PANEL_HSR_OFF)
    {
        return hsr_parse_eye_diagrams_setting(src, len, type);
    }
