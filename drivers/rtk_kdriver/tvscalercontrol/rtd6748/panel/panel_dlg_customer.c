...
static void panel_dlg_customer_eye_diagrams_setting_by_config(PANEL_DLG_MODE panelDlgMode);
...
static void panel_dlg_customer_clock_set_pll(UINT8 M_Code, UINT8 N_Code, UINT8 div, UINT8 CPC)
{
    ...
    pll_disp1_reg.dpll_cp = 0;
    pll_disp1_reg.dpll_rs = 5;
...
#if 0
    pll_disp3_reg.regValue = IoReg_Read32(PLL27X_REG_SYS_PLL_DISP3_reg);
    pll_disp3_reg.dpll_reser = 1;
    pll_disp3_reg.dpll_dummy = 1;
...
static unsigned char panel_dlg_customer_clock_set_dclk(unsigned int ulFreq)
{
    ...
    crt_pll_ssc0_reg.fcode_t_ssc = 0;
    crt_pll_ssc0_reg.oc_step_set = 0x200;
    IoReg_Write32(PLL27X_REG_PLL_SSC0_reg, crt_pll_ssc0_reg.regValue);

    panel_dlg_customer_clock_set_pll(nMCode, nNCode, div, cpc);
...
void panel_dlg_customer_get_4k2k60_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info)
{
    DLG_PRINT("dlg for 4k1k120 panel parameter\n");
    ...
    if(panel_dlg_parameter_info != NULL)
    {
        ...
        if(hsr_parameter_info != NULL)
        {
            if(hsr_parameter_info->panel_parameter[HSR_4k2k_60].refresh == 0)
            {
                DLG_PRINT("Error get panel parameter info failed, please check it\n");
                panel_dlg_parameter_info->refresh = 60;
                panel_dlg_parameter_info->typical = 594000000;
                panel_dlg_parameter_info->htotal = 4400;
                panel_dlg_parameter_info->vtotal = 2250;
                panel_dlg_parameter_info->den_hstart = 280;
            ...
            {
                panel_dlg_parameter_info->refresh = hsr_parameter_info->panel_parameter[HSR_4k2k_60].refresh;
                panel_dlg_parameter_info->typical = hsr_parameter_info->panel_parameter[HSR_4k2k_60].typical;
                panel_dlg_parameter_info->htotal = hsr_parameter_info->panel_parameter[HSR_4k2k_60].htotal;
                panel_dlg_parameter_info->vtotal = hsr_parameter_info->panel_parameter[HSR_4k2k_60].vtotal;
                panel_dlg_parameter_info->den_hstart = hsr_parameter_info->panel_parameter[HSR_4k2k_60].den_hstart;
                panel_dlg_parameter_info->den_hend = hsr_parameter_info->panel_parameter[HSR_4k2k_60].den_hend;
                panel_dlg_parameter_info->den_vstart = hsr_parameter_info->panel_parameter[HSR_4k2k_60].den_vstart;
                panel_dlg_parameter_info->den_vend = hsr_parameter_info->panel_parameter[HSR_4k2k_60].den_vend;
                panel_dlg_parameter_info->act_hstart = hsr_parameter_info->panel_parameter[HSR_4k2k_60].act_hstart;
                panel_dlg_parameter_info->act_hend = hsr_parameter_info->panel_parameter[HSR_4k2k_60].act_hend;
                panel_dlg_parameter_info->act_vstart = hsr_parameter_info->panel_parameter[HSR_4k2k_60].act_vstart;
                panel_dlg_parameter_info->act_vend = hsr_parameter_info->panel_parameter[HSR_4k2k_60].act_vend;
                panel_dlg_parameter_info->min_vtotal = hsr_parameter_info->panel_parameter[HSR_4k2k_60].min_vtotal;
                panel_dlg_parameter_info->max_vtotal = hsr_parameter_info->panel_parameter[HSR_4k2k_60].max_vtotal;
                panel_dlg_parameter_info->hsync_last_line = hsr_parameter_info->panel_parameter[HSR_4k2k_60].hsync_last_line;
            }
        }
        else
        {
...
void panel_dlg_customer_init_down_sample(PANEL_DLG_MODE panelDlgMode)
{
    ...
    {
        //default run 4k1k120 dtg flow for PQ OD
        enable_down_sample = 0;//Get_DISPLAY_HSR_DOWN_SAMPLE();
    }
    else
    {
...
static void panel_dlg_customer_set_dpll_clk(DPLL_TYPE dpllType)
{
    ...
    panel_dlg_customer_clock_set_dclk(Get_DISPLAY_CLOCK_TYPICAL());
}
...
static void panel_dlg_uzudtg_setting(void)
{
    ...
    if(!panel_dlg_customer_get_down_sample() && (Get_DISP_DEN_END_VPOS() - Get_DISP_DEN_STA_VPOS() == 1080))
    {
...
static void panel_dlg_apply_double_buffer(void)
{
    ...
    while((double_buffer_ctrl_reg.dreg_dbuf_set || double_buffer_ctrl_reg.dmainreg_dbuf_set || double_buffer_ctrl_reg.drm_multireg_dbuf_set
        || double_buffer_ctrl2_reg.uzudtgreg_dbuf_set || double_buffer_ctrl2_reg.memc_dtgreg_dbuf_set || ppoverlay_double_buffer_ctrl3_reg.dmainreg_other_dbuf_set) && --count)
    {
        double_buffer_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL_reg);
        double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
        ppoverlay_double_buffer_ctrl3_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL3_reg);
...
void panel_dlg_customer_timing_setting(PANEL_DLG_MODE panelDlgMode)
{
    ...
    if((Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI))
    {
        ...
        msleep(60); //disable tcon need wait for apply ok
    }

    panel_dlg_sfg_setting();

    panel_dlg_customer_eye_diagrams_setting(panelDlgMode);
...
static void panel_dlg_customer_eye_diagrams_setting_by_config(PANEL_DLG_MODE panelDlgMode)
{
    const HSR_EYE_DIAGRAMS_SETTING *p_eye_diagrams_setting = NULL;
    int i = 0;
    HSR_MODE_TYPE hsrIndex = HSR_NOMAL;

    hsrIndex = panel_dlg_customer_get_hsr_index_by_dlg_mode(panelDlgMode);

    p_eye_diagrams_setting = hsr_get_diagrams_setting_by_type(hsrIndex);

    if(p_eye_diagrams_setting == NULL)
    {
        DLG_PRINT("Error tcon config is NULL\n");
        return;
    }

    if(p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].len > 0)
    {
        for(i = 0; i < p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].len / sizeof(BIN_SETTING_DATA); i++)
        {
            switch(p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].type)
            {
                case 0:
                    DLG_PRINT("IoReg_Write32(0x%08x,0x%08x)\n", p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].addr,p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value1);
                    IoReg_Write32(p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].addr, p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value1);
                    break;

                case 1:
                    DLG_PRINT("IoReg_Mask32(0x%08x,0x%08x,0x%08x)\n",p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].addr,p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value1,p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value2);
                    IoReg_Mask32(p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].addr, p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value1, p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].value2);
                    break;
                ...
                case 3:
                    msleep(p_eye_diagrams_setting->eye_diagrams_data[hsrIndex].eye_digrams[i].addr);
                    break;
...
static void panel_dlg_customer_tconless_setting_by_config(PANEL_DLG_MODE panelDlgMode)
{
    ...
    {
        for(i = 0; i < p_tcon_setting->tcon_data[hsrIndex].len / sizeof(BIN_SETTING_DATA); i++)
        {
            if(p_tcon_setting->tcon_data[hsrIndex].tcon[i].addr == TCON_TCON_IP_EN_reg)
            {
                msleep(60);
            }
            ...
            {
                case 0:
                    //DLG_PRINT("IoReg_Write32(0x%08x,0x%08x)\n", p_tcon_setting->tcon_data[hsrIndex].tcon[i].addr,p_tcon_setting->tcon_data[hsrIndex].tcon[i].value1);
                    IoReg_Write32(p_tcon_setting->tcon_data[hsrIndex].tcon[i].addr, p_tcon_setting->tcon_data[hsrIndex].tcon[i].value1);
                    break;

                case 1:
                    //DLG_PRINT("IoReg_Mask32(0x%08x,0x%08x,0x%08x)\n",p_tcon_setting->tcon_data[hsrIndex].tcon[i].addr,p_tcon_setting->tcon_data[hsrIndex].tcon[i].value1,p_tcon_setting->tcon_data[hsrIndex].tcon[i].value2);
...
void panel_dlg_customer_eye_diagrams_setting(PANEL_DLG_MODE panelDlgMode)
{
    panel_dlg_customer_eye_diagrams_setting_by_config(panelDlgMode);
}
