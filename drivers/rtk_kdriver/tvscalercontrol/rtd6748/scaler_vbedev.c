...
//	unsigned char memc_forcebg_enable = 0;
	...
	if(b_gameMode_onOff==1){
		...
		(Scaler_DispGetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) == 1)) || is_vrr_or_freesync_mode()){
			set_memc_bypass = 1;
		}
	}
...
//#ifndef CONFIG_MEMC_BYPASS
	...
	if(Get_DISPLAY_REFRESH_RATE() == 144){
        ...
        if((Get_DISP_HORIZONTAL_TOTAL() == 4480) && (Get_DISP_DEN_STA_HPOS() == 280))
        {
           memc_goldenvs_delay_pixel = DTGM_2_GOLDEN_VSYNC_DELAY_PIXEL_39;
        }
	}
		...
		if(0){
			...
			if((mode == 1)||(mode == 2)||(mode == 3)){
				...
				if(!PPOVERLAY_Display_Timing_CTRL1_get_disp_fsync_en(IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg))){
					ppoverlay_dv_total_reg.dv_total += ABS(ppoverlay_memcdtg_control3_reg.dtgm2goldenpostvs_line, u32_dtgm2goldenpostvs_line);
...
unsigned int VSYNCGEN_get_vodma_pll(void)
{
    unsigned int nMCode, nNCode, nOCode, vodmapll = 0;
    pll_reg_sys_pll_vodma1_RBUS pll_reg_sys_pll_vodma1_reg;
    pll_reg_sys_pll_vodma1_reg.regValue = rtd_inl(PLL_REG_SYS_PLL_VODMA1_reg);

    nMCode = pll_reg_sys_pll_vodma1_reg.pllvodma_m;
    nNCode = pll_reg_sys_pll_vodma1_reg.pllvodma_prediv;
    nOCode = pll_reg_sys_pll_vodma1_reg.pllvodma_o;
    vodmapll = XTAL_TICK *(nMCode + VODMA_PLL_MCODE_BASIC)/(nNCode+1)/(nOCode+1);
    return vodmapll;
}
...
void scaler_disp_dtg_set_s1ip_dleay(unsigned char bEnable_DB)
{
    ...
    unsigned int ed_time = 0;

    st_time = IoReg_Read32(TIMER_VCPU_CLK90K_LO_reg);
    ...
    if((ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_v == time_borrow)
        && (ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay == s1ip_delay))
    {
        rtd_pr_vbe_notice("[%s]no change, time_borrow=%d s1ip_delay=%d", __FUNCTION__, time_borrow, s1ip_delay);
        ed_time = IoReg_Read32(TIMER_VCPU_CLK90K_LO_reg);
        rtd_pr_vbe_notice("[%s]wait time cost = %d ms", __FUNCTION__, (ed_time-st_time)/90);
        return;
    }
    else
    {
        rtd_pr_vbe_notice("[%s]need change, time_borrow= %d->%d s1ip_delay= %d->%d", __FUNCTION__, ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_v, time_borrow, ppoverlay_uzudtg_control3_reg.s1ip_max_line_delay, s1ip_delay);
    }
    ...
    if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE) || judge_set_s1ip_run_slow_tuning())
	{
		s1ip_delay = scaler_disp_dtg_get_s1ip_delay_value();
...
extern void drvif_color_Set_sr_mdom_sub_en(unsigned char Enable);
extern void disable_uzu_dtg_remove_multiple(unsigned char m_lowdelay_en);
...
void drvif_scaler_for_wb(unsigned char ptgEnable)
{
    static unsigned int uzu_4k120_pre;
    static unsigned int vo_gatting_pre;
    static unsigned int merge_mainSub_pre;
    ...
    rtd_pr_vbe_info("[%s] ptgEnable=%d\n",__FUNCTION__,ptgEnable);
    if(ptgEnable){
        if(Get_tv006_wb_pattern())
            return;
        Set_tv006_wb_pattern(1);
        uzudtg_control1_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control1_reg);
        uzu_4k120_pre = uzudtg_control1_reg.uzu_4k_120_mode;
        merge_mainSub_pre = TWO_STEP_UZU_DM_TWO_STEP_SR_Ctrl_get_sr_mdom_sub_en(IoReg_Read32(TWO_STEP_UZU_DM_TWO_STEP_SR_Ctrl_reg));
        ...
        //disable 2step uzu merge main sub line
        if(merge_mainSub_pre) {
        ...
        if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
            IoReg_SetBits(SOFTWARE1_SOFTWARE1_00_reg, _BIT9);
        }
        Scaler_MEMC_MEMC_CLK_OnOff(0, 0, 0);
        Scaler_MEMC_Bypass_On(1);
        IoReg_SetBits(PPOVERLAY_uzudtg_control3_reg, PPOVERLAY_uzudtg_control3_s1ip_line_delay_sw_mode_mask);
        ...
        //set 120fps
        uzudtg_control1_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control1_reg);
        if((uzu_4k120_pre == 0) && (get_panel_pixel_mode() == PANEL_2_PIXEL)){
            uzudtg_control1_reg.uzu_4k_120_mode = 1;
        }
        IoReg_Write32(PPOVERLAY_uzudtg_control1_reg, uzudtg_control1_reg.regValue);
        ...
        if(timeout_count == 0)
            rtd_pr_vbe_emerg("[%s] wait dtg db timeout!!! %d\n",__FUNCTION__,timeout_count);
        Scaler_MEMC_output_force_bg_enable(0,__func__,__LINE__);
    }
    else{/*exit WB*/
        Set_tv006_wb_pattern(0);
        Scaler_MEMC_output_force_bg_enable(1,__func__,__LINE__);
        uzudtg_control1_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_control1_reg);
        if((uzu_4k120_pre == 0) && uzudtg_control1_reg.uzu_4k_120_mode){
            ...
            {
                ppoverlay_double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
                if((ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_en && ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_set) && wait_timeout)
                {
                    wait_timeout--;
                    msleep(10);//100 ms
                }
                else
                {
                    if(wait_timeout == 0)
                    {
                        unsigned char db_enable;
                        rtd_pr_vbe_err("\r### func:%s disable 4k120 timeout####\r\n",__FUNCTION__);
                        //disbale double buffer force apply
                        ppoverlay_double_buffer_ctrl2_reg.regValue = IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
                        db_enable = ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_en;
                        ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_en = 0;
                        IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL2_reg, ppoverlay_double_buffer_ctrl2_reg.regValue);

                        ppoverlay_double_buffer_ctrl2_reg.regValue = rtd_inl(PPOVERLAY_Double_Buffer_CTRL2_reg);
                        ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_en = db_enable;
                        IoReg_Write32(PPOVERLAY_Double_Buffer_CTRL2_reg, ppoverlay_double_buffer_ctrl2_reg.regValue);
        ...
        //restore 2step uzu merge main sub line
        if(merge_mainSub_pre) {
            drvif_color_Set_sr_mdom_sub_en(1);
        }

        disable_uzu_dtg_remove_multiple(0);//enable uzudtg remove/multipe function
        if(!is_vrr_or_freesync_mode() && !uzu_4k120_pre){
            if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
                rtd_clearbits(SOFTWARE1_SOFTWARE1_00_reg, _BIT9);
            }
            Scaler_MEMC_Bypass_On(0);
            Scaler_MEMC_MEMC_CLK_OnOff(1, 1, 1);
            rtd_clearbits(PPOVERLAY_uzudtg_control3_reg, PPOVERLAY_uzudtg_control3_s1ip_line_delay_sw_mode_mask);
        }
...
void drvif_scaler_set_Pattern_mute(unsigned char enable,unsigned char mute_flag, unsigned char display_flag,StructSrcRect* sdp_info)
{
...
*/
	...
	if(Get_DISPLAY_REFRESH_RATE() >= 120){
		if(get_bypass_memc_cma_alloc_flag_scaler() == TRUE){	//bypass memc case
			//disable MEMC clock
			Scaler_MEMC_MEMC_CLK_OnOff(0, 0, 1);
			Scaler_MEMC_Bypass_On(1);
		}
		else if(((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) >= 994) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) <= 1005)) ||
	...
	else  // 60hz panel
	{
		...
		else if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) &&    //DTV/USB playback 4K 50/60 default ME off, MC on
	...
	//		Scaler_MEMC_DTV_ME_CLK_OnOff(0, 1);
			if(Get_tv006_wb_pattern()){
              ...
              if(get_vt_EnableFRCMode() != FALSE)
                {
#ifndef UT_flag
                    if(Scaler_Get_Film_Control_MEMC())
                    {
                        SLR_VOINFO *pVOInfo = Scaler_VOInfoPointer(Scaler_Get_CurVoInfo_plane());
                        if(pVOInfo && pVOInfo->film_en)
                        {
                            rtd_pr_vbe_notice("[%s]oriFilmFramerate=%d, v_freq=%d, film_mode=%d", __FUNCTION__, pVOInfo->oriFilmFramerate, pVOInfo->v_freq, pVOInfo->film_mode);
                            if((pVOInfo->oriFilmFramerate != 0 && pVOInfo->oriFilmFramerate != pVOInfo->v_freq)
                                || ((pVOInfo->film_mode) ==  _VO_CAD_11112))
                            {
                                Scaler_MEMC_MEMC_CLK_OnOff(1, 1, 1);
                            }
                            else
...
//#ifdef CONFIG_MEMC_BYPASS
		if((get_MEMC_bypass_status_refer_platform_model()) || (get_bypass_memc_cma_alloc_flag_scaler() == TRUE)){
...
unsigned int scaler_disp_smooth_get_memc_protect_vtotal_By_100(void)
{
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
    {
        rtd_pr_vbe_notice("[%s] vdec source: voRealFramerate=%d", __FUNCTION__, pVOInfo->voRealFramerate);
        vFreq = (pVOInfo->voRealFramerate / 10);
    }
...
//#endif
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		rtd_pr_vbe_notice("[%s][%d]Covertor_TO_HDMI=FRC\n",__FUNCTION__,__LINE__);
		modestate_set_fll_running_flag(_DISABLE);
		Scaler_Set_Display_Timing_Mode(DISPLAY_MODE_FRC);
		drvif_scaler_set_display_mode(DISPLAY_MODE_FRC);
	}
...
void HAL_VBE_DISP_SetDisplayOutput(BOOLEAN bOnOff)
{
	...
	if(bOnOff)
	{
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) ||
			(Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CHPI)){
			msleep(25);
			rtd_pr_vbe_emerg("HAL_VBE_DISP_SetDisplayOutput. TCON++ (%x)\n", IoReg_Read32(TCON_TCON_IP_EN_reg));
                        IoReg_SetBits(TCON_TCON_IP_EN_reg, _BIT1);
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)
			|| (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CHPI)){
	...
	else
	{
		...
		if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
            ...
            if((strcmp(pPanelCOnfigParameter->sPanelName,"CV500U5-L04_HSR120") != 0) &&
		(strcmp(pPanelCOnfigParameter->sPanelName,"CV700U3-L01_HSR120") != 0))
			    msleep(25);
			rtd_pr_vbe_notice("[HAL_VBE_DISP_SetDisplayOutput][%d][,%x,%x,%x,%x,%x,][,%x,%x,%x,%x,%x,][,%x,%x,%x,%x,%x,%x]\n\r", __LINE__,
				rtd_inl(KME_DM_TOP0_KME_00_AGENT_reg), rtd_inl(KME_DM_TOP0_KME_01_AGENT_reg), rtd_inl(KME_DM_TOP0_KME_02_AGENT_reg), rtd_inl(KME_DM_TOP0_KME_03_AGENT_reg), rtd_inl(KME_DM_TOP0_KME_04_AGENT_reg),
				rtd_inl(KME_DM_TOP0_KME_05_AGENT_reg), rtd_inl(KME_DM_TOP0_KME06AGENT_reg), rtd_inl(KME_DM_TOP0_KME_07_AGENT_reg), rtd_inl(KME_DM_TOP1_KME_DM_TOP1_14_reg), rtd_inl(KME_DM_TOP1_KME_DM_TOP1_1C_reg),
...
void vbe_disp_panel_suspend(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CHPI))
	{
...
void vbe_disp_panel_resume(void)
{
	...
	else if((Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CHPI))
	{
        ...
        // tconless flow start
    if((Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_CEDS))
    {
        vbe_disp_lane_power_off();
    }
...
void vbe_disp_orbit_justscan_HW_Ctrl(int shift_x, int shift_y, unsigned char justscan_position, unsigned char bEnable_DB)
{
	...
	if(((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)||force_enable_two_step_uzu() || get_hdmi_vrr_4k60_mode() || (ppoverlay_uzudtg_control1_reg.uzu_4k_120_mode == 1)) && (get_panel_pixel_mode() > PANEL_1_PIXEL)/* && justscan_position ==1*/)
	{
		if(shift_x <= 0)
		{
			if((shift_x%2) ==0)
...
void vbe_disp_set_freesync_mode_flag (unsigned char enable)
{
    ...
    if(!panel_dlg_get_panel_vrr_freesync_timing_is_valid())
    {
        rtd_pr_vbe_err("[%s(%d)] freesync case, but panel unsupported !!!\n", __FUNCTION__,__LINE__);
...
#if defined(CONFIG_H5CX_SUPPORT)
    vbe_rlink_set_freesync_info(Freesync_mode_enable);
#endif
}
...
void vbe_disp_set_VRR_timingMode_flag(unsigned char flag)
{
    ...
    if(!panel_dlg_get_panel_vrr_freesync_timing_is_valid())
    {
        rtd_pr_vbe_err("[%s(%d)] vrr case, but panel unsupported !!!\n", __FUNCTION__,__LINE__);
        scaler_VRR_timingMode_flag = 0;
    }

    panel_dlg_operator_panel_vrr_mode(scaler_VRR_timingMode_flag);
#if defined(CONFIG_H5CX_SUPPORT)
    vbe_rlink_set_vrr_info(scaler_VRR_timingMode_flag);
#endif
}
...
{
    PACKAGE_FEATURE packageInfo = rtk_get_package_feature();
    ...
    switch(packageInfo)
    {
        case PACKAGE_FEATURE_4K_60:
            ...
            break;
#if 0
        case PACKAGE_FEATURE_4K_120:
            {
                if(dclk > 1200000000ULL)
                {
                    //osd and video all mute
                    vbe_disp_sfg_inner_ptg(1, VBE_DISP_INNER_PTG_TYPE_COLOR_BLACK);
                }
        ...
        case PACKAGE_FEATURE_2K_60:
        case PACKAGE_FEATURE_2K_60_FOR_4K:
            {
                if(dclk > 200000000ULL)
                {
                    //osd and video all mute
                    vbe_disp_sfg_inner_ptg(1, VBE_DISP_INNER_PTG_TYPE_COLOR_BLACK);   
                }
	...
	if(bMute)
	{
		rtd_pr_vbe_notice("[%s %d]packageInfo:%d, dclk:%lld, bMute:%d\n", __FUNCTION__, __LINE__, packageInfo, dclk, bMute);
	}
#if 0 // debug msg
	else
	{
...
#ifndef UT_flag
    // command : echo film_ctrl_memc=0 > /dev/vbedev
	if(strcmp(cmd_buf, "film_ctrl_memc=0\n") == 0)
	{
		rtd_pr_vbe_emerg("film_ctrl_memc=0\n");
		Scaler_Set_Film_Control_MEMC(FALSE);
	}
	else if(strcmp(cmd_buf, "film_ctrl_memc=1\n") == 0)
	{
		rtd_pr_vbe_emerg("film_ctrl_memc=1\n");
		Scaler_Set_Film_Control_MEMC(TRUE);
	}
#endif
...
long vbe_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd)
	{
        ...
        case VBE_IOC_BYPASS_MEMC_CMA_ALLOC:
        {
            unsigned char enable;

            if (copy_from_user((void *)&enable, (const void __user *)arg, sizeof(unsigned char))) {
                retval = -EFAULT;
                rtd_pr_vbe_debug("scaler vbe ioctl code=VBE_IOC_BYPASS_MEMC_CMA_ALLOC failed!!!!!!!!!!!!!!!\n");
            } else {
                set_bypass_memc_cma_alloc_flag_ap(enable);
            }
            break;
        }
		default:
