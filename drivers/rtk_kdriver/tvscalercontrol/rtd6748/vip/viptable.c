...
void* GetColorData(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	return (void*)&m_colorDataTable[0];	// use same structure
}
...
void* GetColorFacMode(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	return (void*)&m_colorFacModeTable[0];	// use same structure
}
...
void* GetColorTempLevel(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	return (void *)&m_picModeTable[0].picMode[0].ColorTemp;	// use same structure
}
...
void* GetColorTempData(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	return &m_colorTempData[0].colorTempData[0];	// use same structure
}
...
//	SourceOption sourceOption = xGetCurSource();
	...
	return (void *)&m_picModeTable[0].picMode[0];	// use same structure
}
...
void* GetDnrMode(UINT32 source, UINT32 timing, UINT32 color)
{
	...
	if (mode >= PICTURE_MODE_MAX)
		mode = PICTURE_MODE_USER;
	return (void *)&m_picModeTable[0].picMode[0].DNR;	// use same structure
}
...
PICTURE_MODE GetPictureMode(SourceOption source)
{
	...
	return m_pictureMode[0];	// use same structure
}
...
UINT8 GetIcmBySrcPicmode(UINT8 picMode)
{
	...
	return *(((UINT8 *)m_pVipTable->ICM_by_timing_picmode) + 0 * VIP_PICTURE_MODE_MAX + 0);		// use same structure
}
