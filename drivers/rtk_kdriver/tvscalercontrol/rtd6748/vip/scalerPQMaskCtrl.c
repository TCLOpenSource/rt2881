...
extern unsigned long get_memory_size_total(void);
...
static int PQMaskDumpData2File(char* tmpname, unsigned int * vir_y, unsigned int * vir_c, unsigned int y_size, unsigned int c_size)
{
	...
	if( rtk_get_usb_path(usbPath, 256) != 0) {
		...
		if( sprintf(filename, "%s/%s.raw", "/tmp/usb/sda/sda1", tmpname) < 0 ) {
			rtd_pr_vpq_err("[%s] construct file name error\n", __FUNCTION__);
			return 0;
		}
	} else {
...
static inline void scalerPQMask_Memcpy(
	void *DstVirtAddr_base, void *DstVirtAddr, void *DstPhysAddr, bool DstIsMem,
	void *SrcVirtAddr_base, void *SrcVirtAddr, void *SrcPhysAddr, bool SrcIsMem,
	unsigned int CopySize) {

	// error checking
	if( DstVirtAddr_base == 0 || SrcVirtAddr_base ==0 ) {
		rtd_pr_vpq_emerg("[%s] error address, Dst=%lx, Src=%lx\n", __FUNCTION__, (unsigned long)DstVirtAddr_base, (unsigned long)SrcVirtAddr_base);
		return;
	}
			...
			if( ( PQMaskGlbMgr.Status.SE_OPMODE_DepthResize == 1 ) && ( mColorMgr.Param.FusionRatio_Dep != 0 ) ) {
				...
				if( true == SeProcStatus ) {
					...
					if( true == SeSetupStatus ) {
						...
						rtd_pr_vpq_emerg("SE cmd setup erorr, depth info resize\n");
						SeProcStatus = false;
					}
				}
					...
					if( true == SeProcStatus ) {
						...
						if( KGAL_PQMaskLockSE() == 0 ) {
							...
							if( true == SeSetupStatus ) {
								...
								rtd_pr_vpq_emerg("SE task grp %d cmd setup error, depth palette\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							if( true == SeSetupStatus ) {
								...
								rtd_pr_vpq_emerg("SE task grp %d cmd setup error, depth label2weight\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							rtd_pr_vpq_emerg( "SE task grp %d lock error, depth label2weight\n", ProcGrp);
						}
					...
					if( true == SeProcStatus ) {
						...
						if( KGAL_PQMaskLockSE() == 0 ) {
							...
							if( true == SeSetupStatus ) {
								...
								rtd_pr_vpq_emerg("SE task grp %d cmd setup error, semantic palette\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							if( true == SeSetupStatus ) {
								...
								rtd_pr_vpq_emerg("SE task grp %d cmd setup error, semantic label2weight\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							rtd_pr_vpq_emerg("SE task grp %d lock error, semantic label2weight\n", ProcGrp);
						}
					}
				...
				if( ( AI_SEMANTIC_OUTPUT_WIDTH != PQMASK_PROC_WIDTH || AI_SEMANTIC_OUTPUT_HEIGHT != PQMASK_PROC_HEIGHT ) &&
					...
					if( true == SeProcStatus ) {
						...
						if( true == SeSetupStatus ) {
							...
							rtd_pr_vpq_emerg("SE task grp %d cmd setup error, sematic stretch\n", ProcGrp);
							SeProcStatus = false;
						}
					}
				...
				if( mColorMgr.Param.FusionRatio_Dep == 255 && mColorMgr.Param.FusionRatio_Sem == 0 ) {
					...
					if( true == SeProcStatus ) {
						...
						if( true == SeSetupStatus ) {
							...
							rtd_pr_vpq_emerg("SE task grp %d cmd setup error, semantic depth fusion\n", ProcGrp);
							SeProcStatus = false;
						}
					}
				...
				if( pColorMgr->Status.SCRstFrm > 0 ) {
					...
					if( TNRFusion_Pre == 255 && TNRFusion_Cur == 0 ) {
						for( i=0; i<3; i++ ) {
							...
							if( TNRModuleSel < PQMASK_MODULE_NUM ) {
								...
								scalerPQMask_Memcpy(
									(void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.PhysAddr), 1,
									(void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufReadPtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufReadPtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufReadPtr].AddrInfo.PhysAddr), 1,
						...
						if( true == SeSetupStatus ) {
							...
							rtd_pr_vpq_emerg("SE task grp %d cmd setup error, scene change only do RTNR\n", ProcGrp);
							SeProcStatus = false;
						}
					}
						...
						for( i=0; i<mColorMgr.Param.SpatialFltCycle; i++ ) {
							...
							if( true == SeProcStatus ) {
								...
								if( true == SeSetupStatus ) {							
									...
									rtd_pr_vpq_emerg("SE task grp %d cmd setup error, se spatial filter cycle %d\n", ProcGrp, i);
									SeProcStatus = false;
								}
							}
					...
					for( i=0; i<3; i++ ) {
						...
						if( TNRModuleSel < PQMASK_MODULE_NUM ) {
							...
							scalerPQMask_Memcpy(
								(void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.PhysAddr), 1,
								(void *)(PQMaskBufMgr.SeBuffer[OutBuf_Tnr].AddrInfo.VirtAddr), (void *)(PQMaskBufMgr.SeBuffer[OutBuf_Tnr].AddrInfo.VirtAddr + SingleWeightPlaneSize*i), (void *)(PQMaskBufMgr.SeBuffer[OutBuf_Tnr].AddrInfo.PhysAddr + SingleWeightPlaneSize*i), 1,
								PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.Size
							);
						}
					}
					...
					if( mColorMgr.Param.SpatialFltCycle > 0 ) {
						...
						if( true == SeProcStatus ) {
							...
							if( true == SeSetupStatus ) {
								...
								rtd_pr_vpq_emerg("SE task grp %d cmd setup error, spatial (%s) temporal combine\n", ProcGrp, PQMaskSEBufName[OutBuf_SpatialFlt]);
								SeProcStatus = false;
							}
						...
						if( true == SeSetupStatus ) {
							...
							rtd_pr_vpq_emerg("SE task grp %d cmd setup error, simple RTNR with %d pre %d cur\n", ProcGrp, TNRFusion_Pre, TNRFusion_Cur);
							SeProcStatus = false;
						}
...
void scalerPQMask_postprocessing_entry(void) {
	...
	if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_READY )	{
		...
		if( PQMaskGlbMgr.Ctrl.bSemNewInfo != 0 ) {
			...
			scalerPQMask_Memcpy(
				(void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_SEMANTIC].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_SEMANTIC].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_SEMANTIC].AddrInfo.PhysAddr, 1,
				(void *)pNN->NN_info_Addr[semantic_idx].pVir_addr_align, (void *)pNN->NN_info_Addr[semantic_idx].pVir_addr_align, 0, 1,
			...
			if( pPQMaskColorMgr->ImgBySemanticHist[0][0].Info.enable != 0 ) {
				...
				for(i=0; i<PQMASK_LABEL_NUM; i++){
					for(j=0; j<PQMASK_IMGCH_NUM; j++){
						scalerPQMask_Memcpy(
							(void *)(pPQMaskColorMgr->ImgBySemanticHist[i][j].Data.Histogram), (void *)(pPQMaskColorMgr->ImgBySemanticHist[i][j].Data.Histogram), 0, 0,
							(void *)(&pPQMaskFlag->Histogram[i*PQMASK_IMGCH_NUM*AI_SEM_HIST_BIN+j*AI_SEM_HIST_BIN]), (void *)(&pPQMaskFlag->Histogram[i*PQMASK_IMGCH_NUM*AI_SEM_HIST_BIN+j*AI_SEM_HIST_BIN]), 0, 1,
		...
		if( PQMaskGlbMgr.Ctrl.bDepNewInfo != 0 ) {
			...
			scalerPQMask_Memcpy(
				(void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEPTH].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEPTH].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEPTH].AddrInfo.PhysAddr, 1,
				(void *) pNN->NN_info_Addr[depth_idx].pVir_addr_align, (void *) pNN->NN_info_Addr[depth_idx].pVir_addr_align, 0, 1,
...
void scalerPQMask_SetModuleAddr(unsigned char ModuleSel, unsigned long TargetAddr) {
	...
	switch(ModuleSel) {
		...
		case PQMASK_MODULE_LC:
			...
			break;
		default:
			rtd_pr_vpq_err("[%s] index %d overflow\n", __FUNCTION__, ModuleSel);
			break;
	}
...
void scalerPQMask_SetBlkModuleIoSize(unsigned char BlkModuleSel) {
	...
	if( PQMaskBlkModuleCfg[BlkModuleSel].IoSize.Output.Width == 0 || PQMaskBlkModuleCfg[BlkModuleSel].IoSize.Output.Height == 0 ) {
		rtd_pr_vpq_err("[%s] output width = %d, height = %d error\n",
