...
{	
	...
	if (phy_Addr == 0 || vir_Addr == NULL) {
		rtd_pr_vpq_info("[VPQ_LED] drvif_color_Set_Local_Dimming_Object_Gain_DMA_Enable NUL\n");
		return -1;
	}
...
void drvif_color_set_LDSPI_SRAM_HW_WRITE_done_IntEn(unsigned char enable, unsigned short hw_write_num)
{
	...
	IoReg_Write32(LDSPI_RX_data_CTRL_reg, ldspi_rx_data_ctrl.regValue);
	rtd_pr_vpq_emerg("[VPQ_LED] sram_hw_write_done_int_en = %d hw_write_num=%d\n",enable,hw_write_num);

}

unsigned char drvif_color_get_LDSPI_DataSRAM(unsigned char *ld_blk_hnum, unsigned char *ld_blk_vnum,  unsigned short *LDSPI_BLValue)
{
    unsigned char ret = 1;
    ...
    //rtd_printk(KERN_EMERG, TAG_NAME, "drvif_color_get_LDSPI_DataSRAM");

    if(LDSPI_BLValue == NULL) {
        rtd_pr_vpq_emerg("[VPQ_LED] LDSPI_BLValue is NULL\n");
        ret = 0;
        return ret;
    }
    ...
    if (ld_blk_type == 1 || ld_blk_type==2) {/*edge type*/
        if (*ld_blk_vnum == 0) {/*1 sided*/
            for(i=0;i<2*(*ld_blk_hnum+1);i+=2) {
                ...
                *(LDSPI_BLValue+i/2) = (unsigned short) ldspi_data_rwport.data_rw_data_port;
            ...
            for(i=0;i<2*(*ld_blk_hnum+1);i++) {
                ldspi_data_rwport.regValue = IoReg_Read32(LDSPI_Data_Rwport_reg);
                *(LDSPI_BLValue+i) = (unsigned short) ldspi_data_rwport.data_rw_data_port;
            }
        }
    } else if(ld_blk_type == 0) { /*direct type*/
        for(i=0;i<(*ld_blk_vnum+1)*(*ld_blk_hnum+1);i++) {
        ...
        rtd_pr_vpq_emerg( "Get LDSPI DataSRAM is not correct, id type = %d\n", ld_blk_type);
    }*/
    return ret;
}
