...
char drvif_color_set_DTG_valc_enable(unsigned char bEnable)
{
	...
	pixel_mode = get_panel_pixel_mode();
	...
	if ((pixel_mode == PANEL_1_PIXEL) && (bEnable == 1))
	{
	...
	rtd_pr_vpq_info("drvif_color_set_DTG_valc_enable, ret=%d, t0=%d, t1=%d, t2=%d, bEnable=%d, pixel_mode=%d,\n", ret, t0, t1, t2, bEnable, pixel_mode);

	return ret;
}

void drvif_color_set_valc_loc(unsigned char loc)
{
	valc_pcid_valc_ctrl_RBUS valc_pcid_valc_ctrl_reg;

	valc_pcid_valc_ctrl_reg.regValue = IoReg_Read32(VALC_PCID_VALC_CTRL_reg);
	valc_pcid_valc_ctrl_reg.valc_location = loc;
	IoReg_Write32(VALC_PCID_VALC_CTRL_reg, valc_pcid_valc_ctrl_reg.regValue);
