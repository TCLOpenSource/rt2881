...
static PQMASK_COLOR_MGR_T PQMaskColorMgr = {
	...
	// Parameters
	{
		...
		32, 		// SceneChangeRstFrm : frame to bypass curve and frame to target cruve
		0, 255 , 	// ScFusionRatio_toRst_Pre/ScFusionRatio_toRst_Cur : blending weight on Pre/Cur toward reset when scene change happend
		0, 255  , 	// ScFusionRatio_toTarget_Pre/ScFusionRatio_toTarget_Cur : blending weight on Pre/Cur toward target when scene change happend
...
#if 1	// table from bin
		ScalerPQMaskColor_PQMASK_Set_Semantic_Depth_Weight_from_AIPQ_BIN_by_NNSR_mode();
...
//#ifndef BUILD_QUICK_SHOW
	...
	if (mode == 0) {
		for (i=0;i<AI_RESOLUTION_TYPE_NUM;i++)
			for (j=0;j<PQMASK_MODULE_NUM;j++)
				AI_DepthWeightTbl_SQM_TBL[i][j] = 0;
	} 
	else 
	{
		idx = (mode == 1)?(0):(1);
		for (j=0;j<PQMASK_MODULE_NUM;j++)
		{			
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2K_BAD][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2K_BAD][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2K_NORMAL][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2K_NORMAL][j];
			//AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2K_HIGH][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2K_HIGH][j];	// no "SLR_AI_SQD_RESOLUTION_2K_HIGH"
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_4K_NORMAL][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_4K_NORMAL][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_4K_GOOD][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_4K_GOOD][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2KTIMING_2K_WORSE][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2KTIMING_2K_WORSE][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2KTIMING_2K_BAD][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2KTIMING_2K_BAD][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2KTIMING_2K_NORMAL][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2KTIMING_2K_NORMAL][j];
			AI_DepthWeightTbl_SQM_TBL[AI_RESOLUTION_2KTIMING_2K_GOOD][j] = m_VIP_AIPQTable.SQD_TBL[idx][SLR_AI_SQD_RESOLUTION_2KTIMING_2K_GOOD][j];			
		}
	}
	scalerPQMaskColor_SetReloadPalette(PQMASK_COLOR_PALETTE_DEPTH);
	scalerPQMaskColor_SetReloadPalette(PQMASK_COLOR_PALETTE_SEMANTIC);						
...
char ScalerPQMaskColor_PQMASK_Smooth_AIPQ_BIN(RTK_AI_PQ_mode *ap_mode, SLR_VIP_TABLE_AIPQ *pSLR_VIP_TABLE_AIPQ)
{
	//int depth, semantic, defocus;
	if (pSLR_VIP_TABLE_AIPQ == NULL) {
...
#if 0 //def VIP_SUPPORT_AIPQ_NNSR_BIN_V2
	...
	// pq mask								
	memcpy(&PQMaskColorMgr.Param, &m_VIP_AIPQTable.SLR_PQMASK_COLOR_PARAM_T, sizeof(PQMASK_COLOR_PARAM_T));
	if (m_VIP_AIPQTable.AI_Ctrl_byte_b[SLR_AI_CTRL_Byte_Sat_Gain_en] == 1)
		PQMaskColorMgr.ModuleSetting.ICM.SatGain = m_VIP_AIPQTable.AI_Ctrl_byte_b[SLR_AI_CTRL_Byte_Sat_Gain];

	return 0;
}
extern unsigned char g_bUseAIPQBin;
char ScalerPQMaskColor_PQMASK_Set_Semantic_Depth_Weight_from_AIPQ_BIN_by_NNSR_mode(void)
{
#if IS_ENABLED(CONFIG_RTK_AI_DRV)
//#ifndef BUILD_QUICK_SHOW
	void * TablePtr = NULL;
	
	TablePtr = scaler_VIP_PQMask_GetSemanticTable(1, g_bUseAIPQBin);	
	if( TablePtr != NULL ) {
		memcpy((void *)PQMaskColorMgr.SemanticWeightTbl, TablePtr, PQMASK_MODULE_NUM*PQMASK_LABEL_NUM*sizeof(unsigned char));
	} else {
		rtd_pr_vpq_emerg("[%s] init default semantic weight table fail\n", __FUNCTION__);
	}
	scalerPQMaskColor_SetReloadPalette(PQMASK_COLOR_PALETTE_SEMANTIC);


	TablePtr = scaler_VIP_PQMask_GetDepthTable(1, g_bUseAIPQBin, aipq_mode.depth_mode);
	if( TablePtr != NULL ) {
		memcpy((void *)PQMaskColorMgr.DepthWeightTbl, TablePtr, PQMASK_MODULE_NUM*PQMASK_MAPCURVE_LEN*sizeof(unsigned char));
	} else {
		rtd_pr_vpq_emerg("[%s] init default depth weight table fail\n", __FUNCTION__);
	}
	scalerPQMaskColor_SetReloadPalette(PQMASK_COLOR_PALETTE_DEPTH);
