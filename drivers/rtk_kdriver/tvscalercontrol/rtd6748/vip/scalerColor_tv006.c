...
{
	extern unsigned char is_HDR_DM_DB_Clean_Waitting;
	static unsigned int dm_hdr_path_sel_reg_ini = 0;
	unsigned char hist_position = 0;
	unsigned char IRQ_Status = 0;

	if(HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[29] == 3){
		return 0;
	}
	if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) && (DM2_Hist_AutoMode_Flag == 1) && (is_HDR_DM_DB_Clean_Waitting == 0)) {
		...
		if (DM2_Hist_AutoMode_Counter == 0) {
			if (IRQ_Status == 1) {
				...
				if (hist_position == DM2_Hist_RGB_MAX_Mode) {
...
UINT8 fwif_color_DM2_EOTF_OETF_3D_LUT_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing)
{
	...
	if ((HDR_control[HDR_timing].EOTF_ctrl.IPCtrlBits) & HFC_bit) {
		...
		rtd_pr_vpq_info("[HFC]EOTF tbl_index=%d,OETF_set_by_user=%d \n",tbl_index,OETF_set_by_user);
		if (tbl_index == HFC_EOTF_OFF) {
			drvif_DM2_EOTF_Enable(0, 1);
	...
	if ((HDR_control[HDR_timing].OETF_ctrl.IPCtrlBits) & HFC_bit) {
		...
		rtd_pr_vpq_info("[HFC]OETF tbl_index=%d \n",tbl_index);
		if (tbl_index == HFC_OETF_OFF) {
			drvif_DM2_OETF_Enable(0, 1);
		} else if (tbl_index == HFC_OETF_CTRL_BY_MAXCLL) {	/*HFC OETF special table index, OETF table control by MAXCLL*/
			drvif_DM2_OETF_Set_Prog_Index(&OETF_prog_idx_tbl[1]);
	...
	if(Need_recheck_hdr_type){
		if(VIP_system_info_structure_table != NULL){
			if(VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10){
				...
				rtd_pr_vpq_info("[HDR10] Need_recheck_hdr_type done \n");
			}
		}
		Need_recheck_hdr_type = 0;
	}
	if(VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_HDR10)
		drvif_color_Set_ST2094_3Dlut_CInv(1024<<5, 0,10);
	else
...
UINT8 fwif_color_DM2_EOTF_OETF_3D_LUT_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing)
{
	...
	rtd_pr_vpq_info("DM_OETF_DMA, set by HFC, HFC_bit_oetf=%d,ret_val=%d,ctrl_bit=%d,tbl_idx=%d, HDR_timing=%d\n",
		HFC_bit_oetf, ret_val, HDR_control[HDR_timing].OETF_ctrl.IPCtrlBits, tbl_index, HDR_timing);
