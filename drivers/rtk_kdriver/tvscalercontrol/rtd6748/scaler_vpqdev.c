...
bool aipq_face_osd_enable=FALSE;
bool aipq_sqm_osd_enable=FALSE;
static bool aipq_genre_demo_enable=FALSE;
// end 0622 lsy
...
*/
	{
		...
		if (mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.od_en == 1) {
			// Od contraint check
			if (/*(Get_DISPLAY_REFRESH_RATE() >= 120)*/ (get_panel_pixel_mode() > PANEL_1_PIXEL) && (mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.Y_mode_en == 0))
			{
				//mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.Y_mode_en = 1;
				rtd_pr_vpq_info("fwif_color_od_init, need to force Y mode,\n");
			}
			if (/*(Get_DISPLAY_REFRESH_RATE() >= 120)*/ (get_panel_pixel_mode() > PANEL_1_PIXEL) && (mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.Rec_OD_en == 1))
			{
				//mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.Rec_OD_en = 0;
				rtd_pr_vpq_info("fwif_color_od_init, need to force disable Rec_OD_en,\n");
			}

			// need disable first
			fwif_color_od_DMA_reset();

			if (fwif_color_od_init(fwif_color_od_Y_Mode_check(mVIP_ODTable.OD_Main_Ctrl.OD_Ctrl.Y_mode_en))) {
				fwif_color_set_od_by_TBL(&mVIP_ODTable);
				bODInited = 1;
				fwif_color_set_od_dma_enable(1);
				usleep_range(21000, 22000);
				fwif_color_set_od_enable(1);
				rtd_pr_vpq_info("vpq_do_OD_init_from_tbl,, enable\n");
			} else {
				bODInited = 0;
...
void vpq_init_gamma_invgamma(void)
{
	...
	extern unsigned short Gamma_Linear2sRGB_12bit[1025];
	//if (Scaler_APDEM_En_Access(0, 0) == 1)
	{
		fwif_color_set_InvGamma(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 0);
		fwif_set_gamma_system(SLR_MAIN_DISPLAY, &Gamma_Linear2sRGB_12bit[0], &Gamma_Linear2sRGB_12bit[0], &Gamma_Linear2sRGB_12bit[0], 0);
...
#endif
extern void scalerVIP_colorMaAuto_TASK_ini(void);
...
void vpq_boot_init(void)
{
	...
	scalerVIP_colorMaAuto_TASK_ini();
	...
	/* demura ini */
	if (drvif_color_get_DeMura_En() == 0)
	{
		fwif_color_DeMura_init(1, 0);
		rtd_pr_vpq_info("demura in boot ini\n");
	}
	else
	{
		rtd_pr_vpq_info("skip demura in boot ini\n");
	}
#endif
	/* register TC DMA*/
...
void vpq_do_OUTGamma_init(void)
{
	...
	if (pTemp16 == NULL)
	{
		rtd_pr_vpq_info("vpq_do_OUTGamma_init,alloc fail,\n");
		return;
	}

	if (mVIP_TABLE_AUTO_ACC_CTRL.Header[AutoACC_CTRL_mode] == AUTO_ACC_Mode_AUTOACC_Fix_Freq)
	{
		//pTemp16 = &mVIP_TABLE_AUTO_ACC_CTRL.OUTGAMMA_TBL[0][0];
		for (i=0;i<1024;i++)
		{
			pTemp16[i] = mVIP_TABLE_AUTO_ACC_CTRL.OUTGAMMA_TBL[VIP_CHANNEL_R][i];
			pTemp16[i+1024] = mVIP_TABLE_AUTO_ACC_CTRL.OUTGAMMA_TBL[VIP_CHANNEL_G][i];
			pTemp16[i+1024*2] = mVIP_TABLE_AUTO_ACC_CTRL.OUTGAMMA_TBL[VIP_CHANNEL_B][i];
		}
	...
	else if (mVIP_TABLE_AUTO_ACC_CTRL.Header[AutoACC_CTRL_mode] == AUTO_ACC_Mode_OUTGAMMA_BIN_Fix_Freq)
	{
		...
		memcpy(pTemp16, &mVIP_OutputGamma.OUTGAMMA_TBL_Extend[tbl_sel][0][0], sizeof(short)*1024*3);
	...
	else
	{
		...
		memcpy(pTemp16, &mVIP_OutputGamma.OUTGAMMA_TBL[tbl_sel][0][0], sizeof(short)*1024*3);
	...
	fwif_color_set_OutputGamma_System_Decode(tmp0, tmp1, pTemp16, pTemp16+SLR_VIP_OUTGAMMA_Index_Num, pTemp16+2*SLR_VIP_OUTGAMMA_Index_Num);
...
{
	extern unsigned char skip_pcid_compare;
...
static int VPQ_detect_tsk(void *p)
{
    ...
    while (1)
    {
		...
		fwif_color_GLC_byMotion_VPQTSK();
...
static __always_inline void checkAllAiApModeAndConf(void)
{
...
int vpq_enable_ai_dbgosd(NN_DBGOSD_T *demo)
{
	static int pre_sqm_mode=0;
	static int pre_face_mode=0;
	static int pre_genre_mode=0;
	RTK_AI_PQ_mode aipq_mode_set;

	if(demo==NULL)
		return -1;

	//SQM
	if ( (demo->model==AI_SQM_DETECT)&& (AI_SQM_DETECT & aipq_support_model_list) ){
		aipq_mode_set=aipq_mode;
		if(demo->enDbgOsd){
			//enable debug osd
			pre_sqm_mode=aipq_mode.sqm_mode;
			aipq_mode_set.sqm_mode=SQM_MODE_DEMO_BAR;
			aipq_sqm_osd_enable=TRUE;
		}else{
			//restore to origin status
			aipq_mode_set.sqm_mode=pre_sqm_mode;
			aipq_sqm_osd_enable=FALSE;
		}
		scalerAI_pq_mode_ctrl(aipq_mode_set,0);
		checkAllAiApModeAndConf();
		return 0;
	}

	//FACE
	if ( (demo->model==AI_FACE_DETECT)&& (AI_FACE_DETECT & aipq_support_model_list) ){
		aipq_mode_set=aipq_mode;
		if(demo->enDbgOsd){
			//enable debug osd
			pre_face_mode=aipq_mode.face_mode;
			aipq_mode_set.face_mode=FACE_MODE_DEMO;
			aipq_face_osd_enable=TRUE;
		}else{
			//restore to origin status
			aipq_mode_set.face_mode=pre_face_mode;
			aipq_face_osd_enable=FALSE;
		}
		scalerAI_pq_mode_ctrl(aipq_mode_set,0);
		checkAllAiApModeAndConf();
		return 0;
	...
	//GENRE,CONTENT
	if ( (demo->model==AI_GENRE_DETECT)&& (AI_GENRE_DETECT & aipq_support_model_list) ){
		aipq_mode_set=aipq_mode;
		if(demo->enDbgOsd){
			//enable genre_demo
			if(aipq_genre_demo_enable==FALSE){
				aipq_genre_demo_enable=TRUE;
				//update genre mode
				pre_genre_mode=aipq_mode.genre_mode;
				aipq_mode_set.genre_mode=GENRE_MODE_PERF;
				scalerAI_pq_mode_ctrl(aipq_mode_set,0);
				checkAllAiApModeAndConf();
			}
		}else{
			//disable genre demo
			if(aipq_genre_demo_enable==TRUE){
				//update genre mode
				aipq_genre_demo_enable=FALSE;
				aipq_mode_set.genre_mode=pre_genre_mode;
				scalerAI_pq_mode_ctrl(aipq_mode_set,0);
				checkAllAiApModeAndConf();
			}
		}
		rtd_pr_vpq_emerg("genre enDbgOsd =%d\n",demo->enDbgOsd);
		return 0;
	}

	//not support for other models
	return -1;
}
...
/*========================= for INNX demura =================================*/
...
#endif
        rtd_pr_vpq_info("VPQ_IOC_AI_DETECTION_ON:list=%d\n",list);
		down(&VPQ_AI_Semaphore);
		if(list&AI_AQ_DETECT) ai_aq_mode=1;
	...
	case VPQ_IOC_AI_DETECTION_OFF:
	{
		...
		down(&VPQ_AI_Semaphore);
		if(list&AI_AQ_DETECT) ai_aq_mode=0;
	...
	case VPQ_IOC_EN_AI_DBGOSD:
	{
		NN_DBGOSD_T demo;

		if (copy_from_user((void *)&demo, (const void __user *)arg, sizeof(NN_DBGOSD_T)))
		{
			rtd_pr_vpq_emerg("VPQ_IOC_EN_AI_DBGOSD fail\n");
			return -1;
		}
		//cann't turn on AI PQ during playing protected content
		if (get_svp_protect_status_aipq()) {
		...
		rtd_pr_vpq_info("VPQ_IOC_EN_AI_DBGOSD:model %d set %d\n",demo.model,demo.enDbgOsd);

		return vpq_enable_ai_dbgosd(&demo);
	}
