...
unsigned char CMA_ME_release_flag = 0;
unsigned char CMA_semaphone_status = 0;
extern unsigned char CMA_release_tsk_flag;
//extern unsigned char CMA_ME_release_tsk_flag;
...
long HAL_VPQ_MEMC_SetMotionComp(unsigned char blurLevel, unsigned char judderLevel, VPQ_MEMC_TYPE_T motion){
	...
	if(g_new_sport_mode){
		motion = 4; //user mode
		ReadRegister(SOFTWARE2_SOFTWARE2_41_reg, 0,  7, &u8_tmp);
		blurLevel = (u8_tmp<128) ? u8_tmp : 128;
		ReadRegister(SOFTWARE2_SOFTWARE2_41_reg, 8, 15, &u8_tmp);
...
unsigned char Scaler_MEMC_CLK_Check(void){
		...
		return FALSE;
#else
	const unsigned int time_60s_in_90K_clk = 60*90000; //60s calculate by 90K clock
	static unsigned int time_pre = 0;
	unsigned int time_cur = 0, time_diff = 0;
	...
	{
		time_cur = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		time_diff = (time_cur>time_pre) ? time_cur-time_pre : time_pre-time_cur;

		if(time_diff > time_60s_in_90K_clk){
			time_pre = time_cur;
...
unsigned char gME_dynamic_allocate_Enable=0;

void Scaler_Set_ME_dynamic_allocate_Enable(unsigned char i){
	gME_dynamic_allocate_Enable=i;
	//rtd_pr_memc_emerg("[%s] DISPScaler_set_MEMC_Low_Rate =%d \n", __FUNCTION__, gME_dynamic_allocate_Enable);
}

extern unsigned char CMA_MEMC_need_reduce_memory(void);

unsigned char Scaler_Get_ME_dynamic_allocate_Enable(void){
	#if 0
	...
	if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
	{
		...
		#endif
		if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY)==3 && Scaler_VOFromVDEC(VO_VIDEO_PLANE_V1)==3 && get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY)==1)
		{
			ret = TRUE;
		}
		else
	...
	rtd_pr_memc_emerg("Scaler_Get_ME_dynamic_allocate_Enable=%d",ret);

	return (ret&gME_dynamic_allocate_Enable);
	#else
	return (CMA_MEMC_need_reduce_memory());
	#endif
}
...
#if 0
unsigned char g_Dynamic_Allocate_or_Release_ME_Memory_status=0;
void Dynamic_Allocate_or_Release_ME_Memory(void){
	bool carvedout_en = carvedout_enable();
	if(!carvedout_en && Scaler_Get_ME_dynamic_allocate_Enable()){
		if(g_Dynamic_Allocate_or_Release_ME_Memory_status == 2){
			CMA_release_tsk_flag=2;//
		}
		else if(g_Dynamic_Allocate_or_Release_ME_Memory_status==1)
		{
			Scaler_MEMC_CMA_Memory_Alloc(3);
		}
	}
	g_Dynamic_Allocate_or_Release_ME_Memory_status=0;
}
#endif

unsigned char g_dynamic_me_release=0;
int Scaler_MEMC_MEMC_CLK_OnOff(unsigned char ME_enable, unsigned char MC_enable, unsigned char bMute)
{
	...
	unsigned char CMA_ME_Allocate_flag=0;
	unsigned char CMA_ME_Release_flag=0;
	bool carvedout_en = carvedout_enable();
...
//	return 0;   // YE Test just for demo to disable power saving
	...
	if((VR360_en == 1) && (ME_enable || MC_enable))
	{
		...
		MEMC_Lib_set_memc_PowerSaving_Mode(0);
		if((Me_clk_state == 0) || (Mc_clk_state == 0) || (crtc1_frc_hact_state == 0) || (MEMC_drvif_mux_state == 0) ){
		...
		if(((Me_clk_state == 1) || (Mc_clk_state == 0) || (crtc1_frc_hact_state == 0) || (MEMC_drvif_mux_state == 0) || (VR360_stage != 0) || (VR360_en == 0 && MC_DMA_read_i_state == 0))){
...
//	if(carvedout_en == FALSE && do_power_saving == 1){
//		do_power_saving = 2;
//		MEMC_Lib_set_memc_PowerSaving_Mode(2);
//	}
	...
	down(&MEMC_CLK_Semaphore);
		if(do_power_saving == 2){  //MC clk on, ME clk off
			...
			if(bMute == 1){
				if(Mc_clk_state == 0){
					Scaler_MEMC_Set_BlueScreen(1);
					rtd_pr_memc_emerg("[Scaler_MEMC_MEMC_CLK_OnOff][MEMC off to MC only][Mute !!]");
				}
				else{
//					if(Get_DISPLAY_PANEL_TYPE()== P_VBY1_TO_HDMI_MEMC_FREERUN){
//						Scaler_MEMC_Set_BlueScreen(1);
//					}	
					rtd_pr_memc_emerg("[Scaler_MEMC_MEMC_CLK_OnOff][MEMC on to MC only][No Mute !!]");
					CMA_ME_Release_flag=1;
				...
				if(Mc_clk_state == 0){
					rtd_pr_memc_emerg("[Scaler_MEMC_MEMC_CLK_MCOn_MEOff][BB][%d]\n", g_MEMC_DMA_state);
					IoReg_Write32(SYS_REG_SYS_SRST3_reg, 0x80000000);//rtd_outl(0xb800010c, 0xc0000000);
					IoReg_Write32(SYS_REG_SYS_CLKEN3_reg, 0x80000001);//rtd_outl(0xb800011c, 0xc0000001);
					IoReg_Write32(SYS_REG_SYS_CLKEN3_reg, 0x80000000);//rtd_outl(0xb800011c, 0xc0000000);
					IoReg_Write32(SYS_REG_SYS_SRST3_reg, 0x80000001);//rtd_outl(0xb800010c, 0xc0000001);
...
unsigned char g_Memc_CMA_Status=0;
...
//					me_size, tmp_addr[u8_k], (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-t1) /90);
			}
			else{
...
//				desc.pa_size[u8_k] = mc_size;
				...
				time_tmp[u8_k] = (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-t1) /90;
//				rtd_pr_memc_emerg("[%s][u8_k,%d,][allcate %x bytes memory %px spend %03d ms]\n", __FUNCTION__, u8_k, 
//					mc_size, tmp_addr[u8_k], (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-t1) /90);
			}
//			rtd_pr_memc_emerg("[%s][addr_align,%x,]\n", __FUNCTION__, tmp_addr_aligned[u8_k]);
			#if 1
...
//				rtd_pr_memc_info("[%s::%d][CMA_ERR] addr[%d] = 0\n", __FUNCTION__, __LINE__, u8_k);
			}
			#else
			if(tmp_addr_aligned[u8_k] != 0 && tmp_addr_aligned[u8_k] != 0xffffffff){
				CMA_memory_allocate_cnt++;
        	          	desc.region_cnt++;
				unconsistant_addr_aligned[u8_k] = tmp_addr_aligned[u8_k];
				desc.pa_start[u8_k] = tmp_addr_aligned[u8_k];
...
//				rtd_pr_memc_info("[%s::%d][CMA_ERR] addr[%d] = 0\n", __FUNCTION__, __LINE__, u8_k);
	...
	// set DCMT
	desc.region_cnt = CMA_memory_allocate_cnt;
...
void Regen_MEMC_ChangeSize(void){
	...
	if(PCModeEnable == TRUE)
	{
		...
		// set release_flag 0, set release_flag =1, after InOutMode of the Regen_MEMC_ChangeSize in CMA process
		CMA_memory_release_flag = 0;
		CMA_ME_release_flag=0;
	}
	else
...
unsigned char OnlyRegenMe=0;

void Regen_ME_ChangeSize(void){
	unsigned char PCModeEnable = MEMC_LibGetMEMC_PCModeEnable();
	OnlyRegenMe=1;
	if(PCModeEnable == TRUE)
	{
...
void Scaler_ME_allocate_memory(void)
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
		return;
#else
...
//				rtd_pr_memc_info("[%s::%d][CMA_ERR] addr[%d] = 0\n", __FUNCTION__, __LINE__, u8_k);
		...
		desc.pa_size[u8_k] = unconsistant_mem_size[u8_k];
		desc.pa_start[u8_k] = unconsistant_addr_aligned[u8_k];
		rtd_pr_memc_emerg("[%s] u8_k=%d,desc.pa_size=%x,desc.pa_start=%x\n", __FUNCTION__,desc.pa_size[u8_k],desc.pa_start[u8_k]);
	}

	//mc
	for(u8_k = 1; u8_k < (mc_bsize+1); u8_k++)
	{
		desc.pa_size[u8_k] = unconsistant_mem_size[u8_k];
		desc.pa_start[u8_k] = unconsistant_addr_aligned[u8_k];
		//CMA_memory_allocate_cnt++;
...
unsigned char Scaler_ME_CMA_Memory_Alloc(void){
	...
	if(CMA_release_tsk_flag && MEMC_Lib_Get_Memory_Quary_Success() == 1){
		rtd_pr_memc_emerg("[%s][%d] [Need to wait ME CMA Release flow][CMA_ME_release_flag=%d][MEMC_Memory_Quary_Success()=%d]\n", __FUNCTION__, __LINE__, CMA_ME_release_flag,MEMC_Lib_Get_Memory_Quary_Success());
		t1 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
		wait_cnt = 0;
		while(MEMC_Lib_Get_Memory_Quary_Success() == 1){
			usleep_range(1000, 2000);
			if(wait_cnt++ > 50){
				Scaler_MEMC_CMA_Memory_Release(CMA_release_tsk_flag);
			}
		}
		rtd_pr_memc_emerg("[%s][%d] %d %d [wait_cnt,%d,] CMA_alloc release ME flow\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)-t1, wait_cnt);
	}

	CMA_semaphone_status = 1;
	if(MEMC_Lib_Get_Memory_Quary_Success() == 0){
		// allocate memory
		Scaler_ME_allocate_memory();
	}

	if(MEMC_Lib_Get_Memory_Quary_Success() == 1&& CMA_release_tsk_flag==0){
		...
		// set release_flag 0, make sure the release_flag=0 after CMA_Memory_Alloc
		//if(CMA_memory_release_flag==1){ 
			//CMA_memory_release_flag = 0;
		//}
		if(CMA_ME_release_flag==1){
			CMA_ME_release_flag=0;
		...
		rtd_pr_memc_emerg("[%s::%d][CMA_ERR][MEMC_Lib_Get_Memory_Quary_Success = %d][CMA_release_tsk_flag=%d]\n", __func__, __LINE__,MEMC_Lib_Get_Memory_Quary_Success(),CMA_release_tsk_flag);
		ret = FALSE;
	}
...
unsigned char MEMC_Check_CMA_Allocate_Status(unsigned char i){
	//g_Memc_CMA_Status	
	//allocate memc 1
	//allocate me 2
	//release me 3
	...
	unsigned int cma_status=0;
	
	down(&MEMC_CMA_Semaphore);
	if(g_Memc_CMA_Status==0){
		cma_status=i;
	}
	else if(i==1){////allocate memc 1
		if(g_Memc_CMA_Status==4){//release memc
			cma_status= i;
		}
		else if(g_Memc_CMA_Status==3){//release me
			cma_status=2;
		}
		else{
			cma_status=0;
	...
	else if(i==2){//allocate me
		if(g_Memc_CMA_Status==3){//release me
			cma_status=i;
		}
		else{
			cma_status=0;
...
unsigned char MEMC_Check_CMA_Release_Status(unsigned char i){
	//g_Memc_CMA_Status	
	//allocate memc 1
	//allocate me 2
	//release me 3
	...
	else if(i==1){////release memc 1
		cma_status=i;
	}
	else if(i==2){//release me
		if(g_Memc_CMA_Status==4){//release memc
			cma_status=0;
		}
		else if(g_Memc_CMA_Status==3){//release me
...
unsigned char Scaler_MEandMC_CMA_Memory_Alloc(void)
{//Scaler_MEMC_CMA_Memory_Alloc
	memc_mux_memc_mux_func_ctrl_RBUS memc_mux_memc_mux_func_ctrl_reg;
	unsigned char ret = FALSE;
	unsigned int t1 = 0;
	unsigned char wait_cnt = 0;
	...
	if(CMA_memory_release_flag && MEMC_Lib_Get_Memory_Quary_Success() == 1){
		rtd_pr_memc_emerg("[%s][%d] [Need to wait MEMC CMA Release flow][CMA_memory_release_flag=%d][MEMC_Memory_Quary_Success()=%d]\n", __FUNCTION__, __LINE__, CMA_memory_release_flag,MEMC_Lib_Get_Memory_Quary_Success());
		t1 = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
		wait_cnt = 0;
		while(MEMC_Lib_Get_Memory_Quary_Success() == 1){
			usleep_range(1000, 2000);
			if(wait_cnt++ > 50){
				Scaler_MEMC_CMA_Memory_Release(CMA_memory_release_flag);
			}
		}
		rtd_pr_memc_emerg("[%s][%d] %d %d [wait_cnt,%d,] CMA_alloc release flow\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg)-t1, wait_cnt);
	}
...
//		MEMC_Lib_Set_MEMC_Regen_Address();
		...
		*/
		...
		WriteRegister(LBMC_LBMC_EC_reg, 2, 2, 0x1);
...
unsigned char Scaler_MEMC_CMA_Memory_Alloc(unsigned char i)
{

	i=MEMC_Check_CMA_Allocate_Status(i);
	
	if(i==0){
		return 1;
	}

	if(i==2)//ME_CMA_Memory_Alloc
	{	
		return Scaler_ME_CMA_Memory_Alloc();
	}
	else if(i==1){//ME_MC_CMA_Memory_Alloc
		return Scaler_MEandMC_CMA_Memory_Alloc();
	}
...
void Scaler_ME_CMA_Memory_Release(void){
	...
	//ReadRegister(SOFTWARE2_SOFTWARE2_48_reg, 0, 11, &test_val);
	//memc_mux_memc_mux_func_ctrl_reg.regValue = rtd_inl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg);
	...
	//rtd_outl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg, memc_mux_memc_mux_func_ctrl_reg.regValue);
	#if 0 
	rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][wait_cnt,%d,][vsync_cnt,%d,%d,]\n", __FUNCTION__, __LINE__, 
		(rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, wait_cnt, g_memc_outpur_vsync_cnt, frm_cnt_pre);
	#endif
	//wait
	#if 0
	time1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	...
	#endif
#if 0 
	rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][wait_cnt,%d,][in_vsync_cnt,%d,%d,][out_vsync_cnt,%d,%d,]\n", __FUNCTION__, __LINE__, 
		(rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, wait_cnt, g_memc_input_vsync_cnt, input_frm_cnt_pre, g_memc_outpur_vsync_cnt, frm_cnt_pre);
#endif
 
	// release memory
	rtd_pr_memc_emerg("[%s] MUTEX DOWN@%x\n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
	t0 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	if(CMA_semaphone_status == 0 || CMA_semaphone_status == 2){
	down(&MEMC_CMA_Semaphore);
	
	rtd_pr_memc_emerg("[%s::%d][desc 1 =!!][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,]\n", __func__, __LINE__,
		desc.pa_start[0], desc.pa_start[1], desc.pa_start[2], desc.pa_start[3], desc.pa_start[4],
		desc.pa_start[5], desc.pa_start[6], desc.pa_start[7], desc.pa_start[8],
		desc.pa_size[0], desc.pa_size[1], desc.pa_size[2], desc.pa_size[3], desc.pa_size[4],
		desc.pa_size[5], desc.pa_size[6], desc.pa_size[7], desc.pa_size[8]);
	
	// unset DCMT
	dcmt_cma_memc_unset(&desc);
 
	for(u8_k = 0; u8_k < 1; u8_k++){//release me
		t1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		if(unconsistant_addr[u8_k] != 0){
			// set DCMT addr
			//desc.pa_start[u8_k] = unconsistant_addr_aligned[u8_k];
	...
	if(CMA_memory_allocate_cnt>0){
		CMA_memory_allocate_cnt --;
	}
	
	for(u8_k = 0; u8_k < CMA_memory_allocate_cnt; u8_k++){
		desc.pa_size[u8_k] = unconsistant_mem_size[u8_k+1];
		desc.pa_start[u8_k] = unconsistant_addr_aligned[u8_k+1];
	}
	...
	rtd_pr_memc_emerg("[%s::%d][desc 2 =!!][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,]\n", __func__, __LINE__,
		desc.pa_start[0], desc.pa_start[1], desc.pa_start[2], desc.pa_start[3], desc.pa_start[4],
		desc.pa_start[5], desc.pa_start[6], desc.pa_start[7], desc.pa_start[8],
		desc.pa_size[0], desc.pa_size[1], desc.pa_size[2], desc.pa_size[3], desc.pa_size[4],
		desc.pa_size[5], desc.pa_size[6], desc.pa_size[7], desc.pa_size[8]);
	...
	CMA_release_tsk_flag=0;//CMA_ME_release_tsk_flag = 0;
	MEMC_Lib_Set_Memory_Quary_Success(0);
	MEMC_Lib_Set_Memory_Write_Done(0);
	}
...
void Scaler_MEandMC_CMA_Memory_Release(void){//release MEMC_CMA_Memory 
#if 1
	unsigned char u8_k = 0;
	unsigned int time1 = 0, time2 = 0, t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4 = 0, wait_cnt = 0;
	unsigned int test_val = 0;
	...
	time2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	wait_cnt = 0;
	rtd_pr_memc_emerg("[%s][%d][%d][%d,%d][clock,%x,%x,]\n", __FUNCTION__, __LINE__, (rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>12)&0x1, wait_cnt, (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-time2), IoReg_Read32(SYS_REG_SYS_CLKEN3_reg), IoReg_Read32(SYS_REG_SYS_SRST3_reg));
	...
	rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,cnt,%d,]\n", __FUNCTION__, __LINE__, 
		rtd_inl(DBUS_WRAPPER_Memc_wrapper_status1_reg), (rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-time2), wait_cnt);
 
	time2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	wait_cnt = 0;
	while(rtd_inl(DBUS_WRAPPER_Memc_wrapper_status1_reg) != 0){
		hw_msleep(1);
		if(wait_cnt++>30)
			break;
	}
	...
	rtd_pr_memc_emerg("[%s::%d][MEMC release end !!][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,][time_stemp,%d,%d,%d,%d,%d,]\n", __func__, __LINE__,
		...
		t0, t1, t2, t3, t4);
#else
	unsigned int t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4 = 0, t5 = 0;
	memc_mux_memc_mux_func_ctrl_RBUS memc_mux_memc_mux_func_ctrl_reg;
 
	rtd_pr_memc_emerg("[%s::%d][MEMC release start !!][,%x,%x,%x,%x,%x,]\n", __func__, __LINE__,
		unconsistant_addr[0], unconsistant_addr[1], unconsistant_addr[2], unconsistant_addr[3], unconsistant_addr[4]);
 
	rtd_pr_memc_emerg("[%s] MUTEX DOWN @%d\n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
	t0 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	down(&MEMC_CMA_Semaphore);
	...
	MEMC_Lib_Set_CMA_Release_Stage(1);
 
	up(&MEMC_CMA_Semaphore);
	t5 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	rtd_pr_memc_emerg("[%s] MUTEX UP @%d\n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
 
	rtd_pr_memc_emerg("[%s::%d][MEMC release stage 1 !!][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,][time_stemp,%d,%d,%d,%d,%d,%d,]\n", __func__, __LINE__,
		unconsistant_addr[0], unconsistant_addr[1], unconsistant_addr[2], unconsistant_addr[3], unconsistant_addr[4],
		unconsistant_addr[5], unconsistant_addr[6], unconsistant_addr[7], unconsistant_addr[8],
		t0, t1, t2, t3, t4, t5);
 
	rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,]\n", __FUNCTION__, __LINE__, 
		rtd_inl(DBUS_WRAPPER_Memc_wrapper_status1_reg), (rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
#endif
}

void Scaler_MEMC_CMA_Memory_Release(unsigned char i){

	i=MEMC_Check_CMA_Release_Status(i);
	if(i==0){
		CMA_release_tsk_flag = 0;
		return;
	}

	if(i==2){//release ME_CMA_Memory 
		Scaler_ME_CMA_Memory_Release();
	}
	else if(i==1){//release ME_MC_CMA_Memory 
		Scaler_MEandMC_CMA_Memory_Release();
	}
}
...
void Scaler_MEMC_Handler(void){
	...
	if(MEMC_instanboot_resume_Done == 1){
		if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_4K1K){ //4k1k
