	...
	if (sm_info_pkg) {
        ...
        if(true == update_info.sm_info[display].done_flag){
			...
			if (0==scaler_before) {//after scaler need to update smoothtoggle
				...
				if((display == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) ||(force_enable_mdomain_2disp_path())))
...
void zoom_smoothtoggle_gamemode_set_smt_status(unsigned char display,unsigned char value)//value:1 set, 0 clear
{
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		if(value == 0){
			...
			rtd_pr_smt_notice("[SMT]clear screen change status\n");
			return;
		}
		...
		if(((main_active_v_start_end_reg.mv_act_end - main_active_v_start_end_reg.mv_act_sta) < _DISP_LEN)){
			if(Scaler_DispWindowGet().src_height ==  _DISP_LEN){//small to full screen
				...
				rtd_pr_smt_notice("[SMT]small to full screen\n");
				...
				rtd_pr_smt_notice("[SMT]small to small screen\n");
			}
			...
			rtd_pr_smt_notice("[SMT]full to small screen\n");
		}else if(((main_active_v_start_end_reg.mv_act_end - main_active_v_start_end_reg.mv_act_sta) == _DISP_LEN)
			&& (Scaler_DispWindowGet().src_height == _DISP_LEN)){//full to full screen
			rtd_pr_smt_notice("[SMT]full to full screen\n");
		}
	}
}
...
//	Set_vo_smoothtoggle_changeratio_flag2(TRUE,display);
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		...
		IoReg_Write32(MDOMAIN_CAP_DDR_In1_3rdAddr_reg, (drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_THIRD) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_DDR_In1_2ndAddr_reg, (drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_SEC) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_DDR_In1Addr_reg, (drvif_memory_get_memtag_startaddr(MEMIDX_MAIN) & 0xfffffff0));
		//M-domain boundary
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_THIRD) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(drvif_memory_get_memtag_startaddr(MEMIDX_MAIN)+buffer_size-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
#endif
#else
#ifdef CONFIG_ENABLE_3BUFFER_NEW_RULE
