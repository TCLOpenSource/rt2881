...
//#else
	...
	{
		rtd_pr_vbe_info("#### [%s %d] bypass memc case\n", __FUNCTION__, __LINE__);
		sys_dispclksel_reg.regValue = IoReg_Read32(SYS_REG_SYS_DISPCLKSEL_reg);
	...
	if((Scaler_DispGetInputInfo(SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL) || (get_panel_pixel_mode() > PANEL_1_PIXEL)){
		if(Scaler_DispGetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) % 2)
    ...
    {
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"enter vbe_disp_decide_uzuclock_mapping_vo\n");
	...
	if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) > 990)){
		dclk_fract_a = 0;
		dclk_fract_b = 0;
	}
...
//#else
    ...
    if(get_panel_pixel_mode() > PANEL_1_PIXEL){
		if(Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_FRAC_FS_MULTIPLE_RATIO) % 2)
