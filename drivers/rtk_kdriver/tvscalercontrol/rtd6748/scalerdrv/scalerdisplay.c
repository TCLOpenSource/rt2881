...
unsigned int scalerdisplay_get_vtotal_by_vfreq(UINT16 vsyncFreq)
{
    ...
    if((0 != h_total) && (vsyncFreq != 0))
    {
        if(get_panel_pixel_mode() > PANEL_1_PIXEL)
        {
            vertical_total = Get_DISPLAY_CLOCK_TYPICAL() * 2 / h_total / vsyncFreq;
        }
        else
        {
