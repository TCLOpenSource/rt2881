...
static unsigned int uninterrupted_ivs_cnt_ori_2_frame[2] = {0, 0};

static unsigned char enable_ivs_cnt_ori_uninterrupted_2_frame = FALSE;

void set_uninterrupted_ivs_cnt_ori_for_2_frame(unsigned char index, unsigned int cnt)
{
    if(index > 1)
    {
        return;
    ...
    uninterrupted_ivs_cnt_ori_2_frame[index] = cnt;
}

unsigned int get_uninterrupted_ivs_cnt_ori_for_2_frame(unsigned char index)
{
    if(index > 1)
    {
        return 0;
    ...
    return uninterrupted_ivs_cnt_ori_2_frame[index];
}

void set_enable_ivs_cnt_ori_uninterrupted_2_frame(unsigned char enable)
{
    enable_ivs_cnt_ori_uninterrupted_2_frame = enable;
}

unsigned char get_enable_ivs_cnt_ori_uninterrupted_2_frame(void)
{
    return enable_ivs_cnt_ori_uninterrupted_2_frame;
}
...
//#endif
    ...
    {
        set_enable_ivs_cnt_ori_uninterrupted_2_frame(1);
        
        count = 0x3fffff;

        while(--count)
        {
            if(get_enable_ivs_cnt_ori_uninterrupted_2_frame() == 0)
            {
                break;
            }
        ...
        if(count == 0)
        {
            rtd_pr_vbe_notice("[%s %d]ERROR Wait for ivs ori cnt timeout\n", __FUNCTION__, __LINE__);
        ...
        {
            rtd_pr_vbe_notice("[%s %d] ivs ori cnt(%d %d)\n", __FUNCTION__, __LINE__, get_uninterrupted_ivs_cnt_ori_for_2_frame(0), get_uninterrupted_ivs_cnt_ori_for_2_frame(1));
            intputVsyncCount = (get_uninterrupted_ivs_cnt_ori_for_2_frame(0) + get_uninterrupted_ivs_cnt_ori_for_2_frame(1)) / 2;
        }

        intputFrameRate = 2700000000UL/intputVsyncCount;
    }
    else
	{
...
unsigned int framesync_get_iv2dv_delay(unsigned int dispTimingMode)
{
	unsigned int iv2dv_delay = 0;
    unsigned char isGameModeiv2dv = false;
    ...
    if(dispTimingMode == DISPLAY_MODE_RESET_VODMA){
		...
		if(drv_memory_get_low_delay_game_mode_dynamic()){//if game mode case, need to use calculation result @Crixus 20170712
			...
			if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC)){
				...
				if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())&& ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_LEN)== _DISP_LEN)){
				...
				else if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (!(drv_memory_get_vdec_direct_low_latency_mode()&& (get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY) == ADAPTIVE_STREAM_ON)))){  //vdec no need calculate iv2dv delay
					iv2dv_delay = 2;
		...
		else{
			{
				if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC)){
					...
					if((drv_memory_get_vdec_direct_low_latency_mode() && (get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY) == ADAPTIVE_STREAM_ON))){
					...
					else if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())&& ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_LEN)== _DISP_LEN)){
					...
					else if((get_panel_pixel_mode() == PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())){
						iv2dv_delay = drv_GameMode_decided_iv2dv_delay_new_mode();
                        isGameModeiv2dv = true;
					}
                    else
                    {
                        ...
                        {
                            iv2dv_delay = framesync_get_video_line_buffer_delay();
                            rtd_pr_vbe_notice("[%s][line:%d] data frc with adaptive stream\n", __FUNCTION__, __LINE__);
                        }
                        else
                        {
                            iv2dv_delay = 2;
                            rtd_pr_vbe_notice("[%s][line:%d] data frc else case\n", __FUNCTION__, __LINE__);
                        }
                    }
				}
			}
        ...
        if(!isGameModeiv2dv) // iv2dv get by drv_GameMode_decided_iv2dv_delay_new_mode() no need to add offset
        {
            ...
            {
                iv2dv_delay += DTG_VS_BY_HS_EN_IV2DV_OFFSET;
                rtd_pr_vbe_emerg(KERN_NOTICE, TAG_NAME_VBE,"VS_BY_HS_EN, iv2dv=%d", iv2dv_delay);
            }
        }
#ifndef BUILD_QUICK_SHOW
        if(Get_Force_run_fake_scaler_State())
        {
            ...
            iv2dv_delay = _VO_GATTING_OFFSET;
...
void framesync_set_iv2dv_delay_disable_db(unsigned int iv2dv_delay)
{
...
void framesync_set_iv2dv_delay(unsigned int dispTimingMode)
{
    ...
    iv2dv_delay = framesync_get_iv2dv_delay(dispTimingMode);
...
unsigned int drv_framesync_get_gamemode_iv2dv_linedelay(void)
{
    ...
    if(!get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && (Scaler_InputSrcGetType(SLR_MAIN_DISPLAY) != _SRC_VO))
    {
        ...
        {
            iClk = hdmi_htotal;
            iClk = iClk * hdmi_vtotal * hdmi_vfreq / 10;
            iHtotal = hdmi_htotal;
        }
        else
        {
            iClk = htotal;
            iClk =iClk * vtotal * vfreq / 10;
            iHtotal = htotal;
        }
    }
    else
    ...
    rtd_pr_new_game_mode_notice("[Game Mode][%s] line = %d, dOneLineTime:%d\n", __FUNCTION__, line, dOneLineTime);
