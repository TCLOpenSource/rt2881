...
unsigned char drvif_hdr_idma_color_colorspacergb2yuvtransfer_check(void)
{
	h3ddma_rgb2yuv_dither_4xxto4xx_hdmi_rgb2yuv_ctrl_RBUS  i3ddma_rgb2yuv_ctrl_reg;

	//DolbyVision sinkled mode
	if (get_HDMI_HDR_mode() == HDR_DOLBY_HDMI && (get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_h14B_VSIF || get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_STD)) {
		i3ddma_rgb2yuv_ctrl_reg.regValue = rtd_inl(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_RGB2YUV_CTRL_reg);

		if(0 != i3ddma_rgb2yuv_ctrl_reg.en_rgb2yuv) {
			rtd_pr_hdr_emerg("%s Error, DVmode=%x, H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_RGB2YUV_CTRL=%x, H3DDMA_I3DDMA_enable=%x\n", 
				__FUNCTION__, get_HDMI_Dolby_VSIF_mode(), i3ddma_rgb2yuv_ctrl_reg.regValue, rtd_inl(H3DDMA_I3DDMA_enable_reg));
