...
static AI_TRANSLATION_INFO ai_translation_info = {0, 0};

/*================================== extern ==============================*/
unsigned short GetAiTransFuncInfo(unsigned short *frameNum)
{//get ai translation enable or not
	if(frameNum)
		*frameNum = ai_translation_info.Frame_Delay_Num;
	return ai_translation_info.Enable;
}

void SetAiTransFuncInfo(unsigned short enable, unsigned short delaynum)
{//update ai translation info
	rtd_pr_vsc_info("[%s-%d] enable: %d -> %d, frame delay num: %d -> %d\n", __func__, __LINE__, ai_translation_info.Enable, enable, ai_translation_info.Frame_Delay_Num, delaynum);
	ai_translation_info.Frame_Delay_Num = delaynum;
	ai_translation_info.Enable = enable;
}
...
static unsigned char	Scaler_FreeRun_To_FrameSync_Reset_VODMA_Vgip_Hs_Iv2dv_Flag = _DISABLE;
void Scaler_Set_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(unsigned int ucEnable)
{
	Scaler_FreeRun_To_FrameSync_Reset_VODMA_Vgip_Hs_Iv2dv_Flag = ucEnable;
}

unsigned int Scaler_Get_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(void)
{
	return Scaler_FreeRun_To_FrameSync_Reset_VODMA_Vgip_Hs_Iv2dv_Flag;
}

static unsigned int Scaler_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv = 0;
void Scaler_Set_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv(unsigned int iv2dv)
{
	Scaler_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv = iv2dv;
}

unsigned int Scaler_Get_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv(void)
{
	return Scaler_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv;
}

#ifdef CONFIG_RESET_VODMA_BY_MEASURE
// 0: Not Running Reset VODMA by Measure
// 1: Running Reset VODMA by Measure
// 2: Reset VODMA by Measure Failed
static unsigned char	Scaler_FreeRun_To_FrameSync_Reset_VODMA_Measure_Flag = _DISABLE;
void Scaler_Set_Reset_VODMA_Measure_Flag(unsigned int flag)
{
	Scaler_FreeRun_To_FrameSync_Reset_VODMA_Measure_Flag = flag;
}
unsigned int Scaler_Get_Reset_VODMA_Measure_Flag(void)
{
	return Scaler_FreeRun_To_FrameSync_Reset_VODMA_Measure_Flag;
}
static unsigned int	Scaler_FreeRun_To_FrameSync_Reset_VODMA_UzuLc = _DISABLE;
void Scaler_Set_Reset_VODMA_UzuLc(unsigned int uzuLc)
{
	Scaler_FreeRun_To_FrameSync_Reset_VODMA_UzuLc = uzuLc;
}
unsigned int Scaler_Get_Reset_VODMA_UzuLc(void)
{
	return Scaler_FreeRun_To_FrameSync_Reset_VODMA_UzuLc;
}
#endif
...
#ifndef UT_flag
extern unsigned int framesync_get_iv2dv_delay(unsigned int dispTimingMode);
#endif
...
OLED_RESET_TARGET_CASE Get_OledResetTargetCase(void)
{
	...
	{
		return NON_OLED_2K_RESET_TARGET_CASE;
	}
    else if(Get_DISPLAY_PANEL_OLED_TYPE() == 1)
    {
        ...
        if(Get_DisplayMode_Src(display) == VSC_INPUTSRC_VDEC)
        {
            if(get_vsc_run_adaptive_stream(display))
            {
                if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) > 300)
                {
                    if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN) <= 1440)
                    {
                        if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) >= 595 && Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) <= 605)
                        {
                            return NON_OLED_RESET_TARGET_CASE_CP_RPL_60HZ;
                        }
                        else
                        {
...
void set_HDMI_Dolby_H14B_VSIF_mode(DOLBY_HDMI_VSIF_T vsif_mode, unsigned char port)
{
	...
	if (dolby_h14b_vsif_mode[port] != vsif_mode) {
		rtd_pr_vbe_notice("[Dolby][%s:%d] (%ld -> %d) port (%d %d)\n", __func__, __LINE__
...
void drvif_scaler_ddomain_DTG2_line_compare_irq(unsigned char enable, unsigned char vlc_mode, unsigned int vlc_src_sel, unsigned int dtg_vln)
{
	ppoverlay_dtg_lc_RBUS dtg_lc_reg;
	...
	dtg_lc_reg.dtg_vlc_mode= vlc_mode;	// mode
	dtg_lc_reg.dtg_vln = dtg_vln;	// v length
	dtg_ie_2_reg.dtg_vlc_ie_2 = enable;	// function en
	dtg_lc_reg.dtg_vlcen= enable;	// Interrupt en
	dtg_lc_reg.dtg_vlc_src_sel = vlc_src_sel;

	IoReg_Write32(PPOVERLAY_DTG_LC_reg, dtg_lc_reg.regValue);
	IoReg_Write32(PPOVERLAY_DTG_ie_2_reg, dtg_ie_2_reg.regValue);
	rtd_pr_vbe_notice("[%s][VDF_GLC]%d,%d,%d,%d\n", __FUNCTION__, enable, vlc_mode, vlc_src_sel, dtg_vln);
}
...
//#endif
    ...
    if(Get_Vgip_Hs_En_Iv2dv_condition()
        && (display == SLR_MAIN_DISPLAY))
    {
        vgip_chn1_ctrl_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg);
        if(vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n == 0)
        {
            vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n = 1;
            IoReg_Write32(VGIP_VGIP_CHN1_CTRL_reg, vgip_chn1_ctrl_reg.regValue);
            rtd_pr_vbe_notice("Set ch1_vs_by_hs_en_n %d (%x)", vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n, IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg));
        }
    }
	...
	else
    {
        ...
        if(Get_DISP_VERTICAL_TOTAL() - Get_DISP_DEN_END_VPOS() < 45)
        {
            ...
            {
                ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target = Get_DISP_DEN_END_VPOS() + PANEL_DEN_PROTECT_PORCH / 2 + 8;
            }
            else
            {
...
#define VODMA_IV2DV_0_RESET_TARGET_2K_OFFSET 0
#define NON_OLED_RESET_TARGET_CASE_CP_RPL_60HZ_OFFSET 6
...
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
void drvif_scaler_set_Vodma_reset_target_line_count(RESET_VODMA_PHASE resetVodmaPhase)
{
	ppoverlay_dtg_vo_lc_RBUS ppoverlay_dtg_vo_lc_reg;

	rtd_pr_vbe_notice("[voreset]RESET_VODMA_PHASE=%d", resetVodmaPhase);

	if(resetVodmaPhase == RESET_VODMA_PHASE_MEASURE) // measure
	{
		...
		ppoverlay_dtg_vo_lc_reg.dtg_vo_vln = 0;
		IoReg_Write32(PPOVERLAY_DTG_VO_LC_reg, ppoverlay_dtg_vo_lc_reg.regValue);

		Scaler_Set_Reset_VODMA_Measure_Flag(1);
	}
	else if(resetVodmaPhase == RESET_VODMA_PHASE_RESULT)
	{
		...
		reset_target = ((((dVtotal * (mDtg_mul-1) + frame_sync_den_target)*dHTotal*dclk_frac_a)/(dclk_frac_b*b_2pixel_mode*uzuHTotal)) - ((mDtg_rem-1)*uzuVTotal))-1;

		offset = Scaler_Get_Reset_VODMA_UzuLc();
		...
		IoReg_Write32(PPOVERLAY_DTG_VO_LC_reg, ppoverlay_dtg_vo_lc_reg.regValue);
		rtd_pr_vbe_notice("[voreset]dtg_vo_vln=%d,reset_target=%d,offset=%d", dtg_vo_vln, reset_target, offset);
		rtd_pr_vbe_notice("[voreset]frame_sync_den_target=%d,frac_a=%d,frac_b=%d,2px=%d,mDtg_rem=%d,mDtg_mul=%d,pnlVTotal=%d,dVtotal=%d,dHTotal=%d,uzuVTotal=%d,uzuHTotal=%d\n",
		frame_sync_den_target, dclk_frac_a, dclk_frac_b, b_2pixel_mode, mDtg_rem, mDtg_mul, pnlVTotal, dVtotal, dHTotal, uzuVTotal, uzuHTotal);
	}
	else
	{
		rtd_pr_vbe_notice("[voreset]RESET_VODMA_PHASE ERROR: unknow phase type");
	}
}
#else
void drvif_scaler_set_Vodma_reset_target_line_count(OLED_RESET_TARGET_CASE resetCase)
{
    ...
    if(resetCase != OLED_RESET_TARGET_CASE_NONE)
    {
        switch (resetCase)
        {
            ...
            case NON_OLED_RESET_TARGET_CASE_CP:
            {
                ...
                if((panel_dlg_get_cur_dlg_mode() == PANEL_DLG_4k1k120) && !panel_dlg_get_down_sample())
                {
                    reset_target_offset -= 2;
                }
            }
            break;
            ...
            break;
			case NON_OLED_2K_RESET_TARGET_CASE:
			{
				reset_target_offset = VODMA_IV2DV_0_RESET_TARGET_2K_OFFSET + reset_target_clk_offset;
			}
            break;
            case NON_OLED_RESET_TARGET_CASE_CP_RPL_60HZ:
            {
                reset_target_offset = NON_OLED_RESET_TARGET_CASE_CP_RPL_60HZ_OFFSET + reset_target_clk_offset;
            }
			break;
            default:
            break;
    ...
    else
    {
        ...
        if((Get_DISPLAY_PANEL_OLED_TYPE()==1) && (get_panel_pixel_mode() == PANEL_1_PIXEL))
...
unsigned char Get_Vgip_Hs_En_Iv2dv_condition(void)
{
    SLR_VOINFO* pVOInfo = Scaler_VOInfoPointer(Scaler_Get_CurVoInfo_plane());
    bool ret = false;
    if(pVOInfo)
    {
        ret = ((Get_DISPLAY_PANEL_OLED_TYPE()==TRUE)
...
void Set_Vgip_hs_en_Disabled(void)
{
	vgip_vgip_chn1_ctrl_RBUS vgip_chn1_ctrl_reg;
	vgip_chn1_ctrl_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg);
	if(vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n == 0)
	{
		vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n = 1; // ch1_vs_by_hs_en_n = 1 means disable
		IoReg_Write32(VGIP_VGIP_CHN1_CTRL_reg, vgip_chn1_ctrl_reg.regValue);
		rtd_pr_vsc_notice("Set ch1_vs_by_hs_en_n %d (%x)", vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n, IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg));
	}
}

void Clear_Vgip_hs_en(void)
{
	vgip_vgip_chn1_ctrl_RBUS vgip_chn1_ctrl_reg;
	vgip_chn1_ctrl_reg.regValue = IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg);
	if(vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n == 1)
	{
		vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n = 0;
		IoReg_Write32(VGIP_VGIP_CHN1_CTRL_reg, vgip_chn1_ctrl_reg.regValue);
		rtd_pr_vsc_notice("Clear ch1_vs_by_hs_en_n %d (%x)", vgip_chn1_ctrl_reg.ch1_vs_by_hs_en_n, IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg));
	}
}
...
//	UINT32 count = 20;
    ...
    if(Get_Vgip_Hs_En_Iv2dv_condition())
    {
        Set_Vgip_hs_en_Disabled();
        iv2dv_delay = framesync_get_iv2dv_delay(DISPLAY_MODE_FRAME_SYNC);
        rtd_pr_vsc_notice("pre calculate iv2dv_delay = %d", iv2dv_delay);
...
#ifdef CONFIG_RESET_VODMA_BY_NEW_MODE
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
	drvif_scaler_set_Vodma_reset_target_line_count(RESET_VODMA_PHASE_MEASURE);
#else
...
//	speedup_rerun_resetvo_by_90K = (90000*10/Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ));//*3/4; //3/4 frame time
...
#ifdef CONFIG_RESET_VODMA_BY_NEW_MODE
#ifdef CONFIG_RESET_VODMA_BY_MEASURE
	while(Scaler_Get_Reset_VODMA_Measure_Flag() == 1)
	{
		msleep(0);
	}
	if(Scaler_Get_Reset_VODMA_Measure_Flag() == 2)
	{
		TimeOutCounter = TimeOutCounterThreshold + 1;
		rtd_pr_vbe_emerg("[voreset]Force error wait timeout\n");
		Scaler_Set_Reset_VODMA_Measure_Flag(0);
	}
	else
	{
		drvif_scaler_set_Vodma_reset_target_line_count(RESET_VODMA_PHASE_RESULT);
		drvif_scaler_run_Reset_VODMA();
	}
#endif
#endif
    ...
    if(Get_Vgip_Hs_En_Iv2dv_condition())
    {
        ...
        {
            int TimeOutCt = 0;
            Scaler_Set_FreeRun_To_FrameSync_Vgip_Hs_Iv2dv(iv2dv_delay);
            Scaler_Set_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(_ENABLE);
            while(Scaler_Get_Reset_VODMA_Vgip_Hs_Iv2dv_Flag())
            {
                TimeOutCt++;
                if(TimeOutCt > TimeOutCounterThreshold)
                {
                    rtd_pr_vbe_notice("Vgip_Hs_Iv2dv wait timeout......\n");
                    Scaler_Set_Reset_VODMA_Vgip_Hs_Iv2dv_Flag(_DISABLE);
                    framesync_set_iv2dv_delay(DISPLAY_MODE_FRAME_SYNC);
                    break;
                }
                msleep(0);
...
int drvif_scaler_get_abnormal_dvs_protect(unsigned int *dvs_long, unsigned int *dvs_short)
{
    ...
    switch(get_panel_res())
    {
        ...
        default:
            if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
            {
                *dvs_long = DVS_4K_VRR_LONG_LINE_THRESHOLD;
            }
            else
            {
...
void WaitFor_DEN_STOP_MEMCDTG(void)
{
    ...
    if(timeoutcnt == 0)
    {
        rtd_pr_vbe_err("[%s %d] ERROR not wait the correct line %d, denEnd:%d\n", __FUNCTION__, __LINE__, ppoverlay_new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt, denEnd);
    }
}
...
//			Scaler_DispSetInputInfo(SLR_INPUT_DI_WID,Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID));
            ...
            {
                unsigned short rotate_wid, rotate_len;
                unsigned short cap_wid, cap_len;
...
//frank@02012013 Change below code to solve set sub display window BUG --
...
#endif
	{
		if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && force_enable_mdomain_2disp_path()) {
		...
		{//decide mdoamin size to caveout size
			...
			if (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC) == FALSE) {
				if(get_bypass_memc_cma_alloc_flag_scaler() == TRUE)
					rtd_pr_vsc_info("[%s] %d %d bypass memc case, use 4k data FRC\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
				else
				{
	...
	//if (Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE())
	{
		...
		rtd_pr_vbe_emerg("IV_FREQ = %d, 422=%d\n", Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ), Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422));
		...
		rtd_pr_vbe_emerg("OLED = %d, M2disp = %d, Game = %d, PC = %d, HFR = %d\n", Get_DISPLAY_PANEL_OLED_TYPE(), force_enable_mdomain_2disp_path(), drv_memory_get_game_mode_flag(), get_vsc_run_pc_mode(), jude_input_timing_is_hfr(display));

#endif

	if((display == SLR_MAIN_DISPLAY) && ((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_0) ||(force_enable_mdomain_2disp_path())))
...
//	tRPCShareMem->ulAddr[EMF_COEF_MEM_IDX] = g_atShareMemInfo[SCALERIOC_SETEMF].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[HDMI_DOLBY_TYPE_IDX] = g_atShareMemInfo[HDMI_DOLBY_TYPE_FLAG].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	//#endif
	...
	tRPCShareMem->ulAddr[SCALER_VIDEO_LATENCY_IDX] = g_atShareMemInfo[SCALER_VIDEO_LATENCY].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[HDMI_DOLBY_TYPE_IDX] = htonl(tRPCShareMem->ulAddr[HDMI_DOLBY_TYPE_IDX]);
	//#endif
	...
	tRPCShareMem->ulAddr[SCALER_VIDEO_LATENCY_IDX] = htonl(tRPCShareMem->ulAddr[SCALER_VIDEO_LATENCY_IDX]);
...
//#if RUN_MERLIN_MEMC_ENABLE
		...
		SetRPCShareMem(HDMI_DOLBY_TYPE_FLAG, sizeof(DOLBY_HDMI_DP_TYPE));
...
//#endif
		...
		SetRPCShareMem(SCALER_VIDEO_LATENCY, sizeof(SCALER_VIDEO_LATENCY_DATA));
...
{
    unsigned int caveout_wid;
    unsigned int caveout_len;
    unsigned int caveout_len_c;

    calculate_carveout_size(SLR_MAIN_DISPLAY);

    caveout_wid = drvif_memory_get_data_align(Get_Val_Max_Width(), 16);
    caveout_len = drvif_memory_get_data_align(Get_Val_Max_Len(), 16);
    caveout_len_c = caveout_len / 2;

    rotate_block_size = (caveout_wid * (caveout_len + caveout_len_c) + FINAL_BURST_LENTH);
...
void scaler_Rotate_VI_init_send_rpc(void)
{
	...
	unsigned int i = 0;
	int ret;
    unsigned int block_size = Get_Val_rotate_block_size();
    unsigned int c_offset =  drvif_memory_get_data_align(Get_Val_Max_Width(), 16) *  drvif_memory_get_data_align(Get_Val_Max_Len(), 16);
    ...
    rotate_data->block_size = block_size;
    rotate_data->buffer_addr_y[ROTATEBUF_IDX_CAP] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_CAP);
    rotate_data->buffer_addr_c[ROTATEBUF_IDX_CAP] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_CAP) + c_offset;
    rotate_data->buffer_addr_y[ROTATEBUF_IDX_DISP_FIRST] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST);
    rotate_data->buffer_addr_c[ROTATEBUF_IDX_DISP_FIRST] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST)+ c_offset;
    rotate_data->buffer_addr_y[ROTATEBUF_IDX_DISP_SEC] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC);
    rotate_data->buffer_addr_c[ROTATEBUF_IDX_DISP_SEC] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC) + c_offset;
    rotate_data->buffer_addr_y[ROTATEBUF_IDX_DISP_THIRD] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD) ;
    rotate_data->buffer_addr_c[ROTATEBUF_IDX_DISP_THIRD] = memory_get_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD) + c_offset;
...
{
	if((get_rotate_mode(display) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_180) && (Scaler_DispGetStatus(display, SLR_DISP_INTERLACE) == FALSE))
		return TRUE;
	else
		return FALSE;
}
...
void modestate_decide_dtg_m_mode(void)
{
	...
	rtd_pr_vbe_emerg("pixel : %d, input framerate = %d, ", Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE), Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ));
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)){
		...
		if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)
			rtd_pr_vbe_notice("[LVDS_TO_HDMI]output=%d \n", Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ));
		else
			rtd_pr_vbe_notice("[VBY1_TO_HDMI]output=%d \n", Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ));
		return;
	}

	if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_24000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_24500 ) )					// 24Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 60 )
		{
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_24500 ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_25000_pOFFSET ) )				// 25Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_30000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_30000_pOFFSET ) )		// 30Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_48000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_48000_pOFFSET ) )		// 48Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_50000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_50000_pOFFSET ) )		// 50Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 100);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_60000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_60000_pOFFSET ) )		// 60Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_100000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_100000_pOFFSET ) )	// 100Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 120 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 100);
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_120000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_120000_pOFFSET ) )	// 120Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 120 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_144000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_144000_pOFFSET ) )	// 144Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 144 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 144);
		else
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_165000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_165000_pOFFSET ) )	// 165Hz
	{
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_200000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_200000_pOFFSET ) )	// 200Hz
	{
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_240000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_240000_pOFFSET ) )	// 240Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 240 )
			Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 240);
		else if( ( ( Get_DISPLAY_REFRESH_RATE() == 120 ) || ( Get_DISPLAY_REFRESH_RATE() == 144 ) ) && ( get_panel_pixel_mode() > PANEL_1_PIXEL ) )
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) >= V_FREQ_288000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ) < V_FREQ_288000_pOFFSET )  )	// 288Hz
	{
...
void modestate_decide_dtg_m_modeByDisp(unsigned char _channel)
{
	...
	rtd_pr_vbe_notice("[%s] => ", __FUNCTION__);
	rtd_pr_vbe_notice("pixel : %d, input framerate = %d, ", Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE), Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ));
	rtd_pr_vbe_emerg("Get_DISPLAY_PANEL_TYPE : %d, ", Get_DISPLAY_PANEL_TYPE());
	...
	if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_24000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_24500 ) )// 24Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 60 )
		{
			if(get_vsc_gamemode_datafrc_mode_flag())
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_24500 ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_25000_pOFFSET ) )				// 25Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_30000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_30000_pOFFSET ) )		// 30Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_48000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_48000_pOFFSET ) )		// 48Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_50000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_50000_pOFFSET ) )		// 50Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 100);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_60000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_60000_pOFFSET ) )		// 60Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() == 120 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else if( Get_DISPLAY_REFRESH_RATE() == 144 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_100000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_100000_pOFFSET ) )		// 100Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 120 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 100);
		else if( Get_DISPLAY_REFRESH_RATE() == 60 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_120000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_120000_pOFFSET ) )		// 120Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 120 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 120);
		else
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_144000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_144000_pOFFSET ) )		// 144Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 144 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 144);
		else
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_165000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_165000_pOFFSET ) )		// 165Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 165 )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_200000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_200000_pOFFSET ) )		// 200Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 240 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 200);
		else if( ( ( Get_DISPLAY_REFRESH_RATE() == 120 ) || ( Get_DISPLAY_REFRESH_RATE() == 144 ) ) && ( get_panel_pixel_mode() > PANEL_1_PIXEL ) )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_240000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_240000_pOFFSET ) )		// 240Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 240 )
			Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 240);
		else if( ( ( Get_DISPLAY_REFRESH_RATE() == 120 ) || ( Get_DISPLAY_REFRESH_RATE() == 144 ) ) && ( get_panel_pixel_mode() > PANEL_1_PIXEL ) )
	...
	else if( ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) >= V_FREQ_288000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_V_FREQ) < V_FREQ_288000_pOFFSET ) )		// 288Hz
	{
		if( Get_DISPLAY_REFRESH_RATE() >= 288 )
	...
	rtd_pr_vbe_notice("output framerate = %d\n", Scaler_DispGetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ));
}
...
unsigned char modestate_get_mdomain_frc_flag(unsigned char _channel)
{
	...
	{
		rtd_pr_vbe_emerg("modestate_get_mdomain_frc_flag return false, source=%x, game mode=%x\n", Get_DisplayMode_Src(_channel), drv_memory_get_game_mode());
			...
			(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_get_mdomain_frc_flag input 24hz, uzu 48hz\n");
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_get_mdomain_frc_flag input 25hz, uzu 50hz\n");
	}
	// input 30hz, uzu 60hz
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 305))	//30Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_get_mdomain_frc_flag input 30hz, uzu 60hz\n");
	...
	rtd_pr_vbe_emerg("modestate_get_mdomain_frc_flag=%x, channel=%d\n", g_ucMdomainFrc[_channel], _channel);
	return g_ucMdomainFrc[_channel];
}
...
unsigned char modestate_check_mdomain_frc_scaler_rerun(unsigned char _channel)
{
	...
	rtd_pr_vbe_emerg("ucPreFlag=%x, ucCurFlag=%x\n", ucPreFlag, ucCurFlag);

	if (ucPreFlag != ucCurFlag)
	{
		rtd_pr_vbe_emerg("modestate_check_mdomain_frc_scaler_rerun=TRUE\n");
		return TRUE;
	}
	else
	{
		rtd_pr_vbe_emerg("modestate_check_mdomain_frc_scaler_rerun=FALSE\n");
		return FALSE;
	}
}
...
{
	rtd_pr_vbe_emerg("modestate_get_mdomain_frc_vfreq=%x, channel=%d\n", g_ucMDomainFrcVFreq[_channel], _channel);
...
unsigned char modestate_decide_mdomain_frc_fractional(unsigned char _channel)
{
	...
	rtd_pr_vbe_emerg("modestate_decide_mdomain_frc_fractional game mode=%x, game mode flag=%x, game_mode_dynamic=%x\n", drv_memory_get_game_mode(), drv_memory_get_game_mode_flag(), drv_memory_get_low_delay_game_mode_dynamic());
	...
	{
		rtd_pr_vbe_emerg("Don't do modestate_decide_mdomain_frc_fractional, source=%x\n", Get_DisplayMode_Src(_channel));
			...
			(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 245))	//24Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_decide_mdomain_frc_fractional input 24hz, uzu 48hz\n");

		// UZU 48hz
		if(Get_DISPLAY_REFRESH_RATE() == 120){
			rtd_pr_vbe_emerg( "2/1, 1pixel \n");
			Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
			Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 2);
		}else{
			if(Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ) == 60){
				rtd_pr_vbe_emerg( "4/5, 1pixel \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 4);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 5);
			}else{
				rtd_pr_vbe_emerg( "1/1, 1pixel \n");
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_REMOVE_RATIO, 1);
				Scaler_DispSetInputInfo(SLR_INPUT_FRAC_FS_MULTIPLE_RATIO, 1);
			}
		}
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 255))	//25Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_decide_mdomain_frc_fractional input 25hz, uzu 50hz\n");

		if(Get_DISPLAY_REFRESH_RATE() == 120){
			if(Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_DTG_MASTER_V_FREQ) == 60){
				rtd_pr_vbe_emerg( "12/5, 2pixel \n");
			  ...
			  (Scaler_DispGetInputInfoByDisp(channel,SLR_INPUT_V_FREQ) < 305))	//30Hz
	{
		...
		rtd_pr_vbe_emerg("modestate_decide_mdomain_frc_fractional input 30hz, uzu 60hz\n");
	...
	rtd_pr_vbe_emerg("modestate_decide_mdomain_frc_fractional g_ucMdomainFrc[_channel]=%x\n", g_ucMdomainFrc[_channel]);
...
//#ifdef CONFIG_MEMC_BYPASS
	if((get_MEMC_bypass_status_refer_platform_model() == TRUE) || (get_bypass_memc_cma_alloc_flag_scaler() == TRUE))
	{
		// don't set uzudtg fractional fs.
		rtd_pr_vbe_info("Bypass memc case [%s %d]!! Don't set uzudtg fractional fs. \n", __FUNCTION__, __LINE__);
		uzudtg_fractional_fsync_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_fractional_fsync_reg);
		uzudtg_fractional_fsync_reg.uzudtg_remove_half_ivs_mode = 0;
		uzudtg_fractional_fsync_reg.uzudtg_multiple_vsync = 0;
		IoReg_Write32(PPOVERLAY_uzudtg_fractional_fsync_reg, uzudtg_fractional_fsync_reg.regValue);
...
unsigned char judge_main_hdmi_vrr_mode(void){
    return ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag()));
}
...
void decide_hdmi_dp_framerate_for_timing_protect(unsigned char display)
{
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI )
    {
        ...
        {//120 panel
                ...
                && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_120_PANEL)
            {
                ...
                {
                    if((Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 60100)   && (!judge_main_hdmi_vrr_mode()))//60.2Hz~99.4Hz //to be compatible with OLED panel
                    {
        ...
        else
        {
                ...
                && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE) <= HDMI_TO_VO_FRC_60_MAX_LENTH_60_PANEL )//60.2Hz~99.4Hz //to be compatible with OLED panel
            {
                ...
                else if((Get_DISPLAY_PANEL_OLED_TYPE() == TRUE) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) > 24010) && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW) <= 24500))//to be compatible with OLED panel, 24.016 case
                {
                    rtd_pr_vsc_info("[OLED] input frame bigger than 24, VO force to 24hz\n");
                    new_framerate = 240;//i domain go 24
                }
    ...
    if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_INTERLACE))
    {
        ...
        new_framerate = ori_framerate;// interlace i domain is real frame rate
    }

	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && GetAiTransFuncInfo(NULL))
	{
        new_framerate = AI_TRANSLATION_FIXED_FRAME_RATE;// AI translati enale force fix frame rate
        rtd_pr_vsc_info("[%s] AI translati enale force fix frame rate(%d)!!\n", __FUNCTION__, new_framerate);
	}

    if((new_framerate != ori_framerate) && !get_scaler_qms_mode_flag())
    {
...
VSC_DRIVER_PATTERN vsc_driver_pattern_table[] ={
	...
	{VO_4K2K_60, 		3, 2, 0, 1, 1, 0, 0},	// will force disable in "fwif_color_DI_IP_ini()" on ml7 32b platform
	{VO_8K4K_30, 		0, 2, 0, 1, 1, 0, 0},
	{VO_8K4K_60, 		0, 2, 0, 1, 1, 0, 0},
	...
	{CP_4K2K_60, 		3, 2, 0, 1, 1, 0, 0},		// will force disable in "fwif_color_DI_IP_ini()" on ml7 32b platform
	{CP_8K4K_30, 		0, 2, 0, 1, 1, 0, 0},
	{CP_8K4K_60, 		0, 2, 0, 1, 1, 0, 0},
...
void copy_current_driver_pattern_for_RTNR(VSC_DRIVER_PATTERN driver_pattern)
{
	INPUT_TIMING_INDEX i = HDMI_2K1K_30;
	for(i = HDMI_2K1K_30; i < TIMING_INVAILD; i++)
	{
		if(driver_pattern.timing_index == i)
		{
			vsc_driver_pattern_table[i].rtnr_mode = driver_pattern.rtnr_mode;
			vsc_driver_pattern_table[i].dipqc_bit = driver_pattern.dipqc_bit;
			rtd_pr_vbe_emerg("match driver pattern[%d],copy RTNR rtnr_mode=%d,dipqc_bit=%d,\n", i, driver_pattern.rtnr_mode, driver_pattern.dipqc_bit);
		}
	}
	if(i == TIMING_INVAILD)
	{
		rtd_pr_vbe_emerg("no match the driver pattern fail for RTNR\n");
	}
	//driver_config_data_to_video();
}
...
INPUT_TIMING_INDEX get_current_pattern_index(void)
{
	...
	else if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)
	{
		...
		if ((index!=TIMING_INVAILD)&&(((Scaler_VOFromVDEC(Scaler_Get_CurVoInfo_plane()) == SOURCE_DIRECTVO) || Get_Force_run_fake_scaler_State()) &&
...
void set_sharememory_hdmi_dolby_vsem(unsigned int type)
{
	...
	DOLBY_HDMI_DP_TYPE *dolby_type = NULL;

	dolby_vsem = (unsigned int *)Scaler_GetShareMemVirAddr(HDMI_DOLBY_VSEM_FLAG);
	if (dolby_vsem)
		*dolby_vsem = ntohl(type);

	dolby_type = (DOLBY_HDMI_DP_TYPE *)Scaler_GetShareMemVirAddr(HDMI_DOLBY_TYPE_FLAG);
	if (dolby_type)
		dolby_type->dolby_vsem = ntohl(type);
}

void set_sharememory_hdmi_dolby_type(unsigned char display, unsigned int type)
{
	DOLBY_HDMI_DP_TYPE *dolby_type = (DOLBY_HDMI_DP_TYPE *)Scaler_GetShareMemVirAddr(HDMI_DOLBY_TYPE_FLAG);

	if (dolby_type == NULL)
		return;

	rtd_pr_vsc_info("[DolbyVision][%s] set display %d dolby type %d -> %d\n", __func__, display, ntohl(dolby_type->main_dolby_type), type);
	if (display == SLR_MAIN_DISPLAY)
		dolby_type->main_dolby_type = htonl(type);
	else
		dolby_type->sub_dolby_type = htonl(type);
}

void reset_sharememory_hdmi_dolby_type(void)
{
	DOLBY_HDMI_DP_TYPE *dolby_type = (DOLBY_HDMI_DP_TYPE *)Scaler_GetShareMemVirAddr(HDMI_DOLBY_TYPE_FLAG);
	...
	dolby_type->dolby_vsem = 0;
	dolby_type->main_dolby_type = 0;
	dolby_type->sub_dolby_type = 0;
}
...
{
	if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) > (Get_DISPLAY_REFRESH_RATE()*10/2 + 10)))
		return TRUE;
	else
		return FALSE;
}
...
unsigned char force_enable_mdomain_2disp_path(void)
{
	...
	if(is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
		return FALSE;
	else if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) &&
        (jude_input_timing_is_hfr(SLR_MAIN_DISPLAY)) && !((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > 2048) && (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) < 1088)) && 
        !(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
        ){
        
        rtd_pr_vbe_emerg("[%s %d] true\n", __FUNCTION__, __LINE__);
		return TRUE;
    }
    else{
        return FALSE;
...
/*
	Some special case in CMA needs to reduce MEMC memory.
	Ex: Youtube film mode with DDR 1.5GB case

	return TRUE  : can reduce MEMC memory size
	return FALSE : can NOT reduce MEMC memory size
*/
unsigned char CMA_MEMC_need_reduce_memory(void)
{
#define DDR_TOTAL_SIZE_1_DOT_5_GB 0x60000000 // DDR 1.5GB (1024*1024*1024*1.5)

	unsigned char ret = FALSE;

	if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
	{
		unsigned long long ddr_total_size = get_memory_size_total();

		rtd_pr_vsc_info("[%s] %d %d src=%d, vdec_source=%d, film_mode=%d, ddr_total_size=0x%llx\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), Get_DisplayMode_Src(SLR_MAIN_DISPLAY), Scaler_VOFromVDEC(VO_VIDEO_PLANE_V1), get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY), ddr_total_size);

		if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC)  // only VDEC source need to consider MEMC reduce or not
			&& ((Scaler_VOFromVDEC(VO_VIDEO_PLANE_V1) == SOURCE_DIRECTVO) && (get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY) == TRUE)) // only Youtube film mode uses direct VO
			&& (ddr_total_size == DDR_TOTAL_SIZE_1_DOT_5_GB)) // only DDR 1.5GB case
		{
			ret = TRUE;
		}
		else
	...
	rtd_pr_vsc_info("[%s] %d %d ret=%d\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), ret);
