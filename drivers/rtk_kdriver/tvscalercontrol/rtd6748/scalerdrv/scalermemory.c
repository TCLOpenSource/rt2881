...
unsigned int	mainBurstLength = 0;//record main burstLen
...
static unsigned int rotate_buffer_addr[ROTATEBUF_IDX_UNKNOWN] = {0};
void memory_set_rotate_buffer_addr(eRotateBufIdx buf_idx, unsigned int addr)
{
    rotate_buffer_addr[buf_idx] = addr;
}

unsigned int memory_get_rotate_buffer_addr(eRotateBufIdx buf_idx)
{
    return rotate_buffer_addr[buf_idx];
}
...
static unsigned long CMA_m_domain_rotate_buffer = 0;
/* get origianl carvedout memory addr from cma */
unsigned long CMA_get_m_domain_rotate_buffer(void)
{
	return CMA_m_domain_rotate_buffer;
}
...
void CMA_release_m_domain_original_buffer(void)
{
    ...
    if (CMA_m_domain_rotate_buffer != 0){
        pli_free(CMA_m_domain_rotate_buffer);
    }
    CMA_m_domain_rotate_buffer = 0;
#endif
...
unsigned char CMA_alloc_m_domain_original_buffer(void)
{
#ifndef UT_flag
	unsigned char CMA_buffer_index = 0;
	unsigned char CMA_DCMT_protect_buffer_number = 0;
	unsigned long CMA_m_domain_buffer = 0;
	dcmt_cma_desc_t desc = {0};
    unsigned int rotate_block_size = Get_Val_rotate_block_size();
    unsigned char rotate_alloc_buffer_flag = FALSE;

	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_DP))
	{
		CMA_buffer_number = CMA_NEED_BUFFER_NUMBER_3;
		CMA_buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_16BITS_4K120_CO_BUFFER;

		Mdomain_buffer_shift_size = MDOMAIN_MAIN_BUFFER_SIZE_16BITS_4K120_CO_BUFFER;
	}
	else
	{
		CMA_buffer_number = CMA_NEED_BUFFER_NUMBER_1;
		CMA_buffer_size = rotate_block_size * 3; // Rotate case. Must use continuous address for 4 blocks. Set_Val_rotate_block_size(rotate_wid * rotate_len * 12 / 8 + 0x40000)

		Mdomain_buffer_shift_size = rotate_block_size;

		if(get_bypass_memc_cma_alloc_flag_scaler() == TRUE)
		{
			CMA_buffer_number = CMA_NEED_BUFFER_NUMBER_1;
			CMA_buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_10BITS_4K * 3; // Bypass memc cma alloc. case. Use Mdomain data FRC

			Mdomain_buffer_shift_size = MDOMAIN_MAIN_BUFFER_SIZE_10BITS_4K;
		}
	}
	...
	CMA_DCMT_protect_buffer_number = CMA_buffer_number;
	...
	for(CMA_buffer_index = 0; CMA_buffer_index < CMA_buffer_number; CMA_buffer_index++)
	{
		...
		if((CMA_m_domain_buffer != 0) && (CMA_get_m_domain_original_size(CMA_buffer_index) != CMA_buffer_size))
		{
			rtd_pr_scaler_memory_info("[%s] buffer #%d size not patch(ori_size:0x%x need_size:0x%x bytes), release all buffer and allocate again.\n", __func__, CMA_buffer_index, CMA_get_m_domain_original_size(CMA_buffer_index), CMA_buffer_size);
			CMA_release_m_domain_original_buffer(); // release all buffer
			CMA_buffer_index = 0; // reset buffer index to default
			CMA_m_domain_buffer = CMA_get_m_domain_original_buffer(CMA_buffer_index); // reset buffer address
		}
		...
		if (CMA_m_domain_buffer == INVALID_VAL)
		{
			...
			CMA_DCMT_protect_buffer_number--;
			return FALSE;
		}
	}

    if (CMA_buffer_size > rotate_block_size * Rotate_Mdomain_Buf_Num){
        rotate_alloc_buffer_flag = FALSE;
    } else {
        rotate_alloc_buffer_flag = TRUE;
    }

#ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
    if (Get_rotate_function(SLR_MAIN_DISPLAY)){
        if (rotate_alloc_buffer_flag){
            if (CMA_m_domain_rotate_buffer == 0){
                CMA_m_domain_rotate_buffer = pli_malloc(rotate_block_size, GFP_DCU2_FIRST);
            }
            
            if (CMA_m_domain_rotate_buffer == INVALID_VAL)
            {
                rtd_pr_scaler_memory_err("###[Bug] func:%s [rotate] no memory from cma ##\n", __func__);
                return FALSE;
            }

            if (CMA_buffer_number == CMA_NEED_BUFFER_NUMBER_3){
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_CAP, CMA_m_domain_rotate_buffer);
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST, CMA_get_m_domain_original_buffer(MEMIDX_CMA_FIRST));
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC, CMA_get_m_domain_original_buffer(MEMIDX_CMA_SEC));
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD, CMA_get_m_domain_original_buffer(MEMIDX_CMA_THIRD));
            } else {
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_CAP, CMA_m_domain_rotate_buffer);
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST, CMA_m_domain_buffer);
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC, CMA_m_domain_buffer + rotate_block_size);
                memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD, CMA_m_domain_buffer + rotate_block_size * 2);
            }
        } else
        {
            memory_set_rotate_buffer_addr(ROTATEBUF_IDX_CAP, CMA_m_domain_buffer + rotate_block_size * 3);
            memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST, CMA_m_domain_buffer);
            memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC, CMA_m_domain_buffer + rotate_block_size);
            memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD, CMA_m_domain_buffer + rotate_block_size * 2);
        }
	...
	for(CMA_buffer_index = 0; CMA_buffer_index < CMA_DCMT_protect_buffer_number; CMA_buffer_index++)
	{
...
#ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
    if (Get_rotate_function(SLR_MAIN_DISPLAY) && rotate_alloc_buffer_flag){
        desc.pa_start[CMA_DCMT_protect_buffer_number] = CMA_m_domain_rotate_buffer;
		desc.pa_size[CMA_DCMT_protect_buffer_number] = rotate_block_size;
		desc.region_cnt++;
    }
#endif
...
void mcap_outstanding_ctrl(unsigned char display, unsigned char enable)
{
	...
	{//sub
		...
		if(mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_max_outstanding != cap_outstanding)
		{
			...
			{
				IoReg_ClearBits(MDOMAIN_CAP_Cap_In2_enable_reg, MDOMAIN_CAP_Cap_In2_enable_in2_cap_enable_mask);
				mdelay(1);
...
unsigned long get_query_start_address(unsigned char idx){
	...
	if(idx == QUERY_IDX_MDOMAIN_MAIN){
		...
		if ((is_m_domain_data_frc_need_borrow_memory() && m_domain_memory_from_type() == BORROW_FROM_MEMC)) {
			...
			if(carvedout_enable() == 1)  // use carvedout memory
			{
				size = (unsigned int)carvedout_buf_query(CARVEDOUT_SCALER_MEMC, (void **)&addr) ;
...
void memory_set_input_format(unsigned char display, unsigned char bDouble)
{
	...
	//IoReg_Write32(display? MDOMAIN_DISP_Disp_sub_enable_reg: MDOMAIN_DISP_Disp_main_enable_reg,  _BIT0);  // enable Mdisp_en in the end of dvrif_memory_handler
#else
	IoReg_Write32(MDOMAIN_DISP_DDR_MainCtrl_reg, mdomain_disp_ddr_mainctrl_reg.regValue);
	//IoReg_Write32(/*display? MDOMAIN_DISP_DDR_SubCtrl_reg : */MDOMAIN_DISP_Disp_main_enable_reg,  _BIT0);  // enable Mdisp_en in the end of dvrif_memory_handler
#endif
	// write data & set double buffer write enable
	if(display == SLR_MAIN_DISPLAY)
	{
...
//	unsigned char lineRepeatEnable = FALSE;
	...
	if(Cap_type == MEMCAPTYPE_LINE){
		...
		if(force_enable_mdomain_2disp_path() || (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
...
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...

		{
			...
			if(get_bypass_memc_cma_alloc_flag_scaler() == TRUE)
			{
				buffer_size = MDOMAIN_MAIN_BUFFER_SIZE_10BITS_4K;//4k2k for bypass memc case
			}
			...
			// 4k60 vrr/game mode
				...
				if(carvedout_enable() == 0)  // do NOT use carvedout memory, use CMA malloc
				{
					buffer_size = Mdomain_buffer_shift_size;
				}
				else  // use carvedout memory
				{
					if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE) && get_vsc_run_pc_mode())
			...
			if((carvedout_enable() == 0) && (drv_memory_Get_multibuffer_flag() == 0) && (query_addr == 0))  // do NOT use carvedout memory, use CMA malloc
			{
				rtd_pr_scaler_memory_err("###[Bug] func:%s %d no memory from cma ##\n", __func__, __LINE__);
				return;
			}
...
#ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
            if (Get_rotate_function(SLR_MAIN_DISPLAY)){
                unsigned int rotate_buffer_size = Get_Val_rotate_block_size();
                if(carvedout_enable() != 0){
                    memory_set_rotate_buffer_addr(ROTATEBUF_IDX_CAP, align_addr + rotate_buffer_size * 3);
                    memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_FIRST, align_addr);
                    memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_SEC, align_addr + rotate_buffer_size);
                    memory_set_rotate_buffer_addr(ROTATEBUF_IDX_DISP_THIRD, align_addr + rotate_buffer_size * 2);
                }
            }
#endif
			...
			if((carvedout_enable() == 0) && (drv_memory_Get_multibuffer_flag() == 0))  // do NOT use carvedout memory, use CMA malloc
			{
				if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_DP))
				{
					unsigned long addr_1 = 0;
					unsigned long addr_2 = 0;
					unsigned long CMA_m_domain_buffer_1;
					unsigned long CMA_m_domain_buffer_2;

					CMA_m_domain_buffer_1 = CMA_get_m_domain_original_buffer(MEMIDX_CMA_SEC);
					CMA_m_domain_buffer_2 = CMA_get_m_domain_original_buffer(MEMIDX_CMA_THIRD);

					if(CMA_m_domain_buffer_1 == INVALID_VAL)
					{
						addr_1 = 0;
						rtd_pr_scaler_memory_err("###[Bug] func:%s %d no memory from cma ##\n", __func__, __LINE__);
						return;
					...
					{
						addr_1 = CMA_m_domain_buffer_1;
						addr_1 = dvr_memory_alignment(addr_1, buffer_size);
					}

					if(CMA_m_domain_buffer_2 == INVALID_VAL)
					{
						addr_2 = 0;
						rtd_pr_scaler_memory_err("###[Bug] func:%s %d no memory from cma ##\n", __func__, __LINE__);
						return;
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		drvif_memory_free_block(MEMIDX_SUB_THIRD);	// free block befor getting a new one
		//sub use cap max size @Crixus 20151231
		if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) || force_enable_mdomain_2disp_path())
		{
...
void dvrif_memory_handler(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		{//run sub mdomain for two pixel mode only for 120 panel
			mdomain_disp_ddr_subprevstart_RBUS mdomain_disp_ddr_subprevstart_Reg;
			mdomain_disp_ddr_mainprevstart_RBUS ddr_mainprevstart_reg;
			rtd_pr_scaler_memory_emerg("[%s %d] 4k120\n", __FUNCTION__, __LINE__);
			if(dvrif_memory_compression_get_enable(SLR_SUB_DISPLAY) == TRUE)
			{
				if(dvrif_memory_get_compression_mode(display) == COMPRESSION_LINE_MODE)
				{
			...
			mdomain_disp_ddr_subprevstart_RBUS mdomain_disp_ddr_subprevstart_Reg;
			mdomain_disp_ddr_mainprevstart_RBUS ddr_mainprevstart_reg;
			rtd_pr_scaler_memory_emerg("[%s %d] 2disp_path\n", __FUNCTION__, __LINE__);
			memory_set_sub_for_pip(SLR_SUB_DISPLAY);
			//limit boundary enable @Crixus 20180320
			memory_set_input_format(SLR_SUB_DISPLAY, TRUE);
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(drv_memory_get_vdec_direct_low_latency_mode() && (get_vsc_run_adaptive_stream(SLR_MAIN_DISPLAY) == ADAPTIVE_STREAM_ON) && !((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)))
			drv_memory_SingleBuffer_GameMode(_ENABLE);
//remove in k4lp
#if 0
		else{
	...
	// Just condiser input slow case now, input fast case needs to fix in the future.
...
void dvrif_memory_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if(Get_PANEL_VFLIP_ENABLE() || force_enable_mdomain_2disp_path())
...
#define UZU_LINE_BUF_WID 3840
#define UZU_LINE_BUF_NUM 5
...
//	ppoverlay_fs_iv_dv_fine_tuning5_RBUS fs_iv_dv_fine_tuning5_reg;
	...
	if((get_panel_pixel_mode() > PANEL_1_PIXEL)&& (PPOVERLAY_uzudtg_control1_get_uzu_4k_120_mode(rtd_inl(PPOVERLAY_uzudtg_control1_reg)))){
...
*/
    /* Formula
    total_pixels = (uzu_line_buffer_width*buffer_number) * d_hact / (d_htotal-d_hact) + (uzu_line_buffer_width*buffer_number) + (wtlvl_128bit + burst_lenght_128bit)*128/pqc_bit_per_pixel;
    iv2dv = total_pixels*uzd_in_vact/uzd_out_hact/uzd_out_vact
    */
    //Fill up the Uzu buffer * 5
    //if(Scaler_DispGetScaleStatus(SLR_MAIN_DISPLAY, SLR_SCALE_H_DOWN))
    {
        temp1 = UZU_LINE_BUF_WID*UZU_LINE_BUF_NUM * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_WID) / (PPOVERLAY_DH_Total_Last_Line_Length_get_dh_total(IoReg_Read32(PPOVERLAY_DH_Total_Last_Line_Length_reg)) + 1 - Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_WID));
        temp2 = UZU_LINE_BUF_WID*UZU_LINE_BUF_NUM + (MDOMAIN_DISP_DDR_MainNumWTL_get_main_wtlvl(IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg)) + mainBurstLength)*128 / dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY);
        iv2dv_delay2 = (temp1 + temp2) * Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_SDNRV_ACT_LEN) / Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_CAP_WID) / Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_CAP_LEN);
        rtd_pr_scaler_memory_emerg("hsd iv2dv delay = %d\n", iv2dv_delay2);
        iv2dv_delay = ((iv2dv_delay>iv2dv_delay2)?iv2dv_delay:iv2dv_delay2);
    }
...
#else
		if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())){
			return;
		}
...
void drv_memory_display_set_sub_frc_style(void)
{
	...
	if (!get_sub_OutputVencMode()) {//sub m domain iterrupt open so don't need wait lastwrite @qing_liu --s4ap
		...
		if(!force_enable_mdomain_2disp_path() && ((get_hdmi_4k_hfr_mode() == HDMI_NON_4K120) || (get_hdmi_4k_hfr_mode() == HDMI_3K120))&&
...
unsigned int drv_Calculate_m_pre_read_value(void)
{
	...
	if(get_HD_FHD_platform_model())
		m_pre_read = DVStar - timeborrow - _M_PRE_READ_MARGIN_FHD;
	else
...
void drvif_memory_wait_dbuffer_apply(unsigned char display)
{
	int apply_timeout_cnt = 10;
	mdomain_disp_ddr_mainsubctrl_RBUS ddr_mainsubctrl_reg;
	ddr_mainsubctrl_reg.regValue =IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(ddr_mainsubctrl_reg.disp2_double_enable) {
			while(apply_timeout_cnt-- && (ddr_mainsubctrl_reg.disp2_double_apply))
			{
				ddr_mainsubctrl_reg.regValue =IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
				msleep(10);
			}
			if(!apply_timeout_cnt)
				rtd_pr_scaler_memory_err("[%s] %d using sub db to disable wait timeout!!\n", __FUNCTION__, __LINE__);
			else
				rtd_pr_scaler_memory_info("[%s] %d using sub db to disable\n", __FUNCTION__, __LINE__);
		}
	}
}
...
void drvif_memory_EnableMDisp_Or_DisableMDisp(unsigned char display, bool enable)
{
	...
	if(enable == _ENABLE)
	{
		//enable main Mdisp double buffer
		if(display == SLR_MAIN_DISPLAY)
			IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp1_double_enable_mask);
		...
		//apply main Mdisp double buffer
		if(display == SLR_MAIN_DISPLAY) {
			IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp1_double_apply_mask);
		}
	}
	else
	{
		if(display == SLR_MAIN_DISPLAY)
		{
			...
			//wait double buffer apply
			IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp1_double_apply_mask);//drvif_memory_set_dbuffer_write(SLR_MAIN_DISPLAY);
		...
		else
		{
			...
			//wait double buffer apply
			IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg, MDOMAIN_DISP_DDR_MainSubCtrl_disp2_double_apply_mask);//drvif_memory_set_dbuffer_write(SLR_SUB_DISPLAY);
		}
	}
#else
	if(enable == _ENABLE)
