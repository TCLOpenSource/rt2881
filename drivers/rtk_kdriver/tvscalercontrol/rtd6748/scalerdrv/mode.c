...
#define get_hdmi_onms_mask_flag(x)	((x>>30)&0x3)
#define get_hdmi_onms_mask_bits(x)	(x&0x3fffffff)
...
unsigned int get_result_with_hdmi_mask(unsigned int original_value)
{
	...
	unsigned int get_hdmi_mask_bits;

	is_need_mask=get_hdmi_onms_mask_flag(check_hdmi_mask);
	get_hdmi_mask_bits=get_hdmi_onms_mask_bits(check_hdmi_mask);
	if(is_need_mask==1)//add 
		final_result=original_value|get_hdmi_mask_bits;
	else if(is_need_mask==2)//del 
		{
		final_result = original_value -get_hdmi_mask_bits& original_value; 
		}

	if(original_value != final_result)
	{
		rtd_pr_hdmi_info("[online] Original onms_en:%08x, is_need_mask:%08x, final onms_en value:%08x\n", original_value, is_need_mask, final_result);
	}
	return final_result;
}
...
void drvif_hdmi_onlinemeasure_watchdog_early_enable_setting(void)
{
	...
	// only enable main online measure watchdog first
	drvif_mode_onlinemeasure_setting(_CHANNEL1, _SRC_HDMI, _ENABLE, _DISABLE);
}
...
void drvif_hdmi_dma_onlinemeasure_watchdog_early_enable_setting(void)
{
	...
	// only enable DMA online measure watchdog first
	drvif_mode_dma_onlinemeasure_setting(_ENABLE, _DISABLE);
}
		...
		if(_CHANNEL1 == channel)
		{
			...
			if (src == _SRC_HDMI) {
				{// Only for HDMI source, check particular device or debugging command
					...
					{
						rtd_pr_onms_info("[%s][HDMI] channel: %d, Update onms wdg from %x to %x\n", __FUNCTION__, channel, original_value,  onms_onms1_watchdog_en_reg.regValue);
					}
				}
			}
...
{//This function is to set onlinemeasure watchdog and interrupt setting
	...
	if ((src == VSC_INPUTSRC_HDMI)&&(wdgenable || interruptenable)) {
		...
		rtd_pr_onms_info("[%s][HDMI]  Update current_hdmi_onms_watchdog_setting=%x, current_hdmi_onms_is_i3ddma=%x\n", __FUNCTION__, current_hdmi_onms_watchdog_setting,  current_hdmi_onms_is_i3ddma);
			...
			{//hdmi
...
#endif
				{
					...
					if(Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ)) {
						...
						onms_onms2_demode_vs_of_reg.on2_de_vsper_of_th = ((unsigned int)CONFIG_ONLINE_XTAL_FREQ*(unsigned int)11)/600;
					}
...
{
#ifdef CONFIG_DUAL_CHANNEL
	drvif_memory_DisableMCap(display);//frank@0203 disable capture
...
#ifndef UT_flag
	reset_scaler_video_latency_data(display);
#endif
...
*/
	...
	if(SLR_MAIN_DISPLAY == display)
		Scaler_color_Set_PQ_ByPass_Lv(255, 0);
...
void scaler_mdom_stop(void)
{
    ...
    rtd_pr_vsc_emerg("scaler_mdom_stop!\n");
