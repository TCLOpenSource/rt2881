...
void lib_hdmi_stb_wakeup_by_tmds_init(unsigned char nport, unsigned char en)
{
    if(en)
    {
        hdmi_out(HDMI_STB_GDI_TMDSCLK_SETTING_00_reg, 0x10081); //Set TMDS 128 count (129-1)
...
// Packet Variation
//------------------------------------------------------------------
unsigned int lib_hdmi_get_video_packet_variation_irq(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
        return hdmi_in(HD21_HDMI_PAMICR_reg);
    else
        return hdmi_in(HDMI_HDMI_PAMICR_reg);
}
...
void lib_hdmi_clr_video_packet_variation_irq(unsigned char nport)
{
    if (hdmi_rx[nport].hdmi_2p1_en)
        hdmi_mask(HD21_HDMI_PAMICR_reg, ~HDMI21_P0_HD21_HDMI_PAMICR_icpvsb_mask, 0);
    else
        hdmi_mask(HDMI_HDMI_PAMICR_reg, ~HDMI_HDMI_PAMICR_icpvsb_mask, 0);
}

unsigned char lib_hdmi_get_video_dvs_packet_status(unsigned char nport)
{
    if(hdmi_rx[nport].hdmi_2p1_en)
        return HDMI21_P0_HD21_HDMI_GPVS_get_dvsps_v(hdmi_in(HD21_HDMI_GPVS_reg));
    else
        return HDMI_P0_HDMI_GPVS_get_dvsps_v(hdmi_in(HDMI_HDMI_GPVS_reg));
}

void lib_hdmi_clr_video_packet_status(unsigned char nport)
{
    if(hdmi_rx[nport].hdmi_2p1_en)
        hdmi_out(HD21_HDMI_GPVS_reg, hdmi_in(HD21_HDMI_GPVS_reg));
    else
        hdmi_out(HDMI_HDMI_GPVS_reg, hdmi_in(HDMI_HDMI_GPVS_reg));
}
...
void lib_hdmi_cts_fifo_clk_reset(unsigned char nport)
{
    switch(nport){
        case 1: // only reset port1,other port test OK now  20250122
            hdmi_mask(HDMI_EX_P0_EX_PHY_FIFO_CR0_reg,
                ~(HDMI_EX_P0_EX_PHY_FIFO_CR0_port1_b_afifo_en_mask|HDMI_EX_P0_EX_PHY_FIFO_CR0_port1_g_afifo_en_mask|HDMI_EX_P0_EX_PHY_FIFO_CR0_port1_r_afifo_en_mask), 0);    // Port1
...
int newbase_hdmi_set_dpm_enable(unsigned int nport, int enable)
{
    ...
    else
    {
        ...
        {
            hdmi_mask(HDMI_STB_CLKDETSR_reg, ~(_BIT6), _BIT6); //[6]Clock detect enable
            CLK_Wake_Up |= (unsigned char)(1 << nport);
        }
        else
            CLK_Wake_Up &= (unsigned char)(~(1 << nport));
    }

    HDMI_INFO("%s [port:%u] CLK_Wake_Up = 0x%x, ret =%d\n", __func__, nport, CLK_Wake_Up, ret);
    return ret;
}
