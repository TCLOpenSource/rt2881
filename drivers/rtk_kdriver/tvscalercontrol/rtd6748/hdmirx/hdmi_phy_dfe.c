...
unsigned char  bLan0_LE,bLan1_LE,bLan2_LE;
	...
	if (lane_mode == HDMI_3LANE) {
		...
		if((max_tap0-min_tap0 >= 16)&&((b_clk > 1300)&&(b_clk <1500))&&(lib_hdmi_dfe_get_tap0(nport, 2)>=0x20)&&(nport==3))
		 {
			bLan0_LE = lib_hdmi_dfe_get_le(nport, 0);
			bLan1_LE = lib_hdmi_dfe_get_le(nport, 1);
			bLan2_LE = lib_hdmi_dfe_get_le(nport, 2);
		    HDMI_PRINTF ("%s(%d)-(dfe bLan0_LE: 0x%x;  bLan1_LE: 0x%x;  bLan2_LE: 0x%x)\n",  __func__, nport,bLan0_LE,bLan1_LE,bLan2_LE);
			if(bLan2_LE > 0)
			{
			    bLan2_LE = 0;
				lib_hdmi_dfe_update_le(nport, bLan0_LE, bLan1_LE, bLan2_LE);  // update LE paramete
				HDMI_PRINTF ("%s(%d)-(dfe bLan2_LE force to 0x%x)\n",  __func__, nport,lib_hdmi_dfe_get_le(nport, 2));
			}
		  }
		...
		if ( (max_tap0-min_tap0 > 10) && ((b_clk > 2000) && (b_clk <2200)) ) {
			...
			if(lib_hdmi_dfe_get_le(nport, 2) < 4){
				if(0)			//TCL2875P-5978 , set LE2=0x6 will no signal, here do nothing will ok
				{
				...
				HDMI_EMG("[RELE_L2] NO LE reload 0x6 , maybe no signal\n");
			}
		}
	}
