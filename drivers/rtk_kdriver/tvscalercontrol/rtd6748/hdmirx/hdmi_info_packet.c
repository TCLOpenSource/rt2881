...
static INFO_PACKET_CONFIG info_packet_config_array[INFO_TYPE_NUM] =
{
    ...
    {INFO_TYPE_RSV1, TRUE, TYPE_CODE_VS_PACKET, MAIN_FSM_HDMI_MEASURE, NO_RSV1_INFO_MAX}, //Reserved
...
INFOFRAME_READ_RESULT newbase_hdmi_update_dvs_info(unsigned char port, unsigned char *pkt_buf, INFO_PACKET_CONFIG *config)
{
    ...
    else
    {
        ...
        {
            if(get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_STD || get_HDMI_Dolby_VSIF_mode() == DOLBY_HDMI_VSIF_LL)
            {
...
INFOFRAME_READ_RESULT newbase_hdmi_update_spd_info(unsigned char port, unsigned char *pkt_buf, INFO_PACKET_CONFIG *config)
{
    ...
    if ((result = lib_hdmi_read_spd_info_packet(port, pkt_buf, &bch_error)) == INFOFRAME_READ_SUCCESS)
    {
        ...
        if (bch_error || p_spd->ver > 3 || p_spd->len > 28)  // check version and length (MSPG sometimes send incorrect SPD info)
        {
...
INFOFRAME_READ_RESULT newbase_hdmi_update_RSV1_info(unsigned char port, unsigned char *pkt_buf, INFO_PACKET_CONFIG *config)
{
    INFOFRAME_READ_RESULT result = 0;
    unsigned int index = 0;

    lib_hdmi_set_rsv_packet_type(port, 1, config->type_code);
    if ((result = lib_hdmi_read_reserved_packet(port, 1, pkt_buf, 31)) == INFOFRAME_READ_SUCCESS)
    {
        unsigned int ieeeoui_type = 0;

        ieeeoui_type = (pkt_buf[3]) | (pkt_buf[4]<<8) | (pkt_buf[5]<<16);
        //For xperi
        if(ieeeoui_type ==IEEE_OUI_XPERI)
        {
            hdmi_rx[port].xperi_vsi_t.VSIF_TypeCode  = TYPE_CODE_VS_PACKET;
            hdmi_rx[port].xperi_vsi_t.VSIF_Version = pkt_buf[1];
            hdmi_rx[port].xperi_vsi_t.Length = pkt_buf[2];
            hdmi_rx[port].xperi_vsi_t.Checksum = 0;
            index = 3;
            memcpy(&hdmi_rx[port].xperi_vsi_t.Reg_ID, pkt_buf+index, HAL_VFE_HDMI_VENDOR_SPECIFIC_REGID_LEN);
            index += HAL_VFE_HDMI_VENDOR_SPECIFIC_REGID_LEN;
            memcpy(&hdmi_rx[port].xperi_vsi_t.Payload, pkt_buf+index, \
                HAL_VFE_HDMI_VENDOR_SPECIFIC_PAYLOAD_LEN);
        }
    ...
    {
        if (++hdmi_rx[port].no_rsv1_cnt > NO_RSV1_INFO_MAX)
        {
            memset(&hdmi_rx[port].xperi_vsi_t, 0, sizeof(HDMI_VSI_T));
            hdmi_rx[port].no_rsv1_cnt = 0;
        }
...
void newbase_hdmi_update_infoframe(unsigned char port)
{
    ...
    unsigned char is_xperi_vsi_here = hdmi_rx[port].xperi_vsi_t.VSIF_TypeCode;
    ...
    for(i = 0; i < INFO_TYPE_NUM; i++)
    {
        if(TRUE == newbase_hdmi_check_infoframe_config(port, info_packet_config_array[i].type_name))
        {
            ...
            case INFO_TYPE_RSV0:     // For Audio Content Protection Packet (ACP)
                ...
                break;
            case INFO_TYPE_RSV1:   // Currently for Xperi
                curr_result = newbase_hdmi_update_RSV1_info(port, pkt_buf, info_packet_config_array+i);
                if (curr_result > 0) total_result |= curr_result;
                break;
    ...
    if(hdmi_rx[port].vsi_t.VSIF_TypeCode==0 &&hdmi_rx[port].dvsi_t.VSIF_TypeCode==0 )
    {
    ...
    if(g_infoframe_xperi_vsi_debug_print&&(port==newbase_hdmi_get_current_display_port())) is_xperi_vsi_here++;
    ...
    {
        HDMI_WARN("GET VSI INFO=%x, pkt_cnt=%d, VIC=%d, pb4=%d, pb5=%d, VER=%x, LEN=%x, OUI=(%x,%x,%x)\n",
            hdmi_rx[port].vsi_t.VSIF_TypeCode, hdmi_rx[port].pkt_cnt, newbase_hdmi_get_vic_from_vsi(hdmi_rx[port].vsi_t.Reg_ID, hdmi_rx[port].vsi_t.Payload[0], hdmi_rx[port].vsi_t.Payload[1]), hdmi_rx[port].vsi_t.Payload[0], hdmi_rx[port].vsi_t.Payload[1], hdmi_rx[port].vsi_t.VSIF_Version, hdmi_rx[port].vsi_t.Length, hdmi_rx[port].vsi_t.Reg_ID[0], hdmi_rx[port].vsi_t.Reg_ID[1], hdmi_rx[port].vsi_t.Reg_ID[2]);
    ...
    if (is_xperi_vsi_here != hdmi_rx[port].xperi_vsi_t.VSIF_TypeCode)
    {
        HDMI_WARN("GET Xperi VSI INFO=%x, pkt_cnt=%d, VER=%x, LEN=%x, OUI=(%x,%x,%x), payload(%x,%x)\n",
            hdmi_rx[port].xperi_vsi_t.VSIF_TypeCode, hdmi_rx[port].pkt_cnt, \
            hdmi_rx[port].xperi_vsi_t.VSIF_Version, hdmi_rx[port].xperi_vsi_t.Length, \
            hdmi_rx[port].xperi_vsi_t.Reg_ID[0], hdmi_rx[port].xperi_vsi_t.Reg_ID[1], \
            hdmi_rx[port].xperi_vsi_t.Reg_ID[2], hdmi_rx[port].xperi_vsi_t.Payload[0], \
            hdmi_rx[port].xperi_vsi_t.Payload[1]);
    }
...
/*------------------------------------------------------------------
 * Func : newbase_hdmi_get_xperi_infoframe
 *
 * Desc : get xperi VSIF from a given hdmi port
 *
 * Para : [IN] port  : HDMI port
 *        [IN] p_vsi : VSIF output 
 *
 * Retn : 0 : no xperi VSIF received
 *        1 : xperi VSIF received
 *------------------------------------------------------------------*/
unsigned char newbase_hdmi_get_xperi_infoframe(unsigned char port, HDMI_VSI_T *p_vsi_t)
{
    if (FALSE == newbase_hdmi_check_infoframe_config(port, INFO_TYPE_RSV1))
        return 0;

    if (hdmi_rx[port].xperi_vsi_t.VSIF_TypeCode == 0)
        return 0;

    if (lib_hdmi_get_fw_debug_bit(DEBUG_21_FORCE_NO_INFOPKT))
        return 0;

    memcpy(p_vsi_t, &hdmi_rx[port].xperi_vsi_t, sizeof(HDMI_VSI_T));
    return 1;
}
    ...
    )
{
    ...
    if (n_vsi && newbase_hdmi_get_xperi_infoframe(port, p_vsi)) {
        p_vsi++;
        vsi_cnt++;
        n_vsi--;
    }
...
unsigned char newbase_hdmi_get_avi_infoframe(unsigned char port, HDMI_AVI_T *p_avi_t)
{
    ...
    {
        if(TRUE == newbase_hdmi_is_run_dsc(port))
        {
            if(TRUE == newbase_hdmi_get_convert_rgb(port))
                hdmi_rx[port].avi_t.Y = COLOR_RGB;
            else if((1 == newbase_hdmi_get_native_422(port)) && (1 != newbase_hdmi_get_version_minor(port))
                && (0 == newbase_hdmi_get_simple_422(port)) && (0 == newbase_hdmi_get_native_420(port)))
            {
                hdmi_rx[port].avi_t.Y = COLOR_YUV422;
            }
            else if((0 == newbase_hdmi_get_native_422(port)) && (1 == newbase_hdmi_get_simple_422(port))
                && (0 == newbase_hdmi_get_native_420(port)))
            {
                hdmi_rx[port].avi_t.Y = COLOR_YUV422;
            }
            else if((0 == newbase_hdmi_get_native_422(port)) && (1 != newbase_hdmi_get_version_minor(port))
                && (0 == newbase_hdmi_get_simple_422(port)) && (1 == newbase_hdmi_get_native_420(port)))
            {
                hdmi_rx[port].avi_t.Y = COLOR_YUV420;
            }
            else if((0 == newbase_hdmi_get_native_422(port)) && (0 == newbase_hdmi_get_native_420(port))
                && (0 == newbase_hdmi_get_simple_422(port)))
            {
                hdmi_rx[port].avi_t.Y = COLOR_YUV444;
            }
        }
    }
...
FREESYNC_INFO_VERSION_TYPE newbase_hdmi_check_freesync_info_version(HDMI_SPD_T *p_spd_t)
{
    ...
    else if (p_spd_t->ver==3 && (p_spd_t->len == 0xF||p_spd_t->len == 0x10) && oui==IEEE_OUI_AMD_VSDB)
        return FREESYNC_INFO_VERSION3;
    else
        return FREESYNC_INFO_INVALID;
}
...
unsigned char newbase_hdmi_get_freesync_info(unsigned char port, HDMI_AMD_FREE_SYNC_INFO* p_info)
{
    ...
    if (newbase_hdmi_get_spd_infoframe(port, &spd))
    {
        ...
        else if (newbase_hdmi_check_freesync_info_version(&spd) == FREESYNC_INFO_VERSION3)
        {
            p_info->version = 3;
            p_info->freesync_supported        = (p_pb[5])    & 0x1;
            p_info->freesync_enabled          = (p_pb[5]>>1) & 0x1;
            ...
            p_info->native_color_space_active = (p_pb[5]>>3) & 0x1;
            p_info->local_diming_disalbe      = (p_pb[5]>>5) & 0x1;
            p_info->frame_type                = (p_pb[5]>>6) & 0x3;
            p_info->min_refresh_rate          = p_pb[6] | ((p_pb2[2] & 0x3) << 8);
            p_info->max_refresh_rate          = p_pb[7] | ((p_pb2[3] & 0x3) << 8);
            p_info->gamma_2p2_eotf_active     = (p_pb2[0]>>2) & 0x1;
            p_info->pq_eotf_active            = (p_pb2[0]>>5) & 0x1;
            p_info->fast_tranport_active      = (p_pb2[0]>>7) & 0x1;
            p_info->target_output_pixel_rate  = p_pb2[4];
            p_info->target_output_pixel_rate |= p_pb2[5] << 8;
            p_info->target_output_pixel_rate |= p_pb2[6] << 16;
            p_info->fixed_rate_content_active = p_pb2[7] & 0x1;
...
unsigned char newbase_hdmi_get_vsem_is_dolby(unsigned char port)
{
...
unsigned char newbase_hdmi_get_vic_from_vsi(unsigned char pkt_buf[HAL_VFE_HDMI_VENDOR_SPECIFIC_REGID_LEN], unsigned char pb4, unsigned char pb5)
{
    unsigned int ieeeoui_type = 0;

    ieeeoui_type = pkt_buf[0] | (pkt_buf[1]<<8) | (pkt_buf[2]<<16);
    if ((ieeeoui_type == IEEE_OUI_HDMI_VSDB) && pb4 == 0x20) //Video formats defined in table 8.14 of HDMI spec v1.4b
    {
        return _hdmi_vic_to_vic(pb5);
    } else {
        return 0;
    }
}

unsigned char newbase_hdmi_get_vic_state(unsigned char port, HDMI_AVI_T *p_avi_t, HDMI_VSI_T *p_vsi_t, unsigned char *p_vic)
{
    unsigned char avi_state = 0;
    unsigned char vsi_state = 0;
    unsigned char vsi_vic = 0;

    memset(p_avi_t, 0, sizeof(HDMI_AVI_T)); //init avi_info
    memset(p_vsi_t, 0, sizeof(HDMI_VSI_T)); //init vsi_info

    avi_state = newbase_hdmi_get_avi_infoframe(port, p_avi_t);
    vsi_state = newbase_hdmi_get_vs_infoframe(port, p_vsi_t);

    if(vsi_state == 1)
        vsi_vic = newbase_hdmi_get_vic_from_vsi(p_vsi_t->Reg_ID, p_vsi_t->Payload[0], p_vsi_t->Payload[1]);
