...
HDMI_bool drvif_Hdmi_GetRawTimingInfo(HDMI_TIMING_T* timing_info)
{
    ...
    return newbase_hdmi_get_timing(port, timing_info);
}
...
unsigned char drvif_hdmi_get_current_display_port(void)
{
    return newbase_hdmi_get_current_display_port();
}

unsigned char drvif_Hdmi_get_dolby_vsem_infoframe(unsigned char port, unsigned char *p_vsem)
{
    if(port>=HDMI_PORT_TOTAL_NUM)
        return 0;

    return newbase_hdmi_get_dolby_vsem_infoframe(port, p_vsem);
}

unsigned int drvif_hdmi_get_no_dolby_vsem_emp_cnt(unsigned char port)
{
    if(port >= HDMI_PORT_TOTAL_NUM)
        return 0;
    return hdmi_rx[port].no_dolby_vsem_emp_cnt;
}
