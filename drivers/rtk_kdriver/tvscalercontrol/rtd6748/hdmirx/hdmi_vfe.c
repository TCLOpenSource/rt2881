...
static unsigned char check_panel_is_2k = 0;
...
#define PANEL_NO_SUPPORT_4K_TIMGING	0
...
int vfe_hdmi_drv_init(void)
{
	...
	newbase_hdmi_spin_lock_init();
	newbase_hdmi_scdc_spin_lock_init();
...
{    
    ...
    if((tm.h_act_len == 640)&&(tm.v_act_len == 480)&&(HDMI_ABS(tm.v_freq, 72000) <1000))// tcl 22293 timingID 005
    {
        tm.v_freq=72000;
    }

#if PANEL_NO_SUPPORT_4K_TIMGING
		if((tm.h_act_len>3000 || tm.v_act_len>2000)&& check_panel_is_2k==1)
			return HDMI_DRV_NO_ERR;
#endif
	...
	timing_info->isDolbyHDR = is_dolby_hdr(ch);
#else
	...
	else if (newbase_hdmi_get_vrr_enable(port)) {
    ...
    )
{
    ...
    case VFE_HDMI_PHY_FRL_STATUS :
    {
        ...
        *param = (unsigned int)support;
        HDMI_INFO("[%s] frl_support ==%d",__FUNCTION__,*param);
        break;
    }
    default:
        return -1;
...
int vfe_hdmi_drv_get_AMDFreesyncInfo(VFE_HDMI_AMD_FREE_SYNC_INFO* p_info)
{
    ...
    {
        HDMI_EMG("[WARNING] zxc_NOT_AMDFreesync_info !!!\n");
    }
...
/**
 * @brief Get base and real-time frame rate with an accuracy of 0.01
 */
int vfe_hdmi_drv_get_GetCurrentPrescionFrameRate(unsigned char ch, unsigned int* baseFrameRate, unsigned int* currentVRRFrameRate)
{
	unsigned char port = 0x0F;
	HDMI_TIMING_T tm;

	if (g_hdmi_drv_state != VFE_HDMI_DRV_SERVICE_STATE) {
		HDMI_EMG("[%s][- STATE ERR] fsm = %d\n", __func__, g_hdmi_drv_state);
		return HDMI_DRV_STATE_ERR;
	}

	*baseFrameRate = *currentVRRFrameRate = 0;
	if(HAL_VFE_HDMI_CURRENT_PORT== ch)
	{//Detect non-specified port, force use current port.
		if (hdmi_vfe_get_connected_port(&port)<0)
		{
			HDMI_EMG("[WARNING1] %s can't get current port\n", __func__);
			return HDMI_DRV_ARG_ERR;
		}
	}
	else if (hdmi_vfe_get_hdmi_port(ch, &port)<0)
	...
	*baseFrameRate = tm.v_freq/10;

	if (newbase_hdmi_get_freesync_enable(port) ||newbase_hdmi_get_vrr_enable(port) ||newbase_hdmi_get_qms_enable(port))
	{
		*currentVRRFrameRate = newbase_hdmi_get_real_framerate(port);
	}
...
void vfe_hdmi_drv_shutdown(void)
{
	unsigned char ch;
	unsigned char port;
	...
	if (g_hdmi_drv_state == VFE_HDMI_DRV_SERVICE_STATE)
	{
		...
		HDMI_PRINTF("vfe_hdmi_drv_shutdown : channel :%d\n",g_resume_param.channel);

		newbase_hdmi_uninit();
	}
}

void vfe_hdmi_check_panel_is_2k(void)
{
	unsigned int panel_refresh_rate;
	unsigned int panel_is_dynamic_DisplayTiming;
	unsigned int panel_vertical_total;
	unsigned int panel_horizontal_total;

	check_panel_is_2k=0;

	panel_refresh_rate= GET_PANEL_REFRESH_RATE();
	panel_is_dynamic_DisplayTiming=GET_PANEL_Is_Dynamic_DisplayTiming();
	panel_vertical_total= GET_PANEL_VERTICAL_TOTAL();
	panel_horizontal_total= GET_PANEL_HORIZONTAL_TOTAL();
	HDMI_INFO(" vfe_hdmi_check_panel_is_2k==>htotal=%d,vtotal=%d,rate=%d,dynamic_DisplayTiming=%d",panel_horizontal_total,panel_vertical_total,panel_refresh_rate,panel_is_dynamic_DisplayTiming);
	if(panel_is_dynamic_DisplayTiming==1)
		return ;
	if(panel_horizontal_total <1000 || panel_vertical_total==0 )
		return ;

	if(panel_horizontal_total <3000 && panel_vertical_total<2000 &&panel_refresh_rate <75)
		check_panel_is_2k=1;
