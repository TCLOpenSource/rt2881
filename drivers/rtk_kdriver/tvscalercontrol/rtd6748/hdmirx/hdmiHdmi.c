...
#define MUTE_AV_REASON_PACKET_VAR   (0x1<<2)
static int _hdmi_interrupt_handler(unsigned char port)
{
	...
	if (lib_hdmi_hd21_is_irq_pending(port))
		lib_hdmi_hd21_clr_irq_pending(port);
	//----------------------------------------------
	// DVI/HDMI Mode
	//----------------------------------------------
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
	{
		if (port == newbase_hdmi_get_current_display_port() && !hdmi_rx[port].hdmi_2p1_en)
		{
			if (lib_hdmi_get_hdirq_en(port)) {
				...
				if(lib_hdmi_is_irq_pending(port)||lib_hdmi_hd21_is_irq_pending(port)){
	...
	//----------------------------------------------
	// Dolby packet varition detector
	//----------------------------------------------
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_TIMING_MEASURE, HDMI_FLOW_CFG7_PACKET_VARIATION_IRQ_ENABLE))
	{
		if (port == newbase_hdmi_get_current_display_port())
		{
			if(lib_hdmi_get_video_dvs_packet_status(port)){
				mute_av |= MUTE_AV_REASON_PACKET_VAR;
				lib_hdmi_clr_video_packet_status(port);
				force_dvi_hdmi_mode_chg = 0;
			}
		}
	}
	...
	if (mute_av || force_dvi_hdmi_mode_chg)
	{
		...
		if(lib_hdmi_get_video_packet_variation_irq(port))
			lib_hdmi_clr_video_packet_variation_irq(port);
		...
		else if ((mute_av & MUTE_AV_REASON_PACKET_VAR))
		{
			if (GET_H_VIDEO_FSM(port) > MAIN_FSM_HDMI_SETUP_VEDIO_PLL)
				SET_HDMI_DETECT_EVENT(port, HDMI_DETECT_AVMUTE_IRQ_HAPPENED);

			HDMI_WARN("HDMI[p%d] disable A/V output due to dolby change detected\n", port);
		}
...
{
    return lib_hdmims_calc_vfreq(hdmi_rx[port].video_t.vs2vs_cnt)/10;
}
...
int newbase_hdmi_enable_hdirq(unsigned char port)
{
	...
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
	{
		HDMI_INFO("%s[%d] is_fw_mode=%d, current_auto_mode_result=%d\n", __func__, port, lib_hdmi_is_modedet_fwmode(port), lib_hdmi_get_hdmi_mode_reg(port));
		if ((p_phy_st->frl_mode == MODE_TMDS) && !lib_hdmi_get_hdirq_en(port))
		{
	...
	switch (GET_H_VIDEO_FSM(port)) {
		...
		case MAIN_FSM_HDMI_DISPLAY_ON:
			...
			if (newbase_hdmi_display_on(port, p_phy_st->frl_mode)) {
				...
				// Dolby Vendor Specific content variation IRQ
				if (GET_FLOW_CFG(HDMI_FLOW_CFG_TIMING_MEASURE, HDMI_FLOW_CFG7_PACKET_VARIATION_IRQ_ENABLE))
				{
					HDMI_INFO("FSM_HDMI_DISPLAY_ON[%d] Dolby Vendor Specific content variation IRQ enable\n", port);
					lib_hdmi_set_video_packet_variation_irq(port, 0x100);// bit8
				}
...
unsigned char newbase_hdmi_is_need_check_vic_table (HDMI_AVI_T* avi_info,  HDMI_TIMING_T *tm, unsigned char select_vic, const VIC_TIMING_INFO* vic_timing_info)
{
...
unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	...
	{// measure fail handler.
		...
		if (hdmi_rx[port].meas_retry_cnt >= GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_MEASURE_ERR_CNT_THD))
		{
			...
			#if 0 //move to newbase_hdmi_error_handler, need to remove
		...
		if ((hdmi_rx[port].fw_bit_disp_error_detected == TRUE) && (!newbase_hdmi_err_detect_stop(port)))
		{
			...
			if (!newbase_hdmi_get_badcable_flag(port))
			{
				if (hdmi_rx[port].timing_t.tmds_clk_b > 2900) {  // >3G donothing (already do CED detect)			
					...
					if (!newbase_hdmi_get_err_recovery(port)) {
						 HDMI_EMG(" MTap0 and LE\n");
						newbase_hdmi_dfe_recovery_lo_speed(port);
					}
				}	
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    {
        unsigned char reset_phy_recover_cnt = 0x7; 		//default value
        if((hdmi_rx[port].hpd_recover_cnt > 2) && (reset_phy_recover_cnt <= 0X7))
           reset_phy_recover_cnt+=10;					//add 3s delay

        if (newbase_hdmi_tmds_scramble_off_sync(port) || (hdmi_rx[port].reset_phy_recover_cnt & reset_phy_recover_cnt)==reset_phy_recover_cnt)
        {
            ...
            hpd_toggle = 1;
            hdmi_rx[port].reset_phy_recover_cnt = 0;
        }
        else
        {
...
void newbase_hdmi_detect_flow_startover (unsigned char port, unsigned char fully_reset)
{
	...
	if (fully_reset) {
		...
		SET_HDMI_DETECT_EVENT(port, HDMI_DETECT_ONMS_QUICK_RESET);
		 if (hdmi_rx[port].timing_t.tmds_clk_b < 430) { 
			if (!newbase_hdmi_get_err_recovery(port)) {
				 HDMI_EMG(" Adjust Tap0 and LE\n");
				newbase_hdmi_dfe_recovery_lo_speed(port);
			}
		}
...
unsigned int newbase_hdmi_get_device_onms_det_mask(unsigned char port)
{
