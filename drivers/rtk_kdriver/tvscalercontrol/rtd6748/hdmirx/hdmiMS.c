...
HDMIMS_RESULT_CODE newbase_hdmims_measure_video(unsigned char port, HDMI_TIMING_T *tm, unsigned char frl_mode)
{
	...
	if(is_hv_sync_mode == TRUE)
	{
		...
		if(is_vrr_mode == TRUE)
		{
			...
			unsigned int temp_vrr2_v_total=0;
			unsigned int temp_vrr2_v_freq=0;
			unsigned int temp_vrr3_v_freq;
			unsigned long long temp_vrr3_pixel_clock = (unsigned long long)(tm->v_freq)* (unsigned long long)(tm->v_total)*(unsigned long long)(tm->h_total);
			unsigned long long temp_vrr3_h_total = (unsigned long long) (tm->h_total);
			unsigned long long temp_vrr3_v_total = tm->v_total;
			unsigned long long pixel_clock = temp_vrr3_pixel_clock;
			...
			{// no vic, check base timing v freq and v front, measure once!!
				temp_vrr2_v_freq = emp_vtem.Base_Refresh_Rate *1000;
				temp_vrr2_v_total = tm->v_act_len + tm->v_act_sta + emp_vtem.Base_Vfront;
				HDMI_OFMS_FSM_PRINT("Measure mode: VRR2 with VTEM Base, port=%d, VTEM_V_freq=%d, VTEM_V_front=%d, Final V freq=%d, V total=%d, Final H total=%d, H act=%d, V act=%d\n",
					port, emp_vtem.Base_Refresh_Rate, emp_vtem.Base_Vfront, temp_vrr2_v_freq, temp_vrr2_v_total, tm->h_total, tm->h_act_len, tm->v_act_len);
			}
			if(1)
			{// no VIC, no VTEM base v info, force set max V freq,
				do_div(temp_vrr3_pixel_clock, (unsigned long long) tm->h_total);
				temp_vrr3_v_total = tm->v_act_len + tm->v_act_sta;
				temp_vrr3_v_freq = (unsigned int)(temp_vrr3_pixel_clock)/temp_vrr3_v_total;
				HDMI_OFMS_WARN("port=%d, PixelColok(x10)=%lld, H total=%lld, v_freq = %d\n", port, pixel_clock, temp_vrr3_h_total,temp_vrr3_v_freq);
		
				for(i = 0; i < VRR_NO_VIC_MAX_VFREQ_TB_NUM; i++){
					if(temp_vrr3_v_freq >= g_video_freq_table[i].freq && temp_vrr3_v_freq < g_video_freq_table[i].freq_max){
						temp_vrr3_v_freq = g_video_freq_table[i].freq;
						do_div(pixel_clock,temp_vrr3_h_total);
						temp_vrr3_v_total = (unsigned int)pixel_clock/temp_vrr3_v_freq;
						break;
					}
				}
				...
				HDMI_EMG("Measure mode: VRR3 , without VIC, without VTEM Base, port=%d, is_get_vtem =%d, Force V freq=%d, V total=%d, Final H total=%d, H act=%d, V act=%d\n",
						port, is_get_vtem, temp_vrr3_v_freq, temp_vrr3_v_total, tm->h_total, tm->h_act_len, tm->v_act_len);
			}
			if((temp_vrr2_v_total!=0)&&(ABS(temp_vrr2_v_total,temp_vrr3_v_total)<10)){
				tm->v_total =temp_vrr2_v_total;
				tm->v_freq=temp_vrr2_v_freq;
			}
			else{
				tm->v_total = (unsigned int)temp_vrr3_v_total;
				tm->h_total = (unsigned int)temp_vrr3_h_total;
				tm->v_freq = temp_vrr3_v_freq;
			}
	...
	{// Non VRR, no h sync
		...
		if((tm->run_dsc) || (GET_DSCD_FSM() >= DSCD_FSM_PPS_MEASURE))
			is_force_VIC_Correction = true;

		if((is_get_vic != 0) && (select_vic !=0) && ((true == is_force_VIC_Correction) || (GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_DEMODE_USE_VIC_CORRECTION) == 1)))
		{//use VIC
			unsigned char vic_correction_result = 0; 
//			HDMI_OFMS_FSM_PRINT("[DE_WITH_VIC] Measure mode: No H sync with VIC, port=%d, Use VIC=%d\n", port,  select_vic);
			HDMI_OFMS_FSM_PRINT("[DE_WITH_VIC] Measure mode: No H sync with VIC, port=%d, Use VIC=%d, is_DSC=%d, GET_DSCD_FSM=%d\n", port, select_vic, tm->run_dsc, GET_DSCD_FSM());
			vic_correction_result = newbase_hdmi_vic_correction(port, tm, TRUE);
			if(vic_correction_result)
		...
		else
		{
			...
			if (hdmi_rx[port].video_fsm != MAIN_FSM_HDMI_VIDEO_READY) {
				...
				if((0 == tm->run_dsc) && (GET_DSCD_FSM() >= DSCD_FSM_PPS_MEASURE))
				{
					HDMI_OFMS_FSM_PRINT("[newbase_hdmims_measure_video] error, DSCD ReMeasure @ run_dsc=%d, GET_DSCD_FSM=%d, h_act_len=%d, v_act_len=%d\n", tm->run_dsc, GET_DSCD_FSM(), tm->h_act_len, tm->v_act_len);
					return HDMIMS_FAIL_OFMS_NOT_FINISH;
				}
			}
...
//#ifdef CONFIG_RTK_KDRV_HDMI_ENABLE_HDMI21
				...
				if(tm->h_act_len != dsc_ppc_timing->h_act_len)
				{
					...
					{
						HDMI_OFMS_WARN("DSCD correction, set Hact the same with PPS (4096x2160)!!. tm->h_total=%d, tm->h_act_len=%d, VIC=%d\n", tm->h_total, tm->h_act_len, avi_info.VIC);
						tm->h_act_len = dsc_ppc_timing->h_act_len;
					}
					else if((3840 == dsc_ppc_timing->h_act_len) && (2160 == dsc_ppc_timing->v_act_len))
					{
						HDMI_OFMS_WARN("DSCD correction, set Hact the same with PPS (3840x2160)!!. tm->h_total=%d, tm->h_act_len=%d, VIC=%d\n", tm->h_total, tm->h_act_len, avi_info.VIC);
						tm->h_act_len = dsc_ppc_timing->h_act_len;
					}
					else
					{
