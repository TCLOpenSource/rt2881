...
extern void panel_dlg_customer_get_default_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k1k120_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k2k60_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k1k144_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k2k144_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k1k240_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);
extern void panel_dlg_customer_get_4k1k288_dlg_parameter_info(PANEL_DLG_PARAMETER_INFO *panel_dlg_parameter_info);


static ssize_t Check_PanelInformation(
    struct device* dev,
...
)
{
    ...
    PANEL_DLG_PARAMETER_INFO panel_dlg_parameter_info = {0};
    PANEL_DLG_MODE dlgMode = PANEL_DLG_DEFAULT;

    dlgMode = panel_dlg_get_panel_dlg_mode();

    if (strcmp(attr->attr.name, "panelInfo") != 0)
        return -EINVAL;

    n = scnprintf(ptr, count, "---------------- Panel Display Information --------------\n\n");
    ptr += n; count -= n;

    // Call the function to initialize panel_dlg_parameter_info
    if(dlgMode == PANEL_DLG_4k1k120){
		panel_dlg_customer_get_4k1k120_dlg_parameter_info(&panel_dlg_parameter_info);
	}
	else if(dlgMode == PANEL_DLG_4k1k144){
        panel_dlg_customer_get_4k1k144_dlg_parameter_info(&panel_dlg_parameter_info);
	}
	else if(dlgMode == PANEL_DLG_4k2k144){
		panel_dlg_customer_get_4k2k144_dlg_parameter_info(&panel_dlg_parameter_info);
		}
	else if(dlgMode == PANEL_DLG_4k1k240){
		panel_dlg_customer_get_4k1k240_dlg_parameter_info(&panel_dlg_parameter_info);
		}
	else if(dlgMode == PANEL_DLG_4k1k288){
		panel_dlg_customer_get_4k1k288_dlg_parameter_info(&panel_dlg_parameter_info);
		}
	else if(dlgMode == PANEL_DLG_4k2k60){
		panel_dlg_customer_get_4k2k60_dlg_parameter_info(&panel_dlg_parameter_info);
		}
	else if(dlgMode == PANEL_DLG_DEFAULT){

		panel_dlg_customer_get_default_dlg_parameter_info(&panel_dlg_parameter_info);
	}

        n = scnprintf(ptr, count, "       panel refresh=%d\n", panel_dlg_parameter_info.refresh);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel typical=%d\n", panel_dlg_parameter_info.typical);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel htotal=%d\n", panel_dlg_parameter_info.htotal);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel vtotal=%d\n", panel_dlg_parameter_info.vtotal);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel den_hstart=%d\n", panel_dlg_parameter_info.den_hstart);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel den_hend=%d\n", panel_dlg_parameter_info.den_hend);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel den_vstart=%d\n", panel_dlg_parameter_info.den_vstart);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel den_vend=%d\n", panel_dlg_parameter_info.den_vend);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel act_hstart=%d\n", panel_dlg_parameter_info.act_hstart);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel act_hend=%d\n", panel_dlg_parameter_info.act_hend);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel act_vstart=%d\n", panel_dlg_parameter_info.act_vstart);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel act_vend=%d\n", panel_dlg_parameter_info.act_vend);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel min_vtotal=%d\n", panel_dlg_parameter_info.min_vtotal);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel max_vtotal=%d\n", panel_dlg_parameter_info.max_vtotal);
        ptr += n; count -= n;
        n = scnprintf(ptr, count, "       panel hsync_last_line=%d\n\n\n", panel_dlg_parameter_info.hsync_last_line);
        ptr += n; count -= n;

    return ptr - buf;
}

  static DEVICE_ATTR(panelInfo, 0444, Check_PanelInformation, NULL);
    ...
    )
{
    ...
    if (strcmp(attr->attr.name, "debug_print")==0)
    {
        ...
        ptr+=n; count-=n;
        n = scnprintf(ptr, count, "XPERI_VSI=%d  (Default: 0)\n", g_infoframe_xperi_vsi_debug_print);
        ptr+=n; count-=n;
                   ...
                   const char *buf, size_t count)
{
    ...
    if (strcmp(attr->attr.name, "debug_print")==0)
    {
        ...
        else if (sscanf(buf, "XPERI_VSI=%1d", &val)==1)
        {
            if (val)
            {
                g_infoframe_xperi_vsi_debug_print = 1;
            }
            else
            {
                g_infoframe_xperi_vsi_debug_print = 0;
            }
        }
        ...
        else if (sscanf(buf, "CVTEM=%1d", &val)==1)
        {
            if (val)
            {
                g_infoframe_cvtem_debug_print = val;
            }
            else
            {
...
int __init hdmi_module_init(void)
{
    ...
    device_create_file(hdmi_miscdev.this_device, &dev_attr_panelInfo);
    return 0;
}
