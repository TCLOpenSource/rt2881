...
static char VPQEX_VPQ_EXTERN_IOC_SET_COLOR_TEMP_EXTERN(TV001_COLOR_TEMP_DATA_S *pcolor_temp_data_s)
{
	...
	rtd_pr_vpq_info("Old user menu\n");
        ...
        rtd_pr_vpq_info("VPQEX_VPQ_EXTERN_IOC_SET_COLOR_TEMP_EXTERN, fwif_color_set_OutputGamma_System\n");
		fwif_color_set_OutputGamma_System();		
	} else {
	rtd_pr_vpq_info("New user menu\n");
    ...
    case VPQ_EXTERN_IOC_SET_VDTABLE:
    	{
    		...
    		if (copy_from_user(pVD, (void __user *)arg, sizeof(SLR_VIP_TABLE_VD))) {
                ...
                for (vd_src=0; vd_src<8; vd_src++){
                      ATV_VD_Table[vd_src][2]  = pVD->mBLMTbl[1].data[vd_src][0];
                      ATV_VD_Table[vd_src][3]  = pVD->mBLMTbl[1].data[vd_src][1];
                      ATV_VD_Table[vd_src][0]  = pVD->mBLMTbl[1].data[vd_src][2];
                      ATV_VD_Table[vd_src][1]  = pVD->mBLMTbl[1].data[vd_src][3];              
                }
                for (vd_src=0; vd_src<8; vd_src++){
                      AV_VD_Table[vd_src][2]   = pVD->mBLMTbl[0].data[vd_src][0];
                      AV_VD_Table[vd_src][3]   = pVD->mBLMTbl[0].data[vd_src][1];
                      AV_VD_Table[vd_src][0]   = pVD->mBLMTbl[0].data[vd_src][2];
                      AV_VD_Table[vd_src][1]   = pVD->mBLMTbl[0].data[vd_src][3];            
                }
 ...
 ************************************************************************/
		...
		{
			unsigned int film_mode_enable = 0;
			extern unsigned char g_film_mode_pending;
			...
			if (copy_from_user(&film_mode_enable,  (int __user *)arg,sizeof(unsigned int))) {
				...
				film_mode.film_status = film_mode_enable?28:0;
				if (down_trylock(get_DI_semaphore()) == 0) {
					drvif_module_film_mode((DRV_film_mode *) &film_mode);
					g_film_mode_pending = 0;
					up(get_DI_semaphore());
				} else {
					g_film_mode_pending = 1;
				}
				ret = 0;
			}
		}
					...
					if (DEM_arg == NULL) {
						...
						if(copy_from_user(DEM_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN.length))
						{
							...
							if (gVIP_VPQ_IOC_CT_DelaySet_CTRL.Enable == 1)
							{
								...
								memcpy(&gVIP_VPQ_IOC_CT_DelaySet_CTRL.Gamma_10p[0], &DEM_arg[0], sizeof(int)*VIP_DEM_10p_Gamma_Offset_Num*VIP_GAMMA_CHANNEL_RGB_MAX);
		...
		case VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA:
		{
			...
			if (copy_from_user(&hal_VPQ_DATA_EXTERN, (int __user *)arg, sizeof(HAL_VPQ_DATA_EXTERN_T))) {
				rtd_pr_vpq_err("%d,VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA hal_VPQ_DATA_EXTERN copy fail\n",__LINE__);
				ret = -1;
			} else {				
				Data_addr = hal_VPQ_DATA_EXTERN.pData;
				...
				if (hal_VPQ_DATA_EXTERN.wId == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Set)
				{
					if (hal_VPQ_DATA_EXTERN.version == VPQ_DEV_EXT_APDEM_INIT_PARA_Set_Ini_Table) 
					{
						...
						if (hal_VPQ_DATA_EXTERN.length != panel_ini_size) {
							rtd_pr_vpq_info("%d,kernel VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA len error, len=%d,\n", __LINE__,hal_VPQ_DATA_EXTERN.length);
							ret = -1;
						} else {
							DEM_arg = (unsigned int *)dvr_malloc_specific(hal_VPQ_DATA_EXTERN.length, GFP_DCU2_FIRST);
							if (DEM_arg == NULL) {
								rtd_pr_vpq_info("%d,kernel VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA alloc fail, len=%d,\n", __LINE__,hal_VPQ_DATA_EXTERN.length);
								ret = -1;
							} else {
								if(copy_from_user(DEM_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN.length))
								{
									rtd_pr_vpq_err("%d,VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA table copy fail\n",__LINE__);
									ret = -1;
								} else {
									ret = Scaler_set_APDEM(DEM_ARG_Gamma_CURVE_TBL_PANEL_Chromaticity, (void *)DEM_arg);
									rtd_pr_vpq_info("%d,kernel VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA done done\n",__LINE__);
								}
								dvr_free((void *)DEM_arg);
							}
					...
					{
						rtd_pr_vpq_emerg("%d,kernel VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA ver error ver=%d,\n", __LINE__,hal_VPQ_DATA_EXTERN.version);
						ret = -1;
					}
				}
				else if (hal_VPQ_DATA_EXTERN.wId == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Get)
				{
					if (hal_VPQ_DATA_EXTERN.version == VPQ_DEV_EXT_APDEM_INIT_PARA_Set_Factory_Color_Chromaticity)
					{
						if (hal_VPQ_DATA_EXTERN.length != sizeof(VIP_DEM_PANEL_INI_Color_Chromaticity_Facotry)) {
							...
							if(copy_to_user((void __user *)(Data_addr), &gColor_Chromaticity_Facotry_Set, hal_VPQ_DATA_EXTERN.length)) {
								rtd_pr_vpq_emerg("%d,VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA table copy fail\n",__LINE__);
								ret = -1;
							}
							else
							{
					...
					{
						rtd_pr_vpq_emerg("%dkernel VPQ_EXTERN_IOC_SET_APDEM_INIT_PARA ver error ver=%d,\n", __LINE__,hal_VPQ_DATA_EXTERN.version);
						ret = -1;
					}
				}
				else
