...
#if IS_ENABLED(CONFIG_RTK_KDRV_GDMA) && (0 == IS_ENABLED(CONFIG_CUSTOMER_TV006))
...
unsigned char modestate_decide_display_timing(void)
{
	if(Disp_Get_isForceFreeRun()==DISP_FREE_RUN){
		...
		return DISPLAY_MODE_FRC;
	}else if((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)){
		return DISPLAY_MODE_FRC;
	}else
...
//	unsigned int vtotal;
	...
	if ((Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI)) {
...
void Scaler_wait_for_input_one_frame(unsigned char display)
{
	//when disp_frc_fsync raised, position is at den_end, front porch may not at LCM framecnt.
	//need to wait a LCM period to make sure vsync aligned done.
	__maybe_unused unsigned int start_time = 0, end_time = 0;
	start_time = IoReg_Read32(TIMER_VCPU_CLK90K_LO_reg);
	if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) > 235) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ) < 245))
...
//	ppoverlay_uzudtg_control1_RBUS ppoverlay_uzudtg_control1_reg;
	...
	UINT32 mul_dbuf_cnt=0;
//	UINT32 mul_dbuf_cnt2=0;
	__maybe_unused UINT32 meas0_lineCnt1=0;
	__maybe_unused UINT32 meas0_lineCnt2=0;
	__maybe_unused UINT32 meas0_lineCnt3=0;
	__maybe_unused UINT32 meas0_lineCnt4=0;
	__maybe_unused UINT32 meas0_lineCnt5=0;
	__maybe_unused UINT32 meas0_lineCnt6=0;
	__maybe_unused UINT32 meas1_lineCnt1=0;
	__maybe_unused UINT32 meas1_lineCnt2=0;
	__maybe_unused UINT32 meas1_lineCnt3=0;
	__maybe_unused UINT32 meas1_lineCnt4=0;
	__maybe_unused UINT32 meas1_lineCnt5=0;
	__maybe_unused UINT32 meas1_lineCnt6=0;
...
void HAL_VBE_DISP_SetDisplayOutput(BOOLEAN bOnOff)
{
	...
	if(bOnOff)
	{
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS) || (Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) ||
                   (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)){
			msleep(25);
		...
		if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K) ||
                   (Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K) || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) ||
                   (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)){
			vbe_disp_lane_power_on();
		}

        if((Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || 
           (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS))
            vbe_write_PMIC();
	}
	else
...
void HAL_VBE_DISP_SetSpreadSpectrum(BOOLEAN bEnable, UINT16 u16Percent, UINT16 u16Period)
{
	...
	if((bEnable == 0) || (u16Percent ==0) || (Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) ||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)) {
...
void vbe_disp_panel_suspend(void)
{
    ...
    else if(Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS){
        vbe_disp_panel_tcon_suspend_resume.TCON_TCON_IP_EN_S_R =                    IoReg_Read32(TCON_TCON_IP_EN_reg);
        vbe_disp_panel_tcon_suspend_resume.TCON_TCON0_HSE_S_R =                     IoReg_Read32(TCON_TCON0_HSE_reg);
        vbe_disp_panel_tcon_suspend_resume.TCON_TCON0_VSE_S_R =                     IoReg_Read32(TCON_TCON0_VSE_reg);
        vbe_disp_panel_tcon_suspend_resume.TCON_TCON0_Ctrl_S_R =                    IoReg_Read32(TCON_TCON0_Ctrl_reg);
	...
	if(vbe_suspend_instant_mode){	// Only STR need to control Tcon off
        ...
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s]MINILVDS tcon suspend finished \n",__FUNCTION__);
...
void vbe_disp_panel_resume(void)
{
    ...
    else if(Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)
	{
        ...
        rtd_printk(KERN_NOTICE, TAG_NAME_VBE,"[%s] MINILVDS tcon resume finished\n",__FUNCTION__);
...
void vbe_disp_init_flow_finish(void)
{
	...
	if((Get_DISPLAY_PANEL_TYPE() == P_CEDS)||(Get_DISPLAY_PANEL_TYPE() == P_EPI_TYPE)||(Get_DISPLAY_PANEL_TYPE() == P_VBY1_4K2K)||(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K)
	   || (Get_DISPLAY_PANEL_TYPE() == P_CSPI) || (Get_DISPLAY_PANEL_TYPE() == P_CMPI) || (Get_DISPLAY_PANEL_TYPE() == P_ISP) || (Get_DISPLAY_PANEL_TYPE() == P_MINI_LVDS)){
...
unsigned char drvif_factory_set_lvds_current(KADP_VBE_DISP_LVDS_T *lvds_info)
{
	...
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_64_RBUS		phy_ctrl_1_64_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_68_RBUS		phy_ctrl_1_68_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_72_RBUS		phy_ctrl_1_72_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_76_RBUS		phy_ctrl_1_76_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_80_RBUS		phy_ctrl_1_80_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_84_RBUS		phy_ctrl_1_84_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_88_RBUS		phy_ctrl_1_88_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_92_RBUS		phy_ctrl_1_92_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_96_RBUS		phy_ctrl_1_96_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_100_RBUS		phy_ctrl_1_100_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_104_RBUS		phy_ctrl_1_104_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_108_RBUS		phy_ctrl_1_108_reg;
	...
	if(len-- > 0)
	{
		phy_ctrl_1_64_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_64_reg);
		phy_ctrl_1_64_reg.pif_tx16_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_64_reg, phy_ctrl_1_64_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_68_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_68_reg);
		phy_ctrl_1_68_reg.pif_tx17_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_68_reg, phy_ctrl_1_68_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_72_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_72_reg);
		phy_ctrl_1_72_reg.pif_tx18_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_72_reg, phy_ctrl_1_72_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_76_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_76_reg);
		phy_ctrl_1_76_reg.pif_tx19_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_76_reg, phy_ctrl_1_76_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_80_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_80_reg);
		phy_ctrl_1_80_reg.pif_tx20_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_80_reg, phy_ctrl_1_80_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_84_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_84_reg);
		phy_ctrl_1_84_reg.pif_tx21_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_84_reg, phy_ctrl_1_84_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_88_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_88_reg);
		phy_ctrl_1_88_reg.pif_tx22_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_88_reg, phy_ctrl_1_88_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_92_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_92_reg);
		phy_ctrl_1_92_reg.pif_tx23_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_92_reg, phy_ctrl_1_92_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_96_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_96_reg);
		phy_ctrl_1_96_reg.pif_tx24_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_96_reg, phy_ctrl_1_96_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_100_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_100_reg);
		phy_ctrl_1_100_reg.pif_tx25_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_100_reg, phy_ctrl_1_100_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_104_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_104_reg);
		phy_ctrl_1_104_reg.pif_tx26_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_104_reg, phy_ctrl_1_104_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_108_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_108_reg);
		phy_ctrl_1_108_reg.pif_tx27_idrv_sel = lvds_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_108_reg, phy_ctrl_1_108_reg.regValue);
	}
...
unsigned char drvif_factory_get_lvds_current(KADP_VBE_DISP_LVDS_T *lvds_info)
{
	...
	regValue[index++] = phy_ctrl_1_64_reg.pif_tx16_idrv_sel;
	regValue[index++] = phy_ctrl_1_68_reg.pif_tx17_idrv_sel;
	regValue[index++] = phy_ctrl_1_72_reg.pif_tx18_idrv_sel;
	regValue[index++] = phy_ctrl_1_76_reg.pif_tx19_idrv_sel;
	regValue[index++] = phy_ctrl_1_80_reg.pif_tx20_idrv_sel;
	regValue[index++] = phy_ctrl_1_84_reg.pif_tx21_idrv_sel;
	regValue[index++] = phy_ctrl_1_88_reg.pif_tx22_idrv_sel;
	regValue[index++] = phy_ctrl_1_92_reg.pif_tx23_idrv_sel;
	regValue[index++] = phy_ctrl_1_96_reg.pif_tx24_idrv_sel;
	regValue[index++] = phy_ctrl_1_100_reg.pif_tx25_idrv_sel;
	regValue[index++] = phy_ctrl_1_104_reg.pif_tx26_idrv_sel;
	regValue[index++] = phy_ctrl_1_108_reg.pif_tx27_idrv_sel;
...
unsigned char drvif_factory_set_pre_emphasis_current(KADP_VBE_DISP_Emphasis_T *emp_info)
{
	...
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_65_RBUS		phy_ctrl_1_65_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_69_RBUS		phy_ctrl_1_69_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_73_RBUS		phy_ctrl_1_73_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_77_RBUS		phy_ctrl_1_77_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_81_RBUS		phy_ctrl_1_81_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_85_RBUS		phy_ctrl_1_85_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_89_RBUS		phy_ctrl_1_89_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_93_RBUS		phy_ctrl_1_93_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_97_RBUS		phy_ctrl_1_97_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_101_RBUS		phy_ctrl_1_101_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_105_RBUS		phy_ctrl_1_105_reg;
	pinmux_lvdsphy_lvdsepivby1_phy_ctrl_1_109_RBUS		phy_ctrl_1_109_reg;
	...
	if(len-- > 0)
	{
		phy_ctrl_1_65_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_65_reg);
		phy_ctrl_1_65_reg.pif_tx16_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_65_reg, phy_ctrl_1_65_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_69_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_69_reg);
		phy_ctrl_1_69_reg.pif_tx17_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_69_reg, phy_ctrl_1_69_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_73_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_73_reg);
		phy_ctrl_1_73_reg.pif_tx18_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_73_reg, phy_ctrl_1_73_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_77_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_77_reg);
		phy_ctrl_1_77_reg.pif_tx19_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_77_reg, phy_ctrl_1_77_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_81_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_81_reg);
		phy_ctrl_1_81_reg.pif_tx20_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_81_reg, phy_ctrl_1_81_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_85_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_85_reg);
		phy_ctrl_1_85_reg.pif_tx21_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_85_reg, phy_ctrl_1_85_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_89_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_89_reg);
		phy_ctrl_1_89_reg.pif_tx22_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_89_reg, phy_ctrl_1_89_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_93_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_93_reg);
		phy_ctrl_1_93_reg.pif_tx23_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_93_reg, phy_ctrl_1_93_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_97_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_97_reg);
		phy_ctrl_1_97_reg.pif_tx24_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_97_reg, phy_ctrl_1_97_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_101_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_101_reg);
		phy_ctrl_1_101_reg.pif_tx25_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_101_reg, phy_ctrl_1_101_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_105_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_105_reg);
		phy_ctrl_1_105_reg.pif_tx26_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_105_reg, phy_ctrl_1_105_reg.regValue);
	}

	if(len-- > 0)
	{
		phy_ctrl_1_109_reg.regValue = IoReg_Read32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_109_reg);
		phy_ctrl_1_109_reg.pif_tx27_iem_sel = emp_info->current_val[index++];
		IoReg_Write32(PINMUX_LVDSPHY_LVDSEPIVBY1_PHY_CTRL_1_109_reg, phy_ctrl_1_109_reg.regValue);
	}
...
unsigned char drvif_factory_get_pre_emphasis_current(KADP_VBE_DISP_Emphasis_T *emp_info)
{
	...
	regValue[index++] = phy_ctrl_1_65_reg.pif_tx16_iem_sel;
	regValue[index++] = phy_ctrl_1_69_reg.pif_tx17_iem_sel;
	regValue[index++] = phy_ctrl_1_73_reg.pif_tx18_iem_sel;
	regValue[index++] = phy_ctrl_1_77_reg.pif_tx19_iem_sel;
	regValue[index++] = phy_ctrl_1_81_reg.pif_tx20_iem_sel;
	regValue[index++] = phy_ctrl_1_85_reg.pif_tx21_iem_sel;
	regValue[index++] = phy_ctrl_1_89_reg.pif_tx22_iem_sel;
	regValue[index++] = phy_ctrl_1_93_reg.pif_tx23_iem_sel;
	regValue[index++] = phy_ctrl_1_97_reg.pif_tx24_iem_sel;
	regValue[index++] = phy_ctrl_1_101_reg.pif_tx25_iem_sel;
	regValue[index++] = phy_ctrl_1_105_reg.pif_tx26_iem_sel;
	regValue[index++] = phy_ctrl_1_109_reg.pif_tx27_iem_sel;
...
void vbe_disp_orbit_reset_position_overscan(void)
{
    ...
    ppoverlay_main_active_v_start_end_RBUS main_active_v_start_end_reg;
	SCALER_ORBIT_SHIFT_INFO scalerOrbitMemInfo = {0};
	__maybe_unused SCALER_ORBIT_SHIFT_INFO *p_orbitShiftInfo = NULL;
   	StructSrcRect input_size;
	StructSrcRect input_timing;
...
{
	__maybe_unused UINT32 u32_iFrameRate = 0;
