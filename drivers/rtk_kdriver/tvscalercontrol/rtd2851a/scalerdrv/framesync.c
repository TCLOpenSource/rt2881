...
unsigned char drv_framesync_check_iv2dv_phaseErr_on_fixlastline(void){
	...
	__maybe_unused UINT32 temp1, temp2, temp3;
...
//	ppoverlay_new_meas1_linecnt_real_RBUS new_meas1_linecnt_real_reg;
	...
	ppoverlay_new_meas0_linecnt_real_RBUS ppoverlay_new_meas0_linecnt_real_reg;
	UINT32 voHtotal;
	__maybe_unused UINT32 voVtotal;
	UINT32 voClock;
	UINT32 dHtotal;
	UINT32 dVFreq;
	UINT32 timeStamp1, timeStamp2,timeStamp3;
...
//	unsigned int MLines = 0;
	...
	__maybe_unused unsigned int voAddr = 0;
...
//	SET_PANEL_DCLK_DIV(1); //mag2 un-used dclk sel, move to dpll_o
...
#ifdef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
	// Force 422
	disp_ddr_mainctrl_reg.main_in_format = !(Scaler_DispGetStatus(display,SLR_DISP_422CAP));
#else
	disp_ddr_mainctrl_reg.main_in_format = (!(Scaler_DispGetStatus(display,SLR_DISP_422CAP)) || dvrif_memory_compression_get_enable(SLR_MAIN_DISPLAY));
#endif
	disp_ddr_mainctrl_reg.main_bit_num = Scaler_DispGetStatus(display,SLR_DISP_10BIT);
	...
	while(!(IoReg_Read32(display ? MDOMAIN_DISP_DDR_SubCtrl_reg : MDOMAIN_DISP_DDR_MainCtrl_reg)&(_BIT17|(Scaler_DispGetStatus(display,SLR_DISP_10BIT)?_BIT16:0))));
#else
	...
	IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_reg ,	_BIT0);
#ifndef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
	while(!(IoReg_Read32(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_DISP_DDR_SubCtrl_reg : */MDOMAIN_DISP_DDR_MainCtrl_reg)&(_BIT17|(Scaler_DispGetStatus(display,SLR_DISP_10BIT)?_BIT16:0))));
#else
	while(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg)& _BIT0);
#endif
#endif
...
//	ich2_ibuff_STATUS_RBUS ich2_ibuff_STATUS_reg;
	...
	if(_CHANNEL1 == display)
	{
...
#ifdef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
		ddr_mainctrl_reg.main_in_format = !Scaler_DispGetStatus(display,SLR_DISP_422CAP);
#else
		ddr_mainctrl_reg.main_in_format = 1; // 444
#endif
		ddr_mainctrl_reg.main_bit_num = Scaler_DispGetStatus(display,SLR_DISP_10BIT); // 8bit or 10bit
		IoReg_Write32(MDOMAIN_DISP_DDR_MainCtrl_reg, ddr_mainctrl_reg.regValue);
		...
		timeout = 0xffff;
		ddr_mainctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg);
		while(((1 != ddr_mainctrl_reg.main_in_format) || (Scaler_DispGetStatus(display,SLR_DISP_10BIT) != ddr_mainctrl_reg.main_bit_num)) && (timeout))
		{
			ddr_mainctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg);
			timeout--;
			msleep(10);
	...
	else// if(_CHANNEL2 == info->channel)
	{
		...
		ddr_subctrl_reg.sub_bit_num = Scaler_DispGetStatus(display,SLR_DISP_10BIT); // 8bit or 10bit
		...
		timeout = 0xffff;
		ddr_subctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg);
		while(((1 != ddr_subctrl_reg.sub_in_format) || (Scaler_DispGetStatus(display,SLR_DISP_10BIT) != ddr_subctrl_reg.sub_bit_num)) && (timeout))
		{
			ddr_subctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg);
			timeout--;
		}
