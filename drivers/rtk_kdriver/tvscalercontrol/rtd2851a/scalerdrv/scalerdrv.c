...
void drvif_scaler_set_display_mode_frc2fsync_by_hw_speedup_Mcode_HW_CTRL_3_impl(void)
{
		...
		ppoverlay_double_buffer_ctrl_RBUS ppoverlay_double_buffer_ctrl_reg;
		ppoverlay_dv_total_RBUS dv_total_reg;
		__maybe_unused unsigned int vFreq = 0;
		...
		rtd_pr_vbe_notice("###[%s(%d)] iv2dv_line = 8\n", __func__, __LINE__);
...
//		unsigned int vFreq = 0;
		...
		if(!((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPH_ACT_WID_PRE)> 1920) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY,SLR_INPUT_IPV_ACT_LEN_PRE) > 1080))){
			...
			if(drv_memory_get_game_mode() || (vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
			...
			rtd_pr_vbe_notice("###[%s(%d)] iv2dv = 0x%x\n", __func__, __LINE__, fs_iv_dv_fine_tuning5_reg.regValue);
#ifdef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
		}
#endif
		uzudtg_fractional_fsync_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_fractional_fsync_reg);
...
void drvif_scaler_set_display_mode_frc2fsync_by_I2D_CTRL(void)
{
	...
	rtd_outl(PPOVERLAY_FS_IV_DV_Fine_Tuning5_reg, fs_iv_dv_fine_tuning5_reg.regValue);
	rtd_pr_vbe_notice("###[%s(%d)] iv2dv_line = 2\n", __func__, __LINE__);
#ifdef CONFIG_DATA_FRAMESYNC_FORCE_422_CAP
	}
#endif
...
void fw_scaler_cal_scale_down_size(void)
{
	...
	if (display > SLR_SUB_DISPLAY) {
		rtd_pr_vsc_err("###[%s] Invalid display path!!!\n", __func__);
...
//		Scaler_DispSetInputInfo(SLR_INPUT_DI_WID,Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID));
	...
	if((Scaler_DispGetInputInfo(SLR_INPUT_SDNRH_ACT_WID) > Scaler_DispGetInputInfo(SLR_INPUT_CAP_WID)))
		  Scaler_DispSetScaleStatus(display, SLR_SCALE_H_DOWN, TRUE);
  	else
		  Scaler_DispSetScaleStatus(display, SLR_SCALE_H_DOWN, FALSE);
    ...
    if ((display == SLR_MAIN_DISPLAY) && (get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1)) {
		if((Scaler_DispGetInputInfo(SLR_INPUT_SDNRV_ACT_LEN) >> 1) > Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN))
			Scaler_DispSetScaleStatus(display, SLR_SCALE_V_DOWN, TRUE);
		else
			Scaler_DispSetScaleStatus(display, SLR_SCALE_V_DOWN, FALSE);
    ...
    if(Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) < Scaler_DispGetInputInfo(SLR_INPUT_DISP_WID)){
		...
		Scaler_DispSetScaleStatus(display, SLR_SCALE_H_UP, TRUE);	 //PQ Request
	}else{
		Scaler_DispSetScaleStatus(display, SLR_SCALE_H_UP, FALSE);
	}
    ...
    if(Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN) < Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN))
		...
		Scaler_DispSetScaleStatus(display, SLR_SCALE_V_UP, TRUE);		//PQ Request
	else
		Scaler_DispSetScaleStatus(display, SLR_SCALE_V_UP, FALSE);
...
//	ppoverlay_memc_mux_ctrl_RBUS memc_mux_ctrl_reg;
	...
	unsigned int count=0x3fffff;
//	int delay_frame;
	__maybe_unused int me_bypass;
//	ppoverlay_new_meas1_linecnt_real_RBUS new_meas1_linecnt_real_reg;
	__maybe_unused static int film_mode_type = VO_FILM_TYPE_NONE;
...
//	tRPCShareMem->ulAddr[EMF_COEF_MEM_IDX] = g_atShareMemInfo[SCALERIOC_SETEMF].ulOffset + g_ulPhyAddr + 0xa0000000;
	...
	tRPCShareMem->ulAddr[HDMI_DOLBY_VSEM_IDX] = g_atShareMemInfo[HDMI_DOLBY_VSEM_FLAG].ulOffset + g_ulPhyAddr + 0xa0000000;
	//#endif
    ...
    tRPCShareMem->ulAddr[FORCE_RUN_FAKE_SCALER_IDX] = g_atShareMemInfo[FORCE_RUN_FAKE_SCALER_STATE].ulOffset + g_ulPhyAddr + 0xa0000000;
    tRPCShareMem->ulAddr[SCALERIOC_SET_I3DDMA_SW_BUF_NUM_IDX] = g_atShareMemInfo[SCALERIOC_SET_I3DDMA_SW_BUF_BUM].ulOffset + g_ulPhyAddr + 0xa0000000;
    tRPCShareMem->ulAddr[SCLAERIOC_SET_I3DDMA_SW_CAP0_LAST_WRITE_FLAG_IDX] = g_atShareMemInfo[SCALERIOC_I3DDMA_CAP0_LAST_WRITE_FLAG].ulOffset + g_ulPhyAddr + 0xa0000000;
...
INT8 Scaler_Get3D_IsSupport3D(unsigned char is2Dcvt3D, unsigned char uc3DType)
{
	...
	__maybe_unused unsigned int curMode = 0;
...
void check_vo3d_detect_result(SCALER_DISP_CHANNEL display)
{
	...
	if(Scaler_Get3D_VoAutoDetectMode() == TRUE){
		...
		{
			__maybe_unused static int count = 0;
			//if (count++ > 10)
			{
				// copy from RPC share memory
...
void scaler_Rotate_VI_init_send_rpc(void)
{
	...
	__maybe_unused unsigned int ulCount = 0;//, i = 0;
	int ret;
