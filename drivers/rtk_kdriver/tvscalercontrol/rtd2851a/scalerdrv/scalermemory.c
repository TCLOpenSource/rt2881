...
//	unsigned int  TotalSizeFrame=0;
	...
	if (display > SLR_SUB_DISPLAY) {
			rtd_pr_scaler_memory_err("###[%s] Invalid display path!!!\n", __func__);
			display = SLR_MAIN_DISPLAY;
	}
	...
	rtd_pr_scaler_memory_notice("### [%s(%d)] main_pre_rd_v_start=%d \n", __FUNCTION__, __LINE__, ddr_mainprevstart_reg.main_pre_rd_v_start);
	...
	{
		MemGetBlockShiftAddr(display, Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_HSTA),Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_VSTA), &MemShiftAddr, &droppixel);
	#ifdef CONFIG_ENABLE_SE_ROTATE_M_DOMAIN_BLOCK_MODE
		if((display == SLR_MAIN_DISPLAY) && Get_rotate_function(SLR_MAIN_DISPLAY)) {
	...
	#endif
		TotalSize = memory_get_disp_line_size1(display, Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) + droppixel);	// calculate the memory size of capture
	...
	Scaler_Disp3dSetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_DISPLAY_SIZE, memory_get_display_size(display));
...
#endif
	Scaler_Disp3dSetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_MAIN_ADDR, ucStartAddress);
	Scaler_Disp3dSetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_MAIN_SEC_ADDR, ucStartAddress2);
	Scaler_Disp3dSetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_MAIN_THIRD_ADDR, ucStartAddress3);
...
//	unsigned int  TotalSizeFrame=0;
	...
	if((display == _CHANNEL1) && ( !Scaler_DispGetStatus((SCALER_DISP_CHANNEL)_CHANNEL1, SLR_DISP_422CAP))){
		burstLen=0xf0;
	}
...
void zoom_set_input_format(unsigned char display, unsigned char bDouble)
{
	...
	if ( bDouble )
	{
		...
		temp1 =  (display ? _BIT1 : 0) ;
	temp1 |= display ? 0 : (Scaler_DispGetStatus(display,SLR_DISP_10BIT) ? _BIT16 : 0);
	if((display == SLR_MAIN_DISPLAY) && (dvrif_memory_compression_get_enable(display) == TRUE)){
		temp1 |= _BIT17;
	}else{
	...
	else
	{
		...
		if(!is_2k_memory_buffer_source(SLR_MAIN_DISPLAY))
		{
			...
			if(get_platform_model() == PLATFORM_MODEL_2K) {
				if(!query_addr)
				{
					if(Get_Val_domain_2k_memory_addr() != INVALID_VAL && Get_Val_domain_2k_memory_addr() != 0) {
...
#else
            if (drv_memory_Get_multibuffer_flag()){
                rtd_pr_scaler_memory_notice("[VideoDelay] multibuffer update mem tag\n");
...
#endif
		{
			...
			if(get_platform_model() == PLATFORM_MODEL_2K) {
				if(!query_addr)
				{
					...
					if(mdomain_2k_memory_addr == INVALID_VAL)
					{
						rtd_pr_scaler_memory_emerg("mdomain 2k pli alloc memory fail!\n");
						return;
					}
...
void jpeg_memory_alloc_capture2(eMemCapAccessType Cap_type,eMemIdx idx)
{
	...
	TotalSize1 = memory_get_capture_max_size(display, Cap_type);

	TotalSize1 = SHL(TotalSize1, 3); //unit conversion. 64bits to 8bits

	if(Cap_type == MEMCAPTYPE_LINE){
		TotalSize1 *=Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_IPV_ACT_LEN_PRE);
		TotalSize *=Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_CAP_LEN);
	}
...
//	unsigned int  TotalSizeFrame;
	...
	if(Cap_type == MEMCAPTYPE_LINE){
		...
		blockstep = (TotalSize*Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_CAP_LEN));
		blockstep = (blockstep % 2)? blockstep + 1: blockstep;
...
//-	rtd_pr_scaler_memory_debug("\n Calculate FIFO Size !");
...
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	IoReg_Mask32(display ? MDOMAIN_CAP_DDR_In2LineNum_reg: MDOMAIN_CAP_DDR_In1LineNum_reg, 0xfffee000, (Cap_type<<16)|AND(Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN), 0x1FFF));
	...
	IoReg_SetBits(display?MDOMAIN_CAP_DDR_In2Ctrl_reg:MDOMAIN_CAP_DDR_In1Ctrl_reg, _BIT0 | (bDouble ? MDOMAIN_CAP_DDR_In1Ctrl_in1_double_enable_mask : 0));
	IoReg_SetBits(display ? MDOMAIN_CAP_Cap_In2_enable_reg: MDOMAIN_CAP_Cap_In1_enable_reg, _BIT0);
#else
	IoReg_Mask32(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2LineNum_reg : */MDOMAIN_CAP_DDR_In1LineNum_reg, 0xfffee000, (Cap_type<<16)|AND(Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN), 0x1FFF));
...
void dvrif_memory_handler_for_main(SCALER_DISP_CHANNEL display)
{
	...
	IoReg_ClearBits(MDOMAIN_CAP_Cap_In1_enable_reg, _BIT0); // Disable M-cap before do M-cap related setting
...
void drv_memory_set_3D_SBS_LR_swap(unsigned char lr_swap)
{
	...
	TotalSize = Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)display, SLR_DISP_3D_BLKSTEP);
	...
	if(display == SLR_MAIN_DISPLAY){
		do{
        	...
        	while(!(MDOMAIN_CAP_Cap_DDR_FIFOStatus_get_in1_cap_last_wr_flag(IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg))) && --count){
                ...
                {
                    rtd_pr_scaler_memory_notice(KERN_NOTICE "%s m cap is disable, return!", __FUNCTION__);
                    return;
                }
                 #ifdef BUILD_QUICK_SHOW
...
//			IoReg_SetBits(PPOVERLAY_iv2dv_Double_Buffer_CTRL_reg,_BIT2);
			...
			rtd_pr_new_game_mode_notice("###[%s(%d)][Game Mode] id2dv_delay=0x%x\n", __func__, __LINE__, id2dv_delay);
...
unsigned int drv_GameMode_decided_iv2dv_delay_new_mode(void)
{
	...
	rtd_pr_scaler_memory_emerg("###[%s(%d)] iv2dv_delay = %d\n", __func__, __LINE__, id2dv_delay);
	return id2dv_delay;
...
void drv_memory_capture_multibuffer_update_boundary(unsigned char buf_num, unsigned int buffer_size){
    ...
    min_addr= MemTag[MEMIDX_MAIN].StartAddr;
    max_addr= MemTag[MEMIDX_MAIN].StartAddr;

    for(i = MEMIDX_MAIN; i < (MEMIDX_MAIN + drv_memory_Get_multibuffer_number()); i++)
    {
    ...
    rtd_pr_scaler_memory_emerg("[videodelay] low=%x, high=%x", min_addr, max_addr);
    IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg,(min_addr & 0xfffffff0));
    IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,(max_addr+buffer_size-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
    IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg,(min_addr & 0xfffffff0));
    IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg,(max_addr+buffer_size-CONFIG_MDOMAIN_BURST_SIZE) & 0xfffffff0);
}
...
void drv_memory_cap_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	if(display == SLR_MAIN_DISPLAY){
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg, ((MemTag[MEMIDX_MAIN].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg, ((MemTag[MEMIDX_MAIN].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg, ((MemTag[MEMIDX_MAIN_SEC].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg, ((MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg, ((MemTag[MEMIDX_MAIN_THIRD].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg, ((MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
	}
	else if(display == SLR_SUB_DISPLAY){
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg, ((MemTag[MEMIDX_SUB].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, ((MemTag[MEMIDX_SUB].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, ((MemTag[MEMIDX_SUB_SEC].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, ((MemTag[MEMIDX_SUB_SEC].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK2_reg, ((MemTag[MEMIDX_SUB_THIRD].StartAddr - FINAL_BURST_LENTH) & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK2_reg, ((MemTag[MEMIDX_SUB_THIRD].StartAddr+buffer_size) - FINAL_BURST_LENTH) & 0xfffffff0);
	}
}
