...
void memory_set_subcapture_vt(unsigned char bDouble, eMemCapAccessType Cap_type, unsigned int capWid, unsigned int capLen)
{
	...
	if(bDouble)
	{
		IoReg_Write32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg, CaptureCtrl_VT.cap_buffer[1].phyaddr & 0xfffffff0);
	}

	IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, CaptureCtrl_VT.cap_buffer[0].phyaddr & 0xfffffff0);
	rtd_pr_vt_debug("###################sub buffer TotalSize=%d;;\n", buffer_size);

	/*restore buffer freeze use at MDOMAIN_CAP_DDR_In2_3rdAddr_reg*/
	IoReg_Write32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg, CaptureCtrl_VT.cap_buffer[2].phyaddr & 0xfffffff0);

	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg, (CaptureCtrl_VT.cap_buffer[0].phyaddr & 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, (CaptureCtrl_VT.cap_buffer[0].phyaddr+CaptureCtrl_VT.cap_buffer[0].size) & 0xfffffff0);
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, (CaptureCtrl_VT.cap_buffer[1].phyaddr & 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, (CaptureCtrl_VT.cap_buffer[1].phyaddr+CaptureCtrl_VT.cap_buffer[0].size) & 0xfffffff0);
...
void drvif_vt_color_ultrazoom_set_scale_down(SIZE *ptInSize, SIZE *ptOutSize)
{
	...
	__maybe_unused signed short *coef_pt;
	unsigned int nFactor;
...
void drvif_sub_mdomain_blk_cap_for_Vt(unsigned int capWid, unsigned int capLen, unsigned int data_fmt)
{
	...
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, (CaptureCtrl_VT.cap_buffer[0].phyaddr + CaptureCtrl_VT.cap_buffer[0].size) & 0xfffffff0);
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, (CaptureCtrl_VT.cap_buffer[1].phyaddr & 0xfffffff0));
	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, (CaptureCtrl_VT.cap_buffer[1].phyaddr + CaptureCtrl_VT.cap_buffer[1].size) & 0xfffffff0);

	IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK2_reg, (CaptureCtrl_VT.cap_buffer[2].phyaddr & 0xfffffff0));
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	__maybe_unused struct timespec64 timeout_s;
#else
	struct timespec timeout_s;
#endif
