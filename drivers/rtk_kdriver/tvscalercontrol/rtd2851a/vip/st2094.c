...
{
	#define OETF_DMA_ERR_Status_Check_NUM 30
	...
	check_times = 0;
	while ((drvif_DM2_EOTF_OETF_3DLUT_DMA_Err_Check() != 0) && (check_times<OETF_DMA_ERR_Status_Check_NUM)) 
	{
		drvif_DM2_EOTF_OETF_3DLUT_DMA_Err_Reset();
		check_times++;
	};
	
	if (check_times >= OETF_DMA_ERR_Status_Check_NUM) {
		rtd_pr_vpq_emerg("DM_OETF_DMA, 10+ can't reset OETF Err status, check times = %d,\n", check_times);
		drvif_DM2_EOTF_OETF_3DLUT_DMA_Err_Check();
	}
	else
	{
...
void drvif_fwif_color_ScalerVIP_set_ST2094_metadata2reg_frameSync(int display_Brightness, _RPC_system_setting_info *RPC_sys_info, VIP_ST2094_CTRL* ST2094_CTRL)
{
	...
	//	drvif_color_Set_ST2094_3Dlut_CInv(gain, 0, 10);
...
#else
	if(dim == 0xFF) {	// write register with val
		threedlut_minmaxinv1_reg.idistc1inv = gain;
		threedlut_minmaxinv2_reg.idistc2inv = gain;
		threedlut_minmaxinv3_reg.idistc3inv = gain;
	} 
	else if( dim == 1 ) // 24x24x24 LUT
	{
