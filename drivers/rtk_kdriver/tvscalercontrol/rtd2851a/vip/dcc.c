...
unsigned char drvif_color_dcc_db_switch(void)
{
	...
	do {
		color_dcc_d_dcc_ctrl_reg.regValue = rtd_inl(COLOR_DCC_D_DCC_CTRL_reg);
		timecnt++;
		} while (color_dcc_d_dcc_ctrl_reg.main_table_sel != color_dcc_sram_tab_ctl_0_reg.sram_tab_acc_sel && timecnt < 0x032500);
...
unsigned char drvif_color_dcc_Curve_Write(unsigned char display, unsigned char CurveSegNum, unsigned int *sAccLUT)
{
	...
	__attribute__((unused)) unsigned short Base = 0;
