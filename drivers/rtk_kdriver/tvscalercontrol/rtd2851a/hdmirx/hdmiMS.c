...
unsigned int lib_hdmims_calc_freq100(HDMI_TIMING_T *tm)
{
	unsigned long long Q, D;
	unsigned int v_freq = 0;

	if (tm->tmds_clk_b < 100 || tm->v_total < 100 || 0 == tm->h_total)
		return 0;

	// implement Q = (27000000 * hdmi.b / 256 * RatioM) / RatioN
	Q = 27000000ULL * 100ULL * dpll_ratio[tm->colordepth].RatioM;
	D = dpll_ratio[tm->colordepth].RatioN * 256ULL;
	do_div(Q, D);
