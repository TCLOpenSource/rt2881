...
void newbase_hdmi_scdc_flow_print(unsigned char port)
{
    ...
    if (hdmi_char_bit_debug_print_cnt[port] >= g_char_bit_print_cycle_cnt_thd)
    {
        ...
        else
        {
            ...
            CEDERR_FLOW_PRINTF("Signal STS Port[%d]: PhyState=%d; CHAR(b,g,r)=(%d,%d,%d)); SCDC(b,g,r)=(%d,%d,%d); BCH_ERR cnt(1,2)=(%d,%d); hw_reauth=%d; AVMUTE=%d; BGRHV=0x%x; COMMCRC=0x%x; FSM=%s(%d), PP_CRC=0x%08x\n",
                port,
                phy_proc_state,
                char_b_err,
                char_g_err,
