...
void newbase_hdmi_bch_error_detection_error_handler(unsigned char port, unsigned char frl_mode, unsigned char* rest_phy)
{
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        HDCP_INFO("HDMI[p%d][BCH] HDCP mode:%s, RxStatus(%02x, %02x), hdcp_enc=%d, bch err 1bit=%d, 2bit err (detect,duration,cnt)=(%d, %d, %d), bch_pkt_value=%d, I2c_status=%d, hw_reauth=%d\n", 
