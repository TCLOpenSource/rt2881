...
unsigned char fwif_color_get_colorspacergb2yuvtransfer_table_tv006(unsigned char channel, unsigned char nSrcType, unsigned char nSD_HD, bool bUse_HDMI_Range_Detect)
{
	...
	if (fwif_color_get_force_run_i3ddma_enable(channel)) {
		if (nSrcType == _SRC_HDMI) {
			...
			if (drvif_IsHDMI() && drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))) != 0 && (Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_MODECURR) != _MODE_NEW))
	...
	rtd_pr_vpq_info("drvif_color_get_colorspacergb2yuvtransfer_table_tv006, nSrcType = %d, nSD_HD = %d\n", nSrcType, nSD_HD);
	...
	if (VIP_system_info_structure_table->BT2020_CTRL.Enable_Flag == 1 && VIP_system_info_structure_table->BT2020_CTRL.Mode == BT2020_MODE_Constant) {
        ...
        rtd_pr_vpq_info("ori_CS=%d,new_CS=%d,%s->%d,%s\n",drvif_Hdmi_GetColorSpace(),drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))), __FILE__, __LINE__, __FUNCTION__);
			...
			rtd_pr_vpq_info(" == =  RGB2YUV video_format = %d == = 	\n", video_format);
			if (CHOOSE_DATA_RANGE) {
				data_range = drvif_IsRGB_YUV_RANGE();  /*0 :default 1 : Limited Range 2: Full Range*/
				rtd_pr_vpq_info(" == =  RGB2YUV  DATA_RANGE = %d  (0 :default 1 : Limited Range 2: Full Range) == =   \n", data_range);
			}
			/* get csc*/
			...
			/*which_tab = 0;*/
			rtd_pr_vpq_info("unhandler type%d\n", nSrcType);
			table_num = RGB2YUV_COEF_BYPASS_NORMAL_GAIN;
			VIP_system_info_structure_table->using_YUV2RGB_Matrix_Info.Input_Data_Mode = YUV2RGB_INPUT_601_Limted;
			/*return NULL;*/
	  }
	rtd_pr_vpq_info("ch=%d, Src=%d, nSD_HD=%d, HDMI_Range_Detect=%d, blk_lv=%d, force_run_i3ddma_en()=%d, IsHDMI()=%d, Hdmi_ColorSpace()=%d\n",
		channel, nSrcType, nSD_HD, bUse_HDMI_Range_Detect, blk_lv, fwif_color_get_force_run_i3ddma_enable(channel), drvif_IsHDMI(), drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))));
        rtd_pr_vpq_info("ori_CS=%d,new_CS=%d,%s->%d,%s\n",drvif_Hdmi_GetColorSpace(),drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))), __FILE__, __LINE__, __FUNCTION__);
...
void fwif_color_Set_MPEGNR_tv006(unsigned char level)
{
	...
	if (pMPEGNR_Table->MPEG_LEVEL < 4) {
		...
		//pNR_ITEM->S_SNR_TABLE.S_SNR_Mosquito_NR.SNR_blendsnr_byedge_en = pMPEGNR_Table->MNR_TABLE.MNR_T.BlendByEdge_enable; //mac9q no this
		...
		//drvif_color_DRV_SNR_Mosquito_NR_Oneside_Edge(&(pNR_ITEM->S_SNR_TABLE.S_SNR_MNR_Oneside_Edge)); //merlin5 add //mac9q no this
...
int fwif_color_setCon_Bri_Color_Tint_tv006(unsigned char display, _system_setting_info *system_info_struct)
{
	...
	for (i = 0; i < VIP_YUV2RGB_Y_Seg_Max; i++) {
		...
		table_idx[1][0][i] = tYUV2RGB_COEF[YUV2RGB_TBL_Idx][tUV2RGB_COEF_K21];
		...
		table_idx[2][0][i] = tYUV2RGB_COEF[YUV2RGB_TBL_Idx][tUV2RGB_COEF_K31];
...
void fwif_color_set_sharpness_table_TV006(CHIP_SHARPNESS_UI_T *ptCHIP_SHARPNESS_UI_T, CHIP_SR_UI_T *ptCHIP_SR_UI_T)
{
	...
	unsigned short value = Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY); /* Get from VIP Table */
	...
	sharp.Edge_Shp_coef1.en_edg = gVip_Table->Ddomain_SHPTable[value].Edge_Shp_coef1.en_edg;
	sharp.Texture_Shp_coef1.en_tex = gVip_Table->Ddomain_SHPTable[value].Texture_Shp_coef1.en_tex;

	sharp.Edge_Shp_set.Edg_D2_Shift_Bit = gVip_Table->Ddomain_SHPTable[value].Edge_Shp_set.Edg_D2_Shift_Bit;
	...
	sharp.Vertical_edg.enable= gVip_Table->Ddomain_SHPTable[value].Vertical_edg.enable;//merlin
	sharp.Vertical_edg.LV2= gVip_Table->Ddomain_SHPTable[value].Vertical_edg.LV2;//merlin
	...
	sharp.Edge_EMF_Table.Seg_2_Offset = gVip_Table->Ddomain_SHPTable[value].Edge_EMF_Table.Seg_2_Offset;
 	sharp.Texture_EMF_Table.Seg_1_Offset = gVip_Table->Ddomain_SHPTable[value].Texture_EMF_Table.Seg_1_Offset;
	sharp.Texture_EMF_Table.Seg_2_Offset = gVip_Table->Ddomain_SHPTable[value].Texture_EMF_Table.Seg_2_Offset;

 	sharp.TwoD_Peak_AdaptCtrl2.Edge_Filter_Sel = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.Edge_Filter_Sel;
	sharp.TwoD_Peak_AdaptCtrl2.PkDlti_comsign = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.PkDlti_comsign;
	sharp.TwoD_Peak_AdaptCtrl2.edg_dlti = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edg_dlti;
	sharp.TwoD_Peak_AdaptCtrl2.edg_dlti_tex = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edg_dlti_tex;
	sharp.TwoD_Peak_AdaptCtrl2.zdiffl_en = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.zdiffl_en;
	sharp.TwoD_Peak_AdaptCtrl2.zdiffl_lowbnd = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.zdiffl_lowbnd;
	sharp.TwoD_Peak_AdaptCtrl2.zdiffl_stepbit = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.zdiffl_stepbit;
	sharp.TwoD_Peak_AdaptCtrl2.edge_mono_en = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edge_mono_en;
	sharp.TwoD_Peak_AdaptCtrl2.edge_mono_stepbit = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edge_mono_stepbit;
	sharp.TwoD_Peak_AdaptCtrl2.edge_mono_stepbiu = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edge_mono_stepbiu;
	sharp.TwoD_Peak_AdaptCtrl2.edge_mono_upbnd = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edge_mono_upbnd;
	sharp.TwoD_Peak_AdaptCtrl2.edge_mono_lowbnd = gVip_Table->Ddomain_SHPTable[value].TwoD_Peak_AdaptCtrl2.edge_mono_lowbnd;

 	sharp.Slope_gain_mask.lowbound= gVip_Table->Ddomain_SHPTable[value].Slope_gain_mask.lowbound;//merlin3
	sharp.Slope_gain_mask.lowbound_v= gVip_Table->Ddomain_SHPTable[value].Slope_gain_mask.lowbound_v;//merlin3
	sharp.Slope_gain_mask.use_ramp= gVip_Table->Ddomain_SHPTable[value].Slope_gain_mask.use_ramp;//merlin3
	sharp.Slope_gain_mask.extend2= gVip_Table->Ddomain_SHPTable[value].Slope_gain_mask.extend2;//merlin3//juwen, delete
	...
	sharp.After_Filter.Rgblimit_neg= gVip_Table->Ddomain_SHPTable[value].After_Filter.Rgblimit_neg;
 	...
 	sharp.SR_Control.detail_tex_en = gVip_Table->Ddomain_SHPTable[value].SR_Control.detail_tex_en;
	sharp.SR_Control.detail_edg_en = gVip_Table->Ddomain_SHPTable[value].SR_Control.detail_edg_en;
	sharp.SR_Control.detail_v_tex_en = gVip_Table->Ddomain_SHPTable[value].SR_Control.detail_v_tex_en;
	sharp.SR_Control.detail_v_edg_en = gVip_Table->Ddomain_SHPTable[value].SR_Control.detail_v_edg_en;
	...
	sharp.Sub_Gain_Modify.EDG_Gain_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.EDG_Gain_Pos;
	sharp.Sub_Gain_Modify.EDG_Gain_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.EDG_Gain_Neg;
	sharp.Sub_Gain_Modify.EDG_HV_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.EDG_HV_Pos;
	sharp.Sub_Gain_Modify.EDG_HV_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.EDG_HV_Neg;
	sharp.Sub_Gain_Modify.EDG_LV = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.EDG_LV;
	sharp.Sub_Gain_Modify.TEX_Gain_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.TEX_Gain_Pos;
	sharp.Sub_Gain_Modify.TEX_Gain_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.TEX_Gain_Neg;
	sharp.Sub_Gain_Modify.TEX_HV_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.TEX_HV_Pos;
	sharp.Sub_Gain_Modify.TEX_HV_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.TEX_HV_Neg;
	sharp.Sub_Gain_Modify.TEX_LV = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.TEX_LV;
	sharp.Sub_Gain_Modify.V_Gain_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_Gain_Pos;
	sharp.Sub_Gain_Modify.V_Gain_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_Gain_Neg;
	sharp.Sub_Gain_Modify.V_HV_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_HV_Pos;
	sharp.Sub_Gain_Modify.V_HV_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_HV_Neg;
	sharp.Sub_Gain_Modify.V_LV = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_LV;
	sharp.Sub_Gain_Modify.V_EDG_Gain_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_EDG_Gain_Pos;
	sharp.Sub_Gain_Modify.V_EDG_Gain_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_EDG_Gain_Neg;
	sharp.Sub_Gain_Modify.V_EDG_HV_Pos = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_EDG_HV_Pos;
	sharp.Sub_Gain_Modify.V_EDG_HV_Neg = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_EDG_HV_Neg;
	sharp.Sub_Gain_Modify.V_EDG_LV = gVip_Table->Ddomain_SHPTable[value].Sub_Gain_Modify.V_EDG_LV;
	...
	sharp_other_ctrl.edgpk_en               = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.edgpk_en;
        sharp.TwoD_Peak_AdaptCtrl2.Edge_Filter_Sel         = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.edge_select_filter;//default value is 0
        sharp_other_ctrl.edg_lpf_shift          = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.edge_lpf_shift;//default value is 8


    sharp.Edge_Shp_coef1.C0	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_0;
	sharp.Edge_Shp_coef1.C1	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c1_0;
	sharp.Edge_Shp_coef1.C2	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c2_0;
	sharp.Edge_Shp_coef1.C3	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c3_0;
	sharp.Edge_Shp_coef1.C4	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c4_0;
	sharp.Edge_Shp_coef1.C5	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c5_0;
	sharp.Edge_Shp_coef1.C6	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c6_0;
	sharp.Edge_Shp_coef1.C7	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c7_0;
	sharp.Edge_Shp_coef1.C8	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c8_0;
	sharp.Edge_Shp_coef1.C9	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c9_0;
	sharp.Edge_Shp_coef1.C10	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c10_0;
	sharp.Edge_Shp_coef1.C0H1	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_1;
	sharp.Edge_Shp_coef1.C0H2	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_2;
	sharp.Edge_Shp_coef1.C0H3	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_3;
	sharp.Edge_Shp_coef1.C0H4	= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeHPF.coef_c0_4;
	...
	sharp.Edge_Neg_Curve.G14		= ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeCurveMappingUI.neg_g14;
      ...
      //  sharp.TwoD_tex.en_tex            = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.texpk_en;
        ...
        sharp.SR_Control.Tex_Filter_Sel   = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.text_select_filter;
        sharp_other_ctrl.tex_lpf_shift   = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.tex_lpf_shift;
        sharp.Texture_Shp_coef1.C0                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_0;
        sharp.Texture_Shp_coef1.C1                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c1_0;
        sharp.Texture_Shp_coef1.C2                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c2_0;
        sharp.Texture_Shp_coef1.C3                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c3_0;
        sharp.Texture_Shp_coef1.C4                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c4_0;
        sharp.Texture_Shp_coef1.C5                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c5_0;
        sharp.Texture_Shp_coef1.C6                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c6_0;
        sharp.Texture_Shp_coef1.C7                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c7_0;
        sharp.Texture_Shp_coef1.C8                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c8_0;
        sharp.Texture_Shp_coef1.C9                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c9_0;
        sharp.Texture_Shp_coef1.C10               = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c10_0;
        sharp.Texture_Shp_coef1.C0H1              = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_1;
        sharp.Texture_Shp_coef1.C0H2              = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_2;
        sharp.Texture_Shp_coef1.C0H3              = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_3;
        sharp.Texture_Shp_coef1.C0H4              = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureHPF.coef_c0_4;
	...
	sharp.Tex_Neg_Curve.G14			 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureCurveMappingUI.neg_g14;
        ...
        sharp.Slope_gain_mask.flat_method              = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.FH;
        ...
        sharp.Slope_gain_mask.flat_range_h             = ptCHIP_SHARPNESS_UI_T->stSharpness.stSlopeGainMask.flat_range;
	...
	sharp.Slope_gain_mask.flat_range_v		 = ptCHIP_SHARPNESS_UI_T->stSharpness.stVerticalSlopeGainMask.flat_range_v;
	...
	sharp.Texture_Flat_Shp_coef1.en_tex	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.tex_flat_filter_en;
	sharp.Texture_Flat_Shp_coef1.C0H1 	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c0_2;
	sharp.Texture_Flat_Shp_coef1.C0H2     = ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c0_1;
	sharp.Texture_Flat_Shp_coef1.C0	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c0_0;
	sharp.Texture_Flat_Shp_coef1.C1	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c1_0;
	sharp.Texture_Flat_Shp_coef1.C2	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c2_0;
	sharp.Texture_Flat_Shp_coef1.C3	= ptCHIP_SHARPNESS_UI_T->stSharpness.stFlatfilter.coef_c3_0;
       ...
       sharp.Edge_EMF_Table.EMF_fixentent_neg  = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeEMF.fix_extend_neg;
       sharp.Edge_EMF_Table.EMF_fixentent_pos  = ptCHIP_SHARPNESS_UI_T->stSharpness.stEdgeEMF.fix_extend_pos;
       ...
       sharp.Texture_EMF_Table.EMF_En             = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.emf_enable;
       sharp.Texture_EMF_Table.EMF_Range          = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.emf_range;
       sharp.Texture_EMF_Table.EMF_Shift          = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.emf_shiftbit;
       sharp.Texture_EMF_Table.Blend_wt           = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.blend_wt;
       sharp.Texture_EMF_Table.EMF_fixentent_neg  = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.fix_extend_neg;
       sharp.Texture_EMF_Table.EMF_fixentent_pos  = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.fix_extend_pos;
       sharp.Texture_EMF_Table.Seg_0_x            = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.seg0_x;
       sharp.Texture_EMF_Table.Seg_1_x            = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.seg1_x;
       sharp.Texture_EMF_Table.Seg_0_Gain         = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.gain0;
       sharp.Texture_EMF_Table.Seg_1_Gain         = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.gain1;
       sharp.Texture_EMF_Table.Seg_2_Gain         = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.gain2;
       sharp.Texture_EMF_Table.Seg_0_Offset       = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.Offset0;
       sharp.Texture_EMF_Table.EMF_texgain        = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.emf_texgain;
       sharp.Texture_EMF_Table.v5                 = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.v5;
       sharp.Texture_EMF_Table.h11                = ptCHIP_SHARPNESS_UI_T->stSharpness.stTextureEMF.h11;
	...
	fwif_color_set_sharpness_ms_ctrl_sharp_bySubFollowMain(SLR_MAIN_DISPLAY, sharp.TwoD_Peak_AdaptCtrl.Peaking_Enable);
...
void fwif_color_Get_sharpness_table_TV006(short *xTmp)
{
...

	{
		...
		xTmp[13] = pSharp->Edge_Coring.Edge_Coring_En;
		xTmp[14] = pSharp->Edge_Coring.Edge_Gain_En;
		xTmp[15] = pSharp->Edge_Coring.Bigedge_Ratio;
		xTmp[16] = pSharp->Edge_Coring.Bigedge_Ratio;
		xTmp[17] = pSharp->Edge_Coring.Deltaedge_Ratio;
		xTmp[18] = pSharp->Edge_Coring.Smalledge_Ulevel;
		xTmp[19] = pSharp->Edge_Coring.DeltaEdge_Ulevel;
		xTmp[20] = pSharp->Edge_Coring.Edge_Coring_Clip;
		xTmp[21] = pSharp->Edge_Coring.Edge_Gain_Bound;
		xTmp[22] = pSharp->Edge_Coring.Edge_Ser_Range;
		xTmp[23] = pSharp->Edge_Coring.Edge_Delta_Ext_Range;
		...
		xTmp[24] = pSharp->Y_Remap.LinearY_UM;
		xTmp[25] = pSharp->Y_Remap.LinearY_LM;
		xTmp[26] = pSharp->Y_Remap.LinearY_UB;
		xTmp[27] = pSharp->Y_Remap.LinearY_LB;
		...
		xTmp[29] = pSharp->TwoD_Peak_AdaptCtrl2.Edge_Filter_Sel; //must be 0
		xTmp[30] = sharp_other_ctrl.edg_lpf_shift; //must be 8
		xTmp[31] = pSharp->Edge_Shp_coef1.C0;
		xTmp[32] = pSharp->Edge_Shp_coef1.C1;
		xTmp[33] = pSharp->Edge_Shp_coef1.C2;
		xTmp[34] = pSharp->Edge_Shp_coef1.C3;
		xTmp[35] = pSharp->Edge_Shp_coef1.C4;
		xTmp[36] = pSharp->Edge_Shp_coef1.C5;
		xTmp[37] = pSharp->Edge_Shp_coef1.C6;
		xTmp[38] = pSharp->Edge_Shp_coef1.C7;
		xTmp[39] = pSharp->Edge_Shp_coef1.C8;
		xTmp[40] = pSharp->Edge_Shp_coef1.C9;
		xTmp[41] = pSharp->Edge_Shp_coef1.C10;
		xTmp[42] = pSharp->Edge_Shp_coef1.C0H1;
		xTmp[43] = pSharp->Edge_Shp_coef1.C0H2;
		xTmp[44] = pSharp->Edge_Shp_coef1.C0H3;
		xTmp[45] = pSharp->Edge_Shp_coef1.C0H4;
		...
		xTmp[114] = pSharp->SR_Control.Tex_Filter_Sel;
		xTmp[115] = sharp_other_ctrl.tex_lpf_shift;
		xTmp[116] = pSharp->Texture_Shp_coef1.C0;
		xTmp[117] = pSharp->Texture_Shp_coef1.C1;
		xTmp[118] = pSharp->Texture_Shp_coef1.C2;
		xTmp[119] = pSharp->Texture_Shp_coef1.C3;
		xTmp[120] = pSharp->Texture_Shp_coef1.C4;
		xTmp[121] = pSharp->Texture_Shp_coef1.C5;
		xTmp[122] = pSharp->Texture_Shp_coef1.C6;
		xTmp[123] = pSharp->Texture_Shp_coef1.C7;
		xTmp[124] = pSharp->Texture_Shp_coef1.C8;
		xTmp[125] = pSharp->Texture_Shp_coef1.C9;
		xTmp[126] = pSharp->Texture_Shp_coef1.C10;
		xTmp[127] = pSharp->Texture_Shp_coef1.C0H1;
		xTmp[128] = pSharp->Texture_Shp_coef1.C0H2;
		xTmp[129] = pSharp->Texture_Shp_coef1.C0H3;
		xTmp[130] = pSharp->Texture_Shp_coef1.C0H4;
		...
		xTmp[161] = pSharp->Texture_Shp_set.G_Pos2;
		xTmp[162] = pSharp->Texture_Shp_set.G_Neg2;
		...
		xTmp[166] = pSharp->Texture_Shp_set.Edg_D2_Shift_Bit;
		...
		xTmp[236] = pSharp->Slope_gain_mask.Slope_gain_en  ;
		xTmp[237] = pSharp->Slope_gain_mask.Slope_gain_en;
		xTmp[238] = pSharp->Slope_gain_mask.flat_method ;
		xTmp[239] = pSharp->Slope_gain_mask.Gain_boundPos;
		xTmp[240] = pSharp->Slope_gain_mask.Gain_boundNeg;
		xTmp[241] = pSharp->Slope_gain_mask.Gain_externdPos ;
		xTmp[242] = pSharp->Slope_gain_mask.Gain_externdNeg ;
		xTmp[243] = pSharp->Slope_gain_mask.flat_enable;
		xTmp[244] = pSharp->Slope_gain_mask.flat_offset;
		xTmp[245] = pSharp->Slope_gain_mask.flat_gain ;
		xTmp[246] = pSharp->Slope_gain_mask.flat_range_h;
		...
		xTmp[292]= pSharp->Vertical_edg.G_Pos2;
		xTmp[293]= pSharp->Vertical_edg.G_Neg2;
		...
		xTmp[329]= pSharp->Slope_gain_mask.v_slope_gain_en;
		xTmp[330]= pSharp->Slope_gain_mask.dir_v ;
		xTmp[331]= pSharp->Slope_gain_mask.gain_boundpos_v;
		xTmp[332]= pSharp->Slope_gain_mask.gain_boundneg_v;
		xTmp[333]= pSharp->Slope_gain_mask.gain_extendpos_v;
		xTmp[334]= pSharp->Slope_gain_mask.gain_extendneg_v;
		xTmp[335]= pSharp->Slope_gain_mask.flat_v_enable;
		xTmp[336]= pSharp->Slope_gain_mask.flat_offset_v;
		xTmp[337]= pSharp->Slope_gain_mask.flat_gain_v;
		xTmp[338]= pSharp->Slope_gain_mask.flat_range_v;
		...
		xTmp[339]= pSharp->Gain_By_Y.Gain_By_Y_En;
		xTmp[340]= pSharp->Gain_By_Y.gain_by_y_v_en;
		xTmp[341]= pSharp->Gain_By_Y.gain_by_y_tex_en;
		xTmp[342]= pSharp->Gain_By_Y.gain_by_y_edg_en;
		xTmp[343]= pSharp->Max_Min_Lpf.Weigh_En;
		xTmp[344]= pSharp->Max_Min_Lpf.lpf_weigh_edg_en;
		xTmp[345]= pSharp->Max_Min_Lpf.lpf_weigh_tex_en;
		xTmp[346]= pSharp->Max_Min_Gain.Max_Min_Gain_En;
		xTmp[347]= pSharp->Max_Min_Gain.max_min_gain_v_en ;
		xTmp[348]= pSharp->Max_Min_Gain.max_min_gain_tex_en;
		xTmp[349]= pSharp->Max_Min_Gain.max_min_gain_edg_en;
		xTmp[350]= pSharp->Gain_By_Y.lv_by_y_enable;
		xTmp[351]= pSharp->Gain_By_Y.lv_by_y_edg_enable;
		xTmp[352]= pSharp->Gain_By_Y.lv_by_y_v_edg_enable;
		xTmp[353]= pSharp->Gain_By_Y.lv_by_y_tex_enable;
		xTmp[354]= pSharp->Gain_By_Y.lv_by_y_v_enable;
		xTmp[355]= pSharp->Mk3_Lpf.C0 ;
		xTmp[356]= pSharp->Mk3_Lpf.C1;
		xTmp[357]= pSharp->Mk3_Lpf.C2;
		xTmp[358]= pSharp->Mk3_Lpf.C3;
		xTmp[359]= pSharp->Mk3_Lpf.C4;
		xTmp[360]= pSharp->Mk3_Lpf.C5;
		xTmp[361]= pSharp->Max_Min_Gain.Step_1;
		xTmp[362]= pSharp->Max_Min_Gain.Step_2;
		xTmp[363]= pSharp->Max_Min_Gain.Step_3;
		xTmp[364]= pSharp->Max_Min_Gain.Step_4;
		xTmp[365]= pSharp->Max_Min_Gain.Step_5;
		xTmp[366]= pSharp->Max_Min_Gain.Step_6;
		xTmp[367]= pSharp->Max_Min_Gain.Gain_0;
		xTmp[368]= pSharp->Max_Min_Gain.Gain_1;
		xTmp[369]= pSharp->Max_Min_Gain.Gain_2;
		xTmp[370]= pSharp->Max_Min_Gain.Gain_3;
		xTmp[371]= pSharp->Max_Min_Gain.Gain_4;
		xTmp[372]= pSharp->Max_Min_Gain.Gain_5;
		xTmp[373]= pSharp->Max_Min_Gain.Gain_6;
		xTmp[374]= pSharp->Max_Min_Lpf.Step_1;
		xTmp[375]= pSharp->Max_Min_Lpf.Step_2;
		xTmp[376]= pSharp->Max_Min_Lpf.Step_3;
		xTmp[377]= pSharp->Max_Min_Lpf.Step_4;
		xTmp[378]= pSharp->Max_Min_Lpf.Step_5;
		xTmp[379]= pSharp->Max_Min_Lpf.Step_6;
		xTmp[380]= pSharp->Max_Min_Lpf.Weit_0;
		xTmp[381]= pSharp->Max_Min_Lpf.Weit_1;
		xTmp[382]= pSharp->Max_Min_Lpf.Weit_2;
		xTmp[383]= pSharp->Max_Min_Lpf.Weit_3;
		xTmp[384]= pSharp->Max_Min_Lpf.Weit_4;
		xTmp[385]= pSharp->Max_Min_Lpf.Weit_5;
		xTmp[386]= pSharp->Max_Min_Lpf.Weit_6;
		xTmp[387]= pSharp->Gain_By_Y.Step_1;
		xTmp[388]= pSharp->Gain_By_Y.Step_2;
		xTmp[389]= pSharp->Gain_By_Y.Step_3;
		xTmp[390]= pSharp->Gain_By_Y.Step_4;
		xTmp[391]= pSharp->Gain_By_Y.Step_5;
		xTmp[392]= pSharp->Gain_By_Y.Step_6;
		xTmp[393]= pSharp->Gain_By_Y.Gain_0;
		xTmp[394]= pSharp->Gain_By_Y.Gain_1;
		xTmp[395]= pSharp->Gain_By_Y.Gain_2;
		xTmp[396]= pSharp->Gain_By_Y.Gain_3;
		xTmp[397]= pSharp->Gain_By_Y.Gain_4;
		xTmp[398]= pSharp->Gain_By_Y.Gain_5;
		xTmp[399]= pSharp->Gain_By_Y.Gain_6;
	...
	xTmp[507] = pSharp->Texture_Flat_Shp_coef1.en_tex;
	xTmp[508] = pSharp->Texture_Flat_Shp_coef1.C0H1;
	xTmp[509] = pSharp->Texture_Flat_Shp_coef1.C0H2;
	xTmp[510] = pSharp->Texture_Flat_Shp_coef1.C0;
	xTmp[511] = pSharp->Texture_Flat_Shp_coef1.C1;
	xTmp[512] = pSharp->Texture_Flat_Shp_coef1.C2;
	xTmp[513] = pSharp->Texture_Flat_Shp_coef1.C3;
	...
	xTmp[518] = pSharp->Edge_EMF_Table.EMF_fixentent_neg;
	xTmp[519] = pSharp->Edge_EMF_Table.EMF_fixentent_pos;
	...
	xTmp[528] = pSharp->Texture_EMF_Table.EMF_En;
	xTmp[529] = pSharp->Texture_EMF_Table.EMF_Range;
	xTmp[530] = pSharp->Texture_EMF_Table.EMF_Shift;
	xTmp[531] = pSharp->Texture_EMF_Table.Blend_wt;
	xTmp[532] = pSharp->Texture_EMF_Table.EMF_fixentent_neg;
	xTmp[533] = pSharp->Texture_EMF_Table.EMF_fixentent_pos;
	xTmp[534] = pSharp->Texture_EMF_Table.Seg_0_x;
	xTmp[535] = pSharp->Texture_EMF_Table.Seg_1_x;
	xTmp[536] = pSharp->Texture_EMF_Table.Seg_0_Gain;
	xTmp[537] = pSharp->Texture_EMF_Table.Seg_1_Gain;
	xTmp[538] = pSharp->Texture_EMF_Table.Seg_2_Gain;
	xTmp[539] = pSharp->Texture_EMF_Table.Seg_0_Offset;
	xTmp[540] = pSharp->Texture_EMF_Table.EMF_texgain;
	xTmp[541] = pSharp->Texture_EMF_Table.v5;
	xTmp[542] = pSharp->Texture_EMF_Table.h11;
...
char fwif_color_set_DDomain_3dLUT_32_TV006(unsigned int *p, unsigned char enable)
{
	...
	if (enable == 0 || p == NULL)
		...
		ret = fwif_color_set_D_3dLUT(enable, p);
	...
	if (enable == 0 || p == NULL) {
		...
		if (check_times >= check_times_max) {
			...
			ret = fwif_color_D_LUT_By_DMA(0, enable, p);	
			if (ret < 0) {
...
char fwif_color_set_bt2020_Handler_tv006(unsigned char display, unsigned char Enable_Flag, unsigned char bt2020_Mode)
{
	...
	/* set Inv Gamma*/
	fwif_color_set_Gamma_InvGamma_Ctrl(display, InvGamma_TBL_Sel, invGammaTBL, invGammaTBL, invGammaTBL, 1);
...
void fwif_color_set_DM_HDR_3dLUT_CInv(unsigned int hdr3dlut){
	drvif_color_Set_ST2094_3Dlut_CInv(hdr3dlut, 0XFF, 0);
}
...
void fwif_color_set_HDR10_runmode(void)
{
	...
	unsigned char in_sel=0;
	unsigned char inbits_sel=0;

	I3DDMA_Color_format = drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_TARGET_COLOR);
...
void fwif_color_set_output_gamma_4CHDGA_TV006 (UINT32 *pRedGammaTable, UINT32 * pGreenGammaTable, UINT32 * pBlueGammaTable, UINT32 * pWhiteGammaTable, UINT16 nTableSize)
{
	...
	drvif_color_colorWrite_outGamma_dual_panel( 0, 0, 0, final_OutGAMMA_R_d0d1,final_OutGAMMA_R_index, GAMMA_CHANNEL_R,
													     final_OutGAMMA_R_d0d1,final_OutGAMMA_R_index, GAMMA_CHANNEL_R);

	drvif_color_colorWrite_outGamma_dual_panel( 0, 0, 0, final_OutGAMMA_G_d0d1,final_OutGAMMA_G_index, GAMMA_CHANNEL_G,
													     final_OutGAMMA_G_d0d1,final_OutGAMMA_G_index, GAMMA_CHANNEL_G);
														 
	drvif_color_colorWrite_outGamma_dual_panel( 0, 0, 0, final_OutGAMMA_B_d0d1, final_OutGAMMA_B_index,GAMMA_CHANNEL_B,
													     final_OutGAMMA_B_d0d1, final_OutGAMMA_B_index,GAMMA_CHANNEL_B);
	...
	if ((HDR_MODE != HAL_VPQ_HDR_MODE_HDR10) && (HDR_MODE != HAL_VPQ_HDR_MODE_HLG) && (HDR_MODE != HAL_VPQ_HDR_MODE_ST2094)) {
		...
		for (DMA_set_times = 0;DMA_set_times<DMA_set_times_max;DMA_set_times++) {
			...
			if (HDR_MODE == HAL_VPQ_HDR_MODE_HDR10) {
					...
					//drvif_color_set_HDR_OETF_Lmix(1, OETF_Lmix); //move to Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance
...
UINT8 fwif_color_DM2_EOTF_OETF_3D_LUT_by_HFC(UINT8 HFC_bit, HDR_flow_control_timing HDR_timing)
{
	...
	if ((HDR_control[HDR_timing].EOTF_ctrl.IPCtrlBits) & HFC_bit) {
		...
		if (tbl_index == HFC_EOTF_OFF) {
			...
			Scaler_color_Get_HDR10_EOTF_ByMaxCLL_ByPanelLuminance(0);
		} else if (tbl_index == HFC_EOTF_CTRL_BY_LUMINANCE){ /*HFC OETF special table index, OETF table control by LUMINANCE*/
			Scaler_color_Get_HLG_EOTF_ByMaxCLL_ByPanelLuminance();
		} else {
	...
	if ((HDR_control[HDR_timing].OETF_ctrl.IPCtrlBits) & HFC_bit) {
		...
		} else if (tbl_index == HFC_OETF_CTRL_BY_LUMINANCE){
			drvif_DM2_OETF_Set_Prog_Index(&OETF_prog_idx_tbl[0]);
			drvif_DM2_OETF_Set_Prog_Index_th(OETF_prog_th_tbl[0]);
			Scaler_color_Get_HLG_OETF_ByMaxCLL_ByPanelLuminance();
		} else {
			drvif_DM2_OETF_Set_Prog_Index(&OETF_prog_idx_tbl[0]);
			drvif_DM2_OETF_Set_Prog_Index_th(OETF_prog_th_tbl[0]);
...
void fwif_color_Set_De_Contour_tv006(RTK_DECONTOUR_T *level)
{
        ...
        drvif_color_Set_sub_I_De_Contour(&ptr_i_de);
