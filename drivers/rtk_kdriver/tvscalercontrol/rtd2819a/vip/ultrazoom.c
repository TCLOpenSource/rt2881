...
void DIHSD_ultrazoom_coef_db_apply(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = rtd_inl( HSD_DITHER_DI_UZD_COEF_DB_CTRL_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , HSD_DITHER_DI_UZD_COEF_DB_CTRL_coef_db_apply , 1 );
	rtd_outl( HSD_DITHER_DI_UZD_COEF_DB_CTRL_reg , z_RBUS_reg );
}

void IMAIN_ultrazoom_auto_four_tap(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = rtd_inl( SCALEDOWN_ICH1_UZD_Ctrl1_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , SCALEDOWN_ICH1_UZD_Ctrl1_auto_four_tap_en , 1 );
	rtd_outl( SCALEDOWN_ICH1_UZD_Ctrl1_reg , z_RBUS_reg );
}

void IMAIN_ultrazoom_coef_db_apply(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = rtd_inl( SCALEDOWN_ICH1_UZD_COEF_DB_CTRL_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , SCALEDOWN_ICH1_UZD_COEF_DB_CTRL_coeff_db_apply , 1 );
	rtd_outl( SCALEDOWN_ICH1_UZD_COEF_DB_CTRL_reg , z_RBUS_reg );
}

void IMAIN_ultrazoom_set_hor_ini_desire(
	scaledown_ich1_uzd_initial_value_RBUS* uzd_Initial_Value_REG ,
	scaledown_ich1_uzd_initial_int_value_RBUS* uzd_initial_int_value_REG ,
...
void IMAIN_ultrazoom_set_ver_ini_desire(
	scaledown_ich1_uzd_initial_value_RBUS* uzd_Initial_Value_REG ,
	scaledown_ich1_uzd_initial_int_value_RBUS* uzd_initial_int_value_REG ,
...
void ISUB_ultrazoom_auto_four_tap(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = rtd_inl( SCALEDOWN_ICH2_UZD_Ctrl1_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , SCALEDOWN_ICH2_UZD_Ctrl1_auto_four_tap_en , 1 );
	rtd_outl( SCALEDOWN_ICH2_UZD_Ctrl1_reg , z_RBUS_reg );
}

void ISUB_ultrazoom_coef_db_apply(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = rtd_inl( SCALEDOWN_ICH2_UZD_COEF_DB_CTRL_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , SCALEDOWN_ICH2_UZD_COEF_DB_CTRL_coeff_db_apply , 1 );
	rtd_outl( SCALEDOWN_ICH2_UZD_COEF_DB_CTRL_reg , z_RBUS_reg );
}

void ISUB_ultrazoom_set_hor_ini_desire(
	scaledown_ich2_uzd_initial_value_RBUS* uzd_Initial_Value_REG ,
	scaledown_ich2_uzd_initial_int_value_RBUS* uzd_initial_int_value_REG ,
long long In_Width , long long HOR_FAC , int xdesire )
{
	int Vini = 0;
	if( !uzd_Initial_Value_REG || !uzd_initial_int_value_REG )
...
void ISUB_ultrazoom_set_ver_ini_desire(
	scaledown_ich2_uzd_initial_value_RBUS* uzd_Initial_Value_REG ,
	scaledown_ich2_uzd_initial_int_value_RBUS* uzd_initial_int_value_REG ,
long long In_Height , long long VER_FAC , int xdesire )
{
...
void drvif_color_ultrazoom_Adapt8tapV(unsigned char display, unsigned char V8tap_mode, unsigned char table_sel)
{
	if (display == SLR_MAIN_DISPLAY) {
		...
		if( Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID)>1920 )
		{
...
/*				nFactor = (((unsigned int)(ptInSize->nWidth)<< 21) - ((unsigned int)Hini<<13))  / (ptOutSize->nWidth-1);*/
	    ...
	    rtd_setbits(SCALEUP_DS_UZU_DB_CTRL_reg, SCALEUP_DS_UZU_DB_CTRL_db_apply_mask|SCALEUP_DS_UZU_DB_CTRL_db_en_mask);// DB_APPLY
...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
	...
	scaledown_ich1_ibuff_ctrli_buff_ctrl_RBUS			ich1_ibuff_ctrli_buff_ctrl_REG;
	...
	scaledown_ich2_ibuff_ctrli_buff_ctrl_RBUS		ich2_ibuff_ctrli_buff_ctrl_REG;
	...
	scaledown_ich2_uzd_initial_int_value_RBUS			ich2_uzd_initial_int_value_REG;
#endif //CONFIG_DUAL_CHANNEL

	ich1_uzd_Ctrl0_REG.regValue		= IoReg_Read32(SCALEDOWN_ICH1_UZD_Ctrl0_reg);
	ich1_uzd_Ctrl1_REG.regValue		= IoReg_Read32(SCALEDOWN_ICH1_UZD_Ctrl1_reg);
	ich1_ibuff_ctrli_buff_ctrl_REG.regValue = IoReg_Read32(SCALEDOWN_ICH1_IBUFF_CTRLI_BUFF_CTRL_reg);
	...
	ich2_ibuff_ctrli_buff_ctrl_REG.regValue = IoReg_Read32(SCALEDOWN_ICH2_IBUFF_CTRLI_BUFF_CTRL_reg);
	ich2_uzd_Initial_Value_REG.regValue	= IoReg_Read32(SCALEDOWN_ICH2_UZD_Initial_Value_reg);
	ich2_uzd_initial_int_value_REG.regValue	= IoReg_Read32(SCALEDOWN_ICH2_UZD_Initial_int_Value_reg);
#endif //CONFIG_DUAL_CHANNEL
	...
	if ((ptInSize->nWidth > ptOutSize->nWidth)) {
		...
		if (channel == _CHANNEL1) {		/*channel 1*/
			...
			if (SDFilter == 0)	{		//narrow
				ich2_uzd_Ctrl0_REG.h_c_table_sel = 1;
				ich2_uzd_Ctrl0_REG.h_y_table_sel = 1;
			} else if (SDFilter == 1)	{	//mid
				ich2_uzd_Ctrl0_REG.h_c_table_sel = 2;
				ich2_uzd_Ctrl0_REG.h_y_table_sel = 2;
			} else {					//wider
				ich2_uzd_Ctrl0_REG.h_c_table_sel = 0;
				ich2_uzd_Ctrl0_REG.h_y_table_sel = 0;
			}
			*/
#endif //CONFIG_DUAL_CHANNEL
		}
...
/*
			if (SDFilter == 0)	{		//narrow
				ich2_uzd_Ctrl0_REG.v_c_table_sel = 1;
				ich2_uzd_Ctrl0_REG.v_y_table_sel = 1;
			} else if (SDFilter == 1)	{	//mid
				ich2_uzd_Ctrl0_REG.v_c_table_sel = 2;
				ich2_uzd_Ctrl0_REG.v_y_table_sel = 2;
			} else {					//wider
				ich2_uzd_Ctrl0_REG.v_c_table_sel = 0;
				ich2_uzd_Ctrl0_REG.v_y_table_sel = 0;
			}
*/
#endif //CONFIG_DUAL_CHANNEL
		}
...
/*============H Scale-down============*/
		...
		IMAIN_ultrazoom_set_hor_ini_desire(
			&ich1_uzd_Initial_Value_REG ,
			&ich1_uzd_initial_int_value_REG ,
		ptInSize->nWidth , nFactor , ptOutSize->nWidth );
		ich1_ibuff_ctrli_buff_ctrl_REG.ibuf_h_size = S2;

/*VIPprintf("reg_seg1 =%x, reg_Seg_all =%x\n", 0, ich1_uzd_Hor_Segment_REG.regValue);*/
...
/*			ich1_uzd_Ctrl0_REG.buffer_mode = 0x02;*/	/*buffer assign to vertical uzd*/
			...
			ich1_uzd_initial_int_value_REG.ver_ini_int= 0;
			//ich1_uzd_Initial_Value_REG.ver_ini_l = Vini;
			//ich1_uzd_initial_int_value_REG.ver_ini_int_l= 1;
			/*nFactor = (unsigned int)((ptInSize->nLength <<20) - ((unsigned int)Vini<<12)) / (ptOutSize->nLength - 1);*/
			if (ptOutSize->nLength == 0) {
			...
			nFactor = SHR(nFactor + 1, 1); /*rounding*/
			if( nFactor>>25 )
			{//factor only has 25 bits
				nFactor = 0x1ffffff;
				VIPprintf("############[UZD]vlen_factor size overflow############\n");
			}
			ich1_uzd_Scale_Ver_Factor_REG.ver_fac = nFactor;
			ich1_uzd_initial_int_value_REG.ver_ini_int= 1;
			IMAIN_ultrazoom_set_ver_ini_desire(
				&ich1_uzd_Initial_Value_REG ,
				&ich1_uzd_initial_int_value_REG ,
			ptInSize->nLength , nFactor , ptOutSize->nLength );
		} else {
...
/*	CLR_422_Fr_SD();*/
		...
		IoReg_Write32(SCALEDOWN_ICH1_IBUFF_CTRLI_BUFF_CTRL_reg, ich1_ibuff_ctrli_buff_ctrl_REG.regValue);
	}
	...
	if (channel == _CHANNEL2) {
		...
		if (ptInSize->nWidth > ptOutSize->nWidth) {    /*H scale-down*/
			...
			ISUB_ultrazoom_set_hor_ini_desire(
				&ich2_uzd_Initial_Value_REG ,
				&ich2_uzd_initial_int_value_REG ,
			ptInSize->nWidth , nFactor , ptOutSize->nWidth );
		} else {
			ich2_uzd_Scale_Hor_Factor_REG.hor_fac = 0x100000;
		}
		ich2_ibuff_ctrli_buff_ctrl_REG.ibuf_h_size = ptOutSize->nWidth;
...
/*			ich2_uzd_Ctrl0_REG.buffer_mode = 0x02;*/
			...
			ISUB_ultrazoom_set_ver_ini_desire(
				&ich2_uzd_Initial_Value_REG ,
				&ich2_uzd_initial_int_value_REG ,
			ptInSize->nLength , nFactor , ptOutSize->nLength );
		} else {
...
/*	CLR_422_Fr_SD();*/
		...
		IoReg_Write32(SCALEDOWN_ICH2_IBUFF_CTRLI_BUFF_CTRL_reg, ich2_ibuff_ctrli_buff_ctrl_REG.regValue);
		...
		IoReg_Write32(SCALEDOWN_ICH2_UZD_Initial_int_Value_reg, ich2_uzd_initial_int_value_REG.regValue);
#endif //CONFIG_DUAL_CHANNEL
	}
#ifndef BUILD_QUICK_SHOW
...
void drvif_color_scaler_set_subscaleup_hcoef(unsigned char display, DRV_FIR_Coef *ptr)/*add for Merlin2 (jyyang_2016/06/15)*/
{
	...
	if (display == SLR_SUB_DISPLAY) {	/*0:main / 1:sub*/
		...
		coef_table_rw_ctl_REG.coef_tab_sel = 0; // 0:4tap-1, 1:4tap-2
...
void drvif_color_scaler_set_subscaleup_vcoef(unsigned char display, DRV_FIR_Coef *ptr)/*add for Merlin2 (jyyang_2016/06/15)*/
{
	...
	if (display == SLR_SUB_DISPLAY) {	/*0:main / 1:sub*/
		...
		coef_table_rw_ctl_REG.coef_rw_en = 1;
		coef_table_rw_ctl_REG.coef_rw_sel = 0; /*0:w, 1:r*/
		coef_table_rw_ctl_REG.coef_tab_sel = 1; // 0:4tap-1, 1:4tap-2
		IoReg_Write32(SCALEUP_DS_COEF_TABLE_RW_CTL_reg, coef_table_rw_ctl_REG.regValue);
...
/*	ScalerSetScaleUpCtrl(info->display);*/
	...
	inSize.nWidth = (unsigned int)Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID);
		...
		IoReg_Write32(SCALEUP_DM_UZU_Ctrl_reg, dm_uzu_Ctrl_REG.regValue);
		rtd_pr_vpq_info("dm_uzu_Ctrl_REG.regValue =%x\n", dm_uzu_Ctrl_REG.regValue);
	} else {
#ifdef CONFIG_DUAL_CHANNEL
		scaleup_ds_uzu_ctrl_RBUS ds_uzu_Ctrl_REG;
		ds_uzu_Ctrl_REG.regValue = IoReg_Read32(SCALEUP_DS_UZU_Ctrl_reg);
        drvif_color_scaler_set_subscaleup_hcoef(SLR_SUB_DISPLAY, FIR_Coef_Ctrl+10);
        drvif_color_scaler_set_subscaleup_vcoef(SLR_SUB_DISPLAY, FIR_Coef_Ctrl+10);
		...
		IoReg_Write32(SCALEUP_DS_UZU_Ctrl_reg, ds_uzu_Ctrl_REG.regValue);
		rtd_pr_vpq_info("ds_uzu_Ctrl_REG.regValue =%x\n", ds_uzu_Ctrl_REG.regValue);
#endif //CONFIG_DUAL_CHANNEL
	}
	inSize.nLength = (unsigned int)Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN);
	inSize.nWidth = (unsigned int)Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID);
...
/*update H/V scaling up enable control bit for 3D overscan*/
	if (drvif_color_ultrazoom_set_3D_overscan(&inSize) != 0) {
		...
		if (inSize.nWidth > 1920)
			...
			dm_uzu_Ctrl_REG.bypassfornr = !Scaler_DispGetScaleStatus(display, SLR_SCALE_V_UP); /*nr always available*/
		IoReg_Write32(SCALEUP_DM_UZU_Ctrl_reg, dm_uzu_Ctrl_REG.regValue);
		rtd_pr_vpq_info("[3D] dm_uzu_Ctrl_REG.regValue =%x\n", dm_uzu_Ctrl_REG.regValue);
	} else
#endif

	{
		...
		//	H8tap_en = 1;
	/*=================================================(jyyang_20140207)*/
	rtd_pr_vpq_info("[UZU] [ultrazoom_config_scaling_up] set coeff for all tap\n");
		...
		if (PC_mode_check != VIP_Disable_PC_RGB444){ //need 4-tap in pc mode
			H8tap_en = 0;
			V8tap_mode = 0;
		}else if (drvif_color_ultrazoom_scalerup_PR_mode(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID)) == NOT_PR_MODE) {
...
void drvif_color_scaler_set_scaleup_coefRGB(DRV_FIR_Coef* ptrH,DRV_FIR_Coef* ptrV)
{
	...
	if( ptrH || ptrV )
	{
		...
		dm_uzu_Ctrl_REG.hor_mode_sel = 0;
		{
			...
			if ((dm_uzu_V8CTRL_reg.ver_mode_sel > 3) || (Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID) > 1920))
			{
				dm_uzu_V8CTRL_reg.ver_mode_sel = 3;				
				IoReg_Write32(SCALEUP_DM_UZU_V8CTRL_reg, dm_uzu_V8CTRL_reg.regValue);
			}
			else if( dm_uzu_V8CTRL_reg.ver_mode_sel==1 || dm_uzu_V8CTRL_reg.ver_mode_sel==2 )
			{
				dm_uzu_V8CTRL_reg.ver_mode_sel = 0;
				IoReg_Write32(SCALEUP_DM_UZU_V8CTRL_reg, dm_uzu_V8CTRL_reg.regValue);
			}
			rtd_pr_vpq_info("scaleup_coefRGB,ver_mode_sel=%d,MEM_ACT_WID=%d,\n", dm_uzu_V8CTRL_reg.ver_mode_sel, Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID));
		}
		/*0:sram1, 1:sram2*/
		IoReg_Write32(SCALEUP_DM_UZU_Ctrl_reg, dm_uzu_Ctrl_REG.regValue);
	}
...
void drvif_uvc_ultrazoom_setscaleup(unsigned char display, SIZE *ptInSize, SIZE *ptOutSize, unsigned char panorama)
{
	...
	{
		vc_uvc_scaleup_uvc_uzu_scale_hor_factor_RBUS		vc_uvc_scaleup_uvc_uzu_Scale_Hor_Factor_REG;
		vc_uvc_scaleup_uvc_uzu_scale_ver_factor_RBUS		vc_uvc_scaleup_uvc_uzu_Scale_Ver_Factor_REG;
		vc_uvc_scaleup_uvc_uzu_initial_value_RBUS			vc_uvc_scaleup_uvc_uzu_Initial_Value_REG;

		vc_uvc_scaleup_uvc_uzu_input_size_RBUS	vc_uvc_scaleup_uvc_uzu_Input_Size_REG;

		vc_uvc_scaleup_uvc_uzu_Initial_Value_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Initial_Value_reg);
		vc_uvc_scaleup_uvc_uzu_Scale_Ver_Factor_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Scale_Ver_Factor_reg);
		vc_uvc_scaleup_uvc_uzu_Scale_Hor_Factor_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Scale_Hor_Factor_reg);

		if (ptOutSize->nWidth > ptInSize->nWidth) {    /*H scale-up, set h scaling factor*/
			Hini = 0xff;
			vc_uvc_scaleup_uvc_uzu_Initial_Value_REG.hor_ini = Hini;
			/*nFactor = (((unsigned int)(ptInSize->nWidth)<< 21) - ((unsigned int)Hini<<13))  / (ptOutSize->nWidth-1);*/
			ulparam1 = (((unsigned long long)(ptInSize->nWidth)<< 21));
			ulparam2 = (unsigned long long)(ptOutSize->nWidth);
			do_div(ulparam1, ulparam2);
		...
		vc_uvc_scaleup_uvc_uzu_Scale_Hor_Factor_REG.regValue = nFactor;
		...
		if (ptOutSize->nLength > ptInSize->nLength) {   /* V scale-up, set v scaling factor*/
			if (Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_INTERLACE))
				...
				Vini = 0xFF;
			vc_uvc_scaleup_uvc_uzu_Initial_Value_REG.ver_ini = Vini;
			/*nFactor = ((unsigned int)((ptInSize->nLength) << 20) - ((unsigned int)Vini<<12)) / ((ptOutSize->nLength)-1);*/
			ulparam1 = ((unsigned long long)(ptInSize->nLength) << 21);
			ulparam2 = (unsigned long long)(ptOutSize->nLength);
			do_div(ulparam1, ulparam2);
		...
		vc_uvc_scaleup_uvc_uzu_Scale_Ver_Factor_REG.regValue = nFactor;

		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Scale_Ver_Factor_reg, vc_uvc_scaleup_uvc_uzu_Scale_Ver_Factor_REG.regValue);
		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Scale_Hor_Factor_reg, vc_uvc_scaleup_uvc_uzu_Scale_Hor_Factor_REG.regValue);
		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Initial_Value_reg, vc_uvc_scaleup_uvc_uzu_Initial_Value_REG.regValue);

		vc_uvc_scaleup_uvc_uzu_Input_Size_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Input_Size_reg);
		vc_uvc_scaleup_uvc_uzu_Input_Size_REG.hor_input_size = ptInSize->nWidth;
		/*frank@0602 solve IC BUG to avoid main channel bandwidth not enough*/
		/*vc_uvc_scaleup_uvc_uzu_Input_Size_REG.ver_input_size = ptInSize->nLength;*/
		vc_uvc_scaleup_uvc_uzu_Input_Size_REG.ver_input_size = ptInSize->nLength;
...
void drvif_uvc_ultrazoom_setscaleup_panorama(unsigned char display, SIZE *ptInSize, SIZE *ptOutSize, unsigned char panorama)
{
	unsigned char Hini, Vini;
	unsigned long long ulparam1 =0, nFactor;
	unsigned long long ulparam2 =0;
...
void drvif_uvc_scaler_set_subscaleup_hcoef(unsigned char display, DRV_FIR_Coef *ptr)/*add for Merlin2 (jyyang_2016/06/15)*/
{
	...
	vc_uvc_scaleup_uvc_uzu_coef_table_rw_ctl_RBUS coef_table_rw_ctl_REG;
	vc_uvc_scaleup_uvc_uzu_coef_table_ctl_0_RBUS coef_table_ctl_0_REG;
	vc_uvc_scaleup_uvc_uzu_coef_table_ctl_1_RBUS coef_table_ctl_1_REG;
	vc_uvc_scaleup_uvc_uzu_coef_table_ctl_RBUS coef_table_ctl_REG;
	
	{
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg);
		coef_table_rw_ctl_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_RW_CTL_reg);
		coef_table_ctl_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_CTL_reg);

		/* H and V use table 2 */ //mac6p sub no tab sel
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.h_table_sel = 1; // 0:tab1, 1:tab2
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.v_table_sel = 1;
		//IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg, vc_uvc_scaleup_uvc_uzu_ctrl_REG.regValue);

		/* sel and write sram_Tab (sub1)*/
		coef_table_rw_ctl_REG.coef_rw_en = 1;
		coef_table_rw_ctl_REG.coef_rw_sel = 0; /*0:w, 1:r*/
		coef_table_rw_ctl_REG.coef_tab_sel = 0; /* 0:tap-1, 1:tap-2 */
		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_RW_CTL_reg, coef_table_rw_ctl_REG.regValue);
		...
		for (i = 0; i < 16; i++) {
			...
			coef_table_ctl_0_REG.coef0 = (c[0] & 0x0fff);
			coef_table_ctl_0_REG.coef1 = (c[1] & 0x0fff);
			IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_CTL_0_reg, coef_table_ctl_0_REG.regValue);

			coef_table_ctl_1_REG.coef2 = (c[2] & 0x0fff);
			coef_table_ctl_1_REG.coef3 = (c[3] & 0x0fff);
			IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_CTL_1_reg, coef_table_ctl_1_REG.regValue);

			coef_table_ctl_REG.coef_write = 1;
			IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_CTL_reg, coef_table_ctl_REG.regValue);
		...
		/* H use table 1*/ //mac6p sub no tab sel
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.h_table_sel= 0;
		//IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg, vc_uvc_scaleup_uvc_uzu_ctrl_REG.regValue);
	}
}
void drvif_uvc_scaler_set_subscaleup_vcoef(unsigned char display, DRV_FIR_Coef *ptr)/*add for Merlin2 (jyyang_2016/06/15)*/
{
	...
	
	{
		...
		/* H and V use table 1 */
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.h_table_sel = 0; // 0:tab1, 1:tab2
		//vc_uvc_scaleup_uvc_uzu_ctrl_REG.v_table_sel = 0;
		//IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg, vc_uvc_scaleup_uvc_uzu_ctrl_REG.regValue);

		/* sel and write sram_Tab (sub2)*/
		coef_table_rw_ctl_REG.coef_rw_en = 1;
		coef_table_rw_ctl_REG.coef_rw_sel = 0; /*0:w, 1:r*/
		coef_table_rw_ctl_REG.coef_tab_sel = 1; /* 0:tap-1, 1:tap-2 */
		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_COEF_TABLE_RW_CTL_reg, coef_table_rw_ctl_REG.regValue);
...
void drvif_uvc_ultrazoom_config_scaling_up(unsigned char panorama)
{
	SIZE inSize, outSize;
	//unsigned char V8tap_mode = 0, v8_table_sel = 0; // mode = 0:3840*2, 1:1920*4
...
/*	ScalerSetScaleUpCtrl(info->display);*/
	{
		vc_uvc_scaleup_uvc_uzu_ctrl_RBUS vc_uvc_scaleup_uvc_uzu_Ctrl_REG;
		vc_uvc_scaleup_uvc_uzu_Ctrl_REG.regValue = IoReg_Read32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg);
		vc_uvc_scaleup_uvc_uzu_Ctrl_REG.v_zoom_en = (Scaler_DispGetScaleStatus(display, SLR_SCALE_V_UP) != 0);
		vc_uvc_scaleup_uvc_uzu_Ctrl_REG.h_zoom_en = (Scaler_DispGetScaleStatus(display, SLR_SCALE_H_UP) != 0);
		vc_uvc_scaleup_uvc_uzu_Ctrl_REG.in_fmt_conv = (Scaler_DispGetStatus(display, SLR_DISP_422CAP) ? 1 : 0);

		inSize.nWidth = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID);
		//if(inSize.nWidth <=1920) //mac6p sub no ver_mode_sel
		//	vc_uvc_scaleup_uvc_uzu_Ctrl_REG.ver_mode_sel = 1; // 0:V2tap, 1:V4tap (jyyang 2016.0616)
		
		IoReg_Write32(VC_UVC_SCALEUP_UVC_UZU_Ctrl_reg, vc_uvc_scaleup_uvc_uzu_Ctrl_REG.regValue);
		VIPprintf("vc_uvc_scaleup_uvc_uzu_Ctrl_REG.regValue =%x\n", vc_uvc_scaleup_uvc_uzu_Ctrl_REG.regValue);
	}
	inSize.nLength = (unsigned int)Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_LEN);
	inSize.nWidth = (unsigned int)Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_WID);
...
#ifdef ENABLE_SG_3D_SUPPORT_ALL_SOURCE_3D_MODE
/*update H/V scaling up enable control bit for 3D overscan*/
	if (drvif_uvc_ultrazoom_set_3D_overscan(&inSize) != 0) {
	} else
#endif
	...
	VIPprintf("[UZU] [ultrazoom_config_scaling_up] set coeff for all tap\n");
        ...
        #ifndef BUILD_QUICK_SHOW
		if (fwif_VIP_get_Project_ID() == VIP_Project_ID_TV030)
			drvif_uvc_ultrazoom_setscaleup_panorama(display, &inSize, &outSize, panorama);	/* Display scaling up control, linear or non-linear either*/
		else
           #endif
			drvif_uvc_ultrazoom_setscaleup(display, &inSize, &outSize, panorama);	/* Display scaling up control, linear or non-linear either*/
	/*}*/
}
