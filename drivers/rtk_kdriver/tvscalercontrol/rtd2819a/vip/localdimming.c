...
void drvif_color_set_LD_Global_Ctrl(DRV_LD_Global_Ctrl *ptr)
{
	...
	if(ptr->ld_blk_type == 0)
	{
	    ...
	    if(h_repeat_val > 255)
	    {
	    	remain = h_large_val*255 % (h_repeat_val);
    		if(remain != 0)
    			remain = 1;	
	        h_large_val = h_large_val /(h_repeat_val/256) + remain;
	        h_repeat_val = 255;
	    }
	    ...
	    if(v_repeat_val > 255)
	    {
	    	remain = v_large_val*255 % (v_repeat_val);
    		if(remain != 0)
    			remain = 1;	
	        v_large_val = v_large_val/(v_repeat_val/256) + remain;
	        v_repeat_val = 255;
	    }
	}
...
void drvif_color_get_LD_Avg_Bv(DRV_LD_Average_Bv *ptr)
{
	...
	ptr->ld_apl_global_mean_avg= blu_ld_avg_bv3.ld_apl_global_mean;

}

void drvif_color_get_LD_Avg_Bv_pif2(DRV_LD_Average_Bv *ptr)
{
	blu_ld_pif2_avg_bv0_RBUS blu_ld_pif2_avg_bv0;
	blu_ld_pif2_avg_bv1_RBUS blu_ld_pif2_avg_bv1;
	blu_ld_pif2_avg_bv2_RBUS blu_ld_pif2_avg_bv2;
	blu_ld_pif2_avg_bv3_RBUS blu_ld_pif2_avg_bv3;

	blu_ld_pif2_avg_bv0.regValue = IoReg_Read32(BLU_LD_PIF2_AVG_BV0_reg);
	blu_ld_pif2_avg_bv1.regValue = IoReg_Read32(BLU_LD_PIF2_AVG_BV1_reg);
	blu_ld_pif2_avg_bv2.regValue = IoReg_Read32(BLU_LD_PIF2_AVG_BV2_reg);
	blu_ld_pif2_avg_bv3.regValue = IoReg_Read32(BLU_LD_PIF2_AVG_BV3_reg);

	ptr->ld_global_gain_avg = blu_ld_pif2_avg_bv0.ld_pif2_global_gain_avg_bv;
	ptr->ld_bpl_first_avg = blu_ld_pif2_avg_bv0.ld_pif2_bpl_first_avg_bv;
	ptr->ld_ai_brightness_avg = blu_ld_pif2_avg_bv1.ld_pif2_ai_brightness_avg_bv;
	ptr->ld_local_bv_gain_avg = blu_ld_pif2_avg_bv1.ld_pif2_local_bv_gain_avg_bv;
	ptr->ld_abi_avg = blu_ld_pif2_avg_bv2.ld_pif2_abi_avg_bv;
	ptr->ld_third_bpl_avg = blu_ld_pif2_avg_bv2.ld_pif2_third_bpl_avg_bv;
	ptr->ld_backlightboost_avg = blu_ld_pif2_avg_bv3.ld_pif2_backlightboost_avg_bv;
...
void drvif_color_set_LD_Boost(DRV_LD_Boost *ptr)
{
	...
	/* Gain LUT*/
	for (i = 0; i < 32; i++) {
		blu_ld_spatial_ctrlc.ld_spatialremapadr = i+0;
		IoReg_Write32(BLU_LD_Spatial_CtrlC_reg, blu_ld_spatial_ctrlc.regValue);
	...
	/* Boost LUT*/
	for (i = 0; i < 32; i++) {
		blu_ld_spatial_ctrlc.ld_spatialremapadr = i+32;
		IoReg_Write32(BLU_LD_Spatial_CtrlC_reg, blu_ld_spatial_ctrlc.regValue);
...
void drvif_color_set_LD_Temporal_Filter(DRV_LD_Temporal_Filter *ptr)
{
	...
	blu_ld_temporal_ctrl3.ld_tmp_gain_sel = ptr->ld_tmp_gain_sel;  // feature for Dual
...
// add leakage level set
void drvif_color_set_leakage_th(DRV_LD_Leakage_TH *ptr)
{
	blu_ld_apl_th_RBUS ld_apl_th_reg;

	ld_apl_th_reg.regValue = IoReg_Read32(BLU_LD_APL_LEAKAGE_LEVEL_reg);
...
//k25 add leakage level get 
void drvif_color_get_leakage_th(DRV_LD_Leakage_TH *ptr)
{
	blu_ld_apl_th_RBUS ld_apl_th_reg;

	ld_apl_th_reg.regValue = IoReg_Read32(BLU_LD_APL_TH_reg);
...
{
	blu_ld_apl_leakage_level_RBUS ld_apl_leakage_level_reg;
	unsigned int Leakage_Level = 0;

	ld_apl_leakage_level_reg.regValue = IoReg_Read32(BLU_LD_APL_LEAKAGE_LEVEL_reg);
...
unsigned char drvif_color_get_LD_APL_ave( unsigned int* APL_data )
{
    ...
    ld_apl_read_ave_2_reg.ld_aplave_en= 1;
    ...
    ld_apl_read_ave_2_reg.regValue = IoReg_Read32(BLU_LD_APL_read_ave_2_reg);
    ld_apl_read_ave_2_reg.ld_aplave_en = 0;
    IoReg_Write32(BLU_LD_APL_read_ave_2_reg, ld_apl_read_ave_2_reg.regValue);
...
unsigned char drvif_color_get_LD_APL_max( unsigned int* APL_data )
{
    ...
    ld_apl_read_max_2_reg.ld_aplmax_en= 1;
    ...
    ld_apl_read_max_2_reg.regValue = IoReg_Read32(BLU_LD_APL_read_max_2_reg);
    ld_apl_read_max_2_reg.ld_aplmax_en = 0;
    IoReg_Write32(BLU_LD_APL_read_ave_2_reg, ld_apl_read_max_2_reg.regValue);
...
unsigned char drvif_color_get_LD_APL_max_avg(void){
	...
	for( i=0; i<vnum; i++ )
    {
        for( j=0; j<((hnum+1)>>1); j++ )
        {
			...
			access_ptr_max = (ld_apl_read_max_3_reg.regValue & 0x1FF00000) >> 20;
            mdata_max = (ld_apl_read_max_3_reg.regValue & 0x000FFC00) >> 10;
            ldata_max = (ld_apl_read_max_3_reg.regValue & 0x000003FF);
							

			access_ptr_avg = (ld_apl_read_ave_3_reg.regValue & 0x1FF00000) >> 20;
            mdata_avg = (ld_apl_read_ave_3_reg.regValue & 0x000FFC00) >> 10;
            ldata_avg = (ld_apl_read_ave_3_reg.regValue & 0x000003FF);
...
char drvif_color_get_LD_AlgoSRAM(unsigned int length, unsigned short *BLtable) 
{
	...
	if (blu_ld_sram_ctrl.ld_blight_sw_mode == 1) {/*SW Mode enabled*/
		...
		blu_ld_blight_ap.ld_blight_delta = 0;
		blu_ld_blight_ap.ld_blight_offset = 0;
		IoReg_Write32(BLU_LD_BLIGHT_AP_reg, blu_ld_blight_ap.regValue);
	...
	if (blu_ld_sram_ctrl.ld_blight_sw_mode == 1) {/*SW Mode enabled*/
		...
		if(ld_blk_type == 1) {	// top/bottom mode
			...
			for(j = 0; j < ld_blk_hnum + 1; j++) {
				...
				IoReg_Write32(BLU_LD_BLIGHT_AP_reg, reg_blu_ld_blight_ap.regValue);
				for(i = 0; i < ld_blk_vnum; i++) {
					// data
					reg_blu_ld_blight_dp.regValue = IoReg_Read32(BLU_LD_BLIGHT_DP_reg);
					*(BLtable+ (j*(ld_blk_hnum+1))+i) = reg_blu_ld_blight_dp.ld_blight_dat_lsb;
					*(BLtable+ (j*(ld_blk_hnum+1))+i+1) = reg_blu_ld_blight_dp.ld_blight_dat_msb;
				}
			}
		}
		else if(ld_blk_type == 2) {	// left/right mode
			...
			while(i < (ld_blk_vnum * ld_blk_hnum) + 1) {
				...
				*(BLtable+i+1) = reg_blu_ld_blight_dp.ld_blight_dat_msb;
			}
			...
			for(i = 0; i < ld_blk_hnum + 1; i++) {
				for(j = 0; j < ld_blk_vnum + 1;j++) {
					...
					*(BLtable+ (i*(ld_blk_hnum+1))+j+1) = reg_blu_ld_blight_dp.ld_blight_dat_msb;
				}
			}
		}
	}
