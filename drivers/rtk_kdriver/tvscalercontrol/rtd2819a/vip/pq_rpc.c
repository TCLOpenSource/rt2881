...
FILM_FWMode DETECTION_scalerVIP_film_FWmode(void)
{
	...
	if(enchance22_en)
	{
		...
		if(HFT_prev==1 && (FW_HFT<FW_HFT_thd) && (FrameDiff>700)
			&& (((ABS_(PrevDiff-FW_HFT)<((FW_HFT_rato_thd*FW_HFT)>>7)) || (ABS_(PrevDiff-FW_HFT)< FW_HFT_diff_thd)) ||
		...
		)
		{
 		...
 		&& (((ABS_(NextDiff-FW_HFT)<((FW_HFT_rato_thd*FW_HFT)>>7)) || (ABS_(NextDiff-FW_HFT)< FW_HFT_diff_thd))||
...
*/
	...
	else
	{
		...
		//*/
		if( DetInformation.Fw_StatusFilmDetected && !DetInformation.fw_Status22Detected )
		{
			im_di_film_new_function_main_reg.fw_film_en = 1;
