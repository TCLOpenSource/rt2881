...
DRV_od_dma oddma_g[] = {
...

	{
		...
		{3840, 2160, 0, 0, 0, 0, 0},
		{1, 0, 0, 0xffffffff, 0, 0xffffffff, 0,0, 0, 0, 0, 7, 1, 0},
...
DRV_od_ctrl od_ctrl_g[] = {
	...
	/*		{enable, weight, nr_thd, edge_hhf_thd, local_gain0~9}*/
	...
	{ // y mode
		{1, 0x4D, 0x96, 0x1D},
		{1, 0, 32, 24, 16, 8, 0, 3, 3, 3},
		{1, 7, 3, 3, 63,48,32,24,24,32,40,48,48,48},
		{{1, 1, 1, 1, 7, 7, 7}, {1, 1, 1, 1, 7, 7, 7},},
		{{64, 64, 64}, {64, 64, 64},},
	},
	{ 	// rgb mode no scaling
		{0, 0x4D, 0x96, 0x1D},
...
DRV_OD_VerGain od_vergain_g[] = {
	// normal vergain LUT
	{
		0, 17476 /* (32/1920)<<20 */ , 100, // en, factor, offset
		{
...
void drvif_color_od_enable_set(unsigned char od_enable)
{
	...
	/* OD enable */
	od_od_ctrl_reg.od_en = (bODInited==true)?(od_enable):(0);
...
void drvif_color_od_scaling_set(DRV_od_scaling od_scaling){
	...
	// set enable by "drvif_color_od_scaling_enable_set()"
	/*od_od_ctrl_reg.regValue = IoReg_Read32(OD_OD_CTRL_reg);
...
void drvif_color_od_localgain_set(DRV_od_localgain od_localgain){
	...
	od_od_local_edge_gain_0_RBUS od_od_local_edge_gain_0_reg;
	od_od_local_edge_gain_1_RBUS od_od_local_edge_gain_1_reg;

	od_od_local_gain_reg.regValue = IoReg_Read32(OD_OD_local_gain_reg);
	...
	od_od_local_gain_reg.localgain_hhf_thd= od_localgain.edge_hhf_thd;

	od_od_local_edge_gain_0_reg.regValue = IoReg_Read32(OD_OD_local_edge_gain_0_reg);
	od_od_local_edge_gain_1_reg.regValue = IoReg_Read32(OD_OD_local_edge_gain_1_reg);
	od_od_local_edge_gain_0_reg.localgain_edge_gain0 = od_localgain.localgain_edge_gain0;
	od_od_local_edge_gain_0_reg.localgain_edge_gain1 = od_localgain.localgain_edge_gain1;
	od_od_local_edge_gain_0_reg.localgain_edge_gain2 = od_localgain.localgain_edge_gain2;
	od_od_local_edge_gain_0_reg.localgain_edge_gain3 = od_localgain.localgain_edge_gain3;
	od_od_local_edge_gain_0_reg.localgain_edge_gain4 = od_localgain.localgain_edge_gain4;
	od_od_local_edge_gain_1_reg.localgain_edge_gain5 = od_localgain.localgain_edge_gain5;
	od_od_local_edge_gain_1_reg.localgain_edge_gain6 = od_localgain.localgain_edge_gain6;
	od_od_local_edge_gain_1_reg.localgain_edge_gain7 = od_localgain.localgain_edge_gain7;
	od_od_local_edge_gain_1_reg.localgain_edge_gain8 = od_localgain.localgain_edge_gain8;
	od_od_local_edge_gain_1_reg.localgain_edge_gain9 = od_localgain.localgain_edge_gain9;
	
	IoReg_Write32(OD_OD_local_gain_reg, od_od_local_gain_reg.regValue);
	IoReg_Write32(OD_OD_local_edge_gain_0_reg, od_od_local_edge_gain_0_reg.regValue);
	IoReg_Write32(OD_OD_local_edge_gain_1_reg, od_od_local_edge_gain_1_reg.regValue);
...
void drvif_color_od_dma_addr_set(DRV_od_dma *oddma)
{
	...
	IoReg_Write32(OD_OD_status_check_up_reg, oddma->dma_pair.up_limit_lut_data);
	IoReg_Write32(OD_OD_status_check_low_reg, oddma->dma_pair.low_limit_lut_data);
...
void drvif_color_od_dma_cap_enable_set(unsigned char enable)
{
	...
	od_dma_oddma_wr_ctrl_reg.cap_en = (bODInited==true || SLD_DMA_Ready == 1)?(enable):(0);
#else
...
void drvif_color_od_dma_disp_enable_set(unsigned char enable)
{
	...
	od_dma_oddma_rd_ctrl_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_RD_Ctrl_reg);
#ifndef BUILD_QUICK_SHOW	// lut dma is no need on qs	
	od_dma_oddma_rd_ctrl_reg.disp_en = (bODInited==true || SLD_DMA_Ready == 1)?(enable):(0);
#else
...
char drvif_color_get_od_DB_status(unsigned char *cap_en, unsigned char *cap_lv, unsigned char *cap_apply, 
	unsigned char *disp_en, unsigned char *disp_lv, unsigned char *disp_apply, unsigned char *od_en, unsigned char *od_lv, unsigned char *od_apply)
{
	if (cap_en == NULL || cap_lv == NULL || cap_apply == NULL || disp_en == NULL || disp_lv == NULL || disp_apply == NULL || 
		od_en == NULL || od_lv == NULL || od_apply == NULL)
	{
		rtd_pr_vpq_emerg("drvif_color_get_od_DB_status = NULL\n"); 	
		return -1;
	}

	*cap_apply = OD_DMA_OD_DMA_DB_EN_get_disp_db_apply(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg)); 
	*cap_lv = OD_DMA_OD_DMA_DB_EN_get_disp_db_rd_sel(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg));
	*cap_en = OD_DMA_OD_DMA_DB_EN_get_disp_db_en(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg));	
	*disp_apply = OD_DMA_OD_DMA_DB_EN_get_cap_db_apply(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg));	
	*disp_lv = OD_DMA_OD_DMA_DB_EN_get_cap_db_rd_sel(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg)); 
	*disp_en = OD_DMA_OD_DMA_DB_EN_get_cap_db_en(IoReg_Read32(OD_DMA_OD_DMA_DB_EN_reg)); 	
	*od_apply = OD_OD_CTRL1_get_db_apply(IoReg_Read32(OD_OD_CTRL1_reg));
	*od_lv = OD_OD_CTRL1_get_db_read_level(IoReg_Read32(OD_OD_CTRL1_reg));
	*od_en = OD_OD_CTRL1_get_od_en_dbf_en(IoReg_Read32(OD_OD_CTRL1_reg));

	return 0;
}

char drvif_color_set_od_Rolling_En(unsigned char cap_en, unsigned char disp_en, unsigned char force_DB_set)
{
	...
	/* cap en db */
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_disp_db_en_mask);
	/* cap en db */
	IoReg_SetBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_cap_db_en_mask);
	
	od_dma_oddma_wr_num_bl_wrap_rolling_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_WR_num_bl_wrap_rolling_reg);
	od_dma_oddma_rd_num_bl_wrap_rolling_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_RD_num_bl_wrap_rolling_reg);
	
	od_dma_oddma_wr_num_bl_wrap_rolling_reg.cap_rolling_enable = cap_en;
	od_dma_oddma_rd_num_bl_wrap_rolling_reg.disp_rolling_enable = disp_en;
	...
	{
		/* cap en db */
		IoReg_ClearBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_disp_db_en_mask);
		/* cap en db */
		IoReg_ClearBits(OD_DMA_OD_DMA_DB_EN_reg, OD_DMA_OD_DMA_DB_EN_cap_db_en_mask);
...
void drvif_color_od_table_33x33_set(unsigned char *pODtable, unsigned char tableType, unsigned char colorChannel)
{
	...
	lut_addr_reg.table_type = tableType;
	//if(tableType == 0)
	lut_addr_reg.table_type_use_target = 0;// new feature target table run delta mode flow
	lut_addr_reg.od_lut_ax_en = 1;
	lut_addr_reg.od_lut_ax_mode = 0;
	lut_addr_reg.od_lut_adr_mode = 0; //col(cur) indexfirst
	lut_addr_reg.od_lut_column_addr = 0;
	...
	for (i = 0; i < OD_table_length; i++) {
		...
		#endif
		value = ((*(pODtable+i))<<16) + *(pODtable+i);
		//IoReg_Write32(OD_OD_LUT_DATA_reg, *(pODtable+i));
		IoReg_Write32(OD_OD_LUT_DATA_reg, value);
	}
...
void drvif_color_set_od_bits(unsigned char FrameLimitBit)
{
	...
	oddma_pq_cmp_bit_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_PQ_CMP_BIT_reg);
        ...
        /*if (get_panel_pixel_mode() > PANEL_1_PIXEL) // ((Get_DISPLAY_REFRESH_RATE()!=60)){ //120hz panel
        ...
        else if (get_panel_pixel_mode() == PANEL_1_PIXEL) // ((Get_DISPLAY_REFRESH_RATE()==60)){
	...
	if (FrameLimitBit > 0) {
		// pqc hw performance limitation
                /*if ((get_panel_pixel_mode() > PANEL_1_PIXEL) && FrameLimitBit < 8){
                        FrameLimitBit = 8;
                }
                else if ((get_panel_pixel_mode() == PANEL_1_PIXEL) && FrameLimitBit < 2){
                        FrameLimitBit = 2;
                }*/
		...
		// DMA buffer line limit clamp no line_limit_bit
		//if(dma_mode == 0)
			//oddma_pq_cmp_bit_reg.line_limit_bit = oddma_pq_cmp_bit_reg.frame_limit_bit;
		//else if(dma_mode == 1)
			//oddma_pq_cmp_bit_reg.line_limit_bit = FrameLimitBit + 6;
		...
		if(FrameLimitBit <= 6) {
			...
			oddma_pq_decmp_pair_reg.decmp_data_bit_width = 1;
                ...
                /*if (get_panel_pixel_mode() > PANEL_1_PIXEL)// (Get_DISPLAY_REFRESH_RATE() >= 120) //120hz
                {
                        oddma_pq_cmp_pair_reg.cmp_data_bit_width = 1;
                        oddma_pq_decmp_pair_reg.decmp_data_bit_width = 1;
		...
		//pqc_line_sum_bit = ((oddma->dma_pqc_pair.width_div32 << 5) * (oddma->dma_comp.frame_limit_bit)/4 + 672) / 128; // sync with DIC 672 = 7*32*3
		//pqc_line_sum_bit += ((((oddma->dma_pqc_pair.width_div32 << 5) * oddma->dma_comp.frame_limit_bit)/4 + 672) % 128)!=0; // ceil

		//pqc_line_sum_bit = (width * 32 * dma_pqc_pair.width_div32 + 256) / 128 + ((width * 32 * dma_pqc_pair.width_div32 + 256)%128!=0);
				
		pqc_line_sum_bit = ((width << 5) * (FrameLimitBit)/4 + 672) / 128; // sync with DIC 672 = 7*32*3
		pqc_line_sum_bit += ((((width << 5) * FrameLimitBit)/4 + 672) % 128)!=0; // ceil
#ifdef CONFIG_BW_96B_ALIGNED
		// for cap/disp line_step (128 bits unit)
        align_value = drvifget_align_value(pqc_line_sum_bit, 6);
#else
...
void drvif_color_set_od_dma_HW_reset(void)
{
	...
	if (drvif_color_od_enable_get()==1){ 
		...
		od_od_ctrl_reg.regValue = IoReg_Read32(OD_OD_CTRL_reg);
		/* OD enable */
		od_od_ctrl_reg.od_en = (bODInited==true)?(1):(0);
		IoReg_Write32(OD_OD_CTRL_reg, od_od_ctrl_reg.regValue);
		/* db apply */
		IoReg_SetBits(OD_OD_CTRL1_reg, OD_OD_CTRL1_db_apply_mask); //_BIT26 
