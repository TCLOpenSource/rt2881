...
static unsigned char hdmi_multi_port_conncet_status[4] = {0, 0, 0, 0};
...
// static spinlock_t HDMI_ZERO_TIMING_CTRL[MAX_PORT_NUM];
// static unsigned char HDMI_Reply_Zero_Timing[MAX_PORT_NUM] = {0};/*If true, let webos get zero timing*/
static unsigned char HDMI_Reply_Zero_Timing_mute_cnt[4] = {0};
static unsigned int HDMI_ZERO_REPORT_RECORD_TIME[4] = {0};//when ap polling the timing and the zero timing flag is ture. Record the time
...
void Set_Val_HDMI_Reply_Zero_Timing_mute_cnt(unsigned char port, unsigned char cnt)
{
	if(port >= 4)
		HDMI_Reply_Zero_Timing_mute_cnt[0] = cnt;
	else
		HDMI_Reply_Zero_Timing_mute_cnt[port] = cnt;
}

unsigned char Get_Val_HDMI_Reply_Zero_Timing_mute_cnt(unsigned char port)
{
	if(port >= 4)
		return HDMI_Reply_Zero_Timing_mute_cnt[0];
	return HDMI_Reply_Zero_Timing_mute_cnt[port];
}


void Set_Reply_Zero_Timing_Flag(unsigned char src, unsigned char flag, unsigned char port)
{//src type is from VSC_INPUT_TYPE_T
	unsigned long flags;//for spin_lock_irqsave

	if (port >= MAX_PORT_NUM) {
		rtd_pr_vfe_err("###[%s(%d) Invalid HDMI port number: %d\n", __func__, __LINE__, port);
		return;
	}
	...
	switch (src) {
		...
		case VSC_INPUTSRC_AVD:
			...
			rtd_pr_vfe_info("###[%s] AVD value:(%d %d)##\r\n",__FUNCTION__, flag, AVD_Reply_Zero_Timing);
		break;

		case VSC_INPUTSRC_HDMI:
			if(vfe_hdmi_get_zero_timing_flag(port) == SOURCE_GOOD_TIMING_READY) //ignore the case
				return;

			vfe_hdmi_set_zero_timing_flag(port, flag);
			rtd_pr_vfe_info("###[%s] HDMI port: %d, value:(%d %d)##\r\n",__FUNCTION__, port, flag, vfe_hdmi_get_zero_timing_flag(port));
		break;

#ifdef CONFIG_SUPPORT_SRC_DPRX
		case VSC_INPUTSRC_DP:
			Set_Dprx_Reply_Zero_Timing_Flag(flag, port);
			break;
#endif
		default:
		break;
...
void VBI_Control_setting(unsigned char vdc_mode)
{
	...
	if ((VBI_support_type_get() !=KADP_VFE_AVD_CC_IS_SUPPORTED) &&((vdc_mode == VDC_MODE_PALI)||(vdc_mode == VDC_MODE_NTSC50)||(vdc_mode == VDC_MODE_SECAM))) {//tt setting
		...
		if (0 ==(IoReg_Read32(VBI_TT_VBI_DATA_STATUS_reg) &VBI_TT_VBI_DATA_STATUS_tt_vps_int_en_mask)) {
			if ((drvif_module_vdc_read_coch_detect_status() == 1)  && (/*(get_avd_auto_tuning_mode() == 1) ||*/
...
int sm_vfe_qs_hdmi_init(void)
{
	...
	if (vfe_hdmi_qs_drv_init() != 0) {
		...
		hdmi_multi_port_conncet_status[get_QS_portnum()] = 1;
		newbase_set_hdmi_power_saving_enable();
	}
    return 0;
}
EXPORT_SYMBOL(sm_vfe_qs_hdmi_init);

int sm_vfe_hdmi_init(void)
{
#if BACKGROUND_DET
	u_int32_t tout = 200;
#endif
	if(HDMI_Global_Status > SRC_NOTHING) {
		rtd_pr_vfe_warn("\r\n VFE_IOC_HDMI_INIT successed."
                   "Source has been initialized already (FSM = %d)\n", 
                   HDMI_Global_Status);
		return 0;
	}
...
EXPORT_SYMBOL(sm_vfe_hdmi_init);

int sm_vfe_hdmi_open(void)
{
    if (HDMI_Global_Status == SRC_NOTHING) {
		rtd_pr_vfe_warn("\r\n VFE_IOC_HDMI_OPEN fail. (FSM = %d)\n", HDMI_Global_Status);
		return -1;
    }

    if(HDMI_Global_Status >= SRC_OPEN_DONE) {
		rtd_pr_vfe_warn("\r\n VFE_IOC_HDMI_OPEN successed. HDMI has been opened already (FSM = %d)\n",
                HDMI_Global_Status);
		return 0;
	}
...
EXPORT_SYMBOL(sm_vfe_hdmi_open);

int sm_vfe_hdmi_close(void)
{
    if(HDMI_Global_Status != SRC_OPEN_DONE) {
...
EXPORT_SYMBOL(sm_vfe_hdmi_close);

int sm_vfe_hdmi_connect(unsigned char port)
{
    int ret = 0;

	if(port == VFE_HDMI_H5X_EXTERNAL_PORT_INDEX){
		return vfe_hdmi_drv_connect_h5x(port);
	}

    if(HDMI_Global_Status != SRC_OPEN_DONE && hdmi_multi_port_conncet_status[port] == 1) {
		rtd_pr_vfe_debug("\r\n VFE_IOC_HDMI_CONNECT Fail. Source is not SRC_OPEN_DONE (FSM = %d)\n",
                HDMI_Global_Status);
		return -1; // Fail Source is not SRC_INIT_DONE
	}
	...
	if (vfe_hdmi_drv_connect(port) == 0)
	{
		 ...
		 * 
         * P_only mode hdmo disconnect /close => vsc disconnet => 
         * hdmi open/connect => vsc open/connect
         *
		 * Normal mode vsc disconnect => HDMI_set_detect_flag=1 => 
         * hdmi disconnect /close /open/ connect => vsc open/connect
        */
		HDMI_set_detect_flag(TRUE);
		HDMI_Global_Status = SRC_CONNECT_DONE;
		hdmi_multi_port_conncet_status[port] = 1;
	} else {
        ret = -1;
	}
    up(&HDMI_DetectSemaphore);
...
EXPORT_SYMBOL(sm_vfe_hdmi_connect);

int sm_vfe_hdmi_get_timing_info(vfe_hdmi_timing_info_t *hdmitiminginfo)
{
	int ret = 0;
	unsigned char port = hdmitiminginfo->port;

	unsigned char hdmiport = 0;
	unsigned int timing_ready = 0;

	if(port == VFE_HDMI_H5X_EXTERNAL_PORT_INDEX){
		return vfe_hdmi_drv_get_port_timing_info_h5x(hdmitiminginfo);
	}
	if (port > 3 && port !=HAL_VFE_HDMI_CURRENT_PORT)
		return -1;

	if (HDMI_Global_Status == SRC_CONNECT_DONE) {
		down(&HDMI_DetectSemaphore);

		if (vfe_hdmi_get_zero_timing_flag(port) & VFE_HDMI_REPORT_ZERO_TIMING) {
			vfe_hdmi_set_zero_timing_flag(port, VFE_HDMI_CLEAR_ZERO_FLAG);
			Set_Val_HDMI_Reply_Zero_Timing_mute_cnt(port, HDMI_REPLY_ZERO_TIMING_COUNT);
			HDMI_ZERO_REPORT_RECORD_TIME[port == HAL_VFE_HDMI_CURRENT_PORT? 0: port] = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
			/*
			 * FixMe, 20190919
			 *
			 * HDMI_EMG("HDMI_Reply_Zero_Timing is set,
			 * is current port, return zero timing for %d times,
			 * and reset flag (port=%d)\n",
			 * HDMI_Reply_Zero_Timing_mute_cnt, hdmitiminginfo.port);
			 */
		}

		if (Get_Val_HDMI_Reply_Zero_Timing_mute_cnt(port) == ACTUAL_DETECT_RESULT) {
			...
			if(Get_Val_HDMI_Reply_Zero_Timing_mute_cnt(port) == CHECK_TIME_PERIOD) {
				//Next time return actually timing
				if(!check_polling_time_period(HDMI_ZERO_REPORT_RECORD_TIME[port == HAL_VFE_HDMI_CURRENT_PORT? 0: port],
				  IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), HDMI_TIME_PERIOD_THRESHOULD)) {
					Set_Val_HDMI_Reply_Zero_Timing_mute_cnt(port, ACTUAL_DETECT_RESULT);
				}
			} else {
				/*
				* FixMe, 20190919
				* HDMI_EMG("HDMI_Reply_Zero_Timing_mute_cnt=%d > 0,
				* return zero timing\n", HDMI_Reply_Zero_Timing_mute_cnt);
				*/

				Set_Val_HDMI_Reply_Zero_Timing_mute_cnt(port, Get_Val_HDMI_Reply_Zero_Timing_mute_cnt(port) - 1);

				if (Get_Val_HDMI_Reply_Zero_Timing_mute_cnt(port) > HDMI_REPLY_ZERO_TIMING_COUNT) {
					/*
					 * FixMe, 20190919
					 *
					 * HDMI_EMG("Something wrong, HDMI_Reply_Zero_Timing_mute_cnt %d
					 * should less than %d !!!!!!!!\n",
...
EXPORT_SYMBOL(sm_vfe_hdmi_get_timing_info);

int sm_vfe_hdmi_disconnect(int port)
{
	int ret = 0;
	unsigned char i = 0;

	if(HDMI_Global_Status < SRC_CONNECT_DONE) {
		rtd_pr_vfe_debug("\r\n VFE_IOC_HDMI_DISCONNECT Fail. Source is not SRC_CONNECT_DONE, current : %d",HDMI_Global_Status);
		return -1; //Fail Source is not SRC_INIT_DONE
	}

	/* excute driver api */
	down(&HDMI_DetectSemaphore);
	hdmi_multi_port_conncet_status[port] = 0;
	for(i = 0; i < 4; i++){
		if(hdmi_multi_port_conncet_status[i] != 0)
			break;
	}
	// all port not connect
	if(i == 4){
		HDMI_Global_Status = SRC_OPEN_DONE;
	}
	up(&HDMI_DetectSemaphore);

    ret = vfe_hdmi_drv_disconnect(port);
	if (ret) {
        ret = -1;
		return ret;
	} else {
		...
		Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_HDMI, CLEAR_ZERO_FLAG, port);
		up(&SetSource_Semaphore);
	}
    return ret;
}
EXPORT_SYMBOL(sm_vfe_hdmi_disconnect);
...
//	abl_reserved_RBUS abl_Reserved_reg;//for debug test
	...
	vfe_hdmi_bypass_mode_set_t bypass_mode_control;
	...
	switch(cmd)
	{
				...
				if (ADC_Reply_Zero_Timing == SOURCE_GOOD_TIMING_READY) {
					...
					if(p_dispinfo->IPH_ACT_WID_PRE != 0 && p_dispinfo->IPV_ACT_LEN_PRE != 0)
					{
						if(p_dispinfo->IPH_ACT_WID_PRE == 4095 && p_dispinfo->IPV_ACT_LEN_PRE == 4095) {
							...
							adctiminginfo.vFreq = p_dispinfo->IVFreq_1000 ? (p_dispinfo->IVFreq_1000 / 100) : 600;
						} else {
							adctiminginfo.active.w = p_dispinfo->IPH_ACT_WID_PRE;
							adctiminginfo.active.h = p_dispinfo->IPV_ACT_LEN_PRE;
							...
							adctiminginfo.vFreq = p_dispinfo->IVFreq_1000 / 100;
		...
		case VFE_IOC_AVD_ATV_DISCONNECT:
			if (SRC_CONNECT_DONE == ATV_Status && SRC_CONNECT_DONE == AVD_Global_Status)
			{
				...
				Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_AVD, CLEAR_ZERO_FLAG, 0);
		...
		case VFE_IOC_AVD_GET_TIMINGINFO:
			...
			if (AVD_Global_Status == SRC_CONNECT_DONE) {
				...
				if (copy_to_user((int __user *)arg, (const void * )ptTimingInfo, sizeof(KADP_VFE_AVD_TIMING_INFO_T))) {
					...
					//rtd_pr_vfe_notice("NaOK,s=%d,w=%d,h=%d,f=%d\n", ptTimingInfo->standard,ptTimingInfo->active.w,ptTimingInfo->active.h,ptTimingInfo->vFreq_1000);
		...
		{
			ret = sm_vfe_hdmi_init();
			if(ret == 0){
rtd_pr_vfe_warn("HDCP sm_vfe_hdmi_init load hdcp key\n");
#ifndef UT_flag
				vfe_hdmi_hdcp2_loadkey();
				vfe_hdmi_hdcp14_loadkey(1);
#endif
			}
			break;
		}
...
//			DRB_VOINFO  voTimingInfo_pr;
			...
			voTimingInfo_drvbase.v_freq = voTimingInfo->v_freq_1000 / 100;
...
*/
...
#ifdef CONFIG_USE_VIDEO_COMMON_CONNECT_IOCTL
		case VFE_IOC_SET_VIDEO_COMMON:
		{
			unsigned char vc_id, vfe_sel,  port_id;
			unsigned char vc_port;
			vc_id = VIDEO_COMMON_VFE_COMMAND_DECONDE_VC_ID(arg); //1~4 : VC1/VC2/VC3/VC4. 0: disconnect VC
			vfe_sel = VIDEO_COMMON_VFE_COMMAND_DECONDE_VFE_SEL(arg); // vfe_sel: encode by src_type & port_id
			port_id = VIDEO_COMMON_VFE_SEL_DECODE_PORT(vfe_sel); // port_index
			
			rtd_pr_vfe_warn("VFE_IOC_SET_VIDEO_COMMON. Arg:%d, vc:%d, vfe_sel:%d, port:%d, IsDP:%d\n",arg,vc_id,vfe_sel, port_id,VIDEO_COMMON_VFE_SEL_IS_DPRX(vfe_sel));

			switch(vc_id)
			{
				case 0:
					vc_port = VIDEO_COMMON_NONE;
					break;
				case 1:
					vc_port = VIDEO_COMMON_0;
					break;
				case 2:
					vc_port = VIDEO_COMMON_1;
					break;
				case 3:
					vc_port = VIDEO_COMMON_2;
					break;
				case 4:
					vc_port = VIDEO_COMMON_3;
					break;
			}

			if(VIDEO_COMMON_VFE_SEL_IS_DPRX(vfe_sel))
			{

				drvif_Dprx_SetPortVideoCommonSel(port_id, vc_id);
				rtk_video_common_set_ownership(VIDEO_COMMON_DPRX, port_id, vc_port);
			}
			else if(VIDEO_COMMON_VFE_SEL_IS_HDMI(vfe_sel))
			{
				rtk_video_common_set_ownership(VIDEO_COMMON_HDMIRX, port_id, vc_port);
			}
...
#endif
		case VFE_IOC_GET_VIDEO_COMMON_COUNT:
		{
			unsigned char vc_cnt = 1;
			ret = 0;
#ifdef CONFIG_ARCH_RTK2819A
			vc_cnt = 4; // mac9q have 4 VC.
#endif
			/* update info to user */
			if (copy_to_user((void *)arg, &vc_cnt, sizeof(unsigned char)))
			{
				rtd_pr_vfe_debug("[ERR] copy_to_user error\n");
				ret = EFAULT; //bad addr
			}
		...
		case VFE_IOC_HDMI_BYPASS_MODE_SET:
		{
			/* get port info */
			if (copy_from_user(&bypass_mode_control, (const void __user *)arg, sizeof(vfe_hdmi_bypass_mode_set_t))) {
				rtd_pr_vfe_warn("[VFE_IOC_HDMI_BYPASS_MODE_SET] copy_from_user erro\n");
				ret = EFAULT; //bad addr
			}
			else
			{
				/* excute driver api */
				vfe_hdmi_bypass_mode_set(bypass_mode_control.port, bypass_mode_control.bypass_mode);
			}
			break;
		}
		...
		case VFE_IOC_HDMI_QS_INIT:
		{
            ...
            if(ret == 0){
                rtd_pr_vfe_warn("HDCP vfe_hdmi_qs_drv_init load hdcp key\n");
#ifndef UT_flag
                vfe_hdmi_hdcp2_loadkey();
                vfe_hdmi_hdcp14_loadkey(0);  //hdcp vcpu will write key, kdriver only load
#endif
		...
		case VFE_IOC_HDMI_GET_FRAME_RATE:
		{
			vfe_hdmi_frame_rate_t frame_rate = {0xff, 0, 0};

			if (copy_from_user((void *)&frame_rate, (const void __user *)arg, sizeof(vfe_hdmi_frame_rate_t)))
			{
				rtd_pr_vfe_debug("[ERR] VFE_IOC_HDMI_GET_FRAME_RATE copy_from_user error\n");
				ret = -EFAULT;
			}
			else
			{
				ret = vfe_hdmi_drv_get_GetCurrentPrescionFrameRate(frame_rate.port, &frame_rate.baseFrameRate, &frame_rate.currentVRRFrameRate);
				/* update info to user */
				if (!ret && copy_to_user((void *)arg, &frame_rate, sizeof(vfe_hdmi_frame_rate_t)))
				{
					rtd_pr_vfe_debug("[ERR]get frame rate error\n");
					ret = -EFAULT; //bad addr
				}
			}
			break;
		}
		case VFE_IOC_HDMI_IS_FAST_SWITCH:
		{
			ret = 0;
			hdmi_value=1;
			/* update info to user */
			if (copy_to_user((void *)arg, &hdmi_value, sizeof(unsigned char)))
			{
				rtd_pr_vfe_warn("[ERR] copy_to_user error\n");
				ret = EFAULT; //bad addr
			}
			break;
		}
#ifdef VFE_IOC_HDMI_HDCP_REPEATER_MODE
		case VFE_IOC_HDMI_HDCP_REPEATER_MODE:
		{
			unsigned char value;
			if (copy_from_user(&value, (const void __user *)arg, sizeof(unsigned char)) == 0) {
				rtd_pr_vfe_warn("[VFE_IOC_HDMI_HDCP_REPEATER_MODE] value: %d\n", value);
				ret = vfe_hdmi_drv_hdcp_repeater_en(value);
			}
			break;
		}
#endif
...
{
	rtd_printk(KERN_NOTICE, TAG_NAME_VFE, "do vfe_probe\n");
	return 0;
}
*/
/*
...
{
	rtd_printk(KERN_NOTICE, TAG_NAME_VFE, "do vfe_remove\n");
	return 0;
}
*/
static void vfe_shutdown(struct platform_device *pdev)
{
	rtd_printk(KERN_NOTICE, TAG_NAME_VFE, "do vfe_shutdown\n");
	vfe_hdmi_drv_shutdown();
}
#endif
...
#ifndef UT_flag
atomic_t *sm_state_vfe = NULL;
EXPORT_SYMBOL(sm_state_vfe);
#endif
...
#ifndef UT_flag
    if (sm_state_vfe) {
        atomic_set(sm_state_vfe, 1);
        rtd_pr_sm_emerg("Source Monitor State = Polling Source\n");
    }
#endif
#endif /* CONFIG_RTK_KDRV_QUICKSHOW */
	...
	switch(Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
	{
		case VSC_INPUTSRC_AVD:
			if (_SRC_TV != AVD_Input_Source) {
				...
				Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_AVD, REPORT_ZERO_TIMING, 0);
		...
		case VSC_INPUTSRC_HDMI:
			...
			if	(HDMI_Global_Status == SRC_CONNECT_DONE) {
				...
				if(decide_do_hdmi_error_handler())
					vfe_hdmi_drv_handle_on_line_measure_error(port, 1);//k3l error will mark 0603
#endif
				Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_HDMI, REPORT_ZERO_TIMING, port);
				HDMI_set_detect_flag(TRUE);
			}
		...
		case VSC_INPUTSRC_DP:
			...
			vfe_dprx_drv_handle_online_measure_error(port);
			if (DPRX_Global_Status == SRC_CONNECT_DONE) {
				Set_Reply_Zero_Timing_Flag(VSC_INPUTSRC_DP, REPORT_ZERO_TIMING, port);
				//HDMI_set_detect_flag(TRUE);
			}
			//Scaler_Disp3dSetInfo(SLR_MAIN_DISPLAY, SLR_DISP_3D_3DTYPE, SLR_3D_2D_ONLY);
			//Scaler_Set3DMode_Attr(SLR_MAIN_DISPLAY, SLR_3DMODE_2D);
...
{
	unsigned char port = Get_DisplayMode_Port(SLR_SUB_DISPLAY);
...
//	drvif_scaler_voFrameRateControl_init();
	...
	if(is_QS_hdmi_enable() || is_QS_dp_enable()){
...
void HDMI_set_detect_flag(unsigned char enable)//Set HDMI detect timing flag.
{
	...
	if(HDMI_DetectFlag != enable)
	{
		...
		rtd_pr_vfe_info("### HDMI_set_detect_flag enable:%d ###\r\n", enable);
	}
}
#ifndef BUILD_QUICK_SHOW
#ifndef UT_flag
