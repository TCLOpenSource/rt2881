...
void scalerdisplay_set_main_act_window(UINT16 DispHSta, UINT16 DispHEnd, UINT16 DispVSta, UINT16 DispVEnd)
{
	...
	scaler_dtg_get_main_active_start_end(&hv_info);
	hv_info.h_start = DispHSta;
	hv_info.h_end = DispHEnd;
	hv_info.v_start = DispVSta;
	hv_info.v_end = DispVEnd;
	scaler_dtg_set_main_active_start_end(&hv_info);
...
#ifdef SCALER_ERROR // Scaler fix compiler error for mac9q  //
...
//#ifdef CONFIG_MEMC_BYPASS
	if(get_MEMC_bypass_status_refer_platform_model() == TRUE){
		main_den_h_start_end_reg.mh_den_sta = (DispHSta)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_den_h_start_end_reg.mh_den_end = (DispHEnd)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_background_h_start_end_reg.mh_bg_sta = (DispHSta + Border)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
		main_background_h_start_end_reg.mh_bg_end = (DispHEnd - Border)/(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) +1);
//#else
	}else{
#ifndef BUILD_QUICK_SHOW
	if(Get_MEMC_Enable_Dynamic() == 0){
		...
		main_den_h_start_end_reg.mh_den_sta = DispHSta;
		main_den_h_start_end_reg.mh_den_end = DispHEnd;
		main_background_h_start_end_reg.mh_bg_sta = (DispHSta + Border);
		main_background_h_start_end_reg.mh_bg_end = (DispHEnd - Border);
		}
	}
//#endif
...
void scalerdisplay_set_main_display_window_V(unsigned short DispVSta, unsigned short DispVEnd, unsigned char Border)
{
	...
	main_den_v_start_end_reg.mv_den_sta = DispVSta;
	main_den_v_start_end_reg.mv_den_end = DispVEnd;

	main_background_v_start_end_reg.mv_bg_sta = (DispVSta + Border);
	main_background_v_start_end_reg.mv_bg_end = (DispVEnd - Border);
...
unsigned char get_use_tconless_force_bg_setting_en(void)
{
#ifndef UT_flag
#ifndef BUILD_QUICK_SHOW
    if(pPanelCOnfigParameter == NULL)
        return 0;
    if(strcmp(pPanelCOnfigParameter->sPanelName, "P_MINI_LVDS_BOE_HD_F56_V40_PANEL") == 0)
        return 1;
    else
#endif
#endif
...
void drvif_scalerdisplay_set_panel_bg_color_for_tconless(unsigned int ColorR, unsigned int ColorG, unsigned int ColorB)
{//set panel forcebg color
    sfg_sfg_patgen_0_RBUS sfg_sfg_patgen_0_reg;

    sfg_sfg_patgen_0_reg.regValue = IoReg_Read32(SFG_SFG_patgen_0_reg);
    sfg_sfg_patgen_0_reg.sfg_pat_r0 = ColorR;
    sfg_sfg_patgen_0_reg.sfg_pat_g0 = ColorG;
    sfg_sfg_patgen_0_reg.sfg_pat_b0 = ColorB;
    IoReg_Write32(SFG_SFG_patgen_0_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_1_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_2_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_3_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_4_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_5_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_6_reg, sfg_sfg_patgen_0_reg.regValue);
    IoReg_Write32(SFG_SFG_patgen_7_reg, sfg_sfg_patgen_0_reg.regValue);
}
...
{//set panel forcebg color
    ...
    if(get_use_tconless_force_bg_setting_en())
    {
        ColorR = ColorR >> 2;
        ColorG = ColorG >> 2;
        ColorB = ColorB >> 2;
        drvif_scalerdisplay_set_panel_bg_color_for_tconless(ColorR,ColorG,ColorB);
        return;
    }
...
void drvif_scalerdisplay_set_freeze(unsigned char display, unsigned char bFreeze)
{
	...
	if(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC) ||( (Scaler_InputSrcGetType((SCALER_DISP_CHANNEL)display) == _SRC_VO) && (Scaler_VOFrom(Scaler_Get_CurVoInfo_plane())==1)) ){//JPEG_mark:need to check
		if(bFreeze)
			drvif_scaler_set_display_mode(DISPLAY_MODE_FRC, scaler_dtg_get_app_type());
		else
	...
	if(!Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC)) {
		if(bFreeze){
#ifdef CONFIG_DUAL_CHANNEL
			IoReg_SetBits(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CHANNEL) ? MDOM_SUB_CAP_Cap1_buf_control_reg : MDOMAIN_CAP_Cap0_buf_control_reg, _BIT1);
#else
			IoReg_SetBits(MDOMAIN_CAP_Cap0_buf_control_reg, _BIT1);
#endif

#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
			display ? IoReg_BusyCheckRegBit(MDOM_SUB_CAP_Cap1_buf_control_reg, _BIT6,0x1FFFF) : IoReg_BusyCheckRegBit(MDOMAIN_CAP_Cap0_buf_control_reg, _BIT6,0x1FFFF);
#else
			IoReg_BusyCheckRegBit(MDOMAIN_CAP_Cap0_buf_control_reg, _BIT6,0x1FFFF);
#endif
		} else {
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
			IoReg_ClearBits(Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CHANNEL) ? MDOM_SUB_CAP_Cap1_buf_control_reg : MDOMAIN_CAP_Cap0_buf_control_reg, _BIT1);
#else
...
//			if((GET_DRAGON_IC_TYPE() ==  _DRAGON_IC_DRAGON_P) && (GET_INTERLACE_MODE(info->display) || GET_RTNR_MODE(info->display)) ) {
	...
	//			IoReg_Mask32(MDOMAIN_DDR_MAINSTATUS_VADDR, ~(_BIT1 | _BIT0), _BIT1 | _BIT2);
		...
		if (display == SLR_MAIN_DISPLAY) {
			...
			mdomain_cap_cap0_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap0_reg_doublbuffer_reg);
			while( mdomain_cap_cap0_reg_doublbuffer_reg.cap0_db_apply) {
				timeout++;
				msleep(10);
				mdomain_cap_cap0_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap0_reg_doublbuffer_reg);
				if(timeout >= 5)
			...
			mdom_sub_cap_cap1_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOM_SUB_CAP_cap1_reg_doublbuffer_reg);
			while( mdom_sub_cap_cap1_reg_doublbuffer_reg.cap1_db_apply) {
				timeout++;
				msleep(10);
				mdom_sub_cap_cap1_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOM_SUB_CAP_cap1_reg_doublbuffer_reg);
				if(timeout >= 5)
...
unsigned char fw_scaler_cal_sdnr_capture_for_hal_orgInput(unsigned char display)
{
	...
	if ( ((src == _SRC_HDMI) || (src == _SRC_VO)) &&
		 ((colorspace != SCALER_INPUT_COLOR_YUV444) && (colorspace!= SCALER_INPUT_COLOR_RGB)) ) {
...
unsigned char fw_scaler_cal_sdnr_capture(unsigned char display)
{
	...
	//when input 422 cap, m-domain hsta & hact must be even
	if((Scaler_DispGetStatus((SCALER_DISP_CHANNEL)display,SLR_DISP_422CAP) && Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC)) || (SLR_SUB_DISPLAY == (SCALER_DISP_CHANNEL)display))
	{
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_STA) & ~0x1));
		Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID, (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_SDNRH_ACT_WID) & ~0x1));
	}
...
{//for 0xb8028028
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(En) {
			IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL4_reg, PPOVERLAY_Double_Buffer_CTRL4_dsubreg_dbuf_en_mask  | PPOVERLAY_Double_Buffer_CTRL4_dsubreg_dbuf_set_mask);
		} else {
			IoReg_Mask32(PPOVERLAY_Double_Buffer_CTRL4_reg, ~PPOVERLAY_Double_Buffer_CTRL4_dsubreg_dbuf_en_mask, 0);
		}
...
{
    if(PPOVERLAY_Display_Timing_CTRL1_get_dout_force_bg(IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg))
        || PPOVERLAY_Main_Display_Control_RSV_get_m_force_bg(IoReg_Read32(PPOVERLAY_Main_Display_Control_RSV_reg))) {
        return TRUE;
    } else {
        return FALSE;
    }
...
void drvif_scalerdisplay_set_sharpness_line_mode(void)
{
	...
	if(drv_memory_get_game_mode() ==_ENABLE){
		display_timing_ctrl2_reg.shpnr_line_mode_sel = 3;
	}
	else if(_DISP_WID < 2592){//2560+32
		display_timing_ctrl2_reg.shpnr_line_mode_sel = 0;
	}else{
		display_timing_ctrl2_reg.shpnr_line_mode_sel = 2;
	}
    ...
    IoReg_SetBits(PPOVERLAY_Double_Buffer_CTRL_reg, PPOVERLAY_Double_Buffer_CTRL_dreg_dbuf_set_mask);
}
...
void scaler_dtg_reconfig(void)
{
	...
	rtd_pr_vbe_debug("[scaler_dtg_reconfig] dtg 0xb802800c ~ 0xb8028018\n");
		...
		dv_total_reg.dv_total = decide_vbe_disp_vtotal(VBE_DVTOTAL_BY_DISP,0,0,0,0) - 1;
...
void Scaler_ForcePanelBg_for_tconless(unsigned char enable)
{
...
void Scaler_ForcePanelBg(unsigned char enable)
{
    ...
    if(get_use_tconless_force_bg_setting_en())
    {
        Scaler_ForcePanelBg_for_tconless(enable);
        return;
    }
	WaitFor_DEN_STOP();
	sfg_force_bg_at_dif_reg.regValue = IoReg_Read32(SFG_SFG_FORCE_BG_AT_DIF_reg);
