#include "common_header/rtd2819a/include/scaler/scalerDrvCommon.h"
...
// #include <hdmirx/hdmi_reg.h> // fixme 6982
...
bool is_dolby_vision_hdmi_sink_led(unsigned char port);
...
static DEFINE_SPINLOCK(DolbyVision_Video_Correspond_Data_lock);
...
unsigned char HDR_DolbyVision_HDMI_check_mode(void)
{
	...
	return is_dolby_vision_hdmi_sink_led(port);
}
...
void mdomain_capture_disable(int enable)
{
		...
		if(enable){
			mdomain_cap_cap0_buf_control_reg.cap0_buf_num = 0;
		}
		else{
			mdomain_cap_cap0_buf_control_reg.cap0_buf_num = 2;
		}
...
static void disp_buffer(bool enable)
{
    ...
    if (enable) {
        ...
        block_sel =  mdomain_disp_disp0_buf_status_0_reg.disp0_buf_index_real;
        ...
        mdomain_disp_frc_disp0_buffer_select_control_reg.disp0_buf_force_addr = block_sel;
...
void mdomain_capture_freeze(int enable)
{
    ...
    if (enable) {
			mdomain_cap_cap0_buf_control_reg.cap0_buf_freeze_en = 1;
			disp_buffer(true);
	} else {
	...
	if(mdomain_cap_cap0_reg_doublbuffer_reg.cap0_db_en){
		mdomain_cap_cap0_reg_doublbuffer_reg.cap0_db_apply = 1;
		rtd_outl(MDOMAIN_CAP_cap0_reg_doublbuffer_reg, mdomain_cap_cap0_reg_doublbuffer_reg.regValue);
	}
...
static unsigned char is_i3ddma_triple_buffer(void)
{
	h3ddma_cap0_cap_ctl0_RBUS h3ddma_cap0_cap_ctl0_reg;
...
{
	uint8_t *buffer = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache;

	return buffer ? buffer + 0x30000 : NULL;
}

void reset_dolbyVision_HDMI_video_parameter(void)
{
	...
	if(dolbyVideoCor) {
		spin_lock_irqsave(&DolbyVision_Video_Correspond_Data_lock, flags);
		dolbyVideoCor->video0.metadataBuffer=0xFF;
		dolbyVideoCor->video0.frameIndex=0xFF;
		dolbyVideoCor->video1.metadataBuffer=0xFF;
		dolbyVideoCor->video1.frameIndex=0xFF;
		dolbyVideoCor->video2.metadataBuffer=0xFF;
		dolbyVideoCor->video2.frameIndex=0xFF;
		spin_unlock_irqrestore(&DolbyVision_Video_Correspond_Data_lock, flags);
	}
}
...
void HDR_DolbyVision_HDMI_md_packet_process(void)
{
	...
	unsigned char *md_m1_cache_adress = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache + 0x00;
	unsigned char *md_m2_cache_adress = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache + (0x10000);
	unsigned char *md_m3_cache_adress = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache + (0x20000);
	unsigned char *md_m4_cache_adress = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache + (0x30000);
	...
	if (((rtd_inl(H3DDMA_I3DDMA_MDP_CR_reg) & _BIT0) == 0) ||
		(i3ddmaCtrl[I3DDMA_DEV].cap_buffer[I3DDMA_MAIN_CAP_BUF_NUM].uncache == 0) ||
			...
			if(hdrDVShmAddr){
				...
				if(dolbyVideoCor) {
					...
					if(cap0_idx==0) {
						...
						dolbyVideoCor->video0.frameIndex=curWrIdx;
					}
					if(cap0_idx==1) {
						dolbyVideoCor->video1.metadataBuffer=rdIdx;
						dolbyVideoCor->video1.frameIndex=curWrIdx;
					}
					if(cap0_idx==2) {
						dolbyVideoCor->video2.metadataBuffer=rdIdx;
						dolbyVideoCor->video2.frameIndex=curWrIdx;
					}
					spin_unlock_irqrestore(&DolbyVision_Video_Correspond_Data_lock, flags);
				}

				//trigger dolby moves to video firmware
				if(VODMA_VODMA_V1_DCFG_dma_auto_mode(rtd_inl(VODMA_VODMA_V1_DCFG_reg))) {
...
void HDR_DolbyVision_HDMI_Flow_controller(void)
{
	...
	if(!get_hdr_semaless_active()/*(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE)*/ || ((rtd_inl(VGIP_VGIP_CHN1_CTRL_reg) & (_BIT0 | _BIT31)) != (_BIT0 | _BIT31))
		|| (i3ddmaCtrl[I3DDMA_DEV].cap_buffer[0].phyaddr == 0)) {
		return;
			}
...
void drvif_hdr_idma_color_colorspacergb2yuvtransfer(void)
{
	...
	h3ddma_0_rgb2yuv_dither_4xxto4xx_hdmi3_0_dither_ctrl1_RBUS hdmi_dither_ctrl1_reg;
	...
	rtd_outl(H3DDMA_0_RGB2YUV_DITHER_4XXTO4XX_HDMI3_0_DITHER_CTRL1_reg, hdmi_dither_ctrl1_reg.regValue);
	...
	if (is_dolby_vision_hdmi_sink_led(0)) {
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    h3ddma_hsd_i3ddma0_uzd_ctrl0_RBUS			i3ddma0_uzd_ctrl0_reg;
    h3ddma_hsd_i3ddma0_uzd_scale_hor_factor_RBUS	i3ddma0_uzd_scale_hor_factor_reg;
    h3ddma_hsd_i3ddma0_uzd_scale_ver_factor_RBUS   	i3ddma0_uzd_Scale_Ver_Factor_reg;
    h3ddma_hsd_i3ddma0_uzd_initial_value_RBUS		i3ddma0_uzd_initial_value_reg;
    h3ddma_hsd_i3ddma0_uzd_initial_int_value_RBUS	i3ddma0_uzd_initial_int_value_reg;

    i3ddma0_uzd_ctrl0_reg.regValue		= rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_CTRL0_reg);
    i3ddma0_uzd_scale_hor_factor_reg.regValue	= rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_SCALE_HOR_FACTOR_reg);
    i3ddma0_uzd_Scale_Ver_Factor_reg.regValue	= rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_SCALE_VER_FACTOR_reg);
    i3ddma0_uzd_initial_value_reg.regValue	= rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_INITIAL_VALUE_reg);
    i3ddma0_uzd_initial_int_value_reg.regValue	= rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_INITIAL_INT_VALUE_reg);
    ...
    if (in_Size->nWidth > out_Size->nWidth) {
        ...
        i3ddma0_uzd_ctrl0_reg.h_y_table_sel = 0;	// TAB1
        i3ddma0_uzd_ctrl0_reg.h_c_table_sel = 0;	// TAB1
        for (i=0; i<16; i++)
        {
            tmp_data = ((unsigned int)(*coef_pt++)<<16);
            tmp_data += (unsigned int)(*coef_pt++);
            rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_FIR_COEF_TAB1_C0_reg + i*4, tmp_data);
        }
    }
	...
	if (in_Size->nLength > out_Size->nLength) {
		...
		//o---------------- fill coefficients into access port--------------------o
		coef_pt = tScaleDown_COEF_TAB[SDFilter];
		i3ddma0_uzd_ctrl0_reg.v_y_table_sel = 1;	// TAB2
		i3ddma0_uzd_ctrl0_reg.v_c_table_sel = 1;	// TAB2
		for (i=0; i<16; i++)
		{
			tmp_data = ((unsigned int)(*coef_pt++)<<16);
			tmp_data += (unsigned int)(*coef_pt++);
			rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_FIR_COEF_TAB2_C0_reg + i*4, tmp_data);
		}
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if (in_Size->nWidth > out_Size->nWidth) {    // H scale-down
		Hini = 0x80;//0xff;//0x78;
		i3ddma0_uzd_initial_value_reg.hor_ini = Hini;
		i3ddma0_uzd_initial_int_value_reg.hor_ini_int= 1;
		{
		...
		//nFactor = SHR(nFactor + 1, 1); //rounding
		i3ddma0_uzd_scale_hor_factor_reg.hor_fac = nFactor;
	}else {
		i3ddma0_uzd_scale_hor_factor_reg.hor_fac = 0x100000;
	}

	if (in_Size->nLength > out_Size->nLength) {    // V scale-down
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0x02;
		Vini = 0x80;//0xff;//0x78;
		i3ddma0_uzd_initial_value_reg.ver_ini =Vini;
		...
		//nFactor = SHR(nFactor+1, 1); //rounding
		i3ddma0_uzd_Scale_Ver_Factor_reg.ver_fac = nFactor;
	}else{
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0;
		i3ddma0_uzd_Scale_Ver_Factor_reg.ver_fac = 0x100000;
	}

//	ich2_uzd_Ctrl0_REG.output_fmt = 1;
//	CLR_422_Fr_SD();
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_CTRL0_reg, i3ddma0_uzd_ctrl0_reg.regValue);
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_SCALE_HOR_FACTOR_reg, i3ddma0_uzd_scale_hor_factor_reg.regValue);
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_SCALE_VER_FACTOR_reg, i3ddma0_uzd_Scale_Ver_Factor_reg.regValue);
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_INITIAL_VALUE_reg, i3ddma0_uzd_initial_value_reg.regValue);
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_INITIAL_INT_VALUE_reg, i3ddma0_uzd_initial_int_value_reg.regValue);
...
void drvif_hdr_idma_ultrazoom_config_scaling_down(HDR_SZIE* in_Size, HDR_SZIE* out_Size, unsigned char panorama)
{
	...
	i3ddma0_uzd_ctrl0_reg.regValue = rtd_inl(H3DDMA_HSD_I3DDMA0_UZD_CTRL0_reg);
	i3ddma0_uzd_ctrl0_reg.h_zoom_en = 0;

	i3ddma0_uzd_ctrl0_reg.v_zoom_en = 0;
	i3ddma0_uzd_ctrl0_reg.out_bit = 0;
	i3ddma0_uzd_ctrl0_reg.sort_fmt = 1; // 444
	i3ddma0_uzd_ctrl0_reg.video_comp_en = 0;
	// i3ddma_vsd_ctrl0_reg.truncationctrl = 0; // fixme 6982
	rtd_outl(H3DDMA_HSD_I3DDMA0_UZD_CTRL0_reg, i3ddma0_uzd_ctrl0_reg.regValue);
...
bool get_dolby_vision_sdp_data_by_port(unsigned char port, DOLBY_DP_TYPE type, void *data)
{
	...
	if (ret_get_sdp < 0) {
		...
		rtd_pr_hdr_info("[%s-%d][DV] get dp data error (%d) type = %d %d\n", __func__, __LINE__, ret_get_sdp, sdp_type, type);
#endif
		return false;
	}
