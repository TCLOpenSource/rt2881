...
#define MAIN_MDISP_START_REGISTER					MDOMAIN_DISP_FRC_DISP0_RD_Ctrl_reg
#define MAIN_MDISP_END_REGISTER						MDOMAIN_DISP_DISP0_PQ_DECMP_SAT_EN_reg
...
#define MAIN_MDISP_DDR_range(addr)					((((addr>=MDOMAIN_DISP_FRC_DISP0_RD_Ctrl_reg) &&(addr <=MDOMAIN_DISP_FRC_DISP0_RD_Ctrl_reg)) \
												|| ((addr>=MDOMAIN_DISP_FRC_DISP0_num_bl_wrap_line_step_reg) && (addr<=MDOMAIN_DISP_FRC_DISP0_num_bl_wrap_word_reg))  \
												|| ((addr>=MDOMAIN_DISP_FRC_DISP0C_num_bl_wrap_line_step_reg) && (addr<=MDOMAIN_DISP_FRC_DISP0C_num_bl_wrap_word_reg))  \
												|| ((addr>=MDOMAIN_DISP_DISP0_PQ_DECMP_reg)&&(addr<=MDOMAIN_DISP_DISP0_PQ_DECMP_SAT_EN_reg)) \
												|| ((addr>=MDOMAIN_DISP_DISP0_DMA_CROP_reg)&&(addr<=MDOMAIN_DISP_disp0_US_Ctrl_reg)) \
												|| (addr==MDOMAIN_DISP_FRC_DISP0C_RD_Ctrl_reg) \
												|| ((addr>=MDOMAIN_DISP_DISP0_EXTEND_0_reg)&&(addr<=MDOMAIN_DISP_DISP0_EXTEND_1_reg))) ? 1: 0 )
...
#define MAIN_2STEPUZU_START_REGISTER				0//TWO_STEP_UZU_MFLTI_Ctrl_reg
#define MAIN_2STEPUZU_END_REGISTER					0//TWO_STEP_UZU_ReadData_DATA_C4_SR_reg
#define MAIN_2STEPUZU_REGISTER_NUM					0//((MAIN_2STEPUZU_END_REGISTER -MAIN_2STEPUZU_START_REGISTER + 4)/4)
#define MAIN_2STEPUZU_register_range(addr)			0//	(((addr>=MAIN_2STEPUZU_START_REGISTER) &&(addr <=MAIN_2STEPUZU_END_REGISTER)) ? 1: 0 )
#ifdef CONFIG_HW_SUPPORT_TWOSTEP_SCALEUP
#define MAIN_2STEPUZU_DDR_range(addr)				(((addr==TWO_STEP_UZU_SR_Ctrl_reg) \
													||((addr>=TWO_STEP_UZU_DM_TWO_STEP_SR_Ctrl_reg) &&(addr <=TWO_STEP_UZU_DM_TWO_STEP_SR_Input_Size_reg)) \
													||((addr>=TWO_STEP_UZU_DM_TWO_STEP_SRMAIN_H_PARTIAL_DROP_reg) && (addr <=TWO_STEP_UZU_DM_TWO_STEP_SRMAIN_V_PARTIAL_DROP_reg))) ? 1: 0 )
#else
#define MAIN_2STEPUZU_DDR_range(addr) (0)
#endif

//nnsr   0xb8028c00 - 0xb8028df8
#define MAIN_NNSR_START_REGISTER				0//SRNN_PRE_POST_SRNN_PRE_POS_UZU_SEL_reg
#define MAIN_NNSR_END_REGISTER					0//SRNN_PRE_POST_srnn_prepost_timing_monitor_st_reg
#define MAIN_NNSR_REGISTER_NUM					0//((MAIN_NNSR_END_REGISTER -MAIN_NNSR_START_REGISTER + 4)/4)
#define MAIN_NNSR_register_range(addr)			0//	(((addr>=MAIN_NNSR_START_REGISTER) &&(addr <=MAIN_NNSR_END_REGISTER)) ? 1: 0 )
#define MAIN_NNSR_DDR_range(addr)					0//(((addr>=SRNN_SRNN_CTRL_reg) && (addr <=SRNN_SRNN_CTRL_reg)) ? 1: 0 )

//nnsr mask   0xb8079400 - 0xb807941c
#define MAIN_NNSR_MASK1_START_REGISTER				0//SRNN_SRNN_INPUT_SIZE
#define MAIN_NNSR_MASK1_END_REGISTER				0//	SRNN_SRNN_SU_RATIO_SEL
#define MAIN_NNSR_MASK1_REGISTER_NUM				0//	((MAIN_NNSR_MASK1_END_REGISTER - MAIN_NNSR_MASK1_START_REGISTER + 4)/4)
#define MAIN_NNSR_MASK1_register_range(addr)		0//		(((addr>=MAIN_NNSR_MASK1_START_REGISTER) &&(addr <=MAIN_NNSR_MASK1_END_REGISTER)) ? 1: 0 )
#define MAIN_NNSR_MASK1_DDR_range(addr)				0//	(((addr>=MAIN_NNSR_MASK1_START_REGISTER) && (addr <= MAIN_NNSR_MASK1_END_REGISTER))  ? 1: 0 )


//nnsr mask   0xb8079240 - 0xb8079284
#define MAIN_NNSR_MASK2_START_REGISTER				0//PQMASK_SRNN_PQMASK_SRNN_O_Scaleup_Ver
#define MAIN_NNSR_MASK2_END_REGISTER				0//	PQMASK_SRNN_PQMASK_SRNN_I_OutputSize_Ctrl2
#define MAIN_NNSR_MASK2_REGISTER_NUM				0//	((MAIN_NNSR_MASK2_END_REGISTER - MAIN_NNSR_MASK2_START_REGISTER + 4)/4)
#define MAIN_NNSR_MASK2_register_range(addr)		0//		(((addr>=MAIN_NNSR_MASK2_START_REGISTER) &&(addr <=MAIN_NNSR_MASK2_END_REGISTER)) ? 1: 0 )
#define MAIN_NNSR_MASK2_DDR_range(addr)				0//	(((addr>=MAIN_NNSR_MASK2_START_REGISTER) && (addr <= MAIN_NNSR_MASK2_END_REGISTER))  ? 1: 0 )
...
//sub m domain display 0xb8027400 - 0xB80279F8
#define SUB_MDISP_START_REGISTER				MDOM_SUB_DISP_FRC_DISP1_RD_Ctrl_reg
#define SUB_MDISP_END_REGISTER					MDOM_SUB_DISP_DISP1_PQ_DECMP_SAT_EN_reg
...
#define SUB_MDISP_DDR_range(addr)					((((addr>=MDOM_SUB_DISP_FRC_DISP1_RD_Ctrl_reg) &&(addr <=MDOM_SUB_DISP_FRC_DISP1_RD_Ctrl_reg)) \
												|| ((addr>=MDOM_SUB_DISP_FRC_DISP1_num_bl_wrap_line_step_reg)&&(addr<=MDOM_SUB_DISP_FRC_DISP1_num_bl_wrap_word_reg)) \
												|| ((addr>=MDOM_SUB_DISP_FRC_DISP1C_num_bl_wrap_line_step_reg) &&(addr <=MDOM_SUB_DISP_FRC_DISP1C_num_bl_wrap_word_reg)) \
												|| ((addr>=MDOM_SUB_DISP_DISP1_CROP_reg) &&(addr <=MDOM_SUB_DISP_DISP1_OUT_CROP_reg)) \
												|| ((addr>=MDOM_SUB_DISP_DISP1_EXTEND_0_reg) &&(addr <=MDOM_SUB_DISP_DISP1_EXTEND_1_reg)) \
												|| ((addr>=MDOM_SUB_DISP_FRC_disp1_buffer_select_control_reg) &&(addr <=MDOM_SUB_DISP_disp1_buffer_change_line_num_0_reg)) \
												|| ((addr>=MDOM_SUB_DISP_disp1_unpack_ctrl_reg) &&(addr <=MDOM_SUB_DISP_disp1_US_Ctrl_reg)) \
												|| ((addr>=MDOM_SUB_DISP_DISP1_PQ_DECMP_reg) &&(addr <=MDOM_SUB_DISP_DISP1_PQ_DECMP_SAT_EN_reg))) ? 1: 0 )
...
#define PST_DISPM_REGISTER_NUMBER (MAIN_MDISP_REGISTER_NUM + MAIN_UZU_REGISTER_NUM + MAIN_DTG_REGISTER_NUM + MAIN_2STEPUZU_REGISTER_NUM + MAIN_ORBIT_REGISTER_NUM + MAIN_LC_REGISTER_NUM + MAIN_DTG_IV2DV_REGISTER_NUM + MAIN_NNSR_REGISTER_NUM + MAIN_NNSR_MASK1_REGISTER_NUM + MAIN_NNSR_MASK2_REGISTER_NUM)
...
void drvif_scaler_pst_top_ctrl(bool enable, _PST_MODE_T mode)
{
	...
	rtd_outl(PST_I2RND_TOP_CTRL_SPECIAL_reg, top_ctrl_special.regValue);
...
unsigned char Scaler_pst_setbuffer(_PST_MODE_T mode)
{
	...
	if (mode == _MAIN_MD) {
		...
		pst_dispm_scpu_vir_addr.s0_nnsr_mask1_st_vir_addr	= pst_dispm_scpu_vir_addr.s0_nnsr_st_vir_addr		+ MAIN_NNSR_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
		pst_dispm_scpu_vir_addr.s0_nnsr_mask2_st_vir_addr	= pst_dispm_scpu_vir_addr.s0_nnsr_mask1_st_vir_addr		+ MAIN_NNSR_MASK1_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
		pst_dispm_scpu_vir_addr.s0_2stepuzu_st_vir_addr 	= pst_dispm_scpu_vir_addr.s0_nnsr_mask2_st_vir_addr		+ MAIN_NNSR_MASK2_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
		...
		pst_imd_scpu_vir_addr.s0_nnsr_mask1_st_vir_addr	= pst_imd_scpu_vir_addr.s0_nnsr_st_vir_addr		+ MAIN_NNSR_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
		pst_imd_scpu_vir_addr.s0_nnsr_mask2_st_vir_addr	= pst_imd_scpu_vir_addr.s0_nnsr_mask1_st_vir_addr		+ MAIN_NNSR_MASK1_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
		pst_imd_scpu_vir_addr.s0_2stepuzu_st_vir_addr 	= pst_imd_scpu_vir_addr.s0_nnsr_mask2_st_vir_addr		+ MAIN_NNSR_MASK2_REGISTER_NUM * PST_REGISTER_DDR_SIZE;
