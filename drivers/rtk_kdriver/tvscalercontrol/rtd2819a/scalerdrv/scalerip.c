...
static void scalerip_bypass(unsigned char byPass)
{
	...
	//BringUp_Compiler vgip_data_path_select_sub_RBUS data_path_select_sub_reg;
	data_path_select_reg.regValue = IoReg_Read32(VGIP_Data_Path_Select_reg);
	//BringUp_Compiler data_path_select_sub_reg.regValue = IoReg_Read32(VGIP_Data_Path_Select_sub_reg);
	...
	if(byPass){ // bypass
		...
		if(display){
			...
			if ((Get_sub_from_DI_mux_path()) && (data_path_select_reg.uzd1_in_sel ==1)) {
				data_path_select_sub_reg.uzd2_in_sel = 1;
				//data_path_select_reg.xcdi_in_sel = 1;
			} else if ((Get_sub_from_DI_mux_path()) && (data_path_select_reg.uzd1_in_sel ==0)) {
				data_path_select_sub_reg.uzd2_in_sel = 3;
				//data_path_select_reg.xcdi_in_sel = 1;
			} else
				data_path_select_sub_reg.uzd2_in_sel = 0;
		...
		if(display){
			//BringUp_Compiler data_path_select_reg.xcdi_in_sel = 1;
			//BringUp_Compiler IoReg_Write32(VGIP_Data_Path_Select_reg, data_path_select_reg.regValue);
			//BringUp_Compiler data_path_select_sub_reg.uzd2_in_sel = 1;
			//BringUp_Compiler IoReg_Write32(VGIP_Data_Path_Select_sub_reg, data_path_select_sub_reg.regValue);
		}else{
			/* always on for snr hw issue, elieli*/
			if (drvif_color_Get_DRV_SNR_Clock(display, 0) == 1) {
 				{
					...
					//BringUp_Compiler data_path_select_reg.xcdi_in_sel = _CHANNEL1;
...
//	unsigned short temp;
	...
	//BringUp_Compiler unsigned int h_porch_num = 0;
	vgip_data_path_select_RBUS data_path_select_reg;
	//BringUp_Compiler vgip_data_path_select_sub_RBUS data_path_select_sub_reg;
	...
	//BringUp_Compiler vgip_vgip_chn1_misc_RBUS vgip_chn1_misc_reg;
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	//BringUp_Compiler sub_vgip_vgip_chn2_misc_RBUS vgip_chn2_misc_reg;
	//BringUp_Compiler sub_vgip_vgip_chn2_ctrl_RBUS vgip_chn2_ctrl_reg;
#endif
...
//	vp9_ctrl0_RBUS vodma_vp9_ctrl0_reg;
	...
	//	di_rtnr_demode_ctrl();//control de mode
		...
		{
			//BringUp_Compiler data_path_select_reg.regValue = IoReg_Read32(VGIP_Data_Path_Select_reg);
			//BringUp_Compiler data_path_select_reg.xcdi_in_sel = _CHANNEL2;
			//BringUp_Compiler IoReg_Write32(VGIP_Data_Path_Select_reg, data_path_select_reg.regValue);

			//BringUp_Compiler data_path_select_sub_reg.regValue = IoReg_Read32(VGIP_Data_Path_Select_sub_reg);
			//BringUp_Compiler data_path_select_sub_reg.uzd2_in_sel = _ENABLE;
			//BringUp_Compiler IoReg_Write32(VGIP_Data_Path_Select_sub_reg, data_path_select_sub_reg.regValue);
		}
...
void fw_scalerip_set_IEGSM(void)
{
	...
	drvif_sub_IESM_format_convert(); //drvif_IESM_format_convert() have to before drvif_IESM_frontporch_set()

	if (display == SLR_SUB_DISPLAY) {
		drvif_color_sub_iEdgeSmooth_init();
		drvif_sub_IESM_frontporch_set(0);
	}
}
