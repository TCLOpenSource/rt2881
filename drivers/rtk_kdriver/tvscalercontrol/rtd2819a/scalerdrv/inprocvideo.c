...
#define VDC_MEASURE_NTSC_VFREQ_MAX          62000
#define VDC_MEASURE_NTSC_VFREQ_MIN          58000
#define VDC_MEASURE_PAL_VFREQ_MAX           52000
#define VDC_MEASURE_PAL_VFREQ_MIN           48000
...
unsigned char drvif_inprocvideo_setup(void)
{
	...
	Scaler_DispSetInputInfo(SLR_INPUT_V_FREQ_1000,Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000));
...
//	vdc_mode_detect = ucVideoMode & 0xf;
	...
	if(drvif_mode_offlinemeasure(_OFFLINE_MS_SRC_VDC, ANALOG_MODE_MEASUREMENT, a_ptDispInfo, a_ptTimingInfo) == _MODE_SUCCESS)
	{
		...
		a_ptDispInfo->IVFreq_1000 = a_ptTimingInfo->IVFreq_1000;
...
//	vdc_mode_detect = ucVideoMode & 0xf;
	...
	if(drvif_mode_offlinemeasure(_OFFLINE_MS_SRC_VDC, ANALOG_MODE_MEASUREMENT, a_ptDispInfo, a_ptTimingInfo) == _MODE_SUCCESS)
	{
		usVFreq = a_ptTimingInfo->IVFreq_1000;
		ucRet = TRUE;
	}
	else
	{
...
void drvif_inprocvideo_offline_measure_check(SCALER_DISP_CHANNEL Display)
{
	...
	while (_FALSE == drvif_video_status_reg(VDC_hv_lock))
	{
		...
		if (counter >20)
		{
			...
			{
				Scaler_ModeSetModeInfo(SLR_MODE_IVFREQ_1000, 60000);
				rtd_pr_avd_debug("VDC offline measure %d times error, force Vfreq as 60Hz\n", VDC_RE_MEASURE_COUNTER);
			}
			else
			{
				Scaler_ModeSetModeInfo(SLR_MODE_IVFREQ_1000, 50000);
	...
	rtd_pr_avd_debug("CureentMode=%d, VdcMode=%d, SLR_MODE_IVFREQ_1000=%d\n", CureentMode, VdcMode, Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000));
	...
	case VDC_MODE_PALM:
		if(_MODE_480I == CureentMode)
		{
			if(Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000) < VDC_MEASURE_NTSC_VFREQ_MIN)
			{
				rtd_pr_avd_debug("VDC Error Case1 : SLR_MODE_IVFREQ_1000 : %x, too small!\n", Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000));
			}
			else
			{
				return;
	...
	default:
		...
		{
			if(Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000) > VDC_MEASURE_PAL_VFREQ_MAX)
			{
				rtd_pr_avd_debug("VDC Error Case4 : SLR_MODE_IVFREQ_1000 : %x, too large!\n", Scaler_ModeGetModeInfo(SLR_MODE_IVFREQ_1000));
			}
			else
			{
				return;
...
//	 vdc_mode_detect = ucVideoMode & 0xf;
	...
	if(drvif_mode_offlinemeasure(_OFFLINE_MS_SRC_VDC, ANALOG_MODE_MEASUREMENT, a_ptDispInfo, a_ptTimingInfo) == _MODE_SUCCESS)
	{
		...
		if (_MODE_480I == a_ptDispInfo->ucMode_Curr)
			a_ptDispInfo->IVFreq_1000 = a_ptTimingInfo->IVFreq_1000 = 60000;
		else
			a_ptDispInfo->IVFreq_1000 = a_ptTimingInfo->IVFreq_1000 = 50000;
