...
extern int scaler_dtg_get_abnormal_dvs_short_flag(DTG_TYPE dtg_type);
extern int scaler_dtg_get_abnormal_dvs_long_flag(DTG_TYPE dtg_type);
...
DC2H_Enable_rgb2yuv vr_colorformattransfer_enable(DC2H_IN_SEL dc2h_in_sel)
{
    ...
    if((get_VR_Pixel_Format() == VR_CAP_NV12)||(get_VR_Pixel_Format() == VR_CAP_NV16)||(get_VR_Pixel_Format() == VR_CAP_NV21))
    {
        ...
        {
            if((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_COLOR_SPACE) == SCALER_INPUT_COLOR_RGB) && (RGB2YUV_ICH1_RGB2YUV_CTRL_get_en_rgb2yuv(RGB2YUV_ICH1_RGB2YUV_CTRL_reg) == 0))
            {
    ...
    rtd_pr_vr_notice("[VR]get_VR_Pixel_Format()=%x, dc2h_in_sel=%x, dc2h_vr_en_rgb2yuv=%d\n",get_VR_Pixel_Format(), dc2h_in_sel,dc2h_enrgb2yuv_state);
    return dc2h_enrgb2yuv_state;

}

void vr_set_dc2h_cap0_line_burst_num(int burst_num, unsigned int capLen, int cap_mode)
{
	...
	dc2h_cap0_wr_dma_num_bl_wrap_addr_reg.dc2h_cap0_sta_addr = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr >> 4;
	if(Get_PANEL_VFLIP_ENABLE()){
	...
	dc2h_cap0_wr_dma_num_bl_wrap_ctl_reg.dc2h_cap0_burst_len = 0x10; /* dc2h_cap0_burst_len*128bit */
	...
	dc2h_cap0_dma_wr_rule_check_low_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
	IoReg_Write32(DC2H_CAP_DC2H_CAP0_DMA_WR_Rule_check_low_reg, dc2h_cap0_dma_wr_rule_check_low_reg.regValue);

	dc2h_cap0_dma_wr_rule_check_up_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + CaptureCtrl_VR.cap_buffer[0].size - 0x10;
	IoReg_Write32(DC2H_CAP_DC2H_CAP0_DMA_WR_Rule_check_up_reg, dc2h_cap0_dma_wr_rule_check_up_reg.regValue);
}

void vr_set_dc2h_cap1_line_burst_num(int burst_num, unsigned int capLen, int cap_mode)
{
	...
	dc2h_cap1_wr_dma_num_bl_wrap_addr_reg.dc2h_cap1_sta_addr = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr >> 4;
	if(Get_PANEL_VFLIP_ENABLE()){
	...
	dc2h_cap1_wr_dma_num_bl_wrap_ctl_reg.dc2h_cap1_burst_len = 0x10; /* dc2h_cap0_burst_len*128bit */
	...
	dc2h_cap1_dma_wr_rule_check_low_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
	IoReg_Write32(DC2H_CAP_DC2H_CAP1_DMA_WR_Rule_check_low_reg, dc2h_cap1_dma_wr_rule_check_low_reg.regValue);

	dc2h_cap1_dma_wr_rule_check_up_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + CaptureCtrl_VR.cap_buffer[0].size - 0x10;
...
void drvif_vr_color_ultrazoom_set_dc2h_scale_down(DC2H_HANDLER *pdc2h_Vrhdl)
{
	...
	if (pdc2h_Vrhdl->in_size.nLength > pdc2h_Vrhdl->out_size.nLength) {    // V scale-down
		...
		dc2h_uzd_initial_int_value_reg.ver_ini_int = 0;
...
void drvif_vr_color_ultrazoom_config_dc2h_scaling_down(DC2H_HANDLER *pdc2h_Vrhdl)
{
	...
	dc2h_uzd_ctrl1_reg.regValue = rtd_inl(DC2H_SCALEDOWN_DC2H1_UZD_CTRL1_reg);
	dc2h_uzd_ctrl1_reg.uzd_htotal = pdc2h_Vrhdl->in_size.nWidth;
	rtd_outl(DC2H_SCALEDOWN_DC2H1_UZD_CTRL1_reg, dc2h_uzd_ctrl1_reg.regValue);

	dc2h_uzd_ibuff_ctrl_reg.regValue = rtd_inl(DC2H_SCALEDOWN_DC2H1_UZD_IBUFF_CTRL_reg);
	dc2h_uzd_ibuff_ctrl_reg.ibuf_h_size = pdc2h_Vrhdl->out_size.nWidth;
	rtd_outl(DC2H_SCALEDOWN_DC2H1_UZD_IBUFF_CTRL_reg, dc2h_uzd_ibuff_ctrl_reg.regValue);

	dc2h_uzd_ctrl0_reg.regValue = rtd_inl(DC2H_SCALEDOWN_DC2H1_UZD_CTRL0_reg);
	dc2h_uzd_ctrl0_reg.h_zoom_en = (pdc2h_Vrhdl->in_size.nWidth > pdc2h_Vrhdl->out_size.nWidth);

	if(pdc2h_Vrhdl->in_size.nLength > pdc2h_Vrhdl->out_size.nLength){
		dc2h_uzd_ctrl0_reg.v_zoom_en = 1;
	}else{
		dc2h_uzd_ctrl0_reg.v_zoom_en = 0;
	}
...
void drvif_vr_dc2h_input_overscan_config(unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
	...
	// dc2h_cap_dc2h_3dmaskto2d_doublebuffer_RBUS dc2h_cap_3dmaskto2d_doublebuffer_reg;
	// DC2H_CAP_DC2H_3DMaskTo2D_doublebuffer_reg
	...
	dc2h_cap_overscan_ctrl0_reg.dc2h_overscan_right = x+w-1;
	...
	dc2h_cap_3dmaskto2d_ctrl_reg.dc2h_3dmaskto2d_en = 1;
...
void drvif_vr_vi_config(unsigned int nWidth, unsigned int nLength, unsigned int data_format)
{
	...
	cap0_total_bit = bitperpixel * nWidth;
	cap1_total_bit = bitperpixel * nWidth;
	...
	dc2h_cap_size_reg.dc2h_dma_hact = nWidth;
    dc2h_cap_size_reg.dc2h_dma_vact = nLength;
	...
	if(get_VR_Pixel_Format() == VR_CAP_NV12 || get_VR_Pixel_Format() == VR_CAP_NV16)
	{
		...
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_1byte_swap = 0;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_2byte_swap = 0;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_4byte_swap = 0;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_8byte_swap = 0;
		...
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_1byte_swap = 1;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_2byte_swap = 0;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_4byte_swap = 0;
		dc2h_cap1_dma_wr_ctrl_reg.dc2h_cap1_dma_8byte_swap = 0;
	}
	...
	dc2h_cap_color_format_reg.dc2h_frame_access_mode = 0;
	dc2h_cap_color_format_reg.dc2h_seq_blk_sel = 1;
	dc2h_cap_color_format_reg.dc2h_pixel_encoding = 2;
	dc2h_cap_color_format_reg.dc2h_bit_sel = 0;
	dc2h_cap_color_format_reg.dc2h_yv12_en = 0;
	dc2h_cap_color_format_reg.dc2h_p010_en = 0;
	dc2h_cap_color_format_reg.dc2h_cb_cr_swap = 0;
	dc2h_cap_color_format_reg.dc2h_rgb_to_argb_en = 0;
	dc2h_cap_color_format_reg.dc2h_channel_swap = 0;
	...
	vr_set_dc2h_cap0_line_burst_num(cap0_burst_num, nLength, 0);
	vr_set_dc2h_cap1_line_burst_num(cap1_burst_num, nLength / 2, 0);
...
void drvif_vr_dc2h_seq_config(DC2H_HANDLER *pdc2h_Vrhdl)
{
	...
	int cap0_burst_num  = 0;
	int bitperpixel = 0;
	// int odd_line = (pdc2h_Vrhdl->out_size.nLength) % 2;
	//VT_CAP_FRAME_LINE_MODE dc2h_cap_mode = DC2H_CAP_FRAME_MODE;
	...
	if(0/*dc2h_cap_mode == DC2H_CAP_FRAME_MODE*/)
	{
		cap0_total_bit = bitperpixel * pdc2h_Vrhdl->out_size.nWidth * pdc2h_Vrhdl->out_size.nLength;
	}else{
		cap0_total_bit = bitperpixel * pdc2h_Vrhdl->out_size.nWidth;
	}

	cap0_burst_num = ((cap0_total_bit + 127) / 128);
	
	dc2h_cap_size_reg.regValue = rtd_inl(DC2H_CAP_DC2H_CAP_Size_reg);
	dc2h_cap_size_reg.dc2h_dma_hact = pdc2h_Vrhdl->out_size.nWidth;
    dc2h_cap_size_reg.dc2h_dma_vact = pdc2h_Vrhdl->out_size.nLength;
    rtd_outl(DC2H_CAP_DC2H_CAP_Size_reg, dc2h_cap_size_reg.regValue);
	...
	dc2h_cap_color_format_reg.regValue = rtd_inl(DC2H_CAP_DC2H_CAP_Color_format_reg);
	dc2h_cap_color_format_reg.dc2h_frame_access_mode = 0;
	dc2h_cap_color_format_reg.dc2h_seq_blk_sel = 0;
	dc2h_cap_color_format_reg.dc2h_pixel_encoding = 0;
	dc2h_cap_color_format_reg.dc2h_bit_sel = 0;
	dc2h_cap_color_format_reg.dc2h_yv12_en = 0;
	dc2h_cap_color_format_reg.dc2h_p010_en = 0;
	dc2h_cap_color_format_reg.dc2h_cb_cr_swap = 0;
	if((get_VR_Pixel_Format() == VR_CAP_ABGR8888) || (get_VR_Pixel_Format() == VR_CAP_ARGB8888) || (get_VR_Pixel_Format() == VR_CAP_RGBA8888)){
		dc2h_cap_color_format_reg.dc2h_rgb_to_argb_en = 1;
		dc2h_cap_color_format_reg.dc2h_argb_dummy_loc = 0;
		dc2h_cap_color_format_reg.dc2h_argb_dummy_data = 0xFF;
	}else{
		dc2h_cap_color_format_reg.dc2h_rgb_to_argb_en = 0;
	}
	dc2h_cap_color_format_reg.dc2h_channel_swap = 5;
	rtd_outl(DC2H_CAP_DC2H_CAP_Color_format_reg, dc2h_cap_color_format_reg.regValue);
	...
	vr_set_dc2h_cap0_line_burst_num(cap0_burst_num, pdc2h_Vrhdl->out_size.nLength, 0);
...
void set_DC2H_rgb2yuv_ctrl_reg_refer_dump_location_vr(KADP_VR_DUMP_LOCATION_TYPE_T in_position, SIZE *in_size, unsigned char *dc2h_in_sel)
{
	...
	*dc2h_in_sel = in_sel;
	dc2h_cap_dc2h_ctrl_reg.dc2h_in_sel = in_sel;
	dc2h_cap_dc2h_ctrl_reg.dc2h_in_vact = in_size->nLength;
	rtd_outl(DC2H_CAP_DC2H_CTRL_reg, dc2h_cap_dc2h_ctrl_reg.regValue);

    // SDNR input W >4096 or V >2160, dc2h output capture max 2k
    if(in_size->nWidth > VR_CAP_FRAME_WIDTH_4K2K || in_size->nLength > VR_CAP_FRAME_HEIGHT_4K2K){
        if(pdc2h_Vrhdl->out_size.nWidth > VR_CAP_FRAME_WIDTH_2K1k || pdc2h_Vrhdl->out_size.nLength > VR_CAP_FRAME_HEIGHT_2K1k)
        {
            pdc2h_Vrhdl->out_size.nWidth = VR_CAP_FRAME_WIDTH_2K1k;
            pdc2h_Vrhdl->out_size.nLength = VR_CAP_FRAME_HEIGHT_2K1k;

            curCapInfo.capLen = VR_CAP_FRAME_HEIGHT_2K1k;
            curCapInfo.capWid =  VR_CAP_FRAME_WIDTH_2K1k;
            rtd_pr_vr_notice("[warning] Input Source Hact over 3840, output only support 2k1k!!!!\n");
        }
            rtd_pr_vr_notice("[warning] Input Source Hact over 3840 !!!!\n");
     }
}
...
void set_vr_vfod_freezed(unsigned char boffon)
{
	...
	if(((scaler_dtg_get_abnormal_dvs_long_flag(DTG_MASTER)==TRUE) || (scaler_dtg_get_abnormal_dvs_long_flag(DTG_MASTER)==TRUE))
...
void vr_set_DC2H_cap_boundaryaddr(unsigned int VrBufferNum)
{
	dc2h_cap_dc2h_cap0_dma_wr_rule_check_up_RBUS dc2h_cap0_dma_up_limit_reg;
	dc2h_cap_dc2h_cap0_dma_wr_rule_check_low_RBUS dc2h_cap0_dma_low_limit_reg;

	dc2h_cap_dc2h_cap1_dma_wr_rule_check_up_RBUS dc2h_cap1_dma_up_limit_reg;
	dc2h_cap_dc2h_cap1_dma_wr_rule_check_low_RBUS dc2h_cap1_dma_low_limit_reg;

	dc2h_cap_dc2h_cap2_dma_wr_rule_check_up_RBUS dc2h_cap2_dma_up_limit_reg;
	dc2h_cap_dc2h_cap2_dma_wr_rule_check_low_RBUS dc2h_cap2_dma_low_limit_reg;
	...
	{
		dc2h_cap0_dma_up_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - 0x10;
		IoReg_Write32(DC2H_CAP_DC2H_CAP0_DMA_WR_Rule_check_up_reg, dc2h_cap0_dma_up_limit_reg.regValue);
		dc2h_cap0_dma_low_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
		IoReg_Write32(DC2H_CAP_DC2H_CAP0_DMA_WR_Rule_check_low_reg, dc2h_cap0_dma_low_limit_reg.regValue);

		dc2h_cap1_dma_up_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - 0x10;
		IoReg_Write32(DC2H_CAP_DC2H_CAP1_DMA_WR_Rule_check_up_reg, dc2h_cap1_dma_up_limit_reg.regValue);
		dc2h_cap1_dma_low_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
		IoReg_Write32(DC2H_CAP_DC2H_CAP1_DMA_WR_Rule_check_low_reg, dc2h_cap1_dma_low_limit_reg.regValue);

		dc2h_cap2_dma_up_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr +  (UINT32)CaptureCtrl_VR.cap_buffer[0].size - 0x10;
		IoReg_Write32(DC2H_CAP_DC2H_CAP2_DMA_WR_Rule_check_up_reg, dc2h_cap2_dma_up_limit_reg.regValue);
		dc2h_cap2_dma_low_limit_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
		IoReg_Write32(DC2H_CAP_DC2H_CAP2_DMA_WR_Rule_check_low_reg, dc2h_cap2_dma_low_limit_reg.regValue);
	}
}
void drvif_vr_set_DC2H_CaptureConfig(VR_CUR_CAPTURE_INFO capInfo)
{
	if(capInfo.enable == TRUE){
		...
		if((get_VR_Pixel_Format() == VR_CAP_NV12)||(get_VR_Pixel_Format() == VR_CAP_NV16)||(get_VR_Pixel_Format() == VR_CAP_NV21))
		{		
			dc2h_cap_dc2h_cap0_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap0_sta_addr_reg;  /* y */
			dc2h_cap_dc2h_cap1_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap1_sta_addr_reg;  /* c */
			//dc2h_cap_dc2h_cap2_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap2_sta_addr_reg; 
			if(Get_PANEL_VFLIP_ENABLE())
			{
				dc2h_cap0_sta_addr_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + (vr_cap_frame_max_width*vr_cap_frame_max_height - vr_cap_frame_max_width));
				IoReg_Write32(DC2H_CAP_DC2H_CAP0_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap0_sta_addr_reg.regValue);

				if((get_VR_Pixel_Format() == VR_CAP_NV12) || (get_VR_Pixel_Format() == VR_CAP_NV21))
                    dc2h_cap1_sta_addr_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_bufAddr_align(vr_cap_frame_max_width*vr_cap_frame_max_height) + (vr_cap_frame_max_width*vr_cap_frame_max_height/2 - vr_cap_frame_max_width));
				else //NV16
                    dc2h_cap1_sta_addr_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_bufAddr_align(vr_cap_frame_max_width*vr_cap_frame_max_height) + (vr_cap_frame_max_width*vr_cap_frame_max_height - vr_cap_frame_max_width));
				IoReg_Write32(DC2H_CAP_DC2H_CAP1_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap1_sta_addr_reg.regValue);			
			}
			else
			{
				dc2h_cap0_sta_addr_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr;
				IoReg_Write32(DC2H_CAP_DC2H_CAP0_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap0_sta_addr_reg.regValue);

                dc2h_cap1_sta_addr_reg.regValue = (UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + curCapInfo.capWid*curCapInfo.capLen; //vr_bufAddr_align(vr_cap_frame_max_width*vr_cap_frame_max_height);
				IoReg_Write32(DC2H_CAP_DC2H_CAP1_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap1_sta_addr_reg.regValue);
				rtd_pr_vt_emerg("y/c reg %x,%x \n", IoReg_Read32(DC2H_CAP_DC2H_CAP0_WR_DMA_num_bl_wrap_addr_reg), IoReg_Read32(DC2H_CAP_DC2H_CAP1_WR_DMA_num_bl_wrap_addr_reg));			
			}
		...
		{
			dc2h_cap_dc2h_cap0_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap0_sta_addr_reg;
			dc2h_cap_dc2h_cap1_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap1_sta_addr_reg;  /* c */
			dc2h_cap_dc2h_cap2_wr_dma_num_bl_wrap_addr_RBUS dc2h_cap2_sta_addr_reg; 
            ...
            if(Get_PANEL_VFLIP_ENABLE())
            {
                if(get_VR_Pixel_Format() == VR_CAP_RGB888)
                    dc2h_cap0_sta_addr_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*3 - vr_cap_frame_max_width*3);
                else if(get_VR_Pixel_Format() == VR_CAP_ARGB8888 || get_VR_Pixel_Format() == VR_CAP_RGBA8888 || get_VR_Pixel_Format() == VR_CAP_ABGR8888)
                    dc2h_cap0_sta_addr_reg.regValue = ((UINT32)CaptureCtrl_VR.cap_buffer[0].phyaddr + vr_cap_frame_max_width*vr_cap_frame_max_height*4 - vr_cap_frame_max_width*4);
                else
            ...
            dc2h_cap1_sta_addr_reg.regValue = 0;
            IoReg_Write32(DC2H_CAP_DC2H_CAP1_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap1_sta_addr_reg.regValue);

            dc2h_cap2_sta_addr_reg.regValue = 0;
            IoReg_Write32(DC2H_CAP_DC2H_CAP2_WR_DMA_num_bl_wrap_addr_reg, dc2h_cap2_sta_addr_reg.regValue);
	...
	if(bEnableFRC == FALSE){
		if((get_film_mode_parameter().enable == _DISABLE) /*&& (Get_DISPLAY_REFRESH_RATE() != 120)*/){
			if(1 /*(Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE) > 2560) &&
				...
				if(1 /*Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) > 490*/){
...
{
#if 0 // Scaler fixme compiler error for register change
...
static void sb4_dc2h_debug_info(void)
{
	...
	rtd_pr_vr_err("TVSB4_DC2H module!\n");
