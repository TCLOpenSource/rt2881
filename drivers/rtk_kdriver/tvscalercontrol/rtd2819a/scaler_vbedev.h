...
typedef enum
{
    ULTRALOW_DELAY_MODE_DEFAULT,
    ULTRALOW_DELAY_MODE_IN,
    ULTRALOW_DELAY_MODE_OUT
}ULTRALOW_DELAY_MODE_MODE;
...
void scaler_disp_dtg_pre_i_domain_setting(unsigned char display, DTG_APP_TYPE dtgAppType);
...
void vbe_disp_set_force_panel_split_mode(unsigned char bEnable);
unsigned char vbe_disp_get_force_panel_split_mode(void);
void vbe_disp_config_split_mode_panel_interface(unsigned char bEnable);
unsigned char vbe_disp_set_VRR_fractional_framerate_mode(void);
unsigned char vbe_disp_get_VRR_timingMode_flag(void);
unsigned char get_VRR_timingMode_flag_by_display(SCALER_DISP_CHANNEL display);
unsigned char vbe_disp_get_VRR_ALLM_flag(void);
unsigned char vbe_disp_get_pre_VRR_ALLM_flag(void);
void vbe_disp_set_VRR_timingMode_flag(SCALER_DISP_CHANNEL display, unsigned char flag);
void vbe_disp_set_pre_VRR_ALLM_flag (unsigned char enable);
unsigned char vbe_disp_get_freesync_mode_flag(void);
unsigned char get_freesync_mode_flag_by_display(SCALER_DISP_CHANNEL display);
void vbe_disp_set_freesync_mode_flag (SCALER_DISP_CHANNEL display, unsigned char enable);
...
void Scaler_wait_for_input_one_frame(DTG_TYPE dtg_type, unsigned char display);
...
void panel_enable_ddr_crash_check(void);
#ifndef BUILD_QUICK_SHOW
...
unsigned int scaler_disp_dtg_get_time_borrow_value(unsigned char type);
void scaler_disp_dtg_set_s1ip_delay(unsigned char bEnable_DB);
unsigned int scaler_disp_dtg_get_s1ip_delay_value(unsigned char type);
void wait_dispd_stage1_smooth_toggle_apply_done(void);
void scaler_disp_dtg_set_db_latch_line(void);
...
void set_ultra_low_delay_mode(ULTRALOW_DELAY_MODE_MODE ultralowdelay);
ULTRALOW_DELAY_MODE_MODE get_ultra_low_delay_mode(void);
void set_ultra_low_delay_8644_compensate(unsigned int value);
unsigned int get_ultra_low_delay_8644_compensate(void);
void vbe_read_PMIC(void);
void vbe_disp_golden_vsync_formula(void);
unsigned int get_golden_vsync_line(void);
void set_golden_vsync_line(unsigned int line);
unsigned int get_golden_vsync_pixel(void);
void set_golden_vsync_pixel(unsigned int pixel);
...
int scaler_dtg_pre_i_domain_setting_display_monitor(unsigned char display);
int scaler_dtg_pre_i_domain_setting_dual_display(unsigned char display);
int scaler_dtg_wait_for_double_buffer_can_work(DTG_DOUBLE_BUFFER_DX dx, unsigned char vsync_sel);
int scaler_dtg_stage1_smooth_apply_pre_handler(void);
int scaler_dtg_wait_frame_cnt_align(DTG_TYPE dtg_type, unsigned int line_cnt_start, unsigned int line_cnt_end, unsigned char line_cnt_align);
void vbe_disp_vs_gen_by_hw_control_mipi(unsigned char enable);
void vbe_disp_en_control_mipi(unsigned char enable);
void vbe_disp_mipi_command_mode_initialization(void);
int scaler_dtg_pre_i_domain_setting_6_window(unsigned char display);
int scaler_dtg_pre_i_domain_setting_commercial_monitor(unsigned char display);
int scaler_dtg_pre_i_domain_setting_live_streaming(unsigned char display);
int scaler_dtg_pre_i_domain_setting_non_6_window(unsigned char display, OSD_SPLIT_TYPE osd_split);
int scaler_dtg_pre_i_domain_setting_stream_switcher(unsigned char display);
int scaler_dtg_disp_setting_timing_control_display_monitor(unsigned char display);
int scaler_dtg_handler_non_6_window(OSD_SPLIT_TYPE osd_split);
#endif
