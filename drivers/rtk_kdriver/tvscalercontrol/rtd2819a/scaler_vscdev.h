...
KADP_VIDEO_RECT_T get_dlg_convert_region(KADP_VIDEO_RECT_T outregion);
...
unsigned char check_hdmi_need_rerun_condition(unsigned char display, unsigned char *unstable_hander);
unsigned char check_dp_need_rerun_condition(unsigned char display, unsigned char *unstable_hander);
...
extern unsigned char (*scaler_force_run_idma)(unsigned char, unsigned char, VSC_INPUT_TYPE_T);
...
void Set_Force_run_fake_scaler_State(unsigned int state);
...
int change_h5x_vsc_source(unsigned char source_type);
