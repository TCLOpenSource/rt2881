...
void Scaler_SET_VSCDispinfo_WithVFEDispinfo(unsigned char display, StructDisplayInfo* VFEDispInfo)
{
	...
	Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000, VFEDispInfo->IVFreq_1000);
	...
	Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_VO_SRC_FRAMERATE_1000, VFEDispInfo->SrcIVFreq_1000);
	Scaler_DispSetInputInfoByDisp(display, SLR_INPUT_SOURCE_PIXEL_MODE, 0);//FIXME later
	...
	rtd_pr_vsc_debug("VSC timing info from VFE:SLR_INPUT_V_FREQ_1000:%d\n",Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000));
...
unsigned char vsc_timinginfo_check(void)
{
	...
	else if(Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) ==0)
	{
		return FALSE;
	}
...
unsigned char vsc_timinginfo_check_for_iddma(unsigned char dev)
{
	...
	if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_IPV_ACT_LEN_PRE)==0)
	{
		return FALSE;
	}
	else if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_IPH_ACT_WID_PRE)==0)
	{
		return FALSE;
	}
	else if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_H_FREQ) ==0)
	{
		return FALSE;
	}
	else if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_V_FREQ_1000) ==0)
	{
		return FALSE;
	}
	else if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_H_LEN) ==0)
	{
		return FALSE;
	}
	else if(IDDMA_GetVFEInfoByPort(dev, SLR_INPUT_V_LEN) ==0)
	{
		return FALSE;
	}
	else
...
unsigned int Scaler_ModeGetModeInfo(SLR_MODE_INFO infoList)
{
	switch (infoList)
	{
		...
		case SLR_MODE_IVFREQ_1000:
			return stModeInfo.IVFreq_1000;
		case SLR_MODE_IHLEN:
...
unsigned int Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_INFO infoList)
{
	switch (infoList)
	{
		...
		case SLR_MODE_IVFREQ_1000:
			return stHdmiModeInfo.IVFreq_1000;
		case SLR_MODE_IHLEN:
...
void Scaler_ModeSet_Hdmi_ModeInfo(SLR_MODE_INFO infoList, unsigned int data)
{
	switch (infoList)
	{
		...
		case SLR_MODE_IHFREQ:
			...
			break;
		case SLR_MODE_IVFREQ_1000:
			stHdmiModeInfo.IVFreq_1000 = data;
			break;
		case SLR_MODE_IHLEN:
...
// frank@1017 add below code to solve scan tool warning
    ...
    if(channel > SLR_SUB_DISPLAY) {
		rtd_pr_vsc_err("[Error] Wrong input channel:%d, force Main channel\n", channel);
		channel = SLR_MAIN_DISPLAY;
	}
...
unsigned int Scaler_DispGetInputInfo(SLR_INPUT_INFO infoList)
{
	return Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)info->display,infoList);
}

void Scaler_DispSetInputInfo(SLR_INPUT_INFO infoList, unsigned int value)
{
...
void Scaler_DispSetInputInfoByDisp(SCALER_DISP_CHANNEL channel, SLR_INPUT_INFO infoList, unsigned int value)
{
    if(channel > SLR_SUB_DISPLAY) {
		rtd_pr_vsc_err("[Error] Wrong input channel:%d, force Main channel\n", channel);
		channel = SLR_MAIN_DISPLAY;
	...
	switch (infoList)
	{
		...
		case SLR_INPUT_V_FREQ_1000:
			disp_info[channel].IVFreq_1000 = value;
			break;
		...
		case SLR_INPUT_COLOR_SPACE:
			...
			break;
		case SLR_INPUT_ORIGINAL_COLOR_SPACE:
			disp_info[channel].original_color_space= value;
			break;
		...
		case SLR_INPUT_VO_SRC_FRAMERATE_1000:
			disp_info[channel].SrcIVFreq_1000 = value;
			break;

        case SLR_INPUT_SUB_SNRR_ENABLE:
            disp_info[channel].subSrnnEnable = value;
            break;
		case SLR_INPUT_SOURCE_PIXEL_MODE:
			disp_info[channel].input_src_pixel_mode = value;
			break;
		case SLR_INPUT_VSC_SOURCE_TYPE:
			disp_info[channel].vsc_src_type = value;
			break;
		default:
...
unsigned int Scaler_DispGetInputInfoByDisp(unsigned char channel, SLR_INPUT_INFO infoList)
{
...
// frank@1017 add below code to solve scan tool warning
	...
	switch (infoList)
	{
		...
		case SLR_INPUT_V_FREQ:
			return disp_info[channel].IVFreq_1000 / 100;	// in digit 1 : reserved for dolby common code project
		case SLR_INPUT_V_FREQ_1000:
			return disp_info[channel].IVFreq_1000;
		case SLR_INPUT_H_LEN:
		...
		case SLR_INPUT_ORIGINAL_COLOR_SPACE:
			return disp_info[channel].original_color_space;
		case SLR_INPUT_COLOR_DEPTH:
		...
		case SLR_INPUT_VO_SRC_FRAMERATE_1000:
			return disp_info[channel].SrcIVFreq_1000;
        case SLR_INPUT_SUB_SNRR_ENABLE:
            return disp_info[channel].subSrnnEnable;
		case SLR_INPUT_SOURCE_PIXEL_MODE:
			return disp_info[channel].input_src_pixel_mode;
		case SLR_INPUT_VSC_SOURCE_TYPE:
			return disp_info[channel].vsc_src_type;
		default:
...
{
    StructSrcRect camera_output_region = {0, 0, Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPH_ACT_WID_PRE), Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_LEN_PRE)};
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA)
        custom_dispwin = camera_output_region;
    else
        custom_dispwin= m_dispwin;
...
{
	int res=0, hz=0;
	int mode, srcType, srcFrom, width=0, height=0, isI=0;
	unsigned int ivFreq=0;
	int tempHz = 0;
    if(channel > SLR_SUB_DISPLAY) {
		rtd_pr_vsc_err("[Error] Wrong input channel:%d, force Main channel\n", channel);
		channel = SLR_MAIN_DISPLAY;
...
// FixMe, 20140928
	...
	if(drvif_scaler3d_get_IDMA3dMode() != SLR_IDMA_MODE_DISABLE){
		I3DDMA_TIMING_T *i3ddma_timing = (I3DDMA_TIMING_T*)drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_TIMING);

		srcType = drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_SRC_TYPE);
		srcFrom = drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_SRC_FROM);
		mode = drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_MODE_CURR);
		if(i3ddma_timing){
			isI = !i3ddma_timing->progressive;
			width = i3ddma_timing->h_act_len;
			height = i3ddma_timing->v_act_len;
			ivFreq = i3ddma_timing->v_freq_1000;
			// monitor always show 72.8xx as 72Hz
			tempHz = ( i3ddma_timing->v_freq_1000 + 500 ) / 1000;
		}
	}else
#endif
...
#endif
	{
		...
		ivFreq = disp_info[channel].IVFreq_1000;
		// monitor always show 72.8xx as 72Hz
		tempHz = (int)(ivFreq + 500) / 1000;
	...
	if (ABS(ivFreq, 72800) < 800) {
		if(tempHz>72)
			tempHz--;
	}
#else
	if (ABS(ivFreq, 72800) <= 300) {
			tempHz--;
	}
#endif
