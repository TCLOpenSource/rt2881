...
extern unsigned int g_pq_bypass_lv_ctrl_by_AP;
...
extern struct semaphore VPQ_ColorMapping_Semaphore;
...
unsigned char g_reset_sharp_level_flag = 0;
//unsigned short res_check_last_input_w = 0;
unsigned short res_check_last_input_h = 0;
...
extern struct semaphore HDR_Setting_Semaphore;//this is for HDR V top setting Semaphore
...
StructColorDataFacModeType *fwif_color_get_color_fac_mode(unsigned char src_idx, unsigned char timing)
{
	...
	return (StructColorDataFacModeType *)m_cbFunc_getColorFacMode(0, 0, 0);// use same structure
}
...
SLR_COLORTEMP_DATA *scaler_get_color_tempset(unsigned char src_idx, unsigned char timing)
{
	...
	return (SLR_COLORTEMP_DATA *)m_cbFunc_getColorTempData(0, 0, 0);	// use same structure
}
...
SLR_COLORTEMP_LEVEL scaler_get_color_temp_level(unsigned char src_idx, unsigned char timing)
{
	...
	pValue = (unsigned char *)m_cbFunc_getColorTempLevel(0, 0, 0);	// use same structure
	if (pValue == NULL)
		return SLR_COLORTEMP_NORMAL;
...
unsigned char fwif_color_get_cur_input_timing_mode(unsigned char display)
{
		if (display == SLR_MAIN_DISPLAY)
			return drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_MODE_CURR);
		else
...
#endif
unsigned char fwif_vip_src_get_hdr_sel_index(unsigned char bHDR_flag, unsigned char bDolby_HDR_flag)
{
...
unsigned char fwif_vip_src_parsing_hdmi_new_mode(unsigned char bHDR_flag, unsigned char bDolby_HDR_flag)
{
	unsigned short hor = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IHWID);
	unsigned short ver = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVHEIGHT);
	const unsigned short h_enum[6] = { 3840, 2560, 1920, 1280, 720, 720};
	const unsigned short v_enum[6] = { 2160, 1440, 1080, 720, 480, 576};
	const unsigned char vipsrc_enum[3][6] = {
		{VIP_QUALITY_HDMI_4k2k, VIP_QUALITY_HDMI_1440P, VIP_QUALITY_HDMI_1080P, VIP_QUALITY_HDMI_720P, VIP_QUALITY_HDMI_480P, VIP_QUALITY_HDMI_576P},
		{VIP_QUALITY_HDR_HDMI_4k2k, VIP_QUALITY_HDR_HDMI_1440p, VIP_QUALITY_HDR_HDMI_1080p, VIP_QUALITY_HDR_HDMI_720P, VIP_QUALITY_HDR_HDMI_480P, VIP_QUALITY_HDR_HDMI_576P},
		{VIP_QUALITY_DolbyHDR_HDMI_4k2k, VIP_QUALITY_DolbyHDR_HDMI_1440P, VIP_QUALITY_DolbyHDR_HDMI_1080P, VIP_QUALITY_DolbyHDR_HDMI_720P, VIP_QUALITY_DolbyHDR_HDMI_480P, VIP_QUALITY_DolbyHDR_HDMI_576P},
	};
	int i;

	for (i = 0; i < 6; i++) {
		if (h_enum[i] == hor && v_enum[i] == ver)
			return vipsrc_enum[fwif_vip_src_get_hdr_sel_index(bHDR_flag, bDolby_HDR_flag)][i];
	}

	return VIP_QUALITY_VGA;
}
...
#define VIP_Source_Timing_V_ACT_TOLERANCE 5
#define VIP_Source_Timing_V_Len_480I 240
#define VIP_Source_Timing_V_Len_576I 288
#define VIP_Source_Timing_V_Len_1080I 540
#define VIP_Source_Timing_V_Len_480P 480
#define VIP_Source_Timing_V_Len_576P 576
#define VIP_Source_Timing_V_Len_720P 720
#define VIP_Source_Timing_V_Len_1080P 1080
#define VIP_Source_Timing_V_Len_1440P 1440
#define VIP_Source_Timing_V_Len_4K 2160
#define VIP_Source_Timing_MatchV(vi, vt) ((vi <= (vt+VIP_Source_Timing_V_ACT_TOLERANCE)) && (vi >= (vt-VIP_Source_Timing_V_ACT_TOLERANCE)))
#define VIP_Source_Timing_isHDR(bHDR_flag) ((bHDR_flag == HAL_VPQ_HDR_MODE_HDR10) || (bHDR_flag == HAL_VPQ_HDR_MODE_HLG) || (bHDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR))
unsigned char fwif_vip_source_check(unsigned char display, unsigned char callmode)
{
	SLR_VOINFO* pVOInfo = NULL;
	unsigned char vdecSrcType, srcType, port, isInterlace, input_src_type, input_src_form, which_source, isHDMI;
	unsigned char vdc_ColorStandard = 0, Dprx_GetInputType = 0;	
	...
	vdecSrcType = Get_Val_scaler_vdec_source_type(display);
	...
	v_len = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN);
	pVOInfo = Scaler_VOInfoPointer(port);	// use display to get vo info
	...
	isHDMI = drvif_Multi_port_IsHDMI(port);
	...
	{
		bHDR_flag = VIP_system_info_structure_table->HDR_flag_sub;
		bDolby_HDR_flag = VIP_system_info_structure_table->DolbyHDR_flag_sub;
	}

	if (srcType == VSC_INPUTSRC_DP)
	{
#if IS_ENABLED(CONFIG_RTK_DPRX)
		Dprx_GetInputType = drvif_Dprx_GetPortInputType(port);

		if (Dprx_GetInputType == DP_TYPE_USB_TYPE_C)
		{
			if (isInterlace == 1)
			{
				if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_480I))
				{
					which_source = VIP_QUALITY_TYPEC_480I;
				}
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_576I))
				{
			...
			{
				if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_480P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_TYPEC_480P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_TYPEC_480p;
					else
						which_source = VIP_QUALITY_TYPEC_480P;
				}
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_576P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
				...
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_720P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
				...
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_1080P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
				...
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_1440P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_TYPEC_1440p;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_TYPEC_1440p;
					else				
			...
			else
			{
				...
				else if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_1440P))
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						...
						which_source = VIP_QUALITY_DolbyHDR_MINI_DP_1440p;
					else				
	...
	{
#ifndef BUILD_QUICK_SHOW
		if(module_vdc_check_vd_clock() == true)
	...
	{
		if (vdecSrcType == VSC_VDEC_CASE_MIRACAST)
		{
			if (isInterlace == 1)
			{
				if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_480I))
			...
			else
			{
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_MIRACAST_480P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_MIRACAST_480P;
					else
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_MIRACAST_576P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_MIRACAST_576P;
					else				
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_MIRACAST_720P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_MIRACAST_720P;
					else				
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_MIRACAST_1440p;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_MIRACAST_1440p;
					else				
						which_source = VIP_QUALITY_MIRACAST_1440p;
				}*/
				else
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_MIRACAST_4k2k;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_MIRACAST_4k2k;
					else				
						which_source = VIP_QUALITY_MIRACAST_4k2k;
				}
			}
		}
		else if (vdecSrcType == VSC_VDEC_CASE_AIRPLAY)
		{
			if (isInterlace == 1)
			{
				if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_480I))
			...
			else
			{
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_AIRPLAY_480P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_AIRPLAY_480P;
					else
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_AIRPLAY_576P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_AIRPLAY_576P;
					else				
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_AIRPLAY_720P;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_AIRPLAY_720P;
					else				
				...
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_AIRPLAY_1440p;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_AIRPLAY_1440p;
					else				
						which_source = VIP_QUALITY_AIRPLAY_1440p;
				}*/
				else
				{
					if(VIP_Source_Timing_isHDR(bHDR_flag))
						which_source = VIP_QUALITY_HDR_AIRPLAY_4k2k;
					else if (bDolby_HDR_flag == 1)
						which_source = VIP_QUALITY_DolbyHDR_AIRPLAY_4k2k;
					else				
						which_source = VIP_QUALITY_AIRPLAY_4k2k;
				}
			}
		}	
		else if (vdecSrcType == VSC_VDEC_CASE_DTV)
		{
			if (isInterlace == 1)
			{
				if (VIP_Source_Timing_MatchV(v_len, VIP_Source_Timing_V_Len_480I))
	...
	else if (srcType == VSC_INPUTSRC_ADC)
	{
		...
		else
		{
			...
			{
				/*if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_480P;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_480P;
				else*/
			...
			{
				/*if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_576P;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_576P;
				else*/			
			...
			{
				/*if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_720P;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_720P;
				else*/			
			...
			{
				/*if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_1080P;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_1080P;
				else*/
			...
			{
				if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_1440p;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_1440p;
				else				
					which_source = VIP_QUALITY_YPbPr_1440p;
			}
			else
			{
				if(VIP_Source_Timing_isHDR(bHDR_flag))
					which_source = VIP_QUALITY_HDR_YPbPr_4k2k;
				else if (bDolby_HDR_flag == 1)
					which_source = VIP_QUALITY_DolbyHDR_YPbPr_4k2k;
				else				
					which_source = VIP_QUALITY_YPbPr_4k2k;
			}*/
		}
	}
	...
	if (display == SLR_MAIN_DISPLAY)
	{
		...
		VIP_system_info_structure_table->Input_VO_Form		= vdecSrcType;
		VIP_system_info_structure_table->Input_TVD_Form 	= srcType;
		VIP_system_info_structure_table->vdc_color_standard = vdc_ColorStandard;
		VIP_system_info_structure_table->Timing 		= isInterlace;
		VIP_system_info_structure_table->HDMI_video_format	= isHDMI;
		...
		VIP_system_info_structure_table->HDMI_data_range	= v_len;
	...
	{
		VIP_RPC_system_info_structure_table->VIP_source_sub 	= which_source;
	}
...
unsigned char fwif_vip_3Dsource_check(unsigned char display, unsigned char callmode)
{
	...
	if (input_src_from == _SRC_FROM_VDC) {
		...
		port = drvif_hdmi_get_current_display_port();
		video_format =drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))) ;//drvif_Hdmi_GetColorSpace();
        rtd_pr_vpq_info("ori_CS=%d,new_CS=%d,%s->%d,%s\n",drvif_Hdmi_GetColorSpace(),drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))), __FILE__, __LINE__, __FUNCTION__);
		if (drvif_IsHDMI() && ((drvif_Hdmi_HdcpEnabled()) || (video_format != 0) || (ForceHDMIIntoVIPMode) || (drvif_Hdmi_IsAudioLock(port)))) {
			/* HDMI*/
...
void fwif_color_video_quality_handler_quick_show(void)
{
	...
	unsigned int *vir_addr0, *vir_addr1, *vir_addr2, *vir_addr3, *vir_addr4, *vir_addr5, *vir_addr6, *vir_addr7, *vir_addr8, *vir_addr9, *vir_addr_end;
	...
	unsigned int gainOD, OD_tbl_sel, OD_UI_MODE;
	PQSrcData_Ver2 *pPQSrcData_Ver2;
	unsigned int dr = 0;
	unsigned char PC_mode_check;
	unsigned char is_ECO_Mode = 0;
	unsigned char isPC_flag, isLowLatency, is_pcGamingMode;
	...
	if(panel_bin_addr ==NULL){
		...
		Scaler_SetBrightness(SLR_MAIN_DISPLAY, 50);
		Scaler_SetContrast(SLR_MAIN_DISPLAY, 50);
		Scaler_SetSaturation(SLR_MAIN_DISPLAY, 50);
		Scaler_SetHue(SLR_MAIN_DISPLAY, 50);
	...
	isPC_flag = Get_Val_vsc_run_pc_mode();
	isLowLatency = Get_Val_vsc_run_low_latency_mode();
	is_pcGamingMode = is_QS_game_mode();	// pcGamingMode flag	
	if((isPC_flag == TRUE) && (isLowLatency == TRUE))
	{
		if(is_pcGamingMode) 
	...
	else if (isPC_flag == TRUE)
	{	
		cur_mode = VIP_QS_PQ_cur_PcMode;
	}
	else if (isLowLatency == TRUE)
	{	
		cur_mode = VIP_QS_PQ_cur_GameMode;
	}
	else
	...
	// move to qs ini
	//fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 1);
	//fwif_set_gamma_system_enhance(SLR_MAIN_DISPLAY, &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0], 1);
	...
	{
		Scaler_SetBrightness(SLR_MAIN_DISPLAY, (unsigned char)Cur_PicModeData->Brightness);
	...
	{
		Scaler_SetContrast(SLR_MAIN_DISPLAY, (unsigned char)Cur_PicModeData->Contrast);
	...
	{
		Scaler_SetSaturation(SLR_MAIN_DISPLAY, (unsigned char)Cur_PicModeData->Saturation);
	...
	{
		Scaler_SetHue(SLR_MAIN_DISPLAY, (unsigned char)Cur_PicModeData->Hue);
		printf("\n #9 Hue=%d \n",Cur_PicModeData->Hue);
	...
	{
		Cur_PicModeData->Gamma = Cur_PicModeData->Gamma + Gamma_exp_2p2;	// Gamma_exp_2p2 is center
	...
	{
		ret = Scaler_set_APDEM(DEM_ARG_HDR_OETF_Setting_Ctrl, (void *)&Cur_PicModeData->HDR10_Dynamic_En);
	...
	{
		ret = Scaler_set_APDEM(DEM_ARG_HDR_OETF_Curve_Ctrl, (void *)&Cur_PicModeData->HDR10_Curve);
	...
	{
		// 31: ui indicator (1 for ui level mode) , 30~16: table index (if 31bit == 1) , 15~0: gain
		gainOD = (pPQSrcData_Ver2->gainOverDriver)&0xFF/*+64*/; //
	...
	// #33 VPQ_EXTERN_IOC_SET_Gamma_byParameters
	{
		...
		printf("\n #33 VPQ_EXTERN_IOC_SET_Gamma_byParameters, low blue level =%d,\n", Cur_PicModeData->Low_Blue_Level);
	}

	// #34 DEM_ARG_Smooth_Gradation
	{
		ret = Scaler_set_APDEM(DEM_ARG_Smooth_Gradation, (void *)&Cur_PicModeData->Smooth_Gradation);
		printf("\n #34 DEM_ARG_Smooth_Gradation, low blue level =%d,\n", Cur_PicModeData->Smooth_Gradation);
	}

	// #35 DEM_ARG_OD_TBL_set_by_idx_Sel
	{
		// 31: ui indicator (1 for ui level mode) , 30~16: table index (if 31bit == 1) , 15~0: gain
		OD_UI_MODE = ((pPQSrcData_Ver2->gainOverDriver)&_BIT31)>>31;
		OD_tbl_sel = ((pPQSrcData_Ver2->gainOverDriver)&0x7FFF0000)>>16;
		if ((OD_UI_MODE == 1) && (OD_tbl_sel < VIP_OD_LEVEL_MAX))
			ret = Scaler_set_APDEM(DEM_ARG_OD_TBL_set_by_idx_Sel, (void *)&OD_tbl_sel);
		printf("\n	#35 DEM_ARG_OD_TBL_set_by_idx_Sel, low blue level =%d,\n", Cur_PicModeData->Smooth_Gradation);
	}

	// for PcMode
	PC_mode_check = Scaler_VPQ_check_PC_RGB444(SLR_MAIN_DISPLAY);
	...
	qs2kernel_size = sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA) + sizeof(StructColorDataFacModeType) + sizeof(char) + sizeof(char) + sizeof(dm_dm_submodule_enable_RBUS);	//
	...
	vir_addr6 = (unsigned int*)(base_addr0 + sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA));
	vir_addr7 = (unsigned int*)(base_addr0 + sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA) + sizeof(StructColorDataFacModeType));
	vir_addr8 = (unsigned int*)(base_addr0 + sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA) + sizeof(StructColorDataFacModeType) + sizeof(char));
	vir_addr9 = (unsigned int*)(base_addr0 + sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA) + sizeof(StructColorDataFacModeType) + sizeof(char) + sizeof(char));
	...
	memcpy(vir_addr6, &g_curColorFacTableEx, sizeof(StructColorDataFacModeType));
	memcpy(vir_addr7, &g_bUseMiddleWareOSDmap, sizeof(char));
	memcpy(vir_addr8, &bODInited, sizeof(char));
	memcpy(vir_addr9, &dm_submodule_temp, sizeof(dm_dm_submodule_enable_RBUS));
	...
	pTmp8 = (unsigned char*)&g_curColorFacTableEx;
	for (i=0;i<sizeof(StructColorDataFacModeType);i++)
		qs2kernel_crc = (qs2kernel_crc + pTmp8[i])&0xFF;

	pTmp8 = (unsigned char*)&g_bUseMiddleWareOSDmap;
	for (i=0;i<sizeof(char);i++)
		qs2kernel_crc = (qs2kernel_crc + pTmp8[i])&0xFF;

	pTmp8 = (unsigned char*)&bODInited;
	for (i=0;i<sizeof(char);i++)
		qs2kernel_crc = (qs2kernel_crc + pTmp8[i])&0xFF;
	
	pTmp8 = (unsigned char*)&dm_submodule_temp;
	for (i=0;i<sizeof(dm_dm_submodule_enable_RBUS);i++)
		qs2kernel_crc = (qs2kernel_crc + pTmp8[i])&0xFF;

	printf2("[VIP_QS_END]base_addr=%x,pa=%x,%x,va=%x,%x,qs2kernel_crc=%d,pc444=%d,ECO=%d,isPC_flag=%d,isLowLatency=%d,is_pcGamingMode=%d,\n", 
		base_addr, base_addr0, base_addr_end, (unsigned long)vir_addr0, (unsigned long)vir_addr_end, qs2kernel_crc, 
		PC_mode_check, is_ECO_Mode, isPC_flag, isLowLatency, is_pcGamingMode);
}
...
void Scaler_Copy_PQbin(void)
{
    ...
    unsigned char mean_value_index=0;
    ...
    for(mean_value_index=0;mean_value_index<mean_value_index_MAX;mean_value_index++)
        S_curve_mean[mean_value_index] = m_pVipTable->S_curve_mean[mean_value_index];
...
{
	unsigned char HSR_mode = 0, source = 0;
	...
	source = fwif_vip_source_check(SLR_SUB_DISPLAY, 0);
	...
	if (m_cbFunc_getQualityExtend3 != NULL)
		...
		rtd_pr_vpq_emerg("PQ table m_cbFunc_getQualityExtend3 read fail !!!\n");
		return;
	}

	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		if (HSR_mode && pVipCoefExt4Array) {
			Scaler_SetSharpnessTable(SLR_SUB_DISPLAY, pVipCoefExt4Array[VIP_QUALITY_FUNCTION_HSR_SharpTable]);
		} else {
			Scaler_SetSharpnessTable(SLR_SUB_DISPLAY, pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable]);
		}
	}
	
	rtd_pr_vpq_info("quality_handler_Sub, source=%d,\n", source);
}

unsigned char RGB_SharpTable_idx=0;
...
/*o------ VIP table : VIP_QUALITY_Extend3_Coef ------o*/
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		...
		if (HSR_mode && pVipCoefExt4Array)
			Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, pVipCoefExt4Array[VIP_QUALITY_FUNCTION_HSR_SharpTable]);
		else
			Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable]);
		rtd_pr_vpq_info("[RGB_Sharp],qua,RGB_SharpTable_idx=%d\n",RGB_SharpTable_idx);
		RGB_SharpTable_idx = pVipCoefArray[VIP_QUALITY_FUNCTION_RGB_SharpTable];
		if (vpq_project_id != 0x00060000 || vpqex_project_id != 0x00060000) {
			if (HSR_mode && pVipCoefExt4Array)
	...
	res_check_last_input_h = Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN);
	...
	//AT V6.2 Patch 2022/03/31 - END
#endif
	rtd_pr_vpq_info("quality_handler, source=%d,\n", source);

}

#if IS_ENABLED(CONFIG_RTK_AI_DRV)
unsigned char bBinAIPQ_Table = 0;
...
void* scaler_VIP_PQMask_GetSemanticTable(unsigned char bLogEnable) {
	...
	if( gVip_Table == NULL ) {
		rtd_pr_vpq_emerg("[%s] VIP table null\n", __FUNCTION__);
		return NULL;
	}
...
{
	unsigned char *pVipCoefArray = NULL;
	unsigned char APDEM_LC_idx, APDEM_LC_LV;
	APDEM_LC_idx = Scaler_APDEM_Arg_Access(DEM_ARG_LC_Idx, 0, 0);
	APDEM_LC_LV = Scaler_APDEM_Arg_Access(DEM_ARG_LC_level, 0, 0);

	if (APDEM_LC_idx == 0xFF)
	{
		if (m_cbFunc_getQualityExtend != NULL)
			pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend((VIP_SOURCE_TIMING)m_nowSource);
		else
	...
	{
		// set
		Scaler_SetLocalContrastTable(APDEM_LC_idx);
		//fwif_color_set_LC_Global_Region_Num_Size(arg, LC_BLOCK_48x27);
		if( (drvif_color_get_LC_size_support()!=0) && (APDEM_LC_LV != 0) )
			Scaler_SetLocalContrastEnable(1);
		else
			Scaler_SetLocalContrastEnable(0);
...
{
	unsigned char *pVipCoefArray = NULL;
	unsigned char APDEM_DCC_idx;
	APDEM_DCC_idx = Scaler_APDEM_Arg_Access(DEM_ARG_DCC_Idx, 0, 0);

	if (APDEM_DCC_idx == 0xFF)
	{
		if (m_cbFunc_getQualityExtend3 != NULL)
			pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend3((VIP_SOURCE_TIMING)m_nowSource);
		else{
	...
	{
		Scaler_Set_DCC_Color_Independent_Table(APDEM_DCC_idx);
		rtd_pr_vpq_info("Scaler_VIP_DCC_Color_Independent_reset, APDEM idx=%d,\n", APDEM_DCC_idx);
	}
}
#endif
...
void fwif_color_video_3Dmode_quality_handler(void)
{
	...
	if (!(pVipCoefArray == NULL)) {
		...
		if (vpq_project_id != 0x00060000 || vpqex_project_id != 0x00060000) {
			Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, pVipCoefArray[VIP_QUALITY_3DFUNCTION_Sharpness_table]);
...
void *fwif_color_get_picture_mode_data(unsigned char src_idx, unsigned char value)
{
	...
	return m_cbFunc_getPictureModeData(0, 0, 0);	// always use same structure
}
...
#else	// always use same structure index for kernel
	SLR_PICTURE_MODE_DATA *pData = (SLR_PICTURE_MODE_DATA *)fwif_color_get_picture_mode_data(0, 0xFF);
#endif
	return pData;
}
...
#ifndef BUILD_QUICK_SHOW
static VIP_HDRCID_CTRL HDRCID_CTRL = 
{
	//short Pattern_Enable[HDRCID_Pattern_MAX]; // RADCR_Pattern_APL_th
	{0},
...
static VIP_HDRCID_INFO HDRCID_INFO = {0};
...
{
	if ((pRADCR_Pattern_BL_adj == NULL) || (pRADCR_Pattern_APL_th == NULL) || (pRADCR_Pattern_APL_th_range == NULL) || 
		(pRADCR_Pattern_Gain_adj_Offset == NULL) || (pRADCR_Pattern_Offset_adj_Offset == NULL))
	{		
		rtd_pr_vpq_emerg("Scaler_VIP_HDRCID_Table_update NULL., %s->%d, %s~\n", __FILE__, __LINE__, __FUNCTION__);
		return -1;
	}

	if (isPQbin_update == 1) // from pq bin
	{
		memcpy(&HDRCID_CTRL.Pattern_th[0], pRADCR_Pattern_APL_th, sizeof(short)*HDRCID_Pattern_MAX);		
		memcpy(&HDRCID_CTRL.Pattern_CTRL[0], pRADCR_Pattern_APL_th_range, sizeof(short)*HDRCID_Pattern_MAX);
		memcpy(&HDRCID_CTRL.Pattern_BL_adj[0], pRADCR_Pattern_BL_adj, sizeof(short)*HDRCID_Pattern_MAX);
		memcpy(&HDRCID_CTRL.Pattern_Gain_adj_Offset[0], pRADCR_Pattern_Gain_adj_Offset, sizeof(short)*HDRCID_Pattern_MAX);
		memcpy(&HDRCID_CTRL.Pattern_Offset_adj_Offset[0], pRADCR_Pattern_Offset_adj_Offset, sizeof(short)*HDRCID_Pattern_MAX);
		
		HDRCID_CTRL.isPQbin_update = 1;
	}
	else	// from ini
	{
		if (HDRCID_CTRL.isPQbin_update == 0)	// isPQbin_update = 1, only can update from pq bin
		{	
			memcpy(&HDRCID_CTRL.Pattern_th[0], pRADCR_Pattern_APL_th, sizeof(short)*HDRCID_Pattern_MAX);		
			memcpy(&HDRCID_CTRL.Pattern_CTRL[0], pRADCR_Pattern_APL_th_range, sizeof(short)*HDRCID_Pattern_MAX);
			memcpy(&HDRCID_CTRL.Pattern_BL_adj[0], pRADCR_Pattern_BL_adj, sizeof(short)*HDRCID_Pattern_MAX);
	...
	rtd_pr_vpq_info("HDRCID_TBL_update,isPQbin_update=%d,%d,\n", isPQbin_update, HDRCID_CTRL.isPQbin_update);
	rtd_pr_vpq_info("HDRCID_TBL_update,BL_adj=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\n", 
		pRADCR_Pattern_BL_adj[0], pRADCR_Pattern_BL_adj[1], pRADCR_Pattern_BL_adj[2], pRADCR_Pattern_BL_adj[3], pRADCR_Pattern_BL_adj[4], 
		pRADCR_Pattern_BL_adj[5], pRADCR_Pattern_BL_adj[6], pRADCR_Pattern_BL_adj[7], pRADCR_Pattern_BL_adj[8], pRADCR_Pattern_BL_adj[9], 
		pRADCR_Pattern_BL_adj[10]);
	rtd_pr_vpq_info("HDRCID_TBL_update,APL_th=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\n", 
		pRADCR_Pattern_APL_th[0], pRADCR_Pattern_APL_th[1], pRADCR_Pattern_APL_th[2], pRADCR_Pattern_APL_th[3], pRADCR_Pattern_APL_th[4], 
		pRADCR_Pattern_APL_th[5], pRADCR_Pattern_APL_th[6], pRADCR_Pattern_APL_th[7], pRADCR_Pattern_APL_th[8], pRADCR_Pattern_APL_th[9], 
		pRADCR_Pattern_APL_th[10]);
	rtd_pr_vpq_info("HDRCID_TBL_update,APL_th_range=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\n", 
		pRADCR_Pattern_APL_th_range[0], pRADCR_Pattern_APL_th_range[1], pRADCR_Pattern_APL_th_range[2], pRADCR_Pattern_APL_th_range[3], pRADCR_Pattern_APL_th_range[4], 
		pRADCR_Pattern_APL_th_range[5], pRADCR_Pattern_APL_th_range[6], pRADCR_Pattern_APL_th_range[7], pRADCR_Pattern_APL_th_range[8], pRADCR_Pattern_APL_th_range[9], 
		pRADCR_Pattern_APL_th_range[10]);
	rtd_pr_vpq_info("HDRCID_TBL_update,gain=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\n", 
		pRADCR_Pattern_Gain_adj_Offset[0], pRADCR_Pattern_Gain_adj_Offset[1], pRADCR_Pattern_Gain_adj_Offset[2], pRADCR_Pattern_Gain_adj_Offset[3], pRADCR_Pattern_Gain_adj_Offset[4], 
		pRADCR_Pattern_Gain_adj_Offset[5], pRADCR_Pattern_Gain_adj_Offset[6], pRADCR_Pattern_Gain_adj_Offset[7], pRADCR_Pattern_Gain_adj_Offset[8], pRADCR_Pattern_Gain_adj_Offset[9], 
		pRADCR_Pattern_Gain_adj_Offset[10]);
	rtd_pr_vpq_info("HDRCID_TBL_update,offset=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\n", 
		pRADCR_Pattern_Offset_adj_Offset[0], pRADCR_Pattern_Offset_adj_Offset[1], pRADCR_Pattern_Offset_adj_Offset[2], pRADCR_Pattern_Offset_adj_Offset[3], pRADCR_Pattern_Offset_adj_Offset[4], 
		pRADCR_Pattern_Offset_adj_Offset[5], pRADCR_Pattern_Offset_adj_Offset[6], pRADCR_Pattern_Offset_adj_Offset[7], pRADCR_Pattern_Offset_adj_Offset[8], pRADCR_Pattern_Offset_adj_Offset[9], 
		pRADCR_Pattern_Offset_adj_Offset[10]);
...
unsigned char  Scaler_update_DCC_Mode_Sel(unsigned char value)
{
	SLR_PICTURE_MODE_DATA *pTable = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
	...
	pTable = (SLR_PICTURE_MODE_DATA *)fwif_color_get_cur_picture_mode_data(src_idx);
	if (pTable == NULL) {
		rtd_pr_vpq_emerg("~Warning!! Get colordata NULL., %s->%d, %s~\n", __FILE__, __LINE__, __FUNCTION__);
		return FALSE;
	}
	pTable->DccMode = value;
	return 0;
...
void Scaler_SetDNR(unsigned char value)
{
	...
	rtd_pr_vpq_info("==== Scaler_SetDNR, %d\n", value);
...
unsigned char Scaler_GetAutoMA_VD_3D(void) /* enable/disable AutoMA*/
{
	int ret;
		...
		return -1;
     	...
     	for(i=0;i<3;i++){
       	 for(j=0;j<3;j++)
		 	...
		 	#if defined(CONFIG_ARM64) || defined(BUILD_QUICK_SHOW)
			matrix_inv[i][j]=(b[i][j]*scale)/(determinant/scale);	// (b[i][j]*scale)/(determinant/scale) = b[i][j]*scale*scale/determinant
			#else
			matrix_inv[i][j]=div_s64(div_s64(b[i][j]*scale, 16), div_s64(div_s64(determinant, scale), 16));
			#endif
       }
#if 0
...
static int Scaler_APDEM_Color_Space_CM_Cal(unsigned int panal_para_scale)
{
	...
	xyz_of_W[0][0]= 31270; // use D65, same as tool // VIP_AP_DEM_TBL.PANEL_DATA_.DEM_PANEL_INI_Color_Chromaticity.Panel_white_x;	// primary_wihte.un16_CIE_x;//xw_target;
	xyz_of_W[1][0]= 32900; // use D65, same as tool // VIP_AP_DEM_TBL.PANEL_DATA_.DEM_PANEL_INI_Color_Chromaticity.Panel_white_y;	// primary_wihte.un16_CIE_y;//yw_target;
	...
	rtd_pr_vpq_info("APDEM_CM_Cal, xyz_of_RGB=%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,\n",
	...
	rtd_pr_vpq_info("APDEM_CM_Cal, xyz_of_RGB_inv=%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,\n",
	...
	rtd_pr_vpq_info("APDEM_CM_Cal, beta3X1z=%lld,%lld,%lld,\n",
		xyz_of_W[0][0], xyz_of_W[1][0], xyz_of_W[2][0]);
	rtd_pr_vpq_info("APDEM_CM_Cal, beta3X1=%lld,%lld,%lld,\n",
	...
	rtd_pr_vpq_info("APDEM_CM_Cal, RGB2XYZ=%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,\n",
	...
	rtd_pr_vpq_info("APDEM_CM_Cal, XYZ2RGB=%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,\n",
	...
	//step2 xyz_of_RGB's inverse * xyz_of_W
	y=xyz_of_W[1][0];
	xyz_of_W[0][0]=div_s64(xyz_of_W[0][0]*panal_para_scale, y);//---w/y
	xyz_of_W[1][0]=div_s64(xyz_of_W[1][0]*panal_para_scale, y);
	xyz_of_W[2][0]=div_s64(xyz_of_W[2][0]*panal_para_scale, y);
	result_2=mul3x1(xyz_of_RGB_inv,xyz_of_W,beta3X1, panal_para_scale);
	...
	//step3 xyz_of_RGB*beta3X3-->RGB2XYZ
	// return to scale^2 domain
	beta3X3[0][0]=div_s64(beta3X1[0][0], panal_para_scale);
	beta3X3[0][1]=0;
	beta3X3[0][2]=0;
	beta3X3[1][0]=0;
	beta3X3[1][1]=div_s64(beta3X1[1][0], panal_para_scale);
	beta3X3[1][2]=0;
	beta3X3[2][0]=0;
	beta3X3[2][1]=0;
	beta3X3[2][2]=div_s64(beta3X1[2][0], panal_para_scale);
	result_3=mul3x3(xyz_of_RGB,beta3X3,RGB2XYZ, panal_para_scale);
	...
	for (i=0;i<3;i++)
		for (j=0;j<3;j++)
			RGB2XYZ[i][j] = div_s64(RGB2XYZ[i][j], panal_para_scale);
	result_4=get_iveMatrix(RGB2XYZ,XYZ2RGB, panal_para_scale);
	...
	for (i=0;i<3;i++)
		for (j=0;j<3;j++)
			sRGB[i][j] = div_s64(sRGB[i][j], panal_para_scale);
	for(i=0;i<3;i++){
       	 for(j=0;j<3;j++)
				VIP_AP_DEM_TBL.Color_Space_Target_Apply[Color_Space_Target_BT2020][i][j]=div_s64(sRGB[i][j]*2048, panal_para_scale);	// return to reg domain from 100000 domain;
		}
	...
	for(i=0;i<3;i++){
       	 for(j=0;j<3;j++)
				VIP_AP_DEM_TBL.Color_Space_Target_Apply[Color_Space_Target_DCIP3][i][j]=div_s64(sRGB[i][j]*2048, panal_para_scale);	// return to reg domain from 100000 domain;
		}
	...
	for(i=0;i<3;i++){
       	 for(j=0;j<3;j++)
				VIP_AP_DEM_TBL.Color_Space_Target_Apply[Color_Space_Target_AdobeRGB][i][j]=div_s64(sRGB[i][j]*2048, panal_para_scale);	// return to reg domain from 100000 domain;
		}
	...
	for(i=0;i<3;i++){
       	 for(j=0;j<3;j++)
				VIP_AP_DEM_TBL.Color_Space_Target_Apply[Color_Space_Target_sRGB][i][j]=div_s64(sRGB[i][j]*2048, panal_para_scale);	// return to reg domain from 100000 domain;
		}
#endif
...
static const unsigned char VIP_APDEM_MAX_VAL[DEM_ARG_CTRL_ITEMS_Max] = {
	...
	DRV_NR_Level_MAX,  // off/low/mid/high/auto, AP only set off or low
	VIP_OD_LEVEL_MAX, // OD level
	3, // HDR curve, 0:calman, 1:normal, 2:normal2(dummy)
	2, // HDR dynamic, 0:off, 1:on
	VIP_DEM_Shadow_Detail_Boot_TBL_NUM,// offset table select
	VIP_CSC_DATA_RANGE_MODE_MAX, 		// auto, full, limit for sub
};
...
unsigned char Scaler_APDEM_Arg_Access(VIP_DEM_ARG_CTRL_ITEMS ID, unsigned char val, unsigned char isSetFlag)
{
	static unsigned char DEM_arg[DEM_ARG_CTRL_ITEMS_Max] =
		{0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
// color mapping or csc polling
int Scaler_set_APDEM_CM_flow_Polling_TASK(void)
{
	unsigned char CM_UI;
	unsigned char CM_index;

	down(&VPQ_ColorMapping_Semaphore);

	CM_UI = Scaler_APDEM_Arg_Access(DEM_ARG_Color_Space, 0, 0);
	if (CM_UI == Color_Space_Target_Auto)
	{
		CM_index = fwif_color_get_APDEM_CM_IDX();
		if (CM_index != Scaler_Get_ColorMapping_3x3_Table())
		{
			Scaler_Set_ColorMapping_3x3_Table(CM_index);

			Scaler_Set_ColorSpace_D_3dlut_APDEM(VIP_D3DLUTbyCS_setByIniColorSpace);
		}
	}

	up(&VPQ_ColorMapping_Semaphore);
...
{
#if 0
	short OD_gain_r, OD_gain_g, OD_gain_b;
	short OD_gain_r_rec, OD_gain_g_rec, OD_gain_b_rec;
	DRV_od_gain od_gain[2];
#endif
	unsigned int arg;
	unsigned int i;
	unsigned char idx_temp, tbl_idx;
	int *pINT_Temp32;
	...
	if (Scaler_APDEM_En_Access(0, 0) == 1) {
		switch(ID)
		{
			...
			case DEM_ARG_After_Filter_En:
			{
				...
				Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY));
			...
			case DEM_ARG_Data_Range:
			{
				...
				Scaler_Set_HDR_YUV2RGB(SLR_MAIN_DISPLAY, VIP_system_info_structure_table->HDR_flag, HAL_VPQ_COLORIMETRY_BT2020);
				Scaler_SetDataFormatHandler(SLR_MAIN_DISPLAY);

			}
			break;

			case DEM_ARG_Data_Range_sub:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range_sub, arg, 1);

				Scaler_Set_HDR_YUV2RGB(SLR_SUB_DISPLAY, VIP_system_info_structure_table->HDR_flag_sub, HAL_VPQ_COLORIMETRY_BT2020);
				Scaler_SetDataFormatHandler(SLR_SUB_DISPLAY);
			...
			case DEM_ARG_Color_Space:
			{
				...
				if (Scaler_APDEM_Arg_Access(DEM_ARG_Color_Space, 0, 0) == 0xFF) {
					...
					//vpq_set_inv_gamma(1);
					fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Power2p2Gamma_14bit[0], &Power2p2Gamma_14bit[0], &Power2p2Gamma_14bit[0]);
					fwif_set_gamma_system_enhance(SLR_MAIN_DISPLAY, &Power0p45Gamma_14bit[0], &Power0p45Gamma_14bit[0], &Power0p45Gamma_14bit[0]);
				} else {
					Scaler_APDEM_Arg_Access(DEM_ARG_Color_Space, arg, 1);
				}
#else
                ...
                rtd_pr_vpq_info("DEM_arg_Color_Space:%d, %d\n", arg, idx_temp);

				//update inv_gamma and gamma table. [RKTC2851F-585][NUMDEV-1479]
				//fwif_color_update_invgamma_gamma_table();
				if ((arg == Color_Space_Target_OFF) || ((arg >= Color_Space_Target_And14_Mono_BLIND_MODE) && (arg <= Color_Space_Target_And14_Trita_BLIND_MODE)))
				{
					drvif_color_inv_gamma_enable(SLR_MAIN_DISPLAY, 0, 0);
			...
			case DEM_ARG_D_3DLUT_Offset:
			{
				...
				Scaler_Set_ColorSpace_D_3dlut_APDEM(VIP_D3DLUTbyCS_setByIni6Axis);
			}
			break;
			...
			{
				pINT_Temp32 = (int*)DEM_arg;				
				tbl_idx = (Scaler_APDEM_Arg_Access(DEM_ARG_OD_TBL_set_by_idx_Sel, 0, 0)>=VIP_OD_LEVEL_MAX)?(0):(Scaler_APDEM_Arg_Access(DEM_ARG_OD_TBL_set_by_idx_Sel, 0, 0));
#if 1				
				VIP_AP_DEM_TBL.OverDriver_Gain.gain_r = pINT_Temp32[0];
				VIP_AP_DEM_TBL.OverDriver_Gain.gain_g = pINT_Temp32[0];
				VIP_AP_DEM_TBL.OverDriver_Gain.gain_b = pINT_Temp32[0];
				
				fwif_color_set_ODPH_Gain(&VIP_AP_DEM_TBL.OverDriver_Gain, &mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain, &mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain_REC);
#else
				...
				OD_gain_r = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain.gain_r + VIP_AP_DEM_TBL.OverDriver_Gain[0] - 64;
				OD_gain_g = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain.gain_g + VIP_AP_DEM_TBL.OverDriver_Gain[1] - 64;
				OD_gain_b = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain.gain_b + VIP_AP_DEM_TBL.OverDriver_Gain[2] - 64; 
				OD_gain_r_rec = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain_REC.gain_r + VIP_AP_DEM_TBL.OverDriver_Gain[0] - 64;
				OD_gain_g_rec = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain_REC.gain_g + VIP_AP_DEM_TBL.OverDriver_Gain[1] - 64;
				OD_gain_b_rec = mVIP_ODTable[tbl_idx].OD_Main_Ctrl.OD_Gain_REC.gain_b + VIP_AP_DEM_TBL.OverDriver_Gain[2] - 64; 
				
				od_gain[0].r_gain = od_clamp_0_127(OD_gain_r);
				od_gain[0].g_gain = od_clamp_0_127(OD_gain_g); 
				od_gain[0].b_gain = od_clamp_0_127(OD_gain_b);
				od_gain[1].r_gain = od_clamp_0_127(OD_gain_r_rec);
				od_gain[1].g_gain = od_clamp_0_127(OD_gain_g_rec);
				od_gain[1].b_gain = od_clamp_0_127(OD_gain_b_rec);
				
				//fwif_color_set_od_gain_RGB((unsigned char)OD_gain_r, (unsigned char)OD_gain_g, (unsigned char)OD_gain_b);				
				drvif_color_od_gain_set(&od_gain[0]);
#endif				
			...
			case DEM_ARG_Smooth_Gradation:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, arg, 1);

				// arg can't exceed max
				arg = Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, 0, 0);

				Scaler_Set_I_De_Contour(Scaler_Get_I_De_Contour());
				fwif_color_set_LC_decontour(0, Scaler_APDEM_Arg_Access(DEM_ARG_LC_Idx, 0, 0), arg);
			}
			break;

			case DEM_ARG_OD_TBL_set_by_idx_Sel:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_OD_TBL_set_by_idx_Sel, arg, 1);

				// arg can't exceed max
				arg = Scaler_APDEM_Arg_Access(DEM_ARG_OD_TBL_set_by_idx_Sel, 0, 0);

				fwif_color_set_od_by_TBL(&mVIP_ODTable[arg], 1);
			}
			break;

			case DEM_ARG_Shadow_Boot_Offset:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_Shadow_Boot_Offset, arg, 1);
			...
			case DEM_ARG_HDR_OETF_Curve_Ctrl:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Curve_Ctrl, arg, 1);

				// arg can't exceed max
				arg = Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Curve_Ctrl, 0, 0);

				// set 
				Scaler_color_copy_HDR_table_oetf_curve_ctrl_from_AP(arg);
			}
			break;

			case DEM_ARG_HDR_OETF_Setting_Ctrl:
			{
				arg = *((unsigned int*)DEM_arg);
				Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Setting_Ctrl, arg, 1);

				// arg can't exceed max
				arg = Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Setting_Ctrl, 0, 0);

				// set 
				Scaler_color_copy_HDR_table_oetf_setting_ctrl_from_AP(arg);
			}
			break;
...
char Scaler_APDEM_PTG_set(VIP_APDEM_PTG_CTRL *PTG_CTRL)
{
	...
	// enable y2R, need to enable again in pure RGB
	drv_vipCSMatrix_t.CTRL_ITEM.YUV2RGB_Enable_Main = 1;
...
static void Scaler_APDEM_D3DLUTbyCS_D_3DLUT_Array_linear_reset(void)
{
	unsigned int i, j, k;
	...
	for (i=0;i<VIP_D_3D_LUT_INDEX;i++)
	{
		...
		for (j=0;j<VIP_D_3D_LUT_INDEX;j++)
		{
			...
			for (k=0;k<VIP_D_3D_LUT_INDEX;k++)
			{
				...
				D_3DLUT[addrRGB][VIP_8VERTEX_R] = R_val;
				D_3DLUT[addrRGB][VIP_8VERTEX_G] = G_val;
				D_3DLUT[addrRGB][VIP_8VERTEX_B] = B_val;
			}
		}
	}
...
static char Scaler_APDEM_D3DLUTbyCS_D_3DLUT_Array_Fac_set(unsigned char CS_idx)
{
	unsigned int i, j, k;
	unsigned int addrB, addrG, addrR, addrRGB;
	unsigned int R_val, G_val, B_val;
	unsigned char isD3DLUT_linear = 1;
	...
	for (i=0;i<VIP_D_3D_LUT_INDEX;i++)
	{
		...
		for (j=0;j<VIP_D_3D_LUT_INDEX;j++)
		{
			...
			for (k=0;k<VIP_D_3D_LUT_INDEX;k++)
			{
				...
				R_val = ((k * 4096)>65535)?(65535):(k * 4096);
				addrRGB = addrB + addrG + addrR;
				D_3DLUT[addrRGB][VIP_8VERTEX_R] = gD3DLUTbyCS_Decode_TBL->D3DLUTbyCS_Decode_TBL[CS_idx].pt[addrRGB].r_data;
				D_3DLUT[addrRGB][VIP_8VERTEX_G] = gD3DLUTbyCS_Decode_TBL->D3DLUTbyCS_Decode_TBL[CS_idx].pt[addrRGB].g_data;
				D_3DLUT[addrRGB][VIP_8VERTEX_B] = gD3DLUTbyCS_Decode_TBL->D3DLUTbyCS_Decode_TBL[CS_idx].pt[addrRGB].b_data;
				if ((D_3DLUT[addrRGB][VIP_8VERTEX_R] != R_val) || (D_3DLUT[addrRGB][VIP_8VERTEX_G] != G_val) ||
					(D_3DLUT[addrRGB][VIP_8VERTEX_B] != B_val))
					isD3DLUT_linear = 0;
			}
		}
	...
	return isD3DLUT_linear;
}

static void Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByIni6Axis(unsigned char CS_idx)
{
	...
	{
		ptemp32 = &gD3DLUTbyCS_6axis_TBL->D3DLUTbyCS_6axis[CS_idx].offset_6axis[0];
		VPQ_COLORSPACE.CS_CUSTOM_RED.red				= VPQ_COLORSPACE.CS_CUSTOM_RED.red			+ ptemp32[0];
		VPQ_COLORSPACE.CS_CUSTOM_RED.green			= VPQ_COLORSPACE.CS_CUSTOM_RED.green			+ ptemp32[1];
		VPQ_COLORSPACE.CS_CUSTOM_RED.blue 			= VPQ_COLORSPACE.CS_CUSTOM_RED.blue			+ ptemp32[2];
		VPQ_COLORSPACE.CS_CUSTOM_GREEN.red			= VPQ_COLORSPACE.CS_CUSTOM_GREEN.red			+ ptemp32[3];
		VPQ_COLORSPACE.CS_CUSTOM_GREEN.green			= VPQ_COLORSPACE.CS_CUSTOM_GREEN.green		+ ptemp32[4];
		VPQ_COLORSPACE.CS_CUSTOM_GREEN.blue			= VPQ_COLORSPACE.CS_CUSTOM_GREEN.blue 		+ ptemp32[5];
		VPQ_COLORSPACE.CS_CUSTOM_BLUE.red 			= VPQ_COLORSPACE.CS_CUSTOM_BLUE.red			+ ptemp32[6];
		VPQ_COLORSPACE.CS_CUSTOM_BLUE.green			= VPQ_COLORSPACE.CS_CUSTOM_BLUE.green 		+ ptemp32[7];
		VPQ_COLORSPACE.CS_CUSTOM_BLUE.blue			= VPQ_COLORSPACE.CS_CUSTOM_BLUE.blue			+ ptemp32[8];
		VPQ_COLORSPACE.CS_CUSTOM_YELLOW.red			= VPQ_COLORSPACE.CS_CUSTOM_YELLOW.red 		+ ptemp32[9];
		VPQ_COLORSPACE.CS_CUSTOM_YELLOW.green 		= VPQ_COLORSPACE.CS_CUSTOM_YELLOW.green		+ ptemp32[10];
		VPQ_COLORSPACE.CS_CUSTOM_YELLOW.blue			= VPQ_COLORSPACE.CS_CUSTOM_YELLOW.blue		+ ptemp32[11];
		VPQ_COLORSPACE.CS_CUSTOM_CYAN.red 			= VPQ_COLORSPACE.CS_CUSTOM_CYAN.red			+ ptemp32[15];
		VPQ_COLORSPACE.CS_CUSTOM_CYAN.green			= VPQ_COLORSPACE.CS_CUSTOM_CYAN.green 		+ ptemp32[16];
		VPQ_COLORSPACE.CS_CUSTOM_CYAN.blue			= VPQ_COLORSPACE.CS_CUSTOM_CYAN.blue			+ ptemp32[17];
		VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.red			= VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.red		+ ptemp32[12];
		VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.green		= VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.green		+ ptemp32[13];
		VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.blue 		= VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.blue		+ ptemp32[14];
		//VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.red 	= VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.red 	+ ptemp32[];
		//VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.green	= VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.green	+ ptemp32[];
		//VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.blue	= VPQ_COLORSPACE.CS_CUSTOM_FLESH_TONE.blue	+ ptemp32[];

#if 0 //for debug
		rtd_pr_vpq_info("CSD3D,TBL_R:%d,%d,%d,\n", ptemp32[0], ptemp32[1], ptemp32[2]);
		rtd_pr_vpq_info("CSD3D,TBL_G:%d,%d,%d,\n", ptemp32[3], ptemp32[4], ptemp32[5]);
		rtd_pr_vpq_info("CSD3D,TBL_B:%d,%d,%d,\n", ptemp32[6], ptemp32[7], ptemp32[8]);
		rtd_pr_vpq_info("CSD3D,TBL_Y:%d,%d,%d,\n", ptemp32[9], ptemp32[10], ptemp32[11]);
		rtd_pr_vpq_info("CSD3D,TBL_C:%d,%d,%d,\n", ptemp32[15], ptemp32[16], ptemp32[17]);
		rtd_pr_vpq_info("CSD3D,TBL_M:%d,%d,%d,CS_idx=%d,writeMode=%d,\n", ptemp32[12], ptemp32[13], ptemp32[14], CS_idx, writeMode);
#endif
	}

	if (gD3DLUTbyCS_Decode_TBL != NULL)
	{
		isD3DLUT_linear = Scaler_APDEM_D3DLUTbyCS_D_3DLUT_Array_Fac_set(CS_idx);
	}

	En_flag = (VPQ_COLORSPACE.CS_CUSTOM_RED.red | VPQ_COLORSPACE.CS_CUSTOM_RED.green | VPQ_COLORSPACE.CS_CUSTOM_RED.blue |
		VPQ_COLORSPACE.CS_CUSTOM_GREEN.red | VPQ_COLORSPACE.CS_CUSTOM_GREEN.green | VPQ_COLORSPACE.CS_CUSTOM_GREEN.blue |
		VPQ_COLORSPACE.CS_CUSTOM_BLUE.red | VPQ_COLORSPACE.CS_CUSTOM_BLUE.green | VPQ_COLORSPACE.CS_CUSTOM_BLUE.blue |
		VPQ_COLORSPACE.CS_CUSTOM_YELLOW.red | VPQ_COLORSPACE.CS_CUSTOM_YELLOW.green | VPQ_COLORSPACE.CS_CUSTOM_YELLOW.blue |
		VPQ_COLORSPACE.CS_CUSTOM_CYAN.red | VPQ_COLORSPACE.CS_CUSTOM_CYAN.green | VPQ_COLORSPACE.CS_CUSTOM_CYAN.blue |
		VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.red | VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.green | VPQ_COLORSPACE.CS_CUSTOM_MAGENTA.blue);

	if ((En_flag == 0) && (isD3DLUT_linear == 1))
	{
		fwif_color_set_DDomain_3dLUT_32_TV006(NULL, 0);
	}
	else
	{
	...
	rtd_pr_vpq_info("CSD3D,En_flag=%d,isD3DLUT_linear=%d,\n", En_flag, isD3DLUT_linear);

}

static void Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByIniColorSpace(unsigned char CS_idx)
{
	RTK_VPQ_COLORSPACE_INDEX VPQ_COLORSPACE = {0};
	unsigned char En_flag;
	unsigned int *ptemp32;
	...
	if ((gD3DLUTbyCS_6axis_TBL != NULL) || (gD3DLUTbyCS_Decode_TBL != NULL))
	{
		VPQ_COLORSPACE.CS_CUSTOM_RED.red = VIP_AP_DEM_TBL.VPQ_COLORSPACE_INDEX[0];
		VPQ_COLORSPACE.CS_CUSTOM_RED.green = VIP_AP_DEM_TBL.VPQ_COLORSPACE_INDEX[1];
		VPQ_COLORSPACE.CS_CUSTOM_RED.blue = VIP_AP_DEM_TBL.VPQ_COLORSPACE_INDEX[2];
...
static void Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByFactory6Axis(unsigned char CS_idx)
{
	RTK_VPQ_COLORSPACE_INDEX VPQ_COLORSPACE = {0};
	unsigned int *ptemp32;

	// set linear for "D_3DLUT[VIP_D_3D_LUT_SIZE][VIP_8VERTEX_RGB_MAX]"
	Scaler_APDEM_D3DLUTbyCS_D_3DLUT_Array_linear_reset();

	if (gD3DLUTbyCS_6axis_tmp != NULL)
	{
		ptemp32 = &gD3DLUTbyCS_6axis_tmp->offset_6axis[0];
		VPQ_COLORSPACE.CS_CUSTOM_RED.red			= /*VPQ_COLORSPACE.CS_CUSTOM_RED.red			+ */ ptemp32[0];
		VPQ_COLORSPACE.CS_CUSTOM_RED.green			= /*VPQ_COLORSPACE.CS_CUSTOM_RED.green			+ */ ptemp32[1];
		VPQ_COLORSPACE.CS_CUSTOM_RED.blue 			= /*VPQ_COLORSPACE.CS_CUSTOM_RED.blue			+ */ ptemp32[2];
		VPQ_COLORSPACE.CS_CUSTOM_GREEN.red			= /*VPQ_COLORSPACE.CS_CUSTOM_GREEN.red			+ */ ptemp32[3];
		...
		rtd_pr_vpq_info("CSD3D,TBL_Y:%d,%d,%d,\n", ptemp32[9], ptemp32[10], ptemp32[11]);
		rtd_pr_vpq_info("CSD3D,TBL_C:%d,%d,%d,\n", ptemp32[15], ptemp32[16], ptemp32[17]);
		rtd_pr_vpq_info("CSD3D,TBL_M:%d,%d,%d,writeMode=%d,\n", ptemp32[12], ptemp32[13], ptemp32[14], writeMode);
#endif
	}
...
static void Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByFactoryDecodeTBL(unsigned char CS_idx)
{
	RTK_VPQ_COLORSPACE_INDEX VPQ_COLORSPACE = {0};
	unsigned int i, j, k;
	unsigned int addrB, addrG, addrR, addrRGB;
		...
		for (i=0;i<VIP_D_3D_LUT_INDEX;i++)
		{
			...
			for (j=0;j<VIP_D_3D_LUT_INDEX;j++)
			{
				...
				for (k=0;k<VIP_D_3D_LUT_INDEX;k++)
				{
					...
					R_val = ((k * 4096)>65535)?(65535):(k * 4096);
					addrRGB = addrB + addrG + addrR;
					D_3DLUT[addrRGB][VIP_8VERTEX_R] = gD3DLUTbyCS_Decode_TBL_tmp->pt[addrRGB].r_data;
					D_3DLUT[addrRGB][VIP_8VERTEX_G] = gD3DLUTbyCS_Decode_TBL_tmp->pt[addrRGB].g_data;
					D_3DLUT[addrRGB][VIP_8VERTEX_B] = gD3DLUTbyCS_Decode_TBL_tmp->pt[addrRGB].b_data;
				}
			}
		}
	}

	// write fac tbl to 3d lut and save to "D_3DLUT_Reseult[VIP_D_3D_LUT_SIZE][VIP_8VERTEX_RGB_MAX]"
	Scaler_Set_ColorSpace_D_3dlutTBL(&VPQ_COLORSPACE);
}

char Scaler_Set_ColorSpace_D_3dlut_APDEM(unsigned char writeMode)
{
	unsigned char CS_idx;

	CS_idx = Scaler_Get_ColorMapping_3x3_Table();
	if (writeMode == VIP_D3DLUTbyCS_setByIni6Axis)
	{
		Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByIni6Axis(CS_idx);
	}
	else if (writeMode == VIP_D3DLUTbyCS_setByIniColorSpace)
	{
		Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByIniColorSpace(CS_idx);
	}
	else if (writeMode == VIP_D3DLUTbyCS_setByFactory6Axis)
	{
		Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByFactory6Axis(CS_idx);
	}
	else if (writeMode == VIP_D3DLUTbyCS_setByFactoryDecodeTBL)
	{
		Scaler_Set_ColorSpace_D_3dlut_APDEM_D3DLUTbyCS_setByFactoryDecodeTBL(CS_idx);
	}

	rtd_pr_vpq_info("CSD3D,CS_idx=%d,writeMode=%d,6axisNULL=%d,DecodeNULL=%d,6axisTmpNULL=%d,DecodeTmpNULL=%d,\n",
		CS_idx, writeMode, (gD3DLUTbyCS_6axis_TBL==NULL)?(1):(0), (gD3DLUTbyCS_Decode_TBL==NULL)?(1):(0),
		(gD3DLUTbyCS_6axis_tmp==NULL)?(1):(0), (gD3DLUTbyCS_Decode_TBL_tmp==NULL)?(1):(0));
...
void Scaler_VIP_PQ_QS_init(void)
{
	...
	Scaler_SetDNR(VIP_system_info_structure_table->OSD_Info.OSD_NR_Mode);
	fwif_VIP_set_Project_ID(VIP_Project_ID_TV001);
	fwif_color_SetAutoMAFlag(1);
	rtd_pr_vpq_info("Scaler_VIP_PQ_QS_init\n");
#endif
}
...
void Scaler_SetPQrBringUp(void)
{
	...
	dm_uzu_Ctrl_reg.hor12_table_sel = 0;
...
void Scaler_SetDataFormatHandler(unsigned char display)
{
	...
	//SCALER_DISP_CHANNEL display;
	unsigned short IVLg = (Scaler_DispGetStatus(display, SLR_DISP_INTERLACE) == 1)?(2):(1);
	
	down(&VPQ_I_CSC_Semaphore);
	mode = ((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN) * IVLg)>700)?(1):(0);	//Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR);
	//display = (SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY);
	channel = Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL);	
	channel = display;
	en_422to444 = (unsigned char)Scaler_DispGetStatus(display, SLR_DISP_422);
	InputSrcGetType = Get_DisplayMode_Src(display); // Scaler_InputSrcGetType(display);
#if 0
	/*
...
unsigned short Scaler_GetSharpnessTable(unsigned char display)
{
	...
	src_idx = (display >= 2)?(SLR_MAIN_DISPLAY):(display);
	return pTable->Shp_Table[src_idx];
}
...
void Scaler_SetSharpnessTable(unsigned char display, unsigned char value)
{
	...
	if (pTable == NULL) {
		rtd_pr_vpq_err("fwif_color_set_sharpness_table error %d \n", value);
		return;
	}
	...
	pTable->Shp_Table[src_idx] = value;
	rtd_pr_vpq_info("[RGB_Sharp],Scaler_SetSharpnessTable,display=%d,value=%d,src_idx=%d\n",display,value,src_idx);
	if (vpq_project_id == 0x00060000 && vpqex_project_id == 0x00060000) {
        ...
        #ifdef LGDB_OFF //control table by rtk
		fwif_color_set_sharpness_table(display, src_idx, value);//fixed me after demo please remove
		...
		fwif_color_set_sharpness_table(display, src_idx, value);
	}
...
void Scaler_SetScaleUP_YCHV(unsigned char HY, unsigned char VY, unsigned char HC, unsigned char VC)
{
	...
	PC_mode_check = Scaler_VPQ_check_PC_RGB444(SLR_MAIN_DISPLAY);
	if (PC_mode_check != VIP_Disable_PC_RGB444) // C need to use same table as Y in pc mode
	{
		HC = HY;
		VC = VY;
	}
	...
	fwif_color_set_scaleup_hv4tap_coef(src_idx, display, (signed char)HY, (signed char)VY, (signed char)HC, (signed char)VC);
}
...
void Scaler_Get_color_access_TVStatus_DisplayInfo(unsigned int *VIP_Access_Memory_DisplayInfo_Table)
{
	...
	VIP_Access_Memory_DisplayInfo_Table[Frame_rate_input] = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 100;
...
{
	_system_setting_info *VIP_system_info_structure_table = NULL;
	unsigned char APDEM_decontourLv;
	VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	...
	rtd_pr_vpq_info("===[scalerColorLib_Default.cpp] TableIdx = %d\n", TableIdx);
    ...
    fwif_color_Set_LC_texture(TableIdx);
	if (vpq_project_id != 0x00060000 || vpqex_project_id != 0x00060000)
	{
	...
	APDEM_decontourLv = (Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, 0, 0) == 0xFF)?(2):(Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, 0, 0));
	fwif_color_set_LC_decontour(src_idx, TableIdx, APDEM_decontourLv); // default set to middle
    fwif_color_Set_CDLC(TableIdx);
    fwif_color_Set_LC_Tonemapping_Yavg_Yin_Blend(TableIdx);
    fwif_color_Set_LC_shp_diff_gain(TableIdx);
...
void Scaler_SetPictureMode(unsigned char level)
{
	...
	Scaler_SetContrast(SLR_MAIN_DISPLAY, pData->Contrast[SLR_MAIN_DISPLAY]);
	Scaler_SetBrightness(SLR_MAIN_DISPLAY, pData->Brightness[SLR_MAIN_DISPLAY]);
	Scaler_SetSaturation(SLR_MAIN_DISPLAY, pData->Saturation[SLR_MAIN_DISPLAY]);
	/*Scaler_seticmhuesat_style(_MAIN_DISPLAY); */ /*set OSD contrast and brightness */ /* need set hue, can't not mark, elieli*/
	Scaler_SetHue(SLR_MAIN_DISPLAY, pData->Hue[SLR_MAIN_DISPLAY]);
...
void Scaler_ResetPictureQuality(SCALER_DISP_CHANNEL display)
{
	...
	Scaler_SetContrast(SLR_MAIN_DISPLAY, Scaler_GetContrast(SLR_MAIN_DISPLAY));
	Scaler_SetBrightness(SLR_MAIN_DISPLAY, Scaler_GetBrightness(SLR_MAIN_DISPLAY));
	/*Scaler_seticmhuesat_style(display);*/
	Scaler_SetSaturation(SLR_MAIN_DISPLAY, Scaler_GetSaturation(SLR_MAIN_DISPLAY));
	Scaler_SetHue(SLR_MAIN_DISPLAY, Scaler_GetHue(SLR_MAIN_DISPLAY));
...
void Scaler_Set_PQ_RGB444MODE(unsigned char path)
{
	...
	extern void VPQ_RgbShp_RegCtrl_SetRgb_shp_reg_table_yuvmode(unsigned char x);
    ...
    if(path==1){
		...
		VPQ_RgbShp_RegCtrl_SetRgb_shp_reg_table_yuvmode( 0 );
    }else{
		...
		VPQ_RgbShp_RegCtrl_SetRgb_shp_reg_table_yuvmode( 1 );
    }
#endif
}
...
unsigned char Scaler_VPQ_check_PC_RGB444(unsigned char display)
{
	...
	extern bool is_dolby_vision_source(unsigned char display);
	if (Get_Val_vsc_run_pc_mode())
	{

		if (Get_DisplayMode_Src(display) == VSC_INPUTSRC_HDMI)
		{
			rtd_pr_vpq_info("[for PcMode]dis=%d,port=%d,pc_mode=%d,DisplayMode_Src=%d,Hdmi_ColorSpace()=%d,is_dv_src()=%d,interlace=%d,\n",
				display, port, Get_Val_vsc_run_pc_mode(),Get_DisplayMode_Src(display), drvif_Hdmi_Multi_port_GetColorSpace(port), 
				is_dolby_vision_source(display), Scaler_DispGetStatus(display, SLR_DISP_INTERLACE));
			if ((drvif_Hdmi_Multi_port_GetColorSpace(port) == COLOR_RGB) && (!is_dolby_vision_source(display)) && (!Scaler_DispGetStatus(display, SLR_DISP_INTERLACE))){
		...
		{
#if IS_ENABLED(CONFIG_RTK_DPRX)
			rtd_pr_vpq_info("[for PcMode]dis=%d,port=%d,pc_mode=%d,DisplayMode_Src=%d,Dprx_ColorSpace()=%d,is_dv_src()=%d,interlace=%d,\n",
				display, port, Get_Val_vsc_run_pc_mode(),Get_DisplayMode_Src(display), drvif_Dprx_GetPortColorSpace(port), 
				is_dolby_vision_source(display), Scaler_DispGetStatus(display, SLR_DISP_INTERLACE));
			if ((drvif_Dprx_GetPortColorSpace(port) == DP_COLOR_SPACE_RGB) && (!Scaler_DispGetStatus(display, SLR_DISP_INTERLACE))	 && (!is_dolby_vision_source(display))){
				ret_val = VIP_DP_PC_RGB444;
			}else{
				ret_val = VIP_DP_PC_YUV444;
			}
#else
			rtd_pr_vpq_info("[for PcMode]CONFIG_RTK_DPRX\n");
			ret_val = VIP_Disable_PC_RGB444;
#endif
		} else {
			rtd_pr_vpq_info("[for PcMode]display=%d,port=%d,Get_DisplayMode_Src(display)=%d,\n", display, port, Get_DisplayMode_Src(display));
			ret_val = VIP_Disable_PC_RGB444;
		}
	}
    else {
		rtd_pr_vpq_info("[for PcMode]UI Off\n");
		ret_val = VIP_Disable_PC_RGB444;
        //Scaler_Set_PQ_RGB444MODE(0);
    }
#endif
...
void Scaler_RefreshPictureMode_sub(void)
{
	...
	Scaler_SetContrast(SLR_SUB_DISPLAY, Scaler_GetContrast(SLR_SUB_DISPLAY));
	Scaler_SetBrightness(SLR_SUB_DISPLAY, Scaler_GetBrightness(SLR_SUB_DISPLAY));
	Scaler_SetHue(SLR_SUB_DISPLAY, Scaler_GetHue(SLR_SUB_DISPLAY));
	Scaler_SetSaturation(SLR_SUB_DISPLAY, Scaler_GetSaturation(SLR_SUB_DISPLAY));

	rtd_pr_vpq_info("RefreshPictureMode_sub\n");
...
void Scaler_RefreshPictureMode(void)
{
...
#endif
#if 0	// no need, OSD ini and aP will control bypass items
	// for Dolby HDR
	if (VIP_system_info_structure_table->DolbyHDR_flag) {
	...
	// bypass ctrl
	pq_bypass_lv = (g_pq_bypass_lv_ctrl_by_AP>>8)&0xFF;
	if (pq_bypass_lv < VIP_PQ_ByPass_TBL_Max && pq_bypass_lv != 0)	// no need to reset again while idx = 0xFF/0x00
		Scaler_color_Set_PQ_ByPass_Lv(pq_bypass_lv);

	rtd_pr_vpq_info("PC mode check = %d, g_pq_bypass_lv_ctrl_by_AP=0x%x,pq_bypass_lv=%d,\n",
		PC_mode_check, g_pq_bypass_lv_ctrl_by_AP, pq_bypass_lv);
	...
	// for Miracast
	{
		extern void VPQ_Cbrev_Lib_SetCbrev_PqLevel(unsigned char gain);
		SLR_VOINFO* pVOInfo = Scaler_VOInfoPointer(Scaler_Get_CurVoInfo_plane());
	...
	// AI SR Demo Mode
	fwif_color_set_ai_sr_demo_mode_update(3);
}
...
unsigned char Scaler_GetContrast(unsigned char dis)
{
	...
	if (pData == NULL)
		return 0;
	display = (dis>SLR_SUB_DISPLAY)?(SLR_MAIN_DISPLAY):(dis);
	return pData->Contrast[display];
}

/*range 0~100*/
void Scaler_SetContrast(unsigned char dis, unsigned char value)
{
	SLR_PICTURE_MODE_DATA *pData = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
	...
	//fwif_color_setContrast(src_idx, SLR_SUB_DISPLAY, value);
}
...
unsigned char Scaler_GetBrightness(unsigned char dis)
{
	...
	if (pData == NULL)
		return 0;	
	display = (dis>SLR_SUB_DISPLAY)?(SLR_MAIN_DISPLAY):(dis);
	return pData->Brightness[display];
}

void Scaler_SetBrightness(unsigned char dis, unsigned char value)
{
	SLR_PICTURE_MODE_DATA *pData = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
...
unsigned char Scaler_GetSaturation(unsigned char dis)
{
	...
	if (pData == NULL)
	 return 0;
	display = (dis>SLR_SUB_DISPLAY)?(SLR_MAIN_DISPLAY):(dis);
	return pData->Saturation[display];
}

/*range 0~100*/
void Scaler_SetSaturation(unsigned char dis, unsigned char value)
{
	SLR_PICTURE_MODE_DATA *pData = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
...
unsigned char Scaler_GetHue(unsigned char dis)
{
	...
	if (pData == NULL)
		return 50;
	display = (dis>SLR_SUB_DISPLAY)?(SLR_MAIN_DISPLAY):(dis);
	return pData->Hue[display];
}

/*range 0~100*/
void Scaler_SetHue(unsigned char dis, unsigned char value)
{
	SLR_PICTURE_MODE_DATA *pData = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
	...
	if (pData == NULL)
		return;
	display = (dis>SLR_SUB_DISPLAY)?(SLR_MAIN_DISPLAY):(dis);
	pData->Hue[display] = value;

	/*Scaler_seticmhuesat_style(display);*/
	fwif_color_setHue(src_idx, display, value);
	//fwif_color_setHue(src_idx, SLR_SUB_DISPLAY, value);
}
...
void Scaler_SetSharpness(unsigned char value)
{
	...
	fwif_color_set_sharpness_level(Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY), value);
...
void Scale_SetHDR3DLUT_Offset(int offset)
{
	...
	//Scaler_color_HDRIP_AutoRun(Scaler_color_HAL_VPQ_HDR_Type_to_HDR_DM_Type(HDR_flag));	// apply 3D lut cinv in Scaler_color_Get_HDR10_EOTF_OETF_ByMaxCLL_ByPanelLuminance_for_task
...
void Scaler_color_copy_scalerinfo_to_vip(unsigned char display)
{
	...
	VIP_system_info_structure_table->HDMI_video_format	= drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)));
	VIP_system_info_structure_table->Display_RATIO_TYPE	= Scaler_DispGetRatioMode();
...
unsigned char Scaler_get_HDR_YUV2RGB_SDR_MAX_RGB_TV006(RGB2YUV_BLACK_LEVEL blk_lv)
{
	...
	if (srcType == _SRC_HDMI) {
		...
		video_format = drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)));
		rtd_pr_vpq_info("ori_CS=%d,new_CS=%d,%s->%d,%s\n",drvif_Hdmi_GetColorSpace(),drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))), __FILE__, __LINE__, __FUNCTION__);
        isHDMI = drvif_IsHDMI();
...
unsigned char Scaler_get_HDR_YUV2RGB_SDR_MAX_RGB(void)
{
	...
	if (srcType == _SRC_VGA) {
		...
		if (VIP_system_info_structure_table->HDR_flag == HAL_VPQ_HDR_MODE_SDR_MAX_RGB) {	// max RGB
			if ((Scaler_DispGetInputInfo(SLR_INPUT_COLOR_SPACE) == SCALER_INPUT_COLOR_RGB) || (Scaler_VOFrom(Scaler_Get_CurVoInfo_plane()) == 2)) {
				if (colorFMT_info == isBT2020) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_BT2020_255_to_255;
				} else if (colorFMT_info == is709) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_BT709_255_to_255;
...
unsigned char Scaler_get_HDR_YUV2RGB(unsigned char color_format)
{
	...
	unsigned char video_format = COLOR_UNKNOW;
	unsigned char vsc_input_src = Get_DisplayMode_Src(SLR_MAIN_DISPLAY);
    unsigned char srcType = _SRC_HDMI;
	...
	if (vsc_input_src == VSC_INPUTSRC_HDMI) {
		...
		if (display == SLR_MAIN_DISPLAY) {
			srcType = Scaler_InputSrcGetType(display);
			get_vtop_input_info(display, &twopixel_mode, &hdmi_input, &hdmi_color_format);
		}
	...
	rtd_pr_vpq_info("CSC1, display=%d,vsc_input_src=%d,srcType=%d,color_fmt=%d,video_fmt=%d,FullRange=%d,ret=%d\n",
		display, vsc_input_src,srcType, color_format, video_format, isFullRange,  ret_idx);

	return ret_idx;

}

unsigned char Scaler_get_APDEM_HDR_YUV2RGB(unsigned char display, unsigned char HDR_mode)
{
	unsigned char ret_idx=0;
	unsigned char isHD=0/*, SrcGetFrom=0*/;
	//unsigned char display = (unsigned char)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY);
	...
	unsigned char IsRGB_YUV_RANGE = 255;
	//unsigned char srcType = Scaler_InputSrcGetType(display);
	//unsigned short mode = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MODE_CURR); //Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR);
	unsigned char isHDMI=0;
	unsigned char twopixel_mode = 0, hdmi_input = 0, color_format = 0;
	...
	unsigned char vdecSrcType;// = Get_Val_scaler_vdec_source_type();
	//unsigned char bUse_HDMI_Range_Detect = 1;
	//unsigned char isHDR_Signal = 0;
	unsigned char src_type = Get_DisplayMode_Src(display);
	unsigned char port = Get_DisplayMode_Port(display);	
	unsigned short IVLg = (Scaler_DispGetStatus(display, SLR_DISP_INTERLACE) == 1)?(2):(1);
	unsigned short v_len = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN) * IVLg;
	unsigned int APDEM_Data_Range;
#if 0	
	if (fwif_color_get_force_run_i3ddma_enable(display)) {
		fwif_color_vsc_to_scaler_src_mapping(Get_DisplayMode_Src(display), &srcType, &SrcGetFrom);
		mode = fwif_color_get_cur_input_timing_mode(display);
	...
	pVOInfo = Scaler_VOInfoPointer(port);	// use display to get vo info	
	//drvif_Hdmi_GetAviInfoFrame((HDMI_AVI_T *) (&pAviInfo));
	drvif_Hdmi_Multi_port_GetAviInfoFrame(port, (HDMI_AVI_T *) (&pAviInfo)); 	// use display to get hdmi info
	...
	if (display == SLR_MAIN_DISPLAY)
		APDEM_Data_Range = Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range, 0, 0);
	else
		APDEM_Data_Range = Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range_sub, 0, 0);
	
	if (v_len > 700)
		isHD = 1;

	PCRGB444 = 	Scaler_VPQ_check_PC_RGB444(display);

#if IS_ENABLED(CONFIG_RTK_DPRX)
	if (src_type == VSC_INPUTSRC_DP) {	// get DP info
		DP_PIXEL_ENCODING_FORMAT_T dp_info;
		//if (drvif_Dprx_GetPixelEncodingFormat(&dp_info)) {
		if (drvif_Dprx_GetPortPixelEncodingFormat(port, &dp_info)) {
			...
			if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_AUTO) {
				DP_data_range = (dp_info.dynamic_range == DP_DYNAMIC_RANGE_VESA) ? CSC_DATA_RANGE_MODE_FULL : CSC_DATA_RANGE_MODE_LIMITED;
			} else {
				DP_data_range = APDEM_Data_Range;
			}
		}
		isDP_src = 1;
		DP_video_format = drvif_Dprx_GetPortColorSpace(port); //drvif_Dprx_GetColorSpace();
	...
	if ((PCRGB444 == VIP_HDMI_PC_RGB444) || (PCRGB444 == VIP_DP_PC_RGB444)) {
		if (isDP_src == 1) {
			...
			IsRGB_YUV_RANGE = drvif_Multi_port_IsRGB_YUV_RANGE(port); // drvif_IsRGB_YUV_RANGE();
			if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_AUTO) {
				if (IsRGB_YUV_RANGE==MODE_RAG_LIMIT) {
					data_range = CSC_DATA_RANGE_MODE_LIMITED;
				} else {
					data_range = CSC_DATA_RANGE_MODE_FULL;
				}
			} else {
				data_range = APDEM_Data_Range;
			}
		}
		colorFMT_info = isBT2020;
		isRGB_in = 0;
		...
		if (HDR_mode == HAL_VPQ_HDR_MODE_ST2094){
			...
			if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_FULL)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else
				data_range = CSC_DATA_RANGE_MODE_LIMITED;
		} else if (src_type == VSC_INPUTSRC_HDMI) {
			if (isDP_src == 1) {
				...
				IsRGB_YUV_RANGE = drvif_Multi_port_IsRGB_YUV_RANGE(port);	// drvif_IsRGB_YUV_RANGE();
				video_format = drvif_Hdmi_Multi_port_GetColorSpace(port);
				//rtd_pr_vpq_info("ori_CS=%d,new_CS=%d,%s->%d,%s\n",drvif_Hdmi_GetColorSpace(),drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))), __FILE__, __LINE__, __FUNCTION__);
				isHDMI = drvif_Multi_port_IsHDMI(port);		// drvif_IsHDMI();
		...
		} else if ((src_type == VSC_INPUTSRC_VDEC) || (src_type == VSC_INPUTSRC_JPEG)) {
			if (pVOInfo->colour_primaries == 1) {
				colorFMT_info = is709;
			} else if (pVOInfo->colour_primaries == 6 || pVOInfo->colour_primaries == 7) {
				colorFMT_info = is601;
			...
			if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_FULL)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_AUTO && Get_DisplayMode_Src(display) == VSC_INPUTSRC_CAMERA)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_AUTO && Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE) == 1) //Photo
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else if (vdecSrcType == VSC_VDEC_CASE_AIRPLAY)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else
		...
		else if (srcType == _SRC_VGA) 
		{
			...
			if (APDEM_Data_Range == CSC_DATA_RANGE_MODE_LIMITED)
				data_range = CSC_DATA_RANGE_MODE_LIMITED;
			else
				data_range = CSC_DATA_RANGE_MODE_FULL;
	...
	// get ret_index
	//   1.pure RGB flow: FtoL or LtoL
	//   2.HDR_data_range: FtoF or LtoF
	//   3.SDR_data_range: FtoF or LtoL
	if (isRGBin_RGBout == 1) {
		if((HDR_mode == HAL_VPQ_HDR_MODE_HDR10) || (HDR_mode == HAL_VPQ_HDR_MODE_HLG))	{
			...
			if(data_range == CSC_DATA_RANGE_MODE_FULL){
				ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_255_to_235;
			} else{
				ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_255_to_255;
			}
		}
	...
	rtd_pr_vpq_info("[CSC1] APDEM_CSC1,HDR_mode=%d,PCRGB444=%d,isRGBin_RGBout=%d,hdmi_input=%d,color_format=%d,dis=%d,isHD=%d,data_range=%d,IsRGB_YUV_RANGE=%d,isRGB_in=%d,APDEM_lv=%d,isPhoto=%d\n",
		HDR_mode, PCRGB444, isRGBin_RGBout, hdmi_input, color_format, display, isHD, data_range, IsRGB_YUV_RANGE, isRGB_in, 
		APDEM_Data_Range, Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_VO_SOURCE_TYPE));

	rtd_pr_vpq_info("[CSC1] APDEM_CSC1,video_format=%d,isHDMI=%d,.EC=%d,.C=%d,colorFMT_info=%d,data_range=%d,colour_primaries=%d,port=%d,v_len=%d,IVLg=%d,ret_idx=%d,\n",
		video_format, isHDMI, pAviInfo.EC, pAviInfo.C, colorFMT_info, data_range, pVOInfo->colour_primaries, 
		port, v_len, IVLg, ret_idx);
...
void Scaler_Set_HDR_YUV2RGB(unsigned char display, unsigned char HDR_mode, unsigned char color_format)
{
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		{
			index = Scaler_get_APDEM_HDR_YUV2RGB(display, HDR_mode);
		...
		index = Scaler_get_APDEM_HDR_YUV2RGB(display, HDR_mode);
		//drvif_HDR_YUV2RGB(1, hdr_YUV2RGB[index]);
		fwif_color_HDR_sub_set_yuv2rgb_en(1);
		fwif_color_HDR_sub_set_yuv2rgb(index);
	}
}
#if 0
...
void drvif_TV006_SET_HDR_mode(UINT32* modeinfo)
{
	...
	case HAL_VPQ_HDR_MODE_HDR10:
		...
		Scaler_Set_HDR_YUV2RGB((unsigned char)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), modeinfo[0], modeinfo[1]);
...
void Scaler_hdr_setting_SEAMLESS(unsigned char hdr_enable)
{
	...
	sdr_force_vtop = get_support_vo_force_v_top(SLR_MAIN_DISPLAY, support_sdr_max_rgb);
	if(hdr_enable)
	{
		if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) {	/* HDMI case*/
			...
			else if (vdec_cur_hdr_type == HDR_DM_VIVID)
			{
	...
	*/
	...
	//Scaler_Set_HDR_YUV2RGB(VIP_system_info_structure_table->HDR_flag, HAL_VPQ_COLORIMETRY_BT2020);	// semaphore use, move to vpq task
...
unsigned char DataFormatHandle_Seamless_flag = false;
void Scaler_SetDataFormatHandle_Seamless(void)
{
	...
	DataFormatHandle_Seamless_flag = true;
	display = SLR_MAIN_DISPLAY;	// (SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY);
	IVLg = (Scaler_DispGetStatus(display, SLR_DISP_INTERLACE) == 1)?(2):(1);
	mode = ((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN) * IVLg)>700)?(1):(0);	//Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR);
	channel = display;	// Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL);
	en_422to444 = (unsigned char)Scaler_DispGetStatus(display, SLR_DISP_422);
	InputSrcGetType = Get_DisplayMode_Src(display); // Scaler_InputSrcGetType(display);
#if 0
	/*
	...
	*/
	...
	if (vpq_project_id != 0x00060000 || vpqex_project_id != 0x00060000)
		...
		fwif_color_SetDataFormatHandler_tv006();
	DataFormatHandle_Seamless_flag = false;
	//up(&VPQ_I_CSC_Semaphore);
}
...
void Scaler_color_set_HDR_AutoRun_TBL(void)
{
	...
	if ((HDR_AutoRun_TBL_Flag != NULL) && (VIP_system_info_structure_table != NULL)) 
	{
		// seamless for main, sub is not yet
		Scaler_Set_HDR_YUV2RGB(SLR_MAIN_DISPLAY, VIP_system_info_structure_table->HDR_flag, HAL_VPQ_COLORIMETRY_BT2020);
		
		Scaler_color_HDRIP_AutoRun(SLR_MAIN_DISPLAY, HDR_AutoRun_TBL_Flag[0]);
...
//========check HDR statu ===========
	...
	unsigned int modeinfo[5] = {0};
#ifdef CONFIG_CUSTOMER_TV002
	extern unsigned char g_bHDR_is601;
#endif
	input_src_type = Get_DisplayMode_Src(display);

#if 0
 	{
		if (fwif_color_get_force_run_i3ddma_enable(display) && (Get_DisplayMode_Src(display) != VSC_INPUTSRC_VDEC) &&
	...
	fwif_vip_HDR10_check(display, input_src_type);
#endif
	bHDR_flag = VIP_system_info_structure_table->HDR_flag;
...
//==========================================
	...
	if(Scaler_color_Get_HDR_AutoRun()) {
		...
		Scaler_color_HDRInfo_Handler(display, modeinfo);
		#endif
	}
	rtd_pr_vpq_info("HDR auto, dis=%d, HDR_Type=%d, input_src_type=%d, i3d_en=%d, Get_DisplayMode_Src=%d\n",
		display, HDR_Type, input_src_type, fwif_color_get_force_run_i3ddma_enable(display), Get_DisplayMode_Src(display));
...
HDR_flow_control_timing Scaler_color_get_HDR_timing(unsigned char display)
{
	HDR_flow_control_timing result = HFC_SDR_YUV;	/*default*/
	_system_setting_info *VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	unsigned char port = Get_DisplayMode_Port(display);
	//UINT8 hdmi_video_format = drvif_Hdmi_Multi_port_GetColorSpace(port);
	unsigned char video_format = 0xFF;
	//UINT8 input_timming_info = Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR);
	unsigned short input_timming_info = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN);
	UINT8 uhd_flag = 0;
	//UINT8 vo_video_format = Scaler_DispGetInputInfo(SLR_INPUT_COLOR_SPACE);
	UINT8 vo_video_format = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_COLOR_SPACE);
	//UINT8 input_src_type = Scaler_InputSrcGetMainChType();
	UINT8 input_src_type = Get_DisplayMode_Src(display);
	unsigned char vdecSrcType = Get_Val_scaler_vdec_source_type(display);
	...
	{
		DolbyHDR_flag = VIP_system_info_structure_table->DolbyHDR_flag_sub;
		HDR_flag = VIP_system_info_structure_table->HDR_flag_sub;
	}

	if (input_timming_info >= 1200)
		uhd_flag = 1;

	//if ((input_src_type == VSC_INPUTSRC_VDEC) && (input_VO_Form != VO_FROM_JPEG) && (input_TVD_Form == TVD_INFMT_VODMA1))
	if (vdecSrcType == VSC_VDEC_CASE_PLAYBACK)
		usb_flag = 1;

	// video_format: 0:RGB, 1:422, 2:444, 3:420
	if (input_src_type == VSC_INPUTSRC_HDMI)
	{
		video_format = drvif_Hdmi_Multi_port_GetColorSpace(port);
	}
	else if (input_src_type == VSC_INPUTSRC_DP)
	{
		video_format = drvif_Dprx_GetPortColorSpace(port);
		if (video_format == DP_COLOR_SPACE_YUV444)
			video_format = 2;
		else if (video_format == DP_COLOR_SPACE_YUV422)
			video_format = 1;
	}
	else
	{
...
#endif
	if (DolbyHDR_flag) {	//dolby case
		result = HFC_DOLBY;
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_HDR10_VIVID) {
		result = HFC_HDR_VIVI;
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_SDR_MAX_RGB) {
		result = HFC_SDR_MAX_RGB;
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_SDR) {	//SDR case
		if (video_format == 0)
			result = HFC_SDR_RGB;
		else
			result = HFC_SDR_YUV;
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_HDR10) {	/*HDR10*/
		if (video_format != 0xFF) {		/*HDMI or DP*/
			if (video_format == 0) {			/*RGB*/
				if (uhd_flag) result = HFC_HDMI_HDR10_RGB_UHD;
				else result = HFC_HDMI_HDR10_RGB_HD;
			} else if (video_format == 1) {	/*YUV422*/
				if (uhd_flag) result = HFC_HDMI_HDR10_YUV422_UHD;
				else result = HFC_HDMI_HDR10_YUV422_HD;
			} else if (video_format == 2) {	/*YUV444*/
			...
			if (vo_video_format == SCALER_INPUT_COLOR_RGB) {			/*RGB*/
				if (uhd_flag) result = HFC_USB_HDR10_RGB_UHD;
				else result = HFC_USB_HDR10_RGB_HD;
			} else if (vo_video_format == SCALER_INPUT_COLOR_YUV422) {	/*YUV422*/
				if (uhd_flag) result = HFC_USB_HDR10_YUV422_UHD;
				else result = HFC_USB_HDR10_YUV422_HD;
			} else if (vo_video_format == SCALER_INPUT_COLOR_YUV444) {	/*YUV444*/
	...
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_HLG) {	//HLG case
		if (video_format != 0xFF) {		/*HDMI or DP*/
			if (video_format == 0) {			/*RGB*/
	...
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_TECHNICOLOR) {	//TECHNICOLOR case
		result = HFC_TC;
	} else if (HDR_flag == HAL_VPQ_HDR_MODE_ST2094) {	//HDR10+ case
		result = HFC_ST2094;
	} else {
		result = HFC_SDR_YUV;	/*default*/
	}

	rtd_pr_vpq_info("[HFC]dis=%d,DV_flag,HDR_flag=%d,port=%d,uhd_flag=%d,vo_video_format=%d,input_src_type=%d,vdecSrcType=%d,usb_flag=%d,result=%d,\n", 
		display, DolbyHDR_flag, HDR_flag, port, uhd_flag, vo_video_format, input_src_type, vdecSrcType, usb_flag, result);
...
void Scaler_color_HDRInfo_Update_by_HFC(unsigned char display, UINT8 HFC_bit, HDR_flow_control_timing HDR_timing, UINT32* modeinfo)
{
	extern unsigned char g_HDR3DLUTForceWrite;
	extern HDR_flow_control HDR_control[HFC_SDR_TIMING_NUM];
	extern unsigned int HDR_sub_EOTF_HDR10[HDR2_EOTF_CURVE_LEN];
	extern unsigned int HDR_sub_EOTF_HLG[HDR2_EOTF_CURVE_LEN];
	extern unsigned short HDR_sub_OETF_HDR10[HDR2_OETF_CURVE_LEN];
	extern unsigned short HDR_sub_OETF_HLG [HDR2_OETF_CURVE_LEN];
	extern HDR2_OETF_PRO HDR2_OETF_index[OETF_prog_idx_tbl_num];	
	unsigned char tbl_index, IPCtrlBits;	
	_system_setting_info *sysInfo = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	...
	if (sysInfo == NULL)
	{
		rtd_pr_vpq_emerg("Scaler_color_HDRInfo_Update_by_HFC NULL\n");
		return;
	}
	...
	{
		// EOTF
		IPCtrlBits = HDR_control[HDR_timing].EOTF_ctrl.IPCtrlBits;
		if (IPCtrlBits & HFC_bit) {
			tbl_index = HDR_control[HDR_timing].EOTF_SPtable;
			if (tbl_index == HFC_EOTF_OFF) {
				fwif_color_HDR_sub_set_EOTF_en(0);
			} else {
				if (sysInfo->HDR_flag_sub == HAL_VPQ_HDR_MODE_HDR10)
					fwif_color_HDR_sub_set_EOTF(&HDR_sub_EOTF_HDR10[0], &HDR_sub_EOTF_HDR10[0], &HDR_sub_EOTF_HDR10[0], 0);
				else
					fwif_color_HDR_sub_set_EOTF(&HDR_sub_EOTF_HLG[0], &HDR_sub_EOTF_HLG[0], &HDR_sub_EOTF_HLG[0], 0);
				fwif_color_HDR_sub_set_EOTF_en(1);
			}
		}
		rtd_pr_vpq_info("DM_EOTF_DMA,sub set by HFC,HDR_timing=%d,IPCtrlBits=%d,tbl_index=%d,HFC_bit=%d,HDR_flag_sub=%d,\n",
			HDR_timing, IPCtrlBits, tbl_index, HFC_bit, sysInfo->HDR_flag_sub);
		
		// 3x3
		IPCtrlBits = HDR_control[HDR_timing].HDR_33_1_ctrl.IPCtrlBits;
		if (IPCtrlBits & HFC_bit)
		{
			tbl_index = HDR_control[HDR_timing].HDR_33_1_SPtable;
			if (tbl_index == HFC_RGB2OPT_OFF)
			{
				fwif_color_HDR_sub_set_3x3_en(0);
			}
			else
			{
				fwif_color_HDR_sub_set_3x3_en(1);
				fwif_color_HDR_sub_set_3x3(tbl_index);
			}
		}		
		rtd_pr_vpq_info("DM_3x3, sub set by HFC, HDR_timing=%d, IPCtrlBits=%d, tbl_index=%d, HFC_bit=%d,\n",
			HDR_timing, IPCtrlBits, tbl_index, HFC_bit);

		// OETF
		IPCtrlBits = HDR_control[HDR_timing].OETF_ctrl.IPCtrlBits;
		if (IPCtrlBits & HFC_bit) {
			tbl_index = HDR_control[HDR_timing].OETF_SPtable;
			if (tbl_index == HFC_OETF_OFF) {
				fwif_color_HDR_sub_set_BBC_Gain_en(0);
				fwif_color_HDR_sub_set_OETF_en(0);
			} else {				
				if (sysInfo->HDR_flag_sub == HAL_VPQ_HDR_MODE_HDR10) 
					fwif_color_HDR_sub_set_OETF(&HDR_sub_OETF_HDR10[0], &HDR_sub_OETF_HDR10[0], &HDR_sub_OETF_HDR10[0], 0);
				else
					fwif_color_HDR_sub_set_OETF(&HDR_sub_OETF_HLG[0], &HDR_sub_OETF_HLG[0], &HDR_sub_OETF_HLG[0], 0);		
				fwif_color_HDR_sub_set_BBC_Gain_en(0);
				fwif_color_HDR_sub_set_OETF_en(1);
			}
		}
		rtd_pr_vpq_info("DM_OETF_DMA,sub set by HFC,HDR_timing=%d,IPCtrlBits=%d,tbl_index=%d,HFC_bit=%d,HDR_flag_sub=%d,\n",
			HDR_timing, IPCtrlBits, tbl_index, HFC_bit, sysInfo->HDR_flag_sub);
...
{
	HDR_flow_control_timing HDR_timing = Scaler_color_get_HDR_timing(display);
	Scaler_color_HDRInfo_Update_by_HFC(display, HFC_control_by_PQ, HDR_timing, modeinfo);
}
...
unsigned char  Scaler_Set_I_De_Contour(unsigned char value)
{
	...
	lv = (Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, 0, 0) == 0xFF)?(Scaler_GetMPEGNR()):(Scaler_APDEM_Arg_Access(DEM_ARG_Smooth_Gradation, 0, 0));
	return fwif_color_Set_I_De_Contour(value, lv);
...
char Scaler_scalerVIP_PQ_IP_Enable_Monitor(SLR_VIP_TABLE* VIP_TBL, _RPC_clues* RPC_SmartPic_clue, _clues* SmartPic_clue, _RPC_system_setting_info* RPC_sysInfoTBL, _system_setting_info* sysInfoTBL, unsigned int MA_print_count)
{
	...
	// FixMe, 20190925
	//unsigned char *FilmInfo = Scaler_VPQ_MEMC_GetCadence();
	...
	if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) ==  VSC_INPUTSRC_VDEC) {
		frame_rate = Scaler_VOInfoPointer(0)->v_freq_1000;
		height = Scaler_VOInfoPointer(0)->v_length;
	} else {
		frame_rate = Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000);
		height = sysInfoTBL->I_Height;
	}
	/* condition 1, 4k/60, video*/
	if ((height > 1300) && (frame_rate > 31000) && (isFilm == 0))
...
SLR_HDR_TABLE* ptr_HDR_bin;
SLR_HDR_TABLE_v4* ptr_HDR_bin_v4;
unsigned int HDR_bin_version = 4;
extern unsigned short panel_luminance;
extern VIP_HLG_EOTF_ByLuminace_TBL HLG_EOTF_ByLuminance_TBL_cur;
extern VIP_HLG_OETF_ByLuminace_TBL HLG_OETF_ByLuminance_TBL_cur;
extern VIP_HDR10_EOTF_ByLuminace_TBL_v4 HDR10_EOTF_ByLuminace_TBL_cur;
...
unsigned int Scaler_color_get_HDRver(void){
	return HDR_bin_version;
}

void Scaler_color_ChangeVer_EOTF_Table(VIP_HDR10_EOTF_ByLuminace_TBL_v4* new_table, VIP_HDR10_EOTF_ByLuminace_TBL* old_table_v3){
	memcpy(new_table->EOTF_setting, old_table_v3->EOTF_setting, sizeof(new_table->EOTF_setting));
	memcpy(new_table->EOTF_calman, old_table_v3->EOTF_calman, sizeof(new_table->EOTF_calman));
	memcpy(new_table->EOTF_normal, old_table_v3->EOTF_normal, sizeof(new_table->EOTF_normal));
	memcpy(new_table->EOTF_dummy, old_table_v3->EOTF_normal, sizeof(new_table->EOTF_dummy));
	memcpy(new_table->EOTF_WithoutMetadata, old_table_v3->EOTF_WithoutMetadata, sizeof(new_table->EOTF_WithoutMetadata));
}

void Scaler_color_ChangeVer_OETF_Table(VIP_HDR10_OETF_ByLuminace_TBL_v4* new_table, VIP_HDR10_OETF_ByLuminace_TBL* old_table_v3){
	memcpy(new_table->OETF_setting, old_table_v3->OETF_setting, sizeof(new_table->OETF_setting));
	memcpy(new_table->OETF_calman, old_table_v3->OETF_calman, sizeof(new_table->OETF_calman));
	memcpy(new_table->OETF_normal, old_table_v3->OETF_normal, sizeof(new_table->OETF_dummy));
	memcpy(new_table->OETF_WithoutMetadata, old_table_v3->OETF_WithoutMetadata, sizeof(new_table->OETF_WithoutMetadata));
	memcpy(new_table->OETF_dummy, old_table_v3->OETF_normal, sizeof(new_table->OETF_normal));
	memcpy(new_table->OETF_calamn_Lmix, old_table_v3->OETF_calamn_Lmix, sizeof(new_table->OETF_calamn_Lmix));
	memcpy(new_table->OETF_normal_Lmix, old_table_v3->OETF_normal_Lmix, sizeof(new_table->OETF_normal_Lmix));
	memcpy(new_table->OETF_dummy_Lmix, old_table_v3->OETF_normal_Lmix, sizeof(new_table->OETF_dummy_Lmix));
	memcpy(new_table->OETF_WithoutMetadata_Lmix, old_table_v3->OETF_WithoutMetadata_Lmix, sizeof(new_table->OETF_WithoutMetadata_Lmix));
}

void Scaler_color_Set_EOTF_OETF_Table(void)
{	
	rtd_pr_vpq_emerg("Scaler_color_Set_EOTF_OETF_Table, panel_luminance=%d, HDR_ver=%d", panel_luminance, Scaler_color_get_HDRver());

	if(ptr_HDR_bin == NULL && ptr_HDR_bin_v4 == NULL){
		rtd_pr_vpq_info("Scaler_color_Set_EOTF_OETF_Table, ptr_HDR_bin=NULL, ptr_HDR_bin_v4=NULL");
		return;
	} else if(ptr_HDR_bin == NULL && ptr_HDR_bin_v4 != NULL){
		rtd_pr_vpq_info("Scaler_color_Set_EOTF_OETF_Table, ptr_HDR_bin=NULL, ptr_HDR_bin_v4!=NULL");
	} else if(ptr_HDR_bin != NULL && ptr_HDR_bin_v4 == NULL){
		rtd_pr_vpq_info("Scaler_color_Set_EOTF_OETF_Table, ptr_HDR_bin!=NULL, ptr_HDR_bin_v4==NULL");
	}
	...
	if((ptr_HDR_bin != NULL) && (Scaler_color_get_HDRver() == 3)){
		if(panel_luminance <= 275){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_250N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_250N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_250N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_250N);
			
		} else if(panel_luminance > 275 && panel_luminance<= 350){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_300N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_300N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_300N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_300N);
			
		} else if(panel_luminance > 350 && panel_luminance <= 450){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_400N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_400N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_400N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_400N);
			
		} else if(panel_luminance > 450 && panel_luminance <= 550){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_500N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_500N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_500N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_500N);
			
		} else if(panel_luminance > 550 && panel_luminance <= 650){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_600N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_600N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_600N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_600N);
			
		} else if(panel_luminance > 650 && panel_luminance <= 750){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_700N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_700N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_700N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_700N);
			
		} else if(panel_luminance > 750 && panel_luminance <= 850){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_800N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_800N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_800N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_800N);
			
		} else if(panel_luminance > 850 && panel_luminance <= 950){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_900N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_900N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_900N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_900N);
			
		} else if(panel_luminance > 950){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_EOTF_ByLuminance_TBL_1000N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin->HLG_OETF_ByLuminance_TBL_1000N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			Scaler_color_ChangeVer_EOTF_Table(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_EOTF_ByLuminace_TBL_1000N);
			Scaler_color_ChangeVer_OETF_Table(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin->HDR10_OETF_ByLuminace_TBL_1000N);
			
		}

	}else if((ptr_HDR_bin_v4 != NULL) && (Scaler_color_get_HDRver() == 4)){
		if(panel_luminance <= 275){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_250N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_250N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_250N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_250N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 275 && panel_luminance<= 350){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_300N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_300N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_300N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_300N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 350 && panel_luminance <= 450){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_400N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_400N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_400N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_400N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 450 && panel_luminance <= 550){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_500N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_500N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_500N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_500N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 550 && panel_luminance <= 650){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_600N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_600N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_600N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_600N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 650 && panel_luminance <= 750){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_700N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_700N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_700N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_700N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 750 && panel_luminance <= 850){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_800N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_800N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_800N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_800N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 850 && panel_luminance <= 950){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_900N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_900N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_900N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_900N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		} else if(panel_luminance > 950){
			memcpy(&HLG_EOTF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_EOTF_ByLuminance_TBL_1000N, sizeof(VIP_HLG_EOTF_ByLuminace_TBL));
			memcpy(&HLG_OETF_ByLuminance_TBL_cur, &ptr_HDR_bin_v4->HLG_OETF_ByLuminance_TBL_1000N, sizeof(VIP_HLG_OETF_ByLuminace_TBL));
			memcpy(&HDR10_EOTF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_EOTF_ByLuminace_TBL_1000N, sizeof(VIP_HDR10_EOTF_ByLuminace_TBL_v4));
			memcpy(&HDR10_OETF_ByLuminace_TBL_cur, &ptr_HDR_bin_v4->HDR10_OETF_ByLuminace_TBL_1000N, sizeof(VIP_HDR10_OETF_ByLuminace_TBL_v4));
			
		}
	}

	HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[30] = (Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Setting_Ctrl, 0, 0) == 0xFF)? HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[30]:Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Setting_Ctrl, 0, 0);
	HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[31] = (Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Curve_Ctrl, 0, 0) == 0xFF)? HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[31]:Scaler_APDEM_Arg_Access(DEM_ARG_HDR_OETF_Curve_Ctrl, 0, 0);
}

extern long_int long_multiply( unsigned int a, unsigned int b );
extern long_int long_divide( long_int a, unsigned int b );
unsigned short hdr_flag;
...
void EOTF_1024point(unsigned short maxlevel, VIP_HDR10_EOTF_ByLuminace_TBL_v4* EOTF_table, VIP_HDR10_OETF_ByLuminace_TBL_v4* OETF_table, unsigned int* EOTFyi_curve)
{
	...
	extern unsigned int HDR10_EOTF_calman_byMaxcll[maxcll_num][EOTF_size_1025p];
	extern unsigned int HDR10_maxcll_define[maxcll_num];
	unsigned short range_idx = 0;
	unsigned short norm_div, den_div;
	long_int tmp1, tmp2;

	// Get Maxcll interval
	//---idx0----|---idx1--|---idx2---|---idx3---|---idx4----|---idx5------
	//----- 422----- 600 ----- 1000 ----- 1400 ----- 40000 ----- 10000 ----- 
	for(i=0; i<maxcll_num; i++){
		if((i == maxcll_num-1) || (HDR10_maxcll_define[i+1] == 0)){
			range_idx = i;
			break;
		} else if(maxlevel < (HDR10_maxcll_define[i]+HDR10_maxcll_define[i+1])/2){
			range_idx = i;
			break;
		}
	}
	...
	// Get calman_flag, dynamic_hdr_flag 
	if(OETF_table->OETF_setting[31] == 0){
		hdr_flag = HDR_calman;
	} else if(OETF_table->OETF_setting[31] == 1){
		if(maxlevel == 0){
			hdr_flag = HDR_withoutmetadata;
		} else{
			hdr_flag = HDR_normal;
		}
	} else if(OETF_table->OETF_setting[31] == 2){
		hdr_flag = HDR_dummy;
	}
	...
	{
		if(hdr_flag==HDR_calman){
			if(Scaler_color_get_HDRver() == 3){
				norm_div = EOTF_table->EOTF_calman[range_idx][0];
				den_div  = EOTF_table->EOTF_calman[range_idx][1];
				tmp1 = long_multiply(HDR10_EOTF_2084[i], norm_div);
				tmp2 = long_divide(tmp1, den_div);
			} else if(Scaler_color_get_HDRver() == 4){
				norm_div = HDR10_EOTF_calman_byMaxcll[range_idx][0];
				den_div  = HDR10_EOTF_calman_byMaxcll[range_idx][1];
				tmp1 = long_multiply(HDR10_EOTF_calman_byMaxcll[range_idx][i+2], norm_div);
				tmp2 = long_divide(tmp1, den_div);
			}
			
		} else if(hdr_flag==HDR_normal){
			norm_div = EOTF_table->EOTF_normal[range_idx][0];
			den_div  = EOTF_table->EOTF_normal[range_idx][1];
			tmp1 = long_multiply(EOTF_table->EOTF_normal[range_idx][i+2], norm_div);
			tmp2 = long_divide(tmp1, den_div);
			
		} else if(hdr_flag==HDR_dummy){
			norm_div = EOTF_table->EOTF_dummy[range_idx][0];
			den_div  = EOTF_table->EOTF_dummy[range_idx][1];
			tmp1 = long_multiply(EOTF_table->EOTF_dummy[range_idx][i+2], norm_div);
			tmp2 = long_divide(tmp1, den_div);
			
		} else if(hdr_flag==HDR_withoutmetadata){
			norm_div = EOTF_table->EOTF_WithoutMetadata[range_idx][0];
			den_div  = EOTF_table->EOTF_WithoutMetadata[range_idx][1];
			tmp1 = long_multiply(EOTF_table->EOTF_WithoutMetadata[range_idx][i+2], norm_div);
			tmp2 = long_divide(tmp1, den_div);
		}
	...
	rtd_pr_vpq_info("EOTF get, hdr_flag=%d, dynamic_hdr_flag=%d, HDR_version=%d, norm_div=%d, den_div=%d, MaxLevel=%d, range_idx=%d\n", hdr_flag, dynamic_hdr_flag, Scaler_color_get_HDRver(), norm_div, den_div, maxlevel, range_idx);
}
...
char Scaler_color_Get_HDR10_EOTF_ByMaxCLL_ByPanelLuminance(unsigned short forceIndex)
{
	...
	if (fwif_color_get_force_run_i3ddma_enable(SLR_MAIN_DISPLAY) && (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC) &&
		...
		input_timming_info 	= drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_MODE_CURR);
	} else {
		input_src_type = Scaler_InputSrcGetMainChType();
		input_src_form = Scaler_InputSrcGetMainChFrom();
	...
	/* Maxcll check */
	if (MaxCLL == 0)
		MaxLevel = (HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5]==0)? 0: HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5];
	else
		MaxLevel = MaxCLL;
	/* force select oetf*/
	if (forceIndex != 0)
...
void OETF_inner_64point(int row, int col, unsigned short maxlevel, unsigned int OETFyi_curve[][1028], unsigned short OETF_Lmix[][16], unsigned int* OETFyi_inner, unsigned int* shift_bit, unsigned short* _3dlut){
	...
	// Shift BIT
	*shift_bit = OETFyi_curve[range_idx][0];

	// 3DLUT
	*_3dlut = OETFyi_curve[range_idx][2];
	/*
	...
	*/
	...
	// OETF curve
	for(i=3; i<col; i++){
		OETFyi_inner[i-3] = OETFyi_curve[range_idx][i];
		//rtd_printk(KERN_EMERG, TAG_NAME, "OETFyi_inner[%d]=%d\n", j-3, OETFyi_curve[range_idx][j]);
	}

	/* maxRGB-OETF_Lmix*/
	drvif_color_set_HDR_OETF_Lmix(1, OETF_Lmix[range_idx]);
}
...
char Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance(unsigned short forceIndex)
{
	...
	unsigned int *OETFyi_inner_int = NULL;
	unsigned short *OETFyi_inner_short = NULL;
	...
	if (OETF_MaxCLL == 0)
		OETF_MaxLevel = (HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5]==0)? 0: HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5];
	else
	...
	/* malloc */
	OETFyi_inner_int = (unsigned int*)dvr_malloc(1025 * sizeof(unsigned int));
	if(OETFyi_inner_int == NULL){
		rtd_pr_vpq_err("Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance, OETFyi_inner_int malloc fail\n");
		return -1;
	}

	OETFyi_inner_short = (unsigned short*)dvr_malloc(1025 * sizeof(unsigned short));
	if(OETFyi_inner_short==NULL){
		dvr_free(OETFyi_inner_int);
		rtd_pr_vpq_err("Scaler_color_Get_HDR10_OETF_ByMaxCLL_ByPanelLuminance, OETFyi_inner_int malloc fail\n");
		return -1;
	}
	...
	/* OETF curve */
	if(hdr_flag == HDR_calman){
		OETF_inner_64point(maxcll_num, OETF_size_1025p, OETF_MaxLevel, HDR10_OETF_ByLuminace_TBL_cur.OETF_calman, HDR10_OETF_ByLuminace_TBL_cur.OETF_calamn_Lmix, OETFyi_inner_int, &OETF_shift_bit, &OETF_3dlut);
		// change type(int to short)
		for(i=0;i<OETF_size;i++){
			OETFyi_inner_short[i] = (unsigned short)(OETFyi_inner_int[i]);
		}
		
	} else if(hdr_flag == HDR_withoutmetadata){
		OETF_inner_64point(maxcll_num, OETF_size_1025p, OETF_MaxLevel, HDR10_OETF_ByLuminace_TBL_cur.OETF_WithoutMetadata, HDR10_OETF_ByLuminace_TBL_cur.OETF_WithoutMetadata_Lmix, OETFyi_inner_int, &OETF_shift_bit, &OETF_3dlut);
		// change type(int to short)
		for(i=0;i<OETF_size;i++){
			OETFyi_inner_short[i] = (unsigned short)(OETFyi_inner_int[i]);
		}
		...
		OETF_inner_64point(maxcll_num, OETF_size_1025p, OETF_MaxLevel, HDR10_OETF_ByLuminace_TBL_cur.OETF_normal, HDR10_OETF_ByLuminace_TBL_cur.OETF_normal_Lmix, OETFyi_inner_int, &OETF_shift_bit, &OETF_3dlut);
		// change type(int to short)
		for(i=0;i<OETF_size;i++){
			OETFyi_inner_short[i] = (unsigned short)(OETFyi_inner_int[i]);
		}
		...
		OETF_inner_64point(maxcll_num, OETF_size_1025p, OETF_MaxLevel, HDR10_OETF_ByLuminace_TBL_cur.OETF_dummy, HDR10_OETF_ByLuminace_TBL_cur.OETF_dummy_Lmix, OETFyi_inner_int, &OETF_shift_bit, &OETF_3dlut);
		// change type(int to short)
		for(i=0;i<OETF_size;i++){
			OETFyi_inner_short[i] = (unsigned short)(OETFyi_inner_int[i]);
		}
	...
	rtd_pr_vpq_info("OETF get, input_src_type=%d, hdr_flag=%d, panel_Luminance=%d, MaxCLL=%d, OETF_MaxLevel=%d, MaxFALL=%d, MaxDML=%d\n", input_src_type, hdr_flag, panel_luminance, OETF_MaxCLL, OETF_MaxLevel, OETF_MaxFALL, OETF_MaxDML);
	...
	// 3dlut set in fwif_color_set_DM_HDR_3dLUT_CInv()
	if(OETF_shift_bit<=4){
		drvif_color_set_BBC_shift_bit(OETF_shift_bit);
		fwif_color_set_DM_HDR_3dLUT_CInv(OETF_3dlut);
	}else{
	...
	*/
	...
	fwif_color_set_DM2_OETF_TV006(OETFyi_inner_short, 0);
	HDR10_OETF_ByMaxCLL_ByPanelLuminance_for_task_run = 1;

	/* free */
	if(OETFyi_inner_int)
		dvr_free((OETFyi_inner_int));
	if(OETFyi_inner_short)
		dvr_free(OETFyi_inner_short);
...
void Scaler_color_Get_HDR10_EOTF_OETF_ByMaxCLL_ByPanelLuminance_for_task(void)
{
	...
	static int cur_KL = 0;
	static unsigned int cur_gain_rate=256;
	static unsigned int pre_probability[256] = {0};
	unsigned int prot_hdr3dlut;
	HDMI_DRM_T drm_info;
	SLR_VOINFO* pVOInfo = NULL;
	unsigned char input_src_type;
	unsigned char input_src_form;
	...
	if (HDR_flag == HAL_VPQ_HDR_MODE_HDR10) {
		if(is_QS_pq_enable()==1)
			cur_gain_rate = 256;	// start from QS, set from linear
		/*=========================CHECK MAXCLL=========================*/
		if(Scaler_DispGetInputInfoByDisp_res != _MODE_STATE_ACTIVE){
			if((HDR_flag == HAL_VPQ_HDR_MODE_HDR10) && (VGIP_VGIP_CHN1_CTRL_res)){
				...
				if (Dynamic_MaxCLL == 0){
					...
					Dynamic_MaxLevel = (HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5]==0)? 0: HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[5];
				} else{
			...
			if (dynamic_hdr_flag){
				...
				if((HDR_flag == HAL_VPQ_HDR_MODE_HDR10) && (VGIP_VGIP_CHN1_CTRL_res)){
					...
					if(hist_map2_nit[HDR_Y_hist_mean_index]<=x0)
						...
						gain_rate = y1+((int)(x1-hist_map2_nit[HDR_Y_hist_mean_index])*(int)(y0-y1))/(int)(x1-x0);
					else if(hist_map2_nit[HDR_Y_hist_mean_index]<x2)
						gain_rate = y2+((int)(x2-hist_map2_nit[HDR_Y_hist_mean_index])*(int)(y1-y2))/(int)(x2-x1);
					else if(hist_map2_nit[HDR_Y_hist_mean_index]<x3)
						gain_rate = y3+((int)(x3-hist_map2_nit[HDR_Y_hist_mean_index])*(int)(y2-y3))/(int)(x3-x2);
					else if(hist_map2_nit[HDR_Y_hist_mean_index]<x4)
						gain_rate = y4+((int)(x4-hist_map2_nit[HDR_Y_hist_mean_index])*(int)(y3-y4))/(int)(x4-x3);
					else
						gain_rate = y4;
				...
				if((oetf_lut_enable == 1 && b0105_or_oetf == 1) || HDR10_OETF_ByMaxCLL_ByPanelLuminance_for_task_run){
					...
					if(OETF_shift_bit<=4){
						drvif_color_set_BBC_shift_bit(OETF_shift_bit);
						fwif_color_set_DM_HDR_3dLUT_CInv(OETF_3dlut * cur_gain_rate / 256);
					}else{
						drvif_color_set_BBC_shift_bit(4);
						fwif_color_set_DM_HDR_3dLUT_CInv(OETF_3dlut * cur_gain_rate / 256);
					}
				...
				HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[7] = 0;
				HDR10_OETF_ByLuminace_TBL_cur.OETF_setting[8] = 0;
			}
		}
	}
	else if (HDR_flag == HAL_VPQ_HDR_MODE_HLG) 
	{
		HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0] = ((HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0] & 0xFFFFFFFF) != 0) ? HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0]: 0;
		HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[1] = ((HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[1] & 0xFFFFFFFF) == 0) ? 0x00004000: HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[1];

		if(HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0] == 0){ //standrd
			fwif_color_set_DM_HDR_3dLUT_CInv(0x00004000);
			drvif_color_set_BBC_shift_bit(1);
		} else{ //normal
			prot_hdr3dlut = min(max((int)HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[1] + HDR_3DLUT_offset, 0), 0x000FFFFF);
			fwif_color_set_DM_HDR_3dLUT_CInv(prot_hdr3dlut);
			drvif_color_set_BBC_shift_bit(1);
		}
	}
...
char Scaler_color_Get_HLG_EOTF_ByMaxCLL_ByPanelLuminance(void){
	...
	if(HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0] == 0){
		/* HLG standard */
		fwif_color_set_DM2_EOTF_TV006(HLG_EOTF_LUT_R, 0);
		rtd_pr_vpq_info("Scaler_color_Get_HLG_EOTF_ByMaxCLL_ByPanelLuminance, standrd.");
	} else{
		/* HLG normal */
		fwif_color_set_DM2_EOTF_TV006(HLG_EOTF_ByLuminance_TBL_cur.EOTF_normal, 0);
		rtd_pr_vpq_info("Scaler_color_Get_HLG_EOTF_ByMaxCLL_ByPanelLuminance, normal.");
	}
	return 0; 
}
...
char Scaler_color_Get_HLG_OETF_ByMaxCLL_ByPanelLuminance(void){
	...
	OETFyi_inner_short = (unsigned short*)dvr_malloc(1025 * sizeof(unsigned short));
	if(OETFyi_inner_short == NULL){
		rtd_pr_vpq_err("Scaler_color_Get_HLG_OETF_ByMaxCLL_ByPanelLuminance, malloc fail\n");
		return -1;
	}
	...
	if(HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0] == 0){
		...
		rtd_pr_vpq_info("Scaler_color_Get_HLG_OETF_ByMaxCLL_ByPanelLuminance, standrd.");
	
	} else{
		/* HLG normal */
		for(i=0;i<OETF_size;i++){
			OETFyi_inner_short[i] = (unsigned short)(HLG_OETF_ByLuminance_TBL_cur.OETF_normal[i]);
		}
		fwif_color_set_DM2_OETF_TV006(OETFyi_inner_short, 0);
		rtd_pr_vpq_info("Scaler_color_Get_HLG_OETF_ByMaxCLL_ByPanelLuminance, normal.");
	}
...
{
	_system_setting_info *VIP_system_info_structure_table = (_system_setting_info *)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
	rtd_pr_vpq_info("setHDR_sub_Type=%d,\n", HDR_Type);
	if(VIP_system_info_structure_table == NULL){
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s:%d] error here!! system_info=NULL! \n",__FILE__, __LINE__);
		return;
	}
	...
	if (HDR_Type == HDR_DM_DOLBY) {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_SDR;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 1;
	} else if (HDR_Type == HDR_DM_HDR10) {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_HDR10;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;

	} else if (HDR_Type == HDR_DM_HLG) {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_HLG;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;

	} else if (HDR_Type == HDR_DM_TECHNICOLOR) {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_TECHNICOLOR;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;

	} else if (HDR_Type == HDR_DM_ST2094) {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_ST2094;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;

	} else if (HDR_Type == HDR_DM_SDR_MAX_RGB) {
		VIP_system_info_structure_table ->HDR_flag_sub = HAL_VPQ_HDR_MODE_SDR_MAX_RGB;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;

	}
	else if (HDR_Type == HDR_DM_VIVID) {
		VIP_system_info_structure_table ->HDR_flag_sub = HAL_VPQ_HDR_MODE_HDR10_VIVID;
		VIP_system_info_structure_table->DolbyHDR_flag_sub = 0;
	}
	else {
		VIP_system_info_structure_table->HDR_flag_sub = HAL_VPQ_HDR_MODE_SDR;
...
void Scaler_color_copy_HDR_table_from_AP(SLR_HDR_TABLE* ptr_HDR_bin_temp)
{
	extern unsigned char hdr_table[HDR_TABLE_NUM][HDR_ITEM_MAX];
	...
	memcpy(hdr_table, ptr_HDR_bin_temp->hdr_table, sizeof(UINT8)* HDR_TABLE_NUM * HDR_ITEM_MAX);
	//memcpy(hdr_RGB2OPT, ptr_HDR_bin_temp->hdr_RGB2OPT, sizeof(UINT32)* HDR_RGB2OPT_TABLE_NUM * 9);
	memcpy(EOTF_LUT_R, ptr_HDR_bin_temp->EOTF_LUT_R, sizeof(UINT32)* 1025);
	memcpy(OETF_LUT_R, ptr_HDR_bin_temp->OETF_LUT_R, sizeof(UINT16)* 1025);
	memcpy(HLG_EOTF_LUT_R, ptr_HDR_bin_temp->HLG_EOTF_LUT_R, sizeof(HLG_EOTF_LUT_R));
	memcpy(HLG_OETF_LUT_R, ptr_HDR_bin_temp->HLG_OETF_LUT_R, sizeof(HLG_OETF_LUT_R));
	memcpy(HDR10_EOTF_2084, ptr_HDR_bin_temp->HDR10_EOTF_2084, sizeof(HDR10_EOTF_2084));
	memcpy(HDR10_maxcll_define, ptr_HDR_bin_temp->HDR10_maxcll_define, sizeof(HDR10_maxcll_define));
	if (panel_luminance_updated == 0)
		memcpy(&panel_luminance, &(ptr_HDR_bin_temp->panel_luminance), sizeof(UINT16));
	memcpy(&TCHDR_Mode_TBL, &(ptr_HDR_bin_temp->TCHDR_Mode_TBL), sizeof(VIP_TCHDR_Mode_TBL));
	memcpy(TCHDR_Table, ptr_HDR_bin_temp->TCHDR_Table, sizeof(DRV_TCHDR_Table) * TC_Table_NUM);
	memcpy(&TCHDR_COEF_I_LUT_TBL, &(ptr_HDR_bin_temp->TCHDR_COEF_I_LUT_TBL), sizeof(DRV_TCHDR_COEF_I_lut));
	memcpy(&TCHDR_COEF_S_LUT_TBL, &(ptr_HDR_bin_temp->TCHDR_COEF_S_LUT_TBL), sizeof(DRV_TCHDR_COEF_S_lut));
	memcpy(TCHDR_v130_D_Table_settings, ptr_HDR_bin_temp->TCHDR_v130_D_Table_settings, sizeof(DRV_TCHDR_v130_D_Table_settings)* NUM_D_LUT_DEFAULT);

	/* According to panel luminance to get current EOTF and OETF table */
	Scaler_color_Set_EOTF_OETF_Table();
}

void Scaler_color_copy_HDR_table_from_AP_v4(SLR_HDR_TABLE_v4* ptr_HDR_bin_temp)
{
	extern unsigned char hdr_table[HDR_TABLE_NUM][HDR_ITEM_MAX];
	extern unsigned int hdr_RGB2OPT[HDR_RGB2OPT_TABLE_NUM][9];
	extern unsigned int EOTF_LUT_R[1025];
	...
	memcpy(HLG_OETF_LUT_R, ptr_HDR_bin_temp->HLG_OETF_LUT_R, sizeof(HLG_OETF_LUT_R));
	memcpy(HDR10_EOTF_2084, ptr_HDR_bin_temp->HDR10_EOTF_2084, sizeof(HDR10_EOTF_2084));
	memcpy(HDR10_EOTF_calman_byMaxcll, ptr_HDR_bin_temp->HDR10_EOTF_calman_byMaxcll, sizeof(HDR10_EOTF_calman_byMaxcll));
	memcpy(HDR10_maxcll_define, ptr_HDR_bin_temp->HDR10_maxcll_define, sizeof(HDR10_maxcll_define));
	if (panel_luminance_updated == 0)
		memcpy(&panel_luminance, &(ptr_HDR_bin_temp->panel_luminance), sizeof(UINT16));
	memcpy(&TCHDR_Mode_TBL, &(ptr_HDR_bin_temp->TCHDR_Mode_TBL), sizeof(VIP_TCHDR_Mode_TBL));
	...
	/* According to panel luminance, get current EOTF and OETF table */
	Scaler_color_Set_EOTF_OETF_Table();
}
...
void Scaler_color_copy_HDR_table_oetf_setting_ctrl_from_AP(unsigned short oetf_setting_ctrl_from_ini)
{
	...
	Scaler_color_HDRIP_AutoRun(SLR_MAIN_DISPLAY, Scaler_color_HAL_VPQ_HDR_Type_to_HDR_DM_Type(HDR_flag));
	up(&HDR_Setting_Semaphore);
...
void scaler_pq_sub_mode_set(unsigned char sub_mode)
{
	...
	temp_width = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)SLR_SUB_DISPLAY, SLR_INPUT_IPH_ACT_WID);
	temp_height = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)SLR_SUB_DISPLAY,SLR_INPUT_IPV_ACT_LEN);
	rtd_pr_vpq_info("scaler_pq_sub_mode_set, sub_mode=%d, temp_width = %d, temp_height=%d\n", sub_mode, temp_width,temp_height);
	//ve-dither request don't apply dither solution, return it
	//fwif_color_ve_dither_value_reset(0x3ff);//avoid ve-dither garbage, first 10 frame disable ve-dither

	//fwif_color_hist_init(SLR_SUB_DISPLAY, temp_width, temp_height,Mode_Orig_32Bin);

	if(sub_mode==1){ //PBP mode
		//fwif_color_UV_Delay_Enable(0,0); //use 2nd pip have to disable uv delay.

		//drvif_color_icm_set_bypass_onoff(SLR_SUB_DISPLAY, 0); //
		//drvif_color_set_sub_Icm_Enable(1);
		//drvif_color_dcc_sub_mode_sel(1);
		//drvif_color_set_sub_yuv2rgb_funciton_Enable(1);
		//drvif_set_sub_sharpness_En(1);//ve-dither  request don't apply dither solution, return it
		drvif_color_set_overlay_control(1);
		//drvif_color_set_dtm_sub_en(1);

	}else { //pip or others
		//drvif_color_icm_set_bypass_onoff(SLR_SUB_DISPLAY, 1);
		//drvif_color_set_sub_Icm_Enable(0);
		//drvif_color_dcc_sub_mode_sel(0);
		//drvif_color_set_sub_yuv2rgb_funciton_Enable(0);
		drvif_color_set_overlay_control(0);
		//drvif_set_sub_sharpness_En(0);//ve-dither  request don't apply dither solution, return it
		//drvif_color_set_dtm_sub_en(0);
	}
}
...
void Scaler_recheck_sharpness_table(void)
{
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		if (HSR_mode && pVipCoefExt4Array) {
			if (Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY) != pVipCoefExt4Array[VIP_QUALITY_FUNCTION_HSR_SharpTable]) {
				Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, pVipCoefExt4Array[VIP_QUALITY_FUNCTION_HSR_SharpTable]);
				Scaler_SetSharpness(Scaler_GetSharpness());
			}
		} else {
			if (Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY) != pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable]) {
				Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable]);
				Scaler_SetSharpness(Scaler_GetSharpness());
			}
		}
...
{
	// only use table 0 for DMA setting
	if (mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.od_en == 1)
		return fwif_color_od_init(mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.Y_mode_en); // 1: y mode, 0:RGB mode
	else
		return 0;
}
...
{
#if 0	// no change by xml, bit is set on ac/dc on
...
char Scaler_Set_DI_SNR_Borrow_Sram_Mode(VIP_SNR_DI_Sram_Borrow_Mode mode)
{
	...
	unsigned char PC_mode_check = Scaler_VPQ_check_PC_RGB444(SLR_MAIN_DISPLAY);
	...
	rtd_pr_vpq_info("Set_VE_Borrow_Sram_Mode, set mode=%d, mode from %d to %d, PC_mode_check=%d, ret_val=%d,\n",
		mode, mode_ori, rpc_vipSysInfo->SNR_DI_Sram_Borrow_Mode, PC_mode_check, ret_val);
...
void Scaler_color_Dither_Table_Ctrl(void)
{
	...
	unsigned char nIdex=0,nDITHER_ENABLE=0,nDITHER_MAP_BIT=0,nDITHER_LUT=0,dual_panel=0;
	unsigned int nWidth=0,nLength=0;
	...
	for(nIdex = 0;nIdex<VIP_DITHER_IP_MAX;nIdex++ )
	{
		...
		rtd_pr_vpq_info("[%d:%d:%d] Dither_Table[%d] \n",nDITHER_ENABLE, nDITHER_MAP_BIT,nDITHER_LUT, nIdex);
		switch(nIdex) {
		...
		switch(nIdex) {
            ...
            case VIP_DITHER_IP_PANEL:
				...
				if (fwif_color_set_Panel_Dither_By_ini() == 1)
					rtd_pr_vpq_info("Dither_Table, fwif_color_set_Panel_Dither_By_ini\n");
				else if (Get_DISPLAY_PANEL_MPLUS_RGBW() == TRUE || Get_DISPLAY_PANEL_BOW_RGBW() == TRUE)
					fwif_color_set_Panel_Dither(TRUE,VIP_PANEL_BIT8,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15,dual_panel,nWidth,nLength);
					...
					fwif_color_set_Panel_Dither(nDITHER_ENABLE,VIP_PANEL_BIT8,nDITHER_LUT,dual_panel,nWidth,nLength);
				else
					fwif_color_set_Panel_Dither(nDITHER_ENABLE,VIP_PANEL_BIT10,nDITHER_LUT,dual_panel,nWidth,nLength);
				break;
		}
	}
	rtd_pr_vpq_info("Dither_Table,MPLUS_RGBW=%d,BOW_RGBW=%d,PANEL_TYPE=%d,ld_cmp=%d,\n",
		Get_DISPLAY_PANEL_MPLUS_RGBW(), Get_DISPLAY_PANEL_BOW_RGBW(), Get_DISPLAY_PANEL_TYPE(),
		BLU_LD_Global_Ctrl0_get_ld_comp_en(IoReg_Read32(BLU_LD_Global_Ctrl0_reg)));
}
...
unsigned short g_reset_sharp_idx_flag = 0xffff;
void Scaler_Reset_Sharpness_Table(void)
{
	unsigned char idx;
	if (g_reset_sharp_idx_flag < Sharp_table_num)
	{
		idx = g_reset_sharp_idx_flag;
		g_reset_sharp_idx_flag = 0xffff;

		Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, idx);
		g_reset_sharp_level_flag = 1;
	}
}

void Scaler_recheck_QualityCoef(unsigned source)
{
	if (m_cbFunc_getQualityCoef != NULL) {
		unsigned char *pVipCoefArray = (unsigned char *)m_cbFunc_getQualityCoef((VIP_SOURCE_TIMING)source);
		if (pVipCoefArray) {
			Scaler_SetDLti(pVipCoefArray[VIP_QUALITY_FUNCTION_DLTi]);
			Scaler_SetDCti(pVipCoefArray[VIP_QUALITY_FUNCTION_DCTi]);
			Scaler_SetIDCti(pVipCoefArray[VIP_QUALITY_FUNCTION_I_DCTi]);
		...
		//rtd_pr_vpq_info("PQ table m_cbFunc_getQualityCoef read fail !!!\n");
	}
}

void Scaler_recheck_QualityExtend(unsigned source)
{
	if (m_cbFunc_getQualityExtend != NULL) {
		unsigned char *pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend((VIP_SOURCE_TIMING)source);
		if (pVipCoefArray) {
			Scaler_SetCDSTable(pVipCoefArray[VIP_QUALITY_FUNCTION_CDS_Table]);
			Scaler_Set_I_De_Contour(pVipCoefArray[VIP_QUALITY_FUNCTION_I_De_Contour]);
		}
	} else {
		//rtd_pr_vpq_info("PQ table m_cbFunc_getQualityExtend read fail !!!\n");
	}
}

void Scaler_recheck_QualityExtend2(unsigned source)
{
	if (m_cbFunc_getQualityExtend2 != NULL) {
		unsigned char *pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend2((VIP_SOURCE_TIMING)source);
		if (pVipCoefArray) {
			Scaler_setiESM(pVipCoefArray[VIP_QUALITY_FUNCTION_iESM_table]);
		}
	} else {
		//rtd_pr_vpq_info("PQ table m_cbFunc_getQualityExtend2 read fail !!!\n");
	}
}

void Scaler_recheck_QualityExtend3(unsigned source)
{
	if (m_cbFunc_getQualityExtend3 != NULL) {
		unsigned char *pVipCoefArray = (unsigned char *)m_cbFunc_getQualityExtend3((VIP_SOURCE_TIMING)source);
		if (pVipCoefArray) {
			unsigned int nr_trigger=0;

			//Sharpness / NR recheck
			if (Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY) != pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable]) {
				g_reset_sharp_idx_flag = pVipCoefArray[VIP_QUALITY_FUNCTION_SharpTable];
			}
			...
			if (nr_trigger) {
				Scaler_SetDNR(Scaler_GetDNR());
				Scaler_SetMPEGNR(Scaler_GetMPEGNR(), false);
			}
		}
	} else {
		//rtd_pr_vpq_info("PQ table m_cbFunc_getQualityExtend3 read fail !!!\n");
		return;
	}
}
...
{
	unsigned char source = fwif_vip_source_check(3, NOT_BY_DISPLAY);/* get vip source by input src and timing*/

	if (source >= VIP_QUALITY_TOTAL_SOURCE_NUM)
		return;

	Scaler_recheck_QualityCoef(source);
	Scaler_recheck_QualityExtend(source);
	Scaler_recheck_QualityExtend2(source);
	Scaler_recheck_QualityExtend3(source);
}
...
void Scaler_resolution_change_check(void)
{
	if (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE ||
		!VGIP_VGIP_CHN1_CTRL_get_ch1_vact_end_ie(IoReg_Read32(VGIP_VGIP_CHN1_CTRL_reg)) ||
		Get_DisplayMode_Src(SLR_MAIN_DISPLAY) != VSC_INPUTSRC_VDEC)
	{
		g_reset_sharp_level_flag = 0;
		return;
	}

	if (res_check_last_input_h != Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN))
		Scaler_recheck_sharp_nr_table();
	else if (g_reset_sharp_level_flag)
	{
		g_reset_sharp_level_flag = 0;
		Scaler_SetSharpness(Scaler_GetSharpness());
	}
...
void Scaler_fwif_color_set_pip_colorkey(CONBRI_VIDEO_COLORKEY_PARA video_color)
{
    ...
    unsigned int color_range_r_max=0;
    unsigned int color_range_g_max=0;
    unsigned int color_range_b_max=0;
    unsigned int color_range_r_min=0;
    unsigned int color_range_g_min=0;
    unsigned int color_range_b_min=0;

    unsigned int color_replace_r=0;
    unsigned int color_replace_g=0;
    unsigned int color_replace_b=0;

    source  = video_color.colorkey_source;
    mode    = video_color.replace_mode;

    color_range_r_max   =   video_color.color_key_y_max;
    color_range_g_max   =   video_color.color_key_u_max;
    color_range_b_max   =   video_color.color_key_v_max;
    color_range_r_min   =   video_color.color_key_y_min;
    color_range_g_min   =   video_color.color_key_u_min;
    color_range_b_min   =   video_color.color_key_v_min;

    color_replace_r   =   video_color.color_replace_y;
    color_replace_g   =   video_color.color_replace_u;
    color_replace_b   =   video_color.color_replace_v;

    if(mode==REPLACE_DATA){
        fwif_color_set_pip_colorkey_replace(source,color_range_r_max,color_range_g_max,color_range_b_max,color_range_r_min,color_range_g_min,color_range_b_min,color_replace_r,color_replace_g,color_replace_b);
    }else{
        fwif_color_set_pip_colorkey_transparency(source,color_range_r_max,color_range_g_max,color_range_b_max,color_range_r_min,color_range_g_min,color_range_b_min);
    }
}
...
void Scaler_Set_PQ_UVC_DataPath(UVC_CAPTURE_FORMAT_TYPE  UVC_OutputFormat,unsigned char enable,unsigned char colorspce,unsigned int inSize_nLength , unsigned int inSize_nWidth,unsigned int outSize_nLength , unsigned int outSize_nWidth)
{
    ...
    unsigned char yuv_en=0;
    unsigned int con_r=512,con_g=512,con_b=512;
    int bri_r=0,bri_g=0,bri_b=0;
    extern DRV_FIR_Coef FIR_Coef_Ctrl[DRV_FIR_Coef_4Tap_NUM];
    extern unsigned int tColorSpace_Convert_3x3_coeff[7][15];
    extern unsigned int tSatHue_SatBySat[33];

    inSize.nLength  = inSize_nLength;
	inSize.nWidth   = inSize_nWidth;
	outSize.nLength = outSize_nLength;
	outSize.nWidth  = outSize_nWidth;

    if(colorspce==COLOR_RGB){
        yuv_en=0;
    }else{
        yuv_en=1;
    }

    rtd_pr_vpq_info("[%s][%d] UVC_OutputFormat=%d,enable=%d,colorspce=%d,in(%d,%d),out(%d,%d)\n",__FUNCTION__,__LINE__,UVC_OutputFormat,enable,colorspce,inSize_nLength,inSize_nWidth,outSize_nLength,outSize_nWidth);
    rtd_pr_vpq_info("[%s][%d] colorspce=%d,COLOR_RGB=%d,yuv_en=%d\n",__FUNCTION__,__LINE__,colorspce,COLOR_RGB,yuv_en);


    //YUV2RGB 3*3
    fwif_color_set_uvc_yuv2rgb(yuv_en,&tColorSpace_Convert_3x3_coeff[1][0],4095,0,4095,0);

    //RGB ConBri
    fwif_color_set_uvc_conbri(1, con_r,con_g,con_b,bri_r,bri_g,bri_b);

    //RGB SatHue
    fwif_color_set_uvc_satbysat_curve(enable,enable,0,0,0,0,tSatHue_SatBySat);
    
    //RGB2YUV 3*3
    switch(UVC_OutputFormat)
	{
		case _NV12: 
        case _YUYV:
        case _I420: 
            //yuv 709 limit
			fwif_color_set_uvc_rgb2yuv(1,&tColorSpace_Convert_3x3_coeff[6][0],4095,0,4095,0);
			break;
            
		case _RGB24:
        case _RGB32:
            //XRGB RGB Full
			fwif_color_set_uvc_rgb2yuv(0,&tColorSpace_Convert_3x3_coeff[1][0],4095,0,4095,0);
			break;
            
        case _MotionJPEG:
            //yuv 601 Full
			fwif_color_set_uvc_rgb2yuv(1,&tColorSpace_Convert_3x3_coeff[3][0],4095,0,4095,0);
			break;
		default:
			break;
	}    
    ...
    //Dither
    fwif_color_set_Dither(VIP_DITHER_IP_UVC,enable,VIP_DITHER_MAINTABLE_DEFAULT_12to10);

    //uzu uzd
    drvif_uvc_ultrazoom_setscaleup(SLR_MAIN_DISPLAY,&inSize, &outSize, 0);
    drvif_uvc_scaler_set_subscaleup_hcoef(SLR_MAIN_DISPLAY, FIR_Coef_Ctrl+10);
    drvif_uvc_scaler_set_subscaleup_vcoef(SLR_MAIN_DISPLAY, FIR_Coef_Ctrl+10);
}
