...
/***************************Enginner Menu Start ********************************************/
	...
	case TOOLACCESS_Contrast:   	/*  0*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		buf[0] = Scaler_GetContrast(rtice_param.mode0);
		*num_type = VIP_UINT8;
		break;
	...
	case TOOLACCESS_Brightness:   	/*  1*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		buf[0] = Scaler_GetBrightness(rtice_param.mode0);
		*num_type = VIP_UINT8;
		break;
	...
	case TOOLACCESS_Saturation:   	/*  2*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		buf[0] = Scaler_GetSaturation(rtice_param.mode0);
		*num_type = VIP_UINT8;
		break;
	...
	case TOOLACCESS_Hue:   			/*  3*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		buf[0] = Scaler_GetHue(rtice_param.mode0);
		*num_type = VIP_UINT8;
		break;
...
*/
	...
	case TOOLACCESS_Function_Extend3_Sharpness_Table:
		...
		if (rtice_param.mode0 == 0)
			buf[0] = Scaler_GetSharpnessTable(rtice_param.mode1);
		else
			buf[0] = 19;
...
*/
	...
	case TOOLACCESS_SUB_HDR_EOTF_TBL:
	case TOOLACCESS_SUB_HDR_OETF_TBL:
	case TOOLACCESS_SUB_HDR_Y2GAIN_TBL:
    case TOOLACCESS_SUB_HDR_SUBPATH:
		ret = rtice_SetGet_HDR_SUB(rtice_param, num_type, buf, 0);
		if (ret < 0)
			return -ID_SIZE_ERR;
...
*/
	...
	case TOOLACCESS_VIP_SLD:/*k7, SLD*/
		...
		pwUINT32Temp = (int *)vip_malloc(50 * sizeof(int));
		if (pwUINT32Temp == NULL)
			return -ID_SIZE_ERR;

		pwUINT32Temp[0] = cmd_dY_preCur_mouseCon;
		pwUINT32Temp[1] = cmd_LC_period;
		pwUINT32Temp[2] = cmd_thl_counter_UR_con;
		pwUINT32Temp[3] = cmd_counter_frame_highY_blk;
		pwUINT32Temp[4] = cmd_lock_fw_gain_frame;
		pwUINT32Temp[5] = cmd_pixel_gain_step;
		pwUINT32Temp[6] = cmd_thl_counter_blk_pre_cur_same_mouseCon;
		pwUINT32Temp[7] = cmd_thl_error_time_box_logo_case;
		pwUINT32Temp[8] = cmd_thl_counter_box_logo_case;
		pwUINT32Temp[9] = cmd_thl_error_time_NEWS_case;
		pwUINT32Temp[10] = cmd_thl_counter_NEWS_case;
		pwUINT32Temp[11] = cmd_thl_error_time_noise_case;
		pwUINT32Temp[12] = cmd_thl_counter_noise_case;
		pwUINT32Temp[13] = cmd_thl_dC;
		pwUINT32Temp[14] = cmd_thl_dUDL;
		pwUINT32Temp[15] = cmd_rgb_box_leave_idx_delete_step;
		pwUINT32Temp[16] = cmd_box_logo_pixel_gain;
		pwUINT32Temp[17] = cmd_box_logo_maxGain_time;
		pwUINT32Temp[18] = cmd_box_lock_frame;
	...
	case TOOLACCESS_VIP_LEA:
		...
		pwUINT32Temp = (int *)vip_malloc(26 * sizeof( int));

		if (pwUINT32Temp == NULL) {
			rtd_pr_vpq_emerg("TOOLACCESS_VIP_LEA, pwUINT32Temp = NULL\n");
			break;
		} 
		
		pwUINT32Temp[0]	= LSC_by_memc_logo_en;
		pwUINT32Temp[1]	= memc_logo_to_demura_wait_time;
		pwUINT32Temp[2]	= memc_logo_to_demura_drop_time;
		pwUINT32Temp[3]	= memc_logo_to_demura_drop_limit;
		pwUINT32Temp[4]	= memc_logo_to_demura_ascend_speed;
		pwUINT32Temp[5]	= memc_logo_read_en;
		pwUINT32Temp[6]	= logo_to_demura_en;
		pwUINT32Temp[7]	= memc_logo_fill_hole_en;
		pwUINT32Temp[8]	= memc_logo_filter_en;
		pwUINT32Temp[9]	= memc_logo_to_demura_update_cnt;
		pwUINT32Temp[10] = (unsigned int)before_phyAddr;
		pwUINT32Temp[11] = (unsigned int)after_phyAddr;
		pwUINT32Temp[12] = (unsigned long)LD_virAddr;
		pwUINT32Temp[13] = sld_ddr_offset_auto_get;
		pwUINT32Temp[14] = (unsigned int)sld_ddr_offset;
		pwUINT32Temp[15] = (unsigned long)LD_virAddr;
		pwUINT32Temp[16] = (unsigned int)SLD_SW_En;
		pwUINT32Temp[17] = (unsigned int)SLD_BY_MEMC_DEMURA_LOGO_ONLY;
		pwUINT32Temp[18] = (unsigned int)memc_logo_LD_APL_check;
		pwUINT32Temp[19] = (unsigned int)memc_logo_to_demura_APL_max_th;
		pwUINT32Temp[20] = (unsigned int)fill_hole_th;
		pwUINT32Temp[21] = (unsigned int)memc_logo_to_demura_drop_limit_gain;
		pwUINT32Temp[22] = (unsigned int)memc_logo_filter_SLD_APL_SE_en;
		pwUINT32Temp[23] = (unsigned int)memc_logo_to_demura_drop_limit_gain_SP;
		pwUINT32Temp[24] = (unsigned int)SLD_APL_reset_cnt_th;
		pwUINT32Temp[25] = (unsigned int)Gloabal_SLD_En;
		
		memcpy( &buf[0], (unsigned char*)(pwUINT32Temp+rtice_param.saddr), size);
		fwif_color_ChangeINT32Endian((int*)&buf[0], size/sizeof(int), 1);
		*num_type = VIP_INT32;
		vip_free(pwUINT32Temp);
...
//	DRV_srnn_table  *srnn_bbbse_table;
	...
	case TOOLACCESS_Contrast:   	/*  0*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		Scaler_SetContrast(rtice_param.mode0, buf[0]);
		break;

	case TOOLACCESS_Brightness:   	/*  1*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		Scaler_SetBrightness(rtice_param.mode0, buf[0]);
		break;

	case TOOLACCESS_Saturation:   	/*  2*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		Scaler_SetSaturation(rtice_param.mode0, buf[0]);
		break;

	case TOOLACCESS_Hue:   			/*  3*/
		if (size > sizeof(unsigned char))
			return -ID_SIZE_ERR;
		Scaler_SetHue(rtice_param.mode0, buf[0]);
		break;
...
*/
	...
	case TOOLACCESS_Function_Extend3_Sharpness_Table:
		...
		Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, buf[0]);
...
//			Scaler_SetMA_Chroma_Error(buf[12]); // merlin4 remove
	...
	case TOOLACCESS_Function_Extend3_ALL:
		...
		if (rtice_param.mode0 == 0) {
			...
			Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, buf[9]);
	...
	case TOOLACCESS_SUB_HDR_Y2GAIN_TBL:
    case TOOLACCESS_SUB_HDR_SUBPATH:
		ret = rtice_SetGet_HDR_SUB(rtice_param, NULL, buf, 1);
		if (ret < 0)
			return -ID_SIZE_ERR;
	...
	case TOOLACCESS_DM2_HLG_EOTF_byLuminance_CTRL: /*1929*/
		...
		Scaler_color_HDRIP_AutoRun(SLR_MAIN_DISPLAY, HDR_DM_HLG); // re-run the HLG flow
		break;
	...
	case TOOLACCESS_DM2_HDR10_EOTF_byLuminance_CTRL: /*1930*/
		...
		Scaler_color_HDRIP_AutoRun(SLR_MAIN_DISPLAY, HDR_DM_HDR10); // re-run the HDR10 flow
		break;
	...
	{		
		...
		fwif_color_set_Gamma_InvGamma_Ctrl(0, 0, pwINT16Temp, pwINT16Temp+1025, pwINT16Temp+2050, 1);
...
/*
	case TOOLACCESS_VIP_SLD:
		...
		pwUINT32Temp = (int *)vip_malloc(sizeof(int)*8);

		if(pwUINT32Temp == NULL)
			return -ID_SIZE_ERR;

		memcpy(pwUINT32Temp,(int *)buf , size);

		fwif_color_ChangeINT32Endian(pwUINT32Temp, size/sizeof(int), 1);

		thl_count_static_time = pwUINT32Temp[5];
		thl_stop_global_drop_count_frame = pwUINT32Temp[6];

                  fwif_k6_sld(pwUINT32Temp[0], pwUINT32Temp[1], pwUINT32Temp[2], pwUINT32Temp[3], (unsigned char)pwUINT32Temp[4],(unsigned int)pwUINT32Temp[7]); //for rtice use

	         //fwif_color_sld_still_image(&(pwUINT32Temp[5]),&(pwUINT32Temp[6]));
...
*/
	...
	case TOOLACCESS_VIP_SLD:
		...
		pwUINT32Temp = (int *)vip_malloc(sizeof(int)*50);
		...
		cmd_dY_preCur_mouseCon		     							= pwUINT32Temp[0];
		cmd_LC_period				     								= pwUINT32Temp[1];
		cmd_thl_counter_UR_con		     								= pwUINT32Temp[2];
		cmd_counter_frame_highY_blk		 						= pwUINT32Temp[3];
		cmd_lock_fw_gain_frame										= pwUINT32Temp[4];
		cmd_pixel_gain_step											= pwUINT32Temp[5];
		cmd_thl_counter_blk_pre_cur_same_mouseCon				= pwUINT32Temp[6];
		cmd_thl_error_time_box_logo_case							= pwUINT32Temp[7];
		cmd_thl_counter_box_logo_case								= pwUINT32Temp[8];
		cmd_thl_error_time_NEWS_case								= pwUINT32Temp[9];
		cmd_thl_counter_NEWS_case									= pwUINT32Temp[10];
		cmd_thl_error_time_noise_case								= pwUINT32Temp[11];
		cmd_thl_counter_noise_case									= pwUINT32Temp[12];
		cmd_thl_dC													= pwUINT32Temp[13];
		cmd_thl_dUDL												= pwUINT32Temp[14];
		cmd_rgb_box_leave_idx_delete_step							= pwUINT32Temp[15];
		cmd_box_logo_pixel_gain									= pwUINT32Temp[16];
		cmd_box_logo_maxGain_time								= pwUINT32Temp[17];
		cmd_box_lock_frame											= pwUINT32Temp[18];
	...
	case TOOLACCESS_VIP_LEA:
		...
		pwUINT32Temp = (int *)vip_malloc(sizeof(int)*26);
		...
		memcpy(pwUINT32Temp+rtice_param.saddr, (int *)buf , size);
		fwif_color_ChangeINT32Endian(pwUINT32Temp, size/sizeof(int), 1);

		/*
		filename_memc = pwUINT32Temp[3];
	       start_offset_byte= pwUINT32Temp[4];

		if (pwUINT32Temp[0] == 1) {
			Scaler_Get_Logo_Detect_VirAddr();
			dump_num_ = pwUINT32Temp[1];
			while(dump_num_ > 0) {
				garbage_offset_ = pwUINT32Temp[2];
		...
		*/
		...
		LSC_by_memc_logo_en = pwUINT32Temp[0];
		memc_logo_to_demura_wait_time = pwUINT32Temp[1];
		memc_logo_to_demura_drop_time = pwUINT32Temp[2];
		memc_logo_to_demura_drop_limit = pwUINT32Temp[3];
		memc_logo_to_demura_ascend_speed = pwUINT32Temp[4];
		memc_logo_read_en = pwUINT32Temp[5];
		logo_to_demura_en = pwUINT32Temp[6];
		memc_logo_fill_hole_en = pwUINT32Temp[7];
		memc_logo_filter_en = pwUINT32Temp[8];
		memc_logo_to_demura_update_cnt = pwUINT32Temp[9];
		//before_phyAddr = pwUINT32Temp[10];
		//after_phyAddr = pwUINT32Temp[11];
		//LD_virAddr = pwUINT32Temp[12];
		sld_ddr_offset_auto_get = pwUINT32Temp[13];
		sld_ddr_offset = pwUINT32Temp[14];
		SLD_SW_En = pwUINT32Temp[16];
		SLD_BY_MEMC_DEMURA_LOGO_ONLY = pwUINT32Temp[17];
		memc_logo_LD_APL_check = pwUINT32Temp[18];
		memc_logo_to_demura_APL_max_th = pwUINT32Temp[19];
		fill_hole_th = pwUINT32Temp[20];
		//
		memc_logo_to_demura_drop_limit_gain = pwUINT32Temp[21];
		memc_logo_filter_SLD_APL_SE_en = pwUINT32Temp[22];
		memc_logo_to_demura_drop_limit_gain_SP = pwUINT32Temp[23];
		SLD_APL_reset_cnt_th = pwUINT32Temp[24];
		Gloabal_SLD_En = pwUINT32Temp[25];
	...
	case TOOLACCESS_PQ_DYNAMIC_SHP_TV002:
	{
		...
		if (size >= sizeof(int)) {
			...
			if (g_TV002_Dynamic_Shp_Gain_En == 0) {
				...
				fwif_color_set_sharpness_level_only_gain(Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY), Scaler_GetSharpness());
			}

			if (g_TV002_Dynamic_Shp_Gain_By_Y_En == 0) {
				g_TV002_Dynamic_Shp_Gain_By_Y = 128;
				fwif_color_Set_Sharpness_gain_by_y_only_gain(Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY));
			}
...
int rtice_SetGet_HDR_SUB(ap_data_param rtice_param, unsigned char *num_type, unsigned char *buf, unsigned char isSet_Flag)
{
	unsigned int* pwUINT32Temp;
	unsigned short* pwUINT16Temp;
	//unsigned char* pwUINT8Temp;
	...
	if (isSet_Flag == 1) {	/* write */
		if (rtice_param.id == TOOLACCESS_SUB_HDR_EOTF_TBL) {
			if (size > (sizeof(unsigned int) * (HDR2_EOTF_CURVE_LEN * 3)))
				return -ID_SIZE_ERR;
			pwUINT32Temp = (unsigned int *)vip_malloc((HDR2_EOTF_CURVE_LEN * 3) * sizeof(int));
			if (pwUINT32Temp == NULL)
				return -ID_SIZE_ERR;

			if (size == (sizeof(unsigned int) * (HDR2_EOTF_CURVE_LEN * 3))) {
				memcpy((char*) pwUINT32Temp, buf, size);
				fwif_color_ChangeUINT32Endian(pwUINT32Temp, HDR2_EOTF_CURVE_LEN*3, 1);
				fwif_color_rtice_HDR2_EOTF_Set(pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 0), pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 1), pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 2), rtice_param.mode0);
			}

			vip_free(pwUINT32Temp);

		} else if (rtice_param.id == TOOLACCESS_SUB_HDR_OETF_TBL) {
			if (size > (sizeof(unsigned short) * (HDR2_OETF_CURVE_LEN * 3)))
				return -ID_SIZE_ERR;
			pwUINT16Temp = (unsigned short *)vip_malloc((HDR2_OETF_CURVE_LEN * 3) * sizeof(short));
			if (pwUINT16Temp == NULL)
				return -ID_SIZE_ERR;

			if (size == (sizeof(short) * (HDR2_OETF_CURVE_LEN * 3))) {
				memcpy((char*) pwUINT16Temp, buf, size);
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, HDR2_OETF_CURVE_LEN*3, 1);
				fwif_color_rtice_HDR2_OETF_Set(pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 0), pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 1), pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 2), rtice_param.mode0);
			}

			vip_free(pwUINT16Temp);

		}else if (rtice_param.id == TOOLACCESS_SUB_HDR_Y2GAIN_TBL) {
			if (size > (sizeof(unsigned short) * (HDR2_Y2Gain_CURVE_LEN * 1)))
				return -ID_SIZE_ERR;
			pwUINT16Temp = (unsigned short *)vip_malloc((HDR2_Y2Gain_CURVE_LEN * 1) * sizeof(short));
			if (pwUINT16Temp == NULL)
				return -ID_SIZE_ERR;

			if (size == (sizeof(short) * (HDR2_Y2Gain_CURVE_LEN * 1))) {
				memcpy((char*) pwUINT16Temp, buf, size);
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, HDR2_Y2Gain_CURVE_LEN*1, 1);
				fwif_color_rtice_DM2_Y2GAIN_Set(pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 0), rtice_param.mode0);
			}

			vip_free(pwUINT16Temp);

		}else if (rtice_param.id == TOOLACCESS_SUB_HDR_SUBPATH) {
			if (size > (sizeof(unsigned short) * (14 * 1)))
				return -ID_SIZE_ERR;
			pwUINT16Temp = (unsigned short *)vip_malloc((14 * 1) * sizeof(short));
			if (pwUINT16Temp == NULL)
				return -ID_SIZE_ERR;

			if (size == (sizeof(short) * (14 * 1))) {
				memcpy((char*) pwUINT16Temp, buf, size);
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, 14*1, 1);
				fwif_color_rtice_HDR2_SubPath_Set(pwUINT16Temp + (14 * 0));
			}
		...
		if (rtice_param.id == TOOLACCESS_SUB_HDR_EOTF_TBL) {
			...
			if (size == (sizeof(unsigned int) * (HDR2_EOTF_CURVE_LEN * 3))) {
				fwif_color_rtice_HDR2_EOTF_Get(pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 0), pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 1), pwUINT32Temp + (HDR2_EOTF_CURVE_LEN * 2));
				fwif_color_ChangeUINT32Endian(pwUINT32Temp, HDR2_EOTF_CURVE_LEN*3, 1);
				memcpy(buf, (char*) pwUINT32Temp, size);
			}
			*num_type = VIP_UINT32;
			...
			if (size == (sizeof(short) * (HDR2_OETF_CURVE_LEN * 3))) {
				fwif_color_rtice_HDR2_OETF_Get(pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 0), pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 1), pwUINT16Temp + (HDR2_OETF_CURVE_LEN * 2));
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, HDR2_OETF_CURVE_LEN*3, 1);
				memcpy(buf, (char*) pwUINT16Temp, size);
			}
			*num_type = VIP_UINT16;
			...
			if (size == (sizeof(short) * (HDR2_Y2Gain_CURVE_LEN * 1))) {
				fwif_color_rtice_DM2_Y2GAIN_Get(pwUINT16Temp + (HDR2_Y2Gain_CURVE_LEN * 0));
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, HDR2_Y2Gain_CURVE_LEN*1, 1);
				memcpy(buf, (char*) pwUINT16Temp, size);
			}
			*num_type = VIP_UINT16;
			...
			if (size == (sizeof(short) * (14 * 1))) {
				fwif_color_rtice_HDR2_SubPath_Get(pwUINT16Temp + (14 * 0));
				fwif_color_ChangeUINT16Endian(pwUINT16Temp, 14*1, 1);
				memcpy(buf, (char*) pwUINT16Temp, size);
			}
			*num_type = VIP_UINT16;
...
int rtice_set_Function_Extend3_Coef(ap_data_param rtice_param, unsigned char *buf, unsigned char isSet_Flag)
{
	...
	if(isSet_Flag == 1){
		...
		switch (rtice_param.mode1) {
			...
			break;
			case VIP_QUALITY_FUNCTION_SharpTable://[9]
				buf[0] = Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY);
			break;
			case VIP_QUALITY_FUNCTION_PQA_Table://[10]
				buf[0] = scaler_get_PQA_table();
			break;
