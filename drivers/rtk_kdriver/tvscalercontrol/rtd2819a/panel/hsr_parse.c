...
{
    TCON_SETTING_HEADER header = {0};
    TCON_SETTING_DATA Data = {0};
    int i = 0;
    int index = 0;
    HSR_MODE_TYPE hsrIndex = HSR_NOMAL;
    ...
    while(i < len)
    {
        if(i < sizeof(TCON_SETTING_HEADER))
        {
            if((i+8)<=len)
            {
                ...
                if(header.Offset != sizeof(TCON_SETTING_HEADER) || header.endCode != 0xffffffff)
                    return 0;

                i += sizeof(TCON_SETTING_HEADER);
                src += sizeof(TCON_SETTING_HEADER);;
            }
            else
                break;
        ...
        {
            if((i+sizeof(TCON_SETTING_DATA))<=len)
            {
                ...
                if(index < TCON_SETTING_MAX)
                {
                    ...
                    s_tconSetting.tcon_data[hsrIndex].len += sizeof(TCON_SETTING_DATA);
                }

                index++;
                i += sizeof(TCON_SETTING_DATA);
                src += sizeof(TCON_SETTING_DATA);
            }
            else
            {
                break;
