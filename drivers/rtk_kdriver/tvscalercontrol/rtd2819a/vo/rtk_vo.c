...
/**************************** VO GAV APIs start ***********************************/
//--- GAV BBD ---
extern unsigned char GAV_flow;
extern BBD_UPDATE_INFO bbd_active_info_GAV[2];
extern void Set_Val_bbd_enable(unsigned char display, unsigned char enable);
extern void scalerVIP_Set_BlackDetection_EN(unsigned int display, unsigned char bEnable);
extern char scalerVIP_SMF_BlackDetection_SUB(unsigned char enable);
//--- GAV State ---
void Scaler_Set_GAV_Enable_State(unsigned int state)
{
	unsigned int *gav_enable_state_addr = NULL;
	gav_enable_state_addr = (unsigned int*)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_ENABLE_STATE);
	if(gav_enable_state_addr)
	{
		*gav_enable_state_addr = htonl(state);
	}
	else
	{
		rtd_pr_vo_err("[GAV]: Scaler_Set_GAV_Enable_State: gav_enable_state_addr is NULL!, ignore set state!\n");
	}
}
unsigned int Scaler_Get_GAV_Enable_State_impl(void)
{
	unsigned int *gav_enable_state_addr = NULL;
	gav_enable_state_addr = (unsigned int*)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_ENABLE_STATE);
	if (!gav_enable_state_addr)
	{
		rtd_pr_vo_err("[GAV]: Scaler_Get_GAV_Enable_State_impl: gav_enable_state_addr is NULL!, ret 0!\n");
		return 0;
	}
	else
	{
		return htonl(*gav_enable_state_addr);
	}
}
unsigned int (*Scaler_Get_GAV_Enable_State)(void) = Scaler_Get_GAV_Enable_State_impl;
//--- GAV Data ---
void store_gav_queue_data(VO_GAV_DATA_INARG_T *arg)
{
	unsigned int data_in_read, data_in_write, data_out_read, data_out_write = 0;
	VO_GAV_QUEUE_T* gav_queue_data = (VO_GAV_QUEUE_T *)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_QUEUE_DATA);
	if (gav_queue_data)
	{
		//sharememory storage
		data_in_read	= gav_queue_data->in_read; //videofw read count: videofw control
		data_in_write	= gav_queue_data->in_write; //kernel queue to shm count: kernel control
		data_out_read	= gav_queue_data->out_read; //kernel read output data count: kernel control
		data_out_write	= gav_queue_data->out_write; //videofw queue to shm count: videofw control
		//queue pt: wr/rd(0)->max
		//queue pt: 0->wr->rd->max
		//queue pt: 0->rd->wr->max
		//queue pt: 0->wr/rd(max)
		// Fill up shm data
		if (data_in_write >= GAV_PIC_QUEUE_MAX) /* in write control by kernel */
		{
			data_in_write = 0;
		}
		//[Queue Debug]: rtd_pr_vo_info
		rtd_pr_vo_debug("#### %s: inQ rd/wr %d/%d outQ rd/wr %d/%d @OutY %x ####\n", __FUNCTION__,
						data_in_read, data_in_write, data_out_read, data_out_write, arg->output.buffer.phyaddrY);
		memcpy(&gav_queue_data->q_data_in[data_in_write], arg, sizeof(VO_GAV_DATA_INARG_T)); // store
		data_in_write++; //move write index after storing
		if (data_in_write >= GAV_PIC_QUEUE_MAX) //wr:4
		{
			data_in_write = 0; //reset write index, max keeping 4, do not access out of bound
		}
		gav_queue_data->in_write = data_in_write; //update
	} //valid shm
}
void reset_gav_queue_data(void)
{
	VO_GAV_QUEUE_T* gav_queue_data = (VO_GAV_QUEUE_T *)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_QUEUE_DATA);
	if (gav_queue_data)
	{
		memset(gav_queue_data, 0, sizeof(VO_GAV_QUEUE_T)); //init data
		/* Start from MAX=4, Rd shall then < Wr */
		gav_queue_data->in_read		= GAV_PIC_QUEUE_MAX; //videofw read count: videofw control
		gav_queue_data->in_write	= GAV_PIC_QUEUE_MAX; //kernel queue to shm count: kernel control
		gav_queue_data->out_read	= GAV_PIC_QUEUE_MAX; //kernel read output data count: kernel control
		gav_queue_data->out_write	= GAV_PIC_QUEUE_MAX; //videofw queue to shm count: videofw control
	}
	else
	{
		rtd_pr_vo_err("\r### error func:%s NULL gav_queue_data ###\r\n", __FUNCTION__);
	}
}
VO_GAV_DATA_OUTPUT_T Scaler_Get_Data_From_GAV(int *status_flag)
{
	unsigned int gav_current_state = Scaler_Get_GAV_Enable_State();
	VO_GAV_DATA_OUTPUT_T output;
	memset(&output, 0, sizeof(VO_GAV_DATA_OUTPUT_T));
	switch (gav_current_state)
	{
		case GAV_START_STATE:
		{
			rtd_pr_vo_debug("#### Scaler_Get_GAV_Enable_State %d : No first queue in ####\n", gav_current_state);
			*status_flag = -GAV_NOT_RUN;
			break;
		}
		case GAV_RUN_STATE:
		{
			unsigned int data_in_read	= 0;
			unsigned int data_in_write	= 0;
			unsigned int data_out_read	= 0;
			unsigned int data_out_write	= 0;
			VO_GAV_QUEUE_T* gav_queue_data = (VO_GAV_QUEUE_T *)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_QUEUE_DATA);
			if (!gav_queue_data)
			{
				rtd_pr_vo_err("#### %s %d : NULL gav_queue_data ####\n", __FUNCTION__, gav_current_state);
				*status_flag = -GAV_SHM_NULL;
				break;
			}
			data_in_read	= gav_queue_data->in_read; //videofw read count: videofw control
			data_in_write	= gav_queue_data->in_write; //kernel queue to shm count: kernel control
			data_out_read	= gav_queue_data->out_read; //kernel read output data count: kernel control
			data_out_write	= gav_queue_data->out_write; //videofw queue to shm count: videofw control
			if (data_out_read != data_out_write)
			{
				/* dequeue */
				if (data_out_read >= GAV_PIC_QUEUE_MAX)
				{
					data_out_read = 0; //reset read index, move to first
				}
				memcpy(&output, &gav_queue_data->q_data_out[data_out_read], sizeof(VO_GAV_DATA_OUTPUT_T));
				//[Queue Debug]: rtd_pr_vo_info
				rtd_pr_vo_debug("#### %s %d : inQ rd/wr %d/%d outQ rd/wr %d/%d @OutY %x ####\n", __FUNCTION__, gav_current_state,
									data_in_read, data_in_write, data_out_read, data_out_write, output.buffer.phyaddrY);
				data_out_read++;
				if (data_out_read >= GAV_PIC_QUEUE_MAX) //4:0,1,2,3
				{
					data_out_read = 0; //reset read index, move to first
				}
				gav_queue_data->out_read = data_out_read;
				*status_flag = GAV_OK;
				break;
			}
			else
			{
				//[Queue Debug]: rtd_pr_vo_info
				rtd_pr_vo_debug("#### %s %d : inQ rd/wr %d/%d outQ rd/wr %d/%d => SKIP! ####\n", __FUNCTION__, gav_current_state,
									data_in_read, data_in_write, data_out_read, data_out_write);
				if (data_out_write == GAV_PIC_QUEUE_MAX) //no data from videofw 4:4 reset state
				{
					rtd_pr_vo_debug("#### %s %d : Q 0, videofw not yet output first pic! ####\n", __FUNCTION__, gav_current_state);
					*status_flag = -GAV_PENDING_PIC_OUT;
					break;
				}
				else
				{	//output not ready, do not dequeue: pic Q = 0:0, 1:1, 2:2, 3:3
					rtd_pr_vo_debug("#### %s %d : Q done! wait new pic to videofw process and dequeue! ####\n", __FUNCTION__, gav_current_state);
					*status_flag = -GAV_PENDING_PIC_OUT;
					break;
				}
			}
		...
		case GAV_STOP_STATE:
		{
			rtd_pr_vo_err("#### Scaler_Get_GAV_Enable_State %d : Not Started !!! ####\n", gav_current_state);
			*status_flag = -GAV_ERROR;
			break;
		}
		default:
		{
...
//--- BBD Set Crop
int VO_Set_BBD_Crop(VO_GAV_DATA_OUTPUT_T *data_out, VO_GAV_BBD_CROP_T crop)
{
	if (!data_out)
	{
		rtd_pr_vo_err("[%s] Error: NULL GAV_data_output\n",
						__FUNCTION__);
		return -1; // return -1 for error data_out
	}
	// if (data_out->flags != FLAG_ENABLE_BBD)
	// {
	// 	rtd_pr_vo_err("[%s] Error: BBD not supported, flag: %d\n",
	// 					__FUNCTION__, data_out->flags);
	// 	return -1; // return -1 for error data_out
	// }
	if ((crop.left > data_out->buffer.width) || (crop.right > data_out->buffer.width) ||
		(crop.top > data_out->buffer.height) || (crop.bottom > data_out->buffer.height))
	{
		rtd_pr_vo_err("[%s] Error: Invalid crop size( %d, %d, %d, %d ), out of bound, don't set crop\n",
						__FUNCTION__, crop.left, crop.right,
										crop.top, crop.bottom);
		return -1; // return -1 for error crop size out of bound
	}
	data_out->crop = crop; // set crop to dequeue out buf
	return 0;
}
//--- GAV IOC ---
int VO_GAV_start(VO_GAV_MODE_T mode) {
	/* gav mode for 2k/4k clock timing */
	rtd_pr_vo_info("%s(): mode=%d\n", __func__, mode);
	if ((Scaler_Get_GAV_Enable_State() == GAV_START_STATE) ||
		(Scaler_Get_GAV_Enable_State() == GAV_RUN_STATE))
	{
		rtd_pr_vo_err("#### Scaler_Get_GAV_Enable_State: Already Start / Run !!! ####\n");
		return -GAV_HAS_RUN;
	}
	else
	{
		// VO_GAV_MODE_T: not sure what is the purpose, store it first
		VO_GAV_MODE_T* gav_start_data = (VO_GAV_MODE_T *)Scaler_GetShareMemVirAddr(SCALERIOC_GAV_START_MODE);
		if (!gav_start_data) {
			return -GAV_SHM_NULL;
		}
		// copy start mode
		memcpy(gav_start_data, &mode, sizeof(VO_GAV_MODE_T));
		// init queue data and task
		reset_gav_queue_data();
		// rpc to fw vo
		if (Scaler_SendRPC(SCALERIOC_GAV_ENABLE_STATE, GAV_START_STATE, 1) == RPC_FAIL) {
			rtd_pr_vo_err("#### VO_GAV_start: fw ret RPC FAIL !!! ####\n");
			return -GAV_RPC_FAIL;
		}
		else
		{
			// set gav state
			Scaler_Set_GAV_Enable_State(GAV_START_STATE);
			// flag for PQ BBD to use GAV
			GAV_flow = GAV_START_STATE;
			// set bbd enable flag
			Set_Val_bbd_enable(SLR_SUB_DISPLAY, 1);
			// set bbd start detect
			scalerVIP_Set_BlackDetection_EN(SLR_SUB_DISPLAY, 1);
			// Update BBD
			scalerVIP_SMF_BlackDetection_SUB(1);
		}
		return GAV_OK;
	}
}
int VO_GAV_stop(void) {
	rtd_pr_vo_info("%s()\n", __func__);
	if(Scaler_Get_GAV_Enable_State())
	{
		//rpc to fw vo
		if (Scaler_SendRPC(SCALERIOC_GAV_ENABLE_STATE, GAV_STOP_STATE, 1) == RPC_FAIL)
		{
			rtd_pr_vo_err("#### VO_GAV_stop: fw ret RPC FAIL !!! ####\n");
			return -GAV_RPC_FAIL;
		}
		else
		{
			rtd_pr_vo_info("#### VO_GAV_stop: fw ret ok, set kernel gav to stop ####\n");
			// reset queue of stored incoming data
			reset_gav_queue_data();
			// set gav state to stop
			Scaler_Set_GAV_Enable_State(GAV_STOP_STATE);
			// reset bbd detect
			scalerVIP_SMF_BlackDetection_SUB(0);
			// flag for PQ BBD to use GAV set to disable
			GAV_flow = GAV_STOP_STATE;
			// set bbd enable flag to disable
			Set_Val_bbd_enable(SLR_SUB_DISPLAY, 0);
			// set bbd stop detect
			scalerVIP_Set_BlackDetection_EN(SLR_SUB_DISPLAY, 0);
		}
		return GAV_OK;
	}
	else
	{
		rtd_pr_vo_err("#### Scaler_Get_GAV_Enable_State: Already Stop !!! ####\n");
		return -GAV_NOT_RUN;
	}
}
int VO_GAV_queue(VO_GAV_DATA_INARG_T *arg) {
	if (!arg)
	{
		rtd_pr_vo_err("\r### error func:%s NULL VO_GAV_DATA_INARG_T arg ###\r\n", __FUNCTION__);
		return -GAV_ERROR;
	}
	else
	{
		/* for debug print only -- start */
		VO_GAV_DATA_INPUT_T *input = &arg->input;
		VO_GAV_DATA_OUTPUT_T *output = &arg->output;
		if (!input) {
			rtd_pr_vo_err("\r### error func:%s NULL VO_GAV_DATA_INPUT_T input ###\r\n", __FUNCTION__);
			return -GAV_ERROR;
		}
		if (!output) {
			rtd_pr_vo_err("\r### error func:%s NULL VO_GAV_DATA_OUTPUT_T output ###\r\n", __FUNCTION__);
			return -GAV_ERROR;
		}
		rtd_pr_vo_info(
			"%s(): input: version=%d, flags=%x, cmprsMode=%u, phyaddr=%x/%x, "
			"pitch=%u/%u, w/h=%u/%u, bitDepthY/C=%u/%u\n",
			__func__, input->version, input->flags, input->cmprsMode,
			input->buffer.phyaddrY, input->buffer.phyaddrC, input->buffer.pitchY,
			input->buffer.pitchC, input->buffer.width, input->buffer.height,
			input->buffer.bitDepthY, input->buffer.bitDepthC);
		rtd_pr_vo_info(
			"%s(): output: version=%d, flags=%x, phyaddr=%x/%x, "
			"pitch=%u/%u, w/h=%u/%u, bitDepthY/C=%u/%u, crop=%u/%u/%u/%u\n",
			__func__, output->version, output->flags, output->buffer.phyaddrY,
			output->buffer.phyaddrC, output->buffer.pitchY, output->buffer.pitchC,
			output->buffer.width, output->buffer.height, output->buffer.bitDepthY,
			output->buffer.bitDepthC, output->crop.left, output->crop.top,
			output->crop.right, output->crop.bottom);
		/* for debug print only --- end */
		if(Scaler_Get_GAV_Enable_State() == GAV_START_STATE) //first in queue
		{
			//rpc to fw vo
			if (Scaler_SendRPC(SCALERIOC_GAV_ENABLE_STATE, GAV_RUN_STATE, 1) == RPC_FAIL)
			{
				rtd_pr_vo_err("#### VO_GAV_queue: fw ret RPC FAIL !!! ####\n");
				return -GAV_RPC_FAIL;
			}
			else
			{
				// set gav state
				Scaler_Set_GAV_Enable_State(GAV_RUN_STATE); //set to run
			}
		}
		//continue, after set to run
		if(Scaler_Get_GAV_Enable_State() != GAV_STOP_STATE)
		{
			store_gav_queue_data(arg); //direct storage
			return GAV_OK;
		}
		else
		{
			rtd_pr_vo_err("#### Scaler_Set_GAV_Enable_State: Not Started !!! ####\n");
			return -GAV_NOT_RUN;
		}
	}
}
int VO_GAV_dequeue(VO_GAV_DATA_OUTPUT_T *output) {
	int status_flag = 0;
	if (!output)
	{
		rtd_pr_vo_err("\r### error func:%s NULL output ###\r\n", __FUNCTION__);
		return -GAV_ERROR;
	}
	else
	{
		*output = Scaler_Get_Data_From_GAV(&status_flag);
		if (status_flag == GAV_OK)
		{
			//only gen crop for valid size
			if ((output->buffer.width > 0) && (output->buffer.height > 0))
			{
				//VO_Set_BBD_Crop(output, generate_random_crop(output->buffer.width, output->buffer.height)); //Random BBD crop for debug
				VO_GAV_BBD_CROP_T full_crop = {
						.left = 0,
						.top = 0,
						.right = output->buffer.width,
						.bottom = output->buffer.height
				};
				VO_GAV_BBD_CROP_T bbd_crop = {
						.left = 0,
						.top = 0,
						.right = 0,
						.bottom = 0
				...
				// Update BBD
				scalerVIP_SMF_BlackDetection_SUB(1);
				// Assign crop structure
				bbd_crop.left = bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.x;
				bbd_crop.top = bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.y;
				bbd_crop.right = bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.x + bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.width;
				bbd_crop.bottom = bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.y + bbd_active_info_GAV[SLR_SUB_DISPLAY].active_size.height;
				// If crop 0, give some value!
				if ((bbd_crop.left == 0) && (bbd_crop.top == 0) &&
					(bbd_crop.right == 0) && (bbd_crop.bottom == 0))
				{
					rtd_pr_vo_info(
						"%s(): bbd detect crop return %d/%d/%d/%d,"
						"set to no crop 0/0/w/h=%d/%d/%d/%d\n",
						__func__, bbd_crop.left, bbd_crop.top, bbd_crop.right, bbd_crop.bottom,
									full_crop.left, full_crop.top, full_crop.right, full_crop.bottom);
					VO_Set_BBD_Crop(output, full_crop); //No crop
				}
				else
				{
					rtd_pr_vo_info(
						"%s(): bbd crop GET! return %d/%d/%d/%d\n",
						__func__, bbd_crop.left, bbd_crop.top, bbd_crop.right, bbd_crop.bottom);
					VO_Set_BBD_Crop(output, bbd_crop); //Apply bbd crop
				}
			}
			rtd_pr_vo_info(
				"%s(): output: version=%d, flags=%x, phyaddr=%x/%x, "
...
int VO_GAV_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {

	int ret = 0;

	switch (cmd)
	{
		case VO_IOC_GAV_START:
		{
			VO_GAV_MODE_T data;
			if (copy_from_user(&data, (const void __user *)arg, sizeof(data))) {
				rtd_pr_vo_err("#### VO_IOC_GAV_START kern copy_from_user fail ####\n");
				ret = -EFAULT;
				break;
			}
			ret = VO_GAV_start(data);
			if (ret == GAV_OK)
			{
				rtd_pr_vo_info("#### VO_IOC_GAV_START success, ok! ####\n");
				ret = 0;
			}
			else if (ret == -GAV_RPC_FAIL)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_START ret fail: fw rpc ret fail! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_HAS_RUN)
			{
				rtd_pr_vo_info("#### VO_IOC_GAV_START ret invalid: gav has run! ####\n");
				ret = 0;
			}
			else if (ret == -GAV_SHM_NULL)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_START ret fail: shm is NULL! ####\n");
				ret = -EFAULT;
			}
			else
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_START invalid status, ret: %d ####\n", ret);
				ret = -EFAULT;
			}
		}
		break;
		case VO_IOC_GAV_STOP:
		{
			ret = VO_GAV_stop();
			if (ret == GAV_OK)
			{
				rtd_pr_vo_info("#### VO_IOC_GAV_STOP success, ok! ####\n");
				ret = 0;
			}
			else if (ret == -GAV_RPC_FAIL)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_STOP ret fail: fw rpc ret fail! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_NOT_RUN)
			{
				rtd_pr_vo_info("#### VO_IOC_GAV_STOP ret invalid: gav is stopped! ####\n");
				ret = 0;
			}
			else
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_STOP invalid status, ret: %d ####\n", ret);
				ret = -EFAULT;
			}
		}
		break;
		case VO_IOC_GAV_QUEUE:
		{
			VO_GAV_DATA_INARG_T data;
			if (copy_from_user(&data, (const void __user *)arg, sizeof(data))) {
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE kern copy_from_user fail ####\n");
				ret = -EFAULT;
				break;
			}
			ret = VO_GAV_queue(&data);
			if (ret == GAV_OK)
			{
				rtd_pr_vo_info("#### VO_IOC_GAV_QUEUE success, ok! ####\n");
				ret = 0;
			}
			else if (ret == -GAV_ERROR)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE ret fail: check kern error! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_RPC_FAIL)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE ret fail: fw rpc ret fail! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_NOT_RUN)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE ret fail: gav not run! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_OVERFLOW)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE ret fail: picQ overflow! fw fail! ####\n");
				ret = -EFAULT;
			}
			else
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_QUEUE invalid status, ret: %d ####\n", ret);
				ret = -EFAULT;
			}
		}
		break;
		case VO_IOC_GAV_DEQUEUE:
		{
			VO_GAV_DATA_OUTPUT_T data;
			if (copy_from_user(&data, (const void __user *)arg, sizeof(data))) {
				rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE kern copy_from_user fail ####\n");
				ret = -EFAULT;
				break;
			}
			ret = VO_GAV_dequeue(&data);
			if (ret == GAV_OK)
			{
				/* Send the updated output back to user rhal */
				if (copy_to_user((void __user *)arg, (const void *)&data, sizeof(VO_GAV_DATA_OUTPUT_T))) {
					rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE kern copy_to_user fail ####\n");
					ret = -EFAULT;
				}
				else
				{
					rtd_pr_vo_info("#### VO_IOC_GAV_DEQUEUE success, ok! ####\n");
					ret = 0;
				}
			}
			else if (ret == -GAV_ERROR)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE ret fail: check kern error! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_NOT_RUN)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE ret fail: gav not run! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_SHM_NULL)
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE ret fail: shm is NULL! ####\n");
				ret = -EFAULT;
			}
			else if (ret == -GAV_PENDING_PIC_IN)
			{
				rtd_pr_vo_debug("#### VO_IOC_GAV_DEQUEUE ret invalid: waiting pic from media ####\n");
				ret = 0;
			}
			else if (ret == -GAV_PENDING_PIC_OUT)
			{
				rtd_pr_vo_debug("#### VO_IOC_GAV_DEQUEUE ret invalid: waiting pic from fw ####\n");
				ret = 0;
			}
			else
			{
				rtd_pr_vo_err("#### VO_IOC_GAV_DEQUEUE invalid status, ret: %d ####\n", ret);
				ret = -EFAULT;
			}
		}
		break;
		default:
		{
			rtd_pr_vo_err("#### %s: cmd %d not support! ####\n", __FUNCTION__, cmd);
			ret = -EFAULT;
		}
		break;
	}
...
long vodev_ioctl(struct file *filp, unsigned int cmd,  unsigned long arg)
{
	...
	if ((cmd == VO_IOC_GAV_QUEUE) || (cmd == VO_IOC_GAV_DEQUEUE)) //avoid printing too much
	{
		rtd_pr_vo_debug("VO: ioctl code = %x\n", cmd);
	}
	else
	{
...
int vodma_set_vsyncgen(unsigned int ch, VO_TIMING *tmode)
{
    ...
    pll_reg_sys_pll_vodma_1_RBUS pll_reg_sys_pll_vodma_1_reg;
    pll_reg_sys_pll_vodma_2_RBUS pll_reg_sys_pll_vodma_2_reg;

	unsigned int vodmapll, vodmapll0, freerun_clk;
	unsigned int nNCode, nFcode, nPrediv, nOCode = 0;
	long long busclk = 0;
    ...
    pll_reg_sys_pll_vodma_1_reg.regValue = rtd_inl(PLL_REG_SYS_PLL_VODMA_1_reg);
    pll_reg_sys_pll_vodma_2_reg.regValue = rtd_inl(PLL_REG_SYS_PLL_VODMA_2_reg);
	...
	case 0:
	default:
        nOCode  = pll_reg_sys_pll_vodma_1_reg.pllvodma_o;
        nPrediv = pll_reg_sys_pll_vodma_1_reg.pllvodma_prediv;
        nFcode  = pll_reg_sys_pll_vodma_2_reg.pllvodma_fcode;
        nNCode  = pll_reg_sys_pll_vodma_2_reg.pllvodma_ncode;
        busclk = ((XTAL_TICK / 1000000) * ( ((nNCode + VODMA_PLL_MCODE_BASIC) * 1000000) + (nFcode * 1000000 / VODMA_PLL_FCODE_BASIC) ) ) / ( (nPrediv + 1) / (nOCode + 1) );
        vodmapll = (unsigned int)busclk;
        rtd_pr_vo_info("[vo] pll_vodma: %d/%d/%d/%d/%d, %x/%x\n", nNCode, nFcode, nPrediv, nOCode, vodmapll, pll_reg_sys_pll_vodma_1_reg.regValue, pll_reg_sys_pll_vodma_2_reg.regValue);
		break;
	}
	vodmapll0 = vodmapll;
...
unsigned long rpcVoReady(unsigned long para1, unsigned long para2)
{
		...
		!force_change_vo_flag[ch])
	{
        ...
        if(((&vo->videoPlane[ch].VOInfo)->film_mode < _VO_CAD_BAD_EDIT) && (pVOInfo->film_mode >= _VO_CAD_BAD_EDIT)){
            ...
            rtd_pr_vo_notice("[FILM] INPUT FAST=%d@VideoMode\n", MDOMAIN_CAP_Cap0_buf_control_get_cap0_input_fast(IoReg_Read32(MDOMAIN_CAP_Cap0_buf_control_reg)));
        }
...
void __drv_I3ddmaVodma_SingleBuffer_GameMode(unsigned char enable)
{
	...
	I3DDMA_TIMING_T *timing = (I3DDMA_TIMING_T*) drvif_I3DDMA_Get_Input_Info_Vaule(I3DDMA_DEV, I3DDMA_INFO_INPUT_TIMING);
	...
	if(i3ddmaCtrl[I3DDMA_DEV].ptx_timing && (i3ddmaCtrl[I3DDMA_DEV].ptx_timing->color < I3DDMA_COLOR_UNKNOW))
		HDMI_INFO.input_fmt = VOColorMap[i3ddmaCtrl[I3DDMA_DEV].ptx_timing->color];
	else
	...
	HDMI_INFO.cap_buffer_size = i3ddmaCtrl[I3DDMA_DEV].cap_buffer[0].size;
	...
	if((Get_DISPLAY_REFRESH_RATE() == 120) && (get_vsc_src_is_hdmi_or_dp(SLR_MAIN_DISPLAY)) && (is_vrr_or_freesync_mode())) // vrr case do not to do vo shift, because frame buffer mode
	{
...
void VODMA_Compression_Mask_Setting(bool bEnable,unsigned int width,unsigned int length)
{
    ...
    //rtd_outl(VODMA_VODMA_V1VGIP_BG_CLR_reg, vodma->V1_vgipPos_bg_clr.regValue);
	if(bEnable){
...
void timinggen_set_v1(VO_VIDEO_PLANE plane, VO_TIMING *tmode)
{
    ...
    V1_syncGen.vthr_rst1 = 0;
	V1_syncGen.v_thr = 0xfff; // fixme 6982
...
void vsyncgen_set_v1(VO_VIDEO_PLANE plane, VO_TIMING *tmode)
{
    ...
    if (plane == VO_VIDEO_PLANE_V1)
    {
		if(((unsigned long long)tmode->HTotal* (unsigned long long)tmode->VTotal* (unsigned long long)tmode->FrameRate / 1000) > (unsigned long long)VO_Get_vodmapll()){ // 4K120
			clkgen.vodma_2p_gate_sel = 0;
			clkgen.vodma_hdr_gate_sel = 0;
		//	printf2("[vo] v1 vodma_2p_gate_sel = 1x 1p\n");
		}
		else{
			clkgen.vodma_2p_gate_sel = 1; // <4K60 =1;
			clkgen.vodma_hdr_gate_sel = 0;
		//	printf2("[vo] v1 vodma_2p_gate_sel = 1/2x 1p\n");
		}
        rtd_outl(VODMA_VODMA_CLKGEN_reg, clkgen.regValue);
        rtd_outl(VODMA_VODMA_PVS_Free_reg, pvs_free.regValue);
        rtd_outl(VODMA_VODMA_PVS_WIDTH_CTRL_reg, pvs_width_ctrl.regValue);
...
void VODMA_SetVOInfo(unsigned int ch, VO_TIMING *timemode, VO_SOURCE_TYPE_INFO source,VODMA_DDOMAIN_CHROMA Ddomain_Chroma,VO_COLORSPACE colorspace)
{
        ...
        vsyncgen_pixel_freq = VSYNCGEN_get_vodma_pixel_clk_0();
...
void VODMA_ConfigTiming(VO_TIMING *timemode, VIDEO_RPC_VOUT_CONFIG_HDMI_3D *HDMI_INFO)
{
	...
	if(timemode->HTotal * timemode->VTotal * (timemode->FrameRate/1000) > (VO_Get_vodmapll()*check_i3_vo_pixel_mode())) {
...
//Mark because QS compile fail, size not enough cannot add new function
unsigned int VO_Get_vodmapll(void)
{
	/* -------------- calc vodmapll (busclk) formula-----------------------------
	DIVMODE (default always 0)
	For PLL   GPU/ NNSR/ VODMA/ UVC
	For DIVMODE = 0:
		Fout =  Fref  * ( (NCODE + 4 ) + (FCODE/2048) ) / (PREDIV + 1) / (2 ^ O) <== Default
	For DIVMODE = 1:
		Fout =  Fref  * ( (NCODE + 3 ) + (FCODE/2048) ) / (PREDIV + 1) / (2 ^ O)
	-----------------------------------------------------------------------------*/
	unsigned int nNCode, nFcode, nPrediv, nOCode, vodmapll = 0;
    long long vodmapll0 = 0;
	pll_reg_sys_pll_vodma_1_RBUS pll_reg_sys_pll_vodma_1_reg;
	pll_reg_sys_pll_vodma_2_RBUS pll_reg_sys_pll_vodma_2_reg;
	pll_reg_sys_pll_vodma_1_reg.regValue = rtd_inl(PLL_REG_SYS_PLL_VODMA_1_reg);
	pll_reg_sys_pll_vodma_2_reg.regValue = rtd_inl(PLL_REG_SYS_PLL_VODMA_2_reg);
	...
	nFcode  = pll_reg_sys_pll_vodma_2_reg.pllvodma_fcode;
	nNCode  = pll_reg_sys_pll_vodma_2_reg.pllvodma_ncode;
	//vodmapll = (XTAL_TICK * 1.0 * ( (nNCode + VODMA_PLL_MCODE_BASIC) + (nFcode * 1.0 / VODMA_PLL_FCODE_BASIC) ) ) / ( (nPrediv + 1) / (nOCode + 1) ); // need decimal precision
    vodmapll0 = ((XTAL_TICK / 1000000) * ( ((nNCode + VODMA_PLL_MCODE_BASIC) * 1000000) + (nFcode * 1000000 / VODMA_PLL_FCODE_BASIC) ) ) / ( (nPrediv + 1) / (nOCode + 1) );
	vodmapll = (unsigned int)vodmapll0;
    rtd_pr_vo_info("[vo] pll_vodma: %d/%d/%d/%d/%d, %x/%x\n", nNCode, nFcode, nPrediv, nOCode, vodmapll, pll_reg_sys_pll_vodma_1_reg.regValue, pll_reg_sys_pll_vodma_2_reg.regValue);
	return vodmapll;
}

unsigned int VSYNCGEN_get_vodma_pixel_clk_0(void)
{
    unsigned int vo_clk;
    unsigned int ratio_n_off, divide_n;
    ...
    divide_n = vodma_clkgen_reg.vodma_clk_div_n;

    vo_clk = VO_Get_vodmapll();
    vo_clk = vo_clk / 64 * ( 64 - ratio_n_off );
    vo_clk = vo_clk / ( divide_n + 1 );
  ...
  //  rtd_pr_vbe_emerg("[VO][%d,%d,%d,%d,%d,%d]\n", nFcode, nNCode, nPrediv, nOCode, ratio_n_off, divide_n);
...
unsigned int VSYNCGEN_get_vodma_clk_0_max(void)
{
    ...
    if(vodma_clkgen_reg.vodma_clk_div2_en){//interlace should div2
        vodmapll = vodmapll / 2;
        rtd_pr_vbe_notice("vo clk div2 for interlace timing\n");
    }
    rtd_pr_vbe_notice("[vo] vsyncgen busclk:%d, divider_num=%d\n", busclk, divider_num);
...
int vodma_init_pll(void)
{
	...
	rtd_outl(0x18000374,0x00000004);    //PLLVODMA power down,hold reset,output disable
	rtd_outl(0x18000370,0x00000019);    //[30:28]VODMA_O=0x0,[21]PLLVODMA_WDSET=0, [20]PLLVODMA_WDRST=0,[14]CP=0,[10:8]ICP=b'000,
										//[5:4]pllvodma_prediv=1, [3:0]RS=b'1001
	rtd_outl(0x18000374,0x038F3205);    //[26:16]Fcode=0x38F(911), [15:8]Ncode=0x32(50), 27*((911/2048)+(50+4))/(1+1) = 735MHz, PLLVODMA power on
	ScalerTimer_DelayXms(0x10);         //at least > 100us
	rtd_outl(0x18000378,0x50200805);    //PLLVOMDA_OC_EN=1,OC_MODE=0,OC_STEP=0x1,PI_CUR_SEL=0x0,EN_PI_DEBUG=1
	rtd_outl(0x18000374,0x038F3207);    //release PLLVODMA reset
	//polling OC_EN=1
	rtd_inl(0x18000378);                //polling [1]PLLVODMA_OC_DONE==1
	ScalerTimer_DelayXms(0x10);         //at least > 100us
	rtd_outl(0x18000374,0x038F3203);	// PLLVODMA output enable
	*/
	pll_reg_sys_pll_vodma_1_RBUS pll_reg_sys_pll_vodma_1_reg;
	pll_reg_sys_pll_vodma_2_RBUS pll_reg_sys_pll_vodma_2_reg;
	rtd_outl(PLL_REG_SYS_PLL_VODMA_2_reg, 0x00000004);          //PLLVODMA power down, hold reset, output disable
	pll_reg_sys_pll_vodma_1_reg.pllvodma_o      = PLLVODMA_O;      //O=d'0
	pll_reg_sys_pll_vodma_1_reg.pllvodma_ip     = PLLVODMA_ICP;    //ICP=b'000
	pll_reg_sys_pll_vodma_1_reg.pllvodma_cp     = PLLVODMA_CP;     //CP=b'0
	pll_reg_sys_pll_vodma_1_reg.pllvodma_wdset  = PLLVODMA_WDSET;  //d'0
	pll_reg_sys_pll_vodma_1_reg.pllvodma_wdrst  = PLLVODMA_WDRST;  //d'0
	pll_reg_sys_pll_vodma_1_reg.pllvodma_prediv = PLLVODMA_PREDIV; //PREDIV=1
	pll_reg_sys_pll_vodma_1_reg.pllvodma_rs     = PLLVODMA_RS;     //RS=b'1001=d'9
	//<Default>
	pll_reg_sys_pll_vodma_2_reg.regValue        = rtd_inl(PLL_REG_SYS_PLL_VODMA_2_reg);
	pll_reg_sys_pll_vodma_2_reg.pllvodma_fcode  = PLLVODMA_F;      //Fcode=0x38F(911)
	pll_reg_sys_pll_vodma_2_reg.pllvodma_ncode  = PLLVODMA_N;      //Ncode=0x32(50)
	// rtd_outl(0x18000374,0x038F3205);	//PLLVODMA power on
	pll_reg_sys_pll_vodma_2_reg.pllvodma_pow    = 1;               //PLLVODMA power on control: 1
	pll_reg_sys_pll_vodma_2_reg.pllvodma_rstb   = 0;               //PLLVODMA reset control: 0
	pll_reg_sys_pll_vodma_2_reg.pllvodma_oeb    = 1;               //PLLVODMA output enable control: 1
	rtd_outl(PLL_REG_SYS_PLL_VODMA_1_reg, pll_reg_sys_pll_vodma_1_reg.regValue);
	rtd_outl(PLL_REG_SYS_PLL_VODMA_2_reg, pll_reg_sys_pll_vodma_2_reg.regValue);
	// rtd_outl(0x18000378,0x50200805);	//PLLVOMDA_OC_EN=1,OC_MODE=0,OC_STEP=0x1,PI_CUR_SEL=0x0,EN_PI_DEBUG=1
	rtd_outl(PLL_REG_SYS_DVFS_VODMA_1_reg, 0x50200805);
	msleep(1); //ScalerTimer_DelayXms(0x10);			//at least > 100us
	// rtd_outl(0x18000374,0x038F3207);	//release PLLVODMA reset
	pll_reg_sys_pll_vodma_2_reg.regValue        = rtd_inl(PLL_REG_SYS_PLL_VODMA_2_reg);
	pll_reg_sys_pll_vodma_2_reg.pllvodma_pow    = 1;               //PLLVODMA power on control: 1
	pll_reg_sys_pll_vodma_2_reg.pllvodma_rstb   = 1;               //PLLVODMA reset control: 1
	pll_reg_sys_pll_vodma_2_reg.pllvodma_oeb    = 1;               //PLLVODMA output enable control: 1
	rtd_outl(PLL_REG_SYS_PLL_VODMA_2_reg, pll_reg_sys_pll_vodma_2_reg.regValue);
	msleep(1); //ScalerTimer_DelayXms(0x10);			//at least > 100us
	// rtd_outl(0x18000374,0x038F3203);	//PLLVODMA output enable
	...
	pll_reg_sys_pll_vodma_2_reg.pllvodma_pow    = 1;               //PLLVODMA power on control: 1
	pll_reg_sys_pll_vodma_2_reg.pllvodma_rstb   = 1;               //PLLVODMA reset control: 1
	pll_reg_sys_pll_vodma_2_reg.pllvodma_oeb    = 0;               //PLLVODMA output enable control: 0
	rtd_outl(PLL_REG_SYS_PLL_VODMA_2_reg, pll_reg_sys_pll_vodma_2_reg.regValue);
