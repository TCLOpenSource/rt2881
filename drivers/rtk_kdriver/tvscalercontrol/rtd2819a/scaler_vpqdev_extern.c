...
extern unsigned int Scaler_Vpqmemcdev_MEMC_GetAVSyncDelay(void);
...
char VPQEX_VIP_VPQ_IOC_CT_DelaySet_Process_VPQTASK(void)
{
	...
	if (Enable == 1)
	{
		...
		memc_delay_90k = Scaler_Vpqmemcdev_MEMC_GetAVSyncDelay()*90;	// ms to 90 clk
...
extern StructColorDataFacModeType g_curColorFacTableEx_sub;
static int VPQEX_VPQ_EXTERN_IOC_ACCESS_VPQ_byPara(HAL_VPQ_DATA_EXTERN_T *hal_VPQ_DATA_EXTERN)
{
	int ret = 0;
	unsigned long Data_addr;
	unsigned int VPQ_byPara_arg = 0;	//	

	if (hal_VPQ_DATA_EXTERN == NULL)
	{
		rtd_pr_vpq_emerg("VPQEX_VPQ_EXTERN_IOC_ACCESS_VPQ_byPara NULL\n");
		ret = -1;
	}
	else
	{
		...
		if (hal_VPQ_DATA_EXTERN->wId == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Set)
		{
			...
			{
				rtd_pr_vpq_emerg("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara version error\n");
				ret = -1;
			}
		}
		else if (hal_VPQ_DATA_EXTERN->wId == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Get)
		{
			rtd_pr_vpq_emerg("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara no get now\n");
			ret = -1;
		}
		else if (hal_VPQ_DATA_EXTERN->wId == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Set_Sub)
		{
			switch (hal_VPQ_DATA_EXTERN->version) {
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_Contrast:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(int)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
						...
						if(copy_from_user(&VPQ_byPara_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							...
							Scaler_SetContrast(SLR_SUB_DISPLAY, (unsigned char)VPQ_byPara_arg);
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara VPQ_byPara_arg=%d,\n", VPQ_byPara_arg);						
							ret = 0;
						}
					}
				}
				break;
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_Brightness:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(int)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
						...
						if(copy_from_user(&VPQ_byPara_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							...
							Scaler_SetBrightness(SLR_SUB_DISPLAY, (unsigned char)VPQ_byPara_arg);
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara VPQ_byPara_arg=%d,\n", VPQ_byPara_arg);						
							ret = 0;
						}
					}
				...
				break;
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_Hue:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(int)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
						...
						if(copy_from_user(&VPQ_byPara_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							...
							Scaler_SetHue(SLR_SUB_DISPLAY, (unsigned char)VPQ_byPara_arg);
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara VPQ_byPara_arg=%d,\n", VPQ_byPara_arg);						
							ret = 0;
						}
					}
				...
				break;
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_Saturation:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(int)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
						...
						if(copy_from_user(&VPQ_byPara_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							...
							Scaler_SetSaturation(SLR_SUB_DISPLAY, (unsigned char)VPQ_byPara_arg);
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara VPQ_byPara_arg=%d,\n", VPQ_byPara_arg);						
							ret = 0;
						}
					}
				...
				break;
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_StructColorDataFacMode_EXTERN:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(StructColorDataFacModeType)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
					} else {
						if(copy_from_user(&g_curColorFacTableEx_sub, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							rtd_pr_vpq_err("VPQ_EXTERN_IOC_ACCESS_VPQ_byPara table copy fail\n");
							ret = -1;
						} else {
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara FacTableEx_sub=%d,%d,%d,%d,%d,%d,%d,%d,\n", 
								g_curColorFacTableEx_sub.Brightness_0, g_curColorFacTableEx_sub.Brightness_100, 
								g_curColorFacTableEx_sub.Contrast_0, g_curColorFacTableEx_sub.Contrast_100, 
								g_curColorFacTableEx_sub.Saturation_0, g_curColorFacTableEx_sub.Saturation_100, 
								g_curColorFacTableEx_sub.Hue_0, g_curColorFacTableEx_sub.Hue_100);						
							ret = 0;
						}
					}
				}
				break;
				case VPQ_DEV_EXTERN_ACCESS_VPQ_byPara_Data_Range:
				{
					if (hal_VPQ_DATA_EXTERN->length != sizeof(int)) {
						rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara len error len=%d,\n", hal_VPQ_DATA_EXTERN->length);
						ret = -1;
						...
						if(copy_from_user(&VPQ_byPara_arg, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN->length))
						{
							...
							ret = Scaler_set_APDEM(DEM_ARG_Data_Range_sub, (void *)&VPQ_byPara_arg);
							rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara Data_Range_sub=%d,\n", VPQ_byPara_arg);						
							ret = 0;
						}
					}
				}
		...
		{
			rtd_pr_vpq_emerg("kernel VPQ_EXTERN_IOC_ACCESS_VPQ_byPara CMD error\n");
			ret = -1;
		}
	}
...
long vpqex_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	#endif
#ifndef BUILD_QUICK_SHOW	
	short RADCR_Pattern_BL_adj[11] = {0}; // Backlight_Offset_percentage for Pattern, -100~100
	short RADCR_Pattern_APL_th[11] = {0}; // APL th for Pattern
	short RADCR_Pattern_APL_th_range[11] = {0}; // APL th range for Pattern
	short RADCR_Pattern_Gain_adj_Offset[11] = {0}; // ColorTemp Gain Offset for Pattern
	short RADCR_Pattern_Offset_adj_Offset[11] = {0}; // ColorTemp Offset Offset for Pattern
#endif
 ...
 ************************************************************************/
...
#ifndef BUILD_QUICK_SHOW
			if ( (m_pVipTable->Auto_Function_Array3[0]=='I')&&
				(m_pVipTable->Auto_Function_Array3[1]=='D')&&
				(m_pVipTable->Auto_Function_Array3[2]=='C')&&
				(m_pVipTable->Auto_Function_Array3[3]=='H')&&
				(m_pVipTable->Auto_Function_Array3[4]=='P'))
			{
				
				RADCR_Pattern_APL_th[CHESS5p1] = 370;
				RADCR_Pattern_APL_th[CHESS5p2] = 130;
				RADCR_Pattern_APL_th_range[HDRCID_Target_Th] = 50;
				RADCR_Pattern_Gain_adj_Offset[CHESS5p1] = (((unsigned short)m_pVipTable->Auto_Function_Array3[5])<<8) + m_pVipTable->Auto_Function_Array3[6];
				RADCR_Pattern_Offset_adj_Offset[CHESS5p1] = (((unsigned short)m_pVipTable->Auto_Function_Array3[7])<<8) + m_pVipTable->Auto_Function_Array3[8]; 			
				RADCR_Pattern_Gain_adj_Offset[CHESS5p2] = RADCR_Pattern_Gain_adj_Offset[CHESS5p1];
				RADCR_Pattern_Offset_adj_Offset[CHESS5p2] = RADCR_Pattern_Offset_adj_Offset[CHESS5p2];
				RADCR_Pattern_APL_th_range[HDRCID_Contrast_Step] = (((unsigned short)m_pVipTable->Auto_Function_Array3[9])<<8) + m_pVipTable->Auto_Function_Array3[10];
				RADCR_Pattern_APL_th_range[HDRCID_Brightness_Step] = (((unsigned short)m_pVipTable->Auto_Function_Array3[11])<<8) + m_pVipTable->Auto_Function_Array3[12];
				RADCR_Pattern_APL_th_range[HDRCID_delay] = (((unsigned short)m_pVipTable->Auto_Function_Array3[13])<<8) + m_pVipTable->Auto_Function_Array3[14];

				Scaler_VIP_HDRCID_Table_update(1, &RADCR_Pattern_BL_adj[0], &RADCR_Pattern_APL_th[0], 
					&RADCR_Pattern_APL_th_range[0], &RADCR_Pattern_Gain_adj_Offset[0], &RADCR_Pattern_Offset_adj_Offset[0]);

				rtd_pr_vpq_info("HDRCID_TBL_update[2]=1, con=%d,bri=%d,step=%d,%d,delay=%d,\n", 
					RADCR_Pattern_Gain_adj_Offset[CHESS5p1],
					RADCR_Pattern_Offset_adj_Offset[CHESS5p1], RADCR_Pattern_APL_th_range[HDRCID_Contrast_Step], 
					RADCR_Pattern_APL_th_range[HDRCID_Brightness_Step], RADCR_Pattern_APL_th_range[HDRCID_delay]);
			}
#endif	
            Scaler_Copy_PQbin();
		}
		...
		case VPQ_EXTERN_IOC_GET_BRIGHTNESS:
		{
			...
			brightness = (unsigned int)Scaler_GetBrightness(SLR_MAIN_DISPLAY);
		...
		{			
			...
			if (copy_from_user(&args,  (int __user *)arg,sizeof(unsigned int))) {
				...
				ret = 0;
				Scaler_SetBrightness(SLR_MAIN_DISPLAY, (unsigned char)args);
				HSBC_Chg = true;
			}
		}
		break;
		case VPQ_EXTERN_IOC_GET_CONTRAST:
		{
			...
			contrast = (unsigned int)Scaler_GetContrast(SLR_MAIN_DISPLAY);
		...
		case VPQ_EXTERN_IOC_SET_CONTRAST:
		{
			...
			if (copy_from_user(&args,  (int __user *)arg,sizeof(unsigned int))) {
				...
				ret = 0;
				Scaler_SetContrast(SLR_MAIN_DISPLAY, (unsigned char)args);
				HSBC_Chg = true;
			}
		}
		break;
		case VPQ_EXTERN_IOC_GET_SATURATION:
		{
			...
			saturation = (unsigned int)Scaler_GetSaturation(SLR_MAIN_DISPLAY);
		...
		case VPQ_EXTERN_IOC_SET_SATURATION:
		{
			...
			if (copy_from_user(&args,  (int __user *)arg,sizeof(unsigned int))) {
				...
				ret = 0;
				Scaler_SetSaturation(SLR_MAIN_DISPLAY, (unsigned char)args);
				HSBC_Chg = true;
			}
		}
		break;
		case VPQ_EXTERN_IOC_GET_HUE:
		{
			...
			hue = (unsigned int)Scaler_GetHue(SLR_MAIN_DISPLAY);
		...
		case VPQ_EXTERN_IOC_SET_HUE:
		{
			...
			if (copy_from_user(&args,  (int __user *)arg,sizeof(unsigned int))) {
				...
				ret = 0;
				Scaler_SetHue(SLR_MAIN_DISPLAY, (unsigned char)args);
				HSBC_Chg = true;
			}
		}
		break;
			...
			if (copy_from_user(&PQModule_T,  (int __user *)arg,sizeof(RTK_PQModule_T))) {
				...
				if (PQModule_T.PQModule==TV001_PQ_MODULE_444MODE ||PQModule_T.PQModule==TV001_PQ_MODULE_RGB444MODE) {
					if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) {
						if ((drvif_Hdmi_Multi_port_GetColorSpace(Get_DisplayMode_Port(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY))) == COLOR_RGB && !is_dolby_vision_source(SLR_MAIN_DISPLAY)) && !Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)) {
						...
						if ((drvif_Dprx_GetColorSpace() == DP_COLOR_SPACE_RGB && !is_dolby_vision_source(SLR_MAIN_DISPLAY)) && !Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)) {
							Scaler_SetPQModule(TV001_PQ_MODULE_RGB444MODE,0);
						} else {
							Scaler_SetPQModule(TV001_PQ_MODULE_444MODE,0);
						}
					...
					if (hal_VPQ_DATA_EXTERN.version == VPQ_DEV_EXT_SET_AI_Para_TBL_set) 
					{
						if (hal_VPQ_DATA_EXTERN.length != sizeof(SLR_VIP_TABLE_AIPQ)) {
                            ...
                            if (sizeof(DRV_AI_Tune_ICM_table) != sizeof(SLR_AI_Tune_ICM_table)) {
                                ...
                                memcpy(&AI_Tune_ICM_TBL[0], &m_VIP_AIPQTable.Tune_ICM_table[0], sizeof(DRV_AI_Tune_ICM_table)*SLR_AI_TBL_NUM);
                            }
#ifdef AIPQ_2819a
                            // AI_Face_DCC
                            {
                                rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_SET_AI_Para AI_Face_DCC(%d)\n",sizeof(SLR_AI_Old_Skin_DCC));
                                for (i=0;i<SLR_AI_TBL_NUM;i++)
                                {
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio0 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_0;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio1 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_1;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio2 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_2;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio3 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_3;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio4 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_4;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio5 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_5;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio6 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_6;
                                    AI_Ctrl_TBL[i].ai_global2.dcc_uv_blend_ratio7 = m_VIP_AIPQTable.DCC_Old_Skin_table[i].uv_blend_7;
                                }
                            }
                            //SLR_AI_Shp_CDS AI_Shp_CDS_TBL[SLR_AI_TBL_NUM]
                            {
                                rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_SET_AI_Para AI_Shp_CDS_TBL(%d)\n",sizeof(SLR_AI_Shp_CDS));
                                for (i=0;i<SLR_AI_TBL_NUM;i++) {
                                    //34_0~34_3
                                    AI_Ctrl_TBL[i].ai_shp_tune.edg_gain_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].edg_gain_pos;
                                    AI_Ctrl_TBL[i].ai_shp_tune.tex_gain_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].tex_gain_pos;
                                    AI_Ctrl_TBL[i].ai_shp_tune.vpk_gain_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].vpk_gain_pos;
                                    AI_Ctrl_TBL[i].ai_shp_tune.vpk_edg_gain_level   = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].vpk_edg_gain_pos;
                                    //35_6~35_9
                                    AI_Ctrl_TBL[i].ai_shp_tune.edg_gain_neg_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].edg_gain_neg;
                                    AI_Ctrl_TBL[i].ai_shp_tune.tex_gain_neg_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].tex_gain_neg;
                                    AI_Ctrl_TBL[i].ai_shp_tune.vpk_gain_neg_level       = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].vpk_gain_neg;
                                    AI_Ctrl_TBL[i].ai_shp_tune.vpk_edg_gain_neg_level   = m_VIP_AIPQTable.AI_Shp_CDS_TBL[i].vpk_edg_gain_neg;					
                                }
                            }								
                            bBinAIPQ_Table = 1;
                            }
						...
						if (hal_VPQ_DATA_EXTERN.length != sizeof(int)) {
							...
							if(copy_from_user(&AP_mode, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN.length))
							{
								...
								if ((AP_mode & AI_DEPTH_DETECT & (vpq_get_aipq_support_model_list())) != 0){
									if(aipq_mode.depth_mode<1) aipq_mode.depth_mode = 1;
								}else
								...
								if ((AP_mode & AI_SEMATIC_DETECT & (vpq_get_aipq_support_model_list())) != 0){
									if(aipq_mode.semantic_mode<1) aipq_mode.semantic_mode = 1;
								}else
		...
		case VPQ_EXTERN_IOC_ACCESS_VPQ_byPara:
		{
			if (copy_from_user(&hal_VPQ_DATA_EXTERN, (int __user *)arg, sizeof(HAL_VPQ_DATA_EXTERN_T))) {
				...
				ret = VPQEX_VPQ_EXTERN_IOC_ACCESS_VPQ_byPara(&hal_VPQ_DATA_EXTERN);
			}
