...
#include <rbus/uvc_rgb2yuv_reg.h>
...
void UVC_color_colorspacergb2yuvtransfer(DISPD_CAP_SRC idmaDispD_in_sel)
{
#if 0// different register set format , stop
	unsigned short *table_index = 0;
	uvc_rgb2yuv_uvc_rgb2yuv_ctrl_RBUS  i3ddma_rgb2yuv_ctrl_reg;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_m11_m12_RBUS tab2_M11_M12_REG;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_m13_m21_RBUS tab2_M13_M21_REG;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_m22_m23_RBUS tab2_M22_M23_REG;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_m31_m32_RBUS tab2_M31_M32_REG;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_m33_ygain_RBUS tab2_M33_YGain_REG;
	uvc_rgb2yuv_uvc_rgb2yuv_tab_yo_RBUS tab2_Yo_REG;
	...
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_M11_M12_reg, tab2_M11_M12_REG.regValue);

	tab2_M13_M21_REG.m13 = table_index [_RGB2YUV_m13];
	tab2_M13_M21_REG.m21 = table_index [_RGB2YUV_m21];
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_M13_M21_reg, tab2_M13_M21_REG.regValue);
	...
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_M22_M23_reg, tab2_M22_M23_REG.regValue);

	tab2_M31_M32_REG.m31 = table_index [_RGB2YUV_m31];
	tab2_M31_M32_REG.m32 = table_index [_RGB2YUV_m32];
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_M31_M32_reg, tab2_M31_M32_REG.regValue);
	...
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_M33_YGain_reg, tab2_M33_YGain_REG.regValue);

	tab2_Yo_REG.yo_odd= table_index [_RGB2YUV_Yo_odd];
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_TAB_Yo_reg, tab2_Yo_REG.regValue);

	i3ddma_rgb2yuv_ctrl_reg.regValue = IoReg_Read32(UVC_RGB2YUV_UVC_RGB2YUV_CTRL_reg);
	...
	IoReg_Write32(UVC_RGB2YUV_UVC_RGB2YUV_CTRL_reg, i3ddma_rgb2yuv_ctrl_reg.regValue);
#endif
	return;
}

void UVC_ultrazoom_auto_four_tap(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = IoReg_Read32( UVC_UZD_UVC_UZD_CTRL1_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , UVC_UZD_UVC_UZD_CTRL1_auto_four_tap_en , 1 );
	IoReg_Write32( UVC_UZD_UVC_UZD_CTRL1_reg , z_RBUS_reg );
}

void UVC_ultrazoom_coef_db_apply(void)
{
	unsigned int z_RBUS_reg;
	z_RBUS_reg = IoReg_Read32( UVC_UZD_UVC_UZD_COEF_DB_CTRL_reg );
	VPQ_SET_REG_FIELD( z_RBUS_reg , UVC_UZD_UVC_UZD_COEF_DB_CTRL_coef_db_apply , 1 );
	IoReg_Write32( UVC_UZD_UVC_UZD_COEF_DB_CTRL_reg , z_RBUS_reg );
}
 ...
 */
void UVC_ultrazoom_set_scale_down(SIZE* in_Size, SIZE* out_Size, unsigned char panorama)
{
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    uvc_uzd_uvc_uzd_scale_hor_factor_RBUS	uvc_uzd_uvc_uzd_scale_hor_factor_reg;
    uvc_uzd_uvc_uzd_scale_ver_factor_RBUS   	uvc_uzd_uvc_uzd_scale_ver_factor_reg;
    uvc_uzd_uvc_uzd_initial_value_RBUS		uvc_uzd_uvc_uzd_initial_value_reg;
    uvc_uzd_uvc_uzd_initial_int_value_RBUS		uvc_uzd_uvc_uzd_initial_int_value_reg;
    uvc_uzd_uvc_uzd_ibuff_ctrl_RBUS		uvc_uzd_uvc_uzd_ibuff_ctrl_reg;
	
    uvc_uzd_uvc_uzd_ctrl0_reg.regValue		= IoReg_Read32(UVC_UZD_UVC_UZD_CTRL0_reg);
    uvc_uzd_uvc_uzd_scale_hor_factor_reg.regValue	= IoReg_Read32(UVC_UZD_UVC_UZD_SCALE_HOR_FACTOR_reg);
    uvc_uzd_uvc_uzd_scale_ver_factor_reg.regValue = IoReg_Read32(UVC_UZD_UVC_UZD_SCALE_VER_FACTOR_reg);
   // i3ddma_hsd_hor_delta1_reg.regValue	= IoReg_Read32(UVC_UZD_UVC_UZD_HSD_Hor_Delta1_reg);
    uvc_uzd_uvc_uzd_initial_value_reg.regValue	= IoReg_Read32(UVC_UZD_UVC_UZD_INITIAL_VALUE_reg);
    uvc_uzd_uvc_uzd_initial_int_value_reg.regValue = IoReg_Read32(UVC_UZD_UVC_UZD_INITIAL_INT_VALUE_reg);
    uvc_uzd_uvc_uzd_ibuff_ctrl_reg.regValue = IoReg_Read32(UVC_UZD_UVC_UZD_IBUFF_CTRL_reg);
    ...
    if (in_Size->nWidth > out_Size->nWidth) {
        ...
        uvc_uzd_uvc_uzd_ctrl0_reg.h_y_table_sel = 0;	// TAB1
        uvc_uzd_uvc_uzd_ctrl0_reg.h_c_table_sel = 0;	// TAB1

        for (i=0; i<16; i++)
        {
            tmp_data = ((unsigned int)(*coef_pt++)<<16);
            tmp_data += (unsigned int)(*coef_pt++);
            IoReg_Write32(UVC_UZD_UVC_UZD_FIR_COEF_TAB1_C0_reg+ i*4, tmp_data);
        }
    }
	...
	if (in_Size->nLength > out_Size->nLength) {
		...
		uvc_uzd_uvc_uzd_ctrl0_reg.v_y_table_sel = 1;	// TAB2
		uvc_uzd_uvc_uzd_ctrl0_reg.v_c_table_sel = 1;	// TAB2

		for (i=0; i<16; i++)
		{
			tmp_data = ((unsigned int)(*coef_pt++)<<16);
			tmp_data += (unsigned int)(*coef_pt++);
			IoReg_Write32(UVC_UZD_UVC_UZD_FIR_COEF_TAB2_C0_reg+ i*4, tmp_data);
		}

	}
	UVC_ultrazoom_coef_db_apply();

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

	if (in_Size->nWidth > out_Size->nWidth) {    // H scale-down
		Hini = 0x80;// actual phase is 0x180;
		uvc_uzd_uvc_uzd_initial_value_reg.hor_ini = Hini;
		uvc_uzd_uvc_uzd_initial_int_value_reg.hor_ini_int = 0;
		{
		...
		uvc_uzd_uvc_uzd_scale_hor_factor_reg.hor_fac = nFactor;
	}else {
		uvc_uzd_uvc_uzd_scale_hor_factor_reg.hor_fac = 0x100000;
	}
	uvc_uzd_uvc_uzd_ibuff_ctrl_reg.ibuf_h_size = out_Size->nWidth;
	uvc_uzd_uvc_uzd_ibuff_ctrl_reg.ibuf_v_size = in_Size->nLength;

	if (in_Size->nLength > out_Size->nLength) {    // V scale-down
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0x02;
		Vini = 0x80;// actual 0x180;
		uvc_uzd_uvc_uzd_initial_value_reg.ver_ini = Vini;
		uvc_uzd_uvc_uzd_initial_int_value_reg.ver_ini_int = 0;
		...
		//nFactor = SHR(nFactor+1, 1); //rounding
		uvc_uzd_uvc_uzd_scale_ver_factor_reg.ver_fac = nFactor;
	}else{
//			ich2_uzd_Ctrl0_REG.buffer_mode = 0;
		uvc_uzd_uvc_uzd_scale_ver_factor_reg.ver_fac = 0x100000;
	}

//	ich2_uzd_Ctrl0_REG.output_fmt = 1;
//	CLR_422_Fr_SD();
	UVC_ultrazoom_auto_four_tap();

	IoReg_Write32(UVC_UZD_UVC_UZD_CTRL0_reg, uvc_uzd_uvc_uzd_ctrl0_reg.regValue);
	IoReg_Write32(UVC_UZD_UVC_UZD_SCALE_HOR_FACTOR_reg, uvc_uzd_uvc_uzd_scale_hor_factor_reg.regValue);
	IoReg_Write32(UVC_UZD_UVC_UZD_SCALE_VER_FACTOR_reg, uvc_uzd_uvc_uzd_scale_ver_factor_reg.regValue);
	IoReg_Write32(UVC_UZD_UVC_UZD_INITIAL_VALUE_reg, uvc_uzd_uvc_uzd_initial_value_reg.regValue);
	IoReg_Write32(UVC_UZD_UVC_UZD_INITIAL_INT_VALUE_reg, uvc_uzd_uvc_uzd_initial_int_value_reg.regValue);
	IoReg_Write32(UVC_UZD_UVC_UZD_IBUFF_CTRL_reg, uvc_uzd_uvc_uzd_ibuff_ctrl_reg.regValue);

	return;
}

void UVC_ultrazoom_config_scaling_down(SIZE* in_Size, SIZE* out_Size, unsigned char panorama)
{
#ifdef CONFIG_ENABLE_HDMI3_0_NN
	rtd_pr_i3ddma_emerg("Don't do UVC_ultrazoom_config_scaling_down\n");
#else
	// Scale down setup for Channel1
	uvc_uzd_uvc_uzd_ctrl0_RBUS uvc_uzd_uvc_uzd_ctrl0_reg;
	
	uvc_uzd_uvc_uzd_ctrl0_reg.regValue = IoReg_Read32(UVC_UZD_UVC_UZD_CTRL0_reg);
	uvc_uzd_uvc_uzd_ctrl0_reg.h_zoom_en = (in_Size->nWidth > out_Size->nWidth);
	
	uvc_uzd_uvc_uzd_ctrl0_reg.v_zoom_en = (in_Size->nLength > out_Size->nLength);

    //uvc cannot use scaler sub value
	//uvc_uzd_uvc_uzd_ctrl0_reg.sort_fmt = (Scaler_DispGetStatus(SLR_SUB_DISPLAY,SLR_DISP_422CAP) ? 0 : 1);
	//uvc_uzd_uvc_uzd_ctrl0_reg.video_comp_en = (Scaler_DispGetStatus(SLR_SUB_DISPLAY,SLR_DISP_COMP) ? 1 : 0 );
	//uvc_uzd_uvc_uzd_ctrl0_reg.truncationctrl = (Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_10BIT)? 0: 1);
	IoReg_Write32(UVC_UZD_UVC_UZD_CTRL0_reg, uvc_uzd_uvc_uzd_ctrl0_reg.regValue);

	UVC_ultrazoom_set_scale_down(in_Size, out_Size, panorama);
#endif
	return;
}
