...
struct semaphore VPQ_DM2_EOTF_1D_Buff_Semaphore;
struct semaphore VPQ_DM2_OETF_Y2Gain_1D_Buff_Semaphore;
...
// -----------------------------------------------------
#ifdef K25LP_PQ_BringUp_Compile
...
extern int scalerAI_config(void);
...
void vpq_init_gamma_invgamma(void)
{
	...
	fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 0);
...
void vpq_boot_init(void)
{
	...
	if(is_QS_pq_enable()==1) { //current is QS flow pq_quick_show flow
...
#endif
		{
			if (Get_DISPLAY_PANEL_MPLUS_RGBW() == TRUE || Get_DISPLAY_PANEL_BOW_RGBW() == TRUE)
				...
				fwif_color_set_Panel_Dither(TRUE,VIP_PANEL_BIT10,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15,dual_panel,nWidth,nLength);
		}
		/* ST2094 Ctrl ini*/
	...
	/* It is for workaround End  */
		...
		Scaler_MEMC_register_MEMC_thermal_handler();
#endif
#endif		
...
/*==============================================*/
		...
		Scaler_SetSharpnessTable(SLR_MAIN_DISPLAY, 255);
...
void vpq_do_suspend(void)
{
	*((unsigned char *)Scaler_GetShareMemVirAddr(SCALERIOC_SETAUTOMAFLAG)) = 0;
	...
	pcid_do_suspend();
	//scalerPQMask_exit(); // ml9 hw not support
...
static int se_tsk(void *p)//This task run se proc
{
	...
	while (1)
	{
		...
		// end lesley 0920
		//scalerPQMask_postprocessing_entry(); // ml9 hw not support
		...
		if((vgip_isr_cnt != vgip_isr_cnt_pre))
		{
			...
			if(od_ctrl_reg.dummy1802ca00_31_4>>27 & 1) nn_en = 1; // bit 31
#else
			nn_en=1;
#endif
	...
	*/
	...
	ret = 0;
		...
		if (copy_from_user(&tShp_Val, (int __user *)arg, sizeof(UINT16)*7)) {
			...
			fwif_color_set_sharpness_level(Scaler_GetSharpnessTable(SLR_MAIN_DISPLAY),level_tmp);
	...
	case VPQ_IOC_SET_3D_COLOR_CORRECTION:
		{
			...
			*/
			...
			if (copy_from_user(&args, (int __user *)arg, sizeof(unsigned int))) {
				...
				fwif_color_set_hist_3d_en(bForceHist3DCtrl);//9Q remove
				ret = 0;
			}
...
*/
	...
	case VPQ_IOC_SET_DITHER_BIT:
		{
			unsigned int args,nWidth=0,nLength=0;
			unsigned char dual_panel=0;
			if (copy_from_user(&args, (int __user *)arg, sizeof(unsigned int))) {
				rtd_pr_vpq_err("kernel copy VPQ_IOC_SET_PIC_CTRL fail\n");
				ret = -1;
			} else {
				fwif_color_set_Panel_Dither(TRUE,args,VIP_DITHER_PANELTABLE_BLANCE_ROLL_15,dual_panel,nWidth,nLength);
				ret = 0;
			}
		}
		break;
...
/*========================= for INNX demura =================================*/
	...
	case VPQ_IOC_AI_DETECTION_ON:
	{
		...
		if (list & AI_SCENE_DETECT & aipq_support_model_list)
			scalerAI_pq_scene_mode_ctrl_tv001(1);
		if (list & AI_FACE_DETECT & aipq_support_model_list)
			/*scalerAI_pq_mode_ctrl_tv001(1, Scaler_GetDCC_Mode())*/; // contrl ap mode in scaler vpq extern "VPQ_EXTERN_IOC_SET_AI_Para"
		if (list & AI_SQM_DETECT & aipq_support_model_list)
			scalerAI_pq_sqm_mode_ctrl_tv001(1);
		if (list & AI_DEPTH_DETECT & aipq_support_model_list)
			scalerAI_pq_depth_mode_ctrl_tv001(1);
		if (list & AI_NOISE_DETECT & aipq_support_model_list)
	...
	case VPQ_IOC_AI_DETECTION_OFF:
	{
		...
		if (list & AI_FACE_DETECT & aipq_support_model_list)
			/*scalerAI_pq_mode_ctrl_tv001(0, 0)*/;		// contrl ap mode in scaler vpq extern "VPQ_EXTERN_IOC_SET_AI_Para"
		if (list & AI_SQM_DETECT & aipq_support_model_list)
                ...
                case V4L2_CID_EXT_VPQ_SHARPNESS:
                {
                        ...
                        else
                        {
                                ...
                                if(sharpness.pst_chip_data ==NULL) //LG DB NULL, workaround
                                {
                                        ...
                                        fwif_color_set_sharpness_table(SLR_MAIN_DISPLAY, 0, idx);//fixed me after demo please remove
				...
				else
				{
					...
					if (vip_sysInfo->DolbyHDR_flag == 0) {
						if (((v4l2_blockbypass.bypassMask & V4L2_VPQ_BYPASS_MASK_HDR_EXCEPT_PCC_RTK) != 0) && (v4l2_blockbypass.bOnOff == 1)) {
							...
							Scaler_color_HDRIP_AutoRun(SLR_MAIN_DISPLAY, hdrtype_vsc);
						}
					}
				}
			}
...
int vpq_module_init(void)
{
	...
	sema_init(&VPQ_DM2_EOTF_1D_Buff_Semaphore, 1);	
	sema_init(&VPQ_DM2_OETF_Y2Gain_1D_Buff_Semaphore, 1);
