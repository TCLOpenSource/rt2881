...
void newbase_hdmi_port_var_init(unsigned char port, unsigned char full_reset)
{
	...
	if (full_reset == 0)//backup
	{
		...
		hdmi_2p1_fw_pol_check_cnt_backup = hdmi_rx[port].hdmi_2p1_fw_pol_check_cnt;
	}
	...
	if (full_reset == 0)//store variable
	{
		...
		hdmi_rx[port].hdmi_2p1_fw_pol_check_cnt = hdmi_2p1_fw_pol_check_cnt_backup;
	}
...
void newbase_hdmi_setup_yuv420_after_measure(unsigned char nport, unsigned int pixel_clock, MEASURE_TIMING_T* tm )
{
    ...
    if(pixel_clock<= HDMI_CLK_GEN_PIXEL_MODE_THD || (lib_hdmi_is_420_over_600m_1p_mode() == 1))
    {
...
void newbase_hdmi_set_video_common_sel(unsigned char port)
{
	...
	else if(timing_pixel_clock<= HDMI_CLK_GEN_PIXEL_MODE_THD || (lib_hdmi_is_420_over_600m_1p_mode() == 1))
...
unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	...
	if(lib_hdmi_is_420_over_600m_1p_mode() == 1)
		hdmi_rx[port].timing_t.pixel_mode = PIXEL_MODE_SUPPORT_420_OVER_600M;
	...
	if(measure_result > MEASURE_SUCCESS_NUM)
	{
		...
		{// Non VRR, no h sync
			...
			if((GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_DEMODE_USE_VIC_CORRECTION) == 1) && (is_get_vic != 0) && (select_vic != 0))
			{//use VIC
