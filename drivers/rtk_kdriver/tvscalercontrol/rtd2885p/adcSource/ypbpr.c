...
unsigned char YPbPr_DetectTiming(void)
{
	...
	if (YPbPr_TimingDetectStage == YPBPR_SOY_STATE) {/* Around 90ms */
		...
		if (ret != _MODE_SUCCESS) {
			...
			rtd_pr_adc_notice("IHFreq:%d,IVFreq_1000:%d\n", p_timinginfo->IHFreq, p_timinginfo->IVFreq_1000);
			...
			rtd_pr_adc_notice("**IHFreq=%d,IVFreq_1000=%d,IHTotal=%d,IVTotal=%d,ADC_CLOCK:%d\n",
				p_dispinfo->IHFreq,
