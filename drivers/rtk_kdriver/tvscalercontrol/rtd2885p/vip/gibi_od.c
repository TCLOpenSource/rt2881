...
void drvif_color_od_localgain_set(DRV_od_localgain od_localgain){
	...
	if(get_mach_type()==MACH_ARCH_RTK2885P){
	    ...
	    M8P_od_od_local_gain_RBUS od_od_local_gain_reg;
		M8P_od_od_local_edge_gain_0_RBUS od_od_local_edge_gain_0_reg;
		M8P_od_od_local_edge_gain_1_RBUS od_od_local_edge_gain_1_reg;

		od_od_local_gain_reg.regValue = IoReg_Read32(M8P_OD_OD_local_gain_reg);
		od_od_local_gain_reg.localgain_enable = od_localgain.enable;
		od_od_local_gain_reg.localgain_weight = od_localgain.weight;
		od_od_local_gain_reg.localgain_nr_thd = od_localgain.nr_thd;
		od_od_local_gain_reg.localgain_hhf_thd= od_localgain.edge_hhf_thd;

		od_od_local_edge_gain_0_reg.regValue = IoReg_Read32(M8P_OD_OD_local_edge_gain_0_reg);
		od_od_local_edge_gain_1_reg.regValue = IoReg_Read32(M8P_OD_OD_local_edge_gain_1_reg);
		/* TDB
		...
		*/
		IoReg_Write32(M8P_OD_OD_local_gain_reg, od_od_local_gain_reg.regValue);
		IoReg_Write32(M8P_OD_OD_local_edge_gain_0_reg, od_od_local_edge_gain_0_reg.regValue);
		IoReg_Write32(M8P_OD_OD_local_edge_gain_1_reg, od_od_local_edge_gain_1_reg.regValue);
...
void drvif_color_od_dma_addr_set(DRV_od_dma *oddma)
{
	...
	if(get_mach_type()==MACH_ARCH_RTK2885P){
	    ...
	    // merlin8p IC, run merlin8 flow
	    IoReg_Write32(M8P_OD_OD_status_check_up_reg, oddma->dma_pair.up_limit_lut_data);
		IoReg_Write32(M8P_OD_OD_status_check_low_reg, oddma->dma_pair.low_limit_lut_data);
	}else if(get_mach_type()==MACH_ARCH_RTK2885PP){
	    // merlin8p IC , run merlin8p flow
	    IoReg_Write32(M8P_OD_OD_status_check_up_reg, oddma->dma_pair.up_limit_lut_data);
		IoReg_Write32(M8P_OD_OD_status_check_low_reg, oddma->dma_pair.low_limit_lut_data);
...
void drvif_color_od_dma_pqc_bit_set(DRV_od_dma * oddma)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P){
	    ...
	    // merlin8p IC, run merlin8 flow
	    M8P_od_dma_oddma_pq_cmp_bit_RBUS od_dma_oddma_pq_cmp_bit_reg;
	
		od_dma_oddma_pq_cmp_bit_reg.regValue = IoReg_Read32(M8P_OD_DMA_ODDMA_PQ_CMP_BIT_reg);
		od_dma_oddma_pq_cmp_bit_reg.first_line_more_bit = oddma->dma_comp.line_more_bit;
		od_dma_oddma_pq_cmp_bit_reg.block_limit_bit = oddma->dma_comp.block_limit_bit;
		od_dma_oddma_pq_cmp_bit_reg.frame_limit_bit = oddma->dma_comp.frame_limit_bit;
		IoReg_Write32(M8P_OD_DMA_ODDMA_PQ_CMP_BIT_reg, od_dma_oddma_pq_cmp_bit_reg.regValue);
...
void drvif_color_set_od_bits(unsigned char FrameLimitBit)
{
	...
	oddma_pq_cmp_bit_reg.regValue = IoReg_Read32(OD_DMA_ODDMA_PQ_CMP_BIT_reg);
        ...
        if (/*(Get_DISPLAY_REFRESH_RATE()!=60)*/(get_panel_pixel_mode() > PANEL_1_PIXEL)){ //120hz panel
        ...
        else if (/*(Get_DISPLAY_REFRESH_RATE()==60)*/(get_panel_pixel_mode() == PANEL_1_PIXEL)){
	...
	if (FrameLimitBit > 0) {
		// pqc hw performance limitation
                if (/*Get_DISPLAY_REFRESH_RATE() >= 120*/ (get_panel_pixel_mode() > PANEL_1_PIXEL) && FrameLimitBit < 8){
                        FrameLimitBit = 8;
                }
                else if (/*(Get_DISPLAY_REFRESH_RATE()==60)*/ (get_panel_pixel_mode() == PANEL_1_PIXEL) && FrameLimitBit < 2){
                        FrameLimitBit = 2;
                }
