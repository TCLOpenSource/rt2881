...
void drvif_color_set_LD2LDSPI_Enable(unsigned char enable)
{
	...
	blu_ld_backlight_ctrl1.ld_blu_spien = enable;
...
#else 
	h_s = Get_DISP_DEN_STA_HPOS();
	h_e = Get_DISP_DEN_END_HPOS();
	v_s = Get_DISP_DEN_STA_VPOS();
	v_e = Get_DISP_DEN_END_VPOS();	
#endif
...
void drvif_color_get_LD_Global_Ctrl(DRV_LD_Global_Ctrl *ptr)
{
	...
	ptr->ld_comp_en = blu_ld_global_ctrl0.ld_comp_en;
}
...
void drvif_color_set_LD_Spatial_Filter(DRV_LD_Spatial_Filter *ptr)
{
	...
	*/
	blu_ld_spatial_ctrl5.spatialcoef_div = ptr->ld_spatialcoef_div;/*add SpatialCoef_div*/
...
void drvif_color_get_LD_Spatial_Filter(DRV_LD_Spatial_Filter *ptr)
{
	...
	*/
	ptr->ld_spatialcoef_div = blu_ld_spatial_ctrl5.spatialcoef_div;/*add SpatialCoef_div*/
...
void drvif_color_set_LD_Global_BV_Gain(DRV_LD_Global_Gain *ptr)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P){
	    ...
	    M8P_blu_ld_global_gain_RBUS blu_ld_global_gain;

		blu_ld_global_gain.regValue = IoReg_Read32(M8P_BLU_LD_GLOBAL_GAIN_reg);

		blu_ld_global_gain.ld_global_bvgain_en = ptr->ld_global_bvgain_en;
		blu_ld_global_gain.ld_global_bvgain = ptr->ld_global_bvgain;

		IoReg_Write32(M8P_BLU_LD_GLOBAL_GAIN_reg, blu_ld_global_gain.regValue);
	}
...
void drvif_color_set_LD_Spatial_Remap2(DRV_LD_Spatial_Remap2 *ptr)
{
	...
	for (i = 0; i < 32; i++) {
		blu_ld_spatial_ctrlc.ld_spatialremapadr = 96 + i;
		IoReg_Write32(BLU_LD_Spatial_CtrlC_reg, blu_ld_spatial_ctrlc.regValue);
		blu_ld_spatial_ctrl3.ld_spatialremaptab0 = (ptr->ld_spatialremaptab2[2*i+1] > 1023) ? 1023 : ptr->ld_spatialremaptab2[2*i+1];
		blu_ld_spatial_ctrl3.ld_spatialremaptab1 = (ptr->ld_spatialremaptab2[2*i+2] > 1023) ? 1023 : ptr->ld_spatialremaptab2[2*i+2];
		IoReg_Write32(BLU_LD_Spatial_Ctrl3_reg, blu_ld_spatial_ctrl3.regValue);
...
void drvif_color_set_LD_Pixel_Bv_Gain_Offset(DRV_LD_Bl_Pixel_Bv_Gain_Offset *ptr)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P){
	    ...
	    // ml8p IC , run ml8p flow
	    M8P_blu_ld_pixel_bv_gain_RBUS blu_ld_pixel_bv_gain;

		blu_ld_pixel_bv_gain.regValue = IoReg_Read32(M8P_BLU_LD_PIXEL_BV_GAIN_reg);
...
void drvif_color_set_LD_Spatial_Remap(DRV_LD_Spatial_Remap *ptr)
{
	...
	for (i = 0; i < 32; i++) {
		blu_ld_spatial_ctrlc.ld_spatialremapadr = 64 + i;
		IoReg_Write32(BLU_LD_Spatial_CtrlC_reg, blu_ld_spatial_ctrlc.regValue);
		blu_ld_spatial_ctrl3.ld_spatialremaptab0 = (ptr->ld_spatialremaptab[2*i+1] > 1023) ? 1023 : ptr->ld_spatialremaptab[2*i+1];
		blu_ld_spatial_ctrl3.ld_spatialremaptab1 = (ptr->ld_spatialremaptab[2*i+2] > 1023) ? 1023 : ptr->ld_spatialremaptab[2*i+2];
		IoReg_Write32(BLU_LD_Spatial_Ctrl3_reg, blu_ld_spatial_ctrl3.regValue);
...
void drvif_color_set_LD_Boost(DRV_LD_Boost *ptr)
{
	...
	blu_ld_spatial_ctrlb.ld_boostcurve_initial= ptr->ld_boost_curve_lut[0];
	IoReg_Write32(BLU_LD_Spatial_Ctrl4_reg, blu_ld_spatial_ctrl4.regValue);
	IoReg_Write32(BLU_LD_Spatial_CtrlB_reg, blu_ld_spatial_ctrlb.regValue);
...
void drvif_color_set_LD_Backlight_Final_Decision(DRV_LD_Backlight_Final_Decision *ptr)
{
	...
	blu_ld_backlight_ctrl1.ld_spi_out_num = ptr->ld_spi_out_num;
	blu_ld_backlight_ctrl1.ld_spatial_throughput = ptr->ld_spatial_throughput;
...
void drvif_color_get_LD_Backlight_Final_Decision(DRV_LD_Backlight_Final_Decision *ptr)
{
	...
	ptr->ld_spi_out_num = blu_ld_backlight_ctrl1.ld_spi_out_num;
	ptr->ld_spatial_throughput = blu_ld_backlight_ctrl1.ld_spatial_throughput;
...
void drvif_color_get_LD_Backlight_Profile_Interpolation(DRV_LD_Backlight_Profile_Interpolation *ptr)
{
	...
	ptr->ld_tab_hsize = blu_ld_inter_hctrl1.ld_tab_hsize;
	ptr->ld_hfactor = blu_ld_inter_hctrl1.ld_hfactor;
	ptr->ld_hboundary = blu_ld_inter_hctrl2.ld_hboundary;
	ptr->ld_hinitphase1 = blu_ld_inter_hctrl2.ld_hinitphase1;
	ptr->ld_hinitphase2 = blu_ld_inter_hctrl2.ld_hinitphase2;
	ptr->ld_hinitphase3_left = blu_ld_inter_hctrl2.ld_hinitphase3_left;
	ptr->ld_tab_vsize = blu_ld_inter_vctrl1.ld_tab_vsize;
	ptr->ld_vfactor = blu_ld_inter_vctrl1.ld_vfactor;
	ptr->ld_vboundary = blu_ld_inter_vctrl2.ld_vboundary;
	ptr->ld_vinitphase1 = blu_ld_inter_vctrl2.ld_vinitphase1;
	ptr->ld_vinitphase2 = blu_ld_inter_vctrl2.ld_vinitphase2;
	ptr->ld_vinitphase3_left = blu_ld_inter_vctrl2.ld_vinitphase3_left;
		
	ptr->ld_blight_update_en = blu_ld_sram_ctrl.ld_blight_update_en;
	ptr->ld_blight_sw_mode = blu_ld_sram_ctrl.ld_blight_sw_mode;
		
		
	ptr->ld_htabsel_0 = blu_ld_inter_htabsel1.ld_htabsel_0;
	ptr->ld_htabsel_1 = blu_ld_inter_htabsel1.ld_htabsel_1;
	ptr->ld_htabsel_2 = blu_ld_inter_htabsel1.ld_htabsel_2;
	ptr->ld_htabsel_3 = blu_ld_inter_htabsel1.ld_htabsel_3;
	ptr->ld_htabsel_4 = blu_ld_inter_htabsel2.ld_htabsel_4;
	ptr->ld_htabsel_5 = blu_ld_inter_htabsel2.ld_htabsel_5;
	ptr->ld_htabsel_6 = blu_ld_inter_htabsel2.ld_htabsel_6;
	ptr->ld_htabsel_7 = blu_ld_inter_htabsel2.ld_htabsel_7;
	ptr->ld_htabsel_8 = blu_ld_inter_htabsel3.ld_htabsel_8;
	ptr->ld_htabsel_9 = blu_ld_inter_htabsel3.ld_htabsel_9;
	ptr->ld_htabsel_10 = blu_ld_inter_htabsel3.ld_htabsel_10;
	ptr->ld_htabsel_11 = blu_ld_inter_htabsel3.ld_htabsel_11;
	ptr->ld_htabsel_12 = blu_ld_inter_htabsel4.ld_htabsel_12;
	ptr->ld_htabsel_13 = blu_ld_inter_htabsel4.ld_htabsel_13;
	ptr->ld_htabsel_14 = blu_ld_inter_htabsel4.ld_htabsel_14;
	ptr->ld_htabsel_15 = blu_ld_inter_htabsel4.ld_htabsel_15;
	ptr->ld_htabsel_16 = blu_ld_inter_htabsel5.ld_htabsel_16;
	ptr->ld_htabsel_17 = blu_ld_inter_htabsel5.ld_htabsel_17;
	ptr->ld_htabsel_18 = blu_ld_inter_htabsel5.ld_htabsel_18;
	ptr->ld_htabsel_19 = blu_ld_inter_htabsel5.ld_htabsel_19;
	ptr->ld_htabsel_20 = blu_ld_inter_htabsel6.ld_htabsel_20;
	ptr->ld_htabsel_21 = blu_ld_inter_htabsel6.ld_htabsel_21;
	ptr->ld_htabsel_22 = blu_ld_inter_htabsel6.ld_htabsel_22;
	ptr->ld_htabsel_23 = blu_ld_inter_htabsel6.ld_htabsel_23;
	ptr->ld_htabsel_24 = blu_ld_inter_htabsel7.ld_htabsel_24;
	ptr->ld_htabsel_25 = blu_ld_inter_htabsel7.ld_htabsel_25;
	ptr->ld_htabsel_26 = blu_ld_inter_htabsel7.ld_htabsel_26;
	ptr->ld_htabsel_27 = blu_ld_inter_htabsel7.ld_htabsel_27;
	ptr->ld_htabsel_28 = blu_ld_inter_htabsel8.ld_htabsel_28;
	ptr->ld_htabsel_29 = blu_ld_inter_htabsel8.ld_htabsel_29;
	ptr->ld_htabsel_30 = blu_ld_inter_htabsel8.ld_htabsel_30;
	ptr->ld_htabsel_31 = blu_ld_inter_htabsel8.ld_htabsel_31;
	ptr->ld_vtabsel_0 = blu_ld_inter_vtabsel1.ld_vtabsel_0;
	ptr->ld_vtabsel_1 = blu_ld_inter_vtabsel1.ld_vtabsel_1;
	ptr->ld_vtabsel_2 = blu_ld_inter_vtabsel1.ld_vtabsel_2;
	ptr->ld_vtabsel_3 = blu_ld_inter_vtabsel1.ld_vtabsel_3;
	ptr->ld_vtabsel_4 = blu_ld_inter_vtabsel2.ld_vtabsel_4;
	ptr->ld_vtabsel_5 = blu_ld_inter_vtabsel2.ld_vtabsel_5;
	ptr->ld_vtabsel_6 = blu_ld_inter_vtabsel2.ld_vtabsel_6;
	ptr->ld_vtabsel_7 = blu_ld_inter_vtabsel2.ld_vtabsel_7;
	ptr->ld_vtabsel_8 = blu_ld_inter_vtabsel3.ld_vtabsel_8;
	ptr->ld_vtabsel_9 = blu_ld_inter_vtabsel3.ld_vtabsel_9;
	ptr->ld_vtabsel_10 = blu_ld_inter_vtabsel3.ld_vtabsel_10;
	ptr->ld_vtabsel_11 = blu_ld_inter_vtabsel3.ld_vtabsel_11;
	ptr->ld_vtabsel_12 = blu_ld_inter_vtabsel4.ld_vtabsel_12;
	ptr->ld_vtabsel_13 = blu_ld_inter_vtabsel4.ld_vtabsel_13;
	ptr->ld_vtabsel_14 = blu_ld_inter_vtabsel4.ld_vtabsel_14;
	ptr->ld_vtabsel_15 = blu_ld_inter_vtabsel4.ld_vtabsel_15;
	ptr->ld_vtabsel_16 = blu_ld_inter_vtabsel5.ld_vtabsel_16;
	ptr->ld_vtabsel_17 = blu_ld_inter_vtabsel5.ld_vtabsel_17;
	ptr->ld_vtabsel_18 = blu_ld_inter_vtabsel5.ld_vtabsel_18;
	ptr->ld_vtabsel_19 = blu_ld_inter_vtabsel5.ld_vtabsel_19;
	ptr->ld_vtabsel_20 = blu_ld_inter_vtabsel6.ld_vtabsel_20;
	ptr->ld_vtabsel_21 = blu_ld_inter_vtabsel6.ld_vtabsel_21;
	ptr->ld_vtabsel_22 = blu_ld_inter_vtabsel6.ld_vtabsel_22;
	ptr->ld_vtabsel_23 = blu_ld_inter_vtabsel6.ld_vtabsel_23;
	ptr->ld_vtabsel_24 = blu_ld_inter_vtabsel7.ld_vtabsel_24;
	ptr->ld_vtabsel_25 = blu_ld_inter_vtabsel7.ld_vtabsel_25;
	ptr->ld_vtabsel_26 = blu_ld_inter_vtabsel7.ld_vtabsel_26;
	ptr->ld_vtabsel_27 = blu_ld_inter_vtabsel7.ld_vtabsel_27;
	ptr->ld_vtabsel_28 = blu_ld_inter_vtabsel8.ld_vtabsel_28;
	ptr->ld_vtabsel_29 = blu_ld_inter_vtabsel8.ld_vtabsel_29;
	ptr->ld_vtabsel_30 = blu_ld_inter_vtabsel8.ld_vtabsel_30;
	ptr->ld_vtabsel_31 = blu_ld_inter_vtabsel8.ld_vtabsel_31;
...
void drvif_color_set_LD_BL_Profile_Interpolation_Table(unsigned int *ptr)
{
	...
	unsigned char ld_comp_ori;		
	unsigned char ld_db_ori;
	/*---- by table ----*/
	...
	blu_ld_global_ctrl0.regValue = IoReg_Read32(BLU_LD_Global_Ctrl0_reg);	
	blu_ld_global_ctrl1.regValue = IoReg_Read32(BLU_LD_Global_Ctrl1_reg);	
	/*write table protection : use db to disable ld_comp_en start*/	
	ld_comp_ori = blu_ld_global_ctrl0.ld_comp_en;	
	ld_db_ori = blu_ld_global_ctrl1.ld_db_en;			
	blu_ld_global_ctrl1.ld_db_en = 1;	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);		
	blu_ld_global_ctrl0.ld_comp_en = 0;	IoReg_Write32(BLU_LD_Global_Ctrl0_reg, blu_ld_global_ctrl0.regValue);		
	blu_ld_global_ctrl1.ld_db_apply = 1;	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);	
	msleep(20);
	...
	/*write table protection : use db to disable ld_comp_en recover*/	
	blu_ld_global_ctrl1.ld_db_en = ld_db_ori;	
	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);	
	blu_ld_global_ctrl0.ld_comp_en = ld_comp_ori;	
	IoReg_Write32(BLU_LD_Global_Ctrl0_reg, blu_ld_global_ctrl0.regValue);	
	blu_ld_global_ctrl1.ld_db_apply = 1;	
	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);
...
void drvif_color_set_LD_BL_Profile_Interpolation_Table_HV(unsigned int *Hptr, unsigned int *Vptr)
{
	...
	blu_ld_global_ctrl1.regValue = IoReg_Read32(BLU_LD_Global_Ctrl1_reg);	
	/*write table protection : use db to disable ld_comp_en start*/	
	ld_comp_ori = blu_ld_global_ctrl0.ld_comp_en;	ld_db_ori = blu_ld_global_ctrl1.ld_db_en;			
	blu_ld_global_ctrl1.ld_db_en = 1;	
	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);		
	blu_ld_global_ctrl0.ld_comp_en = 0;	
	IoReg_Write32(BLU_LD_Global_Ctrl0_reg, blu_ld_global_ctrl0.regValue);		
	blu_ld_global_ctrl1.ld_db_apply = 1;	
	IoReg_Write32(BLU_LD_Global_Ctrl1_reg, blu_ld_global_ctrl1.regValue);	
	msleep(20);
...
{
	M8P_blu_ld_apl_th_RBUS ld_apl_th_reg;

	ld_apl_th_reg.regValue = IoReg_Read32(M8P_BLU_LD_APL_TH_reg);
	
	ld_apl_th_reg.ld_apl_th1 = ptr->Leakage_TH1;
	ld_apl_th_reg.ld_apl_th2 = ptr->Leakage_TH2;

	IoReg_Write32(M8P_BLU_LD_APL_TH_reg, ld_apl_th_reg.regValue);
...
void drvif_color_get_leakage_th(DRV_LD_Leakage_TH *ptr)
{
	...
	ptr->Leakage_TH1 = ld_apl_th_reg.ld_apl_th1;
	ptr->Leakage_TH2 = ld_apl_th_reg.ld_apl_th2;

}

unsigned int drvif_color_get_LD_Leakage_Level_HW(void) 
{
	M8P_blu_ld_apl_leakage_level_RBUS ld_apl_leakage_level_reg;
	unsigned int Leakage_Level = 0;

	ld_apl_leakage_level_reg.regValue = IoReg_Read32(M8P_BLU_LD_APL_LEAKAGE_LEVEL_reg);

	Leakage_Level = ld_apl_leakage_level_reg.ld_apl_leakage_level;
	
	return Leakage_Level;
}

void drvif_color_set_LD_Gamma_en(unsigned char enable)
{
	M8P_blu_ld_gamma_lut_ctrl_RBUS M8P_blu_ld_gamma_lut_ctrl_reg;

	M8P_blu_ld_gamma_lut_ctrl_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg);

	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_en = enable;

	IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg, M8P_blu_ld_gamma_lut_ctrl_reg.regValue);
}

char drvif_color_get_LD_Gamma_en(void)
{
	M8P_blu_ld_gamma_lut_ctrl_RBUS M8P_blu_ld_gamma_lut_ctrl_reg;

	M8P_blu_ld_gamma_lut_ctrl_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg);

	return M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_en;
}

void drvif_color_set_LD_Gamma_index_setting(DRV_LD_Gamma *ptr)
{
	M8P_blu_ld_gamma_index_0_RBUS M8P_blu_ld_gamma_index_0_reg;
	M8P_blu_ld_gamma_index_1_RBUS M8P_blu_ld_gamma_index_1_reg;
	M8P_blu_ld_gamma_index_2_RBUS M8P_blu_ld_gamma_index_2_reg;
	M8P_blu_ld_gamma_index_3_RBUS M8P_blu_ld_gamma_index_3_reg;
	M8P_blu_ld_gamma_index_4_RBUS M8P_blu_ld_gamma_index_4_reg;
	M8P_blu_ld_gamma_index_5_RBUS M8P_blu_ld_gamma_index_5_reg;
	M8P_blu_ld_gamma_index_6_RBUS M8P_blu_ld_gamma_index_6_reg;
	M8P_blu_ld_gamma_index_7_RBUS M8P_blu_ld_gamma_index_7_reg;
	M8P_blu_ld_gamma_index_8_RBUS M8P_blu_ld_gamma_index_8_reg;
	M8P_blu_ld_gamma_index_9_RBUS M8P_blu_ld_gamma_index_9_reg;
	M8P_blu_ld_gamma_index_10_RBUS M8P_blu_ld_gamma_index_10_reg;
	M8P_blu_ld_gamma_index_11_RBUS M8P_blu_ld_gamma_index_11_reg;
	M8P_blu_ld_gamma_index_12_RBUS M8P_blu_ld_gamma_index_12_reg;
	M8P_blu_ld_gamma_index_13_RBUS M8P_blu_ld_gamma_index_13_reg;
	M8P_blu_ld_gamma_index_14_RBUS M8P_blu_ld_gamma_index_14_reg;
	M8P_blu_ld_gamma_index_15_RBUS M8P_blu_ld_gamma_index_15_reg;

	M8P_blu_ld_gamma_index_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_0_reg);
	M8P_blu_ld_gamma_index_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_1_reg);
	M8P_blu_ld_gamma_index_2_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_2_reg);
	M8P_blu_ld_gamma_index_3_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_3_reg);
	M8P_blu_ld_gamma_index_4_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_4_reg);
	M8P_blu_ld_gamma_index_5_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_5_reg);
	M8P_blu_ld_gamma_index_6_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_6_reg);
	M8P_blu_ld_gamma_index_7_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_7_reg);
	M8P_blu_ld_gamma_index_8_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_8_reg);
	M8P_blu_ld_gamma_index_9_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_9_reg);
	M8P_blu_ld_gamma_index_10_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_10_reg);
	M8P_blu_ld_gamma_index_11_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_11_reg);
	M8P_blu_ld_gamma_index_12_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_12_reg);
	M8P_blu_ld_gamma_index_13_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_13_reg);
	M8P_blu_ld_gamma_index_14_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_14_reg);
	M8P_blu_ld_gamma_index_15_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_INDEX_15_reg);

	M8P_blu_ld_gamma_index_0_reg.ld_gamma_index_1 = ptr->Gamma_Index.Index_1;
	M8P_blu_ld_gamma_index_0_reg.ld_gamma_index_2 = ptr->Gamma_Index.Index_2;
	M8P_blu_ld_gamma_index_1_reg.ld_gamma_index_3 = ptr->Gamma_Index.Index_3;
	M8P_blu_ld_gamma_index_1_reg.ld_gamma_index_4 = ptr->Gamma_Index.Index_4;
	M8P_blu_ld_gamma_index_2_reg.ld_gamma_index_5 = ptr->Gamma_Index.Index_5;
	M8P_blu_ld_gamma_index_2_reg.ld_gamma_index_6= ptr->Gamma_Index.Index_6;
	M8P_blu_ld_gamma_index_3_reg.ld_gamma_index_7= ptr->Gamma_Index.Index_7;
	M8P_blu_ld_gamma_index_3_reg.ld_gamma_index_8= ptr->Gamma_Index.Index_8;
	M8P_blu_ld_gamma_index_4_reg.ld_gamma_index_9= ptr->Gamma_Index.Index_9;
	M8P_blu_ld_gamma_index_4_reg.ld_gamma_index_10= ptr->Gamma_Index.Index_10;
	M8P_blu_ld_gamma_index_5_reg.ld_gamma_index_11= ptr->Gamma_Index.Index_11;
	M8P_blu_ld_gamma_index_5_reg.ld_gamma_index_12= ptr->Gamma_Index.Index_12;
	M8P_blu_ld_gamma_index_6_reg.ld_gamma_index_13= ptr->Gamma_Index.Index_13;
	M8P_blu_ld_gamma_index_6_reg.ld_gamma_index_14= ptr->Gamma_Index.Index_14;
	M8P_blu_ld_gamma_index_7_reg.ld_gamma_index_15= ptr->Gamma_Index.Index_15;
	M8P_blu_ld_gamma_index_7_reg.ld_gamma_index_16= ptr->Gamma_Index.Index_16;
	M8P_blu_ld_gamma_index_8_reg.ld_gamma_index_17= ptr->Gamma_Index.Index_17;
	M8P_blu_ld_gamma_index_8_reg.ld_gamma_index_18= ptr->Gamma_Index.Index_18;
	M8P_blu_ld_gamma_index_9_reg.ld_gamma_index_19= ptr->Gamma_Index.Index_19;
	M8P_blu_ld_gamma_index_9_reg.ld_gamma_index_20= ptr->Gamma_Index.Index_20;
	M8P_blu_ld_gamma_index_10_reg.ld_gamma_index_21= ptr->Gamma_Index.Index_21;
	M8P_blu_ld_gamma_index_10_reg.ld_gamma_index_22= ptr->Gamma_Index.Index_22;
	M8P_blu_ld_gamma_index_11_reg.ld_gamma_index_23= ptr->Gamma_Index.Index_23;
	M8P_blu_ld_gamma_index_11_reg.ld_gamma_index_24= ptr->Gamma_Index.Index_24;
	M8P_blu_ld_gamma_index_12_reg.ld_gamma_index_25= ptr->Gamma_Index.Index_25;
	M8P_blu_ld_gamma_index_12_reg.ld_gamma_index_26= ptr->Gamma_Index.Index_26;
	M8P_blu_ld_gamma_index_13_reg.ld_gamma_index_27= ptr->Gamma_Index.Index_27;
	M8P_blu_ld_gamma_index_13_reg.ld_gamma_index_28= ptr->Gamma_Index.Index_28;
	M8P_blu_ld_gamma_index_14_reg.ld_gamma_index_29= ptr->Gamma_Index.Index_29;
	M8P_blu_ld_gamma_index_14_reg.ld_gamma_index_30= ptr->Gamma_Index.Index_30;
	M8P_blu_ld_gamma_index_15_reg.ld_gamma_index_31= ptr->Gamma_Index.Index_31;

	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_0_reg, M8P_blu_ld_gamma_index_0_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_1_reg, M8P_blu_ld_gamma_index_1_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_2_reg, M8P_blu_ld_gamma_index_2_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_3_reg, M8P_blu_ld_gamma_index_3_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_4_reg, M8P_blu_ld_gamma_index_4_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_5_reg, M8P_blu_ld_gamma_index_5_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_6_reg, M8P_blu_ld_gamma_index_6_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_7_reg, M8P_blu_ld_gamma_index_7_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_8_reg, M8P_blu_ld_gamma_index_8_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_9_reg, M8P_blu_ld_gamma_index_9_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_10_reg, M8P_blu_ld_gamma_index_10_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_11_reg, M8P_blu_ld_gamma_index_11_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_12_reg, M8P_blu_ld_gamma_index_12_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_13_reg, M8P_blu_ld_gamma_index_13_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_14_reg, M8P_blu_ld_gamma_index_14_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_INDEX_15_reg, M8P_blu_ld_gamma_index_15_reg.regValue);
	
}

void drvif_color_set_LD_Gamma_bv_avg_setting(DRV_LD_Gamma *ptr)
{
	M8P_blu_ld_gamma_bv_avg_0_RBUS M8P_blu_ld_gamma_bv_avg_0_reg;
	M8P_blu_ld_gamma_bv_avg_1_RBUS M8P_blu_ld_gamma_bv_avg_1_reg;
	M8P_blu_ld_gamma_bv_avg_2_RBUS M8P_blu_ld_gamma_bv_avg_2_reg;

	M8P_blu_ld_gamma_bv_avg_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_BV_AVG_0_reg);
	M8P_blu_ld_gamma_bv_avg_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_BV_AVG_1_reg);
	M8P_blu_ld_gamma_bv_avg_2_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_BV_AVG_2_reg);

	M8P_blu_ld_gamma_bv_avg_0_reg.ld_gamma_bv_avg_1 = ptr->BV_avg.BV_avg_1;
	M8P_blu_ld_gamma_bv_avg_0_reg.ld_gamma_bv_avg_2 = ptr->BV_avg.BV_avg_2;
	M8P_blu_ld_gamma_bv_avg_0_reg.ld_gamma_bv_avg_3 = ptr->BV_avg.BV_avg_3;
	M8P_blu_ld_gamma_bv_avg_1_reg.ld_gamma_bv_avg_4 = ptr->BV_avg.BV_avg_4;
	M8P_blu_ld_gamma_bv_avg_1_reg.ld_gamma_bv_avg_5 = ptr->BV_avg.BV_avg_5;
	M8P_blu_ld_gamma_bv_avg_1_reg.ld_gamma_bv_avg_6 = ptr->BV_avg.BV_avg_6;
	M8P_blu_ld_gamma_bv_avg_2_reg.ld_gamma_bv_avg_7 = ptr->BV_avg.BV_avg_7;
	M8P_blu_ld_gamma_bv_avg_2_reg.ld_gamma_bv_avg_8 = ptr->BV_avg.BV_avg_8;
	M8P_blu_ld_gamma_bv_avg_2_reg.ld_gamma_bv_avg_9 = ptr->BV_avg.BV_avg_9;

	IoReg_Write32(M8P_BLU_LD_GAMMA_BV_AVG_0_reg, M8P_blu_ld_gamma_bv_avg_0_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_BV_AVG_1_reg, M8P_blu_ld_gamma_bv_avg_1_reg.regValue);
	IoReg_Write32(M8P_BLU_LD_GAMMA_BV_AVG_2_reg, M8P_blu_ld_gamma_bv_avg_2_reg.regValue);
	
}

void drvif_color_set_LD_Gamma_Tbl_HW(unsigned int *pTblValue, unsigned int TblSel, unsigned int Channel)
{
	int iter = 33;
	M8P_blu_ld_gamma_lut_ctrl_RBUS M8P_blu_ld_gamma_lut_ctrl_reg;
	M8P_blu_ld_gamma_lut_data_0_RBUS M8P_blu_ld_gamma_lut_data_0_reg;
	M8P_blu_ld_gamma_lut_data_1_RBUS M8P_blu_ld_gamma_lut_data_1_reg;

	M8P_blu_ld_gamma_lut_ctrl_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg);
	M8P_blu_ld_gamma_lut_data_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg);
	M8P_blu_ld_gamma_lut_data_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg);

	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_bv_avg_en = 0; // Use HW bv_avg
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_blending = 0; //  Use HW blending
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sel = TblSel; // sel LD gamma table 1~9 (R/W)
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_ax_rw_sel = 1; // Enable Access Channels for Gamma LUT Writing
	...
	if(Channel == 0) // R
	{
		...
		{
			M8P_blu_ld_gamma_lut_data_0_reg.ld_gamma_lut_data_r = pTblValue[iter];
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg, M8P_blu_ld_gamma_lut_data_0_reg.regValue);
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg, M8P_blu_ld_gamma_lut_data_1_reg.regValue);
		}
	}
	else if(Channel == 1) // G
	{
		for(iter=0; iter<33; iter++)
		{
			...
			M8P_blu_ld_gamma_lut_data_0_reg.ld_gamma_lut_data_g = pTblValue[iter];
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg, M8P_blu_ld_gamma_lut_data_0_reg.regValue);
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg, M8P_blu_ld_gamma_lut_data_1_reg.regValue);
		}
	}
	else if(Channel == 2) // B
	{
		for(iter=0; iter<33; iter++)
		{
			M8P_blu_ld_gamma_lut_data_1_reg.ld_gamma_lut_data_b = pTblValue[iter];
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg, M8P_blu_ld_gamma_lut_data_0_reg.regValue);
			IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg, M8P_blu_ld_gamma_lut_data_1_reg.regValue);
		}
	}
	...
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_ax_rw_sel = 0; // Disable Access Channels for Gamma LUT Writing

	IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg, M8P_blu_ld_gamma_lut_ctrl_reg.regValue);
}

void drvif_color_get_LD_Gamma_Tbl_HW(unsigned int *pTblValue, unsigned int TblSel, unsigned int Channel)
{
	int iter = 33;
	M8P_blu_ld_gamma_lut_ctrl_RBUS M8P_blu_ld_gamma_lut_ctrl_reg;
	M8P_blu_ld_gamma_lut_data_0_RBUS M8P_blu_ld_gamma_lut_data_0_reg;
	...
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_blending = 0; //  Use HW blending
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sel = TblSel; // sel LD gamma table 1~9 (R/W)
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_ax_rw_sel = 2; // Enable Access Channels for Gamma LUT Reading
	...
	if(Channel == 0)// R
	{
		for(iter=0; iter<33; iter++)
		{
			M8P_blu_ld_gamma_lut_data_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg);
			M8P_blu_ld_gamma_lut_data_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg);
			pTblValue[iter] = M8P_blu_ld_gamma_lut_data_0_reg.ld_gamma_lut_data_r;
		}
	}
	else if(Channel == 1)// G
	{
		for(iter=0; iter<33; iter++)
		{
			M8P_blu_ld_gamma_lut_data_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg);
			M8P_blu_ld_gamma_lut_data_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg);
			pTblValue[iter] = M8P_blu_ld_gamma_lut_data_0_reg.ld_gamma_lut_data_g;
		}
	}
	else if(Channel == 2)// B
	{
		for(iter=0; iter<33; iter++)
		{
			M8P_blu_ld_gamma_lut_data_0_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_0_reg);
			M8P_blu_ld_gamma_lut_data_1_reg.regValue = IoReg_Read32(M8P_BLU_LD_GAMMA_LUT_DATA_1_reg);
			pTblValue[iter] = M8P_blu_ld_gamma_lut_data_1_reg.ld_gamma_lut_data_b;
		}
	}
	else
	{
	...
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_ax_rw_sel = 0; // Disable Access Channels for Gamma LUT Reading

	IoReg_Write32(M8P_BLU_LD_GAMMA_LUT_CTRL_reg, M8P_blu_ld_gamma_lut_ctrl_reg.regValue);
}


void drvif_color_set_LD_Gamma_Tbl_SW(unsigned int *pTblValue, unsigned int Channel)
{
	int value;
	int iter = 33;
	M8P_blu_ld_gamma_lut_ctrl_RBUS M8P_blu_ld_gamma_lut_ctrl_reg;
	...
	if(value>11)
		value = 11;
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_bv_avg_en = 0; // Use HW bv_avg
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_blending = 1; //  Use SW blending
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_ax_rw_sel = 1; // Enable Access Channels for Gamma LUT Writing
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sel = value; // sel LD gamma table 9~11 (R/W) R:9 G:10 B:11
...
void drvif_color_set_LD_Gamma_force_bv_avg(unsigned int BV_avg)
{
	...
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_bv_avg_en = 1;    // Use HW bv_avg
	M8P_blu_ld_gamma_lut_ctrl_reg.ld_gamma_lut_sw_bv_avg = BV_avg; //  Use HW blending
...
unsigned char drvif_color_get_LD_8bin( unsigned short blk_start, unsigned short read_num, unsigned int** blk_hist )
{
	if(get_mach_type()==MACH_ARCH_RTK2885P){
		...
		ld_apl_read_8bin_4_reg.regValue = IoReg_Read32(BLU_LD_APL_read_8bin_4_reg);
	    ...
	    // ml8p IC, run ml8 flow
	    rtd_pr_vpq_emerg("ml8p IC no 8 bin\n");
	    return 0;
	}else if(get_mach_type()==MACH_ARCH_RTK2885PP){
	    // ml8p IC , run ml8p flow
	    rtd_pr_vpq_emerg("ml8p IC no 8 bin\n");
...
{
	M8P_blu_ld_demo_window_ctrl0_RBUS blu_ld_demo_window_ctrl0;
	blu_ld_demo_window_ctrl1_RBUS blu_ld_demo_window_ctrl1;
	...
	unsigned char ucld_blk_hnum, ucld_blk_vnum = 0;
	unsigned int ucld_blk_hsize, ucld_blk_vsize = 0;
	unsigned short spinBox_Width, spinBox_Height, h_s, h_e, v_s, v_e;
	...
	ucld_blk_hsize = blu_ld_global_ctrl3.ld_blk_hsize;
	ucld_blk_vsize = blu_ld_global_ctrl3.ld_blk_vsize;
	...
	/*
		LD_demo_mode = 2, Left/Right Demo Mode 
		LD_demo_bottom = frame height-1
		LD_demo_top/LD_demo_bottom/left register setting no effect
		LD_demo_right = ( (LD_Blk_Hnum+2)/2 ) * LD_Blk_Hsize / 2pixel

		LD_demo_mode = 3, Top/Bottom Demo Mode 
		LD_demo_bottom = ( (LD_Blk_Vnum+2)/2 ) * LD_Blk_Vsize
		LD_demo_right = frame width
		LD_demo_top/right/left register register setting no effect
	...
	*/
	...
	if (bCtrl) {
		...
		{
			blu_ld_demo_window_ctrl0.ld_demo_en = bCtrl;
			blu_ld_demo_window_ctrl0.ld_demo_mode = ptr->ctrlMode;
			blu_ld_demo_window_ctrl0.ld_demo_top = ptr->top;
			blu_ld_demo_window_ctrl0.ld_demo_bottom = spinBox_Height-1;
			blu_ld_demo_window_ctrl1.ld_demo_left = ptr->left;
			blu_ld_demo_window_ctrl1.ld_demo_right = ( (ucld_blk_hnum+2)/2 ) * ucld_blk_hsize / 2;
		}
		else if(ptr->ctrlMode == 3)
		{
			blu_ld_demo_window_ctrl0.ld_demo_en = bCtrl;
			blu_ld_demo_window_ctrl0.ld_demo_mode = ptr->ctrlMode;
			blu_ld_demo_window_ctrl0.ld_demo_top = ptr->top;
			blu_ld_demo_window_ctrl0.ld_demo_bottom = ( (ucld_blk_vnum+2)/2 ) * ucld_blk_vsize;
			blu_ld_demo_window_ctrl1.ld_demo_left = ptr->left;
			blu_ld_demo_window_ctrl1.ld_demo_right = spinBox_Width-1;
		}
		else
		{
			blu_ld_demo_window_ctrl0.ld_demo_en = bCtrl;
			blu_ld_demo_window_ctrl0.ld_demo_mode = ptr->ctrlMode;
			blu_ld_demo_window_ctrl0.ld_demo_top = ptr->top;
			blu_ld_demo_window_ctrl0.ld_demo_bottom = ptr->bottom;
			blu_ld_demo_window_ctrl1.ld_demo_left = ptr->left;
			blu_ld_demo_window_ctrl1.ld_demo_right = ptr->right;
		}
...
void drvif_color_set_LDSPI_DataSRAM_Data_Continuous(DRV_LD_LDSPI_DATASRAM_TYPE *LDSPI_DataSRAM, unsigned char Wait_Tx_Done)
{
	...
	if (LDSPI_DataSRAM == NULL) {
		rtd_pr_vpq_emerg("[VPQ_LED] drvif_color_set_LDSPI_DataSRAM_Data_Continuous, LDSPI_DataSRAM is NULL\n");
		return;
	}
	if (LDSPI_DataSRAM->SRAM_Value == NULL) {
		rtd_pr_vpq_emerg("[VPQ_LED] drvif_color_set_LDSPI_DataSRAM_Data_Continuous, LDSPI_DataSRAM->SRAM_Value is NULL\n");
		return;
	}
	...
	if (Wait_Tx_Done == 1) {/*Normal Usage Wait tx_done*/
		...
		if(timeoutcnt == 0){
			rtd_pr_vpq_emerg("[VPQ_LED] drvif_color_set_LDSPI_DataSRAM_Data_Continuous, timeoutcnt == 0\n");
		}
	} else {/*tx_done*/
		ldspi_data_addr_ctrl.regValue = IoReg_Read32(LDSPI_Data_Addr_CTRL_reg);
		ldspi_data_addr_ctrl.data_rw_pointer_set = LDSPI_DataSRAM->SRAM_Position;
...
{	
	...
	blu_ld_rd_ctrl_reg.ld_dma_line_mode_en = line_mode;
	blu_ld_ai_gain_reg.ld_ai_gain_shift = gain_shift;
	...
	blu_ld_rd_num_bl_wrap_ctl_reg.ld_burst_len = 32;
...
void drvif_color_set_LDSPI_TxDoneIntEn(unsigned char enable)
{
	...
	rtd_pr_vpq_info("[VPQ_LED] LDSPI_TxDoneIntEn = %d\n",enable);

}
void drvif_color_get_LD_Algo_sram(unsigned short length, unsigned short * BLtable)
{
	...
	if(ld_blk_type == 1) {	// top/bottom mode
		...
		for(j = 0; j < ld_blk_hnum + 1; j++) {
			...
			for(i = 0; i < ld_blk_vnum;i+=2) {
				...
				BLtable[j] = reg_blu_ld_blight_dp.ld_blight_dat_lsb;
				BLtable[ld_blk_hnum+j+1]= reg_blu_ld_blight_dp.ld_blight_dat_msb;
			}
		}
	}
		...
		while(i < (ld_blk_vnum * ld_blk_hnum) + 1) {
			...
			// data
			reg_blu_ld_blight_dp.regValue = IoReg_Read32(BLU_LD_BLIGHT_DP_reg);
			BLtable[i] = reg_blu_ld_blight_dp.ld_blight_dat_lsb;
			BLtable[ld_blk_vnum + i+1] = reg_blu_ld_blight_dp.ld_blight_dat_msb;
		...
		for(i = 0; i < ld_blk_vnum + 1; i++) {
			for(j = 0; j < ld_blk_hnum + 1;) {
				// data
				reg_blu_ld_blight_dp.regValue = IoReg_Read32(BLU_LD_BLIGHT_DP_reg);
				*(BLtable+i*(ld_blk_hnum + 1)+j++) = reg_blu_ld_blight_dp.ld_blight_dat_lsb;
				*(BLtable+i*(ld_blk_hnum + 1)+j++) = reg_blu_ld_blight_dp.ld_blight_dat_msb;
			}
		}
...
unsigned char drvif_color_get_Temp(void)
{
	blu_ld_global_ctrl0_RBUS blu_ld_global_ctrl0;
	blu_ld_global_ctrl0.regValue = IoReg_Read32(BLU_LD_Global_Ctrl0_reg);

	return (blu_ld_global_ctrl0.dummy1802c200_30_1 >>4) & 0x1;

}
unsigned char drvif_global_ctrl0_dummy(unsigned char index)
{
	blu_ld_global_ctrl0_RBUS blu_ld_global_ctrl0;
	blu_ld_global_ctrl0.regValue = IoReg_Read32(BLU_LD_Global_Ctrl0_reg);

	return (blu_ld_global_ctrl0.dummy1802c200_30_1 >>index) & 0x1;
