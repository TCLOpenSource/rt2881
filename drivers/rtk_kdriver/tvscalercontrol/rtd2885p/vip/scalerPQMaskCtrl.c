...
#endif
/****************************** Configurations ******************************/
extern struct semaphore VPQ_PQMASK_Semaphore;
...
#define PQMASKPrintf_Srnndomain(fmt, args...)	VIPprintf(VIP_LOGLEVEL_DEBUG, VIP_LOGMODULE_VIP_PQMask_Srnndomain_DEBUG, PQMaskPrintCount, fmt, ##args)
...
#define PQMASK_SE_DEFOCUS_CPU 0
...
static PQMASK_ONLINE_MONITOR_DEFOUCS_T PQMaskMonitorMgrDefocus = {		// Monitor for defocus
	// Control
	{
		1,	// bEnable : enable online monitor
		1,	// bLogEnable : enable print error log
		15,	// LogPeriod : unit=sec
		10,	// Model_TargetFps : lowest tolerance input depth information FPS
		64,	// Model_MeasureRstFrm
		40,	// SEProc_TargetTime : largest se process time, unit=ms
	},
	// Status
	{
		...
		0,	// SEPostProc_Time
		0,	// Model_Fps
		0,	// Model_MeasureStartTime
		0,	// Model_MeauserEndTime
		0,	// Model_Counter
		0,	// SECurWholeProcStart
		0,	// SECurWholeProcEnd
		{0, 0, 0, 0},	// SECurPreProcStart[MODULE_GRP_MAX]
		{0, 0, 0, 0},	// SECurPreProcEnd[MODULE_GRP_MAX]
		{0, 0, 0, 0},	// SECurPostProcStart[MODULE_GRP_MAX]
		{0, 0, 0, 0},	// SECurPostProcEnd[MODULE_GRP_MAX]
	},
};
...
static PQMASK_GLB_MGR_T PQMaskGlbMgr = {
	...
	// status
	{
		...
		PQMASK_SRNN_STAT_UNKNOWN,	// PQMaskSrnnStatus : PQMask Srnn internal status control
		0,	// PQMaskSrnnClkOffLater : signal of turning off Srnn clk later
		// OrbitStatus
		{
			0,	// bIsOled
			0,	// bOrbitEn
		...
		{
			0, // bIsSubDisplay
			VSC_SUB_MODE_EX_NONE, // PipSubMode
			{ 0, 0, 0, 0 }, // MainDisp
		},
...
static PQMASK_BUFINFO_T PQMaskBufMgr = {
	...
	// se processing dma
	{
		...
		// source depth info temporal filter buffer 0
		{ { AI_DEPTHMAP_WIDTH, AI_DEPTHMAP_HEIGHT, sizeof(unsigned char), 0x0 }, { 0, 0, 0, 0 } },
		// source depth info temporal filter buffer 1
		{ { AI_DEPTHMAP_WIDTH, AI_DEPTHMAP_HEIGHT, sizeof(unsigned char), 0x0 }, { 0, 0, 0, 0 } },
		// source depth info resize
		{ { PQMASK_PROC_WIDTH, PQMASK_PROC_HEIGHT, sizeof(unsigned char), 0x0 }, { 0, 0, 0, 0 } },
		// source defocus info
		{ { AI_DEFOCUS_OUTPUT_WIDTH, AI_DEFOCUS_OUTPUT_HEIGHT, sizeof(unsigned char), 0x0 }, { 0, 0, 0, 0 } },
...
const unsigned int SEProcGrpMapping[MODULE_GRP_MAX][3] = {
	...
	/* 2 */	{ PQMASK_MODULE_INT, PQMASK_MODULE_LC, PQMASK_MODULE_SRNN_IN_0 },
	/* 3 */	{ PQMASK_MODULE_SRNN_IN_1, PQMASK_MODULE_SRNN_IN_2, PQMASK_MODULE_SRNN_OUT_0 },
};
...
// for histogram information
unsigned char Histo_DepthImage[AI_DEPTHMAP_WIDTH*AI_DEPTHMAP_HEIGHT];
...
unsigned char OSDLabelColor[PQMASK_LABEL_NUM][3] = {
	//					R		G		B
	/*basic*/		{	128,	128,	128 },
	/*sky*/			{	128,	196,	255 },
	/*people*/		{	255,	128,	255 },
...
static void scalerPQMask_SETask(struct work_struct *work) {
	...
	unsigned char OutBuf_Defocus2Wei = PQMASK_SE_BUF_NUM;
	unsigned char OutBuf_DefocusWeiResize = PQMASK_SE_BUF_NUM;
	// depth information TNR parameters
	static unsigned char BufSelModeSwitch = 0;
	static unsigned char DepthTNRRoundingActionSel = 0; // 0:rounding 1:round-down 2: round-up (should toggle at every loop)
	unsigned char TnrPreBuf = PQMASK_SE_BUF_NUM;
	unsigned char OutBuf_DepthTnr = PQMASK_SE_BUF_NUM;
	unsigned char DepthTnrFusion_Pre = 0;
	unsigned char DepthTnrFusion_Cur = 0;
	// depth/semantic function parameters
	static unsigned char DepthSemFusionRoundingActionSel = 0; // 0:rounding 1:round-down 2: round-up (should toggle at every loop)
	// SNR ping-pong buffer
	unsigned char SpatialFltBuf_0 = PQMASK_SE_BUF_NUM;
	unsigned char SpatialFltBuf_1 = PQMASK_SE_BUF_NUM;
	// TNR parameters
	static unsigned char TNRRoundingActionSel = 0; // 0:rounding 1:round-down 2: round-up (should toggle at every loop)
	...
	// defocus blending parameters, use cpu do blending
	unsigned char DefocusEnableTbl[PQMASK_MODULE_NUM] = {0};
#if PQMASK_SE_DEFOCUS_CPU
	int j = 0;
	unsigned int DefocusBlendingVal = 0;
	unsigned char* DefocusBlendingBuf = NULL; 
	unsigned char* SemBlendingBuf = NULL; 
#endif

	//PQMASK_COLOR_DEMO_CTRL_T *pColorDemo = (PQMASK_COLOR_DEMO_CTRL_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DEMO);
	...
	if( mColorMgr.Param.bRoundActionManualSet != 0 ) {
		TNRRoundingActionSel = mColorMgr.Param.RoundAction_TNR;
		DepthTNRRoundingActionSel = mColorMgr.Param.RoundAction_DepthTNR;
		DepthSemFusionRoundingActionSel = mColorMgr.Param.RoundAction_DepthSemFusion;
	} else {
		TNRRoundingActionSel = (TNRRoundingActionSel == 1)? 2 : 1;
		DepthTNRRoundingActionSel = (DepthTNRRoundingActionSel == 1)? 2 : 1;
		DepthSemFusionRoundingActionSel = (DepthSemFusionRoundingActionSel == 1)? 2 : 1;
	}
	...
	if( PQMaskGlbMgr.Status.bAllowExec == 0 ) {
		...
		if( bPreAllowExec != PQMaskGlbMgr.Status.bAllowExec ) {
			...
			rtd_pr_vpq_emerg("[%s] change from allow to dis-allow pqmask\n", __FUNCTION__);
		}
		...
		if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_SE_PROC ) {
			...
			if( PQMaskGlbMgr.Status.bDepNeedUptHist != 0 && pColorMgr->DepthHist.Info.enable != 0 ) {
				pColorMgr->DepthHist.Info.width = AI_DEPTHMAP_WIDTH;
				pColorMgr->DepthHist.Info.height = AI_DEPTHMAP_HEIGHT;
				scalerPQMask_Memcpy(
					(void *)Histo_DepthImage, 0, 0,
					...
					PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEPTH].AddrInfo.Size
				);
				scalerPQMaskColor_DepthHistogram((void *)Histo_DepthImage);
				scalerPQMaskColor_Img2DepthHistogram(
					(void *)PQMaskBufMgr.DepthSrcImg.AddrInfo.VirtAddr_nonCached,
					(void *)Histo_DepthImage
				);
			...
			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
			// [SE] temporal filter on depth source information
			// input  : PQMASK_SE_SRCBUF_DEPTH
			// output : OutBuf_DepthTnr (PQMASK_SE_SRCBUF_DEPTH_TNR_00 or PQMASK_SE_SRCBUF_DEPTH_TNR_01)
			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****

			OutBuf_DepthTnr = PQMASK_SE_SRCBUF_DEPTH;

			if( mColorMgr.Status.bSceneChange != 0 ) {
				DepthTnrFusion_Pre = 0;
				DepthTnrFusion_Cur = 255;
			} else {
				DepthTnrFusion_Pre = mColorMgr.Param.DepthFusiontRatio_Pre;
				DepthTnrFusion_Cur = mColorMgr.Param.DepthFusiontRatio_Cur;
			}

			if( !(DepthTnrFusion_Cur == 255 && DepthTnrFusion_Pre == 0) ) {
				
				if( BufSelModeSwitch == 0 ) {
					TnrPreBuf = PQMASK_SE_SRCBUF_DEPTH_TNR_00;
					OutBuf_DepthTnr = PQMASK_SE_SRCBUF_DEPTH_TNR_01;
				} else {
					TnrPreBuf = PQMASK_SE_SRCBUF_DEPTH_TNR_01;
					OutBuf_DepthTnr = PQMASK_SE_SRCBUF_DEPTH_TNR_00;
				}
				...
				if( true == SeProcStatus ) {
					...
					SeSrc2_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[TnrPreBuf].AddrInfo.PhysAddr;
					SeSrc2_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					SeSrc2_Surface[0].pitch = AI_DEPTHMAP_WIDTH*8/8; // bytes per line
					SeSrc2_Surface[0].width = AI_DEPTHMAP_WIDTH;
					SeSrc2_Surface[0].height = AI_DEPTHMAP_HEIGHT;
					SeSrc2_Surface[0].bpp = 1;
					SeDst_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_DepthTnr].AddrInfo.PhysAddr;
					SeDst_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					SeDst_Surface[0].pitch = AI_DEPTHMAP_WIDTH*8/8; // bytes per line
					SeDst_Surface[0].width = AI_DEPTHMAP_WIDTH;
					SeDst_Surface[0].height = AI_DEPTHMAP_HEIGHT;
					SeDst_Surface[0].bpp = 1;
					SeSetupStatus = KGAL_PQMask_Bitblit(
						&SeSrc1_Surface[0], &SeSrc2_Surface[0], &SeDst_Surface[0],
						DepthTnrFusion_Pre /*on src2*/,
						DepthTnrFusion_Cur /*on src1*/,
						0,
						DepthTNRRoundingActionSel
					);
					if( true == SeSetupStatus ) {
						SeProcStatus = scalerPQMask_WaitSEDone();
					} else {
						rtd_printk(KERN_EMERG, TAG_NAME, "SE cmd setup erorr, depth temporal filter\n");
						SeProcStatus = false;	
					}
				}

				if( true == SeProcStatus ) {
					PQMASKPrintf_SEtask("[%s] SE depth info temporal filter, Buf %s(Pre) %s(Cur) to %s(Out)\n",
						TAG_NAME,
						PQMaskSEBufName[TnrPreBuf], PQMaskSEBufName[PQMASK_SE_SRCBUF_DEPTH], PQMaskSEBufName[OutBuf_DepthTnr]
					);
				} else {
					rtd_printk(KERN_EMERG, TAG_NAME, "SE error, depth temporal filter\n");
				}

				BufSelModeSwitch = (BufSelModeSwitch + 1) % 2;

			}

#if PQMASK_FUNC_DUMPBUF // debug dump
			if( sprintf(DumpFileName, "%s", "pqmask_DepTnrPre") >= 0 ) {
				PQMaskDumpSEBufCheck(TnrPreBuf, DumpFileName);
			}
			if( sprintf(DumpFileName, "%s", "pqmask_DepTnrOut") >= 0 ) {
				PQMaskDumpSEBufCheck(OutBuf_DepthTnr, DumpFileName);
			}
#endif

			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
			// [SE] resize the input depth information if necessary
			// input  : OutBuf_DepthTnr
			// output : OutBuf_DepResize (PQMASK_SE_SRCBUF_DEPTH_RESIZE or OutBuf_DepthTnr)
			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****

			OutBuf_DepResize = OutBuf_DepthTnr;

			// if( ( AI_DEPTHMAP_WIDTH != PQMASK_PROC_WIDTH || AI_DEPTHMAP_HEIGHT != PQMASK_PROC_HEIGHT ) && ( mColorMgr.Param.FusionRatio_Dep != 0 ) ) {
			if( ( PQMaskGlbMgr.Status.SE_OPMODE_DepthResize == 1 ) && ( mColorMgr.Param.FusionRatio_Dep != 0 ) ) {
				...
				if( true == SeProcStatus ) {
					...
					memset(&SeDst_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
					// fill information
					SeSrc1_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_DepthTnr].AddrInfo.PhysAddr;
					SeSrc1_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					SeSrc1_Surface[0].pitch = AI_DEPTHMAP_WIDTH*8/8; // bytes per line
					SeSrc1_Surface[0].width = AI_DEPTHMAP_WIDTH;
					SeSrc1_Surface[0].height = AI_DEPTHMAP_HEIGHT;
					...
					SeSetupStatus = KGAL_PQMaskStretch(&SeSrc1_Surface[0], &SeDst_Surface[0]);
					if( true == SeSetupStatus ) {
						SeProcStatus = scalerPQMask_WaitSEDone();
					} else {
						rtd_printk(KERN_EMERG, TAG_NAME, "SE cmd setup erorr, depth info resize\n");
						SeProcStatus = false;
					}
				}
				...
				if( true == SeProcStatus ) {
					PQMASKPrintf_SEtask("[%s] SE depth info resize from %dx%d to %dx%d done, Buf %s to %s\n",
						...
						PQMaskSEBufName[OutBuf_DepthTnr], PQMaskSEBufName[OutBuf_DepResize]
					);
				} else {
...
#if 0
			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
			// [SE] blending defocus and depth to get map for demo mode
			// input  : OutBuf_DepResize + PQMASK_SE_SRCBUF_DEFOCUS
			// output : PQMASK_SE_DEMOBUF_00
			// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****

			if (pColorDemo->bDeepnessDemoEn != 0) {
				if( true == SeProcStatus ) {
					// initial struct
					memset(&SeSrc1_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
					memset(&SeSrc2_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
					memset(&SeDst_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
					// fill information
					SeSrc1_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_DepResize].AddrInfo.PhysAddr;
					SeSrc1_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					...
					SeSrc1_Surface[0].bpp = 1;
					SeSrc2_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEFOCUS].AddrInfo.PhysAddr;
					SeSrc2_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					SeSrc2_Surface[0].pitch = AI_DEFOCUS_OUTPUT_WIDTH*8/8; // bytes per line
					SeSrc2_Surface[0].width = AI_DEFOCUS_OUTPUT_WIDTH;
					SeSrc2_Surface[0].height = AI_DEFOCUS_OUTPUT_HEIGHT;
					SeSrc2_Surface[0].bpp = 1;
					SeDst_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[PQMASK_SE_DEMOBUF_00].AddrInfo.PhysAddr;
					SeDst_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
					SeDst_Surface[0].pitch = PQMASK_PROC_WIDTH*8/8; // bytes per line
					SeDst_Surface[0].width = PQMASK_PROC_WIDTH;
					SeDst_Surface[0].height = PQMASK_PROC_HEIGHT;
					...
					if( true == SeSetupStatus ) {
						...
						rtd_printk(KERN_EMERG, TAG_NAME, "SE error, depth defocus demo fusion\n", ProcGrp);
						SeProcStatus = false;
					}
				}

				if( true == SeProcStatus ) {
					PQMASKPrintf_SEtask("[%s] SE depth defocus demo fusion, Buf %s(Pre) %s(Cur) to %s(Out)\n",
						TAG_NAME,
						PQMaskSEBufName[OutBuf_DepthTnr], PQMaskSEBufName[PQMASK_SE_SRCBUF_DEFOCUS], PQMaskSEBufName[PQMASK_SE_DEMOBUF_00]
					);
				} else {
					rtd_printk(KERN_EMERG, TAG_NAME, "SE error, depth defocus demo fusion\n");
				}
			...
			for( ProcGrp=0; ProcGrp<MODULE_GRP_MAX; ProcGrp++ ) {
				...
				OutBuf_Defocus2Wei = PQMASK_SE_BUF_NUM;
				OutBuf_DefocusWeiResize = PQMASK_SE_BUF_NUM;

				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				// [SE] setup depth to weight palette and depth info to module weight
				...
				if( mColorMgr.Param.FusionRatio_Dep != 0 ) {
					...
					if( true == SeProcStatus ) {
						...
						if( KGAL_PQMaskLockSE() == 0 ) {

							// trigger SE to setup palette
							SeSetupStatus = KGAL_SetSurfacePalette(&SeColorLut, false);
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
								if( true == SeProcStatus ) {
								...
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, depth palette\n", ProcGrp);
								SeProcStatus = false;
							}

							// trigger SE
							SeSetupStatus = KGAL_PQMaskLabel2Weight(&SeSrc1_Surface[0], &SeDst_Surface[0]);
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
								if( true == SeProcStatus ) {
								...
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, depth label2weight\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d lock error, depth label2weight\n", ProcGrp);
						}
				...
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				// [SE] setup semantic to weight palette and semantic label to module weight
					...
					if( true == SeProcStatus ) {
						...
						if( KGAL_PQMaskLockSE() == 0 ) {
							...
							if( true == SeSetupStatus ) {
								...
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, semantic palette\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							if( true == SeSetupStatus ) {
								...
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, semantic label2weight\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d lock error, semantic label2weight\n", ProcGrp);
						}
					}
				}
				...
				if( ( AI_SEMANTIC_OUTPUT_WIDTH != PQMASK_PROC_WIDTH || AI_SEMANTIC_OUTPUT_HEIGHT != PQMASK_PROC_HEIGHT ) &&
					...
					if( true == SeProcStatus ) {
						...
						if( true == SeSetupStatus ) {
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, sematic stretch\n", ProcGrp);
							SeProcStatus = false;
						}
					}
				...
				if( mColorMgr.Param.FusionRatio_Dep == 255 && mColorMgr.Param.FusionRatio_Sem == 0 ) {
					...
					if (aipq_mode.semantic_mode == 0){
						// npu semantic off, ignore Fusion ratio, use depth result only
						OutBuf_SemDepFusion = OutBuf_Dep2Wei;
						PQMASKPrintf_SEtask("[%s] grp %d, Fusion OutBuf %s\n", TAG_NAME, ProcGrp, PQMaskSEBufName[OutBuf_SemDepFusion]);
					}
					else if (aipq_mode.depth_mode == 0){
						// npu depth off, ignore Fusion ratio, use semantic result only
						OutBuf_SemDepFusion = OutBuf_SemWeiResize;
						PQMASKPrintf_SEtask("[%s] grp %d, Fusion OutBuf %s\n", TAG_NAME, ProcGrp, PQMaskSEBufName[OutBuf_SemDepFusion]);
					}
					else{
						...
						if( true == SeProcStatus ) {
							...
							SeSetupStatus = KGAL_PQMask_Bitblit(
								...
								0,
								DepthSemFusionRoundingActionSel
							);
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
							} else {
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, semantic depth fusion\n", ProcGrp);
								SeProcStatus = false;
							}
						}
				...
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				// [SE] setup defocus to weight palette and defocus label to module weight
				// input  : PQMASK_SE_SRCBUF_DEFOCUS
				// output : OutBuf_Defocus2Wei ( PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02 )
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				PQMaskMonitorMgrDefocus.Status.SECurPreProcStart[ProcGrp] = drvif_Get_90k_Lo_clk();

				// decide buffer
				if (OutBuf_SemDepFusion == PQMASK_SE_PROCBUF_00){	
					OutBuf_Defocus2Wei = PQMASK_SE_PROCBUF_01;
				}
				else if (OutBuf_SemDepFusion == PQMASK_SE_PROCBUF_01){
					OutBuf_Defocus2Wei = PQMASK_SE_PROCBUF_02;
				}
				else{
					OutBuf_Defocus2Wei = PQMASK_SE_PROCBUF_00;
				}

				if( mColorMgr.Param.FusionRatio_Defocus != 0 ) {
					...
					if( true == SeProcStatus ) {
						...
						memset(&SeDst_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
						// fill information
						scalerPQMaskColor_Defocus2Clut(ProcGrp, &SeColorLut.palette);
						SeColorLut.Surface.physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEFOCUS].AddrInfo.PhysAddr;
						SeColorLut.Surface.pixelFormat = KGAL_PIXEL_FORMAT_LUT8;
						SeColorLut.Surface.pitch = AI_DEFOCUS_OUTPUT_WIDTH*8/8; // bytes per line
						SeColorLut.Surface.width = AI_DEFOCUS_OUTPUT_WIDTH;
						SeColorLut.Surface.height = AI_DEFOCUS_OUTPUT_HEIGHT;
						SeColorLut.Surface.bpp = 1;
						memcpy(&SeSrc1_Surface[0], &SeColorLut.Surface, sizeof(KGAL_SURFACE_INFO_T));
						SeDst_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_Defocus2Wei].AddrInfo.PhysAddr;
						SeDst_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_YUV444;
						SeDst_Surface[0].pitch = AI_DEFOCUS_OUTPUT_WIDTH*8/8; // bytes per line
						SeDst_Surface[0].width = AI_DEFOCUS_OUTPUT_WIDTH;
						SeDst_Surface[0].height = AI_DEFOCUS_OUTPUT_HEIGHT;
						SeDst_Surface[0].bpp = 1;
						...
						if( KGAL_PQMaskLockSE() == 0 ) {
							...
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
								if( true == SeProcStatus ) {
									PQMASKPrintf_SEtask("[%s] SE task grp %d done, defocus palette\n", TAG_NAME, ProcGrp);
								} else {
									rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d error, defocus palette\n", ProcGrp);
								}
							} else {
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, defocus palette\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
								if( true == SeProcStatus ) {
									PQMASKPrintf_SEtask("[%s] SE task grp %d done, defocus label2weight, Buf %s to %s\n",
										TAG_NAME, ProcGrp, PQMaskSEBufName[PQMASK_SE_SRCBUF_DEFOCUS], PQMaskSEBufName[OutBuf_Defocus2Wei]
									);
								} else {
									rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d error, defocus label2weight\n", ProcGrp);
								}
							} else {
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, defocus label2weight\n", ProcGrp);
								SeProcStatus = false;
							}
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d lock error, defocus label2weight\n", ProcGrp);
						}
					}
				}

				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				// [SE] resize the defocus weighting information if necessary
				// input  : OutBuf_Defocus2Wei ( PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02 )
				// output : OutBuf_DefocusResize (PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02)
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****

				if( ( AI_DEFOCUS_OUTPUT_WIDTH != PQMASK_PROC_WIDTH || AI_DEFOCUS_OUTPUT_HEIGHT != PQMASK_PROC_HEIGHT ) &&
					( mColorMgr.Param.FusionRatio_Defocus != 0 ) ) {

					// decide buffer
					if (OutBuf_SemDepFusion == PQMASK_SE_PROCBUF_00 && OutBuf_Defocus2Wei == PQMASK_SE_PROCBUF_01){
						OutBuf_DefocusWeiResize = PQMASK_SE_PROCBUF_02;
					}
					else if (OutBuf_SemDepFusion == PQMASK_SE_PROCBUF_01 && OutBuf_Defocus2Wei == PQMASK_SE_PROCBUF_02){
						OutBuf_DefocusWeiResize = PQMASK_SE_PROCBUF_00;
					}
					else{
						OutBuf_DefocusWeiResize = PQMASK_SE_PROCBUF_01;
					}
					...
					if( true == SeProcStatus ) {
						...
						memset(&SeDst_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
						// fill information
						SeSrc1_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_Defocus2Wei].AddrInfo.PhysAddr;
						SeSrc1_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_YUV444;
						SeSrc1_Surface[0].pitch = AI_DEFOCUS_OUTPUT_WIDTH*8/8; // bytes per line
						SeSrc1_Surface[0].width = AI_DEFOCUS_OUTPUT_WIDTH;
						SeSrc1_Surface[0].height = AI_DEFOCUS_OUTPUT_HEIGHT;
						SeSrc1_Surface[0].bpp = 1;
						SeDst_Surface[0].physicalAddress = (UINT32)PQMaskBufMgr.SeBuffer[OutBuf_DefocusWeiResize].AddrInfo.PhysAddr;
						SeDst_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_YUV444;
						SeDst_Surface[0].pitch = PQMASK_PROC_WIDTH*8/8; // bytes per line
						SeDst_Surface[0].width = PQMASK_PROC_WIDTH;
						SeDst_Surface[0].height = PQMASK_PROC_HEIGHT;
						...
						if( true == SeSetupStatus ) {
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, defocus stretch\n", ProcGrp);
							SeProcStatus = false;
						}
					}

					if( true == SeProcStatus ) {
						PQMASKPrintf_SEtask("[%s] SE task grp %d done, defocus resize from %dx%d to %dx%d, Buf %s to %s\n",
							TAG_NAME, ProcGrp,
							AI_DEFOCUS_OUTPUT_WIDTH, AI_DEFOCUS_OUTPUT_HEIGHT,
							PQMASK_PROC_WIDTH, PQMASK_PROC_HEIGHT,
							PQMaskSEBufName[OutBuf_Defocus2Wei], PQMaskSEBufName[OutBuf_DefocusWeiResize]
						);
					} else {
						rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d error, defocus stretch\n", ProcGrp);
					}

				} else {
					OutBuf_DefocusWeiResize = OutBuf_Defocus2Wei;
				}

				PQMaskMonitorMgrDefocus.Status.SECurPreProcEnd[ProcGrp] = drvif_Get_90k_Lo_clk();
				
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
				// [CPU/SE] blending defocus and semantic&depth module weight result, use cpu instead of se
				// input  : OutBuf_DefocusWeiResize (PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02) + OutBuf_SemDepFusion (PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02)
				// output : OutBuf_SemDepFusion ( PQMASK_SE_PROCBUF_00 or PQMASK_SE_PROCBUF_01 or PQMASK_SE_PROCBUF_02 )
				// ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****

				PQMaskMonitorMgrDefocus.Status.SECurPostProcStart[ProcGrp]= drvif_Get_90k_Lo_clk();

				if( mColorMgr.Param.FusionRatio_Defocus == 0) {
					PQMASKPrintf_SEtask("[%s] grp %d, Fusion OutBuf %s\n", TAG_NAME, ProcGrp, PQMaskSEBufName[OutBuf_SemDepFusion]);
				} 
				else if (mColorMgr.Param.FusionRatio_Dep == 0 && mColorMgr.Param.FusionRatio_Sem == 0 && mColorMgr.Param.FusionRatio_Defocus != 0){
					OutBuf_SemDepFusion = OutBuf_DefocusWeiResize;
				}
				else {
					if (aipq_mode.defocus_mode == 0){
						// npu defocus off, ignore Fusion ratio, use SemDepFusion result only
						PQMASKPrintf_SEtask("[%s] grp %d, Fusion OutBuf %s\n", TAG_NAME, ProcGrp, PQMaskSEBufName[OutBuf_SemDepFusion]);
					}
					else if (aipq_mode.semantic_mode == 0 && aipq_mode.depth_mode == 0){
						// npu semantic & depth off, ignore Fusion ratio, use defocus result only
						OutBuf_SemDepFusion = OutBuf_DefocusWeiResize;
					}
					else{
#if PQMASK_SE_DEFOCUS_CPU
						// use cpu to do semantic * defocus/256
						scalerPQMaskColor_GetDefocusEnableTbl(DefocusEnableTbl);
						DefocusBlendingBuf = (unsigned char *)vip_malloc(PQMASK_PROC_WIDTH*PQMASK_PROC_HEIGHT*sizeof(unsigned char));
						SemBlendingBuf = (unsigned char *)vip_malloc(PQMASK_PROC_WIDTH*PQMASK_PROC_HEIGHT*sizeof(unsigned char));
						
						for (i=0; i<3; i++){
							if (DefocusEnableTbl[SEProcGrpMapping[ProcGrp][i]] == 1){
								scalerPQMask_Memcpy(
									(void *)SemBlendingBuf, 0, 0,
									(void *)(PQMaskBufMgr.SeBuffer[OutBuf_SemDepFusion].AddrInfo.VirtAddr+(SingleWeightPlaneSize*i)), (void *)(PQMaskBufMgr.SeBuffer[OutBuf_SemDepFusion].AddrInfo.PhysAddr+(SingleWeightPlaneSize*i)), 1,
									SingleWeightPlaneSize
								);

								scalerPQMask_Memcpy(
									(void *)DefocusBlendingBuf, 0, 0,
									(void *)(PQMaskBufMgr.SeBuffer[OutBuf_DefocusWeiResize].AddrInfo.VirtAddr+(SingleWeightPlaneSize*i)), (void *)(PQMaskBufMgr.SeBuffer[OutBuf_DefocusWeiResize].AddrInfo.PhysAddr+(SingleWeightPlaneSize*i)), 1,
									SingleWeightPlaneSize
								);

								for (j=0; j<SingleWeightPlaneSize; j++){
									//DefocusBlendingVal = DefocusBlendingBuf[j];
									DefocusBlendingVal = (DefocusBlendingBuf[j] * mColorMgr.Param.FusionRatio_Defocus + (1<<7)) >> 8;		// use FusionRatio_Defocus as 0~1 gain
									DefocusBlendingVal = (DefocusBlendingVal * SemBlendingBuf[j] + (1<<7)) >> 8;		// use DefocusBlendingVal as 0~1 gain
									DefocusBlendingBuf[j] = (DefocusBlendingVal > 255) ? 255 : DefocusBlendingVal;
								}
						...
						vip_free(DefocusBlendingBuf);
						vip_free(SemBlendingBuf);
						DefocusBlendingBuf = NULL;
						SemBlendingBuf = NULL;
#else
						// use se to do semantic * defocus/256
						// decide buffer
						scalerPQMaskColor_GetDefocusEnableTbl(DefocusEnableTbl);
						for (i=0; i<3; i++){
							if (DefocusEnableTbl[SEProcGrpMapping[ProcGrp][i]] == 1){
								if( true == SeProcStatus ) {
									...
									memset(&SeDst_Surface[0], 0, sizeof(KGAL_SURFACE_INFO_T));
									// fill information
									SeSrc1_Surface[0].physicalAddress = (UINT32)(PQMaskBufMgr.SeBuffer[OutBuf_DefocusWeiResize].AddrInfo.PhysAddr+(SingleWeightPlaneSize*i));
									SeSrc1_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_A8;
									SeSrc1_Surface[0].pitch = PQMASK_PROC_WIDTH*8/8; // bytes per line
									SeSrc1_Surface[0].width = PQMASK_PROC_WIDTH;
									SeSrc1_Surface[0].height = PQMASK_PROC_HEIGHT;
									SeSrc1_Surface[0].bpp = 1;
									SeSrc2_Surface[0].physicalAddress = (UINT32)(PQMaskBufMgr.SeBuffer[OutBuf_SemDepFusion].AddrInfo.PhysAddr+(SingleWeightPlaneSize*i));
									SeSrc2_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
									SeSrc2_Surface[0].pitch = PQMASK_PROC_WIDTH*8/8; // bytes per line
									SeSrc2_Surface[0].width = PQMASK_PROC_WIDTH;
									SeSrc2_Surface[0].height = PQMASK_PROC_HEIGHT;
									SeSrc2_Surface[0].bpp = 1;
									SeDst_Surface[0].physicalAddress = (UINT32)(PQMaskBufMgr.SeBuffer[OutBuf_SemDepFusion].AddrInfo.PhysAddr+(SingleWeightPlaneSize*i));
									SeDst_Surface[0].pixelFormat = KGAL_PIXEL_FORMAT_NV12_Y;
									SeDst_Surface[0].pitch = PQMASK_PROC_WIDTH*8/8; // bytes per line
									SeDst_Surface[0].width = PQMASK_PROC_WIDTH;
									SeDst_Surface[0].height = PQMASK_PROC_HEIGHT;
									SeDst_Surface[0].bpp = 1;
									// trigger SE
									SeSetupStatus = KGAL_PQMask_Bitblit_MultiplyByAlpha(
										&SeSrc1_Surface[0], &SeSrc2_Surface[0], &SeDst_Surface[0],
										mColorMgr.Param.FusionRatio_Defocus /*on src2*/,
										0 /*on src1*/,
										0,
										0 //DepthSemFusionRoundingActionSel
									);
									if( true == SeSetupStatus ) {
										SeProcStatus = scalerPQMask_WaitSEDone();
									} else {
								...
								if( true == SeProcStatus ) {
									PQMASKPrintf_SEtask("[%s] SE task grp %d done, semantic defocus fusion, Buf %s, %s to %s\n",
										TAG_NAME, ProcGrp, PQMaskSEBufName[OutBuf_DefocusWeiResize], PQMaskSEBufName[OutBuf_SemDepFusion], PQMaskSEBufName[OutBuf_SemDepFusion]
									);
								} else {
									rtd_pr_vpq_emerg("SE task grp %d error, semantic defocus fusion\n", ProcGrp);
								}
							}
						}
#endif
				...
				PQMaskMonitorMgrDefocus.Status.SECurPostProcEnd[ProcGrp]= drvif_Get_90k_Lo_clk();
				...
				if( pColorMgr->Status.SCRstFrm > 0 ) {
					...
					if( pColorMgr->Param.bSceneChangeEnable == 1 ) {
						if( (pColorMgr->Param.SceneChangeRstFrm/2) != 0 ) {
							...
							TNRFusion_Cur = PQMASK_BITBLIT_FAC_SUM-TNRFusion_Pre;
						} else {
							if( pColorMgr->Status.SCRstFrm > (pColorMgr->Param.SceneChangeRstFrm/2) ) {
					...
					if( TNRFusion_Pre == 255 && TNRFusion_Cur == 0 ) {
						...
						for( i=0; i<3; i++ ) {
							...
							if( TNRModuleSel < PQMASK_MODULE_NUM ) {
								...
								scalerPQMask_Memset(
									...
									PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].BasicInfo.RstValue,
									PQMaskBufMgr.ModuleDma[TNRModuleSel][RingBufWritePtr].AddrInfo.Size
								);
							}
						}
						...
						if( pColorMgr->Status.SCRstFrm > (pColorMgr->Param.SceneChangeRstFrm/2) ) {
							for(i=0; i<3; i++) {
								TNRModuleSel = SEProcGrpMapping[ProcGrp][i];
#if 1 // 230825, willy fix
			                                scalerPQMask_Memset(
			                                    (void *)(PQMaskBufMgr.SeBuffer[OutBuf_SpatialFlt].AddrInfo.VirtAddr + SingleWeightPlaneSize*i),
			                                    (void *)(PQMaskBufMgr.SeBuffer[OutBuf_SpatialFlt].AddrInfo.PhysAddr + SingleWeightPlaneSize*i),
			                                    PQMaskBufMgr.ModuleDma[TNRModuleSel][0].BasicInfo.RstValue,
			                                    SingleWeightPlaneSize
			                                );
#else
						...
						// trigger SE
						SeSetupStatus = KGAL_PQMask_Bitblit_SpearateAddr(
						...
						if( true == SeSetupStatus ) {
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, scene change only do RTNR\n", ProcGrp);
							SeProcStatus = false;
						}
					}
					...
					if( mColorMgr.Param.SpatialFltCycle > 0 ) {
						...
						for( i=0; i<mColorMgr.Param.SpatialFltCycle; i++ ) {
							...
							if( true == SeProcStatus ) {
								...
								// trigger SE
								SeSetupStatus = KGAL_PQMaskStretch_Coef(&SeSrc1_Surface[0], &SeDst_Surface[0],
								...
								if( true == SeSetupStatus ) {
									...
									rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, se spatial filter cycle %d\n", ProcGrp, i);
									SeProcStatus = false;
								}
							}
					...
					if( mColorMgr.Param.SpatialFltCycle > 0 ) {
						...
						if( pColorMgr->Status.Genre_CG_Frm != 0 ) {
							if( pColorMgr->Param.bGenre_CG_Enable == 1 ) {
								if( pColorMgr->Param.Genre_CG_toTargetFrm != 0 ) {
									// as the blending goes, the ration getting close to pre=0, cur=254
						...
						if( true == SeProcStatus ) {
							...
							SeSetupStatus = KGAL_PQMask_Stretch_Coef_Bitblit_SpearateAddr(
								&SeSrc1_Surface[0], &SeSrc1_Surface[1], &SeSrc1_Surface[2],
								&SeSrc2_Surface[0], &SeSrc2_Surface[1], &SeSrc2_Surface[2],
								&SeDst_Surface[0], &SeDst_Surface[1], &SeDst_Surface[2],
							...
							if( true == SeSetupStatus ) {
								...
								rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, spatial (%s) temporal combine\n", ProcGrp, PQMaskSEBufName[OutBuf_SpatialFlt]);
								SeProcStatus = false;
							}
						...
						if( true == SeSetupStatus ) {
							...
							rtd_printk(KERN_EMERG, TAG_NAME, "SE task grp %d cmd setup error, simple RTNR with %d pre %d cur\n", ProcGrp, TNRFusion_Pre, TNRFusion_Cur);
							SeProcStatus = false;
						}
 ...
 */
 /*
void scalerPQMask_OnlineMonitorDefocus(void) {

	unsigned int TimeDiff = 0;
	unsigned int SEProcTimeMS = 0;
	unsigned int DefocusFps = 0;
	unsigned char ProcGrp = 0;

	if( PQMaskMonitorMgrDefocus.Ctrl.bEnable == 0 )
		return;

	// check input depth
	if( PQMaskGlbMgr.Ctrl.bIsDefocusReady != 0 ) {
		if( PQMaskMonitorMgrDefocus.Status.Model_Counter >= PQMaskMonitorMgrDefocus.Ctrl.Model_MeasureRstFrm ) {

			if( PQMaskMonitorMgrDefocus.Status.Model_Counter != 0 ) {
				if(PQMaskMonitorMgrDefocus.Status.Model_MeauserEndTime > PQMaskMonitorMgrDefocus.Status.Model_MeasureStartTime) {
					TimeDiff = PQMaskMonitorMgrDefocus.Status.Model_MeauserEndTime - PQMaskMonitorMgrDefocus.Status.Model_MeasureStartTime;
				} else {
					TimeDiff = (0xffffffff) - (PQMaskMonitorMgrDefocus.Status.Model_MeasureStartTime - PQMaskMonitorMgrDefocus.Status.Model_MeauserEndTime);
				}

				DefocusFps = (90*1000*PQMaskMonitorMgrDefocus.Status.Model_Counter)/TimeDiff;
				if( DefocusFps < PQMaskMonitorMgrDefocus.Ctrl.Model_TargetFps ) {
					PQMaskMonitorMgrDefocus.Status.Model_FpsError = DefocusFps;
				}
				PQMaskMonitorMgrDefocus.Status.Model_Fps = DefocusFps;
			}
			PQMaskMonitorMgrDefocus.Status.Model_Counter = 0;
		} else {
			PQMaskMonitorMgrDefocus.Status.Model_Counter++;
		}
	}

	// print log
	if( PQMaskMonitorMgrDefocus.Ctrl.bLogEnable && PQMaskMonitorMgrDefocus.Status.LogCounter >= PQMaskGlbMgr.Ctrl.LimitFPS*PQMaskMonitorMgrDefocus.Ctrl.LogPeriod ) {
		if( PQMaskMonitorMgrDefocus.Status.Model_FpsError != 0 ) {
			rtd_printk(KERN_EMERG, TAG_AIPQ_ERR, "PQMask input defocus fps %d less than expected %d\n",
				PQMaskMonitorMgrDefocus.Status.Model_FpsError, PQMaskMonitorMgrDefocus.Ctrl.Model_TargetFps
			);
		}
		
		// check SE process time
		if( PQMaskMonitorMgrDefocus.Status.SECurWholeProcEnd >= PQMaskMonitorMgrDefocus.Status.SECurWholeProcStart ) {
			TimeDiff = PQMaskMonitorMgrDefocus.Status.SECurWholeProcEnd - PQMaskMonitorMgrDefocus.Status.SECurWholeProcStart;
		} else {
			TimeDiff = (0xffffffff) - (PQMaskMonitorMgrDefocus.Status.SECurWholeProcStart-PQMaskMonitorMgrDefocus.Status.SECurWholeProcEnd);
		}

		SEProcTimeMS = TimeDiff;
		
		PQMaskMonitorMgrDefocus.Status.SEPreProc_Time = 0;
		PQMaskMonitorMgrDefocus.Status.SEPostProc_Time = 0;
		for( ProcGrp=0; ProcGrp<MODULE_GRP_MAX; ProcGrp++ ) {
			if(PQMaskMonitorMgrDefocus.Status.SECurPreProcEnd[ProcGrp] >= PQMaskMonitorMgrDefocus.Status.SECurPreProcStart[ProcGrp]) {
				TimeDiff = PQMaskMonitorMgrDefocus.Status.SECurPreProcEnd[ProcGrp] - PQMaskMonitorMgrDefocus.Status.SECurPreProcStart[ProcGrp];
			} else {
				TimeDiff = (0xffffffff) - (PQMaskMonitorMgrDefocus.Status.SECurPreProcStart[ProcGrp] - PQMaskMonitorMgrDefocus.Status.SECurPreProcEnd[ProcGrp]);
			}

			PQMaskMonitorMgrDefocus.Status.SEPreProc_Time += TimeDiff;
			
			if(PQMaskMonitorMgrDefocus.Status.SECurPostProcEnd[ProcGrp] >= PQMaskMonitorMgrDefocus.Status.SECurPostProcStart[ProcGrp]) {
				TimeDiff = PQMaskMonitorMgrDefocus.Status.SECurPostProcEnd[ProcGrp] - PQMaskMonitorMgrDefocus.Status.SECurPostProcStart[ProcGrp];
			} else {
				TimeDiff = (0xffffffff) - (PQMaskMonitorMgrDefocus.Status.SECurPostProcStart[ProcGrp] - PQMaskMonitorMgrDefocus.Status.SECurPostProcEnd[ProcGrp]);
			}

			PQMaskMonitorMgrDefocus.Status.SEPostProc_Time += TimeDiff;
		}
		
		rtd_pr_vpq_info("PQMask Defocus %d fps\n", PQMaskMonitorMgrDefocus.Status.Model_Fps);
		//rtd_pr_vpq_info("PQMask SEWholeProc_Time %d (90k)\n", SEProcTimeMS);
		rtd_pr_vpq_info("PQMask Defocus SEPreProc_Time %d (90k)\n", PQMaskMonitorMgrDefocus.Status.SEPreProc_Time);
		rtd_pr_vpq_info("PQMask Defocus SEPostProc_Time %d (90k)\n", PQMaskMonitorMgrDefocus.Status.SEPostProc_Time);
		
		PQMaskMonitorMgrDefocus.Status.LogCounter = 0;
		// clear the error flags
		PQMaskMonitorMgrDefocus.Status.Model_FpsError = 0;
	}

	PQMaskMonitorMgrDefocus.Status.LogCounter++;
}
*/
...
void scalerPQMask_init(void) {
	...
	if( scalerPQMask_WaitSETaskFinish(60) != 0 ) {
		...
		up(&VPQ_PQMASK_Semaphore);
		return;
	}

	// change pqmaks srnn status
	if( PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_UNKNOWN && gSrnnTopInfo.ClkEn) {
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] PQmask Srnn Status is Unknown, need init\n", __FUNCTION__);
		PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_STANDBY;
	}

	// init procedure
	if( needInit == 1 ) {
		if (PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_STANDBY || PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_RUN){
			PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_WORK;
		}
		...
		scalerPQMask_disableModuleDMA(PQMASK_DOMAIN_I);
		scalerPQMask_disableModuleDMA(PQMASK_DOMAIN_D);
		if(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_WORK){
			scalerPQMask_disableModuleDMA(PQMASK_DOMAIN_SRNN);
		}

		// co-buffer content
		rtd_printk(KERN_INFO, TAG_NAME, "[%s] init co-buffer\n", __FUNCTION__);
		if( pNN->NN_flag_Addr[idx].pVir_addr_align == 0 || pNN->NN_flag_Addr[idx_defocus].pVir_addr_align == 0) {
			...
			//Defocus
			pPQMaskFlagDefocus = (PQMaskFlag*)(pNN->NN_flag_Addr[idx_defocus].pVir_addr_align);
			for(i=0; i<PQMASK_TUNNEL_NUM; i++) {
				pPQMaskFlagDefocus[i].TimeStamp = 0;
				pPQMaskFlagDefocus[i].Update = 0;
			}
		}
		...
		if( status == RET_OK ) {
			scalerPQMask_InitModuleConfig(PQMASK_DOMAIN_I);
			scalerPQMask_InitModuleConfig(PQMASK_DOMAIN_D);
			if(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_WORK ){
				scalerPQMask_InitModuleConfig(PQMASK_DOMAIN_SRNN);
				PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_RUN;
			}
		}
...
/*
	if(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_ASK_TO_SUSPEND){
		drvif_srnn_clk_enable_pqmasktask(0);
		PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_SUSPEND;
		rtd_printk(KERN_EMERG, TAG_NAME, "[%s] NNSR dbg, PQMASK_SRNN_STAT_SUSPEND \n", __FUNCTION__);
	}	
*/
	PQMaskGlbMgr.Status.PQMaskStatus = PQMASK_STAT_UNKOWN;

	PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_UNKNOWN;
	PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater = 0;
...
void scalerPQMask_suspend(void) {
	...
	if( PQMaskGlbMgr.Status.PQMaskStatus != PQMASK_STAT_UNKOWN ) {
		...
		PQMaskGlbMgr.Ctrl.bIsDefocusReady = 0;
	}
...
void scalerPQMask_postprocessing_entry(void) {
	...
	const int defocus_idx = VIP_NN_BUFFER_480FULLDEFOCUS;
	VIP_NN_CTRL *pNN = scalerAI_Access_NN_CTRL_STRUCT();
	PQMaskFlag *pPQMaskFlag = 0;
	PQMaskFlag *pPQMaskFlag_Defocus = 0;
	PQMASK_COLOR_MGR_T *pPQMaskColorMgr = scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_MGR);

	#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	if (ai_optee_buf_rw_trylock())
		return;
	#endif


	down(&VPQ_PQMASK_Semaphore);
	// will not turn off srnn clk
	/*
	// check srnn clk status, only entry for srnn clk off ifdef CONFIG_RTK_AI_DRV
	if (PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater == 1){
		if (PQMaskGlbMgr.Status.PQMaskSrnnStatus != PQMASK_SRNN_STAT_WORK){
			drvif_srnn_clk_enable_pqmasktask(0);
			PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_SUSPEND;
			PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater = 0;
	...
	*/
	...
	if( pNN->NN_flag_Addr[semantic_idx].pVir_addr_align == NULL ||
		pNN->NN_flag_Addr[defocus_idx].pVir_addr_align == NULL ||
		pNN->NN_info_Addr[semantic_idx].pVir_addr_align == NULL ||
		pNN->NN_info_Addr[depth_idx].pVir_addr_align == NULL ||
		pNN->NN_info_Addr[defocus_idx].pVir_addr_align == NULL) {
		goto finish;
	}
	...
	pPQMaskFlag_Defocus = (&((PQMaskFlag*)(pNN->NN_flag_Addr[defocus_idx].pVir_addr_align))[PQMASK_TUNNEL_A2K]);
	...
	if( pPQMaskColorMgr->Param.FusionRatio_Dep != 0 && PQMaskGlbMgr.Ctrl.bIsDepthReady == 0 && aipq_mode.depth_mode != 0) {
		goto finish;
	}
	
	if( pPQMaskColorMgr->Param.FusionRatio_Defocus != 0 && PQMaskGlbMgr.Ctrl.bIsDefocusReady == 0 && aipq_mode.defocus_mode != 0) {
		goto finish;
	}
	
	if( pPQMaskColorMgr->Param.FusionRatio_Sem != 0 && PQMaskGlbMgr.Ctrl.bIsSemReady == 0  && aipq_mode.semantic_mode != 0) {
		goto finish;
	}
	...
	if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_READY )	{
		...
		PQMaskGlbMgr.Ctrl.bDefocusNewInfo = ( pPQMaskFlag_Defocus->Update != 0 )? 1 : 0;
		...
		// copy defocus data from co-buffer
		if( PQMaskGlbMgr.Ctrl.bDefocusNewInfo != 0 ) {
			scalerPQMask_Memcpy(
				(void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEFOCUS].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_SRCBUF_DEFOCUS].AddrInfo.PhysAddr, 1,
				(void *)pNN->NN_info_Addr[defocus_idx].pVir_addr_align, 0, 1,
				AI_DEFOCUS_OUTPUT_WIDTH*AI_DEFOCUS_OUTPUT_HEIGHT*sizeof(char)
			);

			if( PQMaskMonitorMgrDefocus.Status.Model_Counter == 0 ) {
				PQMaskMonitorMgrDefocus.Status.Model_MeasureStartTime = drvif_Get_90k_Lo_clk();
			} else {
				PQMaskMonitorMgrDefocus.Status.Model_MeauserEndTime = drvif_Get_90k_Lo_clk();
			}
		}
		...
		if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_PREPARE ) {
			...
			pPQMaskFlag_Defocus->Update = 0;
			
			scalerPQMaskColor_SceneChange();
			scalerPQMaskColor_GetMotionFlag();
			...
			PQMaskMonitorMgrDefocus.Status.SECurWholeProcStart = drvif_Get_90k_Lo_clk();
			scalerPQMask_SETask(0);
			PQMaskMonitorMgr.Status.SECurProcEnd = drvif_Get_90k_Lo_clk();
			PQMaskMonitorMgrDefocus.Status.SECurWholeProcEnd= drvif_Get_90k_Lo_clk();
		}
	...
	//scalerPQMask_OnlineMonitorDefocus();
finish:
	up(&VPQ_PQMASK_Semaphore);

	#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	ai_optee_buf_rw_unlock();
	#endif
}

/**
 * @brief
 * pqmask post procssing in idomain
 */
...
void scalerPQMask_postprocessing_srnndomain(void) {
	...
	if(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_RUN)
	{
		PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_WORK;
		for( i=0; i<(sizeof(SRNNDomainModuleGrp)/sizeof(unsigned int)); i++ ) {
			...
			ModuleDmaIdx[moduleSel][DMA_IDX_NXT] = PQMaskRingBuf[moduleSel].FuncPeekIdx((void *)&PQMaskRingBuf[moduleSel], VIP_RINGBUF_PTR_READ);
			scalerPQMask_SetModuleAddr(moduleSel, PQMaskBufMgr.ModuleDma[moduleSel][ModuleDmaIdx[moduleSel][DMA_IDX_NXT]].AddrInfo.PhysAddr);
			PQMASKPrintf_Srnndomain("[%s][%-*s] cur = %d, nxt = %d\n",
				TAG_NAME, PQMASK_STRING_LEN, PQMaskRingBuf[moduleSel].Name, ModuleDmaIdx[moduleSel][DMA_IDX_CUR], ModuleDmaIdx[moduleSel][DMA_IDX_NXT]);
		}
 ...
 */
void scalerPQMask_ErrorHandler(void) { // scalerPQMask_RecoverHandler
	...
	if(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_STANDBY){
		PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_WORK;
		scalerPQMask_InitModuleConfig(PQMASK_DOMAIN_SRNN);
		PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_RUN;
	}
	...
	if( PQMaskGlbMgr.Status.PQMaskStatus != PQMASK_STAT_ERR ) {
		...
		if( scalerPQMask_CheckDMA() != RET_OK ) {
			bDmaFail = 1;
			PQMaskGlbMgr.Status.PQMaskStatus = PQMASK_STAT_ERR;
			rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] check dma error, status to %s\n", PQMaskStatusStr[PQMaskGlbMgr.Status.PQMaskStatus]);
		}
		if( scalerPQMask_CheckResolution() != RET_OK ) {
			bSizeFail = 1;
			PQMaskGlbMgr.Status.PQMaskStatus = PQMASK_STAT_ERR;
			rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] check module io size change, status to %s\n", PQMaskStatusStr[PQMaskGlbMgr.Status.PQMaskStatus]);
		}
		...
		if( PQMaskGlbMgr.Status.ErrRecoverTimes > 0 ) {
			...
			if( bDmaFail != 0 ) {
				if( scalerPQMask_DmaAllocate() != RET_OK ) {
					...
					bDmaFail = 0;
					rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] recover from dma error done\n");
				}
			}
			...
			if( bSizeFail != 0 ) {
				...
				bSizeFail = 0;
				rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] recover from module size change done\n");
			}

			// summary the status
			if( RecoverStatus != RET_OK ) {
				rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] recover fail with times %d\n", PQMaskGlbMgr.Status.ErrRecoverTimes);
				PQMaskGlbMgr.Status.ErrRecoverTimes--;
			} else {
				...
				if( PQMaskGlbMgr.Status.PQMaskStatus == PQMASK_STAT_ERR ) {
					PQMaskGlbMgr.Status.PQMaskStatus = PQMASK_STAT_READY;
					PQMaskGlbMgr.Status.ErrRecoverTimes = PQMaskGlbMgr.Ctrl.ErrRecoverRstTimes;
					rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] recover success\n");
				} else {
					rtd_printk(KERN_EMERG, TAG_NAME, "[scalerPQMask_RecoverHandler] try to change status to %s at status %s, recover flow interrupted!\n",
						 PQMaskStatusStr[PQMASK_STAT_READY], PQMaskStatusStr[PQMaskGlbMgr.Status.PQMaskStatus]
					);
				}
			}
...
/**
 * @brief
 * get pqmask srnn status, especially for srnn
 */
unsigned char scalerPQMask_GetPqmaskSrnnStatus(void){
	return PQMaskGlbMgr.Status.PQMaskSrnnStatus;
}

/**
 * @brief
 * set pqmask srnn status, especially for srnn
 */
void scalerPQMask_SetPqmaskSrnnStatus(unsigned char status){
	if(status >= PQMASK_SRNN_STAT_MAX){
		rtd_pr_vpq_emerg("[%s] Pqmask srnn status overflow!\n", __FUNCTION__);
		return;
	}

	switch(status){
		case PQMASK_SRNN_STAT_STANDBY:
			if (!(PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_WORK || PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_RUN)){
				PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_STANDBY;
				PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater = 0;
			}
			break;
		case PQMASK_SRNN_STAT_ASK_TO_SUSPEND:
			if (PQMaskGlbMgr.Status.PQMaskSrnnStatus == PQMASK_SRNN_STAT_WORK){
				PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater = 1;		// PQmask init is still running, ask_to_suspend later
				rtd_pr_vpq_info("[%s] PQMaskSrnnClkOffLater\n", __FUNCTION__);
			}
			else{
				PQMaskGlbMgr.Status.PQMaskSrnnStatus = PQMASK_SRNN_STAT_ASK_TO_SUSPEND;
				PQMaskGlbMgr.Status.PQMaskSrnnClkOffLater = 0;
			}
			break;
		default:
	...
	rtd_pr_vpq_info("[%s] pre_status=%d, cur_status=%d\n", __FUNCTION__, PQMaskGlbMgr.Status.PQMaskSrnnStatus, status);
}
	...
	for(py=0; py<(AI_SEMANTIC_OUTPUT_HEIGHT/vRatio); py++) {
		for(px=0; px<(AI_SEMANTIC_OUTPUT_WIDTH/hRatio); px++) {
			...
			if( labelValue >= PQMASK_LABEL_NUM )
				labelValue = 0;
			if( CurrentGenre == AI_GENRE_PATTERN && pPQMaskColorMgr->Param.bGenre_CG_Enable != 0){
				// show basic result when enter cg mode
				dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+0] = Alpha; // alpha
				dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+1] = 0; // red channel
				dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+2] = 0; // green channel
				dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+3] = 0; // blue channel

			}
			else{
				if (pPQMaskColorMgr->Status.MotionFlag == 1 && pPQMaskColorMgr->Param.bMotion_Enable != 0){
					dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+0] = Alpha; // alpha
					dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+1] = (labelValue==PQMASK_LABEL_TEXT)? OSDLabelColor[PQMASK_LABEL_BASIC][0] : OSDLabelColor[labelValue][0]; // red channel
					dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+2] = (labelValue==PQMASK_LABEL_TEXT)? OSDLabelColor[PQMASK_LABEL_BASIC][1] : OSDLabelColor[labelValue][1]; // green channel
					dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+3] = (labelValue==PQMASK_LABEL_TEXT)? OSDLabelColor[PQMASK_LABEL_BASIC][2] : OSDLabelColor[labelValue][2]; // blue channel
				}
				else{
					dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+0] = Alpha; // alpha
...
/**
 * @brief
 * set the AIPQ semantic demo mode by vendor
 * @param Mode enable or disable
 */
void scalerPQMask_SetSemanticDemoMode_Vendor(unsigned char Mode) {
	PQMASK_COLOR_DEMO_CTRL_T *pColorDemo = (PQMASK_COLOR_DEMO_CTRL_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DEMO);
	if ( pColorDemo == NULL){
		return;
	}
		
	pColorDemo->bSemanticDemoEn = Mode;
}

/**
 * @brief
 * fill the raw semantic data to dst buffer, the size is fixed to 480x256
 * @param dstBuf destination buffer
 */
void scalerPQMask_GetSemanticRawBuffer_Vendor(unsigned char *dstBuf) {
	unsigned char *pLabelBuffer;
	PQMASK_COLOR_DEMO_CTRL_T *pColorDemo = (PQMASK_COLOR_DEMO_CTRL_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DEMO);
	#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	if (ai_optee_buf_rw_trylock())
		return;
	...
	pLabelBuffer = (unsigned char *)(scalerAI_Access_NN_CTRL_STRUCT()->NN_info_Addr[VIP_NN_BUFFER_480FULL].pVir_addr_align);
	if ( pLabelBuffer == NULL || pColorDemo == NULL)
		goto finish;

	if ( pColorDemo->bSemanticDemoEn == 0)
		goto finish;

	scalerPQMask_Memcpy(
		(void *)dstBuf, 0, 0,
		(void *)pLabelBuffer, 0, 1,
		AI_SEMANTIC_OUTPUT_WIDTH*AI_SEMANTIC_OUTPUT_HEIGHT*sizeof(unsigned char)
	);
finish:
#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	ai_optee_buf_rw_unlock();
...
/**
 * @brief
 * set the AIPQ deepness demo mode by vendor
 * @param Mode enable or disable
 */
void scalerPQMask_SetDeepnessDemoMode_Vendor(unsigned char Mode) {
	PQMASK_COLOR_DEMO_CTRL_T *pColorDemo = (PQMASK_COLOR_DEMO_CTRL_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DEMO);
	if ( pColorDemo == NULL){
		return;
	}
		
	pColorDemo->bDeepnessDemoEn = Mode;
}

/**
 * @brief
 * fill the deepness data to dst buffer, the size is fixed to 480x256
 * @param dstBuf destination buffer
 */
void scalerPQMask_GetDeepnessRawBuffer_Vendor(unsigned char *dstBuf) {
	PQMASK_COLOR_DEMO_CTRL_T *pColorDemo = (PQMASK_COLOR_DEMO_CTRL_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DEMO);
	if ( pColorDemo == NULL)
		return;
	...
	scalerPQMask_Memcpy(
		(void *)dstBuf, 0, 0,
		(void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_DEMOBUF_00].AddrInfo.VirtAddr, (void *)PQMaskBufMgr.SeBuffer[PQMASK_SE_DEMOBUF_00].AddrInfo.PhysAddr, 1,
		AI_DEFOCUS_OUTPUT_WIDTH*AI_DEFOCUS_OUTPUT_HEIGHT*sizeof(unsigned char)
	);
}
#endif
...
/**
 * @brief
 * fill the defocus data to OSD buffer, input OSD size is fixed to 480x256
 * @param dstBuf destination buffer for whole OSD
 * @param hRatio full OSD buffer width(480) / real semantic data width
 * @param vRatio full OSD buffer height(256) / real semantic data height
 */
void scalerPQMask_GetDefocusARGBBuffer(unsigned char *dstBuf, unsigned char hRatio, unsigned char vRatio, unsigned char Alpha) {
	...
	int yStart = 0;
	int labelValue = 0;
	unsigned char *pLabelBuffer = (unsigned char *)(scalerAI_Access_NN_CTRL_STRUCT()->NN_info_Addr[VIP_NN_BUFFER_480FULLDEFOCUS].pVir_addr_align);

	if( pLabelBuffer == NULL )
		return;
	if( AI_DEFOCUS_OUTPUT_WIDTH/hRatio > OSD_WIDTH || AI_DEFOCUS_OUTPUT_HEIGHT/vRatio > OSD_HEIGHT )
		return;
	...
	// -- show at left bot
	xStart = 0;
	yStart = OSD_HEIGHT-(AI_DEFOCUS_OUTPUT_HEIGHT/vRatio);
	// -- show at right top
	// xStart = OSD_WIDTH-1-(AI_SEMANTIC_OUTPUT_WIDTH/hRatio);
	// yStart = 0;
	// -- show at right bot
	...
	for(py=0; py<(AI_DEFOCUS_OUTPUT_HEIGHT/vRatio); py++) {
		for(px=0; px<(AI_DEFOCUS_OUTPUT_WIDTH/hRatio); px++) {

			labelValue = pLabelBuffer[(py*vRatio)*AI_DEFOCUS_OUTPUT_WIDTH+(px*hRatio)];

			dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+0] = Alpha; // alpha
			dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+1] = labelValue; // red channel
			dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+2] = labelValue; // green channel
			dstBuf[((py+yStart)*OSD_WIDTH*4)+((px+xStart)*4)+3] = labelValue; // blue channel
		}
	}
}
...
void scalerPQMask_GetSemanticARGBBuffer_Coordinate2(
	...
	int py=0, px=0;
	int labelValue = 0;
	unsigned int lbuf_offset = 0;
	unsigned char *pLabelBuffer = (unsigned char *)(scalerAI_Access_NN_CTRL_STRUCT()->NN_info_Addr[VIP_NN_BUFFER_480FULL].pVir_addr_align);

	if( pLabelBuffer == NULL )
		return;
	if( hTarget==0 || vTarget==0 )
		return;

	for(py=0; py<vTarget; py++) {
		lbuf_offset = AI_SEMANTIC_OUTPUT_WIDTH*(py*AI_SEMANTIC_OUTPUT_HEIGHT/vTarget);
		for(px=0; px<hTarget; px++) {

			labelValue = pLabelBuffer[lbuf_offset+(px*AI_SEMANTIC_OUTPUT_WIDTH/hTarget)];
...
/**
 * @brief
 * draw the defocus result on osd
 * @param dstBuf
 * @param bufferWidth
 * @param xStart
 * @param yStart
...
void scalerPQMask_GetDefocusARGBBuffer_Coordinate(
	...
	for(py=0; py<(AI_DEFOCUS_OUTPUT_HEIGHT/vRatio); py++) {
		for(px=0; px<(AI_DEFOCUS_OUTPUT_WIDTH/hRatio); px++) {
			...
			dstBuf[ ( ((py+yStart)*bufferWidth)+(px+xStart) )*4 + 0 ] = Alpha; // alpha
			dstBuf[ ( ((py+yStart)*bufferWidth)+(px+xStart) )*4 + 1 ] = labelValue; // red channel
			dstBuf[ ( ((py+yStart)*bufferWidth)+(px+xStart) )*4 + 2 ] = labelValue; // green channel
			dstBuf[ ( ((py+yStart)*bufferWidth)+(px+xStart) )*4 + 3 ] = labelValue; // blue channel
...
void scalerPQMask_GetDefocusARGBBuffer_Coordinate2(
	...
	for(py=0; py<vTarget; py++) {
		lbuf_offset = AI_DEFOCUS_OUTPUT_WIDTH*(py*AI_DEFOCUS_OUTPUT_HEIGHT/vTarget);
		for(px=0; px<hTarget; px++) {
			labelValue = pLabelBuffer[lbuf_offset+(px*AI_DEFOCUS_OUTPUT_WIDTH/hTarget)];
...
void scalerPQMask_InitModuleConfig(unsigned char DomainItem) {
	...
	switch(DomainItem){
		case PQMASK_DOMAIN_I:
			drvif_module_set_pqmask_domain_db_enable(1, PQMASK_DOMAIN_I);
			// scaling and size setting, dma configuration
			scalerPQMask_SetBlkModuleIoSize(PQMASK_BLKMOD_SPNR);
			scalerPQMask_SetBlkModuleDmaConfig(PQMASK_BLKMOD_SPNR);
			// initial individual module pqmask weight plane address
			for(i=PQMASK_I_SNR; i<=PQMASK_I_DECONT; i++) {
			...
			// double buffer apply
			drvif_module_set_pqmask_domain_db_apply(PQMASK_DOMAIN_I);
			drvif_module_wait_pqmask_domain_db_apply_done(40, PQMASK_DOMAIN_I);
			break;
		case PQMASK_DOMAIN_D:
			drvif_module_set_pqmask_domain_db_enable(1, PQMASK_DOMAIN_D);
			// scaling and size setting, dma configuration
			for(i=PQMASK_BLKMOD_SHARPNESS; i<=PQMASK_BLKMOD_LOCALCONTRAST; i++) {
				scalerPQMask_SetBlkModuleIoSize(i);
				scalerPQMask_SetBlkModuleDmaConfig(i);
			}
			// initial individual module pqmask weight plane address
			for(i=PQMASK_D_SHP_EDGE; i<=PQMASK_MODULE_LC; i++) {
				scalerPQMask_SetModuleAddr(i, PQMaskBufMgr.ModuleDma[i][PQMaskRingBuf[i].FuncGetIdx((void *)&PQMaskRingBuf[i], VIP_RINGBUF_PTR_READ)].AddrInfo.PhysAddr);
			}
			// double buffer apply
			drvif_module_set_pqmask_domain_db_apply(PQMASK_DOMAIN_D);
			drvif_module_wait_pqmask_domain_db_apply_done(40, PQMASK_DOMAIN_D);
			break;
		case PQMASK_DOMAIN_SRNN:
			drvif_module_set_pqmask_domain_db_enable(1, PQMASK_DOMAIN_SRNN);
			// scaling and size setting, dma configuration
			for(i=PQMASK_BLKMOD_SRNN_IN; i<=PQMASK_BLKMOD_SRNN_OUT; i++) {
				scalerPQMask_SetBlkModuleIoSize(i);
				scalerPQMask_SetBlkModuleDmaConfig(i);
			}
			// initial individual module pqmask weight plane address
			for(i=PQMASK_SRNN_IN_0; i<=PQMASK_SRNN_OUT_0; i++) {
				scalerPQMask_SetModuleAddr(i, PQMaskBufMgr.ModuleDma[i][PQMaskRingBuf[i].FuncGetIdx((void *)&PQMaskRingBuf[i], VIP_RINGBUF_PTR_READ)].AddrInfo.PhysAddr);
			}
			// double buffer apply
			drvif_module_set_pqmask_domain_db_apply(PQMASK_DOMAIN_SRNN);
			drvif_module_wait_pqmask_domain_db_apply_done(40, PQMASK_DOMAIN_SRNN);
			break;
		default:
			rtd_printk(KERN_ERR, TAG_NAME, "[%s] DomainItem %d overflow\n", __FUNCTION__, DomainItem);
			break;
	}
	rtd_printk(KERN_INFO, TAG_NAME, "[%s] done\n", __FUNCTION__);
}
...
void scalerPQMask_disableModuleDMA(unsigned char DomainItem) {
	...
	switch(DomainItem){
		...
		case PQMASK_DOMAIN_D:
			// disable module enable
			for(i=PQMASK_D_SHP_EDGE; i<=PQMASK_D_LC; i++) {
				drvif_color_set_pqmask_module_enable(0, i);
			}
			// double buffer apply
			drvif_module_set_pqmask_domain_db_apply(PQMASK_DOMAIN_D);
...
void scalerPQMask_SetBlkModuleDmaConfig(unsigned char BlkModuleSel) {
	...
	unsigned int HSum = 0;
	PQMASK_DOMAIN_DMA_CONFIG_T DmaCfg;
	...
	BlockDmaCfg.LineStep = PQMASK_PROC_WIDTH >> 4; // PQMaskBlkModuleCfg[BlkModuleSel].IoSize.Input.Width >> 4; // LineAddrOffset (align 16Byte)

	switch(BlkModuleSel) {
		case PQMASK_BLKMOD_SPNR:
			scalerPQMask_GetDMALimit(PQMASK_DOMAIN_I, &DmaCfg, BlockDmaCfg.LineStep);
			drvif_color_set_pqmask_domain_DMA_config(&DmaCfg, PQMASK_DOMAIN_I);
			...
			if( pPQMaskColorMgr->ModuleSetting.Nr.bNrEnable != 0 ) {
				...
				drvif_color_set_pqmask_module_enable(0, PQMASK_I_SNR);
			}
			if( pPQMaskColorMgr->ModuleSetting.Nr.bIBlockDecontEnable != 0 || pPQMaskColorMgr->ModuleSetting.Nr.bIDecontEnable != 0 ) {
				...
				drvif_color_set_pqmask_module_enable(0, PQMASK_I_DECONT);
			}
			break;
		case PQMASK_BLKMOD_SHARPNESS:
			scalerPQMask_GetDMALimit(PQMASK_DOMAIN_D, &DmaCfg, BlockDmaCfg.LineStep);
			drvif_color_set_pqmask_domain_DMA_config(&DmaCfg, PQMASK_DOMAIN_D);
			...
			if( pPQMaskColorMgr->ModuleSetting.Sharpness.bLabelSharpnessEnable != 0 ) {
				drvif_color_set_pqmask_module_enable(1, PQMASK_D_SHP_EDGE);
				drvif_color_set_pqmask_module_enable(1, PQMASK_D_SHP_TEXT);
			} else {
				drvif_color_set_pqmask_module_enable(0, PQMASK_D_SHP_EDGE);
				drvif_color_set_pqmask_module_enable(0, PQMASK_D_SHP_TEXT);
			}
			break;
		...
		case PQMASK_BLKMOD_LOCALCONTRAST:
			...
			if( pPQMaskColorMgr->ModuleSetting.LocalContrast.bSharpEnable != 0 || pPQMaskColorMgr->ModuleSetting.LocalContrast.bToneMappingEnable != 0 ) {
				...
				drvif_color_set_pqmask_module_enable(0, PQMASK_D_LC);
			}
			break;
		case PQMASK_BLKMOD_SRNN_IN:
			scalerPQMask_GetDMALimit(PQMASK_DOMAIN_SRNN, &DmaCfg, BlockDmaCfg.LineStep);
			drvif_color_set_pqmask_domain_DMA_config(&DmaCfg, PQMASK_DOMAIN_SRNN);
...
void scalerPQMask_GetDMALimit(unsigned char domain, PQMASK_DOMAIN_DMA_CONFIG_T *DmaCfg, unsigned int LineStep) {
	...
	} else if( domain == PQMASK_DOMAIN_SRNN ) {
		for(i=PQMASK_MODULE_SRNN_IN_0; i<=PQMASK_MODULE_SRNN_OUT_0; i++) {
			for(j=0; j<PQMASK_MODULE_DMA_NUM; j++) {
				if( DmaCfg->AddrUpLimit <= PQMaskBufMgr.ModuleDma[i][j].AddrInfo.PhysAddr) {
					DmaCfg->AddrUpLimit = PQMaskBufMgr.ModuleDma[i][j].AddrInfo.PhysAddr;
				}
	...
	DmaCfg->AddrUpLimit = DmaCfg->AddrUpLimit + 2*(LineStep<<4);
	DmaCfg->AddrUpLimit = DmaCfg->AddrUpLimit >> 4;
