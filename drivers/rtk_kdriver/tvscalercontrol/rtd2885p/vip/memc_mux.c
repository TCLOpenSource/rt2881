...
#include <rbus/M8P_rtmc_ippre_reg.h>
...
void drvif_memc_internal_Dither_ctrl(bool enable, bool temp_en)
{
	M8P_rtmc_ippre_ippre_dither_ctrl1_RBUS color_temp_ctrl_reg;
	color_temp_ctrl_reg.regValue = rtd_inl(M8P_RTMC_IPPRE_IPPRE_DITHER_CTRL1_reg);
	color_temp_ctrl_reg.dither_en = enable;
	color_temp_ctrl_reg.dither_temporal_enable = temp_en;
	rtd_outl(M8P_RTMC_IPPRE_IPPRE_DITHER_CTRL1_reg, color_temp_ctrl_reg.regValue);
}
