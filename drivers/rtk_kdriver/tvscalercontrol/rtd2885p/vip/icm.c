...
#include <rbus/M8P_color_icm_reg.h>
...
char drvif_Wait_DMAto3DLUT_frame_act_fw(void)
{
	UINT32 timeoutcnt = 0x032500;

	do {
		if(DMATO3DLUT_DMAto3DLUT_Table0_Status_get_frame_act_fw(IoReg_Read32(DMATO3DLUT_DMAto3DLUT_Table0_Status_reg))) {
			return 0;
		}
	} while(timeoutcnt-- != 0);

	if(timeoutcnt == 0){
		rtd_pr_vpq_emerg("%s ERROR not wait the correct frame act fw \n", __FUNCTION__);
	}
	return -1;
}
...
//	buf = (unsigned int *)dvr_malloc_specific(buf_size * sizeof(unsigned int), GFP_DCU2_FIRST);
	...
	drvif_Wait_DMAto3DLUT_frame_act_fw();
...
void drvif_color_Icm_ctrl_YUV2RGB(unsigned char data_format_sel, unsigned char data_range_sel)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P) { // merlin8 IC , run merlin8 flow
		...
		M8P_color_icm_dm_icm_ctrl_RBUS				  dm_icm_ctrl_reg;
		dm_icm_ctrl_reg.regValue = IoReg_Read32(M8P_COLOR_ICM_DM_ICM_CTRL_reg);

		dm_icm_ctrl_reg.data_format_sel = data_format_sel;
		dm_icm_ctrl_reg.data_range_sel = data_range_sel;

		IoReg_Write32(M8P_COLOR_ICM_DM_ICM_CTRL_reg, dm_icm_ctrl_reg.regValue);
...
void drvif_color_icm_pqmask_SatHue_byhue_Ctrl(unsigned char obj1_hue_byhue_en,
										      unsigned char obj2_hue_byhue_en,
										      unsigned char *p_obj1_hue_GainByHue_curve,
										   	  unsigned char *p_obj2_hue_GainByHue_curve,
										   	  unsigned char obj1_sat_byhue_en,
											  unsigned char obj2_sat_byhue_en,
											  unsigned char *p_obj1_sat_GainByHue_Curve,
											  unsigned char *p_obj2_sat_GainByHue_Curve,
										      unsigned int obj1_target_hue,
										      unsigned int obj2_target_hue)
{
	/*fix me later, this part is hue by hue.*/
	...
	// workaround for DIA2DEV-261, en off -> write curve
	color_icm_d_icm_pq_mask_enable_REG.regValue = IoReg_Read32(COLOR_ICM_D_ICM_PQ_MASK_ENABLE_reg);
	color_icm_d_icm_pq_mask_enable_REG.pq_mask_obj1_sat_byhue_enable = 0;
	color_icm_d_icm_pq_mask_enable_REG.pq_mask_obj2_sat_byhue_enable = 0;
	IoReg_Write32(COLOR_ICM_D_ICM_PQ_MASK_ENABLE_reg,color_icm_d_icm_pq_mask_enable_REG.regValue);

	color_icm_d_icm_pq_mask_enable_REG.regValue = IoReg_Read32(COLOR_ICM_D_ICM_PQ_MASK_ENABLE_reg);
	color_icm_d_icm_pq_mask_enable_REG.pq_mask_obj1_hue_byhue_enable = 0;
	color_icm_d_icm_pq_mask_enable_REG.pq_mask_obj2_hue_byhue_enable = 0;
	IoReg_Write32(COLOR_ICM_D_ICM_PQ_MASK_ENABLE_reg,color_icm_d_icm_pq_mask_enable_REG.regValue);
	...
	/* write sat_gainbyhue*/
	for(i=0;i<24;i++){
        color_icm_dm_icm_gainby_hue_sram_ctrl_REG.gainby_hue_read_sel = 0;
        color_icm_dm_icm_gainby_hue_sram_ctrl_REG.gainby_hue_write_en = 0;
        color_icm_dm_icm_gainby_hue_sram_ctrl_REG.gainby_hue_read_en = 0;
        color_icm_dm_icm_gainby_hue_sram_ctrl_REG.gainby_hue_addr = i;
		IoReg_Write32(COLOR_ICM_DM_ICM_GAINBY_HUE_SRAM_CTRL_reg,color_icm_dm_icm_gainby_hue_sram_ctrl_REG.regValue);

		color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.wr_gainby_hue_curve_s0 = p_obj1_sat_GainByHue_Curve[i*2];
		color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.wr_gainby_hue_curve_h0 = p_obj1_hue_GainByHue_curve[i*2];				// step2: set obj1 satGainByHue 2*i
		color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.wr_gainby_hue_curve_s1 = p_obj2_sat_GainByHue_Curve[i*2];
		color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.wr_gainby_hue_curve_h1 = p_obj2_hue_GainByHue_curve[i*2];				// step4: set obj2 satGainByHue 2*i
		IoReg_Write32(COLOR_ICM_DM_ICM_GAINBY_HUE_SRAM_WR_DATA0_reg, color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.regValue);

		color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.wr_gainby_hue_curve_s0_1 = p_obj1_sat_GainByHue_Curve[i*2+1];
		color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.wr_gainby_hue_curve_h0_1 = p_obj1_hue_GainByHue_curve[i*2+1];			// step3: set obj1 satGainByHue 2*i + 1
		color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.wr_gainby_hue_curve_s1_1 = p_obj2_sat_GainByHue_Curve[i*2+1];
		color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.wr_gainby_hue_curve_h1_1 = p_obj2_hue_GainByHue_curve[i*2+1];			// step5: set obj2 satGainByHue 2*i + 1
		IoReg_Write32(COLOR_ICM_DM_ICM_GAINBY_HUE_SRAM_WR_DATA1_reg, color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.regValue);

		printk(KERN_EMERG "[WILLY] iter %d\n", i);
		printk(KERN_EMERG "[WILLY] 0x%x, 0x%x\n", color_icm_dm_icm_gainby_hue_sram_wr_data0_REG.regValue, color_icm_dm_icm_gainby_hue_sram_wr_data1_REG.regValue);
