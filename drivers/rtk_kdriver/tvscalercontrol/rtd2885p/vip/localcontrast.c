...
char drvif_color_set_LC_DB(unsigned char Enable, unsigned char Level, unsigned char Apply)
{
	lc_lc_global_ctrl1_RBUS lc_lc_global_ctrl1_reg;

	lc_lc_global_ctrl1_reg.regValue = IoReg_Read32(LC_LC_Global_Ctrl1_reg);
	lc_lc_global_ctrl1_reg.lc_db_en = Enable;
	lc_lc_global_ctrl1_reg.lc_db_read_level = Level;
	lc_lc_global_ctrl1_reg.lc_db_apply = Apply;
	IoReg_Write32(LC_LC_Global_Ctrl1_reg, lc_lc_global_ctrl1_reg.regValue);
	return 0;
}

char drvif_color_Get_LC_DB_Status(unsigned char *pEnable, unsigned char *pLevel, unsigned char *pApply)
{
	lc_lc_global_ctrl1_RBUS lc_lc_global_ctrl1_reg;

	if ((pEnable == NULL) || (pLevel == NULL) || (pApply == NULL))
	{
		return -1;
	}

	lc_lc_global_ctrl1_reg.regValue = IoReg_Read32(LC_LC_Global_Ctrl1_reg);
	*pEnable = lc_lc_global_ctrl1_reg.lc_db_en;
	*pLevel = lc_lc_global_ctrl1_reg.lc_db_read_level;
	*pApply = lc_lc_global_ctrl1_reg.lc_db_apply;
...
void drvif_color_set_LC_texture(DRV_LC_Texture *tex)
{
	...
	{
		M8P_lc_lc_texture_gain_RBUS lc_lc_texture_gain_reg;

		M8P_lc_lc_texture_tmap_curve0_RBUS lc_lc_texture_tmap_curve0_reg;
		M8P_lc_lc_texture_tmap_curve1_RBUS lc_lc_texture_tmap_curve1_reg;
		M8P_lc_lc_texture_tmap_curve2_RBUS lc_lc_texture_tmap_curve2_reg;
		M8P_lc_lc_texture_tmap_curve3_RBUS lc_lc_texture_tmap_curve3_reg;
		M8P_lc_lc_texture_tmap_curve4_RBUS lc_lc_texture_tmap_curve4_reg;

		M8P_lc_lc_texture_shp_curve0_RBUS lc_lc_texture_shp_curve0_reg;
		M8P_lc_lc_texture_shp_curve1_RBUS lc_lc_texture_shp_curve1_reg;
		M8P_lc_lc_texture_shp_curve2_RBUS lc_lc_texture_shp_curve2_reg;
		M8P_lc_lc_texture_shp_curve3_RBUS lc_lc_texture_shp_curve3_reg;
		M8P_lc_lc_texture_shp_curve4_RBUS lc_lc_texture_shp_curve4_reg;

		M8P_lc_lc_global_ctrl1_RBUS blu_lc_global_ctrl1;

		int i,j;
		int step[17] = {0,16,32,48,64,80,96,112,128,144,160,176,192,208,224,240,255};
		int tmap_curve[17] = {0};
		int shp_curve[17] = {0};
		int x_idx[5] = {0};

		
		lc_lc_texture_gain_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_Gain_reg);
		lc_lc_texture_gain_reg.lc_tex_gain = tex->lc_tex_ctrl.lc_tex_gain;
		lc_lc_texture_gain_reg.lc_tex_shp_en= tex->lc_tex_ctrl.lc_tex_shp_en;
		lc_lc_texture_gain_reg.lc_tex_tmap_en= tex->lc_tex_ctrl.lc_tex_tmap_en;
		lc_lc_texture_gain_reg.lc_tex_offset = 0; // merlin8 doesn't have this reg
		IoReg_Write32(M8P_LC_LC_Texture_Gain_reg, lc_lc_texture_gain_reg.regValue);

		// interpolation 5 point curve to 17 point curve
		x_idx[0] = 0;
		x_idx[1] = step[tex->lc_tex_tmap.th[0]/16];
		x_idx[2] = step[tex->lc_tex_tmap.th[1]/16];
		x_idx[3] = step[tex->lc_tex_tmap.th[2]/16];
		x_idx[4] = 255;
		...
		{
			if(step[i] == 255)
				tmap_curve[i] = tex->lc_tex_tmap.curve[4];
			else
			{
				for(j=0;j<4;j++)
				{
					if(step[i]>=x_idx[j] && step[i]<x_idx[j+1])
					{
						tmap_curve[i] = (((x_idx[j+1] - step[i])*tex->lc_tex_tmap.curve[j]) + ((step[i] - x_idx[j])*tex->lc_tex_tmap.curve[j+1]))/(x_idx[j+1] - x_idx[j]);
						break;
					}
		...
		lc_lc_texture_tmap_curve0_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_tmap_curve0_reg);
		lc_lc_texture_tmap_curve0_reg.lc_tex_tmap_curve0 = tmap_curve[0];
		lc_lc_texture_tmap_curve0_reg.lc_tex_tmap_curve1 = tmap_curve[1];
		lc_lc_texture_tmap_curve0_reg.lc_tex_tmap_curve2 = tmap_curve[2];
		IoReg_Write32(M8P_LC_LC_Texture_tmap_curve0_reg, lc_lc_texture_tmap_curve0_reg.regValue);

		lc_lc_texture_tmap_curve1_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_tmap_curve1_reg);
		lc_lc_texture_tmap_curve1_reg.lc_tex_tmap_curve3 = tmap_curve[3];
		lc_lc_texture_tmap_curve1_reg.lc_tex_tmap_curve4 = tmap_curve[4];
		IoReg_Write32(M8P_LC_LC_Texture_tmap_curve1_reg, lc_lc_texture_tmap_curve1_reg.regValue);

		lc_lc_texture_tmap_curve2_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_tmap_curve2_reg);
		lc_lc_texture_tmap_curve2_reg.lc_tex_tmap_curve5 = tmap_curve[5];
		lc_lc_texture_tmap_curve2_reg.lc_tex_tmap_curve6 = tmap_curve[6];
		lc_lc_texture_tmap_curve2_reg.lc_tex_tmap_curve7 = tmap_curve[7];
		lc_lc_texture_tmap_curve2_reg.lc_tex_tmap_curve8 = tmap_curve[8];
		IoReg_Write32(M8P_LC_LC_Texture_tmap_curve2_reg, lc_lc_texture_tmap_curve2_reg.regValue);

		lc_lc_texture_tmap_curve3_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_tmap_curve3_reg);
		lc_lc_texture_tmap_curve3_reg.lc_tex_tmap_curve9 = tmap_curve[9];
		lc_lc_texture_tmap_curve3_reg.lc_tex_tmap_curve10 = tmap_curve[10];
		lc_lc_texture_tmap_curve3_reg.lc_tex_tmap_curve11 = tmap_curve[11];
		lc_lc_texture_tmap_curve3_reg.lc_tex_tmap_curve12 = tmap_curve[12];
		IoReg_Write32(M8P_LC_LC_Texture_tmap_curve3_reg, lc_lc_texture_tmap_curve3_reg.regValue);

		lc_lc_texture_tmap_curve4_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_tmap_curve4_reg);
		lc_lc_texture_tmap_curve4_reg.lc_tex_tmap_curve13 = tmap_curve[13];
		lc_lc_texture_tmap_curve4_reg.lc_tex_tmap_curve14 = tmap_curve[14];
		lc_lc_texture_tmap_curve4_reg.lc_tex_tmap_curve15 = tmap_curve[15];
		lc_lc_texture_tmap_curve4_reg.lc_tex_tmap_curve16 = tmap_curve[16];
		IoReg_Write32(M8P_LC_LC_Texture_tmap_curve4_reg, lc_lc_texture_tmap_curve4_reg.regValue);

		// interpolation 5 point curve to 17 point curve
		x_idx[0] = 0;
		x_idx[1] = step[tex->lc_tex_shp.th[0]/16];
		x_idx[2] = step[tex->lc_tex_shp.th[1]/16];
		x_idx[3] = step[tex->lc_tex_shp.th[2]/16];
		x_idx[4] = 255;
		
		for(i=0;i<17;i++)
		{
			if(step[i] == 255)
				shp_curve[i] = tex->lc_tex_shp.curve[4];
			else
			{
				for(j=0;j<4;j++)
				{
					if(step[i]>=x_idx[j] && step[i]<x_idx[j+1])
					{
						shp_curve[i] = (((x_idx[j+1] - step[i])*tex->lc_tex_shp.curve[j]) + ((step[i] - x_idx[j])*tex->lc_tex_shp.curve[j+1]))/(x_idx[j+1] - x_idx[j]);
						break;
					}
		...
		lc_lc_texture_shp_curve0_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_shp_curve0_reg);
		lc_lc_texture_shp_curve0_reg.lc_tex_shp_curve0 = shp_curve[0];
		lc_lc_texture_shp_curve0_reg.lc_tex_shp_curve1 = shp_curve[1];
		lc_lc_texture_shp_curve0_reg.lc_tex_shp_curve2 = shp_curve[2];
		IoReg_Write32(M8P_LC_LC_Texture_shp_curve0_reg, lc_lc_texture_shp_curve0_reg.regValue);

		lc_lc_texture_shp_curve1_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_shp_curve1_reg);
		lc_lc_texture_shp_curve1_reg.lc_tex_shp_curve3 = shp_curve[3];
		lc_lc_texture_shp_curve1_reg.lc_tex_shp_curve4 = shp_curve[4];
		IoReg_Write32(M8P_LC_LC_Texture_shp_curve1_reg, lc_lc_texture_shp_curve1_reg.regValue);

		lc_lc_texture_shp_curve2_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_shp_curve2_reg);
		lc_lc_texture_shp_curve2_reg.lc_tex_shp_curve5 = shp_curve[5];
		lc_lc_texture_shp_curve2_reg.lc_tex_shp_curve6 = shp_curve[6];
		lc_lc_texture_shp_curve2_reg.lc_tex_shp_curve7 = shp_curve[7];
		lc_lc_texture_shp_curve2_reg.lc_tex_shp_curve8 = shp_curve[8];
		IoReg_Write32(M8P_LC_LC_Texture_shp_curve2_reg, lc_lc_texture_shp_curve2_reg.regValue);

		lc_lc_texture_shp_curve3_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_shp_curve3_reg);
		lc_lc_texture_shp_curve3_reg.lc_tex_shp_curve9 = shp_curve[9];
		lc_lc_texture_shp_curve3_reg.lc_tex_shp_curve10 = shp_curve[10];
		lc_lc_texture_shp_curve3_reg.lc_tex_shp_curve11 = shp_curve[11];
		lc_lc_texture_shp_curve3_reg.lc_tex_shp_curve12 = shp_curve[12];
		IoReg_Write32(M8P_LC_LC_Texture_shp_curve3_reg, lc_lc_texture_shp_curve3_reg.regValue);

		lc_lc_texture_shp_curve4_reg.regValue = IoReg_Read32(M8P_LC_LC_Texture_shp_curve4_reg);
		lc_lc_texture_shp_curve4_reg.lc_tex_shp_curve13 = shp_curve[13];
		lc_lc_texture_shp_curve4_reg.lc_tex_shp_curve14 = shp_curve[14];
		lc_lc_texture_shp_curve4_reg.lc_tex_shp_curve15 = shp_curve[15];
		lc_lc_texture_shp_curve4_reg.lc_tex_shp_curve16 = shp_curve[16];
		IoReg_Write32(M8P_LC_LC_Texture_shp_curve4_reg, lc_lc_texture_shp_curve4_reg.regValue);
...
/* merlin8p new register */
void drvif_color_set_LC_shp_diff_gain(DRV_LC_Shp_DiffGain *diff_gain)
{
	if(get_mach_type()==MACH_ARCH_RTK2885PP) //merlin8p IC , run merlin8p flow
	{
		M8P_lc_lc_local_shpnr_diff_gain_RBUS lc_lc_local_shpnr_diff_gain_reg;
		M8P_lc_lc_global_ctrl1_RBUS blu_lc_global_ctrl1;
		
		lc_lc_local_shpnr_diff_gain_reg.regValue = IoReg_Read32(M8P_LC_LC_Local_Shpnr_diff_gain_reg);
		lc_lc_local_shpnr_diff_gain_reg.lc_shp_positive_negative_diff_gain_en = diff_gain->diff_gain_en;
		lc_lc_local_shpnr_diff_gain_reg.lc_shp_positive_diff_gain = diff_gain->positive_diff_gain;
		lc_lc_local_shpnr_diff_gain_reg.lc_shp_negative_diff_gain = diff_gain->negative_diff_gain;
		IoReg_Write32(M8P_LC_LC_Local_Shpnr_diff_gain_reg, lc_lc_local_shpnr_diff_gain_reg.regValue);

		//double buffer apply
		blu_lc_global_ctrl1.regValue = IoReg_Read32(M8P_LC_LC_Global_Ctrl1_reg);
		blu_lc_global_ctrl1.lc_db_apply = 1;
		IoReg_Write32(M8P_LC_LC_Global_Ctrl1_reg, blu_lc_global_ctrl1.regValue);
	}
}