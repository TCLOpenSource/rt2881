...
extern void fwif_color_ChangeUINT32Endian_Copy(unsigned int *pwTemp, int nSize, unsigned int *pwCopyTemp, unsigned char convert_endian_flag);
extern unsigned int debug_decontour_blkinfo; // for rtice 2265 
/*================================ Definitions ==============================*/
/*=======================*/
...
void drvif_IESM_format_convert(unsigned short display)// Merlin4 add
{
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		else if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_THRIP) == TRUE)// merlin4: after di path is 422
		{
			...
			((get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_HDMI) || (get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_MINI_DP)))
		{
			edsm_format_convert_reg.main_444_to_422_en = 0;
			edsm_format_convert_reg.main_422_to_444_en = 0;
		}
...
void drvif_IESM_frontporch_set(unsigned char game_mode_flag)
{
	...
	else if(front_porch_num >= front_porch_num_17line && I_width <= WIDTH_FOR_EDGESMOOTH_DECIDE_LINE_MODE   && I_height>8)
		linemode_limit = 3; // 17 line mode
	else if(front_porch_num >= 6 && (fmt_422to444==1 || I_width <= WIDTH_FOR_EDGESMOOTH_DECIDE_LINE_MODE  ) && I_height>5)
		linemode_limit = 2; // 11 line mode
	else if(front_porch_num >= 3 && I_height>2)
		linemode_limit = 1; // 5 line mode
	else
		linemode_limit = 0; // disable
#endif
	...
	rtd_pr_vpq_info("front_porch_num=vlen-actlen-actsta: %d=%d-%d-%d, game_mode=%d,size=%d-%d,flag=%d,lineMode=%d,fmt_422to444=%d,PC_mode_check=%d,\n",
	...
	isGameMode, I_width, I_height, game_mode_flag, linemode_limit, fmt_422to444, PC_mode_check);
...
void drvif_color_Set_Block_DeContour_MAD(DRV_Block_DeContour_MAD *ptr) // ml7
{
	...
	//M8P_ipq_decontour_blk_de_contour_mad_info_4_RBUS blk_de_contour_mad_info_4_reg;
	...
	if(get_mach_type()==MACH_ARCH_RTK2885P2)// ml8p_add
	{
		M8P_ipq_decontour_blk_de_contour_mad_info_4_RBUS blk_de_contour_mad_info_4_reg;
		blk_de_contour_mad_info_4_reg.regValue = IoReg_Read32(M8P_IPQ_DECONTOUR_BLK_De_Contour_mad_info_4_reg);
		blk_de_contour_mad_info_4_reg.bdecont_mad_shift = ptr->bdecont_mad_shift;
		IoReg_Write32(M8P_IPQ_DECONTOUR_BLK_De_Contour_mad_info_4_reg, blk_de_contour_mad_info_4_reg.regValue);
	}
...
void drvif_color_Block_DeContour_enable_recover_flow(void)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		nr_nr_dummy_RBUS nr_dummy_reg;
		nr_dummy_reg.regValue = IoReg_Read32(NR_NR_DUMMY_reg);

		nr_dummy_reg.dummy18025598_31_16 = (nr_dummy_reg.dummy18025598_31_16) | (0x8000);
		IoReg_Write32(NR_NR_DUMMY_reg, nr_dummy_reg.regValue);
	}
}
...
unsigned char drvif_color_Block_DeContour_run_recover_flow(void)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		//0xb8025598[31] to control the flow
		return (nr_dummy_reg.dummy18025598_31_16>>15);
	}
	else
		return 0;
}
...
unsigned char drvif_color_Block_DeContour_debug_recover_flow(void)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		// 0xb8025598[30] to enable log
		return ((nr_dummy_reg.dummy18025598_31_16>>14) & 0x1);
	}
	else
		return 0;
}
...
unsigned char drvif_color_Block_DeContour_check_dma_errFlag(void)
{
	...
	{
		bool err_status = 0;
		ipq_decontour_bdecont_dma_err_status_RBUS bdecont_dma_err_status_reg;
		ipq_decontour_bdecont_dma_wr_status_RBUS bdecont_dma_wr_status_reg;

		bdecont_dma_err_status_reg.regValue = IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_Err_Status_reg);
		bdecont_dma_wr_status_reg.regValue = IoReg_Read32(IPQ_DECONTOUR_BDECONT_DMA_WR_status_reg);

		err_status = (bdecont_dma_wr_status_reg.wr_data_fifo_underflow_err)||
			(bdecont_dma_err_status_reg.wdma_fifo_ovfl)/*||
			(bdecont_dma_wr_status_reg.wr_dma_data_water)*/;

		if(err_status==1 && drvif_color_Block_DeContour_debug_recover_flow())
			rtd_pr_vpq_emerg("lsy %s(%d)  err_status = %d (%d %d %d)\n",__func__, __LINE__, err_status, (bdecont_dma_wr_status_reg.wr_data_fifo_underflow_err), (bdecont_dma_err_status_reg.wdma_fifo_ovfl), (bdecont_dma_wr_status_reg.wr_dma_data_water));
...
void drvif_color_Block_DeContour_clear_wr_force_all_rst(unsigned char value)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		// need to set wr_force_all_rst=1 and next frame wr_force_all_rst=0
		bdecont_dma_wr_ctrl_reg.wr_force_all_rst = value;
...
void drvif_color_Block_DeContour_clear_wr_data_fifo_underflow_err(void)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		// wr_data_fifo_underflow_err is write 1 clear
		bdecont_dma_wr_status_reg.wr_data_fifo_underflow_err = 1;
		IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_WR_status_reg, bdecont_dma_wr_status_reg.regValue);	
				
		//rtd_pr_vpq_emerg("lsy %s(%d)  \n",__func__, __LINE__);
	}
}
...
void drvif_color_Block_DeContour_clear_wdma_fifo_ovfl_clr(unsigned char value)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		// to clear wdma_fifo_ovfl, need to set wdma_fifo_ovfl_clr=1 and next frame wdma_fifo_ovfl_clr=0
		bdecont_dma_err_status_reg.wdma_fifo_ovfl_clr = value;
		IoReg_Write32(IPQ_DECONTOUR_BDECONT_DMA_Err_Status_reg, bdecont_dma_err_status_reg.regValue);	
...
void drvif_color_Block_DeContour_set_snr_ssb_arb_en(unsigned char enable)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		snr_ssb_snr_ssb_arb_cr1_RBUS 	snr_ssb_arb_cr1_reg;	
		snr_ssb_arb_cr1_reg.regValue 	= IoReg_Read32(SNR_SSB_SNR_SSB_ARB_CR1_reg);

		//rtd_pr_vpq_emerg("lsy %s(%d)  arb_en = %d \n",__func__, __LINE__, enable);
		snr_ssb_arb_cr1_reg.arb_en = (snr_ssb_arb_cr1_reg.arb_en & (0x37)) | (enable<<3); // bit3: blk_decont write dma

		IoReg_Write32(SNR_SSB_SNR_SSB_ARB_CR1_reg, snr_ssb_arb_cr1_reg.regValue);
	}
}

extern void fw_scalerip_set_SPNR_size(void);

void drvif_color_Block_DeContour_set_dma_config(bool isErrorFlow)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		//rtd_pr_vpq_emerg("lsy %s(%d)  errorFlow = %d \n",__func__, __LINE__, errorFlow);
		
		if(isErrorFlow==1)
		{
			bdecont_wddr_dma_wtlennum_reg.wr_dat_len = 8;
			bdecont_wddr_dma_wtlennum_reg.wr_dat_num = 5;
			bdecont_wddr_datdma_rm_reg.wr_dat_remain = 3;
		...
		else
		{
			...
			{
				rtd_pr_vpq_emerg("fw_scalerip_set_I_BLK_decontour, no buffer\n");
				return;
			}
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				MemEndtAddr = nonlibdma_drv_memory_blk_decontour_Config(bdecont_dma->phy_addr_align);
			}
			rtd_pr_vpq_info("lsy %s(%d) cma phy mem_addr=%x~%x \n", __func__, __LINE__, bdecont_dma->phy_addr_align, MemEndtAddr);
...
/*void drvif_color_Block_DeContour_set_dma_enable(unsigned char enable)
{
	...
	bdecont_dma_rd_ctrl_reg.rd_dma_enable = enable;
	bdecont_dma_wr_ctrl_reg.wr_dma_enable = enable;
...
void drvif_color_Get_Block_DeContour_MAD(void)// ml8_add
{
	if(get_mach_type()==MACH_ARCH_RTK2885P)
	{
		...
		if(debug_decontour_blkinfo>0)
		{
			for(i=0; i<20; i++)
			{
				fwif_color_ChangeUINT32Endian_Copy(virAddr+i+16*(debug_decontour_blkinfo-1), 1, &tmp, 1);
				rtd_pr_vpq_info( "lsy (%d)  %d, %d, %x -> %x  \n", __LINE__, debug_decontour_blkinfo, i+16*(debug_decontour_blkinfo-1), *(virAddr+i+16*(debug_decontour_blkinfo-1)), tmp);
			}
		}
		...
		for(i=0; i<cnt16; i++)
		{
			...
			fwif_color_ChangeUINT32Endian_Copy(virAddr+20*i, 20, &tmpbuf[0], 1);
			pblk_info = (blk_decont_info *)(&tmpbuf[0]);

			if(debug_decontour_blkinfo-1==i)
			{
				rtd_pr_vpq_info( "lsy (%d) %x %x %x %x \n", __LINE__, pblk_info->u2_h, pblk_info->y2, pblk_info->m2, pblk_info->v3_l);
				rtd_pr_vpq_info( "lsy (%d) %x %x %x %x \n", __LINE__,pblk_info->y1_h, pblk_info->m1,pblk_info->v2,pblk_info->u2_l);
				rtd_pr_vpq_info( "lsy (%d) %x %x %x %x \n", __LINE__,pblk_info->m0_h, pblk_info->v1,pblk_info->u1,pblk_info->y1_l);
				rtd_pr_vpq_info( "lsy (%d) %x %x %x %x \n", __LINE__,pblk_info->v0,pblk_info->u0,pblk_info->y0,pblk_info->m0_l);
				rtd_pr_vpq_info( "lsy (%d) %x %x %x %x \n", __LINE__,pblk_info->y5_h,pblk_info->m5,pblk_info->v6,pblk_info->u6_l);

				rtd_pr_vpq_info( "lsy (%d) blk0 %x %x %x %x \n", __LINE__,(pblk_info->m0_h<<2) + pblk_info->m0_l, pblk_info->y0, pblk_info->u0, pblk_info->v0);
				rtd_pr_vpq_info( "lsy (%d) blk1 %x %x %x %x \n", __LINE__,(pblk_info->m1), (pblk_info->y1_h<<4)+pblk_info->y1_l, pblk_info->u1, pblk_info->v1);
				rtd_pr_vpq_info( "lsy (%d) blk2 %x %x %x %x \n", __LINE__,(pblk_info->m2), pblk_info->y2, (pblk_info->u2_h<<6)+pblk_info->u2_l, pblk_info->v2);
				rtd_pr_vpq_info( "lsy (%d) blk3 %x %x %x %x \n", __LINE__,(pblk_info->m3), pblk_info->y3, pblk_info->u3, (pblk_info->v3_h<<8)+pblk_info->v3_l);

				rtd_pr_vpq_info( "lsy (%d) blk4 %x %x %x %x \n", __LINE__,(pblk_info->m4_h<<2) + pblk_info->m4_l, pblk_info->y4, pblk_info->u4, pblk_info->v4);
				rtd_pr_vpq_info( "lsy (%d) blk5 %x %x %x %x \n", __LINE__,(pblk_info->m5), (pblk_info->y5_h<<4)+pblk_info->y5_l, pblk_info->u5, pblk_info->v5);
				rtd_pr_vpq_info( "lsy (%d) blk6 %x %x %x %x \n", __LINE__,(pblk_info->m6), pblk_info->y6, (pblk_info->u6_h<<6)+pblk_info->u6_l, pblk_info->v6);
				rtd_pr_vpq_info( "lsy (%d) blk7 %x %x %x %x \n", __LINE__,(pblk_info->m7), pblk_info->y7, pblk_info->u7, (pblk_info->v7_h<<8)+pblk_info->v7_l);

				rtd_pr_vpq_info( "lsy (%d) blk8 %x %x %x %x \n", __LINE__,(pblk_info->m8_h<<2) + pblk_info->m8_l, pblk_info->y8, pblk_info->u8, pblk_info->v8);
				rtd_pr_vpq_info( "lsy (%d) blk9 %x %x %x %x \n", __LINE__,(pblk_info->m9), (pblk_info->y9_h<<4)+pblk_info->y9_l, pblk_info->u9, pblk_info->v9);
				rtd_pr_vpq_info( "lsy (%d) blk10 %x %x %x %x \n", __LINE__,(pblk_info->m10), pblk_info->y10, (pblk_info->u10_h<<6)+pblk_info->u10_l, pblk_info->v10);
				rtd_pr_vpq_info( "lsy (%d) blk11 %x %x %x %x \n", __LINE__,(pblk_info->m11), pblk_info->y11, pblk_info->u11, (pblk_info->v11_h<<8)+pblk_info->v11_l);

				rtd_pr_vpq_info( "lsy (%d) blk12 %x %x %x %x \n", __LINE__,(pblk_info->m12_h<<2) + pblk_info->m12_l, pblk_info->y12, pblk_info->u12, pblk_info->v12);
				rtd_pr_vpq_info( "lsy (%d) blk13 %x %x %x %x \n", __LINE__,(pblk_info->m13), (pblk_info->y13_h<<4)+pblk_info->y13_l, pblk_info->u13, pblk_info->v13);
				rtd_pr_vpq_info( "lsy (%d) blk14 %x %x %x %x \n", __LINE__,(pblk_info->m14), pblk_info->y14, (pblk_info->u14_h<<6)+pblk_info->u14_l, pblk_info->v14);
				rtd_pr_vpq_info( "lsy (%d) blk15 %x %x %x %x \n", __LINE__,(pblk_info->m15), pblk_info->y15, pblk_info->u15, (pblk_info->v15_h<<8)+pblk_info->v15_l);
			}
	...
	{
		unsigned int *virAddr = NULL;
		unsigned int flat_th=100, texture_th=900, flat_cnt=0, texture_cnt=0/*, flat_ratio=0, texture_ratio=0*/, i=0;
		VIP_Block_DeContour_DMA_CTRL* dma_ctrl = NULL;
		
		ipq_decontour_blk_de_contour_size0_RBUS blk_de_contour_size0_reg;
		ipq_decontour_blk_de_contour_size1_RBUS blk_de_contour_size1_reg;

		unsigned int data_tmp[4]={0};
		unsigned int data32_0, data32_1, data32_2, data32_3, data32_4;
		unsigned int data_buf_cnt=0, total_blk_num=0, total_info_num=0, cnt_info_num=0, info_state=0, info=0, virAddr_idx=0;
		...
		virAddr = dma_ctrl->pVir_addr_align;
		total_blk_num = blk_de_contour_size0_reg.bdecont_blk_hnum * blk_de_contour_size1_reg.bdecont_blk_vnum;
		total_info_num = total_blk_num*5;

		if(debug_decontour_blkinfo>0)
		{
			rtd_pr_vpq_info( "lsy (%d)  debug_decontour_blkinfo=%d  \n", __LINE__, debug_decontour_blkinfo);
			for(i=0; i<8; i++)
			{
				fwif_color_ChangeUINT32Endian_Copy(virAddr+i, 1, &data_tmp[0], 1);
				rtd_pr_vpq_info( "lsy (%d)  %x -> %x  \n", __LINE__, *(virAddr+i), data_tmp[0]);
			}
		}

		fwif_color_ChangeUINT32Endian_Copy(virAddr, 4, &data_tmp[0], 1);
		virAddr_idx++;
		data_buf_cnt+=128;
		data32_0 = data_tmp[3];
		data32_1 = data_tmp[2];
		...
		while(cnt_info_num < total_info_num)
		{
			while((info_state==4 && data_buf_cnt>9)|| (info_state<4 && data_buf_cnt>10))
			{
				switch(info_state)
				{
					case 0: //avg_v
						...
						data32_0 = ((data32_0>>10)&0x3fffff) | ((data32_1&0x3ff)<<22);
						data32_1 = ((data32_1>>10)&0x3fffff) | ((data32_2&0x3ff)<<22);
						data32_2 = ((data32_2>>10)&0x3fffff) | ((data32_3&0x3ff)<<22);
						data32_3 = ((data32_3>>10)&0x3fffff) | ((data32_4&0x3ff)<<22);
						data32_4 = ((data32_4>>10)&0x3fffff);
						data_buf_cnt = data_buf_cnt-10;
						info_state = 1;	
						cnt_info_num++;

						if(debug_decontour_blkinfo>0)
						{
							if((cnt_info_num-1)/5+1>=debug_decontour_blkinfo && (cnt_info_num-1)/5+1<=debug_decontour_blkinfo+5)
								rtd_pr_vpq_info( "lsy (%d)  v%d= %x  \n", __LINE__, (cnt_info_num-1)/5+1, info);
						}
						break;
					...
					case 3: //mad
						...
						if(info<flat_th) flat_cnt++;
						else if(info>texture_th) texture_cnt++;

						if(debug_decontour_blkinfo>0)
						{
							if((cnt_info_num-1)/5+1>=debug_decontour_blkinfo && (cnt_info_num-1)/5+1<=debug_decontour_blkinfo+5)
								rtd_pr_vpq_info( "lsy (%d)  m%d= %x  \n", __LINE__, (cnt_info_num-1)/5+1, info);
						}
						break;
					...
					case 4: //diff_cnt
						...
						data32_0 = ((data32_0>>9)&0x7fffff) | ((data32_1&0x1ff)<<23);
						data32_1 = ((data32_1>>9)&0x7fffff) | ((data32_2&0x1ff)<<23);
						data32_2 = ((data32_2>>9)&0x7fffff) | ((data32_3&0x1ff)<<23);
						data32_3 = ((data32_3>>9)&0x7fffff) | ((data32_4&0x1ff)<<23);
						data32_4 = ((data32_4>>9)&0x7fffff);
						data_buf_cnt = data_buf_cnt-9;
						info_state = 0;
						cnt_info_num++;
						...
						{
							if((cnt_info_num-1)/5+1>=debug_decontour_blkinfo && (cnt_info_num-1)/5+1<=debug_decontour_blkinfo+5)
								rtd_pr_vpq_info( "lsy (%d)  c%d= %x  \n", __LINE__, (cnt_info_num-1)/5+1, info);
						}
						break;
				}
			}
			
			fwif_color_ChangeUINT32Endian_Copy(virAddr+4*virAddr_idx, 4, &data_tmp[0], 1);	
			virAddr_idx++;						
			data32_0 = data32_0 | ((data_tmp[3] & ((1<<(32-data_buf_cnt))-1))<<data_buf_cnt);
			data32_1 = ((data_tmp[3]>>(32-data_buf_cnt))&((1<<data_buf_cnt)-1))|((data_tmp[2] & ((1<<(32-data_buf_cnt))-1))<<data_buf_cnt);
			data32_2 = ((data_tmp[2]>>(32-data_buf_cnt))&((1<<data_buf_cnt)-1))|((data_tmp[1] & ((1<<(32-data_buf_cnt))-1))<<data_buf_cnt);
			data32_3 = ((data_tmp[1]>>(32-data_buf_cnt))&((1<<data_buf_cnt)-1))|((data_tmp[0] & ((1<<(32-data_buf_cnt))-1))<<data_buf_cnt);
			data32_4 = ((data_tmp[0]>>(32-data_buf_cnt))&((1<<data_buf_cnt)-1));	
			data_buf_cnt += 128;
		}
...
void drvif_color_Set_DRV_SNR_Clock(SCALER_DISP_CHANNEL display, unsigned char En_flag)
{
	...
	else
	{
		if(display == SLR_MAIN_DISPLAY)
		{
			mask = VGIP_Data_Path_Select_s1_clk_en_mask;
		...
		{
			mask = M8P_VGIP_Data_Path_Select_sub_s2_clk_en_new_mask;

			if (En_flag)
				IoReg_SetBits(M8P_VGIP_Data_Path_Select_sub_reg, mask);
			else
				IoReg_ClearBits(M8P_VGIP_Data_Path_Select_sub_reg, mask);
...
unsigned char drvif_color_Get_DRV_SNR_Clock(SCALER_DISP_CHANNEL display, unsigned char flag)
{
	if (SLR_SUB_DISPLAY == display)
	{
		if(get_mach_type() == MACH_ARCH_RTK2885P)
			...
			return M8P_VGIP_Data_Path_Select_sub_get_s2_clk_en_new(IoReg_Read32(M8P_VGIP_Data_Path_Select_sub_reg));
	}
	else
