...
#include <rbus/M8P_yuv2rgb_reg.h>
#include <rbus/M8P_dither_reg.h>
#include <rbus/M8P_main_dither_reg.h>
#include <rbus/M8P_dmato3dlut_reg.h>
#include <rbus/M8P_od_dma_reg.h>
...
void drvif_color_set_Panel_Dither_Enable(unsigned char enable, unsigned char enable_temporal)
{
	dither_dithering_ctrl_RBUS	dithering_ctrl_reg;
	dithering_ctrl_reg.regValue = IoReg_Read32(DITHER_DITHERING_CTRL_reg);
	dithering_ctrl_reg.function_enable = enable;
	dithering_ctrl_reg.temporal_enable = enable_temporal;
...
void drvif_color_set_Panel_Dither_OLED_mode( unsigned short th2,unsigned short th3,unsigned short shift,unsigned short gain)
{
	...
	if (get_mach_type()!=MACH_ARCH_RTK2885P){
		M8P_dither_dithering_ctrl3_RBUS	M8P_dithering_ctrl3_reg;		
		M8P_dithering_ctrl3_reg.regValue = IoReg_Read32(M8P_DITHER_DITHERING_CTRL3_reg);
		M8P_dithering_ctrl3_reg.dither_oled_gain_sft = shift;
		M8P_dithering_ctrl3_reg.dither_oled_gain = gain;
		IoReg_Write32(M8P_DITHER_DITHERING_CTRL3_reg, M8P_dithering_ctrl3_reg.regValue);		
	}
	else{	
...
{
#if IS_ENABLED(CONFIG_RTD2885P_TCON_EXT)
	if (get_mach_type()==MACH_ARCH_RTK2885P) {
			...
			switch(DitherIP) {
				...
				case VIP_DITHER_IP_MEMC_INTERNAL:
					if (jj == 3) {
						switch (ii) {
						case 0:/* red*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_R_00_15_reg, dither_32BIT_DATA);
								break;
						case 1:/* green*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_G_00_07_reg, dither_32BIT_DATA);
								break;
						case 2:/* blue*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_B_00_15_reg, dither_32BIT_DATA);
								break;
						}
					}
					if (jj == 7) {
						switch (ii) {
						case 0:/* red*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_R_16_31_reg, dither_32BIT_DATA);
								break;
						case 1:/* green*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_G_16_31_reg, dither_32BIT_DATA);
								break;
						case 2:/* blue*/
								IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_SEQ_B_16_31_reg, dither_32BIT_DATA);
								break;
						}
					}
					break;
...
{
	unsigned int dither_32BIT_DATA = 0;
	unsigned short i,ii, jj,kk, mm, col_id, data_shift = 0;
			...
			switch(DitherIP) {
				...
				case VIP_DITHER_IP_PANEL:
					...
					for (kk = 0; kk < VIP_DITHERLUT_DEP_MAX; kk++) {
						...
						else if(Panel_Bit == 4){	//10bit
							dither_32BIT_DATA += (pDITHER_TABLE[ii][jj][kk]) << data_shift;
							for(i = 0;i < 9;i++)
								yshift[i] = 0;
							drvif_color_set_Panel_Dither_table_shift_curve(yshift,yth);
						}						
						else	{// other bit, 11 bit?
				...
				case VIP_DITHER_IP_MEMC_INTERNAL:
					...
					for (mm = 0; mm<2; mm++){
						col_id = (jj*2) + mm;
						for (kk = 0; kk < VIP_DITHERLUT_DEP_MAX; kk++) {
							dither_32BIT_DATA += (pDITHER_TABLE[ii][col_id][kk]) << data_shift;
							data_shift += 4;
						}
					}
					switch (ii) {
					case 0: /* red*/
							IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_TB_R_00_reg+4*jj, dither_32BIT_DATA);
							break;
					case 1: /* green*/
							IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_TB_G_00_reg+4*jj, dither_32BIT_DATA);
							break;
					case 2: /* blue*/
							IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_TB_B_00_reg+4*jj, dither_32BIT_DATA);
							break;
					}
					break;
			}
...
void drvif_color_set_Dither_Temporal(unsigned char DitherIP,unsigned char * pDITHER_TEMPORAL)
{
	...
	switch(DitherIP) {
		...
		case VIP_DITHER_IP_COLORTEMP:
			...
			break;
		case VIP_DITHER_IP_MEMC_INTERNAL:
			IoReg_Write32(M8P_RTMC_IPPRE_IPPRE_DITHER_Temp_TB_reg, dither_32BIT_DATA);
			break;
	}
}
/*============================================================================*/
...
void drvif_color_colorspacergb2yuvtransfer(unsigned char channel, unsigned short *table_index)
{
	...
	if (channel == _CHANNEL1)  { /*main always access tab-1*/
		...
		IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_en_mask); 
		...
		IoReg_Write32(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, ich2_RGB2YUV_CTRL_REG.regValue);
		IoReg_SetBits(RGB2YUV_ICH2_RGB2YUV_CTRL_reg, RGB2YUV_ICH2_RGB2YUV_CTRL_rgb2yuv_db_apply_mask);//db apply
...
void drvif_color_colorwrite_mixer_gamma1_Enable(unsigned char Enable)
{
	...
	osdovl_mixer_osd_gamma_ctrl_REG.osd_gamma_position = 4; // [NUMDEV-1579]louis.Liao suggest
...
#endif
	gamma_ctrl_2_REG.gamma_overlap = 1;
...
#endif
	gamma_gamma_ctrl_2_RBUS_reg.gamma_overlap = 1;
...
void drvif_color_setYUV2RGB_CSMatrix_M8(DRV_VIP_YUV2RGB_CSMatrix *ptr, VIP_CSMatrix_WriteType WriteMode, VIP_YUV2RGB_Y_SEG seg,
...
/*juwen, Merlin3, newAlgo : UV offset by UV*/
	...
	case VIP_CSMatrix_YUV2RGB_Base_Ctrl:
		...
		if (ch == SLR_MAIN_DISPLAY) {
			d_yuv2rgb_control_REG.yuv2rgb_overlay = 1;	// ptr->CTRL_ITEM.Overlay;	// don't control by VIP table, use fix
...
void drvif_color_setYUV2RGB_CSMatrix_M8P(DRV_VIP_YUV2RGB_CSMatrix *ptr, VIP_CSMatrix_WriteType WriteMode, VIP_YUV2RGB_Y_SEG seg,
unsigned char display, VIP_Table_Select tbl)
{
	SCALER_DISP_CHANNEL ch;
	unsigned char i;
	const unsigned char M8P_YUV2RGB_C_BIT_SHIFT=4;
	unsigned short buffer_status_count, buffer_status_count_th = 300;
	M8P_yuv2rgb_d_yuv2rgb_control_RBUS d_yuv2rgb_control_REG;
	M8P_yuv2rgb_d_yuv2rgb_control_1_RBUS d_yuv2rgb_control_1_reg;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_clamp_RBUS yuv2rgb_d_yuv2rgb_tab1_data_clamp_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab2_data_clamp_RBUS yuv2rgb_d_yuv2rgb_tab2_data_clamp_REG;
	M8P_yuv2rgb_d_yuv2rgbbyy_control_RBUS d_yuv2rgbbyy_control_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_control_RBUS d_uvoffsetbyy_control_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_0_RBUS yuv2rgb_d_yuv2rgb_tab1_data_0_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_1_RBUS yuv2rgb_d_yuv2rgb_tab1_data_1_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_2_RBUS yuv2rgb_d_yuv2rgb_tab1_data_2_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_3_RBUS yuv2rgb_d_yuv2rgb_tab1_data_3_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_4_RBUS yuv2rgb_d_yuv2rgb_tab1_data_4_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_5_RBUS yuv2rgb_d_yuv2rgb_tab1_data_5_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_6_RBUS yuv2rgb_d_yuv2rgb_tab1_data_6_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_8_RBUS yuv2rgb_d_yuv2rgb_tab1_data_8_REG = {0};
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_data_1_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_data_1_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_data_2_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_data_2_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_data_3_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_data_3_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_data_4_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_data_4_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_data_5_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_data_5_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_data_1_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_data_1_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_data_2_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_data_2_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_data_3_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_data_3_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_data_4_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_data_4_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_data_5_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_data_5_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_step_1_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_step_1_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_step_2_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_step_2_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_step_3_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_step_3_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_step_4_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_step_4_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_u_tab_step_5_RBUS yuv2rgb_d_uvoffsetbyy_u_tab_step_5_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_step_1_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_step_1_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_step_2_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_step_2_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_step_3_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_step_3_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_step_4_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_step_4_REG;
	M8P_yuv2rgb_d_uvoffsetbyy_v_tab_step_5_RBUS yuv2rgb_d_uvoffsetbyy_v_tab_step_5_REG;
/*juwen, Merlin3, newAlgo : UV offset by UV*/
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_1_RBUS yuv2rgb_d_offsetbyuv_nonlinear_1_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_2_RBUS yuv2rgb_d_offsetbyuv_nonlinear_2_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_3_RBUS yuv2rgb_d_offsetbyuv_nonlinear_3_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_4_RBUS yuv2rgb_d_offsetbyuv_nonlinear_4_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_5_RBUS yuv2rgb_d_offsetbyuv_nonlinear_5_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_6_RBUS yuv2rgb_d_offsetbyuv_nonlinear_6_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_7_RBUS yuv2rgb_d_offsetbyuv_nonlinear_7_REG;
	M8P_yuv2rgb_d_offsetbyuv_nonlinear_8_RBUS yuv2rgb_d_offsetbyuv_nonlinear_8_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_1_RBUS yuv2rgb_d_offsetbyuv_gain_tab_1_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_2_RBUS yuv2rgb_d_offsetbyuv_gain_tab_2_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_3_RBUS yuv2rgb_d_offsetbyuv_gain_tab_3_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_4_RBUS yuv2rgb_d_offsetbyuv_gain_tab_4_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_5_RBUS yuv2rgb_d_offsetbyuv_gain_tab_5_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_6_RBUS yuv2rgb_d_offsetbyuv_gain_tab_6_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_7_RBUS yuv2rgb_d_offsetbyuv_gain_tab_7_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_8_RBUS yuv2rgb_d_offsetbyuv_gain_tab_8_REG;
	M8P_yuv2rgb_d_offsetbyuv_gain_tab_9_RBUS yuv2rgb_d_offsetbyuv_gain_tab_9_REG;

	M8P_yuv2rgb_d_coefbyy_nonlinear_0_RBUS yuv2rgb_d_coefbyy_nonlinear_0_REG;			/* Mac3 add by chris @20140721*/
	M8P_yuv2rgb_d_offsetbyy_nonlinear_0_RBUS yuv2rgb_d_offsetbyy_nonlinear_0_REG;		/* Mac3 add by chris @20140721*/

	M8P_yuv2rgb_d_yuv2rgb_tab1_data_7_RBUS yuv2rgb_d_yuv2rgb_tab1_data_7_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab2_data_7_RBUS yuv2rgb_d_yuv2rgb_tab2_data_7_REG;

#ifdef CONFIG_HDR_DEMO
	_system_setting_info *VIP_system_info_structure_table=NULL;
	VIP_system_info_structure_table = (_system_setting_info*)Scaler_GetShareMemVirAddr(SCALERIOC_VIP_system_info_structure);
#endif
	d_yuv2rgb_control_1_reg.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Control_1_reg);
	d_yuv2rgb_control_1_reg.yuv2rgb_db_en = 1;
	d_yuv2rgb_control_1_reg.yuv2rgb_db_read_level = 1;
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Control_1_reg, d_yuv2rgb_control_1_reg.regValue);
	...
	switch (WriteMode) {
	case VIP_CSMatrix_YUV2RGB_Base_Ctrl:
		d_yuv2rgb_control_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Control_reg);
		yuv2rgb_d_yuv2rgb_tab1_data_clamp_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_Clamp_reg);
		yuv2rgb_d_yuv2rgb_tab2_data_clamp_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_Clamp_reg);
		if ((ch > SLR_SUB_DISPLAY) || (tbl > VIP_Table_Select_2)) {
			VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, CH or tbl select error, CH = %d, TBL = %d~~\n ", ch, tbl);
			return;
		}
		...
		yuv2rgb_d_yuv2rgb_tab1_data_clamp_REG.cb_cr_clamp = 1; // ptr->CTRL_ITEM.CbCr_Clamp;	// don't control by VIP table, use fix
		yuv2rgb_d_yuv2rgb_tab2_data_clamp_REG.cb_cr_clamp = 1; // ptr->CTRL_ITEM.CbCr_Clamp;	// don't control by VIP table, use fix
		IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_Clamp_reg, yuv2rgb_d_yuv2rgb_tab1_data_clamp_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Control_reg, d_yuv2rgb_control_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_Clamp_reg, yuv2rgb_d_yuv2rgb_tab2_data_clamp_REG.regValue);
		break;

	case VIP_CSMatrix_Coef_Y_Only:
		if (seg == VIP_YUV2RGB_Y_Seg_0) {
			yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11 = ptr->COEF_By_Y.K11[seg];
			yuv2rgb_d_yuv2rgb_tab1_data_8_REG.k31 = yuv2rgb_d_yuv2rgb_tab1_data_8_REG.k21 = yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11;
			if (tbl == VIP_Table_Select_1) {
				IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_8_reg, yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
			} else if (tbl == VIP_Table_Select_2) {
				IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_8_reg, yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
			} else {
				VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, Table2 can't set matrix by Y~~\n ");
				return;
			}
			...
			for (i = 0; i < VIP_YUV2RGB_Y_Seg_Max; i++) {
				...
				if (i == VIP_YUV2RGB_Y_Seg_0) {
					...
					IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_0_reg+((i-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_4_reg+((i-1)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
				} else if (i < VIP_YUV2RGB_Y_Seg_15) {
					IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_4_reg+((i-1)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
				} else {
					IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_15_4_reg+((i-15)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
				}
			}
		} else {
			if (tbl != VIP_Table_Select_1) {
			...
			yuv2rgb_d_yuv2rgb_tab1_data_8_REG.k31 = yuv2rgb_d_yuv2rgb_tab1_data_8_REG.k21 = yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11;
			if (seg < VIP_YUV2RGB_Y_Seg_5) {
				IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_0_reg+((seg-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_4_reg+((seg-1)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
			} else if (seg < VIP_YUV2RGB_Y_Seg_15) {
				IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_4_reg+((seg-1)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
			} else {
				IoReg_WriteWord1(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k11);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_15_4_reg+((seg-15)*4), yuv2rgb_d_yuv2rgb_tab1_data_8_REG.regValue);
			}
		}
		buffer_status_count = 0;
		break;
	...
	case VIP_CSMatrix_Coef_C_Only:
		if (seg == VIP_YUV2RGB_Y_Seg_0) {
			yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12 = ptr->COEF_By_Y.K12[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_1_REG.k13 = ptr->COEF_By_Y.K13[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_2_REG.k22 = ptr->COEF_By_Y.K22[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_2_REG.k23 = ptr->COEF_By_Y.K23[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_3_REG.k32 = ptr->COEF_By_Y.K32[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_3_REG.k33 = ptr->COEF_By_Y.K33[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			if (tbl == VIP_Table_Select_1) {
				IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_1_reg, yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_2_reg, yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_3_reg, yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
			} else if (tbl == VIP_Table_Select_2) {
				IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_1_reg, yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_2_reg, yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_3_reg, yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
			} else {
				VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, TBL selec error~~\n ");
				return;
			}
			...
			for (i = 0; i < VIP_YUV2RGB_Y_Seg_Max; i++) {
				yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12 = ptr->COEF_By_Y.K12[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				yuv2rgb_d_yuv2rgb_tab1_data_1_REG.k13 = ptr->COEF_By_Y.K13[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				yuv2rgb_d_yuv2rgb_tab1_data_2_REG.k22 = ptr->COEF_By_Y.K22[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				yuv2rgb_d_yuv2rgb_tab1_data_2_REG.k23 = ptr->COEF_By_Y.K23[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				yuv2rgb_d_yuv2rgb_tab1_data_3_REG.k32 = ptr->COEF_By_Y.K32[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				yuv2rgb_d_yuv2rgb_tab1_data_3_REG.k33 = ptr->COEF_By_Y.K33[i]<<M8P_YUV2RGB_C_BIT_SHIFT;
				if (i == VIP_YUV2RGB_Y_Seg_0) {
					IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_1_reg, yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_2_reg, yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
					...
					IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_0_reg+((i-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_1_reg+((i-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_2_reg+((i-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_3_reg+((i-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
				} else if (i == VIP_YUV2RGB_Y_Seg_4) {
					IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_1_reg, yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_2_reg, yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_3_reg, yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
				} else {
					IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_1_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_2_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
					IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_3_reg+((i-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
				}
			}
		} else {
			if (tbl != VIP_Table_Select_1) {
			...
			yuv2rgb_d_yuv2rgb_tab1_data_3_REG.k33 = ptr->COEF_By_Y.K33[seg]<<M8P_YUV2RGB_C_BIT_SHIFT;
			if (seg < VIP_YUV2RGB_Y_Seg_4) {
				IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_0_reg+((seg-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_1_reg+((seg-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_2_reg+((seg-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_1_3_reg+((seg-1)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
			} else if (seg == VIP_YUV2RGB_Y_Seg_4) {
				IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_0_reg, yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_1_reg, yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_4_2_reg, yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
				...
				IoReg_WriteWord0(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_0_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_0_REG.k12);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_1_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_1_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_2_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_2_REG.regValue);
				IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_Tab_Data_5_3_reg+((seg-5)*4*4), yuv2rgb_d_yuv2rgb_tab1_data_3_REG.regValue);
			}
		}
		buffer_status_count = 0;
		break;
	...
	case VIP_CSMatrix_Offset_Only:
		...
		yuv2rgb_d_yuv2rgb_tab1_data_6_REG.boffset = ptr->RGB_Offset.B_offset;
		if (tbl == VIP_Table_Select_1) {
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_4_reg, yuv2rgb_d_yuv2rgb_tab1_data_4_REG.regValue);
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_5_reg, yuv2rgb_d_yuv2rgb_tab1_data_5_REG.regValue);
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_6_reg, yuv2rgb_d_yuv2rgb_tab1_data_6_REG.regValue);
		} else if (tbl == VIP_Table_Select_2) {
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_4_reg, yuv2rgb_d_yuv2rgb_tab1_data_4_REG.regValue);
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_5_reg, yuv2rgb_d_yuv2rgb_tab1_data_5_REG.regValue);
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_6_reg, yuv2rgb_d_yuv2rgb_tab1_data_6_REG.regValue);
		} else {
			VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, TBL selec error~~\n ");
			return;
		}
	...
	case VIP_CSMatrix_Y_Clamp:
		...
		if (tbl == VIP_Table_Select_1)
			...
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_Clamp_reg, yuv2rgb_d_yuv2rgb_tab1_data_clamp_REG.regValue);
		else {
			VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, TBL selec error~~\n ");
			return;
		}
	...
	case VIP_CSMatrix_CoefByY_ctrl:
		d_yuv2rgbbyy_control_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGBBYY_CONTROL_reg);
		d_yuv2rgbbyy_control_REG.yuv2rgbbymode_main_en = ptr->CTRL_ITEM.CoefByY_En;
		/*d_yuv2rgbbyy_control_REG.yuv2rgbbyy_sub_en = ptr->CoefByY_En;*/
		...
		IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_CONTROL_reg, d_yuv2rgbbyy_control_REG.regValue);
		buffer_status_count = 0;
		break;
	case VIP_CSMatrix_UVOffset_ctrl:
		d_uvoffsetbyy_control_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_UVOFFSETBYY_CONTROL_reg);
		d_uvoffsetbyy_control_REG.uvoffsetbyy_main_en = ptr->CTRL_ITEM.UVOffset_En;
		d_uvoffsetbyy_control_REG.uvoffsetbyy_mode = ptr->CTRL_ITEM.UVOffset_Mode_Ctrl;
		d_uvoffsetbyy_control_REG.uvoffset_mode = ptr->CTRL_ITEM.UVOffset_Mode_byUV_byY;/*juwen, Merlin3, newAlgo : UV offset by UV*/
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_CONTROL_reg, d_uvoffsetbyy_control_REG.regValue);
		buffer_status_count = 0;
		break;
...
/*juwen, Merlin3, newAlgo : UV offset by UV*/
		...
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Data_1_reg, yuv2rgb_d_uvoffsetbyy_u_tab_data_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Data_2_reg, yuv2rgb_d_uvoffsetbyy_u_tab_data_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Data_3_reg, yuv2rgb_d_uvoffsetbyy_u_tab_data_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Data_4_reg, yuv2rgb_d_uvoffsetbyy_u_tab_data_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Data_5_reg, yuv2rgb_d_uvoffsetbyy_u_tab_data_5_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Data_1_reg, yuv2rgb_d_uvoffsetbyy_v_tab_data_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Data_2_reg, yuv2rgb_d_uvoffsetbyy_v_tab_data_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Data_3_reg, yuv2rgb_d_uvoffsetbyy_v_tab_data_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Data_4_reg, yuv2rgb_d_uvoffsetbyy_v_tab_data_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Data_5_reg, yuv2rgb_d_uvoffsetbyy_v_tab_data_5_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Step_1_reg, yuv2rgb_d_uvoffsetbyy_u_tab_step_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Step_2_reg, yuv2rgb_d_uvoffsetbyy_u_tab_step_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Step_3_reg, yuv2rgb_d_uvoffsetbyy_u_tab_step_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Step_4_reg, yuv2rgb_d_uvoffsetbyy_u_tab_step_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_U_Tab_Step_5_reg, yuv2rgb_d_uvoffsetbyy_u_tab_step_5_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Step_1_reg, yuv2rgb_d_uvoffsetbyy_v_tab_step_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Step_2_reg, yuv2rgb_d_uvoffsetbyy_v_tab_step_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Step_3_reg, yuv2rgb_d_uvoffsetbyy_v_tab_step_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Step_4_reg, yuv2rgb_d_uvoffsetbyy_v_tab_step_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_UVOFFSETBYY_V_Tab_Step_5_reg, yuv2rgb_d_uvoffsetbyy_v_tab_step_5_REG.regValue);

/*juwen, Merlin3, newAlgo : UV offset by UV*/
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_1_reg, yuv2rgb_d_offsetbyuv_nonlinear_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_2_reg, yuv2rgb_d_offsetbyuv_nonlinear_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_3_reg, yuv2rgb_d_offsetbyuv_nonlinear_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_4_reg, yuv2rgb_d_offsetbyuv_nonlinear_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_5_reg, yuv2rgb_d_offsetbyuv_nonlinear_5_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_6_reg, yuv2rgb_d_offsetbyuv_nonlinear_6_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_7_reg, yuv2rgb_d_offsetbyuv_nonlinear_7_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_NONLINEAR_8_reg, yuv2rgb_d_offsetbyuv_nonlinear_8_REG.regValue);

		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_1_reg, yuv2rgb_d_offsetbyuv_gain_tab_1_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_2_reg, yuv2rgb_d_offsetbyuv_gain_tab_2_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_3_reg, yuv2rgb_d_offsetbyuv_gain_tab_3_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_4_reg, yuv2rgb_d_offsetbyuv_gain_tab_4_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_5_reg, yuv2rgb_d_offsetbyuv_gain_tab_5_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_6_reg, yuv2rgb_d_offsetbyuv_gain_tab_6_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_7_reg, yuv2rgb_d_offsetbyuv_gain_tab_7_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_8_reg, yuv2rgb_d_offsetbyuv_gain_tab_8_REG.regValue);
		IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYUV_Gain_Tab_9_reg, yuv2rgb_d_offsetbyuv_gain_tab_9_REG.regValue);
...
//no IoReg_Read32 ???
	...
	case VIP_CSMatrix_CoefByY_Index:		/* Mac3 add by chris @20140721*/
		...
		for (i = 0; i < VIP_YUV2RGB_Y_Seg_Max-1; i += 4) {
			...
			yuv2rgb_d_coefbyy_nonlinear_0_REG.index_3 = ptr->COEF_By_Y.Y_index[i+2];
			yuv2rgb_d_coefbyy_nonlinear_0_REG.index_4 = ptr->COEF_By_Y.Y_index[i+3];
			IoReg_Write32(M8P_YUV2RGB_D_COEFBYY_NONLINEAR_0_reg+i, yuv2rgb_d_coefbyy_nonlinear_0_REG.regValue);
		}
		buffer_status_count = 0;
		break;
	...
	case VIP_CSMatrix_UVOffset_Index:		/* Mac3 add by chris @20140721*/
		...
		for (i = 0; i < VIP_YUV2RGB_Y_Seg_Max-1; i += 4) {
			...
			yuv2rgb_d_offsetbyy_nonlinear_0_REG.index_3 = ptr->UV_Offset.UV_index[i+2];
			yuv2rgb_d_offsetbyy_nonlinear_0_REG.index_4 = ptr->UV_Offset.UV_index[i+3];
			IoReg_Write32(M8P_YUV2RGB_D_OFFSETBYY_NONLINEAR_0_reg+i, yuv2rgb_d_offsetbyy_nonlinear_0_REG.regValue);
		}
		buffer_status_count = 0;
		break;
	...
	case VIP_CSMatrix_Coef_BT2020_Only:
		if (tbl == VIP_Table_Select_1) {
			yuv2rgb_d_yuv2rgb_tab1_data_7_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_7_reg);
			yuv2rgb_d_yuv2rgb_tab1_data_7_REG.k132 = ptr->COEF_By_Y.bt2020_K13<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab1_data_7_REG.k322 = ptr->COEF_By_Y.bt2020_K32<<M8P_YUV2RGB_C_BIT_SHIFT;
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_7_reg, yuv2rgb_d_yuv2rgb_tab1_data_7_REG.regValue);
		} else if (tbl == VIP_Table_Select_2) {
			yuv2rgb_d_yuv2rgb_tab2_data_7_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_7_reg);
			yuv2rgb_d_yuv2rgb_tab2_data_7_REG.k132 = ptr->COEF_By_Y.bt2020_K13<<M8P_YUV2RGB_C_BIT_SHIFT;
			yuv2rgb_d_yuv2rgb_tab2_data_7_REG.k322 = ptr->COEF_By_Y.bt2020_K32<<M8P_YUV2RGB_C_BIT_SHIFT;
			IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab2_Data_7_reg, yuv2rgb_d_yuv2rgb_tab2_data_7_REG.regValue);
		} else {
			VIPprintf("~~drvif_color_setYUV2RGB_CSMatrix Error, Table select Error~~\n ");
		}
		buffer_status_count = 0;
...
{
	if (get_mach_type()==MACH_ARCH_RTK2885P)
		drvif_color_setYUV2RGB_CSMatrix_M8(ptr, WriteMode, seg, display, tbl);
	else
		drvif_color_setYUV2RGB_CSMatrix_M8P(ptr, WriteMode, seg, display, tbl);
}
/*============================================================================*/
/**
...
void drvif_color_write_Gamma_By_DMA(unsigned int phy_addr)
{
	gamma_gamma_ctrl_1_RBUS gamma_ctrl_1_REG;
	...
	gamma_ctrl_1_REG.regValue = IoReg_Read32(GAMMA_GAMMA_CTRL_1_reg);
	gamma_ctrl_1_REG.gamma_ax_ch_sel = VIP_GAMMA_CHANNEL_RGB_MAX; //write RGB at the same time
	IoReg_Write32(GAMMA_GAMMA_CTRL_1_reg, gamma_ctrl_1_REG.regValue);

	if (M8P_DMATO3DLUT_DMAto3DLUT_Table2_ErrStatus2Irq_get_table2_err_status(IoReg_Read32(M8P_DMATO3DLUT_DMAto3DLUT_Table2_ErrStatus2Irq_reg)))
		IoReg_SetBits(M8P_DMATO3DLUT_DMAto3DLUT_Table2_ErrStatus2Irq_reg, M8P_DMATO3DLUT_DMAto3DLUT_Table2_ErrStatus2Irq_table2_err_clr_mask);

	IoReg_SetBits(M8P_DMATO3DLUT_DMAto3DLUT_db_ctl_reg, M8P_DMATO3DLUT_DMAto3DLUT_db_ctl_db_en_mask);
	IoReg_Write32(M8P_DMATO3DLUT_DMAto3DLUT_Table2_Addr_reg, phy_addr);
	IoReg_SetBits(M8P_DMATO3DLUT_DMAto3DLUT_Table2_Format0_reg, M8P_DMATO3DLUT_DMAto3DLUT_Table2_Format0_table2_dma_en_mask);
	IoReg_SetBits(M8P_DMATO3DLUT_DMAto3DLUT_db_ctl_reg, M8P_DMATO3DLUT_DMAto3DLUT_db_ctl_wtable_apply_mask);
}

void drvif_color_write_InvOutGamma_By_DMA(unsigned int phy_addr)
{
	invoutputgamma_inv_out_gamma_ctrl_1_RBUS inv_out_gamma_ctrl_1_REG;
	...
	inv_out_gamma_ctrl_1_REG.regValue = IoReg_Read32(INVOUTPUTGAMMA_INV_OUT_GAMMA_CTRL_1_reg);
	inv_out_gamma_ctrl_1_REG.inv_out_gamma_ax_ch_sel = VIP_GAMMA_CHANNEL_RGB_MAX; //write RGB at the same time
	IoReg_Write32(INVOUTPUTGAMMA_INV_OUT_GAMMA_CTRL_1_reg, inv_out_gamma_ctrl_1_REG.regValue);

	// clr err status
	if (M8P_OD_DMA_INV_OGAMMA_ErrStatus2Irq_get_inv_ogamma_err_status(IoReg_Read32(M8P_OD_DMA_INV_OGAMMA_ErrStatus2Irq_reg)))
		IoReg_SetBits(M8P_OD_DMA_INV_OGAMMA_ErrStatus2Irq_reg, M8P_OD_DMA_INV_OGAMMA_ErrStatus2Irq_inv_ogamma_err_clr_mask);

	IoReg_SetBits(M8P_OD_DMA_GAMMA_DB_EN_reg, M8P_OD_DMA_GAMMA_DB_EN_inv_ogamma_db_en_mask);
	IoReg_Write32(M8P_OD_DMA_INV_OGAMMA_Addr_reg, phy_addr);
	IoReg_SetBits(M8P_OD_DMA_INV_OGAMMA_Format0_reg, M8P_OD_DMA_INV_OGAMMA_Format0_inv_ogamma_dma_en_mask);
	IoReg_SetBits(M8P_OD_DMA_GAMMA_DB_EN_reg, M8P_OD_DMA_GAMMA_DB_EN_inv_ogamma_db_apply_mask);
}

void drvif_color_write_OutGamma_By_DMA(unsigned int phy_addr)
{
	outputgamma_out_gamma_ctrl_1_RBUS out_gamma_ctrl_1_REG;
	...
	out_gamma_ctrl_1_REG.regValue = IoReg_Read32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg);
	out_gamma_ctrl_1_REG.out_gamma_ax_ch_sel = VIP_GAMMA_CHANNEL_RGB_MAX; //write RGB at the same time
	IoReg_Write32(OUTPUTGAMMA_OUT_GAMMA_CTRL_1_reg, out_gamma_ctrl_1_REG.regValue);

	if (M8P_OD_DMA_OGAMMA_ErrStatus2Irq_get_ogamma_err_status(IoReg_Read32(M8P_OD_DMA_OGAMMA_ErrStatus2Irq_reg)))
		IoReg_SetBits(M8P_OD_DMA_OGAMMA_ErrStatus2Irq_reg, M8P_OD_DMA_OGAMMA_ErrStatus2Irq_ogamma_err_clr_mask);

	IoReg_SetBits(M8P_OD_DMA_GAMMA_DB_EN_reg, M8P_OD_DMA_GAMMA_DB_EN_ogamma_db_en_mask);
	IoReg_Write32(M8P_OD_DMA_OGAMMA_Addr_reg, phy_addr);
	IoReg_SetBits(M8P_OD_DMA_OGAMMA_Format0_reg, M8P_OD_DMA_OGAMMA_Format0_ogamma_dma_en_mask);
	IoReg_SetBits(M8P_OD_DMA_GAMMA_DB_EN_reg, M8P_OD_DMA_GAMMA_DB_EN_ogamma_db_apply_mask);
}
...
void drvif_color_inv_gamma_enable(unsigned char display, unsigned char enable, unsigned char useRPC)
{
	...
	if(useRPC && display == SLR_MAIN_DISPLAY) {
		...
		if (display == SLR_MAIN_DISPLAY) {
			...
			inv_gamma_ctrl_2_REG.inv_gamma_overlap = 1;
		}
#ifdef CONFIG_DUAL_CHANNEL  /* There are two channels*/
		else {
...
int drvif_Wait_MEMCDTG_start(short delay_st, short delay_end)
{
	...
	denST = memcdtg_dv_den_start_end.memcdtg_dv_den_sta;
	...
	if (Bnd_1 <= Bnd_0) {
		rtd_pr_vpq_emerg("drvif_Wait_MEMCDTG_start, check para, denST=%d, denEnd=%d, delay_st=%d, delay_end=%d\n",
			denST, denEnd, delay_st, delay_end);
		Bnd_0 = denST;
		Bnd_1 = denEnd;
	}
	...
	do {
		...
		lineCNT = new_meas1_linecnt_real_reg.memcdtg_line_cnt_rt;
		if((lineCNT < Bnd_1) && (lineCNT > Bnd_0)) {
			return 0;
		}
	} while(timeoutcnt-- != 0);
...
char drvif_Get_UZUDTG_Info_HV(unsigned short *h_sta, unsigned short *h_end, unsigned short *h_total,
	unsigned short *v_sta, unsigned short *v_end,  unsigned short *v_total)
{
	ppoverlay_dv_den_start_end_RBUS dv_den_start_end_reg;
	ppoverlay_dh_den_start_end_RBUS dh_den_start_end_reg;
	...
	if ((v_sta == NULL) || (v_end == NULL) || (v_total == NULL) || (h_sta == NULL) || (h_end == NULL) || (h_total == NULL)) {
		rtd_pr_vpq_emerg("drvif_Get_UZUDTG_Info_HV, NULL\n");
		return -1;
	}

	*v_sta = dv_den_start_end_reg.dv_den_sta;
	*v_end = dv_den_start_end_reg.dv_den_end;
	*v_total = uzudtg_dv_total_reg.uzudtg_dv_total;

	*h_sta = dh_den_start_end_reg.dh_den_sta;
	*h_end = dh_den_start_end_reg.dh_den_end;
	*h_total = uzudtg_dh_total_reg.uzudtg_dh_total;

	return 0;

}

char drvif_Get_UZUDTG_Info_TimeBorrow(unsigned short *mode, unsigned short *timeBorrow_h, unsigned short *timeBorrow_v)
{
	ppoverlay_uzudtg_stage1_time_borrow_RBUS ppoverlay_uzudtg_stage1_time_borrow_reg;

	ppoverlay_uzudtg_stage1_time_borrow_reg.regValue = rtd_inl(PPOVERLAY_uzudtg_stage1_time_borrow_reg);

	if ((mode == NULL) || (timeBorrow_h == NULL) || (timeBorrow_v == NULL)) {
		rtd_pr_vpq_emerg("drvif_Get_UZUDTG_Info_TimeBorrow, NULL\n");
		return -1;
	}

	*mode = ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_sw_mode;
	*timeBorrow_h = ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_h;
	*timeBorrow_v = ppoverlay_uzudtg_stage1_time_borrow_reg.stage1_time_borrow_v;
...
char drvif_Set_DM_HDR_3dLUT_17x17x17_16_By_DMA(unsigned char enable, unsigned short *pArray, unsigned int addr, unsigned int* vir_addr, unsigned int flush_size)
{
	...
	int x,y,z;//,k;
	yuvdata *p_table;
	extern unsigned char skip_HDR10_3dLUT_C123Inv_nouse_set;
	...
	IoReg_Write32(DM_ThreeDLUT_MinMaxC2_reg, threedlut_minmaxc2_reg.regValue);
	IoReg_Write32(DM_ThreeDLUT_MinMaxC3_reg, threedlut_minmaxc3_reg.regValue);
	if (!skip_HDR10_3dLUT_C123Inv_nouse_set) {
		IoReg_Write32(DM_ThreeDLUT_MinMaxInv1_reg, threedlut_minmaxinv1_reg.regValue);
		IoReg_Write32(DM_ThreeDLUT_MinMaxInv2_reg, threedlut_minmaxinv2_reg.regValue);
		IoReg_Write32(DM_ThreeDLUT_MinMaxInv3_reg, threedlut_minmaxinv3_reg.regValue);
	}
...
unsigned char drvif_color_set_HDR10_Enable(unsigned char* enable)
{
	...
	if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (get_OTT_HDR_mode() == HDR_DOLBY_COMPOSER || get_OTT_HDR_mode() == HDR_DOLBY_DTV))
		isdolby_ott = TRUE;
...
unsigned char drvif_color_set_HDR10_Enable_Seamless(unsigned char* enable)
{
	...
	else
	{
		...
		if (get_OTT_HDR_mode()!=HDR_DOLBY_COMPOSER && get_OTT_HDR_mode() != HDR_DOLBY_DTV && get_HDMI_HDR_mode()!=HDR_DOLBY_HDMI) {	// don't disable dolby_mode when real DolbyVision enables
			dm_submodule_temp.regValue = 0;
			//dm_submodule_temp.b0501_enable = 1;
...
char drvif_color_PQ_CLK_init(void)
{	
	// enable vtop DM and comp clk
	//IoReg_Write32(SYS_REG_SYS_CLKEN1_reg, SYS_REG_SYS_CLKEN1_clken_hdr1_dm_mask | SYS_REG_SYS_CLKEN1_clken_hdr1_comp_mask | SYS_REG_SYS_CLKEN1_write_data_mask);
	// disable vtop 2 clk	
	//IoReg_Write32(SYS_REG_SYS_CLKEN0_reg, SYS_REG_SYS_CLKEN0_clken_hdr2_mask);	
	//IoReg_Write32(SYS_REG_SYS_CLKEN4_reg, SYS_REG_SYS_CLKEN4_clken_hdr2_all_mask);	
	// disable A10
	IoReg_Write32(SYS_REG_SYS_CLKEN3_reg, SYS_REG_SYS_CLKEN3_clken_disp_lg_a10_mask);

	
	rtd_pr_vpq_info("force_VPQ_bypass, drvif_color_PQ_CLK_init,,\n");
	return 0;
}
...
char drvif_color_set_PQ_ByPass(unsigned char bypass_item, unsigned char isInterlaceMode)
{
	...
	switch(bypass_item) {
		...
		case PQ_ByPass_NNSR:
#if 0	// active smt flow to bypass nnsr		
		...
		if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE) &&
			...
			//drvif_srnn_clk_enable(OFF);		//need smt flow // fix_me Merlin8 need refine clk off flow
		}
#endif			
		break;
	...
	case PQ_ByPass_D_RGB_SHP:
		IoReg_ClearBits(RGB_SHPNR_RGB_SHPNR_CTRL_reg, RGB_SHPNR_RGB_SHPNR_CTRL_rgb_sharp_enable_mask);
		IoReg_SetBits(RGB_SHPNR_RGB_SHPNR_CTRL_DB_reg, RGB_SHPNR_RGB_SHPNR_CTRL_DB_db_load_mask);
	break;
...
void drvif_color_set_PQ_ByPass_2P_mode_misc(void)
{
	...
	//HDR
	if(get_mach_type()==MACH_ARCH_RTK2885P) { // merlin8 IC , run merlin8 flow
		IoReg_ClearBits(HDR_SUB_HDR2_YUV2RGB_Control_reg, HDR_SUB_HDR2_YUV2RGB_Control_yuv2rgb_en_mask);
	}/* else if(get_mach_type()==MACH_ARCH_RTK2885P2) { // merlin8p IC, run merlin8 flow

	} else if(get_mach_type()==MACH_ARCH_RTK2885PP) //merlin8p IC , run merlin8p flow
...
void drvif_I_Main_DITHER_Y_HISTOGRAM_init(void)
{
	if(get_mach_type()==MACH_ARCH_RTK2885P) { // merlin8 IC , run merlin8 flow
		...
		M8P_main_dither_i_main_dither_y_histogram_ctrl_RBUS main_dither_i_main_dither_y_histogram_ctrl_reg;
		M8P_main_dither_i_main_dither_y_histogram_h_RBUS main_dither_i_main_dither_y_histogram_h_reg;
		M8P_main_dither_i_main_dither_y_histogram_v_RBUS main_dither_i_main_dither_y_histogram_v_reg;

		main_dither_i_main_dither_y_histogram_ctrl_reg.regValue = IoReg_Read32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_CTRL_reg);
		main_dither_i_main_dither_y_histogram_ctrl_reg.hist_rgb2y_en = 1;
		main_dither_i_main_dither_y_histogram_ctrl_reg.hist_mode_sel = 1;
		main_dither_i_main_dither_y_histogram_ctrl_reg.hist_en = 1;
		IoReg_Write32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_CTRL_reg, main_dither_i_main_dither_y_histogram_ctrl_reg.regValue);

		main_dither_i_main_dither_y_histogram_h_reg.regValue = IoReg_Read32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_H_reg);
		main_dither_i_main_dither_y_histogram_h_reg.hist_endh = 0;	// whole frame
		main_dither_i_main_dither_y_histogram_h_reg.hist_starth = 0;	// whole frame
		IoReg_Write32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_H_reg, main_dither_i_main_dither_y_histogram_h_reg.regValue);

		main_dither_i_main_dither_y_histogram_v_reg.regValue = IoReg_Read32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_V_reg);
		main_dither_i_main_dither_y_histogram_v_reg.hist_endv = 0;	// whole frame
		main_dither_i_main_dither_y_histogram_v_reg.hist_startv = 0;	// whole frame
		IoReg_Write32(M8P_MAIN_DITHER_I_Main_DITHER_Y_HISTOGRAM_V_reg, main_dither_i_main_dither_y_histogram_v_reg.regValue);
	} else if(get_mach_type()==MACH_ARCH_RTK2885PP) { //merlin8p IC , run merlin8p flow
		M8P_main_dither_i_main_dither_y_histogram_ctrl_RBUS main_dither_i_main_dither_y_histogram_ctrl_reg;
		M8P_main_dither_i_main_dither_y_histogram_h_RBUS main_dither_i_main_dither_y_histogram_h_reg;
		M8P_main_dither_i_main_dither_y_histogram_v_RBUS main_dither_i_main_dither_y_histogram_v_reg;
...
char drvif_color_Get_DTG_S2_Timing(unsigned short *lineCNT, unsigned short *h_sta, unsigned short *v_sta, unsigned short *h_end, unsigned short *v_end)
{
	ppoverlay_memcdtg_dh_den_start_end_RBUS dh_memcdtg_den_start_end_reg;
	ppoverlay_memcdtg_dv_den_start_end_RBUS dv_memcdtg_den_start_end_reg;

	if (h_sta == NULL ||v_sta == NULL || h_end == NULL || v_end == NULL || lineCNT == NULL) {
		rtd_pr_vpq_emerg("drvif_color_Get_DTG_S1_Timing,NULL\n");
		return -1;
	}

	dh_memcdtg_den_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_memcdtg_DH_DEN_Start_End_reg);
	dv_memcdtg_den_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_memcdtg_DV_DEN_Start_End_reg);

	*h_sta = dh_memcdtg_den_start_end_reg.memcdtg_dh_den_sta;
	*v_sta = dv_memcdtg_den_start_end_reg.memcdtg_dv_den_sta;
	*h_end = dh_memcdtg_den_start_end_reg.memcdtg_dh_den_end;
	*v_end = dv_memcdtg_den_start_end_reg.memcdtg_dv_den_end;	
	*lineCNT = PPOVERLAY_new_meas1_linecnt_real_get_memcdtg_line_cnt_rt(IoReg_Read32(PPOVERLAY_new_meas1_linecnt_real_reg));

	return 0;
}

unsigned int drvif_color_get_memcdtg_DVS(void)
{
	return PPOVERLAY_memcdtg_DVS_cnt_get_memcdtg_dvs_cnt(IoReg_Read32(PPOVERLAY_memcdtg_DVS_cnt_reg));
}

unsigned int drvif_color_get_uzudtg_DVS(void)
{
	return PPOVERLAY_uzudtg_DVS_cnt_get_uzudtg_dvs_cnt(IoReg_Read32(PPOVERLAY_uzudtg_DVS_cnt_reg));
}

signed char drvif_color_get_MEMC_Mux_Status(unsigned char *mux_sel, unsigned char *DB_apply_status)
{
	ppoverlay_memc_mux_ctrl_RBUS memc_mux_ctrl_reg;	
    ppoverlay_double_buffer_ctrl2_RBUS ppoverlay_double_buffer_ctrl2_reg;
	if (mux_sel == NULL || DB_apply_status == NULL)
	{
		rtd_pr_vpq_emerg("drvif_color_get_MEMC_Mux_Status, NULL\n");
		return -1;
	}
	...
	if ((ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_en == 1) && (ppoverlay_double_buffer_ctrl2_reg.uzudtgreg_dbuf_set == 1))
		*DB_apply_status = 1;
	else
		*DB_apply_status = 0;

	*mux_sel = memc_mux_ctrl_reg.memc_outmux_sel;
...
unsigned char drvif_color_Disable_MEMC_RTK2885p(unsigned char enable)
{
...
//FIX_MISMATCH
unsigned char drvif_color_Disable_MEMC_RTK2885pp(unsigned char enable)
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
return 0;
#else
	if(enable){
		HAL_MC_SetLBMCPCMode(1);
}
	return TRUE;
#endif
}
...
unsigned char drvif_color_Disable_MEMC(unsigned char enable)
{
	...
	if(RUN_MERLIN8P()){
		result = drvif_color_Disable_MEMC_RTK2885pp(enable);
	}else if(RUN_MERLIN8()){
		result = drvif_color_Disable_MEMC_RTK2885p(enable);
...
{/*DEMURA1*/1, /*DEMURA2*/0, /*SRNN*/1, /*SRS*/0, /*HCIC*/0, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/0, /*A10*/0, /*OD*/1},	/* MEX_UNKNOW  */
{/*DEMURA1*/1, /*DEMURA2*/0, /*SRNN*/1, /*SRS*/0, /*HCIC*/0, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/0, /*A10*/0, /*OD*/0},	/* MEX_SlAVE_IC  */
{/*DEMURA1*/1, /*DEMURA2*/0, /*SRNN*/1, /*SRS*/0, /*HCIC*/1, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/1, /*A10*/0, /*OD*/1},	/* MEX_LGD */
{/*DEMURA1*/1, /*DEMURA2*/0, /*SRNN*/1, /*SRS*/0, /*HCIC*/1, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/1, /*A10*/0, /*OD*/1},	/* MEX_LGD_120  */
{/*DEMURA1*/1, /*DEMURA2*/0, /*SRNN*/1, /*SRS*/0, /*HCIC*/0, /*SLD*/1, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/0, /*A10*/0, /*OD*/0},	/* MEX_LGD_OLED  */
{/*DEMURA1*/1, /*DEMURA2*/1, /*SRNN*/1, /*SRS*/0, /*HCIC*/0, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/1, /*VALC*/1, /*POD*/0, /*A10*/0, /*OD*/1},	/* MEX_BOE  */
{/*DEMURA1*/1, /*DEMURA2*/1, /*SRNN*/1, /*SRS*/0, /*HCIC*/0, /*SLD*/0, /*LINE_OD*/1, /*PCLRC*/0, /*VALC*/1, /*POD*/0, /*A10*/0, /*OD*/1},	/* MEX_BOE_120  */
...
void drvif_color_set_Partten4AutoGamma_M8(unsigned char  Enable, unsigned short  r_Val,unsigned short  g_Val,unsigned short  b_Val)
{
...
void drvif_color_set_Partten4AutoGamma_M8P(unsigned char  Enable, unsigned short  r_Val,unsigned short  g_Val,unsigned short  b_Val)
{
	static bool intPtnenabled = false;
	unsigned int temp;
	unsigned int yuv2rgbmode=0;
	...
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_0_RBUS d_yuv2rgb_tab1_data_0_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_1_RBUS d_yuv2rgb_tab1_data_1_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_2_RBUS d_yuv2rgb_tab1_data_2_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_3_RBUS d_yuv2rgb_tab1_data_3_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_4_RBUS d_yuv2rgb_tab1_data_4_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_5_RBUS d_yuv2rgb_tab1_data_5_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_6_RBUS d_yuv2rgb_tab1_data_6_REG;
	M8P_yuv2rgb_d_yuv2rgb_tab1_data_8_RBUS d_yuv2rgb_tab1_data_8_REG;
	M8P_yuv2rgb_d_yuv2rgbbyy_control_RBUS    d_yuv2rgbbyy_control_REG;

	d_yuv2rgbbyy_control_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGBBYY_CONTROL_reg);

	d_yuv2rgb_tab1_data_0_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_0_reg);
	d_yuv2rgb_tab1_data_1_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_1_reg);
	d_yuv2rgb_tab1_data_2_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_2_reg);
	d_yuv2rgb_tab1_data_3_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_3_reg);
	d_yuv2rgb_tab1_data_4_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_4_reg);
	d_yuv2rgb_tab1_data_5_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_5_reg);
	d_yuv2rgb_tab1_data_6_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_6_reg);
	d_yuv2rgb_tab1_data_8_REG.regValue = IoReg_Read32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_8_reg);
	...
	d_yuv2rgb_control_1_reg.yuv2rgb_db_apply = 1;
	//IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Control_reg, d_yuv2rgb_control_1_reg.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Control_1_reg, d_yuv2rgb_control_1_reg.regValue);
	...
	if(Enable)
	{
		...
		d_yuv2rgb_tab1_data_2_REG.regValue = 0;
		d_yuv2rgb_tab1_data_3_REG.regValue = 0;
		d_yuv2rgb_tab1_data_8_REG.regValue = 0;
	...
	else
	{
		...
		//Set yuv2rgb matrix
		d_yuv2rgb_tab1_data_0_REG.k11 = table_index[_YUV2RGB_k11];
		d_yuv2rgb_tab1_data_8_REG.k31 = d_yuv2rgb_tab1_data_8_REG.k21 = d_yuv2rgb_tab1_data_0_REG.k11;
		d_yuv2rgb_tab1_data_0_REG.k12 = table_index[_YUV2RGB_k12];
		d_yuv2rgb_tab1_data_1_REG.k13 = table_index[_YUV2RGB_k13];
		d_yuv2rgb_tab1_data_2_REG.k22 = table_index[_YUV2RGB_k22];
		d_yuv2rgb_tab1_data_2_REG.k23 = table_index[_YUV2RGB_k23];
	...
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_0_reg, d_yuv2rgb_tab1_data_0_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_1_reg, d_yuv2rgb_tab1_data_1_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_2_reg, d_yuv2rgb_tab1_data_2_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_3_reg, d_yuv2rgb_tab1_data_3_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_4_reg, d_yuv2rgb_tab1_data_4_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_5_reg, d_yuv2rgb_tab1_data_5_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_6_reg, d_yuv2rgb_tab1_data_6_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Tab1_Data_8_reg, d_yuv2rgb_tab1_data_8_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGB_Control_reg, d_yuv2rgb_control_REG.regValue);
	IoReg_Write32(M8P_YUV2RGB_D_YUV2RGBBYY_CONTROL_reg,d_yuv2rgbbyy_control_REG.regValue);
}
...
{
	if (get_mach_type()==MACH_ARCH_RTK2885P)
		drvif_color_set_Partten4AutoGamma_M8(Enable, r_Val, g_Val, b_Val);
	else
		drvif_color_set_Partten4AutoGamma_M8P(Enable, r_Val, g_Val, b_Val);
}
/*subHDR=======================================================================*/
...
void drvif_color_data_access_debug_check(VIP_DRV_DataAccess_Debug data)
{
...
#ifndef CONFIG_MEMC_NOTSUPPORT
	unsigned char MEMC_bBgEn = HAL_POST_GetMuteScreenEnable();
	...
	if ((En_mode&DAD_ColorTemp_En) != 0) {
		...
		if(color_temp_coloraccessdata_ctrl_reg.read_en)
		{
			//pcid_pcid_accessdata_ctrl_reg.access_read_en = 0;
			//IoReg_Write32(PCID_PCID_AccessData_CTRL_reg, pcid_pcid_accessdata_ctrl_reg.regValue);
			rtd_pr_vpq_info("ColorTemp access data not ready \n");
		}
		else
		{//output PCID data
			...
			rtd_pr_vpq_info("ColorTemp access data RGB(%d %d %d) \n", color_temp_colorreaddata_data_y1_reg.r00,
				color_temp_colorreaddata_data_c1_reg.g00, color_temp_colorreaddata_data_c1_reg.b00);
	...
	// valc
	if ((En_mode&DAD_VALC_En) != 0) {
		valc_valc_accessdata_ctrl_reg.regValue = IoReg_Read32(VALC_VALC_AccessData_CTRL_reg);
		if(valc_valc_accessdata_ctrl_reg.access_read_en)
		{
			//pcid_pcid_accessdata_ctrl_reg.access_read_en = 0;
			//IoReg_Write32(PCID_PCID_AccessData_CTRL_reg, pcid_pcid_accessdata_ctrl_reg.regValue);
			rtd_pr_vpq_info("VALC access data not ready \n");
		}
		else
		{//output PCID data
			...
			rtd_pr_vpq_info("VALC access data RGB(%d %d %d) \n", valc_valc_accessdata_r_reg.access_read_r,
				valc_valc_accessdata_g_reg.access_read_g, valc_valc_accessdata_b_reg.access_read_b);
...
void drvif_color_data_access_debug_Set(VIP_DRV_DataAccess_Debug data)
{
	...
	if ((En_mode&DAD_ColorTemp_En) != 0) {
		...
		color_temp_coloraccessdata_posctrl_reg.startx = data.output_x;
		color_temp_coloraccessdata_posctrl_reg.starty = data.output_y;
	...
	if ((En_mode&DAD_VALC_En) != 0) {
		...
		valc_valc_accessdata_posctrl_reg.access_startx = data.output_x;
		valc_valc_accessdata_posctrl_reg.access_starty = data.output_y;
...
unsigned char drvif_Get_AIPQ_EFuse_Status(void)
{
	int ret = rtk_otp_field_read_int_by_name("sw_func_aipq");

	rtd_pr_vpq_info("AIPQ_otp_field ret=%d\n", ret);//0:support, -1:func error, other:not support
	if (ret == 0)
		return ret;
	else
		return 1;
...
void drvif_demo_window_split_line_colortemp(unsigned char en, unsigned int x, unsigned int y)
{
	color_temp_coloraccessdata_ctrl_RBUS color_temp_coloraccessdata_ctrl_reg;
	color_temp_coloraccessdata_ctrl_reg.regValue = IoReg_Read32(COLOR_TEMP_ColorAccessData_CTRL_reg);
	if (en) {
		color_temp_coloraccessdata_posctrl_RBUS color_temp_coloraccessdata_posctrl_reg;
		color_temp_colorwritedata_data_RBUS color_temp_colorwritedata_data_reg;

		color_temp_coloraccessdata_posctrl_reg.regValue = IoReg_Read32(COLOR_TEMP_ColorAccessData_CTRL_reg);
		color_temp_colorwritedata_data_reg.regValue = IoReg_Read32(COLOR_TEMP_ColorWriteData_DATA_reg);

		color_temp_coloraccessdata_posctrl_reg.startx = x;
		color_temp_coloraccessdata_posctrl_reg.starty = y;
		color_temp_colorwritedata_data_reg.r = color_temp_colorwritedata_data_reg.g = color_temp_colorwritedata_data_reg.b = 0;

		IoReg_Write32(COLOR_TEMP_ColorWriteData_DATA_reg, color_temp_colorwritedata_data_reg.regValue);
		IoReg_Write32(COLOR_TEMP_ColorAccessData_PosCtrl_reg, color_temp_coloraccessdata_posctrl_reg.regValue);
	}
	color_temp_coloraccessdata_ctrl_reg.cross_bar_en = color_temp_coloraccessdata_ctrl_reg.write_en = en;
	IoReg_Write32(COLOR_TEMP_ColorAccessData_CTRL_reg, color_temp_coloraccessdata_ctrl_reg.regValue);
}

void drvif_demo_window_split_line_memc_kpost(unsigned char en, unsigned int x, unsigned int y, unsigned char thickness)
{
	if (en) {
		HAL_POST_SetCursorPosition(x-(thickness/2), y);
		HAL_POST_SetCursorColor(0);
		HAL_POST_SetCursorWidth(thickness);
	}

	HAL_POST_SetCursorEnable(en);
	HAL_POST_SetCursorShow(en);
}
