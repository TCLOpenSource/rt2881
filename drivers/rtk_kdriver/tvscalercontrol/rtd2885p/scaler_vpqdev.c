...
struct semaphore VPQ_PQMASK_Semaphore;
...
struct semaphore VPQ_OD_DMA_TBL_Semaphore;
struct semaphore VPQ_ColorMapping_Semaphore;
struct semaphore VPQ_VPQIOC_CT_DelaySet_Semaphore;
struct semaphore VPQ_SQM_SHP_Semaphore;
struct semaphore VPQ_AI_Semaphore;
...
extern char VPQEX_VIP_VPQ_IOC_CT_DelaySet_ini(void);
extern char VPQEX_VIP_VPQ_IOC_CT_DelaySet_Process_VPQTASK(void);
...
extern void h3ddma_set_svp_protection(unsigned char bOnOff);
#ifndef BUILD_QUICK_SHOW
...
// 1015
extern unsigned char dtv_secure_flag, dtv_secure_flag_pre, vdec_secure_flag, vdec_secure_flag_pre;
// end 1015
...
// -----------------------------------------------------
// define the supported models for the current chip
// -----------------------------------------------------
static unsigned int aipq_support_model_list =	\
		AI_FACE_DETECT| 	\
		AI_SCENE_DETECT|	\
		AI_SQM_DETECT|		\
		AI_DEPTH_DETECT|	\
...
static AIAP_INFO_T aiap_info={NN_AIAP_STOP,0,0,0};
...
extern unsigned char NN_Meomory_CMA_release(void);
#if !IS_ENABLED(CONFIG_RTK_AI_OPTEE)
extern unsigned char NN_Meomory_CMA_is_empty(void);
#endif
#endif
...
/**************************************************
 HDR related
**************************************************/
	if( strcmp(cmd_buf, "force_cuva\n")==0 ) {
		Scaler_color_set_HDR_Type(HDR_DM_VIVID);
		scaler_vtop_dolby_mode_setting(HDR_DM_VIVID);
		if(Scaler_color_Get_HDR_AutoRun())
			Scaler_color_HDRIP_AutoRun(HDR_DM_VIVID);
		scaler_hdr_mode_setting(SLR_MAIN_DISPLAY, HDR_DM_VIVID);
		Scaler_SetDataFormatHandler();
	}
	if( strcmp(cmd_buf, "force_hdr10\n")==0 ) {
		Scaler_color_set_HDR_Type(HDR_DM_HDR10);
		scaler_vtop_dolby_mode_setting(HDR_DM_HDR10);
		if(Scaler_color_Get_HDR_AutoRun())
			Scaler_color_HDRIP_AutoRun(HDR_DM_HDR10);
		scaler_hdr_mode_setting(SLR_MAIN_DISPLAY, HDR_DM_HDR10);
		Scaler_SetDataFormatHandler();
	}
...
{
#if IS_ENABLED(CONFIG_RTK_AI_DRV)
	unsigned long pid=(unsigned long)filep->private_data;
	rtd_pr_vpq_info("vpq_release:pid=%ld\n",pid);
	down(&VPQ_AI_Semaphore);
	if(aiap_info.pid==pid){
		rtd_pr_vpq_info("vpq_release:reset aiap_info\n");
		aiap_info.state=NN_AIAP_STOP;
		aiap_info.pid=0;
		aiap_info.foundMask=0;
		aiap_info.readyMask=0;
	}
	#if !IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	if(aiap_info.state==NN_AIAP_STOP) NN_Meomory_CMA_release();
	#endif
	up(&VPQ_AI_Semaphore);
...
#if 1
/*========================= for INNX demura =================================*/
extern int VPQEX_internal_Demura_TBL_Max_check(short *demura_decode_tbl, unsigned char mode, unsigned char isSeparateRGB, unsigned char layerNum);
extern int VPQEX_internal_Demura_TBL_Diff_check(short *demura_decode_tbl, unsigned char mode, unsigned char isSeparateRGB, unsigned char layerNum);
extern int VPQEX_internal_Demura_CTRL_setting(short *demura_decode_tbl, unsigned char mode, unsigned char isSeparateRGB, unsigned char layerNum, unsigned short *layer_th_r, unsigned short *layer_th_g, unsigned short *layer_th_b);
char vpq_INNX_Demura_toDemura_Conv(unsigned char *pINNXDemura, unsigned int TBL_Size)
{
	char ret = 0;
#ifndef UT_flag	
	unsigned int i ,j ,k;
	unsigned char plane_num = 3;	// ori code flow onlu support 3 layer
	unsigned short layer_th[7];
	short *demura_decode_tbl = NULL;
	unsigned short tbl_height, tbl_width;
	short *pINNX_Demura_DeLut = NULL;
	
	tbl_height = 271;
	tbl_width = 481;
	
	pINNX_Demura_DeLut = (short *)dvr_malloc_specific(INNX_Demura_DecodeLUT_Size * sizeof(short), GFP_DCU2_FIRST);
	demura_decode_tbl = (short *)dvr_malloc_specific(plane_num * tbl_height * tbl_width * sizeof(short), GFP_DCU2_FIRST);

	if (pINNX_Demura_DeLut == NULL || demura_decode_tbl == NULL)
	{
		rtd_pr_vpq_emerg("INNX_Demura, pINNX_Demura_DeLut = NULL\n");

		if (pINNX_Demura_DeLut != NULL)
			dvr_free((void *)pINNX_Demura_DeLut);
		if (demura_decode_tbl != NULL)
			dvr_free((void *)demura_decode_tbl);
		
		ret = -1;
	}
	else
	{
		if (fwif_color_INNX_Demura_TBL(pINNXDemura, TBL_Size, pINNX_Demura_DeLut) != 0)
		{
			rtd_pr_vpq_emerg("INNX_Demura, fwif_color_INNX_Demura_TBL fail\n");
			ret = -1;
		}
		else
		{	
			//vpq_INNX_Demura_data_to_dma(&DeMura_TBL, pINNX_Demura_DeLut);
			// change format to 481x271
			for (k=0;k<plane_num;k++)
			{
				for (i=0;i<tbl_height;i++)
				{
					...
					{
						demura_decode_tbl[k*tbl_height*tbl_width+i*tbl_width+j] = pINNX_Demura_DeLut[k*tbl_height*INNX_Demura_W16+i*INNX_Demura_W16+j];

					}
				}
			}
			// innx is only 3 layer
			layer_th[0] = INNX_Demura_gray_setting[0];
			layer_th[1] = INNX_Demura_gray_setting[2];
			layer_th[2] = INNX_Demura_gray_setting[3];
			layer_th[3] = INNX_Demura_gray_setting[3];
			layer_th[4] = INNX_Demura_gray_setting[3];
			layer_th[5] = INNX_Demura_gray_setting[4];
			layer_th[6] = INNX_Demura_gray_setting[1];
			...
			rtd_pr_vpq_info("INNXPDM, convert ,plane_num=%d,th=%d,%d,%d,%d,%d,%d,%d,\n", 
				plane_num, layer_th[0],layer_th[1],layer_th[2],layer_th[3],layer_th[4],layer_th[5],layer_th[6]);
			
			fwif_color_DeMura_init(1, 0);
				
			ret = 0;
			rtd_pr_vpq_info("INNX_Demura, vpq_INNX_Demura_toDemura_Conv done\n");
		}
...
int vpq_decide_OD_byPanelTconType(void)
{
	...
	case P_CHPI:
	case P_ISP:
		return 0;	// control by od bin flow, always 0
	//otherwise
	default:
		return 0;
	}
...
void vpq_do_OD_init_from_tbl(void)
{
	unsigned char tbl_idx;
#ifndef UT_flag
#ifndef BUILD_QUICK_SHOW
	if(is_QS_pq_enable()==1) {
		rtd_pr_vpq_info("vpq_do_OD_init_from_tbl,, init from qs, bODInited=%d,\n", bODInited);
	} else 
#endif
	{
		bODInited = 0;
		...
		if (mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.od_en == 1) {
			// need disable first
			fwif_color_od_DMA_reset();
			
			if (fwif_color_od_init(mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.Y_mode_en)) {
				fwif_color_set_od_by_TBL(&mVIP_ODTable[tbl_idx], 0);
				bODInited = 1;
				fwif_color_set_od_dma_enable(1);
				usleep_range(21000, 22000);
				fwif_color_set_od_enable(1);
				rtd_pr_vpq_info("vpq_do_OD_init_from_tbl,tbl_idx=%d, enable\n", tbl_idx);
			} else {
				bODInited = 0;
#ifndef BUILD_QUICK_SHOW				
				if (SLD_DMA_Ready == 0)
				...
				rtd_pr_vpq_info("vpq_do_OD_init_from_tbl,, mem alloc fail, disabel\n");
			}
		} else {
			bODInited = 0;
#ifndef BUILD_QUICK_SHOW
			...
			fwif_color_set_od_enable(0);		
			rtd_pr_vpq_info("vpq_do_OD_init_from_tbl,,	disabel\n");
		}
	}	
#endif
}

void vpq_init_gamma_invgamma(void)
{
	extern UINT32 Identity_Gamma;
	extern unsigned short Gamma_sRGB2Linear_14bit[1025];
	extern unsigned short Gamma_Linear2sRGB_14bit[1025];
#ifndef UT_flag
	fwif_color_set_invgamma_gamma_mode(0xFF);
#endif
	...
	fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 0, 0);
#if 0//ndef UT_flag
	fwif_color_inv_gamma_control_back(SLR_MAIN_DISPLAY, 1);
	fwif_color_gamma_control_back_enhance(SLR_MAIN_DISPLAY, 1);
#endif	
}
...
void vpq_boot_init(void)
{
...
#ifndef UT_flag
	drvif_color_PQ_CLK_init();
#endif	
	if(is_QS_pq_enable()==1) { //current is QS flow pq_quick_show flow
#ifndef UT_flag
		fwif_color_load_QS_vip_memory_from_carveout();
		Scaler_RADCR_ColorTemp_Init();
#endif
#ifndef UT_flag		
			VPQEX_VIP_VPQ_IOC_CT_DelaySet_ini();
#endif
		fwif_color_od_BND_early_init();
		...
		{
			rtd_pr_vpq_info("fwif_color_set_Panel_Dither_By_ini\n");
		}
		else
#endif			
		{
		...
		if(get_mach_type()==MACH_ARCH_RTK2885P){
			...
			// merlin8p IC, run merlin8 flow
			sw_sld_Nomemc_sld_logo_to_demura_init();
		}else if(get_mach_type()==MACH_ARCH_RTK2885PP){
			// merlin8p IC , run merlin8p flow
			sw_sld_Nomemc_sld_logo_to_demura_init();
		}
	...
	/* It is for workaround End  */
		...
		//fwif_color_PQ_SR_MODE_init();	// move to vpq_boot__ini_QS()
		...
		fwif_color_gamma_control_back(SLR_SUB_DISPLAY, 1);
#endif
		//fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0]);
		//fwif_set_gamma_system_enhance(SLR_MAIN_DISPLAY, &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0]);
		vpq_init_gamma_invgamma();
	...
	//#ifndef CONFIG_MEMC_BYPASS
		...
		rtd_pr_vpq_emerg("[%s][MEMC][,%d,%d,]\n", __FUNCTION__, get_MEMC_bypass_status_refer_platform_model(), MEMC_First_Run_Done);
...
void vpq_boot_init_QS(void)
{
	...
	// load od bin from qs
	fwif_color_OD_init_quick_show();
	vpq_do_OD_init_from_tbl();
	fwif_color_set_od_2p();

	// load D 3D lut bin
	fwif_color_D3DLUT_init_quick_show();

	// NNSR init on qs
	fwif_color_PQ_SR_MODE_init();
	
	//load ld/blpf bin from qs
	if (fwif_color_LD_init_quick_show() && fwif_color_BLPF_init_quick_show()) {
		vpq_do_LD_init_from_tbl();
	}	
...
void vpq_do_OUTGamma_init(void)
{
	extern SLR_VIP_TABLE_AUTO_ACC_CTRL mVIP_TABLE_AUTO_ACC_CTRL;
	//extern SLR_VIP_TABLE_OUTGAMMA mVIP_OutputGamma;
	unsigned char tbl_sel = 0xFF;
	unsigned int i;
	unsigned char tmp0 = 0, tmp1 = 0;
	unsigned short *pTemp16;

	pTemp16 = (unsigned short *)kmalloc(VIP_CHANNEL_RGB_MAX * SLR_VIP_OUTGAMMA_Index_Num * sizeof(short), GFP_KERNEL);
	if (pTemp16 == NULL)
	{
		rtd_pr_vpq_emerg("vpq_do_OUTGamma_init,alloc fail,\n");
		return;
	}

	if (mVIP_TABLE_AUTO_ACC_CTRL.Header[AutoACC_CTRL_mode] != AUTO_ACC_Mode_UNKONW)
	{
		for (i=0;i<1024;i++)
		{
	...
	rtd_pr_vpq_info("vpq_do_OUTGamma_init,mode=%d,freq=%d,tbl_sel=%d,en=%d,loc=%d,\n", 
		mVIP_TABLE_AUTO_ACC_CTRL.Header[AutoACC_CTRL_mode], mVIP_TABLE_AUTO_ACC_CTRL.Header[AutoACC_CTRL_Freq_for_Fix_TBL], tbl_sel, tmp0, tmp1);
...
{	
	if (mVIP_PCIDTable.lineod_en == 1)	// resume from bin table
	{
		rtd_pr_vpq_info("pcid_do_resume, resume from bin flow, pcid_en=%d, pcod2_en=%d,\n", 
			drvif_color_get_pcid_enable(), drvif_color_get_pcid2_enable());
	
		fwif_color_set_LINEOD_by_pSLRTBL(&mVIP_PCIDTable);	
	}
	rtd_pr_vpq_info("pcid_do_resume, resume done, table_en=%d,\n", mVIP_PCIDTable.lineod_en);
}

extern SLR_VIP_TABLE_VALC mVIP_VALC_Table;
void VALC_do_resume(void)
{	
	if (mVIP_VALC_Table.VALC_En == 1)	// resume from bin table
	{
		rtd_pr_vpq_info("VALC_do_resume, resume from bin flow, valc_en=%d, DTG_valc_en=%d,\n", 
			driver_color_get_valc_enable(), driver_color_get_DTG_valc_enable());	
		fwif_color_set_VALC_by_pSLRTBL(&mVIP_VALC_Table);
	}
	rtd_pr_vpq_info("VALC_do_resume, resume done, table_en=%d,\n", mVIP_VALC_Table.VALC_En);
}
...
void vpq_do_resume(void)
{
	...
	extern unsigned short g_reset_sharp_idx_flag;
	extern unsigned char g_reset_sharp_level_flag;
	DRV_Sharpness_Level Sharpness_Level;
...
fwif_color_PQ_SR_MODE_init();
	...
	scalerAI_pq_mode_ctrl(aipq_mode, Scaler_GetDCC_Mode());
	scalerAI_config(aipq_mode.ap_mode);
#endif
...
static bool cma_release_tsk_running_flag = FALSE;
static struct task_struct *p_cma_release_tsk = NULL;
...
unsigned char CMA_release_tsk_flag = 0;
static int CMA_release_tsk(void *p)//This task run new_game_mode_tsk
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
		return 0;
#else
	unsigned int time1 = 0;
	rtd_pr_vpq_notice("CMA_release_tsk()\n");
	current->flags &= ~PF_NOFREEZE;
	...
	while (1)
	{
		...
		if (kthread_should_stop()){
			rtd_pr_vpq_notice("CMA_release_tsk stop!!\n");
			return 0;
		}

		if(CMA_release_tsk_flag == 1){

			//Add CMA release code below++
			time1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
			Scaler_MEMC_CMA_Memory_Release();
			rtd_pr_vpq_notice("CMA_release_tsk_mid(%d)\n", rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-time1);
			//Add CMA release code above--
	...
	rtd_pr_vpq_notice("\r\n####CMA_release_tsk: exit...####\n");
	do_exit(0);
	return 0;
#endif
}

void create_cma_release_tsk(void)
{
	int err;
	if (cma_release_tsk_running_flag == FALSE) {
		p_cma_release_tsk = kthread_create(CMA_release_tsk, NULL, "CMA_release_tsk");

	    if (p_cma_release_tsk) {
			wake_up_process(p_cma_release_tsk);
			cma_release_tsk_running_flag = TRUE;
	    } else {
	    	err = PTR_ERR(p_cma_release_tsk);
		rtd_pr_vpq_emerg("Unable to start CMA_release_tsk (err_id = %d)./n", err);
	    }
	}
}

void delete_cma_release_tsk(void)
{
	int ret;
	if (cma_release_tsk_running_flag && p_cma_release_tsk) {
 		ret = kthread_stop(p_cma_release_tsk);
 		if (!ret) {
 			p_cma_release_tsk = NULL;
 			cma_release_tsk_running_flag = FALSE;
			rtd_pr_vpq_emerg("CMA_release_tsk thread stopped\n");
 		}
	}
}
...
static int se_tsk(void *p)//This task run se proc
{
	...
	while (1)
	{
		...
		// 1015
		if(vdec_secure_flag != vdec_secure_flag_pre)
		{
			down(get_force_i3ddma_semaphore());
			h3ddma_set_svp_protection(vdec_secure_flag);
			up(get_force_i3ddma_semaphore());

			set_vdec_securestatus_aipq_proc(vdec_secure_flag);
			vdec_secure_flag_pre = vdec_secure_flag;
		}
		if(dtv_secure_flag != dtv_secure_flag_pre)
		{
			down(get_force_i3ddma_semaphore());
			h3ddma_set_svp_protection(dtv_secure_flag);
			up(get_force_i3ddma_semaphore());

			set_dtv_securestatus_aipq_proc(dtv_secure_flag);
			dtv_secure_flag_pre = dtv_secure_flag;
		}
		// end 1015
...
extern unsigned char* LOGO_480_virAddr;
void Scaler_Get_Logo_Detect_VirAddr(void);
void Scaler_Check_Logo_Detect_VirAddr(void);
void Scaler_Get_Logo_480_Detect_VirAddr(void);
void Scaler_Check_Logo_480_Detect_VirAddr(void);
...
static int VPQ_detect_tsk(void *p)
{
    ...
    while (1)
    {
		...
		fwif_color_update_invgamma_gamma_table_TASK();
		
		// color mapping or csc polling
		Scaler_set_APDEM_CM_flow_Polling_TASK();

		fwif_color_set_PQ_ByPass_Handler_VPQ_TSK();
		fwif_color_NNSR_model_TBL_update_VPQTSK();

#ifdef CONFIG_LOGO_480_DDR
		if(LOGO_480_virAddr == NULL && MEMC_First_Run_force_SetInOutUseCase_Done == 1)
		{
			Scaler_Get_Logo_480_Detect_VirAddr();
		}

		if(LOGO_480_virAddr != NULL && LOGO_480_DDR_En==1)
		{
			Scaler_Check_Logo_480_Detect_VirAddr();
		}
#endif

		fwif_color_Sharpness_bySQM_VPQTSK();

		//	apply OD table by frame rate on VRR case 
		fwif_color_set_od_table_VRR_TASK();

		// apply ACC by vrr
		fwif_color_AUTO_ACC_OutputGamma_VRR_TASK();

		// d2 ip delay set from vpq ex ioctl
		VPQEX_VIP_VPQ_IOC_CT_DelaySet_Process_VPQTASK();

		// AI SR Demo
		fwif_color_Demo_AI_SR_update_VPQTSK();
		
#ifdef CONFIG_SUPPORT_SLD_BY_MEMC_DEMURA
			if( LD_virAddr == NULL && MEMC_First_Run_force_SetInOutUseCase_Done == 1 && SLD_SW_En == 1 && memc_logo_read_en == 1)
			{
				Scaler_Get_Logo_Detect_VirAddr();
			}
	
			if( LD_virAddr != NULL && SLD_SW_En == 1 && memc_logo_read_en == 1)
			{
		...
		// for smoothtoogle sharpness/nr change
		Scaler_resolution_change_check();
...
#if IS_ENABLED(CONFIG_RTK_AI_DRV)
unsigned int vpq_get_aipq_support_model_list(void)
{
	return aipq_support_model_list;
}
#endif
COLORTEMP_ELEM_T ct;
...
void vpq_ioctl_cmd_VPQ_IOC_INIT(void)
{
	if(is_QS_pq_enable()==1) {
		//fwif_color_set_DCC_Init(SLR_MAIN_DISPLAY);
		/*fwif_color_set_DCC_Init_tv006();*/
	...
	*/
	...
	case VPQ_IOC_GET_Input_V_Freq:
		{
			...
			Input_V_Freq = (Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 100);
	...
	case VPQ_IOC_SET_IRE_INNER_PATTERN:
		{
			...
			if (copy_from_user(&args, (int __user *)arg, sizeof(unsigned int))) {
				...
				#ifndef UT_flag
				fwif_color_WB_Pattern_ctrl_osd_on_off(!(args>>16));
				#endif
	...
	case VPQ_IOC_SET_PQModeInfo:
	{
		...
		else
		{
			...
			if (get_support_vo_force_v_top(SLR_MAIN_DISPLAY, support_sdr_max_rgb) && (PQModeInfo_flag[0] == HAL_VPQ_HDR_MODE_SDR))
					...
					if(lowdelaymode) {
						...
						//merlin8/8p mdomain one-bin
                                                if(get_mach_type() == MACH_ARCH_RTK2885P) {
                                                        ...
                                                        nonlibdma_drv_memory_set_game_mode_dynamic(_ENABLE);
                                                }
						...
						//merlin8/8p mdomain one-bin
                                                if(get_mach_type() == MACH_ARCH_RTK2885P) {
						        //drv_memory_set_game_mode_dynamic(_DISABLE);
						        nonlibdma_drv_memory_set_game_mode_dynamic(_DISABLE);
                                                }
...
/*========================= for INNX demura =================================*/
	...
	case VPQ_IOC_AI_DETECTION_ON:
	{
		...
		{
			int i=0;
			//check cma is released
			for(i=0;i<50;i++){
				if(NN_Meomory_CMA_is_empty())
					break;
				msleep(20);
			}
			rtd_pr_vpq_info("VPQ_IOC_AI_DETECTION_ON:list=%d,wait %dms for cma release\n",list,i*20);
		}
#endif

		down(&VPQ_AI_Semaphore);
		if(list) scalerAI_config(AI_MODE_ON);
		up(&VPQ_AI_Semaphore);
		ret = 0;
		break;
	}
	case VPQ_IOC_AI_DETECTION_OFF:
	{
		...
		rtd_pr_vpq_info("VPQ_IOC_AI_DETECTION_OFF:list=%d\n",list);
		down(&VPQ_AI_Semaphore);
		if(list) scalerAI_config(AI_MODE_OFF);
		up(&VPQ_AI_Semaphore);
		ret = 0;
		break;
	}
	case VPQ_IOC_AI_MODEL_LIST:
	{
		unsigned int list=0;

		if(rtk_is_NNIP_function_disable()){
			rtd_pr_vpq_emerg("VPQ_IOC_AI_MODEL_LIST: nnip otp disable\n");
			aipq_support_model_list=0;
		}else{
			list=aipq_support_model_list;
			rtd_pr_vpq_info("VPQ_IOC_AI_MODEL_LIST: %d\n",list);
		}

		if (copy_to_user((void __user *)arg, (void *)&list, sizeof(unsigned int))) {
			rtd_pr_vpq_emerg("kernel VPQ_IOC_AI_MODEL_LIST fail\n");
			ret = -1;
		} else {
			ret = 0;
		}
	...
	case VPQ_IOC_AI_ALLOW_MODE:
	{

	#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
		unsigned int allowMode = 1 << VIP_NN_BUFFER_224GENRE;
		if (scalerAI_check_if_AIPQ_enabled())
			allowMode = (unsigned int)-1;
	#else
		unsigned int allowMode=(unsigned int)scalerAI_check_if_AIPQ_enabled();
	#endif

		rtd_pr_vpq_info("VPQ_IOC_AI_ALLOW_MODE: 0x%x\n",allowMode);
		if (copy_to_user((void __user *)arg, (void *)&allowMode, sizeof(unsigned int))) {
			rtd_pr_vpq_emerg("kernel VPQ_IOC_AI_ALLOW_MODE fail\n");
			ret = -1;
		} else {
			ret = 0;
		}
	...
	case VPQ_IOC_AI_SET_AP_MASK:
	{
		NN_APMASK_T apMask;

		down(&VPQ_AI_Semaphore);
		if (copy_from_user((void *)&apMask, (const void __user *)arg, sizeof(NN_APMASK_T)))
		{
			rtd_pr_vpq_emerg("VPQ_IOC_AI_SET_AP_STATE fail\n");
			up(&VPQ_AI_Semaphore);
			return -1;
		}else
			rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state(%d),init(%d),pid(%d),mask(%d)\n",aiap_info.state,apMask.init,apMask.pid,apMask.mask);

		if(apMask.init){
			if(aiap_info.state==NN_AIAP_STOP && aiap_info.pid==0){
				aiap_info.state=NN_AIAP_INIT;
				aiap_info.pid=apMask.pid;
				aiap_info.foundMask=apMask.mask;
				aiap_info.readyMask=0;
				file->private_data = (void*)((unsigned long)apMask.pid);
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d\n",aiap_info.state);
			}else if(aiap_info.state==NN_AIAP_INIT && aiap_info.pid==apMask.pid){
				aiap_info.readyMask|=apMask.mask;
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d,readyMask=%d\n",aiap_info.state,aiap_info.readyMask);
				if(aiap_info.readyMask==aiap_info.foundMask){
					aiap_info.state=NN_AIAP_READY;
					rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d\n",aiap_info.state);
				}
			}else if(aiap_info.state==NN_AIAP_INIT && aiap_info.pid==apMask.pid){
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:init fail case,mask=%d\n",apMask.mask);
				aiap_info.foundMask&=~apMask.mask;
				if(aiap_info.readyMask==aiap_info.foundMask){
					aiap_info.state=NN_AIAP_READY;
					rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d\n",aiap_info.state);
				...
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:do nothing\n");
		}else{
			if(aiap_info.state==NN_AIAP_READY && aiap_info.pid==apMask.pid){
				aiap_info.readyMask&=~apMask.mask;
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d,readyMask=%d\n",aiap_info.state,aiap_info.readyMask);
				if(aiap_info.readyMask==0){
					aiap_info.state=NN_AIAP_STOP;
					aiap_info.pid=0;
					rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:state=%d\n",aiap_info.state);
				}
			}else
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:wrong state(%d) or pid(%d,%d)\n",aiap_info.state,aiap_info.pid,apMask.pid);
		}
		up(&VPQ_AI_Semaphore);
	...
	case VPQ_IOC_AI_GET_AP_STATE:
	{
		rtd_pr_vpq_info("VPQ_IOC_AI_GET_AP_STATE:state=%d\n",aiap_info.state);

		down(&VPQ_AI_Semaphore);
		if (copy_to_user((void __user *)arg, (void *)&aiap_info.state, sizeof(unsigned int))) {
			rtd_pr_vpq_emerg("kernel VPQ_IOC_AI_GET_AP_STATE fail\n");
			ret = -1;
		} else {
			ret = 0;
		}
	...
	case VPQ_IOC_AI_GET_FACE_LIST:
	{
		NN_FACE_T faceList;
		scalerAI_face_get_list(&faceList);
		rtd_pr_vpq_info("VPQ_IOC_AI_GET_FACE_LIST:count=%d\n",faceList.count);

		if (copy_to_user((void __user *)arg, (void *)&faceList, sizeof(NN_FACE_T))) {
			rtd_pr_vpq_emerg("kernel VPQ_IOC_AI_GET_FACE_LIST fail\n");
			ret = -1;
		} else {
			ret = 0;
		}
	...
	#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	case VPQ_IOC_GET_NN_TEE_BUF:
	{
		NN_BUF_INFO_T info;
		int copyRet = copy_from_user((void *)&info, (void __user *)arg, sizeof(NN_BUF_INFO_T));
		if( copyRet != 0 ) {
			rtd_pr_vpq_emerg("[AI] VPQ_IOC_GET_NN_TEE_BUF copy from user fail %d\n", copyRet);
			return -1;
		}
		if(!ai_optee_get_nn_tee_buf(&info))
			return -1;

		copyRet = copy_to_user((void __user *)arg, (void *)&info, sizeof(NN_BUF_INFO_T));
		if( copyRet != 0 ) {
			rtd_pr_vpq_emerg("[AI]VPQ_IOC_GET_NN_TEE_BUF copy to user fail %d\n", copyRet);
			return -1;
		}
	...
	case VPQ_IOC_FREE_NN_TEE_BUF:
	{
		NN_BUF_INFO_T info;
		int copyRet = copy_from_user((void *)&info, (void __user *)arg, sizeof(NN_BUF_INFO_T));
		if( copyRet != 0 ) {
			rtd_pr_vpq_emerg("[AI]VPQ_IOC_FREE_NN_TEE_BUF copy from user fail %d\n", copyRet);
			return -1;
		}

		if(!ai_optee_release_nn_tee_buf(&info))
			return -1;
...
#include <dolby_idk_5/dolby_adapter.h>
#endif
        ...
        switch(cmd)
        {
			...
			if (Get_Factory_SelfDiagnosis_Mode()) {
				...
				if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_ACTIVE){
					...
					else if(get_mach_type() == MACH_ARCH_RTK2885P && v4l2_vpq_lowdelaymode != nonlibdma_drv_memory_get_game_mode_flag())	// game_mode_cmd_check + 1 when current  != AP setting in picture mode
						game_mode_cmd_check = game_mode_cmd_check + 1;
					up(get_gamemode_check_semaphore());
					//printk(KERN_EMERG"[crixus]game_mode_cmd_check = %d\n", game_mode_cmd_check);
				}
				rtd_pr_vsc_notice("[%s][%d][LowDelayMode][memc,%d,vpq,%d,]", __FUNCTION__, __LINE__, fV4l2_memc_lowdeay, v4l2_vpq_lowdelaymode);
				ret = 0;
			}
		}
                break;
...
int vpq_module_init(void)
{
	...
	sema_init(&VPQ_OD_DMA_TBL_Semaphore, 1);
	sema_init(&VPQ_ColorMapping_Semaphore, 1);
	sema_init(&VPQ_VPQIOC_CT_DelaySet_Semaphore, 1);
	sema_init(&VPQ_SQM_SHP_Semaphore, 1);
	sema_init(&VPQ_AI_Semaphore, 1);
	sema_init(&VPQ_PQMASK_Semaphore, 1);
