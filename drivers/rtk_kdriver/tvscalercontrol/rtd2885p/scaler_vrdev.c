...
void set_DC2H_rgb2yuv_ctrl_reg_refer_dump_location_vr(KADP_VR_DUMP_LOCATION_TYPE_T in_position, SIZE *in_size, unsigned char *dc2h_in_sel)
{
    ...
    else if(in_position == KADP_VR_SCALER_INPUT)
    {
        ...
        pixel_clock_vo = (unsigned int)( (unsigned long long)htotal * (unsigned long long)vtotal * (unsigned long long)Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000ULL );
        ...
        *dc2h_in_sel = _Idomain_Sdnr_input;
        ...
        //always 4k120 source
        if(is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))//4k120 source
        {
            if(vr_is_4K2K_capture() == TRUE)
            {
                if(get_VR_Pixel_Format() == VR_CAP_NV12)  //4k2k120 NV12
