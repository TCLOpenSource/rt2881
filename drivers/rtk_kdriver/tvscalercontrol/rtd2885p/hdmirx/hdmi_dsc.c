...
#include "hdmi_dp_common_sel.c"  // for hdmi_dp_sel api
...
//--------------------------------------------------
// Func  : lib_dprx_dscd_get_dprx_dscd_en
//
// Desc  : get DSCD input source
//
// Param : port   : dprx port
//
// Retn  : 0: DSCD input = HDMI
//         1 : DSCD input = DPRX
//--------------------------------------------------
unsigned char lib_hdmi_dsc_get_hdmi_dsc_enable(void)
{
    return DSCD_CTRL_get_dscd_dp_en(hdmi_in(DSCD_CTRL_reg)) ? 0 : 1;
}

//--------------------------------------------------
// Func  : lib_hdmi_dsc_set_hdmi_dsc_enable
//
// Desc  :
//
// Param : port   : dprx port
...
// Retn  : N/A
//--------------------------------------------------
void lib_hdmi_dsc_set_hdmi_dsc_enable(unsigned char enable)
{
    hdmi_mask(DSCD_CTRL_reg, ~DSCD_CTRL_dscd_dp_en_mask, (enable) ? 0 : DSCD_CTRL_dscd_dp_en_mask);
}

unsigned char lib_hdmi_is_hdmi_dsc_available(void)
{
    return (lib_hdmi_dsc_get_crt_on() && lib_hdmi_dsc_get_hdmi_dsc_enable()) ? 1 : 0;
}

unsigned char lib_hdmi_get_dsc_enable(void)
{
    if(!lib_hdmi_is_hdmi_dsc_available())
    {
...
const char* _hdmi_hd21_dsc_fsm_str(HDMI_DSCD_FSM_T fsm)
{
    ...
    case DSCD_FSM_REQUEST_DSCD: return "DSCD_FSM_REQUEST_DSCD";
...
unsigned char newbase_hdmi_hd21_dsc_pps_measure(unsigned char port, MEASURE_TIMING_T *tm)
{
    ...
    tm->dsc_version_minor = dscd_hw_pps.dsc_version_minor;
    tm->convert_rgb = (unsigned char)dscd_hw_pps.convert_rgb;
    tm->simple_422 = (unsigned char)dscd_hw_pps.simple_422;
    tm->native_420 = dscd_hw_pps.native_420;
    tm->native_422 = dscd_hw_pps.native_422;

    SET_H_DSC_VERSION_MINOR(port, dscd_hw_pps.dsc_version_minor);
    SET_H_DSC_CONVERT_RGB(port, (unsigned char)dscd_hw_pps.convert_rgb);
    SET_H_DSC_SIMPLE_422(port, (unsigned char)dscd_hw_pps.simple_422);
    SET_H_DSC_NATIVE_420(port, dscd_hw_pps.native_420);
    SET_H_DSC_NATIVE_422(port, dscd_hw_pps.native_422);
    ...
    HDMI_WARN("[DSCD] dsc_version_minor: %d\n", tm->dsc_version_minor);
    HDMI_WARN("[DSCD] convert_rgb: %d\n", tm->convert_rgb);
    HDMI_WARN("[DSCD] simple_422: %d\n", tm->simple_422);
    HDMI_WARN("[DSCD] native_420: %d\n", tm->native_420);
    HDMI_WARN("[DSCD] native_422: %d\n", tm->native_422);
...
static unsigned int  ownership_print = 0;
static unsigned int  pre_hdmi_dp_sel = 0;

//--------------------------------------------------
// Func  : _check_share_dscd_ownership
//
// Desc  : check the HDMI own DSCD or not
//
// Param : N/A
//
// Retn  : 0 : DSCD is not ready for HDMI
//         1 : DSCD is ready for HDMI
//--------------------------------------------------
unsigned int _check_share_dscd_ownership(void)
{
    unsigned char hdmi_dp_sel = newbase_hdmi_check_hdmi_dp_sel_setting();

    // print hdmi_dp_sel message
    if (pre_hdmi_dp_sel != hdmi_dp_sel)
    {
        pre_hdmi_dp_sel = hdmi_dp_sel;
        HDMI_WARN("[SHARE_DSCD] DSCD ownership changed to %s\n", (hdmi_dp_sel) ? "DP" : "HDMI");
        SET_DSCD_FSM(DSCD_FSM_WAIT_LT_PASS);  //.. force reset FSM
        ownership_print = 0; // reset print counter
    }

    if (ownership_print++ >= 800 && newbase_hdmi_is_dispsrc(SLR_MAIN_DISPLAY))
    {
        HDMI_WARN("[SHARE_DSCD] DSCD ownership is %s\n", (hdmi_dp_sel) ? "DP" : "HDMI");
        ownership_print = 0;
    }

    return hdmi_dp_sel ? 0 : 1;
}


unsigned char _fake_dscd_pps_measure(CVTEM_INFO* p_info, MEASURE_TIMING_T *tm)
{
    DSC_PPS dscd_hw_pps;
    ...
    _parse_dsc_pps(p_info->pps, &dscd_hw_pps);
    ...
    tm->h_act_len = dscd_hw_pps.pic_width;
    tm->v_act_len = dscd_hw_pps.pic_height;
    tm->h_total   = dscd_hw_pps.pic_width + p_info->hback + p_info->hfront + p_info->hsync;
    tm->h_act_sta = p_info->hsync + p_info->hback;
    tm->polarity = 3; //force set H/V positive
    tm->IHSyncPulseCount = p_info->hsync;
...
void _fake_dscd_handler(unsigned char port)
{
    ...
    case DSCD_FSM_WAIT_PPS_READY:
    {
        ...
        if (_parse_cvtem_info(hdmi_rx[port].cvtem_emp, &info) == TRUE)
        {
            DSC_PPS dscd_hw_pps;
            unsigned int current_pps_crc;

            _parse_dsc_pps(info.pps, &dscd_hw_pps);

            current_pps_crc = _compute_checksum(info.pps, 128);
    ...
    case DSCD_FSM_PPS_MEASURE:
    {
        ...
        else if(!_fake_dscd_pps_measure(&info, &dscd_fsm_st.pps_timing))
        {
    ...
    case DSCD_FSM_RUN:
    {
        ...
        else
        {
            ...
            if (_parse_cvtem_info(hdmi_rx[port].cvtem_emp, &info) != TRUE)
            {
                HDMI_WARN("[newbase_hdmi_hd21_dsc_pps_measure] Invalid DSC PPS before set FW PPS\n");
                SET_DSCD_FSM(DSCD_FSM_WAIT_CVTEM);
            }
            current_pps_crc = _compute_checksum(info.pps, 128);
    ...
    if ((GET_DSCD_FSM() >DSCD_FSM_REQUEST_DSCD) && (hdmi_dscd_print_cycle_cnt++ >= g_dscd_print_cycle_cnt_thd))
    {
        DSCD_FLOW_PRINTF("DSCD FSM:%s,(Current,DSC) port=(%d,%d), PPS hactive:%d, vactive:%d, output_vback:%d\n",
            _hdmi_hd21_dsc_fsm_str(GET_DSCD_FSM()),
...
#endif // CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP

void newbase_hdmi_hd21_dsc_handler(unsigned char port)
{
#ifdef CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
    if (_check_share_dscd_ownership()==0)
    {
        _fake_dscd_handler(port);  // run shared dscd handler
        return ;
    }
#endif // CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
    ...
    {
        if (lib_hdmi_get_hdmi_dp_sel()!=0)
        {
            HDMI_DSC_ERR("DSCD clksel is not hdmi, force reset DSCD FSM!!\n");
            SET_DSCD_FSM(DSCD_FSM_WAIT_LT_PASS);
        }
    ...
    case DSCD_FSM_WAIT_CVTEM:
    {
        ...
        {
            SET_DSCD_FSM(DSCD_FSM_REQUEST_DSCD);
        }
        break;
    }
    case DSCD_FSM_REQUEST_DSCD:
    {
...
void newbase_hdmi_hd21_dsc_enanble(unsigned char port, unsigned char enable)
{
    ...
    if (enable)
    {
        lib_hdmi_set_hdmi_dp_sel(0);  // force 420 common to HDMIRX (including DSCD)
...
unsigned char newbase_hdmi_check_dscd_output_valid(void)
{
...
#ifdef CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
    if (_check_share_dscd_ownership()==0 && GET_DSCD_FSM() > DSCD_FSM_PPS_MEASURE)
        return TRUE;  // force report DSCD output valid
#endif
    ...
    while(wait_count>0 && lib_hdmi_is_hdmi_dsc_available());
