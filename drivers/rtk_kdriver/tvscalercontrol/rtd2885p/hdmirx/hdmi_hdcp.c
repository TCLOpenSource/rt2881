...
{
#if defined(HD21_hdcp_status_reg)
    if (hdmi_rx[nport].hdmi_2p1_en)
        return HD21_hdcp_status_get_enc_tog(hdmi_in(HD21_hdcp_status_reg));
#endif
...
#if defined(HD21_hdcp_status_reg)
    if (hdmi_rx[nport].hdmi_2p1_en){
        hdmi_out(HD21_hdcp_status_reg, hdmi_in(HD21_hdcp_status_reg) | HD21_hdcp_status_enc_tog_mask);    // write one clear
    }
    else
#endif
