...
extern unsigned char emcu_get_hdmi_det(unsigned char channel);
extern unsigned char emcu_set_hdmi_det(unsigned int nport, unsigned int enable);
...
unsigned char per_port_cec_osdname[4][HDMI_COMP_CEC_OSDNAME_LEGNTH];
...
static unsigned char hdmicec_device_num = 0;
static unsigned int hdmicec_msg_cnt = 0;
static unsigned char hdmicec_HPD_st[4] = {0, 0, 0, 0};
static unsigned int g_hdmicec_print_cycle_cnt_thd = 10000;
...
#ifndef BUILD_QUICK_SHOW
void newbase_hdmi_qs_set_irq_status(unsigned char port,unsigned char frl_mode)
{
	if(GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_AVMUTE_IRQ_ENABLE))
	{
	...
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_HDMI_DVI_MODE_IRQ_ENABLE))
	{
		HDMI_INFO("newbase_hdmi_qs_set_irq_status[%d] is_fw_mode=%d, current_auto_mode_result=%d\n", port, lib_hdmi_is_modedet_fwmode(port), lib_hdmi_get_hdmi_mode_reg(port));
...
void newbase_hdmi_qs_init(unsigned char port)
{
	...
	unsigned int vs2vs = 0;
	unsigned int last_pixel2vs = 0;
	...
	newbase_hdmi21_rxphy_set_frl_info(port,frl_mode);
	if (frl_mode != MODE_TMDS)
	{
		newbase_hdmi_hd21_lt_fsm_status_init(port); // reset hdmi2.1 fsm status.
		hdmi_rx[port].hdmi_2p1_en = 1;
		SET_FRL_LT_FSM(port, LT_FSM_LTSP_CHECK);

		newbase_hdmi_hd21_qs_init_fw_polarity(port);
	}
	if (lib_hdmi_get_dsc_enable())
	{
	...
	is_interlace = newbase_hdmi_get_is_interlace_reg(port, newbase_rxphy_get_frl_mode(port), HDMI_MS_ONESHOT);
	SET_H_INTERLACE(port, is_interlace);
	memset(hdmi_rx[port].video_t.last_interlace_status, is_interlace, sizeof(hdmi_rx[port].video_t.last_interlace_status));
	...
	hdmi_rx[port].timing_t.single_p_mode_thd = GET_FLOW_CFG(HDMI_FLOW_CFG_HDMI21_LT2, HDMI_FLOW_CFG3_HD21_SETUP_PLL_6G );
	...
	newbase_hdmi_qs_set_irq_status(port,frl_mode);
	_init_hpd_delay_wq();
...
void newbase_hdmi_port_var_init(unsigned char port, unsigned char full_reset)
{
	...
	unsigned char hdmi_2p1_fw_pol_check_cnt_backup =0;
	unsigned char dsc_version_minor_backup =0;
	unsigned char dsc_convert_rgb_backup =0;
	unsigned char dsc_simple_422_backup =0;
	unsigned char dsc_native_420_backup =0;
	unsigned char dsc_native_422_backup =0;
	...
	if (full_reset == 0)//backup
	{
		...
		hdmi_2p1_fw_pol_check_cnt_backup = hdmi_rx[port].hdmi_2p1_fw_pol_check_cnt; //don't check polarity alone

		// DSCD
		dsc_version_minor_backup = GET_H_DSC_VERSION_MINOR(port);
		dsc_convert_rgb_backup = GET_H_DSC_CONVERT_RGB(port);
		dsc_simple_422_backup = GET_H_DSC_SIMPLE_422(port);
		dsc_native_420_backup = GET_H_DSC_NATIVE_420(port);
		dsc_native_422_backup = GET_H_DSC_NATIVE_422(port);
	}
	...
	if (full_reset == 0)//store variable
	{
		...
		hdmi_rx[port].hdmi_2p1_fw_pol_check_cnt = hdmi_2p1_fw_pol_check_cnt_backup; //don't check polarity alone

		// DSCD
		SET_H_DSC_VERSION_MINOR(port, dsc_version_minor_backup);
		SET_H_DSC_CONVERT_RGB(port, dsc_convert_rgb_backup);
		SET_H_DSC_SIMPLE_422(port, dsc_simple_422_backup);
		SET_H_DSC_NATIVE_420(port, dsc_native_420_backup);
		SET_H_DSC_NATIVE_422(port, dsc_native_422_backup);
	}
...
/**
 * @brief Get real-time frame rate with an accuracy of 0.01
 */
unsigned int newbase_hdmi_get_real_framerate(unsigned char port)
{
    return lib_measure_calc_vfreq(hdmi_rx[port].video_t.vs2vs_cnt)/10;
}
		...
		case MAIN_FSM_HDMI_VIDEO_READY:
			...
			if (newbase_hdmi_video_ready(port, p_phy_st->frl_mode)) {
				...
				if(is_QS_hdmi_enable() && qs_init == 0){
					MEASURE_TIMING_T hd21_timing;
					// hdmi21 need to use hd21 measure
					// dsc no need
					if (p_phy_st->frl_mode != MODE_TMDS && lib_hdmi_get_mac_h_sync(port, 0) == 1)
						lib_hd21_measure(port, hdmi_rx[port].timing_t.run_dsc, &hd21_timing);
					else
...
HDMI_CONDITION_CHG_TYPE newbase_hdmi_check_condition_change(unsigned char port)
{
	...
	unsigned int vs_pol = 0, is_invert = 0;
	...
	if (MODE_HDMI == GET_H_MODE(port)) {
		...
		//USER:LewisLee DATE:2023/03/28
		//SKY2885N-2219. to prevent DSC PPS mode color space error
		if((1 == hdmi_rx[port].hdmi_2p1_en) && GET_H_RUN_DSC(port))
		{
			if (GET_H_COLOR_SPACE(port) != newbase_hdmi_get_colorspace_reg(port)) // Software updating is more later than IC register. Need to check VIIDEO READY.
			{
				HDMI_EMG("[condition_change][port: %d][DSC_PPS] VIDEO_FSM:%s, colorspace chg, A:from %d to reg: %d, B:from %d  avi.y: %d, convert_rgb: %d, simple_422: %d, native_422: %d, native_420: %d, version_minor: %d\n",
					...
					newbase_hdmi_get_convert_rgb(port), newbase_hdmi_get_simple_422(port), newbase_hdmi_get_native_422(port), newbase_hdmi_get_native_420(port), newbase_hdmi_get_version_minor(port));
...
//			return HDMI_CONDITION_CHG_RESET_PHY;
	...
	vs_pol = lib_hdmi_hd21_meas_get_vs_polarity(port);

	if(newbase_rxphy_get_frl_mode(port) != MODE_TMDS && (vs_pol)){
		is_invert = lib_hdmi_hd21_fec_get_vs_polar_invert(port);
		HDMI_EMG("[condition_change][port: %d] detect vs_pol = %d, is_invert = %d \n", port, vs_pol, is_invert);
		return HDMI_CONDITION_CHG_RESET_PHY;
	}
	...
	if(GET_H_RUN_DSC(port) != (GET_DSCD_FSM() >= DSCD_FSM_RUN))
	{
		...
		//to prevent Ap layer not get zero timing
		if (MAIN_FSM_HDMI_VIDEO_READY == GET_H_VIDEO_FSM(port))
...
unsigned char newbase_hdmi_setup_video_pll(unsigned char port, unsigned char frl_mode)
{
	...
	// check VPLL is occupied or not
	if (newbase_hdmi_get_hdmi_vpll_ownership()==0)
	{
		HDMI_INFO("FSM_HDMI_SETUP_VEDIO_PLL[%d], VPLL occupied by other module, skip it\n");
		ret_set_pll = TRUE;
		goto pll_complete;
	}
...
void newbase_hdmi_setup_yuv420_after_measure(unsigned char nport, unsigned int pixel_clock, unsigned int single_pixel_mode_clk_thd, MEASURE_TIMING_T* tm )
{
    unsigned char is_yuv420 = (tm->colorspace ==COLOR_YUV420);
    unsigned char fva_factor = tm->fva_factor;

    if(pixel_clock<= single_pixel_mode_clk_thd)
    {
...
#ifndef UT_flag
MEASURE_RESULT_CODE newbase_hdmi_vrr_no_vic_measure(unsigned char port, MEASURE_TIMING_T* tm)
{
    ...
    is_get_vtem = newbase_hdmi_get_vtem_info(0, &emp_vtem);
    ...
    {// no vic, check base timing v freq and v front, measure once!!
        ...
        HDMI_EMG("Measure mode: VRR2 with VTEM Base, port=%d, VTEM_V_freq=%d, VTEM_V_front=%d, Final V freq=%d, V total=%d, Final H total=%d, H act=%d, V act=%d\n",
    ...
    return MEASURE_SUCCESS_VRR_BY_OFMS;
}
#endif // UT_flag

void newbase_hdmi_n_frame_delay(unsigned char port, unsigned char n)
{
	unsigned int temp_v_freq = 0;
	unsigned int temp_wait_ms = 0;

	temp_v_freq = lib_measure_calc_vfreq(hdmi_rx[port].video_t.vs2vs_cnt);
	if (temp_v_freq != 0)
	{
		temp_wait_ms = (1000000/(temp_v_freq));
		if (temp_wait_ms < 1100/24) // one frame no larger than 1000 ms /24 frame *1.1 tolerance
			msleep(temp_wait_ms*n);
	}
}

#ifdef CONFIG_RTK_KDRV_HDMI_ENABLE_DSCD_MEASURE_CHECK

//For 144hz
#define HDMIMS_144HZ_VARIATION_MIN    143000
#define HDMIMS_144HZ_VARIATION_MAX    144500

void newbase_hdmi_dsc_pps_measure(unsigned char port, MEASURE_TIMING_T* tm)
{
	if(tm->run_dsc)
	{// get timing info by DSCD PPS
		...
		if(tm->h_act_len != dsc_ppc_timing->h_act_len)
		{
			...
			else if((tm->h_act_len == 3844) || (tm->h_act_len == 4100) || (tm->h_act_len == 3444))
			{
				HDMI_OFMS_WARN("DSCD correction, set Hact the same with PPS!!. tm->h_act_len=%d\n", tm->h_act_len);
				tm->h_act_len = dsc_ppc_timing->h_act_len;
			}
			else if((4096 == dsc_ppc_timing->h_act_len) && (2160 == dsc_ppc_timing->v_act_len))
			{
				HDMI_OFMS_WARN("DSCD correction, set Hact the same with PPS (4096x2160)!!. tm->h_total=%d, tm->h_act_len=%d\n", tm->h_total, tm->h_act_len);
				tm->h_act_len = dsc_ppc_timing->h_act_len;
			}
			else if((3840 == dsc_ppc_timing->h_act_len) && (2160 == dsc_ppc_timing->v_act_len))
			{
				HDMI_OFMS_WARN("DSCD correction, set Hact the same with PPS (3840x2160)!!. tm->h_total=%d, tm->h_act_len=%d\n", tm->h_total, tm->h_act_len);
				tm->h_act_len = dsc_ppc_timing->h_act_len;
			}
			else
			{
...
MEASURE_RESULT_CODE newbase_hdmi_demode_find_table(unsigned char port, MEASURE_TIMING_T* tm)
{
	const VIC_TIMING_INFO* vic_timing_info_ptr; 	
	...
	if(vic_timing_info_ptr != NULL)
	{
		...
		return MEASURE_SUCCESS_NO_H_FIND_TABLE_BY_DE;
	}
	else
	{
		HDMI_OFMS_WARN("Search VIC Timing Fail, Measure become fail!!\n");
		return MEASURE_FAIL_SEARCH_VIC_TABLE;
	}
}

#endif // HDMI_ENABLE_DSCD_MEASURE_CHECK

void newbase_hdmi_set_video_common_sel(unsigned char port)
{
	unsigned int timing_pixel_clock = 0, single_pixel_mode_clk_thd = 0;
	...
	single_pixel_mode_clk_thd = hdmi_rx[port].timing_t.single_p_mode_thd * 10000;

	newbase_hdmi_setup_yuv420_after_measure(port, timing_pixel_clock, single_pixel_mode_clk_thd, &hdmi_rx[port].timing_t); // YUV420_to_444 Engine config

	if(hdmi_rx[port].timing_t.is_v_drop)
	{//Multi pixel mode for 4k2k240
		lib_hdmi_set_hd20_21_ctrl_pixel_mode(port, DEFAULT_HDMI_MULTI_PIXEL_MODE);// 2/pixel
		newbase_hdmi_hd21_clk_gen(port, GET_H_RUN_DSC(port), 4); // 2/4 pixel
		hdmi_rx[port].timing_t.pixel_mode = TWO_PIXEL_MODE;
		HDMI_WARN("[newbase_hdmi_measure port:%d] 4k2k240 dsc use 4p setting\n", port);
	}
	else if(timing_pixel_clock<= single_pixel_mode_clk_thd)
...
#ifdef CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
extern unsigned int _check_share_dscd_ownership(void);
#endif

unsigned char newbase_hdmi_measure(unsigned char port, unsigned char frl_mode)
{
	unsigned int timing_gen_fifo_status = 0;
	unsigned char measure_result = MEASURE_FAIL_OFMS_NOT_FINISH;
	...
	HDMI_VSI_T vsi_info;
	unsigned char select_vic = 0;
	unsigned char vic_correction_result = MEASURE_FAIL_OFMS_NOT_FINISH;
	bool is_force_VIC_Correction = false;
	...
	if (frl_mode != MODE_TMDS)
	{
		...
		if(newbase_hdmi_hd21_is_data_align_lock(port) == TRUE)
		{
			...
			if (g_offms_out_sel[port] == OFFMS_SOURCE_HDMI21 && lib_hdmi_get_mac_h_sync(port, 0) == 1)
			{
				newbase_hdmi_n_frame_delay(port, 2);
			}
#endif
			...
			lib_hdmi_hd21_clr_measure_last_vs_cnt(port);
		}
		else
	...
	if(GET_H_RUN_VRR(port) != 0)
	{
		...
		while(wait_avi_count)
		{
			...
			if ((is_get_vic != 0) || wait_avi_count==0)
		...
		if((is_get_vic != 0) && (select_vic != 0))
		{         
			HDMI_OFMS_FSM_PRINT("[VRR]Measure mode: VRR1 with VIC, port=%d, Use VIC=%d\n", port, select_vic);
			vic_correction_result = newbase_hdmi_vic_correction(port, &hdmi_rx[port].timing_t, TRUE);
			if(vic_correction_result)
				measure_result = MEASURE_SUCCESS_VRR_BY_VIC;
			else
				measure_result = MEASURE_FAIL_CORRECTION_BY_VIC;
		}
	}

	if(measure_result > MEASURE_SUCCESS_NUM)
	{
		if(is_hv_sync_mode == TRUE)
		{
			...
			if(MEASURE_SUCCESS_NORMAL_BY_HV == measure_result && (GET_H_RUN_VRR(port) != 0))
			{
				measure_result =  newbase_hdmi_vrr_no_vic_measure(port, &hdmi_rx[port].timing_t);
			}
		}
		else
		{// Non VRR, no h sync
			...
			if((GET_H_RUN_DSC(port)) || (GET_DSCD_FSM() >= DSCD_FSM_PPS_MEASURE))
				is_force_VIC_Correction = true;

			if(((true == is_force_VIC_Correction) || (GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_DEMODE_USE_VIC_CORRECTION) == 1)) && (is_get_vic != 0) && (select_vic != 0))
			{//use VIC
				unsigned char vic_correction_result = 0;
//				HDMI_OFMS_FSM_PRINT("[DE_WITH_VIC] Measure mode: No H sync with VIC, port=%d, Use VIC=%d\n", port,  select_vic);
				HDMI_OFMS_FSM_PRINT("[DE_WITH_VIC] Measure mode: No H sync with VIC, port=%d, Use VIC=%d, is_DSC=%d, GET_DSCD_FSM=%d\n", port,  select_vic, GET_H_RUN_DSC(port), GET_DSCD_FSM());
				vic_correction_result = newbase_hdmi_vic_correction(port, &hdmi_rx[port].timing_t, TRUE);
				if(vic_correction_result)
					measure_result = MEASURE_SUCCESS_NO_H_WITH_VIC;
				else
					measure_result = MEASURE_FAIL_CORRECTION_BY_VIC;
			}
			else
			{
				HDMI_OFMS_FSM_PRINT("Measure mode: No H sync without VIC, run DE Mode measure, port=%d, is_DSC=%d\n", port, hdmi_rx[port].timing_t.run_dsc);

				measure_result = newbase_hdmi_dp_measure_video_ex(g_offms_out_sel[port], port, &hdmi_rx[port].timing_t, frl_mode,is_hv_sync_mode);

				if((0 == GET_H_RUN_DSC(port)) && (GET_DSCD_FSM() >= DSCD_FSM_PPS_MEASURE))
				{
					HDMI_OFMS_FSM_PRINT("[newbase_hdmims_measure_video] error, DSCD ReMeasure @ run_dsc=%d, GET_DSCD_FSM=%d, h_act_len=%d, v_act_len=%d\n", GET_H_RUN_DSC(port), GET_DSCD_FSM(), hdmi_rx[port].timing_t.h_act_len, hdmi_rx[port].timing_t.v_act_len);
					measure_result = MEASURE_FAIL_OFMS_NOT_FINISH;
				}
				else
				{
#ifdef CONFIG_RTK_KDRV_HDMI_ENABLE_DSCD_MEASURE_CHECK
					newbase_hdmi_dsc_pps_measure(port, &hdmi_rx[port].timing_t);
					measure_result =  newbase_hdmi_demode_find_table(port, &hdmi_rx[port].timing_t);
#endif
				}
			}
		}
...
#ifdef CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
	if (GET_DSCD_FSM() > DSCD_FSM_WAIT_PPS_READY && _check_share_dscd_ownership()==0)  // if HDMI DSC is running without own DSCD, force vic measure
		use_vic_measure = 1;
#endif
...
#ifdef CONFIG_RTK_KDRV_HDMI_ENABLE_DSCD_MEASURE_CHECK
	if (use_vic_measure == FALSE && (measure_result > MEASURE_SUCCESS_NUM || timing_valid_check == FALSE))
#else
...
#ifdef CONFIG_RTK_KDRV_HDMI_CHECK_DSCD_OWNERSHIP
		if (GET_DSCD_FSM() > DSCD_FSM_WAIT_PPS_READY && _check_share_dscd_ownership()==0)  // if HDMI DSC is running without own DSCD, force vic measure
			hdmi_rx[port].meas_confirm_vic_cnt = GET_FLOW_CFG(HDMI_FLOW_CFG_ERR_DET, HDMI_FLOW_CFG4_VIC_CONFIRM_CNT_THD) + 1; // force vic confirm pass
#endif
	...
	newbase_hdmi_set_video_common_sel(port);
...
unsigned char newbase_hdmi_display_on(unsigned char port, unsigned char frl_mode)
{
	...
	if (g_offms_out_sel[port] == OFFMS_SOURCE_HDMI21 && lib_hdmi_get_mac_h_sync(port, 0) == 1) {
		MEASURE_TIMING_T hd21_timing;
		lib_hd21_measure(port, hdmi_rx[port].timing_t.run_dsc, &hd21_timing);
		if((hdmi_rx[port].timing_t.run_vrr != 0) && (hdmi_rx[port].timing_t.info_vic == 0)){
			//newbase_hdmi_vrr_no_vic_measure(port, &hdmi_rx[port].timing_t);
		}
	}
#endif
...
unsigned char newbase_hdmi_good_timing_check(unsigned char port, unsigned char frl_mode)
{
	...
	if(newbase_hdmi_is_dispsrc(SLR_MAIN_DISPLAY) && (GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_VIDEO_FSM_MONITOR_ENABLE) == TRUE))
	{
		...
		else if((measure_result = newbase_hdmi_dp_measure_video_ex(g_offms_out_sel[port], port, &tm,  frl_mode,is_hv_sync_mode)) < MEASURE_SUCCESS_NUM)
		{
...
unsigned char newbase_hdmi_get_version_minor(unsigned char port)
{
       return GET_H_DSC_VERSION_MINOR(port);
}

unsigned char newbase_hdmi_get_convert_rgb(unsigned char port)
{
       return GET_H_DSC_CONVERT_RGB(port);
}

unsigned char newbase_hdmi_get_simple_422(unsigned char port)
{
       return GET_H_DSC_SIMPLE_422(port);
}

unsigned char newbase_hdmi_get_native_420(unsigned char port)
{
       return GET_H_DSC_NATIVE_420(port);
}

unsigned char newbase_hdmi_get_native_422(unsigned char port)
{
       return GET_H_DSC_NATIVE_422(port);
}
...
void newbase_hdmi_set_hpd(unsigned char nport, unsigned char high)
{
    ...
    else 
    {
        ...
        {
            HDMI_WARN("[newbase_hdmi_set_hpd port:%d] Hdmi_Av_Out_Disable_Vsc_Pre_Action\n", nport);
            //HDMI_AV_OUT_DISABLE_VSC_PRE_ACTION();//vsc to set free run first
...
void newbase_hdmi_error_handler(unsigned char port)
{
    ...
    {
        hdmi_rx[port].hdcp_status.hdcp_enc=1;
    } else {
        hdmi_rx[port].hdcp_status.hdcp_enc=0;
    }
    ...
    if (time_after(jiffies, hdmi_rx[port].err_msg_display_timeout))
    {
        ...
        HDCP_INFO("HDMI[p%d][BCH] HDCP mode:%s, RxStatus(%02x, %02x), hdcp_enc=%d, bch err 1bit=%d, 2bit err (detect,duration,cnt)=(%d, %d, %d), bch_pkt_value=%d, raw_tmds_phy_md_clk=%d, I2c_status=%d, hw_reauth=%d\n",
            ...
            newbase_hdmi_hdcp22_get_hw_reauth(port));
        lib_hdmi_bch_err2_clr(port);
...
void newbase_hdmi_check_cec_state(unsigned char port)
{
	unsigned char real_5v_st = 0xF;
	unsigned char device_num = 0;
	unsigned char index = 0;
	unsigned char cec_debug_en = 0;
	unsigned char current_ch;
	CEC_DEVICE_INFO cec_device_list[CEC_DEVICE_TOTAL_NUM];
	unsigned char j = 0;

	real_5v_st = newbase_hdmi_get_5v_state(port);
	device_num = lib_hdmicec_get_device_num();
	lib_hdmicec_get_device_list(cec_device_list);

	if(hdmi_vfe_get_hdmi_channel(port, &current_ch)!=0)
	{
		return; //invalid port
	}

	if (real_5v_st != hdmicec_HPD_st[port])
	{
		if(real_5v_st == 0x00)
		{
			lib_hdmicec_remove_device(current_ch);
		}
		hdmicec_HPD_st[port] = real_5v_st;
		hdmicec_msg_cnt = g_hdmicec_print_cycle_cnt_thd;
		cec_debug_en = 0;
	}

	//Update OSDNAME to per port array
	for (index=0; index < hdmicec_device_num; index++) 
	{
		if( cec_device_list[index].physical_addr[0] == (current_ch+1))
		{
			for(j=0; j <HDMI_COMP_CEC_OSDNAME_LEGNTH; j++)
			{
				per_port_cec_osdname[port][j] = cec_device_list[index].osd_name[j];
			}
		}
		else
		{
			for(j=0; j <HDMI_COMP_CEC_OSDNAME_LEGNTH; j++)
			{
				per_port_cec_osdname[port][j] = 0;
			}
		}
	}

	if(device_num != hdmicec_device_num)
	{
		hdmicec_device_num = device_num;
		hdmicec_msg_cnt = 0;
		cec_debug_en = 1;
	}

	if(hdmicec_msg_cnt > g_hdmicec_print_cycle_cnt_thd)
	{
		hdmicec_msg_cnt = 0;
		cec_debug_en = 1;
	}
	...
	if(cec_debug_en == 1)
	{
		for (index=0; index < hdmicec_device_num; index++) 
		{
			HDMI_INFO("[CEC][CEC_Device_List] device_num = %d", device_num);
			HDMI_INFO("[CEC][CEC_Device_List]logic_addr = 0x%2x, logic_name = %s, osd_name = %s, physical_addr = [%d.%d.%d.%d]\n" \
				, cec_device_list[index].logic_addr, cec_device_list[index].logic_addr_name, cec_device_list[index].osd_name \
				, cec_device_list[index].physical_addr[0], cec_device_list[index].physical_addr[1], cec_device_list[index].physical_addr[2], cec_device_list[index].physical_addr[3]);
		}
	}
...
unsigned char newbase_hdmi_check_cec_osdname(unsigned char port, unsigned char *cec_osdname)
{
	unsigned char i = 0;

	for (i=0; i<HDMI_COMP_CEC_OSDNAME_LEGNTH; i++)
	{
		if (per_port_cec_osdname[port][i] != cec_osdname[i])
		{
			return 0;
		}
	}
...
unsigned char newbase_hdmi_compatibility_match(unsigned char port)
{
	...
	for(i = 0; i< g_device_compatibility_config_table_size; i ++)
	{
		...
		if(CHECK_FLAG_CEC_OSDNAME == ( CHECK_FLAG_CEC_OSDNAME &g_device_compatibility_config[i].check_characteristic_flag))
		{//With CEC OSDNAME
			unsigned char k = 0;
			for(k = 0; k < g_device_check_cec_table_size; k++)
			{
				if(g_device_check_cec[k].device_id == current_check_device_id)
				{
					if(newbase_hdmi_check_cec_osdname(port, (unsigned char *)g_device_check_cec[k].check_cec_osdname))
					{
						check_point ++;
						HDMI_PRINTF("[COMP] CEC Ponit: Device id [%d], CEC OSDNAME match, OSD name=%s\n", current_check_device_id, g_device_check_cec[k].check_cec_osdname);
					}
...
unsigned char newbase_hdmi_ced_err_thd_check(unsigned char port, unsigned int err_threshold)
{
	if (lib_hdmi_get_fw_debug_bit(DEBUG_23_BITERR_DET))
		return 0;
	if(memcmp(hdmi_rx[port].last_fw_char_error, hdmi_rx[port].fw_char_error, 4) != 0){
		memcpy(hdmi_rx[port].last_fw_char_error, hdmi_rx[port].fw_char_error, 4);
	}else{
		return 0;
	}
...
unsigned char newbase_hdmi_tmds_err_thd_check(unsigned char port, unsigned int err_threshold)
{
	if (lib_hdmi_get_fw_debug_bit(DEBUG_23_BITERR_DET))
		return 0;
	if(memcmp(hdmi_rx[port].last_fw_bit_error, hdmi_rx[port].fw_bit_error, 4) != 0){
		memcpy(hdmi_rx[port].last_fw_bit_error, hdmi_rx[port].fw_bit_error, 4);
	}else{
		return 0;
	}
...
void newbase_hdmi_enable_dpm(void)
{
	unsigned char ch;
	unsigned char port;
	unsigned char dpm_enable = 0;

	// Enable tmds clock wake up function when dpm is enabled before goto standby.
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_DFE, HDMI_FLOW_CFG5_DPM_ENABLE))
	{
		for(ch = 0 ; ch < HDMI_PORT_TOTAL_NUM; ch++)
		{
			...
			if (emcu_get_hdmi_det(ch))
			{
				emcu_set_hdmi_det(port, 1);
				newbase_hdmi_set_dpm_enable(port, 1);
				dpm_enable = 1;
			}
			else
				newbase_hdmi_set_dpm_enable(port, 0);
		}
		if (dpm_enable)
			newbase_hdmi_wakeup_by_tmds_en(1);
	}
}

void newbase_hdmi_disable_dpm(void)
{
	unsigned char ch;
	unsigned char port;

	// Disable tmds clock wake up when dpm is enabled.
	if (GET_FLOW_CFG(HDMI_FLOW_CFG_DFE, HDMI_FLOW_CFG5_DPM_ENABLE))
	{
		for(ch = 0 ; ch < HDMI_PHY_TOTAL_NUM; ch++)
		{
			if (hdmi_vfe_get_hdmi_port(ch, &port)!=0)
				continue;

			if (newbase_hdmi_get_dpm_enable(port))
			{
				newbase_hdmi_wakeup_by_tmds_en(0);
				break;
			}
		}
	}
