...
void vfe_hdmi_sync_qs_status(void)
{
	...
	is_get_vic = newbase_hdmi_get_vic_state(nport, &avi_info, &vsi_info, &select_vic);
	if(is_get_vic){
		newbase_hdmi_set_current_vic(nport, select_vic) ;
	}
	...
	// other ports are be overwritten by the default EDIDs. The overwriting 
	// process is shown in HdmiTable_Init() in vfe_hdmi_qs_drv_init().
	newbase_hdmi_sync_qs_edid(nport);
}
...
{    
	...
	timing_info->pixel_mode = (vfe_hdmi_pixel_mode_e)tm.pixel_mode;
