...
//            inputFrameRate = (unsigned int)( 27000000000ULL / (unsigned long long)inputVsyncCount );    // multiple 1000
            inputFrameRate = (unsigned int)div64_u64(27000000000ULL, (unsigned long long)inputVsyncCount);
        }
        else
        {
...
//            DVtotal = (unsigned int)( ( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 2000ULL ) / ( (unsigned long long)Get_DISP_HORIZONTAL_TOTAL() * (unsigned long long)outputFrameRate ) );
            DVtotal = (unsigned int)div64_u64(( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 2000ULL ), ( (unsigned long long)Get_DISP_HORIZONTAL_TOTAL() * (unsigned long long)outputFrameRate ));
        }
        else
        {
//            DVtotal = (unsigned int)( ( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 4000ULL ) / ( (unsigned long long)Get_DISP_HORIZONTAL_TOTAL() * (unsigned long long)outputFrameRate ) );
            DVtotal = (unsigned int)div64_u64(( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 4000ULL ), ( (unsigned long long)Get_DISP_HORIZONTAL_TOTAL() * (unsigned long long)outputFrameRate ));
        }
...
void framesync_fixlastline_set_precision_vtotal(void)
{
    ...
    verticalTotal = decide_vbe_disp_vtotal(VBE_DVTOTAL_PRECISE,VBE_INPUT_FRAMERATE_INFO,FALSE,0,SLR_MAIN_DISPLAY);
        ...
        else if( multiview_1p_input_in_2p_panel() && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
        {
            ...
            if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) != 0 )
                if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) ) != 0 )
                    uzudtg_vtotal = verticalTotal / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) );
        }
...
void framesync_set_vtotal_by_vodma_freerun_period(void)
{
    ...
    verticalTotal = decide_vbe_disp_vtotal(VBE_DVTOTAL_PRECISE,VBE_INPUT_FRAMERATE_VOPERIOD,FALSE,0,SLR_MAIN_DISPLAY);
    ...
    if(verticalTotal)
    {
        ...
        {
            uzudtg_vtotal = decide_uzudtg_vtotal_by_expand_uzudtg_htotal(VBE_INPUT_FRAMERATE_VOPERIOD,0);
...
void framesync_set_vtotal_by_vodma_freerun_period_isr(void)
{
    ...
    verticalTotal = decide_vbe_disp_vtotal(VBE_DVTOTAL_PRECISE,VBE_INPUT_FRAMERATE_VOPERIOD,TRUE,0,SLR_MAIN_DISPLAY);
...
unsigned int get_HDMI_QMS_VRR_minimum_framerate(void)
{
    unsigned int qms_vrr_minimum_framerate = 24000;

    return qms_vrr_minimum_framerate;
}
...
unsigned int framesync_get_video_line_buffer_delay(void)
{
    ...
    unsigned int vodma_pll = 0;
    unsigned int iv2dv_htotal = 0;
    unsigned int iv2dv_based_clock = 0;
    unsigned int vsync_line_width_ratio = 0;

    // Set iv2dv delay as VO backporch
    ...
    {
        // iv2dv is based on Xtal 27MHz
        // The shortest VO backporch is the case of VO clock = VODMAPLL
        ...
        vo_den_start -= MAX_VO_VSYNC_DELAY_LINE_CNT;
        vodma_pll = drvif_framesync_get_vo_clk();
        ...
        iv2dv_htotal = ppoverlay_dctl_iv2dv_ihs_ctrl_reg.iv2dv_ih_slf_width;
        iv2dv_based_clock = 27000000;
        ...
        else
        {
            ...
            iv2dv_line = (unsigned int)div64_u64(( (unsigned long long)vo_htotal * (unsigned long long)vo_den_start * (unsigned long long)iv2dv_based_clock ), ( (unsigned long long)vodma_pll * (unsigned long long)iv2dv_htotal ));
            if( iv2dv_line >= 3 )
            {
    ...
    {
        // iv2dv is based on VO pixel clock
        // iv2dv Htotal is the same with VO Htotal
        // => iv2dv_line = VO_den_start
        vodma_vodma_v1vgip_vact1_reg.regValue = IoReg_Read32(VODMA_VODMA_V1VGIP_VACT1_reg);
        vo_den_start = vodma_vodma_v1vgip_vact1_reg.v_st;
        vo_den_start -= MAX_VO_VSYNC_DELAY_LINE_CNT;
        ...
        if( iv2dv_line >= 3 )
        {
            ...
            if(get_scaler_qms_mode_flag())  // Mdisp Vsync width                      : n >= 1 ( 1 line Mdisp Vsync width should be expanded to n line based on VO clock. Otherwise set 1 line )
            {
                if( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_VO_SRC_FRAMERATE_1000) % get_HDMI_QMS_VRR_minimum_framerate() )
                    vsync_line_width_ratio = ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_VO_SRC_FRAMERATE_1000) / get_HDMI_QMS_VRR_minimum_framerate() ) + 1;
                else
                    vsync_line_width_ratio = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_VO_SRC_FRAMERATE_1000) / get_HDMI_QMS_VRR_minimum_framerate();
                    
                if( iv2dv_line >= vsync_line_width_ratio )
                {
                    iv2dv_line -= vsync_line_width_ratio;
                }
                else
                {
                    iv2dv_line = 0;
                    rtd_pr_vbe_notice("[%s][WARNING] iv2dv_line < vsync_line_width_ratio ( %d ), set iv2dv_line = 0\n", __FUNCTION__, vsync_line_width_ratio);
                }
            }
            else
            {
        ...
        {
            rtd_pr_vbe_notice("[%s][WARNING] vo_den_start = %d < 3, set iv2dv_line = 0\n", __FUNCTION__, vo_den_start);
            iv2dv_line = 0;
        }
    }
    rtd_pr_vbe_notice("[%s]vodma_pll %d, vo_htotal %d, vo_den_start %d,iv2dv_htotal=%d,  iv2dv= %d\n", __FUNCTION__, vodma_pll, vo_htotal, vo_den_start, iv2dv_htotal, iv2dv_line);

    return iv2dv_line;
}

unsigned int drv_memory_tran_iv2dv_for_vdec_lowdelay(unsigned int ori_iv2dv)
{
	...
	if(PPOVERLAY_FS_IV_DV_Fine_Tuning2_get_iv2dv_xclk_en(IoReg_Read32(PPOVERLAY_FS_IV_DV_Fine_Tuning2_reg)))
	{
		vo_iv2dv_ih_slf_width = pVOInfo->h_total / ( pVOInfo->pixel_clk / 27000000 );
		iv2dv = ori_iv2dv * vo_iv2dv_ih_slf_width / PPOVERLAY_dctl_iv2dv_ihs_ctrl_get_iv2dv_ih_slf_width(IoReg_Read32(PPOVERLAY_dctl_iv2dv_ihs_ctrl_reg));
...
unsigned int framesync_set_iv2dv_delay(unsigned int dispTimingMode)
{
    ...
    // decide clock which iv2dv based on and Htotal which iv2dv referred
    tuning2_reg.regValue = IoReg_Read32(PPOVERLAY_FS_IV_DV_Fine_Tuning2_reg);
    ppoverlay_dctl_iv2dv_ihs_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_dctl_iv2dv_ihs_ctrl_reg);
    if( ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC ) || ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA ) || ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG ) )
    {
        tuning2_reg.iv2dv_xclk_en = 1;  // iv2dv based on Xtal 27MHz
        ppoverlay_dctl_iv2dv_ihs_ctrl_reg.iv2dv_ih_slf_width = Get_DISP_HORIZONTAL_TOTAL() / ( Get_DISPLAY_CLOCK_TYPICAL() / 27000000 );
    }
    else
    {
        tuning2_reg.iv2dv_xclk_en = 0;  // iv2dv based on VO pixel clock
    ...
    iv2dv_diff = fs_iv_dv_fine_tuning5_reg.iv2dv_line;

    if( dispTimingMode == DISPLAY_MODE_RESET_VODMA )
    {
        tuning2_reg.regValue = IoReg_Read32(PPOVERLAY_FS_IV_DV_Fine_Tuning2_reg);
        tuning2_reg.iv2dv_pixel = 20;
        tuning2_reg.iv2dv_pixel2 = 20;  // can't be zero
        IoReg_Write32(PPOVERLAY_FS_IV_DV_Fine_Tuning2_reg, tuning2_reg.regValue);
	...
	//merlin8/8p mdomain one-bin
        ...
        if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic())    // if game mode case, need to use calculation result @Crixus 20170712
        {
            if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC))
            {
                iv2dv_delay = framesync_get_video_line_buffer_delay();
            }
            else
            {
                if( ( Get_DISPLAY_REFRESH_RATE() >= 120 ) && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) && ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DISP_LEN) == _DISP_LEN ) )
                {
                    ...
                    rtd_pr_vbe_emerg("[%s][line:%d][VRR condition] iv2dv delay = %d\n", __FUNCTION__, __LINE__, iv2dv_delay);
                }
#ifndef BUILD_QUICK_SHOW
                else if((zoom_smoothtoggle_gamemode_get_smt_status(SLR_MAIN_DISPLAY) == SMT_SMALL_TO_FULL_SCREEN_DONE ) && get_current_driver_pattern(DRIVER_FREERUN_PATTERN)){
		...
		//merlin8/8p mdomain one-bin
                ...
                else if( ( ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC ) || ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA ) ) && (get_mach_type() == MACH_ARCH_RTK2885P && !nonlibdma_drv_memory_get_vdec_direct_low_latency_mode()) ) // VDEC no need calculate iv2dv delay
                {
                    iv2dv_delay = 2;
                    rtd_pr_vbe_emerg("[%s][line:%d] cp game mode set normal mode iv2dv\n", __FUNCTION__, __LINE__);
                }
                else
                {
				//merlin8/8p mdomain one-bin
				...
				if(dvrif_memory_compression_get_enable(SLR_MAIN_DISPLAY)) {
					...
					iv2dv_delay = nonlibdma_drv_GameMode_iv2dv_delay_compress_margin(iv2dv_delay);
				}
		    ...
		    if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_vdec_direct_low_latency_mode())
                        iv2dv_delay = drv_memory_tran_iv2dv_for_vdec_lowdelay(iv2dv_delay);
                }
            }
        }
        else
		...
		//merlin8/8p mdomain one-bin
                ...
                {
                    if( vbe_disp_get_VRR_device_max_framerate() >= V_FREQ_120000_mOFFSET ) {  // 120Hz
					//merlin8/8p mdomain one-bin
					if(get_mach_type() == MACH_ARCH_RTK2885P) {
						//iv2dv_delay = drv_GameMode_decided_iv2dv_delay_new_mode();
						iv2dv_delay = nonlibdma_drv_GameMode_decided_iv2dv_delay_new_mode();
                        ...
                        iv2dv_delay = ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_LEN) * ( _VRR_MAX_SUPPORT_FRAMERATE - _VRR_MIN_SUPPORT_FRAMERATE ) ) / _VRR_MAX_SUPPORT_FRAMERATE;
                    rtd_pr_vbe_notice("[%s][line:%d][VRR condition] iv2dv delay = %d\n", __FUNCTION__, __LINE__, iv2dv_delay);
                }
        ...
        if(Get_Force_run_fake_scaler_State())
        {
            ...
            iv2dv_delay = framesync_get_video_line_buffer_delay();
            rtd_pr_vbe_notice("[%s][%d]fake case set iv2dv delay = %d\n", __FUNCTION__, __LINE__, iv2dv_delay);
        }
#endif
        fs_iv_dv_fine_tuning5_reg.iv2dv_line = iv2dv_delay;
        fs_iv_dv_fine_tuning5_reg.iv2dv_line_2 = iv2dv_delay;
    ...
    if( iv2dv_diff >= iv2dv_delay )
        iv2dv_diff -= iv2dv_delay;

    rtd_pr_vbe_notice("[%s] iv2dv_delay : %d, timing : %s\n", __FUNCTION__, PPOVERLAY_FS_IV_DV_Fine_Tuning5_get_iv2dv_line(IoReg_Read32(PPOVERLAY_FS_IV_DV_Fine_Tuning5_reg)), ( ( dispTimingMode == 1 ) ? "FSYNC" : ( ( dispTimingMode == 2 ) ? "FLL" : ( ( dispTimingMode == 3 ) ? "ResetVODMA" : "FRC" ) ) ));
	...
	if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC) == TRUE) && drv_get_datafs_gatting_enable_done()
		...
		if(get_mach_type()==MACH_ARCH_RTK2885P) {
			...
			if(_CHANNEL1 == Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL))//If Framesync case, we just check the following flow
			{
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P && (nonlibdma_get_cap_ich12_ibuff_status_reg_ibuff_ovf(0) | nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_ovf(0) | nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_udf(0)))
				{
					...
					if(fwif_color_get_osd_Outer() ==1) /* it's for playback photo patch . rord.tsao */
					{
						...
						if(get_mach_type() == MACH_ARCH_RTK2885P) {
							...
							rtd_pr_vbe_err("A ch1 AAA Frame Sync check error=%d %d %d\n", nonlibdma_get_cap_ich12_ibuff_status_reg_ibuff_ovf(0), nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_ovf(0), nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_udf(0));
						}
						//mdomain_cap_ich12_ibuff_status_reg.regValue = IoReg_Read32(MDOMAIN_CAP_ICH12_IBUFF_STATUS_reg);
						//merlin8/8p mdomain one-bin
						//if(mdomain_cap_ich12_ibuff_status_reg.ibuff_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_ovf | mdomain_cap_ich12_ibuff_status_reg.fsync_udf)
			...
			else
			{
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P && (nonlibdma_get_cap_ich12_ibuff_status_reg_ibuff_ovf(1) | nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_ovf(1) | nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_udf(1)))
				{
					...
					rtd_pr_vbe_err("ch2 Frame Sync check error=%d %d %d\n", nonlibdma_get_cap_ich12_ibuff_status_reg_ibuff_ovf(1), nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_ovf(1), nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_udf(1));
					return FALSE;
				}
			}
#endif
...
static unsigned char framesync_testsync(unsigned short usOffset, unsigned char* retValue)
{
	...
	if(get_mach_type()==MACH_ARCH_RTK2885P) {
		...
		if((TRUE == nonlibdma_get_cap_ich12_ibuff_status_reg_ibuff_ovf(0)) || (TRUE == nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_ovf(0)))
			*retValue = (*retValue) &  _BIT1;

		//if(TRUE == mdomain_cap_ich12_ibuff_status_reg.fsync_udf)
		if(TRUE == nonlibdma_get_cap_ich12_ibuff_status_reg_fsync_udf(0))
...
unsigned int drvif_framesync_get_vo_clk(void)
{
	unsigned int vodma_pll;
	unsigned int nMCode, nDivMode, nNCode, nOCode;
	unsigned int o_exponent, o_result = 1;
	pll_reg_sys_pll_vodma1_RBUS pll_vodma1_reg;
	pll_reg_sys_pll_cpu_RBUS pll_cpu_reg;

	pll_vodma1_reg.regValue= IoReg_Read32(PLL_REG_SYS_PLL_VODMA1_reg);
	pll_cpu_reg.regValue= IoReg_Read32(PLL_REG_SYS_PLL_CPU_reg);

	nMCode = pll_vodma1_reg.pllvodma_m;
	nDivMode = pll_cpu_reg.plls_divmode;
	nNCode = pll_vodma1_reg.pllvodma_prediv;
	...
	// Merlin8 : VODMA_PLL = 27M x [ ( PLLVODMA_M + 3 ) + ( 1 - PLLS_DIVMODE ) ] / [ ( PLLVODMA_PREDIV + 1 ) * ( 2 ^ PLLVODMA_O ) ]
	vodma_pll = 27000000 * ( ( nMCode + 3 ) + ( 1 - nDivMode ) ) / ( ( nNCode + 1 ) * o_result );

	rtd_pr_vbe_emerg("[VO][%d,%d,%d,%d]\n", nMCode, nDivMode, nNCode, nOCode);
	...
	if(Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC)== TRUE){
		...
		if(IoReg_Read32(VGIP_VGIP_VBISLI_reg)&_BIT19){ //for debug only
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				id2dv_delay = nonlibdma_drv_GameMode_decided_iv2dv_delay_old_mode();
			}
		}else{
			//merlin8/8p mdomain one-bin
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				//id2dv_delay = drv_GameMode_decided_iv2dv_delay_new_mode();
				id2dv_delay = nonlibdma_drv_GameMode_decided_iv2dv_delay_new_mode();
			}
		}
		...
		if(dvrif_memory_compression_get_enable(SLR_MAIN_DISPLAY)) {
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				id2dv_delay = nonlibdma_drv_GameMode_iv2dv_delay_compress_margin(id2dv_delay);
			}
		}
...
unsigned int drv_framesync_get_gamemode_iv2dv_linedelay(void)
{
    ...
    if(!get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && (Scaler_InputSrcGetType(SLR_MAIN_DISPLAY) != _SRC_VO))
    {
        if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) && ((hdmi_htotal != 0) && (hdmi_vtotal != 0) && (hdmi_vfreq != 0)))
        {
            ...
            iClk = div64_u64(iClk * hdmi_vtotal * hdmi_vfreq, 1000);
            iHtotal = hdmi_htotal;
        }
        else
        {
            ...
            iClk = div64_u64(iClk * vtotal * vfreq, 1000);
            iHtotal = htotal;
        }
    }
    else
...
//	SET_PANEL_DCLK_DIV(1); //mag2 un-used dclk sel, move to dpll_o
	...
	//merlin8/8p mdomain one-bin
	if(get_mach_type()==MACH_ARCH_RTK2885P) {
		nonlibdma_framesyncGattingDo_mainSubCtrl();
	}

	//merlin8/8p mdomain one-bin
	if(get_mach_type()==MACH_ARCH_RTK2885P) {
		nonlibdma_framesyncGattingDo_mainSubSdnrCxxto64();
	}
...
//	ich2_ibuff_STATUS_RBUS ich2_ibuff_STATUS_reg;
	...
	//merlin8/8p mdomain one-bin
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		nonlibdma_framesyncDo_mainSubCtrl();
	}
...
unsigned char drvif_framesync_hs_bias(void)
{
	...
	//mdomain_cap_ich12_ibuff_status_reg.regValue = IoReg_Read32(MDOMAIN_CAP_ICH12_IBUFF_STATUS_reg);
	nonlibdma_get_mdomain_cap_ich12_ibuff_status_reg();
	nonlibdma_get_mdomain_cap_ich12_ibuff_status_reg();

	if((TRUE == nonlibdma_get_mdomain_cap_ich12_ibuff_status_reg_ibuff_ovf()) || (TRUE == nonlibdma_get_mdomain_cap_ich12_ibuff_status_reg_fsync_ovf()))
	{
		rtd_pr_vbe_debug("drvif_framesync_hs_bias : overflow\n");
		return FALSE;
	}
	else if(TRUE == nonlibdma_get_mdomain_cap_ich12_ibuff_status_reg_fsync_udf())
	{
...
{
    // expand uzudtg htotal if vo active zone > uzudtg active zone ( i.e. vo_active_rec < uzudtg_active_rec )
    sys_reg_sys_dispclksel_RBUS sys_dispclksel_reg;
    unsigned int vo_active_rec = 0;
    unsigned int vodma_pll = drvif_framesync_get_vo_clk();
    unsigned int vo_htotal = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_H_LEN);
    unsigned int vo_vactive = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DI_LEN);
    unsigned int vo_clock = 0;
    unsigned int vo_vtotal = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_LEN);
    unsigned int vo_framerate = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000);
    unsigned int uzudtg_active_rec = 0;
    unsigned int dpll = Get_DISPLAY_CLOCK_TYPICAL();
    unsigned int uzudtg_htotal = Get_DISP_HORIZONTAL_TOTAL();
    unsigned int uzudtg_vactive = Get_DISP_ACT_END_VPOS() - Get_DISP_ACT_STA_VPOS();
    ...
    if( ( vodma_pll != 0 ) && ( vo_htotal != 0 ) && ( vo_vactive != 0 ) && ( dpll != 0 ) && ( uzudtg_htotal != 0 ) && ( uzudtg_vactive != 0 ) && ( fract_b != 0 ) )
    {
        // Only vo_active_rec ( with VODMA_PLL ) < uzudtg_active_rec ( with DPLL ) need to expand uzu_htotal
        if( ( vodma_pll / ( vo_htotal * vo_vactive ) ) < ( dpll / ( uzudtg_htotal * uzudtg_vactive ) ) )
        {
            // vo_clock = vo_htotal * vo_vtotal * vo_framerate ( do not consider vo increasing clock )
            vo_clock = (unsigned int)div64_u64( ( (unsigned long long)vo_htotal * (unsigned long long)vo_vtotal * (unsigned long long)vo_framerate ), 1000ULL );

            // vo_active_rec = vo_clock / ( vo_htotal * vo_vactive ) : 2-digit accuracy
            vo_active_rec = (unsigned int)div64_u64( ( (unsigned long long)vo_clock * 100ULL ), ( (unsigned long long)vo_htotal * (unsigned long long)vo_vactive ) );

            // uzudtg_clock = dpll * fract_a / fract_b
            uzudtg_clock = dpll * fract_a / fract_b;

            // uzudtg_active_rec = uzudtg_clock / ( uzudtg_htotal * uzudtg_vactive ) : 2-digit accuracy
            uzudtg_active_rec = (unsigned int)div64_u64( ( (unsigned long long)uzudtg_clock * 100ULL ), ( (unsigned long long)uzudtg_htotal * (unsigned long long)uzudtg_vactive ) );
...
unsigned int decide_uzudtg_vtotal_by_expand_uzudtg_htotal(unsigned int vbe_input_framerate_source, unsigned char display)
{
    vodma_vodma_pvs_free_RBUS vodma_vodma_pvs_free_reg;
    sys_reg_sys_dispclksel_RBUS sys_dispclksel_reg;
    unsigned int uzudtg_vtotal = 0;
...
//    uzudtg_vtotal = (unsigned int)( ( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * (unsigned long long)fract_a * 2000ULL ) / ( (unsigned long long)fract_b * (unsigned long long)uzudtg_htotal * (unsigned long long)inputFrameRate ) );
    uzudtg_vtotal = (unsigned int)div64_u64(( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * (unsigned long long)fract_a * 2000ULL ), ( (unsigned long long)fract_b * (unsigned long long)uzudtg_htotal * (unsigned long long)inputFrameRate ));
    uzudtg_vtotal = ( ( uzudtg_vtotal + 1 ) >> 1 );

    return uzudtg_vtotal;
}

void decide_uzudtg_v_den_by_expand_uzudtg_htotal(unsigned int uzudtg_vtotal)
{
    ppoverlay_dv_den_start_end_RBUS dv_den_start_end_reg;
    ...
    if( uzudtg_vtotal >= Get_DISP_ACT_END_VPOS() )
    {
        ...
        dv_den_start_end_reg.regValue = IoReg_Read32(PPOVERLAY_DV_DEN_Start_End_reg);
        dv_den_start_end_reg.dv_den_sta = uzudtg_backporch;
        dv_den_start_end_reg.dv_den_end = uzudtg_backporch + Get_DISP_ACT_END_VPOS();
        IoReg_Write32(PPOVERLAY_DV_DEN_Start_End_reg, dv_den_start_end_reg.regValue);
