...
extern unsigned int get_reset_vodma_reserved_line_count(void);
#define UZUDenEndProtectResetVODMAOffset (5)
#define UZUDenEndProtectOffset (2)
...
DOLBY_HDMI_VSIF_T get_HDMI_Dolby_VSIF_mode_by_port(unsigned char port)
{
	...
	flag_dolby_vsif = is_available_dolby_vsif() ? dolby_hdmi_vsif_mode[port] : DOLBY_HDMI_VSIF_DISABLE;

	flag_dolby_h14b_vsif = is_available_dolby_h14b_vsif() ? dolby_h14b_vsif_mode[port] : DOLBY_HDMI_VSIF_DISABLE;
...
DOLBY_HDMI_VSIF_T get_HDMI_Dolby_VSIF_mode(void)
{
	...
	unsigned char port = drvif_hdmi_get_current_display_port();
...
#if 0 //IS_ENABLED(CONFIG_SUPPORT_DOLBY_VISION_DP)
		extern long dolby_adapter_get_dolby_vision_dp_mode(void);
		DPRX_AVI_INFO_T avi_info = { 0 };

		if (dolby_adapter_get_dolby_vision_dp_mode() == DOLBY_DP_SDP) {
			if (drvif_Dprx_GetAviInfoFrame(&avi_info)) {
				if (drvif_Dprx_GetColorSpace() == DP_COLOR_SPACE_RGB
						&& drvif_Dprx_GetColorDepth() == DP_COLOR_DEPTH_8B
						&& avi_info.eRGBQuantizationRange == DP_AVI_RGB_QUANTIZATION_RANGE_FULLRANGE)
					ret = true;
			}
		}
#endif
...
{/*TRUE: memc bypass; FALSE: memc on */
...
#ifndef UT_flag
	else if(Scaler_MEMC_Get_Size_Addr(&memc_addr_aligned, &memc_addr) == 0 && carvedout_en == TRUE){
		flg = TRUE;
	}
#endif // #ifndef UT_flag
    ...
    else if(panel_dlg_get_bypass_memc())
    {
        return TRUE;
    }
#endif
...
#endif // #ifndef UT_flag
	else if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI || Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI){
...
unsigned char decide_2step_uzu_merge_sub(void)  // TRUE : merge sub, FALSE : not merge sub
{
    unsigned char vuzd_flag = 0;

    //merlin8/8p mdomain one-bin
    //if( is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) && ( ( get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_90 ) || ( get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_270 ) ) )
    if((get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) && ( ( get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_90 ) || ( get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_270 ) ) )
        return FALSE;

    if( get_panel_pixel_mode() == PANEL_1_PIXEL )   // converted board does not go main sub merge
        return FALSE;
    ...
    if(force_enable_mdomain_2disp_path())
        return TRUE;
    //merlin8/8p mdomain one-bin
    //else if(is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
    else if(get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
...
unsigned char calculate_scaler_hdmi_hfr_mode(void)
...
{
	scaler_hmdi_hfr_mode = calculate_scaler_hdmi_hfr_mode();
}
...
unsigned char (*get_hdmi_vrr_4k60_mode)(void) = get_hdmi_vrr_4k60_mode_impl;
...
*/
unsigned char is_scaler_input_2p_mode_impl(unsigned char display)
{
	unsigned char ret = FALSE;
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		if(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_SUB_DISPLAY)
			return ret;//return FALSE when display is sub
...
unsigned char (*is_scaler_input_2p_mode)(unsigned char) = is_scaler_input_2p_mode_impl;
...
void set_i3ddma_2p_mode_flag(unsigned char bOnoff)
{
	...
	rtd_pr_vsc_info("4k qms-vrr or 4k120 pass i3ddma enable 2p mode=%d, %d!!! \n", bOnoff, i3ddma_2p_mode_en);
}
...
// If this mode is enable, all video path(including I3DDMA & IMD domain) will use RGB mode.
	...
	if((Scaler_VPQ_check_PC_RGB444() == VIP_HDMI_PC_RGB444) || (Scaler_VPQ_check_PC_RGB444() == VIP_DP_PC_RGB444))
...
static unsigned char vpq_check_pc_yuv422_in_444_out_mode_en = _DISABLE;
void set_vpq_check_pc_yuv422_in_444_out_mode_flag(unsigned char bOnoff)
{
	vpq_check_pc_yuv422_in_444_out_mode_en = bOnoff;
	rtd_pr_vsc_info("[%s] %d set VPQ check PC YUV422 in, 444 out mode enable=%d\n", __FUNCTION__, __LINE__, vpq_check_pc_yuv422_in_444_out_mode_en);
}

unsigned char get_vpq_check_pc_yuv422_in_444_out_mode_flag(void)
{
	return vpq_check_pc_yuv422_in_444_out_mode_en;
}

void decide_vpq_check_pc_yuv422_in_444_out_mode_on_off(void)
{
// Decide Mdomain PQC & PQDC will use 444 out mode enable or not.
// If input is PC mode HDMI YUV422 in case , Mdomain PQC & PQDC will use 444 mode.
	unsigned char val = _DISABLE;

	if(Scaler_VPQ_check_pc_yuv422_in() == TRUE)
		val = _ENABLE; // PC mode HDMI YUV422 in case , Mdomain PQC & PQDC will use 444 out
	else
		val = _DISABLE;

	set_vpq_check_pc_yuv422_in_444_out_mode_flag(val);
}

unsigned char Scaler_VPQ_check_pc_yuv422_in(void)
{
	...
	if (Get_Val_vsc_run_pc_mode())
	{
		...
		if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI)
		{
			rtd_pr_vsc_info("[%s] %d pc_mode=%d,DisplayMode_Src=%d,drvif_Hdmi_GetColorSpace()=%d, dolby_vision_source()=%d,interlace=%d\n",
				__FUNCTION__, __LINE__, Get_Val_vsc_run_pc_mode(), Get_DisplayMode_Src(SLR_MAIN_DISPLAY), drvif_Hdmi_GetColorSpace(), is_dolby_vision_source(SLR_MAIN_DISPLAY),
				Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE));
			if ((drvif_Hdmi_GetColorSpace() == COLOR_YUV422) && (!is_dolby_vision_source(SLR_MAIN_DISPLAY)) && (!Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE)))
			{
				ret_val = TRUE;
			}
			else
		...
		{
#if IS_ENABLED(CONFIG_RTK_DPRX)
			rtd_pr_vsc_info("[%s] %d pc_mode=%d,DisplayMode_Src=%d,drvif_Dprx_GetColorSpace()=%d, dolby_vision_source()=%d,interlace=%d\n",
				__FUNCTION__, __LINE__, Get_Val_vsc_run_pc_mode(), Get_DisplayMode_Src(SLR_MAIN_DISPLAY), drvif_Dprx_GetColorSpace(), is_dolby_vision_source(SLR_MAIN_DISPLAY),
				Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE));
			if ((drvif_Dprx_GetColorSpace() == DP_COLOR_SPACE_YUV422) && (!Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE))	 && (!is_dolby_vision_source(SLR_MAIN_DISPLAY)))
			{
				ret_val = TRUE;
			}
			else
...
//	vgip_chn1_3d_right_act_vsta_length_RBUS vgip_chn1_3d_right_act_vsta_length_reg;
	...
	if(Scaler_InputSrcGetMainChType() == _SRC_HDMI) {
		...
		if((SCALER_DISP_CHANNEL)_CHANNEL1 == (SCALER_DISP_CHANNEL)display)
		{
			...
			if (Get_Val_DP_Vsync_Regen_Pos() == DP_NO_REGEN_VSYNC) {
				if ((polarity>>1)&0x01)
...
void scaler_set_vgip_datapath_uzd2_inl_sel(unsigned int val)
{
	M8P_vgip_data_path_select_sub_RBUS m8p_vgip_data_path_select_sub_reg;
	...
	m8p_vgip_data_path_select_sub_reg.regValue = IoReg_Read32(M8P_VGIP_Data_Path_Select_sub_reg);
	m8p_vgip_data_path_select_sub_reg.uzd2_in_sel_new = val;
	IoReg_Write32(M8P_VGIP_Data_Path_Select_sub_reg, m8p_vgip_data_path_select_sub_reg.regValue);
}
...
void vbe_disp_set_VRR_set_display_vtotal_60Hz(void)
{
	...
	rtd_pr_vbe_notice("[%s] reset vertical_total(%d) after timing sync", __FUNCTION__, vertical_total);
}
		...
		if(DISPLAY_MODE_FRAME_SYNC == timing_mode)
		{
            ...
            if(_ENABLE == Scaler_Get_FreeRun_To_FrameSync_By_HW_Enable())
			{
				if(_DISABLE == display_timing_ctrl1_reg.disp_fsync_en)
				{
                    ...
                    // HDMI/DP 1p VRR input makes D1/D2 framerate align input framerate
                    if( !multiview_1p_input_in_2p_panel() && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
	...
	if(DISPLAY_MODE_FRAME_SYNC == timing_mode)
	{
		if(_ENABLE == Scaler_Get_FreeRun_To_FrameSync_By_HW_Enable())
		{
			if(_DISABLE == display_timing_ctrl1_reg.disp_fsync_en)
			{
				...
				drvif_scaler_set_display_mode_to_framesync_by_hw();
				if(	Scaler_Get_FreeRun_To_FrameSync_By_HW_SpeedUp_ByMcode_HW_CTRL4() )
				{
					before_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg) / 90;
					drvif_scaler_set_display_mode_frc2fsync_check_lock();
					aftertime = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg) / 90;
...
//					display_timing_ctrl1_reg.disp_fix_last_line =0;
	...
	display_timing_ctrl1_reg.regValue = IoReg_Read32(PPOVERLAY_Display_Timing_CTRL1_reg);
	if((DISPLAY_MODE_FRAME_SYNC == timing_mode) && (1 == display_timing_ctrl1_reg.disp_frc_fsync)){
		drvif_scaler_enable_abnormal_dvs_protect(1, 1);
	}else{
...
void drvif_scaler_set_frc2fsync_vtotal_protect_off(void)
{
    ...
    unsigned int temp_uzd_hactive = 0;
    unsigned int temp_expand_uzudtg_htotal = 0;
    unsigned int temp_shrink_uzudtg_vtotal = 0;
    ...
    if( vbe_disp_get_adaptivestream_fs_mode()
        ...
        DVtotal = decide_vbe_disp_vtotal(VBE_DVTOTAL_PRECISE,VBE_INPUT_FRAMERATE_INFO,FALSE,0,SLR_MAIN_DISPLAY);
    else
    ...
    if( multiview_1p_input_in_2p_panel() && multiview_1p_input_decide_D1_timing(&temp_uzd_hactive, &temp_expand_uzudtg_htotal, &temp_shrink_uzudtg_vtotal) )
    {
        uzudtg_vtotal = temp_shrink_uzudtg_vtotal;
    ...
    else if( multiview_1p_input_in_2p_panel() && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
    {
        // HDMI/DP 1p VRR input makes D1/D2 framerate align input framerate
        if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) != 0 )
            if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) ) != 0 )
                uzudtg_vtotal = DVtotal / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) );
    }
    else if(data_fsync_go_expand_uzudtg_htotal())
    {
        uzudtg_vtotal = decide_uzudtg_vtotal_by_expand_uzudtg_htotal(VBE_INPUT_FRAMERATE_INFO,0);
...
void drvif_scaler_enable_frc2fsync_HW_speedup_vtotal(UINT8 bEnable)
{
    ...
    if(bEnable)
    {
        ...
        // HDMI/DP 1p LFR VRR input makes D1/D2 framerate align input framerate
        if( multiview_1p_input_in_2p_panel() && !jude_input_timing_is_hfr(SLR_MAIN_DISPLAY)  && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
            speedUp_vtotal *= ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) );

        if( ( get_panel_res() == PANEL_RES_FHD ) || ( get_panel_res() == PANEL_RES_HD ) )
            speedUp_vtotal /= 2;
        else if( ( Get_DISP_ACT_END_VPOS() -  Get_DISP_ACT_STA_VPOS() ) == 1080 )   // 4k1k panel
            speedUp_vtotal /= 2;
...
void drvif_scaler_set_display_mode_frc2fsync_by_hw_speedup_timeout_handler(void){
	...
	if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI){
#ifndef BUILD_QUICK_SHOW       
		vfe_hdmi_drv_reset_hdmi_timing_ready(HAL_VFE_HDMI_CURRENT_PORT);
		drvif_Hdmi_set_infoframe_thread_stop(FALSE);
		down(get_hdmi_detectsemaphore());
		if (get_HDMI_Global_Status() == SRC_CONNECT_DONE) {
			vfe_hdmi_drv_handle_on_line_measure_error(HAL_VFE_HDMI_CURRENT_PORT, 0);
...
unsigned int get_min_dv_den_cnt_target_by_uzu_den_end(unsigned int offset)
{
    ...
    unsigned int uzuDenEnd = PPOVERLAY_DV_DEN_Start_End_get_dv_den_end(IoReg_Read32(PPOVERLAY_DV_DEN_Start_End_reg));
    unsigned int dClk = Get_DISPLAY_CLOCK_TYPICAL();
    unsigned int min_frame_sync_den_target = 0;

    unsigned int uzuDenEndTarget = (uzuDenEnd + offset);
    if(uzuDenEndTarget > uzuVTotal)
    {
        uzuDenEndTarget = uzuDenEnd + UZUDenEndProtectOffset;
        if(uzuDenEndTarget > uzuVTotal)
        {
            uzuDenEndTarget = uzuDenEnd;
        }
        rtd_pr_vsc_notice("[%s]uzuDenEndTarget over uzuVTotal, use pretected uzuDenEndTarget(%d)", __FUNCTION__, uzuDenEndTarget);
    }

    min_frame_sync_den_target = ((((uzuVTotal*(mDtg_rem-1))+(uzuDenEndTarget))*uzuHTotal)/dHTotal*b_2pixel_mode/dclk_frac_a*dclk_frac_b)-(dVtotal*(mDtg_mul-1));
    rtd_pr_vsc_notice("[%s]min_frame_sync_den_target=%d,frac_a=%d,frac_b=%d,2px=%d,mDtg_rem=%d,mDtg_mul=%d,dVtotal=%d,dHTotal=%d,uzuDenEndTarget=%d,uzuDenEnd=%d,offset=%d,uzuVTotal=%d,uzuHTotal=%d,uzuClk=%d,dClk=%d\n",
        __FUNCTION__, min_frame_sync_den_target, dclk_frac_a, dclk_frac_b, b_2pixel_mode, mDtg_rem, mDtg_mul, dVtotal, dHTotal, uzuDenEndTarget, uzuDenEnd, offset, uzuVTotal, uzuHTotal, uzuClk, dClk);

    return min_frame_sync_den_target;
}

void check_min_dv_den_cnt_target_by_uzu_den_end(unsigned int offset)
{
    unsigned int cur_frame_sync_den_target = 0;
    unsigned int min_frame_sync_den_target = 0;
    ...
    cur_frame_sync_den_target = ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target;

    min_frame_sync_den_target = get_min_dv_den_cnt_target_by_uzu_den_end(offset);

    if(cur_frame_sync_den_target < min_frame_sync_den_target)
    {
        unsigned int dVtotal = PPOVERLAY_DV_total_get_dv_total(IoReg_Read32(PPOVERLAY_DV_total_reg))+1;
        if(min_frame_sync_den_target < dVtotal)
        {
            ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target = min_frame_sync_den_target;
            IoReg_Write32(PPOVERLAY_frc_to_frame_sync_den_end_ctrl_reg,ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.regValue);
            rtd_pr_vbe_notice("[%s]frame_sync_den_target(%d) < min_frame_sync_den_target(%d), use min_frame_sync_den_target", __FUNCTION__, cur_frame_sync_den_target, min_frame_sync_den_target);
        }
        else
        {
            rtd_pr_vbe_notice("[%s]Invalid min_frame_sync_den_target=%d, dVtotal=%d" ,min_frame_sync_den_target ,dVtotal);
        }
    }
}
...
void drvif_scaler_set_display_mode_frc2fsync_by_hw_speedup_Mcode_HW_CTRL_4(void)
{
	...
	iv2dv_diff = framesync_set_iv2dv_delay(DISPLAY_MODE_FRAME_SYNC);
    //iv2dv delay change more than 10 line, need wait more 1 frame for iv2dv delay apply successfull
    if(iv2dv_diff >= 10)
    {
    ...
    // HDMI/DP 1p LFR VRR input makes D1/D2 framerate align input framerate
    if( multiview_1p_input_in_2p_panel() && !jude_input_timing_is_hfr(SLR_MAIN_DISPLAY)  && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
        ppoverlay_frc_to_frame_sync_den_end_ctrl_reg.dv_den_cnt_target *= ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) );
    ...
    {
        check_min_dv_den_cnt_target_by_uzu_den_end(UZUDenEndProtectOffset);
    }
...
void drvif_scaler_set_Vodma_reset_target_line_count(void)
{
	...
	*/
    ...
    if(data_fsync_go_expand_uzudtg_htotal())
    {
        check_min_dv_den_cnt_target_by_uzu_den_end(UZUDenEndProtectResetVODMAOffset + (get_reset_vodma_reserved_line_count()+1) + offset);
        frame_sync_den_target = PPOVERLAY_frc_to_frame_sync_den_end_ctrl_get_dv_den_cnt_target(IoReg_Read32(PPOVERLAY_frc_to_frame_sync_den_end_ctrl_reg));
    }
...
int drvif_scaler_get_abnormal_dvs_protect(unsigned int *dvs_long, unsigned int *dvs_short)
{
    ...
    switch(get_panel_res())
    {
        case PANEL_RES_FHD:
            *dvs_long = DVS_2K_LONG_LINE_THRESHOLD;
            *dvs_short = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN() - 1;
            break;

        case PANEL_RES_HD:
            *dvs_long = DVS_HD_LONG_LINE_THRESHOLD;
            *dvs_short = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN() - 1;
            break;

        case PANEL_RES_WQHD:
            *dvs_long = DVS_WQHD_LONG_LINE_THRESHOLD;
            *dvs_short = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN() - 1;
            break;
        ...
        default:
            if(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
            {
                if(Get_DISPLAY_PANEL_OLED_TYPE())
                {
                    *dvs_long = DVS_4k_OLED_VRR_LONG_LINE_THRESHOLD;
                }
                else
                {
                    *dvs_long = DVS_4k_LED_VRR_LONG_LINE_THRESHOLD;
                }
            }
            else
            {
                *dvs_long = DVS_4K_LONG_LINE_THRESHOLD;
            }
    ...
    if(*dvs_short == 0 || *dvs_long == 0)
    {
        rtd_pr_vbe_err("[%s %d][ERROR]dvs_long or dvs short value error\n", __FUNCTION__, __LINE__);
        return -1;
    }

    rtd_pr_vbe_notice("[%s %d]*dvs_short:%d, *dvs_long:%d\n", __FUNCTION__, __LINE__, *dvs_short, *dvs_long);
...
void drvif_scaler_enable_abnormal_dvs_protect(unsigned char bEnable_long, unsigned char bEnable_short)
{
    ...
    ppoverlay_dvs_abnormal_control_reg.dvs_long_line_th = dvs_long;
    ppoverlay_dvs_abnormal_control_reg.dvs_short_line_th = dvs_short;
	...
	rtd_pr_vbe_notice("### func:%s long:%d short:%d value:0x:%x ###\r\n", __FUNCTION__, bEnable_long, bEnable_short, (unsigned int)ppoverlay_dvs_abnormal_control_reg.regValue);
}
...
void drvif_scaler_handle_abnormal_dvs_event(void)
{
    ...
    if(Get_fake_scaler_connect() && Get_fake_scaler_on_going() && Get_Force_run_fake_scaler_State()){
        ...
        Set_Force_run_fake_scaler_State(0,0,0);
...
{//if true: main m domain use 2k size(k8 carveout size). using decide scaling down or not
	...
	//merlin8/8p mdomain one-bin
        ...
        if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_vdec_data_frc_need_borrow_cma_buffer(display))
        {
        	ret = false;
        }
        else{
        ...
        // 3. run i3:
            if (get_vsc_src_is_hdmi_or_dp()) {
				...
				} else if ((((scaler_vsc_get_gamemode_go_datafrc_mode() == TRUE) && (is_game_mode_set_line_buffer() == false)&& ((get_mach_type() == MACH_ARCH_RTK2885P && Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > nonlibdma_get_m_domain_width_carveout_2k()))
				//merlin8/8p mdomain one-bin
				//&& (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > M_DOMAIN_LEN_CARVEOUT_2K))
				&& (get_mach_type() == MACH_ARCH_RTK2885P && Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > nonlibdma_get_m_domain_len_carveout_2k()))
				...
				|| ((get_mach_type() == MACH_ARCH_RTK2885P && Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > nonlibdma_get_m_domain_width_carveout_2k())
				//merlin8/8p mdomain one-bin
				//&& (Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > M_DOMAIN_LEN_CARVEOUT_2K)
				&& (get_mach_type() == MACH_ARCH_RTK2885P && Scaler_DispGetInputInfo(SLR_INPUT_IPV_ACT_LEN) > nonlibdma_get_m_domain_len_carveout_2k())
...
void calculate_carveout_size(unsigned char display)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		if (Get_rotate_function(display)){
			...
			carveout_width = nonlibdma_get_m_domain_width_business_display();
			carveout_len = nonlibdma_get_m_domain_len_business_display();
		} else {
			//carveout_width = M_DOMAIN_WIDTH_CARVEOUT;
			//carveout_len = M_DOMAIN_LEN_CARVEOUT;
			carveout_width = nonlibdma_get_m_domain_width_carveout();
			carveout_len = nonlibdma_get_m_domain_len_carveout();
		}
		...
		max_width = (display == SLR_MAIN_DISPLAY) ? carveout_width : nonlibdma_get_m_domain_width_2k();
		max_len   = (display == SLR_MAIN_DISPLAY) ? carveout_len   : nonlibdma_get_m_domain_len_2k();
	}
}
...
//	unsigned char ucTemp = 0;
...
#endif        
	{
...
#endif    
			{
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(display))
                {
                    temp = min((unsigned int)Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN) / 2, (unsigned int)((Scaler_DispGetInputInfo(SLR_INPUT_SDNRV_ACT_LEN))/2));	//4k1k
...
//			Scaler_DispSetInputInfo(SLR_INPUT_DI_WID,Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID));
		...
		//merlin8/8p mdomain one-bin
                //if (is_scaler_input_2p_mode(display)){
                if (get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(display)) {
                    cap_len = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_SDNRV_ACT_LEN) / 2;
                } else {
...
//frank@02012013 Change below code to solve set sub display window BUG --
...
#endif
	{
		if (force_enable_mdomain_2disp_path()) {
            ...
            #endif
            //merlin8/8p mdomain one-bin
            //&& ((is_caveout_memory_buffer_source(display)) ||((Scaler_InputSrcGetType(display)==_SRC_VO) && is_scaler_input_2p_mode(display)))) ||(display == SLR_SUB_DISPLAY))
            && ((is_caveout_memory_buffer_source(display)) ||((Scaler_InputSrcGetType(display)==_SRC_VO) && (get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(display))))) ||(display == SLR_SUB_DISPLAY))
        ...
        {//decide mdoamin size to caveout size
		    ...
		    //merlin8/8p mdomain one-bin
                    ...
                    if(get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(display))
                    {
                        Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_LEN, Get_Val_Max_Len()/2);
                        Scaler_DispSetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_MEM_ACT_LEN, Get_Val_Max_Len()/2);
                    }
                    else
                    {
...
#endif
		//merlin8/8p mdomain one-bin
		//if ((display==SLR_MAIN_DISPLAY) && (is_scaler_input_2p_mode(display))) {
		if ((display==SLR_MAIN_DISPLAY) && (get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(display))) {
...
#endif
	//merlin8/8p mdomain one-bin
	//else if ((display==SLR_MAIN_DISPLAY) && (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))) {
	else if ((display==SLR_MAIN_DISPLAY) && (get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))) {
	...
	//if (Get_PANEL_3D_DISPLAY_SUPPORT_ENABLE())
	{
		...
		printf2("panel_type=%d,panel_rate=%d,panel_pixel=%d,input_2p=%d\n", Get_DISPLAY_PANEL_TYPE(), Get_DISPLAY_REFRESH_RATE(), get_panel_pixel_mode(), is_scaler_input_2p_mode(display));
		...
		rtd_pr_vbe_emerg("Panel_type = %d, panel_rate = %d, panel_pixel = %d, input_2p = %d\n", Get_DISPLAY_PANEL_TYPE(), Get_DISPLAY_REFRESH_RATE(), get_panel_pixel_mode(), is_scaler_input_2p_mode(display));
...
//		}
	...
	if ((TRUE == me_bypass) && (get_vt_EnableFRCMode() == FALSE)){
		...
		(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic()==TRUE)){
...
//		drvif_scalerdisplay_set_freeze(0, FALSE);
	...
	if(get_new_game_mode_vdec_memc_bypass_condition() == TRUE){
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic()==TRUE){
...
//	tRPCShareMem->ulAddr[EMF_COEF_MEM_IDX] = g_atShareMemInfo[SCALERIOC_SETEMF].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[DOLBY_VISION_L1L4_STATS_IDX] = g_atShareMemInfo[DOLBY_VISION_L1L4_STATS].ulOffset + g_ulPhyAddr + MEM_OFFSET;
    ...
    tRPCShareMem->ulAddr[FILM_CTRL_MEMC_IDX] = g_atShareMemInfo[SCALERIOC_FILM_CTRL_MEMC].ulOffset + g_ulPhyAddr + MEM_OFFSET;
    tRPCShareMem->ulAddr[SMT_STATUS_UNDER_GAME_MODE_IDX] = g_atShareMemInfo[SCALERIOC_SMOOTH_TOGGLE_STATUS_UNDER_GAME_MODE].ulOffset + g_ulPhyAddr + MEM_OFFSET;
    ...
    tRPCShareMem->ulAddr[ECO_MODE_IDX] = g_atShareMemInfo[SCALERIOC_ECO_MODE].ulOffset + g_ulPhyAddr + MEM_OFFSET;
	...
	tRPCShareMem->ulAddr[DOLBY_VISION_L1L4_STATS_IDX] = htonl(tRPCShareMem->ulAddr[DOLBY_VISION_L1L4_STATS_IDX]);
    ...
    tRPCShareMem->ulAddr[FILM_CTRL_MEMC_IDX] = htonl(tRPCShareMem->ulAddr[FILM_CTRL_MEMC_IDX]);
    ...
    tRPCShareMem->ulAddr[SMT_STATUS_UNDER_GAME_MODE_IDX] = htonl(tRPCShareMem->ulAddr[SMT_STATUS_UNDER_GAME_MODE_IDX]);
    tRPCShareMem->ulAddr[SCLAERIOC_SET_VT_DC2H_BUF_STATUS_IDX] = htonl(tRPCShareMem->ulAddr[SCLAERIOC_SET_VT_DC2H_BUF_STATUS_IDX]);
    tRPCShareMem->ulAddr[ECO_MODE_IDX] = htonl(tRPCShareMem->ulAddr[ECO_MODE_IDX]);
...
//#if RUN_MERLIN_MEMC_ENABLE
		...
		SetRPCShareMem(DOLBY_VISION_L1L4_STATS, sizeof(struct dolby_vision_l1l4_stats));
...
//#endif
		...
		SetRPCShareMem(SCALERIOC_FILM_CTRL_MEMC, sizeof(unsigned int));
        ...
        SetRPCShareMem(SCALERIOC_SMOOTH_TOGGLE_STATUS_UNDER_GAME_MODE, sizeof(SMOOTH_TOGGLE_STATUS_UNDER_GAME_MODE));
        SetRPCShareMem(SCLAERIOC_SET_VT_DC2H_BUF_STATUS, sizeof(VT_BUF_STATUS_T));
        SetRPCShareMem(SCALERIOC_ECO_MODE, sizeof(unsigned int));
        SetRPCShareMem(FORCE_RUN_FAKE_SCALER_STATE, sizeof(FAKE_SHAREMEM_PARAM));
		...
		if(Get_PANEL_3D_LINE_ALTERNATIVE_SUPPORT_ENABLE()){ // PR 3D
			if(modestate_I3DDMA_get_In3dMode()){
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P) {
					...
					nonlibdma_drvif_memory_set_3D_LR_swap(message,Scaler_Disp3dGetInfo((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_DISP_3D_3DTYPE));
				}
				return isHandled;
			}
		}
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_drvif_memory_set_3D_LR_swap_for_SG_frc_in_mDomain(FALSE);
			}
			...
			if(modestate_get_vo3d_in_framePacking_FRC_mode()){
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P) {
					...
					nonlibdma_drvif_memory_set_3D_LR_swap_for_SG_frc_in_mDomain(Scaler_Get_CurVoInfo_plane() == VO_VIDEO_PLANE_V1? FALSE: TRUE);
#else
					//drvif_memory_set_MVC_FRC_LR_swap(TRUE);
					nonlibdma_drvif_memory_set_MVC_FRC_LR_swap(TRUE);
#endif
				}
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P) {
					...
					nonlibdma_drvif_memory_set_MVC_FRC_LR_swap(FALSE);
				}
			}else{
			;//flow_message_push(_UI2FL_3D_FORCE_LR_CHANGE);
			}
...
void VI_close(void)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nonlibdma_set_vi_inten_reg_interrupt(0, 0);
		...
		nonlibdma_set_vi_intst_reg_status(0, 1);
	}
}
...
unsigned char get_rtnr_hflip_flag(void)
{
    ...
    if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE) || (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)))
    {
        return FALSE;
    }
...
{
    if (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) == FALSE)
    {
        return FALSE;
    }
...
void modestate_decide_dtg_m_mode(void)
{
    ...
    if( (Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || ( Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI ) || ( Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI ) )
    {
        Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, Get_DISPLAY_REFRESH_RATE());
        if( Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI )
            rtd_pr_vbe_notice("[LVDS_TO_HDMI] output framerate = %d\n", Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ));
        else
    ...
    if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_24000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_24500 ) )                   // 24Hz
    {
        if( Get_DISPLAY_REFRESH_RATE() == 60 )
        {
            ...
            else
            {
                ...
                {
                    if( ( vbe_disp_get_adaptivestream_fs_mode() && ( vbe_disp_get_adaptivestream_force_framerate() == 0 ) ) || get_scaler_qms_mode_flag() )
                        Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 48);
                    else
                        Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 60);
                }
    ...
    else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_144000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_144000_pOFFSET ) )    // 144Hz
    {
        if( Get_DISPLAY_REFRESH_RATE() == 144 )
            ...
            Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 144);
        else if( Get_DISPLAY_REFRESH_RATE() == 288 )
    ...
    else                                                                                                                                                                     // others ( unknown input framerate => viewed as input 60Hz )
    {
        if( Get_DISPLAY_REFRESH_RATE() >= 240 )
            Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 240);
        else if( Get_DISPLAY_REFRESH_RATE() >= 180 )
            Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 180);
        else if( Get_DISPLAY_REFRESH_RATE() >= 120 )
            Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 120);
        else
            Scaler_DispSetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ, 60);
...
void modestate_decide_dtg_m_mode_scaler_set_cinema_mode(void)
{
	...
	if( ( ( Get_DISPLAY_REFRESH_RATE() == 60 ) || ( Get_DISPLAY_REFRESH_RATE() == 120 ) ) && ( Scaler_get_realcinema_mode_condition() ) )
...
void modestate_decide_dtg_m_modeByDisp(unsigned char _channel)
{
    ...
    if( ( Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI ) || ( Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI ) || ( Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI ) )
    {
        Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, Get_DISPLAY_REFRESH_RATE());
        if( Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI )
    ...
    else                                                                                                                                                                                                     // others ( unknown input framerate => viewed as input 60Hz )
    {
        if( Get_DISPLAY_REFRESH_RATE() >= 240 )
            ...
            Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 180);
        else if( Get_DISPLAY_REFRESH_RATE() >= 120 )
            Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 120);
        else
            Scaler_DispSetInputInfoByDisp(channel, SLR_INPUT_DTG_MASTER_V_FREQ, 60);
...
unsigned char modestate_get_mdomain_frc_flag(unsigned char _channel)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		rtd_pr_scaler_memory_emerg("modestate_get_mdomain_frc_flag game mode=%x, game mode flag=%x, game_mode_dynamic=%x\n", drv_memory_get_game_mode(), drv_memory_get_game_mode_flag(), nonlibdma_drv_memory_get_low_delay_game_mode_dynamic());
	}
	...
	if ((Get_DisplayMode_Src(_channel) != VSC_INPUTSRC_HDMI) || (TRUE == drv_memory_get_game_mode()) || (get_mach_type() == MACH_ARCH_RTK2885P && TRUE == nonlibdma_drv_memory_get_low_delay_game_mode_dynamic())
...
unsigned char modestate_decide_mdomain_frc_fractional(unsigned char _channel)
{
	...
	//	|| TRUE == drv_memory_get_low_delay_game_mode_dynamic())
		|| (get_mach_type() == MACH_ARCH_RTK2885P && TRUE == nonlibdma_drv_memory_get_low_delay_game_mode_dynamic()))
	{
...
void modestate_decide_fractional_framesync(void)
{
	...
	//if (get_mach_type() == MACH_ARCH_RTK2885P && FALSE == nonlibdma_drv_memory_get_game_mode())
	{
...
unsigned char modestate_set_display_timing_genByDisp(unsigned char _channel)
{
    ...
    /*setup dtg master.*/
    if( Get_TIMING_PROTECT_PANEL_TYPE() && !vbe_disp_decide_uzuclock_mapping_vo() )
    {
        dv_total_reg.regValue = IoReg_Read32(PPOVERLAY_DV_total_reg);
        uzudtg_dv_total_reg.regValue = IoReg_Read32(PPOVERLAY_uzudtg_DV_TOTAL_reg);
...
//                rtd_pr_vbe_notice("+m_adjust_vtotal=%d, b8028014=%x,b802850c=%x (loop %d\n", m_adjust_vtotal, IoReg_Read32(0xb8028014),IoReg_Read32(0xb8028504), m_loop_timeout);
    ...
    else
    {
        ...
        else if( multiview_1p_input_in_2p_panel() && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
        {
            ...
            if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) != 0 )
                if( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) ) != 0 )
                    uzudtg_vtotal = vertical_total / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_DTG_MASTER_V_FREQ) / ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) / 1000 ) );
        }
        else if(data_fsync_go_expand_uzudtg_htotal())
        {
    ...
    if( multiview_1p_input_in_2p_panel() && multiview_1p_input_decide_D1_timing(&temp_uzd_hactive, &temp_expand_uzudtg_htotal, &temp_shrink_uzudtg_vtotal) )
    {
        uzudtg_htotal = temp_expand_uzudtg_htotal;
    ...
    dv_den_start_end_reg.dv_den_end = Get_DISP_DEN_END_VPOS();
    IoReg_Write32(PPOVERLAY_DV_DEN_Start_End_reg, dv_den_start_end_reg.regValue);
    if((uzudtg_vtotal < Get_DISP_VERTICAL_TOTAL()) && (uzudtg_htotal != Get_DISP_HORIZONTAL_TOTAL()))
    {
    ...
    if( ( Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI_MEMC_FREERUN ) || ( get_vo_camera_flow_flag(Get_DisplayMode_Port(SLR_MAIN_DISPLAY)) == 1 ) || ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA ) )
        drvif_scaler_memc_set_display_mode_frc2fsync_by_hw_speedup(0);  // FRC
    else
        drvif_scaler_memc_set_display_mode_frc2fsync_by_hw_speedup(1);  // FRC2FS
    ...
    if(get_mach_type() == MACH_ARCH_RTK2885PP) // ml8p
    {
...
//			UINT32 i = 0, j=0, tmp=0;
			...
			if((Scaler_InputSrcGetMainChType() == _SRC_VO) && (Scaler_Get_CurVoInfo_plane() == VO_VIDEO_PLANE_V2)){
				i3ddma_timing.color = (I3DDMA_COLOR_SPACE_T)(i3dFmt == I3DDMA_SENSIO? I3DDMA_COLOR_YUV422: I3DDMA_COLOR_YUV444);
				...
				{
					switch (Scaler_DispGetInputInfo(SLR_INPUT_COLOR_SPACE))
					{
			...
			i3ddma_timing.v_active_space2 = 0; //hdmi.tx_timing.v_active_space2; //get from I3DDMA_MeasureActiveSpace
                        ...
                        if( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI )
                          i3ddma_timing.v_freq_1000 = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW);  // hdmi.tx_timing.v_freq ( 0.001Hz )
                        else
                          i3ddma_timing.v_freq_1000 = Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000);
			...
			rtd_pr_vbe_debug("################# 111 i3ddma_timing.v_freq_1000 : %d\n", i3ddma_timing.v_freq_1000);
...
void set_i3ddma_compression_mode(void)
{
	...
	//if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_game_mode() == _ENABLE) {
...
unsigned char drivf_i3ddma_config(unsigned char display, unsigned char src)
{
	...
	if((src != VGIP_SRC_TMDS && src != VGIP_SRC_HDMI2 && src != VGIP_SRC_ATV && src != VGIP_SRC_HDR1)
...
// timing dectect done
	...
	else
	{
		...
		if(get_vsc_run_pc_mode() && get_vsc_src_is_hdmi_or_dp()){
	...
	if((src == VGIP_SRC_HDR1) && (get_nn_force_i3ddma_enable()))
	{
		if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_JPEG) ||
			(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA) ||
			((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && (get_OTT_HDR_mode() == HDR_DOLBY_COMPOSER)))
	...
	{
		switch (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_COLOR_SPACE))
		{
			case SCALER_INPUT_COLOR_RGB:
				i3ddma_timing.color = I3DDMA_COLOR_RGB;
				break;
    ...
    if( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI )
      i3ddma_timing.v_freq_1000 = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVFREQ_NEW);  // hdmi.tx_timing.v_freq ( 0.001 Hz )
    else
      i3ddma_timing.v_freq_1000 = Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000);
	...
	else if(get_nn_force_i3ddma_enable())
	{
		drvif_I3DDMA_Set_Input_Info_Vaule(I3DDMA_INFO_TARGET_DEPTH, I3DDMA_COLOR_DEPTH_8B);
		drvif_I3DDMA_Set_Input_Info_Vaule(I3DDMA_INFO_TARGET_COLOR, I3DDMA_COLOR_YUV444);
		rtd_pr_vbe_notice("\r\n#### func:%s vdec force (yuv444)#######\r\n", __FUNCTION__);
	}
	...
	if(((Get_TIMING_PROTECT_PANEL_TYPE()==TRUE)||(Get_VO_Tracking_D_PANEL_TYPE()==TRUE))
...
char drivf_Sub_I2rnd_SetupVODMA(void) {
	...
	config_data->framerate =  Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_V_FREQ_1000);//timing->v_freq;
...
void drvif_scaler_sub_mdomain_switch_irq(unsigned char enable)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nonlibdma_set_cap_ie_wd_ctrl_reg_in_cap_last_wr_ie(1, enable);
	}
}
#endif
...
{//only for vtop mode setting for scaler
	...
	switch(hdrtype)
	{
		...
		case HDR_DM_SDR_MAX_RGB:
			...
			hdr_all_top_top_ctl_reg.hdr1_2p = is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) ? 1 : 0;
...
void scaler_hdr_output_select(DOLBY_HDR_OUTPUT_SELECT_T dolby_hdr_output_select)
{
	dolby_hw5_dolby_hw5_top_ctrl_RBUS dolby_hw5_dolby_hw5_top_ctrl_reg;

	dolby_hw5_dolby_hw5_top_ctrl_reg.regValue = IoReg_Read32(DOLBY_HW5_DOLBY_HW5_TOP_CTRL_reg);

	if (dolby_hdr_output_select == DOLBY_HDR1_OUTPUT_SELECT_HDR1)
		dolby_hw5_dolby_hw5_top_ctrl_reg.hw5_hdr_out_sel = 0; // output data to dispi1 select HDR1
	else if (dolby_hdr_output_select == DOLBY_HDR1_OUTPUT_SELECT_HW5)
		dolby_hw5_dolby_hw5_top_ctrl_reg.hw5_hdr_out_sel = 1; // output data to dispi1 select dolby HW5
	else if (dolby_hdr_output_select == DOLBY_HDR2_OUTPUT_SELECT_HDR2)
		dolby_hw5_dolby_hw5_top_ctrl_reg.hw5_hdr2_out_sel = 0; // output data to dispi2 select HDR2
	else if (dolby_hdr_output_select == DOLBY_HDR2_OUTPUT_SELECT_HW5_CORE2_SLICE0)
		dolby_hw5_dolby_hw5_top_ctrl_reg.hw5_hdr2_out_sel = 2; // output data to dispi2 select dolby hw5_core2_slice_0
	else if (dolby_hdr_output_select == DOLBY_HDR2_OUTPUT_SELECT_HW5_CORE2_SLICE1)
		dolby_hw5_dolby_hw5_top_ctrl_reg.hw5_hdr2_out_sel = 1; // output data to dispi2 select dolby hw5_core2_slice_1

	IoReg_Write32(DOLBY_HW5_DOLBY_HW5_TOP_CTRL_reg, dolby_hw5_dolby_hw5_top_ctrl_reg.regValue);

	rtd_printk(KERN_INFO, TAG_NAME_VSC, "[%s] %d dolby_hdr_output_select=%d, 0xB81C2A00=0x%x\n", __FUNCTION__, __LINE__, dolby_hdr_output_select, IoReg_Read32(DOLBY_HW5_DOLBY_HW5_TOP_CTRL_reg));
}
#endif
...
VSC_DRIVER_PATTERN vsc_driver_pattern_table[] ={
#ifdef CONFIG_RTK_BUSINESS_DISPLAY_MODEL
    {HDMI_2K1K_30,		0, 2, 0, 0, 0, 1, 0},
    {HDMI_2K1K_60, 		0, 2, 0, 0, 0, 1, 0},
    {HDMI_2K1K_120,	 	0, 2, 0, 0, 0, 1, 0},
    ...
    {VO_2K1K_30, 		0, 2, 0, 0, 1, 1, 0},
    {VO_2K1K_60, 		0, 2, 0, 0, 1, 1, 0},
    {VO_2K1K_120, 		0, 2, 0, 0, 1, 1, 0},
    ...
    {CP_2K1K_30, 		0, 2, 0, 1, 1, 1, 0},
    {CP_2K1K_60, 		0, 2, 0, 1, 1, 1, 0},
    {CP_2K1K_120, 		0, 2, 0, 0, 1, 1, 0},
...
#else
    {HDMI_2K1K_30,		1, 2, 0, 0, 0, 1, 0},
    {HDMI_2K1K_60, 		1, 2, 0, 0, 0, 1, 0},
    {HDMI_2K1K_120,	 	0, 2, 0, 0, 0, 1, 0},
    {HDMI_2K1K_240,	 	0, 2, 0, 0, 0, 1, 0},
    ...
    {HDMI_RGB_8K4K_30, 	0, 2, 0, 1, 0, 1, 0},
    {HDMI_RGB_8K4K_60, 	0, 2, 0, 1, 0, 1, 0},
    {VO_2K1K_30, 		1, 2, 0, 0, 1, 1, 0},
    {VO_2K1K_60, 		1, 2, 0, 0, 1, 1, 0},
    {VO_2K1K_120, 		0, 2, 0, 0, 1, 1, 0},
    {VO_4K1K_120, 		0, 2, 0, 1, 1, 0, 0},
    ...
    {VO_8K4K_30, 		0, 2, 0, 1, 1, 0, 0},
    {VO_8K4K_60, 		0, 2, 0, 1, 1, 0, 0},
    {CP_2K1K_30, 		1, 2, 0, 1, 1, 1, 0},
    {CP_2K1K_60, 		1, 2, 0, 1, 1, 1, 0},
    {CP_2K1K_120, 		0, 2, 0, 1, 1, 1, 0},
    {CP_4K1K_120, 		0, 2, 0, 1, 1, 0, 0},
...
#ifndef BUILD_QUICK_SHOW
		|| Get_Force_run_fake_scaler_State()
#endif
		)
...
SRC_TIMING_INDEX get_current_timing_index(void)
{
    ...
    if(Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_AVD) {
        ...
        cur_is_2p = ((pVOInfo->v_freq_1000 / 1000) * (pVOInfo->h_total * pVOInfo->v_total)) > VSYNCGEN_get_vodma_pll(); // Change the order of operations to avoid overflow
...
void print_current_driver_pattern(void)
{
	INPUT_TIMING_INDEX i = HDMI_2K1K_30;
	char *timing_string[] = {
        "HDMI_2K1K_30",     //hdmi timimng<=2k1k framerate<=30
        "HDMI_2K1K_60",     //hdmi timimng<=2K1K  30<framerate<=60
        "HDMI_2K1K_120",        //hdmi timimng<=2K1K  framerate iss 100 or 120
        "HDMI_2K1K_240",        //hdmi timimng<=2K1K  framerate iss 240 or more than 240
        "HDMI_25K1K_30",        //hdmi timimng<=2.5k1k framerate<=30
        "HDMI_25K1K_60",        //hdmi timimng<=2.5K1K  30<framerate<=60
        "HDMI_25K1K_120",       //hdmi timimng<=2.5K1K  framerate iss 100 or 120
        "HDMI_25K1K_240",         //hdmi timimng<=2.5K1K  framerate is over 120
        "HDMI_3K1K_30",     //hdmi timimng<=3k1k framerate<=30
        "HDMI_3K1K_60",     //hdmi timimng<=3K1K  30<framerate<=60
        "HDMI_3K1K_120",        //hdmi timimng<=3K1K  framerate iss 100 or 120
        "HDMI_3K1K_240",         //hdmi timimng<=3K1K  framerate is over 120     
        ...
        "HDMI_4K2K_240",        //hdmi timimng<=4K1K  framerate iss is over 240     
        "HDMI_8K4K_30",     //hdmi 4k2k<timimng<=8K4K  framerate<=30
        "HDMI_8K4K_60",     //hdmi 4k2k<timimng<=8K4K  30<framerate<=60
        "HDMI_RGB_2K1K_30",     //hdmi RGB_timimng<=2k1k framerate<=30
        "HDMI_RGB_2K1K_60",     //hdmi RGB_timimng<=2K1K  30<framerate<=60
        "HDMI_RGB_2K1K_120",        //hdmi RGB_timimng<=2K1K  framerate iss 100 or 120
        "HDMI_RGB_2K1K_240",         //hdmi RGB_timimng<=2K1K  framerate is over 120
        "HDMI_RGB_25K1K_30",        //hdmi RGB_timimng<=2.5k1k framerate<=30
        "HDMI_RGB_25K1K_60",        //hdmi RGB_timimng<=2.5K1K  30<framerate<=60
        "HDMI_RGB_25K1K_120",       //hdmi RGB_timimng<=2.5K1K  framerate iss 100 or 120
        "HDMI_RGB_25K1K_240",         //hdmi RGB_timimng<=2.5K1K  framerate is over 120
        "HDMI_RGB_3K1K_30",     //hdmi RGB_timimng<=3k1k framerate<=30
        "HDMI_RGB_3K1K_60",     //hdmi RGB_timimng<=3K1K  30<framerate<=60
        "HDMI_RGB_3K1K_120",        //hdmi RGB_timimng<=3K1K  framerate iss 100 or 120
        "HDMI_RGB_3K1K_240",         //hdmi RGB_timimng<=3K1K  framerate is over 120
        "HDMI_RGB_4K2K_30",     //hdmi RGB_2k1k<timimng<=4K2K  framerate<=30
        "HDMI_RGB_4K2K_60",     //hdmi RGB_2k1k<timimng<=4K2K  30<framerate<=60
        "HDMI_RGB_4K2K_120",        //hdmi RGB_ imimng<=2K1K  framerate iss 100  or 120
        "HDMI_RGB_4K2K_240",        //hdmi RGB_timimng<=2K1K  framerate over 120
        "HDMI_RGB_8K4K_30",     //hdmi RGB_4k2k<timimng<=8K4K  framerate<=30
        "HDMI_RGB_8K4K_60",     //hdmi RGB_4k2k<timimng<=8K4K  30<framerate<=60     
        ...
        "VO_4K1K_120",      //vo 2k1k<timimng<=4K1K  framerate is 120
        "VO_4K2K_30",       //vo 2k1k<timimng<=4K2K  framerate<=30
        "VO_4K2K_60",       //vo 2k1k<timimng<=4K2K  30<framerate<=60
        "VO_4K2K_120",      //vo 2k1k<timimng<=4K2K  framerate > 60
        "VO_8K4K_30",        //vo 4k2k<timimng<=8K4K  framerate<=30
        "VO_8K4K_60",       //vo 4k2k<timimng<=8K4K  30<framerate<=60
        "CP_2K1K_30",       //vo timimng<=2k1k framerate<=30
        "CP_2K1K_60",       //vo timimng<=2K1K  30<framerate<=60
        "CP_2K1K_120",      //vo timimng<=2K1K  framerate iss 100 or 120
        "CP_4K1K_120",      //vo 2k1k<timimng<=4K1K  framerate is 120
        "CP_4K2K_30",       //vo 2k1k<timimng<=4K2K  framerate<=30
        "CP_4K2K_120",      //vo 2k1k<timimng<=4K2K  framerate > 60
        "CP_4K2K_60",       //vo 2k1k<timimng<=4K2K  30<framerate<=60
        "CP_8K4K_30",        //vo 4k2k<timimng<=8K4K  framerate<=30
        "CP_8K4K_60",       //vo 4k2k<timimng<=8K4K  30<framerate<=60
        "HDMI_HDR_4K2K_60", //hdmi hdr 2k1k<timimng<=4K2K  30<framerate<=60
        "HDMI_HDR_8K4K_60", //hdmi hdr 4k2k<timimng<=8K4K  30<framerate<=60
        "JPEG_2K1K_30",      //jpeg timimng<=2K1K framerate=30
        "JPEG_4K2K_30",      //jpeg 2k1k<timimng<=4K2K framerate=30
        "JPEG_8K4K_30",      //jpeg 4k2k<timimng<=8K4K framerate=30
        "TIMING_INVAILD"
	};
	INPUT_TIMING_INDEX index = get_current_pattern_index();
...
void i3ddma_dolby_reorder_update_color_format(void)
{
	...
	Scaler_DispSetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_COLOR_SPACE, SCALER_INPUT_COLOR_YUV422);
...
void get_sharememory_dolby_vision_l1l4_stats(struct dolby_vision_l1l4_stats *stats)
{
	struct dolby_vision_l1l4_stats *src = (struct dolby_vision_l1l4_stats *)Scaler_GetShareMemVirAddr(DOLBY_VISION_L1L4_STATS);

	if (src != NULL && stats != NULL) {
		stats->min = ntohs(src->min);
		stats->max = ntohs(src->max);
		stats->mid = ntohs(src->mid);
		stats->std = ntohs(src->std);
	}
}

void set_dolby_vision_l1l4_mode(int16_t mode)
{
	struct dolby_vision_l1l4_stats *src = (struct dolby_vision_l1l4_stats *)Scaler_GetShareMemVirAddr(DOLBY_VISION_L1L4_STATS);
...
{//for 4k 120/ 2 pixel mode
	...
	if(enable)
	{
		...
		{
			m8p_vgip_data_path_select_sub_reg.regValue = IoReg_Read32(M8P_VGIP_Data_Path_Select_sub_reg);
			m8p_vgip_data_path_select_sub_reg.s2_clk_en_new = 1;
			m8p_vgip_data_path_select_sub_reg.uzd2_in_sel_new = 3;
			IoReg_Write32(M8P_VGIP_Data_Path_Select_sub_reg, m8p_vgip_data_path_select_sub_reg.regValue);
		}
...
void get_vtop_input_info(unsigned char *twopixel_mode, unsigned char *hdmi_input, unsigned char *color_format)
{
	...
	*twopixel_mode = (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) ? TRUE: FALSE;
...
unsigned char force_enable_two_step_uzu(void)
{
    if( get_panel_pixel_mode() > PANEL_1_PIXEL )
    {
        ...
        {
            if(jude_input_timing_is_hfr(SLR_MAIN_DISPLAY))
                return TRUE;
        }
        else
        {
            if(get_MEMC_bypass_status_refer_platform_model())
                return TRUE;
            else if( get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
                return TRUE;
            else if(scaler_vsc_get_gamemode_force_framerate_lowdelay_mode())
                return TRUE;
            else if(jude_input_timing_is_hfr(SLR_MAIN_DISPLAY))
                return TRUE;
            else if(force_enable_mdomain_2disp_path())
                return TRUE;
        }
    }
...
unsigned char force_enable_mdomain_2disp_path(void)
{
//    no need to check 2nd-uzu fifo case due to already seperate multiview case - ML8QC-1852
//    unsigned int input_wid = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID);
//    unsigned int input_len = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN);
//    unsigned int disp_wid_1000 = Get_DISP_HORIZONTAL_TOTAL() * 1000;
//    unsigned int disp_len_1000 = Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN) * 1000;
//    unsigned int wid_scaleup_ratio_1000 = 0;
//    unsigned int len_scaleup_ratio_1000 = 0;
    ...
    if(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_SUB_DISPLAY)
        return FALSE;//return FALSE when display is sub
...
//        if( ( input_wid == 0 ) || ( input_len == 0 ) || ( disp_wid_1000 == 0 ) || ( disp_len_1000 == 0 ) )
//        {
//            rtd_pr_vbe_err("[%s][Error] No correct in/out region size !!!\n", __FUNCTION__);
//            return FALSE;
//        }

//        wid_scaleup_ratio_1000 = disp_wid_1000 / input_wid;
//        len_scaleup_ratio_1000 = disp_len_1000 / input_len;

//        rtd_pr_vbe_notice("[%s] wid_scaleup_ratio_1000 = %d, len_scaleup_ratio_1000 = %d\n", __FUNCTION__, wid_scaleup_ratio_1000, len_scaleup_ratio_1000);
...
//              && ( ( wid_scaleup_ratio_1000 * len_scaleup_ratio_1000 ) / 1000 < 2000 ) // constraint ( 1p in + 2p out in 2nd-uzu linebuffer ) : ( Mdisp H_total / Mcap Hactive ) * ( Mdisp V_active / Mcap V_active ) >= 2
              && !( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
            return TRUE;
        else
            return FALSE;
...
{//updatehdmi qms info to share memory
	...
	unsigned int cur_i3ddma_block = 0;
	static unsigned int pre_i3ddma_block = 0;
	...
	if(hdmi_qms_status)
	{
		...
		{//QMS mode
			cur_i3ddma_block = H3DDMA_CAP0_Cap_Status_get_cap0_cap_block_sel(IoReg_Read32(H3DDMA_CAP0_Cap_Status_reg));
			cur_time = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
			...
			if(((pre_i3ddma_block+1) % QMS_BUFFER_NUM) != cur_i3ddma_block)
				rtd_pr_vsc_emerg("### QMS Err func:%s block (%d => %d) bg:%d ####\r\n",__FUNCTION__, pre_i3ddma_block, cur_i3ddma_block, PPOVERLAY_Main_Display_Control_RSV_get_m_force_bg(IoReg_Read32(PPOVERLAY_Main_Display_Control_RSV_reg)));

			pre_i3ddma_block = cur_i3ddma_block;
#ifdef QMS_DEBUG_PRINT
...
unsigned char check_sub_is_idle_for_srnn(unsigned char display)
{
    ...
    if((get_mach_type() == MACH_ARCH_RTK2885P) && (display == SLR_MAIN_DISPLAY))
    {
	//merlin8/8p mdomain one-bin
        //if(get_multiview_enable() || get_hdmi_vrr_4k60_mode() || is_scaler_input_2p_mode(display)
        if(get_multiview_enable() || get_hdmi_vrr_4k60_mode() || is_scaler_input_2p_mode(display)
...
#endif
#if defined (CONFIG_ENABLE_HDR10_HDMI_AUTO_DETECT) || defined(CONFIG_ENABLE_DOLBY_VISION_HDMI_AUTO_DETECT)
            || (((get_vsc_src_is_hdmi_or_dp()) && (Scaler_Hdmi_DM_Get_Status() == SLR_HDMI_DM_CONNECT))
...
void set_sub_for_srnn(SUB_SRNN_TYPE_E type, unsigned char enable)
{
    switch(type)
    {
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_drvif_sub_mdomain_for_srnn(enable);
		}
            break;
...
void set_sub_vgip_for_srnn(void)
{
...
#ifndef BUILD_QUICK_SHOW
    else if(inputSrctype == VSC_INPUTSRC_ADC)
    {
        ...
        {
            sub_vgip_vgip_chn2_ctrl_reg.ch2_in_sel = VGIP_SRC_ADC;
        }
    }
    ...
    {
        sub_vgip_vgip_chn2_ctrl_reg.ch2_in_sel = ((display == SLR_MAIN_DISPLAY) && (get_support_vo_force_v_top(display, support_sdr_max_rgb) || (get_force_hdmi_hdr_flow_enable(SLR_MAIN_DISPLAY) == true))) ? VGIP_SRC_HDR1 : get_vgip_in_sel_by_display(SLR_MAIN_DISPLAY);
    }
    else if(inputSrctype == VSC_INPUTSRC_DP)
    {
        if(decide_dp_source_regen_vsync())
        {
            if(display == SLR_MAIN_DISPLAY)
            {
                ...
                {
                    sub_vgip_vgip_chn2_ctrl_reg.ch2_in_sel = get_vgip_in_sel_by_display(SLR_MAIN_DISPLAY);
                }
            }
        }
        else
...
unsigned char multiview_1p_input_in_2p_panel(void)
{
    unsigned char status_flag = FALSE;

//    rtd_pr_vbe_notice("[%s][EricTang] %d, %d, %d\n", __FUNCTION__, get_panel_pixel_mode(), vbe_disp_get_multiview_flag(), is_scaler_input_2p_mode(SLR_MAIN_DISPLAY));

    if( ( get_panel_pixel_mode() > PANEL_1_PIXEL ) && vbe_disp_get_multiview_flag() && ( is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) == FALSE ) )
        status_flag = TRUE;

    return status_flag;
}

unsigned char multiview_1p_input_decide_D1_timing(unsigned int* uzd_hactive, unsigned int* expand_uzudtg_htotal, unsigned int* shrink_uzudtg_vtotal)
{
    unsigned int mdisp_hactive = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID);
    unsigned int mdisp_vactive = Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN);
    unsigned int uzudtg_htotal = Get_DISP_HORIZONTAL_TOTAL() * 1000;
    unsigned int uzudtg_vactive = Scaler_DispGetInputInfo(SLR_INPUT_DISP_LEN) * 1000;
    unsigned int second_uzu_width_scaleup_ratio_1000 = 0;
    unsigned int second_uzu_length_scaleup_ratio_1000 = 0;
    unsigned int uzudtg_htotal_temp = 0;
    unsigned int uzudtg_vtotal_temp = 0;

    *uzd_hactive = 0;

    if( jude_input_timing_is_hfr(SLR_MAIN_DISPLAY) == FALSE )
        return FALSE;

    if( ( mdisp_hactive == 0 ) || ( mdisp_vactive == 0 ) )
    {
        rtd_pr_vbe_err("[%s] abnormal mdisp_hactive = %d, mdisp_vactive = %d\n", __FUNCTION__, mdisp_hactive, mdisp_vactive);
        return FALSE;
    }

    second_uzu_width_scaleup_ratio_1000 = uzudtg_htotal / ( mdisp_hactive + PQDC_MINI_PORCH );
    second_uzu_length_scaleup_ratio_1000 = uzudtg_vactive / mdisp_vactive;

//    rtd_pr_vbe_notice("[%s][EricTang] %d, %d, %d, %d\n", __FUNCTION__, uzudtg_htotal, uzudtg_vactive, mdisp_hactive, mdisp_vactive);

    if( second_uzu_width_scaleup_ratio_1000 * second_uzu_length_scaleup_ratio_1000 >= 2000000 ) // 2nd-uzu line-buffer : 1p input + 2p output OK
    {
        return FALSE;
    }
    else                                                                                        // 2nd-uzu line-buffer : 1p input + 2p output need to be modified
    {
        uzudtg_htotal_temp = 2 * ( mdisp_hactive + PQDC_MINI_PORCH ) * mdisp_vactive / ( uzudtg_vactive / 1000 );

        if( uzudtg_htotal_temp % 2 )    // odd
        {
            uzudtg_htotal_temp += 1;
        }
        else                            // even
        {
            if( ( ( 2 * ( mdisp_hactive + PQDC_MINI_PORCH ) * mdisp_vactive ) % ( uzudtg_vactive / 1000 ) ) != 0 )    // Remainder exists
                uzudtg_htotal_temp += 2;
        }

//        uzudtg_vtotal_temp = (unsigned int)( ( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 2000ULL ) / ( (unsigned long long)uzudtg_htotal_temp * (unsigned long long)Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) ) );
        uzudtg_vtotal_temp = (unsigned int)div64_u64(( (unsigned long long)Get_DISPLAY_CLOCK_TYPICAL() * 2000ULL ), ( (unsigned long long)uzudtg_htotal_temp * (unsigned long long)Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) ));

//        rtd_pr_vbe_notice("[%s][EricTang] %d, %d\n", __FUNCTION__, uzudtg_htotal_temp, uzudtg_vtotal_temp);

        if( uzudtg_vtotal_temp >= Get_DISP_VERTICAL_TOTAL() )
        {
            *expand_uzudtg_htotal = uzudtg_htotal_temp;
            *shrink_uzudtg_vtotal = uzudtg_vtotal_temp;
            return TRUE;
        }
        else
        {
            *uzd_hactive = ( Get_DISP_HORIZONTAL_TOTAL() / 2 ) - PQDC_MINI_PORCH;
            return FALSE;
        }
    }
}
...
#ifndef BUILD_QUICK_SHOW
#ifndef UT_flag
void Scaler_Set_Film_Control_MEMC(unsigned int enable)
{
    unsigned int *film_ctrl_memc_addr = NULL;
    film_ctrl_memc_addr = (unsigned int*)Scaler_GetShareMemVirAddr(SCALERIOC_FILM_CTRL_MEMC);
    if(film_ctrl_memc_addr)
        *film_ctrl_memc_addr = Scaler_ChangeUINT32Endian(enable);
    rtd_pr_vsc_info("[%s]enable=%d", __FUNCTION__, enable);
}
unsigned int Scaler_Get_Film_Control_MEMC(void)
{
    unsigned int *film_ctrl_memc_addr = NULL;
    film_ctrl_memc_addr = (unsigned int*)Scaler_GetShareMemVirAddr(SCALERIOC_FILM_CTRL_MEMC);
    return (film_ctrl_memc_addr? htonl(*film_ctrl_memc_addr): 0);
}

unsigned char get_eco_mode_shared_flag(void)
{
    unsigned int *sync_info = NULL;
    sync_info = (unsigned int*)Scaler_GetShareMemVirAddr(SCALERIOC_ECO_MODE);
    return (sync_info ? htonl(*sync_info): 0);
}
void set_eco_mode_shared_flag(unsigned char flag)
{
    unsigned int *sync_info = NULL;
    sync_info = (unsigned int *)Scaler_GetShareMemVirAddr(SCALERIOC_ECO_MODE);
    if(sync_info)
    {
        *sync_info = htonl(flag);
        rtd_pr_vsc_info("[%s] set flag = %d\n", __FUNCTION__, flag);
    }
    else
    {
        rtd_pr_vsc_info("[%s] sync_info not exist\n", __FUNCTION__);
    }
}
...
unsigned char get_all_timing_support_4k1k240(void)
{
    if((panel_dlg_get_cur_dlg_mode() == PANEL_DLG_4k1k240) && panel_dlg_get_dlg_support_all_timing())
    {
        return 1;
    }
    else
...
int m8p_dtg_set_sub_double_buffer_enable(DTG_DOUBLE_BUFFER_INDEX index)
{
    ...
    return m8p_dtg_set_double_buffer_enable(index, &enable);
}

int m8p_dtg_set_sub_double_buffer_disable(DTG_DOUBLE_BUFFER_INDEX index)
{
...
int m8p_dtg_set_sub_double_buffer_apply(DTG_DOUBLE_BUFFER_INDEX index)
{
    unsigned char apply = 1;
    ...
    return m8p_dtg_set_double_buffer_apply(index, &apply);
}

int m8p_dtg_set_sub_double_buffer_not_apply(DTG_DOUBLE_BUFFER_INDEX index)
{
...
int m8p_dtg_wait_for_sub_double_buffer_apply(DTG_DOUBLE_BUFFER_INDEX index)
{
    unsigned char apply = 0;
    int timeout = 30;
    ...
    m8p_dtg_get_double_buffer_apply(index, &apply);
    ...
    if(timeout == 0)
    {
        rtd_pr_vsc_err("[%s %d]ERROR, wait for timeout for sub dtg double buffer apply\n", __FUNCTION__, __LINE__);
    }
