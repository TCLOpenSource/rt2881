...
static struct semaphore SMT_state_Semaphore;
void smt_state_semaphore_init(void)
{//init SMT_state_Semaphore
	sema_init(&SMT_state_Semaphore, 1);
}

struct semaphore *get_smt_state_semaphore(void)
{//get SMT_state_Semaphore
	return &SMT_state_Semaphore;
}
...
#endif
//	unsigned int plane = get_vo_plane_flag(display);
	down(get_smt_state_semaphore());
	if((!compare_rerify_vsc_source_num_result(display)) || (get_scaler_stop_flag(display))) {
		if(state == SLR_SMOOTH_TOGGLE_ACTIVE) {
			rtd_pr_smt_notice("set_vo_smoothtoggle_state=2 when already disconnect\n");
		}
		state = SLR_SMOOTH_TOGGLE_NONE;
		rtd_pr_smt_notice("set_vo_smoothtoggle_state force to 0 for already disconnect\n");
	}
	vo_source_state_smooth_toggle[display] = state;
	up(get_smt_state_semaphore());
	if(VO_VIDEO_PLANE_V1 == plane)
	{
		vodma_vodma_smooth_toggle_RBUS vodma_smooth_toggle_reg;
...
void panel_parameter_send_info(void)
{
	...
	parameter_info.disp_hsr_down_sample=Get_DISPLAY_HSR_DOWN_SAMPLE();
#if 0
   if ((webos_tooloption.eModelModuleType == module_LGD) && (webos_tooloption.eBackLight == direct ) && (webos_tooloption.eLEDBarType == local_dim_block_32)){
	...
	parameter_info.disp_v_total_60Hz_min = Get_PANEL_DISP_VERTICAL_TOTAL_60Hz_MIN();
	parameter_info.disp_panel_pixel_mode_count = (unsigned int)get_panel_pixel_mode();
...
{
	unsigned int smt_apply_status = 0;
	...
	{
		if(display == SLR_MAIN_DISPLAY)
			smt_apply_status = VGIP_Smooth_tog_ctrl_get_main_imd_smooth_toggle_apply(rtd_inl(VGIP_Smooth_tog_ctrl_reg));
		else
			smt_apply_status = VGIP_Smooth_tog_ctrl_get_sub_imd_smooth_toggle_apply(rtd_inl(VGIP_Smooth_tog_ctrl_reg));
	}
	else
	{
		if(display == SLR_MAIN_DISPLAY)
			...
			smt_apply_status = M8P_VGIP_Smooth_tog_ctrl_sub_get_sub_imd_smooth_toggle_apply_new(rtd_inl(M8P_VGIP_Smooth_tog_ctrl_sub_reg));
	}

	if (smt_apply_status == 0) {
...
unsigned int check_smooth_toggle_apply_status(unsigned display)
{
	unsigned int smt_apply_status = 0;
...
void Check_smooth_toggle_update_flag(unsigned char display)
{
	...
	{//VO and Not JPEG case, jpag goes kernel smooth toggle
		...
		while(frametimeout) {
			smt_apply_status = check_smooth_toggle_apply_status(display);
			if (smt_apply_status==0) {
				unsigned int *pulTemp;
				unsigned int i = 0, ulItemCount = 0;
				SMOOTH_TOGGLE_UPDATE_FWINFO_PKG *sm_info_pkg = NULL;
		...
		if(frametimeout==0){
			if(display == SLR_MAIN_DISPLAY)
				rtd_clearbits(VGIP_Smooth_tog_ctrl_reg,VGIP_Smooth_tog_ctrl_main_imd_smooth_toggle_apply_mask);
			else
			{
				if(get_mach_type() == MACH_ARCH_RTK2885P)
					rtd_clearbits(VGIP_Smooth_tog_ctrl_reg,VGIP_Smooth_tog_ctrl_sub_imd_smooth_toggle_apply_mask);
				else
					rtd_clearbits(M8P_VGIP_Smooth_tog_ctrl_sub_reg,M8P_VGIP_Smooth_tog_ctrl_sub_sub_imd_smooth_toggle_apply_new_mask);
			}
		}
...
void send_memory_mapping_for_adaptive_streaming(void)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		scaler_memory_info.M_main_first_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN);
		scaler_memory_info.M_main_sec_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_SEC);
#ifdef CONFIG_RTK_BUSINESS_DISPLAY_MODEL
		//scaler_memory_info.M_caveout_size=M_DOMAIN_WIDTH_BUSINESS_DISPLAY;
		scaler_memory_info.M_caveout_size = nonlibdma_get_m_domain_width_business_display();
#else
		//scaler_memory_info.M_caveout_size=M_DOMAIN_WIDTH_CARVEOUT_2K;
		scaler_memory_info.M_caveout_size = nonlibdma_get_m_domain_width_carveout_2k();
#endif
		//scaler_memory_info.M_main_third_addr = drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_THIRD);
		scaler_memory_info.M_main_third_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_THIRD);
#ifdef CONFIG_I2RND_ENABLE
		...
		scaler_memory_info.M_main_i2r_first_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_2);
		scaler_memory_info.M_main_i2r_sec_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_2_SEC);
		scaler_memory_info.M_main_i2r_third_addr = nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_2_THIRD);
		scaler_memory_info.DI_i2r_addr = nonlibdma_get_query_start_address(nonlibdma_QUERY_IDX_DI_2);
#endif
	}
...
void vo_smoothtoggle_memory_alloc(unsigned char display)
{
	...
	if (display==SLR_MAIN_DISPLAY) {
		...
		else*/ //if(vo_force_data_mode_get_enable()==SLR_DISPLAY_DATA_FRC)
		{
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				//di_addr=drvif_memory_get_block_addr(MEMIDX_DI);
				di_addr=nonlibdma_drvif_memory_get_block_addr(MEMIDX_DI);
#ifdef CONFIG_MDOMAIN_RESERVED_MEMORY
				...
				m_addr_third=nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_THIRD);
				m_addr_sec=nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN_SEC);
				m_addr_first=nonlibdma_drvif_memory_get_memtag_startaddr(MEMIDX_MAIN);
#else
				...
				m_addr_third=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD); // the max: 444 ,8 bit:1920*1080*3byte
				m_addr_sec=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN_SEC);  // the max: 444 ,10 bit:2160*1440*30bit
				m_addr_first=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN);
#endif
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			di_size=nonlibdma_drv_memory_di_buffer_get();
			m_sub_third=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB_THIRD);
			m_sub_sec=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB_SEC);
			m_sub_first=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB);
		} else { // for pass compile
			di_size = 0;
			m_sub_third = 0;
			m_sub_sec = 0;
		...
		else*/ //if(vo_force_data_mode_get_enable()==SLR_DISPLAY_DATA_FRC)
		{
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				//m_sub_first=drvif_memory_get_block_addr(MEMIDX_SUB);
				m_sub_third=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB_THIRD);//0xffffffff;//nonlibdma_drvif_memory_alloc_block(MEMIDX_MAIN_THIRD,0x600000, MEMALIGN_ANY); // the max: 444 ,8 bit:1920*1080*3byte
				m_sub_sec=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB_SEC);  // the max: 444 ,10 bit:2160*1440*30bit
				m_sub_first=nonlibdma_drvif_memory_get_block_addr(MEMIDX_SUB);
			} else { // for pass compile
				m_sub_third = 0;
				m_sub_sec = 0;
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			di_size=nonlibdma_drv_memory_di_buffer_get();
			di_addr=nonlibdma_drvif_memory_get_block_addr(MEMIDX_DI);
			m_addr_third=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN_THIRD);
			m_addr_sec=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN_SEC);
			m_addr_first=nonlibdma_drvif_memory_get_block_addr(MEMIDX_MAIN);
		} else { //for pass compile
			di_size = 0;
			di_addr = 0;
...
{//return TRUE: can run sub smooth toggle
	...
	if((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE) || 
		(Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_V_FREQ_1000) <= Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) + 100))
	{
		return TRUE;
	}
	else
