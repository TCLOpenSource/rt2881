...
#else
unsigned char set_di_doublebuf_write_TV006(unsigned char diflag,unsigned char DB_CTRL_Mode)
{
...
//return 0; //please remove it
	...
	if (diflag == 0) {
		...
		timeout_1 = 30;
		if (DB_CTRL_Mode == 0)	// wait DB apply done
		{
			do {
				di_db_reg_ctl_reg.regValue = IoReg_Read32(DI_db_reg_ctl_reg);
				timeout_1--;
		...
		else if (DB_CTRL_Mode == 1) // disable DB to force write
		{
		...
		db_apply_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
		if (timeout_1 < 0) {
			rtd_pr_vbe_emerg("DI_Enable time out, timeout2=%d, db_st=%d, db_end=%d, diflag=%d, DB_CTRL_Mode=%d, h_rotate=%d,\n", 
				timeout_1, db_apply_st, db_apply_end, diflag, DB_CTRL_Mode, Scaler_color_access_RTNR_H_Rotate_Flag(0, 0));
		}
		rtd_pr_vbe_info("DI_Enable f2D ,t_0=%d, set_di_sta_90k=%d, db_st=%d, db_end=%d, diflag=%d, DB_CTRL_Mode=%d, h_rotate=%d,\n", 
			timeout_1, set_di_sta_90k, db_apply_st, db_apply_end, diflag, DB_CTRL_Mode, Scaler_color_access_RTNR_H_Rotate_Flag(0, 0));	
#endif		
		...
		db_apply_end = IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg);
		if (timeout_1 < 0) {
			rtd_pr_vbe_emerg("DI_Enable time out nw, timeout2=%d, db_st=%d, db_end=%d, diflag=%d, DB_CTRL_Mode=%d, h_rotate=%d,\n", 
				timeout_1, db_apply_st, db_apply_end, diflag, DB_CTRL_Mode, Scaler_color_access_RTNR_H_Rotate_Flag(0, 0));
		}

		rtd_pr_vbe_info("DI_Enable unf2d ,t_0=%d, set_di_sta_90k=%d, db_st=%d, db_end=%d, diflag=%d, DB_CTRL_Mode=%d, h_rotate=%d,\n", 
			timeout_1, set_di_sta_90k, db_apply_st, db_apply_end, diflag, DB_CTRL_Mode, Scaler_color_access_RTNR_H_Rotate_Flag(0, 0));	
#endif
	}
...
//					if(Scaler_DispGetInputInfo(SLR_INPUT_CAP_WID) >= 960)
					...
					{	//sync from pacific
						...
						if(get_mach_type() == MACH_ARCH_RTK2885P) {
							...
							nonlibdma_drv_memory_set_ip_fifo(0);
						}
					...
					{	// 5A
						...
						if(get_mach_type() == MACH_ARCH_RTK2885P) {
							...
							nonlibdma_drv_memory_set_ip_fifo(1);
						}
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_drv_memory_set_ip_fifo(2);
			}
			DI_SW_Detection_in_VPQ_TSK_Stable_CNT = 0;
			IoReg_ClearBits(DI_IM_DI_BTR_CONTROL_reg, _BIT9);
			im_di_control_reg.regValue = IoReg_Read32(DI_IM_DI_CONTROL_reg);
		...
		if(/*Scaler_PipGetInfo(SLR_PIP_ENABLE) && */((display == SLR_MAIN_DISPLAY) && ((Scaler_InputSrcGetType(SLR_MAIN_DISPLAY) == _SRC_VGA) &&  ((Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR) == _MODE_1080I25) || (Scaler_DispGetInputInfo(SLR_INPUT_MODE_CURR) == _MODE_1080I30))))) {
			...
			if ((Scaler_PipGetInfo(SLR_PIP_TYPE)>=SUBTYPE_PIPMLSR)&&(Scaler_PipGetInfo(SLR_PIP_TYPE)<=SUBTYPE_PIPMLSR_4x3))
			{
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P) {
					...
					nonlibdma_drvif_memory_free_block(MEMIDX_DI);
				}
			...
			if(Scaler_InputSrcGetType(display) == _SRC_VO) {
				if(vbe_disp_get_adaptivestream_fs_mode()==1)  {
					if( (Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID_PRE)>3000) &&
						( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_30000_pOFFSET ) )
...
//		scalerip_modify_rdstart();
		...
		set_di_doublebuf_write_TV006(diflag, di_doublebuf_wait_apply_done);
#ifdef CONFIG_DUAL_CHANNEL
			...
			if(get_vsc_run_adaptive_stream(display)
				...
				if(get_mach_type() == MACH_ARCH_RTK2885P) {
					if(Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_INTERLACE)) {
						...
						nonlibdma_drv_memory_set_ip_fifo(SLR_DI_3A_mode);
					} else {
						//drv_memory_set_ip_fifo(SLR_DI_progress_mode);
						nonlibdma_drv_memory_set_ip_fifo(SLR_DI_progress_mode);
					}
				}
			}
...
//	di_im_di_control_RBUS im_di_control_reg;
	...
	else
	{
		...
		scaler_set_vgip_datapath_uzd2_inl_sel(0);
	}
...
{//this api is for wait DI buffer ready
	...
	if(DI_wait_frame_num != 0)
	{
		...
		if(vfreq)
			need_time = 90000000 * DI_wait_frame_num / vfreq;
		else
		{
...
extern unsigned int blk_decont_dma_vs_cnt;
extern unsigned int blk_decont_dma_sdnr_cnt;
extern unsigned char blk_decont_dma_errFlow;
// end for ml8 dma recovery flow
void fw_scalerip_set_I_BLK_decontour(void)// ml8_add
{
#ifndef BUILD_QUICK_SHOW	// no isr to do recovery flow
#ifndef UT_flag
	...
	if(blk_decont_dma_errFlow == 0)
	{
		...
		if(enable_log) rtd_pr_vbe_info("lsy %s(%d) errFlow %d cnt (%d, %d) end to set config\n",__func__, __LINE__, blk_decont_dma_errFlow, blk_decont_dma_vs_cnt, blk_decont_dma_sdnr_cnt);
