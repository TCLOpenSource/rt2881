...
void mdomain_capture_disable(int enable)
{
		...
		if(enable){
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_set_cap_inctrl_reg_double_enable(0, 0);
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_set_cap_inctrl_reg_double_enable(0, 1);
				nonlibdma_set_cap_inctrl_reg_v_flip_3buf_en(0, 1);
			}
		}
...
static void disp_buffer(bool enable)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		block_sel = nonlibdma_get_disp_status_reg_block_select(0);
		...
		nonlibdma_set_disp_mainctrl_reg_single_buffer_select(0, block_sel);
	}
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		//IoReg_Write32(MDOMAIN_DISP_DDR_MainCtrl_reg, ddr_mainctrl_reg.regValue);
		nonlibdma_set_disp_ctrl_reg_double_en(0, 1);
	}
    }
...
void mdomain_capture_freeze(int enable)
{
    ...
    if (enable) {
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_set_cap_inctrl_reg_in_freeze_option(0, 1);
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		if(nonlibdma_get_cap_doublebuffer_reg_doublebuffer_enable(0)) {
