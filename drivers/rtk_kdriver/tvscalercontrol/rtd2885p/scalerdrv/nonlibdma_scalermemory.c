...
void mcap_outstanding_ctrl(unsigned char display, unsigned char enable)
{
	...
	cap_outstanding = enable ? _CAPTURE_OUTSTANDING : 0;
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		...
		if(mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_max_outstanding != cap_outstanding)
		{
			if(MDOMAIN_CAP_Cap_In1_enable_get_in1_cap_enable(IoReg_Read32(MDOMAIN_CAP_Cap_In1_enable_reg)))
			{
			...
			mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_max_outstanding = cap_outstanding;//max
			mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_force_all_rst = enable;//force reset when finish
			IoReg_Write32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Main_Cap_reg, mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.regValue);
			rtd_pr_scaler_memory_info("[%s] %d set main cap_outstanding %d\n",__FUNCTION__, __LINE__, mdomain_cap_ddr_outstanding_ctrl_main_cap_reg.in1_max_outstanding);
		}
	}
	...
	{//sub
		...
		if(mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_max_outstanding != cap_outstanding)
		{
			if(MDOMAIN_CAP_Cap_In2_enable_get_in2_cap_enable(IoReg_Read32(MDOMAIN_CAP_Cap_In2_enable_reg)))
			{
				drvif_memory_DisableMCap(display);
				mdelay(1);
			...
			mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_max_outstanding = cap_outstanding;//max
			mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_force_all_rst = enable;//force reset when finish
			IoReg_Write32(MDOMAIN_CAP_DDR_Outstanding_Ctrl_Sub_Cap_reg, mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.regValue);
			rtd_pr_scaler_memory_info("[%s] %d set sub cap_outstanding %d\n",__FUNCTION__, __LINE__, mdomain_cap_ddr_outstanding_ctrl_sub_cap_reg.in2_max_outstanding);
		}
	}
#endif
}
...
void mdisp_outstanding_ctrl(unsigned char display, unsigned char enable)
{
	...
	disp_outstanding = enable ? _DISPLAY_OUTSTANDING : 0;
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		...
		if(mdomain_disp_ddr_outstanding_ctrl_main_reg.max_outstanding != disp_outstanding)
		{
			...
			mdomain_disp_ddr_outstanding_ctrl_main_reg.max_outstanding = disp_outstanding;
			mdomain_disp_ddr_outstanding_ctrl_main_reg.force_all_rst = enable;
			IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Main_reg, mdomain_disp_ddr_outstanding_ctrl_main_reg.regValue);
			rtd_pr_scaler_memory_info("[%s] %d set main disp_outstanding %d\n",__FUNCTION__, __LINE__, mdomain_disp_ddr_outstanding_ctrl_main_reg.max_outstanding);
		}
	}
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	else
	{//sub
		...
		if(mdomain_disp_ddr_outstanding_ctrl_sub_reg.sub_max_outstanding != disp_outstanding)
		{
			if(MDOMAIN_DISP_Disp_sub_enable_get_sub_disp_en(IoReg_Read32(MDOMAIN_DISP_Disp_sub_enable_reg)))
			{
				drvif_memory_EnableMDisp_Or_DisableMDisp(display, _DISABLE);
				mdelay(1);
			...
			mdomain_disp_ddr_outstanding_ctrl_sub_reg.sub_max_outstanding = disp_outstanding;
			mdomain_disp_ddr_outstanding_ctrl_sub_reg.sub_force_all_rst = enable;
			IoReg_Write32(MDOMAIN_DISP_DDR_Outstanding_Ctrl_Sub_reg, mdomain_disp_ddr_outstanding_ctrl_sub_reg.regValue);
			rtd_pr_scaler_memory_info("[%s] %d set sub disp_outstanding %d\n",__FUNCTION__, __LINE__, mdomain_disp_ddr_outstanding_ctrl_sub_reg.sub_max_outstanding);
		}
	}
#endif
}
	...
	if((Idx >= MEMIDX_VDC) && (Idx < MEMIDX_ALL)) {
			...
			if(MEMIDX_MAIN_THIRD == Idx){
                ...
                {//co buffer
					...
					MemTag[Idx].Status = MemTag[MEMIDX_MAIN_SEC].Status;
					if(carvedout_enable() == 0)  // do NOT use carvedout memory, use CMA malloc
						MemTag[Idx].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_MAIN].StartAddr+space, DMA_SPEEDUP_ALIGN_VALUE);
					else  // use carvedout memory
						MemTag[Idx].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_MAIN_SEC].StartAddr+space, DMA_SPEEDUP_ALIGN_VALUE);
#ifdef CONFIG_ARM64
                    rtd_pr_scaler_memory_debug("[MEM] Idx[%d]=%lx(%d KB)\n", Idx, MemTag[Idx].StartAddr, space >> 10);
				...
				if(MemTag[MEMIDX_MAIN_SEC].StartAddr != _DUMMYADDR) {
					if((Scaler_DispGetInputInfo(SLR_INPUT_IPH_ACT_WID) > 2048)
						...
						if(drvif_scaler_getValidMemInfo_fromVideo(&stAllocBufferInfo_idma) < 0){
							...
							MemTag[Idx].StartAddr = stAllocBufferInfo_idma.normal_mem_addr&0xfffffff0;
							MemTag[Idx].Size = space;
...
static void memory_set_capture_for_mp(void)
{
	...
	//Crixus@20140722 - Mac3 no in2
	drvif_memory_DisableMCap(SLR_MAIN_DISPLAY);	// disable Capture
	//frank@0503 add below delay to confirm M capture have disable
	msleep(20);
...
void set_sub_for_mdomain_2p_video_delay_address(unsigned int shift_addr)
{
    mdomain_disp_dispsub_boundaryaddr_ctrl2_RBUS mdomain_disp_dispsub_boundaryaddr_ctrl2_reg;
    unsigned int buffer_size = drv_memory_Get_multibuffer_size();
    rtd_pr_scaler_memory_emerg("[Videodelay] sub buffer_size=%d shift_addr = %d\n", drv_memory_Get_multibuffer_size(), shift_addr);
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DispSub_BoundaryAddr_CTRL2_reg);
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk0_sel = 0; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk1_sel = 1; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk2_sel = 2;
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk3_sel = 3; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk4_sel = 4; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk5_sel = 5; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk6_sel = 6; 
    mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.sub_blk7_sel = 7; 
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_CTRL2_reg, mdomain_disp_dispsub_boundaryaddr_ctrl2_reg.regValue);
    IoReg_Write32(MDOMAIN_DISP_DDR_SubAddr_reg, MemTag[MEMIDX_MAIN].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr1_reg, MemTag[MEMIDX_MAIN].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_reg, MemTag[MEMIDX_MAIN].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_DDR_Sub2ndAddr_reg, MemTag[MEMIDX_MAIN_SEC].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg, MemTag[MEMIDX_MAIN_SEC].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg, MemTag[MEMIDX_MAIN_SEC].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_DDR_Sub3rdAddr_reg, MemTag[MEMIDX_MAIN_THIRD].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg, MemTag[MEMIDX_MAIN_THIRD].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg, MemTag[MEMIDX_MAIN_THIRD].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_sub_forth_buf_addr_reg, MemTag[MEMIDX_MAIN_FOURTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK2_reg, MemTag[MEMIDX_MAIN_FOURTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK2_reg, MemTag[MEMIDX_MAIN_FOURTH].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_sub_fifth_buf_addr_reg, MemTag[MEMIDX_MAIN_FIFTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr1_reg, MemTag[MEMIDX_MAIN_FIFTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_reg, MemTag[MEMIDX_MAIN_FIFTH].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_sub_sixth_buf_addr_reg, MemTag[MEMIDX_MAIN_SIXTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, MemTag[MEMIDX_MAIN_SIXTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, MemTag[MEMIDX_MAIN_SIXTH].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_sub_seventh_buf_addr_reg, MemTag[MEMIDX_MAIN_SEVENTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, MemTag[MEMIDX_MAIN_SEVENTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, MemTag[MEMIDX_MAIN_SEVENTH].StartAddr + buffer_size);
    IoReg_Write32(MDOMAIN_DISP_sub_eightth_buf_addr_reg, MemTag[MEMIDX_MAIN_EIGHTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, MemTag[MEMIDX_MAIN_EIGHTH].StartAddr + shift_addr);
    IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, MemTag[MEMIDX_MAIN_EIGHTH].StartAddr + buffer_size);
}
...
static void memory_set_sub_for_pip(unsigned char display)
{
	...
	unsigned int fifoLen = _FIFOLENGTH2;
	unsigned int burstLen = _BURSTLENGTH_MDISPLAY_OUTSTANDING;
	...
	//rotate disable outstanding
	if((get_rotate_mode(display) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_0) || (get_rotate_mode(display) == DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
	{
		burstLen = _BURSTLENGTH_MDISPLAY_OUTSTANDING;
		mdisp_outstanding_ctrl(display, _ENABLE);//enable outstanding
	}
	else
	{
		burstLen = _BURSTLENGTH2;
		mdisp_outstanding_ctrl(display, _DISABLE);//disable outstanding
	}
...
#endif
    if (drv_memory_Get_multibuffer_flag()){
        set_sub_for_mdomain_2p_video_delay_address((ulLineStep + ulLineStep%2) << 3);
    }
...
//	unsigned int  TotalSizeFrame=0;
	...
	unsigned int burstLen = _BURSTLENGTH_MDISPLAY_OUTSTANDING;//use disaply outstanding burstlength
	...
	if ((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_HSTA) == 0) && (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_MEM_ACT_VSTA) == 0) )
	{
		...
		if((display == SLR_MAIN_DISPLAY) && (dvrif_memory_compression_get_enable(display) == TRUE)){
			...
			TotalSize = memory_get_capture_line_size_compression(display, dvrif_memory_get_compression_bits(display));
		}
		else{
			mdomain_disp_ddr_mainnumwtl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg);
	...
	else if((is_scaler_input_2p_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
		mdomain_disp_ddr_mainlinestep_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainLineStep_reg);
...
#endif
	{
		...
		if(Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000)>65000)
		{
		...
		if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000)>65000)
            ...
            || drvif_scaler3d_decide_3d_SG_data_FRC()
				...
				|| ( modestate_get_pr3d_in_fp_3d_mode() && ( ( Scaler_InputSrcGetMainChType() == _SRC_VO ) || ( Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000) >= V_FREQ_30000_pOFFSET ) ) )
				||Get_vscFilmMode()
			#endif
				)
		{
...
#else
	IoReg_Write32(/*display? MDOMAIN_DISP_DDR_SubCtrl_reg : */MDOMAIN_DISP_DDR_MainCtrl_reg,  temp1);
	drvif_memory_EnableMDisp_Or_DisableMDisp(SLR_MAIN_DISPLAY, _ENABLE);
#endif
	drvif_memory_set_dbuffer_write(display);
}
...
void memory_set_input_format(unsigned char display, unsigned char bDouble)
{
...
#endif
	{
		...
		if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) > ( Get_DISPLAY_REFRESH_RATE() * 1000 + 500 ) ) &&
			( display == SLR_MAIN_DISPLAY ) && !force_enable_mdomain_2disp_path() && !is_scaler_input_2p_mode(display) )
		{
	...
	if((display == SLR_SUB_DISPLAY) && (force_enable_mdomain_2disp_path()))
...
//	unsigned char lineRepeatEnable = FALSE;
	...
	unsigned int burstLen = _BURSTLENGTH_MCAP_OUTSTANDING;
	//Update the fifo depth for each IC @Crixus 20170418

	if((get_rotate_mode(display) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_0) || (get_rotate_mode(display) == DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
	{
		mcap_outstanding_ctrl(display, _ENABLE);//enable outstanding
	}
	else
	{
		burstLen = (display ?_BURSTLENGTH2: _BURSTLENGTH);
		mcap_outstanding_ctrl(display, _DISABLE);//disable outstanding
	}
	...
	drvif_memory_DisableMCap(display);	// disable Capture
#else
	...
	if(dvrif_memory_compression_get_enable(display) == TRUE)
	{
		...
		{
			TotalSize = memory_get_capture_line_size_compression(display, dvrif_memory_get_compression_bits(display));//compression test
		}
		else{
			TotalSize = memory_get_capture_frame_size_compression(display, dvrif_memory_get_compression_bits(display));
		}
	}
	else
	...
	if(Cap_type == MEMCAPTYPE_LINE){
		...
		if(force_enable_mdomain_2disp_path() || (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
...
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	drvif_memory_free_block(MEMIDX_MAIN_THIRD);
		...
		{
#ifdef CONFIG_ENABLE_3BUFFER_NEW_RULE
			if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000)>65000) && (display ==_CHANNEL1))
			{
		...
		if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000)>65000)
            ...
            || drvif_scaler3d_decide_3d_SG_data_FRC()
				|| ((drvif_scaler3d_decide_2d_cvt_3d_SG_frc_in_mDomain() == TRUE) && drvif_scaelr3d_decide_is_3D_display_mode())
				|| (Get_PANEL_3D_PR_TO_SG_OUTPUT_FRC_ENABLE() && drvif_scaelr3d_decide_is_3D_display_mode())
				|| ( modestate_get_pr3d_in_fp_3d_mode() && ( ( Scaler_InputSrcGetType(display) == _SRC_VO ) || ( Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000) >= V_FREQ_30000_pOFFSET ) ) )
				||Get_vscFilmMode()
				#endif
				)
		{
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		if((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) && (get_force_i3ddma_enable(SLR_MAIN_DISPLAY) == FALSE)) {
			...
			IoReg_Write32(MDOMAIN_CAP_DDR_In2_3rdAddr_reg, MemTag[MEMIDX_SUB_THIRD].StartAddr & 0xfffffff0);
			IoReg_Write32(MDOMAIN_CAP_DDR_In2_2ndAddr_reg, MemTag[MEMIDX_SUB_SEC].StartAddr & 0xfffffff0);
			IoReg_Write32(MDOMAIN_CAP_DDR_In2Addr_reg, MemTag[MEMIDX_SUB].StartAddr & 0xfffffff0);
		} else {
#ifndef BUILD_QUICK_SHOW
#ifdef ENABLE_3_BUFFER_DELAY_MODE
	...
	if(display){
		...
		if (((get_sub_m_domain_1st_buf_address() != NULL) && (get_sub_m_domain_2nd_buf_address() != NULL) && (get_sub_m_domain_3rd_buf_address() != NULL))
			|| ((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) && (get_panel_pixel_mode() > PANEL_1_PIXEL))) {
		...
		} else if ((get_sub_m_domain_1st_buf_address() != NULL) && (get_sub_m_domain_2nd_buf_address() != NULL)) {
			mdomain_cap_ddr_in2ctrl_reg.in2_double_enable = 1;
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
...
#endif
		{
			...
			if(vdec_data_frc_need_borrow_cma_buffer(display)){
				...
				rtd_pr_vsc_notice("##func:%s main m domain borrow memory from CMA size:0x%x ###\r\n", __FUNCTION__, borrow_size);
			    if(get_m_domain_borrow_buffer())
				{//alread has size from cma
					if(borrow_size <= get_m_domain_borrow_size())
					{//no need to borrow again
				...
				{//need borrrow memory from cam
					...
					{
						rtd_pr_vsc_err("###[Bug] func:%s no memory from cma ##\r\n",__FUNCTION__);
						return;
					}
				}

				if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
					CMA_reset_DCMT(MemTag[MEMIDX_MAIN].StartAddr, borrow_size, ADDITIONAL_CMA_DCMT_VDEC_DATA_FRC);
			}
			...
			IoReg_Mask32(MDOMAIN_CAP_DDR_In1LineNum_reg, 0xfffee000, (Cap_type<<16)|AND(Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN), 0x1FFF));
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		drvif_memory_free_block(MEMIDX_SUB_THIRD);	// free block befor getting a new one
		//sub use cap max size @Crixus 20151231
		if((is_scaler_input_2p_mode(SLR_SUB_DISPLAY)) || force_enable_mdomain_2disp_path())
		{
		...
		IoReg_Mask32(MDOMAIN_CAP_DDR_In2LineNum_reg, 0xfffee000, (Cap_type<<16)|AND(Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN), 0x1FFF));
	}
#endif
}
...
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	IoReg_ClearBits(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2Ctrl_reg: MDOMAIN_CAP_DDR_In1Ctrl_reg, MDOMAIN_CAP_DDR_In1Ctrl_in1_double_enable_mask);	// disable Capture
	drvif_memory_DisableMCap(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL));	// disable Capture
#else
	IoReg_ClearBits(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2Ctrl_reg : */MDOMAIN_CAP_DDR_In1Ctrl_reg, MDOMAIN_CAP_DDR_In1Ctrl_in1_double_enable_mask); // disable Capture
	drvif_memory_DisableMCap(SLR_MAIN_DISPLAY); // disable Capture
#endif
...
//-	rtd_pr_scaler_memory_debug("\n Calculate FIFO Size !");
	...
	drvif_memory_EnableMCap(Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL));
#else
	IoReg_Mask32(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2LineNum_reg : */MDOMAIN_CAP_DDR_In1LineNum_reg, 0xfffee000, (Cap_type<<16)|AND(Scaler_DispGetInputInfo(SLR_INPUT_CAP_LEN), 0x1FFF));
	IoReg_SetBits(/*Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) ? MDOMAIN_CAP_DDR_In2Ctrl_reg : */MDOMAIN_CAP_DDR_In1Ctrl_reg, _BIT0 | (bDouble ? MDOMAIN_CAP_DDR_In1Ctrl_in1_double_enable_mask : 0));
...
//	unsigned int  TotalSizeFrame;
	...
	mcap_outstanding_ctrl((Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) == _CHANNEL1) ? SLR_MAIN_DISPLAY : SLR_SUB_DISPLAY, _DISABLE);//disable outstanding
...
//	unsigned int  TotalSizeFrame=0;
	...
	if(force_enable_mdomain_2disp_path() || (is_scaler_input_2p_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
...
void memory_set_main_displaywindow_change(void)
{
			...
			|| ( modestate_get_pr3d_in_fp_3d_mode() && ( ( Scaler_InputSrcGetMainChType() == _SRC_VO ) || ( Scaler_DispGetInputInfoByDisp(display,SLR_INPUT_V_FREQ_1000) >= V_FREQ_30000_pOFFSET ) ) ) )
	{
		// Input source frame rate faster than display (main_frc_style = 1)
		// M-display IVS refer to front of VGIP signal (for HDMI FP 3D, IVS refer to the source v-sync)
		if((Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_V_FREQ_1000) > 65000) && (Scaler_Get3DMode((SCALER_DISP_CHANNEL)display) == SLR_3DMODE_2D))
			mainctrltemp |= _BIT3;
...
static void memory_set_sub_vflip_for_pip(unsigned char display)
{
    ...
    //if hfr mode, main and sub path capture same data, so sub path need set burst/lenth setting as main path
    if((SLR_SUB_DISPLAY == display) && ((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || force_enable_mdomain_2disp_path())){
        fifoLen = _FIFOLENGTH;
...
void set_sub_for_mdomain_2p_video_delay(void)
{
    mdomain_disp_ddr_subctrl_RBUS mdomain_disp_ddr_subctrl_reg;
    mdomain_disp_ddr_subctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg);
    mdomain_disp_ddr_subctrl_reg.sub_double_en = 1;
    mdomain_disp_ddr_subctrl_reg.sub_v_flip_3buf_en = 1;
    mdomain_disp_ddr_subctrl_reg.sub_bufnum = (drv_memory_Get_multibuffer_number() > 1) ? drv_memory_Get_multibuffer_number() - 1 : 0;
    mdomain_disp_ddr_subctrl_reg.sub_cap2disp_distance = (drv_memory_Get_multibuffer_number() > 2) ? drv_memory_Get_multibuffer_number() - 2 : 1;
    IoReg_Write32(MDOMAIN_DISP_DDR_SubCtrl_reg, mdomain_disp_ddr_subctrl_reg.regValue);
}
...
void dvrif_memory_handler(unsigned char display)
{
    ...
    {
        rtd_pr_scaler_memory_debug("display id is not ok\n");
        return;
    }
#else
	...
	if (display == SLR_MAIN_DISPLAY) {	// Main
		...
		if((is_scaler_input_2p_mode(display)) && (get_panel_pixel_mode() > PANEL_1_PIXEL)
#ifndef BUILD_QUICK_SHOW
                        && (Get_rotate_function(display) == FALSE)
#endif
       && decide_2step_uzu_merge_sub())
		{//run sub mdomain for two pixel mode only for 120 panel
			...
			IoReg_Write32(MDOMAIN_DISP_DDR_SubPreVStart_reg, mdomain_disp_ddr_subprevstart_Reg.regValue);
            if (drv_memory_Get_multibuffer_flag()){
                set_sub_for_mdomain_2p_video_delay();
            }
            drvif_memory_EnableMDisp_Or_DisableMDisp(SLR_SUB_DISPLAY, _ENABLE);
	...
	if (display == SLR_MAIN_DISPLAY) {
		...
		if(drv_memory_get_vdec_direct_low_latency_mode() && !((Get_DISPLAY_PANEL_TYPE() == P_LVDS_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI) || (Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_LVDS_TO_HDMI)))
			drv_memory_SingleBuffer_GameMode(_ENABLE);
		...
		if(((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_ADC)
			...
			&& (Scaler_DispGetInputInfoByDisp(display, SLR_INPUT_IPV_ACT_LEN_PRE) <= 1080)){
	...
	{
		// Enable Mdomain capture
		drvif_memory_EnableMCap(display);

		// Input fast case should enable Mdomain display first
		if(get_mdomain_input_fast_status(display) == TRUE)
		{
			rtd_pr_scaler_memory_info("[%s] %d %d channel:%d input fast case, enable Mdisp_en first\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), display);

#ifndef BUILD_QUICK_SHOW
			if(judge_scaler_break_case(display))
			{
				rtd_pr_scaler_memory_info("[%s] %d %d channel:%d condition change, don't enable Mdisp_en\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg), display);
			}
			else
#endif
			{
		...
		// Wait Mdomain buffer write done
		wait_m_cap_last_write_done(display);

		// Enable Mdomain display. It must wait Mdomain buffer write done, avoid buffer no valid data and PQDC decompress fail causing Mdisp request too much.
		// Just condiser input slow case now, input fast case no solution so far.
		if(get_mdomain_input_fast_status(display) == FALSE)
		{
#ifndef BUILD_QUICK_SHOW
			if(judge_scaler_break_case(display))
			{
		...
		if ((display == SLR_MAIN_DISPLAY)
#ifndef BUILD_QUICK_SHOW
         && ((TRUE != Get_rotate_function(SLR_MAIN_DISPLAY)) || (get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
#endif
            )
		{
		...
		rtd_pr_scaler_memory_emerg("[%s] %d ERROR! Don't enable Mcap_en & Mdisp_en. (display=%d, err_status=%d)\n", __FUNCTION__, __LINE__, display, get_m_domain_setting_err_status(display));
...
void drvif_memory_set_fs_display_fifo(void)
{
	...
	if((Scaler_DispGetInputInfo(SLR_INPUT_CHANNEL) == _CHANNEL1) ){
		if((get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_ROTATE_0) || (get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_DEFAULT))
		{
			mcap_outstanding_ctrl(SLR_MAIN_DISPLAY, _ENABLE);//enable outstanding
			mcap_outstanding_ctrl(SLR_SUB_DISPLAY, _ENABLE);//enable outstanding
		}
		else
		{
		    burstLen = _BURSTLENGTH;
			mcap_outstanding_ctrl(SLR_MAIN_DISPLAY, _DISABLE);//disable outstanding
			mcap_outstanding_ctrl(SLR_SUB_DISPLAY, _DISABLE);//disable outstanding
		}
	}

	//data fs pre read no effect, but SRNN needs to set it
	set_main_pre_read_v_start(drv_Calculate_m_pre_read_value());
	...
	if(is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
	{
		IoReg_Write32(MDOMAIN_DISP_Display_sub_byte_channel_swap_reg, IoReg_Read32(MDOMAIN_DISP_Display_byte_channel_swap_reg));
		IoReg_Write32(MDOMAIN_CAP_Capture2_byte_swap_reg, IoReg_Read32(MDOMAIN_CAP_Capture1_byte_swap_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_SubPixNumWTL_reg, IoReg_Read32(MDOMAIN_DISP_DDR_MainNumWTL_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_SubLineNum_reg, IoReg_Read32(MDOMAIN_DISP_DDR_MainLineNum_reg));
		IoReg_Write32(MDOMAIN_DISP_DDR_SubRdNumLenRem_reg, IoReg_Read32(MDOMAIN_DISP_DDR_MainRdNumLenRem_reg));
...
void drvif_memory_wait_dbuffer_write(unsigned char display)
{
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		...
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d ERROR!!! Main Mdisp db_apply timeout! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d ERROR!!! Main Mdisp db_apply timeout! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#endif
		}
		else
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d Main Mdisp db_apply success! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d Main Mdisp db_apply success! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#endif
		}
	}
	else if(display == SLR_SUB_DISPLAY)
	{
		...
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d ERROR!!! Sub Mdisp db_apply timeout! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d ERROR!!! Sub Mdisp db_apply timeout! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#endif
		}
		else
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d Sub Mdisp db_apply success! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d Sub Mdisp db_apply success! 7334=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg));
#endif
		}
	}
}
...
// it must be times of 4
	...
	IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg,0xfffffffc);//Set upper boundary of DDR address
#endif
	...
	//drvif_memory_EnableMCap(SLR_MAIN_DISPLAY); //Capture will enable after setting DMA mode
}
...
void dvrif_memory_comp_setting(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	rtd_pr_scaler_memory_info("[crixus]enable_compression = %d, comp_wid = %d, comp_len = %d, comp_ratio = %d\n",enable_compression, comp_wid, comp_len, comp_ratio);
	...
	*/
	...
	{
	// This reg. only set when PQC & PQDC is enable. It will drop pixel at every line end in non-32-pixel align frame width.
	// If this reg. is set when PQC & PQDC is disable. You will drop pixel from start and see video broken in begin.
		if((comp_wid % 32) != 0){
			//drop bits
	...
	{
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_color = 0; // RGB , pure RGB case
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_format = 0; // 444 , pure RGB case
	}
	else
	{//pc: force 444/non-pc: dynamic 444 - cmp_data_format = 0
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_color = 1; // YUV
		mdomain_cap_cap_pq_cmp_pair_reg.cmp_data_format = 0; // 444 out case
	}
	...
	if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
		//pc: force 444 - do_422_mode = 2, g_ratio = 12
		mdomain_cap_cap_pq_cmp_enable_reg.g_ratio = 12;
		mdomain_cap_cap_pq_cmp_enable_reg.do_422_mode = 2;
	}
	else{
		//non-pc: dynamic 444 - do_422_mode = 0, g_ratio = 14
		mdomain_cap_cap_pq_cmp_enable_reg.g_ratio = 14;
		mdomain_cap_cap_pq_cmp_enable_reg.do_422_mode = 0;
	}
	...
	//RGB444 setting for PQC @Crixus 20170718
	if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP) == FALSE)){
		///pc: force 444 - dynamic_allocate_ratio_max = 13, dynamic_allocate_ratio_min = 9
		...
		//non-pc: dynamic 444 - dynamic_allocate_ratio_max = 16, dynamic_allocate_ratio_min = 13
	...
	if(get_vpq_check_pc_rgb444_mode_flag() == _ENABLE)
	{
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_data_color = 0; // RGB , pure RGB case
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_data_format = 0; // 444 , pure RGB case
	}
	else
	{//pc: force 444/non-pc: dynamic 444 - cmp_data_format = 0
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_data_color = 1; // YUV
		mdomain_disp_dispm_main_pq_decmp_pair_reg.decmp_data_format = 0;// 444 out case
	}
	...
	if(enable_compression == TRUE){
		...
		*/
		...
		if((get_vpq_check_pc_rgb444_mode_flag() == _ENABLE) &&
			((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) && (get_panel_pixel_mode() > PANEL_1_PIXEL)
#ifndef BUILD_QUICK_SHOW
			&& (Get_rotate_function(SLR_MAIN_DISPLAY) == FALSE)
#endif
		))
		{
		// Set Mcap sub path setting in pure RGB mode
		// Only scaler input 2p mode case, need to set pure RGB color format setting
			mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_color = 0; // RGB , pure RGB case
			mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_format = 0; // 444 , pure RGB case
		}
		else
		{
			mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_color = 1; // YUV
		...
		if((get_vpq_check_pc_rgb444_mode_flag() == _ENABLE) &&
			((force_enable_mdomain_2disp_path()) ||
			((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) && (get_panel_pixel_mode() > PANEL_1_PIXEL)
#ifndef BUILD_QUICK_SHOW
			&& (Get_rotate_function(SLR_MAIN_DISPLAY) == FALSE)
#endif
		...
		{
		// Set Mdisp sub path setting in pure RGB mode
		// Only when Mdisp sub path is from main path source (vrr case or scaler input 2p mode case), need to set pure RGB color format setting
		// If Mdisp sub path is NOT from main path source, no need to set pure RGB color format setting
			mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_color = 0; // RGB , pure RGB case
			mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_format = 0; // 444 , pure RGB case
		}
		else
		{
			mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_color = 1; // YUV
			if(Scaler_DispGetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP) == FALSE)
			{
				if(pc_mode_run_422())
				{
...
unsigned int drv_memory_blk_decontour_Config(unsigned int phyAddr)// ml8_add
{
	...
	#define BLK_DECONT_DMA_SIZE (34*1024) // ((blk_h * blk_w * info_bit)>>3) = ((103*54*40)>>3) = 27810 (28*1024) //ml8p: ((103*54*49)>>3) =34068 (34*1024)
#ifndef UT_flag
	...
	bdecont_dma_rd_ctrl_reg.rd_max_outstanding = 3; // dic: set max
	bdecont_dma_wr_ctrl_reg.wr_max_outstanding = 1; // dic: to avoid BW issue?
...
char drv_memory_MA_SNR_Enable_Check(unsigned char access ,unsigned int phyAddr, unsigned char DI_MEM_IDX, unsigned int DI_Width, unsigned int DI_Height)
{
	...
	if ((get_mach_type()!=MACH_ARCH_RTK2885P) || (decide_by_AP == 0) || ((vsc_src == VSC_INPUTSRC_VDEC || vsc_src == VSC_INPUTSRC_CAMERA) && (adaptvie_stream_flag >= ADAPTIVE_STREAM_ON)) || ((DISP_THRIP  == FALSE) && (DISP_RTNR == FALSE))) {
		ret = 0;
	} else {
		ret = 1;
	}
...
bool get_mdomain_input_fast_status(unsigned char display)
{
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		input_fast_flag = MDOMAIN_DISP_DDR_MainCtrl_get_main_frc_style(IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg));
	}
	else if(display == SLR_SUB_DISPLAY)
	{
		input_fast_flag = MDOMAIN_DISP_DDR_SubCtrl_get_sub_frc_style(IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg));
	}
...
void wait_m_cap_last_write_done(unsigned char display)
{
	...
	if (display == SLR_MAIN_DISPLAY)
	{
		...
		if ((Scaler_DispGetInputInfo(SLR_INPUT_FRAMESYNC) == FALSE) && ((TRUE != Get_rotate_function(SLR_MAIN_DISPLAY)) || (get_rotate_mode(SLR_MAIN_DISPLAY) == DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
		{
...
void drv_memory_wait_disp_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case)
{
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		do
		{
			if(0 == MDOMAIN_DISP_DDR_MainCtrl_get_main_double_en(IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg)))
			{
			...
			count = 40;
			pre_disp_idx = MDOMAIN_DISP_DDR_MainStatus_get_main_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_MainStatus_reg));
			...
			cur_disp_idx = MDOMAIN_DISP_DDR_MainStatus_get_main_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_MainStatus_reg));
			rtd_pr_scaler_memory_info("[%s] %d M domain main done-1, disp pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_disp_idx, cur_disp_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))));
			...
			if(cur_disp_idx == pre_disp_idx)
			{
				...
				while((MDOMAIN_DISP_DDR_MainStatus_get_main_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_MainStatus_reg)) == pre_disp_idx) && --count)
				{
					if(game_mode_case && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))
					{
						rtd_pr_scaler_memory_info("[%s] %d game mode case not active return\n", __FUNCTION__, __LINE__);
	...
	else
	{
		do
		{
			if(0 == MDOMAIN_DISP_DDR_SubCtrl_get_sub_double_en(IoReg_Read32(MDOMAIN_DISP_DDR_SubCtrl_reg)))
			{
				rtd_pr_scaler_memory_info("[%s(%d)] single buffer no need wait block status\n", __FUNCTION__, __LINE__);
				return;
			}
			...
			count = 40;
			pre_disp_idx = MDOMAIN_DISP_DDR_SubStatus_get_sub_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_SubStatus_reg));
			...
			cur_disp_idx = MDOMAIN_DISP_DDR_SubStatus_get_sub_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_SubStatus_reg));
			rtd_pr_scaler_memory_info("[%s] %d M domain sub done-1, disp pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_disp_idx, cur_disp_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))));
			...
			if(cur_disp_idx == pre_disp_idx)
			{
				...
				while((MDOMAIN_DISP_DDR_SubStatus_get_sub_block_select(IoReg_Read32(MDOMAIN_DISP_DDR_SubStatus_reg)) == pre_disp_idx) && --count)
				{
#ifdef BUILD_QUICK_SHOW
					msleep(10);//no busy delay 10 ms
#else
...
void drv_memory_wait_cap_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case)
{
	...
	if(get_mdomain_input_fast_status(display) == TRUE)
	{
		//Get original Mdisp db_en and pqdc_en status
		unsigned char original_mdisp_db_en = (display? MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_enable(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg)): MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_enable(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg)));
		unsigned char original_pqdc_en = (display? MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_get_s_decmp_en(IoReg_Read32(MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_reg)): MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_get_decmp_en(IoReg_Read32(MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg)));
		...
		rtd_pr_scaler_memory_info("[%s] %d Input fast case, wait disp last write done. (display:%d, wait_buff:%d, ori_pqdc_en:%d, ori_mdisp_db_en:%d)\n", __FUNCTION__, __LINE__, display, wait_frame, original_pqdc_en, original_mdisp_db_en);

		//PQDC decompression needs valid data which is written by PQC.
		//If PQDC decompression uses invalid data, it will make PQDC something wrong and Mdisp buffer index maybe not change.
		//So, it needs to disable pqdc_en before drv_memory_wait_disp_last_write_done.
		//Only pqdc_en needs to disable, pqc_en no needs to disable.
		if(original_pqdc_en)
		{
			//Disable de-compression
			IoReg_ClearBits((display? MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_reg: MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg), (display? MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_s_decmp_en_mask: MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_decmp_en_mask));

			//Double buffer apply
			if(original_mdisp_db_en)
				drvif_memory_set_dbuffer_write(display);
		}

		//Wait Mdisp buffer index change
		drv_memory_wait_disp_last_write_done(display, wait_frame, game_mode_case);

		//Recover de-compression
		if(original_pqdc_en)
		{
			//Enable de-compression
			IoReg_SetBits((display? MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_reg: MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_reg), (display? MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_s_decmp_en_mask: MDOMAIN_DISP_DISPM_MAIN_PQ_DECMP_decmp_en_mask));
			...
			if(original_mdisp_db_en)
			{
				...
				//Wait double buffer apply
				drvif_memory_wait_dbuffer_write(display);
			}
		}
	...
	if(display == SLR_MAIN_DISPLAY)
	{
		do
		{
			if(0 == MDOMAIN_CAP_DDR_In1Ctrl_get_in1_double_enable(IoReg_Read32(MDOMAIN_CAP_DDR_In1Ctrl_reg)))
			{
				rtd_pr_scaler_memory_info("[%s(%d)] single buffer no need wait block status\n", __FUNCTION__, __LINE__);
				return;
			}
			...
			count = 40;
			while(count && (MDOMAIN_CAP_Cap_DDR_FIFOStatus_get_in1_cap_first_wr_flag(IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg)) == 0))
			{
#ifdef BUILD_QUICK_SHOW
				msleep(10);//no busy delay 10 ms
#else
			...
			while((MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))) && --count)
			{
                if ((MDOMAIN_CAP_Cap_In1_enable_get_in1_cap_enable(IoReg_Read32(MDOMAIN_CAP_Cap_In1_enable_reg)) == 0) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))
                {
			...
			rtd_pr_scaler_memory_info("[%s] %d M domain main done-1, cap pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_cap_idx, cur_cap_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))));
			...
			if(cur_cap_idx == pre_cap_idx)
			{
				...
				while((MDOMAIN_CAP_DDR_In1Status_get_in1_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In1Status_reg)) == pre_cap_idx) && --count)
				{
					if(game_mode_case && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))
					{
						rtd_pr_scaler_memory_info("[%s] %d game mode case not active return\n", __FUNCTION__, __LINE__);
	...
	else
	{
		do
		{
			if(0 == MDOMAIN_CAP_DDR_In2Ctrl_get_in2_double_enable(IoReg_Read32(MDOMAIN_CAP_DDR_In2Ctrl_reg)))
			{
				rtd_pr_scaler_memory_info("[%s(%d)] single buffer no need wait block status\n", __FUNCTION__, __LINE__);
				return;
			}
			...
			count = 40;
			while(count && (MDOMAIN_CAP_Cap_DDR_FIFOStatus_get_in2_cap_first_wr_flag(IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg)) == 0))
			{
				msleep(0);//no busy delay 10 ms
				count --;
			}
			...
			rtd_pr_scaler_memory_info("[%s] %d M domain sub done-1, cap pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_cap_idx, cur_cap_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg))));
			...
			if(cur_cap_idx == pre_cap_idx)
			{
				...
				while((MDOMAIN_CAP_DDR_In2Status_get_in2_block_select(IoReg_Read32(MDOMAIN_CAP_DDR_In2Status_reg)) == pre_cap_idx) && --count)
				{
#ifdef BUILD_QUICK_SHOW
					msleep(10);//no busy delay 10 ms
#else
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	//if((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || (force_enable_mdomain_2disp_path()))
	if(get_mach_type() == MACH_ARCH_RTK2885P && ((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || (force_enable_mdomain_2disp_path())))
    {
#ifndef BUILD_QUICK_SHOW
...
//  T1 = VO_Htotal * ( VO_Vsta + N ) / VO_clk
//  T2 = ( iv2dv_Htotal * iv2dv_line / iv2dv_clk ) + ( D_Htotal * D_Vsta / D_clk )
//  T3 = VO_Htotal * ( VO_Vend + N ) / VO_clk
//  T4 = ( iv2dv_Htotal * iv2dv_line / iv2dv_clk ) + ( D_Htotal * D_Vend / D_clk )
//  Note : iv2dv_Htotal = VO_Htotal, iv2dv_clk = VO_clk
//  ========================================================================
    ...
    unsigned int hdmi_vstart = Scaler_ModeGet_Hdmi_ModeInfo(SLR_MODE_IVSTARTPOS);
    unsigned int voClock;
    unsigned int voHtotal;
    unsigned int voVactive;
    unsigned int voVstart;
    unsigned int snrVstart;
    unsigned int vfreq = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000);
    ...
    unsigned int vstart = Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IPV_ACT_STA_PRE);
    unsigned int dCLK;
    unsigned int frac_a;
    unsigned int frac_b;
    unsigned int dHtotal;
    unsigned int dDen_start;
    unsigned int mdisp_preRead;
    unsigned int dMv_act;
    unsigned int dMv_start;
    unsigned int iv2dv_delay;
    unsigned int iv2dv_delay1;
    unsigned int iv2dv_delay2;
    unsigned int temp1;
    unsigned int temp2;
    unsigned int temp3;
    unsigned int orbit_shift_range;
    unsigned int orbit_front;

//    rtd_pr_vbe_notice("[%s] hdmi_vfreq = %d\n", __FUNCTION__, hdmi_vfreq);
//    rtd_pr_vbe_notice("[%s] hdmi_htotal = %d\n", __FUNCTION__, hdmi_htotal);
//    rtd_pr_vbe_notice("[%s] hdmi_vtotal = %d\n", __FUNCTION__, hdmi_vtotal);
//    rtd_pr_vbe_notice("[%s] hdmi_vstart = %d\n", __FUNCTION__, hdmi_vstart);
//    rtd_pr_vbe_notice("[%s] vfreq = %d\n", __FUNCTION__, vfreq);
//    rtd_pr_vbe_notice("[%s] htotal = %d\n", __FUNCTION__, htotal);
//    rtd_pr_vbe_notice("[%s] vtotal = %d\n", __FUNCTION__, vtotal);
//    rtd_pr_vbe_notice("[%s] vstart = %d\n", __FUNCTION__, vstart);
    ...
    pVOInfo = Scaler_VOInfoPointer(VO_VIDEO_PLANE_V1);

    if( !get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && ( Scaler_InputSrcGetType(SLR_MAIN_DISPLAY) !=_SRC_VO ) )
    {
        if( ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI ) && ( ( hdmi_vfreq == 0 ) || ( hdmi_vtotal == 0 ) || ( hdmi_htotal == 0 ) ) )
        {
            rtd_pr_scaler_memory_emerg("[%s][ERROR] Invalid HDMI Info!!!!\n", __FUNCTION__);
            return 0;
        }
        else if( ( vfreq == 0 ) || ( vtotal == 0 ) || ( htotal == 0 ) )
        {
            rtd_pr_scaler_memory_emerg("[%s][ERROR] Invalid Input Info!!!!\n", __FUNCTION__);
            return 0;
        }
    }
    else if( ( pVOInfo == NULL ) || ( pVOInfo->h_total == 0 ) )
    {
        rtd_pr_scaler_memory_emerg("[%s][ERROR] Invalid VOInfo!!!!\n", __FUNCTION__);
        return 0;
    }
...
//    rtd_pr_vbe_notice("[%s] snrVstart = %d\n", __FUNCTION__, snrVstart);
...
//            voClock = (unsigned int)( (unsigned long long)hdmi_htotal * (unsigned long long)hdmi_vtotal * (unsigned long long)hdmi_vfreq / 1000ULL );
            voClock = (unsigned int)div64_u64( ( (unsigned long long)hdmi_htotal * (unsigned long long)hdmi_vtotal * (unsigned long long)hdmi_vfreq ), 1000ULL );
        }
        else
        {
//            voClock = (unsigned int)( (unsigned long long)htotal * (unsigned long long)vtotal * (unsigned long long)vfreq / 1000ULL );
            voClock = (unsigned int)div64_u64( ( (unsigned long long)htotal * (unsigned long long)vtotal * (unsigned long long)vfreq ), 1000ULL );
        }
    }
    else
    {
...
//    rtd_pr_vbe_notice("[%s] voClock = %d\n", __FUNCTION__, voClock);
//    rtd_pr_vbe_notice("[%s] voHtotal = %d\n", __FUNCTION__, voHtotal);
//    rtd_pr_vbe_notice("[%s] voVactive = %d\n", __FUNCTION__, voVactive);
//    rtd_pr_vbe_notice("[%s] voVstart = %d\n", __FUNCTION__, voVstart);
    ...
    if((get_panel_pixel_mode() > PANEL_1_PIXEL) && (PPOVERLAY_uzudtg_control1_get_uzu_4k_120_mode(rtd_inl(PPOVERLAY_uzudtg_control1_reg)) || is_vrr_or_freesync_mode()))   // 120hz panel run 2pixel mode, dCLK need multiple 2 for calculate
    {
        dCLK *= 2;
    }
    else
    {
        ...
        dCLK = ( dCLK * frac_a ) / frac_b;
    }

    rtd_pr_vbe_notice("[%s] dCLK = %d\n", __FUNCTION__, dCLK);
//    rtd_pr_vbe_notice("[%s] frac_a = %d\n", __FUNCTION__, frac_a);
//    rtd_pr_vbe_notice("[%s] frac_b = %d\n", __FUNCTION__, frac_b);
//    rtd_pr_vbe_notice("[%s] dHtotal = %d\n", __FUNCTION__, dHtotal);
//    rtd_pr_vbe_notice("[%s] dDen_start = %d\n", __FUNCTION__, dDen_start);
//    rtd_pr_vbe_notice("[%s] dMv_act = %d\n", __FUNCTION__, dMv_act);
//    rtd_pr_vbe_notice("[%s] dMv_start = %d\n", __FUNCTION__, dMv_start);
...
//    rtd_pr_vbe_notice("[%s] mdisp_preRead = %d\n", __FUNCTION__, mdisp_preRead);
...
//  Formula :
//  T2 >= T1
//  => ( iv2dv_Htotal * iv2dv_line / iv2dv_clk ) + ( D_Htotal * D_Vsta / D_clk ) >= VO_Htotal * ( VO_Vsta + N ) / VO_clk
//  => ( voHtotal * iv2dv_delay1 / voClock ) + ( dHtotal * mdisp_preRead / dCLK ) >= voHtotal * ( voVstart + IDOMAIN_IP_DELAY_MARGIN ) / voClock
//  => iv2dv_delay1 >= ( voVstart + IDOMAIN_IP_DELAY_MARGIN ) - ( dHtotal / voHtotal * voClock / dCLK * mdisp_preRead )
//  Note : temp1 = dHtotal / voHtotal, temp2 = voClock / dCLK, temp3 = temp1 * temp2
//  Note : iv2dv_delay >= 0
...
//    rtd_pr_vbe_notice("[%s] temp1 = %d\n", __FUNCTION__, temp1);
//    rtd_pr_vbe_notice("[%s] temp2 = %d\n", __FUNCTION__, temp2);
//    rtd_pr_vbe_notice("[%s] temp3 = %d\n", __FUNCTION__, temp3);
...
//    rtd_pr_vbe_notice("[%s] iv2dv_delay1 = %d\n", __FUNCTION__, iv2dv_delay1);
...
//  Formula :
//  T4 >= T3
//  => ( iv2dv_Htotal * iv2dv_line / iv2dv_clk ) + ( D_Htotal * D_Vend / D_clk ) >= VO_Htotal * ( VO_Vend + N ) / VO_clk
//  => ( voHtotal * iv2dv_delay2 / voClock ) + ( dHtotal * ( dDen_start + dMv_start + dMv_act ) / dCLK ) >= voHtotal * ( voVstart + voVactive + IDOMAIN_IP_DELAY_MARGIN ) / voClock
//  => iv2dv_delay2 >= ( voVstart + voVactive + IDOMAIN_IP_DELAY_MARGIN ) - ( dHtotal / voHtotal * voClock / dCLK * ( dDen_start + dMv_start + dMv_act ) )
//  Note : temp1 = dHtotal / voHtotal, temp2 = voClock / dCLK, temp3 = temp1 * temp2
//  Note : iv2dv_delay >= 0

    if( ( ( voVstart + voVactive + IDOMAIN_IP_DELAY_MARGIN ) * 100000 ) >= ( temp3 * ( dDen_start + dMv_start + dMv_act ) ) )
    {
...
//    rtd_pr_vbe_notice("[%s] iv2dv_delay2 = %d\n", __FUNCTION__, iv2dv_delay2);
...
void drv_GameMode_iv2dv_delay(unsigned char enable)
{
    ...
    if(enable)
    {
        ...
        if( ( get_panel_pixel_mode() > PANEL_1_PIXEL ) && get_vsc_src_is_hdmi_or_dp() && ( vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag() ) )
            return;
    }
    else
    {
        // do not change the value, it's used for resetting mode
        id2dv_delay = 2;
    }
    ...
    if(drv_memory_get_game_mode_iv2dv_slow_enable())
    {
        if( id2dv_delay != 2 )
        {
    ...
    if(data_path_select_reg.s1_clk_en)  // if no s1 clock no need check apply
    {
        timeout = 0x3FFFFF;
        ppoverlay_iv2dv_double_buffer_ctrl_reg.regValue = IoReg_Read32(PPOVERLAY_iv2dv_Double_Buffer_CTRL_reg);
...
//	unsigned long flags;//for spin_lock_irqsave
    ...
    // multiview with 1p HFR input always go timing fsync
    if( multiview_1p_input_in_2p_panel() && jude_input_timing_is_hfr(SLR_MAIN_DISPLAY) )
    {
        return;
    }
...
void drv_memory_capture_multibuffer_update_addr(unsigned int buffer_size, unsigned int start_addr){
	...
	if(carvedout_enable() == 0) // do NOT use carvedout memory, use CMA malloc
	{
		dcmt_cma_mdomain_unset_without_deferfree();
		rtd_pr_scaler_memory_emerg("[%s] %d %d multibuffer clear all DCMT setting.\n", __FUNCTION__, __LINE__, IoReg_Read32(TIMER_SCPU_CLK90K_LO_reg));
	}
#endif
...
void drv_memory_cap_multibuffer_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk0_sel = 0;
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk1_sel = 1;
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk2_sel = 2;
        mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk3_sel = 3;
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk4_sel = 4;
		mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk5_sel = 5;
        mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk6_sel = 6;
        mdomain_cap_capmain_boundaryaddr_ctrl2_reg.in1_blk7_sel = 7;
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_CTRL2_reg, mdomain_cap_capmain_boundaryaddr_ctrl2_reg.regValue);

		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr1_reg, (MemTag[MEMIDX_MAIN].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_reg, (MemTag[MEMIDX_MAIN].StartAddr+buffer_size) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size) & 0xfffffff0);//need cut burstLen
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_MAIN_FOURTH].StartAddr & 0xfffffff0));
		IoReg_Write32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_MAIN_FOURTH].StartAddr+buffer_size) & 0xfffffff0);//need cut burstLen
...
void drv_memory_disp_multibuffer_limit_boundary_setting(unsigned char display, unsigned int buffer_size){
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk0_sel = 0;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk1_sel = 1;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk2_sel = 2;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk3_sel = 3;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk4_sel = 4;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk5_sel = 5;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk6_sel = 6;
		mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.main_blk7_sel = 7;
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_CTRL2_reg, mdomain_disp_dispmain_boundaryaddr_ctrl2_reg.regValue);

        IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr1_reg, (MemTag[MEMIDX_MAIN_FIFTH].StartAddr) & 0xfffffff0);
        IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_reg, (MemTag[MEMIDX_MAIN_FIFTH].StartAddr+buffer_size) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK0_reg, (MemTag[MEMIDX_MAIN_SIXTH].StartAddr) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK0_reg, (MemTag[MEMIDX_MAIN_SIXTH].StartAddr+buffer_size) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK1_reg, (MemTag[MEMIDX_MAIN_SEVENTH].StartAddr) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK1_reg, (MemTag[MEMIDX_MAIN_SEVENTH].StartAddr+buffer_size) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK2_reg, (MemTag[MEMIDX_MAIN_EIGHTH].StartAddr) & 0xfffffff0);
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK2_reg, (MemTag[MEMIDX_MAIN_EIGHTH].StartAddr+buffer_size) & 0xfffffff0);
	}
}

void drv_memory_capture_multibuffer_update_boundary(unsigned char buf_num, unsigned int buffer_size){
    rtd_pr_scaler_memory_emerg("drv_memory_capture_multibuffer_update_boundary buf_num=%d\n",buf_num);
    drv_memory_cap_multibuffer_limit_boundary_setting(SLR_MAIN_DISPLAY, buffer_size);
    drv_memory_disp_multibuffer_limit_boundary_setting(SLR_MAIN_DISPLAY, buffer_size);
}
...
static void memory_set_capture_i2rnd_sdisp(unsigned char cap_type)
{
	...
	rtd_pr_scaler_memory_emerg("SLR_INPUT_V_FREQ_1000 = 0x%x\n", Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000));
#endif
	...
	if(dvrif_memory_compression_get_enable(SLR_MAIN_DISPLAY) == TRUE){
		...
		TotalSize = memory_get_capture_line_size_compression(display, dvrif_memory_get_compression_bits(SLR_MAIN_DISPLAY));
	}
	else{
		//FIXME test
...
#if 0
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK0_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK0_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK1_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK1_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr_BLK2_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispMain_BoundaryAddr2_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapMain_BoundaryAddr2_BLK2_reg)& 0xfffffff0));
#endif
	}
	else if(display == SLR_SUB_DISPLAY){
...
#if 0
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK0_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK0_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK0_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK1_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK1_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK1_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr_BLK2_reg)& 0xfffffff0));
		IoReg_Write32(MDOMAIN_DISP_DispSub_BoundaryAddr2_BLK2_reg, (IoReg_Read32(MDOMAIN_CAP_CapSub_BoundaryAddr2_BLK2_reg)& 0xfffffff0));
#endif
	}
}
...
{//return true: use
    //merlin8/8p mdomain one-bin
    //if((get_scaler_run_hfr_mode()) && (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)))
    if((get_scaler_run_hfr_mode()) && (get_mach_type() == MACH_ARCH_RTK2885P && is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)))
...
void drv_memory_display_check_sub_frc_style(void)
{
	...
	if (!get_sub_OutputVencMode())
	{
		...
		if( !force_enable_mdomain_2disp_path() && !is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) &&
...
void set_main_pre_read_v_start(unsigned int m_pre_read)
{
	...
	mdomain_disp_ddr_mainprevstart_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainPreVStart_reg);
	rtd_pr_scaler_memory_info("[%s] change main_pre_rd_v_start from %d -> %d\n", __FUNCTION__, mdomain_disp_ddr_mainprevstart_reg.main_pre_rd_v_start, m_pre_read);
	mdomain_disp_ddr_mainprevstart_reg.main_pre_rd_v_start = m_pre_read;
	IoReg_Write32(MDOMAIN_DISP_DDR_MainPreVStart_reg, mdomain_disp_ddr_mainprevstart_reg.regValue);

	ddr_mainsubctrl_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg);
	if( ddr_mainsubctrl_reg.disp1_double_enable == 1 )
	{
		ddr_mainsubctrl_reg.disp1_double_apply = 1;
		IoReg_Write32(MDOMAIN_DISP_DDR_MainSubCtrl_reg,ddr_mainsubctrl_reg.regValue);
	}
...
void drvif_memory_EnableMDisp_Or_DisableMDisp(unsigned char display, bool enable)
{
	...
	if(enable == _ENABLE)
	{
		...
		IoReg_Write32(display? MDOMAIN_DISP_Disp_sub_enable_reg: MDOMAIN_DISP_Disp_main_enable_reg, display? MDOMAIN_DISP_Disp_sub_enable_sub_disp_en_mask: MDOMAIN_DISP_Disp_main_enable_main_disp_en_mask);
	...
	else
	{
		...
		{
			//disable sub disp
			IoReg_ClearBits(MDOMAIN_DISP_Disp_sub_enable_reg, MDOMAIN_DISP_Disp_sub_enable_sub_disp_en_mask);
	...
	{
		//enable main disp
		IoReg_Write32(MDOMAIN_DISP_Disp_main_enable_reg, MDOMAIN_DISP_Disp_main_enable_main_disp_en_mask);
...
void drvif_memory_EnableMCap(unsigned char display)
{
...
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
	IoReg_SetBits(display ? MDOMAIN_CAP_Cap_In2_enable_reg : MDOMAIN_CAP_Cap_In1_enable_reg, display? MDOMAIN_CAP_Cap_In2_enable_in2_cap_enable_mask: MDOMAIN_CAP_Cap_In1_enable_in1_cap_enable_mask);
#else
	...
	if (display == SLR_MAIN_DISPLAY)
	{
		...
		cap_ddr_fifostatus_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg);
		cap_ddr_fifostatus_reg.in1_cap_last_wr_flag = 1;
		cap_ddr_fifostatus_reg.in1_cap_first_wr_flag = 1;
		IoReg_Write32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg, cap_ddr_fifostatus_reg.regValue);
	}
	else
	{
		...
		cap_ddr_fifostatus_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg);
		cap_ddr_fifostatus_reg.in2_cap_last_wr_flag = 1;
		cap_ddr_fifostatus_reg.in2_cap_first_wr_flag = 1;
		IoReg_Write32(MDOMAIN_CAP_Cap_DDR_FIFOStatus_reg, cap_ddr_fifostatus_reg.regValue);
	}
}
...
void drvif_memory_DisableMCap(unsigned char display)
{
	...
	if (SLR_MAIN_DISPLAY == display)
	{
		...
		{
			rtd_pr_scaler_memory_info("Clear cap1 burst length and remain\n");

			//1. enable mcap db
			mdomain_cap_cap_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg);
			mdomain_cap_cap_reg_doublbuffer_reg.cap1_db_en = 1;
			...
			{
				// disable DB
				rtd_pr_scaler_memory_info("Wait cap1, db apply timeout - 1rtk_dc_mt!  disable db, 0xb802726c=%x\n", IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg));

				mdomain_cap_cap_reg_doublbuffer_reg.cap1_db_en = 0;
				IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_reg, mdomain_cap_cap_reg_doublbuffer_reg.regValue);
			}


			//4. Disable MCap
			rtd_pr_scaler_memory_info("Disable mcap\n");
            mdomain_cap_cap_in1_enable_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_In1_enable_reg);
	...
	else
	{
		...
		{
			rtd_pr_scaler_memory_info("Clear cap2 burst length and remain\n");

			//1. enable mcap db, enable vi db
			mdomain_cap_cap_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg);
			mdomain_cap_cap_reg_doublbuffer_reg.cap2_db_en = 1;
			IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_reg, mdomain_cap_cap_reg_doublbuffer_reg.regValue);
			...
			if (ulTimeout >= 25)
			{
				...
				mdomain_cap_cap_reg_doublbuffer_reg.cap2_db_en = 0;
				IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_reg, mdomain_cap_cap_reg_doublbuffer_reg.regValue);
			}


			// Disable MCap
			rtd_pr_scaler_memory_info("Disable mcap2\n");
            mdomain_cap_cap_in2_enable_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Cap_In2_enable_reg);
...
void drvif_sub_mdomain_for_srnn(unsigned char enable)
{
    mdomain_disp_ddr_subprevstart_RBUS mdomain_disp_ddr_subprevstart_Reg;
    mdomain_disp_ddr_mainprevstart_RBUS mdomain_disp_ddr_mainprevstart_reg;
    unsigned char subSrnnEnable = 0;

    subSrnnEnable = Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_SUB_SNRR_ENABLE);

    if((enable == 1) && (subSrnnEnable == 1))
    {
        ...
        drv_memory_set_limit_boundary_mode(SLR_SUB_DISPLAY, drv_memory_get_limit_boundary_mode(SLR_MAIN_DISPLAY));
        Scaler_DispSetStatus(SLR_SUB_DISPLAY, SLR_DISP_422CAP, Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_422CAP));
        set_m_domain_setting_err_status(SLR_SUB_DISPLAY, MDOMAIN_SETTING_NO_ERR); // reset M domain setting error status

        if(dvrif_memory_compression_get_enable(SLR_SUB_DISPLAY))
        {
            if(dvrif_memory_get_compression_mode(SLR_SUB_DISPLAY) == COMPRESSION_LINE_MODE)
            {
                memory_set_capture(TRUE, MEMCAPTYPE_LINE, SLR_SUB_DISPLAY);
        ...
        mdomain_disp_ddr_mainprevstart_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainPreVStart_reg);
        mdomain_disp_ddr_subprevstart_Reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_SubPreVStart_reg);
        mdomain_disp_ddr_subprevstart_Reg.sub_pre_rd_v_start = mdomain_disp_ddr_mainprevstart_reg.main_pre_rd_v_start;
        IoReg_Write32(MDOMAIN_DISP_DDR_SubPreVStart_reg, mdomain_disp_ddr_subprevstart_Reg.regValue);
        if(get_m_domain_setting_err_status(SLR_SUB_DISPLAY) == MDOMAIN_SETTING_NO_ERR)
            drvif_memory_EnableMCap(SLR_SUB_DISPLAY);
        else
            rtd_pr_scaler_memory_emerg("[%s] %d ERROR! Don't enable Mcap_en. (display=%d, err_status=%d)\n", __FUNCTION__, __LINE__, SLR_SUB_DISPLAY, get_m_domain_setting_err_status(SLR_SUB_DISPLAY));
        drvif_memory_EnableMDisp_Or_DisableMDisp(SLR_SUB_DISPLAY, _ENABLE);
        rtd_pr_scaler_memory_info("[%s %d]sub mdomain for srnn enable:%d, wid:%d, lenth:%d\n", __FUNCTION__, __LINE__, enable, Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_CAP_WID), Scaler_DispGetInputInfoByDisp(SLR_SUB_DISPLAY, SLR_INPUT_CAP_LEN));
    }
    else
    {
        drvif_memory_DisableMCap(SLR_SUB_DISPLAY);
        ...
        rtd_pr_scaler_memory_info("[%s %d]sub mdomain disable for srnn\n", __FUNCTION__, __LINE__);
    }
}
#ifdef BUILD_QUICK_SHOW
