	...
	if(!Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_FRAMESYNC)) {
		if(bFreeze){
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_CHANNEL) ? nonlibdma_set_cap_inctrl_reg_in_freeze_enable(1, 1) : nonlibdma_set_cap_inctrl_reg_in_freeze_enable(0, 1);
			}
#else
			//merlin8/8p mdomain one-bin
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				//IoReg_SetBits(MDOMAIN_CAP_DDR_In1Ctrl_reg, _BIT1);
				nonlibdma_set_cap_inctrl_reg_in_freeze_enable(0, 1);
			}
#endif
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				display ? nodlibamd_checkBit_cap_instatus_freeze_ok(1, 0x1FFFF) : nodlibamd_checkBit_cap_instatus_freeze_ok(0, 0x1FFFF);
			}
#else
			//merlin8/8p mdomain one-bin
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				//IoReg_BusyCheckRegBit(MDOMAIN_CAP_DDR_In1Status_reg, _BIT5,0x1FFFF);
				nodlibamd_checkBit_cap_instatus_freeze_ok(0, 0x1FFFF);
			}
#endif
		} else {
#ifdef CONFIG_DUAL_CHANNEL  // There are two channels
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CHANNEL) ? nonlibdma_set_cap_inctrl_reg_in_freeze_enable(1, 0) : nonlibdma_set_cap_inctrl_reg_in_freeze_enable(0, 0);
			}
#else
			//merlin8/8p mdomain one-bin
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				//IoReg_ClearBits(MDOMAIN_CAP_DDR_In1Ctrl_reg, _BIT1);
				nonlibdma_set_cap_inctrl_reg_in_freeze_enable(0, 0);
			}
#endif
		}
	}
	...
	if(!Scaler_I2rnd_get_timing_enable()){
		...
		if (display == SLR_MAIN_DISPLAY) {
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				while(nonlibdma_get_cap_doublebuffer_reg_doublebuffer_apply(0)) {
					timeout++;
					msleep(10);
					//cap_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg);
					if(timeout >= 5)
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				while(nonlibdma_get_cap_doublebuffer_reg_doublebuffer_apply(1)) {
					timeout++;
					msleep(10);
					//cap_reg_doublbuffer_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_reg);
					if(timeout >= 5)
...
void drvif_scalerdisplay_set_sharpness_line_mode(void)
{
	...
	if(get_vsc_run_pc_mode() && 
		((get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_HDMI) || (get_scaler_connect_source(SLR_MAIN_DISPLAY) == _SRC_MINI_DP)))	// RGB444 and HDMI input
...
{//chnage 0xb802850c and 0xb8028510 delay
	...
	{// only 120 panel need to change the setting
		if(force_enable_two_step_uzu() || is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
		{
