...
unsigned char fwif_vip_source_check(unsigned char display, unsigned char callmode)
{
	...
	unsigned char input_VO_Form = VO_FROM_DTV;
	...
	}else if( input_src_form == _SRC_FROM_VDC )
	{
		...
		/*if (NowSourceOption == SOURCE_PLAYBACK)			*/ /* USB source  */ /*fix me Mac3 no this function*/
                    //rtd_pr_vpq_emerg("check a=%d\n",pVOInfo->vo_vdec_src_type);

        if (pVOInfo->vo_vdec_src_type == VDEC_SRC_MIRACAST){
            //rtd_pr_vpq_emerg("miracast=%d !!!!\n",input_timming_info);
		...
		}else if (pVOInfo->vo_vdec_src_type != VDEC_SRC_DTV) {/* USB/OTT/JPEG source*/
			...
			case _MODE_NEW:
                ...
                if((Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_LEN)==1440)&&(Scaler_DispGetInputInfo(SLR_INPUT_MEM_ACT_WID)==2560)){
                   ...
                   which_source = VIP_QUALITY_HDMI_1440P;
					...
					which_source = fwif_vip_src_parsing_hdmi_new_mode(bHDR_flag, bDolby_HDR_flag);//    which_source = VIP_QUALITY_VGA; /* VGA Timing*/
				break;
			default:
				/*which_source = VIP_QUALITY_HDMI_720P;*/
				  which_source = VIP_QUALITY_VGA; /* VGA Timing*/
...
void fwif_color_video_quality_handler_quick_show(void)
{
	...
	unsigned int *vir_addr0, *vir_addr1, *vir_addr2, *vir_addr3, *vir_addr4, *vir_addr5, *vir_addr6, *vir_addr7, *vir_addr8, *vir_addr9, *vir_addr10, *vir_addr_end;	
	...
	//unsigned char ld_lutTableIndex;
	//unsigned char ucqc_ld_Init_Done;
	//unsigned char ucqc_ld_Backlight_value;
	...
	//gamma need init at vpq_boot_init_QS()
	//fwif_set_gamma_system_enhance(SLR_MAIN_DISPLAY, &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0], &Gamma_Linear2sRGB_14bit[0], 0);
	//fwif_color_set_Gamma_InvGamma_Ctrl(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 0, 1);
	...
	//#21 RHAL_VPQEX_Set_LD_byLevel :VPQ_EXTERN_IOC_SET_Local_Dimming_byLevel; RHAL_VPQ_LED_LDSetLUT: VPQ_LED_IOC_SET_LDSetLUT
	{	
		fwif_color_set_LDSetLUT(Cur_PicModeData->LD);		
		fwif_color_Set_LD_lutTableIndex(Cur_PicModeData->LD);
		tQS_LD_Ctrl.QS_LD_Level = Cur_PicModeData->LD;
	...
	{	
		...
		unsigned short DutyLimit[2]={0, 1023};
		...
		if (tQS_LD_Ctrl.QS_LD_INIT) {	/* Local Dimming model */
			tQS_LD_Ctrl.QS_Backlight= Cur_PicModeData->Backlight;
			
			DutyLimit[1] = Cur_PicModeData->Backlight * 1023 / 255;	// recalculate value
			fwif_color_set_LD_Boost_TV043(&DutyLimit[0]);
		}
		printf("\033[1;31m %s:%d **** [VPQ_LED] Cur_PicModeData->Backlight =%d \033[m\n", __FILE__, __LINE__, Cur_PicModeData->Backlight);
	}
	...
	vir_addr10 = (unsigned int*)(base_addr0 + sizeof(_RPC_system_setting_info) + sizeof(_system_setting_info) + sizeof(StructColorDataType) +
		(sizeof(char)*DEM_ARG_CTRL_ITEMS_Max) + sizeof(VIP_DEM_TBL) + sizeof(SLR_PICTURE_MODE_DATA) + sizeof(StructColorDataFacModeType) + sizeof(char) + sizeof(char) + 
		sizeof(QS_LD_Ctrl));
	...
	memcpy(vir_addr10, &tQS_LD_Ctrl, sizeof(QS_LD_Ctrl));
	...
	pTmp8 = (unsigned char*)&tQS_LD_Ctrl;
	for (i=0;i<sizeof(char);i++)
		qs2kernel_crc = (qs2kernel_crc + pTmp8[i])&0xFF;
...
/*o------ VIP table : VIP_QUALITY_Extend3_Coef ------o*/
	...
	if (!(source >= VIP_QUALITY_TOTAL_SOURCE_NUM || pVipCoefArray == NULL)) {
		...
		Scaler_SetRGBSharpnessTable(pVipCoefArray[VIP_QUALITY_FUNCTION_RGB_SharpTable]);
...
void Scaler_SetDNR(unsigned char value)
{
	...
	if((is_scaler_input_2p_mode(display) || force_enable_two_step_uzu() || get_hdmi_vrr_4k60_mode()) && (get_panel_pixel_mode() > PANEL_1_PIXEL))
		return;
...
char get_iveMatrix(long long matrix[3][3], long long matrix_inv[3][3], unsigned int scale)
{
	...
	rtd_pr_vpq_info("get_iveMatrix,scale=%d,determinant=%lld,b=%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,\n",
	...
	if (Scaler_APDEM_En_Access(0, 0) == 1) {
		switch(ID)
		{
			...
			case DEM_ARG_Smooth_Gradation:
			{
				...
				Scaler_Set_Block_DeContour(Scaler_Get_I_De_Contour());
...
void Scaler_VIP_IP_init(unsigned char Vip_source)
{
	...
	temp_width = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display, SLR_INPUT_CAP_WID);
	temp_height = Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)display,SLR_INPUT_CAP_LEN);
	...
	//if (0x00060000 != vpq_project_id)
		//fwif_color_inv_gamma_init();  // gamma, inv_gamma need on or off togethor
...
void Scaler_SetDataFormatHandler(void)
{
	...
	*/
	...
	fwif_color_update_invgamma_gamma_table(); //update inv_gamma and gamma table. [RKTC2851F-585][NUMDEV-1479]
...
unsigned char Scaler_GetRGBSharpnessTable(void)
{
	StructColorDataType *pTable = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
	...
	if (pTable == NULL)
		return 0;
	return pTable->RGB_Shp_Table;
}

void Scaler_SetRGBSharpnessTable(unsigned char value)
{
	StructColorDataType *pTable = NULL;
	unsigned char src_idx = 0;
	unsigned char display = 0;
	...
	if (value >= eRGB_SHP_LEVEL__count - 1)
		value = 255;

	pTable->RGB_Shp_Table = value;
}
...
unsigned char set_LocalContrast_table_ISR = 0;
void Scaler_SetLocalContrastTable(unsigned char TableIdx)
{
	...
	if (pTable == NULL) {
		rtd_pr_vpq_emerg("fwif_color_set_sharpness_table error %d \n", TableIdx);
		return;
	}
	
	down(&LC_Semaphore);

	while (set_LocalContrast_table_ISR == 2 && timeout>0)
	{
		timeout--;
		usleep_range(100, 200); // wait 1 frame to check table apply done
	}
	
	if (timeout <= 0)
	{
		rtd_pr_vpq_emerg("[%s] can't wait for wrinting donw in ISR\n", __func__);
	}
	else
	{
		...
		rtd_pr_vpq_info("Scaler_SetLocalContrastTable, LC_Table = %d\n", pTable->LocalContrastTable);
		...
		if ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) != _MODE_STATE_ACTIVE))
		{
			...
			Scaler_fwif_color_set_LocalContrast_table(src_idx, TableIdx, 0);		
			fwif_color_set_HV_Num_Fac_Size_LC();
			fwif_color_set_Size_LC_Enable_check();
			rtd_pr_vpq_info("LocalContrast_table=%d in mute\n", TableIdx);	
		}
		else
		{
			fwif_color_set_LC_Global_Region_Num_Size(TableIdx, LC_BLOCK_48x27);
			rtd_pr_vpq_info("LocalContrast_table=%d in ISR\n", TableIdx);
			set_LocalContrast_table_ISR = 1;
		}
	}
	up(&LC_Semaphore);
}
...
void Scaler_fwif_color_set_LocalContrast_table(unsigned char src_idx, unsigned char TableIdx, unsigned char skipDB_Apply)
{
	_system_setting_info *VIP_system_info_structure_table = NULL;
	...
	rtd_printk(KERN_DEBUG, TAG_NAME, "===[scalerColorLib_Default.cpp] TableIdx = %d\n", TableIdx);
	drvif_color_Get_LC_DB_Status(&DB_En, &DB_lv, &DB_apply);
	if (skipDB_Apply == 1)
		drvif_color_set_LC_DB(0, DB_lv, DB_apply);
	...
	if (skipDB_Apply == 1)
		drvif_color_set_LC_DB(DB_En, DB_lv, DB_apply);
...
void Scaler_Set_PQ_RGB444MODE(unsigned char path, int PcModeInfo)
{
#if 1
    ...
    if(path==1){
		...
		VPQ_RgbShp_RegCtrl_SetRgb_shp_yuvmode(0);
    }else{
        rtd_pr_vpq_info("NOT !!!Scaler_Set_PQ_RGB444MODE\n");
        drvif_color_set_LC_RGB_mode(0);
        drvif_color_set_Icm_RGB_mode(0);
		VPQ_RgbShp_RegCtrl_SetRgb_shp_yuvmode(1);
		if (!fwif_color_main_d1_is_2p_mode() && PcModeInfo == VIP_Disable_PC_RGB444)
...
unsigned char Scaler_VPQ_check_PC_RGB444(void)
{
	...
	if (Get_Val_vsc_run_pc_mode())
	{
		...
		{
			rtd_pr_vpq_info("[for PcMode]pc_mode=%d,DisplayMode_Src=%d,drvif_Hdmi_GetColorSpace()=%d, dolby_vision_source()=%d,interlace=%d,\n",
				Get_Val_vsc_run_pc_mode(),Get_DisplayMode_Src(SLR_MAIN_DISPLAY), drvif_Hdmi_GetColorSpace(), is_dolby_vision_source(SLR_MAIN_DISPLAY),
				Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE));
			if ((drvif_Hdmi_GetColorSpace() == COLOR_RGB) && (!is_dolby_vision_source(SLR_MAIN_DISPLAY)) && (!Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE))){
		...
		{
#if IS_ENABLED(CONFIG_RTK_DPRX)
			rtd_pr_vpq_info("[for PcMode]pc_mode=%d,DisplayMode_Src=%d,drvif_Dprx_GetColorSpace()=%d, dolby_vision_source()=%d,interlace=%d,\n",
				Get_Val_vsc_run_pc_mode(),Get_DisplayMode_Src(SLR_MAIN_DISPLAY), drvif_Dprx_GetColorSpace(), is_dolby_vision_source(SLR_MAIN_DISPLAY),
				Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE));
			if ((drvif_Dprx_GetColorSpace() == DP_COLOR_SPACE_RGB) && (!Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE))	 && (!is_dolby_vision_source(SLR_MAIN_DISPLAY))){
...
void Scaler_SetSharpness(unsigned char value)
{
	...
	if((is_scaler_input_2p_mode(display) || force_enable_two_step_uzu() || get_hdmi_vrr_4k60_mode()) && (get_panel_pixel_mode() > PANEL_1_PIXEL))
	{
		fwif_color_set_rgb_sharpness_level(value);
		return;
	}
	...
	fwif_color_Set_NN_SRCtrl(Scaler_GetNNSR_Ctrl_Table(), value);
	fwif_color_set_rgb_sharpness_level(value);
...
unsigned char Scaler_SetBackLight(unsigned char level)
{
	...
	// for RADCR
	{
		...
		extern unsigned char RADCR_cur_osd_backlight;
		RADCR_BL_ready = true;
		RADCR_cur_osd_backlight = level;
	}
#endif
	return TRUE;
}
...
unsigned char Scaler_get_APDEM_HDR_YUV2RGB(unsigned char HDR_mode)
{
	...
	else
	{
		if (HDR_mode == HAL_VPQ_HDR_MODE_ST2094){
			...
			if (Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range, 0, 0) == CSC_DATA_RANGE_MODE_FULL)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else if (Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range, 0, 0) == CSC_DATA_RANGE_MODE_AUTO && Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_CAMERA)
				data_range = CSC_DATA_RANGE_MODE_FULL;
			else if (Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range, 0, 0) == CSC_DATA_RANGE_MODE_AUTO && Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_VO_SOURCE_TYPE) == 1) //Photo
				data_range = CSC_DATA_RANGE_MODE_FULL;
	...
	else 
	{
		if((HDR_mode == HAL_VPQ_HDR_MODE_HDR10) || (HDR_mode == HAL_VPQ_HDR_MODE_HLG)){
			...
			if(data_range == CSC_DATA_RANGE_MODE_FULL) {	/* use full2full*/
				if (isRGB_in == 1) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_255_to_255;
				} else { /* bt 2020*/ /* bt 709 */
					ret_idx = HDR_DM_CSC1_YUV2RGB_HDR10_VIVID_235_to_255;
				}
			} else { /* use limit2full*/
				if (isRGB_in == 1) {
					ret_idx = HDR_DM_CSC1_YUV2RGB_ByPass_GBRtoRGB_Limted_235_to_255;
	...
	rtd_pr_vpq_info("[CSC1] APDEM_CSC1, HDR_mode=%d, PCRGB444=%d, isRGBin_RGBout=%d, hdmi_input=%d, color_format=%d, dis=%d, mode=%d, srcType=%d, SrcGetFrom=%d, isHD=%d, data_range=%d, IsRGB_YUV_RANGE=%d, isRGB_in=%d, APDEM_lv=%d, ,isPhoto=%d\n",
		HDR_mode, PCRGB444, isRGBin_RGBout, hdmi_input, color_format, display, mode, srcType, SrcGetFrom, isHD, data_range, IsRGB_YUV_RANGE, isRGB_in, Scaler_APDEM_Arg_Access(DEM_ARG_Data_Range, 0, 0), Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_VO_SOURCE_TYPE));
...
void Scaler_hdr_setting_SEAMLESS(unsigned char hdr_enable)
{
	...
	if(hdr_enable)
	{
		if (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI) {	/* HDMI case*/
			...
			rtd_pr_vpq_info("Scaler_hdr_setting_SEAMLESS, vdec_cur_hdr_type=%d,\n", vdec_cur_hdr_type);
...
extern unsigned char update_invgamma_gamma_table_TASK;
void Scaler_SetDataFormatHandle_Seamless(void)
{
	...
	*/
	...
	update_invgamma_gamma_table_TASK = 1;
...
//==========================================
	...
	if(Scaler_color_Get_HDR_AutoRun()) {
		...
		if(HDR_Type == HDR_DM_HDR10 || HDR_Type == HDR_DM_HLG || HDR_Type == HDR_DM_TECHNICOLOR || HDR_Type == HDR_DM_ST2094 || HDR_Type == HDR_DM_VIVID) {	//	HDR10
...
unsigned char Scaler_Set_Block_DeContour(unsigned char value)
{
	...
	status1 = fwif_color_Set_Block_DeContour(value, lv);
...
unsigned int HDR_bin_version = 3;
...
void EOTF_1024point(unsigned short maxlevel, VIP_HDR10_EOTF_ByLuminace_TBL_v4* EOTF_table, VIP_HDR10_OETF_ByLuminace_TBL_v4* OETF_table, unsigned int* EOTFyi_curve)
{
	...
	unsigned short norm_div=1, den_div=1;
	...
	rtd_pr_vpq_info("EOTF get, hdr_flag=%d, dynamic_hdr_flag=%d, norm_div=%d, den_div=%d, MaxLevel=%d, range_idx=%d\n", hdr_flag, dynamic_hdr_flag, norm_div, den_div, maxlevel, range_idx);
}
...
void OETF_inner_64point(int row, int col, unsigned short maxlevel, unsigned int OETFyi_curve[8][1028], unsigned short OETF_Lmix[8][16], unsigned int* OETFyi_inner, unsigned int* shift_bit, unsigned short* _3dlut){
	int i;
...
unsigned char skip_HDR10_3dLUT_C123Inv_nouse_set = 0;
...
unsigned char Scaler_color_LC_DemoStep(unsigned char mode)
{
	...
	switch(mode)	//HDR mode
	{
		...
		case 3:
			Scaler_SetLocalContrastEnable(1);
			Scaler_fwif_color_set_LocalContrast_table(src_idx,pTable->LocalContrastTable, 0);
			fwif_color_set_LC_Global_Region_Num_Size(pTable->LocalContrastTable, LC_BLOCK_16x16);
			IoReg_Write32(LC_LC_Local_Shpnr_1st_gain_8_reg, 0x00000003);
			IoReg_Write32(LC_LC_Local_Shpnr_2nd_gain_8_reg, 0x00000002);
			/*change Coeff curve for demo*/
	...
	for(nIdex = 0;nIdex<VIP_DITHER_IP_MAX;nIdex++ )
	{
		...
		switch(nIdex) {
			...
			case VIP_DITHER_IP_LGD:
				...
				break;
			case VIP_DITHER_IP_MEMC_INTERNAL:
		...
		switch(nIdex) {
			...
			case VIP_DITHER_IP_MEMC_INTERNAL:
				...
				if ((Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_THRIP) == 0) && (Scaler_DispGetStatus(SLR_MAIN_DISPLAY,SLR_DISP_RTNR) == 0))
					nDITHER_ENABLE = false;
				//MEMC dither, for fix MEMC white window separate issue.
				fwif_color_set_MEMC_internal_dither(nDITHER_ENABLE,nDITHER_MAP_BIT,nDITHER_LUT);
				break;
			case VIP_DITHER_IP_LGD:
...
void Scaler_SetNNSR_Ctrl_Table(unsigned char TableIdx)
{
	...
	fwif_color_Set_NN_SRCtrl(TableIdx, Scaler_GetSharpness());
}
