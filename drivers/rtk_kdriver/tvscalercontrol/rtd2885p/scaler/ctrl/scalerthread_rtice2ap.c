...
static VIP_RTICE_DNSUM_CTRL RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_Max] =
{
	{{"ScalerFlow_is_Active"}, 0},
	{{"SMT_Flow_is_Active"}, 0},
	{{"NNSR_is_Off"}, 0},	
	{{"NNSR_AISR_is_1"}, 0},
	{{"NNSR_AISR_is_2"}, 0},
	{{"NNSR_AISR_is_3"}, 0},
	{{"NNSR_AISR_is_4"}, 0},
	{{"NNSR_AISR_is_5"}, 0},
	{{"Tool_Check_flow_Error"}, 0},
	
	{{"UZU_InOut_Size_Error"}, 0},
	{{"game_mode_flag_on"}, 0},
	{{"PQ_SR_MODE_Error"}, 0},
	{{"byPass_flag_on"}, 0},
	{{"IMD_HFR_VRR_Case"}, 0},
...
#define MEMC_AUTOTEST_INTERNAL_PTG_NUM	(8*3)//INTERNAL_AUTOTEST_NUM
...
unsigned int debug_decontour_blkinfo = 0; // for 2265 
...
extern int scalerAI_config(int ap_mode);
// end 0622 lsy
...
int rtice_get_vip_table(ap_data_param rtice_param, unsigned char *num_type, unsigned char *buf)
{
...
#ifndef BUILD_QUICK_SHOW
	VIP_HDRCID_INFO* HDRCID_INFO;
	VIP_HDRCID_CTRL *HDRCID_CTRL;
#endif
	...
	VIP_HDR10_vivid_CTRL *phdr10vividctrl = NULL;
	...
	PQMASK_COLOR_DYNAMIC_PARAM_T *pPQMaskColorDynaParam;
...
#ifndef BUILD_QUICK_SHOW
	HDRCID_INFO = Scaler_Get_VIP_HDRCID_INFO();
	HDRCID_CTRL = Scaler_Get_VIP_HDRCID_CTRL();
#endif	
...
//merlin7_bring_up			MEMC_RTICE_GetParam((MEMC_SIZE_PARAM*)pUiTemp);
	...
	case TOOLACCESS_VIP_PQ_decotour_blkinfo: /*2265*/
		if (size > sizeof(unsigned int))
			return -ID_SIZE_ERR;

		pUiTemp = (unsigned int *)vip_malloc(sizeof(unsigned int));

		if(pUiTemp == NULL)
			return -ID_SIZE_ERR;

		pUiTemp[0] = debug_decontour_blkinfo;
		fwif_color_ChangeUINT32Endian(pUiTemp, size>>2, 1);
		memcpy(buf, (unsigned char *)pUiTemp, size);
...
// end lesley 1016
	...
	case TOOLACCESS_AI_PQMASK_Control: /*1275 get*/
		...
		if(rtice_param.mode0 == 0) { // performance
			...
			if (size > 35*sizeof(unsigned int))
				return -ID_SIZE_ERR;
			...
			ofst++;
			/* 34 */
			u32tmpValue = pPQMaskColorMgr->Param.bMotion_Enable;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			...
			ofst++;
			/* 14 */
			u32tmpValue = pPQMaskColorMgr->Status.AiGenre_CG_Enter_count;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 15 */
			u32tmpValue = pPQMaskColorMgr->Status.AiGenre_CG_Exit_count;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 16 */
			u32tmpValue = pPQMaskColorMgr->Status.MotionFlag;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 17 */
			u32tmpValue = pPQMaskColorMgr->Status.Motion_CurrentGMV;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 18 */
			u32tmpValue = pPQMaskColorMgr->Status.Motion_Enter_count;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 19 */
			u32tmpValue = pPQMaskColorMgr->Status.Motion_Exit_count;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
		...
		}else if(rtice_param.mode0 == 11) { // size related information

			if (size > 32*sizeof(unsigned int))
				return -ID_SIZE_ERR;

			pPQMaskColorDynaParam = (PQMASK_COLOR_DYNAMIC_PARAM_T *)scalerPQMaskColor_GetStruct(PQMASK_COLORSTRUCT_DYNAMIC);

			if( pPQMaskColorDynaParam == NULL )
				return READ_ERROR;

			ofst = 0;

			/* 0 */
			u32tmpValue = pPQMaskColorDynaParam->SrnnInRstWeight_AiGenre_CG;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 1 */
			u32tmpValue = pPQMaskColorDynaParam->SrnnOutRstWeight_AiGenre_CG;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 2 */
			u32tmpValue = pPQMaskColorDynaParam->AiGenre_CG_Enter_countThres;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 3 */
			u32tmpValue = pPQMaskColorDynaParam->AiGenre_CG_Exit_countThres;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 4 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_GMV_Thres;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 5 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Enter_countThres;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 6 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Exit_countThres;
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 7 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[0];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 8 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[1];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 9 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[2];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 10 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[3];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 11 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[4];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 12 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[5];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
			/* 13 */
			u32tmpValue = pPQMaskColorDynaParam->Motion_Mapping_Class[6];
			memcpy(((unsigned int*)buf)+ofst, &u32tmpValue, sizeof(unsigned int));
			ofst++;
	...
	case TOOLACCESS_DNSUM_INFO:
		if (size+start_addr > sizeof(VIP_Decide_NNSR_scaling_up_INFO))
			return -ID_SIZE_ERR;

		memcpy((void *)buf, ((unsigned char *)&RPC_system_setting_info->ScalingUp_Ctrl.Decide_NNSR_scaling_up_INFO)+start_addr, size);

		*num_type = VIP_UINT32;
	
	break;

	case TOOLACCESS_DNSUM:
	{
		VIP_Decide_NNSR_scaling_up_INFO *DNSUI;

		if (rtice_param.mode0 == 1)	// get DNSUM table
		{
			if (size+start_addr > sizeof(RTICE_DNSUM_CTRL))
				return -ID_SIZE_ERR;
			
			DNSUI = (VIP_Decide_NNSR_scaling_up_INFO *)vip_malloc(sizeof(VIP_Decide_NNSR_scaling_up_INFO));
			if (DNSUI == NULL)
			{
				rtd_pr_vpq_emerg("TOOLACCESS_DNSUM alloc fail\n");
				return -ID_SIZE_ERR;
			}
			memcpy(DNSUI, &RPC_system_setting_info->ScalingUp_Ctrl.Decide_NNSR_scaling_up_INFO, sizeof(VIP_Decide_NNSR_scaling_up_INFO));			
			fwif_color_ChangeUINT32Endian((unsigned int*)DNSUI, sizeof(VIP_Decide_NNSR_scaling_up_INFO)/sizeof(int), 1);
			
			for (i=0;i<VIP_RTICE_DNSUM_Max;i++)
				RTICE_DNSUM_CTRL[i].TF_Flag = 0;
			
			if (DNSUI->final_apply_flag == 1)	// scaler flow
			{
				if ((DNSUI->NNSR_SCALER_INFO.Main_iSize_nWidth <=1920) && (DNSUI->NNSR_SCALER_INFO.Main_iSize_nLength <=1080))	// normal nnsr
				{
					if (((DNSUI->NNSR_SCALER_INFO.Main_iSize_nWidth*2) > (DNSUI->NNSR_SCALER_INFO.Main_oSize_nWidth)) || 
						((DNSUI->NNSR_SCALER_INFO.Main_iSize_nLength*2) > (DNSUI->NNSR_SCALER_INFO.Main_oSize_nLength)))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_UZU_InOut_Size_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.drv_memory_get_game_mode_flag == 1)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_game_mode_flag_on].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.fwif_color_Get_PQ_SR_MODE == SR_Mode_Disable)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_PQ_SR_MODE_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.NNSR_byPass_flag == 1)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_byPass_flag_on].TF_Flag = 1;

					if (((DNSUI->NNSR_SCALER_INFO.is_scaler_input_2p_mode != 0) || ((DNSUI->NNSR_SCALER_INFO.force_enable_two_step_uzu != 0) && 
						(DNSUI->NNSR_SCALER_INFO.ScalerGetMainINFO_SLR_INPUT_V_FREQ_1000 > 121000))) 
						&& (DNSUI->NNSR_SCALER_INFO.Get_DISPLAY_REFRESH_RATE >= 120))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_IMD_HFR_VRR_Case].TF_Flag = 1;

					if ((DNSUI->NNSR_SCALER_INFO.AI_SR_MODE < DNSUI->NNSR_SCALER_INFO.mode_support) /*&& (gSrnnTopInfo.ModeAISR != 0)*/)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_D1_Timing_Error].TF_Flag = 1;
				
				}
				else if ((DNSUI->NNSR_SCALER_INFO.Main_iSize_nWidth > 1920) && (DNSUI->NNSR_SCALER_INFO.Main_iSize_nLength > 1080))// NNSR pre-donw case
				{
					if (DNSUI->NNSR_SCALER_INFO.isSubIdle_forNNSR == 0)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_sub_is_not_available].TF_Flag = 1;

					if ((DNSUI->NNSR_SCALER_INFO.Main_iSize_nWidth > 3840) || (DNSUI->NNSR_SCALER_INFO.Main_iSize_nLength > 2160) || 
						(DNSUI->NNSR_SCALER_INFO.Main_oSize_nWidth > 3840) || (DNSUI->NNSR_SCALER_INFO.Main_oSize_nLength > 2160) || 
						(DNSUI->NNSR_SCALER_INFO.Main_iSize_nWidth != DNSUI->NNSR_SCALER_INFO.Sub_iSize_nWidth) || 
						(DNSUI->NNSR_SCALER_INFO.Main_iSize_nLength != DNSUI->NNSR_SCALER_INFO.Sub_iSize_nLength))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_UZU_InOut_Size_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.drv_memory_get_game_mode_flag == 1)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_game_mode_flag_on].TF_Flag = 1;
					
					if (DNSUI->NNSR_SCALER_INFO.fwif_color_Get_PQ_SR_MODE != SR_MODE_NNSR_V2)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_PQ_SR_MODE_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.dataFs != 0)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_is_DataFs].TF_Flag = 1;
					
					if (DNSUI->NNSR_SCALER_INFO.NNSR_byPass_flag == 1)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_byPass_flag_on].TF_Flag = 1;
					
					if (((DNSUI->NNSR_SCALER_INFO.is_scaler_input_2p_mode != 0) || (DNSUI->NNSR_SCALER_INFO.force_enable_two_step_uzu != 0) ||
						(DNSUI->NNSR_SCALER_INFO.get_hdmi_vrr_4k60_mode != 0)) && (DNSUI->NNSR_SCALER_INFO.Get_DISPLAY_REFRESH_RATE >= 120))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_IMD_HFR_VRR_Case].TF_Flag = 1;

					if (DNSUI->NNSR_SCALER_INFO.iFrameRate > 61000)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_iFrameRate_not_60].TF_Flag = 1;
				...
				RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_ScalerFlow_is_Active].TF_Flag = 1;
			}
			else if (DNSUI->final_apply_flag == 2)	// smt flow
			{
				if ((DNSUI->NNSR_SMT_INFO.Main_iSize_nWidth <=1920) && (DNSUI->NNSR_SMT_INFO.Main_iSize_nLength <=1080))	// normal nnsr
				{
					if (((DNSUI->NNSR_SMT_INFO.Main_iSize_nWidth*2) > (DNSUI->NNSR_SMT_INFO.Main_oSize_nWidth)) || 
						((DNSUI->NNSR_SMT_INFO.Main_iSize_nLength*2) > (DNSUI->NNSR_SMT_INFO.Main_oSize_nLength)))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_UZU_InOut_Size_Error].TF_Flag = 1;

					if ((DNSUI->NNSR_SMT_INFO.scaler_get_gamemode_fs == 1) || (DNSUI->NNSR_SMT_INFO.scaler_get_gamemode_frc == 1))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_game_mode_flag_on].TF_Flag = 1;

					if (DNSUI->NNSR_SMT_INFO.ScalerVIP_Get_SR_MODE == SR_Mode_Disable)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_PQ_SR_MODE_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SMT_INFO.NNSR_byPass_flag == 1)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_byPass_flag_on].TF_Flag = 1;

					if (((DNSUI->NNSR_SMT_INFO.IMD_SCALER_INPUT_2P_MODE != 0) || ((DNSUI->NNSR_SMT_INFO.force_enable_two_step_uzu != 0) && 
						(DNSUI->NNSR_SMT_INFO.IMD_SMOOTH_V_FREQ_1000 > 121000))) && 
						(DNSUI->NNSR_SMT_INFO.Get_DISPLAY_REFRESH_RATE >= 120))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_IMD_HFR_VRR_Case].TF_Flag = 1;

					if ((DNSUI->NNSR_SMT_INFO.AI_SR_MODE < DNSUI->NNSR_SMT_INFO.mode_support) /*&& (gSrnnTopInfo.ModeAISR != 0)*/)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_D1_Timing_Error].TF_Flag = 1;
				
				}
				else if ((DNSUI->NNSR_SMT_INFO.Main_iSize_nWidth > 1920) && (DNSUI->NNSR_SMT_INFO.Main_iSize_nLength > 1080))// NNSR pre-donw case
				{
					if (DNSUI->NNSR_SMT_INFO.isSubIdle_forNNSR == 0)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_sub_is_not_available].TF_Flag = 1;

					if ((DNSUI->NNSR_SMT_INFO.Main_iSize_nWidth > 3840) || (DNSUI->NNSR_SMT_INFO.Main_iSize_nLength > 2160) || 
						(DNSUI->NNSR_SMT_INFO.Main_oSize_nWidth > 3840) || (DNSUI->NNSR_SMT_INFO.Main_oSize_nLength > 2160) || 
						(DNSUI->NNSR_SMT_INFO.Main_iSize_nWidth != DNSUI->NNSR_SMT_INFO.Sub_iSize_nWidth) || 
						(DNSUI->NNSR_SMT_INFO.Main_iSize_nLength != DNSUI->NNSR_SMT_INFO.Sub_iSize_nLength))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_UZU_InOut_Size_Error].TF_Flag = 1;

					if ((DNSUI->NNSR_SMT_INFO.scaler_get_gamemode_fs == 1) || (DNSUI->NNSR_SMT_INFO.scaler_get_gamemode_frc == 1))
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_game_mode_flag_on].TF_Flag = 1;
					
					if (DNSUI->NNSR_SMT_INFO.ScalerVIP_Get_SR_MODE != SR_MODE_NNSR_V2)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_PQ_SR_MODE_Error].TF_Flag = 1;

					if (DNSUI->NNSR_SMT_INFO.dataFs != 0)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_is_DataFs].TF_Flag = 1;
					...
					if (DNSUI->NNSR_SMT_INFO.iFrameRate > 61000)
						RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_iFrameRate_not_60].TF_Flag = 1;
				...
				RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_SMT_Flow_is_Active].TF_Flag = 1;
			}
			else
			{
				RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_NNSR_is_Off].TF_Flag = 1;
			}

			RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_NNSR_is_Off + gSrnnTopInfo.ModeAISR].TF_Flag = 1;

			// check RTICE flow is same as scaler/smt flow or not.
			bTemp = RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_UZU_InOut_Size_Error].TF_Flag | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_game_mode_flag_on].TF_Flag
				 | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_PQ_SR_MODE_Error].TF_Flag | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_byPass_flag_on].TF_Flag 
				 | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_IsNot_DataFs_Case].TF_Flag | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_iFrameRate_not_60].TF_Flag
				 | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_D1_Timing_Error].TF_Flag | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_sub_is_not_available].TF_Flag
				 | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_is_DataFs].TF_Flag | RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_IMD_HFR_VRR_Case].TF_Flag;
			if (((gSrnnTopInfo.ModeAISR != 0) && (bTemp != 0)) || ((gSrnnTopInfo.ModeAISR == 0) && (bTemp == 0) && (DNSUI->final_apply_flag != 0)))
			{
				// reset all
				for (i=0;i<VIP_RTICE_DNSUM_Max;i++)
					RTICE_DNSUM_CTRL[i].TF_Flag = 0;

				RTICE_DNSUM_CTRL[VIP_RTICE_DNSUM_Tool_Check_flow_Error].TF_Flag = 1;
			}

			memcpy(buf, &RTICE_DNSUM_CTRL, size);

			vip_free(DNSUI);
		}
		else		// get DNSUM table lenth
		{
			if (size+start_addr > sizeof(char))
				return -ID_SIZE_ERR;

			buf[0] = VIP_RTICE_DNSUM_Max;
		}
...
*/
	...
	case TOOLACCESS_DM2_HLG_EOTF_byLuminance_CTRL: /*1929*/
		if (size > (sizeof(unsigned short)*2))
			return -ID_SIZE_ERR;
		
		memcpy((unsigned char*)&buf[0], (unsigned char*)&(HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0]), size);
	...
	case TOOLACCESS_DM2_HDR10_EOTF_byLuminance_CTRL: /*1930*/
		if (size > (sizeof(unsigned short)*1))
			return -ID_SIZE_ERR;
	...
	case TOOLACCESS_DM2_HDR10_OETF_byLuminance_CTRL: /*1931*/
	...
	case TOOLACCESS_HDR10_VIVID_Debug: /* 1980 */
		phdr10vividctrl = drvif_fwif_color_Get_HDR10_vivid_Ctrl();

		if (size > (sizeof(VIP_HDR10_vivid_CTRL)))
			return -ID_SIZE_ERR;

		memcpy(buf, (void *)phdr10vividctrl, size);
		fwif_color_ChangeUINT32Endian((unsigned int *)buf, size/sizeof(unsigned int), 1);
	...
	case TOOLACCESS_NR_Table_TV006:
	{
		...
		*num_type = VIP_UINT16;
		...
		break;
#ifndef BUILD_QUICK_SHOW
		case TOOLACCESS_ScalerVIP_HDRID_CTRL:

			if ((rtice_param.saddr*sizeof(short)+size) > sizeof(VIP_HDRCID_CTRL))
				return -ID_SIZE_ERR;

			memcpy(buf, ((unsigned short*)HDRCID_CTRL) + rtice_param.saddr, size);
		...
		case TOOLACCESS_ScalerVIP_HDRID_INFO:

			if ((rtice_param.saddr*sizeof(short)+size) > sizeof(VIP_HDRCID_INFO))
				return -ID_SIZE_ERR;

			memcpy(buf, ((unsigned short*)HDRCID_INFO) + rtice_param.saddr, size);
	...
	case TOOLACCESS_VIP_LD_Boost_Debug: /*2020 get*/
	{
		extern SW_BOOST_INFO tSW_BOOST_INFO;
		extern unsigned short vip_tar_gain;
		extern unsigned short vip_cur_gain;

		if (size > 11*sizeof(unsigned short))
			return -ID_SIZE_ERR;

		pwUINT16Temp = (unsigned short *)vip_malloc(11*sizeof(unsigned short));
		if(pwUINT16Temp == NULL)
			return -ID_SIZE_ERR;

		pwUINT16Temp[0] = (unsigned short)tSW_BOOST_INFO.SW_BoostEn;	
		pwUINT16Temp[1] = (unsigned short)tSW_BOOST_INFO.BoostMode;
		pwUINT16Temp[2] = (unsigned short)tSW_BOOST_INFO.BoostOff_Current;
		pwUINT16Temp[3] = (unsigned short)tSW_BOOST_INFO.BoostOn_Current;
		pwUINT16Temp[4] = (unsigned short)tSW_BOOST_INFO.Boost_Temporal_En;
		pwUINT16Temp[5] = (unsigned short)tSW_BOOST_INFO.Boost_Temporal_step;
		pwUINT16Temp[6] = (unsigned short)tSW_BOOST_INFO.Boost_Temporal_step_diff;
		pwUINT16Temp[7] = (unsigned short)vip_tar_gain;
		pwUINT16Temp[8] = (unsigned short)vip_cur_gain;
		fwif_color_ChangeUINT16Endian(pwUINT16Temp, size/sizeof(unsigned short), 1);
		memcpy(buf, pwUINT16Temp, size);
	...
	case TOOLACCESS_VIP_LD_Mculess_LEDdriver_Debug:	/*2021 get*/
	{

		if (size > sizeof(LEDdriver_Arg))
			return -ID_SIZE_ERR;		

		pwUINT16Temp = (unsigned short *)vip_malloc(sizeof(LEDdriver_Arg));
		if(pwUINT16Temp == NULL)
			return -ID_SIZE_ERR;
		fwif_color_Get_LEDdriver_Args((LEDdriver_Arg*)pwUINT16Temp);
...
*/
	...
	case TOOLACCESS_VIP_SLD_BLK_APL:/*k7, SLD*/
		if (size > (sizeof(unsigned short)*8160))
			return -ID_SIZE_ERR;
		
		pwUINT16Temp = (unsigned short *)vip_malloc(sizeof(unsigned short)*8160);	// (120*68)		// max for 4k. 
		
		if (pwUINT16Temp == NULL) {
			break;
		} 
		
		if (fwif_color_fwif_color_Get_SLD_APL(pwUINT16Temp, 8160) < 0) {
			vip_free(pwUINT16Temp);
			return -READ_ERROR;
		}
...
/************************************************************/
	...
	case TOOLACCESS_RADCR_BL_PWM:
	{
		extern unsigned int scalerVIP_RADCR_get_pwm(void);
		if (start_addr*sizeof(unsigned int)+size > sizeof(unsigned int))
			return -ID_SIZE_ERR;

		if (size) {
			unsigned int pwm = scalerVIP_RADCR_get_pwm();
			memcpy(buf, &pwm, size);
			fwif_color_ChangeUINT32Endian((unsigned int *)buf, size/sizeof(unsigned int), 1);
		}
...
//merlin7_bring_up			MEMC_RTICE_Trigger();
	...
	case TOOLACCESS_VIP_PQ_decotour_blkinfo: /*2265*/
		...
		debug_decontour_blkinfo = pUiTemp[0];
		drvif_color_Get_Block_DeContour_MAD();
...
// lesley 1024
	case TOOLACCESS_AI_Ctrl_Draw: /*1261 set*/
		...
		if(aipq_mode.ap_mode)
			scalerAI_config(AI_MODE_ON);
		else
			scalerAI_config(AI_MODE_OFF);
...
// end lesley 1016
	...
	case TOOLACCESS_AI_PQMASK_Control: /*1275 set*/
		...
		if(rtice_param.mode0 == 1) { // set some color mgr setting
			...
			/* 34 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorMgr->Param.bMotion_Enable= u32tmpValue & 0xff;
			ofst++;
			...
			/* 14 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.AiGenre_CG_Enter_count = u32tmpValue;
			ofst++;
			/* 15 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.AiGenre_CG_Exit_count = u32tmpValue;
			ofst++;
			/* 16 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.MotionFlag = u32tmpValue;
			ofst++;
			/* 17 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.Motion_CurrentGMV = u32tmpValue;
			ofst++;
			/* 18 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.Motion_Enter_count = u32tmpValue;
			ofst++;
			/* 19 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			// pPQMaskColorMgr->Status.Motion_Exit_count = u32tmpValue;
			ofst++;
			...
			/* 0 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->SrnnInRstWeight_AiGenre_CG = u32tmpValue & 0xff;
			ofst++;
			/* 1 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->SrnnOutRstWeight_AiGenre_CG = u32tmpValue & 0xff;
			ofst++;
			/* 2 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->AiGenre_CG_Enter_countThres = u32tmpValue & 0xff;
			ofst++;
			/* 3 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->AiGenre_CG_Exit_countThres = u32tmpValue & 0xff;
			ofst++;
			/* 4 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_GMV_Thres= u32tmpValue & 0xff;
			ofst++;
			/* 5 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Enter_countThres= u32tmpValue & 0xff;
			ofst++;
			/* 6 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Exit_countThres= u32tmpValue & 0xff;
			ofst++;
			/* 7 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[0]= u32tmpValue & 0xff;
			ofst++;
			/* 8 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[1]= u32tmpValue & 0xff;
			ofst++;
			/* 9 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[2]= u32tmpValue & 0xff;
			ofst++;
			/* 10 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[3]= u32tmpValue & 0xff;
			ofst++;
			/* 11 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[4]= u32tmpValue & 0xff;
			ofst++;
			/* 12 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[5]= u32tmpValue & 0xff;
			ofst++;
			/* 13 */
			memcpy(&u32tmpValue, (((unsigned int *)buf)+ofst), sizeof(unsigned int));
			pPQMaskColorDynaParam->Motion_Mapping_Class[6]= u32tmpValue & 0xff;
			ofst++;
	...
	case TOOLACCESS_NNSR_Weight_Tbl:
	   ...
	   case TOOLACCESS_NNSR_SQMInfo:	/*1288 set*/	
		...
		// for autogui partial write
		if (start_addr == 0){
			gSrnnSQMInfo.UpdateFPS = buf[0];
		}
		if (start_addr <= 1 && (size+start_addr > 1)){
			gSrnnSQMInfo.RawRes = buf[1-start_addr];
		}
		if (start_addr <= 6 && (size+start_addr > 6)){
			gSrnnSQMInfo.DirectionCounterThres = buf[6-start_addr];
		}
		if (start_addr <= 8 && (size+start_addr > 8)){
			gSrnnSQMInfo.Step = buf[8-start_addr];
		}
		if (start_addr <= 10 && (size+start_addr > 10)){
			gSrnnSQMInfo.DebugFreeze = buf[10-start_addr];
		}
		if (start_addr <= 11 && (size+start_addr > 11)){
			gSrnnSQMInfo.DebugPosOffset = buf[11-start_addr];
		}
		if (start_addr <= 12 && (size+start_addr > 12)){
			gSrnnSQMInfo.DebugNegOffset = buf[12-start_addr];
		}
		if (start_addr <= 13 && (size+start_addr > 13)){
			gSrnnSQMInfo.SQMFlowEn = buf[13-start_addr];
		}
...
//			Scaler_SetMA_Chroma_Error(buf[12]); // merlin4 remove
...
#ifndef BUILD_QUICK_SHOW
		if ((vip_data_access_debug.debug_en&DAD_Peaking_En) != 0)	// use peaking data access, disable gScalerVIP_ID_BW_ChessPlate
		{	
			for (i=0;i<HDRCID_Pattern_MAX;i++)
				HDRCID_CTRL->Pattern_th[i] = 0;
		}
#endif	
	...
	case TOOLACCESS_DM2_HLG_EOTF_byLuminance_CTRL: /*1929*/
		...
		fwif_color_ChangeUINT16Endian((unsigned short*)&buf[0], size/sizeof(short), 1);
		memcpy((unsigned char*)&(HLG_EOTF_ByLuminance_TBL_cur.EOTF_setting[0]), (unsigned char*)&buf[0], size);
		Scaler_color_HDRIP_AutoRun(HDR_DM_HLG); // re-run the HLG flow
		break;
	...
	case TOOLACCESS_HDR10_VIVID_Debug: /*1980*/
		...
		phdr10vividctrl->Enable = pwUINT32Temp[0];
		// phdr10vividctrl->update_DMA_Flag = pwUINT32Temp[1];
		// phdr10vividctrl->CUVA_K_shift_bit = pwUINT32Temp[2];
		phdr10vividctrl->Debug_Log = pwUINT32Temp[3];
		phdr10vividctrl->Debug_Delay = pwUINT32Temp[4];
		// phdr10vividctrl->Debug_VGIP_Line_sta = pwUINT32Temp[5];
		// phdr10vividctrl->Debug_VGIP_Line_end = pwUINT32Temp[6];
		// phdr10vividctrl->Debug_VGIP_Line_diff = pwUINT32Temp[7];
		// phdr10vividctrl->Debug_90clk_sta = pwUINT32Temp[8];
		// phdr10vividctrl->Debug_90clk_end = pwUINT32Temp[9];
		// phdr10vividctrl->Debug_90clk_diff = pwUINT32Temp[10];
		// phdr10vividctrl->Debug_Cal_skip_flag = pwUINT32Temp[11];
		phdr10vividctrl->MetadataProc_VLC_LineCnt = pwUINT32Temp[12];
		break;
	...
	case TOOLACCESS_VIP_PQ_Power_Saving:
		...
		case TOOLACCESS_VIP_ODPH_LUT_Data_Access:
			...
			fwif_color_set_ODPH_LUT(&gVIP_ODPH_Data.ODPH_TBL, &gVIP_ODPH_Data.ODPH_TBL_REC, 
				&gVIP_ODPH_Data.ODPH_APDEMGain, &gVIP_ODPH_Data.ODPH_Gain, &gVIP_ODPH_Data.ODPH_Gain_REC, rtice_param.mode1);
		...
		case TOOLACCESS_ScalerVIP_HDRID_CTRL:
			...
			fwif_color_ChangeUINT16Endian((unsigned short *)buf, size/sizeof(unsigned short), 1);
			memcpy(((unsigned short*)HDRCID_CTRL) + rtice_param.saddr, buf, size);
	...
	{		
		...
		fwif_color_set_Gamma_InvGamma_Ctrl(0, 0, pwINT16Temp, pwINT16Temp+1025, pwINT16Temp+2050, 0, 1);
	...
	case TOOLACCESS_VIP_DeMura_READ_INX_DATA:
		if(rtice_SetGet_DeMura(&rtice_param, 0, buf, system_setting_info, 1) < 0)
			return -ID_SIZE_ERR;
		break;
	case TOOLACCESS_VIP_LD_Boost_Debug:	/*2020 set*/
	{
		extern SW_BOOST_INFO tSW_BOOST_INFO;
		...
		tSW_BOOST_INFO.SW_BoostEn		= (unsigned char)pwUINT16Temp[0];
		tSW_BOOST_INFO.BoostMode 		= (unsigned char)pwUINT16Temp[1];
		tSW_BOOST_INFO.BoostOff_Current = (unsigned short)pwUINT16Temp[2];
		tSW_BOOST_INFO.BoostOn_Current = (unsigned short)pwUINT16Temp[3];
		tSW_BOOST_INFO.Boost_Temporal_En 		= (unsigned char)pwUINT16Temp[4];
		tSW_BOOST_INFO.Boost_Temporal_step 		= (unsigned char)pwUINT16Temp[5];
		tSW_BOOST_INFO.Boost_Temporal_step_diff = (unsigned char)pwUINT16Temp[6];
		vip_free(pwUINT16Temp);
		pwUINT16Temp = NULL;
	...
	case TOOLACCESS_VIP_LD_Mculess_LEDdriver_Debug:	/*2021 set*/
	{
		if (size > sizeof(LEDdriver_Arg))
			return -ID_SIZE_ERR;		
		...
		fwif_color_Set_LEDdriver_Args((LEDdriver_Arg*)pwUINT16Temp);
...
/************************************************************/
	...
	case TOOLACCESS_RADCR_BL_PWM:
	{
		extern void scalerVIP_RADCR_set_pwm(unsigned int pwm);
		if (start_addr*sizeof(unsigned int)+size > sizeof(unsigned int))
			return -ID_SIZE_ERR;
