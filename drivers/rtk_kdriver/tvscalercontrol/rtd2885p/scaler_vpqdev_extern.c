...
extern unsigned char isOSDSR_update_flag;
...
extern char ScalerPQMaskColor_PQMASK_AIPQ_BIN(RTK_AI_PQ_mode *ap_mode, SLR_VIP_TABLE_AIPQ *pSLR_VIP_TABLE_AIPQ);
 ...
 ************************************************************************/
		...
		{			
			...
			if (copy_from_user(&hal_VPQ_DATA_EXTERN, (int __user *)arg, sizeof(HAL_VPQ_DATA_EXTERN_T))) {
				...
				if (hal_VPQ_DATA_EXTERN.cmd == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Get) // get ioctl
				{	 
					if (hal_VPQ_DATA_EXTERN.version == VPQ_DEV_EXT_SET_AI_Para_TBL_set) // get AP mode from bin
					{
						if (hal_VPQ_DATA_EXTERN.length != sizeof(int)) {
				...
				else if (hal_VPQ_DATA_EXTERN.cmd == VPQ_DEV_EXTERN_VPQ_DATA_EXTERN_CMD_Set)	// set ioctl
				{	
					if (hal_VPQ_DATA_EXTERN.version == VPQ_DEV_EXT_SET_AI_Para_TBL_set) 
					{
						if (hal_VPQ_DATA_EXTERN.length != sizeof(SLR_VIP_TABLE_AIPQ)) {
							...
							if(copy_from_user(&m_VIP_AIPQTable, (int __user *)Data_addr, hal_VPQ_DATA_EXTERN.length))
							{
								...
								memcpy(&AI_ICM_TBL[0], &m_VIP_AIPQTable.DCC_Ctrl_table[0], sizeof(AI_ICM_Ctrl_table)*SLR_AI_TBL_NUM);
								
								// PQ mask
								ScalerPQMaskColor_PQMASK_AIPQ_BIN(&aipq_mode, &m_VIP_AIPQTable);

								memcpy(&AI_Ctrl_TBL[1], &m_VIP_AIPQTable.SLR_Ctrl_table[0], sizeof(DRV_AI_Ctrl_table)*SLR_AI_TBL_NUM);	// use table 1 and 2 in fw
								
								//g_bUseAIPQBin = 1;
								bBinAIPQ_Table = 1;
								
								rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_SET_AI_Para = %d,%d,%d, %d,bBinAIPQ_Table=%d,\n", 
				...
				{
					rtd_pr_vpq_info("kernel VPQ_EXTERN_IOC_SET_AI_Para only set or get,\n");
					ret = -1;
				}
			}	
		}		
