...
//						VPQ_GenFile_setfile_data(vpq_file, &file_info[i], VPQUT_IOC_PQ_CMD_SET_OETF_LUT_HLG_DEFAULT, &OETF_LUT_HLG_DEFAULT, sizeof(OETF_LUT_HLG_DEFAULT));
					...
					VipTableInit();
//					xLoadDefaultTable();
					//Setup_IMD(Scaler_DispGetInfoPtr()); //no more maintain
	...
	switch (cmd){
		...
		case VPQUT_IOC_GET_MEM_INFO:
		{
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				for(i=0;i<MEMIDX_UNKNOW;i++){
					...
					MemTag[i].Allocate_StartAddr = nonlibdma_drvif_memory_get_MemTag_addr(i)->Allocate_StartAddr;
					MemTag[i].Allocate_VirAddr = nonlibdma_drvif_memory_get_MemTag_addr(i)->Allocate_VirAddr;
					MemTag[i].Size = nonlibdma_drvif_memory_get_MemTag_addr(i)->Size;
					MemTag[i].Status = nonlibdma_drvif_memory_get_MemTag_addr(i)->Status;
					MemTag[i].StartAddr = nonlibdma_drvif_memory_get_MemTag_addr(i)->StartAddr;
				}
		...
		case VPQUT_IOC_SET_DI_MEM:
		{
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				DI_mem_info.Allocate_StartAddr = nonlibdma_drvif_memory_get_MemTag_addr(MEMIDX_DI)->Allocate_StartAddr;
				DI_mem_info.Allocate_VirAddr = nonlibdma_drvif_memory_get_MemTag_addr(MEMIDX_DI)->Allocate_VirAddr;
				DI_mem_info.Size = nonlibdma_drvif_memory_get_MemTag_addr(MEMIDX_DI)->Size;
				DI_mem_info.Status = nonlibdma_drvif_memory_get_MemTag_addr(MEMIDX_DI)->Status;
				DI_mem_info.StartAddr = nonlibdma_drvif_memory_get_MemTag_addr(MEMIDX_DI)->StartAddr;

				base = ioremap(DI_mem_info.StartAddr, DI_mem_info.Size);
				if (copy_from_user(base, (int __user *)arg, DI_mem_info.Size)) {
					rtd_pr_vpq_emerg("vpqut ioctl VPQUT_IOC_SET_DI_MEM fail\n");
					ret = -1;
				}
				iounmap(base);
			}
		...
		case VPQUT_IOC_GET_DI_MEM:
		{
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				if(copy_to_user((void __user *)arg, base, DI_mem_info.Size)){
					rtd_pr_vpq_emerg("vpqut ioctl VPQUT_IOC_GET_DI_MEM fail\n");
					ret = -1;
				}
				iounmap(base);
			}
