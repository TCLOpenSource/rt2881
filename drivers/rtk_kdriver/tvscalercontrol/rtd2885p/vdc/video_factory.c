...
static StructMemBlock video_factory_MemTag[MEMIDX_UNKNOW];	// memory tag information
unsigned int drvif_module_vdc_factory_TestDMA_Enable(unsigned int Length)
{
	...
	drvif_module_vdc_factory_TestDMA_Setting();
		...
		for ( i = MEMIDX_VDC ; i < MEMIDX_ALL ; ++i ) {	// clear all tag information
			video_factory_MemTag[i].StartAddr = _NONLIBDMA_DUMMYADDR;
			video_factory_MemTag[i].Size = 0;
			video_factory_MemTag[i].Status = UNDEFINED;
		}
...
unsigned int drvif_module_vdc_factory_IFDDMA_Enable(unsigned int Length)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nonlibdma_drvif_memory_free_block(MEMIDX_ALL);
	}
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nAddr = nonlibdma_drvif_memory_alloc_block(MEMIDX_VDC, Length, MEMALIGN_ANY);
