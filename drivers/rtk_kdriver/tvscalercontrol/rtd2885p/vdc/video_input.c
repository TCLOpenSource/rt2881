...
unsigned char drvif_AV_Detect_OffLine_Signal_new(unsigned char source)
{
#if 1
	int i;
	int toggle_cnt=0;
	...
	for(i=0;i<10;i++)
	{
		tv_sb1_ana_vd_adc_ADC_SOG_reg.tog_adc_sog1 = 1;
...
//        rtd_pr_avd_info(" regValue %d drvif_AV_Detect_OffLine_Signal", tv_sb1_ana_vd_adc_ADC_SOG_reg.regValue);
		if(tv_sb1_ana_vd_adc_ADC_SOG_reg.tog_adc_sog1)
			toggle_cnt++;
	}
	...
	if (toggle_cnt >= 6)
	{
//		rtd_pr_avd_info("detect source %d, signal %d drvif_AV_Detect_OffLine_Signal", source, toggle_cnt);
		return TRUE;
	}
	else
	{
//        rtd_pr_avd_info("detect source %d, no_signal %d drvif_AV_Detect_OffLine_Signal", source, toggle_cnt);
		return FALSE;
	}
#endif
}
