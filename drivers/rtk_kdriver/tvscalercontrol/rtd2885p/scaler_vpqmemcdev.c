...
struct semaphore MEMC_CMA_Semaphore;
#endif

unsigned int unconsistant_addr[9] = {0};
unsigned int unconsistant_addr_aligned[9] = {0};
unsigned int unconsistant_mem_size[9] = {0};
unsigned char CMA_memory_allocate_cnt = 0;
unsigned char CMA_memory_release_flag = 0;
unsigned char CMA_semaphone_status = 0;
extern unsigned char CMA_release_tsk_flag;
...
extern void MEMC_Lib_Set_MEMC_Regen_Address(void);
extern void MEMC_Lib_Set_Memory_Write_Done(unsigned char enable);
extern unsigned int g_memc_input_vsync_cnt, g_memc_outpur_vsync_cnt, g_memc_outpur_vsync_cnt_2;
extern VOID MEMC_MidSetPCOff(VOID);
extern VOID Mid_Mode_SetMEMC_PCModeEnable(BOOL bEnable);
extern unsigned int HAL_MC_GetKphaseMCIndexBsize(VOID);
extern unsigned int HAL_CRTC_GetFRCHact(VOID);
extern unsigned int HAL_MC_GetMC_LF_I_DMA_Enable(VOID);
...
//#endif
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[%s][AAA][%x, %x]\n", __FUNCTION__, rtd_inl(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg));
	}else if(RUN_MERLIN8()){
...
//#endif
	...
	if(status == TRUE){
		...
		check_BSize_status = HAL_MC_GetKphaseMCIndexBsize();
...
//#endif
	...
	if(!(((get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic() == _ENABLE) && (get_new_game_mode_condition() || (Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC))) || (get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_vdec_direct_low_latency_mode())))
	{
		rtd_pr_memc_emerg("\n func:%s, line:%d  \n",__FUNCTION__,__LINE__);
		...
		if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
			...
			if(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
				if(memc_mux_ctrl_reg.memc_outmux_sel)
				{
					...
					if (scaler_vsc_get_adaptive_pst_lowdelay_mode() == 1 && (get_mach_type() == MACH_ARCH_RTK2885P && !nonlibdma_drv_memory_get_vdec_direct_low_latency_mode())){
						...
						if((sys_reg_sys_clken3_reg.clken_memc_me == 0)/* && (pVOInfo->src_v_freq_1000 <= 47000)*/
							...
							&& (get_mach_type() == MACH_ARCH_RTK2885P && !(nonlibdma_drv_memory_get_vdec_direct_low_latency_mode()))){
		...
		if(Get_DISPLAY_REFRESH_RATE() > 60){
...
#endif
			{
				...
				if((vbe_disp_get_adaptivestream_fs_mode()) && (0 == vbe_disp_get_adaptivestream_force_framerate()))
					memc_output_frame_rate = VPQ_OUTPUT_96HZ; //24Hz => 96Hz
				else if(Get_DISPLAY_REFRESH_RATE() == 240)
					memc_output_frame_rate = VPQ_OUTPUT_240HZ; //24Hz => 240Hz
				else if(Get_DISPLAY_REFRESH_RATE() == 160 || Get_DISPLAY_REFRESH_RATE() == 165)
...
#endif
			{
				...
				if((vbe_disp_get_adaptivestream_fs_mode()) && (0 == vbe_disp_get_adaptivestream_force_framerate()))
					memc_output_frame_rate = VPQ_OUTPUT_48HZ; //24Hz => 48Hz
				else
...
//		if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_100000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_100000_pOFFSET ) )	// 100Hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			if(Get_DISPLAY_REFRESH_RATE() == 240)
				memc_output_frame_rate = VPQ_OUTPUT_200HZ; //100Hz => 200Hz
			else
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_120000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_120000_pOFFSET ) )	// 120Hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			if(Get_DISPLAY_REFRESH_RATE() == 240)
				memc_output_frame_rate = VPQ_OUTPUT_240HZ; //120Hz => 240Hz
			else
	...
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_144000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_144000_pOFFSET ) )	// 144Hz
	{
		...
		//if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_PIXEL_MODE) == PIXEL_MODE_2_PIXEL){
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			memc_output_frame_rate = VPQ_OUTPUT_144HZ; //144Hz => 144Hz
		}
		else{
			memc_output_frame_rate = VPQ_OUTPUT_60HZ; //144Hz => 60Hz
		}
	}
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_165000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_165000_pOFFSET ) )	// 165Hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			...
			memc_output_frame_rate = VPQ_OUTPUT_60HZ; //165Hz => 60Hz
		}
	}
	else if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) >= V_FREQ_240000_mOFFSET ) && ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) < V_FREQ_240000_pOFFSET ) )	// 240Hz
	{
		...
		if(Get_DISPLAY_REFRESH_RATE() > 60){
			...
			memc_output_frame_rate = VPQ_OUTPUT_60HZ; //240Hz => 60Hz
		}
	}
	else																																									// others
	{
...
{
	M8P_rthw_lbmc_hwlbmc_ctrl0_RBUS M8P_rthw_lbmc_hwlbmc_ctrl0;
	unsigned char memc_output_panelsize = 0;
	unsigned int DISP_HPOS_ACT_END = 0, DISP_VPOS_ACT_END = 0;
	unsigned int lbmc_h_active = 0, lbmc_v_active = 0;
	static unsigned char print_cnt = 0;
	static unsigned char pre_PanelSize = MEMC_SIZE_NUM;

	DISP_HPOS_ACT_END = Get_DISP_ACT_END_HPOS();
	DISP_VPOS_ACT_END = Get_DISP_ACT_END_VPOS();
...
#if ENABLE_4K1K
		if(DISP_VPOS_ACT_END<=1088){
			memc_output_panelsize = MEMC_SIZE_4K1K;
		}
#endif
	...
	if(RUN_MERLIN8P()){
		M8P_rthw_lbmc_hwlbmc_ctrl0.regValue = rtd_inl(M8P_RTHW_LBMC_HWLBMC_CTRL0_reg);
		lbmc_h_active = M8P_rthw_lbmc_hwlbmc_ctrl0.lbmc_h_act;
		lbmc_v_active = M8P_rthw_lbmc_hwlbmc_ctrl0.lbmc_v_act;
	}else if(RUN_MERLIN8()){
		lbmc_h_active = (rtd_inl(LBMC_LBMC_20_reg)&LBMC_LBMC_20_lbmc_h_act_mask);
		lbmc_v_active = (rtd_inl(LBMC_LBMC_20_reg)&LBMC_LBMC_20_lbmc_v_act_mask);
	}//RUN_MERLIN8P & RUN_MERLIN8
	...
	if( ((print_cnt<=10) && (print_cnt>1)) || (memc_output_panelsize!=pre_PanelSize) ){
		rtd_pr_memc_notice( "[%s][%d][ACTEND,H=%d,V=%d,PANELTYPE=%d,][lbmc,hact=%d,vact=%d,][%d]\n",__FUNCTION__, __LINE__,
											...
											lbmc_h_active, lbmc_v_active, print_cnt);
	}
...
#ifndef BUILD_QUICK_SHOW
void Scaler_MEMC_PowerSaving_Disable_RTK2885p(void){
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
			return;
#else
#if 1//#ifdef KID23_TO_WEBOS
...
void Scaler_MEMC_PowerSaving_Disable_RTK2885pp(void){
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
	return;
#else
	//CONFIG_NEED_TUNNING
...
{
	if(RUN_MERLIN8P()){
		Scaler_MEMC_PowerSaving_Disable_RTK2885pp();
	}else if(RUN_MERLIN8()){
		Scaler_MEMC_PowerSaving_Disable_RTK2885p();
	}//RUN_MERLIN8P & RUN_MERLIN8
}
	...
	if(is_clk_on == 0){
		...
		if( mode == 0 ){
			if(HDMI_PowerSaving_stage == 0){
				...
				if(RUN_MERLIN8P()){
					//Merlin8P don't control the sram
					//Merlin8P don't control the mbist_xxx_ls
				}else if(RUN_MERLIN8()){
				//disable me sram
...
void UpdateSubInfo_ByCLKCtrl(void)
{
	M8P_rthw_crtc1_crtc1_14_RBUS M8P_rthw_crtc1_crtc1_14;
	M8P_rthw_crtc1_crtc1_44_RBUS M8P_rthw_crtc1_crtc1_44;
	...
	if(RUN_MERLIN8P()){
		...
		M8P_rthw_crtc1_crtc1_44.regValue = rtd_inl(M8P_RTHW_CRTC1_CRTC1_44_reg);
		M8P_rthw_crtc1_crtc1_14.crtc1_frc_hact = 0x3c0;
		M8P_rthw_crtc1_crtc1_44.crtc1_mc_vact = 0x438;
		rtd_outl(M8P_RTHW_CRTC1_CRTC1_14_reg, M8P_rthw_crtc1_crtc1_14.regValue);
		rtd_outl(M8P_RTHW_CRTC1_CRTC1_44_reg, M8P_rthw_crtc1_crtc1_44.regValue);
	}else if(RUN_MERLIN8()){
...
int Scaler_MEMC_MEMC_CLK_OnOff(unsigned char ME_enable, unsigned char MC_enable, unsigned char bMute)
{
	...
	static unsigned int frm_cnt_pre = 0;
	...
	if(((get_product_type() == PRODUCT_TYPE_DIAS && (Scaler_MEMC_Get_SW_OPT_Flag() == 1))) && ME_enable == 1){
		ME_enable = 0;
	...
	if(RUN_MERLIN8P()){
		ME_enable = 0;
		rtd_pr_memc_emerg("[%s][bringup] MERLIN8P, keep me=0.\n", __FUNCTION__);
	}//RUN_MERLIN8P & RUN_MERLIN8
	...
	crtc1_frc_hact_state = HAL_CRTC_GetFRCHact();
	MEMC_drvif_mux_state = rtd_inl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg)&0x1;
	if(RUN_MERLIN8P()){
		//Merlin8P don't control the sram
	}else if(RUN_MERLIN8()){
	...
	MC_DMA_read_i_state = HAL_MC_GetMC_LF_I_DMA_Enable();
	force_MEMC_on_en = ((rtd_inl(SOFTWARE1_SOFTWARE1_63_reg)>>29)&0x1);
	...
	down(&MEMC_CLK_Semaphore);
		if(do_power_saving == 2){  //MC clk on, ME clk off
			...
			if(bMute == 1){
				...
				if(RUN_MERLIN8P()){
					rtd_pr_memc_notice("[%s][%d][%d][%d][%x]\n", __FUNCTION__, __LINE__, (rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>12)&0x1, (rtd_inl(0xB801B6B8)-time2), rtd_inl(M8P_RTHW_MC_DBUS_WRAPPER_MEMC_MC_DCU1_ARB_SR0_reg));
				}else if(RUN_MERLIN8()){
					rtd_pr_memc_notice("[%s][%d][%d][%d][%x]\n", __FUNCTION__, __LINE__, (rtd_inl(PPOVERLAY_MEMC_MUX_CTRL_reg)>>12)&0x1, (rtd_inl(0xB801B6B8)-time2), rtd_inl(DBUS_WRAPPER_MEMC_DCU1_ARB_SR0_reg));
				}//RUN_MERLIN8P & RUN_MERLIN8
			...
			wait_cnt = 0;
			g_memc_outpur_vsync_cnt_2 = 0;
			frm_cnt_pre = g_memc_outpur_vsync_cnt_2;
			while(g_memc_outpur_vsync_cnt_2 <= (frm_cnt_pre+1)){
				//hw_msleep(1);
				usleep_range(1000, 2000);
				if((wait_cnt++>30) || (g_memc_outpur_vsync_cnt_2 == (frm_cnt_pre+2)))
					break;
			}
			#else
			time2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
			...
			if(RUN_MERLIN8P()){
				rtd_pr_memc_debug( "[resume_GGG][%x, %x, %x, %x, %x]\n\r", rtd_inl(M8P_RTHW_MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
			}else if(RUN_MERLIN8()){
				rtd_pr_memc_debug( "[resume_GGG][%x, %x, %x, %x, %x]\n\r", rtd_inl(MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
			}//RUN_MERLIN8P & RUN_MERLIN8
...
void Scaler_MEMC_Bypass_On(unsigned char b_isBypass)
{
    ...
    if(get_MEMC_bypass_status_refer_platform_model())   // MEMC Bypass case, default set uzu/memc line pixel delay to 0
    {
    ...
    else
    {
        ...
        if( ( b_isBypass && ( memc_mux_ctrl_reg.memc_outmux_sel == 1 ) ) || ( !b_isBypass && ( memc_mux_ctrl_reg.memc_outmux_sel == 0 ) ) )
            needUpdate = 1;

        if( !( Scaler_MEMC_Get_First_Run_FBG_enable() || Scaler_MEMC_Get_instanboot_resume_Done() || Scaler_MEMC_Get_snashop_resume_Done() ) )  // memc initial ready case need to run
        {
            if( b_isBypass && ( memc_mux_ctrl_reg.memc_outmux_sel == 1 ) )
            {
                if( ( Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC )
                  ...
                  && !( ( PPOVERLAY_uzudtg_control1_get_dtgm2uzuvs_line(IoReg_Read32(PPOVERLAY_uzudtg_control1_reg)) == 0 ) && ( PPOVERLAY_MEMCDTG_CONTROL3_get_dtgm2goldenpostvs_line(IoReg_Read32(PPOVERLAY_MEMCDTG_CONTROL3_reg)) == 0 ) ) )
                {
            ...
            else if( !b_isBypass && ( memc_mux_ctrl_reg.memc_outmux_sel == 0 ) )
            {
                vbe_disp_set_dtgM2uzu_lowdelay_mode(0);
            }
        }
...
//#ifndef CONFIG_MEMC_BYPASS
	...
	if(get_MEMC_bypass_status_refer_platform_model() == FALSE){
		...
		/*if( ( Get_DISPLAY_REFRESH_RATE() == 120 )
			...
			&& ( ( ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) >= V_FREQ_50000_mOFFSET ) && ( Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) < V_FREQ_60000_pOFFSET ) ) || ( enable == 0 ) ) )*/
		//k23
		if(((Get_DISPLAY_REFRESH_RATE() >= 120)
			&& !(vbe_disp_get_VRR_timingMode_flag() || vbe_disp_get_freesync_mode_flag())
			&& ((Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) >= V_FREQ_50000_mOFFSET) && (Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_FREQ_1000) < V_FREQ_60000_pOFFSET)))
			|| (enable == 0))
		{
			ppoverlay_double_buffer_ctrl2_RBUS ppoverlay_double_buffer_ctrl2_reg;
			ppoverlay_double_buffer_ctrl2_reg.regValue =  IoReg_Read32(PPOVERLAY_Double_Buffer_CTRL2_reg);
			...
			if(enable){
                set_ultra_low_delay_mode(ULTRALOW_DELAY_MODE_IN);
                scaler_disp_dtg_set_s1ip_dleay(1);
...
#endif
                    set_ultra_low_delay_mode(ULTRALOW_DELAY_MODE_OUT);
				}
				if(get_ultra_low_delay_mode() == ULTRALOW_DELAY_MODE_OUT)
				{
					vbe_disp_set_dtgM2uzu_lowdelay_mode(1);
				}
				else
				{
					if( !multiview_1p_input_in_2p_panel() && ( force_enable_two_step_uzu() || is_scaler_input_2p_mode(Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY)) || get_hdmi_vrr_4k60_mode() ) )
						vbe_disp_set_dtgM2uzu_lowdelay_mode(2);
					else
						vbe_disp_set_dtgM2uzu_lowdelay_mode(1);
				}
...
#endif
            set_ultra_low_delay_mode(ULTRALOW_DELAY_MODE_DEFAULT);
		}
	}
//#endif
}
...
#else
void Scaler_MEMC_MC_Set_MEMBoundary_RTK2885p(unsigned int  nDownlimit,unsigned int nUplimit)
{
#if 1//#ifdef KID23_TO_WEBOS
...
void Scaler_MEMC_MC_Set_MEMBoundary_RTK2885pp(unsigned int nDownlimit,unsigned int nUplimit)
{
...
{
	if(RUN_MERLIN8P()){
		Scaler_MEMC_MC_Set_MEMBoundary_RTK2885pp(nDownlimit, nUplimit);
	}else if(RUN_MERLIN8()){
		Scaler_MEMC_MC_Set_MEMBoundary_RTK2885p(nDownlimit, nUplimit);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
void Scaler_MEMC_allocate_memory(void){
	...
	unsigned int memc_size = 0, me_size = 0, mc_size = 0;
	unsigned long memc_addr = 0;
	//#ifdef CONFIG_BW_96B_ALIGNED
	unsigned long memc_addr_aligned = 0, tmp_addr_aligned[9] = {0};
	//#endif
	unsigned int tmp_addr[9] = {0};
	unsigned char u8_k = 0, mc_bsize = 0;
	unsigned int t0 = 0, t1 = 0, t2 = 0, t3 = 0, time_tmp[9];
	bool carvedout_en = carvedout_enable();
	...
	if(get_MEMC_bypass_status_refer_platform_model() == TRUE){
		rtd_pr_memc_emerg("[%s::%d][MEMC_bypass,%d,]\n", __func__, __LINE__, get_MEMC_bypass_status_refer_platform_model());
		return;
	}

if(carvedout_en == FALSE){ // CMA
	me_size = 0x9f25e0; // 0x783C00;
	#if 1
	if(MEMC_Lib_get_DisplayMode_Src() == VSC_INPUTSRC_HDMI){
		if(Get_DISPLAY_REFRESH_RATE() <= 60){
			mc_size = 0x11FEE00;
		}
		else{
	...
	rtd_pr_memc_emerg("[%s::%d][mc_bsize,%d,]\n", __func__, __LINE__, mc_bsize);
	#endif
	//CMA_memory_allocate_cnt = 0;
	rtd_pr_memc_emerg("[%s] MUTEX DOWN@%x\n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
	t0 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	down(&MEMC_CMA_Semaphore);
	for(u8_k = 0; u8_k < (mc_bsize + 1); u8_k++){
		t1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		if(unconsistant_addr[u8_k] == 0){
			if(u8_k == 0){
				unconsistant_mem_size[u8_k] = me_size;
				desc.pa_size[u8_k] = me_size;
				tmp_addr[u8_k] = pli_malloc(me_size, GFP_DCU3);
				time_tmp[u8_k] = (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-t1) /90;
			}
			else{
				unconsistant_mem_size[u8_k] = mc_size;
				desc.pa_size[u8_k] = mc_size;
				tmp_addr[u8_k] = pli_malloc(mc_size, GFP_DCU3);
				time_tmp[u8_k] = (rtd_inl(TIMER_SCPU_CLK90K_LO_reg)-t1) /90;
			}
			#if 1
			if(tmp_addr[u8_k] != 0 && tmp_addr[u8_k] != 0xffffffff){
				CMA_memory_allocate_cnt++;
        	          	desc.region_cnt++;
				unconsistant_addr[u8_k] = tmp_addr[u8_k];
				if(u8_k == 0){
					tmp_addr_aligned[u8_k] = dvr_memory_alignment((unsigned long)tmp_addr[u8_k], me_size);
				}
				else{
					tmp_addr_aligned[u8_k] = dvr_memory_alignment((unsigned long)tmp_addr[u8_k], mc_size);
				}
				unconsistant_addr_aligned[u8_k] = tmp_addr_aligned[u8_k];
				desc.pa_start[u8_k] = tmp_addr_aligned[u8_k];
			}
			else{
			}
			#endif
		}
		t2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	}

	// set DCMT
	dcmt_cma_memc_set(&desc);

	up(&MEMC_CMA_Semaphore);
	t3 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	rtd_pr_memc_emerg("[%s] MUTEX UP@%x, status=%d, \n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg), CMA_semaphone_status);
	for(u8_k = 0; u8_k < (mc_bsize + 1); u8_k++){
		if(u8_k == 0){
			rtd_pr_memc_emerg("[%s][u8_k,%d,][allcate %x bytes memory %px spend %03d ms]\n", __FUNCTION__, u8_k, 
				me_size, tmp_addr[u8_k], time_tmp[u8_k]);
		}
		else{
			rtd_pr_memc_emerg("[%s][u8_k,%d,][allcate %x bytes memory %px spend %03d ms]\n", __FUNCTION__, u8_k, 
				mc_size, tmp_addr[u8_k], time_tmp[u8_k]);
		}
	}

	if(CMA_memory_allocate_cnt != (mc_bsize + 1)){
		MEMC_Lib_Set_Memory_Quary_Success(0);
		rtd_pr_memc_emerg("[%s::%d][CMA_ERR][MEMC Get CMA Fail !!][CMA_memory_allocate_cnt = %d][buffer_num,%d,]\n", __FUNCTION__, __LINE__, CMA_memory_allocate_cnt, (mc_bsize + 1));
		return;
	}
	else{
		MEMC_Lib_Set_Memory_Quary_Success(1);
		rtd_pr_memc_emerg("[%s::%d][MEMC Get CMA Success !!][CMA_memory_allocate_cnt = %d][buffer_num,%d,]\n", __FUNCTION__, __LINE__, CMA_memory_allocate_cnt, (mc_bsize + 1));
	}
	
	rtd_pr_memc_emerg("[%s::%d][,%x,%x,%x,%x,%x,][time_stemp,%d,%d,%d,%d,]\n", __func__, __LINE__,
		desc.pa_start[0], desc.pa_start[1], desc.pa_start[2], desc.pa_start[3], desc.pa_start[4],
		t0, t1, t2, t3);
}
else{
	CMA_memory_allocate_cnt = (mc_bsize + 1);
	CMA_memory_release_flag = 0;
	MEMC_Lib_Set_Memory_Quary_Success(1);
	MEMC_Lib_Set_Memory_Write_Done(1);
	//MEMC_Lib_Set_CMA_Release_Stage(0);
...
void Regen_MEMC_ChangeSize(void){
#ifndef UT_flag
	unsigned char PCModeEnable = MEMC_LibGetMEMC_PCModeEnable();

	if(PCModeEnable == TRUE)
	{
		if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI_MEMC_FREERUN){
			if(MEMC_Lib_Get_InputVSXral_PCMode_422_change()){
				MEMC_LibSetInOutMode(MEMC_RGB_IN_PC_FREERUN_OUT,TRUE);
				MEMC_Lib_Set_InputVSXral_PCMode_422_change(0);
			} else {
				MEMC_LibSetInOutMode(MEMC_RGB_IN_PC_FREERUN_OUT,FALSE);
			}
		}else{
			if(MEMC_Lib_Get_InputVSXral_PCMode_422_change()){
				MEMC_LibSetInOutMode(MEMC_RGB_IN_PC_OUT,TRUE);
	...
	Mid_Mode_SetMEMC_PCModeEnable(PCModeEnable);
#endif
}

unsigned char Scaler_MEMC_CMA_Memory_Alloc(void){
#ifndef UT_flag
	memc_mux_memc_mux_func_ctrl_RBUS memc_mux_memc_mux_func_ctrl_reg;
	unsigned char ret = FALSE;

	rtd_pr_memc_emerg("[%s::%d][Memory_Quary_Success,%d,][,%x,%x,%x,%x,%x,][clock,%x,][CMA_semaphone_status,%d,]\n", __func__, __LINE__, MEMC_Lib_Get_Memory_Quary_Success(),
		...
		rtd_inl(SYS_REG_SYS_CLKEN3_reg), CMA_semaphone_status);
	...
	CMA_semaphone_status = 1;
	if(MEMC_Lib_Get_Memory_Quary_Success() == 0){
		// allocate memory
		Scaler_MEMC_allocate_memory();
	}

	if(MEMC_Lib_Get_Memory_Quary_Success() == 1){
		MEMC_LibBlueScreen(TRUE);
		// regen change size
//		MEMC_Lib_Set_MEMC_Regen_Address();
		MEMC_Lib_Set_Memory_Write_Done(0);

		MEMC_Lib_Set_Change_Size_Force_mode(1);
		/*
		rtd_pr_memc_emerg("[%s::%d][DMA check][DMA_en,%d,%d,%d,%d,%d,,%d,%d,%d,%d,%d,]\n", __func__, __LINE__, 
			(rtd_inl(0xb809c038)>>20)&0x1, (rtd_inl(0xb809c040)>>20)&0x1, (rtd_inl(0xb809c048)>>20)&0x1, (rtd_inl(0xb809c050)>>20)&0x1, (rtd_inl(0xb809c070)>>20)&0x1,
			(rtd_inl(0xb809c090)>>20)&0x1, (rtd_inl(0xb809c098)>>20)&0x1, (rtd_inl(0xb809c0a0)>>20)&0x1, (rtd_inl(0xb809c114)>>20)&0x1, (rtd_inl(0xb809c11c)>>20)&0x1,
			(rtd_inl(0xb809c138)>>20)&0x1, (rtd_inl(0xb809c140)>>20)&0x1, (rtd_inl(0xb809c15c)>>20)&0x1, (rtd_inl(0xb809c164)>>20)&0x1, (rtd_inl(0xb809c180)>>20)&0x1,
			(rtd_inl(0xb809c26c)>>20)&0x1, (rtd_inl(0xb809c270)>>20)&0x1, (rtd_inl(0xb809c274)>>20)&0x1, (rtd_inl(0xb809c278)>>20)&0x1, (rtd_inl(0xb809c28c)>>20)&0x1);
		rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,]\n", __FUNCTION__, __LINE__, 
			(rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
			(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
			(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
			(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf);
		*/
		#if 1 // for jenkins
		Regen_MEMC_ChangeSize();
		#else
		if(PCModeEnable == TRUE)
		{
			if(Get_DISPLAY_PANEL_TYPE() == P_VBY1_TO_HDMI_MEMC_FREERUN){
		...
		*/
		...
		memc_mux_memc_mux_func_ctrl_reg.memc_mux_en = 1;
		rtd_outl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg, memc_mux_memc_mux_func_ctrl_reg.regValue);
		// enable DB_en
		MEMC_LibSet_LBMC_DB_en(1);
		HAL_IP_SetLbmcDBAppy(1);
		...
		rtd_pr_memc_emerg("[%s::%d][CMA_ERR][MEMC_Lib_Get_Memory_Quary_Success = 0]\n", __func__, __LINE__);
		ret = FALSE;
	}
	CMA_semaphone_status = 2;
	rtd_pr_memc_emerg("[%s::%d][,%x,%x,%x,%x,%x,][CMA_semaphone_status,%d,]\n", __func__, __LINE__,
		unconsistant_addr_aligned[0], unconsistant_addr_aligned[1], unconsistant_addr_aligned[2], unconsistant_addr_aligned[3], unconsistant_addr_aligned[4], CMA_semaphone_status);
#endif

	return ret;
}

void Scaler_MEMC_CMA_WaitInputVsync(void){
	unsigned char wait_cnt = 0;
	static unsigned int input_frm_cnt_pre = 0, frm_cnt_pre = 0;
	wait_cnt = 0;
	g_memc_input_vsync_cnt = 0;
	g_memc_outpur_vsync_cnt = 0;
	input_frm_cnt_pre = g_memc_input_vsync_cnt;
	frm_cnt_pre = g_memc_outpur_vsync_cnt;
	while(g_memc_input_vsync_cnt <= (input_frm_cnt_pre+0) || g_memc_outpur_vsync_cnt <= (frm_cnt_pre+0)){
		//hw_msleep(1);
		usleep_range(1000, 2000);
		if((wait_cnt++>50) || (g_memc_input_vsync_cnt >= (input_frm_cnt_pre+1) && g_memc_outpur_vsync_cnt >= (frm_cnt_pre+1)))
			break;
	}

	rtd_pr_memc_emerg("[%s][%d][wait_cnt,%d,][in_vsync_cnt,%d,%d,][out_vsync_cnt,%d,%d,]\n", __FUNCTION__, __LINE__, 
		wait_cnt, g_memc_input_vsync_cnt, input_frm_cnt_pre, g_memc_outpur_vsync_cnt, frm_cnt_pre);
}

void Scaler_MEMC_CMA_WaitOutputVsync(void){
	unsigned char wait_cnt = 0;
	static unsigned int frm_cnt_pre = 0;
	...
	g_memc_outpur_vsync_cnt = 0;
	frm_cnt_pre = g_memc_outpur_vsync_cnt;
	while(g_memc_outpur_vsync_cnt <= (frm_cnt_pre+1)){
		//hw_msleep(1);
		usleep_range(1000, 2000);
		if((wait_cnt++>40) || (g_memc_outpur_vsync_cnt == (frm_cnt_pre+2)))
			break;
	}

	rtd_pr_memc_emerg("[%s][%d][wait_cnt,%d,][in_vsync_cnt,%d,%d,][out_vsync_cnt,%d,%d,]\n", __FUNCTION__, __LINE__, 
		wait_cnt, g_memc_outpur_vsync_cnt, frm_cnt_pre);
}

void Scaler_MEMC_CMA_Memory_Release(void){
#ifndef UT_flag
	unsigned char u8_k = 0;
	unsigned int time1 = 0, time2 = 0, t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4 = 0, wait_cnt = 0;
	//static unsigned int input_frm_cnt_pre = 0, frm_cnt_pre = 0;
	dcmt_cma_desc_t desc = 
	{
		.pa_start = {0},
	...
	rtd_pr_memc_emerg("[%s::%d][MEMC release start !!][,%x,%x,%x,%x,%x,]\n", __func__, __LINE__,
		unconsistant_addr[0], unconsistant_addr[1], unconsistant_addr[2], unconsistant_addr[3], unconsistant_addr[4]);
	...
	// set release_flag 1
	CMA_memory_release_flag = 1;

	// Mute to prevent garbage happens
	MEMC_LibBlueScreen(TRUE);

	rtd_pr_memc_emerg("[%s][%d][CMA_semaphone_status check,%d,][time,%d,]\n", __FUNCTION__, __LINE__, CMA_semaphone_status, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
	time1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	wait_cnt = 0;
	while(CMA_semaphone_status == 1){
		//hw_msleep(1);
		usleep_range(1000, 2000);
		if(wait_cnt++>42)
			break;
	...
	time2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	rtd_pr_memc_emerg("[%s][%d][CMA_semaphone_status check,%d,][time,%d,%d,%d,][wait_cnt,%d,]\n", __FUNCTION__, __LINE__, 
		CMA_semaphone_status, time2-time1, time2, time1, wait_cnt);
	...
	time1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	#if 1 // for Jenkins
	Scaler_MEMC_CMA_WaitOutputVsync();
	#else
	wait_cnt = 0;
	g_memc_outpur_vsync_cnt = 0;
	frm_cnt_pre = g_memc_outpur_vsync_cnt;
	...
	// disable mux_en
	memc_mux_memc_mux_func_ctrl_reg.memc_mux_en = 0;
	rtd_outl(MEMC_MUX_MEMC_MUX_FUNC_CTRL_reg, memc_mux_memc_mux_func_ctrl_reg.regValue);
	if(RUN_MERLIN8P()){
		rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][vsync_cnt,%d,]\n", __FUNCTION__, __LINE__, 
			(rtd_inl(M8P_RTHW_MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
			(rtd_inl(M8P_RTHW_MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(M8P_RTHW_MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(M8P_RTHW_MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
			(rtd_inl(M8P_RTHW_MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(M8P_RTHW_MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, g_memc_outpur_vsync_cnt);
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][vsync_cnt,%d,]\n", __FUNCTION__, __LINE__, 
		(rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, g_memc_outpur_vsync_cnt);
	}//RUN_MERLIN8P & RUN_MERLIN8

	//wait
	time1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	#if 1 // for Jenkins
	Scaler_MEMC_CMA_WaitInputVsync();
	#else
	wait_cnt = 0;
	g_memc_input_vsync_cnt = 0;
	g_memc_outpur_vsync_cnt = 0;
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][in_vsync_cnt,%d,][vsync_cnt,%d,]\n", __FUNCTION__, __LINE__, 
			(rtd_inl(M8P_RTHW_MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
			(rtd_inl(M8P_RTHW_MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(M8P_RTHW_MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(M8P_RTHW_MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
			(rtd_inl(M8P_RTHW_MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(M8P_RTHW_MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, g_memc_input_vsync_cnt, g_memc_outpur_vsync_cnt);
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_emerg("[%s][%d][outstanding,%x,%x,%x,%x,,%x,%x,%x,%x,%x,%x,][time,%d,%d,%d,][in_vsync_cnt,%d,][out_vsync_cnt,%d,]\n", __FUNCTION__, __LINE__, 
		(rtd_inl(MVINFO_DMA_MVINFO_WR_status_reg)>>8)&0x7, (rtd_inl(MVINFO_DMA_MVINFO_RD_status_reg)>>8)&0x7,
		(rtd_inl(ME_SHARE_DMA_ME_DMA_WR_status_reg)>>8)&0x7, (rtd_inl(ME_SHARE_DMA_ME_DMA_RD_status_reg)>>8)&0x7, (rtd_inl(MC_DMA_MC_LF_DMA_WR_status_reg)>>8)&0x1f,
		(rtd_inl(MC_DMA_MC_HF_DMA_WR_status_reg)>>8)&0x1f, (rtd_inl(MC_DMA_MC_LF_I_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_I_DMA_RD_status_reg)>>8)&0xf,
		(rtd_inl(MC_DMA_MC_LF_P_DMA_RD_status_reg)>>8)&0xf, (rtd_inl(MC_DMA_MC_HF_P_DMA_RD_status_reg)>>8)&0xf, time2-time1, time2, time1, g_memc_input_vsync_cnt, g_memc_outpur_vsync_cnt);
	}//RUN_MERLIN8P & RUN_MERLIN8

	// release memory
	rtd_pr_memc_emerg("[%s] MUTEX DOWN@%x\n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
	t0 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	if(CMA_semaphone_status != 1){
	down(&MEMC_CMA_Semaphore);
	// unset DCMT
	dcmt_cma_memc_unset(&desc);

	for(u8_k = 0; u8_k < 9; u8_k++){
		t1 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		if(unconsistant_addr[u8_k] != 0){
			// set DCMT addr
			//desc.pa_start[u8_k] = unconsistant_addr_aligned[u8_k];
			//desc.pa_size[u8_k] = unconsistant_mem_size[u8_k];
			// free
			pli_free(unconsistant_addr[u8_k]);
			unconsistant_addr[u8_k] = 0;
			unconsistant_addr_aligned[u8_k] = 0;
			t2 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		}
		t3 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	}
	...
	// reset flag
	CMA_semaphone_status = 0;
	CMA_memory_allocate_cnt = 0;
	CMA_release_tsk_flag = 0;
	MEMC_Lib_Set_Memory_Quary_Success(0);
	MEMC_Lib_Set_Memory_Write_Done(0);
	}
	t4 = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	rtd_pr_memc_emerg("[%s] MUTEX UP@%x, CMA_semaphone_status=%d, \n", __FUNCTION__, rtd_inl(TIMER_SCPU_CLK90K_LO_reg), CMA_semaphone_status);

	rtd_pr_memc_emerg("[%s::%d][MEMC release end !!][,%x,%x,%x,%x,%x,,%x,%x,%x,%x,][time_stemp,%d,%d,%d,%d,%d,]\n", __func__, __LINE__,
		unconsistant_addr[0], unconsistant_addr[1], unconsistant_addr[2], unconsistant_addr[3], unconsistant_addr[4],
		unconsistant_addr[5], unconsistant_addr[6], unconsistant_addr[7], unconsistant_addr[8],
		t0, t1, t2, t3, t4);
...
extern int vbe_dtg_memec_bypass_switch(void);
extern unsigned int memc_dma_error_cnt;
...
void Scaler_MEMC_Handler(void){
...
#endif
	{
		...
		if(get_reset_MEMC_flg()==true){
			rtd_pr_memc_notice("Scaler_MEMC_Handler HSRonoff\n");
	...
	if(MEMC_instanboot_resume_Done == 1){
		if(Get_DISP_ACT_END_VPOS()<=1088) { //4k1k
			Scaler_MEMC_SetInOutUseCase(VPQ_INPUT_3840X1080, VPQ_OUTPUT_3840X1080, VPQ_INPUT_3D_LBL, VPQ_OUTPUT_3D_PR);
		}
		else {
...
void Scaler_ME_OnOff_RTK2885p(unsigned char enable)
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
				return;
#else
	...
	#ifndef BUILD_QUICK_SHOW
		if(g_KERNEL_AFTER_QS_MEMC_FLG==false|| ((get_product_type() != PRODUCT_TYPE_DIAS)&& (Scaler_MEMC_Get_SW_OPT_Flag() == 1))){
...
void Scaler_ME_OnOff_RTK2885pp(unsigned char enable)
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
	return;
#else
	...
	#ifndef BUILD_QUICK_SHOW
		if(g_KERNEL_AFTER_QS_MEMC_FLG==false|| ((get_product_type() != PRODUCT_TYPE_DIAS)&& (Scaler_MEMC_Get_SW_OPT_Flag() == 1))){
			M8P_rthw_me_dma_rtme_dm0_w.ipmeiir_enable = bEnable;
			M8P_rthw_me_dma_rtme_dm0_w.ipme3_enable = bEnable;
		}
	#endif
...
{
	if(RUN_MERLIN8P()){
		Scaler_ME_OnOff_RTK2885pp(enable);
	}else if(RUN_MERLIN8()){
		Scaler_ME_OnOff_RTK2885p(enable);
	}//RUN_MERLIN8P & RUN_MERLIN8
}

void Scaler_MC_OnOff_RTK2885p(UINT8 enable){
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
					return;
#else
...
void Scaler_MC_OnOff_RTK2885pp(UINT8 enable)
{
#ifdef CONFIG_MEMC_NOTSUPPORT // for code size reduce, when MEMC_BYPASS
	return;
#else
	...
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_LF_WDMA_MEND1_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_LF_WDMA_MEND1_reg, 0xffefffff, enable_bit);
	//IoReg_Mask32(mc_01_write_enable_ADDR, 0xffefffff, enable_bit); //register remove
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_NUM_BL_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART1_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART3_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART5_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART7_reg, 0xffefffff, enable_bit);
	IoReg_Mask32(M8P_RTHW_MC_DMA_MC_HF_WDMA_MEND1_reg, 0xffefffff, enable_bit);
#endif
#endif
}
...
{
	if(RUN_MERLIN8P()){
		Scaler_MC_OnOff_RTK2885pp(enable);
	}else if(RUN_MERLIN8()){
		Scaler_MC_OnOff_RTK2885p(enable);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
{		
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("%s %d [,%x,%x,%x,]\n",__func__,__LINE__, rtd_inl(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg), rtd_inl(0xb809d5fc));
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("%s %d [,%x,%x,%x,]\n",__func__,__LINE__, rtd_inl(KMC_TOP_kmc_top_18_reg), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg), rtd_inl(0xb809d5fc));
	}//RUN_MERLIN8P & RUN_MERLIN8
	return 0;
}
...
static int vpq_memc_resume_snashop (struct device *p_dev)
{
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("%s %d [,%x,%x,%x,][%d][%d]\n",__func__,__LINE__, rtd_inl(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg), rtd_inl(0xb809d5fc),MEMC_First_Run_Done,MEMC_First_Run_force_SetInOutUseCase_Done);
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("%s %d [,%x,%x,%x,][%d][%d]\n",__func__,__LINE__, rtd_inl(KMC_TOP_kmc_top_18_reg), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg), rtd_inl(0xb809d5fc),MEMC_First_Run_Done,MEMC_First_Run_force_SetInOutUseCase_Done);
	}//RUN_MERLIN8P & RUN_MERLIN8
...
void vpq_memc_disp_suspend_snashop(void){	
		...
		Scaler_MEMC_Set_MC_MEMBoundary(gphy_addr_kmc00, (gphy_addr_kmc00 + Get_KMC_TOTAL_SIZE())); 
			...
			if(RUN_MERLIN8P()){
				...
				//DMA_CHECK
				//ME memory boundary
				vpq_memc_suspend_resume.MEMC_DBUS_WRAP_ME_DOWNLIMIT = IoReg_Read32(ME_DOWN_LIMIT_REG);
				vpq_memc_suspend_resume.MEMC_DBUS_WRAP_ME_UPLIMIT = IoReg_Read32(ME_UP_LIMIT_REG);

				//MC memory boundary
				vpq_memc_suspend_resume.MC_dma_low_limit = IoReg_Read32(M8P_RTHW_MC_DMA_MC_LF_DMA_WR_Rule_check_low_reg);
				vpq_memc_suspend_resume.MC_dma_up_limit = IoReg_Read32(M8P_RTHW_MC_DMA_MC_HF_DMA_WR_Rule_check_up_reg);
			}else if(RUN_MERLIN8()){
			//ME memory boundary
...
//#endif
	...
	if(get_MEMC_bypass_status_refer_platform_model() != TRUE){//KTASKWBS-24448
		...
		if(RUN_MERLIN8P()){
			...
			//DMA_CHECK
			//ME memory boundary
			Scaler_MEMC_Set_ME_MEMBoundary(vpq_memc_suspend_resume.MEMC_DBUS_WRAP_ME_DOWNLIMIT, vpq_memc_suspend_resume.MEMC_DBUS_WRAP_ME_UPLIMIT);
		}else if(RUN_MERLIN8()){
		//ME memory boundary
...
void get_regVal_by_suspend_RTK2885p(void)
{
	...
	for(u32_i = 0; u32_i <= MAX_PAGE_NUMBER; u32_i++){
		for(u32_j = 0; u32_j < 64; u32_j++){
			...
			tmp_reg = MEMC_REG_BASE_OFFSET + (u32_i*256) + (u32_j*4);
...
void get_regVal_by_suspend_RTK2885pp(void)
{
	unsigned int u32_i = 0, u32_j = 0;
	//can't reset: 0x50, 0x4f
	//empty: 0x10~0x15,  0x2e, 0x56~0x5b
	...
	for(u32_i = 0; u32_i <= MAX_PAGE_NUMBER; u32_i++){
		for(u32_j = 0; u32_j < 64; u32_j++){
			...
			if(u32_i <= 0x0f \
				|| (u32_i >= 0x16 && u32_i <= 0x1f) \
				|| (u32_i >= 0x20 && u32_i <= 0x2d) || u32_i == 0x2f \
				|| (u32_i >= 0x30 && u32_i <= 0x3f) \
				|| (u32_i >= 0x40 && u32_i <= 0x4e) \
				|| (u32_i >= 0x51 && u32_i <= 0x55) || (u32_i >= 0x5c && u32_i <= 0x5f) \
				|| (u32_i >= 0x60 && u32_i <= 0x6f)){
				vpq_memc_suspend_resume.MEMC_reg[u32_i][u32_j] = IoReg_Read32(tmp_reg);
			}else{
				vpq_memc_suspend_resume.MEMC_reg[u32_i][u32_j] = 0;
			}
...
void get_regVal_by_suspend(void)
{
	if(RUN_MERLIN8P()){
		get_regVal_by_suspend_RTK2885pp();
	}else if(RUN_MERLIN8()){
		get_regVal_by_suspend_RTK2885p();
	}//RUN_MERLIN8P & RUN_MERLIN8
}

void set_regVal_by_resume_RTK2885p(void)
{
	unsigned int u32_i = 0, u32_j = 0;
...
void set_regVal_by_resume_RTK2885pp(void)
{
	unsigned int u32_i = 0, u32_j = 0;
	//can't reset: 0x50, 0x4f
	//empty: 0x10~0x15,  0x2e, 0x56~0x5b
...
void set_regVal_by_resume(void)
{
	if(RUN_MERLIN8P()){
		set_regVal_by_resume_RTK2885pp();
	}else if(RUN_MERLIN8()){
		set_regVal_by_resume_RTK2885p();
	}//RUN_MERLIN8P & RUN_MERLIN8
}

unsigned int disable_interrupt_by_suspend(void)
{
	M8P_rthw_ippre_top_ippre1_b0_RBUS M8P_rthw_ippre_top_ippre1_b0;
	unsigned int u32_interrupt_reg = 0;

	if(RUN_MERLIN8P()){
		M8P_rthw_ippre_top_ippre1_b0.regValue = IoReg_Read32(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg);
		M8P_rthw_ippre_top_ippre1_b0.kmc_int_en = 0;
		IoReg_Write32(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg, M8P_rthw_ippre_top_ippre1_b0.regValue);
	}else if(RUN_MERLIN8()){
		//reg_kmc_int_en
	...
	return u32_interrupt_reg;
}

unsigned int ennble_interrupt_by_resume(void)
{
	M8P_rthw_ippre_top_ippre1_b0_RBUS M8P_rthw_ippre_top_ippre1_b0;
	unsigned int u32_interrupt_reg = 0;
	...
	if(RUN_MERLIN8P()){
		M8P_rthw_ippre_top_ippre1_b0.regValue = IoReg_Read32(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg);
		M8P_rthw_ippre_top_ippre1_b0.kmc_int_en |= 0x2;
		IoReg_Write32(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg, M8P_rthw_ippre_top_ippre1_b0.regValue);
	}else if(RUN_MERLIN8()){
		//reg_kmc_int_en
		u32_interrupt_reg = IoReg_Read32(KMC_TOP_kmc_top_18_reg);
...
rtd_pr_memc_notice("[Scaler_MEMC_Bypass_CLK_OnOff_JJJ][%x]\n", rtd_inl(0xb802e800));
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[suspend_FFF][%x, %x, %x, %x, %x]\n\r", rtd_inl(0xb802e800), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg));
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[suspend_FFF][%x, %x, %x, %x, %x]\n\r", rtd_inl(0xb802e800), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg));
	}//RUN_MERLIN8P & RUN_MERLIN8
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[suspend_EEE][%x, %x, %x, %x, %x]\n\r", rtd_inl(M8P_RTHW_MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg));
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[suspend_EEE][%x, %x, %x, %x, %x]\n\r", rtd_inl(MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg));
	}//RUN_MERLIN8P & RUN_MERLIN8
...
//#endif
	...
	MEMC_resume_check = 0;
		...
		rtd_pr_memc_notice("[suspend_K25844][%x, %x, %x, %x, %x][%x, %x, %x, %x, %x]\n\r",MEMC_instanboot_resume_check[0],MEMC_instanboot_resume_check[1],MEMC_instanboot_resume_check[2],MEMC_instanboot_resume_check[3],MEMC_instanboot_resume_check[4]
																			,MEMC_instanboot_resume_check[5],MEMC_instanboot_resume_check[6],MEMC_instanboot_resume_check[7],MEMC_instanboot_resume_check[8],MEMC_instanboot_resume_check[9]);	
	//rtd_pr_memc_notice("[%s][%x, %x, %x, %x, %x]\n\r", __FUNCTION__, rtd_inl(MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
...
void vpq_memc_disp_resume_instanboot(void)
{
	...
	ppoverlay_memc_mux_ctrl_bg_RBUS memc_mux_ctrl_bg_reg;
	unsigned int u32_interrupt_reg;
...
//#endif
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[resume_DDD][%x, %x, %x, %x, %x]\n\r", rtd_inl(M8P_RTHW_MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(SOFTWARE2_SOFTWARE2_63_reg));
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[resume_DDD][%x, %x, %x, %x, %x]\n\r", rtd_inl(MC_DMA_MC_LF_WDMA_MSTART0_reg), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(SOFTWARE2_SOFTWARE2_63_reg));
	}//RUN_MERLIN8P & RUN_MERLIN8

#if 1   //Load part registers dump
	set_regVal_by_resume();
	...
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[resume_AAA][%x, %x, %x, %x, %x]\n\r", rtd_inl(0xb802e800), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[resume_AAA][%x, %x, %x, %x, %x]\n\r", rtd_inl(0xb802e800), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(0xb809d7fc));
	}//RUN_MERLIN8P & RUN_MERLIN8
	...
	// enable interrupt
	u32_interrupt_reg = ennble_interrupt_by_resume();

	//reg_post_int_en		
#if 0
	rtd_pr_memc_notice("[resume_A][%x, %x, %x, %x, %x][%x]\n\r", rtd_inl(0xb802e800), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg),u32_interrupt_reg);
...
#endif
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[resume_AA][%x, %x, %x, %x, %x][%x][%d]\n\r", rtd_inl(0xb802e800), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg),u32_interrupt_reg,MEMC_resume_check);
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[resume_AA][%x, %x, %x, %x, %x][%x][%d]\n\r", rtd_inl(0xb802e800), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg),u32_interrupt_reg,MEMC_resume_check);
	}//RUN_MERLIN8P & RUN_MERLIN8
	...
	MEMC_resume_check = 1;
	if(RUN_MERLIN8P()){
		rtd_pr_memc_notice("[resume_AAA][%x, %x, %x, %x, %x][%x]\n\r", rtd_inl(0xb802e800), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(M8P_RTHW_PHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg),u32_interrupt_reg);
	}else if(RUN_MERLIN8()){
		rtd_pr_memc_notice("[resume_AAA][%x, %x, %x, %x, %x][%x]\n\r", rtd_inl(0xb802e800), rtd_inl(MC_DMA_MC_HF_WDMA_MSTART0_reg), rtd_inl(KPHASE_kphase_00_reg), rtd_inl(0xb809d5fc), rtd_inl(KPOST_TOP_KPOST_TOP_08_reg),u32_interrupt_reg);
	}//RUN_MERLIN8P & RUN_MERLIN8
...
long vpq_memc_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case VPQ_IOC_MEMC_GETFIRMWAREVERSION:
		{
			...
			{
				retval = 0;
				MEMC_DBG_PRINT("Ver:%d.%d Date:%d.%d.%d.%d!!\n", ((u32_Version>>26)&0x3f), ((u32_Version>>19)&0x7f), ((u32_Version>>12)&0x7f), ((u32_Version>>8)&0xf), ((u32_Version>>3)&0x1f), (u32_Version&0x7));
			}
			break;
		...
		case VPQ_IOC_MEMC_SET_TABLE_FROM_BIN:
		//copy from VPQ_EXTERN_IOC_SET_OverDriver_Para
		{
			extern SLR_VIP_TABLE_MEMC g_IDENTIFY_TABLE;

			g_Load_MEMC_Bin = false;
			if (copy_from_user(&HAL_VPQ_DATA_EXTERN, (int __user *)arg, sizeof(HAL_VPQ_DATA_EXTERN_T))) {
				rtd_pr_memc_err("VPQ_IOC_MEMC_SET_TABLE_FROM_BIN hal_VPQ_DATA_EXTERN copy fail\n");
				retval = -1;
			} else {
				Data_addr = HAL_VPQ_DATA_EXTERN.pData;
				
				rtd_pr_vpq_info("kernel VPQ_IOC_MEMC_SET_TABLE_FROM_BIN,version=%d,cmd=%d,length=%d,pData=%x,\n",
				...
				{
					if (HAL_VPQ_DATA_EXTERN.version == VPQ_DEV_EXTERN_MEMC_PARAM_LOAD_BIN) {
						if (HAL_VPQ_DATA_EXTERN.length != sizeof(SLR_VIP_TABLE_MEMC)) {	// no bin file
							rtd_pr_vpq_info("kernel VPQ_IOC_MEMC_SET_TABLE_FROM_BIN, no bin file here. use kernel default len=%d,\n", HAL_VPQ_DATA_EXTERN.length);
							retval = 0;
						} else {
							Data_addr = HAL_VPQ_DATA_EXTERN.pData;
							if(copy_from_user(&g_IDENTIFY_TABLE, (int __user *)Data_addr, HAL_VPQ_DATA_EXTERN.length))
							{	
								rtd_pr_memc_err("VPQ_IOC_MEMC_SET_TABLE_FROM_BIN bin table copy fail\n");
								retval = -1;
							} else {
								rtd_pr_vpq_info("kernel VPQ_IOC_MEMC_SET_TABLE_FROM_BIN bin ok\n");
								g_Load_MEMC_Bin = true;
								Check_MEMC_identify_Bin();
								Set_MEMC_Style_By_Bin();
								retval = 0;
							}
						}
						Check_MEMC_identify_Bin();
					} else {
						rtd_pr_memc_err("kernel VPQ_IOC_MEMC_SET_TABLE_FROM_BIN version error\n");
						retval = -1;
					}
				}
				else 
				{
					rtd_pr_memc_err("kernel VPQ_IOC_MEMC_SET_TABLE_FROM_BIN only set\n");
					retval = -1;
				}
			}
		}
...
int vpq_memc_module_init(void)
{
	...
	sema_init(&MEMC_CMA_Semaphore, 1);
