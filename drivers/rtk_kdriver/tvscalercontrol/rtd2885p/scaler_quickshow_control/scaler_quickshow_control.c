...
unsigned char scaler_quickshow_get_displayinfo_proc(unsigned char eVgipSrcType, unsigned char inputSrctype)//This is for driver base implement open source
{
    ...
    if(drv_memory_get_low_delay_game_mode_dynamic()  || Get_PANEL_VFLIP_ENABLE() || force_enable_mdomain_2disp_path())
    {
	...
	{//copy from main for sub m domain
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_drv_memory_set_limit_boundary_mode(SLR_SUB_DISPLAY, _ENABLE);
		}
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nonlibdma_drv_memory_set_limit_boundary_mode(SLR_MAIN_DISPLAY, 1);
	}
...
unsigned char scaler_quickshow_Setup_IMD(unsigned char inputType, StructDisplayInfo* info)
{
	...
	{//enable online watchdog first to protect panel
		if(get_force_i3ddma_enable(SLR_MAIN_DISPLAY))
			drvif_hdmi_dma_onlinemeasure_watchdog_early_enable_setting();//enable online watchdog first
		else
			drvif_hdmi_onlinemeasure_watchdog_early_enable_setting();//enable online watchdog first
	}
	...
	if((get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_game_mode_dynamic() == _ENABLE) && (!is_vrr_or_freesync_mode())) {//VRR has been set
		...
		{//if game mode data framesync
			...
			if(get_mach_type() == MACH_ARCH_RTK2885P) {
				...
				nonlibdma_drv_GameMode_iv2dv_delay(_ENABLE);
			}
		}
	...
	if((get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic() == _ENABLE) && (drvif_scaler_get_display_mode() == DISPLAY_MODE_FRAME_SYNC)) {
			//drv_memory_GameMode_Switch_SingleBuffer();
			nonlibdma_drv_memory_GameMode_Switch_SingleBuffer();
	}
    ...
    if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_FRAMESYNC) == FALSE){
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_drv_memory_wait_cap_last_write_done(SLR_MAIN_DISPLAY, 2, FALSE);
		}
    }
...
unsigned char scaler_quickshow_run_scaler(unsigned char inputType)
{
...
#endif
   {
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P && nonlibdma_drv_memory_get_low_delay_game_mode_dynamic() == _ENABLE){
		//drv_memory_set_game_mode(_ENABLE);
	} else {
		//drv_memory_set_game_mode(_DISABLE);
	}
    decide_vpq_check_pc_rgb444_mode_on_off();
    decide_vpq_check_pc_yuv422_in_444_out_mode_on_off();
#if 1
...
void scaler_quickshow_prepareInfo(enum SOURCE inputType)
{
    ...
    if(is_QS_game_mode())
    {
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_drv_memory_set_game_mode_dynamic(1);
		}
    }
    else
    {
		...
		if(get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			nonlibdma_drv_memory_set_game_mode_dynamic(0);
		}
    }
...
unsigned char scaler_quickshow_output_connect(enum SOURCE inputType)
{
	...
	if(get_mach_type() == MACH_ARCH_RTK2885P) {
		...
		nonlibdma_scaler_quickshow_memery_init();
	}
