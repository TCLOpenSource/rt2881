...
void drv_I3DDMA_ApplySetting(unsigned char display)
{
	...
	else if((i3ddmaCtrl.ptx_timing->h_act_len ==1440) && (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == VGIP_SRC_ATV))
	{
		...
		&& (DMA_VGIP_DMA_VGIP_CTRL_get_dma_in_sel(rtd_inl(DMA_VGIP_DMA_VGIP_CTRL_reg)) == get_vgip_in_sel_by_display(display)))
	{
...
static void  drvif_I3DDMA_Enable_SqCapture(unsigned char enable, IDMA_DISPD_CAPTURE_INFO *capInfo)
{
	...
	if(capInfo->capSrc == CAP_SRC_UZU )
		CVoSqmodeTVERpc(capInfo->capWid, capInfo->capLen, Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000)/100);
	else
...
static void scaler_vodmatoI3ddma_save_vgip1Info(void)
{
	...
	imdavgip2CtrlInfo.srcInfo.frame_rate		= Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000);
...
void fw_i3ddma_set_vgip(unsigned char src, unsigned char mode)
{
	...
	else if(get_vsc_src_is_hdmi_or_dp())	//hdmi or dprx
	{
		...
		{//only interlace need delay
			...
			rtd_pr_i3ddma_info("[%s] %d dma_delay=%d, interlace=%d, input_vsync_pol=0x%x, input_vtotal=%d, input_vsync_width=%d\n", __FUNCTION__, __LINE__, u32_cal_ivs_dly, Scaler_DispGetStatus(SLR_MAIN_DISPLAY, SLR_DISP_INTERLACE), Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_SYNC_HIGH_PERIOD), Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_V_COUNT), Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_IVSYNCPULSECOUNT));
		}
	}
	else
	{
