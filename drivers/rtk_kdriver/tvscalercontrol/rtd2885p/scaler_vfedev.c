...
{//src type is from VSC_INPUT_TYPE_T
	...
	switch (src) {
		...
		case VSC_INPUTSRC_HDMI:
			if(flag == SOURCE_GOOD_TIMING_READY)//ignore the case
				return;
	...
	if (HDMI_Global_Status == SRC_CONNECT_DONE)	{
		if (vfe_hdmi_drv_is_current_channel(hdmitiminginfo->port)) {
			...
			if (HDMI_Reply_Zero_Timing_mute_cnt == ACTUAL_DETECT_RESULT) {
				...
				if(vbe_check_input_support_otp_clock(hdmitiminginfo->h_total, hdmitiminginfo->v_total, hdmitiminginfo->v_freq/10) == FALSE)
				{
					hdmitiminginfo->ext_flags = VFE_HDMI_TIMING_OUTPUT_OF_DISP_LIMIT; //out limit
				}
				ret = (timing_ready) ? 0 : -1;
   				...
   				if(HDMI_Reply_Zero_Timing_mute_cnt == CHECK_TIME_PERIOD) {
			    	...
			    	if(!check_polling_time_period(HDMI_ZERO_REPORT_RECORD_TIME,
			...
			//    HDMI_EMG("HDMI_Reply_Zero_Timing is set, but not current port, 
            //	        return zero timing (port=%d)\n",
			//	  hdmitiminginfo.port);
			//}
		}
	}
...
int sm_vfe_hdmi_disconnect(int port)
{
    int ret = 0;
    if(HDMI_Global_Status < SRC_CONNECT_DONE) {
        rtd_pr_vfe_debug("\r\n VFE_IOC_HDMI_DISCONNECT Fail. Source is not SRC_CONNECT_DONE, current : %d",
                HDMI_Global_Status);
		return -1; //Fail Source is not SRC_INIT_DONE
	}
...
unsigned int rpc_main_onlinemeasure_isr(unsigned long para1, unsigned long para2)
{
	...
	switch(Get_DisplayMode_Src(SLR_MAIN_DISPLAY))
	{
		...
		case VSC_INPUTSRC_HDMI:
			...
			set_display_forcebg_mask(SLR_MAIN_DISPLAY, SLR_FORCE_BG_TYPE_SIG_UNSTABLE, TRUE);//Enable SLR_FORCE_BG_TYPE_SIG_UNSTABLE mask
			mute_control(SLR_MAIN_DISPLAY, TRUE);/*Auto BG*/
			set_scaler_qms_mode_flag(0);//reset qms flag for online error
			...
			if	(HDMI_Global_Status == SRC_CONNECT_DONE) {
				if(decide_do_hdmi_error_handler())
					vfe_hdmi_drv_handle_on_line_measure_error(HAL_VFE_HDMI_CURRENT_PORT, 1);//k3l error will mark 0603
