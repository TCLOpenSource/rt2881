...
#ifndef UT_flag
#ifndef BUILD_QUICK_SHOW
extern unsigned int vbe_get_eco_mode_status_flag(void);
#endif
#endif
...
unsigned int Get_DISPLAY_CLOCK_MAX(void)
{
	...
	//if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_4K_2K) && Get_DISPLAY_2k1k_output_mode_status())	// 4k2k panel output lvds 2k1k
	 ...
	 if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_DISPLAY_CLOCK_MAX*1000000;

	return CONFIG_DISPLAY_CLOCK_MAX*1000000;
}
//#endif
...
unsigned int Get_DISPLAY_CLOCK_MIN(void)
{
	...
	//if((Get_DISPLAY_PANEL_TYPE() == P_LVDS_4K_2K) && Get_DISPLAY_2k1k_output_mode_status())	// 4k2k panel output lvds 2k1k
	 ...
	 if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_DISPLAY_CLOCK_MIN*1000000;

	return CONFIG_DISPLAY_CLOCK_MIN*1000000;
}
...
unsigned int Get_DISPLAY_CLOCK_TYPICAL(void)
{
    ...
    if(vbe_get_eco_mode_status_flag())
    {
        unsigned int new_dclk = 0;
        new_dclk = Get_DISPLAY_REFRESH_RATE() * Get_DISP_HORIZONTAL_TOTAL() * Get_DISP_VERTICAL_TOTAL(); // (ht=3840) * (vt=1520) * (fr=60) = 350208000;
        if(new_dclk < Get_DISPLAY_CLOCK_MIN())
        {
            return Get_DISPLAY_CLOCK_MIN();
        }
        else
        {
            return new_dclk;
...
{
	if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_EDP_LINK_RATE;

	return 0x14; // _PANEL_DPTX_LINK_HBR2
}

unsigned char Get_PANEL_EDP_MAC_NUM(void)
{
	if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_EDP_MAC_NUM;
...
{
	if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_EDP_FORCE_MSA_ENHANCE_FRAMING;
...
{
	if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_EDP_FORCE_MSA_DOWN_SPREAD;
...
{
	if (pPanelCOnfigParameter!=NULL)
		return pPanelCOnfigParameter->iCONFIG_EDP_FORCE_ALTERNATE_SR_CAP;
...
unsigned int get_panel_pixel_mode_as_uint(void)
{
    if(get_panel_pixel_mode() == PANEL_2_PIXEL)
    {
        return 2;
    }
    else // PANEL_1_PIXEL
...
void Set_DISPLAY_REFRESH_RATE(unsigned int refresh)
{
    if(pPanelCOnfigParameter != NULL)
    {
        pPanelCOnfigParameter->iCONFIG_DISPLAY_REFRESH_RATE = refresh;
    }
}

void Set_DISPLAY_CLOCK_TYPICAL(unsigned int typical)
{
    if(pPanelCOnfigParameter != NULL)
    {
        pPanelCOnfigParameter->iCONFIG_DISPLAY_CLOCK_TYPICAL = typical;
    }
}

void Set_DISP_VERTICAL_TOTAL(unsigned int vtotal)
{
    if(pPanelCOnfigParameter != NULL)
    {
        pPanelCOnfigParameter->iCONFIG_DISP_VERTICAL_TOTAL = vtotal;
    }
}
