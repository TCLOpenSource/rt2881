...
unsigned char drvif_color_dtm_get_One_Table(unsigned short *dtm_one_table)
{
	int data0,data1;
	int timeout = 100000;
	int i;
	...
	for(i = 0;i < 128;i++)
	{
		...
		//rtd_printk(KERN_EMERG, TAG_NAME, "DTM [%d]=%d \n",i,data0);
		//rtd_printk(KERN_EMERG, TAG_NAME, "DTM [%d]=%d \n",i+1,data1);

		
		dtm_one_table[2*i] = data0;
		dtm_one_table[2*i+1] = data1;
	}
...
unsigned char drvif_color_dtm_write_RGB_Table(unsigned short dtm_rgb_table[TBMAX][RGBSEGMAX])
{
	...
	for( tb_idx = 0;tb_idx < 3;tb_idx ++)	//write curve r/g/b
	{
		...
		{
			color_dtm_dm_dtm_tab_wr_data_reg.regValue = rtd_inl(COLOR_DTM_DM_DTM_TAB_WR_DATA_reg);
			color_dtm_dm_dtm_tab_wr_data_reg.wr_tab_data0 = dtm_rgb_table[tb_idx][i*2];
			color_dtm_dm_dtm_tab_wr_data_reg.wr_tab_data1 = dtm_rgb_table[tb_idx][i*2 + 1];
...
unsigned char drvif_color_dtm_get_RGB_Table(unsigned short dtm_rgb_table[TBMAX][RGBSEGMAX])
{
	...
	for(tb_idx = 0;tb_idx < 3;tb_idx ++)
	{
		for(i = 0;i < 128;i++)
		{
			...
			data0 = color_dtm_dm_dtm_tab_rd_data_reg.rd_tab_data0;
			data1 = color_dtm_dm_dtm_tab_rd_data_reg.rd_tab_data1;
			dtm_rgb_table[tb_idx][2*i] = data0;
			dtm_rgb_table[tb_idx][2*i+1] = data1;
