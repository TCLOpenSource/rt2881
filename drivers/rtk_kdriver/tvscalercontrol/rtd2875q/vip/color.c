...
#endif
//#include <rbus/siw_demura_reg.h> //k25lp removed
...
void drvif_color_set_HDMI_444to422_Enable(unsigned char enable,unsigned char drop_enable,unsigned char delay,unsigned char sel)
{
	...
	IoReg_SetBits(H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_444to422_CTRL_reg, H3DDMA_RGB2YUV_DITHER_4XXTO4XX_HDMI_444to422_CTRL_db_apply_444to422_mask);
...
void drvif_color_set_PanelType_Dither_Table(unsigned char DitherIP,unsigned char  (*pDITHER_TABLE)[16][4])
{
	...
	for (ii = 0; ii < VIP_DITHERLUT_ROW_MAX; ii++) {
		for (jj = 0; jj < VIP_DITHERLUT_COL_MAX; jj++) {
			...
			switch(DitherIP) {
				case VIP_DITHER_IP_MEMC_MUX:
					//12 bit to 6bit/7bit/8bit/9bit/10bit/other bit
					...
					break;
				case VIP_DITHER_IP_PANEL:
					//13 bit to 6bit/7bit/8bit/9bit/10bit/other bit
					dithering_ctrl_reg.regValue = IoReg_Read32(DITHER_DITHERING_CTRL_reg);
					for (kk = 0; kk < VIP_DITHERLUT_DEP_MAX; kk++) {
						/* KWarning: checked ok by allen_zhang*/
						if(Panel_Bit == 0)	// 6bit
						...
						{
							dither_32BIT_DATA += (pDITHER_TABLE[ii][jj][kk]>>4) << data_shift; 						
							dithering_ctrl_reg.table_shift = 0;
						}
						data_shift += 6;
					}
 ...
 * d1: 7:0
 */
void drvif_color_colorread_gamma_enhance(unsigned int *pArray, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
	...
	for (ii = 0; ii < Bin_Num_Gamma_k24; ii++) {
		...
		pArray[ii] = (gamma_port_REG.gamma_tab_index<<16) + (gamma_port_REG.gamma_tab_d0<<8) + gamma_port_REG.gamma_tab_d1;
	}
 ...
 * d1: 7:0
 */
void drvif_color_colorwrite_gamma_enhance(unsigned int *pArray, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
	...
	for (ii = 0; ii < Bin_Num_Gamma_k24; ii++) {
        ...
        gamma_port_REG.gamma_tab_index= (pArray[ii]>>16)&0xfff;;
        gamma_port_REG.gamma_tab_d0=(pArray[ii]>>8)&0xff;
        gamma_port_REG.gamma_tab_d1=(pArray[ii])&0xff;
...
/*1. read write gamma table.
	...
	* d1: 7:0
*/
void drvif_color_colorRead_outGamma_enhance(unsigned int *pArray, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
	...
	for (ii = 0; ii < (Bin_Num_Gamma_k24); ii++) {
		...
		pArray[ii] = (out_gamma_port_REG.out_gamma_tab_index<<16) + (out_gamma_port_REG.out_gamma_tab_d0<<8) + out_gamma_port_REG.out_gamma_tab_d1;
...
void drvif_color_colorwrite_outGamma_enhance(unsigned int *pArray, VIP_GAMMA_CHANNEL_RGB RGB_chanel_idx)
{
	...
	for (ii = 0; ii < Bin_Num_Gamma_k24; ii++) {
        ...
        out_gamma_port_REG.out_gamma_tab_index=(pArray[ii]>>16)&0x1fff;
        out_gamma_port_REG.out_gamma_tab_d0=(pArray[ii]>>8)&0xff;
...
char drvif_color_set_PQ_ByPass(unsigned char bypass_item, unsigned char isInterlaceMode)
{
	...
	switch(bypass_item) {
		...
		break;
#ifdef HW_SUPPORT_MB_SU_PEAKING
		case PQ_ByPass_SU_PEAKING:
...
{
	kpost_top_kpost_top_00_RBUS kpost_top_kpost_top_00_reg;
	kpost_top_kpost_top_20_RBUS kpost_top_kpost_top_20_reg;
	kpost_top_kpost_top_00_reg.regValue = IoReg_Read32(KPOST_TOP_KPOST_TOP_00_reg);
	kpost_top_kpost_top_20_reg.regValue = IoReg_Read32(KPOST_TOP_KPOST_TOP_20_reg);
	if (en) {
		kpost_top_kpost_top_00_reg.post_cursor_h = x-(thickness/2);
		kpost_top_kpost_top_00_reg.post_cursor_v = y;
		kpost_top_kpost_top_00_reg.post_cursor_color = 0;//black
		kpost_top_kpost_top_20_reg.post_cursor_h_width = thickness;
		IoReg_Write32(KPOST_TOP_KPOST_TOP_20_reg, kpost_top_kpost_top_20_reg.regValue);
	}

	kpost_top_kpost_top_00_reg.post_cursor_en = kpost_top_kpost_top_00_reg.post_cursor_show = en;
	IoReg_Write32(KPOST_TOP_KPOST_TOP_00_reg, kpost_top_kpost_top_00_reg.regValue);
}
