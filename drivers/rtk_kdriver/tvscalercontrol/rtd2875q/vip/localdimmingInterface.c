...
//#ifdef K24_PQ_COMPILE
    ...
    ldspi_multi_format_unit_delay2_reg.last_zero_dir = ptr->LDINTERFACE_Output.last_zero_dir;
    ldspi_multi_format_unit_delay2_reg.last_zero_dir = ptr->LDINTERFACE_Output.last_zero_padding;
    /*DRV_LDINTERFACE_INV*/
...
void drvif_get_LdInterface_AplBplStatus(DRV_LDINTERFACE_APLBPLStatus *ptr)
{
    ...
    ptr->apl_average_r = 0;//mac9q usage, reserved in merlin9
    ptr->apl_max_r     = 0;//mac9q usage, reserved in merlin9
    ptr->apl_min_r     = 0;//mac9q usage, reserved in merlin9
}
...
void drvif_set_LdInterface_DoubleVs(DRV_LDINTERFACE_DoubleVs *ptr)
{
    ...
    IoReg_Write32(LDSPI_LD_CTRL_reg, ldspi_ld_ctrl_reg.regValue);
    /*update info&write register*/
    ldspi_db_vsync_reg.memc_output_frame_en = ptr->memc_output_frame_en;
...
void drvif_get_LdInterface_RxData(UINT16 *rxData)
{
    ...
    //rxData = (UINT16*)kmalloc(sizeof(UINT16)*16); // set size when usage and set free after usage
    memset(rxData, 0, 16);
    for(i = 0;i<16;i+=2){
...
//    ptr->LDINTERFACE_IntEn.tx_done_int_en = ldspi_rx_data_ctrl_reg.tx_done_int_en;
    ...
    ptr->LDINTERFACE_Output.last_zero_dir = ldspi_multi_format_unit_delay2_reg.last_zero_dir;
    ptr->LDINTERFACE_Output.last_zero_padding = ldspi_multi_format_unit_delay2_reg.last_zero_append;
