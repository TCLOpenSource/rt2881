...
/****************************** Configurations ******************************/
//static unsigned int PQMaskPrintCount = 0; //K25LP_PQ_BringUp_Compile
#ifdef VIPprintf
#undef VIPprintf
...
/* sync from k24, add dummy api */
int KGAL_PQMaskLockSE(void)
{
	return 0;
}

int KGAL_PQMaskUNLockSE(void)
{
	return 1;
}
...
static PQMASK_BUFINFO_T PQMaskBufMgr = {
	...
	// Module weighting dma
	{
		...
		// HUE
		{
			{ { PQMASK_PROC_WIDTH, PQMASK_PROC_HEIGHT, sizeof(unsigned char), 0x00 }, { 0, 0, 0, 0 }, },
			{ { PQMASK_PROC_WIDTH, PQMASK_PROC_HEIGHT, sizeof(unsigned char), 0x00 }, { 0, 0, 0, 0 }, },
			{ { PQMASK_PROC_WIDTH, PQMASK_PROC_HEIGHT, sizeof(unsigned char), 0x00 }, { 0, 0, 0, 0 }, },
		},
		// SAT
				...
				if( mColorMgr.Param.FusionRatio_Dep != 0 ) {
					...
					if( true == SeProcStatus ) {
						...
						if( KGAL_PQMaskLockSE() == 0 ) {
							...
							SeSetupStatus = KGAL_SetSurfacePalette(&SeColorLut);
							if( true == SeSetupStatus ) {
								SeProcStatus = scalerPQMask_WaitSEDone();
								if( true == SeProcStatus ) {
							...
							if (DefocusEnableTbl[SEProcGrpMapping[ProcGrp][i]] == 1){
								if( true == SeProcStatus ) {
									...
									SeSetupStatus = KGAL_PQMask_Bitblit_MultiplyByAlpha(
										&SeSrc1_Surface[0], &SeSrc2_Surface[0], &SeDst_Surface[0],
										mColorMgr.Param.FusionRatio_Sem /*on src2*/,
										0 /*on src1*/,
										0,
...
void scalerPQMask_postprocessing_entry(void) {
	...
	PQMaskFlag *pPQMaskFlag = (&((PQMaskFlag*)(pNN->NN_flag_Addr[semantic_idx].pVir_addr_align))[PQMASK_TUNNEL_A2K]);
	PQMaskFlag *pPQMaskFlag_Defocus = (&((PQMaskFlag*)(pNN->NN_flag_Addr[defocus_idx].pVir_addr_align))[PQMASK_TUNNEL_A2K]);
	...
	if( pPQMaskColorMgr->Param.FusionRatio_Defocus != 0 && PQMaskGlbMgr.Ctrl.bIsDefocusReady == 0 && aipq_mode.semantic_mode != 0) {
		return;
	}
	
	if( pPQMaskColorMgr->Param.FusionRatio_Sem != 0 && PQMaskGlbMgr.Ctrl.bIsSemReady == 0  && aipq_mode.defocus_mode != 0) {
		return;
	}
