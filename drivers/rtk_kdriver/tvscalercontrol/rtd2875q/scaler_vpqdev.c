...
// -----------------------------------------------------
static unsigned int aipq_support_model_list = AI_SQM_DETECT ;
...
#endif
	{
		...
		if (mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.od_en == 1) {
			...
			if (fwif_color_od_init(mVIP_ODTable[0].OD_Main_Ctrl.OD_Ctrl.Y_mode_en)) {
				fwif_color_set_od_by_TBL(&mVIP_ODTable[tbl_idx]);
				bODInited = 1;
...
/*========================= for INNX demura =================================*/
	...
	case VPQ_IOC_AI_MODEL_LIST:
	{
		...
		if(!scalerAI_check_if_AIPQ_enabled()){
			rtd_pr_vpq_emerg("VPQ_IOC_AI_MODEL_LIST: nnsr otp disable\n");
	...
	case VPQ_IOC_AI_ALLOW_MODE:
	{
		...
		rtd_pr_vpq_info("VPQ_IOC_AI_ALLOW_MODE: %d\n",allowMode);
	...
	case VPQ_IOC_AI_SET_AP_MASK:
	{
		...
		if(apMask.init){
			...
			if(aiap_info.state==NN_AIAP_READY && aiap_info.pid==apMask.pid){
				...
				rtd_pr_vpq_info("VPQ_IOC_AI_SET_AP_MASK:wong state(%d) or pid(%d,%d)\n",aiap_info.state,aiap_info.pid,apMask.pid);
		}
		up(&VPQ_AI_Semaphore);
