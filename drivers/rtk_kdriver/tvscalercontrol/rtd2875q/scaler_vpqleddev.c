...
/*
	...
	case VPQ_LED_IOC_InitLDSPI:
		{
			unsigned char bCtrl;
			LEDdriver_Arg leddriver_args;
			if (copy_from_user(&bCtrl, (int __user *)arg, sizeof(unsigned char))) {
				rtd_pr_vpq_emerg("kernel VPQ_LED_IOC_InitLDSPI fail\n");
				ret = -1;
			} else {
				rtd_pr_vpq_err("kernel VPQ_LED_IOC_InitLDSPI start\n");
				down(&VPQ_LED_Semaphore);
				leddriver_args.leddriver_type = LEDdriver_iW7039;
				fwif_color_set_LEDdriver_Init(&leddriver_args);
				up(&VPQ_LED_Semaphore);
				rtd_pr_vpq_err("kernel VPQ_LED_IOC_InitLDSPI success\n");
				ret = 0;
			}
