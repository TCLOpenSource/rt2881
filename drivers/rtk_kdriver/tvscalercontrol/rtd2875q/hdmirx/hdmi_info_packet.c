...
void newbase_hdmi_update_infoframe(unsigned char port)
{
    ...
    {
        HDMI_WARN("GET VSI INFO=%x, pkt_cnt=%d, VIC=%d, VER=%x, LEN=%x, OUI=(%x,%x,%x)\n",
            hdmi_rx[port].vsi_t.VSIF_TypeCode, hdmi_rx[port].pkt_cnt, newbase_hdmi_get_vic_from_vsi(hdmi_rx[port].vsi_t.Payload[0], hdmi_rx[port].vsi_t.Payload[1]), hdmi_rx[port].vsi_t.VSIF_Version, hdmi_rx[port].vsi_t.Length, hdmi_rx[port].vsi_t.Reg_ID[0], hdmi_rx[port].vsi_t.Reg_ID[1], hdmi_rx[port].vsi_t.Reg_ID[2]);
...
unsigned char newbase_hdmi_get_vic_from_vsi(unsigned char pb4, unsigned char pb5)
{
    if (pb4 == 0x20) //Video formats defined in table 8.14 of HDMI spec v1.4b
    {
...
unsigned char newbase_hdmi_get_vic_state(unsigned char port, HDMI_AVI_T *p_avi_t, HDMI_VSI_T *p_vsi_t, unsigned char *p_vic)
{
    ...
    if(vsi_state == 1)
        vsi_vic = newbase_hdmi_get_vic_from_vsi(p_vsi_t->Payload[0], p_vsi_t->Payload[1]);
