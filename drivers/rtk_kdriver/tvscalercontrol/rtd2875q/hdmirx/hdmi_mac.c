...
unsigned char lib_hdmi_is_420_over_600m_1p_mode(void)
{
    unsigned char enable = GET_FLOW_CFG(HDMI_FLOW_CFG_GENERAL, HDMI_FLOW_CFG0_SUPPORT_420_OVER_600M_1P_MODE);

    if(enable)
    {
        HDMI_INFO("lib_hdmi_is_420_over_600m_1p_mode, en=%d\n", enable);
        return TRUE; // for TMDS 2.5K 165Hz 420 pixel clk over 600M/pll clk over 300M, video path only can support single pixel mode
    }
    else
        return FALSE;
}
...
#define FVCO_MAX    500
    ...
    *pll_pixelclockx1024 =pixel_clockx1024;
    ...
    if ((frl_mode == MODE_TMDS) && (is_420) && pixel_clockx1024 > PLL_PIXELCLOCK_X1024_300M && (lib_hdmi_is_420_over_600m_1p_mode() == 0))
    {
