    ...
    )
{
    ...
    n = scnprintf(ptr, count, "Gaming: ALLM=%d, FVA=%d, VRR_EN=%d (QMS=%d, M_CONST=%d, NextTFR=%d), AMD_FREESYNC_EN=%d, Scaler IVS V Freq(x0.001Hz)=%d\n",
    ...
    )
{
    ...
    else if(sscanf(buf, "WRITE_DEF=%1d", &val)==1)
    {
        ...
        if ( edid != NULL)
        {
            ...
            newbase_hdmi_save_edid(port, edid, len);
            if(newbase_hdmi_write_edid(port, edid, len)==0)
            {
                HDMI_EMG("%s: err write %u EDID fail\n", __func__, len);
            }
            else
            {
                newbase_hdmi_set_hpd(newbase_hdmi_get_current_display_port(), 0);
