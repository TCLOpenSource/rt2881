...
#define MAIN_FIFO_GATE_TH 128;  // DIC suggestion for merlin9
#define MAIN_FIFO_GATE_TH_1 256;
#define MAIN_FIFO_GATE_TH_2 384;
#define MAIN_FIFO_GATE_TH_3 500;
...
void memory_set_input_format(unsigned char display, unsigned char bDouble)
{
...
#endif
	{
		...
		if( ( Scaler_DispGetInputInfo(SLR_INPUT_V_FREQ_1000) > ( info_from_scaler->in_frame_rate * 1000 + 5000 ) ) &&
			( display == SLR_MAIN_DISPLAY ) && !get_hdmi_vrr_4k60_mode() && !is_scaler_input_2p_mode(display) )
		{
...
//	unsigned char lineRepeatEnable = FALSE;
    ...
    mdomain_cap_cap_reg_doublbuffer_1_RBUS cap_reg_doublbuffer_1_reg;
	...
	if(display == SLR_MAIN_DISPLAY){
		...
		cap_reg_doublbuffer_1_reg.regValue = rtd_inl(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg);
		cap_reg_doublbuffer_1_reg.cap2_db_en = 0;
		rtd_outl(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg,cap_reg_doublbuffer_1_reg.regValue);
	}
#endif
	//field polarity error check
	...
	if(Cap_type == MEMCAPTYPE_LINE){
		...
		if(get_hdmi_vrr_4k60_mode() || (is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
...
//		,Scaler_DispGetInputInfoByDisp((SCALER_DISP_CHANNEL)Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY), SLR_INPUT_CAP_LEN), drvif_scaler3d_decide_3d_SG_data_FRC());
	...
	if (display == SLR_MAIN_DISPLAY)	 {
...

		{
			...
			if((get_hdmi_4k_hfr_mode() == HDMI_4K120_2_1) && (get_force_i3ddma_enable(SLR_MAIN_DISPLAY) == FALSE))
			{
			...
			else
			{
				...
				{
					if(is_scaler_input_2p_mode(display))
						MemTag[MEMIDX_MAIN_SEC].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_MAIN_THIRD].StartAddr+buffer_size*2, DMA_SPEEDUP_ALIGN_VALUE);
					else
				...
				if(vdec_data_frc_need_borrow_cma_buffer(display)){
					...
					if(is_scaler_input_2p_mode(display))
						MemTag[MEMIDX_MAIN].StartAddr = drvif_memory_get_data_align(MemTag[MEMIDX_MAIN_SEC].StartAddr+buffer_size*2, DMA_SPEEDUP_ALIGN_VALUE);
					else
	...
	if (Scaler_DispGetInputInfo(SLR_INPUT_DISPLAY) == SLR_MAIN_DISPLAY){
...
#endif
		{
            ...
            if(vdec_data_frc_need_borrow_cma_buffer(display))
            {
                ...
                printk(KERN_NOTICE "##func:%s main m domain borrow memory from CMA size:0x%x ###\r\n", __FUNCTION__, borrow_size);
                if(get_m_domain_borrow_buffer())
                {//alread has size from cma
                    if(borrow_size <= get_m_domain_borrow_size())
                    {//no need to borrow again
                ...
                {//need borrrow memory from cam
                    ...
                    {
                        printk(KERN_ERR "###[Bug] func:%s no memory from cma ##\r\n",__FUNCTION__);
                        return;
                    }
                }
            }
...
//-		rtd_pr_scaler_memory_debug("\n Allocate Sub Disp Memory !\n");
		...
		drvif_memory_free_block(MEMIDX_SUB_THIRD);	// free block befor getting a new one
		//sub use cap max size @Crixus 20151231
		if((is_scaler_input_2p_mode(SLR_SUB_DISPLAY)) || get_hdmi_vrr_4k60_mode())
		{
...
//	unsigned int  TotalSizeFrame=0;
	...
	if(get_hdmi_vrr_4k60_mode() || (is_scaler_input_2p_mode(display)) || ((get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_ROTATE_0)&&(get_rotate_mode(SLR_MAIN_DISPLAY) != DIRECT_VO_FRAME_ORIENTATION_DEFAULT)))
	{
...
static void memory_set_sub_vflip_for_pip(unsigned char display)
{
    ...
    //if hfr mode, main and sub path capture same data, so sub path need set burst/lenth setting as main path
    if((SLR_SUB_DISPLAY == display) && ((is_scaler_input_2p_mode(SLR_MAIN_DISPLAY)) || get_hdmi_vrr_4k60_mode())){
        fifoLen = _FIFOLENGTH;
...
void drv_memory_disable_di_write_req(unsigned char disableDiWr)
{
	...
	di_dma_multi_req_num_reg.rdma_req_num = 6;// 3;//(enable? 3: 0);
...
void drvif_memory_set_fs_display_fifo(void)
{
	...
	if(is_scaler_input_2p_mode(SLR_MAIN_DISPLAY))
	{
		...
		//apply sub
		IoReg_SetBits(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg, MDOMAIN_DISP_DDR_MainSubCtrl_1_disp2_double_apply_mask);
	}
...
void drvif_memory_wait_dbuffer_write(unsigned char display)
{
	...
	{
		while(MDOMAIN_DISP_DDR_MainSubCtrl_1_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg)) && --count)
		{
#ifdef BUILD_QUICK_SHOW
			msleep(10);//no busy delay 10 ms
#else
		...
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d ERROR!!! Sub Mdisp db_apply timeout! 7348=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d ERROR!!! Sub Mdisp db_apply timeout! 7348=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg));
#endif
		}
		else
		{
#ifdef BUILD_QUICK_SHOW
			printf2("[%s] %d Sub Mdisp db_apply success! 7348=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg));
#else
			rtd_pr_scaler_memory_info("[%s] %d Sub Mdisp db_apply success! 7348=0x%x\n", __FUNCTION__, __LINE__, IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg));
#endif
		}
	}
}
...
void dvrif_memory_comp_setting_sub(UINT8 enable_compression, UINT16 comp_wid, UINT16 comp_len, UINT8 comp_ratio){
	...
	if(enable_compression == TRUE){
		...
		mdomain_cap_smooth_tog_ctrl_1_reg.in2_pqc_clken = 0; // 0: enable
		...
		*/
		...
		// Disable PQC clk
		mdomain_cap_smooth_tog_ctrl_1_reg.regValue = IoReg_Read32(MDOMAIN_CAP_Smooth_tog_ctrl_1_reg);
		mdomain_cap_smooth_tog_ctrl_1_reg.in2_pqc_clken = 1; // 1: disable
		IoReg_Write32(MDOMAIN_CAP_Smooth_tog_ctrl_1_reg, mdomain_cap_smooth_tog_ctrl_1_reg.regValue);
...
//fixme:vip
	...
	*/
	...
	else	// PQC enable, use line sum bit to cal
	{ 
		size_per_frame = (drv_memory_di_PQC_Line_Sum_Bit(0, 0, 0)<<7) * DI_Height;
		data_r.number = (int)(size_per_frame/128/data_r.length);
		data_r.remain = (size_per_frame - data_r.number*128*data_r.length + 127)/128;
		if (data_r.remain == 0 && data_r.number != 0) {
			data_r.number -= 1;
		...
		rtd_pr_scaler_memory_info("Mer2, PQCLSM=%d,h=%d,\n", drv_memory_di_PQC_Line_Sum_Bit(0, 0, 0), DI_Height);
	}
...
//		value =(info->IPV_ACT_LEN_PRE *info->IPH_ACT_WID_PRE * bitNum)>>4;
	...
	//MemEndtAddr = drv_memory_MA_SNR_Config(access, MemEndtAddr, index); // ml9 hw not support
...
void drv_memory_wait_disp_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case)
{
	...
	else
	{
		do
		{
			...
			rtd_pr_scaler_memory_info("[%s] %d M domain sub done-1, disp pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_disp_idx, cur_disp_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_1_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg))));
...
void drv_memory_wait_cap_last_write_done(unsigned char display, unsigned char wait_frame, unsigned char game_mode_case)
{
	...
	if(get_mdomain_input_fast_status(display) == TRUE)
	{
		...
		unsigned char original_mdisp_db_en = (display? MDOMAIN_DISP_DDR_MainSubCtrl_1_get_disp2_double_enable(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg)): MDOMAIN_DISP_DDR_MainSubCtrl_get_disp1_double_enable(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_reg)));
	...
	else
	{
		do
		{
			...
			rtd_pr_scaler_memory_info("[%s] %d M domain sub done-1, cap pre_index=%d, new_idx=%d, count=%d, mdisp_apply=%d\n", __FUNCTION__, __LINE__, pre_cap_idx, cur_cap_idx, count, (MDOMAIN_DISP_DDR_MainSubCtrl_1_get_disp2_double_apply(IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg))));
...
{//enable: true = single   false: triple  4k120 case
    ...
    mdomain_cap_cap_reg_doublbuffer_1_RBUS cap_reg_doublbuffer_1_reg;
    mdomain_disp_ddr_mainsubctrl_1_RBUS ddr_mainsubctrl_1_Reg;
    ...
    if(enable)
    {//one buffer
        cap_reg_doublbuffer_1_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg);
        cap_db_en = cap_reg_doublbuffer_1_reg.cap2_db_en;
        cap_reg_doublbuffer_1_reg.cap2_db_en = 1;
        IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg,cap_reg_doublbuffer_1_reg.regValue);

        //enable M-disp double buffer
        ddr_mainsubctrl_1_Reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg);
        //disp_db_en = ddr_mainsubctrl_reg.disp1_double_enable;
        ddr_mainsubctrl_1_Reg.disp2_double_enable = 1;
        IoReg_Write32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg,ddr_mainsubctrl_1_Reg.regValue);
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
        ...
        ddr_mainsubctrl_1_Reg.disp2_double_apply = 1;
        IoReg_Write32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg,ddr_mainsubctrl_1_Reg.regValue);
        while((ddr_mainsubctrl_1_Reg.disp2_double_apply == 1) && (timeout--)){
            ddr_mainsubctrl_1_Reg.regValue = IoReg_Read32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg);
        }
        ...
        if(!get_hdmi_vrr_4k60_mode()){
            ...
            cap_reg_doublbuffer_1_reg.cap2_db_apply = 1;
            IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg,cap_reg_doublbuffer_1_reg.regValue);
            while((cap_reg_doublbuffer_1_reg.cap2_db_apply == 1) && (timeout--)){
                cap_reg_doublbuffer_1_reg.regValue = IoReg_Read32(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg);
            }
        }
        ...
        if(Scaler_DispGetInputInfoByDisp(SLR_MAIN_DISPLAY, SLR_INPUT_STATE) == _MODE_STATE_SEARCH) {
            ...
            cap_reg_doublbuffer_1_reg.cap2_db_en = cap_db_en;
        }
        IoReg_Write32(MDOMAIN_CAP_cap_reg_doublbuffer_1_reg, cap_reg_doublbuffer_1_reg.regValue);
    }
    else
    {//three buffer
        ...
        //enable M-disp double buffer
        ddr_mainsubctrl_1_Reg.regValue = rtd_inl(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg);
        //disp_db_en = ddr_mainsubctrl_reg.disp1_double_enable;
        ddr_mainsubctrl_1_Reg.disp2_double_enable = 1;
        IoReg_Write32(MDOMAIN_DISP_DDR_MainSubCtrl_1_reg, ddr_mainsubctrl_1_Reg.regValue);
        /////////////////////////////////////////////////////////////////////////////////////////
...
//	ppoverlay_fs_iv_dv_fine_tuning5_RBUS fs_iv_dv_fine_tuning5_reg;
	...
	if(!get_force_i3ddma_enable(SLR_MAIN_DISPLAY) && (Scaler_InputSrcGetType(SLR_MAIN_DISPLAY) !=_SRC_VO))
	{
		...
		{
			printk("[IV2DV]Invalid Input Info!!!!\n");
			return 0;
		}
	}
	...
	if( ( get_panel_pixel_mode() > PANEL_1_PIXEL ) && PPOVERLAY_uzudtg_control1_get_uzu_4k_120_mode(rtd_inl(PPOVERLAY_uzudtg_control1_reg)) )	// 120hz panel run 2pixel mode, dCLK need multiple 2 for calculate.
		dCLK *= 2;
            ...
            rtd_pr_scaler_memory_emerg("dCLK = %d, frac_a = %d, frac_b = %d\n", dCLK, frac_a, frac_b);
    }
...
//			IoReg_Write32(MDOMAIN_DISP_Sub_disp_blocksel_option_reg,  mdomain_disp_sub_disp_blocksel_option_reg.regValue);
			...
			mdomain_disp_ddr_mainsubctrl_1_reg.disp2_double_apply = 1;
...
void drv_memory_display_check_sub_frc_style(void)
{
	...
	if (!get_sub_OutputVencMode())
	{
		...
		if( !get_hdmi_vrr_4k60_mode() && !is_scaler_input_2p_mode(SLR_MAIN_DISPLAY) &&
...
void drvif_memory_DisableMCap(unsigned char display)
{
    ...
    if (SLR_MAIN_DISPLAY == display)
    {
        ...
        {
            //Disable MCap
            rtd_pr_scaler_memory_info("Disable mcap main\n");
    ...
    else
    {
        ...
        {
        	//Disable MCap
        	rtd_pr_scaler_memory_info("Disable mcap sub\n");
...
void drvif_sub_mdomain_for_srnn(unsigned char enable)
{
    ...
    if((enable == 1) && (subSrnnEnable == 1))
    {
        ...
        dvrif_memory_set_compression_bits(SLR_SUB_DISPLAY, COMPRESSION_10_BITS);
        mdomain_cap_caps_pq_cmp_pair_reg.regValue = IoReg_Read32(MDOMAIN_CAP_CAPS_PQ_CMP_PAIR_reg);
        mdomain_cap_caps_pq_cmp_pair_reg.s_cmp_data_format = 3; //400 mode
        IoReg_Write32(MDOMAIN_CAP_CAPS_PQ_CMP_PAIR_reg, mdomain_cap_caps_pq_cmp_pair_reg.regValue);
        mdomain_disp_dispm_sub_pq_decmp_pair_reg.regValue = IoReg_Read32(MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_PAIR_reg);
        mdomain_disp_dispm_sub_pq_decmp_pair_reg.s_decmp_data_format = 3; // 400 mode
        IoReg_Write32(MDOMAIN_DISP_DISPM_SUB_PQ_DECMP_PAIR_reg, mdomain_disp_dispm_sub_pq_decmp_pair_reg.regValue);        
