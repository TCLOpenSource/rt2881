...
void Scaler_SetColorTempData(TV001_COLOR_TEMP_DATA_S *colorTemp)
{
	...
	// Since iog color temp behind OSD, always need to wait sync
...
void Scaler_SetDemoMode(TV001_DEMO_MODE_E demoMode,unsigned char onOff)
{
	if(!onOff){ //off
		...
		case TV001_DEMO_HDR:
			Scaler_SetMagicPicture(SLR_MAGIC_OFF);
			break;
		case TV001_DEMO_AI_SR:
			fwif_color_Demo_AI_SR(0);
			break;
		default:
			break;
		}
...
long Scaler_SetMemcLevel(TV001_LEVEL_E level)
{
	...
	rtd_pr_vpq_debug("func:%s line:%d level = %d \n",__FUNCTION__,__LINE__,level);
...
void Scaler_Set_ColorSpace_D_3dlutTBL(RTK_VPQ_COLORSPACE_INDEX *ColoSpace_T)
{
...
#if 0 //for debug
	rtd_pr_vpq_debug("osd,R:R=%d,G=%d,B=%d\n",osd[0][0],osd[0][1],osd[0][2]);
	rtd_pr_vpq_debug("osd,G:R=%d,G=%d,B=%d\n",osd[1][0],osd[1][1],osd[1][2]);
	rtd_pr_vpq_debug("osd,B:R=%d,G=%d,B=%d\n",osd[2][0],osd[2][1],osd[2][2]);
	rtd_pr_vpq_debug("osd,Y:R=%d,G=%d,B=%d\n",osd[3][0],osd[3][1],osd[3][2]);
	rtd_pr_vpq_debug("osd,C:R=%d,G=%d,B=%d\n",osd[4][0],osd[4][1],osd[4][2]);
	rtd_pr_vpq_debug("osd,M:R=%d,G=%d,B=%d\n",osd[5][0],osd[5][1],osd[5][2]);
#endif	
	...
	fwif_color_set_DDomain_3dLUT_32_TV006(&D_3DLUT_Reseult[0][0], 1);
