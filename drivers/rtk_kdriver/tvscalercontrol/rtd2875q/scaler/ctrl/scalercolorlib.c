...
void fwif_color_video_quality_handler_quick_show(void)
{
	...
	fwif_color_set_InvGamma(SLR_MAIN_DISPLAY, 0, &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], &Gamma_sRGB2Linear_14bit[0], 1);
	...
	printf2("[%s]base_addr=%x,pa sta=%x,pa end=%x,va=%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,qs2kernel_crc=%d,\n", 
		...
		(unsigned long)vir_addr4, (unsigned long)vir_addr5, (unsigned long)vir_addr6, (unsigned long)vir_addr7,
		(unsigned long)vir_addr8, (unsigned long)vir_addr9, (unsigned long)vir_addr_end, qs2kernel_crc);	
}
	...
	if (Scaler_APDEM_En_Access(0, 0) == 1) {
		switch(ID)
		{
			...
			case DEM_ARG_OD_TBL_set_by_idx_Sel:
			{
				...
				fwif_color_set_od_by_TBL(&mVIP_ODTable[arg]);
			}
			break;
...
void Scaler_fwif_color_set_LocalContrast_table(unsigned char src_idx, unsigned char TableIdx)
{
	...
	//fwif_color_set_LC_saturation(src_idx, TableIdx); // remove on K25
...
void Scaler_Set_PQ_RGB444MODE(unsigned char path)
{
    ...
    if(path==1){
        ...
        //drvif_color_set_LC_RGB_mode(1); // remove on K25
        ...
        //drvif_color_set_LC_RGB_mode(0);// remove on K25
...
void Scaler_color_Get_HDR10_EOTF_OETF_ByMaxCLL_ByPanelLuminance_for_task(void)
{
	...
	if (HDR_flag == HAL_VPQ_HDR_MODE_HDR10) {
		...
		drvif_color_Set_ST2094_3Dlut_CInv(prot_hdr3dlut, 0xFF, 0);
	}
