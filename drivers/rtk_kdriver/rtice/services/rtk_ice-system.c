...
#if IS_ENABLED(CONFIG_RTK_KDRV_PWR_MONITOR)
extern int rtk_pwr_monitor_getValue_VI(unsigned char *pwr_value);
#endif
...
#define CONFIG_RTICE_SUPPORT_AUDIO_PreScale
#endif
...
#ifdef CONFIG_RTICE_SUPPORT_AUDIO_PreScale
#include "audio/hal_common.h"
extern DTV_STATUS_T RHAL_AUDIO_GetPreScaleVolume(UINT32 ch_id, SINT32 *a, SINT32 *b);
extern DTV_STATUS_T RHAL_AUDIO_SetPreScaleVolume(SINT32 vol, UINT32 ch_id);
#endif
...
static int rticeAudioParam(ENUM_RTICE_AUDIO_PARAM_GROUP group,int index,int*v,int size)
{
	...
	{
	#ifdef CONFIG_RTICE_SUPPORT_AUDIO_PreScale
		ENUM_AUDIO_PreScale_PARAM paramType = (index >> 24) & 0xff;
		ENUM_AUDIO_PreScale_Scale_PARAM scale = (index >> 16) & 0xff;
		if(paramType < ENUM_AUDIO_PreScale_PARAM_SIZE && scale < ENUM_AUDIO_PreScale_Scale_PARAM_SIZE){
			int channel = (1 << ((int)paramType * 2));
			int scale_a = 0,scale_b = 0;
			ret = RHAL_AUDIO_GetPreScaleVolume(channel,&scale_a,&scale_b);
			if(ret < 0){
				ret = -RTICE_AUDIO_VX_FAILED;
			}else{
				if(scale == apsScale_a)
		...
		if(paramType < ENUM_AUDIO_PreScale_PARAM_SIZE && scale < ENUM_AUDIO_PreScale_Scale_PARAM_SIZE){
			...
			if(ret < 0){
				...
				if(scale == apsScale_a)
					...
					scale_b = v[0];
				vol = 6 * scale_a + scale_b;
				ret = RHAL_AUDIO_SetPreScaleVolume(vol,channel);
				if(ret < 0)
					ret = -RTICE_AUDIO_VX_FAILED;
				else
					ret = 0;
...
#if IS_ENABLED(CONFIG_RTK_KDRV_SB2)
#define _IGNORE_SB2_ENABLE_ADDR_   (0x8240add2)  //a address of magic number to enable ignore sb2 error, for debugging purpose
#define _IGNORE_SB2_DISABLE_ADDR_   (0x7dbf522d)  //a address of magic number to disable ignore sb2 error, for debugging purpose

static void rtice_disable_exception(unsigned long addr)
{
    if (addr == _IGNORE_SB2_ENABLE_ADDR_)
    {
    ...
    else if (addr == _IGNORE_SB2_DISABLE_ADDR_)
    {
				...
				unsigned int response_buff_size )
{
	...
	break;
	case RTICE_SYS_CMD_READ_PHYS_ADDR:
	{
		unsigned int addr = 0x0,value = 0xdeaddead;
		void *v = NULL;
		if(command_data_len < 1)
			return -RTICE_ERR_INVALID_PARAM;
		...
		if(v){
			...
			LTB4(value,response_data[0],response_data[1],response_data[2],response_data[3]);
			ret = 4;
		}else{
			ret = -RTICE_IOMEM_ERR_INVALID_SYSTEM_INFO;
		}
	...
	break;
	case RTICE_SYS_CMD_READ_POWER_VALUE:
	{
		if(command_data_len < 1)
			return -RTICE_ERR_INVALID_PARAM;
	#if IS_ENABLED(CONFIG_RTK_KDRV_PWR_MONITOR)
		ret = rtk_pwr_monitor_getValue_VI(response_data);
		if(ret == 0)
			ret = 12;
	#else
		ret = -RTICE_ERR_INVALID_PARAM;
