...
void rtice_check_rbus_timeout(unsigned long addr, unsigned int *reg_value)
{
    ...
    {
        if(rtd_inl(SB2_INV_INTSTAT_reg) & SB2_INV_INTSTAT_sciva_int_mask)
        {
            value = rtd_inl(EMCU_wrapper_8051_reg);
            //rtd_pr_hw_monitor_err( "rtice_check_rbus_timeout EMCU_wrapper_8051_reg = 0x%x\n", value);
            if (value & EMCU_wrapper_8051_rbus_timeout_int_mask)
            {
                rtd_outl(EMCU_wrapper_8051_reg, (value | EMCU_wrapper_8051_rbus_timeout_int_mask));
            }
            *reg_value = 0xDEADDEAD;
            rtd_outl(SB2_INV_INTSTAT_reg,SB2_INV_INTSTAT_sciva_int_mask);
        }
    }
}
			...
			if (!Rtice_ChecAddrValid(AcceptedAddr)){
...
#else //#if !defined(CONFIG_RTK_KDRV_SB2)
				{
					...
					*/
					...
					rtice_check_rbus_timeout((unsigned long)AcceptedAddr, &GetData);
