...
//======================================================================
//MPRT NewFunction switch bl_en to pwm/gpio output
//======================================================================
int rtk_pwm_bl_en_switch(PIN_MODE_T mode, int rtk_freq, int rtk_duty)
{
    R_CHIP_T *pchip2 = rtk_pwm_chip_get_by_name("PIN_PWM_BL_EN");
    if(pchip2 == NULL){
        PWM_ERR("PIN_PWM_BL_EN is no define, the func no used\n");
        return -1;
    }
    if(mode == PWM_MODE){
        PWM_ERR("PIN_PWM_BL_En switch pwm\n");
        pchip2->rtk_freq = rtk_freq;
        pchip2->rtk_freq_100times = rtk_freq * 100;
        pchip2->rtk_duty = rtk_duty;

        rtk_pwm_freq_w(pchip2);
        rtk_pwm_duty_w(pchip2,rtk_duty);//If frequency change, duty need re-calculate

        //DB Write
        rtk_pwm_db_wb(pchip2);
        rtk_change_to_pwm_mode(pchip2);
    }else if(mode == GPIO_MODE){
        PWM_ERR("PIN_PWM_BL_EN switch gpio\n");
        rtk_change_to_gpio_mode(pchip2,rtk_gpio_output_get(pchip2->gid));
    }
    return 0;
}
EXPORT_SYMBOL(rtk_pwm_bl_en_switch);

int rtk_pwm_mprt_func(char* pcbname,int rtk_freq, int rtk_duty)
{
    R_CHIP_T *pchip2 = NULL;
    pchip2 = rtk_pwm_chip_get_by_name(pcbname);
    if(pchip2 == NULL){
         PWM_WARN("rtk_pwm_mprt_func failed\n" );
         return -1;
    }
    pchip2->rtk_freq = rtk_freq;
    pchip2->rtk_freq_100times = rtk_freq * 100;
...
EXPORT_SYMBOL(rtk_pwm_mprt_func);
...
void get_v_delay(R_CHIP_T *pchip2)
{
    ...
    unsigned int pos_start_clk_threshold =0;

    pchip2->rtk_pos_start_clk = pchip2->rtk_pos_start*phase_us/pchip2->rtk_duty_max / clk_period;
    pos_start_clk_threshold = 0x1fffff;  
    if(get_mach_type() == MACH_ARCH_RTK2885PP || get_mach_type() == MACH_ARCH_RTK2885P2){
        pos_start_clk_threshold = 0x3fffff;  
    }
    if(pchip2->rtk_pos_start_clk >= pos_start_clk_threshold) {
...
void rtk_pwm_hw_chip_init(R_CHIP_T *pwmPin, int ricindex)
{
    ...
    pwmPin->rtk_polarity = rtk_pwm_ctrl_polarity_r(pwmPin);    //GET_PWM_INVERT(pwmPin->ullPcbMgrValue);
...
int rtk_pwm_set_duty(char *pcbname,unsigned int duty)
{
    ...
    strncpy(param.pcbname, pcbname, sizeof(param.pcbname) - 1);
    param.pcbname[sizeof(param.pcbname) - 1] = '\0';  
    param.val = duty;
    ...
    ret = rtk_pwm_duty_param_set(pchip2,&param);
    if(ret)
    {
        PWM_ERR("PWM %s duty set error!\n",param.pcbname);
        ret = -1;
        return ret;
    }
    //DB Write
