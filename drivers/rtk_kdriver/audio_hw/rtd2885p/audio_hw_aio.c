...
void Audio_HW_Earc_SetAPHY(void)
{
	...
	EARC_ANA_2.regValue = AIO_ReadRegister(EARCMISC_earc_ana_2_reg);
	EARC_ANA_2.df_ib_sel = 0x3;//for 0.95V a0v9
	...
	if (pcb_mgr_get_enum_info_byname("IP_POW_CFG", &val)==0)
	{
		...
		alog_err("IP_POW_CFG support=%x\n",pow_cfg.regValue);
		if(pow_cfg.Pow_CFG==1)
		{
			if(pow_cfg.Pow_Lvl==2)//high level power
			{
				alog_err("IP_POW_CFG=%x\n",pow_cfg.Pow_Lvl);
			}else if(pow_cfg.Pow_Lvl==1)//low level power
			{
				EARC_ANA_2.regValue = AIO_ReadRegister(EARCMISC_earc_ana_2_reg);
				EARC_ANA_2.df_ib_sel = 0x3;
				AIO_WriteRegister(EARCMISC_earc_ana_2_reg, EARC_ANA_2.regValue);
				alog_err("IP_POW_CFG=%x,EARCMISC_earc_ana_2_reg=%x\n",pow_cfg.Pow_Lvl,AIO_ReadRegister(EARCMISC_earc_ana_2_reg));
			}
			else if(pow_cfg.Pow_Lvl==0)
				alog_err("IP_POW_CFG=Default Level\n");
		}
	}
