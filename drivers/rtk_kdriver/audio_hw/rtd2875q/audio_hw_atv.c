...
#define VIDEO_STATUS_REG2_VADDR		(0xb801900c)
...
int32_t Audio_AtvInit(ATV_CFG *p_cfg)
{
	...
	audio_sif_agc2_status_RBUS asif_agc2_status;
	...
	// after K25/M9 start to bypass AGC2 setting
	asif_agc2_status.regValue = AIO_ReadRegister(AUDIO_sif_agc2_status_reg);
	asif_agc2_status.agc2_bypass_en = 1;
	AIO_WriteRegister(AUDIO_sif_agc2_status_reg, asif_agc2_status.regValue);
				...
				if (sep_443_burst_hsync >= BURST_TH){
					...
					//alog_info("sep_443_burst_hsync > BURST_TH count = %d %x\n", sep_443_burst_hsync_count, AIO_ReadRegister(VD_SEP_BURST_RO_2_VADDR));
					 if (sep_443_burst_hsync_count > 15)
					 {
						...
						if ((main_sys == ATV_SOUND_MN_SYSTEM) && (vd_decision_mode_status == 6) && (((AIO_ReadRegister(VIDEO_STATUS_REG2_VADDR)&0x00000080) >> 7)==1))
						{// for 54200 PAL_N system of auto scan modification
							sep_358_burst_hsync_check = 1;
							sep_443_burst_hsync_check = 0;
							alog_info("#####recheck_more_change_to_358\n");
						}
						break;
					 }
