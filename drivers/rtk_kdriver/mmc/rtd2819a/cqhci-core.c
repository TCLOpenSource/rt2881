...
/* Copyright (c) 2015, The Linux Foundation. All rights reserved.
    copy from linux-5.15/drivers/mmc/host/
 */
...
//#define RTK_CQE_SCP
#ifdef RTK_CQE_SCP
    #define EM_CQSCP(fmt, args...) \
            EM_ALERT("mmccqe:%s(%d): " fmt, __func__ ,__LINE__,## args)
#else
    #define EM_CQSCP(fmt, args...)
#endif

//#define RTK_CQE_DEBUG
#ifdef RTK_CQE_DEBUG
    #define EM_CQE(fmt, args...) \
            EM_ALERT("mmccqe:%s(%d): " fmt, __func__ ,__LINE__,## args)
#else
    #define EM_CQE(fmt, args...)
#endif
...
#define CQHCI_DUMP(f, x...) \
	EM_ERR("%s: " DRV_NAME ": " f, mmc_hostname(mmc), ## x)
...
static int cqhci_host_alloc_tdl(struct cqhci_host *cq_host)
{
	...
	EM_DEBUG("%s: cqhci: desc_size: %zu data_sz: %zu slot-sz: %d\n",
	...
	EM_DEBUG("%s: cqhci: desc-base: 0x%p trans-base: 0x%p\n desc_dma 0x%llx trans_dma: 0x%llx\n",
...
static int cqhci_enable(struct mmc_host *mmc, struct mmc_card *card)
{
	...
	if (!card->ext_csd.cmdq_en){
		EM_CQE("ext_csd no support cmdq\n");
		return -EINVAL;
	...
	if (err) {
		EM_ERR("%s: Failed to enable CQE, error %d\n",
...
static void cqhci_off(struct mmc_host *mmc)
{
	...
	if (err < 0)
		EM_ERR("%s: cqhci: CQE stuck on\n", mmc_hostname(mmc));
	else
		EM_DEBUG("%s: cqhci: CQE off\n", mmc_hostname(mmc));
				 ...
				 struct cqhci_host *cq_host, int tag)
{
	...
	if (cq_host->caps & CQHCI_TASK_DESC_SZ_128) {
		...
		EM_DEBUG("%s: cqhci: tag %d task descriptor 0x%016llx%016llx\n",
			 mmc_hostname(mrq->host), mrq->tag, desc1, desc0);
	} else {
		EM_DEBUG("%s: cqhci: tag %d task descriptor 0x%016llx\n",
			       ...
			       struct cqhci_host *cq_host, int tag)
{
	...
	if (sg_count < 0) {
		EM_ERR("%s: %s: unable to map sg lists, %d\n",
...
#ifdef RTKEMMC_CQE
		if(CROSSES_128MB_BOUNDARY(addr, len)){
			u32 aglin_addr = 0;
			u32 byte_cnt;
			aglin_addr = ADDR_ALIGN_BOUNDARY_MORE(addr, _128MB);
			EM_CQSCP("addr=0x%x len=0x%x aglin_addr=0x%x\n",addr,len,aglin_addr);
			byte_cnt = (aglin_addr - addr);
			EM_CQSCP("desc=0x%x addr=0x%x len=0x%x\n",desc,addr,byte_cnt);
			cqhci_set_tran_desc(desc, addr, byte_cnt, false, dma64);

			desc += cq_host->trans_desc_len;
			byte_cnt = len - byte_cnt;
			EM_CQSCP("desc=0x%x addr=0x%x len=0x%x\n",desc,aglin_addr,byte_cnt);
			cqhci_set_tran_desc(desc, aglin_addr, byte_cnt, end, dma64);
			desc += cq_host->trans_desc_len;
		}else{
				   ...
				   struct mmc_request *mrq)
{
	...
	*task_desc |= data;
	desc = (u8 *)task_desc;
	EM_DEBUG("%s: cqhci: dcmd: cmd: %d timing: %d resp: %d\n",
...
static int cqhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!mmc->cqe_on) {
		...
		cqhci_writel(cq_host, 0, CQHCI_CTL);
		mmc->cqe_on = true;
		EM_DEBUG("%s: cqhci: CQE on\n", mmc_hostname(mmc));
		if (cqhci_readl(cq_host, CQHCI_CTL) && CQHCI_HALT) {
			EM_ERR("%s: cqhci: CQE failed to exit halt state\n",
			       mmc_hostname(mmc));
		}
		if (cq_host->ops->enable)
			cq_host->ops->enable(mmc);
	...
	if (mrq->data) {
		...
		if (err) {
			EM_ERR("%s: cqhci: failed to setup tx desc: %d\n",
			       mmc_hostname(mmc), err);
			return err;
		}
	} else {
	...
	if (!(cqhci_readl(cq_host, CQHCI_TDBR) & (1 << tag)))
		EM_DEBUG("%s: cqhci: doorbell not set for tag %d\n",
				  ...
				  bool notify)
{
	...
	if (!cq_host->recovery_halt) {
		...
		EM_DEBUG("%s: cqhci: recovery needed\n", mmc_hostname(mmc));
			    ...
			    int data_error)
{
	...
	EM_DEBUG("%s: cqhci: error IRQ status: 0x%08x cmd error %d data error %d TERRI: 0x%08x\n",
		      ...
		      int data_error)
{
	...
	EM_DEBUG("%s: cqhci: IRQ status: 0x%08x\n", mmc_hostname(mmc), status);
	...
	if (status & CQHCI_IS_TCC) {
		...
		for_each_set_bit(tag, &comp_status, cq_host->num_slots) {
			...
			EM_DEBUG("%s: cqhci: completing tag %lu\n",
				 mmc_hostname(mmc), tag);
			  ...
			  bool *recovery_needed)
{
	...
	if (timed_out) {
		EM_ERR("%s: cqhci: timeout for tag %d\n",
		       mmc_hostname(mmc), tag);
		cqhci_dumpregs(cq_host);
	}
...
static bool cqhci_clear_all_tasks(struct mmc_host *mmc, unsigned int timeout)
{
	...
	if (!ret)
		EM_WARNING("%s: cqhci: Failed to clear tasks\n",
			mmc_hostname(mmc));
...
static bool cqhci_halt(struct mmc_host *mmc, unsigned int timeout)
{
	...
	if (!ret)
		EM_WARNING("%s: cqhci: Failed to halt\n", mmc_hostname(mmc));
...
static void cqhci_recovery_finish(struct mmc_host *mmc)
{
	...
	EM_DEBUG("%s: cqhci: recovery done\n", mmc_hostname(mmc));
}
...
#if 0   //no support mmc->caps2 & MMC_CAP2_CRYPTO
	err = cqhci_crypto_init(cq_host);
	if (err) {
		EM_ERR("%s: CQHCI crypto initialization failed\n",
		       mmc_hostname(mmc));
		goto out_err;
	}
#endif
...
out_err:
	EM_ERR("%s: CQHCI version %u.%02u failed to initialize, error %d\n",
	       mmc_hostname(mmc), cqhci_ver_major(cq_host),
