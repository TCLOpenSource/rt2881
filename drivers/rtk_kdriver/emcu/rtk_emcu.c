...
extern int pm_rtd_cec_ac_on_suspend(void);
extern unsigned char get_ac_on_flag(void);
extern void ac_on_send_cec_msg_enable(void);
...
void wakeup_8051(int flag)
{
    ...
    if(new_protocol_enable == 1){
        if(flag == SUSPEND_BOOTCODE || flag == SUSPEND_NORMAL) {
                emcu_enter_poweroff(0);
                //rtd_pr_emcu_info("@@ send power off command\n");
        }
        else {
                //rtd_pr_emcu_info("@@ suspend not send power off command\n");
        }
    }else{
...
void rtkpm_get_param(void){
    ...
    if(get_ac_on_flag() ==1)
    {
        ac_on_send_cec_msg_enable();
        pm_rtd_cec_ac_on_suspend();
        return;
    }
...
int rtk_pm_load_8051(int flag)
{
	...
	if(get_mio_enable_status()){
	    ...
	    rtd_pr_emcu_alert("MIO gnore the command from Main when power off in Main\n");
#if defined(CONFIG_ARCH_RTK2885P)
	    rtd_pr_emcu_alert("1.MIO get [0x18060070] = %x\n", rtd_inl(0xb8060070));
	    rtd_pr_emcu_alert("1.MIO get [0x1801a008] = %x\n", rtd_inl(0xb801a008));
	    rtd_pr_emcu_alert("1.MIO get [0x18072000] = %x\n", rtd_inl(0xb8072000));
	    rtd_pr_emcu_alert("1.MIO get [0x18072008] = %x\n", rtd_inl(0xb8072008));
	    rtd_pr_emcu_alert("1.MIO get [0x180720c8] = %x\n", rtd_inl(0xb80720c8));
	    rtd_pr_emcu_alert("1.MIO get [0x18070780] = %x\n", rtd_inl(0xb8070780));
	    rtd_pr_emcu_alert("1.MIO get [0x18072030] = %x\n", rtd_inl(0xb8072030));
	    rtd_pr_emcu_alert("1.MIO get [0x18072040] = %x\n", rtd_inl(0xb8072040));
#endif
...
#if defined(CONFIG_ARCH_RTK2885P)
	    rtd_pr_emcu_alert("2.MIO get [0x18060070] = %x\n", rtd_inl(0xb8060070));
	    rtd_pr_emcu_alert("2.MIO get [0x1801a008] = %x\n", rtd_inl(0xb801a008));
	    rtd_pr_emcu_alert("2.MIO get [0x18072000] = %x\n", rtd_inl(0xb8072000));
	    rtd_pr_emcu_alert("2.MIO get [0x18072008] = %x\n", rtd_inl(0xb8072008));
	    rtd_pr_emcu_alert("2.MIO get [0x180720c8] = %x\n", rtd_inl(0xb80720c8));
	    rtd_pr_emcu_alert("2.MIO get [0x18070780] = %x\n", rtd_inl(0xb8070780));
	    rtd_pr_emcu_alert("2.MIO get [0x18072030] = %x\n", rtd_inl(0xb8072030));
	    rtd_pr_emcu_alert("2.MIO get [0x18072040] = %x\n", rtd_inl(0xb8072040));

	    rtd_outl(0xb8060140, 0x028704f46);	// infom emcu
#endif
	}
