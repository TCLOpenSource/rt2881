...
#define LOCK_EMCU_KDV()   mutex_lock(&emcu_kdv_lock)
#define UNLOCK_EMCU_KDV() mutex_unlock(&emcu_kdv_lock)
static DEFINE_MUTEX(emcu_kdv_lock);
...
unsigned char hdmi_ch_det = 0;
static unsigned int wow_keypad_g = 0;
unsigned char hdmi_monitor_signal = 0;
...
#define USB_WAKE_EVENT      BIT(USB_WAKE_SHT)
/**********************************************/
...
#define EMCU_BT_WAKE_UP_ALERT		BIT(6)
...
#define PARAM_PWEN_TYPE  13
#define IDX_A0V9_PIN			(0xB0ULL)	//A0V9 power
...
unsigned int pwrkey_handle_count = 0;;
unsigned int poweron_reg_row = 0;
unsigned int power_reason = 0;
unsigned int wakeup_reg_row = 0;
unsigned int wakeup_reason = 0;
static unsigned int wakeup_type = 0;
static unsigned int wakeup_scancode = 0;
static unsigned int wakeup_flag = 0;

unsigned char get_ac_on_flag(void)
{
    return emcu_ac_on_alt;
}
EXPORT_SYMBOL(get_ac_on_flag);
...
extern void cec_set_wake_source_src(unsigned int wakeupsrc);
#endif
/************************************************************************
 *
 ************************************************************************/
 ...
 *
 ************************************************************************/
u32 emcu_USB_WAKE_event(void)
{
    if(emcu_suspend_event & USB_WAKE_EVENT){
        return (emcu_suspend_event & USB_WAKE_EVENT);
    }
    return 0;
}
EXPORT_SYMBOL(emcu_USB_WAKE_event);
...
{
    hdmi_ch_det = 0;
    EMCU_INFO("emcu_clr_hdmi_det\n");
}
EXPORT_SYMBOL(emcu_clr_hdmi_det);

unsigned char emcu_get_hdmi_det(unsigned char channel)
{
    if ((hdmi_ch_det>>channel)&0x1)
        return 1;
    return 0;
}
EXPORT_SYMBOL(emcu_get_hdmi_det);

unsigned char emcu_set_hdmi_det(unsigned int nport, unsigned int enable)
{
    unsigned int hdmi_det[2] = {0};
    int res = 0;

    if(new_protocol_enable == 1){
        hdmi_det[0] = nport;  //port
        hdmi_det[1] = enable;  //enable
        res = send_packet(EMCU_HDMI_DET, 2, hdmi_det, NULL, 2, 0);
        if(res != EMCU_CMD_DONE){
            EMCU_ERR("set hdmi det wakup failed=%d\n",res);
        }
    }else{
        EMCU_ERR("[%s]not support\n", __func__);
    }
    return 0;
}
EXPORT_SYMBOL(emcu_set_hdmi_det);

// Get the hdmi_enable_dpm variable in emcu.
// hdmi_enable_dpm:  0 -> DPM disabled, 1 -> DPM enabled and wake up when fake wakeup and tmds clock detected
unsigned char emcu_get_emcu_hdmi_enable_dpm(unsigned int nport)
{
    unsigned int ret_data[EMCU_DAT_LEN_MAX]= {0};
    unsigned int hdmi_det = 0;
    int ret = -1;
    ...
    hdmi_det = nport;  //port
    ret = send_packet(EMCU_HDMI_DET, 0, &hdmi_det, ret_data, 1, 1);
    if(ret >= 0){
        if (ret_data[0])
            return 1;
    }
...
EXPORT_SYMBOL(emcu_get_emcu_hdmi_enable_dpm);

void emcu_set_dp_aux(unsigned int value)
{
    int ret = -1;
    if(new_protocol_enable == 1)
    {
        ret = send_packet(EMCU_DP_AUX, value, NULL, NULL, 0, 0);
        if(ret != EMCU_CMD_DONE){
            EMCU_ERR("%s failed=%d\n", __func__, ret);
        }
    }
...
EXPORT_SYMBOL(emcu_set_dp_aux);

unsigned char emcu_wow_keypad_check(void)
{
	if(wow_keypad_g!=0){
		return 1;
	}else{
...
EXPORT_SYMBOL(emcu_wow_keypad_check);

unsigned char emcu_wowlan_enable(void)
{
	...
	if(wow_keypad_g!=0){
		if(emcu_WOW_event()!=0){
			EMCU_NOTICE("AP has setting wow event\n");
		}else{
			EMCU_NOTICE("wowlan_enable set:%x\n", wow_keypad_g);
			if(new_protocol_enable == 1){
				keypad_arry_ext[keypad_valid_num] = wow_keypad_g;
				...
				if(emcu_auto_event & EMCU_WOWLAN_ALERT){
					SET_KEYPAD_PARM(valid_num-1,wow_keypad_g);
				}else{
					...
					SET_KEYPAD_PARM(valid_num,wow_keypad_g);
					valid_num += 1;
				}
				EMCU_DEBUG("wow keypad valid_num:%u\n",valid_num);
				...
				EMCU_INFO("wow pm_event_msg=0x%x\n",pm_event_msg);
				emcu_suspend_event |= WOW_SSP_EVENT;
			}
		}
	}else{
		EMCU_ALERT("wow keypad pin not setting:%d\n", wow_keypad_g);
	}
	return 0;
}
EXPORT_SYMBOL(emcu_wowlan_enable);
...
unsigned long long emcu_get_power_type_idx(char* pin_name)
{
    ...
    int power_cnt = 0;
    int ddr_cnt = 0;
    char *pin_name_type[PARAM_PWEN_TYPE]= {
        ...
        "PIN_RL_ON_SOC",  //8 control POWER BOARD
        "PIN_BT_WAKEUP"    //9 bt power
    };
        ...
        if(strncmp(pin_name, pin_name_type[i], 7)==0){
            switch (i){
                ...
                case 1:
                    ...
                    break;
                case 2:
#ifdef IDX_WIFI_RESET
                    if(strncmp(pin_name, "PIN_WLAN_RESET", 14)==0){
                        ret_idx = IDX_WIFI_RESET << IDX_SHIFT;
                        EMCU_ALERT("wifi reset pin hit\n");
                    }else
#endif
                    {
...
#endif
                        {
                            ret_idx = IDX_A0V9_PIN << IDX_SHIFT;
                ...
                case 8:
                    ret_idx = IDX_POWER_PIN << IDX_SHIFT;
                    EMCU_ALERT("PIN_RL_ON_SOC pin hit\n");
                    break;
                case 9:
#ifdef IDX_BT_WAKE_UP
                    ret_idx = IDX_BT_WAKE_UP << IDX_SHIFT;
                    EMCU_ALERT("PIN_BT_WAKEUP pin hit\n");
#else
                    ret_idx = 0xFAFA;
                    EMCU_ALERT("PIN_BT_WAKEUP no define!!!\n");
#endif
                    break;
                default:
        ...
        bool read)
{
    ...
    LOCK_EMCU_KDV();
    //get suspend flag if use
#ifdef SB2_HD_SEM_NEW2_6_reg
    timeout = 25;
    while(timeout--){
        reg = rtd_inl(SB2_HD_SEM_NEW2_6_reg);
        if ((reg & 0x1))
            break;
        mdelay(2);
    }
    if (!(reg & 0x1)){
        ret = -EMCU_CMD_TIMEOUT;
        EMCU_ERR("%s reg is %x timeout is %d class is %x",__FUNCTION__,reg,timeout,class);
        UNLOCK_EMCU_KDV();
        return ret;
    }
    //write magic num for debug
    rtd_outl(SB2_HD_SEM_NEW2_6_reg,  0x1|0x2468);
#else
    EMCU_DEBUG("NO DEFINE SB2_HD_SEM_NEW2_6_reg");
#endif
    // init
...
#define EMCU_INT_TRIGGER    (EMCU_CPU_INT2_int_sm_mask|EMCU_CPU_INT2_write_data_mask)
    ...
    if(read != 0) {
        ...
        if (checksum != emcu_sum_zero_calc(reg, (unsigned short *)ret_dat, (ret_len<<1))){
            ret = -EMCU_RES_CHK_FAIL;
            goto FINISH_OUT;
        }
        UNLOCK_EMCU_KDV();
#ifdef SB2_HD_SEM_NEW2_6_reg
        rtd_outl(SB2_HD_SEM_NEW2_6_reg,0);
#endif
        return ret_len;
    }
...
FINISH_OUT:
	...
	// clear shared memory
	rtd_outl(RTD_SHARE_MEM_BASE, 0x0);
    return ret;
}
	...
	if(in_val == 0){
		...
		if( (kptype == PCB_PIN_TYPE_ISO_GPIO) ||
			...
			if(new_protocol_enable == 1){
				...
				wow_keypad_g = keypad_arry_ext[keypad_valid_num];
				keypad_valid_num = keypad_valid_num+1;
				emcu_suspend_event |= WOW_SSP_EVENT;
			}else{
				pm_event_msg = GET_EVENT_MSG();
 ...
 ************************************************************************/
bool check_arry_data_same(unsigned int * arry,int num,unsigned int data)
{
    int i=0;
    for(;i<num;i++)
    {
		...
		for(i=0;i<num;i++){
			...
			case PCB_PIN_TYPE_MIO_GPIO:
				...
				if(keypad_arry_ext[valid_num] == 0xff)
					break;
				if(check_arry_data_same(keypad_arry_ext,valid_num,keypad_arry_ext[valid_num]))
				{
					valid_flag =0;
					EMCU_DEBUG("%s %d  0x%08x is exit keypad_arry_ext,skip it\n",__func__, __LINE__,keypad_arry_ext[valid_num]);
				}
				else
					valid_flag =1;
				break;
...
void ac_on_send_cec_msg_enable(void)
{
    unsigned int pm_event_msg;
    int res = 0;
    unsigned long long in_val = 1;
#ifdef EMCU_WIFI_POWER_NAME
    //unsigned int cec_p=3;
#else
    //unsigned int cec_p=15;
 ...
 *
 ************************************************************************/
int emcu_wov_setting(unsigned int enable)
{
	int res = 0;
	unsigned int pm_event_msg;
	...
	const char *buf, size_t count)
{
	...
	EMCU_INFO("emcu_setup_wov:0x%llx\n",in_val);
	...
	if(in_val == 0){
		...
		EMCU_INFO("wov param(0x%llx) no support\n",in_val);
	}
...
int BT_wake_up_pin_parser(void)
{
    int ret = 0;
    unsigned long long param[2] = {0};
#if IS_ENABLED(CONFIG_REALTEK_PCBMGR)
#ifdef EMCU_BT_WAKEUP_PIN_NAME
    unsigned long long  rtk_tmp_gpio;
    unsigned long long ret_value;
#endif
#endif
...
#if IS_ENABLED(CONFIG_REALTEK_PCBMGR)
    #ifdef EMCU_BT_WAKEUP_PIN_NAME
        if(pcb_mgr_get_enum_info_byname(EMCU_BT_WAKEUP_PIN_NAME, &rtk_tmp_gpio)==0){
            ret_value = emcu_get_power_type_idx(EMCU_BT_WAKEUP_PIN_NAME);
            if(ret_value == 0xFAFA)
                goto FINISH_OUT;
            rtk_tmp_gpio |= ret_value;
            cnt_tmp++;
            param[cnt_tmp] = rtk_tmp_gpio;
            EMCU_DEBUG("%s(%d)param[%d]=0x%llx(%s)\n",
                    __func__,__LINE__,cnt_tmp,rtk_tmp_gpio,EMCU_BT_WAKEUP_PIN_NAME);
        }
        else
        {
            goto FINISH_OUT;
    ...
    param[0] = cnt_tmp;
    powerMgr_setup_keypad_multi((unsigned int)param[0],&param[1]);
    emcu_auto_event |= EMCU_BT_WAKE_UP_ALERT;
...
int powerMgr_get_wakeup_source(unsigned int* row, unsigned int* status)
{
    ...
    unsigned int tmp1 = 0;
    unsigned int tmp2 = 0;
    const char* str[]={"UNDEF","KEYPAD","WUT","IRDA","CEC","PPS","WOW","HML","RTC","WOV","WKSOR_EWBS", "WKSOR_UART", \
                        "WKSOR_DP_AUX", "WKSOR_HDMI", "WKSOR_DDC", "WKSOR_USB", "WKSOR_DDCCI"};
    WAKE_UP_T reason;
...
/************************************************************************
 * FUNC: get wake up customer reason  from emcu
 * example:
 *	cat emcu_get_wakeup_reason
 ************************************************************************/
static
const uint32_t wakeKeyMap[][2] = {
	//tcl-RCA
...
void get_wakeup_reason(unsigned int *reg_row, unsigned int *reason)
{
	...
	*reason = *reg_row & 0xff;
	...
	if(*reason == WKSOR_IRDA) { //get the keycode
...
//for silent power on then wakeup by IR/KEYAPD power key, need update wakeup reason
void powerMgr_update_wakeup_reason(unsigned int type, unsigned int scancode, unsigned int flag)
{
	wakeup_type = type;
	wakeup_scancode = scancode;
	wakeup_flag = flag;
}
EXPORT_SYMBOL(powerMgr_update_wakeup_reason);

static
ssize_t emcu_get_wakeup_reason_dev_show(struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	int buf_offset = 0;
	const char* str[] = {"UNDEF","KEYPAD",/*"WUT"*/"RTC","IR","CEC","PPS","WOW","HML","RTC",/*"WOV"*/"VOICE","WOL","OTHER"};
	//const char* new_str[] = {"UNDEF","KEYPAD-0",/*"WUT"*/"RTC","IR-POWER","CEC","PPS","WOW","HML","RTC",/*"WOV"*/"VOICE","WOL","OTHER"};
	char power_reason_str[8]={0};
	char wakeup_reason_str[8]={0};
	memset(power_reason_str, 0, sizeof(power_reason_str));
	...
	if (pwrkey_handle_count != 0) {
		...
		get_wakeup_reason(&wakeup_reg_row, &wakeup_reason);
	}
	...
	// for power on case
	if(GET_WAKE_UP_GPIO_SUB(poweron_reg_row) == WKSOR_BT)
		strncpy(power_reason_str, "bt", 2);
	else
		strncpy(power_reason_str, str[power_reason], strlen(str[power_reason]));

	// for STR case
	if(GET_WAKE_UP_GPIO_SUB(wakeup_reg_row) == WKSOR_BT)
		strncpy(wakeup_reason_str, "bt", 2);
	else if(GET_WAKE_UP_GPIO_SUB(wakeup_reg_row) == WKSOR_WIFI)
		strncpy(wakeup_reason_str, "WOW", 3);
	else
		strncpy(wakeup_reason_str, str[wakeup_reason], strlen(str[wakeup_reason]));

	if(wakeup_flag == 1){ // for silent on then wakeup by IR/keypad, update reason
		buf_offset += sprintf(buf + buf_offset,
			"poweron keycode: %x, poweron reason: %s, wakeup keycode: %x, wakeup reason: %s, resume: %d\n",
			(unsigned int)poweron_reg_row, power_reason_str, (unsigned int)wakeup_scancode, str[wakeup_type], pwrkey_handle_count);
	}
	else
		buf_offset += sprintf(buf + buf_offset,
			"poweron keycode: %x, poweron reason: %s, wakeup keycode: %x, wakeup reason: %s, resume: %d\n",
			(unsigned int)poweron_reg_row, power_reason_str, (unsigned int)wakeup_reg_row, wakeup_reason_str, pwrkey_handle_count);
...
static
ssize_t emcu_get_wakeup_reason_dev_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
...
DEVICE_ATTR(emcu_get_wakeup_reason, S_IRUGO | S_IWUSR,
		emcu_get_wakeup_reason_dev_show,emcu_get_wakeup_reason_dev_store);
...
/************************************************************************
 *  * FUNC: emcu_enable_hdmi_det_wak
 *   * example:
 *   *	cat emcu_enable_hdmi_det_wak
 *   *  set 0,1 > emcu_enable_hdmi_det_wak
 *     ************************************************************************/
static ssize_t emcu_enable_hdmi_det_wak_dev_show(struct device *dev,
			struct device_attribute *attr,
				char *buf)
{
    int  n;
    ...
    n = scnprintf(ptr, count, "please key in \"echo channel,enable > "
            "/sys/devices/virtual/misc/emcu_kdv/emcu_enable_hdmi_det_wak\"\n");
    ptr+=n; count-=n;

    n = scnprintf(ptr, count, "hdmi_ch0=%s\n", (hdmi_ch_det&0x1)?"enabled":"disabled");
    ptr+=n; count-=n;
    n = scnprintf(ptr, count, "hdmi_ch1=%s\n", (hdmi_ch_det&0x2)?"enabled":"disabled");
    ptr+=n; count-=n;
    n = scnprintf(ptr, count, "hdmi_ch2=%s\n", (hdmi_ch_det&0x4)?"enabled":"disabled");
    ptr+=n; count-=n;
    n = scnprintf(ptr, count, "hdmi_ch3=%s\n", (hdmi_ch_det&0x8)?"enabled":"disabled");
    ptr+=n; count-=n;

    return ptr - buf;
}

static ssize_t emcu_enable_hdmi_det_wak_dev_store(struct device *dev,
			struct device_attribute *attr,
				const char *buf, size_t count)
{
    ...
    if(new_protocol_enable == 1){
        if ((param[0]&0xff00) == 0x100)
        {
            hdmi_monitor_signal = (unsigned char)param[1];
            EMCU_INFO("%s signal monitor, hdmi_monitor_signal=%x\n", param[1] ? "enable":"disable", hdmi_ch_det);
        }
        else
        {
            if (param[1]&_BIT1) // Fake wake-up, then standby immediately
                hdmi_ch_det |= (unsigned char)EMCU_DPM_FAKE_WAKEUP_MASK;
            else
                hdmi_ch_det &= ~((unsigned char)EMCU_DPM_FAKE_WAKEUP_MASK);

            if (param[1]&_BIT0) //enable
                hdmi_ch_det |= ((unsigned char)1<<(unsigned char)param[0]); //port
            else
                hdmi_ch_det &= ~((unsigned char)1<<(unsigned char)param[0]); //port
            
            EMCU_INFO("%s DPM, hdmi_ch_det=%x\n", param[1] ? "enable":"disable", hdmi_ch_det);
        }
...
DEVICE_ATTR(emcu_enable_hdmi_det_wak, S_IRUGO | S_IWUSR,
				emcu_enable_hdmi_det_wak_dev_show, emcu_enable_hdmi_det_wak_dev_store);


/************************************************************************
 *  * FUNC: emcu_enable_pd_drving
 *   * example:
 *   *	cat emcu_enable_pd_drving
 *   *  set 0,1 > emcu_enable_pd_drving
 *     ************************************************************************/
static ssize_t emcu_enable_pd_drving_dev_show(struct device *dev,
			struct device_attribute *attr,
				char *buf)
{
    return snprintf(buf, 256,
            "please key in \"echo enable > "
            "/sys/devices/virtual/misc/emcu_kdv/emcu_enable_pd_drving\"\n");
}

static ssize_t emcu_enable_pd_drving_dev_store(struct device *dev,
			struct device_attribute *attr,
				const char *buf, size_t count)
{
    ...
    if(new_protocol_enable == 1){
        param[0] = (unsigned int)in_val;
        res = send_packet(EMCU_PD_DRING, 1, param, NULL, 1, 0);
        if(res != EMCU_CMD_DONE){
            EMCU_ERR("enable pd drving fialed=%d\n",res);
        }
    }else{
        EMCU_ERR("[%s]not support\n", __func__);
    }
...
DEVICE_ATTR(emcu_enable_pd_drving, S_IRUGO | S_IWUSR,
				emcu_enable_pd_drving_dev_show, emcu_enable_pd_drving_dev_store);


/************************************************************************
 *  * FUNC: emcu_enable_mdns_video_handle
 *   * example:
 *   *	cat emcu_enable_mdns_video_handle
 *   *  set 0,1 > emcu_enable_mdns_video_handle
 *     ************************************************************************/
static ssize_t emcu_enable_mdns_video_handle_dev_show(struct device *dev,
			struct device_attribute *attr,
				char *buf)
{
    return snprintf(buf, 256,
            "please key in \"echo enable > "
            "/sys/devices/virtual/misc/emcu_kdv/emcu_enable_mdns_video_handle\"\n");
}

static ssize_t emcu_enable_mdns_video_handle_dev_store(struct device *dev,
			struct device_attribute *attr,
				const char *buf, size_t count)
{
    ...
    if(new_protocol_enable == 1){
        param[0] = (unsigned int)in_val;
        res = send_packet(EMCU_MDNS_VIDEO_HANDLE, 1, param, NULL, 1, 0);
        if(res != EMCU_CMD_DONE){
            EMCU_ERR("enable mdns_video_handle fialed=%d\n",res);
        }
    }else{
        EMCU_ERR("[%s]not support\n", __func__);
    }
...
DEVICE_ATTR(emcu_enable_mdns_video_handle, S_IRUGO | S_IWUSR,
				emcu_enable_mdns_video_handle_dev_show, emcu_enable_mdns_video_handle_dev_store);


/************************************************************************
 *  * FUNC: emcu_enable_usb_wake
 *   * example:
 *   *	cat emcu_enable_usb_wake
 *   *  set 0,1 > emcu_enable_usb_wake
 *     ************************************************************************/
static ssize_t emcu_enable_usb_wake_dev_show(struct device *dev,
			struct device_attribute *attr,
				char *buf)
{
    return snprintf(buf, 256,
            "please key in \"echo enable > "
            "/sys/devices/virtual/misc/emcu_kdv/emcu_enable_usb_wake\"\n");
}

static ssize_t emcu_enable_usb_wake_dev_store(struct device *dev,
			struct device_attribute *attr,
				const char *buf, size_t count)
{
    ...
    if(new_protocol_enable == 1){
        param[0] = (unsigned int)in_val;
        res = send_packet(EMCU_USB, 1, param, NULL, 1, 0);
        if(res != EMCU_CMD_DONE){
            EMCU_ERR("enable enable_usb_wake fialed=%d\n",res);
        }else{
            emcu_suspend_event |= USB_WAKE_EVENT;
        }
    }else{
        EMCU_ERR("[%s]not support\n", __func__);
    }
...
DEVICE_ATTR(emcu_enable_usb_wake, S_IRUGO | S_IWUSR,
				emcu_enable_usb_wake_dev_show, emcu_enable_usb_wake_dev_store);
...
int emcu_chk_event(void)
{
	...
	if(new_protocol_enable == 1){
	    ...
	    /*check wake up pin always*/
	    if(!(emcu_auto_event & EMCU_BT_WAKE_UP_ALERT)){
	        EMCU_ALERT("%s(%d)auto parse BT wake up pin !\n",__func__,__LINE__);
	        if(BT_wake_up_pin_parser()){
	            EMCU_ALERT("%s(%d)BT wake up pin auto parse fail!\n",__func__,__LINE__);
	        }
	    }else{
	        EMCU_ALERT("%s(%d)BT wake up pin set already\n",__func__,__LINE__);
	    }
		...
		/*keypad setting*/
		if(emcu_auto_event & EMCU_KEYPAD_ALERT || emcu_auto_event & WOL_SSP_EVENT || emcu_auto_event & EMCU_BT_WAKE_UP_ALERT|| keypad_valid_num){
...
extern unsigned int str_status;//str status for rtk_get_local_time
#endif
static int emcu_drv_suspend(struct device *dev)
{
	int i;
	...
	memset(local_sharemem_res, 0, sizeof(local_sharemem_res));
	/* clear AC on flag */
	emcu_ac_on_alt  = 0;
	g_rtk_load_8051 = 0;

	//for AP only set in WOV UI on/off
	if((wov_flag == 1) && (emcu_WOV_event()==0)){
		EMCU_INFO("suspend set wov\n");
		emcu_wov_setting(1);
	}
...
#if IS_ENABLED(CONFIG_RTK_KDRV_IR)
    rtd_pr_emcu_info("<< %s >>: send power key\n", __FUNCTION__);
    venus_ir_gen_keyupdown_event(KEY_POWER);
#endif
#endif
}
EXPORT_SYMBOL(pm_send_power_key);



void pm_send_func_key(uint32_t keycode)
{
/*  for Android TV if resume from 8051,
that means wake up source by ir remote, then send a IR func key for wakeup Android.*/
#ifdef CONFIG_ANDROID
#if IS_ENABLED(CONFIG_RTK_KDRV_IR)
    rtd_pr_emcu_info("<< %s >>: send func key\n", __FUNCTION__);
...
static int emcu_drv_resume(struct device *dev)
{
    ...
    if(new_protocol_enable == 0){
        if(((GET_EVENT_MSG() == 0x00) || ((GET_EVENT_MSG() >> loc_rtc) & 0x4)) && (GET_WK_SOR() != 0x00)){
            /*  GET_EVENT_MSG() == 0x00,
        ...
        if(send_packet(EMCU_WAKE_UP_SOURCE, 0, NULL, ret_data, 0, 1) >= 0){
            local_sharemem_res[IDX_REG_WK_STS] = ret_data[0];  //status
            local_sharemem_res[IDX_REG_WK_SOR] = ret_data[1];  //source
            EMCU_INFO("\n%s(%d)wakeup source:sts:0x%08x; sor:0x%08x\n",
                __func__,__LINE__,local_sharemem_res[IDX_REG_WK_STS],local_sharemem_res[IDX_REG_WK_SOR]);
        }
    }
#ifdef CONFIG_MULTI_IR_KEY
    reno_gpi_cnt = 0;
	...
	cec_set_wake_source_src(wakeup_reg_row);

	if(((wakeup_src  < WKSOR_END) || wakeup_src == WKSOR_BT) && (wakeup_src != WKSOR_UNDEF) && ((wakeup_src != WKSOR_RTC) && (wakeup_src != WKSOR_WIFI) && (wakeup_src != WKSOR_WOW)) && (wakeup_src != WKSOR_WOV) && (wakeup_src != WKSOR_IRDA)){
		pm_send_power_key();
	}else if( wakeup_src == WKSOR_IRDA){
		pm_send_func_key(venus_ir_scancode_to_keycode(local_sharemem_res[IDX_REG_WK_STS]));
	}else{
		rtd_pr_emcu_err("No send power key(%x)\n", wakeup_src);
	}
#endif
	//confirm by weihao, for 8051 suspend be abort cause abnormal, need resume to reset.
	if(new_protocol_enable == 0){
		if((rtd_inl(STB_ST_SRST1_reg) & _BIT9) && (rtd_inl(STB_ST_CLKEN1_reg) & _BIT9)) {
			rtd_outl(REG_MAGIC_51, 0);
			udelay(500);
			rtd_outl(STB_ST_SRST1_reg, _BIT9);			// rst off
			rtd_outl(STB_ST_CLKEN1_reg, _BIT9); // clk disable
			rtd_outl(STB_ST_EMCUCLK_reg, 0);
		}
	}
...
int __init rtk_emcu_module_init(void)
{
    ...
    device_create_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_hdmi_det_wak);
    device_create_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_pd_drving);
    device_create_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_mdns_video_handle);
    device_create_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_usb_wake);
    device_create_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_get_wakeup_reason);
...
void __exit rtk_emcu_module_exit(void)
{
    ...
    device_remove_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_hdmi_det_wak);
    device_remove_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_pd_drving);
    device_remove_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_mdns_video_handle);
    device_remove_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_enable_usb_wake);
    device_remove_file(rtk_emcu_miscdev.this_device, &dev_attr_emcu_get_wakeup_reason);
#ifdef CONFIG_PM
