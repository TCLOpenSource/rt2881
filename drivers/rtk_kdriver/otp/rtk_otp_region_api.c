...
int rtk_get_cp_iddq_core_power(void)
{
    int bit_size;
    int ret;
    bit_size = rtk_otp_region_get_size_by_name("hw_mp_status_cp_iddq_done");
    ret = rtk_otp_field_read_int_by_name("hw_mp_status_cp_iddq_done");
    /************MAY NEED CHECK***********/
    if(bit_size != 1 && bit_size != 3)
		return -1;
    if(ret == -1)
    ...
    return rtk_otp_field_read_int_by_name("hw_cp_iddq_core_power");
}
EXPORT_SYMBOL(rtk_get_cp_iddq_core_power);

int rtk_get_cp_iddq_scpu_power(void)
{
    int bit_size;
    int ret;
    bit_size = rtk_otp_region_get_size_by_name("hw_mp_status_cp_iddq_done");
    ...
    return rtk_otp_field_read_int_by_name("hw_cp_iddq_scpu_power");
}
EXPORT_SYMBOL(rtk_get_cp_iddq_scpu_power);
...
int rtk_get_hw_4k165_boost_mode(void)
{
    int val1 = rtk_otp_field_read_int_by_name("hw_4k165_boost_mode_ft_burned");
    int val2 = rtk_otp_field_read_int_by_name("hw_4k165_boost_mode_ft");
    if(val1 == -1 || val2 == -1)
		return -1;
    return ((val1 << 1) | val2);
    
}
EXPORT_SYMBOL(rtk_get_hw_4k165_boost_mode);