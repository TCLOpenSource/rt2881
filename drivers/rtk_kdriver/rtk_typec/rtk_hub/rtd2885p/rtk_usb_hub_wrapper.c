...
{
	if(rtd_inl(SYS_REG_SYS_CLKDIV_reg) & SYS_REG_SYS_CLKDIV_usb_pc_dphy_mux_p2_mask)
		return true;
	else
		return false;
}
#define SUPPORT_VBUS_TOGGLE //for usb cts
...
{
	if((rtd_inl(SYS_REG_SYS_CLKEN2_reg) & SYS_REG_SYS_CLKEN2_clken_usb2_ex_dwc_mask)
		&& (rtd_inl(SYS_REG_SYS_SRST2_reg) & SYS_REG_SYS_SRST2_rstn_usb2_ex_dwc_mask))
		return true;
	else
		return false;
}
