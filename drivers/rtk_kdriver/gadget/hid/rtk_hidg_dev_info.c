...
static struct hid_ll_driver dummy_ll_driver;
extern bool g_hid_fake_standby_usb_filter;
extern bool g_hid_fake_standby_bluetooth_filter;
...
//support absolut mouse
		   0x05,0x01,	/*Usage Page (Generic Desktop Controls)*/
                ...
                0x81,0x02,	/*Input(Data,Variable,Relative,BitFiled)*/
...
static bool rtk_hidg_parse_hid_report_desc(struct rtk_hidg_report_desc_data *data)
{
	 ...
	 hid_dev->ll_driver = &dummy_ll_driver;
...
extern bool g_hid_via_otg_ex_enable;
...
bool rtk_hidg_send_hid_report(int id, const unsigned char *buffer, unsigned int count, unsigned int f_flags)
{
	...
	f_hidg_do_usb_wake();
#endif	
	if(g_hid_via_otg_enable || f_hidg_in_usb_remote_wake()) {
		f_hidg_send_data(id, buffer, count, f_flags);
	} if(g_hid_via_ddc_enable) {
	 	f_hidddc_send_data(id, buffer, count, f_flags);
	} if(g_hid_via_otg_ex_enable) {
		f_hidg_ex_send_data(id, buffer, count, f_flags);
	}
	return true;
}
...
int hid_input_report_callback(struct hid_device *hdev, struct hid_report *report, u8 *data, int size)
{
	...
	if(g_hid_fake_standby_usb_filter && hdev->bus == BUS_USB)
		return -EBUSY;

	if(g_hid_fake_standby_bluetooth_filter && hdev->bus == BUS_BLUETOOTH)
		return -EBUSY;
...
EXIT:
	if(hdev && hdev->driver && hdev->driver->driver.android_kabi_reserved1) {
		RTK_HID_RAW_EVENT raw_event = (RTK_HID_RAW_EVENT )hdev->driver->driver.android_kabi_reserved1;
		ret = (raw_event)(hdev, report, data, size);
	}
#endif
	return ret;
}
		...
		void *data)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)) && defined(CONFIG_TRACEPOINTS) && defined(CONFIG_ANDROID_VENDOR_HOOKS)		
		if(!hdev->android_kabi_reserved1) {
			hdev->android_kabi_reserved1 = (unsigned long)kzalloc(sizeof(struct hid_report_mapping_data), GFP_KERNEL);
		}
#endif		
	...
	case BUS_NOTIFY_BOUND_DRIVER:
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)) && defined(CONFIG_TRACEPOINTS) && defined(CONFIG_ANDROID_VENDOR_HOOKS)
#ifdef CONFIG_RTK_KDRV_USB_GADGET_HID
		if(!hdev->android_kabi_reserved1) {
			hdev->android_kabi_reserved1 = (unsigned long)kzalloc(sizeof(struct hid_report_mapping_data), GFP_KERNEL);
		}
#endif		
		if(hdev->android_kabi_reserved1) {
				...
				if(hdev->driver) {
					RTK_COMPOSITE_INFO("driver_name:%s raw_event:%lx hid_input_report_callback:%lx android_kabi_reserved1:%llx\n", hdev->driver->name ? hdev->driver->name : "", 
						(unsigned long)hdev->driver->raw_event, (unsigned long)hid_input_report_callback, 
						hdev->driver->driver.android_kabi_reserved1);
					if(hdev->driver->raw_event != hid_input_report_callback)
						hdev->driver->driver.android_kabi_reserved1 = (unsigned long)hdev->driver->raw_event;
...
static int __hid_bus_reprobe_drivers(struct device *dev, void *data)
{
	...
	if(ret != 0)
		RTK_COMPOSITE_INFO("reprobe fail\n");
	return 0;
}

static void check_registered_hid_device(void)
{
	bus_for_each_dev(s_hid_bus_type, NULL, NULL,
...
void rtk_hidg_unregister_hid_input_report_callback(void)
{
