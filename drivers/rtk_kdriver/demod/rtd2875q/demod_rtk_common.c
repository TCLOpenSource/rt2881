...
#define AUTO_MEM_ALLOCATE 1

#if AUTO_MEM_ALLOCATE
...
void DtvDemodDcReqOff(unsigned char off)
{

	unsigned int dtvDemodClk = 0, dtvDemodRstN = 0;
	dtvDemodClk = SYS_REG_SYS_CLKEN2_get_clken_dtv_demod(rtd_inl(SYS_REG_SYS_CLKEN2_reg));
	dtvDemodRstN = SYS_REG_SYS_SRST2_get_rstn_dtv_demod(rtd_inl(SYS_REG_SYS_SRST2_reg));


	if (dtvDemodClk == 1 && dtvDemodRstN == 1)
		rtd_maskl(DTV_DEMOD_MISC_DEMOD_DC_REQ_OFF_reg, ~DTV_DEMOD_MISC_DEMOD_DC_REQ_OFF_demod_dc_req_off_mask, DTV_DEMOD_MISC_DEMOD_DC_REQ_OFF_demod_dc_req_off(off));
	tv_osal_msleep(5);
}
...
int DtvIsDemodClockEnable(RTK_DEMOD_MODE mode)
{
	...
	if ((mode == RTK_DEMOD_MODE_ATSC) || (mode == RTK_DEMOD_MODE_QAM) || (mode == RTK_DEMOD_MODE_DVBC) || (mode == RTK_DEMOD_MODE_ISDB) || (mode == RTK_DEMOD_MODE_DTMB)  ) {
...
void DemodAdcSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb8033124, 0xffffeeee, 0x00001111);
		rtd_maskl(0xb8033128, 0x8e088fff, 0x10210000);
		//IQADC+IQPGA power on
		rtd_maskl(0xb8033100, 0xfffff3ff, 0x00000400); //default setting, <bit11:10>REG_I_VCM_SEL=0x1
		rtd_maskl(0xb803312c, 0xfffff3ff, 0x00000400); //default setting, <bit11:10>REG_Q_VCM_SEL=0x1
		rtd_maskl(0xb8033130, 0x33ff3fff, 0x88004000);
		rtd_maskl(0xb8033134, 0xccfffe08, 0x11000091);
		//Set IPGA/QPGA GAIN = 0dB
		rtd_maskl(0xb8033120, 0xfffffffe, 0x00000001); //IF ADC GAIN FIX EN <bit0>= 1(Internal Demod IQ mode)
		...
		QVcmSetting(mode);				//q_vcm setting from OTP setting
	...
	case RTK_DEMOD_MODE_ATV:
		...
		//IQADC+IQPGA power on
		rtd_maskl(0xb8033100, 0xfffff3ff, 0x00000400); //default setting, <bit11:10>REG_I_VCM_SEL=0x1
		rtd_maskl(0xb8033130, 0xf3ffffff, 0x08000000); //default setting, <bit27:26>REG_IADC_I_VREF_SEL=0x2(1.5X)
		rtd_maskl(0xb8033134, 0xfcfffff8, 0x01000001);
		//Set IPGA/QPGA GAIN = 0dB
		rtd_maskl(0xb8033120, 0xfffffffe, 0x00000001); //IF ADC GAIN FIX EN <bit0>= 1(Internal Demod IQ mode)
		rtd_maskl(0xb803312c, 0xfffefcf0, 0x00000000);	//<bit7:4>reg_qpga_d=0x0(0dB), <bit3:0>reg_ipga_d=0x0(0dB)
	...
	default:
		...
		//IQADC+IQPGA power on
		rtd_maskl(0xb8033100, 0xfffff3ff, 0x00000400); //default setting, <bit11:10>REG_I_VCM_SEL=0x1
		rtd_maskl(0xb8033130, 0xf3ffffff, 0x08000000); //default setting, <bit27:26>REG_IADC_I_VREF_SEL=0x3(2X)
		rtd_maskl(0xb8033134, 0xfcfffe78, 0x01000081); //default setting,<bit25:24>REG_IADC_VREF_SEL=0x2(1.4V) <bit2:0>REG_IADC_LDO_SEL=0x5(0.908V)
		//Set IPGA/QPGA GAIN = 0dB
		rtd_maskl(0xb8033120, 0xfffffffe, 0x00000001);
		rtd_maskl(0xb803312c, 0xfffefcf0, 0x00000000);//<bit7:4>reg_qpga_d=0x0(0dB), <bit3:0>reg_ipga_d=0x0(0dB)
...
void DemodAdcCapcalSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
	case RTK_DEMOD_MODE_ATV:
		RTK_DEMOD_INFO("set IADC_CAPCAL: 0xa00\n");
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL1_reg, ~TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal_mask, TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal(0xa00)); //IADC_CAPCAL = 0xa00  for IDNL measurement
		break;
	...
	case RTK_DEMOD_MODE_DVBS2X:
	case RTK_DEMOD_MODE_SPEC_AN:
		RTK_DEMOD_INFO("set QADC_CAPCAL: 0xb00 and IADC_CAPCAL: 0xa00\n");
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL1_reg, ~(TV_SB1_ANA_IQADC_CTRL1_reg_qadc_capcal_mask | TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal_mask), TV_SB1_ANA_IQADC_CTRL1_reg_qadc_capcal(0xb00) | TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal(0xa00));//QADC_CAPCAL = 0xb00 / IADC_CAPCA = 0xa00 for IDNL measurement
		break;
	default:
		RTK_DEMOD_INFO("set QADC_CAPCAL: 0xb00 and IADC_CAPCAL: 0xa00\n");
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL1_reg, ~(TV_SB1_ANA_IQADC_CTRL1_reg_qadc_capcal_mask | TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal_mask), TV_SB1_ANA_IQADC_CTRL1_reg_qadc_capcal(0xb00) | TV_SB1_ANA_IQADC_CTRL1_reg_iadc_capcal(0xa00));//QADC_CAPCAL = 0xb00 / IADC_CAPCA = 0xa00 for IDNL measurement
...
int QVcmSetting(RTK_DEMOD_MODE mode)
{
	...
	if (q_vcm_sel_ft >= 0)
		rtd_maskl(TV_SB1_ANA_IFADC_CTRL1_reg, ~TV_SB1_ANA_IFADC_CTRL1_reg_i_vcm_sel_mask, TV_SB1_ANA_IFADC_CTRL1_reg_i_vcm_sel(q_vcm_sel_ft));
	else
		rtd_maskl(TV_SB1_ANA_IFADC_CTRL1_reg, ~TV_SB1_ANA_IFADC_CTRL1_reg_i_vcm_sel_mask, TV_SB1_ANA_IFADC_CTRL1_reg_i_vcm_sel(0x1));
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL5_reg, ~TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel_mask, TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel(q_vcm_sel_ft));
			...
			rtd_maskl(TV_SB1_ANA_IQADC_CTRL5_reg, ~TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel_mask, TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel(0x1));
		RTK_DEMOD_INFO("IQ_VCM OTP Done = %d, IQ_VCM_SEL = %d\n", q_vcm_sel_ft_done, q_vcm_sel_ft);
		break;
	case RTK_DEMOD_MODE_DTMB:
	case RTK_DEMOD_MODE_ISDB:
	case RTK_DEMOD_MODE_ATSC:
	...
	case RTK_DEMOD_MODE_ATV:
	default:
		RTK_DEMOD_INFO("I_VCM OTP Done = %d, I_VCM_SEL = %d\n", q_vcm_sel_ft_done, q_vcm_sel_ft);
		break;
    }
...
void IfdMbiasPowAndAdcPow(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DVBS2X:
	case RTK_DEMOD_MODE_SPEC_AN:
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL3_reg, ~(TV_SB1_ANA_IQADC_CTRL3_reg_pow_q_pga_mask | TV_SB1_ANA_IQADC_CTRL3_reg_pow_i_pga_mask | TV_SB1_ANA_IQADC_CTRL3_reg_pow_q_adc_mask | TV_SB1_ANA_IQADC_CTRL3_reg_pow_i_adc_mask), (TV_SB1_ANA_IQADC_CTRL3_reg_pow_q_pga(0) | TV_SB1_ANA_IQADC_CTRL3_reg_pow_i_pga(0) | TV_SB1_ANA_IQADC_CTRL3_reg_pow_q_adc(0) | TV_SB1_ANA_IQADC_CTRL3_reg_pow_i_adc(0)));	//IQADC PGA & ADC Power Off
		RTK_DEMOD_INFO("IQADC PGA & ADC Power Off\n");
		break;
	case RTK_DEMOD_MODE_DTMB:
	case RTK_DEMOD_MODE_ISDB:
	case RTK_DEMOD_MODE_ATSC:
...
void DemodPllSetting(RTK_DEMOD_MODE mode)
{
	switch (mode) {
	case RTK_DEMOD_MODE_DTMB:
		//**ADC CLK and System CLK setting(ADC=144M, Sys=192M)**//			//VCO and DIV setting : ADC CLK and System CLK setting(ADC=121.5M, Sys=121.5MM)
		rtd_maskl(0xb8000214, 0xFFFDB7FF, 0x00004000); 				//(B14,B11) = 01, Demod Sel = HDIC Demod(DTMB)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x000E0224);				//VCO = M<6:0>+2 = 16+2, N:00=1, 27 /1 *27=486MHz, PLL27X_IFADC_M<bit22:16> = 0x10(16), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x000000A6);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x110/4), sys_clk = 486 / 4 = 121.5 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x100(/4), Adc_clk = 486 / 4 =121.5MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00021000);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <bit11>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_ATSC:
		...
		rtd_maskl(0xb8000214, 0xFFFDB7FF, 0x00004000); 				//(B14,B11) = 10, Demod Sel = RTK Demod(ATSC/DVBC/ISDBT)
		rtd_maskl(0xb80004dc, 0xFDFFFFFE, 0x02000000);				//PLL_L2H_CML_POW <bit25> = 1(on), PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x000e0224);				//VCO = M<6:0>+2 = 14+2, N:00=1, 27 /1 *16=432MHz, PLL27X_IFADC_M<bit22:16> = 0xE(14), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000000);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x0(/2), sys_clk = 432 / 2 = 216MHz, ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x0(/8), Adc_clk = 432 / 8=54MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00021000);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <bit11>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_QAM:
		...
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x000c0224);				//VCO = M<6:0>+2 = 12+2, N:00=1, 27 /1 *14=378MHz, PLL27X_IFADC_M<bit22:16> = 0xC(12), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000011);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x1(/2.5), sys_clk = 378 / 2.5 = 151.2MHz, ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x1(/15), Adc_clk = 378 / 15 = 25.2MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00021000);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <bit11>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_DVBC:
		...
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x00120224);				//VCO = M<6:0>+2 = 18+2, N:00=1, 27 /1 *20=540MHz, PLL27X_IFADC_M<bit22:16> = 0x12(18), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000011);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x1(/2.5), sys_clk = 540 / 2.5 = 216MHz,  ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x1(/15), Adc_clk = 540 / 15 = 36MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00021000);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div2), <bit13>DIF216M_EN=0x0, <bit11>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x0, <bit2>PLL27X_IFADC_REF_SEL=XTAL
	...
	case RTK_DEMOD_MODE_DVBT2:
		...
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x00160224);				//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000037);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x3(/1.25), sys_clk = 648 / 1.25 = 518.4 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x7(/4.5), Adc_clk = 648 / 4.5 =144MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00027400);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x1(div3), <bit13>DIF216M_EN=0x1, <bit12>DTVDIG_EN=0x1, <bit11>LDPC_DIV=0x0(div1.75), <bit10>LDPC_EN=0x1, <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb80004d8, 0xFFFFFEFF, 0x00000100);				//PLL_RESERVE<bit15:8> = 0x1, Select ADC CLK From PLLDIF
		rtd_maskl(0xb8000500, 0xFFFFFFEE, 0x00000010);				//PLL27x_IFADC DTV CLK<bit4>=0x1
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x00100224);				//VCO = M<6:0>+2 = 16+2, N:00=1, 27 /1 *18=486MHz, PLL27X_IFADC_M<bit22:16> = 0x10(16), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004c4, 0xFFFFFF00, 0x00000074);				//System_CLK : PLLDIF_DIVDIG_SEL<bit6:4> = 0x7(/1), sys_clk = 486 / 1 = 486 MHz, ADC_CLK : PLLDIF_DIVADC_SEL<2:0> = 0x4(/4), Adc_clk = 486 / 4 =121.5MHz
		rtd_maskl(0xb80004b8, 0xFFFD83FB, 0x00021c00);				//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x0(div3), <bit13>DIF216M_EN=0x0, <bit12>DTVDIG_EN=0x1, <bit11>LDPC_DIV=0x1(div1.25), <bit10>LDPC_EN=0x1, <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
	...
	case RTK_DEMOD_MODE_ATV:
		...
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00000100); //<15:8>PLL_RESERVE=0x1
		rtd_maskl(0xb80004dc, 0xfdfcfdfe, 0x02010001); //[30:29]PLL27X_IFADC_DUTY=0x02(vco/4), [25]PLL_L2H_CML_POW=0x1, [17:16]PLL27X_CK108M_DIV=0x1(vco=432M),
		...
		rtd_maskl(0xb8000504, 0xFC80FCC8, 0x00160224);				//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), PLL27X_IFADC PUMP current<bit2:0>=0x4(100:25uF)
		rtd_maskl(0xb80004b8, 0xfffdbdfb, 0x00024200);         //<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF216M_DIV=0x1(div3), <bit9>PLL27X_IFADC_ATV_CKO_EN=0x1, <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
...
//MERLIN5
	...
	if (mode == RTK_DEMOD_MODE_DTMB) {
		...
		// rtk dtmb down sample 3 filter setting#
		rtd_outl(0xb8033900, 0x00000000);
		rtd_outl(0xb8033904, 0x1fff0000);
		rtd_outl(0xb8033908, 0x00001fff);
		rtd_outl(0xb803390c, 0x00040001);
		rtd_outl(0xb8033910, 0x00030006);
		rtd_outl(0xb8033914, 0x1ff21ffc);
		rtd_outl(0xb8033918, 0x1ff01feb);
		rtd_outl(0xb803391c, 0x00230005);
		rtd_outl(0xb8033920, 0x0033003a);
		rtd_outl(0xb8033924, 0x1fb80003);
		rtd_outl(0xb8033928, 0x1f791f79);
		rtd_outl(0xb803392c, 0x00791fd6);
		rtd_outl(0xb8033930, 0x013a011a);
		rtd_outl(0xb8033934, 0x1f53009e);
		rtd_outl(0xb8033938, 0x1d2a1de1);
		rtd_outl(0xb803393c, 0x00d61e06);
		rtd_outl(0xb8033940, 0x09fc0533);
		rtd_outl(0xb8033944, 0x0f1b0db4);
		rtd_outl(0xb8033948, 0x00000000);
		rtd_outl(0xb803394c, 0x00000000);
		rtd_outl(0xb8033950, 0x00000000);
		rtd_outl(0xb8033954, 0x00000000);
		rtd_outl(0xb8033958, 0x00000000);
		rtd_outl(0xb803395c, 0x00000000);
		rtd_outl(0xb8033960, 0x00000000);
		rtd_outl(0xb8033964, 0x00000000);
		rtd_outl(0xb8033968, 0x00000000);
		rtd_outl(0xb803396c, 0x00000000);
		rtd_outl(0xb8033970, 0x00000000);
		rtd_outl(0xb8033974, 0x00000000);
		rtd_outl(0xb8033978, 0x00000000);
		rtd_outl(0xb803397c, 0x00000000);
		rtd_outl(0xb8033980, 0x00000000);
		rtd_outl(0xb8033984, 0x00000000);
		rtd_outl(0xb8033988, 0x00000000);
		rtd_outl(0xb803398c, 0x00000000);
	} else if ((mode == RTK_DEMOD_MODE_DVBT) || (mode == RTK_DEMOD_MODE_DVBT2)) {
...
int DemodFrontendPathSetting(RTK_DEMOD_MODE mode)
{
	...
	switch (mode) {
	case RTK_DEMOD_MODE_DTMB:
		rtd_maskl(0xb8033800, 0xFFD8FEEF, 0x00050010);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit18:16>Dtv_frontend_mode=0x1, <bit8>dtv_idata_sel=0x0(from dn flt), <bit4>Dtv_dnflt_en=0x1(DN6)
		...
		rtd_maskl(0xb8000848, 0xffff0fff, 0x00004000);          //Set  GPIO_61 to DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_ATSC:
	case RTK_DEMOD_MODE_ISDB:
		rtd_maskl(0xb8033800, 0xFFD8FEEF, 0x00040000);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit18:16>Dtv_frontend_mode=0x4, <bit8>dtv_idata_sel, <bit4>Dtv_dnflt_en for RTK Demod don't care
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000848, 0xffff0fff, 0x00004000);           //Set  GPIO_61 to DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_QAM:
		rtd_maskl(0xb8033800, 0xffd8feef, 0x00240000);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid),<bit21>rtk_dtv_demod_sel=0x1(OpenCable), <bit18:16>Dtv_frontend_mode=0x4, <bit8>dtv_idata_sel, <bit4>Dtv_dnflt_en for RTK Demod don't care
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000848, 0xffff0fff, 0x00004000);          //Set  GPIO to DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_DVBT:
	case RTK_DEMOD_MODE_DVBT2:
		rtd_maskl(0xb8033800, 0xFFD8FEEF, 0x00010010);						//<bit18:16>Dtv_frontend_mode=0x1, <bit8>dtv_idata_sel=0x0(from dn flt), <bit4>Dtv_dnflt_en=0x1(DN6)
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000848, 0xffff0fff, 0x00004000);              //Set  GPIO to DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);	//input fifo clr, dtv_input_fifo_clr<bit10> = 0x1, Iadc Input FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);	//input fifo clr, dtv_input_fifo_clr<bit10> = 0x1, Iadc Input FIFO waddr & raddr clear
		rtd_maskl(0xb8000844, 0xffffff0f, 0x000000a0); //GPIO_24, Set pinmux ATV_IF_AGC
	...
	case RTK_DEMOD_MODE_ATV:
		...
		rtd_maskl(0xb8000848, 0xffff0fff, 0x00003000);	//gpio: ATV_IF_AGC
		break;
...
)
{
	...
	//RTK_DEMOD_INFO("===(Write)DemodAddrMsb = 0x%x, DemodAddrLsb = 0x%x===\n", DemodAddrMsb, DemodAddrLsb);
	//RTK_DEMOD_INFO("===(Write)RbusAddr = 0x%x, ByteNum = 0x%x===\n", RbusAddr, ByteNum);
	...
	if (RbusAddr == 0xb814c000) { //8051 SRAM Start: 0x3000
		...
		//RTK_DEMOD_INFO("[8051_SRAM_S]0xB815C004 = 0x%x\n", rtd_inl(0xB815C004));
	}
	...
	if (ByteNum > 2) {
		for (i = 2; i < ByteNum; i++) {
			...
			//RTK_DEMOD_INFO("RbusAddr = 0x%8x, RbusData = 0x%x, 0xB815C004 = 0x%x\n", RbusAddr + (i - 2) * 4, RbusData, rtd_inl(0xB815C004));
	...
	if ((RbusAddr + (i - 3) * 4) == 0xb814effc) { //0x3BFF (only use RAM 3K (3072) for ATSC)
		...
		//RTK_DEMOD_INFO("[8051_SRAM_E]i = %u, 0xB815C004 = 0x%x\n", i, rtd_inl(0xB815C004));
	}
	return FUNCTION_SUCCESS;
...
)
{
	...
	unsigned char i;
	unsigned char RbusData;
	//RTK_DEMOD_INFO("===(Read)RbusAddr = 0x%8x, ByteNum = 0x%x===\n", RbusAddr, ByteNum);
	...
	for (i = 0; i < ByteNum; i++) {
		...
		//RTK_DEMOD_INFO("===(Read)pReadingBytes[%u] = 0x%8x\n", i,  RbusData);
	}
	return FUNCTION_SUCCESS;
...
int DtvDemodMemRemappingInt(size_t size, unsigned long *phyAddr, unsigned int *StaticAllocateSize)
{
	...
	RTK_DEMOD_INFO("\033[1;32;31m""Dynamic allocate memory\n""\033[m");
		...
		if(size > DemodShareMemorySize) {
			...
			DemodShareMemoryPhysicalAddress = (unsigned long)pli_malloc(size, GFP_DCU1);
