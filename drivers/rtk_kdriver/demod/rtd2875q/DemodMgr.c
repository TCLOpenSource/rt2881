...
static void rtk_demod_shutdown(struct platform_device *pdev)
{
	DEMOD_MGR_WARNING(KERN_NOTICE "rtk_demod_shutdown\n");
	if (isDemodConstructors == 1 && isUseDDemod == 1) {
		pDemod_globe->m_pDemod->Suspend(pDemod_globe->m_pDemod);
		pDemod_globe->m_pDemod->SetTvMode(pDemod_globe->m_pDemod, TV_SYS_TYPE_UNKNOWN);
	}
...
static long rtk_demod_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_RTK_DEMOD_CONSTRUCTORS:
		...
		if (copy_from_user((void *)&pDemodData->demod_init_data, (const void __user *)arg, sizeof(RTK_DEMOD_CONSTRUCTORS))) {
			...
			if (pcb_mgr_get_enum_info_byname("DEMOD_I2C", &param) != 0) {
				DEMOD_MGR_WARNING("RTKDEMOD[%d]: Get DEMOD_I2C port fail use default !!!!!!!!!!!!!!!\n", __LINE__);
				param = 4;
			}
		
			DEMOD_MGR_INFO("RTKDEMOD[%d]: set DEMOD_I2C port = %llu\n", __LINE__, param);
			Comm_Constructors(pDemod_globe->m_comm, (unsigned char)param);
