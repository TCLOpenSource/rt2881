...
#endif
#if USE_HW_PREFRM_LOCK
#define BY_HW_PREFRM_LOCK 3
#endif
...
static const s_IsdbtPrefrmPollingNumMapping _gsIsdbtPrefrmPollingNumMappingTable[] = {
	...
	{ISDBT_FFT_MODE_8K,  ISDBT_GUARD_INTERVAL_1_OVER_4,    20},//from 17
	{ISDBT_FFT_MODE_8K,  ISDBT_GUARD_INTERVAL_1_OVER_8,    14},
	{ISDBT_FFT_MODE_8K,  ISDBT_GUARD_INTERVAL_1_OVER_16,   6},//from 14
	{ISDBT_FFT_MODE_8K,  ISDBT_GUARD_INTERVAL_1_OVER_32,   7},//from 13
};
...
int RTK_R_ISDBT_Init(DEMOD* pDemodKernel)
{
	...
	RTK_R_ISDB_DBG("RTK_R_ISDBT_Init\n");
	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_ISDBT_6M, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_ALTOBEAM_ATBM253:
				...
				pRtkrISDB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) ATBM253_OPT_VALUE_ISDBT;
				pRtkrISDB->m_TunerOptRegCnt = sizeof(ATBM253_OPT_VALUE_ISDBT) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			default:
...
int RTK_R_ISDBT_SetTVSys(DEMOD* pDemodKernel, TV_SYSTEM sys)
{
	RTK_R_ISDB* pRtkrISDB;
	pRtkrISDB = (RTK_R_ISDB*)(pDemodKernel->private_data);
	RTK_R_ISDB_DBG("RTK_R_ISDBT::SetTVSys\n");
...
int RTK_R_ISDBT_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO* pSigInfo)
{
	...
	pRtkrISDB = (RTK_R_ISDB*)(pDemodKernel->private_data);
	pDemod = ((RTK_R_ISDBT_DRIVER_DATA*) pRtkrISDB->m_private)->pDemod;
	RTK_R_ISDB_DBG("RTK_R_ISDBT::SetTVSysEx\n");
	pRtkrISDB->m_auto_tune_enable   = 0;
	...
	*/
...
#ifndef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
#ifdef CE_AUTO_MODE
		//RTK_R_ISDBT_CE_Sel(pDemodKernel, &u8CESel);
		u8CurrentCE = ((rtd_inl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg) & 0x10) >> 4);
		if(u8CurrentCE != ISDBT_ATB_CE){
			RTK_R_ISDBT_CE_DefaultSetting(pDemodKernel);
			RTK_R_ISDBT_CE_SettingByCE(pDemodKernel, ISDBT_ATB_CE);
			pDemod->RestartFsm(pDemod);	 // Restart demod FSM.
			RTK_R_ISDB_INFO("\033[1;32;32m""[SetTV_AUTO_MODE]u8CurrentCE = %d, u8CESel=%d(0:ATB/1:RTK)\n""\033[m", u8CurrentCE, ISDBT_ATB_CE);
	...
	pRtkrISDB->m_ce_checking_stime = tv_osal_time() + ISDBT_CE_CHECK_INTERVAL_MS;
...
int RTK_R_ISDBT_SetTSMode(DEMOD* pDemodKernel, TS_PARAM* pParam)
{
	...
	pDemodKernel->Lock(pDemodKernel);
	RTK_R_ISDB_DBG("RTK_R_ISDBT::SetTSMode\n");
	switch (pParam->mode) {
	case PARALLEL_MODE:
...
int RTK_R_ISDBT_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	if (IsCheckLockBy == BY_RSBER_VALID) { //Rsber
		...
		Val |= IsPiSnrOverThd;
#if HW_PREFRM_LOCK
		HwPreframeReg = rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_RO0_reg);
		//ret = DECODE_RET(pDemod->IsInnerLocked(pDemod, &Val));
		RTK_R_ISDB_INFO("[GetLock][HWPRE]HwPreframeReg = 0x%X, Frmlock = %d\n", HwPreframeReg, Val);
		Val = (HwPreframeReg & 0x1) | Val;
#endif
...
#endif
#if USE_HW_PREFRM_LOCK
	if (IsCheckLockBy != BY_HW_PREFRM_LOCK) { //checked by inner/outer lock
#endif
		//add workaround for field CH19 in Brasil
		if (*pLock == DTV_SIGNAL_LOCK) {
			...
			if (snr <= SNR_THD_FOR_FRM_LOCK) {
				OuterLock = (rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg) & 0x7);//[0]: layerA, [1]:layerB, [2]:layerC
...
int RTK_R_ISDBT_GetSignalQuality(DEMOD* pDemodKernel, ENUM_TV_QUAL id, TV_SIG_QUAL* pQuality)
{
	...
	int LockStatus = 0;
	S32BITS IfAgc = -1;
	U32BITS BerNum = RTK_R_ISDBT_BER_DEN_VALUE, BerDen = 1;
	U32BITS PerNum = 1, PerDen = 1;
	...
	case TV_QUAL_SNR :
		...
		RTK_R_ISDB_DBG("[Quality]snr(%d, %d) = %d\n", SnrNum, SnrDen, pQuality->snr);
		break;
	...
	case TV_QUAL_CUSTOMER_LG:
		...
		RTK_R_ISDB_INFO("\033[1;32;32m""ISDBT_layer = %d, BerNum = "PT_U32BITS", BerDen = "PT_U32BITS", PerNum =   "PT_U32BITS"\n""\033[m", MaxSeglayer , BerNum, BerDen, PerNum);
		...
		pQuality->layer_ber.ber_den = BerDen;
		pQuality->ber = BerNum * 10000 / BerDen;
		RTK_R_ISDB_DBG("\033[1;32;32m""Ber = "PT_U32BITS"\n""\033[m", pQuality->ber);
		pQuality->layer_per.per_num = PerNum;
		pQuality->layer_per.per_den = PerDen;
...
#endif
		pQuality->snr = SnrNum * 10000 / SnrDen;
		ret = RTK_R_ISDBT_GetCarrierOffset(pDemodKernel, &pOffset);
#ifndef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
		RTK_R_ISDB_INFO("\033[1;32;32m""SNR: "PT_U32BITS", BER:("PT_U32BITS"), SQI:(%u), AGC:("PT_U32BITS") , packageError:("PT_U32BITS"),  SigValid:(%X)\n""\033[m", pQuality->snr, pQuality->ber, pQuality->quality, pQuality->agc, pQuality->layer_per.per_num, rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg));
#endif
		break;
...
int RTK_R_ISDBT_GetCarrierOffset(DEMOD* pDemodKernel, S32BITS* pOffset)
{
	...
	if (LockStatus == 0) {
		RTK_R_ISDB_DBG("GetCarrierOffset fail beacause demod is unlock\n");
		return TUNER_CTRL_FAIL;
	}
...
int RTK_R_ISDBT_AcquireSignalThread(DEMOD* pDemodKernel)
{
...
#endif
#if HW_PREFRM_LOCK
	unsigned int HwPrefrmReg =0;
	int HwPrefrmLock = 0;
	unsigned int PiSnr = 0;
#endif
#if USE_HW_PREFRM_LOCK
	unsigned int HwPrefrmReg =0;
	int HwPrefrmLock = 0;
	...
	RTK_R_ISDB_INFO("\033[1;32;32m""no_signal = %d, time=%lu\n""\033[m", no_signal, tv_osal_time() - acquire_sig_stime);
	...
	while ((tv_osal_time() - acquire_sig_stime) <= 1400) {
		...
		RTK_R_ISDBT_CE_Sel(pDemodKernel, &u8CESel);
		u8CurrentCE = ((rtd_inl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg) & 0x10) >> 4);
		if(u8CurrentCE != u8CESel){
			RTK_R_ISDBT_CE_DefaultSetting(pDemodKernel);
			RTK_R_ISDBT_CE_SettingByCE(pDemodKernel, u8CESel);
			pDemod->RestartFsm(pDemod);	 // Restart demod FSM.
			RTK_R_ISDB_INFO("\033[1;32;32m""[ACQ_AUTO_MODE]u8CurrentCE = %d, u8CESel=%d(0:ATB/1:RTK)\n""\033[m", u8CurrentCE, u8CESel);
		...
		tv_osal_msleep(5);
	RTK_R_ISDB_DBG("\033[1;32;32m""Preframe = 0x%X, time=%lu\n""\033[m", rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_RO0_reg), tv_osal_time() - acquire_sig_stime);
	RTK_R_ISDB_DBG("\033[1;32;32m""B62C = 0x%X, B628=0x%X, PiSnr =%d, time=%lu\n""\033[m", rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG1_reg), rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG0_reg), ( ( (rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG1_reg)<<8) + rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG0_reg))/4), tv_osal_time() - acquire_sig_stime );


		pDemod->IsInnerLocked(pDemod, &FrmLock);
#if USE_PREFRM_LOCK
		RTK_R_ISDBT_IsPiSnrOverThd(pDemodKernel, &IsPiSnrOverThd);
		RTK_R_ISDB_DBG("\033[1;32;32m""[PRE_API]IsPiSnrOverThd = %d, time=%lu\n""\033[m", IsPiSnrOverThd, tv_osal_time() - acquire_sig_stime);
		if (IsPiSnrOverThd) {
			u8PiSnrOverThdCnt++;
...
#if HW_PREFRM_LOCK
		HwPrefrmReg = rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_RO0_reg);
		if (HwPrefrmReg & 0x1) {
			PiSnr = ( (rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG1_reg)<<8) + rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG0_reg))/4;
			RTK_R_ISDB_DBG("\033[1;32;32m""[HWPRE_API]PiSnr = %d, time=%lu\n""\033[m", PiSnr, tv_osal_time() - acquire_sig_stime);
			if(PiSnr >= 36){	//check prefrm + pilot + data
				RTK_R_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_SNR, &Quality);
				if(Quality.snr >= 300000){
					HwPrefrmLock = 1;
					RTK_R_ISDB_DBG("\033[1;32;32m""[HWPRE_API]HwPrefrmReg= 0x%X, HwPrefram=%d + PiSnr >= 36 + dataSnr>=30, time=%lu\n""\033[m", HwPrefrmReg, HwPrefrmLock, tv_osal_time() - acquire_sig_stime);
				}
			}
			PrefrmLock |= HwPrefrmLock;
		}
#endif
		if (PrefrmLock) {
			lock = PrefrmLock;
		...
		if (HwPrefrmReg & 0x1) {
			PiSnr = ( (rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG1_reg)<<8) + rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_PREFRM_LOCK_DBG0_reg))/4;
			RTK_R_ISDB_DBG("\033[1;32;32m""[HWPRE_API]PiSnr = %d, time=%lu\n""\033[m", PiSnr, tv_osal_time() - acquire_sig_stime);
			if(rtd_inl(0xb8033a20) == 0){	//check prefrm + pilot
				
				if(PiSnr >= 36){
					HwPrefrmLock = 1;
					RTK_R_ISDB_DBG("\033[1;32;32m""[HWPRE_API]HwPrefrmReg= 0x%X, HwPrefram=%d + PiSnr >= 36, time=%lu\n""\033[m", HwPrefrmReg, HwPrefrmLock, tv_osal_time() - acquire_sig_stime);
				}

			}else{							//check prefrm + pilot + data
				if(PiSnr >= 36){
					RTK_R_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_SNR, &Quality);
					if(Quality.snr >= 300000){
						HwPrefrmLock = 1;
...
#if USE_PREFRM_LOCK
	if (PrefrmLock) {
		RTK_R_ISDB_INFO("\033[1;32;31m""[ACQ_O]PrefrmLock = %d, time=%lu, u8PiSnrMaxPollingNum = %d\n""\033[m", PrefrmLock, tv_osal_time() - acquire_sig_stime, u8PiSnrMaxPollingNum);
#if HW_PREFRM_LOCK
		RTK_R_ISDB_INFO("\033[1;32;31m""[ACQ_O]HwPrefrmLock = %d, time=%lu\n""\033[m", HwPrefrmLock, tv_osal_time() - acquire_sig_stime);
#endif
	...
	if (HwPrefrmLock) {
		RTK_R_ISDB_INFO("\033[1;32;31m""[ACQ_O]HwPrefrmLock = %d, time=%lu\n""\033[m", HwPrefrmLock, tv_osal_time() - acquire_sig_stime);
		IsCheckLockBy = BY_HW_PREFRM_LOCK;
	} else {
#endif
		if (snr <= SNR_THD_FOR_FRM_LOCK) {
			RTK_R_ISDB_INFO("\033[1;32;31m""[ACQ_O]signal lock = "PT_U32BITS"/"PT_U32BITS"/"PT_U32BITS", time=%lu\n""\033[m",
							RsberValid[0], RsberValid[1], RsberValid[2],
							tv_osal_time() - acquire_sig_stime);
			IsCheckLockBy = BY_RSBER_VALID;
		} else {
			RTK_R_ISDB_INFO("\033[1;32;31m""[ACQ_O]frm_lock = %d, time=%lu\n""\033[m", FrmLock, tv_osal_time() - acquire_sig_stime);
#if USE_PREFRM_LOCK
			RTK_R_ISDB_INFO("[ACQ_O]u8PiSnrOverThdCnt = %d\n", u8PiSnrOverThdCnt);
#endif
...
#ifndef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
		if ((cur_time > pRtkrISDB->m_ce_checking_stime) || ((pRtkrISDB->m_ce_checking_stime - cur_time) > ISDBT_CE_CHECK_INTERVAL_MS)) {
			RTK_R_ISDBT_CE_Sel(pDemodKernel, &u8CESel);
#ifdef CE_AUTO_MODE
			u8CurrentCE = ((rtd_inl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg) & 0x10) >> 4);
			if(u8CurrentCE != u8CESel){
				...
				RTK_R_ISDBT_CE_SettingByCE(pDemodKernel, u8CESel);
				pDemod->RestartFsm(pDemod);	 // Restart demod FSM.
				RTK_R_ISDB_INFO("\033[1;32;32m""[KernelAutoTune_AUTO_MODE]u8CurrentCE = %d, u8CESel=%d(0:ATB/1:RTK)\n""\033[m", u8CurrentCE, u8CESel);

			}
#endif
			RTK_R_ISDB_INFO("[KernelAutoTune] CESel = %d(0:ATB, 1:RTK)\n", u8CESel);
			pRtkrISDB->m_ce_checking_stime = tv_osal_time() + ISDBT_CE_CHECK_INTERVAL_MS;
		}
#endif
	}
...
int RTK_R_ISDBT_SetTvMode(DEMOD* pDemodKernel, TV_SYSTEM_TYPE mode)
{
	...
	RTK_R_ISDB_DBG("RTK_R_ISDBT::SetTvMode(%d)\n", mode);
...
void ReleaseRtkrDriver(RTK_R_ISDBT_DRIVER_DATA *pDriver)
{
	if (pDriver->pShareMemory) {
		...
		if(rtd_inl(TVSB5_2_TV_SB5_2_DCU1_ARB_CR1_reg)&0x01){
...
RTK_R_ISDBT_DRIVER_DATA* AllocRtkrDriver_ISDBT(COMM* pComm,	unsigned char Addr, U32BITS CrystalFreq, int TSMode)
{
	...
	if (pDriver) {
		...
		if (RTK_R_ISDBTT_DEMUX_DDR_SIZE) {
			...
			pDriver->pShareMemory = &pDriver->ShareMemoryPhysicalAddress;
...
{
	int ret = TUNER_CTRL_OK;
	unsigned int Amod = 0xFF, Bmod = 0xFF, Cmod = 0xFF, partial =0;
	unsigned char Aseg = 0xFF, Bseg = 0xFF, Cseg = 0xFF;
	RTK_R_ISDB* pRtkrISDB;
	ISDBT_DEMOD_MODULE* pDemod;
	pRtkrISDB = (RTK_R_ISDB*)(pDemodKernel->private_data);
	pDemod = ((RTK_R_ISDBT_DRIVER_DATA*) pRtkrISDB->m_private)->pDemod;
	...
	ret |= DECODE_RET(pDemod->GetLayerSegmentNum(pDemod, ISDBT_HIERARCHICAL_LAYER_A, &Aseg));
	ret |= DECODE_RET(pDemod->GetLayerSegmentNum(pDemod, ISDBT_HIERARCHICAL_LAYER_B, &Bseg));
	ret |= DECODE_RET(pDemod->GetLayerSegmentNum(pDemod, ISDBT_HIERARCHICAL_LAYER_C, &Cseg));
	if (ret != TUNER_CTRL_OK)
		goto READ_TMCC_FAIL;
	if ((Aseg == 0) && (Bseg == 0) && (Cseg == 0))// TMCC not ready
		goto READ_TMCC_FAIL;
	ret |= DECODE_RET(pDemod->GetLayerConstellation(pDemod, ISDBT_HIERARCHICAL_LAYER_A, &Amod));
	ret |= DECODE_RET(pDemod->GetLayerConstellation(pDemod, ISDBT_HIERARCHICAL_LAYER_B, &Bmod));
	ret |= DECODE_RET(pDemod->GetLayerConstellation(pDemod, ISDBT_HIERARCHICAL_LAYER_C, &Cmod));
	partial = (rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_DBG_DATA206_reg)& 0x10) >> 4;

	if(Aseg == 13){ // 1 layer
		if(Amod != 0) *u8CESel = ISDBT_ATB_CE;
		else *u8CESel = ISDBT_RTK_CE;
	} else if((Aseg + Bseg) == 13){ // 2 layers
	    if((Amod != 0) && (Bmod != 0)){
			if(partial) *u8CESel = ISDBT_ATB_CE;
			else{
				if(Amod == Bmod) *u8CESel = ISDBT_ATB_CE;
				else *u8CESel = ISDBT_RTK_CE;
			}
		}else *u8CESel = ISDBT_RTK_CE;
	} else { // 3 layers
		if((Amod!=0) && (Bmod != 0) && (Cmod !=0)){
			if(partial){
				if(Bmod == Cmod) *u8CESel = ISDBT_ATB_CE;
				else *u8CESel = ISDBT_RTK_CE;
			} else {
			    if((Amod == Bmod) && (Bmod == Cmod)) *u8CESel = ISDBT_ATB_CE;
				else *u8CESel = ISDBT_RTK_CE;
			}
		} else *u8CESel = ISDBT_RTK_CE;
...
void RTK_R_ISDBT_CE_DefaultSetting(DEMOD* pDemodKernel)
{
	...
	pDemodKernel->Lock(pDemodKernel);
	rtd_outl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg,	0x01);	//[5]:sel_rtk_atb_ce_fix;[4]:sel_atb_rtk_ce_fixd;[0]:sel_atb_rtk_ce_auto
	//ATB use
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C1_CONVERGE_48K_reg,	0x10);	//[7:0]:params_dpll_c1_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C2_CONVERGE_48K_reg,	0x20);	//[7:0]:params_dpll_c2_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C1_CONVERGE_48K_reg,	0x20);	//[7:0]:params_cfo_dpll_c1_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C2_CONVERGE_48K_reg,	0x80);	//[7:0]:params_dpll_c2_converge_48k
	rtd_outl(ATB_DEMOD_TOP_GPO7_REG_reg,	0x0	);  //option2: 0x181600dc=>0x20(frm lock delay 32EQ work)
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK1_reg,	0x90);	//forcast snr from 0x90
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK2_reg,	0x90);	//post snr from 0x90
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK3_reg,	0x78);	//data snr from 0x78
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK4_reg,	0x00);	
	rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_2K_DVBT_reg,	0x00);	//PXP/FPGA alignment: for ATB CE
	rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_8K_DVBT_reg,	0x00);	//PXP/FPGA alignment: for ATB CE
	rtd_outl(ATB_DEMOD_CE_T_ISDBT_CE_4K_15_reg,	0x01);	//PXP/FPGA alignment: for ATB CE
	rtd_outl(ATB_DEMOD_CE_T_ISDBT_NOISE_SEARCH_ALPHA_reg,	0xE1);	////[0]:params_noise_search_alpha_filter_en_isdbt;[7:4]:params_noise_search_alpha_isdbt;noise_search_alpha_filter_en_isdbt (Add by Sherlock @20220826)
	//RTK use
	rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT5_reg,	0x0	);  //0x0: shift FFT start by the ratio of GI size, 0x1: shift FFT start by index.
	rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT2_reg,	0x0	);  //the ratio of GI size for shifting FFT start(full-seg)
	rtd_outl(ATB_DEMOD_SYNC2_DAGC_FAST_CHG_EN_reg,	0x1	);  //[0]:0:disable fast dagc when DVBT mode for RTK CE
	rtd_outl(ATB_DEMOD_LDPC_LDPC_TEST_MODE_7_0_reg,	0x0	);  //eco option for RTK CE
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_15_8_reg,	0x0	);  //(4096) continuous error reset threshold [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_7_0_reg,	0xff);	//(4096) continuous error reset threshold [7:0]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_15_8_reg,	0x00);	//(4096) all error reset threshold [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_7_0_reg,	0x20);	//(4096) all error reset threshold [7:0]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_15_8_reg,	0x0	);  //(4096) all error reset window [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_7_0_reg,	0x20);	//(4096) all error reset window [7:0]
	pDemodKernel->Unlock(pDemodKernel);
...
void RTK_R_ISDBT_CE_SettingByCE(DEMOD* pDemodKernel, unsigned char u8CESel)
{
	...
	pDemodKernel->Lock(pDemodKernel);
	if(u8CESel == ISDBT_ATB_CE){
		RTK_R_ISDB_INFO("\033[1;32;32m""[AUTO_MODE]u8CESel=%d(0:ATB/1:RTK), Switch to ATB\n""\033[m", u8CESel);

		rtd_outl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg,	0x00);	//[5]:sel_rtk_atb_ce_fix;[4]:sel_atb_rtk_ce_fixd;[0]:sel_atb_rtk_ce_auto
		//ATB use
		rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C1_CONVERGE_48K_reg,	0x08);	//[7:0]:params_dpll_c1_converge_48k
		rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C2_CONVERGE_48K_reg,	0x10);	//[7:0]:params_dpll_c2_converge_48k
		rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C1_CONVERGE_48K_reg,	0x10);	//[7:0]:params_cfo_dpll_c1_converge_48k
		rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C2_CONVERGE_48K_reg,	0x40);	//[7:0]:params_dpll_c2_converge_48k
		rtd_outl(ATB_DEMOD_TOP_GPO7_REG_reg,	0x20);  //option2: 0x181600dc=>0x20(frm lock delay 32EQ work)
		rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK1_reg,	0x00);	//forcast snr from 0x90
		rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK2_reg,	0x00);	//post snr from 0x90
		rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK3_reg,	0x1E);	//data snr from 0x78
		rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK4_reg,	0x00);
		rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_2K_DVBT_reg,	0x05);	//PXP/FPGA alignment: for ATB CE
		rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_8K_DVBT_reg,	0x05);	//PXP/FPGA alignment: for ATB CE
		rtd_outl(ATB_DEMOD_CE_T_ISDBT_CE_4K_15_reg,	0x05);	//PXP/FPGA alignment: for ATB CE
		...
		RTK_R_ISDB_INFO("\033[1;32;32m""[AUTO_MODE]u8CESel=%d(0:ATB/1:RTK), Switch to RTK\n""\033[m", u8CESel);
		rtd_outl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg,	0x10);	//[5]:sel_rtk_atb_ce_fix;[4]:sel_atb_rtk_ce_fixd;[0]:sel_atb_rtk_ce_auto
	//RTK use
		rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT5_reg,	0x0	);  //0x0: shift FFT start by the ratio of GI size, 0x1: shift FFT start by index.
		rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT2_reg,	0x0C);  //the ratio of GI size for shifting FFT start(full-seg)
		rtd_outl(ATB_DEMOD_SYNC2_DAGC_FAST_CHG_EN_reg,	0x00);  //[0]:0:disable fast dagc when DVBT mode for RTK CE
		rtd_outl(ATB_DEMOD_LDPC_LDPC_TEST_MODE_7_0_reg,	0x10);  //eco option for RTK CE
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_15_8_reg,	0x10);  //(4096) continuous error reset threshold [15:8]
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_7_0_reg,	0x00);	//(4096) continuous error reset threshold [7:0]
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_15_8_reg,	0x10);	//(4096) all error reset threshold [15:8]
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_7_0_reg,	0x00);	//(4096) all error reset threshold [7:0]
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_15_8_reg,	0x10);  //(4096) all error reset window [15:8]
		rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_7_0_reg,	0x00);	//(4096) all error reset window [7:0]
	}
	pDemodKernel->Unlock(pDemodKernel);
