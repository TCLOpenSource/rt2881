...
int DtvDemodEnable(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			RTK_DEMOD_INFO("DtvDemodEnable to DTMB_H\n");
		} else {
			CRT_CLK_OnOff(DTVRTK, CLK_ON, NULL);
			RTK_DEMOD_INFO("DtvDemodEnable to DTMB_R\n");
		}
		break;
...
void DemodAdcCapcalSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DVBT:
	case RTK_DEMOD_MODE_DVBT2:
		RTK_DEMOD_INFO("IADC_CAPCAL=0x6\n");
	...
	case RTK_DEMOD_MODE_QAM:
	case RTK_DEMOD_MODE_DVBC:
		RTK_DEMOD_INFO("IADC_CAPCAL=0x5\n");
		rtd_maskl(0xb803311c, 0xFFFFF000, 0x00000005);					//QADC_CAPCAL<2:0>=101 / IADC_CAPCAL<2:0>=101 for IDNL measurement
		break;
	case RTK_DEMOD_MODE_DVBS:
	case RTK_DEMOD_MODE_DVBS2:
	case RTK_DEMOD_MODE_DVBS2X:
		RTK_DEMOD_INFO("IADC_CAPCAL=0x5, QADC_CAPCAL=0x5\n");
		rtd_maskl(0xb803311c, 0xF000F000, 0x00050005);					//QADC_CAPCAL<2:0>=101 / IADC_CAPCAL<2:0>=101 for IDNL measurement
		break;
	default:
		RTK_DEMOD_INFO("IADC_CAPCAL=0x4, QADC_CAPCAL=0x4\n");
		rtd_maskl(0xb803311c, 0xF000F000, 0x00040004);					//QADC_CAPCAL<2:0>=100/ IADC_CAPCAL<2:0>=100(Default value) for IDNL measurement
		break;
	}
}
...
void DemodAdcSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		//IQADC+IQPGA power on
		rtd_maskl(0xb8033100, 0xfffff3ff, 0x00000800); //default setting, <bit11:10>REG_I_VCM_SEL=0x1
		rtd_maskl(0xb803312c, 0xfffff3ff, 0x00000800); //default setting, <bit11:10>REG_Q_VCM_SEL=0x1
		rtd_maskl(0xb8033130, 0x33ffffff, 0xcc000000);
		rtd_maskl(0xb8033134, 0xccffff88, 0x22000055);
		//Set IPGA/QPGA GAIN = 0dB
		rtd_maskl(0xb8033120, 0xfffffffe, 0x00000001); //IF ADC GAIN FIX EN <bit0>= 1(Internal Demod IQ mode)
	...
	default:
		...
		//Set IPGA/QPGA GAIN = 0dB
		rtd_maskl(0xb8033120, 0xfffffffe, 0x00000001); //IF ADC GAIN FIX EN <bit0>= 1(Internal Demod IQ mode)
		rtd_maskl(0xb803312c, 0xfffefcf0, 0x00000000);
		//<bit7:4>reg_qpga_d=0x0(0dB), <bit3:0>reg_ipga_d=0x0(0dB)
		//rtd_maskl(0xb80004b8, 0xfffffdff, 0x00000000); //<bit9>IFADC ATV clk enable turn off
		//rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //<bit4>PLL27X_IFADC_DTV_CKO_EN setup
		//rtd_maskl(0xb80004b8, 0xfffffdff, 0x00000200); //<bit9>IFADC ATV clk enable turn on
		//ScalerTimer_DelayXms(1);									//driver delay 50uS
		//rtd_maskl(0xb803311c, 0xfffff000, 0x00000006);
...
int QVcmSetting(void)
{
	...
	if (q_vcm_sel_ft >= 0)
		...
		rtd_maskl(TV_SB1_ANA_IQADC_CTRL5_reg, ~TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel_mask, TV_SB1_ANA_IQADC_CTRL5_reg_q_vcm_sel(0x2));
...
void DemodPllSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			...
			rtd_maskl(0xb80004dc, 0xFFFFFFFE, 0x00000000);				//PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
			...
			RTK_DEMOD_INFO("DemodPllSetting Pll Change to DTMB_H \n");
		} else {
			//ADC CLK and System CLK setting(ADC=144M, Sys=384M)
			rtd_maskl(0xb8000214, 0xfffdb7ff, 0x00004000);
			rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
			rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
			//VCO = M<6:0>+2 = 30+2, N:00=1, 27 /1 *32=864MHz, PLL27X_IFADC_M<bit22:16> = 0x1E(30), PLL27X_IFADC_N<bit25:24>=0x0(1),PUMP current<bit2:0>=0x3(011:30uF)#C1
			// <9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x3(011:30uF)#C1
			rtd_maskl(0xb8000504, 0xfc80fc08, 0x001e0143);
			rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003200); //<15:8>PLL_RESERVE=00110000
			//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 1001 = 0x9(/6), Adc_clk = 864 / 6=144MHz#C1
			//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0110 = 0x6(/2.25), sys_clk = 864 / 2.25 = 384MHz#C1
			rtd_maskl(0xb80004c4, 0xffffff00, 0x00000069);
			//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x0(div2), <bit13>DIF432M_EN=0x0, <bit12>DTVDIG_EN=0x1, <bit10>LDPC_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
			rtd_maskl(0xb80004b8, 0xfffd8b3b, 0x00021000); //PLL27X_IFADC_REF_SEL
			RTK_DEMOD_INFO("DemodPllSetting Pll Change to DTMB_R \n");
		}
		break;
	...
	case RTK_DEMOD_MODE_ATSC:
		...
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
		//VCO = M<6:0>+2 = 14+2, N:00=1, 27 /1 *16=432MHz, PLL27X_IFADC_M<bit22:16> = 0xE(14), PLL27X_IFADC_N<bit25:24>=0x0(1),#C1
		//<9:6>PLL27X_IFADC_LF_RS=0101, <5:4>PLL27X_IFADC_LF_CP=0x0(0.5pF), <2:0>PLL27X_IFADC_IP=0x5(101:15uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x000e0145);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0000 = 0x0(/8), Adc_clk = 432 / 8=54MHz#C1
		//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 1010 = 0xA(/1), sys_clk = 432 / 1 = 432MHz#C1
	...
	case RTK_DEMOD_MODE_QAM:
		...
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
		//VCO = M<6:0>+2 = 26+2, N:00=1, 27 /1 *28=756MHz, PLL27X_IFADC_M<bit22:16> = 0x1A(26), PLL27X_IFADC_N<bit25:24>=0x0(1), #C1
		//<9:6>PLL27X_IFADC_LF_RS=0x8, <5:4>PLL27X_IFADC_LF_CP=0x0(0.5pF), <2:0>PLL27X_IFADC_IP=0x4(101:15uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x001a0204);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 1000 = 0x8(/30), Adc_clk = 756 / 30=25.2MHz#C1
		//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0001 = 0x1(/2.5), sys_clk = 756 / 2.5 = 302.4MHz#C1
	...
	case RTK_DEMOD_MODE_DVBC:
		...
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
		//VCO = M<6:0>+2 = 18+2, N:00=1, 27 /1 *20=540MHz, PLL27X_IFADC_M<bit22:16> = 0x12(18), PLL27X_IFADC_N<bit25:24>=0x0(1), #C1
		//<9:6>PLL27X_IFADC_LF_RS=0101, <5:4>PLL27X_IFADC_LF_CP=0x0(0.5pF), <2:0>PLL27X_IFADC_IP=0x5(101:15uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x00120145);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0001 = 0x1(/15), Adc_clk = 540 / 15=36MHz#C1
		//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0011 = 0x3(/1.25), sys_clk = 864 / 1.25 = 432MHz#C1
	...
	case RTK_DEMOD_MODE_ISDB:
		...
		rtd_maskl(0xb8000214, 0xfffdb7ff, 0x00020000);
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
		//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1)#C1
		//<9:6>PLL27X_IFADC_LF_RS=0101, PLL27X_IFADC PUMP current<bit2:0>=0x5(101:15uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x00160145);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x7(/4.5)(33/67)), Adc_clk = 648 / 4.5 =144MHz#C1
		//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x3(/1.25)(40/60), sys_clk = 648 / 1.25 = 518.4 MHz#C1
		rtd_maskl(0xb80004c4, 0xffffff00, 0x00000037);
		//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x0(div2), <bit13>DIF432M_EN=0x0, <bit12>DTVDIG_EN=0x1, <bit10>LDPC_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
		rtd_maskl(0xb80004b8, 0xfffd8b3b, 0x00027440); //PLL27X_IFADC_REF_SEL
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		...
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000000); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 0(DTV)
		rtd_maskl(0xb8000500, 0xffffffef, 0x00000010); //PLL27x_IFADC DTV CLK<bit4>=0x1
		//VCO = M<6:0>+2 = 25+2, N:00=1, 27 /1 *27=729MHz, PLL27X_IFADC_M<bit22:16> = 0x19(25), PLL27X_IFADC_N<bit25:24>=0x0(1)#C1
		//<9:6>PLL27X_IFADC_LF_RS=0111, PLL27X_IFADC PUMP current<bit2:0>=0x4(100:12.5uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x001901c4);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//ADC_CLK : PLLDIF_DIVADC_SEL<3:0> = 0x9(/6)(33/67)), Adc_clk = 729 / 6 =121.5MHz#C1
		//System_CLK : PLLDIF_DIVDIG_SEL<bit7:4> = 0x4(/1.5)(50/50), sys_clk = 729 / 1.5 = 486 MHz#C1
		rtd_maskl(0xb80004c4, 0xffffff00, 0x00000049);
		//<bit17>ATVADCDIV_RSTB=0x1(normal), <bit14>DIF432M_DIV=0x0(div2), <bit13>DIF432M_EN=0x0, <bit12>DTVDIG_EN=0x1, <bit10>LDPC_EN=0x0, <7:6>LDPC_DIV=0x0(div1.75), <bit2>PLL27X_IFADC_REF_SEL=0(XTAL)
		rtd_maskl(0xb80004b8, 0xfffd8b3b, 0x000214c0); //PLL27X_IFADC_REF_SEL
		RTK_DEMOD_INFO("DemodPllSetting to DVBSx\n");
		break;

	case RTK_DEMOD_MODE_ATV:
		//PLL setting for ATV System CLK=108MHz, VCO=432MHz
		...
		rtd_maskl(0xb80004dc, 0xfffffffe, 0x00000001); //PLL_CK_IFADC_MUX to DTV Mode, <bit0> = 1(ATV)		
		...
		//rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		//rtd_maskl(0xb80004d4, 0xffffffcf, 0x00000010); //[18]PLL_XIXO_LOAD=0, [17:16]PLL_XIXO_DRIVE_SEL=0x3,[5:4]PLL27X_PS_54M_DELAY=0x2(VCO=432M)
		...
		//ATV ADC CLK=162MHz, VCO=648MHz#C1
		//[11:10]PLL27X_IFADC_CK108M_DIV=0x3(VCO=648M),[4]PLL27X_IFADC_DTV_CKO_EN=0x1(enable),[3:2]PLL27X_IFADC_ATV_CKO_SEL=0x11(162M)#C1
		rtd_maskl(0xb8000500, 0xfffff3e3, 0x00000c1c);
		//VCO = M<6:0>+2 = 22+2, N:00=1, 27 /1 *24=648MHz, PLL27X_IFADC_M<bit22:16> = 0x16(22), PLL27X_IFADC_N<bit25:24>=0x0(1), <9:6>PLL27X_IFADC_LF_RS=0101, <bit2:0>PLL27X_IFADC_IP=0x5(101:15uF)#C1
		rtd_maskl(0xb8000504, 0xfc80fc08, 0x00160145);
		rtd_maskl(0xb80004d8, 0xffff00ff, 0x00003000); //<15:8>PLL_RESERVE=00110000
		rtd_maskl(0xb80004c4, 0xfffffff0, 0x00000004);
...
void DtvDemodFrontendFilter(RTK_DEMOD_MODE mode)
{
	...
	if (mode == RTK_DEMOD_MODE_DTMB) {
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			RTK_DEMOD_INFO("Frontend Filter Change : load DTMB_H !\n");
			...
			RTK_DEMOD_INFO("Frontend Filter Change : load DTMB_R !\n");
...
int DemodFrontendPathSetting(RTK_DEMOD_MODE mode)
{
	...
	case RTK_DEMOD_MODE_DTMB:
		if (get_mach_type() == MACH_ARCH_RTK2885P)
			rtd_maskl(0xb8033800, 0xffd8feed, 0x00010010);							//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid), <bit18:16>Dtv_frontend_mode=0x1, <bit8>dtv_idata_sel=0x0(from dn flt), <bit4>Dtv_dnflt_en=0x1(DN6)
		else
			rtd_maskl(0xb8033800, 0xffd8feef, 0x00050010);			//Dtv_dnflt_en <bit4> = 1, dtv_idata_sel<bit8> = 0(from gain adjust), Dtv_frontend_mode <bit17:16> = 0x5#C1
	...
	case RTK_DEMOD_MODE_ATSC:
		rtd_maskl(0xb8033800, 0xffd8feed, 0x00040000);                               //Dtv_dnflt_en <bit4> = 0(bypass), dtv_idata_sel<bit8> = 0(from gain adjust), Dtv_frontend_mode <bit17:16> = 0x4#C1
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000854, 0xFFFFFF0F, 0x00000040);               //GPIO_61, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_QAM:
		rtd_maskl(0xb8033800, 0xffd8feed, 0x00240000);					//<bit29:28>dtmb_data_valid_delay=0x1(delay data valid),<bit21>rtk_dtv_demod_sel=0x1(OpenCable), <bit18:16>Dtv_frontend_mode=0x4, <bit8>dtv_idata_sel, <bit4>Dtv_dnflt_en for RTK Demod don't care
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00040400);				//input fifo clr, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x1, FIFO waddr & raddr clear
		rtd_maskl(0xb8033804, 0xFFFBFBFF, 0x00000000);				//input fifo clr release, dtv_input_fifo_clr<bit18>, iq_adc_fifo<bit10> = 0x0, FIFO waddr & raddr clear
		rtd_maskl(0xb8000854, 0xFFFFFF0F, 0x00000040);               //GPIO_61, Set pinmux DEMOD_IF_AGC
	...
	case RTK_DEMOD_MODE_SPEC_AN:
		if (TunerInputMode <= 1) {
			rtd_maskl(0xb8033800, 0xffd8feed, 0x00020100);       //IQ Normal for Tuner Output
			RTK_DEMOD_INFO("DVBS spectrum normal\n");
		} else {
			rtd_maskl(0xb8033800, 0xffd8feed, 0x00020102);       //IQ Inversion for Tuner Output
