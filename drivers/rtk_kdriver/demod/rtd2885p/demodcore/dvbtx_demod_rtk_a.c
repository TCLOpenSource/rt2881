...
)
{
	...
	//for KTASKWBS-27879 DVBT2 6M
	rtd_maskl(ATB_DEMOD_LDPC_LDPC_CFG_EN_0_reg, ~ATB_DEMOD_LDPC_LDPC_CFG_EN_0_ldpc_cfg_en_0_mask, ATB_DEMOD_LDPC_LDPC_CFG_EN_0_ldpc_cfg_en_0(0x0));	//Set ldpc_cfg_en = 0
	rtd_maskl(ATB_DEMOD_LDPC_LDPC_CFG_ADDR_7_0_reg, ~ATB_DEMOD_LDPC_LDPC_CFG_ADDR_7_0_ldpc_cfg_addr_7_0_mask, ATB_DEMOD_LDPC_LDPC_CFG_ADDR_7_0_ldpc_cfg_addr_7_0(0x2F));	//Set ldpc_cfg_adr = 47
	rtd_maskl(ATB_DEMOD_LDPC_LDPC_CFG_DAT_7_0_reg, ~ATB_DEMOD_LDPC_LDPC_CFG_DAT_7_0_ldpc_cfg_dat_7_0_mask, ATB_DEMOD_LDPC_LDPC_CFG_DAT_7_0_ldpc_cfg_dat_7_0(0x1));	//Set ldpc_cfg_dat = 1
	rtd_maskl(ATB_DEMOD_LDPC_LDPC_CFG_EN_0_reg, ~ATB_DEMOD_LDPC_LDPC_CFG_EN_0_ldpc_cfg_en_0_mask, ATB_DEMOD_LDPC_LDPC_CFG_EN_0_ldpc_cfg_en_0(0x1));	//Set ldpc_cfg_en = 1
	...
	rtd_maskl(ATB_DEMOD_SYNC_DAGV_SCALE_CHECK0_reg, ~ATB_DEMOD_SYNC_DAGV_SCALE_CHECK0_dagc_scale_check_en_mask, ATB_DEMOD_SYNC_DAGV_SCALE_CHECK0_dagc_scale_check_en(0x0));//disable dagc_scale_check_en
...

{
	...
	*pAnswer  = 0; /*Assuming not locked*/
	...
	if (get_mach_type() == MACH_ARCH_RTK2885P) {
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x01, 0x0);
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x02, 0x0);
	}
#endif
...
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P) {
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x01, 0x01);
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x02, 0xE0);
	}
#endif
...
#if IsML8Chip
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x15, 0x0);
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x16, 0x0);
		}	
#endif
...
#if IsML8Chip
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x15, 0x8A);
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x16, 0x0);
		}	
#endif
		ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x01, 0x0d, 0);    //RTK_A_DVBTX_LatchOff();
...
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P)
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x04, 0x0);
#endif
	status |= RTK_A_DVBTX_Read(pDemod, 0x0B, 0x04, &u8Ext);
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P)
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x04, 0xC0);
#endif
...
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P)
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x13, 0x0);
#endif
	status |= RTK_A_DVBTX_Read(pDemod, 0x0B, 0x13,  &u8PilotPattern);
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P)
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x13, 0xD9);
#endif
...
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P) {
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x15, 0x0);
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x16, 0x0);
	}	
#endif
...
#if IsML8Chip
	if (get_mach_type() == MACH_ARCH_RTK2885P) {
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x15, 0x8A);
		status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x16, 0x0);
	}
#endif
...
*********************************************************************************/
int realtek_a_dvbtx_CheckT2TxIDErrSignal(DVBTX_DEMOD_MODULE *pDemod, unsigned char *pProcessEnFlag)
{
	RTK_A_DVBTX_STATUS ui8Status = RTK_A_DVBTX_SUCCESSFUL;
	unsigned char u8DVBTxMode;
	unsigned char u8L1PreCRCRight = 0, u8L1PostCRCRight = 0;
...
#if IsML8Chip
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x01, 0x0);
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x02, 0x0);
		}
#endif
...
#if IsML8Chip
		if (get_mach_type() == MACH_ARCH_RTK2885P) {
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x01, 0x01);
			ui8Status |= RTK_A_DVBTX_Write(pDemod, 0x0b, 0x02, 0xE0);
		}
#endif
		...
		if (((u8L1PreCRCRight & 0x01) != 0x01) && ((u8L1PostCRCRight & 0x01) != 0x01)) { /* Check L1pre error and L1post CRC right*/
			if (u8L1set != 0x01) {
				...
				for (u8Iter = 0; u8Iter < 16; u8Iter++) {
					...
					if (*pProcessEnFlag == 0) {
						RTK_DEMOD_DVBTX_INFO("\033[1;32;32m""force CheckT2TxIDErrSignal stop by user !! ProcessEnFlag = %u" "\033[1;32;32m", *pProcessEnFlag);
						break;
					}
				}
