...
void drvif_ifd_set_scanchannel(StructATVModule *pAtvModule)
{
	...
	CH_mode = 1;		// scan_mode
	IFD_DEBUG_MSG(IFD_MSG_INFO, "drvif_ifd_set_scanchannel(0x%x, 0x%x)\n",
...
void drvif_ifd_set_setchannel(StructATVModule *pAtvModule)
{
	...
	IFD_DEBUG_MSG(IFD_MSG_INFO, "drvif_ifd_set_setchannel(0x%x, 0x%x, %d)\n",
				  (UINT32) pAtvModule->video_mode,
				  (UINT32) pAtvModule->audio_mode,
				  (UINT32) pAtvModule->freq);
	IFD_DEBUG_MSG(IFD_MSG_NONE, "!!!DISPLAY_MODE_FRC!!!\n");
...
{
	UINT32 curTunerIFVal = 0;
	IFD_DEBUG_MSG(IFD_MSG_INFO, "drvif_ifd_group_delay_mode(0x%x)\n", (UINT32) mode);
	switch (mode) {
