...
#include "rbus/atb_demod_sync2_reg.h"
#include "rbus/dtv_demod_misc_reg.h"
#include "rbus/atb_demod_mac_reg.h"
#include "rbus/atb_demod_ce_t_reg.h"
#include "rbus/atb_demod_ce_t2_reg.h"
#include "rbus/atb_demod_top_reg.h"
#include "rbus/atb_demod_ldpc_reg.h"
#include "rbus/atb_demod_ce_wagtail_reg.h"
#include "rbus/atb_demod_spi_reg.h"
#include "rbus/atb_demod_dvbc_fec_reg.h"
#include "rbus/atb_demod_self_bal_reg.h"
#include "rbus/atb_demod_i2cslv_reg.h"
#include "rbus/atb_demod_agc_reg.h"
...
/*
	...
	if (pDemod->SetRegBits(pDemod, ISDBT_SOFT_RST_N, 0x1) != FUNCTION_SUCCESS)
		goto error_status;
*/
	rtd_maskl(RTK_DTV_DEMOD_ISDBT_RESET_reg, 0xfffffffe, 0x1);			//[0]:soft_rst_n = 1
	rtd_outl(ATB_DEMOD_TOP_SOFTCFG_DONE_R_reg, 0x00000001);					//[0]:softcfg_done_r
	rtd_outl(ATB_DEMOD_TOP_SOFTRST_RXDSP_R_reg, 0x00000001);					//[0]:softrst_rxdsp_r
	rtd_outl(ATB_DEMOD_TOP_SOFTRST_RXDSP_R_reg, 0x00000000);					//[0]:softrst_rxdsp_r
...
//#ifdef USE_ATB_CE
	...
	RTK_DEMOD_ISDBT_INFO("[RTK_R_ISDBT] ATB CE setting \n");

	//Merlin8 ATB CE setting
	//Merlin8 reuse ATB CE setting from sky
	rtd_maskl(DTV_DEMOD_MISC_DTV_DEMOD_SEL_reg, 0xfffffffe, 0x00000001);
	rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_cmd_limit_reg, 0xdfffe000, 0x00000000);// from 0x20000000 to 0x0; IC need to transfer to signed
	rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_t2_dit_addr_0_reg, 0xf0ffffff, 0x01000000);
	rtd_outl(ATB_DEMOD_TOP_EN_ISDBT_MODE_reg, 0x00000001);
	rtd_outl(ATB_DEMOD_SYNC_DVBT_SFI_THR_reg, 0x00000019);					//[4:0]:dvbt_sfi_thr;SFI thd in sync from 0xa
	rtd_outl(ATB_DEMOD_RXFLT_RATE_RATIO_31_24_reg, 0x00000017);					//[7:0]:rate_ratio[31:24],params_rate_ratio
	rtd_outl(ATB_DEMOD_RXFLT_RATE_RATIO_23_16_reg, 0x000000a0);					//[7:0]:rate_ratio[23:16],params_rate_ratio
	rtd_outl(ATB_DEMOD_MAC_ISDBT_MAC_RST_ENB_reg, 0x000000ff);					//for continus/all error reset
	rtd_outl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg, 0x00000000);					//[5]:sel_rtk_atb_ce_fix;[4]:sel_atb_rtk_ce_fixd;[0]:sel_atb_rtk_ce_auto,fix ATB CE
	rtd_outl(ATB_DEMOD_CE_T_SYMIDX_MOD4_ERROR_reg, 0x00000009);					//[0]:params_sym_idx_mod4_renew_en_isdbt;[3]:params_dvbt_fft_win_adj_ro_sel
	rtd_outl(ATB_DEMOD_SYNC_CFOINT_IQSWAP_DET_METHOD_reg, 0x00000001);					//[0]:cfoint_iqswap_det_method
	rtd_outl(RTK_DTV_DEMOD_ISDBT_DT_CESM_PHASE_ROT_ON_reg, 0x00000000);					//[7:0]:dt_cesm_phase_rot_on
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK0_reg, 0x00000011);					//[4]:post, [0]:preframe lock forecast enable
	...
	rtd_outl(ATB_DEMOD_RXFLT2_PARAMS_THRESHOLD_POWER_COMPARE_7_0_reg, 0x000000fc);					//params_threshold_power_compare_7_0
	rtd_outl(ATB_DEMOD_RXFLT_PARAMS_INTERF_THRESHOLD_reg, 0x00000024);					//params_interf_threshold
	rtd_outl(ATB_DEMOD_RXFLT_PARAMS_BW_MODE_reg, 0x00000002);					//BW_MODE=6M
	rtd_outl(ATB_DEMOD_SYNC2_CFO_MAX_DVBT_reg, 0x000000af);					//[7:0]:cfo_max_dvbt
	...
	//ATB CE
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C1_CONVERGE_48K_reg, 0x00000008);					//[7:0]:params_dpll_c1_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_DPLL_C2_CONVERGE_48K_reg, 0x00000010);					//[7:0]:params_dpll_c2_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C1_CONVERGE_48K_reg, 0x00000010);					//[7:0]:params_cfo_dpll_c1_converge_48k
	rtd_outl(ATB_DEMOD_CE_T2_PARAMS_CFO_DPLL_C2_CONVERGE_48K_reg, 0x00000040);					//[7:0]:params_cfo_dpll_c2_converge_48k
	//rtd_outl(0xB81600dc, 0x10);//option1: 0x181600dc=> 0x10(prefrm lock delay32EQ work)
	rtd_outl(ATB_DEMOD_TOP_GPO7_REG_reg, 0x20);//option2: 0x181600dc=>0x20(frm lock delay 32EQ work)

	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK1_reg, 0x00000000);					//forcast snr from 0x90
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK2_reg, 0x00000000);					//post snr from 0x90
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK3_reg, 0x0000001E);					//data snr from 0x78
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK4_reg, 0x00000000);					//data snr thd
	// for PXP/FPGA alignment
	rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_2K_DVBT_reg, 0x00000005);
	rtd_outl(ATB_DEMOD_CE_T_PARAMS_POS_NOISE_AVERAGE_START_NUM_FO_8K_DVBT_reg, 0x00000005);
	rtd_outl(ATB_DEMOD_CE_T_ISDBT_CE_4K_15_reg, 0x00000005);
	// for PXP/FPGA alignment
	//full-seg
	rtd_outl(ATB_DEMOD_RXFLT_LP_FILTER_MODE_reg, 0x00000002);					//lp_filter_mode: 2 for ISDBT 6M
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG00_7_0_reg, 0x00000003);					//[7:0]:lpf_coef_reg00[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG00_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg00[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG01_7_0_reg, 0x00000008);					//[7:0]:lpf_coef_reg01[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG01_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg01[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG02_7_0_reg, 0x00000008);					//[7:0]:lpf_coef_reg02[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG02_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg02[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG03_7_0_reg, 0x000000ff);					//[7:0]:lpf_coef_reg03[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG03_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg03[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG04_7_0_reg, 0x000000f3);					//[7:0]:lpf_coef_reg04[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG04_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg04[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG05_7_0_reg, 0x000000ef);					//[7:0]:lpf_coef_reg05[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG05_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg05[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG06_7_0_reg, 0x000000f9);					//[7:0]:lpf_coef_reg06[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG06_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg06[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG07_7_0_reg, 0x0000000f);					//[7:0]:lpf_coef_reg07[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG07_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg07[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG08_7_0_reg, 0x0000001e);					//[7:0]:lpf_coef_reg08[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG08_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg08[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG09_7_0_reg, 0x00000016);					//[7:0]:lpf_coef_reg09[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG09_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg09[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG10_7_0_reg, 0x000000f6);					//[7:0]:lpf_coef_reg10[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG10_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg10[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG11_7_0_reg, 0x000000d5);					//[7:0]:lpf_coef_reg11[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG11_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg11[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG12_7_0_reg, 0x000000d2);					//[7:0]:lpf_coef_reg12[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG12_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg12[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG13_7_0_reg, 0x000000f9);					//[7:0]:lpf_coef_reg13[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG13_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg13[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG14_7_0_reg, 0x00000033);					//[7:0]:lpf_coef_reg14[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG14_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg14[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG15_7_0_reg, 0x0000004e);					//[7:0]:lpf_coef_reg15[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG15_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg15[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG16_7_0_reg, 0x00000028);					//[7:0]:lpf_coef_reg16[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG16_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg16[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG17_7_0_reg, 0x000000d2);					//[7:0]:lpf_coef_reg17[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG17_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg17[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG18_7_0_reg, 0x0000008d);					//[7:0]:lpf_coef_reg18[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG18_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg18[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG19_7_0_reg, 0x0000009f);					//[7:0]:lpf_coef_reg19[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG19_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg19[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG20_7_0_reg, 0x00000012);					//[7:0]:lpf_coef_reg20[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG20_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg20[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG21_7_0_reg, 0x00000099);					//[7:0]:lpf_coef_reg21[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG21_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg21[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG22_7_0_reg, 0x000000bc);					//[7:0]:lpf_coef_reg22[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG22_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg22[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG23_7_0_reg, 0x00000037);					//[7:0]:lpf_coef_reg23[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG23_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg23[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG24_7_0_reg, 0x00000047);					//[7:0]:lpf_coef_reg24[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG24_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg24[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG25_7_0_reg, 0x000000a0);					//[7:0]:lpf_coef_reg25[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG25_11_8_reg, 0x0000000e);					//[3:0]:lpf_coef_reg25[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG26_7_0_reg, 0x0000000a);					//[7:0]:lpf_coef_reg26[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG26_11_8_reg, 0x0000000f);					//[3:0]:lpf_coef_reg26[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG27_7_0_reg, 0x000000cf);					//[7:0]:lpf_coef_reg27[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG27_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg27[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG28_7_0_reg, 0x0000006d);					//[7:0]:lpf_coef_reg28[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG28_11_8_reg, 0x00000003);					//[3:0]:lpf_coef_reg28[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG29_7_0_reg, 0x000000c4);					//[7:0]:lpf_coef_reg29[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG29_11_8_reg, 0x00000005);					//[3:0]:lpf_coef_reg29[11:8]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG30_7_0_reg, 0x000000b4);					//[7:0]:lpf_coef_reg30[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG30_11_8_reg, 0x00000006);					//[3:0]:lpf_coef_reg30[11:8]
	//page3
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_CHAN_CHANGE_PEAK_REG1_FACTOR_DENSE_DVBT_11_8_reg, 0x00000002);					//[3:0]:chan_change_peak_reg1_factor_dense_dvbt[11:8]
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_CHAN_CHANGE_PEAK_REG2_FACTOR_DENSE_DVBT_11_8_reg, 0x00000002);					//[3:0]:chan_change_peak_reg2_factor_dense_dvbt[11:8]
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_DENSE_CHAN_DET_NUM_TH_OUT_8K_DVBT_7_0_reg, 0x000000ca);					//[7:0]:params_dense_chan_det_num_th_out_8k_dvbt[7:0]
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_DENSE_CHAN_DET_RATIO_OUT_DVBT_reg, 0x00000002);					//[3:0]:params_dense_chan_det_ratio_out_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_DENSE_CHAN_SYMBOL_IN_DVBT_reg, 0x00000004);					//[3:0]:params_dense_chan_symbol_in_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_DENSE_CHAN_SYMBOL_OUT_DVBT_reg, 0x00000008);					//[3:0]:params_dense_chan_symbol_out_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_NOISE_CHANGE_VAR_REG1_FACTOR_DENSE_DVBT_reg, 0x0000002e);					//[7:0]:params_noise_change_var_reg1_factor_dense_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_NOISE_CHANGE_VAR_REG2_FACTOR_DENSE_DVBT_reg, 0x0000002e);					//[7:0]:params_noise_change_var_reg2_factor_dense_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_CHAN_CHANGE_PEAK_FO_FACTOR_3_DVBT_11_8_reg, 0x00000000);					//[3:0]:params_chan_change_peak_fo_factor_3_dvbt[11:8]
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_CHAN_CHANGE_PEAK_FO_FACTOR_3_DVBT_7_0_reg, 0x00000004);					//[7:0]:params_chan_change_peak_fo_factor_3_dvbt[7:0]
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_MIN_NOISE_COUNTER_NUMBER_DVBT_reg, 0x000000ff);					//[7:0]:params_min_noise_counter_number_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_SHORT_CHAN_DET_FACTOR_DVBT_reg, 0x00000010);					//[7:0]:params_short_chan_det_factor_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_SHORT_CHAN_DET_FACTOR_DVBT_2_reg, 0x00000030);					//[7:0]:params_short_chan_det_factor_dvbt_2
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_CHAN_STATE4_DET_MODE_DVBT_reg, 0x00000000);					//[7:0]:params_chan_state4_det_mode_dvbt
	rtd_outl(ATB_DEMOD_CE_WAGTAIL_PARAMS_NOISE_VAR_ON_CARRIERS_FILTER_EN_DVBT_reg, 0x00000001);					//[0]:params_noise_var_on_carriers_filter_en_dvbt

	rtd_outl(ATB_DEMOD_SPI_TSO_CONSTCLK_EN_reg, 0x00000001);					//[0]:tso_constclk_en
	rtd_outl(ATB_DEMOD_DVBC_FEC_ISDBT_MR_CTRL_reg, 0x00000011);					//[0]:reg_isdbt_mr_en;[4]:reg_isdbt_mr_opt
	rtd_outl(ATB_DEMOD_SELF_BAL_CR_SFB_EN_T_reg, 0x00000000);					//[0]:cr_sfb_en_t

	rtd_outl(ATB_DEMOD_DVBC_FEC_ISDBT_PER_OPTION_reg, 0x00000001);					//[0]:opt_isdbt_per_window;option for PER window (Add by Alger @20220825)
	rtd_outl(ATB_DEMOD_DVBC_FEC_ISDBT_DEPUNC_reg, 0x00000001);					//[0]:opt_isdbt_depunc_zero;option for Deinterleaver output (Add by Alger @20220825)
	rtd_maskl(ATB_DEMOD_CE_T_ISDBT_NOISE_SEARCH_ALPHA_reg, 0xfffffffe, 0x0);				//[0]:params_noise_search_alpha_filter_en_isdbt;[7:4]:params_noise_search_alpha_isdbt;noise_search_alpha_filter_en_isdbt (Add by Sherlock @20220826)
	//End of Merlin8 ATB CE setting
	RTK_DEMOD_ISDBT_INFO("[RTK_R_ISDBT] Exit ATB isdbt_init \n");
#else
    //RTK
	//Merlin8 reuse ATB CE setting from sky
	RTK_DEMOD_ISDBT_INFO("[RTK_R_ISDBT] RTK CE setting \n");
	rtd_maskl(DTV_DEMOD_MISC_DTV_DEMOD_SEL_reg, 0xfffffffe, 0x00000001);
	rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_cmd_limit_reg, 0xdfffe000, 0x00000000);// from 0x20000000 to 0x0; IC need to transfer to signed
	rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_t2_dit_addr_0_reg, 0xf0ffffff, 0x01000000);
	rtd_outl(ATB_DEMOD_TOP_EN_ISDBT_MODE_reg, 0x00000001);
	...
	rtd_outl(ATB_DEMOD_RXFLT_RATE_RATIO_23_16_reg, 0x000000a0);					//[7:0]:rate_ratio[23:16],params_rate_ratio
	rtd_outl(ATB_DEMOD_MAC_ISDBT_MAC_RST_ENB_reg, 0x000000ff);					//for continus/all error reset
	rtd_outl(RTK_DTV_DEMOD_ISDBT_TMCC_SELECT_CE_reg, 0x00000010);					//[5]:sel_rtk_atb_ce_fix;[4]:sel_atb_rtk_ce_fixd;[0]:sel_atb_rtk_ce_auto,fix RTK CE
	rtd_outl(ATB_DEMOD_CE_T_SYMIDX_MOD4_ERROR_reg, 0x00000009);					//[0]:params_sym_idx_mod4_renew_en_isdbt;[3]:params_dvbt_fft_win_adj_ro_sel
	rtd_outl(ATB_DEMOD_SYNC_CFOINT_IQSWAP_DET_METHOD_reg, 0x00000001);					//[0]:cfoint_iqswap_det_method
	rtd_outl(RTK_DTV_DEMOD_ISDBT_DT_CESM_PHASE_ROT_ON_reg, 0x00000000);					//[7:0]:dt_cesm_phase_rot_on
	rtd_outl(RTK_DTV_DEMOD_ISDBT_IT_TMCC_PREFRM_LOCK0_reg, 0x00000011);					//preframe lock forecast enable
	//rtd_outl(0xb815a320, 0x00000090);					//[7:0]:reg_pilot_snr_th[7:0];for ATB CE, preframe lock pilot thd use this one*
	//rtd_outl(0xb815a324, 0x00000000);					//[0]:reg_pilot_snr_th[8];for ATB CE, preframe lock pilot thd use this one*
	rtd_outl(ATB_DEMOD_RXFLT2_PARAMS_THRESHOLD_POWER_COMPARE_7_0_reg, 0x000000fc);					//params_threshold_power_compare_7_0
	rtd_outl(ATB_DEMOD_RXFLT_PARAMS_INTERF_THRESHOLD_reg, 0x00000024);					//params_interf_threshold
	rtd_outl(ATB_DEMOD_RXFLT_PARAMS_BW_MODE_reg, 0x00000002);					//BW_MODE=6M
	rtd_outl(ATB_DEMOD_SYNC2_CFO_MAX_DVBT_reg, 0x000000af);					//cfo_max_dvbt
	//RTK CE
	rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT5_reg, 0x00000000);					//0x0: shift FFT start by the ratio of GI size, 0x1: shift FFT start by index.
	rtd_outl(ATB_DEMOD_SYNC_SYMBOL_START_SHIFT2_reg, 0x0000000c);					//the ratio of GI size for shifting FFT start(full-seg)
	rtd_outl(ATB_DEMOD_SYNC2_DAGC_FAST_CHG_EN_reg, 0x00000000);					//[0]:0:disable fast dagc when DVBT mode for RTK CE
	rtd_outl(ATB_DEMOD_LDPC_LDPC_TEST_MODE_7_0_reg, 0x00000010);					//eco option for RTK CE
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_15_8_reg, 0x00000010);					//(4096) continuous error reset threshold [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CONERR_RST_NUM_7_0_reg, 0x00000000);					//(4096) continuous error reset threshold [7:0]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_15_8_reg, 0x00000010);					//(4096) all error reset threshold [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_ALLERR_RST_TH_7_0_reg, 0x00000000);					//(4096) all error reset threshold [7:0]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_15_8_reg, 0x00000010);					//(4096) all error reset window [15:8]
	rtd_outl(ATB_DEMOD_MAC_PAR_DAT_CNT_RANGE_7_0_reg, 0x00000000);					//(4096) all error reset window [7:0]
	//full-seg
	rtd_outl(ATB_DEMOD_RXFLT_LP_FILTER_MODE_reg, 0x00000002);					//lp_filter_mode: 2 for ISDBT 6M
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG00_7_0_reg, 0x00000003);					//[7:0]:lpf_coef_reg00[7:0]
	rtd_outl(ATB_DEMOD_RXFLT_LPF_COEF_REG00_11_8_reg, 0x00000000);					//[3:0]:lpf_coef_reg00[11:8]
	...
	rtd_outl(ATB_DEMOD_DVBC_FEC_ISDBT_PER_OPTION_reg, 0x00000001);					//[0]:opt_isdbt_per_window;option for PER window (Add by Alger @20220825)
	rtd_outl(ATB_DEMOD_DVBC_FEC_ISDBT_DEPUNC_reg, 0x00000001);					//[0]:opt_isdbt_depunc_zero;option for Deinterleaver output (Add by Alger @20220825)
	RTK_DEMOD_ISDBT_INFO("[RTK_R_ISDBT] Exit RTK isdbt_init \n");
#endif
	return FUNCTION_SUCCESS;
...
)
{
	...
	rtd_maskl(ATB_DEMOD_SPI_TSO_SSI_EN_reg, 0xfffffffe, Serial);
/*
rm
...
)
{
	...
	rtd_maskl(ATB_DEMOD_SPI_TSO_EDGE_MODE_reg, 0xfffffffe, ~TsClkPol);//0x18166008[0]: 1: aligned with negative edge / 0: aligned with postive edge
/*rm
...
)
{
	...
	RTK_DEMOD_ISDBT_INFO("[RTK_R_ISDBT] CrystalFreqHz = %d\n", CrystalFreqHz);	
...
)
{
	...
	lock_tmp = (rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg) & 0x7);//[0]: layerA, [1]:layerB, [2]:layerC
...
)
{
	...
	rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
	PAPR_2K = ((rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_2K_8_reg) & 0x1) << 8) + rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_2K_7_0_reg);
	PAPR_4K = ((rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_4K_8_reg) & 0x1) << 8) + rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_4K_7_0_reg);
	PAPR_8K = ((rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_8K_8_reg) & 0x1) << 8) + rtd_inl(ATB_DEMOD_SYNC2_DVBT_PAPR_8K_7_0_reg);
	rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
...
)
{
	...
	case ISDBT_HIERARCHICAL_LAYER_A:
		...
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_HIGH_7_0_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_7_0_reg);
	...
	case ISDBT_HIERARCHICAL_LAYER_B:
		...
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_23_16_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_7_0_reg);
		//*pLayerBerDen = (rtd_inl(0xB816e484)<<8) + rtd_inl(0xB816e480);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
		break;
	case ISDBT_HIERARCHICAL_LAYER_C:
		...
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_23_16_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_7_0_reg);
		//*pLayerBerDen = (rtd_inl(0xB816e484)<<8) + rtd_inl(0xB816e480);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
		break;
...
)
{
	...
	switch (HierarchicalLayerIndex) {
	case ISDBT_HIERARCHICAL_LAYER_A:
		VberValid = (rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg) & 0x1);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Vber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_HIGH_7_0_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_7_0_reg);
		*pLayerBerDen = (rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_15_8_reg)<<8) + rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_7_0_reg);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
/*
...
*/
	...
	case ISDBT_HIERARCHICAL_LAYER_B:
		VberValid = ((rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg) & 0x2) >> 1);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Vber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_23_16_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_1ST_7_0_reg);
		*pLayerBerDen = (rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_15_8_reg)<<8) + rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_7_0_reg);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
/*
...
*/
	...
	case ISDBT_HIERARCHICAL_LAYER_C:
		VberValid = ((rtd_inl(ATB_DEMOD_DVBC_FEC_FEC_LOCK_reg) & 0x4) >> 2);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Vber = (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_23_16_reg) << 16) + (rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_15_8_reg)<<8) +rtd_inl(ATB_DEMOD_DVBC_FEC_RS_CORR_BITS_LAYER_C_7_0_reg);
		*pLayerBerDen = (rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_15_8_reg)<<8) + rtd_inl(ATB_DEMOD_DVBC_FEC_PKT_ERROR_WINDOW_7_0_reg);
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
/*
...
*/
	...
	//mapping BEC
	if(Vber >= RTK_R_ISDBT_BER_NUM_VALUE_HI_THD)//BEC >= 100000, /80
		Vber = Vber / RTK_R_ISDBT_BER_NUM_VALUE_OVER_HI_THD_DIV;
	else if(Vber >=  RTK_R_ISDBT_BER_NUM_VALUE_THD)//BEC >=50000, / 90
		Vber = Vber / RTK_R_ISDBT_BER_NUM_VALUE_OVER_THD_DIV;
	else //BEC<50000, / 100
		Vber = Vber / RTK_R_ISDBT_BER_NUM_VALUE_DIV;
...
)
{
	...
	case ISDBT_HIERARCHICAL_LAYER_A:
		//RegBitNameRsber = ISDBT_RSBER_0;
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = ((rtd_inl(ATB_DEMOD_SPI_PER_CAL_ERR_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_PER_CAL_ERR_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_PER_CAL_ERR_7_0_reg) &0xff);// layerA
		*pLayerPerDen = ((rtd_inl(ATB_DEMOD_SPI_PER_CAL_RES_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_PER_CAL_RES_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_PER_CAL_RES_7_0_reg) &0xff);// layerA
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
		break;
	case ISDBT_HIERARCHICAL_LAYER_B:
		//RegBitNameRsber = ISDBT_RSBER_1;
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = ((rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_ERR_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_ERR_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_ERR_7_0_reg) &0xff);// layerB
		*pLayerPerDen = ((rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_RES_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_RES_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_ISDBT_B_PER_CAL_RES_7_0_reg) &0xff);// layerB
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
		break;
	case ISDBT_HIERARCHICAL_LAYER_C:
		//RegBitNameRsber = ISDBT_RSBER_2;
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		Rsber = ((rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_ERR_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_ERR_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_ERR_7_0_reg) &0xff);// layerC
		*pLayerPerDen = ((rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_RES_19_16_reg) & 0xf) << 16) + ((rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_RES_15_8_reg) & 0xff) << 8) + (rtd_inl(ATB_DEMOD_SPI_ISDBT_C_PER_CAL_RES_7_0_reg) &0xff);// layerC
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
		break;
...
)
{
	...
	if((rtd_inl(RTK_DTV_DEMOD_ISDBT_INRX_TMCC_SEL_CE_RO0_reg) & 0x01) == 0x1){//RTK CE
	// Get EST_SNR.
	...
	*pSnrDbDen = RTK_R_ISDBT_SNR_DB_DEN_VALUE;
		...
		/*Latch On*/
		rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
		ui32RoNoiseVar = (rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_NOISE_VAR_LLR_REG_SQI_23_16_reg) << 16) + (rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_NOISE_VAR_LLR_REG_SQI_15_8_reg) << 8) + rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_NOISE_VAR_LLR_REG_SQI_7_0_reg);

		ui32RoSignalVar = (rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_SIGNAL_VAR_23_16_reg) << 16) | (rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_SIGNAL_VAR_15_8_reg) << 8) | rtd_inl(ATB_DEMOD_CE_RO_RO_DVBT_SIGNAL_VAR_7_0_reg);
...
// agc gain, 0: postive, 1:negative
	...
	if (gstRTK_R_ISDBT_DvbConfig.IsdbtTunerInput.ui8TxIFAgcPolarity == RTK_A_ISDBT_IF_AGC_POLARITY_INVERTED) {
		rtd_maskl(ATB_DEMOD_TOP_PWM0_OEN_reg, 0xFFFFFFFD, 0x02);
	} else {
		rtd_maskl(ATB_DEMOD_TOP_PWM0_OEN_reg, 0xFFFFFFFD, 0x00);
	}
...
)
{
	...
	rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x1);   //RTK_A_DVBTX_LatchOn();
	u8AGCPolarity = rtd_inl(ATB_DEMOD_TOP_PWM0_OEN_reg);//fix typo from 0xB8000080 to 0xB8160080
	ui8Pwm0 = rtd_inl(ATB_DEMOD_AGC_N_PWM_7_0_reg);
	ui8Pwm1 = rtd_inl(ATB_DEMOD_AGC_N_PWM_9_8_reg);
	rtd_outl(ATB_DEMOD_I2CSLV_RO_LATCH_reg, 0x0);   //RTK_A_DVBTX_LatchOff();
...
)
{
	...
	MpiDiv(&MpiVar0, &MpiNone, MpiVar0, MpiPrivate0);// MpiVar0 = MpiVar0 / MpiPrivate0
