...
#include "rbus/M8P_rtk_dtv_demod_sys_reg.h"
#include "rbus/M8P_rtk_dtv_demod_dtmb_reg.h"
...
void REALTEK_R_DTMB_Constructors(DEMOD* pDemodKernel, unsigned char addr, unsigned char output_mode, unsigned char output_freq, COMM*	pComm)
{
	...
	pRealtekRDTMB->m_acquire_sig_en = 0;
	pRealtekRDTMB->m_hiSnrTpsLockStatus = 0;
	...
	pDemodKernel->KernelAutoTune = REALTEK_R_DTMB_KernelAutoTune;
...
int REALTEK_R_DTMB_Init(DEMOD* pDemodKernel)
{
	...
	rtd_maskl(M8P_RTK_DTV_DEMOD_SYS_DEMOD_SEL_reg, ~M8P_RTK_DTV_DEMOD_SYS_DEMOD_SEL_demod_sel_mask, M8P_RTK_DTV_DEMOD_SYS_DEMOD_SEL_demod_sel(0x0)); //[1:0]demod_sel, 0x0: DTMB
	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_DTMB_8M, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_ALTOBEAM_ATBM253:
				REALTEK_R_DTMB_DBG("Tuner id %d, TV_TUNER_ALTOBEAM_ATBM253\n", Param.id);
				pRealtekRDTMB->m_pTunerOptReg = (REG_VALUE_ENTRY*) ATBM253_OPT_VALUE_DTMB;
				pRealtekRDTMB->m_TunerOptRegCnt = sizeof(ATBM253_OPT_VALUE_DTMB) / sizeof(REG_VALUE_ENTRY);
				break;
...
int REALTEK_R_DTMB_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO*  pSigInfo)
{
	...
	pRealtekRDTMB->m_acquire_sig_en = 1;
	pRealtekRDTMB->m_auto_tune_enable = 1;
	pRealtekRDTMB->m_hiSnrTpsLockStatus = 0;
	pRealtekRDTMB->m_status_checking_stime = tv_osal_time() + DTMB_STATUS_CHECK_INTERVAL_MS;
...
int REALTEK_R_DTMB_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	if (Val == 0) {
		if (pRealtekRDTMB->m_hiSnrTpsLockStatus == 1)
...
int REALTEK_R_DTMB_AcquireSignal(DEMOD* pDemodKernel, unsigned char WaitSignalLock)
{
	...
	while ((tv_osal_time() - cur_time) <= 15) {
		...
		if (pRealtekRDTMB->m_acquire_sig_en == 0)
			return TUNER_CTRL_OK;
...
#else
	REALTEK_R_DTMB_AcquireSignalThread(pDemodKernel);
	return TUNER_CTRL_OK;
#endif
}

int REALTEK_R_DTMB_AcquireSignalThread(DEMOD* pDemodKernel)
{
	unsigned long det_stime = 0;

	int lock = 0, no_signal = 0, tpsLock = 0;
	int CarrierMode = 0;
	S32BITS SnrNum = 0, SnrDen = 0, DemodSnr = 0;
	unsigned int maxCorr = 0, peakSaveR = 0, aveSaveR = 0, papr10 = 0, hiSnrTpsLockCnt = 0;
	U32BITS TpsLockStatus = 0;
	//unsigned int ce_state = 0, fsm_status = 0;
	...
	while ((tv_osal_time() - acquire_sig_stime) <= 380) { //250
		pDemod->IsNoSig(pDemod, &no_signal);
		tv_osal_msleep(5);
		//if (no_signal == 0) break;
	}

	rtd_maskl(M8P_RTK_DTV_DEMOD_DTMB_LATCH_READ_ONLY_reg, ~M8P_RTK_DTV_DEMOD_DTMB_LATCH_READ_ONLY_latch_read_only_mask, M8P_RTK_DTV_DEMOD_DTMB_LATCH_READ_ONLY_latch_read_only(0x1));  //latch read only data
	maxCorr = (M8P_RTK_DTV_DEMOD_DTMB_MAX_CORR_1_get_max_corr_15_8(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_MAX_CORR_1_reg)) << 8) | (M8P_RTK_DTV_DEMOD_DTMB_MAX_CORR_0_get_max_corr_7_0(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_MAX_CORR_0_reg)));
	if ((no_signal == 0) && (maxCorr < 0x20 * 16)) {
		peakSaveR = (M8P_RTK_DTV_DEMOD_DTMB_AVE_PWR_R_2_get_peak_save_r_20_18(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_AVE_PWR_R_2_reg)) << 18) | (M8P_RTK_DTV_DEMOD_DTMB_PEAK_SAVE_R_2_get_peak_save_r_17_10(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_PEAK_SAVE_R_2_reg)) << 10) | (M8P_RTK_DTV_DEMOD_DTMB_PEAK_SAVE_R_1_get_peak_save_r_9_2(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_PEAK_SAVE_R_1_reg)) << 2) | (M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_PEAK_SAVE_R_get_peak_save_r_1_0(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_PEAK_SAVE_R_reg)));
		aveSaveR = (M8P_RTK_DTV_DEMOD_DTMB_RX_MODE_MD_BACQ_LOCK_R_get_ave_save_r_16_14(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_RX_MODE_MD_BACQ_LOCK_R_reg)) << 14) | (M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_PEAK_SAVE_R_get_ave_save_r_13_8(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_PEAK_SAVE_R_reg)) << 8) | (M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_R_0_get_ave_save_r_7_0(rtd_inl(M8P_RTK_DTV_DEMOD_DTMB_AVE_SAVE_R_0_reg)));
		papr10 = (peakSaveR * 10) / aveSaveR;

		pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_TPS_LOCK, &TpsLockStatus);
		//pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_CE_STATE_R, &ce_state);
		//pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_FSM_STATE_R, &fsm_status);

		REALTEK_R_DTMB_INFO("maxCorr = %u, PAPR10 = %u, TpsLockStatus = %u\n", maxCorr, papr10, TpsLockStatus);
		//REALTEK_R_DTMB_INFO("maxCorr = %u, peakSaveR = %u, aveSaveR = %u, PAPR10 = %u, TpsLockStatus = %u,  ce_state = %u, fsm_status = %u\n", maxCorr, peakSaveR, aveSaveR, papr10, TpsLockStatus, ce_state, fsm_status);
		if ((papr10 < 100) && (TpsLockStatus == 0))
			no_signal = 1;
	}

	if (no_signal == 1) {
		REALTEK_R_DTMB_INFO("no_signal = %d, API execution Time = %lu, total_acquire_time = %lu\n", no_signal, tv_osal_time() - det_stime, tv_osal_time() - acquire_sig_stime);
		goto acquireSignalThread_done;
	}

	while ((tv_osal_time() - acquire_sig_stime) <= 3000) {  //600
		tv_osal_msleep(5);
		...
		if (DemodSnr > 25) {
			pDemod->IsTpsLocked(pDemod, &tpsLock);
	...
	//REALTEK_R_DTMB_INFO("signal inner lock = %d, time=%ld\n", lock, tv_osal_time() - acquire_sig_stime);
	REALTEK_R_DTMB_INFO("signal lock = %d, m_hiSnrTpsLockStatus= %u, API execution Time = %lu, total_acquire_time = %lu\n", lock, pRealtekRDTMB->m_hiSnrTpsLockStatus, tv_osal_time() - det_stime, tv_osal_time() - acquire_sig_stime);


acquireSignalThread_done:
	pDemodKernel->Unlock(pDemodKernel);
...
int REALTEK_R_DTMB_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
	REALTEK_R_DTMB* pRealtekRDTMB;
	unsigned long cur_time;

#ifdef CONFIG_RTK_KDRV_DEMOD_TEST
	...
	if (pRealtekRDTMB->m_auto_tune_enable == 0)
		return TUNER_CTRL_OK;
...
#ifdef CONFIG_RTK_KDRV_DEMOD_TEST
	if ((cur_time > pRealtekRDTMB->m_status_checking_stime) || ((pRealtekRDTMB->m_status_checking_stime - cur_time) > DTMB_STATUS_CHECK_INTERVAL_MS)) {
		REALTEK_R_DTMB_GetLockStatus(pDemodKernel, &LockStatus);
		REALTEK_R_DTMB_INFO("[KernelAutoTune] Lock = %d\n", LockStatus);
		if (LockStatus == 1) {
			REALTEK_R_DTMB_GetSignalQuality(pDemodKernel, TV_QUAL_SNR, &Quality);
			REALTEK_R_DTMB_GetSignalQuality(pDemodKernel, TV_QUAL_PEC, &Quality);
			REALTEK_R_DTMB_INFO("\033[1;32;31m" "SNR = "PT_U32BITS",  pkt_error = "PT_U32BITS", total_pkt = "PT_U32BITS"\n" "\033[m", Quality.snr, Quality.layer_per.per_num, Quality.layer_per.per_den);
		}
		pRealtekRDTMB->m_status_checking_stime = tv_osal_time() + DTMB_STATUS_CHECK_INTERVAL_MS;
	...
	if (pRealtekRDTMB->m_acquire_sig_en) {
#ifdef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
		REALTEK_R_DTMB_AcquireSignalThread(pDemodKernel);
#endif
	}
...
)
{
	...
	if (pDriver) {
		...
		if (RTK_R_DTMB_DDR_SIZE) {
			...
			if (pDriver->ShareMemoryPhysicalAddress & 0x7) {
				REALTEK_R_DTMB_WARNING("AllocRealtekRDtmbDriver failed, share memory allocation is not 8 Bytes align\n");
				//assert(0);
			}
