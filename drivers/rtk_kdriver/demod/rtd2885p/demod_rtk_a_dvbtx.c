...
int REALTEK_A_DVBTX_Init(DEMOD* pDemodKernel)
{
	...
	unsigned long ShareMemoryPhysicalAddress = 0, DvbtxRomPhysicalAddrStart, DvbtxRomVirAddrStart;
        //unsigned long	rst;
	//unsigned char* recv_buf;
	unsigned int chipVerCode = 0;
	...
	TvFrontendPowerControl(0);                    //Turn off TVALL
		...
		DvbtxRomVirAddrStart = (unsigned long)dvr_remap_uncached_memory(DvbtxRomPhysicalAddrStart, PAGE_ALIGN(DvbTxRomArraySize), __builtin_return_address(0));
		...
		dvr_unmap_memory((void *)DvbtxRomVirAddrStart, PAGE_ALIGN(DvbTxRomArraySize));
...
*/
	...
	chipVerCode = rtd_inl(STB_SC_VerID_reg);
	...
	rtd_outl(DTV_DEMOD_MISC_ATB_LDPC_ROM_DMA_8_reg, ShareMemoryPhysicalAddress + RTK_A_DVBT2_DDR_SIZE);
	...
	//merlin8 VV702/705/708/710 mosaic issue
	if(chipVerCode == 0x68560000)   //merlin8 VerA
		rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_t2_dit_addr_0_reg, ~DTV_DEMOD_MISC_atb_wrapper_t2_dit_addr_0_atb_wrapper_cache_en_mask, DTV_DEMOD_MISC_atb_wrapper_t2_dit_addr_0_atb_wrapper_cache_en(0x0));
	else
		rtd_maskl(DTV_DEMOD_MISC_atb_wrapper_debug_sel_reg, ~DTV_DEMOD_MISC_atb_wrapper_debug_sel_atb_wrapper_debugclk_sel_mask, DTV_DEMOD_MISC_atb_wrapper_debug_sel_atb_wrapper_debugclk_sel(0x1));
...
int REALTEK_A_DVBTX_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	while ((tv_osal_time() - acquire_sig_stime) <= dvbt2_timeout) {
		if ((tv_osal_time() - acquire_sig_stime) > NoSignalTimeOut) {
			pDemod->MonitorT2TxID(pDemod, &pRealtekADVBTX->m_auto_tune_en);  //check L1 signals
		}
			...
			if (SyncTLockFlagTmp > 5) {
				...
				while ((tv_osal_time() - det_t_stime) <= dvbt2_timeout) {
					...
					if(pRealtekADVBTX->m_auto_tune_en == 0) {
						REALTEK_A_DVBTX_INFO("\033[1;32;32m""force AcquireSignalThread stop by user !! m_auto_tune_en = %u " "\033[1;32;32m", pRealtekADVBTX->m_auto_tune_en);
						break;			
					}
				}
			}
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_A_DVBTX_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
	unsigned long cur_time ;
	...
	if (pRealtekADVBTX->m_acquire_sig_en) {
		...
		pDemodKernel->Lock(pDemodKernel);
		if (Lock == 0) {
			pDemod->MonitorT2TxID(pDemod, pProcessEnFlag);
