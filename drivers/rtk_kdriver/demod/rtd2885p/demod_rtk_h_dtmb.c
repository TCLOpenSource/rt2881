	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_DTMB_8M, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_NXP_TDA18275A:
				...
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) TDA18275A_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(TDA18275A_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			...
			case TV_TUNER_NO_TUNER:
				...
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) SI2151_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(SI2151_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;

			case TV_TUNER_RAFAEL_R840:
				REALTEK_H_DTMB_INFO("Tuner id %d, TV_TUNER_RAFAEL_R840\n", Param.id);
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) R840_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(R840_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;

			case TV_TUNER_MAXLINEAR_MXL661:
				REALTEK_H_DTMB_INFO("Tuner id %d, TV_TUNER_MAXLINEAR_MXL6X1\n", Param.id);
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) MXL661_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(MXL661_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			...
			case TV_TUNER_SONY_SUTRX243_RA:
				...
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) SUTRX243_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(SUTRX243_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;

			case TV_TUNER_ALTOBEAM_ATBM253:
				REALTEK_H_DTMB_INFO("Tuner id %d, TV_TUNER_ALTOBEAM_ATBM253\n", Param.id);
				pRealtekHDTMB->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) ATBM253_OPT_VALUE_DTMB_H;
				pRealtekHDTMB->m_TunerOptRegCnt = sizeof(ATBM253_OPT_VALUE_DTMB_H) / sizeof(REG_VALUE_ENTRY_RBUS);
				break;
			default:
...
int REALTEK_H_DTMB_GetSignalInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pInfo)
{
	...
	if ((status & 0xf0) > 0x20) {
		...
		switch (PnVariable) {
		case DTMB_H_PN_VARIABLE:
			pInfo->dtmb_ofdm.pn.seq = DTMB_OFDM_PN_SEQ_VARIABLE;
			break;
		case DTMB_H_PN_FIXED:
			pInfo->dtmb_ofdm.pn.seq = DTMB_OFDM_PN_SEQ_CONSTANT;
			break;
		}

		switch (pinterleaverLength) {
		case DTMB_H_INTERLEAVER_240:
			pInfo->dtmb_ofdm.tim = DTMB_OFDM_TIM_240;
			break;
		case DTMB_H_INTERLEAVER_720:
			pInfo->dtmb_ofdm.tim = DTMB_OFDM_TIM_720;
			break;
		}


		REALTEK_H_DTMB_DBG("GetSignalInfo CARRIER = %d, QAM_MODE = %d, PN_MODE = %d, DTMB_H_RATE = %d, INTERLEAVER = %d\n", pInfo->dtmb_ofdm.carrier_mode, pInfo->dtmb_ofdm.fec.constellation, pInfo->dtmb_ofdm.pn.mode, pInfo->dtmb_ofdm.fec.rate, pInfo->dtmb_ofdm.tim);
...
int REALTEK_H_DTMB_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	while ((tv_osal_time() - stime) <= 2500) {  // acquiresignal timeout set 3000mS
		...
		if (Status >= 0x86) {
			...
			if ((CarrierMode == DTMB_H_CARRIER_SINGLE) && (pmode == DTMB_H_16QAM) && (Pnmode == DTMB_H_PN_595) && (prate == DTMB_H_RATE_08)) {
			...
			} else if ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_64QAM) && (Pnmode == DTMB_H_PN_945) && (prate == DTMB_H_RATE_06) && (PnVariable == DTMB_H_PN_VARIABLE)) {
	...
	if (lock == YES) {
		...
		if ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_32QAM) && (Pnmode == DTMB_H_PN_420) && (prate == DTMB_H_RATE_08) && (PnVariable == DTMB_H_PN_FIXED)) { //gz482
			RealtekHDtmbModeChange();
		} else {
			RealtekHDtmbModeDefault();
			if (!(((CarrierMode == DTMB_H_CARRIER_SINGLE) && (pmode == DTMB_H_16QAM) && (Pnmode == DTMB_H_PN_595) && (prate == DTMB_H_RATE_08)) || ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_64QAM) && (Pnmode == DTMB_H_PN_945) && (prate == DTMB_H_RATE_06) && (PnVariable == DTMB_H_PN_VARIABLE)))) {
	...
	if (pRealtekHDTMB->m_auto_tune_enable) {
		...
		if ((LockResult != 1) && (UnlockHappened == 0) && (pRealtekHDTMB->m_auto_tune_reset == 1)) {
			...
			if (LockResult == YES) {
				...
				if ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_32QAM) && (Pnmode == DTMB_H_PN_420) && (prate == DTMB_H_RATE_08))
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_H_DTMB_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
	UINT8 LockResult = 0, locked = 0, Status = 0;
	UINT8 CarrierMode = 0, pmode = 0, prate = 0, pinterleaverLength = 0, Pnmode = 0, PnVariable = 0;
	static unsigned long stime = 0;
	...
	if (pRealtekHDTMB->m_auto_tune_enable) {
		...
		if ((LockResult != 1) && (UnlockHappened == 0) && (pRealtekHDTMB->m_auto_tune_reset == 1)) {
			...
			if (LockResult == YES) {
				...
				if ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_64QAM) && (Pnmode == DTMB_H_PN_945) && (prate == DTMB_H_RATE_06) && (PnVariable == DTMB_H_PN_VARIABLE)) {
					DTMBIP_GetSignalSNR(&SignalSNR);
				...
				if ((CarrierMode == DTMB_H_CARRIER_MULTI) && (pmode == DTMB_H_32QAM) && (Pnmode == DTMB_H_PN_420) && (prate == DTMB_H_RATE_08) && (PnVariable == DTMB_H_PN_FIXED)) {
