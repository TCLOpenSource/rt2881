...
int REALTEK_A_DVBSX_Init(DEMOD* pDemodKernel)
{
	...
	unsigned long ShareMemoryPhysicalAddress, DvbsxRomPhysicalAddrStart, DvbsxRomVirAddrStart;
        //unsigned long	rst;
	REALTEK_A_DVBSX* pRealtekIDVBSX;
	...
	TvFrontendPowerControl(0);                    //Turn off TVALL
		...
		DvbsxRomVirAddrStart = (unsigned long)dvr_remap_uncached_memory(DvbsxRomPhysicalAddrStart, PAGE_ALIGN(DvbSxRomArraySize), __builtin_return_address(0));
		...
		dvr_unmap_memory((void *)DvbsxRomVirAddrStart, PAGE_ALIGN(DvbSxRomArraySize));
...
*/
	...
	rtd_outl(DTV_DEMOD_MISC_ATB_LDPC_ROM_DMA_8_reg, ShareMemoryPhysicalAddress + RTK_A_DVBSX_DDR_SIZE);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_A_DVBSX_KernelAutoTune(DEMOD* pDemodKernel, unsigned char* pProcessEnFlag)
{
#ifdef CONFIG_RTK_KDRV_DEMOD_SCAN_THREAD_ENABLE
	REALTEK_A_DVBSX* pRealtekIDVBSX;
	DVBSX_DEMOD_MODULE* pDemod;
