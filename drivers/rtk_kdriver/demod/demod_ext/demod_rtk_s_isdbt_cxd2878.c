/***************************************************************************************************
  File        : demod_rtk_s_isdbt_cxd2878.c
  Description : CXD2878_S_ISDBT demod
  Author      : Kevin Wang
****************************************************************************************************
    Update List :
----------------------------------------------------------------------------------------------------
...
#include "demod_rtk_s_isdbt_cxd2878_priv.h"
#include <rtk_kdriver/pcbMgr.h>

#include "CXD2878Family_refcode/sony_demod.h"
#include "CXD2878Family_refcode/isdb_terr/sony_isdbt.h"
#include "CXD2878Family_refcode/isdb_terr/sony_demod_isdbt.h"
#include "CXD2878Family_refcode/isdb_terr/sony_demod_isdbt_monitor.h"
#include "CXD2878Family_refcode/isdb_terr/sony_integ_isdbt.h"

static int isCreated = 0;
static BASE_INTERFACE_MODULE      BaseIF2878;
...
{
	CXD2878_S_ISDBT* pRealtekSISDBT;

	pDemodKernel->private_data = kmalloc(sizeof(CXD2878_S_ISDBT), GFP_KERNEL);
	if(pDemodKernel->private_data == NULL) {
		goto kmalloc_fail;
	}
	pRealtekSISDBT = (CXD2878_S_ISDBT*)(pDemodKernel->private_data);
	...
	pRealtekSISDBT->m_private            = (void*) AllocRealtekSIsdbt2878Driver(
	...
	pDemodKernel->m_pComm->SetTargetName(pDemodKernel->m_pComm, pDemodKernel->m_addr, "CXD2878_S_ISDBT");
	pRealtekSISDBT->m_pTunerOptReg       = NULL;
	pRealtekSISDBT->m_TunerOptRegCnt     = 0;
	pRealtekSISDBT->m_acquire_sig_en = 0;
	pDemodKernel->Init_Ext = CXD2878_S_ISDBT_Init;
	pDemodKernel->Reset = ExtDemod_Reset;
	pDemodKernel->AcquireSignal = CXD2878_S_ISDBT_AcquireSignal;
	pDemodKernel->ScanSignal = CXD2878_S_ISDBT_ScanSignal;
	pDemodKernel->SetIF = CXD2878_S_ISDBT_SetIF;
	pDemodKernel->SetTSMode = CXD2878_S_ISDBT_SetTSMode;
	pDemodKernel->SetTVSysEx = CXD2878_S_ISDBT_SetTVSysEx;
	pDemodKernel->SetMode = CXD2878_S_ISDBT_SetMode;
	pDemodKernel->SetTVSys = CXD2878_S_ISDBT_SetTVSys;
	pDemodKernel->GetLockStatus = CXD2878_S_ISDBT_GetLockStatus;
	pDemodKernel->GetSignalInfo = CXD2878_S_ISDBT_GetSignalInfo;
	pDemodKernel->GetSignalQuality = CXD2878_S_ISDBT_GetSignalQuality;
	pDemodKernel->GetDemodStatus = ExtDemod_GetDemodStatus;
	pDemodKernel->GetCarrierOffset = CXD2878_S_ISDBT_GetCarrierOffset;
	pDemodKernel->ForwardI2C = CXD2878_S_ISDBT_ForwardI2C;
	pDemodKernel->AutoTune = CXD2878_S_ISDBT_AutoTune;
	pDemodKernel->KernelAutoTune = CXD2878_S_ISDBT_KernelAutoTune;
	...
	pDemodKernel->SetTvMode = CXD2878_S_ISDBT_SetTvMode;
	pDemodKernel->SetExtDemodInfo = CXD2878_S_ISDBT_SetExtDemodInfo;
	//pDemodKernel->DebugInfo = CXD2878_S_ISDBT_DebugInfo;	  	  // Add monitor function that used to monitor demod status
	pDemodKernel->InspectSignal = CXD2878_S_ISDBT_InspectSignal; // Add monitor function that used to monitor demod status
	pDemodKernel->AttachTuner = ExtDemod_AttachTuner;
	pDemodKernel->GetTSParam = ExtDemod_GetTSParam;
	pDemodKernel->Lock = CXD2878_S_ISDBT_Lock;
	pDemodKernel->Unlock = CXD2878_S_ISDBT_Unlock;
	pDemodKernel->Destory = CXD2878_S_ISDBT_Destructors;

kmalloc_fail:
	CXD2878_S_ISDBT_INFO("CXD2878_S_ISDBT_Constructors kmalloc Failed\n");
}
 ...
 * Func : ~CXD2878_S_ISDBT
 *
 * Desc : Destructor of CXD2878_S_ISDBT
 *
...
void CXD2878_S_ISDBT_Destructors(DEMOD* pDemodKernel)
{
	...
	pRealtekSISDBT = (CXD2878_S_ISDBT*)(pDemodKernel->private_data);
	pDemod = ((CXD2878_S_ISDBT_DRIVER_DATA*) pRealtekSISDBT->m_private)->pDemod;
	
	result = CXD2878_demod_Sleep(pDemod);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBT_INFO("CXD2878_demod_Sleep fail (%d)\n", result);
	}

	pDemod = NULL;
	isCreated = 0;
	//if (pRealtekSISDBT->m_private != NULL)
	ReleaseRealtekSIsdbt2878Driver((CXD2878_S_ISDBT_DRIVER_DATA*)pRealtekSISDBT->m_private);

	kfree(pRealtekSISDBT);
	//pDemodKernel->Destory(pDemodKernel);
	CXD2878_S_ISDBT_INFO("CXD2878_S_ISDBT_Destructors Complete\n");
}
...
int CXD2878_S_ISDBT_Init(DEMOD* pDemodKernel, TV_SYSTEM_TYPE mode)
{
	...
	if (!isCreated) {
		DemodI2c.Read             = CXD2878_i2c_dummyRead;
		DemodI2c.Write            = CXD2878_i2c_dummyWrite;
		...
		if(mode == TV_SYS_TYPE_ISDBT_CXD2878)
			createParam.i2cAddressSLVT = dem_slv[1]; //demod1 i2c is 0xda
		else if(mode == TV_SYS_TYPE_ISDBT_CXD2878_2nd)
			createParam.i2cAddressSLVT = dem_slv[2]; // demod3 i2c is 0xc8
		else
			createParam.i2cAddressSLVT = 0; // do not select the correct mode

		result = CXD2878_demod_Create(pDemod, &createParam, &DemodI2c);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBT_WARNING("CXD2878_demod_Create 1st error (%d)\n", result);
			goto init_fail;
		}
		result = CXD2878_demod_Initialize(pDemod);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBT_WARNING("CXD2878_demod_Initialize 1st error (%d)\n", result);
			goto init_fail;
		}
/*		
		createParam.i2cAddressSLVT =  dem_slv[3]; //demod3 I2C slave address is 0xca
		result = CXD2878_demod_Create(pDemod, &createParam, &DemodI2c);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBT_WARNING("CXD2878_demod_Create error 2nd (%d)\n", result);
			return TUNER_CTRL_FAIL;
		}
		result = CXD2878_demod_Initialize(pDemod);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBT_WARNING("CXD2878_demod_Initialize error 2nd (%d)\n", result);
			goto init_fail;
		}
*/
		isCreated = 1;
		if (pcb_mgr_get_enum_info_byname("EXTDEMOD_I2C_SPEED", &param) != 0) {
			CXD2878_S_ISDBT_WARNING("CXD2878_ISDBT[%d]: Get EXTDEMOD_I2C_SPEED fail use default !!!!!!!!!!!!!!!\n", __LINE__);
			param = 0x0;
		}
		I2c_SpeedByPCB = (unsigned char)param;
		CXD2878_S_ISDBT_INFO("CXD2878_ISDBT[%d]: set EXTDEMOD_I2C speed = %d\n", __LINE__, param);
	}

/*
	result = CXD2878_demod_I2cRepeaterEnable(pDemod, 1);
	if (result != SONY_RESULT_OK) {
		//printf("CXD2878_demod_I2cRepeaterEnable error (%d)\n", result);
		goto init_fail;
	}

	result = CXD2878_demod_Shutdown(pDemod);
	if (result != SONY_RESULT_OK) {
		//printf("CXD2878_demod_Shutdown error (%d)\n", result);
		goto init_fail;
	}
*/
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBT_INFO("CXD2878_S_ISDBT_Init Complete\n");
	return TUNER_CTRL_OK;

init_fail:
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBT_WARNING("CXD2878_S_ISDBT_Init Fail\n");
	return TUNER_CTRL_FAIL;
}
...
int CXD2878_S_ISDBT_SetTVSys(DEMOD* pDemodKernel, TV_SYSTEM sys)
{
	...
	CXD2878_S_ISDBT_INFO("\033[1;32;31m" "%s %s %d, system = %d\n" "\033[m", __FILE__, __func__, __LINE__, sys);
...
int CXD2878_S_ISDBT_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO*  pSigInfo)
{
	...
	//sys = TV_SYS_ISDBT_6M;
	if (!IsExtIsdbSys(sys)) {
		CXD2878_S_ISDBT_WARNING("Set TV system failed, unsupported TV system\n");
		return  TUNER_CTRL_FAIL;
	}

	CXD2878_S_ISDBT_INFO("pSigInfo->isdbt.portIdx=%u pSigInfo->isdbs_ext.portIdx=%u  pSigInfo->isdbs3_ext.portIdx=%u\n", pSigInfo->isdbt.portIdx, pSigInfo->isdbs_ext.portIdx, pSigInfo->isdbs3_ext.portIdx);
	switch (pSigInfo->isdbt.portIdx) {
	case I2C_PORT1:
		i2cAddr = 0xda;
		CXD2878_S_ISDBT_INFO("Use demod1 i2cAddr = 0x%x\n", i2cAddr);
		break;
	case I2C_PORT2:
		i2cAddr = 0xc8;
		CXD2878_S_ISDBT_INFO("Use demod2 i2cAddr = 0x%x\n", i2cAddr);
		break;
	case I2C_PORT3:
		i2cAddr = 0xca;
		CXD2878_S_ISDBT_INFO("Use demod3 i2cAddr = 0x%x\n", i2cAddr);
		break;
	default:
		CXD2878_S_ISDBT_WARNING("unknown Demod port index !!!\n");
		goto SetDemod_fail;
	}
	...
	CXD2878_S_ISDBT_INFO("Tune to ISDB-T signal with the following parameters:\n");
	CXD2878_S_ISDBT_INFO(" - Bandwidth            : %s\n", Common_Bandwidth[ISDBTparam.bandwidth]);
	CXD2878_S_ISDBT_INFO("\033[1;32;31m" "%s %s %d, system = %d\n" "\033[m", __FILE__, __func__, __LINE__, sys);
	...
	if (result != SONY_RESULT_OK) {
		printk("CXD2878_demod_Initialize error (%d)\n", result);
		goto SetDemod_fail;
	}
/*
	result = CXD2878_demod_I2cRepeaterEnable(pDemod, 1);
	if (result != SONY_RESULT_OK) {
		printk("CXD2878_demod_I2cRepeaterEnable error (%d)\n", result);
		goto SetDemod_fail;
	}
*/
	/*
	...
	*/
	...
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBT_INFO ("CXD2878_demod_SetConfig (SONY_DEMOD_CONFIG_TUNER_OPTIMIZE) failed. (%s)\n", Common_Result[result]);
		return result;
	}
#endif
/*
	result = CXD2878_demod_SetTSClockModeAndFreq(pDemod, SONY_DTV_SYSTEM_ISDBT);
	if (result != SONY_RESULT_OK) {
		//printf("CXD2878_demod_Shutdown error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_SetConfig(pDemod, SONY_DEMOD_CONFIG_OUTPUT_ATSC3, SONY_DEMOD_OUTPUT_ATSC3_ALP);
	if (result != SONY_RESULT_OK) {
...
*/
	...
	if(pSigInfo->isdbt.portIdx == I2C_PORT1) {
	...
	result = CXD2878_demod_isdbt_Tune(pDemod, &ISDBTparam);
	...
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBT_WARNING("CXD2878_S_ISDBT_SetTVSysEx done\n");
...
SetDemod_fail:
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBT_WARNING("CXD2878_S_ISDBT_SetTVSysEx Fail\n");
	//pDemodKernel->Unlock(pDemodKernel);
	return TUNER_CTRL_FAIL;
...
int CXD2878_S_ISDBT_SetIF(DEMOD* pDemodKernel, IF_PARAM* pParam)
{
	...
	ret = CXD2878_demod_SetIFFreqConfig (pDemod, &iffreqConfig);

	if (ret != SONY_RESULT_OK) {
		CXD2878_S_ISDBT_WARNING("CXD2878_demod_SetIFFreqConfig failed. (%s)\n", Common_Result[ret]);
		return ret;
	}
	...
	*/   
	...
	ret = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_IFAGCNEG, pParam->agc_pola);

	if (ret != SONY_RESULT_OK) {
		CXD2878_S_ISDBT_WARNING ("sony_demod_SetConfig (SONY_DEMOD_CONFIG_IFAGCNEG) failed. (%s)\n", Common_Result[ret]);
		return ret;
	}
	/*     
	...
	*/  
	...
	ret = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_SPECTRUM_INV, pParam->inversion);

	if (ret != SONY_RESULT_OK) {
		CXD2878_S_ISDBT_WARNING ("sony_demod_SetConfig (SONY_DEMOD_CONFIG_SPECTRUM_INV) failed. (%s)\n", Common_Result[ret]);
		return ret;
	}
	pDemodKernel->Unlock(pDemodKernel);
	return ret;
...
int CXD2878_S_ISDBT_SetMode(DEMOD* pDemodKernel, TV_OP_MODE mode)
{
	...
	case TV_OP_MODE_TP_ONLY:
		break;
	default:
		CXD2878_S_ISDBT_WARNING("unknown TV_OP_MODE\n");
		break;
	}
...
int CXD2878_S_ISDBT_SetTSMode(DEMOD* pDemodKernel, TS_PARAM* pParam)
{
	...
	if(pParam->tsOutEn == 1) {
		...
		if (ret != SONY_RESULT_OK)
			CXD2878_S_ISDBT_INFO("sony_demod_SetStreamOutput error (%d)\n", ret);
	}
	else {
	...
	CXD2878_S_ISDBT_INFO("Get TSTLV Output = %d\n", on_off);
	pDemodKernel->Unlock(pDemodKernel);
...
int CXD2878_S_ISDBT_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	pDemodKernel->Lock(pDemodKernel);
	result = CXD2878_demod_isdbt_monitor_SyncStat(pDemod, &DemodLock[0], &TsLock[0], &EarlyUnlock[0]);
	tv_osal_msleep_ext(5);
	result = CXD2878_demod_isdbt_monitor_SyncStat(pDemod, &DemodLock[1], &TsLock[1], &EarlyUnlock[1]);
	tv_osal_msleep_ext(5);
	result = CXD2878_demod_isdbt_monitor_SyncStat(pDemod, &DemodLock[2], &TsLock[2], &EarlyUnlock[2]);
	pDemodKernel->Unlock(pDemodKernel);

	for(i = 0; i < 3 ; i++) {
		CXD2878_S_ISDBT_INFO("DemodLock[i] = %u, TsLock[i] = %u, EarlyUnLock[i] = %u\n", DemodLock[i], TsLock[i], EarlyUnlock[i]);
	}
...
int CXD2878_S_ISDBT_GetSignalInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pInfo)
{
	...
	//need implement
	pDemodKernel->Lock(pDemodKernel);
	result = CXD2878_demod_isdbt_monitor_ModeGuard (pDemod, &mode, &guard);
	...
	if (result == SONY_RESULT_OK) {
		...
		CXD2878_S_ISDBT_INFO("ISDBT_FFT_Mode = %d\n", pInfo->isdbt.trans_mode);
		...
		CXD2878_S_ISDBT_INFO("guard_interval = %d\n", pInfo->isdbt.guard_interval);
	}else {
		CXD2878_S_ISDBT_INFO (" Mode Guard              | Error           | %s\n", Common_Result[result]);
	}
	

	result |= CXD2878_demod_isdbt_monitor_TMCCInfo (pDemod, &tmccInfo);
	...
	if (result == SONY_RESULT_OK) {
		...
		CXD2878_S_ISDBT_INFO("[Layer A]Segment %d, Constellation %d, Code Rate 0x%X\n",
		...
		CXD2878_S_ISDBT_INFO("[Layer B]Segment %d, Constellation %d, Code Rate 0x%X\n",
		...
		CXD2878_S_ISDBT_INFO("[Layer C]Segment %d, Constellation %d, Code Rate 0x%X\n",
						pInfo->isdbt.layer[2].segment_num,
						pInfo->isdbt.layer[2].constellation,
						pInfo->isdbt.layer[2].code_rate);
	}else {
		CXD2878_S_ISDBT_INFO(" TMCC Information        | Error           |  %s\n", Common_Result[result]);
	}
	pDemodKernel->Unlock(pDemodKernel);
...
int CXD2878_S_ISDBT_GetSignalQuality(DEMOD* pDemodKernel, ENUM_TV_QUAL id, TV_SIG_QUAL* pQuality)
{
	...
	case TV_QUAL_LOCKSTATUS:
		result = CXD2878_demod_isdbt_monitor_SyncStat(pDemod, &DemodLock, &TsLock, &EarlyUnlock);
		CXD2878_S_ISDBT_INFO("DemodLock = %u, TsLock = %u, EarlyUnLock = %u [i2c=0x%x]\n", DemodLock, TsLock, EarlyUnlock, pDemod->i2cAddressSLVT);
	...
	case TV_QUAL_SNR:
		result = CXD2878_demod_isdbt_monitor_SNR(pDemod, &SNR);//SNR(dB) * 1000 
		CXD2878_S_ISDBT_INFO (" SNR | %d x 10^-3 dB\n", SNR);
	...
	case TV_QUAL_BEC:
	case TV_QUAL_BER:
		result = CXD2878_demod_isdbt_monitor_PreRSBER (pDemod, &BerA, &BerB, &BerC);
		pQuality->ber = BerA;
		pQuality->layer_ber.ber_num = BerA;
		pQuality->layer_ber.ber_den = 1000000000;
		//CXD2878_S_ISDBT_INFO (" Pre-RS BER              | Layer A         | %u x 10^-7\n", berA);
		break;

	case TV_QUAL_PEC:
	case TV_QUAL_PER:
 	case TV_QUAL_ISDB_PER:
		result = CXD2878_demod_isdbt_monitor_PER (pDemod, &PerA, &PerB, &PerC);
		CXD2878_S_ISDBT_INFO(" PER  | LayerA %u LayerB %u LayerC %u x 10^-6\n", PerA, PerB, PerC);
	...
	case TV_QUAL_SIGNAL_QUALITY:
		...
		break;
	case TV_QUAL_AGC:
		result = CXD2878_demod_isdbt_monitor_IFAGCOut (pDemod, &ifAGCOut);
		pQuality->agc = ifAGCOut;
		CXD2878_S_ISDBT_INFO(" IF AGC | %u\n", ifAGCOut);
		break;
	case TV_QUAL_CARRIER_OFFSET:
		result = CXD2878_demod_isdbt_monitor_CarrierOffset(pDemod, &offset);
		pQuality->ExtIsdb_Status.freq_offset = offset;
		pQuality->ExtIsdb_Status.demod_ret = result;
		CXD2878_S_ISDBT_INFO(" Carrier Offset | %d\n", offset);
		break;
	...
	case TV_QUAL_PRERSBER:
		result = CXD2878_demod_isdbt_monitor_PreRSBER (pDemod, &BerA, &BerB, &BerC);
		CXD2878_S_ISDBT_INFO(" Pre-RS BER  | Layer A %u Layer B %u Layer C %u x 10^-7\n", BerA, BerB, BerC);
		...
		break;
	default:
		CXD2878_S_ISDBT_WARNING("unknown ENUM_TV_QUAL id = %u\n", (unsigned int) id);
...
int CXD2878_S_ISDBT_GetCarrierOffset(DEMOD* pDemodKernel, S32BITS* pOffset)
{
	...
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBT_INFO("Carrier Offset = "PT_S32BITS" Hz\n", *pOffset);
...
#else
	//CXD2878_S_ISDBT_AcquireSignalThread(pDemodKernel);
...
int CXD2878_S_ISDBT_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	CXD2878_S_ISDBT_INFO("\033[1;32;32m" "%s %s %d\n" "\033[m", __FILE__, __func__, __LINE__);
	...
	while ((tv_osal_time_ext() - stime) <= SONY_ISDBT_WAIT_DEMOD_LOCK) {
		CXD2878_demod_isdbt_monitor_SyncStat(pDemod, &DemodLock, &TsLock, &EarlyUnlock);
		if ((DemodLock) || (EarlyUnlock == 1)) break;
		tv_osal_msleep_ext(5);
	}

	CXD2878_S_ISDBT_INFO("DemodLock = %u, TsLock = %u, EarlyUnLock = %u, Time = %lu\n", DemodLock, TsLock, EarlyUnlock, tv_osal_time_ext() - stime);
...
int CXD2878_S_ISDBT_KernelAutoTune(DEMOD* pDemodKernel)
{
	...
	if ((cur_time > pRealtekSISDBT->m_status_checking_stime) || ((pRealtekSISDBT->m_status_checking_stime - cur_time) > ISDBT_STATUS_CHECK_INTERVAL_MS)) {
		result = CXD2878_demod_isdbt_monitor_EWSChange (pDemod, &ewsChange);

		if (result == SONY_RESULT_OK) {
			CXD2878_S_ISDBT_INFO (" EWSChange               | EWS Change      | %d\n", ewsChange);
			if (ewsChange == 1) {
				CXD2878_demod_isdbt_ClearEWSChange(pDemod);
			}
		}
		CXD2878_S_ISDBT_GetLockStatus(pDemodKernel, &LockStatus);
		if (LockStatus == 1) {
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_SNR, &Quality);
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_BER, &Quality);
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_PER, &Quality);
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_SIGNAL_STRENGTH, &Quality);
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_SIGNAL_QUALITY, &Quality);
			CXD2878_S_ISDBT_GetSignalQuality(pDemodKernel, TV_QUAL_AGC, &Quality);
		}
	}
#endif
...
int CXD2878_S_ISDBT_SetTvMode(DEMOD* pDemodKernel, TV_SYSTEM_TYPE mode)
{
	...
	CXD2878_S_ISDBT_INFO("CXD2878_S_ISDBT_SetTvMode(%d)\n", mode);

	switch (mode) {
	case TV_SYS_TYPE_ISDBT_CXD2878:
	case TV_SYS_TYPE_ISDBT_CXD2878_2nd:
		CXD2878_S_ISDBT_INFO("Set to ISDBT \n");
		ret = pDemodKernel->Init_Ext(pDemodKernel, mode);
		//ret |= pDemodKernel->SetTSMode(pDemodKernel, &pDemodKernel->m_ts);

		if (ret != TUNER_CTRL_OK) {
			CXD2878_S_ISDBT_WARNING("Set CXD2878_S_ISDBT to ISDBT mode failed\n");
			goto set_demod_mode_failed;
		}
		break;

	default:
		CXD2878_S_ISDBT_WARNING("unknown TV_SYSTEM_TYPE\n");
		goto set_demod_mode_failed;
	}

	ret = TUNER_CTRL_OK;
	CXD2878_S_ISDBT_INFO("Set CXD2878_S_ISDBT to appointed DTV mode successed\n");
	pDemodKernel->Unlock(pDemodKernel);
	return ret;
...
int CXD2878_S_ISDBT_SetExtDemodInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pSigInfo)
{
	...
	CXD2878_S_ISDBT_INFO("Update ISDBT CXD2878 successed\n");
	pDemodKernel->Unlock(pDemodKernel);
	return TUNER_CTRL_OK;
...
/*-----------------------------------------------------------------
 *     BASE_INTERFACE for CXD2878_S_ISDBT API
 *----------------------------------------------------------------*/
...
void ReleaseRealtekSIsdbt2878Driver(CXD2878_S_ISDBT_DRIVER_DATA *pDriver)
{
	kfree(pDriver);
}


CXD2878_S_ISDBT_DRIVER_DATA* AllocRealtekSIsdbt2878Driver(
	COMM*               pComm,
	unsigned char       Addr,
	U32BITS       CrystalFreq
	//,int TSMode
...
{
	CXD2878_S_ISDBT_DRIVER_DATA* pDriver = (CXD2878_S_ISDBT_DRIVER_DATA*) kmalloc(
				sizeof(CXD2878_S_ISDBT_DRIVER_DATA) +
				sizeof(sony_demod_t)   +
	...
	if (pDriver) {
		...
		memset(pDriver, 0, sizeof(CXD2878_S_ISDBT_DRIVER_DATA));
		pDriver->pDemod                = (sony_demod_t*)(((unsigned char*)pDriver) + sizeof(CXD2878_S_ISDBT_DRIVER_DATA));
...
sony_result_t CXD2878_i2c_dummyRead(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t* pData, uint32_t size, uint8_t mode)
{
	...
	*/
	...
	unsigned char* pRegisterAddr = NULL;
        ...
        ret = __realtek_ext_i2c_read(&BaseIF2878, deviceAddress, pRegisterAddr, RegisterAddrSize, pData, (U32BITS) size);
...
sony_result_t CXD2878_i2c_dummyWrite(sony_i2c_t* pI2c, uint8_t deviceAddress, const uint8_t *pData, uint32_t size, uint8_t mode)
{
	...
	*/
	...
	int ret = FUNCTION_SUCCESS;
	ret = __realtek_ext_i2c_write(&BaseIF2878, deviceAddress, pData, (U32BITS) size);
