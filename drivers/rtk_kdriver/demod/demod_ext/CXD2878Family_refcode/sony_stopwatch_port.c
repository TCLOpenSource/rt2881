...
{
    SONY_TRACE_ENTER("CXD2878_stopwatch_start");
...
#else
	return (uint32_t) tv_osal_time_ext();
#endif
}
...
{
    SONY_TRACE_ENTER("CXD2878_stopwatch_sleep");
    if (!pStopwatch) {
        SONY_TRACE_RETURN(SONY_RESULT_ERROR_ARG);
    }
    SONY_ARG_UNUSED(*pStopwatch);
...
{
    SONY_TRACE_ENTER("CXD2878_stopwatch_elapsed");
