/*------------------------------------------------------------------------------
  Copyright 2016-2023 Sony Semiconductor Solutions Corporation

  Last Updated    : 2023/06/13
  Modification ID : 8ed46b109dadb902cc5bfaf42fa44bfe296fd5c0
------------------------------------------------------------------------------*/

#include "demod/demod_ext/CXD2878Family_refcode/sony_integ.h"
#include "demod/demod_ext/CXD2878Family_refcode/sony_demod.h"
#include "demod/demod_ext/CXD2878Family_refcode/lib/sony_stdlib.h"
...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
sony_result_t CXD2878_demod_Create (sony_demod_t * pDemod,
                                 sony_demod_create_param_t * pCreateParam,
                                 sony_i2c_t * pDemodI2c)
{
    sony_result_t result = SONY_RESULT_OK;

    SONY_TRACE_ENTER ("CXD2878_demod_Create");
...
sony_result_t CXD2878_demod_Initialize (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_Initialize");
    ...
    result = CXD2878_demod_ChipID (pDemod, &(pDemod->chipId));
    if (result != SONY_RESULT_OK) {
        pDemod->state = SONY_DEMOD_STATE_INVALID;
        SONY_TRACE_RETURN (result);
    }
	CXD2878_S_DEMOD_INFO("pDemod->chipId = 0x%x", pDemod->chipId);
    ...
    /* Setup tuner I2C bus */
    result = CXD2878_demod_TunerI2cEnable (pDemod, pDemod->tunerI2cConfig);
    if (result != SONY_RESULT_OK) {
        pDemod->state = SONY_DEMOD_STATE_INVALID;
        SONY_TRACE_RETURN (result);
    }
...
sony_result_t CXD2878_demod_Sleep (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_Sleep");
    ...
    if (pDemod->state == SONY_DEMOD_STATE_SHUTDOWN) {
        ...
        case SONY_DTV_SYSTEM_ISDBT:
            result = CXD2878_demod_isdbt_Sleep (pDemod);
            break;
        ...
        case SONY_DTV_SYSTEM_ISDBS:
            result = CXD2878_demod_isdbs_Sleep (pDemod);
            break;

        case SONY_DTV_SYSTEM_ISDBS3:
            result = CXD2878_demod_isdbs3_Sleep (pDemod);
            break;
...
sony_result_t CXD2878_demod_Shutdown (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_Shutdown");
...
sony_result_t CXD2878_demod_TuneEnd (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_TuneEnd");
    ...
    result = CXD2878_demod_SoftReset (pDemod);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    //for PIX ctrl, enable TS by airtuner request
...
sony_result_t CXD2878_demod_SoftReset (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t CXD2878_demod_SetConfig (sony_demod_t * pDemod,
                                    sony_demod_config_id_t configId,
                                    int32_t value)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_SetConfig");
    ...
    case SONY_DEMOD_CONFIG_PARALLEL_SEL:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC4, (uint8_t) (value ? 0x00 : 0x80), 0x80);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        if (SONY_DEMOD_CHIP_ID_2878_FAMILY (pDemod->chipId)) {
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xE4, (uint8_t) ((value == 2) ? 0x01 : 0x00), 0x01);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_SER_DATA_ON_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC4, (uint8_t) (value ? 0x08 : 0x00), 0x08);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_OUTPUT_SEL_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC4, (uint8_t) (value ? 0x00 : 0x10), 0x10);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSVALID_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC5, (uint8_t) (value ? 0x00 : 0x02), 0x02);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSSYNC_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC5, (uint8_t) (value ? 0x00 : 0x04), 0x04);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSERR_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xCB, (uint8_t) (value ? 0x00 : 0x01), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_LATCH_ON_POSEDGE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC5, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSCLK_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00,  0xC6, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x60,  0x52, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSVALID_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC8, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TSERR_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC9, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TS_PACKET_GAP:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xD6, (uint8_t)value, 0x07);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    ...
    case SONY_DEMOD_CONFIG_PWM_VALUE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xB7, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
         ...
         */
        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xB2, data[0], 0x1F);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0xB3, data[1], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_TSCLK_CURRENT:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0x95, (uint8_t) (value & 0x03), 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TS_CURRENT:
         ...
         */
        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0x95, data, 0x3C);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
         ...
         */
        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0x96, data, 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0x97, data, 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_GPIO0_CURRENT:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x86, (uint8_t) (value & 0x03), 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_GPIO1_CURRENT:
         ...
         */
        if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x86, (uint8_t) ((value & 0x03) << 2), 0x0C) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_GPIO2_CURRENT:
         ...
         */
        if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x86, (uint8_t) ((value & 0x03) << 4), 0x30) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_CHBOND:
        ...
        */
        ...
        case SONY_DEMOD_CHBOND_CONFIG_ATSC3_MAIN:
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0xC0, (uint8_t)value, 0x03);
            break;
        case SONY_DEMOD_CHBOND_CONFIG_ATSC3_SUB:
            /* Stream input should be disabled */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0xC0, 0x00, 0x03);
            break;
        }
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
    ...
    case SONY_DEMOD_CONFIG_TADC_DC_BIAS:
         ...
         */
        if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x41, (uint8_t) (value ? 0x00 : 0x01), 0x01) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_IFAGCNEG:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0xCB, (uint8_t) (value ? 0x40 : 0x00), 0x40);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        ...
        if (!pDemod->atscCoreDisable) {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVR, 0xA3, 0x54, (uint8_t) (value ? 0x80 : 0x00), 0x80);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_IFAGC_ADC_FS:
        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0xCD, data, 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
        ...
        if (!pDemod->atscCoreDisable) {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVR, 0xA3, 0x50, data[0], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVR, 0xA3, 0x52, data[1], 0x70);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_DVBT_BERN_PERIOD:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x60, (uint8_t) (value & 0x1F), 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBC_BERN_PERIOD:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x40, 0x60, (uint8_t) (value & 0x1F), 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT_VBER_PERIOD:
        ...
        */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x6F, (uint8_t) (value & 0x07), 0x07);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT2C2_BBER_MES:
        ...
        */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x20, 0x72, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT2C2_LBER_MES:
        ...
        */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x20, 0x6F, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT_PER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x5C, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBC_PER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x40, 0x5C, (uint8_t) (value & 0x1F), 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT2C2_PER_MES:
        ...
        */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x24, 0xDC, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBT_BERPER_PERIOD:
        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x60, 0x5B, data[0], 0x7F);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x60, 0x5C, data[1], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC_RSERR_BKLEN:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVR, 0xA2, 0xF2, (uint8_t)(value & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        result = CXD2878_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVR, 0xA2, 0xF3, (uint8_t)((value >> 8) & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        result = CXD2878_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVR, 0xA2, 0xF4, (uint8_t)((value >> 16) & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_BBER_MES:
        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x94, 0x4A, data[0], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x94, 0x4B, data[1], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_LBER_MES:
        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x94, 0x30, data[0], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x94, 0x31, data[1], 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_DVBT_AVEBER_PERIOD_TYPE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x84, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT_AVEBER_PERIOD_TIME:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x82, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x10, 0x83, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_DVBT2_AVEBER_PERIOD_TYPE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x20, 0x7C, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBT2_AVEBER_PERIOD_TIME:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x20, 0x7A, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x20, 0x7B, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TYPE_0:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x7C, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TIME_0:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x7A, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x7B, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TYPE_1:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x80, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TIME_1:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x7E, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x7F, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TYPE_2:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x84, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TIME_2:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x82, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x83, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TYPE_3:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x88, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AVEBER_PERIOD_TIME_3:
...

        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x86, (uint8_t) ((reg >> 8) & 0x07), 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x91, 0x87, (uint8_t) (reg & 0xFF), 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN(result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_AVESNR_PERIOD_TIME:
...

        {
            ...
            if (SONY_DEMOD_CHIP_ID_2856_FAMILY (pDemod->chipId)) {
                 ...
                 */
                result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0x48, (uint8_t) ((reg >> 8) & 0x07), 0x07);
                if (result != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN(result);
                }

                result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0x49, (uint8_t) (reg & 0xFF), 0xFF);
                if (result != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN(result);
                }
                 ...
                 */
                result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0x50, (uint8_t) ((reg >> 8) & 0x07), 0x07);
                if (result != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN(result);
                }

                result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0x51, (uint8_t) (reg & 0xFF), 0xFF);
                if (result != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN(result);
                }
            } else {
    ...
    case SONY_DEMOD_CONFIG_GPIO_EWS_FLAG:
        {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x63, 0x3B, data, 0xFF);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ISDBC_TSMF_HEADER_NULL:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x49, 0x36, (uint8_t)(value ? 0x00 : 0x04), 0x04);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBC_NULL_REPLACED_TS_TSVALID_LOW:
         ...
         * OREG_TSMF_VALID_TSMF_CTRL        (Bit[1]) : 1:Enable, 0:Disable
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x49, 0x36, (uint8_t)(value ? 0x02 : 0x00), 0x02);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_OUTPUT_ISDBC_CHBOND:
...

        {
            ...
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x03, 0xB0, (uint8_t) data[0], 0x01);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }

            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x03, 0x16, (uint8_t) data[1], 0x01);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_ATSC3_KOREAN_MODE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT,
    ...
    case SONY_DEMOD_CONFIG_ATSC3_PLPID_PACKET_MPLP:
        /* CXD2878 family only */
        if (!SONY_DEMOD_CHIP_ID_2878_FAMILY(pDemod->chipId)) {
            SONY_TRACE_RETURN(SONY_RESULT_ERROR_NOSUPPORT);
        }
        ...
        /* slave    Bank    Addr    Bit    default    Name
         * ---------------------------------------------------
         * <SLV-T>  95h     23h     [5:4]    2'd0     OREG_OIF_PLPID_CONVMODE_M
         */
        result = CXD2878_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT,
            0x95, 0x23, (uint8_t)(value & 0x03) << 4, 0x30);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_GPIO_EAS_PN_STATE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x99, 0xAE, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_GPIO_EAS_PN_TRANS:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x99, 0xB0, (uint8_t) (value & 0x03), 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_SAT_IFAGCNEG:
        ...
        */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA0, 0xB9, (uint8_t) (value? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBSS2_BER_PER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA0, 0xBA, (uint8_t)value, 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBS2_BER_FER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA0, 0xBC, (uint8_t)value, 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBS_VBER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA0, 0xBB, (uint8_t)value, 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBS2_LBER_MES:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA0, 0x7A, (uint8_t)value, 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_BERNUMCONF:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xC0, 0xA6, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_BER_PERIOD1:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xC0, 0xB1, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_BER_PERIOD2:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xC0, 0xB2, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_BER_PERIODT:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xC0, 0xB0, (uint8_t)value, 0x07);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LBER_MES1:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xD0, 0xD0, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LBER_MES2:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xD0, 0xD1, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_BER_FER_MES1:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xD0, 0xB3, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_BER_FER_MES2:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xD0, 0xB4, (uint8_t)value, 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_LOWCN_HOLD:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x54, (uint8_t)(value ? 0x11 : 0x00), 0x11);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS_LOWCN_THRESH_H:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x50, (uint8_t)((value >> 8) & 0x1F), 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x51, (uint8_t)(value & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    ...
    case SONY_DEMOD_CONFIG_ISDBS_LOWCN_THRESH_L:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x52, (uint8_t)((value >> 8) & 0x1F), 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x53, (uint8_t)(value & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LOWCN_HOLD:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x5B, (uint8_t)(value ? 0x11 : 0x00), 0x11);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LOWCN_THRESH_H:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x55, (uint8_t)((value >> 16) & 0x01), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x56, (uint8_t)((value >> 8) & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x57, (uint8_t)(value & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LOWCN_THRESH_L:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x58, (uint8_t)((value >> 16) & 0x01), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x59, (uint8_t)((value >> 8) & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0xA1, 0x5A, (uint8_t)(value & 0xFF), 0xFF);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_PARALLEL_SEL:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC1, (uint8_t) (value ? 0x00 : 0x80), 0x80);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xCF, (uint8_t) ((value == 2) ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_SER_DATA_ON_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC1, (uint8_t) (value ? 0x08 : 0x00), 0x08);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_OUTPUT_SEL_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC1, (uint8_t) (value ? 0x00 : 0x10), 0x10);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVVALID_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC2, (uint8_t) (value ? 0x00 : 0x02), 0x02);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVSYNC_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC2, (uint8_t) (value ? 0x00 : 0x04), 0x04);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVERR_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC8, (uint8_t) (value ? 0x00 : 0x01), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_LATCH_ON_POSEDGE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC2, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVCLK_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01,  0xC3, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVVALID_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC5, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVERR_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC6, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLVVALID_MASK_IN_ERRNULL:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x01, 0xC8, (uint8_t) (value ? 0x08 : 0x00), 0x08);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_NULL_FILTER:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xEE, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_PARALLEL_SEL:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC1, (uint8_t) (value ? 0x00 : 0x80), 0x80);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_SER_DATA_ON_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC1, (uint8_t) (value ? 0x08 : 0x00), 0x08);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_OUTPUT_SEL_MSB:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC1, (uint8_t) (value ? 0x00 : 0x10), 0x10);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_SYNC_LENGTH:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC1, (uint8_t) (value ? 0x40 : 0x00), 0x40);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPVALID_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC2, (uint8_t) (value ? 0x00 : 0x02), 0x02);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPSYNC_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC2, (uint8_t) (value ? 0x00 : 0x04), 0x04);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPERR_ACTIVE_HI:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC8, (uint8_t) (value ? 0x00 : 0x01), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_LATCH_ON_POSEDGE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC2, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPCLK_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02,  0xC3, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPVALID_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC5, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALPERR_MASK:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC6, (uint8_t) value, 0x1F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ALP_VALIDCLK_IN_GAP:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xEB, (uint8_t) (value ? 0x02 : 0x00), 0x02);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_BBP_LATCH_ON_POSEDGE:
         ...
         */
        result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x9D, 0xF2, (uint8_t) (value ? 0x01 : 0x00), 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
...
#ifdef SONY_DEMOD_SUPPORT_TERR_OR_CABLE
sony_result_t CXD2878_demod_SetIFFreqConfig (sony_demod_t * pDemod,
...
sony_result_t CXD2878_demod_TunerI2cEnable (sony_demod_t * pDemod, sony_demod_tuner_i2c_config_t tunerI2cConfig)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_TunerI2cEnable");

    if (!pDemod) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_ARG);
    }

    result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x1A,
...
sony_result_t CXD2878_demod_I2cRepeaterEnable (sony_demod_t * pDemod, uint8_t enable)
{
    SONY_TRACE_ENTER ("CXD2878_demod_I2cRepeaterEnable");
...
sony_result_t CXD2878_demod_GPIOSetConfig (sony_demod_t * pDemod,
                                        sony_demod_gpio_pin_t pin,
                                        uint8_t enable,
                                        sony_demod_gpio_mode_t mode)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_GPIOSetConfig");
    ...
    case SONY_DEMOD_GPIO_PIN_GPIO2:
        {
             ...
             */
            if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, gpioModeSelAddr, (uint8_t)mode, 0x0F) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
             ...
             */
            if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x82, enableHiZ, gpioBitSel) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
        }
        break;
    ...
    case SONY_DEMOD_GPIO_PIN_TSDATA7:
        {
            ...
            if (gpioModeSelBitHigh) {
                if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, gpioModeSelAddr, (uint8_t) ((unsigned int)mode << 4), 0xF0) != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
                }
            } else {
                if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, gpioModeSelAddr, (uint8_t)mode, 0x0F) != SONY_RESULT_OK) {
             ...
             */
            result = CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x00, 0x81, enableHiZ, gpioBitSel);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
...
sony_result_t CXD2878_demod_GPIORead (sony_demod_t * pDemod,
                                   sony_demod_gpio_pin_t pin,
                                   uint8_t * pValue)
{
    uint8_t rdata = 0x00;

    SONY_TRACE_ENTER ("CXD2878_demod_GPIORead");
...
sony_result_t CXD2878_demod_GPIOWrite (sony_demod_t * pDemod,
                                    sony_demod_gpio_pin_t pin,
                                    uint8_t value)
{
    SONY_TRACE_ENTER ("CXD2878_demod_GPIOWrite");
    ...
    case SONY_DEMOD_GPIO_PIN_GPIO1:
    case SONY_DEMOD_GPIO_PIN_GPIO2:
        if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0xA2, (uint8_t) (value ? 0x07 : 0x00),
    ...
    case SONY_DEMOD_GPIO_PIN_TSDATA6:
    case SONY_DEMOD_GPIO_PIN_TSDATA7:
        if (CXD2878_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0xB2, (uint8_t) (value ? 0xFF : 0x00),
...
sony_result_t CXD2878_demod_ChipID (sony_demod_t * pDemod,
                                 sony_demod_chip_id_t * pChipId)
{
    uint8_t data[2];
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("CXD2878_demod_ChipID");
...
#ifdef SONY_DEMOD_SUPPORT_TERR_OR_CABLE
sony_result_t CXD2878_demod_terr_cable_monitor_InternalDigitalAGCOut (sony_demod_t * pDemod,
                                                                   uint16_t * pDigitalAGCOut)
{
    uint8_t data[2];
    SONY_TRACE_ENTER ("CXD2878_demod_terr_cable_monitor_InternalDigitalAGCOut");
...
sony_result_t CXD2878_demod_SetAndSaveRegisterBits (sony_demod_t * pDemod,
                                                 uint8_t slaveAddress,
                                                 uint8_t bank,
                                                 uint8_t registerAddress,
                                                 uint8_t value,
                                                 uint8_t bitMask)
{
    ...
    SONY_TRACE_ENTER ("CXD2878_demod_SetAndSaveRegisterBits");
...
#ifdef SONY_DEMOD_SUPPORT_TERR_OR_CABLE
sony_result_t CXD2878_demod_terr_cable_SetScanMode (sony_demod_t * pDemod, sony_dtv_system_t system, uint8_t scanModeEnabled)
{
    sony_result_t result = SONY_RESULT_OK;
    ...
    if ((system == SONY_DTV_SYSTEM_DVBC) || (system == SONY_DTV_SYSTEM_ISDBC) || (system == SONY_DTV_SYSTEM_J83B)) {
...

        {
             ...
             */
            if (CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x86, data, 0x20) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
        }
    }
...
sony_result_t CXD2878_demod_SetTSClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    sony_result_t result = SONY_RESULT_OK;
    uint8_t serialTS = 0;
    ...
    SONY_TRACE_ENTER ("CXD2878_demod_SetTSClockModeAndFreq");
     ...
     */
    {
        ...
        result = CXD2878_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xD3, tsRateCtrlOff, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDE, tsInOff, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }

        result = CXD2878_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDA, tsClkManaulOn, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
    ...
    if (serialTS & 0x80) {
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xC4, tsClkConfiguration.serialClkMode, 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xD1, tsClkConfiguration.serialDutyMode, 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
    ...
    if (((serialTS & 0x80) || (twoBitParallel & 0x01)) && ((system == SONY_DTV_SYSTEM_ISDBS3) || (system == SONY_DTV_SYSTEM_ATSC3)
        ...
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE7, 0x00, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    } else {
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE7, 0x01, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }

    {
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x66, data, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
...
sony_result_t CXD2878_demod_SetTLVClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    sony_result_t result = SONY_RESULT_OK;
    uint8_t serialTLV = 0;
    uint8_t twoBitParallel = 0;
     ...
     */
    result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x83, tlvClkConfiguration.inputPeriod, 0x07);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
    ...
    if (serialTLV & 0x80) {
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xC1, tlvClkConfiguration.serialClkMode, 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xCC, tlvClkConfiguration.serialDutyMode, 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
...
sony_result_t sony_demod_SetALPClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    ...
    if (serialALP & 0x80) {
         ...
         */
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xC1, alpClkConfiguration.serialClkMode, 0x03);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
...
sony_result_t CXD2878_demod_SetTSDataPinHiZ (sony_demod_t * pDemod, uint8_t enable)
{
    uint8_t data = 0;
    ...
    SONY_TRACE_ENTER ("CXD2878_demod_SetTSDataPinHiZ");
...
register_write:
    ...
    if (CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x81, (uint8_t) (enable ? 0xFF : 0x00), tsDataMaskAll) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
sony_result_t sony_demod_SetStreamOutput (sony_demod_t * pDemod, uint8_t enable)
{
	...
	CXD2878_S_DEMOD_INFO("[SetStreamOutput] data=0x%x enable=%u", data, enable);
#ifdef SONY_DEMOD_SUPPORT_TLV
    if (((data & 0x03) == 0x01) || isIsdbCTSTLVAutoOutput) {
        /* TLV output */
        /* Set SLV-T Bank : 0x01 */
...
sony_result_t sony_demod_Get_TSTLV_Output(sony_demod_t * pDemod, uint32_t* on_off)
{
    sony_result_t result = SONY_RESULT_OK;
	uint8_t rdata = 0;
    SONY_TRACE_ENTER("sony_demod_Get_TSTLV_Output");
...
#endif
    {
        ...
        /* Get Enable TS output */
        if (pDemod->pI2c->ReadRegister(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xC3, &rdata, 1) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN(SONY_RESULT_ERROR_I2C);
        }
    }
                                               ...
                                               uint8_t bitMask)
{
    ...
    if (slaveAddress == pDemod->i2cAddressSLVR) {
        ...
        result = CXD2878_i2c_SetRegisterBits (pDemod->pI2c, slaveAddress, registerAddress, value, bitMask);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
