...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
sony_result_t CXD2878_demod_isdbs3_Tune (sony_demod_t * pDemod,
                                      sony_isdbs3_tune_param_t * pTuneParam)
{
    ...
    if ((pDemod->state == SONY_DEMOD_STATE_ACTIVE) && (pDemod->system == SONY_DTV_SYSTEM_ISDBS3)) {
        ...
        /* Set StreamID */
		CXD2878_S_DEMOD_INFO("isdbs3_Tune setStreamID");
        result = CXD2878_demod_isdbs3_SetStreamID (pDemod, pTuneParam->streamid, pTuneParam->streamidType);
		...
		CXD2878_S_DEMOD_INFO("isdbs3_Tune sleep then setStreamID");
...
sony_result_t CXD2878_demod_isdbs3_Sleep (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t CXD2878_demod_isdbs3_CheckTMCCLock (sony_demod_t * pDemod,
...
sony_result_t CXD2878_demod_isdbs3_CheckTSTLVLock (sony_demod_t * pDemod,
                                                sony_demod_lock_result_t * pLock)
{
    sony_result_t result = SONY_RESULT_OK;
    uint8_t agcLock = 0;
...
sony_result_t CXD2878_demod_isdbs3_SetStreamID (sony_demod_t * pDemod,
                                             uint16_t streamid,
...
CXD2878_S_DEMOD_INFO("2878_FAMILY set stream type,i2c=0x%x streamid=0x%x", pDemod->i2cAddressSLVT, streamid);
...
static sony_result_t SLtoAIS3 (sony_demod_t * pDemod)
{
    ...
    if (pDemod->isdbs3Output == SONY_DEMOD_OUTPUT_ISDBS3_TLV) {
        ...
        result = CXD2878_demod_SetTLVClockModeAndFreq (pDemod, SONY_DTV_SYSTEM_ISDBS3);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    } else
    ...
    if (CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x80, 0x10, 0x1F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    /* disable Hi-Z setting 2 */
    result = CXD2878_demod_SetTSDataPinHiZ (pDemod, 0);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
static sony_result_t AIS3toSL (sony_demod_t * pDemod)
{
    ...
    if (CXD2878_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x80, 0x1F, 0x1F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    /* Enable Hi-Z setting 2 */
    result = CXD2878_demod_SetTSDataPinHiZ (pDemod, 1);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
