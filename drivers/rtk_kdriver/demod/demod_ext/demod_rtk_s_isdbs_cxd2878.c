/***************************************************************************************************
  File        : demod_rtk_s_isdbs_cxd2878.cpp
  Description : CXD2878_S_ISDBS demod
  Author      : cs_xiao
****************************************************************************************************
    Update List :
----------------------------------------------------------------------------------------------------
...
#include "demod_rtk_s_isdbs_cxd2878_priv.h"
#include <rtk_kdriver/pcbMgr.h>

#include "CXD2878Family_refcode/sony_demod.h"
#include "CXD2878Family_refcode/isdb_sat/sony_isdbs.h"
#include "CXD2878Family_refcode/isdb_sat/sony_demod_isdbs.h"
#include "CXD2878Family_refcode/isdb_sat/sony_demod_isdbs_monitor.h"
#include "CXD2878Family_refcode/isdb_sat/sony_integ_isdbs.h"
 ...
 *----------------------------------------------------------------------*/
void CXD2878_S_ISDBS_Constructors(DEMOD* pDemodKernel, unsigned char addr, unsigned char output_mode, unsigned char output_freq, COMM* pComm)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;

	pDemodKernel->private_data = kmalloc(sizeof(CXD2878_S_ISDBS), GFP_KERNEL);
	if(pDemodKernel->private_data == NULL) {
		goto kmalloc_fail;
	}
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
	...
	pRealtekSISDBS->m_private            = (void*) AllocRealtekSIsdbs2878Driver(
	...
	pDemodKernel->m_pComm->SetTargetName(pDemodKernel->m_pComm, pDemodKernel->m_addr, "CXD2878_S_ISDBS");
	pRealtekSISDBS->m_pTunerOptReg       = NULL;
	pRealtekSISDBS->m_TunerOptRegCnt     = 0;
	pRealtekSISDBS->m_acquire_sig_en = 0;
	pDemodKernel->Init_Ext = CXD2878_S_ISDBS_Init;
	pDemodKernel->Reset = ExtDemod_Reset;
	pDemodKernel->AcquireSignal = CXD2878_S_ISDBS_AcquireSignal;
	pDemodKernel->ScanSignal = CXD2878_S_ISDBS_ScanSignal;
	pDemodKernel->SetIF = CXD2878_S_ISDBS_SetIF;
	pDemodKernel->SetTSMode = CXD2878_S_ISDBS_SetTSMode;
	pDemodKernel->SetTVSysEx = CXD2878_S_ISDBS_SetTVSysEx;
	pDemodKernel->SetMode = CXD2878_S_ISDBS_SetMode;
	pDemodKernel->SetTVSys = CXD2878_S_ISDBS_SetTVSys;
	pDemodKernel->GetLockStatus = CXD2878_S_ISDBS_GetLockStatus;
	pDemodKernel->GetSignalInfo = CXD2878_S_ISDBS_GetSignalInfo;
	pDemodKernel->GetSignalQuality = CXD2878_S_ISDBS_GetSignalQuality;
	pDemodKernel->GetDemodStatus = ExtDemod_GetDemodStatus;
	pDemodKernel->GetCarrierOffset = CXD2878_S_ISDBS_GetCarrierOffset;
	pDemodKernel->ForwardI2C = CXD2878_S_ISDBS_ForwardI2C;
	pDemodKernel->AutoTune = CXD2878_S_ISDBS_AutoTune;
	pDemodKernel->KernelAutoTune = CXD2878_S_ISDBS_KernelAutoTune;
	...
	pDemodKernel->SetTvMode = CXD2878_S_ISDBS_SetTvMode;
	pDemodKernel->SetExtDemodInfo = CXD2878_S_ISDBS_SetExtDemodInfo;
	//pDemodKernel->DebugInfo = CXD2878_S_ISDBS_DebugInfo;	  	  // Add monitor function that used to monitor demod status
	pDemodKernel->InspectSignal = CXD2878_S_ISDBS_InspectSignal; // Add monitor function that used to monitor demod status
	pDemodKernel->AttachTuner = ExtDemod_AttachTuner;
	pDemodKernel->GetTSParam = ExtDemod_GetTSParam;
	pDemodKernel->Lock = CXD2878_S_ISDBS_Lock;
	pDemodKernel->Unlock = CXD2878_S_ISDBS_Unlock;
	pDemodKernel->Destory = CXD2878_S_ISDBS_Destructors;

kmalloc_fail:
	CXD2878_S_ISDBS_INFO("CXD2878_S_ISDBS_Constructors kmalloc Failed\n");
}


/*----------------------------------------------------------------------
 * Func : ~CXD2878_S_ISDBSND
 *
 * Desc : Destructor of CXD2878_S_ISDBSND
 *
 * Parm : pDemod : handle of Demod
 *
 * Retn : N/A
 *----------------------------------------------------------------------*/
void CXD2878_S_ISDBS_Destructors(DEMOD* pDemodKernel)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	sony_result_t result = SONY_RESULT_OK;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
	pDemod = ((CXD2878_S_ISDBS_DRIVER_DATA*) pRealtekSISDBS->m_private)->pDemod;

	result = CXD2878_demod_Sleep(pDemod);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("CXD2878_demod_Sleep fail (%d)\n", result);
	}

	pDemod = NULL;
	isCreated = 0;
	//if (pRealtekSISDBS->m_private != NULL)
	ReleaseRealtekSIsdbs2878Driver((CXD2878_S_ISDBS_DRIVER_DATA*)pRealtekSISDBS->m_private);

	kfree(pRealtekSISDBS);
	//pDemodKernel->Destory(pDemodKernel);
	CXD2878_S_ISDBS_INFO("CXD2878_S_ISDBS_Destructors Complete\n");
}
 ...
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_Init(DEMOD* pDemodKernel, TV_SYSTEM_TYPE mode)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	...
	if (!isCreated) {
		DemodI2c.Read             = CXD2878_i2c_IsdbsRead;
		DemodI2c.Write            = CXD2878_i2c_IsdbsWrite;
		...
		if(mode == TV_SYS_TYPE_ISDBS_CXD2878)
			createParam.i2cAddressSLVT = dem_slv[0];
		else if(mode == TV_SYS_TYPE_ISDBS_CXD2878_2nd)
		...
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBS_INFO("sony_demod_Create 1st error (%d)\n", result);
			goto init_fail;
		}
		result = CXD2878_demod_Initialize(pDemod);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBS_INFO("sony_demod_Initialize 1st error (%d)\n", result);
			goto init_fail;
		}	
/*
		createParam.i2cAddressSLVT =  dem_slv[1];
		result |= CXD2878_demod_Create(pDemod, &createParam, &DemodI2c);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBS_INFO("sony_demod_Create 2nd error (%d)\n", result);
			return TUNER_CTRL_FAIL;
		}
		
		result = CXD2878_demod_Initialize(pDemod);
		if (result != SONY_RESULT_OK) {
			CXD2878_S_ISDBS_INFO("sony_demod_Initialize 2nd error (%d)\n", result);
			goto init_fail;
		}
*/
		isCreated = 1;
		if (pcb_mgr_get_enum_info_byname("EXTDEMOD_I2C_SPEED", &param) != 0) {
			CXD2878_S_ISDBS_WARNING("CXD2878_ISDBS[%d]: Get EXTDEMOD_I2C_SPEED fail use default !!!!!!!!!!!!!!!\n", __LINE__);
			param = 0x0;
		}
		I2c_SpeedByPCB = (unsigned char)param;
		CXD2878_S_ISDBS_INFO("CXD2878_ISDBS[%d]: set EXTDEMOD_I2C speed = %d\n", __LINE__, param);
	}
/*
...
*/
	...
	CXD2878_S_ISDBS_INFO("CXD2878_S_ISDBS_Init Complete\n");
	return TUNER_CTRL_OK;

init_fail:
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBS_WARNING("CXD2878_S_ISDBS_Init Fail\n");
	//pDemodKernel->Unlock(pDemodKernel);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_SetTVSys(DEMOD* pDemodKernel, TV_SYSTEM sys)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);

	CXD2878_S_ISDBS_INFO("\033[1;32;31m" "%s %s %d, system = %d\n" "\033[m", __FILE__, __func__, __LINE__, sys);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO*  pSigInfo)
{
	TUNER_PARAM Param;
	CXD2878_S_ISDBS* pRealtekSISDBS;
	...
	if (!IsExtIsdbSys(sys)) {
		CXD2878_S_ISDBS_WARNING("Set TV system failed, unsupported TV system\n");
		return  TUNER_CTRL_FAIL;
	}

	CXD2878_S_ISDBS_INFO("pSigInfo->isdbt.portIdx=%u pSigInfo->isdbs_ext.portIdx=%u  pSigInfo->isdbs3_ext.portIdx=%u\n", pSigInfo->isdbt.portIdx, pSigInfo->isdbs_ext.portIdx, pSigInfo->isdbs3_ext.portIdx);
	switch (pSigInfo->isdbs_ext.portIdx) {
	case I2C_PORT1:
		i2cAddr = 0xda;
		CXD2878_S_ISDBS_INFO("Use demod1 i2cAddr = 0x%x\n", i2cAddr);
		break;
	case I2C_PORT2:
		i2cAddr = 0xc8;
		CXD2878_S_ISDBS_INFO("Use demod2 i2cAddr = 0x%x\n", i2cAddr);
		break;
	case I2C_PORT3:
		i2cAddr = 0xca;
		CXD2878_S_ISDBS_INFO("Use demod3 i2cAddr = 0x%x\n", i2cAddr);
		break;
	default:
		CXD2878_S_ISDBS_WARNING("unknown Demod port index !!!\n");
		goto SetDemod_fail;
	}
	...
	CXD2878_S_ISDBS_INFO("Tune to ISDB-S signal with the following parameters:\n");
	CXD2878_S_ISDBS_INFO(" - Center Freq    : %u kHz\n", IsdbsParam.centerFreqKHz);
	CXD2878_S_ISDBS_INFO(" - TS ID          : 0x%04X\n", IsdbsParam.tsid);
	CXD2878_S_ISDBS_INFO(" - TS ID Type     : %s\n", tsidtype_str[pSigInfo->isdbs_ext.ts_type]);
	...
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("CXD2878_demod_Initialize error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_SAT_IFAGCNEG, 1);
    if (result != SONY_RESULT_OK) {
        CXD2878_S_ISDBS_INFO("sony_demod_SetConfig (SONY_DEMOD_CONFIG_SAT_IFAGCNEG) failed. (%s)\n", result);
        return result;
    }
	...
	*/
	...
	//if (result != SONY_RESULT_OK) {
	//	CXD2878_S_ISDBS_INFO ("sony_demod_SetConfig (SONY_DEMOD_CONFIG_TUNER_OPTIMIZE) failed. (%s)\n", Isdbs_Result[result]);
	//	return result;
	//}

	result = CXD2878_demod_SetTSClockModeAndFreq(pDemod, SONY_DTV_SYSTEM_ISDBS);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("CXD2878_demod_SetTSClockModeAndFreq error (%d)\n", result);
		goto SetDemod_fail;
	}

	if(pSigInfo->isdbs_ext.portIdx == I2C_PORT1) {
		result = CXD2878_demod_SetConfig(pDemod, SONY_DEMOD_CONFIG_PARALLEL_SEL, 1);
	}
	else {
		result = CXD2878_demod_SetConfig(pDemod, SONY_DEMOD_CONFIG_PARALLEL_SEL, 0);
	}
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("Set SONY_DEMOD_CONFIG_PARALLEL_SEL error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_SER_DATA_ON_MSB, 0);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("Set SONY_DEMOD_CONFIG_SER_DATA_ON_MSB error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_OUTPUT_SEL_MSB, 1);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("Set SONY_DEMOD_CONFIG_OUTPUT_SEL_MSB error (%d)\n", result);
		goto SetDemod_fail;
	}

	//detault set rising edge
	result = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_LATCH_ON_POSEDGE, 1);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("Set SONY_DEMOD_CONFIG_LATCH_ON_POSEDGE error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_SetConfig (pDemod, SONY_DEMOD_CONFIG_TSCLK_CONT, 0);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("Set SONY_DEMOD_CONFIG_TSCLK_CONT error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_isdbs_Tune(pDemod, &IsdbsParam);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("CXD2878_demod_isdbs_Tune error (%d)\n", result);
		goto SetDemod_fail;
	}

	result = CXD2878_demod_TuneEnd(pDemod);
	if (result != SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("sony_demod_Shutdown error (%d)\n", result);
		goto SetDemod_fail;
	}

	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBS_WARNING("CXD2878_S_ISDBS_SetTVSysEx done\n");
...
SetDemod_fail:
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBS_WARNING("CXD2878_S_ISDBS_SetTVSysEx Fail\n");
	//pDemodKernel->Unlock(pDemodKernel);
	return TUNER_CTRL_FAIL;
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_SetIF(DEMOD* pDemodKernel, IF_PARAM* pParam)
{
	...
	CXD2878_S_ISDBS_INFO("SetIF, IF_freq="PT_U32BITS" spectrumn inv=%d, agc_pola=%d\n", pParam->freq, pParam->inversion, pParam->agc_pola);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_SetMode(DEMOD* pDemodKernel, TV_OP_MODE mode)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
	pDemod = ((CXD2878_S_ISDBS_DRIVER_DATA*) pRealtekSISDBS->m_private)->pDemod;
	...
	case TV_OP_MODE_TP_ONLY:
		break;
	default:
		CXD2878_S_ISDBS_WARNING("unknown TV_OP_MODE\n");
		break;
	}
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_SetTSMode(DEMOD* pDemodKernel, TS_PARAM* pParam)
{
	int ret = TUNER_CTRL_OK;
	...
	if(pParam->tsOutEn == 1) {
		...
		if (ret != SONY_RESULT_OK)
			CXD2878_S_ISDBS_INFO("sony_demod_SetStreamOutput error (%d)\n", ret);
	}
	else {
	...
	CXD2878_S_ISDBS_INFO("Get TSTLV Output = %d\n", on_off);
	pDemodKernel->Unlock(pDemodKernel);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	result = CXD2878_demod_isdbs_monitor_SyncStat(pDemod, &AgcLock, &TsLock, &TmccLock);
	CXD2878_S_ISDBS_INFO("\033[1;32;36m" "AGC Lock = %u, TS Lock = %u TMCC Lock = %u\n" "\033[m", AgcLock, TsLock, TmccLock);
	pDemodKernel->Unlock(pDemodKernel);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_GetSignalInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pInfo)
{
	sony_result_t result = SONY_RESULT_OK;
	CXD2878_S_ISDBS*	pRealtekSISDBS;
	...
	result = CXD2878_demod_isdbs_monitor_ModCod(pDemod, &modCodH, &modCodL);

	if (result == SONY_RESULT_OK) {
		CXD2878_S_ISDBS_INFO("ModCod (High) | %s\n", modcod_str[modCodH]);
		CXD2878_S_ISDBS_INFO("ModCod (Low) | %s\n", modcod_str[modCodL]);
		pInfo->isdbs_ext.modcodH = (isdbs_modcod_t)modCodH;
		pInfo->isdbs_ext.modcodL = (isdbs_modcod_t)modCodL;
	}else {
		CXD2878_S_ISDBS_INFO ("ModCod  | Error  | %s\n", Isdbs_Result[result]);
	}
	
	//slot 
	result |= CXD2878_demod_isdbs_monitor_SlotNum(pDemod, &slotNumH, &slotNumL);
	if (result == SONY_RESULT_OK) {
		//CXD2878_S_ISDBS_INFO("SlotNum (High) = %u\n", slotNumH);
		//CXD2878_S_ISDBS_INFO("SlotNum (Low) = %u\n", slotNumL);
		pInfo->isdbs_ext.slotNumH = slotNumH;
		pInfo->isdbs_ext.slotNumL = slotNumL;
	}else {
		CXD2878_S_ISDBS_INFO ("SlotNum  | Error  | %s\n", Isdbs_Result[result]);
	}

	//tmcc info
	result |= CXD2878_demod_isdbs_monitor_TMCCInfo (pDemod, &tmccInfo);
	if (result == SONY_RESULT_OK) {
		//CXD2878_S_ISDBS_INFO("TMCCInfo | Change Order    | %d\n", tmccInfo.changeOrder);
		//CXD2878_S_ISDBS_INFO("         | EWS Flag        | %d\n", tmccInfo.ewsFlag);
		//CXD2878_S_ISDBS_INFO("         | Uplink Info     | %d\n", tmccInfo.uplinkInfo);
        ...
        for (Cnt = 0; Cnt < 8; Cnt++) {
			//CXD2878_S_ISDBS_INFO("         | TS ID #%d        | 0x%04X\n", Cnt, tmccInfo.tsidForEachRelativeTS[Cnt]);
			pInfo->isdbs_ext.tmccInfo.tsidForEachRelativeTS[Cnt] = tmccInfo.tsidForEachRelativeTS[Cnt];
		}
	}else {
		CXD2878_S_ISDBS_INFO(" TMCC Information        | Error           |  %s\n", Isdbs_Result[result]);
	}
	result |= CXD2878_demod_isdbs_monitor_SiteDiversityInfo (pDemod, &siteDiversityInfo);
	if (result == SONY_RESULT_OK) {
		//CXD2878_S_ISDBS_INFO(" SiteDiversityInfo | Site Diversity  | %u\n", siteDiversityInfo);
		pInfo->isdbs_ext.siteDiversityInfo = siteDiversityInfo;
	}
	else {
		CXD2878_S_ISDBS_INFO(" SiteDiversityInfo | Error           | %s\n", Isdbs_Result[result]);
	}
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_GetSignalQuality(DEMOD* pDemodKernel, ENUM_TV_QUAL id, TV_SIG_QUAL* pQuality)
{
	sony_result_t result = SONY_RESULT_OK;
	...
	case TV_QUAL_LOCKSTATUS:
		result = CXD2878_demod_isdbs_monitor_SyncStat(pDemod, &AgcLock, &TsLock, &TmccLock);
		CXD2878_S_ISDBS_INFO (" Demod i2c  =0x%x\n", pQuality->demod_i2cAddr);
		CXD2878_S_ISDBS_INFO("\033[1;32;36m" "AGC Lock = %u, TS Lock = %u TMCC Lock = %u [i2c=0x%x]\n" "\033[m", AgcLock, TsLock, TmccLock, pDemod->i2cAddressSLVT);
	...
	case TV_QUAL_SNR:
		result = CXD2878_demod_isdbs_monitor_CNR(pDemod, &CNR);//SNR(dB) * 1000 
		CXD2878_S_ISDBS_INFO (" CNR | %d x 10^-3 dB\n", CNR);
	...
	case TV_QUAL_BEC:
	case TV_QUAL_BER:
		//result = CXD2878_demod_isdbs_monitor_PreRSBER(pDemod, &BerH, &BerL, &BerTMCC);
	...
	case TV_QUAL_ISDB_PER:
 		result = CXD2878_demod_isdbs_monitor_PER(pDemod, &PerH, &PerL);
		CXD2878_S_ISDBS_INFO (" Pre-RS PER | PER (High)      | %u x 10^-6\n", PerH);
		CXD2878_S_ISDBS_INFO (" Pre-RS PER | PER (Low)      | %u x 10^-6\n", PerL);
	...
	case TV_QUAL_SIGNAL_QUALITY:
		result = CXD2878_demod_isdbs_monitor_CNR(pDemod, &CNR);//CNR(dB) * 1000
		pQuality->quality = CNR;
		break;
	case TV_QUAL_AGC:
		result = CXD2878_demod_isdbs_monitor_IFAGCOut(pDemod, &ifAGCOut);
		pQuality->agc = ifAGCOut;
		CXD2878_S_ISDBS_INFO (" IF AGC | %u\n", ifAGCOut);
		break;
	case TV_QUAL_CARRIER_OFFSET:
		result = CXD2878_demod_isdbs_monitor_CarrierOffset(pDemod, &offset);
		pQuality->ExtIsdb_Status.freq_offset = offset;
		pQuality->ExtIsdb_Status.demod_ret = result;
		CXD2878_S_ISDBS_INFO (" Carrier Offset | %d\n", offset);
		break;
	case TV_QUAL_PRERSBER:
		result = CXD2878_demod_isdbs_monitor_PreRSBER(pDemod, &BerH, &BerL, &BerTMCC);
		CXD2878_S_ISDBS_INFO (" Pre-RS BER | (High) %u (Low) %u (TMCC) %u| x 10^-7  result=%u\n", BerH, BerL, BerTMCC, result);
	...
	case TV_QUAL_TSIDERROR:
		result = CXD2878_demod_isdbs_monitor_TSIDError(pDemod, &TSIDError);
		CXD2878_S_ISDBS_INFO (" TSIDError = %u\n", TSIDError);
		pQuality->ExtIsdb_Status.TSIDError = TSIDError;
		pQuality->ExtIsdb_Status.demod_ret = result;
		break;

	default:
		CXD2878_S_ISDBS_WARNING("unknown ENUM_TV_QUAL id = %u\n", (unsigned int) id);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_GetCarrierOffset(DEMOD* pDemodKernel, S32BITS* pOffset)
{
	sony_result_t result = SONY_RESULT_OK;
	int offset;
	...
	pDemodKernel->Unlock(pDemodKernel);
	CXD2878_S_ISDBS_INFO("Carrier Offset = "PT_S32BITS" KHz\n", *pOffset);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_GetDebugLogBuf(DEMOD* pDemodKernel, unsigned int* LogBufAddr, unsigned int* LogBufSize)
{
	int ret = TUNER_CTRL_OK;
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_AcquireSignal(DEMOD* pDemodKernel, unsigned char WaitSignalLock)
{
...
#else
	//CXD2878_S_ISDBS_AcquireSignalThread(pDemodKernel);
 ...
 * Retn : TUNER_CTRL_OK / TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_AcquireSignalThread(DEMOD* pDemodKernel)
{
	unsigned long   stime;
	unsigned char AgcLock = 0, TsLock = 0, TmccLock = 0;
	sony_result_t result = SONY_RESULT_OK;
	...
	CXD2878_S_ISDBS_INFO("\033[1;32;32m" "%s %s %d\n" "\033[m", __FILE__, __func__, __LINE__);
	...
	if((TsLock |TmccLock))
	{
		CXD2878_S_ISDBS_INFO("ISDBS LOCK AgcLock= %u, TsLock = %u, TmccLock = %u Time = %lu\n", AgcLock, TsLock, TmccLock, tv_osal_time_ext() - stime);	
	}
	else
	{
		CXD2878_S_ISDBS_INFO("ISDBS Unlock AgcLock= %u, TsLock = %u, TmccLock = %u Time = %lu\n", AgcLock, TsLock, TmccLock, tv_osal_time_ext() - stime);
	}
 ...
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_ScanSignal(DEMOD* pDemodKernel, S32BITS* pOffset, SCAN_RANGE range)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
	pDemod = ((CXD2878_S_ISDBS_DRIVER_DATA*) pRealtekSISDBS->m_private)->pDemod;
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_ForwardI2C(DEMOD* pDemodKernel, unsigned char on_off)
{
	int ret = TUNER_CTRL_OK;
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_AutoTune(DEMOD* pDemodKernel)
{
	CXD2878_S_ISDBS* pRealtekSISDBS;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_KernelAutoTune(DEMOD* pDemodKernel)
{
	...
	if ((cur_time > pRealtekSISDBS->m_status_checking_stime) || ((pRealtekSISDBS->m_status_checking_stime - cur_time) > ISDBS_STATUS_CHECK_INTERVAL_MS)) {
		result = CXD2878_demod_isdbs_monitor_EWSChange (pDemod, &ewsChange);
		if (result == SONY_RESULT_OK) {
			CXD2878_S_ISDBS_INFO (" EWSChange               | EWS Change      | %d\n", ewsChange);
			if (ewsChange == 1) {
				CXD2878_demod_isdbs_ClearEWSChange(pDemod);
			}
		}
		CXD2878_S_ISDBS_GetLockStatus(pDemodKernel, &LockStatus);
		if (LockStatus == 1) {
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_SNR, &Quality);
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_BER, &Quality);
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_PER, &Quality);
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_SIGNAL_STRENGTH, &Quality);
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_SIGNAL_QUALITY, &Quality);
			CXD2878_S_ISDBS_GetSignalQuality(pDemodKernel, TV_QUAL_AGC, &Quality);
		}
	}
#endif
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_InspectSignal(DEMOD* pDemodKernel)
{
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int CXD2878_S_ISDBS_Activate(DEMOD* pDemodKernel, unsigned char force_rst)
{
...
int CXD2878_S_ISDBS_SetTvMode(DEMOD* pDemodKernel, TV_SYSTEM_TYPE mode)
{
	...
	CXD2878_S_ISDBS_INFO("CXD2878_S_ISDBS_SetTvMode(%d)\n", mode);

	switch (mode) {
	case TV_SYS_TYPE_ISDBS_CXD2878:
	case TV_SYS_TYPE_ISDBS_CXD2878_2nd:
		CXD2878_S_ISDBS_INFO("Set to ISDBS \n");
		ret = pDemodKernel->Init_Ext(pDemodKernel, mode);
		//ret |= pDemodKernel->SetTSMode(pDemodKernel, &pDemodKernel->m_ts);

		if (ret != TUNER_CTRL_OK) {
			CXD2878_S_ISDBS_WARNING("Set CXD2878_S_ISDBS to ISDBS mode failed\n");
			goto set_demod_mode_failed;
		}
		break;

	default:
		CXD2878_S_ISDBS_WARNING("unknown TV_SYSTEM_TYPE\n");
		goto set_demod_mode_failed;
	}

	ret = TUNER_CTRL_OK;
	CXD2878_S_ISDBS_INFO("Set CXD2878_S_ISDBS to appointed DTV mode successed\n");
	pDemodKernel->Unlock(pDemodKernel);
	return ret;
...
int CXD2878_S_ISDBS_SetExtDemodInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pSigInfo)
{
	unsigned char i2cAddr = 0;
	CXD2878_S_ISDBS* pRealtekSISDBS;
	pRealtekSISDBS = (CXD2878_S_ISDBS*)(pDemodKernel->private_data);
	...
	CXD2878_S_ISDBS_INFO("Update ISDBS CXD2878 successed\n");
	pDemodKernel->Unlock(pDemodKernel);
	return TUNER_CTRL_OK;
...
/*-----------------------------------------------------------------
 *     BASE_INTERFACE for CXD2878_S_ISDBSND API
 *----------------------------------------------------------------*/
void CXD2878_S_ISDBS_Lock(DEMOD* pDemodKernel)
{
...
void CXD2878_S_ISDBS_Unlock(DEMOD* pDemodKernel)
{
...
void ReleaseRealtekSIsdbs2878Driver(CXD2878_S_ISDBS_DRIVER_DATA *pDriver)
{
	kfree(pDriver);
}


CXD2878_S_ISDBS_DRIVER_DATA* AllocRealtekSIsdbs2878Driver(
	COMM*               pComm,
	unsigned char       Addr,
	U32BITS       CrystalFreq
	//,int TSMode
...
{
	CXD2878_S_ISDBS_DRIVER_DATA* pDriver = (CXD2878_S_ISDBS_DRIVER_DATA*) kmalloc(
				sizeof(CXD2878_S_ISDBS_DRIVER_DATA) +
				sizeof(sony_demod_t)   +
	...
	if (pDriver) {
		...
		memset(pDriver, 0, sizeof(CXD2878_S_ISDBS_DRIVER_DATA));
		pDriver->pDemod                = (sony_demod_t*)(((unsigned char*)pDriver) + sizeof(CXD2878_S_ISDBS_DRIVER_DATA));
...
sony_result_t CXD2878_i2c_IsdbsRead(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t* pData, uint32_t size, uint8_t mode)
{
	/*
...
sony_result_t CXD2878_i2c_IsdbsWrite(sony_i2c_t* pI2c, uint8_t deviceAddress, const uint8_t *pData, uint32_t size, uint8_t mode)
{
	/*
		Implement I2C functions
	*/
	...
	//int i;
    //
	//CXD2878_S_ISDBS_INFO("Write : 0x%02X ", deviceAddress);
	//CXD2878_S_ISDBS_INFO("0x%02X ", pData[0]);
	//for(i = 1; i < size; i++) CXD2878_S_ISDBS_INFO("0x%02X ", pData[i]);
	//CXD2878_S_ISDBS_INFO("\n");
