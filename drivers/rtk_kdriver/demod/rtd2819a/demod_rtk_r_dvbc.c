...
static unsigned int ui32CSRTable[] = {//CSR threshold and search range
	0x04, 0x0c, 0x0a, 0x08, //1st as demo board default value
	0xab, 0x2e, 0x2e, 0x2e  //2nd to 4th for ATB253
};
/*----------------------------------------------------------------------
 * Func : REALTEK_R_DVBC
 *
...
kmalloc_fail:
	REALTEK_R_DVBC_WARNING("REALTEK_R_DVBC_Constructors dvr_malloc Failed\n");
}
...
int REALTEK_R_DVBC_Init(DEMOD* pDemodKernel)
{
	...
	pDemod->TunerType = QAM_TUNER_SILICONLABS_SI2151; //Demo board defualt use SI2151
	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_DVBC_8M, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_ALTOBEAM_ATBM253:
				...
				pDemod->TunerType = QAM_TUNER_ALTOBEAM_ATBM253;
				break;
...
int REALTEK_R_DVBC_SetTVSys(DEMOD* pDemodKernel, TV_SYSTEM sys)
{
	...
	if(pSigInfo == NULL){
		REALTEK_R_DVBC_WARNING("REALTEK_R_DVBC_SetTVSys dvr_malloc Failed\n");
		return TUNER_CTRL_FAIL;
	}
...
int REALTEK_R_DVBC_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	...
	if(pDemod->TunerType == QAM_TUNER_ALTOBEAM_ATBM253)
	{
...
int REALTEK_R_DVBC_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	unsigned long stime = 0, auto_mode_stime = 0, auto_mode_etime = 0;
	unsigned int fsm_state_o = 0, tr_ld_o = 0;
	S32BITS snr = 0, SnrNum = 0, SnrDen = 0;
	unsigned char ccfo_acq_try = 0, IsNoSignal = 0, Qamd = 0, csr_acq_cnt = 0;
	...
	unsigned char IsNoSignal_val[] = {0, 0, 0, 0}; // Is no signal matrix for csr enableTH, 4, 8, 10 ,12
	unsigned char Qamd_val[] = {0, 0, 0, 0};
	S32BITS est_ccfo_val[] = {0, 0, 0, 0};
	U32BITS Coarse_SymbolRate_val[] = {0, 0, 0, 0};
	U32BITS Fine_SymbolRate_val[] = {0, 0, 0, 0};
	...
	for(csr_acq_cnt = 0; csr_acq_cnt < DVBC_ACQ_TR_CNT; csr_acq_cnt++)
	{
		pDemod->CSRTH = ui32CSRTable[csr_acq_cnt];
		pDemod->CSRFindRange = ui32CSRTable[csr_acq_cnt+4];
		REALTEK_R_DVBC_INFO("DVBC AutoMode set CSR_TH = 0x%x CSRFindRange = 0x%x\n", pDemod->CSRTH, pDemod->CSRFindRange);
		auto_mode_stime = tv_osal_time();
		ret = DECODE_RET(pDemod->AutoMode(pDemod, &IsNoSignal, &Coarse_SymbolRate, &Fine_SymbolRate, &est_ccfo, &Qamd));
		REALTEK_R_DVBC_INFO("\033[1;32;31m" "AutoMode Result(CSR_TH[%u] = 0x%x) IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x\n" "\033[m", csr_acq_cnt, pDemod->CSRTH, IsNoSignal, Coarse_SymbolRate, Fine_SymbolRate, est_ccfo,Qamd);

		switch(csr_acq_cnt)
		{
			case 0://CSR_TH = 4
				IsNoSignal_val[csr_acq_cnt] = IsNoSignal;
				Coarse_SymbolRate_val[csr_acq_cnt] = Coarse_SymbolRate;
				Fine_SymbolRate_val[csr_acq_cnt] = Fine_SymbolRate;
				est_ccfo_val[csr_acq_cnt] = est_ccfo;
				Qamd_val[csr_acq_cnt] = Qamd;
				break;
			case 1://CSR_TH = 12
				if(IsNoSignal==0)
				{
					if((Fine_SymbolRate==Fine_SymbolRate_val[0]) && (Qamd==Qamd_val[0]))
					{
						REALTEK_R_DVBC_INFO("Find the same FSR and QAM mode in CSR_TH = 0x%x\n", pDemod->CSRTH);
						IsNoSignal_val[csr_acq_cnt] = 1;
						Coarse_SymbolRate_val[csr_acq_cnt] = 0;
						Fine_SymbolRate_val[csr_acq_cnt] = 0;
						est_ccfo_val[csr_acq_cnt] = 0;
						Qamd_val[csr_acq_cnt] = 0;
					}
					else
					{
						REALTEK_R_DVBC_INFO("Find new FSR or QAM mode in CSR_TH = 0x%x\n", pDemod->CSRTH);
						IsNoSignal_val[csr_acq_cnt] = IsNoSignal;
						Coarse_SymbolRate_val[csr_acq_cnt] = Coarse_SymbolRate;
						Fine_SymbolRate_val[csr_acq_cnt] = Fine_SymbolRate;
						est_ccfo_val[csr_acq_cnt] = est_ccfo;
			...
			case 2://CSR_TH = 10
				if(IsNoSignal==0)
				{
					if(((Fine_SymbolRate==Fine_SymbolRate_val[0]) && (Qamd==Qamd_val[0])) || ((Fine_SymbolRate==Fine_SymbolRate_val[1]) && (Qamd==Qamd_val[1])))
					{
						REALTEK_R_DVBC_INFO("Find the same FSR and QAM mode in CSR_TH = 0x%x\n", pDemod->CSRTH);
						IsNoSignal_val[csr_acq_cnt] = 1;
						Coarse_SymbolRate_val[csr_acq_cnt] = 0;
			...
			case 3://CSR_TH = 8
				if(IsNoSignal==0)
				{
					if(((Fine_SymbolRate==Fine_SymbolRate_val[0]) && (Qamd==Qamd_val[0])) || ((Fine_SymbolRate==Fine_SymbolRate_val[1]) && (Qamd==Qamd_val[1])) || ((Fine_SymbolRate==Fine_SymbolRate_val[2]) && (Qamd==Qamd_val[2])))
					{
						REALTEK_R_DVBC_INFO("Find the same FSR and QAM mode in CSR_TH = 8\n");
						IsNoSignal_val[csr_acq_cnt] = 1;
						Coarse_SymbolRate_val[csr_acq_cnt] = 0;
						Fine_SymbolRate_val[csr_acq_cnt] = 0;
						est_ccfo_val[csr_acq_cnt] = 0;
					...
					{
						REALTEK_R_DVBC_INFO("Find new FSR or QAM mode in CSR_TH = 8\n");
						IsNoSignal_val[csr_acq_cnt] = IsNoSignal;
						Coarse_SymbolRate_val[csr_acq_cnt] = Coarse_SymbolRate;
						Fine_SymbolRate_val[csr_acq_cnt] = Fine_SymbolRate;
						est_ccfo_val[csr_acq_cnt] = est_ccfo;
				...
				break;
			default:
				REALTEK_R_DVBC_INFO("Default break at CSR_TH[%u] = 0x%x\n", csr_acq_cnt, pDemod->CSRTH);
				break;
		}

		auto_mode_etime = tv_osal_time();
		pRealtekRDVBC->m_IsNoSignal = IsNoSignal_val[csr_acq_cnt];
		tr_threshold = pDemod->TrLdThreshold;

		if(pDemodKernel->m_ScanMode == CH_MANUAL_MODE)
		{
			if(pRealtekRDVBC->m_auto_qamd != Qamd_val[csr_acq_cnt] || ABS(pRealtekRDVBC->m_auto_fsr, Fine_SymbolRate_val[csr_acq_cnt]) > 30000)
			{
		...
		if(IsNoSignal == 1)
		{
			REALTEK_R_DVBC_INFO("\033[1;32;31m" "DVBC NO SIGNAL!!! Freq="PT_U32BITS", Total Acq Time="PT_U32BITS", AutoMode Time="PT_U32BITS", IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x\n" "\033[m",
				pRealtekRDVBC->m_info.dvbc_qam.freq_khz, tv_osal_time() - pRealtekRDVBC->m_acquire_stime, auto_mode_etime - auto_mode_stime, IsNoSignal, Coarse_SymbolRate, Fine_SymbolRate, est_ccfo, Qamd);
			if(pDemod->TunerType != QAM_TUNER_ALTOBEAM_ATBM253)
				goto AcquireSignal_end;
		}
		else
		{
			pRealtekRDVBC->m_auto_fsr = Fine_SymbolRate_val[csr_acq_cnt];
			pRealtekRDVBC->m_auto_ccfo = est_ccfo_val[csr_acq_cnt];
			pRealtekRDVBC->m_auto_qamd = Qamd_val[csr_acq_cnt];

			switch(Qamd_val[csr_acq_cnt])
			{
				case 4://16QAM
			...
			ret = DECODE_RET(pDemod->AutoModeSetSrHz(pDemod, Fine_SymbolRate_val[csr_acq_cnt]));
			...
			while ((tv_osal_time() - stime) <= 1200) {
				...
				if((tv_osal_time() - stime) > 60 && (fsm_state_o >= 6) && (tr_ld_o >= tr_threshold) && (pRealtekRDVBC->m_auto_qamd == 0x5 || pRealtekRDVBC->m_auto_qamd == 0x6 || pRealtekRDVBC->m_auto_qamd == 0x7))//128&32&64 QAM retry
				{
					...
					else
					{
						...
						if(pDemod->TunerType == QAM_TUNER_ALTOBEAM_ATBM253)
						{
							break; //for ATB253 go next CSR TH
						}
						else
						{
							goto AcquireSignal_fail; // Demo board ori flow
						}
					}
				}
				...
				if(lock == 1)
				{
					...
					if(pDemodKernel->m_ScanMode != CH_MANUAL_MODE)
					{
						REALTEK_R_DVBC_INFO("\033[1;32;36m" "DVBC LOCK!!! Freq="PT_U32BITS", Total Acq Time="PT_U32BITS", AutoMode Time="PT_U32BITS", Local Time="PT_U32BITS", IsNoSignal=%u, CSR="PT_S32BITS" KHz, FSR="PT_S32BITS" Hz, CCFO="PT_S32BITS" Hz, QamMode=0x%x, Snr="PT_S32BITS"/10000, fsm_state=%u, tr=%u\n" "\033[m", 
							pRealtekRDVBC->m_info.dvbc_qam.freq_khz, tv_osal_time() - pRealtekRDVBC->m_acquire_stime, auto_mode_etime - auto_mode_stime, tv_osal_time() - stime, 
						IsNoSignal_val[csr_acq_cnt], Coarse_SymbolRate_val[csr_acq_cnt], Fine_SymbolRate_val[csr_acq_cnt], est_ccfo_val[csr_acq_cnt], Qamd_val[csr_acq_cnt], snr, fsm_state_o, tr_ld_o);
					}
					else
					{
			...
			if(lock == 0)
			{
				...
				if(pDemod->TunerType != QAM_TUNER_ALTOBEAM_ATBM253) goto AcquireSignal_fail;//for demo board ori flow
			}
		}
	}
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_R_DVBC_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
