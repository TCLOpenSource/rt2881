...
int  realtek_r_atsc3_LDM_EnhancedLayerOnly_detect(ATSC3_DEMOD_MODULE * pDemod, unsigned char* enhancedOnlyFlag)
{

	int ret = FUNCTION_SUCCESS;
	unsigned int ReadingData = 0, swExtractPlp = 0;
	unsigned char L1ParsingDone = 0, PlpDumpAllFlag = 0;
	unsigned char selectedPLP = 0, associatedPLP = 0;

	TV_ATSC3_PLP_ID Atsc3PlpAll;
	int i32CntTmp = 0;

	*enhancedOnlyFlag = 0xf;

	ret |= rtd_4bytes_i2c_rd(pDemod, 0xb816ef40, &ReadingData);
	swExtractPlp = ReadingData;

	if ((swExtractPlp & 0xC0404000) == 0) { //only decode 1 plp

		selectedPLP =  swExtractPlp & 0x3F;
		...
		if (L1ParsingDone == 0x3) {
			...
			if (PlpDumpAllFlag == 0) {
				...
				ret |= realtek_r_atsc3_GetPlpID(pDemod, &Atsc3PlpAll,	FULL_PLP_ID);

				for (i32CntTmp = 0 ; i32CntTmp < Atsc3PlpAll.PLPCnt; i32CntTmp++) {

					if (selectedPLP == (Atsc3PlpAll.PLPId[i32CntTmp] & 0x3f)) {

						if ((Atsc3PlpAll.PLPId[i32CntTmp] & 0x80) ==  0x80) {
							swExtractPlp = ((selectedPLP | 0x40) << 8) | (associatedPLP | 0x40);
							RTK_DEMOD_ATSC3_INFO("LDM enhanced layer decode only and set with associated dcore layer (0x%x)~~~\n", swExtractPlp);
							ret |= rtd_4bytes_i2c_wr(pDemod, 0xb816ef40, swExtractPlp);  //change plp
							ret |= rtd_4bytes_i2c_wr(pDemod, 0xb8160014, 0x00000001);//0x0005 = 0x01//Reset DSP
							ret |= rtd_4bytes_i2c_wr(pDemod, 0xb8160014, 0x00000000);//0x0005 = 0x00//Start DSP
							*enhancedOnlyFlag = 0x1;
						} else {
							*enhancedOnlyFlag = 0x0;
						}
						break;

					} else if ((Atsc3PlpAll.PLPId[i32CntTmp] & 0x80) == 0x0) {
						associatedPLP = Atsc3PlpAll.PLPId[i32CntTmp] & 0x3f;
					}
...
int  realtek_r_atsc3_ctiDepth724_TiExtInter_detect(ATSC3_DEMOD_MODULE * pDemod, unsigned char* flag, TV_ATSC3_PLP_ID* pAtsc3PlpAll, TV_ATSC3_PLP_ID* pAtsc3PlpLlsOnly)
{
	...
	unsigned int  ReadingData = 0;
	int ret = FUNCTION_SUCCESS;
	unsigned char L1ParsingDone = 0, PLPCnt = 0, CtiDetectedPLPNo = 0;
	unsigned char plpACtiDepth = 0, plpBCtiDepth = 0, plpCCtiDepth = 0, plpDCtiDepth = 0, plpCtiDepth = 0;
	unsigned int plpACtiStartRow = 0, plpBCtiStartRow = 0, plpCCtiStartRow = 0, plpDCtiStartRow = 0, plpCtiStartRow = 0;
	unsigned int plpCtiFecBlockStart = 0;

	unsigned char PlpATiMode = 0, PlpBTiMode = 0, PlpCTiMode = 0, PlpDTiMode = 0, PlpATiExtendedInt = 0, PlpBTiExtendedInt = 0, PlpCTiExtendedInt = 0, PlpDTiExtendedInt = 0;
	unsigned char PlpAMod = 0, PlpBMod = 0, PlpCMod = 0, PlpDMod = 0, PlpMod = 0, PlpAFecType = 0, PlpBFecType = 0, PlpCFecType = 0, PlpDFecType = 0, PlpFecType = 0, PlpALayer = 0, PlpBLayer = 0, PlpCLayer = 0, PlpDLayer = 0, PlpACod = 0, PlpBCod = 0, PlpCCod = 0, PlpDCod = 0;
	unsigned char PlpAType = 0, PlpBType = 0, PlpCType = 0, PlpDType = 0;
	unsigned char SwPlp0SubframeId = 0, SwPlp1SubframeId = 0, SwPlp2SubframeId = 0, SwPlp3SubframeId = 0;
	unsigned int PlpASize = 0, PlpBSize = 0, PlpCSize = 0, PlpDSize = 0, PlpSize = 0;
	unsigned int PlpANumSubslices = 0, PlpBNumSubslices = 0, PlpCNumSubslices = 0, PlpDNumSubslices = 0, PlpASubsliceInterval = 0, PlpBSubsliceInterval = 0, PlpCSubsliceInterval = 0, PlpDSubsliceInterval = 0;
	unsigned char PlpAScramblerType = 0, PlpBScramblerType = 0, PlpCScramblerType = 0, PlpDScramblerType = 0;
	unsigned int PlpAStart = 0, PlpBStart = 0, PlpCStart = 0, PlpDStart = 0, PlpStart = 0;
	unsigned char PlpAHtiInterSubframe = 0, PlpBHtiInterSubframe = 0, PlpCHtiInterSubframe = 0, PlpDHtiInterSubframe = 0, PlpAHtiNumTiBlocks = 0, PlpBHtiNumTiBlocks = 0, PlpCHtiNumTiBlocks = 0, PlpDHtiNumTiBlocks = 0;
	unsigned int PlpAHtiFecBlocksMax = 0, PlpBHtiFecBlocksMax = 0, PlpCHtiFecBlocksMax = 0, PlpDHtiFecBlocksMax = 0, PlpAHtiFecBlocks = 0, PlpBHtiFecBlocks = 0, PlpCHtiFecBlocks = 0, PlpDHtiFecBlocks = 0;
	unsigned char PlpAHtiCellInterleaver = 0, PlpBHtiCellInterleaver = 0, PlpCHtiCellInterleaver = 0, PlpDHtiCellInterleaver = 0;
	unsigned char plpALdmInjectLvl = 0, plpBLdmInjectLvl = 0, plpCLdmInjectLvl = 0, plpDLdmInjectLvl = 0;
	unsigned int plpAFecBlockStart = 0, plpBFecBlockStart = 0, plpCFecBlockStart = 0, plpDFecBlockStart = 0;
	unsigned int L1bL1DetailTotalCells = 0, L1bPaprReduction = 0, L1b1stSubScatteredPilotPattern = 0;
	unsigned char L1bNumSubframes = 0;
	unsigned int L1b1stSubNumOfdmSymbols = 0;
	unsigned char L1b1stSubReducedCarriers = 0, L1b1stSubFftSize = 0;
	...
	unsigned int u32DataTmp = 0, u32DataTmp1 = 0;
	unsigned int Nrow = 724;  //for plpCtiDepth =1
	unsigned int u32C = 0, u32C0 = 0, u32C1 = 0;

	unsigned int ldpcN = 0, Ncell = 0, Cnext = 0;

	//unsigned int addrTmp = 0;
	unsigned long L1ParsingDone_stime = 0;
	...
	if (L1ParsingDone == 0x3) {
		*flag = 0x0;
		...
		if (PLPCnt >= 1) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED18, &ReadingData);
			PlpATiMode = (ReadingData >> 16) & 0x3;
			plpAFecBlockStart = ReadingData & 0x7FFF;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED14, &ReadingData);
			PlpATiExtendedInt = (ReadingData >> 24) & 0x1;
			PlpASubsliceInterval = ReadingData & 0xffffff;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED20, &ReadingData);
			plpACtiDepth = (ReadingData >> 16) & 0x7;
			plpACtiStartRow = ReadingData & 0x7FF;
			if ((PlpATiMode == 1) && (PlpATiExtendedInt == 1) && (plpACtiDepth == 1)) {
				*flag = 0x1;
				CtiDetectedPLPNo = 1;
				if (CtiDetectedPLPNo == 1)
					PlpATiExtendedInt = 0;
			}
		}
		...
		if (PLPCnt >= 2) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED60, &ReadingData);
			PlpBTiMode = (ReadingData >> 16) & 0x3;
			plpBFecBlockStart = ReadingData & 0x7FFF;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED5C, &ReadingData);
			PlpBTiExtendedInt = (ReadingData >> 24) & 0x1;
			PlpBSubsliceInterval = ReadingData & 0xffffff;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED68, &ReadingData);
			plpBCtiDepth = (ReadingData >> 16) & 0x7;
			plpBCtiStartRow = ReadingData & 0x7FF;
			if ((PlpBTiMode == 1) && (PlpBTiExtendedInt == 1) && (plpBCtiDepth == 1)) {
				*flag = 0x1;
				CtiDetectedPLPNo = (CtiDetectedPLPNo != 0) ? CtiDetectedPLPNo : 2;
				if (CtiDetectedPLPNo == 2)
					PlpBTiExtendedInt = 0;
			}
		}

		if (PLPCnt >= 3) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDA8, &ReadingData);
			PlpCTiMode = (ReadingData >> 16) & 0x3;
			plpCFecBlockStart = ReadingData & 0x7FFF;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDA4, &ReadingData);
			PlpCTiExtendedInt = (ReadingData >> 24) & 0x1;
			PlpCSubsliceInterval = ReadingData & 0xffffff;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDB0, &ReadingData);
			plpCCtiDepth = (ReadingData >> 16) & 0x7;
			plpCCtiStartRow = ReadingData & 0x7FF;
			if ((PlpCTiMode == 1) && (PlpCTiExtendedInt == 1) && (plpCCtiDepth == 1)) {
				*flag = 0x1;
				CtiDetectedPLPNo = (CtiDetectedPLPNo != 0) ? CtiDetectedPLPNo : 3;

				if (CtiDetectedPLPNo == 3)
					PlpCTiExtendedInt = 0;
			}
		}

		if (PLPCnt >= 4) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDF0, &ReadingData);
			PlpDTiMode = (ReadingData >> 16) & 0x3;
			plpDFecBlockStart = ReadingData & 0x7FFF;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDEC, &ReadingData);
			PlpDTiExtendedInt = (ReadingData >> 24) & 0x1;
			PlpDSubsliceInterval = ReadingData & 0xffffff;
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDF8, &ReadingData);
			plpDCtiDepth = (ReadingData >> 16) & 0x7;
			plpDCtiStartRow = ReadingData & 0x7FF;
			if ((PlpDTiMode == 1) && (PlpDTiExtendedInt == 1) && (plpDCtiDepth == 1)) {
				*flag = 0x1;
				CtiDetectedPLPNo = (CtiDetectedPLPNo != 0)  ? CtiDetectedPLPNo : 4;

				if (CtiDetectedPLPNo == 4)
					PlpDTiExtendedInt = 0;
			}
		}
		...
		if (*flag == 1) {
			...
			if (PLPCnt >= 1) {
				...
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED0C, &ReadingData);
				plpALdmInjectLvl = ReadingData & 0x1f;

				if (CtiDetectedPLPNo == 1) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED1C, &ReadingData);
					plpCtiFecBlockStart = ReadingData & 0x3FFFFF;
					plpCtiDepth = plpACtiDepth;
					plpCtiStartRow = plpACtiStartRow;
					PlpStart = PlpAStart;
					PlpFecType = PlpAFecType;
					PlpMod = PlpAMod;
					PlpSize = PlpASize;
				}
			...
			if (PLPCnt >= 2) {
				...
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED54, &ReadingData);
				plpBLdmInjectLvl = ReadingData & 0x1f;


				if (CtiDetectedPLPNo == 2) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED64, &ReadingData);
					plpCtiFecBlockStart = ReadingData & 0x3FFFFF;
					plpCtiDepth = plpBCtiDepth;
					plpCtiStartRow = plpBCtiStartRow;
					PlpStart = PlpBStart;
					PlpFecType = PlpBFecType;
					PlpMod = PlpBMod;
					PlpSize = PlpBSize;
				}
			...
			if (PLPCnt >= 3) {
				...
				//PLPC
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED98, &ReadingData);
				PlpCMod = (ReadingData >> 4) & 0xf;
				PlpCFecType = (ReadingData >> 8) & 0xf;
				PlpCLayer = (ReadingData >> 16) & 0x3;
				PlpCCod = ReadingData & 0xf;
				PlpCScramblerType = (ReadingData >> 12) & 0x3;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDA0, &ReadingData);
				PlpCType = (ReadingData >> 16) & 0x1;
				PlpCNumSubslices = ReadingData & 0x3fff;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED94, &ReadingData);
				PlpCSize = ReadingData & 0xFFFFFF;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED90, &ReadingData);
				PlpCStart = ReadingData & 0xFFFFFF;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDB4, &ReadingData);
				PlpCHtiInterSubframe = (ReadingData >> 20) & 0x1;
				PlpCHtiNumTiBlocks = ReadingData & 0xf;
				PlpCHtiFecBlocksMax = (ReadingData >> 4) & 0xfff;
				PlpCHtiCellInterleaver = (ReadingData >> 16) & 0x1;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDB8, &ReadingData);
				PlpCHtiFecBlocks = ReadingData & 0xfff;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED9C, &ReadingData);
				plpCLdmInjectLvl = ReadingData & 0x1f;

				if (CtiDetectedPLPNo == 3) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDAC, &ReadingData);
					plpCtiFecBlockStart = ReadingData & 0x3FFFFF;
					plpCtiDepth = plpCCtiDepth;
					plpCtiStartRow = plpCCtiStartRow;
					PlpStart = PlpCStart;
					PlpFecType = PlpCFecType;
					PlpMod = PlpCMod;
					PlpSize = PlpCSize;
				}
			...
			if (PLPCnt >= 4) {
				//PLPB
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDE0, &ReadingData);
				PlpDMod = (ReadingData >> 4) & 0xf;
				PlpDFecType = (ReadingData >> 8) & 0xf;
				PlpDLayer = (ReadingData >> 16) & 0x3;
				PlpDCod = ReadingData & 0xf;
				PlpDScramblerType = (ReadingData >> 12) & 0x3;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDE8, &ReadingData);
				PlpDType = (ReadingData >> 16) & 0x1;
				PlpDNumSubslices = ReadingData & 0x3fff;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDDC, &ReadingData);
				PlpDSize = ReadingData & 0xFFFFFF;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDD8, &ReadingData);
				PlpDStart = ReadingData & 0xFFFFFF;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDFC, &ReadingData);
				PlpDHtiInterSubframe = (ReadingData >> 20) & 0x1;
				PlpDHtiNumTiBlocks = ReadingData & 0xf;
				PlpDHtiFecBlocksMax = (ReadingData >> 4) & 0xfff;
				PlpDHtiCellInterleaver = (ReadingData >> 16) & 0x1;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE00, &ReadingData);
				PlpDHtiFecBlocks = ReadingData & 0xfff;

				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDE4, &ReadingData);
				plpDLdmInjectLvl = ReadingData & 0x1f;

				if (CtiDetectedPLPNo == 4) {
					ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDF4, &ReadingData);
					plpCtiFecBlockStart = ReadingData & 0x3FFFFF;
					plpCtiDepth = plpDCtiDepth;
					plpCtiStartRow = plpDCtiStartRow;
					PlpStart = PlpDStart;
					PlpFecType = PlpDFecType;
					PlpMod = PlpDMod;
					PlpSize = PlpDSize;
				}
			...
			//Write L1 info for SW mode - part1
			rtd_4bytes_i2c_mask(pDemod, 0xB816EE34, 0xE1E1E1E1, (PlpDHtiCellInterleaver << 28) | (PlpDHtiInterSubframe << 27) | (PlpDTiMode << 25) | (PlpCHtiCellInterleaver << 20) | (PlpCHtiInterSubframe << 19) | (PlpCTiMode << 17) | (PlpBHtiCellInterleaver << 12) | (PlpBHtiInterSubframe << 11) | (PlpBTiMode << 9) | (PlpAHtiCellInterleaver << 4) | (PlpAHtiInterSubframe << 3) | (PlpATiMode << 1));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EE38, 0x00000000, (PlpDMod << 28) | (PlpDFecType << 24) | (PlpCMod << 20) | (PlpCFecType << 16) | (PlpBMod << 12) | (PlpBFecType << 8) | (PlpAMod << 4) | (PlpAFecType));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE3C, (PlpBHtiFecBlocksMax << 20) | (PlpBHtiNumTiBlocks << 16) | (PlpAHtiFecBlocksMax << 4) | (PlpAHtiNumTiBlocks));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE40, (PlpDHtiFecBlocksMax << 20) | (PlpDHtiNumTiBlocks << 16) | (PlpCHtiFecBlocksMax << 4) | (PlpCHtiNumTiBlocks));

			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED28, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE44, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED2C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE48, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED30, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE4C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED34, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE50, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED38, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE54, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED3C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE58, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED40, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE5C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED44, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE60, ReadingData);

			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED70, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE64, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED74, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE68, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED78, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE6C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED7C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE70, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED80, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE74, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED84, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE78, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED88, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE7C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED8C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE80, ReadingData);

			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDB8, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE84, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDBC, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE88, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDC0, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE8C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDC4, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE90, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDC8, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE94, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDCC, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE98, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDD0, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EE9C, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDD4, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEA0, ReadingData);

			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE00, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEA4, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE04, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEA8, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE08, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEAC, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE0C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEB0, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE10, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEB4, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE14, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEB8, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE18, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEBC, ReadingData);
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE1C, &ReadingData);
			ReadingData = ((ReadingData & 0x0FFF0000) >> 4) | (ReadingData & 0xFFF);
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEC0, ReadingData);

			rtd_4bytes_i2c_mask(pDemod, 0xB816EEC4, 0x20202020, (PlpDLayer << 30) | (PlpDType << 28) | (PlpDCod << 24) | (PlpCLayer << 22) | (PlpCType << 20) | (PlpCCod << 16) | (PlpBLayer << 14) | (PlpBType << 12) | (PlpBCod << 8) | (PlpALayer << 6) | (PlpAType << 4) | (PlpACod));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEC8, (SwPlp0SubframeId << 24) | (PlpASize));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EECC, (SwPlp1SubframeId << 24) | (PlpBSize));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EED0, (SwPlp2SubframeId << 24) | (PlpBSize));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EED4, (SwPlp3SubframeId << 24) | (PlpBSize));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EED8, (PlpAScramblerType << 30) | (PlpATiExtendedInt << 29) | (plpALdmInjectLvl << 24) | (PlpAStart));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEDC, (PlpBScramblerType << 30) | (PlpBTiExtendedInt << 29) | (plpBLdmInjectLvl << 24) | (PlpBStart));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEE0, (PlpCScramblerType << 30) | (PlpCTiExtendedInt << 29) | (plpCLdmInjectLvl << 24) | (PlpCStart));
			rtd_4bytes_i2c_wr(pDemod, 0xB816EEE4, (PlpDScramblerType << 30) | (PlpDTiExtendedInt << 29) | (plpDLdmInjectLvl << 24) | (PlpDStart));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EEE8, 0xC000C000, (PlpBNumSubslices << 16) | (PlpANumSubslices));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EEEC, 0xC000C000, (PlpDNumSubslices << 16) | (PlpCNumSubslices));

			rtd_4bytes_i2c_mask(pDemod, 0xB816EEF0, 0xFF000000,  PlpASubsliceInterval);
			rtd_4bytes_i2c_mask(pDemod, 0xB816EEF4, 0xFF000000,  PlpBSubsliceInterval);
			rtd_4bytes_i2c_mask(pDemod, 0xB816EEF8, 0xFF000000,  PlpCSubsliceInterval);
			rtd_4bytes_i2c_mask(pDemod, 0xB816EEFC, 0xFF000000,  PlpDSubsliceInterval);
			...
			//rtd_4bytes_i2c_mask(pDemod, 0xB816EF18, 0xFFF8008F, (plpCtiStartRow << 8) | (plpCtiDepth << 4));
			//rtd_4bytes_i2c_mask(pDemod, 0xB816EF1C, 0xFFC00000, plpCtiFecBlockStart);

			//rtd_4bytes_i2c_mask(pDemod, 0xB816EF20, 0x80008000, (plpBFecBlockStart << 16) | (plpAFecBlockStart));
			//rtd_4bytes_i2c_mask(pDemod, 0xB816EF24, 0x80008000, (plpDFecBlockStart << 16) | (plpCFecBlockStart));


			realtek_r_atsc3_SoftwareReset(pDemod);
			L1ParsingDone_stime = tv_osal_time();

			while ((tv_osal_time() - L1ParsingDone_stime) <= 1000) {
				tv_osal_msleep(5);
				ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816E810, &ReadingData);
				L1ParsingDone = ReadingData & 0x3;
			...
			if (L1ParsingDone != 0x3) {
				RTK_DEMOD_ATSC3_WARNING("CtiDepth724_TiExtInter wait L1ParsingDone TimeOut !!!\n");
				*flag = 0xf;
				return FUNCTION_ERROR;
			}

			//parameter calculation for SW mode - part2
/*
			switch (plpCtiDepth) {
			case 0:
				Nrow = 512;
...
*/
			...
			u32DataTmp = plpCtiFecBlockStart + PlpStart;
			u32C0 = u32DataTmp - ((u32DataTmp / Nrow) * Nrow);

			u32DataTmp1 = plpCtiStartRow + u32C0;
			u32C1 = (u32DataTmp / Nrow) - (u32DataTmp1 - ((u32DataTmp1 / Nrow) * Nrow));
			u32C = (u32C1 * Nrow) + u32C0 - PlpStart;

			//RTK_DEMOD_ATSC3_INFO("[pre] plpCtiStartRow = %d, plpCtiFecBlockStart = %d, Nrow = %d, PlpSize = %d, PlpStart = %d\n", plpCtiStartRow, plpCtiFecBlockStart, Nrow, PlpSize, PlpStart);
			//RTK_DEMOD_ATSC3_INFO("u32C = %d, u32C0 = %d, u32C1 = %d\n", u32C, u32C0, u32C1);


			Ncell = ldpcN / (2 * (PlpMod + 1));
			Cnext = Ncell - ((PlpSize - u32C) - (((PlpSize - u32C) / Ncell) * Ncell));
			//RTK_DEMOD_ATSC3_INFO("Ncell = %d, Cnext = %d\n", Ncell, Cnext);

			ReadingData = (PlpSize - ((PlpSize / Nrow) * Nrow));
			//RTK_DEMOD_ATSC3_INFO("mod(plp_size, CTI_depth) ReadingData = %d\n", ReadingData);
			ReadingData += plpCtiStartRow;
			//RTK_DEMOD_ATSC3_INFO("mod(plp_size, CTI_depth)+ plpCtiStartRow  ReadingData = %d\n", ReadingData);
			plpCtiStartRow = ReadingData - ((ReadingData / Nrow) * Nrow);

			ReadingData = plpCtiStartRow + Cnext;
			ReadingData = ReadingData - ((ReadingData / Nrow) * Nrow);
			plpCtiFecBlockStart = Cnext + Nrow * ReadingData;
			//RTK_DEMOD_ATSC3_INFO("[post] plpCtiStartRow = %d, plpCtiFecBlockStart = %d\n", plpCtiStartRow, plpCtiFecBlockStart);


			//Write L1 info for SW mode - part2
			rtd_4bytes_i2c_mask(pDemod, 0xB816EF18, 0xFFF8008F, (plpCtiStartRow << 8) | (plpCtiDepth << 4));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EF1C, 0xFFC00000, plpCtiFecBlockStart);
			rtd_4bytes_i2c_mask(pDemod, 0xB816EF20, 0x80008000, (plpBFecBlockStart << 16) | (plpAFecBlockStart));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EF24, 0x80008000, (plpDFecBlockStart << 16) | (plpCFecBlockStart));
			...
			rtd_4bytes_i2c_mask(pDemod, 0xB816146C, 0x3FFFFFFF, (0x3 << 30));
			rtd_4bytes_i2c_mask(pDemod, 0xB816EF18, 0x7FFFFFFF, (0x1 << 31));

			/*
						RTK_DEMOD_ATSC3_INFO("CtiDepth724_TiExtInter case was detected!! eable L1 SW mode~~~ Dump Enable\n");
						addrTmp = 0xB816EE34;
						for (tmpCnt = 0; tmpCnt < 100; tmpCnt++) {
							ret |= rtd_4bytes_i2c_rd(pDemod, addrTmp, &ReadingData);
							RTK_DEMOD_ATSC3_INFO("rtd_outl(0x%x, 0x%x);\n", addrTmp, ReadingData);
							addrTmp += 4;
							if (addrTmp > 0xB816EF24)
								break;
						}
						ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EE34, &ReadingData);
						RTK_DEMOD_ATSC3_INFO("rtd_outl(0xB816EE34, 0x%x);\n", ReadingData);
						ret |= rtd_4bytes_i2c_rd(pDemod, 0xB8161C04, &ReadingData);
						RTK_DEMOD_ATSC3_INFO("rtd_outl0xB8161C04, 0x%x);\n", ReadingData);
						ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816146C, &ReadingData);
						RTK_DEMOD_ATSC3_INFO("rtd_outl(0xB816146C, 0x%x);\n", ReadingData);
						ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EF18, &ReadingData);
						RTK_DEMOD_ATSC3_INFO("rtd_outl(0xB816EF18, 0x%x);\n", ReadingData);
						RTK_DEMOD_ATSC3_INFO("CtiDepth724_TiExtInter case was detected!! eable L1 SW mode~~~ Dump End\n");
			*/

			RTK_DEMOD_ATSC3_INFO(" CtiDepth724_TiExtInter case was detected!! eable L1 SW mode~~~\n");
...
int  realtek_r_atsc3_restore_default_setting(ATSC3_DEMOD_MODULE *pDemod)
{
	...
	ret |= rtd_4bytes_i2c_wr(pDemod, 0xB8161614, 0x7E907E90); //realtek_r_atsc3_L1BD_latency_modify & realtek_r_atsc3_L1dSwParsingTimeModify

	ret |= rtd_4bytes_i2c_mask(pDemod, 0xB816146C, 0x3FFFFFFF, 0x0);  //restore default vaule for realtek_r_atsc3_ctiDepth724_TiExtInter_detect
	ret |= rtd_4bytes_i2c_mask(pDemod, 0xB816EF18, 0x7FFFFFFF, 0x0);   //restore default vaule for realtek_r_atsc3_ctiDepth724_TiExtInter_detect
	...
	pDemod->mAtsc3_L1D_plpSize_change_flag = 0xf;
	pDemod->mPlpSize[0] = 0;
	pDemod->mPlpSize[1] = 0;
	pDemod->mPlpSize[2] = 0;
	pDemod->mPlpSize[3] = 0;
	...
	if (demodLock == 1) {
		...
		if (plpCnt == 1) {
			if (PlpUpdateSubEnMsb != 0) {
				...
				rtd_4bytes_i2c_wr(pDemod, 0xb816e80c, 0x00020c12); // plp_suben_lsb
...
int  realtek_r_atsc3_L1dSwParsingTimeModify(ATSC3_DEMOD_MODULE *pDemod)
{

	int ret = FUNCTION_SUCCESS;
	unsigned int l1dSwParsingTime = 0x7E90;
	unsigned int u32DataTmp = 0;
	unsigned int L1ParsingDone = 0;
	unsigned int L1dFecType = 0, L1dSizeBytes = 0;
	unsigned int L1dDataSizeBit = 0;
	unsigned int Modulation = 0, Kseg = 0, NL1D_FECFRAME = 0, Ksig = 0, Nouter = 0, Kldpc = 0, Nparity = 0, Nrepeat = 0, Npunc_temp = 0, NFEC_temp = 0, NFEC_final = 0, Npunc = 0, NFEC = 0;
	unsigned int L1D_total_cells = 0, DemapTime = 0, BitDeinterleaverTime = 0, marginTime = 500;
	...
	if (L1ParsingDone == 0x3) {
		...
		ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816E814, &u32DataTmp);
		L1dFecType = (u32DataTmp >> 20) & 0x7;
		L1dSizeBytes = u32DataTmp & 0x1fff;

		L1dDataSizeBit = L1dSizeBytes * 8;
		...
		NL1D_FECFRAME = (L1dDataSizeBit / Kseg * Kseg == L1dDataSizeBit) ? L1dDataSizeBit / Kseg : L1dDataSizeBit / Kseg + 1;

		if (NL1D_FECFRAME > 1)
			return FUNCTION_ERROR;

		Ksig = (L1dSizeBytes / NL1D_FECFRAME * NL1D_FECFRAME == L1dSizeBytes) ? L1dSizeBytes / NL1D_FECFRAME : L1dSizeBytes / NL1D_FECFRAME + 1;
		Ksig *=  8;
		Nouter = Ksig + 168;

		switch (L1dFecType) {
		case 0: //MODE 1
			Nrepeat = (61 * Nouter / 16) * 2 - 508;
			Npunc_temp = (Kldpc - Nouter) * 7 / 2;
			break;
		case 1: //MODE 2
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 2 + 6036;
			break;
		case 2: //MODE 3
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 11 / 16 + 4653;
			break;
		case 3: //MODE 4
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 29 / 32 + 3200;
			break;
		case 4: //MODE 5
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 3 / 4 + 4284;
			break;
		case 5: //MODE 6
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 11 / 16 + 4900;
			break;
		case 6: //MODE 7
			Nrepeat = 0;
			Npunc_temp = (Kldpc - Nouter) * 49 / 256 + 8246;
			break;
		}


		NFEC_temp = Nouter + Nparity - Npunc_temp;
		NFEC_final = (NFEC_temp / Modulation * Modulation == NFEC_temp) ? NFEC_temp / Modulation : NFEC_temp / Modulation + 1;
		NFEC_final *= Modulation;
		Npunc = Npunc_temp + NFEC_temp - NFEC_final;
		NFEC = Nouter + Nparity + Nrepeat - Npunc;

		L1D_total_cells = NL1D_FECFRAME * NFEC / Modulation;

		switch (L1dFecType) {
		case 0: //MODE 1
			DemapTime = L1D_total_cells * 3 / NL1D_FECFRAME;
			break;
		case 1: //MODE 2
			DemapTime = L1D_total_cells * 3 / NL1D_FECFRAME;
			break;
		case 2: //MODE 3
			...
			break;
		case 3: //MODE 4
			DemapTime = L1D_total_cells * 4 / NL1D_FECFRAME;
			break;
		case 4: //MODE 5
			DemapTime = L1D_total_cells * 6 / NL1D_FECFRAME;
			break;
		case 5: //MODE 6
			DemapTime = L1D_total_cells * 8 / NL1D_FECFRAME;
			break;
		case 6: //MODE 7
			DemapTime = L1D_total_cells * 8 / NL1D_FECFRAME;
			break;
		...
		BitDeinterleaverTime = NFEC;

		l1dSwParsingTime = (DemapTime + BitDeinterleaverTime + 16000 + 4600) * NL1D_FECFRAME - L1D_total_cells + marginTime + L1dSizeBytes * 16;


		rtd_4bytes_i2c_mask(pDemod, 0xb8161614, 0x0000FFFF, (l1dSwParsingTime & 0xffff) << 16);
		realtek_r_atsc3_SoftwareReset(pDemod);
		RTK_DEMOD_ATSC3_INFO("PreL1dSwParsingTime change to = 0x%x (default: 0x7E90) \n", l1dSwParsingTime);
...
int  realtek_r_atsc3_L1D_plpSize_change_check(ATSC3_DEMOD_MODULE * pDemod)
{

	unsigned int plpSizePost[4] = {0};
	unsigned int plpSizeChangeFlag = 0xf;
	unsigned int ReadingData = 0;
	unsigned char PLPCnt = 0;
	unsigned int L1ParsingDone = 0;
	int i = 0;
	...
	if (L1ParsingDone == 0x3) {
		...
		if (ReadingData == 0x80000000) {   //PLP auto mode
			PLPCnt = 1;
		} else { //PLP manual mode
		...
		if (PLPCnt >= 1) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED04, &ReadingData);
			plpSizePost[0] = ReadingData & 0xffffff;

			if (pDemod->mPlpSize[0] != 0)
				plpSizeChangeFlag = (plpSizePost[0] != pDemod->mPlpSize[0]) ? 1 : 0xf;
			pDemod->mPlpSize[0] = plpSizePost[0];
		}

		if (PLPCnt >= 2) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED4C, &ReadingData);
			plpSizePost[1] = ReadingData & 0xffffff;

			if (pDemod->mPlpSize[1] != 0 && plpSizeChangeFlag != 1)
				plpSizeChangeFlag = (plpSizePost[1] != pDemod->mPlpSize[1]) ? 1 : 0xf;
			pDemod->mPlpSize[1] = plpSizePost[1];
		}

		if (PLPCnt >= 3) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816ED94, &ReadingData);
			plpSizePost[2] = ReadingData & 0xffffff;

			if (pDemod->mPlpSize[2] != 0 && plpSizeChangeFlag != 1)
				plpSizeChangeFlag = (plpSizePost[2] != pDemod->mPlpSize[2]) ? 1 : 0xf;
			pDemod->mPlpSize[2] = plpSizePost[2];
		}

		if (PLPCnt >= 4) {
			ret |= rtd_4bytes_i2c_rd(pDemod, 0xB816EDDC, &ReadingData);
			plpSizePost[3] = ReadingData & 0xffffff;

			if (pDemod->mPlpSize[3] != 0 && plpSizeChangeFlag != 1)
				plpSizeChangeFlag = (plpSizePost[3] != pDemod->mPlpSize[3]) ? 1 : 0xf;
			pDemod->mPlpSize[3] = plpSizePost[3];
		}

		if (plpSizeChangeFlag == 1) {
			realtek_r_atsc3_L1dSwParsingTimeModify(pDemod);
			RTK_DEMOD_ATSC3_INFO("PLP SIZE changed by frame !!\n");
		}

		//RTK_DEMOD_ATSC3_INFO("[plpSize_change_check]PLPCnt = %d, PLP SIZE = %d, %d, %d, %d!!\n", PLPCnt, plpSize[0], plpSize[1], plpSize[2], plpSize[3]);
		pDemod->mAtsc3_L1D_plpSize_change_flag = plpSizeChangeFlag;
	}
	return ret;

}
int  realtek_r_atsc3_GI4864_SettingModify(ATSC3_DEMOD_MODULE * pDemod, unsigned char* flag)
{

	int ret = FUNCTION_SUCCESS;
	unsigned int u32DataTmp = 0;
	unsigned int L1ParsingDone = 0;
	unsigned int L1BNumSubframes = 0, avgCpEn = 0;
	unsigned int SubGI = 0;
	unsigned char GI4864Flag = 0;
	unsigned int u32LoopCnt = 0, addrTmp = 0;
	...
	*flag = 0xf;
	...
	if (L1ParsingDone == 0x3) {
		...
		ret |= rtd_4bytes_i2c_rd(pDemod, 0xb816C990, &u32DataTmp);
		avgCpEn = u32DataTmp  & 0x1;

		ret |= rtd_4bytes_i2c_rd(pDemod, 0xb816E834, &u32DataTmp);
		L1BNumSubframes = (u32DataTmp >> 16) & 0xff;

		addrTmp = 0xB816E904;
		for (u32LoopCnt = 0; u32LoopCnt <= L1BNumSubframes; u32LoopCnt++) {

			ret |= rtd_4bytes_i2c_rd(pDemod, addrTmp, &u32DataTmp);
			SubGI = u32DataTmp  & 0xF;
			if (SubGI == 0xC) {
				GI4864Flag = 1;
				break;
			}
		...
		if ((GI4864Flag == 1) && (avgCpEn == 1)) {
			ret |= rtd_4bytes_i2c_mask(pDemod, 0xb816C990, 0xFFFFFFFE, 0x0);  //params_average_cp_en disable
			ret |= rtd_4bytes_i2c_wr(pDemod, 0xb8160014, 0x00000001);//0x0005 = 0x01//Reset DSP
			ret |= rtd_4bytes_i2c_wr(pDemod, 0xb8160014, 0x00000000);//0x0005 = 0x00//Start DSP
			RTK_DEMOD_ATSC3_INFO("GI is 4864. disable CP average and reset demod ~~ \n");
		}
