...
)
{
	...
	tr_ld_o = DVBC_INRX_MO2_1_get_inrx_mo2_12_7(rtd_inl(DVBC_INRX_MO2_1_reg));//After merlin6
...
)
{
	...
	if (((PosToneMaxIdx[0] < 3505) && (PosToneMaxIdx[0] > 3465)) || ((PosToneMaxIdx[0] < 3308) && (PosToneMaxIdx[0] > 3268))) {SrsWeight[0] = 6;}
	if (((NegToneMaxIdx[0] < 3505) && (NegToneMaxIdx[0] > 3465)) || ((PosToneMaxIdx[0] < 3308) && (PosToneMaxIdx[0] > 3268))) {SrsWeight[0] = 6;}
	REALTEK_R_QAM_BASE_INFO("[NBD&SYS] PosToneMaxIdx[0] = %u, NegToneMaxIdx[0] = %u, SrsWeight[0] = %u \n", PosToneMaxIdx[0], NegToneMaxIdx[0], SrsWeight[0]);
...
)
{
	...
	U32BITS dagc_msb = 0;
	unsigned char csr_th = 0, csr_range = 0;
	...
	if(pDemod->TunerType == QAM_TUNER_ALTOBEAM_ATBM253) //For ATB253 disable PAPR CFO
	{
		rtd_maskl(DVBC_CCFO_PAPR_reg, ~(DVBC_CCFO_PAPR_opt_ccfo_max_papr_en_mask), DVBC_CCFO_PAPR_opt_ccfo_max_papr_en(0x0)); //disable papr ccfo
		csr_th = pDemod->CSRTH;
		csr_range = pDemod->CSRFindRange;
		rtd_maskl(DVBC_CSRS_CSR_THD_WEIGHT_reg, ~(DVBC_CSRS_CSR_THD_WEIGHT_reg_csrs_csr_threshold_weight_mask), DVBC_CSRS_CSR_THD_WEIGHT_reg_csrs_csr_threshold_weight(csr_th)); //reset csr th
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg_fsrs_search_range_half_add_11_8_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg_fsrs_search_range_half_add_11_8(0x0)); //[11:8]larger FSR search range half add
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg_fsrs_search_range_half_add_7_0_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg_fsrs_search_range_half_add_7_0(csr_range)); //[7:0]FSR search range half add 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg_fsrs_search_range_half_sub_11_8_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg_fsrs_search_range_half_sub_11_8(0x0)); //[11:8]larger FSR search range half sub
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg_fsrs_search_range_half_sub_7_0_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg_fsrs_search_range_half_sub_7_0(csr_range)); //[7:0] FSR search range half sub 0x72=500K 0x89=600K 0xe4=1000K 0xcd=900K 0x94=650K 0xa0=700K 0xab=750K
	}
	else
	{
		...
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg_fsrs_search_range_half_add_11_8_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_1_reg_fsrs_search_range_half_add_11_8(0x0));
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg_fsrs_search_range_half_add_7_0_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_reg_fsrs_search_range_half_add_7_0(0xa0));
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg_fsrs_search_range_half_sub_11_8_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB1_reg_fsrs_search_range_half_sub_11_8(0x0));
		rtd_maskl(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg, ~(DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg_fsrs_search_range_half_sub_7_0_mask), DVBC_OPT_FSRS_SEARCH_RANGE_HALF_SUB_reg_fsrs_search_range_half_sub_7_0(0xab));
	}
...
AutoModeBegin:
	...
	*pNosig = 0;
	...
	while ((tv_osal_time() - stime) < 300)
	{
		...
		pDemod->GetDiAgc(pDemod, &dagc_msb);
		//REALTEK_R_QAM_BASE_INFO("[AutoMode] srs_nbd_done=%u, ccfo_done=%u, fsm_state=%u, tr_ld_o=%u, srs_state=0x%x, time="PT_U32BITS"\n", srs_nbd_done, ccfo_done, fsm_state_o, tr_ld_o, srs_state, tv_osal_time() - stime);
		REALTEK_R_QAM_BASE_INFO("\033[1;32;33m" "[AutoMode] fsm_state=%u, tr_threshold=%u, automode_srs_cnt=%u, dagc_msb=0x"PT_H32BITS"\n" "\033[m", fsm_state_o, tr_threshold, automode_srs_cnt, dagc_msb);
		...
		else if(srs_nbd_done == 0x1 && ccfo_done == 0x1 && fsm_state_o >= 4 && fsm_state_o < 14)
		{
			...
			if(tr_ld_o < tr_threshold) //default tr_threshold = 32
			{
				...
				if(pDemod->TunerType == QAM_TUNER_ALTOBEAM_ATBM253) //ATB253 uses ori CCFO
				{
	...
	if(fsr_KHz == 0)
	{
		REALTEK_R_QAM_BASE_INFO("FSR not found in automode\n");
		*pNosig = 1;
		goto AutoModeEnd;
	}
