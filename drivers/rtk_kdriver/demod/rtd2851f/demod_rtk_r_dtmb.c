...
int REALTEK_R_DTMB_AcquireSignal(DEMOD* pDemodKernel, unsigned char WaitSignalLock)
{
	...
	U32BITS maxCorr = 0, peakSaveR = 0, aveSaveR = 0, papr10 = 0, TpsLockStatus = 0, hiSnrTpsLockCnt = 0;
	...
	while ((tv_osal_time() - stime) <= 380) { //250
		pDemod->IsNoSig(pDemod, &no_signal);
		tv_osal_msleep(5);
		//if (no_signal == 0) break;
	}

	pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_MAX_CORR, &maxCorr);
	if ((no_signal == 0) && (maxCorr < 0x20 * 16)) {
		pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_PEAK_SAVE_R, &peakSaveR);
		pDemod->RegAccess.Addr16Bit.GetRegBits(pDemod, DTMB_AVE_SAVE_R, &aveSaveR);
	...
	REALTEK_R_DTMB_INFO("signal lock = %d, m_hiSnrTpsLockStatus= %u, time=%ld\n", lock, pRealtekRDTMB->m_hiSnrTpsLockStatus, tv_osal_time() - stime);
	pDemodKernel->Unlock(pDemodKernel);
...
)
{
	...
	if (pDriver) {
		...
		if (RTK_R_DTMB_DDR_SIZE) {
			...
			memoyExt = (carvedout_en == FALSE) ? 0x100000 : 0;
				
			ret = DtvDemodMemRemappingInt(RTK_R_DTMB_DDR_SIZE+memoyExt, &pDriver->ShareMemoryPhysicalAddress, &AllocateSize);
			...
			if (ret != TUNER_CTRL_OK) {
				...
				return NULL;
                      ...
                      if (carvedout_en == FALSE)
				pDriver->ShareMemoryPhysicalAddress = (pDriver->ShareMemoryPhysicalAddress + memoyExt) & 0xfff00000;
