...
int REALTEK_R_ATSC_Init(DEMOD* pDemodKernel)
{
	...
	if (IsAtsc3McmPackage(RTK_DEMOD_MODE_ATSC) == 0) {
		...
		ret = DtvDemodMemRemappingInt(RTK_R_ATSC1_DDR_SIZE, &ShareMemoryPhysicalAddress, &AllocateSize);
		if (ret != TUNER_CTRL_OK) {
			REALTEK_R_ATSC_WARNING("allocate share memory failed\n");
			goto init_fail;
		}


		REALTEK_R_ATSC_INFO("allocate share memory(atsc) - Physical_addr=0x%08lx, size=0x%08x\n", ShareMemoryPhysicalAddress, AllocateSize);
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_R_ATSC_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
	unsigned long cur_time;
	unsigned char lock;
