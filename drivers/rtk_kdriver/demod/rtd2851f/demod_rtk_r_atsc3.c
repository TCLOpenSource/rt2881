...
unsigned char Atsc3CtiDepth724TiExtInterFlag = 0xf;
unsigned char Atsc3Gi4864Flag = 0xf;
...
int REALTEK_R_ATSC3_SetTVSysEx(DEMOD* pDemodKernel, TV_SYSTEM sys, const TV_SIG_INFO* pSigInfo)
{
	...
	*/
	...
	Atsc3CtiDepth724TiExtInterFlag = 0xf;
...
int REALTEK_R_ATSC3_AcquireSignalThread(DEMOD* pDemodKernel)
{
	...
	int SyncLockOnceFlag = 0, L1LockOnceFlag = 0;
	unsigned int atsc3_timeout = 3000, NoSignalTimeOut = 700, atsc3_prelock_timeout = 1000;
	unsigned char ATSC3config2MplpCtiFlag = 0xf;
	unsigned char Atsc3_1PlpElOnly_multisub = 0xf;
	unsigned char LdmEnhanvedLayerOnlyFlag = 0xf;
	...
	while ((tv_osal_time() - acquire_sig_stime) <= atsc3_timeout) {
		...
		SyncLockFlag |= SyncLockOnceFlag;
		L1LockFlag |= L1LockOnceFlag;
		SyncLockOnceFlag = SyncLockFlag ?SyncLockFlag : SyncLockOnceFlag; 
		L1LockOnceFlag = L1LockFlag ? L1LockFlag : L1LockOnceFlag;
		...
		if (Atsc3_L1BD_latency_modify_flag == 0xf) {
			...
			if (Atsc3_L1BD_latency_modify_flag == 0x1) {
				atsc3_timeout = 4000;
				NoSignalTimeOut = 800;
				atsc3_prelock_timeout = 1400;
			}	
		}

		if (Atsc3Gi4864Flag == 0xf) {
			realtek_r_atsc3_GI4864_SettingModify(pDemod, &Atsc3Gi4864Flag);
			if (Atsc3Gi4864Flag == 0x1) {
				atsc3_timeout = 4000;
				NoSignalTimeOut = 800;
				atsc3_prelock_timeout = 1400;
			}	
		...
		if (Atsc3CtiDepth724TiExtInterFlag == 0xf)
			realtek_r_atsc3_ctiDepth724_TiExtInter_detect(pDemod, &Atsc3CtiDepth724TiExtInterFlag, &Atsc3PlpAll, &Atsc3PlpLlsOnly);
		...
		if (LdmEnhanvedLayerOnlyFlag == 0xf) {
			realtek_r_atsc3_LDM_EnhancedLayerOnly_detect(pDemod, &LdmEnhanvedLayerOnlyFlag);
			if (LdmEnhanvedLayerOnlyFlag == 0x1) {
				atsc3_prelock_timeout = 1500;
				atsc3_timeout = 4000;
			}
		}

		if (pDemod->mAtsc3_L1D_plpSize_change_flag == 0xf) {
			realtek_r_atsc3_L1D_plpSize_change_check(pDemod);
			if (pDemod->mAtsc3_L1D_plpSize_change_flag == 1) {
				atsc3_timeout = 4000;
				NoSignalTimeOut = 1400;
				atsc3_prelock_timeout = 2000;
				SyncLockFlag = 0;
				FecLockFlag = 0;
				L1LockFlag = 0;
				SyncLockOnceFlag = 0;
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_R_ATSC3_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
	unsigned long cur_time;
	...
	if (pRealtekRATSC3->m_acquire_sig_en) {
		...
		if (LockStatus == 0) {
			...
			if ((cur_time > pRealtekRATSC3->m_autotune_VVPatchCheckTime) || ((pRealtekRATSC3->m_autotune_VVPatchCheckTime - cur_time) > AUTOTUNE_CHECK_VV_PATCH_INTERVAL_MS)) {
				...
				if ((Atsc3Vv526Flag != 0) || (Atsc3KvvCrossSymbFlag != 0) || (Atsc3mSubFrameFlag == 1) || (Atsc32plpHtiTiExtInterFlag == 1) || (Atsc3CtiDepth724TiExtInterFlag != 0xf) || (pDemod->mAtsc3_L1D_plpSize_change_flag == 1) || (Atsc3Gi4864Flag == 1)) {
					realtek_r_atsc3_restore_default_setting(pDemod);
...
int REALTEK_R_ATSC3_GetPLPInfo(DEMOD* pDemodKernel, TV_SIG_INFO* pInfo, unsigned char Atsc3MPlpMode)
{
	...
	if ((Atsc32plpHtiTiExtInterFlag == 1) || (Atsc3CtiDepth724TiExtInterFlag == 1)) {
