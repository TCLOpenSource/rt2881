	...
	if (pDemodKernel->m_pTuner) {
		...
		if (pDemodKernel->m_pTuner->GetParam(pDemodKernel->m_pTuner, TV_SYS_OPENCABLE_64, &Param) == TUNER_CTRL_OK) {
			...
			case TV_TUNER_RAFAEL_R842:
				REALTEK_R_QAM_INFO("Tuner id %u, TV_TUNER_RAFAEL_R842\n", Param.id);
				pRealtekRQAM->m_pTunerOptRegRbus = (REG_VALUE_ENTRY_RBUS*) R842_OPT_VALUE_QAM;
				pRealtekRQAM->m_TunerOptRegCnt = sizeof(R842_OPT_VALUE_QAM) / sizeof(REG_VALUE_ENTRY_RBUS);
...
int REALTEK_R_QAM_GetLockStatus(DEMOD* pDemodKernel, unsigned char* pLock)
{
	int ret = TUNER_CTRL_OK;
	int Val;
#ifdef PRINT_SNR_PKT_ERR_BY_DEMOD
	...
	if (*pLock == 1) {
		...
		if(pkt_cnt == 0) {
			pkt_cnt = 3;
			ret = pDemod->GetSnrDb(pDemod, &SnrNum, &SnrDen); //Get SNR
			snr = (SnrNum * 10000) / SnrDen;
			ret = DECODE_RET(pDemod->GetErrorRate(pDemod, 0, 800, &BerNum, &BerDen, &PerNum, &PerDen));
			REALTEK_R_QAM_INFO("Snr="PT_S32BITS"/10000 BerNum = "PT_U32BITS", BerDen = "PT_U32BITS", PerNum = "PT_U32BITS", PerDen = "PT_U32BITS"\n", snr, BerNum, BerDen, PerNum, PerDen);
		}
		pkt_cnt--;
#endif
		pRealtekRQAM->m_unlock_cnt = 0;
 ...
 * Retn : TUNER_CTRL_OK/TUNER_CTRL_FAIL
 *----------------------------------------------------------------------*/
int REALTEK_R_QAM_KernelAutoTune(DEMOD* pDemodKernel, unsigned char *pProcessEnFlag)
{
...
{
	REALTEK_R_QAM_DRIVER_DATA* pDriver = (REALTEK_R_QAM_DRIVER_DATA*) dvr_malloc(
