...
static unsigned char g_ir_init = 0;
/*
...
EXPORT_SYMBOL(ir_protocol);
static IRDA_BOOT_PARAMETER   ir_boot_parameter_settings[MAX_IRDA_BOOT_PARAMETER_NUM];

static volatile bool irda_pm_nosig_freezing = false;
static volatile uint32_t freezing_keycode = 0;
...
#ifdef CONFIG_DELAY_FIRST_REPEAT
    if(dataRepeat == 1 && firstRepeatInterval > 0 && firstRepeatTriggered == false && get_take_time_ns(PressRecvTick, get_cur_system_counter()) < firstRepeatInterval * NS_PER_MS) {
        IR_DBG("DELAY_FIRST_REPEAT %lluns, ignored for first repeat interval..\n", get_take_time_ns(PressRecvTick, get_cur_system_counter()));
        lastRecvTick = get_cur_system_counter();
	 return received;
    }
#endif
    {
        IR_DBG(" key Symbol frame [%lluns]\n", get_take_time_ns(lastRecvTick, get_cur_system_counter()));
        IR_DBG(" IRRP = [%08X].\n", regValue);
        lastRecvTick = get_cur_system_counter();
        if(ir_protocol == RC6) {
...
#ifdef CONFIG_RTK_KDRV_INPUT_DEVICE
            venus_ir_input_report_key(which,dataRepeat);
...
int IR_protocol_setting(int main1_protocol, int main0_protocol)
{
    ...
    case NEC:
        ...
        rtd_outl(REG_IR_PSR_UP_MAIN1,   0x80303060);
	...
	ir_repeat_interval = 0;
        ...
        break;
    case C03:
        rtd_outl(REG_IR_PSR_MAIN1,      0x27101034);
        rtd_outl(REG_IR_PER_MAIN1,      0x0C252525);
        rtd_outl(REG_IR_PSR_UP_MAIN1,   0x4630276E);
        rtd_outl(REG_IR_PER_UP_MAIN1,   0x40408040);
        rtd_outl(REG_IR_MAIN1,          0x0000021B);
        rtd_outl(REG_IR_DPIR_MAIN1,     0x004103ff);
    ...
    case RC6A:
        ...
        rtd_outl(REG_IR_MAIN1,          0x0000021B);
        rtd_outl(REG_IR_DPIR_MAIN1,     0x00d00d00);
        rtd_outl(REG_IR_CR_MAIN1,       0x0000a681);
        rtd_outl(REG_IR_DATA_NUM_MAIN1, 0x00000024);
        //HW decode int en
        rtd_outl(REG_IR_MAIN1,
                rtd_inl(REG_IR_MAIN1) | (1 << REG_IR_MAIN1_IRDVF_INT_SHIFT));
        break;
    ...
    case NEC:
        ...
        rtd_outl(REG_IR_PSR_UP,   0x80303060);
	...
	ir_repeat_interval = 0;
        ...
        break;
    case C03:
        rtd_outl(REG_IR_PSR,      0x27101034);
        rtd_outl(REG_IR_PER,      0x0C252525);
        rtd_outl(REG_IR_PSR_UP,   0x4630276E);
        rtd_outl(REG_IR_PER_UP,   0x40408040);
        rtd_outl(REG_IR_MAIN0,    0x0000021B);
        rtd_outl(REG_IR_DPIR,     0x004103ff);
    ...
    case RC6A:
        ...
        rtd_outl(REG_IR_MAIN0,    0x0000021B);
        rtd_outl(REG_IR_DPIR,     0x00d00d00);
        rtd_outl(REG_IR_CR,       0x0000a681);
        rtd_outl(REG_IR_DATA_NUM, 0x00000024);
        //HW decode int en
        rtd_outl(REG_IR_MAIN0,
                rtd_inl(REG_IR_MAIN0) | (1 << REG_IR_MAIN0_IRDVF_INT_SHIFT));
        break;
...
bool get_irda_pm_nosig_freezing(void)
{
    IR_DBG("[%s]:%d\n",__func__,irda_pm_nosig_freezing);
    return irda_pm_nosig_freezing;
}
void set_freezing_keycode(uint32_t skeycode)
{
    //IR_ERR("[%s]:%d\n",__func__,skeycode);
    freezing_keycode = skeycode;
}
uint32_t get_freezing_keycode(void)
{
    return freezing_keycode;
}

void ir_decode_key_on_off(bool on)
{
    IR_WARNING("ir_decode_key_on_off:%d\n",on);
    //ir_disable_irda_decode_key = ((on ==ON) ? 0 : 1);
    irda_pm_nosig_freezing = ((on ==ON) ? false : true);
...
int ir_suspend_protocol(void)
{
    ...
    if(g_ir_init == 0){
        ...
        IR_Init(irda_proto_list[ir_support_protocols[0]].proto_standby_id, irda_proto_list[ir_support_protocols[1]].proto_standby_id, ON);
    }
...
int ir_core_prepare(struct device* dev)
{
    if((freezing_keycode == KEY_POWER || freezing_keycode == KEY_NETFLIX || freezing_keycode == KEY_YOUTUBE) && get_irda_pm_nosig_freezing() ){
        venus_ir_gen_keyupdown_event(freezing_keycode);
        set_freezing_keycode(0);
        IR_WARNING("[ir_core_prepare] send power key not go str\n");
        return -EBUSY;
    }
    IR_WARNING("[ir_core_prepare] done\n");
    suspend_flag = 0;
    //set_freezing_keycode(0);
    //ir_local_int_on_off(HW_MODE, OFF);
    //ir_local_int_on_off(RAW_MODE, OFF);
    //IR_ERR("[ir_core_prepare] keycode:%d, irda_pm_nosig_freezing:%d\n",freezing_keycode,irda_pm_nosig_freezing);
    return 0;
}
int ir_core_complete(struct device* dev)
{
    if((freezing_keycode == KEY_POWER || freezing_keycode == KEY_NETFLIX || freezing_keycode == KEY_YOUTUBE) && get_irda_pm_nosig_freezing() && suspend_flag == 0){
        venus_ir_gen_keyupdown_event(freezing_keycode);
        set_freezing_keycode(0);
        IR_WARNING("[irrc_complete] send power key not go str\n");
    }
    IR_WARNING("[ir_core_complete] done\n");
    ir_local_int_on_off(HW_MODE, ON);
    ir_local_int_on_off(RAW_MODE, ON);
    set_freezing_keycode(0);
    return 0;
...
int ir_core_suspend(struct device* dev)
{
    ir_local_int_on_off(HW_MODE, OFF);
    ir_local_int_on_off(RAW_MODE, OFF);
    mdelay(130);
    if((freezing_keycode == KEY_POWER || freezing_keycode == KEY_NETFLIX || freezing_keycode == KEY_YOUTUBE) && get_irda_pm_nosig_freezing() ){
        ...
        set_freezing_keycode(0);
        suspend_flag = 1;
        IR_WARNING("[irrc_suspnd] send power key not go str\n");
        return -EBUSY;
    }
    suspend_flag = 0;
    IR_WARNING("[irrc_core_suspnd] done\n");
    return 0;
}

int ir_core_resume(struct device* dev)
{
    return 0;
}
...
extern void venus_ir_input_dev_resume(void);
...
long ir_core_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
    ...
    case IOCTK_KBD_STATUS:
        ...
        venus_ir_input_report_key(MAIN1,0);
								...
								unsigned int *key_array, unsigned int arr_len)
{
	...
	for(i = 0; i < MAX_IRDA_BOOT_PARAMETER_NUM; i++) {
		if(ir_boot_parameter_settings[i].keylist_proto_id == proto || ir_boot_parameter_settings[1].keylist_proto_id != 0) {
			unsigned int n = 0;
			unsigned int size = (ir_boot_parameter_settings[i].keylist_len > arr_len)
								? arr_len : ir_boot_parameter_settings[i].keylist_len;
			for(n = 0; n < size; n++)
				key_array[index++] = ir_boot_parameter_settings[i].powerup_keylist[n];
		}
	}
	return index;
}
...
#if defined(CONFIG_RTK_KDRV_TV030_IR)
    snprintf(ir_boot_parameter_settings[0].keylist_remoter, 32, "venus_IR_input");
#else
...
int ir_proto_irda1_init(char *options)
{
    ...
    ir_support_protocols[1] = boot_sel_proto;
