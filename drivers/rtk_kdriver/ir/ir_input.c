...
extern bool get_irda_pm_nosig_freezing(void);
extern void set_freezing_keycode(uint32_t skeycode);
extern uint32_t get_freezing_keycode(void);
///////////////////////////////report mode////////////////////////////////////
static void venus_ir_input_report_mode_normal(u32 keycode,u32 custcode)
{
    ...
    if(get_irda_pm_nosig_freezing())
    {
        if((keycode == KEY_POWER   && get_freezing_keycode() != KEY_POWER) ||
           (keycode == KEY_YOUTUBE && get_freezing_keycode() != KEY_YOUTUBE) ||
           (keycode == KEY_NETFLIX && get_freezing_keycode() != KEY_NETFLIX)) {
            set_freezing_keycode(keycode);
        }
        IR_INFO("IR freezing not send date\n");
        return;
    }else{
        set_freezing_keycode(0);
    }
    IR_INFO("IR send key:0x%x,0x%x\n",keycode,custcode);
...
static bool venus_ir_key_has_ended(int which, uint32_t repkey)
{
	if(ir_protocol == NEC) {
		if(!repkey)
			return true;
...
void venus_ir_input_report_key(int which, uint32_t repkey)
{
    u32 keycode = 0;
    u32 custcode = 0;

    if(ir_poll_settings[which][0]) {
        if (data->prev_keycode && (data->prev_keycode != data->keycode || venus_ir_key_has_ended(which, repkey)) && hrtimer_is_queued(&keyup_timer)) {
...
#if defined(CONFIG_RTK_KDRV_TV030_IR )
    if (((value & 0xFF0000) == TV030_JP_FACTORY_CUSTOMER_CODE) || ((value & 0xFF00) == TV030_JP_NEW_FACTORY_CUSTOMER_CODE) ) {
...
void venus_ir_input_dev_resume(void)
{
    if(data->input_dev)
        input_set_timestamp(data->input_dev, ktime_set(0, 0));
}
