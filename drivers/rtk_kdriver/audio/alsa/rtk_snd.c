...
static DEFINE_SEMAPHORE(snd_open_count_sem);
int snd_open_ai_capture_count;	/* ai capture open count */
int snd_open_ao_capture_count;  /* ao capture open count */
static DEFINE_SEMAPHORE(snd_open_capture_count_sem);
...
struct rtk_snd_pcm {
	...
	long curr_time_ms;/* current time for calc time function*/
	long last_time_ms;
	long diff_time_ms;
	long total_time_ms;
	long max_diff;
...
void calc_time_call_me(struct rtk_snd_pcm *dpcm)
{
	struct snd_pcm_substream *substream = dpcm->substream;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	struct timespec64 t;
	ktime_get_real_ts64(&t);
	dpcm->curr_time_ms = t.tv_sec*1000 + t.tv_nsec/1000000;
#else
	struct timeval t;
	do_gettimeofday(&t);
	dpcm->curr_time_ms = t.tv_sec*1000 + t.tv_usec/1000;
#endif

	if(dpcm->last_time_ms) {
		dpcm->diff_time_ms = dpcm->curr_time_ms - dpcm->last_time_ms;
		dpcm->total_time_ms += dpcm->diff_time_ms;
		dpcm->call_cnt++;

		if (dpcm->diff_time_ms > 10 && dpcm->diff_time_ms < 20) {
			dpcm->cnt1++;
		} else if (dpcm->diff_time_ms > 20) {
			dpcm->cnt2++;
		}

		if (dpcm->diff_time_ms > dpcm->max_diff) {
			dpcm->max_diff = dpcm->diff_time_ms;
			rtd_pr_adsp_info("[calc_time]max %d\n", dpcm->max_diff);
		}

		if (dpcm->total_time_ms >= 1000) {
			rtd_pr_adsp_info("[calc_time]dev:%d,max:%dms,>10ms:%d,>20ms:%d,avg:%d\n", substream->pcm->device, dpcm->max_diff, dpcm->cnt1, dpcm->cnt2, dpcm->total_time_ms/dpcm->call_cnt);
			dpcm->total_time_ms = 0;
			dpcm->call_cnt = 0;
		}
	}
	dpcm->last_time_ms = dpcm->curr_time_ms;
}

static void dma_map_flush(struct rtk_snd_pcm *dpcm)
{
	struct snd_pcm_substream *substream = dpcm->substream;
	...
	unsigned long *retvalue)
{
	...
	command == ENUM_KERNEL_RPC_GET_MUTE_N_VOLUME) {
		...
		if (*retvalue != S_OK || ntohl(audio_ret->result) != S_OK) {
			rtd_pr_adsp_err("[ALSA] RPC S_OK fail\n");
			rtd_pr_adsp_err("[ALSA] retvalue %lx, result %x, command %lx, param1=%x, param2=%x\n",
				*retvalue, ntohl(audio_ret->result), command, param1, param2);
			ret = -1;
		}
	}
...
static int rtk_snd_init_ai_ring(struct snd_pcm_substream *substream)
{
	...
	if(dpcm->card->far_field_vir == NULL)
	...
	if (is_far_field(substream->pcm->device)) {
		...
		dpcm->card->far_field_ring_audio->numOfReadPtr = htonl(2);
		...
		dpcm->rd_idx = substream->pcm->device - DMIC_DEVICE;
	} else {
		dpcm->ring = (RINGBUFFER_HEADER *)((unsigned long) dpcm->hw_ring);
...
static int rtk_snd_create_ai_agent(struct snd_pcm_substream *substream)
{
	...
	down(&snd_open_capture_count_sem);
	if (snd_open_ai_capture_count >= MAX_AI_DEVICES) {
		rtd_pr_adsp_err("[ALSA] too more capture open %d\n", __LINE__);
		up(&snd_open_capture_count_sem);
		return -1;
	}
	up(&snd_open_capture_count_sem);
...
#if IS_ENABLED(CONFIG_RTK_WOV)
	if (snd_open_ai_capture_count == 0) {
		set_wov_attr_audio_record(1);
	}
#endif

	snd_open_ai_capture_count++;
	up(&snd_open_capture_count_sem);
...
static int rtk_snd_create_ao_capture_agent(struct snd_pcm_substream *substream)
{
	...
	down(&snd_open_capture_count_sem);
	if (snd_open_ao_capture_count >= MAX_AI_DEVICES) {
...
static int rtk_snd_ao_capture_hw_close(struct snd_pcm_substream *substream)
{
	...
	audioConfig.value[3] = 0x03;
	for (i = 0; i < 2; i++) {
		audioConfig.value[4 + i] = ENUM_AUDIO_DVOL_KMINUS72p0DB;
	}

	ret = rtkaudio_send_audio_config(&audioConfig);
	if(ret != S_OK){
		rtd_pr_adsp_err("%s ret!=S_OK, %x\n",__func__,ret);
		return -1;
	}
...
static void rtk_snd_pcm_capture_timer_start(struct snd_pcm_substream *substream)
{
	...
	if (is_far_field(substream->pcm->device)) {
		spin_lock(&dpcm->card->far_field_count_lock);
		dpcm->card->start_count++;
		spin_unlock(&dpcm->card->far_field_count_lock);
		dpcm->pcm_buf_pos = ntohl(dpcm->card->far_field_ring_audio->readPtr[dpcm->rd_idx]) - ntohl(dpcm->card->far_field_ring_audio->beginAddr);
		rtd_pr_adsp_info("[ALSA] numOfReadPtr %x, rd_idx %d, pcm_buf_pos %d, st_count %d, rp[0] %x, rp[1] %x\n",
			(unsigned int) dpcm->ring[0].numOfReadPtr, dpcm->rd_idx, dpcm->pcm_buf_pos, dpcm->card->start_count,
			ntohl(dpcm->card->far_field_ring_audio->readPtr[0]), ntohl(dpcm->card->far_field_ring_audio->readPtr[1]));
	}
...
static int rtk_snd_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (dpcm->ring_init == 1) {
		spin_lock(dpcm->pcm_lock);
		if (dpcm->pcm_buf_pos != 0) {
			rtd_pr_adsp_info("[ALSA] reprepare!! appl_ptr %d/%d, hw_ptr %d/%d\n", dpcm->appl_ptr, runtime->control->appl_ptr, dpcm->pcm_buf_pos, runtime->status->hw_ptr);
			spin_unlock(dpcm->pcm_lock);
			rtk_snd_close(substream);
			spin_lock(dpcm->pcm_lock);
			dpcm->appl_ptr = 0;
			...
			rtd_pr_adsp_info("[ALSA] reprepare!! reset shared ring wp %x, rp %x\n", ntohl(dpcm->ring[0].writePtr), ntohl(dpcm->ring[0].readPtr[0]));
			return 0;
		} else {
			rtd_pr_adsp_info("[ALSA] reprepare!! appl_ptr %d/%d, hw_ptr %d/%d\n", dpcm->appl_ptr, runtime->control->appl_ptr, dpcm->pcm_buf_pos, runtime->status->hw_ptr);
	...
	if (time_diff >= 3000) {
		if (dpcm->max_level != 0) {
			if (alsa_debug)
			{
				rtd_pr_adsp_debug("[ALSA] total_data_wb %ld, %ld~%ld bytes, time %ldms\n",
						dpcm->total_data_wb, dpcm->min_level, dpcm->max_level, time_diff);
			}
		}
		...
		if (alsa_debug)
		{
			...
			rtd_pr_adsp_debug("[ALSA] pcm_buf_pos %d, hw_ptr %ld, dma_addr %ld\n",
					dpcm->pcm_buf_pos, runtime->status->hw_ptr, (unsigned long) runtime->dma_addr);
			rtd_pr_adsp_debug("[ALSA] appl_ptr = %ld, appl_ptr = %ld\n", dpcm->appl_ptr, runtime->control->appl_ptr);
		}
	...
	long n_period, int write_zero)
{
	...
	if (is_32bits) {
		p_frame_bytes = runtime->channels * 4;
	} else {
...
#ifdef CONFIG_64BIT
			outer_inv_range(dvr_to_phys(dpcm->ring_p), dvr_to_phys(dpcm->ring_p + ntohl(dpcm->ring[0].size)));
#else
			outer_inv_range((phys_addr_t)dvr_to_phys(dpcm->ring_p), (phys_addr_t)dvr_to_phys(dpcm->ring_p + ntohl(dpcm->ring[0].size)));
#endif
		}
...
#ifdef CONFIG_64BIT
				outer_inv_range(ntohl(dpcm->ring[i].beginAddr), ntohl(dpcm->ring[i].beginAddr) + ntohl(dpcm->ring[i].size));
#else
				outer_inv_range((phys_addr_t)(ntohl(dpcm->ring[i].beginAddr)), (phys_addr_t)(ntohl(dpcm->ring[i].beginAddr) + ntohl(dpcm->ring[i].size)));
#endif
			}
		}
...
#endif
{
	...
	long n_data_size, n_data_size_other = 0;
	...
	if (n_data_frame > snd_pcm_capture_hw_avail(runtime)) {
		n_data_frame = snd_pcm_capture_hw_avail(runtime);
	}
	...
	if (n_data_frame >= runtime->period_size) {
		...
		if (is_far_field(substream->pcm->device)) {
			for (i = 0; i < ch; i++) {
				...
				if ((dpcm->card->start_count == 1) && (dpcm->ring[0].numOfReadPtr == htonl(2))) {
					if (dpcm->rd_idx == 1) {
						n_data_size_other = rtk_snd_get_ring_data(dpcm->ring, 0);

						if (n_data_size_other >= n_data_size) {
							/* continue update readPtr[0] when readPtr[0] timer stop */
							dpcm->ring[i].readPtr[0] =
								htonl((ulong) ntohl(dpcm->ring[i].beginAddr) + (dpcm->pcm_buf_pos % runtime->buffer_size) * dpcm->input_sample_bytes);
						}
					} else if (dpcm->rd_idx == 0) {
						n_data_size_other = rtk_snd_get_ring_data(dpcm->ring, 1);

						if (n_data_size_other >= n_data_size) {
							/* continue update readPtr[1] when readPtr[1] timer stop */
							dpcm->ring[i].readPtr[1] =
								htonl((ulong) ntohl(dpcm->ring[i].beginAddr) + (dpcm->pcm_buf_pos % runtime->buffer_size) * dpcm->input_sample_bytes);
						}
					}
				}
				  ...
				  void *buf, unsigned long bytes)
{
	...
	if ((dpcm->running != 0) || is_aaduio_device(substream->pcm->device)) {
		rtk_snd_pcm_playback_update_rp(dpcm);
...
static void snd_card_capture_runtime_free(struct snd_pcm_runtime *runtime)
{
	...
	if (dpcm->ring_p && do_free) {
		if (is_far_field(substream->pcm->device))
		{
			//==> Marked for [TCL2875P-7924], keep allocated shared memory to avoid system crash
			//dvr_free(dpcm->ring_p);
			//dpcm->card->far_field_vir = NULL;
			//<== Marked for [TCL2875P-7924], keep allocated shared memory to avoid system crash
			rtd_pr_adsp_err("[ALSA][%s] Disable ai ring buffer free\n", __func__);
			spin_lock(&dpcm->card->far_field_count_lock);
			dpcm->card->start_count = 0;
			spin_unlock(&dpcm->card->far_field_count_lock);
		}
		else
			dvr_free(dpcm->ring_p);
...
int rtk_snd_dmabuf_allocate_pages(struct snd_pcm_substream *substream, size_t size)
{
	...
	mars->snd_dmafd_handle[substream->pcm->device] = rtkaudio_dmabuf_fd_allocate(size, &mars->snd_dmabuf[substream->pcm->device], substream->pcm->device);
	...
	rtd_pr_adsp_info("[ALSA][%s] get dmabuf fd %d, v %lx, p %lx, dmabuf %lx\n",
		__func__, mars->snd_dmafd_handle[substream->pcm->device], (unsigned long) dmab->area, dmab->addr, mars->snd_dmabuf[substream->pcm->device]);
...
static int rtk_snd_dmabuf_free_pages(struct snd_pcm_substream *substream)
{
	...
	rtd_pr_adsp_info("[ALSA][%s] put dmabuf %lx\n", __func__, mars->snd_dmabuf[substream->pcm->device]);
	if (rtkaudio_dmabuf_free(mars->snd_dmabuf[substream->pcm->device])) {
		rtd_pr_adsp_err("[ALSA][%s] put dmabuf %lx, failed\n", __func__, mars->snd_dmabuf[substream->pcm->device]);
	}
	mars->snd_dmafd_handle[substream->pcm->device] = -1;
	mars->snd_dmabuf[substream->pcm->device] = NULL;
...
static int rtk_snd_playback_open(struct snd_pcm_substream *substream)
{
	...
	if (lowpower_offon) {
		rtd_pr_adsp_err("Do not try to open device %d while deivce is suspended ...\n", substream->pcm->device);
		return -EPERM;
	}
	mars = (struct snd_card_mars *)(substream->pcm->card->private_data);
...
static int rtk_snd_probe(struct platform_device *devptr)
{
	...
	mars->start_count = 0;
	spin_lock_init(&mars->far_field_count_lock);

	for (i = 0; i < MAX_PCM_DEVICES; i++) {
		mars->snd_dmafd_handle[i] = -1;
		mars->snd_dmabuf[i] = NULL;
	}
...
{
	rtd_pr_adsp_info("[ALSA] rtk_snd_pm_suspend!!!\n");
	flush_delayed_fput();
	return 0;
}
...
static int alsa_card_mars_init(void)
{
	...
	down(&snd_open_capture_count_sem);
	snd_open_ai_capture_count = 0;
	snd_open_ao_capture_count = 0;
	up(&snd_open_capture_count_sem);
	alsa_agent = -1;
