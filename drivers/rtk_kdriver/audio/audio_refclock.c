...
ReferenceClock* new_ReferenceClock(void)
{
    ...
    IPCWrite_U32((uint32_t*)&pObj->m_core->videoContext,   (uint32_t)-1);
    IPCWrite_U32((uint32_t*)&pObj->m_core->audioContext,   (uint32_t)-1);
    IPCWrite_S32((int32_t*)&pObj->m_core->videoEndOfSegment, (int32_t)-1);

    IPCWrite_U32((uint32_t*)&pObj->m_core->videoFreeRunThreshold, 0x7FFFFFFF);
    IPCWrite_U32((uint32_t*)&pObj->m_core->audioFreeRunThreshold, 0x7FFFFFFF);
    IPCWrite_U32((uint32_t*)&pObj->m_core->audioFreeRunThresholdToWait, 0x7FFFFFFF);

    IPCWrite_S32((int32_t*)&pObj->m_core->VO_Underflow, 0);
    IPCWrite_S32((int32_t*)&pObj->m_core->AO_Underflow, 0);

    IPCWrite_U32((uint32_t*)&pObj->m_core->clockMode, CLOCK_MODE_MISC);
