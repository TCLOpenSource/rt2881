...
static void setfixedPPInPin(UINT32 instanceID, UINT32 set_inpin)
{
    UINT32 res;
...
UINT32 PPAO_SetRefClock(Base* pBaseObj, UINT32 refClockPhyAddr)
{
    ...
    if(res != S_OK) {
        AUDIO_ERROR("[%s:%d][pBaseObj]RPC return != S_OK\n",__FUNCTION__,__LINE__);
        return res;
    }
...
Base* new_PPAO_QuickShow(PPAO_MODE mode, QuickShowCreateParam param)
{
    ...
    else
    {
        ...
        static int idx = 0;
        pObj->inPinID = idx;
        setfixedPPInPin(pObj->GetAgentID(pObj), pObj->inPinID);
        idx++;
#else
...
Base* new_PPAO(PPAO_MODE mode, UINT32 idx)
{
    /* object init */
    PPAO* pPPAOObj = NULL;
    Base* pObj = new_base();
    ...
    switch(idx) {
        case IDX_PPAO0: {
            char name[] = "Main-PPAO";
            pPPAOObj->ao_instanceID = CreateAgent(AUDIO_OUT);
            if(pPPAOObj->ao_instanceID == UNDEFINED_AGENT_ID)
            {
                pObj->Delete(pObj);
                return NULL;
            }
