...
KADP_STATUS_T KADP_AUDIO_SetModelHash(AUDIO_DUMMY_DATA model_hash)
{
    ...
    rpc_res = RTKAUDIO_RPC_TOAGENT_SET_MODEL_HASH_SVC(&model_hash, &retResult);
    if(rpc_res != S_OK) {
        ERROR("%s return fail\n", __FUNCTION__);
        return KADP_ERROR;
    }
...
{
    gUSER_CONFIG = config;
    if (fpAudio_SetAudioHWConfig != NULL) {
        fpAudio_SetAudioHWConfig(config);
    } else {
        AUDIO_ERROR("KADP_AUDIO_HW_IOCT_Init skip config setting");
    }
    return KADP_OK;
}
...
KADP_STATUS_T KADP_AUDIO_SetILDetectUpmixParam(AUDIO_VIRTUALX_ILDetectUpmix_PARAM_INFO *param){
	KADP_STATUS_T ret;
	...
	memset(&virx_cmd, 0, sizeof(virtualx_cmd_info));
	virx_cmd.type= ENUM_VIRTUALX_CMD_TYPE_ILDETECT_UPMIX_SETPARAM;
	virx_cmd.size= sizeof(*param);
	memcpy(&virx_cmd.data, param, sizeof(*param));
...
KADP_STATUS_T KADP_AUDIO_GetILDetectUpmixParam(AUDIO_VIRTUALX_ILDetectUpmix_PARAM_INFO *param){
	KADP_STATUS_T ret;
	...
	memset(&virx_cmd, 0, sizeof(virtualx_cmd_info));
	virx_cmd.type= ENUM_VIRTUALX_CMD_TYPE_ILDETECT_UPMIX_GETPARAM;
	virx_cmd.size= sizeof(*param);
	memcpy(&virx_cmd.data, param, sizeof(*param));
