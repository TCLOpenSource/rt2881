...
#include "audio_inc.h"
#include "rtkaudio_aidk_control.h"
...
static void rtkaudio_shutdown(struct platform_device *dev);
#endif
...
int lowpower_offon = 0;
EXPORT_SYMBOL(lowpower_offon);
...
char AudioGITHASH[8];
EXPORT_SYMBOL(AudioGITHASH);
char AudioCommonHASH[8];
...
static void *g_SPK_COUNTER_BUF_cached_vir_addr = NULL;
static wifi_speaker_info *g_SPK_COUNTER_BUF_vir_addr = NULL;
...
static int init_wifi_speaker_memory(ret_info_t *ret_info)
{
	AUDIO_CONFIG_COMMAND_RTKAUDIO cmd_info;
	void *vir_addr, *uncache_vir_addr;
	wifi_speaker_info buffer_info;// sizeof using
	int ret = 0;

	rtd_pr_adsp_info("INIT_SPK_COUNTER_SHARE_BUF\n");
	if (g_SPK_COUNTER_BUF_vir_addr == NULL)
	{
		vir_addr = dvr_malloc_uncached_specific(sizeof(buffer_info), GFP_DCU1, &uncache_vir_addr);
		if (!vir_addr) {
			ret = -EFAULT;
			rtd_pr_adsp_err("[rtkaudio] [%s:%d] INIT_SPK_COUNTER_SHARE_BUF cma malloc error\n", __FUNCTION__, __LINE__);
			return ret;
		}
		memset(uncache_vir_addr, 0, sizeof(buffer_info));

		cmd_info.msg_id = AUDIO_CONFIG_CMD_INIT_SPK_COUNTER_BUFFER;
		cmd_info.value[0] = dvr_to_phys(vir_addr);
		cmd_info.value[1] = sizeof(buffer_info);
		rtd_pr_adsp_info("[rtkaudio] [%s:%d] INIT_SPK_COUNTER_SHARE_BUF phy address %x sz::%d\n", __FUNCTION__, __LINE__, cmd_info.value[0], sizeof(void*) );
		ret = rtkaudio_send_audio_config(&cmd_info);

		if (ret != S_OK) {
			ret = -EFAULT;
			rtd_pr_adsp_err("[rtkaudio] [%s:%d] INIT_SPK_COUNTER_SHARE_BUF error\n", __FUNCTION__, __LINE__);
			dvr_free(vir_addr);
			return ret;
		}
		g_SPK_COUNTER_BUF_cached_vir_addr = vir_addr;
		g_SPK_COUNTER_BUF_vir_addr = uncache_vir_addr;
		if (ret_info) {
			ret_info->private_info[0] = (unsigned int)cmd_info.value[0];// phy address
			ret_info->private_info[1] = (unsigned int)sizeof(buffer_info);
		}

		rtd_pr_adsp_info("[rtkaudio] [%s:%d] INIT_SPK_COUNTER_SHARE_BUF cached %x, uncached %x\n",
			__FUNCTION__, __LINE__, g_SPK_COUNTER_BUF_cached_vir_addr, g_SPK_COUNTER_BUF_vir_addr);
	} else {
		rtd_pr_adsp_info("[rtkaudio] [%s:%d] INIT_SPK_COUNTER_SHARE_BUF already ready %p \n", __FUNCTION__, __LINE__, g_SPK_COUNTER_BUF_vir_addr);
	}

	return ret;
}

static void uninit_wifi_speaker_memory(void)
{
	AUDIO_CONFIG_COMMAND_RTKAUDIO cmd_info;
	wifi_speaker_info buffer_info;// sizeof using
	int ret = 0;

	rtd_pr_adsp_info("UNINIT_SPK_COUNTER_SHARE_BUF %p\n", g_SPK_COUNTER_BUF_vir_addr);
	if (g_SPK_COUNTER_BUF_vir_addr != NULL) {
		cmd_info.msg_id = AUDIO_CONFIG_CMD_INIT_SPK_COUNTER_BUFFER;
		cmd_info.value[0] = 0;
		cmd_info.value[1] = sizeof(buffer_info);
		...
		if (ret != S_OK) {
			ret = -EFAULT;
			rtd_pr_adsp_err("[rtkaudio] [%s:%d] UNINIT_SPK_COUNTER_SHARE_BUF error\n", __FUNCTION__, __LINE__);
		}
		dvr_free(g_SPK_COUNTER_BUF_cached_vir_addr);
	} else {
		rtd_pr_adsp_debug("UNINIT_SPK_COUNTER_SHARE_BUF address %p is NULL\n", g_SPK_COUNTER_BUF_vir_addr);
	}

	g_SPK_COUNTER_BUF_vir_addr = NULL;
	g_SPK_COUNTER_BUF_cached_vir_addr = NULL;
	rtd_pr_adsp_info("UNINIT_SPK_COUNTER_SHARE_BUF done\n");
}

static void get_spk_counter(void)
{
	int current_tick;
	int64_t spk_timestamp = 0, spk_timestamp_negative = 0;
	uint64_t spk_div_result = 0;
	...
	if (!g_SPK_COUNTER_BUF_vir_addr) {
		return;
	}

	if (g_SPK_COUNTER_BUF_vir_addr->base_tick == 0) {
		g_SPK_COUNTER_BUF_vir_addr->timestamp = 0;
		rtd_pr_adsp_info("[WIFI SPK] base tick is not ready\n");
		return;
	}

	/* current tick setting is 384000 tick per second */
	current_tick = rtd_inl(AUDIO_aout_counter_reg);
	spk_timestamp = current_tick - g_SPK_COUNTER_BUF_vir_addr->base_tick;
	...
	if (spk_timestamp >= 0) {
		spk_timestamp = spk_timestamp + g_SPK_COUNTER_BUF_vir_addr->overflow_count * 4294967296LL;

		spk_div_result = spk_timestamp * 64 * 1000000;
		do_div(spk_div_result, 24576); /*do spk_timestamp * 64 * 1000000 / 48 / 512*/

		/* every overflow need to add 17.5 us for reset time */
		g_SPK_COUNTER_BUF_vir_addr->timestamp = spk_div_result + g_SPK_COUNTER_BUF_vir_addr->overflow_count * 17500;
		g_SPK_COUNTER_BUF_vir_addr->system_time = (t.tv_sec * 1000000000) + t.tv_nsec;

		if (overflow) {
			overflow = 0;
		}
	} else {
		if ((0 - spk_timestamp) > (48 * 8 * 1000)) {
			/* means tick counter overflow, because base tick is bigger than current tick */
			if (!overflow) {
				g_SPK_COUNTER_BUF_vir_addr->overflow_count++;
				overflow = 1;

				rtd_pr_adsp_info("[WIFI SPK] tick count overflow %d\n", g_SPK_COUNTER_BUF_vir_addr->overflow_count);
			}
		} else {
			if (overflow) {
				overflow = 0;
		...
		if (spk_timestamp < 0) {
			spk_timestamp_negative = (-spk_timestamp * 64 * 1000000);
		} else {
			spk_timestamp_negative = spk_timestamp * 64 * 1000000;
		}

		spk_div_result = (uint64_t) spk_timestamp_negative;
		do_div(spk_div_result, 24576); /*do spk_timestamp * 64 * 1000000 / 48 / 512*/

		if (spk_timestamp < 0) {
			spk_timestamp_negative = (int64_t) spk_div_result;
			spk_timestamp_negative = -spk_timestamp_negative;
		}

		/* every overflow need to add 17.5 us for reset time */
		g_SPK_COUNTER_BUF_vir_addr->timestamp = spk_timestamp_negative + g_SPK_COUNTER_BUF_vir_addr->overflow_count * 17500;
		g_SPK_COUNTER_BUF_vir_addr->system_time = (t.tv_sec * 1000000000) + t.tv_nsec;
	}

	rtd_pr_adsp_info("Spk timestamp %lld ns (%lld ticks, base %d), overflow times %d, STC %lld\n",
		g_SPK_COUNTER_BUF_vir_addr->timestamp, spk_timestamp,
		g_SPK_COUNTER_BUF_vir_addr->base_tick, g_SPK_COUNTER_BUF_vir_addr->overflow_count, pli_getPTS());
}
...
uint32_t rtkaudio_alloc_uncached(int size, unsigned char** uncached_addr_info)
{
	...
	if(getpts() - start_t > 90*100) {
		rtd_pr_adsp_err("[%s %d] dvr_malloc_specific size %d takes %d ms\n", __func__, __LINE__, size, (int)(getpts() - start_t)/90);
	}
	if (!vir_addr) {
		rtd_pr_adsp_err("[%s %d] alloc memory fail %d\n", __func__, __LINE__, size);
		return -ENOMEM;
	}
	...
	rtd_pr_adsp_debug("rtkaudio_alloc(p:%lx,v:%px) for pid(%d)\n", phy_addr, vir_addr, buf_info->task_pid);
...
EXPORT_SYMBOL(rtkaudio_send_hdmi_fs);
...
EXPORT_SYMBOL(RHAL_AUDIO_HDMI_GetAudioReturnChannel);

void RHAL_AUDIO_HDMI_GetEnhancedAudioReturnChannel(bool *onoff)
{
	*onoff = eARC_offon;
}
EXPORT_SYMBOL(RHAL_AUDIO_HDMI_GetEnhancedAudioReturnChannel);
...
DTV_STATUS_T rhal_set_ioctl(HAL_AUDIO_RHAL_INFO_T rhalinfo)
{
	DTV_STATUS_T status;
	...
	switch(rhalinfo.type)
	{
		...
		case SET_SNDOUT_OPEN:
			...
			break;
		case SET_I2SIN_OPEN:
			status = RHAL_AUDIO_I2SIN_Open();
			break;
		case SET_UACIN_OPEN:
			status = RHAL_AUDIO_UACIN_Open();
			break;
		case SET_SPDIFIN_OPEN:
			status = RHAL_AUDIO_SPDIFIN_Open();
			break;
		case SET_TP_CLOSE:
		...
		case SET_SNDOUT_CLOSE:
			...
			break;
		case SET_I2SIN_CLOSE:
			status = RHAL_AUDIO_I2SIN_Close();
			break;
		case SET_UACIN_CLOSE:
			status = RHAL_AUDIO_UACIN_Close();
			break;
		case SET_SPDIFIN_CLOSE:
			status = RHAL_AUDIO_SPDIFIN_Close();
			break;
		...
		case SET_SNDOUT_DISCONNECT:
			...
			break;
		case SET_I2SIN_CONNECT:
			status = RHAL_AUDIO_I2SIN_Connect();
			break;
		case SET_I2SIN_DISCONNECT:
			status = RHAL_AUDIO_I2SIN_Disconnect();
			break;
		case SET_UACIN_CONNECT:
			status = RHAL_AUDIO_UACIN_Connect();
			break;
		case SET_UACIN_DISCONNECT:
			status = RHAL_AUDIO_UACIN_Disconnect();
			break;
		case SET_SPDIFIN_CONNECT:
			status = RHAL_AUDIO_SPDIFIN_Connect();
			break;
		case SET_SPDIFIN_DISCONNECT:
			status = RHAL_AUDIO_SPDIFIN_Disconnect();
			break;
		...
		case SET_MAIN_AUDIO_OUTPUT:
			...
			break;
		case SET_MULTIVIEW_MAIN_AUDIO_OUTPUT:
			status = RHAL_AUDIO_SetMultiViewMainAudioOutput(rhalinfo.audiomixIndex);
			break;
		...
		case SET_ATMOS_ENCODE:
			...
			break;
		case SET_PBOUT_MUTE:
			status = RHAL_AUDIO_SetPBOutMute(rhalinfo.enable, rhalinfo.u32var[0]);
			break;
		case SET_TSOUT_MUTE:
			status = RHAL_AUDIO_SetTSOutMute(rhalinfo.enable);
			break;
		...
		case SET_HPOUT_MUTE:
			...
			break;
		case SET_UACOUT1_MUTE:
			status = RHAL_AUDIO_SetUACOut1Mute(rhalinfo.enable);
			break;
		case SET_UACOUT2_MUTE:
			status = RHAL_AUDIO_SetUACOut2Mute(rhalinfo.enable);
			break;
		...
		case SET_HDMI_CLOSE_PORT:
			...
			break;
		case SET_DP_OPEN_PORT:
			status = RHAL_AUDIO_DP_OpenPort(rhalinfo.u32var[0]);
			break;
		case SET_DP_CLOSE_PORT:
			status = RHAL_AUDIO_DP_ClosePort(rhalinfo.u32var[0]);
			break;
		case SET_DP_CONNECT_PORT:
			status = RHAL_AUDIO_DP_ConnectPort(rhalinfo.u32var[0]);
			break;
		case SET_DP_DISCONNECT_PORT:
			status = RHAL_AUDIO_DP_DisconnectPort(rhalinfo.u32var[0]);
			break;
		...
		case SET_MIXER_INPUT_GAIN:
			...
			break;
		case SET_PRESCALE_VOL:
			status = RHAL_AUDIO_SetPreScaleVolume(rhalinfo.s32var, rhalinfo.u32var[0]);
			break;
		case SET_PBOUT_VOLUME:
			status = RHAL_AUDIO_SetPBOutVolume(rhalinfo.s32var, rhalinfo.u32var[0]);
			break;
		case SET_TSOUT_VOLUME:
			status = RHAL_AUDIO_SetTSOutVolume(rhalinfo.s32var);
			break;
		...
		case SET_SCARTOUT_VOLUME:
			...
			break;
		case SET_UACOUT1_VOLUME:
			status = RHAL_AUDIO_SetUACOut1Volume(rhalinfo.volume);
			break;
		case SET_UACOUT2_VOLUME:
			status = RHAL_AUDIO_SetUACOut2Volume(rhalinfo.volume);
			break;
		case SET_AD_VOLUME:
		...
		case SET_AEC_REFERENCE_SOURCE:
			...
			break;
		case SET_SNDOUT_MIXSRC:
			status = RHAL_AUDIO_SNDOUT_MixSrc(rhalinfo.u32var[0], rhalinfo.u32var[1]);
			break;
		case SET_SNDOUT_MAIN_DEVICE:
			status = RHAL_AUDIO_SetMainAudioSndoutDevice(rhalinfo.u32var[0]);
			break;
		case SET_HDMI_FULLCAP_PORT:
			status = RHAL_AUDIO_HDMI_SetFullCapPort(rhalinfo.u32var[0]);
			break;
		default:
			status = NOT_OK;
			break;
...
long rtkaudio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RTKAUDIO_IOC_SET_WITH_RETURN:
	{
		struct cntl_info_with_return info;

		if(cmd == RTKAUDIO_IOC_SET)
		{
			struct cntl_info info_old;
			if (copy_from_user((void*)&info_old, (const void __user*)arg, sizeof(info_old))) {
				ret = -EFAULT;
			}

			info.addr = info_old.addr;
			info.size = info_old.size;
			info.resource_type = info_old.resource_type;
        }
		else
		{
		...
		case SET_RHAL_INFO:
		{
			...
			int lowpower_count = 0;

			if (copy_from_user(&rhalinfo, (const void __user *) (long)info.addr, info.size))
			{
				rtd_pr_adsp_err("%s [SET_RHAL_INFO] copy_from_user fail\n",__FUNCTION__);
				ret = -EFAULT;
				break;
			}

			while (lowpower_offon) {
				msleep(10);
				lowpower_count++;

				if ((lowpower_count % 20) == 0) {
					rtd_pr_adsp_info("%s [SET_RHAL_INFO] wait lowpower mode resume\n");
				}
			}

			status = rhal_set_ioctl(rhalinfo);

			if(status != OK)
				ret = -EFAULT;
			break;
		}
		case SET_MIXSRC_INFO:
		{
			MIXSRC_INFO_T mixsrcinfo;
			DTV_STATUS_T status;

			if (copy_from_user(&mixsrcinfo, (const void __user *) (long)info.addr, info.size))
			{
				rtd_pr_adsp_err("%s SET_MIXSRC_INFO copy_from_user fail\n",__func__);
				ret = -EFAULT;
				break;
			}
			status = RHAL_AUDIO_SNDOUT_MixSrc(mixsrcinfo.sndoutdevice, mixsrcinfo.mixsrc);
			if(status != OK) {
                rtd_pr_adsp_err("%s RHAL_AUDIO_SNDOUT_MixSrc return fail.\n",__func__);
				ret = -EFAULT;
            }
			rtd_pr_adsp_info("%s called set device:%d with mixsrc:%d\n", __func__, mixsrcinfo.sndoutdevice, mixsrcinfo.mixsrc);
			break;
		}
		case SET_MAIN_DEVICE:
		{
			HAL_AUDIO_SNDOUT_T sndoutdevice;
			DTV_STATUS_T status;

			if (copy_from_user(&sndoutdevice, (const void __user *) (long)info.addr, info.size))
			{
				rtd_pr_adsp_err("%s SET_MAIN_DEVICE copy_from_user fail\n",__func__);
				ret = -EFAULT;
				break;
			}
			status = RHAL_AUDIO_SetMainAudioSndoutDevice(sndoutdevice);
			if(status != OK) {
                rtd_pr_adsp_err("%s RHAL_AUDIO_SetMainAudioSndoutDevice return fail.\n",__func__);
				ret = -EFAULT;
			}
			rtd_pr_adsp_info("%s called set main device:%d\n", __func__, sndoutdevice);
			break;
		}
		...
		case SET_MODEL_HASH:
		{
			...
			rpc_res = RTKAUDIO_RPC_TOAGENT_SET_MODEL_HASH_SVC(&hash, &retExecResult);
			if(rpc_res != S_OK)
				ret = -EFAULT;

			if(retExecResult != S_OK)
				ret = -EFAULT;
		...
		case AIDK_CMD_INFO:
		{
			aidk_cmd_info aidk_info;

			if (copy_from_user(&aidk_info, (const void __user *) (long) info.addr, sizeof(aidk_cmd_info)))
			{
				ret = -EFAULT;
			...
			ret = aidk_ioctl_set(aidk_info);

			break;
		}
#ifdef CONFIG_ARCH_RTK2851F
		case UNINIT_SPK_COUNTER_SHARE_BUF:
		{
			uninit_wifi_speaker_memory();
			break;
		}
#endif
		default:
		...
		if(cmd == RTKAUDIO_IOC_SET_WITH_RETURN)
		{
		    info.result = S_OK;
			if (copy_to_user((void __user *)arg, (void*) &info,  sizeof(info)))
			{
				rtd_pr_adsp_err("[rtkaudio] [%s:%d] copy to user failed\n", __FUNCTION__, __LINE__);
				ret = -EFAULT;
			}
		}
		break;
	...
	case RTKAUDIO_IOC_GET:
	{
		...
		case GET_RHAL_INFO:
		{
			...
			while (lowpower_offon) {
				...
				if ((lowpower_count % 20) == 0) {
					rtd_pr_adsp_info("%s [GET_RHAL_INFO] wait lowpower mode resume\n");
				}
			}
        ...
        case GET_PRIVATE_INFO:
        {
            ...
            parameter.instanceID = res.instanceID; // RTKAUDIO_RPC_TOAGENT_PRIVATEINFO return instanceID
		...
		case AIDK_CMD_INFO:
		{
			...
			ret = aidk_ioctl_get(aidk_info);

			break;
		}
#ifdef CONFIG_ARCH_RTK2851F
		case INIT_SPK_COUNTER_SHARE_BUF:
		{
			ret_info_t ret_info;

			ret = init_wifi_speaker_memory(&ret_info);

			if (copy_to_user((void __user *)(long) info.addr, &ret_info, ret_info.private_info[1]))
			{
				rtd_pr_adsp_err("INIT_SPK_COUNTER_SHARE_BUF copy to user failed\n");
				ret = -EFAULT;
			}
			rtd_pr_adsp_info("INIT_SPK_COUNTER_SHARE_BUF done\n");
			break;
		}
		case GET_SPK_COUNTER:
		{
			get_spk_counter();
			break;
		}
...
long rtkaudio_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case RTKAUDIO_IOC_SET_WITH_RETURN:	{
      ...
      struct cntl_info_with_return info;
        ...
        {
    		if (copy_from_user((void*)&info, (const void __user*)arg, sizeof(info))) {
				rtd_pr_adsp_err("%s copy_from_user fail for resource_type %d\n", __FUNCTION__, info.resource_type);rtd_pr_adsp_err("%s copy_from_user fail for resource_type %d\n", __FUNCTION__, info.resource_type);rtd_pr_adsp_err("%s copy_from_user fail for resource_type %d\n", __FUNCTION__, info.resource_type);rtd_pr_adsp_err("%s copy_from_user fail for resource_type %d\n", __FUNCTION__, info.resource_type);
    			ret = -EFAULT;
		...
		case AIDK_CMD_INFO:
		{
			...
			if (copy_from_user(&aidk_info, compat_ptr(info.addr), sizeof(aidk_cmd_info)))
			{
				ret = -EFAULT;
				rtd_pr_adsp_err("[rtkaudio] [%s:%d] AIDK_CMD_INFO error\n", __FUNCTION__, __LINE__);
				break;
			...
			ret = aidk_ioctl_set_compat(aidk_info);
	...
	case RTKAUDIO_IOC_GET:
	{
		...
		case AIDK_CMD_INFO:
		{
			...
			if (copy_from_user(&aidk_info, (const void __user *) compat_ptr(info.addr), sizeof(aidk_cmd_info)))
			{
				ret = -EFAULT;
				rtd_pr_adsp_err("[rtkaudio] [%s:%d] AIDK_CMD_INFO error\n", __FUNCTION__, __LINE__);
				break;
			...
			ret = aidk_ioctl_get_compat(aidk_info);
		...
		case INIT_SPK_COUNTER_SHARE_BUF:
		{
			...
			if (copy_to_user((void __user *) compat_ptr(info.addr), &ret_info, ret_info.private_info[1]))
			{
				rtd_pr_adsp_err("INIT_SPK_COUNTER_SHARE_BUF copy to user failed\n");
				ret = -EFAULT;
			}
			rtd_pr_adsp_info("INIT_SPK_COUNTER_SHARE_BUF 2 done\n");
			break;
		}
		case GET_SPK_COUNTER:
		{
...
int cmd_queue_create(int num)
{
	...
	cq->pkt_array_kernel = (cmd_packet*)(vir_addr) + 1;
	...
	const char *buf, size_t count, loff_t *f_pos)
{
    ...
    } else if ((cmd_buf[0] == 't') && (cmd_buf[1] == 'i') &&
			(cmd_buf[2] == 'm') && (cmd_buf[3] == 'e') &&
			(cmd_buf[4] == 'r')) {
		if ((cmd_buf[5] == 'd') && (cmd_buf[6] == 'e') &&
			(cmd_buf[7] == 'b') && (cmd_buf[8] == 'u') &&
            (cmd_buf[9] == 'g')) {
			if ((int) cmd_buf[11] == '1')
                timer_debug = 1;
            else
		    ...
		    rtd_pr_adsp_info("[rtkaudio] calc timer debug %s\n", (timer_debug ? "ON" : "OFF"));
        }
    } else if (!strncmp(cmd_buf, "init_wifi_spk", 13)) {
		ret_info_t ret_info;
		init_wifi_speaker_memory(&ret_info);
	} else if (!strncmp(cmd_buf, "uninit_wifi_spk", 15)) {
		uninit_wifi_speaker_memory();
	} else if (!strncmp(cmd_buf, "get_wifi_spk_count", 16)) {
		get_spk_counter();
	} else {
...
{
	int ret;
	rtd_pr_adsp_info("rtkaudio:pm_runtime_get:pm_count=%d,init_done=%d\n", pm_count, init_done);

	if (init_done) {
		ret = pm_runtime_get_sync(&rtkaudio_platform_devs->dev);
		if (ret < 0) {
			rtd_pr_adsp_err("rtkaudio:pm_runtime_get failed\n");
			return ret;
		}
	} else {
		pm_count++;
...
{
	rtd_pr_adsp_info("rtkaudio:pm_runtime_put:pm_count=%d,init_done=%d\n", pm_count, init_done);
...
static int rtkaudio_init(void)
{
	...
	rtkaudio_aidk_init();
	init_rpc_mem();
	rtkaudio_dmabuf_preallocate_init();

	/******************** get audio version ********************/
	memcpy(AudioGITHASH, (char *)vir_addr, 7);
	AudioGITHASH[7] = 0;
	memcpy(AudioCommonHASH, (char *)(vir_addr + 8), 7);
	AudioCommonHASH[7] = 0;
	dvr_free((void *) vir_addr);
	rtd_pr_adsp_err("Audio Git Hash %s, Common Hash %s\n", AudioGITHASH, AudioCommonHASH);
	/***********************************************************/
...
EXPORT_SYMBOL(get_EQ_BASS_enable);
...
EXPORT_SYMBOL(get_EQ_BASS_parameter);
...
EXPORT_SYMBOL(get_EQ_BASS_audio_data_path_status);
...
EXPORT_SYMBOL(set_EQ_BASS_enable);
...
EXPORT_SYMBOL(set_EQ_BASS_parameter);
...
EXPORT_SYMBOL(set_EQ_BASS_Audio_Data_Path_Enable);
