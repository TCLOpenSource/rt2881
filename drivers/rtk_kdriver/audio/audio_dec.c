...
UINT32 DEC_SetSeekingInfo(Base* pBaseObj, SINT32 skip, SINT32 speed)
{
    AUDIO_RPC_SEEK seek_info;
    UINT32 res;

    seek_info.instanceID     = pBaseObj->GetAgentID(pBaseObj);
    seek_info.skip           = skip;
    seek_info.speed          = speed;

    res = RTKAUDIO_RPC_TOAGENT_SETSEEKING_SVC(&seek_info);
    if (res != S_OK) ERROR("[%s:%d] RPC return != S_OK\n",__FUNCTION__,__LINE__);
...
Base* new_DEC(int idx)
{
    ...
    pObj->SetSeekingInfo            = DEC_SetSeekingInfo;
