...
static SINT32 g_instance_list[MAX_INST_NUM] = {-1,-1,-1,-1,-1};
...
Base* new_AIN_QuickShow(UINT32 idx, QuickShowCreateParam param)
{
    /* object init */
    AIN* pAINObj = NULL;
    Base* pObj = new_base();
    ...
    switch(idx) {
        case INST_IDX_MAIN:
        {
            ...
            INFO("%s instanceID=%x\n", __func__, pObj->instanceID);
            if(pObj->instanceID == UNDEFINED_AGENT_ID)
            {
                pObj->Delete(pObj);
                INFO("%s:error Delete idx=%d\n", __func__, idx);
                return NULL;
            }
        ...
        {
            char name[] = "Sub-AIN2";
            pObj->outPinID = PCM_OUT3;
            g_instance_list[INST_IDX_3] = (g_instance_list[INST_IDX_MAIN] & INST_MASK) | pObj->outPinID;
            memcpy(pObj->name, name, sizeof(name));
            break;
        }
        case INST_IDX_4:
        {
            ...
            g_instance_list[INST_IDX_4] = (g_instance_list[INST_IDX_MAIN] & INST_MASK) | pObj->outPinID;
            memcpy(pObj->name, name, sizeof(name));
            break;
        }
        case INST_IDX_5:
        {
            char name[] = "Sub-AIN4";
            pObj->outPinID = EXT_BS_OUT;
            g_instance_list[INST_IDX_5] = (g_instance_list[INST_IDX_MAIN] & INST_MASK) | pObj->outPinID;
            memcpy(pObj->name, name, sizeof(name));
            break;
        }
        default:
            ERROR("[AIN] %s Unknown idx %d\n", __func__, idx);
            return NULL;
    }

    pObj->instanceID = g_instance_list[idx];
