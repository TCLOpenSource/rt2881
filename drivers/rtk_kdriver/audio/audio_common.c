...
void list_add_safe(struct list_head *new, struct list_head *head)
{
    ...
    if (!list_add_valid(new, head, next))
        return;
		      ...
		      struct list_head *next)
{
    ...
    if(prev == NULL) {
        ERROR("list_add corruption. prev is NULL.\n");
        ret = false;
    }
    if(next == NULL) {
        ERROR("list_add corruption. next is NULL.\n");
        ret = false;
    }
    if(next->prev == NULL) {
        ERROR("list_add corruption. next->prev is NULL.\n");
        ret = false;
    }
    if (next->prev != prev) {
        ERROR("list_add corruption. next->prev should be prev (%px), but was %px. (next=%px).\n", prev, next->prev, next);
        ret = false;
    }
    if(prev->next == NULL) {
        ERROR("list_add corruption. prev->next is NULL.\n");
        ret = false;
    }
    if (prev->next != next) {
        ERROR("list_add corruption. prev->next should be next (%px), but was %px. (prev=%px).\n", next, prev->next, prev);
        ret = false;
    }
    if (new == prev || new == next) {
        ERROR("list_add double add: new=%px, prev=%px, next=%px.\n", new, prev, next);
        ret = false;
    }
