...
static void  rtd_cec_intial_hw_setting(void)
{
    ...
    /* setup internal pull high register... */
    write_reg32(GDI_POWER_SAVING_MODE,  CEC_CLK_POL(CEC_POSITIVE_CLOCK) |
                CEC_POWER_SAVING_MODE_cec_rpu_en(1) | CEC_POWER_SAVING_MODE_cec_rsel(0xE));
...
int rtd_ac_on_cec_suspend_inited(void)
{

    unsigned long rtk_cec_ac_standby_config = 0x0;
    unsigned short rtk_cec_ac_standby_physical_addr = 0x0;
    unsigned char rtk_cec_ac_standby_cec_version = CEC_VERSION_1_4;
    unsigned long rtk_cec_ac_standby_vendor_id = 0x000000;

    if( (rtd_inl(STB_ST_CLKEN1_reg) & BIT(STB_ST_CLKEN1_clken_cec_shift)) != 0 ){
        cec_warn("cec is early enable, not enable again\n");
        return 0;
    }
    cec_warn("cec is not enable, reinit\n");
...
#endif
        write_reg32(KDRV_CEC_RTCR0,CEC_PAD_EN | CEC_PAD_EN_MODE | RETRY_NO(5));
        write_reg32(KDRV_CEC_RXCR0, RX_RST);
    ...
    if (rtk_cec_ac_standby_config &
        ...
        write_reg32(GDI_CEC_SEND_OPERAND_NUMBER_02, 2);
        write_reg32(GDI_CEC_OPERAND_04,
                        (rtk_cec_ac_standby_physical_addr >> 8) &
                        0xFF);
        write_reg32(GDI_CEC_OPERAND_05,
                        (rtk_cec_ac_standby_physical_addr) & 0xFF);
            /*write_reg32(0xb8061ecc, (read_reg32(0xb8061ecc)|0x02)); //invalid, disable temporarily*/
    }
        /*-- compare 3*/
    if (rtk_cec_ac_standby_config &
        ...
        write_reg32(GDI_CEC_SEND_OPERAND_NUMBER_03, 1);
        write_reg32(GDI_CEC_OPERAND_07,
                        rtk_cec_ac_standby_cec_version);
        write_reg32(GDI_CEC_OPCODE_ENABLE,
                        (read_reg32(GDI_CEC_OPCODE_ENABLE) | 0x04));
    }
        /*-- compare 4*/
    if (rtk_cec_ac_standby_config &
        ...
        write_reg32(GDI_CEC_SEND_OPERAND_NUMBER_04, 3);
        write_reg32(GDI_CEC_OPERAND_10,
                        (rtk_cec_ac_standby_vendor_id >> 16) &
                        0xFF);
        write_reg32(GDI_CEC_OPERAND_11,
                        (rtk_cec_ac_standby_vendor_id >> 8) &
                        0xFF);
        write_reg32(GDI_CEC_OPERAND_12,
                        (rtk_cec_ac_standby_vendor_id) & 0xFF);
        write_reg32(GDI_CEC_OPCODE_ENABLE,
                        (read_reg32(GDI_CEC_OPCODE_ENABLE) | 0x08));
    }
#endif
...
unsigned int cec_wakeup_src = 0;
void cec_set_wake_source_src(unsigned int wakeupsrc)
{
    cec_wakeup_src = wakeupsrc;
}
EXPORT_SYMBOL(cec_set_wake_source_src);
/************************************************************
** check_wakeup_src_send_cec_opcode
** check the wakeup src is cec, if wakeup by cec
** cec resume will send the wakeup opcode to ap
** Parm : p_this : handle of rtd cec adapter
************************************************************/
void check_wakeup_src_send_cec_opcode(rtk_cec *p_this)
{
    int wakeup_reg_row;
    unsigned int wakeup_src;
    unsigned char opcode[] = {0xd};
    rtk_cec_rcv *p_rcv = &p_this->rcv;

    //powerMgr_get_wakeup_source(&wakeup_reg_row, NULL);
    wakeup_reg_row = cec_wakeup_src;

    wakeup_src = wakeup_reg_row & 0xffff;   //[MK2BU-650][EMCU]Add sub_wakeup source
    opcode[0] = (wakeup_reg_row >> 16) & 0xff;
    if(wakeup_src == WKSOR_CEC)
    {
        if (!p_rcv->cmb) {
            if (cmb_queue_len(&p_this->rx_free_queue)){
                p_rcv->cmb = cmb_dequeue(&p_this->rx_free_queue);
                cec_warn("[CEC]  WARNING rx_free_queuet\n");
            }else {
                cec_warn("[CEC] WARNING, rx over run\n");
                p_rcv->cmb = cmb_dequeue(&p_this->rx_queue);    /* reclaim form rx fifo */
            }
        ...
        if (p_rcv->cmb) {
            cmb_purge(p_rcv->cmb);
            cmb_reserve(p_rcv->cmb, 1);
            memcpy(p_rcv->cmb->tail, opcode, 1);
            cmb_put(p_rcv->cmb, 1);
            *cmb_push(p_rcv->cmb, 1) = ((((wakeup_reg_row>>24)&0xF)<<4) | 0); //such as 0x40;
                                //(((read_reg32(KDRV_CEC_RXCR0) & INIT_ADDR_MASK) >> INIT_ADDR_SHIFT) << 4) |
                                //((read_reg32(KDRV_CEC_CR0) &LOGICAL_ADDR_MASK) >> LOGICAL_ADDR_SHIFT);
            cmb_queue_tail(&p_this->rx_queue,p_rcv->cmb);
            p_rcv->cmb = NULL;
            cec_warn("add the opcode 0x%x  addr:%x to send\n",opcode[0],((((wakeup_reg_row>>24)&0xF)<<4) | 0));
            p_rcv->state = IDEL;
        }
    }
}
...
{
    if( (rtd_inl(STB_ST_CLKEN1_reg) & BIT(STB_ST_CLKEN1_clken_cec_shift)) == 0 ){
        CRT_CLK_OnOff(CEC, 0, NULL);
        CRT_CLK_OnOff(CEC, 1, NULL);
    }
    ...
    check_wakeup_src_send_cec_opcode(p_this);   //if wakeupp by cec, send wakeup opcode to ap
...
int pm_rtd_cec_ac_on_suspend(void)
{
    return rtd_ac_on_cec_suspend_inited();
}
EXPORT_SYMBOL(pm_rtd_cec_ac_on_suspend);
/*------------------------------------------------------------------
