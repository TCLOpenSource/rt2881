...
irqreturn_t rpc_isr(int irq, void *dev_id)
{
	...
	if (itr & (RPC_INT_SCPU_A | RPC_INT_SCPU_V
		...
		| RPC_INT_SCPU_V2
#endif
#if defined(CONFIG_REALTEK_RPC_VCPU3) && defined(CONFIG_ARCH_RTK2819A)
		| RPC_INT_SCPU_V3
#endif
	)) {
...
#endif
#if defined(CONFIG_REALTEK_RPC_VCPU3) && defined(CONFIG_ARCH_RTK2819A)
			if (itr & RPC_INT_SCPU_V3) {
				/*to clear interrupt */
				rtd_outl(SB2_CPU_SW_INT_2_reg, RPC_INT_SCPU_V3);
#ifdef CONFIG_REALTEK_RPC_MULTIPROCESS
				if (rpc_intr_devices[RPC_INTR_DEV_V3S_ID7].ringIn !=rpc_intr_devices[RPC_INTR_DEV_V3S_ID7].ringOut)
					tasklet_schedule(&(rpc_intr_extra[RPC_INTR_DEV_V3S_ID7].tasklet));
#else
				if (rpc_intr_devices[RPC_INTR_DEV_V3S_ID7].ringIn != rpc_intr_devices[RPC_INTR_DEV_V3S_ID7].ringOut)
					wake_up_interruptible(&(rpc_intr_devices[RPC_INTR_DEV_V3S_ID7].ptrSync->waitQueue));
#endif
				if (rpc_kern_devices[RPC_INTR_DEV_V3S_ID7].ringIn != rpc_kern_devices[RPC_INTR_DEV_V3S_ID7].ringOut)
					wake_up_interruptible(&(rpc_kern_devices[RPC_INTR_DEV_V3S_ID7].ptrSync->waitQueue));
			}
#endif
			itr = rtd_inl((unsigned int)SB2_CPU_SW_INT_2_reg);
		}
