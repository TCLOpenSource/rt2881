...
static void inner_wait_done(void) {
	...
	//SE_WaitCmdDone_mode(SE_CMD_BLOCK_MODE_TIMEOUT,0);
#endif
}
...
static bool SEStretchFactorGuard(KGAL_RECT_T* psRect, KGAL_RECT_T* pdRect,
	...
	*pvf = (psRect->h<<14)/pdRect->h;
	...
	else
	{
		...
		rtd_pr_se_err("horizontal factor out of range\n");
	...
	{
		rtd_pr_se_err("vertical factor out of range\n");
		return false; //if there is hw limit.
	}
#endif
...
bool KGAL_Blit(KGAL_SURFACE_INFO_T* pSrcSurface, KGAL_RECT_T* pSrcRect, KGAL_SURFACE_INFO_T* pDstSurface, uint16_t dx, uint16_t dy, KGAL_BLIT_FLAGS_T* pBlitFlags, KGAL_BLIT_SETTINGS_T* pBlitSettings)
{
	...
	if(*pBlitFlags & (KGAL_BLIT_ROTATE90|KGAL_BLIT_ROTATE180|KGAL_BLIT_ROTATE270)){
		ret_bool = inner_rotate(pSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
		up(&sem_nv12);
		return ret_bool;
	}

	if(*pBlitFlags & (KGAL_BLIT_VERTICAL_MIRROR|KGAL_BLIT_HORIZONTAL_MIRROR)){
		ret_bool = inner_mirror(pSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
		up(&sem_nv12);
		return ret_bool;
	}
#else
	if(*pBlitFlags & _strmBlitFg){
		ret_bool = inner_fast_cpy_conv(pfSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
		up(&sem_nv12);
		return ret_bool;
	}
#endif
	...
	{
		ret_bool = inner_fbc_Blit(pSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
		up(&sem_nv12);
		return ret_bool;
	}
#else
	...
	if(pSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_LUT8) {
		if(*pBlitFlags == KGAL_BLIT_NOFX
			&& pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_LUT8) {
#if SE_OLD
				ret_bool = KGAL_FormatConv(pfSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
				up(&sem_nv12);
				return ret_bool;
#else
				ret_bool = inner_fast_cpy_conv(pfSrcSurface, pSrcRect, pDstSurface, dx, dy, pBlitFlags, pBlitSettings);
...
bool KGAL_NV12_StretchBlit_tee(KGAL_SURFACE_INFO_T *pSrcSurface, KGAL_RECT_T *pSrcRect, KGAL_SURFACE_INFO_T *
pDstSurface, KGAL_RECT_T *pDstRect, KGAL_BLIT_FLAGS_T *pBlitFlags, KGAL_BLIT_SETTINGS_T *pBlitSettings, unsigned int 
src_c1_addr, unsigned int dst_c1_addr)
{
	SEINFO_SACLING_TYPE stVType = SEINFO_SCALING_DISABLE;
	SEINFO_SACLING_TYPE stHType = SEINFO_SCALING_DISABLE;
	SEINFO_SETUPMODE stMode = SEINFO_BITBLT_SETUP_MANUAL;
	...
	SE_BLEND_EX_T stEx;
	uint32_t u32VFactor = 0, u32HFactor = 0;
	uint32_t u32SrcSize = 0, u32DstSize = 0;
	KGAL_SURFACE_INFO_T *pfSrcSurface = pSrcSurface;
	...
	if(se1_hwsem_lock() == false)
	{
		up(&sem_nv12);
		rtd_pr_se_err( "[SE] se1 hwsem lock FAIL@%s\n.", __func__);
		return 0;
	}
	...
	if (pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC) {
		...
		up(&sem_nv12);
		se1_hwsem_unlock();
		rtd_pr_se_err("%s Unsupport encode TFBC!\n", __FUNCTION__);
		return false;
	}
	...
	if (inner_stretch_check(pSrcRect, pDstRect) == true) {
		...
		u32SrcSize = pfSrcSurface->width * pfSrcSurface->height * (pfSrcSurface->bpp/8);
		if(pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_NV12)
		{			
			u32DstSize = (pDstSurface->width * pDstSurface->height * 3)/2;
		}
		else
		{			
			u32DstSize = pDstSurface->width * pDstSurface->height * (pDstSurface->bpp/8);
		}
		if(call_se_ta_lock_cmd(pfSrcSurface->physicalAddress, u32SrcSize,pDstSurface->physicalAddress,u32DstSize,1) != 0)
		{
			rtd_pr_se_err("%s Unprotect memory src:%x ,dst:%x!\n", __FUNCTION__, pfSrcSurface->physicalAddress,pDstSurface->physicalAddress);
			up(&sem_nv12);
			se1_hwsem_unlock();
			return false;
		}
		SE_Stretch_ext_tee(&stDstProcInfo, &stSrcProcInfo,
				stVType, u32VFactor, SEINFO_VSCALING_4TAP,
				stHType, u32HFactor, SEINFO_HSCALING_2TAP,
				SEINFO_STRETCH_FIR, SEINFO_ALPHAPROC_DISABLE, 0, (void *)&stExFmt);
		
		call_se_ta_lock_cmd(pfSrcSurface->physicalAddress, u32SrcSize,pDstSurface->physicalAddress,u32DstSize,0);
	}else {
		SE_EXT_FMT_CONV_T stExFmt;
...
EXPORT_SYMBOL(KGAL_NV12_StretchBlit_tee);
...
bool KGAL_SetSurfacePalette(KGAL_PALETTE_SURFACE_INFO_T *local_palette, unsigned char from_ioctl)
{
...
extern struct semaphore sem_pro_func;
int KGAL_PQMaskLockSE(void)
{
	return down_timeout(&sem_pro_func, SE_CMD_TIMEOUT);
}
...
bool KGAL_PQMaskLabel2Weight(KGAL_SURFACE_INFO_T *pSrcSurf, KGAL_SURFACE_INFO_T *pDstSurf) {
	...
	unsigned int planeOffset = 0;
	uint32_t AddrCurMax = 0, AddrCurMin = 0xffffffff;
	uint32_t AddrTtlMax = 0, AddrTtlMin = 0xffffffff;

	// error checking
	if( pSrcSurf->pixelFormat != KGAL_PIXEL_FORMAT_LUT8 || pDstSurf->pixelFormat != KGAL_PIXEL_FORMAT_YUV444 )
		return false;

	if( pSrcSurf->physicalAddress == 0 || pDstSurf->physicalAddress == 0 ) {
		printk(KERN_ERR"[%s] address error\n", __func__);
		return false;
	}
	...
	AddrCurMin = srcInfo.uAddr;
	AddrCurMax = srcInfo.uAddr + pSrcSurf->width*pSrcSurf->height*pSrcSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstYInfo.uAddr;
	AddrCurMax = dstYInfo.uAddr + pDstSurf->width*pDstSurf->height*pDstSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstUInfo.uAddr;
	AddrCurMax = dstUInfo.uAddr + pDstSurf->width*pDstSurf->height*pDstSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstVInfo.uAddr;
	AddrCurMax = dstVInfo.uAddr + pDstSurf->width*pDstSurf->height*pDstSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);

	// align the address
	if( (AddrTtlMin & 0xff) != 0 )
		AddrTtlMin = (AddrTtlMin & 0xffffff00);
	if( (AddrTtlMax & 0xff) != 0 )
		AddrTtlMax = ((AddrTtlMax+0x100) & 0xffffff00);

	SE_PQMask_Label2Weight(&srcInfo, &dstYInfo, &dstUInfo, &dstVInfo, AddrTtlMin, AddrTtlMax);
...
bool KGAL_PQMaskStretch(KGAL_SURFACE_INFO_T *pSrcSurf, KGAL_SURFACE_INFO_T *pDstSurf) {
	...
	AddrCurMin = srcYInfo.uAddr;
	AddrCurMax = srcYInfo.uAddr + pSrcSurf->width*pSrcSurf->height*pSrcSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	if( pSrcSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = srcUInfo.uAddr;
		AddrCurMax = srcUInfo.uAddr + pSrcSurf->width*pSrcSurf->height*pSrcSurf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	if( pSrcSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = srcVInfo.uAddr;
		AddrCurMax = srcVInfo.uAddr + pSrcSurf->width*pSrcSurf->height*pSrcSurf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	)
{
	...
	if( pSrc1Surf->physicalAddress == 0 || pSrc2Surf->physicalAddress == 0 || pDstSurf->physicalAddress == 0 ) {
		printk(KERN_ERR"[%s] address error\n", __func__);
		return false;
	}
	...
	AddrCurMin = src1YInfo.uAddr;
	AddrCurMax = src1YInfo.uAddr + pSrc1Surf->width*pSrc1Surf->height*pSrc1Surf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = src1UInfo.uAddr;
		AddrCurMax = src1UInfo.uAddr + pSrc1Surf->width*pSrc1Surf->height*pSrc1Surf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = src1VInfo.uAddr;
		AddrCurMax = src1VInfo.uAddr + pSrc1Surf->width*pSrc1Surf->height*pSrc1Surf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	AddrCurMin = src2YInfo.uAddr;
	AddrCurMax = src2YInfo.uAddr + pSrc2Surf->width*pSrc2Surf->height*pSrc2Surf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = src2UInfo.uAddr;
		AddrCurMax = src2UInfo.uAddr + pSrc2Surf->width*pSrc2Surf->height*pSrc2Surf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
		AddrCurMin = src2VInfo.uAddr;
		AddrCurMax = src2VInfo.uAddr + pSrc2Surf->width*pSrc2Surf->height*pSrc2Surf->bpp;
		AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
		AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	}
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_YUV444 ) {
	...
	} else if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_NV12_Y && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_NV12_Y && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_NV12_Y ) {
		SE_PQMask_Bitblit_NV12_Y(
			&src1YInfo, &src2YInfo,
			&dstYInfo,
			SrcAlpha, DstAlpha, RltAlpha, RoundSel,
			AddrTtlMin, AddrTtlMax
		...
		printk("[%s] format setting error src1=%d src2=%d src3=%d\n", __FUNCTION__, pSrc1Surf->pixelFormat, pSrc2Surf->pixelFormat, pDstSurf->pixelFormat);
	}
	up(&sem_nv12);
	return true;
}
...
bool KGAL_PQMask_Bitblit_MultiplyByAlpha(
	KGAL_SURFACE_INFO_T *pSrc1Surf, KGAL_SURFACE_INFO_T *pSrc2Surf, KGAL_SURFACE_INFO_T *pDstSurf,
	unsigned int SrcAlpha, unsigned int DstAlpha, unsigned int RltAlpha, unsigned char RoundSel
	)
{
	SE_PROCINFO_T src1YInfo;
	SE_PROCINFO_T src2YInfo;
	SE_PROCINFO_T dstYInfo;
	uint32_t planeOffset = 0;
	uint32_t AddrCurMax = 0, AddrCurMin = 0xffffffff;
	uint32_t AddrTtlMax = 0, AddrTtlMin = 0xffffffff;
	...
	// source 1 information, must be alpha8
	planeOffset = pSrc1Surf->pitch*pSrc1Surf->height;
	src1YInfo.byBaseAddressIndex = 0;
	src1YInfo.uAddr = pSrc1Surf->physicalAddress;
	src1YInfo.uPitch = pSrc1Surf->pitch;
	...
	if( pSrc1Surf->pixelFormat == KGAL_PIXEL_FORMAT_A8 && pSrc2Surf->pixelFormat == KGAL_PIXEL_FORMAT_NV12_Y && pDstSurf->pixelFormat == KGAL_PIXEL_FORMAT_NV12_Y ) {
		SE_PQMask_Bitblit_MultiplyByAlpha(
			&src1YInfo,
			&src2YInfo,
			&dstYInfo,
			SrcAlpha, DstAlpha, RltAlpha, RoundSel,
	...
	)
{
	...
	if( pSrc1_YSurf->physicalAddress == 0 || pSrc1_USurf->physicalAddress == 0 || pSrc1_VSurf->physicalAddress == 0 ||
		pSrc2_YSurf->physicalAddress == 0 || pSrc2_USurf->physicalAddress == 0 || pSrc2_VSurf->physicalAddress == 0 ||
		pDst_YSurf->physicalAddress == 0 || pDst_USurf->physicalAddress == 0 || pDst_VSurf->physicalAddress == 0 ) {
		printk(KERN_ERR"[%s] address error\n", __func__);
		return false;
	}
	...
	AddrCurMin = src1YInfo.uAddr;
	AddrCurMax = src1YInfo.uAddr + pSrc1_YSurf->width*pSrc1_YSurf->height*pSrc1_YSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = src1UInfo.uAddr;
	AddrCurMax = src1UInfo.uAddr + pSrc1_USurf->width*pSrc1_USurf->height*pSrc1_USurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = src1VInfo.uAddr;
	AddrCurMax = src1VInfo.uAddr + pSrc1_VSurf->width*pSrc1_VSurf->height*pSrc1_VSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = src2YInfo.uAddr;
	AddrCurMax = src2YInfo.uAddr + pSrc2_YSurf->width*pSrc2_YSurf->height*pSrc2_YSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = src2UInfo.uAddr;
	AddrCurMax = src2UInfo.uAddr + pSrc2_USurf->width*pSrc2_USurf->height*pSrc2_USurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = src2VInfo.uAddr;
	AddrCurMax = src2VInfo.uAddr + pSrc2_VSurf->width*pSrc2_VSurf->height*pSrc2_VSurf->bpp;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstYInfo.uAddr;
	AddrCurMax = dstYInfo.uAddr + pDst_YSurf->width*pDst_YSurf->height;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstUInfo.uAddr;
	AddrCurMax = dstUInfo.uAddr + pDst_USurf->width*pDst_USurf->height;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
	...
	AddrCurMin = dstVInfo.uAddr;
	AddrCurMax = dstVInfo.uAddr + pDst_VSurf->width*pDst_VSurf->height;
	AddrTtlMin = _MIN_(AddrTtlMin, AddrCurMin);
	AddrTtlMax = _MAX_(AddrTtlMax, AddrCurMax);
...
EXPORT_SYMBOL(KGAL_PQMask_Bitblit_MultiplyByAlpha);
EXPORT_SYMBOL(KGAL_PQMaskLockSE);
EXPORT_SYMBOL(KGAL_PQMaskUNLockSE);
