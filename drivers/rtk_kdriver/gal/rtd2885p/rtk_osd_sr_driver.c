...
unsigned char isOSDSR_update_flag = 0;
...
/*           Super Resolution            */
/* ------------------------------------- */
SLR_VIP_TABLE_OSDSR VIP_TABLE_OSDSR = 
{
	{
		{
			//sr_lpf_gain;
			// v_3x5; sr_maxmin_range sr_lpf_range sr_edge_range
			//v_overshoot; sr_maxmin_limit_en sr_maxmin_shift sr_var_thd
			//v_lpf_range; sr_lpf_range_en sr_lpf_range_thd sr_lpf_range_gain sr_lpf_range_step
			//v_lpf_edge; sr_lpf_edge_en sr_lpf_edge_thd sr_lpf_edge_gain sr_lpf_edge_step sr_delta_gain
			//v_lpmm; sr_lpmm_en sr_lpmm_1px sr_lpmm_corner		
			{63,1,0,0 , 1,0,255 , 1,64,8,2 , 0,0,0,0,0 , 1,8,12},//default
			{61,1,1,1 , 1,32,32 , 1,64,8,2 , 0,0,0,0,1 , 0,8,12},//no lpmm
		},
	},
};
		...
		GDMA_DISPLAY_PLANE disPlane, struct gdma_receive_work *ptr_work)
{
	...
	*(unsigned int *)&line_buffer_end_reg = rtd_inl(GDMA_line_buffer_end_reg);
	...
	if (enable && (bypass_flag == 0)) {
		...
		*/
		...
		//	return -1;
			...
			if (bypass_flag == 0 && (ver_factor_change || hor_factor_change || isOSDSR_update_flag == 1)) {
				/* disable LPF and set back to default value when scaling function is NOT enable */
				if (offset == 0)
					osd_sr_update_txt(&VIP_TABLE_OSDSR.osdsr_table.data[0][0]);
				else
					osd_sr_update_txt(&VIP_TABLE_OSDSR.osdsr_table.data[1][0]);
				osd_sr_set_table_gal( offset , osd_sr_get__gal_osdsr_table_txt() );
		
				rtd_pr_gdma_info("OSDSR, flag=%d,%d,%d, offset=%d,\n", ver_factor_change, hor_factor_change, isOSDSR_update_flag, offset);
				isOSDSR_update_flag = 0;
			}
	...
	if((change_OSD_mode == GDMA_SPLIT_MODE_1 ||
	  ...
	  (gGDMA_Monitor_ctl.enable==1)) || (!osd_sr_scaleup_ctrl0_reg.v_zoom_en && !osd_sr_scaleup_ctrl1_reg.h_zoom_en)
		){
		osd_sr_scaleup_ctrl0_reg.v_zoom_en = 1;
...
void osd_sr_set_lpmm( unsigned int offset , DRV_OSDSR_lpmm* x )
{
	...
	z_RBUS_reg = rtd_inl( OSD_SR_OSD_SR_1_GainY_Ctrl0_reg+offset );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_en_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_en( x->sr_lpmm_en ) );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_1px_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_1px( x->sr_lpmm_1px ) );
	z_RBUS_reg = ( z_RBUS_reg & ~OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_corner_mask) | (OSD_SR_OSD_SR_1_GainY_Ctrl0_sr_lpmm_corner( x->sr_lpmm_corner ) );
	rtd_outl( OSD_SR_OSD_SR_1_GainY_Ctrl0_reg+offset , z_RBUS_reg );
}
...
EXPORT_SYMBOL(isOSDSR_update_flag);
EXPORT_SYMBOL(VIP_TABLE_OSDSR);
