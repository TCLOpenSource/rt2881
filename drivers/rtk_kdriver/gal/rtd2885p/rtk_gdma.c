...
static unsigned int gVsyncPeriod_NS = GDMA_VSYNC_TIMEOUT_NS_60FPS;
static unsigned int gVsyncDiff_NS = 0;
...
#define GDMA_EXPT_TAG_WAIT_VSYNC0 (0X02)
#define GDMA_EXPT_TAG_WAIT_VSYNCX (0X04)
#define GDMA_EXPT_TAG_SHOW	      (0X08)



int gDebugExp_loglevel = 0;

#define GDMA_EXPTIME_LOG(level,fmt, arg...) do { \
		if( unlikely(gDebugExp_loglevel >= level) ) { rtd_pr_gdma_crit("expT TT:%lld "  fmt, ktime_to_us(ktime_get()), ##arg ); } \
	   } while(0) 
	   


#define GDMA_ENABLE_EXP_TIME //enable expected time presentation

#ifdef GDMA_ENABLE_EXP_TIME

static DECLARE_COMPLETION( gdma_expTime_completion );
static DEFINE_SPINLOCK(expTime_completion_lock);
...
int64_t gGDMA_VSYNC_USER_TIME = 0;
int64_t gGDMA_VSYNC_USER_TIME_PREV = 0;
...
//atomic64_t gGDMA_VSYNC_USER_TIME_PREV = ATOMIC64_INIT(0);
...
GDMA_DISPLAY_PLANE main_disPlane = GDMA_PLANE_OSD1;
GDMA_DISPLAY_PLANE sub_disPlane = GDMA_PLANE_OSD2;
VO_GRAPHIC_PLANE main_graphicPlane = VO_GRAPHIC_OSD1;
VO_GRAPHIC_PLANE sub_graphicPlane = VO_GRAPHIC_OSD2;
...
static const char *rtkgdma_cmd_str[] = {
	...
	"osd_show_yuv422=",
	"osd_vsync_info=", 
	"osd_bypass_se_ta_cmd=",
};


int GDMA_Check_Dvr_Reclaim(void)
{
	static bool qs_stop = true; 
	unsigned int osdwi = 0;
	unsigned int offset =0;

	if(main_disPlane == GDMA_PLANE_OSD2)
		offset = 0x100;

	osdwi = (unsigned int)rtd_inl(GDMA_OSD1_WI_reg+offset);
	...
	if( gdma_qs_stop_logo != NULL && qs_stop)
	{			
		rtd_pr_gdma_info("[QS] %s: stop qs logo\n", __func__);
		gdma_qs_stop_logo();
		if(main_disPlane == GDMA_PLANE_OSD2)
			rtd_outl(GDMA_OSD1_CTRL_reg,~1);
		else
			rtd_outl(GDMA_OSD2_CTRL_reg,~1);
		rtd_outl(GDMA_CTRL_reg, 0x7);
		rtd_pr_gdma_info("[QS] %s: stop qs logo done\n", __func__);
	}
	qs_stop = false; 
...
static void gdma_config_mid_blend(void) {
	...
	mid_blend_reg.c0_sel = GDMA_PLANE_OSD2;
	mid_blend_reg.c1_sel = GDMA_PLANE_OSD1;
	...
	//k1 and k2 need change for OSD2 main display
	mixer_bx_reg.k1 = 0xc;
	mixer_bx_reg.k2 = 0x3;
...
void GDMA_UpdateGraphicwindow (int bBlocking, VO_GRAPHIC_PLANE plane)
{
	...
	onlinePlane[onlineMaxNum++] = main_graphicPlane;
	onlinePlane[onlineMaxNum++] = sub_graphicPlane;
		...
		for (i = 0, win = &osdPlane->OSDwinBuf[osdPlane->OSD_ctrl.index][osdPlane->OSD_topid]; i < 1; i++) {
			if (win->ctrl.drawn) {
				...
				*/
				...
				picObj->plane = (plane == 0) ? main_graphicPlane : sub_graphicPlane;//plane;
#else
				picObj->plane = (plane == 0) ? main_graphicPlane : plane;
#endif
...
int GDMA_CreateGraphicWindow (VIDEO_RPC_VOUT_CREATE_GRAPHIC_WIN *cmd)
{
	...
	{
		cmd->plane = main_graphicPlane;
	}
	else if(cmd->plane == VO_GRAPHIC_OSD2)
	{
		cmd->plane = sub_graphicPlane;
	}
...
int GDMA_DeleteGraphicWindow (unsigned char winID, VO_GRAPHIC_PLANE plane, unsigned char go)
{
	...
	{
		plane = main_graphicPlane;
	}
	else if(plane == VO_GRAPHIC_OSD2)
	{
		plane = sub_graphicPlane;
	}
	osdPlane = &gdma->osdPlane[plane];
...
int GDMA_SetAlphaOsdDetectionParameter(unsigned int data)
{
  unsigned int reg=0;
	osdovl_osd_db_ctrl_RBUS osd_db_ctrl_reg;
...
ssize_t GDMA_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (cmd_index < GDMA_DBG_CMD_NUMBER) {
		...
		if (cmd_length <= RTKGDMA_MAX_CMD_LENGTH - 1 && strlen(str) < 128)
...
void Graphic_PatternColor(unsigned int enable, UINT32 patterncolor)
{
	...
	OSD_Pattern(&st);
	if (enable == 1)
		GDMA_ConfigOSDxEnable(main_disPlane, 1);
	else
		GDMA_ConfigOSDxEnable(main_disPlane, 0);
}

void Set_GDMA_ConfigOSDxEnable(unsigned int enable)
{
	GDMA_ConfigOSDxEnable(main_disPlane, enable);
}
	...
	case GDMA_CONFIGURE_GRPAHIC_CANVAS:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIGURE_GRPAHIC_CANVAS, plane %d\n", data.plane);
	...
	case GDMA_CREATE_GRPAHIC_WINDOW:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CREATE_GRPAHIC_WINDOW, plane %d,\n", data.plane);
			break;
		}
	case GDMA_DRAW_GRPAHIC_WINDOW:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_DRAW_GRPAHIC_WINDOW, plane %d\n", data.plane);
			break;
		}
	case GDMA_MODIFY_GRPAHIC_WINDOW:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_ModifyGraphicWindow, plane %d\n", data.plane);
			break;
		}
	case GDMA_HIDE_GRPAHIC_WINDOW:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_HIDE_GRPAHIC_WINDOW, plane %d, winID %d\n", data.plane, data.winID);
			break;
		}
	case GDMA_DELETE_GRPAHIC_WINDOW:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_DELETE_GRPAHIC_WINDOW, plane %d, winID %d ,go %d\n", data.plane, data.winID,data.go);
			break;
		}
	case GDMA_CONFIGURE_OSD_PALETTE:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIGURE_OSD_PALETTE, paletteIndex %d\n", data.paletteIndex);
			break;
		}
	case GDMA_DISPLAY_GRPAHIC:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_DISPLAY_GRPAHIC, plane %d\n", data.plane);
			break;
		}
	case GDMA_QUERY_STATUS:
		ret = GDMA_OSD_INTST_get_osd2_vact_end(rtd_inl(GDMA_OSD_INTST_reg));
	...
	case GDMA_PLANE_CONFIG_DISPLAY_WIN:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_PLANE_CONFIG_DISPLAY_WIN plane %d, x %d, y %d w %d h %d\n", data.plane, data.dispWin.x, data.dispWin.y, data.dispWin.width, data.dispWin.height);
			break;
		}
	...
	case GDMA_PLANE_DISPLAY:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_PLANE_DISPLAY, plane %d, zb %d\n", data.plane, data.zeroBuffer);
			break;
		}
	...
	case GDMA_OSD1_ON_TOP:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_OSD1_ON_TOP %d\n", data);
			break;
		}
	...
	case GDMA_CONFIG_OSD1_MIXER_ENABLE:
		{
			...
			if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(bool))) {
				...
				ret = GDMA_ConfigOSDxMixerEnableNoDelay(main_disPlane,data);//change O2 as main display
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD1_MIXER_ENABLE, enable %d\n", data);
			break;
		}
	case GDMA_CONFIG_OSD4_MIXER_ENABLE:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD4_MIXER_ENABLE, enable %d\n", data);
			break;
		}
	case GDMA_CONFIG_OSD1OSD4_MIXER_ORDER:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD1OSD4_MIXER_ORDER, enable %d\n", data);
			break;
		}
	case GDMA_CONFIG_OSD_ENABLE:
		{
			...
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD_ENABLE, enable %d\n", data);
			break;
		}
	case GDMA_CONFIG_OSD1_ENABLE:
		{
			...
			if (copy_from_user((void *)&data, (const void __user *)arg, sizeof(bool))) {
				...
				ret = GDMA_ConfigOSDxEnable(main_disPlane, data);//change O2 as main display
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD1_ENABLE, ENABLE %d\n", data);
...
#else
				ret = GDMA_ConfigOSDxEnable(sub_disPlane, data);//change O1 as second display
#endif
			pr_info(KERN_EMERG"GDMA: ioctl GDMA_CONFIG_OSD2_ENABLE, ENABLE %d\n", data);
...
irqreturn_t GDMA_irq_handler(int irq, void *dev_id)
{
...
#endif
	{
		...
		if (osd2_sync) {
			...
			#ifdef ENABLE_VSYNC_NOTIFY
				gGDMA_VSYNC_USER_TIME_PREV = gGDMA_VSYNC_USER_TIME;
				gGDMA_VSYNC_USER_TIME = ktime_to_ns (ktime_get());

				gVsyncDiff_NS = gGDMA_VSYNC_USER_TIME - gGDMA_VSYNC_USER_TIME_PREV;
	...
	if (!(osd2_vend || osd2_vend_3D || osd2_finish || osd2_sync)) {
		return IRQ_NONE;
	}
...
static int gdma_probe(struct platform_device* pdev)
{
	...
	/*for OSD shift*/
	memset(&g_osdshift_ctrl, 0x00, sizeof(CONFIG_OSDSHIFT_STRUCT));
	/*MAIN is OSD1, change to OSD2*/
	g_osdshift_ctrl.plane = VO_GRAPHIC_OSD2;
...
*/
	...
	if(rtd_inl(UPGRADE_STATUS_reg) == UPGRADE_QUIESCENT_MAGIC)
	{
		rtd_outl(GDMA_OSD1_CTRL_reg, ~1);
		rtd_outl(UPGRADE_STATUS_reg, 0); // clear magic number
		rtd_pr_gdma_info("[%s][%d] androidboot.quiescent=1, clear magic_num_quiescent to %d \n", __FUNCTION__, __LINE__, rtd_inl(UPGRADE_STATUS_reg));
	}
	...
	*/
	...
	if(rtd_inl(QS_STATUS_2_reg) & QS_STATUS_2_AT_mask)
	{		
	...
	*/
	...
	mixer_layer_sel_reg.c0_sel = main_disPlane;
	mixer_layer_sel_reg.c1_sel = sub_disPlane;
...
DCMT_DEBUG_INFO_REGISTER(osd_mdscpu,osd_debug_info);
	...
	rtd_pr_gdma_info("[GDMA]get total memsize = %x\n", gdma_total_ddr_size);
...
int gdma_suspend_by_vbe(void) {
	...
	GDMA_ConfigOSDxEnableFast(main_disPlane,false);
	gdma_suspend_disable_osd = 1;
	GDMA_Wait_OSD_Prog_Done(main_disPlane);
...
static int gdma_resume_std (struct device *p_dev)
{
	...
	if (osd_ctrl_data[main_disPlane] & GDMA_OSD2_CTRL_osd2_en_mask) {

		for (i = 0; i < pmRegSize; i++) {
			if ((GDMA_OSD1_WI_reg+offset) == GDMA_PM_REGISTER_DATA[i][0]) {
		...
		if(gdma->curPic[main_disPlane] == 0)
			clear_index = GDMA_MAX_PIC_Q_SIZE-1;
		else
			clear_index = gdma->curPic[main_disPlane]-1;
		curPic = gdma->pic[main_disPlane] + clear_index;
...
int GDMA_Debug(KADP_HAL_GAL_CTRL_T *data)
{
	...
	switch (data->type) {
		...
		{
			rtd_pr_gdma_info(" GDMA_DBG_CMD_OSD_MONITOR_CTL \n");
		}
		break;
		case GDMA_DBG_CMD_OSD_RUNTIME_CTL:
		{
			rtd_pr_gdma_info(" GDMA_DBG_CMD_OSD_RUNTIME_CTL \n");
		}
		break;
		case GDMA_DBG_CMD_OSD_BYPASS_SE_CMD:
			break;
		default:
			break;
	}
...
int GDMA_ReceivePictureNoBlock (PICTURE_LAYERS_OBJECT *data)
{
	...
	GDMA_DISPLAY_PLANE disPlane = main_disPlane;
	VO_RECTANGLE disp_res;
...
//for ANDROIDTV-305 test
			if(saveBufAddr >= gdma_total_ddr_size){
				dump_stack();
				rtd_pr_gdma_emerg("%s, saveBufAddr=%x is over OSD_MAX_PHY_ADDR(%x) \n", __FUNCTION__, saveBufAddr, gdma_total_ddr_size);
				GDMA_EXIT_CRITICAL();
				up(&gdma->sem_receive);
				return GDMA_FAIL;
			}
...
#else
#if defined(CONFIG_ARM64)
	osdwi = (unsigned long)rtd_inl(GDMA_OSD1_WI_reg +offset);
#else
	osdwi = (unsigned int)rtd_inl(GDMA_OSD1_WI_reg+offset);
#endif
...

{
	...
	if(enable_stop_update)
	{		
		rtd_pr_gdma_emerg ("%s enable_stop_update , return\n",__FUNCTION__);
		return GDMA_SUCCESS;
	}
	if(HSR_OSD_disable)
	{
		rtd_pr_gdma_emerg ("%s HSR disable osd , return\n",__FUNCTION__);
		return GDMA_SUCCESS;
	}
	...
	if (data->layer_num) {
		if (data->layer[0].normal_num) {
			if (data->layer[0].normal[0].dst_width < data->layer[0].normal[0].width) {
				rtd_pr_gdma_emerg("%s, wrong width size, src=%d, dst=%d\n", __FUNCTION__, data->layer[0].normal[0].width, data->layer[0].normal[0].dst_width);
				return GDMA_FAIL;
			}
			if (data->layer[0].normal[0].dst_height < data->layer[0].normal[0].height) {
				rtd_pr_gdma_emerg("%s, wrong height size, src=%d, dst=%d\n", __FUNCTION__, data->layer[0].normal[0].height, data->layer[0].normal[0].dst_height);
				return GDMA_FAIL;
			}
		}
	}
		...
		if (picObj->layer_used == 1) {
			...
			if (gdma->picQrd[disPlane] == ((gdma->picQwr[disPlane] + 1) & (GDMA_MAX_PIC_Q_SIZE - 1))) {
				...
				*/
				...
				rtd_pr_gdma_emerg("GDMA: zeroBuffer is 1\n");
				GDMA_EXIT_CRITICAL();
				sGdmaReceiveWork[emptyIdx].used = 0;	/* free work */
				up(&gdma->sem_receive);
				return GDMA_SUCCESS;
...
\
			...
			while (rtd_inl(GDMA_CTRL_reg) && (disPlane==main_disPlane)) {
...
int GDMA_Wait_Vsync( u64* nsec )
{
	...
	//*nsec = ktime_to_ns( atomic64_read(&gGDMA_VSYNC_USER_TIME ) );  // s64 ktime_to_ns(const ktime_t kt),  s64 is int64
	*nsec = gGDMA_VSYNC_USER_TIME;

	gGDMA_VSYNC_SW_TIME_PREV = gGDMA_VSYNC_SW_TIME;
	gGDMA_VSYNC_SW_TIME = gGDMA_VSYNC_USER_TIME;
...
int GDMA_Lock_Vsync_Sem(void)
{
		...
		// for speed up 
		if( g_pre_signal_gdma_fence == NULL) {			
...
int GDMA_CloneEnable(unsigned char flag)
{
	...
	rtd_pr_gdma_info("func=%s line=%d flag=%d GDMA_OSD11_reg=%x\n",__FUNCTION__,__LINE__,flag,GDMA_OSD1_reg);
	if(flag){
		...
		rtd_pr_gdma_info("func=%s line=%d flag=%d rtd_inl(GDMA_OSD1_WI_reg)=%x\n",__FUNCTION__,__LINE__,flag,rtd_inl(GDMA_OSD1_WI_reg));
...
void GAL_HSR_Set_disable_OSD(int flag)
{
	HSR_OSD_disable = flag;
}
int GAL_Runtime_Suspend_disable_OSD(void)
{
	...
	//wirte 1 to clear
	rtd_outl(GDMA_OSD2_CTRL_reg,GDMA_OSD2_CTRL_osd2_en(1));
	rtd_outl(GDMA_CTRL_reg, progDone);
	gdma_usleep(16000);
...
int GAL_Runtime_Resume_receive(void)
{
  ...
  GDMA_ConfigOSDxEnableFast(main_disPlane,true);
...
int GAL_Runtime_Suspend(void)
{
  ...
  if(!GAL_Runtime_Suspend_status){
	 GDMA_ConfigOSDxEnableFast(main_disPlane,false);
	  ...
	  if(rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg) & SYS_REG_SYS_DISPCLKSEL_clken_disp_gdma_mask)
  	 {	
	 	CRT_CLK_OnOff(GDMA, CLK_OFF, &gdma_data);	 
		rtd_pr_gdma_emerg("[GDMA][%d] gdma clock 0xb8000208=%x \n",__LINE__,rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg));
  	 }
     return 0;
  }
	gdma = &gdma_devices[0];
...
int GDMA_ConfigNonBlocking (unsigned char enable)
{
	...
	*/
	...
	gSemaphore[main_disPlane] = gSEMAPHORE_MAX = (gdma->ctrl.enableNonBlocking) ? 2 : 1;

	rtd_pr_gdma_emerg("GDMA: Config Non-Blocking mode %d, gSemaphore[%d]=%d\n", gdma->ctrl.enableNonBlocking,main_disPlane,gSemaphore[main_disPlane]);
	...
	if( unlikely(gDebugExp_loglevel >= print_level ) ) 
		{
			...
			int64_t vs_diff_sw = gGDMA_VSYNC_SW_TIME - gGDMA_VSYNC_SW_TIME_PREV;
		
			int64_t send_to_vsync_diff = (gGDMA_EXPTime_Send - gGDMA_VSYNC_USER_TIME)/1000;
			//int64_t send_to_prevVsync_diff = (gGDMA_EXPTime_Send - gGDMA_VSYNC_USER_TIME_PREV)/1000;
			int64_t exp_to_prevVync_diff = (gGDMA_LAST_EXP_Time - gGDMA_VSYNC_USER_TIME_PREV)/1000;
	
			int64_t prev_expTime_diff = (gGDMA_PIC_LAST_SIGNAL_Time - gGDMA_LAST_EXP_Time)/1000;
			//int64_t wait_expect = ( gGDMA_EXPTime_StartWait - gGDMA_StartWait_VsyncTime) / 1000;
			//int64_t wait_vsync_diff = ( gGDMA_EXPTime_StartWait - gGDMA_VSYNC_USER_TIME) / 1000;
			int64_t real_wait = (  cur_time	- gGDMA_EXPTime_StartWait) / 1000;  //this func assume be called immediately after receivePicture()

			int64_t exp_wait = ( cur_expectedPresentTime - gGDMA_StartWait_VsyncTime) / 1000;
			
			if( cur_expectedPresentTime != -1 ) {
			   	complete_diff = ( cur_expectedPresentTime - gGDMA_VSYNC_USER_TIME) / 1000;

				exp_to_vsync_diff = (cur_expectedPresentTime  - gGDMA_VSYNC_USER_TIME)/1000;
			}
			else {
				exp_to_vsync_diff = (gGDMA_LAST_EXP_Time  - gGDMA_VSYNC_USER_TIME)/1000;
			}

			
							
			rtd_pr_gdma_crit("expT_0x%x prevExpDif:%lld compDif:%lld curVS:%lld(%lld) expWait:%lld realWait:%lld exp2vsyncDif:%lld prevSendDif:%lld  exp2prevVsync_Dif:%lld "
						"lastExp:%lld lastSig:%lld stVsync:%lld send:%lld curExp:%lld  cur:%lld vsDiv:%d swVS:%u vs:%u \n", tag, 
						prev_expTime_diff, complete_diff, gGDMA_VSYNC_USER_TIME, gGDMA_VSYNC_SW_TIME, 
						exp_wait, real_wait, exp_to_vsync_diff, send_to_vsync_diff, exp_to_prevVync_diff, 
						gGDMA_LAST_EXP_Time, gGDMA_PIC_LAST_SIGNAL_Time, gGDMA_StartWait_VsyncTime, gGDMA_EXPTime_Send, cur_expectedPresentTime, cur_time, gRefreshInterval, 
						vs_diff_sw, gVsyncDiff_NS );
		}					
...
int GDMA_ReceiveGraphicLayers_exp(GRAPHIC_LAYERS_OBJECT_V2 *v2_data)
{
	...
	startWait_vsyncTime = gGDMA_VSYNC_USER_TIME;  // gGDMA_VSYNC_USER_TIME is ns
...
int GDMA_ReceiveGraphicLayers(GRAPHIC_LAYERS_OBJECT *data)
{
	...
	if(enable_stop_update)
	{
		rtd_pr_gdma_emerg("%s enable_stop_update \n",__FUNCTION__);
		return GDMA_SUCCESS;
	}
	...
	if (data->layer_num > GDMA_LAYER_NUM) {
		rtd_pr_gdma_emerg("%s, Warning. too many layers \n", __FUNCTION__);
		return -100;
	}
	...
	*/
	...
	for (loop = 0; loop < data->layer_num; loop++) {
			...
			if (hasCompress) {
				...
				if (data->layer[loop].decompress) {
					...
					if (loop != 0 && data->layer[loop-1].decompress) {
						...
						if(onlinePlane == VO_GRAPHIC_OSD2 )
							onlinePlane = VO_GRAPHIC_OSD3;
						else
							onlinePlane = (VO_GRAPHIC_PLANE)(onlinePlane + 1);
					}
		...
		if (data->layer[loop].src_type == SRC_FBDC) {
			...
			if(picLayerObj->layer[layerIdx].fbdc[fbdcIdx].height <=1080 && 
			picLayerObj->layer[layerIdx].fbdc[fbdcIdx].width <= 1920)
			{
				main_disPlane = GDMA_PLANE_OSD1;
				sub_disPlane = GDMA_PLANE_OSD2;
				main_graphicPlane = VO_GRAPHIC_OSD1;
				sub_graphicPlane = VO_GRAPHIC_OSD2;
				onlinePlane = VO_GRAPHIC_OSD1;
			}
			else if(picLayerObj->layer[layerIdx].fbdc[fbdcIdx].height >1080 && 
			picLayerObj->layer[layerIdx].fbdc[fbdcIdx].width > 1920)
			{
				main_disPlane = GDMA_PLANE_OSD2;
				sub_disPlane = GDMA_PLANE_OSD1;
				main_graphicPlane = VO_GRAPHIC_OSD2;
				sub_graphicPlane = VO_GRAPHIC_OSD1;
				onlinePlane = VO_GRAPHIC_OSD2;
			}
			if (hasCompress) {
		...
		if (data->layer[loop].src_type == SRC_NORMAL
		   || data->layer[loop].src_type == SRC_NORMAL_1) {
			...
			if(picLayerObj->layer[layerIdx].normal[normalIdx].height <=1080 && 
			picLayerObj->layer[layerIdx].normal[normalIdx].width <= 1920)
			{
				main_disPlane = GDMA_PLANE_OSD1;
				sub_disPlane = GDMA_PLANE_OSD2;
				main_graphicPlane = VO_GRAPHIC_OSD1;
			...
			else if(picLayerObj->layer[layerIdx].normal[normalIdx].height >1080 && 
			picLayerObj->layer[layerIdx].normal[normalIdx].width > 1920)
			{
				main_disPlane = GDMA_PLANE_OSD2;
				sub_disPlane = GDMA_PLANE_OSD1;
	...
	if(data->cookie == NONBLOCKING_COOKIE){
		...
		if (gdmaLayerNum > 0){
			...
			if(gdma_suspend_disable_osd == 1){
				GDMA_ConfigOSDxEnableFast(main_disPlane, 1);
...
int GDMA_AI_SE_draw_block(int s_w, int s_h, int num, unsigned int *color, KGAL_RECT_T *ai_block, unsigned char draw_scene)
{
	...
	if (num == 0) {
		GDMA_ConfigOSDxEnableFast(sub_disPlane, false);
		...
		if (AI_Enable == 0) {
			GDMA_ConfigOSDxEnableFast(sub_disPlane, true);
			AI_Enable = 1;
		}
	}
#if ADD_SE_PERFORMANCE
...
int GDMA_Boot_Image_draw_buffer(int s_w,int s_h,KGAL_PIXEL_FORMAT_T format,unsigned int *dst_phy)
{
	...
	GDMA_ReceivePicture(&patlayerBoot, false);
...
static inline void rtkgdma_dbg_EXECUTE(const int cmd_index, char **cmd_pointer)
{
	...
	char tmpStr[128] = {0};
	KADP_HAL_GAL_CTRL_T st;
	unsigned char tmp_cmd[512];
	int tmp_cmd_size = 512;
	int tmp_cmd_count=0;
	memset(tmp_cmd,0xff,tmp_cmd_size);
		...
		case GDMA_DBG_CMD_OSD_RUNTIME_CTL:
			{
				...
				if(data==1){
					...
					GDMA_ConfigOSDxMixerEnableNoDelay(main_disPlane,1);

					GAL_Runtime_Suspend_enable_OSD();
					GAL_Runtime_Resume_receive();		
				}else if(data==0){
					GDMA_ConfigOSDxMixerEnableNoDelay(main_disPlane,0);
					GAL_Runtime_Suspend_disable_OSD();
				}
			}
		case GDMA_DBG_CMD_OSD_BYPASS_SE_CMD:
			sscanf(*cmd_pointer, "%d \n", &tmp_cmd_count);
			call_se_ta_excute_cmd(tmp_cmd,tmp_cmd_count);
			rtd_pr_gdma_err("GDMA_DBG_CMD_OSD_BYPASS_SE_CMD execute");
			break;
...
EXPORT_SYMBOL(GAL_HSR_Set_disable_OSD);
...
unsigned int virt_to_phys(void * x)
{
...
void getDispSize(VO_RECTANGLE *disp_rec)
{
	...
	if (gdma->f_mixer_after_dfrc) {
		...
		/* 1: Vx2(1->2, 2->4) */
		/* 2: Hx2(1->2, 2->4) */
		/* 3: SR_bypass(1->1, 2->2, 4->4) */

		switch (display_timing_ctrl2.sr_mode) {
		case 0:
			disp_rec->width = active_h>>1;
			disp_rec->height = active_v>>1;
			break;
		case 1:
			disp_rec->width = active_h;
			disp_rec->height = active_v>>1;
...
int drv_scaleup(uint8_t enable, GDMA_DISPLAY_PLANE disPlane)
{
	...
	*(unsigned int *)&line_buffer_end_reg = rtd_inl(GDMA_line_buffer_end_reg);
	...
	if (line_buffer_end_reg.osdsr1_switch_to == disPlane)
		...
		DBG_SR_PRINT("%s, No match with OSDSRx_switch_to to OSD%d !! \n", __FUNCTION__, disPlane) ;
		return GDMA_SUCCESS;
	}
#endif
...
#if 0
			/* if ((osd_sr_scaleup_ctrl1_reg.h_zoom_en!=0)||(osd_sr_scaleup_ctrl0_reg.v_zoom_en!=0)) */
			/* 	fwif_color_set_scaleosd_sharpness_driver(0, 1); */
			/* else */
			/*     fwif_color_set_scaleosd_sharpness_driver(0, 0); */
#endif
...
int GDMA_init_module(unsigned int cmdbaddr)
{
	int j, k;
	gdma_dev *gdma = NULL;
	osdovl_mixer_ctrl2_RBUS mixer_ctrl2;
...
static void GDMA_OSDReg_Set(GDMA_DISPLAY_PLANE disPlane)
{
	...
	*/
	...
	rtd_outl(OSDOVL_Mixer_layer_sel_reg, osdovl_mixer_layer_sel_reg.regValue);
	DBG_PRINT("%s, GDMA_line_buffer_end_reg=0x%x, OSDOVL_Mixer_layer_sel_reg=0x%x \n", __FUNCTION__, rtd_inl(GDMA_line_buffer_end_reg), osdovl_mixer_layer_sel_reg.regValue);
...
static void GDMA_PreOSDReg_Set(struct gdma_receive_work *ptr_work)
{
	...
	if (win->used) {
		...
		/* default OSDSRx_switch_to with online path */
#if 0
		pReg->lb_end_reg.osdsr1_switch_to = GDMA_PLANE_OSD1;
		pReg->lb_end_reg.osdsr2_switch_to = GDMA_PLANE_OSD3;
#endif
		/* whether non-compressed or normal or FBDC layer, canvas always is source size
		*  this is different between mac3 and magellan2, mac3's compressed image that canvas is panel size
		*/
...
#if 0
		hout = win->dst_width ? win->dst_width : (gdma->dispWin[disPlane].width);
		vout = win->dst_height ? win->dst_height : (gdma->dispWin[disPlane].height);
...
picObj->dst_y; //picObj->y; */
			...
			for (idx = 0; idx < com_picObj->fbdc_num; idx++) {
				...
				if (idx == 0) {     /*  FBDC1 */
					...
					win->fbdc_attr.fbdc_3rd_en = 1;
					win->fbdc_attr.const_a_3rd = ((win->fbdc_format.alpha_3rd = picObj->alpha) > 0) ? 1 : 0;
					win->fbdc_attr.plane_alpha_c2 = (picObj->plane_ar.plane_alpha_a != 0xff || picObj->plane_ar.plane_alpha_r != 0xff ||picObj->plane_gb.plane_alpha_g != 0xff || picObj->plane_gb.plane_alpha_b != 0xff) ? 1 : 0;
					win->fbdc_plane_alpha_c0.plane_alpha_a = (picObj->plane_ar.plane_alpha_a > 255) ? 255 : picObj->plane_ar.plane_alpha_a;
					win->fbdc_plane_alpha_c0.plane_alpha_r = (picObj->plane_ar.plane_alpha_r > 255) ? 255 : picObj->plane_ar.plane_alpha_r;
					win->fbdc_plane_alpha_c0.plane_alpha_g = (picObj->plane_gb.plane_alpha_g > 255) ? 255 : picObj->plane_gb.plane_alpha_g;
					win->fbdc_plane_alpha_c0.plane_alpha_b = (picObj->plane_gb.plane_alpha_b > 255) ? 255 : picObj->plane_gb.plane_alpha_b;
...
int GDMA_ConfigOSDEnable (bool flag)
{
	...
	*(volatile u32 *)&mixer_ctrl2 = rtd_inl(OSDOVL_Mixer_CTRL2_reg);
	...
	/* [ML8QC-639] Set b802b0a0[0]=1, to make OSD enabled for "QuickShow" feature */
	rtd_outl(OSDOVL_OSD_Db_Ctrl_reg, rtd_inl(OSDOVL_OSD_Db_Ctrl_reg | 0x1));
...
void GDMA_Clk_Select(void)
{
	...
	/* pick higher freq. as GDMA clock source */
	//printf("[GDMA]BUSH = %d ; DISP = %d\n", GDMA_BusH_DClk_info(0), GDMA_BusH_DClk_info(1));
	printf("[GDMA](Frome bdinfo)BUSH = %d ; DISP = %d\n", get_bush_pll(), get_disp_pll());

	if (get_bush_pll() > get_disp_pll())
		rtd_outl(SYS_REG_SYS_DISPCLKSEL_reg, rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg) & ~(1<<14));
	else
		rtd_outl(SYS_REG_SYS_DISPCLKSEL_reg, rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg) | (1<<14));
...
void GDMA_Clear (void)
{
	...
	while ((rtd_inl(GDMA_CTRL_reg)&GDMA_CTRL_osd1_prog_done_mask) != 0) {
         ...
         to_cnt++;
         if (to_cnt > 40) {
             printf("[GDMA][%s] GDMA HW something wrong. Please check \n", __FUNCTION__);
             break;
         }
     }
}
