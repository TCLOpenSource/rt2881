...
extern GDMA_DISPLAY_PLANE main_disPlane;
extern GDMA_DISPLAY_PLANE sub_disPlane;
extern VO_GRAPHIC_PLANE main_graphicPlane;
extern VO_GRAPHIC_PLANE sub_graphicPlane;
...
int GDMA_hal_gal_resetframebuffer(unsigned int fbHWControllerIndex)
{
	...
	if (gf_data->bVisibility) {
		...
		case 0:
			...
			break;
		case 1:
			GDMA_ConfigOSDxEnable(sub_disPlane, 1);
			break;
		case 2:
		...
		case 0:
			...
			break;
		case 1:
			GDMA_ConfigOSDxEnable(sub_disPlane, 0);
			break;
		case 2:
...
int GDMA_hal_gal_setfbhwcontrollerstereoscope(KDRV_GAL_FRAMEBUFFER_STEREOSCOPE_T *data)
{
	...
	case KADP_HAL_GAL_STEREOSCOPE_2D_TO_3D:
		if (gf_data->bStereoscope) {
			...
			data_3d.offset = 32;
			if (fb_index == 0) {
				data_3d.plane = main_graphicPlane;
				GDMA_Config3DOffset(&data_3d);
			} else if (fb_index == 1) {
				data_3d.plane = sub_graphicPlane;
				GDMA_Config3DOffset(&data_3d);
			}
		} else {
			data_3d.closeToViewer = 1;
