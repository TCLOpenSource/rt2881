...
bool se1_hwsem_lock(void)
{
	unsigned int enter_time = 0;
	unsigned int check_time = 0;
	enter_time = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
	while(!rtd_inl(SB2_HD_SEM_NEW2_1_reg))
	{
		check_time = rtd_inl(TIMER_SCPU_CLK90K_LO_reg);
		if ( (check_time - enter_time)/90 > 10000 )
		{
			rtd_pr_se_err("[%s]wait se_hwsem_lock too long\n",__FUNCTION__);

			return false;
		}
	}
	rtd_outl(SB2_HD_SEM_NEW2_1_reg, 1);
	return true;
}

void se1_hwsem_unlock(void)
{
	rtd_outl(SB2_HD_SEM_NEW2_1_reg, 0);
}
...
long rtk_se_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
	...
	case SE_IOC_CMD_UNLOCK:
		...
		up(&sem_cmdqueue);
		break;		
	case SE_IOC_CMD_FUNC_LOCK:
	{

		if(down_timeout(&sem_pro_func, SE_CMD_TIMEOUT)) {
			retval = -EINTR;
			goto out;
		}
#if PW_SAVING
	...
	case SE_IOC_SETPALTE:
	{
		...
		if (copy_from_user((void *)&pale, (const void __user *)arg,
			...
			if(KGAL_SetSurfacePalette((KGAL_PALETTE_SURFACE_INFO_T*)&pale, true) != true){
				retval = -EFAULT;
			}
		}
		//kfree(pale);
	...
	case SE_IOC_NV12_STCHBLIT:
	{
		
		KADP_SE_NV12_STCHBLIT_ST bst;
		if (copy_from_user((void *)&bst, (const void __user *)arg,
				sizeof(KADP_SE_NV12_STCHBLIT_ST))) {
			retval = -EFAULT;
			rtd_pr_se_err("[SE]NV12 stchblit get value fail!\n");
		}else {
			lord_kspace_p = current->pid;
			lord_uspace_p = bst.pinfo.m_pid;
			lord_uspace_pp = bst.pinfo.m_ppid;
			lord_uspace_t = bst.pinfo.m_tid;
			if(KGAL_NV12_StretchBlit(&bst.tSrcSurface, &bst.tSrcRect,
				&bst.tDstSurface, &bst.tDstRect, &bst.tBlitFlags,
				&bst.tBlitSettings,bst.src_c1_addr,bst.dst_c1_addr) != true) {
				retval = -EFAULT;
			}
		}
		break;
...
static int se_probe(struct platform_device *pdev)
{
	...
	se_devices->size = SE_COMMAND_ENTRIES_1;
...
//#endif	
	...
	while((rtd_inl(0xb800CE90)&0x1) !=0x01){
		...
		if(retry++ > 1000)
		{
			rtd_pr_se_err("[SE]%s-%d timeout retry=%d \n.",__func__,__LINE__, retry);
			break;
		}
	}
...
void WriteCmdTee(uint8_t* pbyCommandBuffer,		int32_t lCommandLength) {
	if (enable_stop_update_se)
		return;
	call_se_ta_excute_cmd(pbyCommandBuffer,lCommandLength);
