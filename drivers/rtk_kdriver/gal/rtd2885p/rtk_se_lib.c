...
#define  SE_BASE_1_reg               0xB800C004
	...
	uint32_t			dwRegisterValue)
{
	...
	WriteCmd(se_devices, (uint8_t *) &SeCmdWriteRegister, sizeof(SECMD_WRITE_REGISTER));
	return true;
}
	...
	uint32_t		dwRegisterValue4)
{
	...
	if(se_use_tee)
	{
		reg_addr = SE_BASE_1_reg + wRegisterAddress1;
		rtd_outl(reg_addr, dwRegisterValue1);		
		reg_addr = SE_BASE_1_reg + wRegisterAddress2;
		rtd_outl(reg_addr, dwRegisterValue2);
		reg_addr = SE_BASE_1_reg + wRegisterAddress3;
		rtd_outl(reg_addr, dwRegisterValue3);
		reg_addr = SE_BASE_1_reg + wRegisterAddress4;
		rtd_outl(reg_addr, dwRegisterValue4);
		//WriteCmdTee((uint8_t *) &SeCmdWriteRegister, sizeof(SECMD_WRITE_REGISTER));
	}
	else
		WriteCmd(se_devices, (uint8_t *) &SeCmdWriteRegister, sizeof(SECMD_WRITE_REGISTER));
	...
	void*			pExtraParam)
{
...
#if 0
	{
		...
		for(i=0;i<sizeof(SECMD_EXT_STCH_BLIT)/4;i++)
			rtd_pr_se_err("data= 0x%08x \n",*(ptr+i));
		rtd_pr_se_err("\n ========= start ========= \n ");
	}
#endif
	if(se_use_tee)
		WriteCmdTee((uint8_t *) &se_ext_stch, sizeof(SECMD_EXT_STCH_BLIT));
	else
	...
	unsigned char from_ioctl)
{
	...
	for(step=0;step < 128; step++) {
		...
		if(slot1 > size) {
			...
			if((SeCmdCLUT[step].Word2.RegisterValue & 0xff000000) == 0 && from_ioctl) {
		...
		if(slot1 > size) {
			...
			if((SeCmdCLUT[step].Word4.RegisterValue & 0xff000000) == 0 && from_ioctl) {
...
)
{
	...
	if(pSeSourceProcInfo->uAddr!=0 && pSeSourceProcInfo->uAddr > max_addr){
		max_addr = pSeSourceProcInfo->uAddr;
		max_size = pSeSourceProcInfo->uPitch*(pSeSourceProcInfo->wDataY +pSeSourceProcInfo->wDataHeight);
	}

	if(pSeSourceProcInfo->uAddr_c1!=0 && pSeSourceProcInfo->uAddr_c1 < min_addr)
		min_addr = pSeSourceProcInfo->uAddr_c1;

	if(pSeSourceProcInfo->uAddr_c1!=0 && pSeSourceProcInfo->uAddr_c1 > max_addr){
		max_addr = pSeSourceProcInfo->uAddr_c1;
	    max_size = pSeSourceProcInfo->uPitch_c1*(pSeSourceProcInfo->wDataY +pSeSourceProcInfo->wDataHeight);
	}

	if(pSeDestinationProcInfo->uAddr!=0 && pSeDestinationProcInfo->uAddr < min_addr)
		min_addr = pSeDestinationProcInfo->uAddr;

	if(pSeDestinationProcInfo->uAddr!=0 && pSeDestinationProcInfo->uAddr > max_addr){
		max_addr = pSeDestinationProcInfo->uAddr;
		max_size = pSeDestinationProcInfo->uPitch*(pSeDestinationProcInfo->wDataY +pSeDestinationProcInfo->wDataHeight);
	}

	if(pSeDestinationProcInfo->uAddr_c1!=0 && pSeDestinationProcInfo->uAddr_c1 < min_addr)
		min_addr = pSeDestinationProcInfo->uAddr_c1;

	if(pSeDestinationProcInfo->uAddr_c1!=0 && pSeDestinationProcInfo->uAddr_c1 > max_addr){
		max_addr = pSeDestinationProcInfo->uAddr_c1;
		max_size = pSeDestinationProcInfo->uPitch_c1*(pSeDestinationProcInfo->wDataY +pSeDestinationProcInfo->wDataHeight);
	}	
	if(max_addr < min_addr){
		se_ext_rop.WordReglock1.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock1_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock2.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock2_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock3.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock3_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock4.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock4_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock5.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock5_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock6.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock6_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock7.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock7_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_rop.WordReglock8.OpCode = SEINFO_OPCODE_NOP;
		se_ext_rop.WordReglock8_1.RegisterValue = SEINFO_OPCODE_NOP;
	

	}else{
		se_ext_rop.WordReglock1.Reserved = 0;
		se_ext_rop.WordReglock1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_rop.WordReglock1.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_rop.WordReglock1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_rop.WordReglock1_1.RegisterValue = 0;

		se_ext_rop.WordReglock2.Reserved = 0;
		se_ext_rop.WordReglock2.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_rop.WordReglock2.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_rop.WordReglock2.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
		se_ext_rop.WordReglock2_1.RegisterValue = (min_addr&0xffffff00);

		se_ext_rop.WordReglock3.Reserved = 0;
		se_ext_rop.WordReglock3.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_rop.WordReglock3.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_rop.WordReglock3.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
		se_ext_rop.WordReglock3_1.RegisterValue = ((max_addr+max_size+0x100)&0xffffff00);

		se_ext_rop.WordReglock4.Reserved = 0;
		se_ext_rop.WordReglock4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_rop.WordReglock4.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_rop.WordReglock4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_rop.WordReglock4_1.RegisterValue = 1;


		se_ext_rop.WordReglock5.Reserved = 0;
		se_ext_rop.WordReglock5.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_rop.WordReglock5.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_rop.WordReglock5.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_rop.WordReglock5_1.RegisterValue = 0;
...
//-------------------------------------------------------------
	...
	if(max_addr < min_addr){
		se_ext_conv.WordReglock1.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock1_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock2.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock2_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock3.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock3_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock4.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock4_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock5.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock5_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock6.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock6_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock7.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock7_1.RegisterValue = SEINFO_OPCODE_NOP;

		se_ext_conv.WordReglock8.OpCode = SEINFO_OPCODE_NOP;
		se_ext_conv.WordReglock8_1.RegisterValue = SEINFO_OPCODE_NOP;
	

	}else{
		se_ext_conv.WordReglock1.Reserved = 0;
		se_ext_conv.WordReglock1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_conv.WordReglock1.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_conv.WordReglock1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_conv.WordReglock1_1.RegisterValue = 0;

		se_ext_conv.WordReglock2.Reserved = 0;
		se_ext_conv.WordReglock2.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_conv.WordReglock2.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_conv.WordReglock2.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
		se_ext_conv.WordReglock2_1.RegisterValue = (min_addr&0xffffff00);

		se_ext_conv.WordReglock3.Reserved = 0;
		se_ext_conv.WordReglock3.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_conv.WordReglock3.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_conv.WordReglock3.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
		se_ext_conv.WordReglock3_1.RegisterValue = ((max_addr+max_size+0x100)&0xffffff00);

		se_ext_conv.WordReglock4.Reserved = 0;
		se_ext_conv.WordReglock4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_conv.WordReglock4.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_conv.WordReglock4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_conv.WordReglock4_1.RegisterValue = 1;


		se_ext_conv.WordReglock5.Reserved = 0;
		se_ext_conv.WordReglock5.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		se_ext_conv.WordReglock5.Clut = SEINFO_REGMODE_REGISTER;
		se_ext_conv.WordReglock5.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		se_ext_conv.WordReglock5_1.RegisterValue = 0;
...
int SE_EXT_FilmDetect(SE_FMT_ST *pInfo)
{
	...
	if(pInfo->s1_Addr!=0 && pInfo->s1_Addr < min_addr)
		min_addr = pInfo->s1_Addr;

	if(pInfo->s1_Addr!=0 && pInfo->s1_Addr > max_addr){
		max_addr = pInfo->s1_Addr;
		max_size = pInfo->width*pInfo->height;
	}


	if(pInfo->s2_Addr!=0 && pInfo->s2_Addr < min_addr)
		min_addr = pInfo->s2_Addr;

	if(pInfo->s2_Addr!=0 && pInfo->s2_Addr > max_addr){
		max_addr = pInfo->s2_Addr;
		max_size = pInfo->width*pInfo->height;
	}

	if(max_addr < min_addr){
		SECmd_Ext_Fmt_Conv.WordReglock1.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock1_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock2.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock2_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock3.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock3_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock4.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock4_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock5.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock5_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock6.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock6_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock7.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock7_1.RegisterValue = SEINFO_OPCODE_NOP;

		SECmd_Ext_Fmt_Conv.WordReglock8.OpCode = SEINFO_OPCODE_NOP;
		SECmd_Ext_Fmt_Conv.WordReglock8_1.RegisterValue = SEINFO_OPCODE_NOP;
	

	}else{
		SECmd_Ext_Fmt_Conv.WordReglock1.Reserved = 0;
		SECmd_Ext_Fmt_Conv.WordReglock1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock1.Clut = SEINFO_REGMODE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		SECmd_Ext_Fmt_Conv.WordReglock1_1.RegisterValue = 0;

		SECmd_Ext_Fmt_Conv.WordReglock2.Reserved = 0;
		SECmd_Ext_Fmt_Conv.WordReglock2.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock2.Clut = SEINFO_REGMODE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock2.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
		SECmd_Ext_Fmt_Conv.WordReglock2_1.RegisterValue = (min_addr&0xffffff00);

		SECmd_Ext_Fmt_Conv.WordReglock3.Reserved = 0;
		SECmd_Ext_Fmt_Conv.WordReglock3.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock3.Clut = SEINFO_REGMODE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock3.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
		SECmd_Ext_Fmt_Conv.WordReglock3_1.RegisterValue = ((max_addr+max_size+0x100)&0xffffff00);

		SECmd_Ext_Fmt_Conv.WordReglock4.Reserved = 0;
		SECmd_Ext_Fmt_Conv.WordReglock4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock4.Clut = SEINFO_REGMODE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		SECmd_Ext_Fmt_Conv.WordReglock4_1.RegisterValue = 1;


		SECmd_Ext_Fmt_Conv.WordReglock5.Reserved = 0;
		SECmd_Ext_Fmt_Conv.WordReglock5.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock5.Clut = SEINFO_REGMODE_REGISTER;
		SECmd_Ext_Fmt_Conv.WordReglock5.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
		SECmd_Ext_Fmt_Conv.WordReglock5_1.RegisterValue = 0;
...
bool SE_PQMask_Label2Weight(
	...
	// address limit
	uint32_t LimitAddrSta, uint32_t LimitAddrEnd
	)
{

	SECMD_PQMASK_EXT_FMT_CONV se_ext_conv;
	SEREG_CLR_FMT SeRegColorFormat;
	memset((void *)&se_ext_conv, 0, sizeof(SECMD_PQMASK_EXT_FMT_CONV));
	memset((void *)&SeRegColorFormat, 0, sizeof(SEREG_CLR_FMT));

	// command summary (32 words)
	// Word Reg1, Reg1_1 : basic setting
	// Word Reg2, Reg2_1 : swap fbdc
	// Word Reg3, Reg3_1 : source color
	// Word Reg4, Reg4_1 : disable limit address
	// Word Reg5, Reg5_1 : setup limit start address
	// Word Reg6, Reg6_1 : setup limit end address
	// Word Reg7, Reg7_1 : enable limit address
	// Word Addr0 : source address
	// Word Pitch0: source pitch
	// Word Addr1 : dest y address
	// Word Pitch1: dest y pitch
	// Word Addr2 : dest u address
	// Word Pitch2: dest u pitch
	// Word Addr3 : dest v address
	// Word Pitch3: dest v address
	// Word1 ~ Word8 : SE combo command
	// Word PostReg1, PostReg1_1  : disable limit address
	...
	// disable limit address
	se_ext_conv.WordReg4.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordReg4.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordReg4.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_conv.WordReg4_1.RegisterValue = 0;

	// setup limit start address
	se_ext_conv.WordReg5.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordReg5.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordReg5.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
	se_ext_conv.WordReg5_1.RegisterValue = LimitAddrSta;

	// setup limit end address
	se_ext_conv.WordReg6.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordReg6.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordReg6.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
	se_ext_conv.WordReg6_1.RegisterValue = LimitAddrEnd;

	// enable limit address
	se_ext_conv.WordReg7.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordReg7.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordReg7.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_conv.WordReg7_1.RegisterValue = 1;
	...
	// disable limit address
	se_ext_conv.WordPostReg1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_conv.WordPostReg1.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_conv.WordPostReg1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_conv.WordPostReg1_1.RegisterValue = 0;

#if 0
	{
		int i=0;
		rtd_pr_gdma_err("[%s][%d] print cmd with size %d\n", __func__, __LINE__, sizeof(SECMD_PQMASK_EXT_FMT_CONV));
		for(i=0; i<(sizeof(SECMD_PQMASK_EXT_FMT_CONV)/4); i++) {
			rtd_pr_gdma_err("[%2d] 0x%08x\n", i, ((unsigned int *)&se_ext_conv)[i]);
		}
	}
#endif

	WriteCmd(se_devices, (uint8_t *)&se_ext_conv, sizeof(SECMD_PQMASK_EXT_FMT_CONV));
	...
	)
{
	...
	// command summary (48 words)
	// Word Reg1 , Reg1_1  : basic setting
	// Word Reg2 , Reg2_1  : dest y info (address)
	// Word Reg3 , Reg3_1  : dest y info (pitch)
	// Word Reg4 , Reg4_1  : strech setting
	// Word Reg5 , Reg5_1  : dest u info (address)
	// Word Reg6 , Reg6_1  : dest u info (pitch)
	// Word Reg7 , Reg7_1  : vertical scaling factor
	// Word Reg8 , Reg8_1  : horizontal scaling factor
	// Word Reg9 , Reg9_1  : disable limit address
	// Word Reg10, Reg10_1 : setup limit start address
	// Word Reg11, Reg11_1 : setup limit end address
	// Word Reg12, Reg12_1 : enable limit address
	// Word Addr0 : source y info (address)
	// Word Pitch0: source y info (pitch)
	...
	// Word1 ~ Word12 : SE combo command
	// Word PostReg1, PostReg1_1 : disable limit address
	// Word PostReg2, PostReg2_1 : NOP
	...
	se_ext_stch.WordReg9.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordReg9.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg9.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_stch.WordReg9_1.RegisterValue = 0;

	// setup limit start address
	se_ext_stch.WordReg10.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordReg10.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg10.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
	se_ext_stch.WordReg10_1.RegisterValue = LimitAddrSta;

	// setup limit end address
	se_ext_stch.WordReg11.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordReg11.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg11.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
	se_ext_stch.WordReg11_1.RegisterValue = LimitAddrEnd;

	// enable limit address
	se_ext_stch.WordReg12.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordReg12.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordReg12.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_stch.WordReg12_1.RegisterValue = 1;
	...
	// disable limit address
	se_ext_stch.WordPostReg1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_stch.WordPostReg1.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_stch.WordPostReg1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_stch.WordPostReg1_1.RegisterValue = 0;

	// NOP
	se_ext_stch.WordPostReg2.OpCode = SEINFO_OPCODE_NOP;
	se_ext_stch.WordPostReg2_1.RegisterValue = SEINFO_OPCODE_NOP;
...
bool SE_PQMask_Stretch_NV12_Y(
	...
	SEINFO_STRETCH		byScalingAlgorithmSelect,
	// address limit
	uint32_t LimitAddrSta,
	uint32_t LimitAddrEnd)
{
	...
	// Word Reg3 , Reg3_1  : dest y info (pitch)
	// Word Reg4 , Reg4_1  : strech setting
	// Word Reg5 , Reg5_1  : NOP
	// Word Reg6 , Reg6_1  : NOP
	// Word Reg7 , Reg7_1  : vertical scaling factor
	// Word Reg8 , Reg8_1  : horizontal scaling factor
	// Word Reg9 , Reg9_1  : disable limit address
	// Word Reg10, Reg10_1 : setup limit start address
...
{
	// local parameters
	SECMD_PQMASK_EXT_2LAYER_BITBLIT se_ext_2layer_bitblit;
	SEREG_CLR_FMT SeRegColorFormat;

	memset((void *)&se_ext_2layer_bitblit, 0x0, sizeof(SECMD_PQMASK_EXT_2LAYER_BITBLIT));
	memset((void *)&SeRegColorFormat, 0x0, sizeof(SeRegColorFormat));

	// command summary
	// write register (72 words)
	// Word Reg1 , Reg1_1  : basic setting
	// Word Reg2 , Reg2_1  : source1 y info (address)
	// Word Reg3 , Reg3_1  : source1 y info (pitch)
	// Word Reg4 , Reg4_1  : source1 u info (address)
	// Word Reg5 , Reg5_1  : source1 u info (pitch)
	// Word Reg6 , Reg6_1  : source1 v info (address)
	// Word Reg7 , Reg7_1  : source1 v info (pitch)
	// Word Reg8 , Reg8_1  : source2 y info (address)
	// Word Reg9 , Reg9_1  : source2 y info (pitch)
	// Word Reg10, Reg10_1 : source2 u info (address)
	// Word Reg11, Reg11_1 : source2 u info (pitch)
	// Word Reg12, Reg12_1 : source2 v info (address)
	// Word Reg13, Reg13_1 : source2 v info (pitch)
	// Word Reg14, Reg14_1 : dst y int (address)
	// Word Reg15, Reg15_1 : dst y int (pitch)
	// Word Reg16, Reg16_1 : dst u int (address)
	// Word Reg17, Reg17_1 : dst u int (pitch)
	// Word Reg18, Reg18_1 : dst v int (address)
	// Word Reg19, Reg19_1 : dst v int (pitch)
	// Word Reg20, Reg20_1 : src alpha
	// Word Reg21, Reg21_1 : dst alpha
	// Word Reg22, Reg22_1 : rst alpha
	// Word Reg23, Reg23_1 : disable limit address
	// Word Reg24, Reg24_1 : setup limit start address
	// Word Reg25, Reg25_1 : setup limit end address
	// Word Reg26, Reg26_1 : enable limit address
	// combo command
	// Word1 ~ Word16 : SE combo command
	// Word PostReg1, PostReg1_1 : disable limit address
	// Word PostReg2, PostReg2_1 : NOP
	...
	// disable limit address
	se_ext_2layer_bitblit.WordReg23.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_2layer_bitblit.WordReg23.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_2layer_bitblit.WordReg23.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_2layer_bitblit.WordReg23_1.RegisterValue = 0;

	// setup limit start address
	se_ext_2layer_bitblit.WordReg24.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_2layer_bitblit.WordReg24.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_2layer_bitblit.WordReg24.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
	se_ext_2layer_bitblit.WordReg24_1.RegisterValue = LimitAddrSta;

	// setup limit end address
	se_ext_2layer_bitblit.WordReg25.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_2layer_bitblit.WordReg25.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_2layer_bitblit.WordReg25.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
	se_ext_2layer_bitblit.WordReg25_1.RegisterValue = LimitAddrEnd;

	// enable limit address
	se_ext_2layer_bitblit.WordReg26.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_2layer_bitblit.WordReg26.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_2layer_bitblit.WordReg26.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_2layer_bitblit.WordReg26_1.RegisterValue = 1;
	...
	// se_ext_2layer_bitblit.Word6.src1_afbc_yuv_trans = 0; // ?
	...
	// disable limit address
	se_ext_2layer_bitblit.WordPostReg1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	se_ext_2layer_bitblit.WordPostReg1.Clut = SEINFO_REGMODE_REGISTER;
	se_ext_2layer_bitblit.WordPostReg1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	se_ext_2layer_bitblit.WordPostReg1_1.RegisterValue = 0;

	// NOP
	se_ext_2layer_bitblit.WordPostReg2.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordPostReg2_1.RegisterValue = SEINFO_OPCODE_NOP;

#if 0
	{
		int i=0;
		rtd_pr_se_err("[%s][%d] print cmd with size %ld\n", __func__, __LINE__, sizeof(SECMD_PQMASK_EXT_2LAYER_BITBLIT));
		for(i=0; i<(sizeof(SECMD_PQMASK_EXT_2LAYER_BITBLIT)/4); i++) {
			rtd_pr_se_err("[%2d] 0x%08x\n", i, ((unsigned int *)&se_ext_2layer_bitblit)[i]);
		}
	}
#endif

	WriteCmd(se_devices, (uint8_t *)&se_ext_2layer_bitblit, sizeof(SECMD_PQMASK_EXT_2LAYER_BITBLIT));

	return true;
}

bool SE_PQMask_Bitblit_MultiplyByAlpha(
	SE_PROCINFO_T *pSeSrc1YProcInfo,
	SE_PROCINFO_T *pSeSrc2YProcInfo,
	SE_PROCINFO_T *pSeDstYProcInfo,
	unsigned int SrcAlpha, unsigned int DstAlpha, unsigned int RltAlpha, unsigned char RoundSel,
	// address limit
	uint32_t LimitAddrSta, uint32_t LimitAddrEnd
)
{
	...
	// Word Reg2 , Reg2_1 : source1 y info (address)
	// Word Reg3 , Reg3_1 : source1 y info (pitch)
	// Word Reg4 , Reg4_1 : NOP
	// Word Reg5 , Reg5_1 : NOP
	// Word Reg6 , Reg6_1 : NOP
	// Word Reg7 , Reg7_1 : NOP
	// Word Reg8 , Reg8_1 : source2 y info (address)
	// Word Reg9 , Reg9_1 : source2 y info (pitch)
	// Word Reg10, Reg10_1 : NOP
	// Word Reg11, Reg11_1 : NOP
	// Word Reg12, Reg12_1 : NOP
	// Word Reg13, Reg13_1 : NOP
	// Word Reg14, Reg14_1 : dst y int (address)
	// Word Reg15, Reg15_1 : dst y int (pitch)
	// Word Reg16, Reg16_1 : NOP
	// Word Reg17, Reg17_1 : NOP
	// Word Reg18, Reg18_1 : NOP
	// Word Reg19, Reg19_1 : NOP
	// Word Reg20, Reg20_1 : src alpha
	// Word Reg21, Reg21_1 : dst alpha
	// Word Reg22, Reg22_1 : rst alpha
	// Word Reg23, Reg23_1 : disable limit address
	...
	// NOP
	se_ext_2layer_bitblit.WordReg4.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg4_1.RegisterValue = SEINFO_OPCODE_NOP;

	// NOP
	se_ext_2layer_bitblit.WordReg5.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg5_1.RegisterValue = SEINFO_OPCODE_NOP;

	//NOP
	se_ext_2layer_bitblit.WordReg6.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg6_1.RegisterValue = SEINFO_OPCODE_NOP;

	// NOP
	se_ext_2layer_bitblit.WordReg7.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg7_1.RegisterValue = SEINFO_OPCODE_NOP;
	...
	// NOP
	se_ext_2layer_bitblit.WordReg10.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg10_1.RegisterValue = SEINFO_OPCODE_NOP;

	// NOP
	se_ext_2layer_bitblit.WordReg11.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg11_1.RegisterValue = SEINFO_OPCODE_NOP;

	// NOP
	se_ext_2layer_bitblit.WordReg12.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg12_1.RegisterValue = SEINFO_OPCODE_NOP;

	// NOP
	se_ext_2layer_bitblit.WordReg13.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg13_1.RegisterValue = SEINFO_OPCODE_NOP;
	...
	// write register : destination u info (address)
	se_ext_2layer_bitblit.WordReg16.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg16_1.RegisterValue = SEINFO_OPCODE_NOP;

	// write register : destination u info (pitch)
	se_ext_2layer_bitblit.WordReg17.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg17_1.RegisterValue = SEINFO_OPCODE_NOP;

	// write register : destination v info (address)
	se_ext_2layer_bitblit.WordReg18.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg18_1.RegisterValue = SEINFO_OPCODE_NOP;

	// write register : destination v info (pitch)
	se_ext_2layer_bitblit.WordReg19.OpCode = SEINFO_OPCODE_NOP;
	se_ext_2layer_bitblit.WordReg19_1.RegisterValue = SEINFO_OPCODE_NOP;
	...
	// combo se command start
	se_ext_2layer_bitblit.Word1.opcode = SEINFO_OPCODE_EXT_FMT;
	se_ext_2layer_bitblit.Word1.rslt_fmt = SEINFO_EXT_COLOR_FORMAT_NV12_420_Y;
	se_ext_2layer_bitblit.Word1.lay_ctl = 0;
	se_ext_2layer_bitblit.Word1.lay_num = 1; // more than 1 layer
	se_ext_2layer_bitblit.Word1.rslt_argb_order = ARGBENDIAN; // ?
	se_ext_2layer_bitblit.Word1.src2_argb_order = ARGBENDIAN; // ?
	...
	se_ext_2layer_bitblit.Word3.rslt_addr_index = (uint32_t)pSeDstYProcInfo->byBaseAddressIndex;
	se_ext_2layer_bitblit.Word3.rslt_chr1_addr_index = 0;
	se_ext_2layer_bitblit.Word3.rslt_chr2_addr_index = 0;

	se_ext_2layer_bitblit.Word4.src1_fmt = SEINFO_EXT_COLOR_FORMAT_reserved3;		// ALPHA8
	se_ext_2layer_bitblit.Word4.src1_inpt_w = pSeSrc1YProcInfo->wDataWidth;
	se_ext_2layer_bitblit.Word4.src1_inpt_h = pSeSrc1YProcInfo->wDataHeight;
	...
	se_ext_2layer_bitblit.Word6.src1_inpt_sel = SEINFO_BITBLT_EXT_SELECT_MANUAL;
	se_ext_2layer_bitblit.Word6.src1_addr_index = (uint32_t)pSeSrc1YProcInfo->byBaseAddressIndex;
	se_ext_2layer_bitblit.Word6.src1_chr1_addr_index = 0;
	se_ext_2layer_bitblit.Word6.src1_chr2_addr_index = 0;

	se_ext_2layer_bitblit.Word7.src2_fmt = SEINFO_EXT_COLOR_FORMAT_NV12_420_Y;
	se_ext_2layer_bitblit.Word7.src2_mat_en = 0;
	se_ext_2layer_bitblit.Word7.src2_inpt_x = pSeSrc2YProcInfo->wDataX;
	se_ext_2layer_bitblit.Word7.src2_inpt_y = pSeSrc2YProcInfo->wDataY;
	...
	se_ext_2layer_bitblit.Word8.src2_tfbdc_lossy = 0;
	se_ext_2layer_bitblit.Word8.src2_addr_index = (uint32_t)pSeSrc2YProcInfo->byBaseAddressIndex;
	se_ext_2layer_bitblit.Word8.src2_chr1_addr_index = 0;
	se_ext_2layer_bitblit.Word8.src2_chr2_addr_index = 0;

	// alpha register 0x1800C0E0 ?
	se_ext_2layer_bitblit.Word9.lay_num_ext = 1; // 2 layer src fcv + stretch
	se_ext_2layer_bitblit.Word9.sel_s1_alpha = 0; // dst alpha
	se_ext_2layer_bitblit.Word9.sel_s2_alpha = 2; // src alpha, 2 = use src1 as alpha
...
bool SE_PQMask_Bitblit_NV12_Y(
	SE_PROCINFO_T *pSeSrc1YProcInfo, SE_PROCINFO_T *pSeSrc2YProcInfo, SE_PROCINFO_T *pSeDstYProcInfo,
	unsigned int SrcAlpha, unsigned int DstAlpha, unsigned int RltAlpha, unsigned char RoundSel,
	// address limit
	uint32_t LimitAddrSta, uint32_t LimitAddrEnd
)
{
	...
	se_ext_2layer_bitblit.Word4.src1_fmt = SEINFO_EXT_COLOR_FORMAT_NV12_420_Y;
	se_ext_2layer_bitblit.Word4.src1_inpt_w = pSeSrc1YProcInfo->wDataWidth;
	se_ext_2layer_bitblit.Word4.src1_inpt_h = pSeSrc1YProcInfo->wDataHeight;
...
bool SE_PQMask_Stretch_Coef_Bitblit(
	...
	// command summary
	// write register ()
	// Word Reg0 , Reg0_1  : basic setting
	// Word Reg1 , Reg1_1  : strech setting
	// Word Reg2 , Reg2_1  : vertical scaling factor
	// Word Reg3 , Reg3_1  : horizontal scaling factor
	// Word Reg4 , Reg4_1  : source1 y info (address)
	// Word Reg5 , Reg5_1  : source1 y info (pitch)
	// Word Reg6 , Reg6_1  : source1 u info (address)
	// Word Reg7 , Reg7_1  : source1 u info (pitch)
	// Word Reg8 , Reg8_1  : source1 v info (address)
	// Word Reg9 , Reg9_1  : source1 v info (pitch)
	// Word Reg10, Reg10_1 : source2 y info (address)
	// Word Reg11, Reg11_1 : source2 y info (pitch)
	// Word Reg12, Reg12_1 : source2 u info (address)
	// Word Reg13, Reg13_1 : source2 u info (pitch)
	// Word Reg14, Reg14_1 : source2 v info (address)
	// Word Reg15, Reg15_1 : source2 v info (pitch)
	// Word Reg16, Reg16_1 : dst y int (address)
	// Word Reg17, Reg17_1 : dst y int (pitch)
	// Word Reg18, Reg18_1 : dst u int (address)
	// Word Reg19, Reg19_1 : dst u int (pitch)
	// Word Reg20, Reg20_1 : dst v int (address)
	// Word Reg21, Reg21_1 : dst v int (pitch)
	// Word Reg22, Reg22_1 : src alpha
	// Word Reg23, Reg23_1 : dst alpha
	// Word Reg24, Reg24_1 : rst alpha
	// Word Reg25, Reg25_1 : disable limit address
	// Word Reg26, Reg26_1 : setup limit start address
	// Word Reg27, Reg27_1 : setup limit end address
	// Word Reg28, Reg28_1 : enable limit address
	// Dummy1 : NOP
	// combo command
	...
	// disable limit address
	Command.WordReg25.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	Command.WordReg25.Clut = SEINFO_REGMODE_REGISTER;
	Command.WordReg25.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	Command.WordReg25_1.RegisterValue = 0;

	// setup limit start address
	Command.WordReg26.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	Command.WordReg26.Clut = SEINFO_REGMODE_REGISTER;
	Command.WordReg26.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_STA;
	Command.WordReg26_1.RegisterValue = LimitAddrSta;

	// setup limit end address
	Command.WordReg27.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	Command.WordReg27.Clut = SEINFO_REGMODE_REGISTER;
	Command.WordReg27.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_END;
	Command.WordReg27_1.RegisterValue = LimitAddrEnd;

	// enable limit address
	Command.WordReg28.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	Command.WordReg28.Clut = SEINFO_REGMODE_REGISTER;
	Command.WordReg28.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	Command.WordReg28_1.RegisterValue = 1;
	...
	// disable limit address
	Command.WordPostReg1.OpCode = SEINFO_OPCODE_WRITE_REGISTER;
	Command.WordPostReg1.Clut = SEINFO_REGMODE_REGISTER;
	Command.WordPostReg1.RegisterAddress = (SEINFO_WRITEREG)SE20INFO_REG_LIMIT_EN;
	Command.WordPostReg1_1.RegisterValue = 0;

	// NOP
	Command.WordPostReg2.OpCode = SEINFO_OPCODE_NOP;
	Command.WordPostReg2_1.RegisterValue = SEINFO_OPCODE_NOP;
