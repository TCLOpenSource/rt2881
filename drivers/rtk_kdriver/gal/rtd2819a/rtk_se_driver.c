...
inline void InitSeReg(void)
{
	...
	rtd_outl(SE_TFBDC_CONFIG_reg,se_devices->tfbcd_config);
#if 1
#if 0
...
inline void StoreSeReg(void)
{
	...
	se_devices->tfbcd_config = rtd_inl(SE_TFBDC_CONFIG_reg);
#if SE_SE20 || SE_OLD
	if(se_s_index8_mem_phy) {
#if SE_SE20
...
static int se_probe(struct platform_device *pdev)
{
	...
	se_tfbdc_config_RBUS se_tfbdc_config;
#endif 
	...
	se_tfbdc_config.regValue = rtd_inl(SE_TFBDC_CONFIG_reg);
	se_tfbdc_config.tfbdc_lossy_group_control = 1;
	se_tfbdc_config.tfbdc_swizzle = 4;
	rtd_outl(SE_TFBDC_CONFIG_reg, se_tfbdc_config.regValue);
#endif
