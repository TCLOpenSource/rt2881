...
static bool inner_set_pmulti(SE_PROCINFO_T* ppi, KGAL_MULTI_SETTINGS_T* pms);
...
static bool inner_set_pmulti(SE_PROCINFO_T* ppi, KGAL_MULTI_SETTINGS_T* pms)
{
	if(pms->trans_start_addr1 !=0 && pms->trans_start_addr2 !=0)
	{
		ppi->ori_start_addr1 = pms->ori_start_addr1;
		ppi->ori_end_addr1 = pms->ori_end_addr1;
		ppi->trans_start_addr1 = pms->trans_start_addr1;
		ppi->trans_end_addr1 = pms->trans_end_addr1;
		ppi->ori_start_addr2 = pms->ori_start_addr2;
		ppi->ori_end_addr2 = pms->ori_end_addr2;
		ppi->trans_start_addr2 = pms->trans_start_addr2;
		ppi->trans_end_addr2 = pms->trans_end_addr2;
	}else
	{
		return false;
	}
...
static int inner_get_compression_ratio(unsigned int *ratio)
{
	...
	*ratio = GDMA_DBG_OSD_WI_30_TFBC_get_tfbc_req_lossy(rtd_inl(GDMA_DBG_OSD_WI_30_TFBC_reg));
...
bool inner_argb_fbc_stchblit_multi(KGAL_SURFACE_INFO_T* pSrcSurface, KGAL_RECT_T* pSrcRect, KGAL_SURFACE_INFO_T* pDstSurface, KGAL_RECT_T* pDstRect, KGAL_BLIT_FLAGS_T* pBlitFlags, KGAL_BLIT_SETTINGS_T* pBlitSettings, KGAL_MULTI_SETTINGS_T *pMultiSettings)
{
	...
	if(inner_set_pmulti(&stSrcProcInfo, pMultiSettings) == false)
		return false;
	if (pSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC)
		inner_get_compression_ratio(&stSrcProcInfo.uCompressionRatio);
...
bool KGAL_StretchBlit_Multi(KGAL_SURFACE_INFO_T* pSrcSurface, KGAL_RECT_T* pSrcRect, KGAL_SURFACE_INFO_T* pDstSurface, KGAL_RECT_T* pDstRect, KGAL_BLIT_FLAGS_T* pBlitFlags, KGAL_BLIT_SETTINGS_T* pBlitSettings, KGAL_MULTI_SETTINGS_T *pMultiSettings)
{
	SEINFO_SACLING_TYPE stVType = SEINFO_SCALING_DISABLE;
	SEINFO_SACLING_TYPE stHType = SEINFO_SCALING_DISABLE;
	SEINFO_SETUPMODE stMode = SEINFO_BITBLT_SETUP_MANUAL;
	...
	if (pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC) {
		rtd_pr_se_emerg("%s Unsupport encode TFBC!\n", __FUNCTION__);
		return false;
	}
	...
	if(pfSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC || pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC) {
		...
		if(inner_stretch_check(pSrcRect, pDstRect) == false) {
			...
			if((pDstRect->x+pDstRect->w)%16<=2) {
				...
				if(pfSrcSurface->pixelFormat != pDstSurface->pixelFormat) {
					return inner_argb_fbc_stchblit_multi(pfSrcSurface, pSrcRect, pDstSurface, &fbcDstRect, pBlitFlags, pBlitSettings, pMultiSettings);
				}else{
					return inner_fbc_stchblit(pSrcSurface, pSrcRect, pDstSurface, &fbcDstRect, pBlitFlags, pBlitSettings);
				}
			}
		...
		if(pfSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_ARGB && pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC) {
			return inner_argb_fbc_stchblit_multi(pfSrcSurface, pSrcRect, pDstSurface, pDstRect, pBlitFlags, pBlitSettings, pMultiSettings);
		}else if(pfSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC && pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_ARGB ) {
			return inner_argb_fbc_stchblit_multi(pfSrcSurface, pSrcRect, pDstSurface, pDstRect, pBlitFlags, pBlitSettings, pMultiSettings);
		}else if(pfSrcSurface->pixelFormat == KGAL_PIXEL_FORMAT_LUT8 && pDstSurface->pixelFormat == KGAL_PIXEL_FORMAT_FBC) {
			return inner_argb_fbc_stchblit_multi(pfSrcSurface, pSrcRect, pDstSurface, pDstRect, pBlitFlags, pBlitSettings, pMultiSettings);
...
EXPORT_SYMBOL(KGAL_StretchBlit_Multi);
