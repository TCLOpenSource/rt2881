		...
		GDMA_DISPLAY_PLANE disPlane, struct gdma_receive_work *ptr_work)
{
	...
	if((change_OSD_mode == GDMA_SPLIT_MODE_1 ||
	  ...
	  (!osd_sr_scaleup_ctrl0_reg.v_zoom_en && !osd_sr_scaleup_ctrl1_reg.h_zoom_en && disPlane == GDMA_PLANE_OSD1 && 
	  GDMA_Check_Clk_Identity_Enable())
		){
		//only osd1(1p to 2p) need to enable identity, osd2(1p to 1p) don't need
		osd_sr_scaleup_ctrl0_reg.v_zoom_en = 1;
