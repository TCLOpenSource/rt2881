...
unsigned char inner_pattern = 0;
EXPORT_SYMBOL(inner_pattern);
...
unsigned int gGDMA_FRAMERATE = 0;
...
int get_VsyncCountSW(void )
{
	...
	else if( local_hz < ((60*3)- 2 ) )
	{
		ret_count = 2;
	}
	else if( local_hz < ((60*4)- 2 ) )
	{
		ret_count = 3;
	}
	else if( local_hz < ((60*5)- 2 ) )
	{
		ret_count = 4;
	}
    else
...
#endif
	{
		...
		if (osd1_sync) {
			...
			if( vsync_count <= 0 ) {
				...
				gVsyncDiff_NS = (unsigned int)(gGDMA_VSYNC_USER_TIME - gGDMA_VSYNC_USER_TIME_PREV);
...
int GDMA_Update (gdma_dev *gdma, int inISR)
{
	...
	for (onlineIdx = cnt = 0; onlineIdx < onlineMaxNum; onlineIdx++) {
        ...
        mixer_ctrl2.regValue = rtd_inl(OSDOVL_Mixer_CTRL2_reg);
        if( gdma->curPic[disPlane] == 2 && mixer_init == 0 ){
            mixer_ctrl2.mixero1_en = 1;
//          mixer_ctrl2.mixero2_en = 0;
            mixer_ctrl2.mixero3_en = 0;
			if(inner_pattern == 0){
            ...
            mixer_init = 1;
            rtd_pr_gdma_debug("L:%d F:%s() ======= *Enable Mixer1 =inner_pattern=%d ====== \n", __LINE__, __func__,inner_pattern);
        }
        rtd_outl(OSDOVL_Mixer_CTRL2_reg, mixer_ctrl2.regValue);
...
//for ANDROIDTV-305 test
			...
			for (idx = 0; idx < com_picObj->fbdc_num; idx++) {
				...
				if (idx == 0) { 
					...
					if((picObj->width + picObj->dst_x) > disp_res.width)
					{
						win->afbc_xy_pixel.x_pic_pixel = disp_res.width - picObj->dst_x;
					}
					else
					{
						win->afbc_xy_pixel.x_pic_pixel = picObj->width;
					}
					if((picObj->height + picObj->dst_y) > disp_res.height){
						win->afbc_xy_pixel.y_pic_pixel = picObj->height - picObj->dst_y;
					}else{
...
#endif
                win->winWH.width = win->afbc_xy_pixel.x_pic_pixel;
                win->winWH.height = win->afbc_xy_pixel.y_pic_pixel;
	...
	dsb();
			...
			else if(disp_res.height == 2160 && picObj->dst_height == 1080 && hsr_on==1)
			{
				win->dst_height = picObj->dst_height = disp_res.height;
				hsr_on =0;
			}
			gdma->picCount[disPlane]++;
...
int GDMA_ConfigOSDxEnableWithoutMixer(GDMA_DISPLAY_PLANE disPlane, bool flag)
{
...
int GAL_Runtime_Resume_receive(void)
{
	...
	rtd_outl(GDMA_OSD1_CTRL_reg,GDMA_OSD1_CTRL_osd1_en(1) |GDMA_OSD1_CTRL_write_data(1));
	rtd_outl(GDMA_CTRL_reg, progDone);

	GDMA_ConfigOSDxEnableWithoutMixer(GDMA_PLANE_OSD1,true);//don't control mixer in android
...
int GAL_Runtime_Suspend_light(void)
{
  ...
  if(!GAL_Runtime_Suspend_status){
     GDMA_ConfigOSDxEnableWithoutMixer(GDMA_PLANE_OSD1,false);
     GAL_Runtime_Suspend_status = 1;
  }else{
     return 0;
  }
...
int GAL_Runtime_Resume_light(void)
{
  ...
  if(GAL_Runtime_Suspend_status){
	...
	GDMA_ConfigOSDxEnableWithoutMixer(GDMA_PLANE_OSD1,true);
...
void print_log_expTme( int tag, int print_level, int64_t  cur_expectedPresentTime)
{
	...
	if( unlikely(gDebugExp_loglevel >= print_level ) )
	{
		...
		rtd_pr_gdma_crit("expT_0x%x prevExpDif:%lld compDif:%lld expWait:%lld realWait:%lld exp2vsyncDif:%lld prevSendDif:%lld  exp2prevVsync_Dif:%lld "
			"lastExp:%lld lastSig:%lld stVsync:%lld send:%lld curExp:%lld  cur:%lld fp:%u vsDiv:%d swVS:%lld vs:%d\n", tag, 
			prev_expTime_diff, complete_diff, exp_wait, real_wait, exp_to_vsync_diff, send_to_vsync_diff, exp_to_prevVync_diff, 
			gGDMA_LAST_EXP_Time, gGDMA_PIC_LAST_SIGNAL_Time, gGDMA_StartWait_VsyncTime, gGDMA_EXPTime_Send, cur_expectedPresentTime, cur_time, gGDMA_FRAMERATE, gSWVsyncInterval, gGDMA_VSYNC_SW_DIFF, gVsyncDiff_NS );
	}
#else
	...
	if( unlikely(gDebugExp_loglevel >= print_level ) )
	{
		...
		int64_t send_to_vsync_diff = div_s64 ( gGDMA_EXPTime_Send - gGDMA_VSYNC_USER_TIME, 1000);
		//int64_t send_to_prevVsync_diff = (gGDMA_EXPTime_Send - gGDMA_VSYNC_USER_TIME_PREV)/1000;
		int64_t exp_to_prevVync_diff = div_s64( gGDMA_LAST_EXP_Time - gGDMA_VSYNC_USER_TIME_PREV, 1000);

		int64_t prev_expTime_diff = div_s64(gGDMA_PIC_LAST_SIGNAL_Time - gGDMA_LAST_EXP_Time, 1000) ;
		//int64_t wait_expect = ( gGDMA_EXPTime_StartWait - gGDMA_StartWait_VsyncTime) / 1000;
		//int64_t wait_vsync_diff = ( gGDMA_EXPTime_StartWait - gGDMA_VSYNC_USER_TIME) / 1000;
		int64_t real_wait =  div_s64( cur_time - gGDMA_EXPTime_StartWait, 1000);	//this func assume be called immediately after receivePicture()

		int64_t exp_wait = div_s64( cur_expectedPresentTime - gGDMA_StartWait_VsyncTime,  1000);

		if( cur_expectedPresentTime != -1 ) {
			complete_diff = div_s64( cur_expectedPresentTime - gGDMA_VSYNC_USER_TIME,  1000);

			exp_to_vsync_diff = div_s64(cur_expectedPresentTime  - gGDMA_VSYNC_USER_TIME, 1000);
		}
		else {
			exp_to_vsync_diff = div_s64(gGDMA_LAST_EXP_Time  - gGDMA_VSYNC_USER_TIME, 1000);
		}
