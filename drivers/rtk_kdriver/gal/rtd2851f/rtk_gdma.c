...
#define GDMA_EXPTIME_LOG(level,fmt, arg...) do { \
		if( unlikely(gDebugExp_loglevel >= level) ) { pr_crit("expT TT:%lld "  fmt, ktime_to_us(ktime_get()), ##arg ); } \
	   } while(0) 

#define GDMA_ENABLE_EXP_TIME
...
#define GDMA_MIXER_CTRL2    0x1ff000f
#define GDMA_MIXER_LAYER_SEL    0x321
...
static const char *rtkgdma_cmd_str[] = {
	...
	"osd_set_osd_gain=",
	"osd_cap_fb=",
	"osd_vsync_info=", //GDMA_DBG_CMD_SHOW_VSYNC 24
};
...
void GDMA_SET_RefreshInterval(void)
{
...
#if IS_ENABLED(CONFIG_SUPPORT_SCALER)
	{
		...
		if( needsetInterval == true && p_gdma_get_disp_refresh_rate != NULL )
		{
 ...
 #else
    gRefreshInterval = 1; 
	rtd_pr_gdma_emerg("%s gRefreshInterval def=%d (fixed)\n", __FUNCTION__,gRefreshInterval);
 #endif//
}
	...
	case GDMA_LOCK_VSYNC_SEM:
		{
			...
			{
				pr_err(" GDMA_Wait_Vsync cp fail: %lld\n", nsecs);
				ret = -ERANGE;	
			}
...
int get_VsyncCountSW(void )
{
    ...
    memcdtg_dvs_cnt_reg.regValue = rtd_inl(PPOVERLAY_memcdtg_DVS_cnt_reg);

	if(memcdtg_dvs_cnt_reg.memcdtg_dvs_cnt)  // 27M
        local_hz = (27000000 / (memcdtg_dvs_cnt_reg.memcdtg_dvs_cnt));
...
int gdma_suspend_by_vbe(void) {
	...
	if(GDMA_PM_REGISTER_DATA[1][1] != GDMA_MIXER_LAYER_SEL)
	{		
		rtd_pr_gdma_emerg("func=%s line=%d, mixer_ctrl=%x, mixer_layer=%x \n",__FUNCTION__,__LINE__,GDMA_PM_REGISTER_DATA[0][1],GDMA_PM_REGISTER_DATA[1][1]);
	}
...
int gdma_resume_by_vbe (void) {
	...
	if(GDMA_PM_REGISTER_DATA[1][1] != GDMA_MIXER_LAYER_SEL)
	{		
		rtd_pr_gdma_emerg("func=%s line=%d,variable mixer_ctrl=%x, mixer_layer=%x \n",__FUNCTION__,__LINE__,GDMA_PM_REGISTER_DATA[0][1],GDMA_PM_REGISTER_DATA[1][1]);
		rtd_outl(OSDOVL_Mixer_CTRL2_reg,GDMA_MIXER_CTRL2);
		rtd_outl(OSDOVL_Mixer_layer_sel_reg,GDMA_MIXER_LAYER_SEL);
		rtd_pr_gdma_emerg("func=%s line=%d,recover mixer_ctrl=%x, mixer_layer=%x \n",__FUNCTION__,__LINE__,rtd_inl(OSDOVL_Mixer_CTRL2_reg),rtd_inl(OSDOVL_Mixer_layer_sel_reg));
	}

	if(GAL_Suspend_free_irq ==1)
	{
		for (i = 0; i < gdma_nr_devs; i++) {
			gdma = &gdma_devices[i];
...
int GDMA_Wait_Vsync( u64* nsec )
{
	...
	{
		// timeout
		if( timeout_warn_us < (unsigned int)wait_time_us ) { //timeout + signaled, warning only 
...
int GDMA_ConfigOSDxEnableOnOff(VO_GRAPHIC_PLANE disPlane, unsigned char flag)
{
	unsigned int offset = 0;
	unsigned int progDone = GDMA_CTRL_write_data(1);
	unsigned int retry_cnt =50;
	if( enable_stop_update == 0 && GAL_Runtime_Suspend_status == 0 ){
		...
		while(rtd_inl(GDMA_CTRL_reg) &&retry_cnt)
		{
			gdma_usleep (1000);
			retry_cnt--;
		}
		rtd_pr_gdma_info("[GDMA][%s] flag = %d, retry_cnt=%d\n",__func__,flag, retry_cnt);
	}
	return GDMA_SUCCESS;
}
...
//			int64_t vs_diff_sw = gGDMA_VSYNC_SW_TIME - gGDMA_VSYNC_SW_TIME_PREV;
			...
			rtd_pr_gdma_crit("expT_0x%x prevExpDif:%lld compDif:%lld expWait:%lld realWait:%lld exp2vsyncDif:%lld prevSendDif:%lld  exp2prevVsync_Dif:%lld "
						"lastExp:%lld lastSig:%lld stVsync:%lld send:%lld curExp:%lld  cur:%lld vs:%u\n", tag, 
						prev_expTime_diff, complete_diff, exp_wait, real_wait, exp_to_vsync_diff, send_to_vsync_diff, exp_to_prevVync_diff, 
						gGDMA_LAST_EXP_Time, gGDMA_PIC_LAST_SIGNAL_Time, gGDMA_StartWait_VsyncTime, gGDMA_EXPTime_Send, cur_expectedPresentTime, cur_time, gVsyncDiff_NS );
		}					
...
int GDMA_Boot_Image_draw_buffer(int s_w,int s_h,KGAL_PIXEL_FORMAT_T format,unsigned int *dst_phy)
{
	...
	picObj->dst_y = gdma->ctrl.enableVFlip?0:disp_res.height-s_h;
	...
	GDMA_ConfigOSDxMixerEnable(GDMA_PLANE_OSD1,true);
...
EXPORT_SYMBOL(GDMA_ConfigOSDxEnableOnOff);
EXPORT_SYMBOL(GDMA_ConfigOSDxMixerEnable); //export for Scaler use
	...
	if (enable) {
		...
		*/
		if (srcWin.width == 0 || srcWin.height == 0 || dispWin.width == 0 || dispWin.height == 0) {
			printf2("GDMA division by :srcWin.width = %d,srcWin.height=%d\n", srcWin.width, srcWin.height) ;
			printf2("GDMA division by :dispWin.width = %d,dispWin.height=%d\n", dispWin.width, dispWin.height) ;
		}
...
int GDMA_init_module(unsigned int cmdbaddr)
{
	...
	rtd_outl(GDMA_CTRL_reg, onlineProgDone);
#ifndef Config_SILENT_ON_TRUE
	printf2("%s, 1st onlineProgDone=0x%x \n", __FUNCTION__, onlineProgDone);
#endif
	/* onlineProgDone reset to default */
	onlineProgDone = GDMA_CTRL_write_data(1);
...
programming done
	...
	*/
	...
	/* blending video layer */
	mixer_bx_reg.k1 = 0x01;
	mixer_bx_reg.k2 = 0x04;
	mixer_bx_reg.k3 = 0x01;
	mixer_bx_reg.k4 = 0x04;
	rtd_outl(OSDOVL_Mixer_b3_reg, mixer_bx_reg.regValue);
	...
	printf2("GDMA module_init finish\n");
	return 0;
}
...
static void GDMA_OSDReg_Set(GDMA_DISPLAY_PLANE disPlane)
{
	...
	DBG_PRINT("%s gdma->curPic[%d]=%d, curPic->onlineOrder=%d\n", __FUNCTION__, disPlane, gdma->curPic[disPlane], curPic
->onlineOrder);
	...
	if (win->used) {
		...
		*/
		...
		if (gdma->ctrl.osdfirstEn[disPlane] == 0) {
			...
			pReg->osd_ctrl_reg.regValue = GDMA_OSD1_CTRL_write_data(1) | GDMA_OSD1_CTRL_osd1_en(curPic->show) | 
GDMA_OSD1_CTRL_rotate(rotateBit) | GDMA_OSD1_CTRL_d3_mode(gdma->ctrl.enable3D);
			...
			pReg->osd_ctrl_reg.regValue = GDMA_OSD1_CTRL_write_data(1) | GDMA_OSD1_CTRL_osd1_en(curPic->show) | 
GDMA_OSD1_CTRL_rotate(rotateBit);
		...
		*/
		...
		DBG_PRINT("%s, plane_alpha_r=0x%x, plane_alpha_a=0x%x \n", __FUNCTION__, curPic->plane_ar.plane_alpha_r, curPic->
plane_ar.plane_alpha_a);
		DBG_PRINT("%s, plane_alpha_g=0x%x, plane_alpha_b=0x%x \n", __FUNCTION__, curPic->plane_gb.plane_alpha_g, curPic->
plane_gb.plane_alpha_b);
		pReg->osd_wi_reg.regValue = GDMA_OSD1_WI_addr (virt_to_phys(win));
		pReg->osd_wi_3d_reg.regValue = GDMA_OSD1_WI_3D_addr (virt_to_phys(win_3d));
		pReg->osd_size_reg.regValue = GDMA_OSD1_SIZE_w(canvasW) | GDMA_OSD1_SIZE_h(canvasH);
		...
		printf2("%s, win->used is NOT one, something wrong \n", __FUNCTION__);
...
#ifdef TRIPLE_BUFFER_SEMAPHORE
		if ((gdma->picQwr[disPlane] > gdma->curPic[disPlane])  && (gdma->picQwr[disPlane] - gdma->curPic[disPlane]) > 2)
			DBG_PRINT("GDMA line=%d : r %d,w %d c %d \n", __LINE__, gdma->picQrd[disPlane], gdma->picQwr[disPlane], gdma->
curPic[disPlane]);
		else {
			if ((gdma->picQwr[disPlane] < gdma->curPic[disPlane]) && ((gdma->picQwr[disPlane]+GDMA_MAX_PIC_Q_SIZE) >= gdma->
curPic[disPlane])  && ((gdma->picQwr[disPlane]+GDMA_MAX_PIC_Q_SIZE) - gdma->curPic[disPlane]) > 2)
				DBG_PRINT("GDMA line=%d : r %d,w %d c %d \n", __LINE__, gdma->picQrd[disPlane], gdma->picQwr[disPlane], gdma->
curPic[disPlane]);
		}
#else
		...
		if (inISR) {
			if (!gdma->ctrl.sync[disPlane]) {
				...
				if ((gdma->picQrd[disPlane] != gdma->picQwr[disPlane]) && (gdma->picQwr[disPlane] >= 1)) {
					...
					gdma->ctrl.sync[disPlane] = 1;
					QueueFlag[disPlane] = 1;
					DBG_PRINT("%s: OSD[%d]Sync ......r %d, w %d\n", __FUNCTION__, disPlane, gdma->picQrd[disPlane], gdma->picQwr[
disPlane]);
				...
				if (gdma->curPic[disPlane] != gdma->picQwr[disPlane]) { /* select next picture to display */
					/*  keep the syncstamp OSD waiting for another OSD which has the same syncstamp */
					if (osdSyncStamp[disPlane] != 1) {
						curPicIdx = gdma->ctrl.displayEachPic ? ((gdma->curPic[disPlane] + 1) & (GDMA_MAX_PIC_Q_SIZE - 1)) : gdma->
picQwr[disPlane];
...
GDMA_CTRL_reg));
					...
					QueueFlag[disPlane] = 1;
					DBG_PRINT("%s: disPlane=%d, next pic c %d, w %d\n", __FUNCTION__, disPlane, gdma->curPic[disPlane], gdma->picQwr[
disPlane]);
				} else {
#ifdef GDMA_REPEAT_LOG
					static int prepic;
...
int GDMA_ReceivePicture (PICTURE_LAYERS_OBJECT *data, bool can_reclaim)
{
	...
	for (ilayer = 0; ilayer < data->layer_num; ilayer++) {
		...
		if (emptyIdx == GDMA_MAX_PIC_Q_SIZE) {
			printf2("%s, Warning! GDMA workqueue is full \n", __FUNCTION__);
			return COMP_BUF_WORK_FULL;
		}
		...
		if (com_picObj->normal_num > 0)
			...
			printf2("%s, PICTURE_LAYERS_OBJECT's COMPOSE_PICTURE_OBJECT has NO layer \n", __FUNCTION__);

		/*	in this function, one layer has only one picture in certain OSD HW */
		if (com_picObj->normal_num > 0 && com_picObj->fbdc_num > 0)
			printf2("%s, PICTURE_LAYERS_OBJECT's COMPOSE_PICTURE_OBJECT layer has something wrong \n", __FUNCTION__);
...
picObj->dst_y; //picObj->y; */
			...
			for (idx = 0; idx < com_picObj->fbdc_num; idx++) {
				...
				if (idx == 0) {     /*  FBDC1 */
					...
					win->fbdc_attr.fbdc_1st_en = 1;
					win->fbdc_attr.const_a_1st = ((win->fbdc_attr.alpha_1st = picObj->alpha) > 0) ? 1 : 0;
					win->fbdc_attr.plane_alpha_c0 = (picObj->plane_ar.plane_alpha_a != 0xff || picObj->plane_ar.plane_alpha_r != 
0xff ||
													 picObj->plane_gb.plane_alpha_g != 0xff || picObj->plane_gb.plane_alpha_b != 0xff) ? 1 : 0;
					win->fbdc_plane_alpha_c2.plane_alpha_a = (picObj->plane_ar.plane_alpha_a > 255) ? 255 : picObj->plane_ar.
plane_alpha_a;
					win->fbdc_plane_alpha_c2.plane_alpha_r = (picObj->plane_ar.plane_alpha_r > 255) ? 255 : picObj->plane_ar.
plane_alpha_r;
					win->fbdc_plane_alpha_c2.plane_alpha_g = (picObj->plane_gb.plane_alpha_g > 255) ? 255 : picObj->plane_gb.
plane_alpha_g;
					win->fbdc_plane_alpha_c2.plane_alpha_b = (picObj->plane_gb.plane_alpha_b > 255) ? 255 : picObj->plane_gb.
plane_alpha_b;
					/*win->fbdc_attr.pre_blend_c0_sel = 1;*/
					win->fbdc_format.fbdc_fmt_1st = picObj->fbdc_format;
					win->fbdc1_totaltile.x_total_tile = (picObj->width / 16);
					...
					win->fbdc_attr.fbdc_2nd_en = 1;
					win->fbdc_attr.const_a_2nd = ((win->fbdc_attr.alpha_2nd = picObj->alpha) > 0) ? 1 : 0;
					win->fbdc_attr.plane_alpha_c1 = (picObj->plane_ar.plane_alpha_a != 0xff || picObj->plane_ar.plane_alpha_r != 
0xff ||
													 picObj->plane_gb.plane_alpha_g != 0xff || picObj->plane_gb.plane_alpha_b != 0xff) ? 1 : 0;
					win->fbdc_plane_alpha_c1.plane_alpha_a = (picObj->plane_ar.plane_alpha_a > 255) ? 255 : picObj->plane_ar.
plane_alpha_a;
					win->fbdc_plane_alpha_c1.plane_alpha_r = (picObj->plane_ar.plane_alpha_r > 255) ? 255 : picObj->plane_ar.
plane_alpha_r;
					win->fbdc_plane_alpha_c1.plane_alpha_g = (picObj->plane_gb.plane_alpha_g > 255) ? 255 : picObj->plane_gb.
plane_alpha_g;
					win->fbdc_plane_alpha_c1.plane_alpha_b = (picObj->plane_gb.plane_alpha_b > 255) ? 255 : picObj->plane_gb.
plane_alpha_b;
					/*win->fbdc_attr.pre_blend_c1_sel = 2;*/
					win->fbdc_format.fbdc_fmt_2nd = picObj->fbdc_format;
					win->fbdc2_totaltile.x_total_tile = (picObj->width / 16);
					...
					win->fbdc_attr.fbdc_3rd_en = 1;
					win->fbdc_attr.const_a_3rd = ((win->fbdc_format.alpha_3rd = picObj->alpha) > 0) ? 1 : 0;
					win->fbdc_attr.plane_alpha_c2 = (picObj->plane_ar.plane_alpha_a != 0xff || picObj->plane_ar.plane_alpha_r != 
0xff ||
													 picObj->plane_gb.plane_alpha_g != 0xff || picObj->plane_gb.plane_alpha_b != 0xff) ? 1 : 0;
					win->fbdc_plane_alpha_c0.plane_alpha_a = (picObj->plane_ar.plane_alpha_a > 255) ? 255 : picObj->plane_ar.
plane_alpha_a;
					win->fbdc_plane_alpha_c0.plane_alpha_r = (picObj->plane_ar.plane_alpha_r > 255) ? 255 : picObj->plane_ar.
plane_alpha_r;
					win->fbdc_plane_alpha_c0.plane_alpha_g = (picObj->plane_gb.plane_alpha_g > 255) ? 255 : picObj->plane_gb.
plane_alpha_g;
					win->fbdc_plane_alpha_c0.plane_alpha_b = (picObj->plane_gb.plane_alpha_b > 255) ? 255 : picObj->plane_gb.
plane_alpha_b;
					/*win->fbdc_attr.pre_blend_c2_sel = 3;*/
					win->fbdc_format.fbdc_fmt_3rd = picObj->fbdc_format;
					win->fbdc3_totaltile.x_total_tile = (picObj->width / 16);
				...
				/* calculate the canvas size */
				win->winWH.width      = ((picObj->width+picObj->dst_x) > win->winWH.width) ? (picObj->width+picObj->dst_x) : win->
winWH.width;
				win->winWH.height     = ((picObj->height+picObj->dst_y) > win->winWH.height) ? (picObj->height+picObj->dst_y) : 
win->winWH.height;
			}
			/*  k blend factor for FBDC */
			win->fbdc_blend1st.k1_blend = 0xc;
...
->winWH.width);*/
			...
			printf2("%s, No layer in use \n", __FUNCTION__);
	}

	printf2("%s, layer loop done \n", __FUNCTION__);
...
unsigned int get_bush(void)
{
	unsigned int  freq, tmp, value_f, value_n, pdiv, div,n_state;

	n_state = rtd_inl(PLL_REG_SYS_PLL_CPU) & 0x00000080;
	tmp = rtd_inl(PLL_REG_SYS_PLL_BUSH2_reg);
	value_f = ((tmp & 0x07FF0000) >> 16);
	value_n = ((tmp & 0x0000FF00) >> 8);
	tmp = rtd_inl(PLL_REG_SYS_PLL_BUSH1_reg);
	div = ((tmp & 0x30000000) >> 28);
	pdiv = ((tmp & 0x00000030)>>4);
	if(n_state) {
		freq = ( 27 * (value_n + 3) + ((27 * value_f) >> 11)) /(pdiv + 1) / (1 << div) / 2;
	}
	else {
		freq = ( 27 * (value_n + 4) + ((27 * value_f) >> 11)) /(pdiv + 1) / (1 << div) / 2;
	}
	return freq;
}
unsigned int get_disppll(void)
{
    unsigned int tmp = 0, value_m = 0, value_n = 0, value_f = 0, value_o = 0, ret_clk = 0;
...
void GDMA_Clk_Select(void)
{
	...
	printf2("[GDMA](Frome bdinfo)BUSH = %d ; DISP = %d\n",  get_bush(), get_disppll());

	if (get_bush() > get_disppll())
		rtd_outl(SYS_REG_SYS_DISPCLKSEL_reg, rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg) & ~(1<<14));
	else
		rtd_outl(SYS_REG_SYS_DISPCLKSEL_reg, rtd_inl(SYS_REG_SYS_DISPCLKSEL_reg) | (1<<14));
