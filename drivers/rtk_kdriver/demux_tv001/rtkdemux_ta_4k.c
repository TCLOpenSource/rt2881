...
#define DEMUX_USER_TA_JP4K_UPDATE_MEM              0x1000
...
static const uuid_t jp4k_ta_uuid =
	UUID_INIT(0xe1cbabcb, 0x6cb8, 0x4b08, 
	         0x9c, 0xf1, 0x29, 0x01, 0xf4, 0x0b, 0x08, 0x9b);

static const uuid_t atsc3_ta_uuid =
	UUID_INIT(0xbe22e836, 0x9bf0, 0x11ea, 
...
int DEMUX_TA_4K_UpdateMeminfo(DEMUX_BUF_T *pBuffer, int status)
{
    int ret = 0, rc = 0;
    struct tee_ioctl_invoke_arg invoke_arg;
    struct tee_param param[TEE_NUM_PARAM];
    ...
    memset(&invoke_arg, 0, sizeof(invoke_arg));
    invoke_arg.func = DEMUX_USER_TA_JP4K_UPDATE_MEM;
    invoke_arg.session = DemuxTa.session;
    invoke_arg.num_params = TEE_NUM_PARAM;
    ...
    param[0].u.memref.shm = pbuf;
    param[0].u.memref.size = sizeof(DEMUX_BUF_T);

    param[1].attr = TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INPUT;
    param[1].u.value.a = status;
...
*/
int DEMUX_TA_4K_UpdatePininfo(unsigned int bufType, unsigned int pin, DEMUX_BUF_T *pBufferH, DEMUX_BUF_T *pBuffer, int status)
{
    int ret = 0, rc = 0;
    struct tee_ioctl_invoke_arg invoke_arg;
    struct tee_param param[TEE_NUM_PARAM];
...
#ifdef CONFIG_RTK_KDRV_JP4K
    if (IS_ERR_OR_NULL(pbufheader_va)) {
        ret = -ENOMEM;
        goto END;
    }
#endif
...
int DEMUX_TA_4K_INIT(void)
{
    ...
    if(RHAL_Delivery_GetMode(0) == TP_DELIVERY_MODE_JAPAN4K){
        memcpy(&arg.uuid, jp4k_ta_uuid.b, TEE_IOCTL_UUID_LEN);
    }else{
        memcpy(&arg.uuid, atsc3_ta_uuid.b, TEE_IOCTL_UUID_LEN);
    }
