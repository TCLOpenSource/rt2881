...
long DEMUX_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	else if (cmd == DEMUX_IOC_GET_ES_BUF_STATUS || cmd == DEMUX_IOC_GET_ES_BUF_STATUS_COMPAT) {
...
int DEMUX_init(void)
{
	...
	if (!(demux_device->pChArray)) {
		dmx_err("DEMUX init kmalloc pChArray Fail L:%d \n", __LINE__);
		goto demux_init_fail;
	}
	...
	if (!(demux_device->pChUsage)){
		dmx_err("DEMUX init kmalloc pChUsage Fail L:%d \n", __LINE__);
		goto demux_init_fail;
	}
	...
	if (DEMUX_BUF_AllocBuffer(&demux_device->bufPoll, size, 0, (LIMIT | 1))   < 0){
		dmx_err("DEMUX init allocate bufpool Fail L:%d \n", __LINE__);
		goto demux_init_fail;
	}
	...
	dmx_notice("DEMUX init success L:%d chsize:%d, poolsize:%d\n", __LINE__, sizeof(demux_channel)*demux_device->chNum, size);
	return 0;
...
demux_init_fail:
	...
	dmx_err("DEMUX init Fail L:%d @#@#!!!!!!!!!!@#@#\n", __LINE__);
	return -ENOMEM;
}

void DEMUX_uninit(void)
{
	int i;
	int mgr_idx;
