...
int IOCTL_InitModule (void)
{
	int ret = 0;
	dmx_function_call();
	ret = tp_allocate_section_info_buffer();
	return ret;
}
int IOCTL_UnInitModule(void)
{
	dmx_function_call();
	dmx_info("func %s, line %d, isInit %d\n", __func__, __LINE__, demux_device->isInit);
	if(0 == demux_device->isInit) {
...
int IOCTL_GetSTC(DEMUX_STC_T *pInfo)
{
	...
	/*stc_hi == 0xFFFFFFFF && stc_lo == 0xFFFFFFFF, return real stc value*/
		if ((rcd == -1) || (pInfo->stc_hi == 0xFFFFFFFF && pInfo->stc_lo == 0xFFFFFFFF)) {
			rcd = 0;
		}
#else
		if (rcd == -1) {
...
static int IOCTL_SetPIDImpl(DEMUX_SET_PID_T *pInfo)
{
	...
	for (i = 0; i < pCh->pidFilterCount; i++) {
		...
		if (haveSamePIDType && haveSameDest) {
			...
			if ( currentFilter->pid == pInfo->pid && (pInfo->pidType == DEMUX_PID_VIDEO || pInfo->pidType == DEMUX_PID_AUDIO) ) {
				if (pInfo->dest == DEMUX_DEST_TSO) {
					rtkdemux_dbg_esbufmonitor_en = 0;
				} else {
					rtkdemux_dbg_esbufmonitor_en = 1;
				}
				dmx_info("func %s, line %d, ch %d, pid:%d,pidType:%d,dest:%d.esbufmonitor:%d\n",
					__func__, __LINE__, pCh->tunerHandle, pInfo->pid, pInfo->pidType, pInfo->dest,rtkdemux_dbg_esbufmonitor_en);
			}
		}
	}
...
int IOCTL_GetPIDFilterIndex(DEMUX_SET_PID_T *pInfo)
{
	...
	dmx_notice("%s ch[%d] pid:0x%x get keyindex:%d", __func__, pInfo->ch, pInfo->pid, ret);
	return ret;
}
	...
	for (j = 0; j < pInfo->count; j++) {
		for (i = 0; i < pCh->pidFilterCount; i++) {
			...
			if (entry->pid == target->pid
				...
				dmx_info("func %s, line %d, ch %d, pid:%d,pidType:%d,dest:%d.esbufmonitor:%d\n",
					__func__, __LINE__, pCh->tunerHandle, target->pid, target->pidType, target->dest,rtkdemux_dbg_esbufmonitor_en);
...
int IOCTL_RemovePIDs(DEMUX_SET_MULTI_PID_T *pInfo)
{
	demux_channel *pCh;
	int i, j;
	dmx_function_call();
	...
	for (i = 0; i < pCh->pidFilterCount; i++) {
		for (j = 0; j < pInfo->count; j++) {
			if (pCh->pidFilterList[i].pid == pInfo->pidlist[j].pid &&
                pCh->pidFilterList[i].pidType == pInfo->pidlist[j].pidType &&
                pCh->pidFilterList[i].pesDest == pInfo->pidlist[j].dest) {
				pCh->pidFilterList[i].refCnt--;
				if( 0 == pCh->pidFilterList[i].refCnt )
					pCh->pidFilterList[i].pid = -1;
...
int IOCTL_SetTPInputConfig(DEMUX_TP_SOURCE_CONFIG_T *pInfo)
{
	...
	tpStatus.tp_param.casType = pInfo->casType & 0x3;
...
#ifdef TP_SUPPORT_CI
	/* 3) set tpo */
	if(pInfo->casType == TPK_WITH_CAS || pInfo->casType == TPK_WITH_CIP_CAS)
	{
		//*******************
		// FOR With CAS case
		//*******************
