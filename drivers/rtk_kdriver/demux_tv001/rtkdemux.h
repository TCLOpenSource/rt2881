...
{\
	if(!demux_device->pChArray || x>= demux_device->chNum || !demux_device->pChArray[x].isInit) {	\
		dmx_warn("func %s, line %d, Error(Device ch isInit %d, ch %d, ch Init %d)!!\n", __func__, __LINE__, (demux_device->pChArray != 0), x, demux_device->pChArray[x].isInit);	\
		return -ENOTTY;	\
	}	\
}
...
#define DEMUX_VD_TLV_BS_SIZE         (6*1024*1024)
