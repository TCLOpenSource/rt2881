...
int DEMUX_BUF_AllocResevredSVPBuffer(DEMUX_BUF_T *pSvpBuf)
{
	...
	pSvpBuf->phyAddr = pli_malloc(DEMUX_DTV_VDEC_RINGBUF, GFP_DCU1);
	if( INVALID_VAL == pSvpBuf->phyAddr ) {
        goto PLI_MALLOC_FAILED;
    }

    dmx_err("Using Reserved Buffer phyAddr: 0x%x\n", pSvpBuf->phyAddr);
    pSvpBuf->allocSize = DEMUX_DTV_VDEC_RINGBUF ;
    pSvpBuf->cachedaddr = NULL;
    pSvpBuf->nonCachedaddr = dvr_remap_uncached_memory(pSvpBuf->phyAddr, DEMUX_DTV_VDEC_RINGBUF, __builtin_return_address(0));
    if (pSvpBuf->nonCachedaddr == NULL) {
        goto VMAP_FAILED;
    }
    pSvpBuf->isCache = 0;
    pSvpBuf->virAddr = pSvpBuf->isCache ? pSvpBuf->cachedaddr : pSvpBuf->nonCachedaddr ;
    pSvpBuf->size = DEMUX_DTV_VDEC_RINGBUF ;
    pSvpBuf->fromPoll = 0 ;
    pSvpBuf->fromVBM = 0;
...
VMAP_FAILED:
    pli_free(pSvpBuf->phyAddr);
    dmx_err("[%s] vmap info. phyAddr:%lx, uncachedAddr 0x%lx, Size:%x\n",__func__, (unsigned long)pSvpBuf->phyAddr, (unsigned long)(uintptr_t)pSvpBuf->nonCachedaddr, DEMUX_DTV_VDEC_RINGBUF);
PLI_MALLOC_FAILED:
    dmx_err("[%s] pli_malloc info. phyAddr:%lx, Size:%x\n",__func__, (unsigned long)pSvpBuf->phyAddr, DEMUX_DTV_VDEC_RINGBUF);
    return -EFAULT;
}
...
#else
    DEMUX_TA_4K_UpdateMeminfo(pSvpBuf, 0);
    dvr_unmap_memory(pSvpBuf->nonCachedaddr, pSvpBuf->allocSize);
    pli_free(pSvpBuf->phyAddr);
    dmx_info("[L %d]return resevered svp buffer.start addr:0x%x, size:%d\n", __LINE__, pSvpBuf->phyAddr, pSvpBuf->allocSize);
    memset(pSvpBuf, 0, sizeof(DEMUX_BUF_T));
#endif
    demux_device->usedSVPPollSize = 0;
}
...
int DEMUX_BUF_AllocBuffer_Ext(DEMUX_BUF_T *pBuf, int size, int isCache, int useZone)
{
	...
	if(isCache){
		...
		if (pBuf->cachedaddr == NULL) {
			dmx_err("[%s] dvr_remap_cached_memory failed. phyAddr:%lx, Size:%x, isCache:%d, useZone:%d \n",__func__, (unsigned long)pBuf->phyAddr, size, isCache, useZone);
		...
		if (pBuf->nonCachedaddr == NULL){
			dmx_err("[%s] dvr_remap_uncached_memory failed. phyAddr:%lx, Size:%x, isCache:%d, useZone:%d \n",__func__, (unsigned long)pBuf->phyAddr, size, isCache, useZone);
			pli_free(pBuf->phyAddr);
			pBuf->phyAddr = 0;
			return -EFAULT;
		}
	...
	for (i = VIDEO_PIN; i < NUMBER_OF_PINS; i++) {
		...
		if (pCh->pinInfo[i].bufStatus == BUF_STAT_FREE) {
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
				...
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_IB, i, pBufIBH, pBufIB, 0);
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_BS, i, pBufBSH, pBufBS, 0);
#if 0
                rtkvdec_svp_disable_demux_protection(pBufIBH->phyAddr, pBufIBH->size);
                rtkvdec_svp_disable_demux_protection(pBufIB->phyAddr, pBufIB->size);

                rtkvdec_svp_disable_demux_protection(pBufBSH->phyAddr, pBufBSH->size);
                rtkvdec_svp_disable_demux_protection(pBufBS->phyAddr, pBufBS->size);
#endif
			}
			...
			if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
				dmx_emerg("[%s %d] BUF_STAT_NEW debug\n", __func__, __LINE__);
                if(flag == 0){
                    DEMUX_TA_4K_UpdateMeminfo(&demux_device->svpBufPoll, 1);
                    flag = 1;
                }

#if 0
                rtkvdec_svp_enable_demux_protection(pBufIBH->phyAddr, pBufIBH->size);
                rtkvdec_svp_enable_demux_protection(pBufIB->phyAddr, pBufIB->size);

                rtkvdec_svp_enable_demux_protection(pBufBSH->phyAddr, pBufBSH->size);
                rtkvdec_svp_enable_demux_protection(pBufBS->phyAddr, pBufBS->size);
#endif
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_IB, i, pBufIBH, pBufIB, 1);
				DEMUX_TA_4K_UpdatePininfo(DEMUX_STREAM_TYPE_BS, i, pBufBSH, pBufBS, 1);
#if 0
				if(IS_AUDIO_PIN(i)){
