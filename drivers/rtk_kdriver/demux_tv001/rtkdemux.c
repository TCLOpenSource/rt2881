...
#define CHECK_VIDEO_SCRAMBLE 1
#if CHECK_VIDEO_SCRAMBLE
int chkScrambleCount = 0;
#endif
...
#define HAS_SCRAMBLE(p__)        (((p__)[3] &  0xC0) != 0)
...
void DEMUX_FreeSecIndex(demux_channel *channel, unsigned char index)
{
    ...
    mutex_lock(&channel->section_mutex);
    if (index < MAX_SECTION_FILTER_NUM && channel->secListCB[index].used) {
		...
		if (pNavBuffer->data.payloadSize > 0) {
			...
			if (pNavBuffer->data.pts >= 0) {
				if (pCh->tp_src == MTP && rtkdemux_dbg_esbufmonitor_en) {
					if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_CI20){
...
int DEMUX_EVENT_StartVideo(demux_channel *pCh, DECODE_MODE decMode, int pin)
{
	...
	if (pCh->tp_src != MTP){
		...
		if(delivery_mode == TP_DELIVERY_MODE_ATSC30_BBF || delivery_mode == TP_DELIVERY_MODE_ATSC30_ALP)
			DEMUX_ResetAVSync(pCh, NAV_AVSYNC_FREE_RUN);
	#ifdef CONFIG_RTK_KDRV_JP4K
		else if(delivery_mode == TP_DELIVERY_MODE_TS)
			DEMUX_ResetAVSync(pCh, NAV_AVSYNC_PCR_MASTER);
		else
			dmx_err("[ch%d]not set avsync mode, current avsync mode:%d", pCh->tunerHandle, pCh->avSyncMode);
	#else
		else
			DEMUX_ResetAVSync(pCh, NAV_AVSYNC_PCR_MASTER);
	#endif
...
#if CHECK_VIDEO_SCRAMBLE
			if (target == DEMUX_TARGET_VIDEO) {
				unsigned char* pPES = payload;
				dmx_mask_print(DEMUX_SCRAMBLE_DEBUG,"[CheckScramble]Video PES Header: 0x%x %x %x %x.\n", pPES[0], pPES[1], pPES[2], pPES[3]);
			}
#endif
		} else {
...
bool ParseTsPacketCB(demux_channel *pCh, int target, NAVDEMUXIN *pDemuxIn)
{
    DEMUX_PES_MAP_T *pMap = &pCh->pesMap[target];
    unsigned char *pData = pDemuxIn->pCurr;
    int TS_PACKET_SIZE = pCh->tsPacketSize;

    {
        ...
        if( space <= TS_PACKET_SIZE && space > 0) { /* ringbuffer full, re-sync StartUnit */
            dmx_crit("func %s, tspacket ringbuffer full(pid=%d), skip this unit(space %lu, size %d)!!\n", __func__, pCh->activePESTarget[target].pid, space, TS_PACKET_SIZE) ;
        ...
        if( pCurWr + TS_PACKET_SIZE >= pMap->pBufferUpper ) {
            space = pMap->pBufferUpper - pCurWr;
            memcpy(pCurWr - pMap->phyAddrOffset, pData, space) ;
            memcpy(pMap->pBufferLower - pMap->phyAddrOffset, pData + space, TS_PACKET_SIZE - space) ;
        }else{
            memcpy(pCurWr - pMap->phyAddrOffset, pData, TS_PACKET_SIZE) ;
        }

        pCurWr += TS_PACKET_SIZE;
        if( pCurWr >= pMap->pBufferUpper )
            pCurWr -= (pMap->pBufferUpper - pMap->pBufferLower);

        pMap->size += TS_PACKET_SIZE;

        if( pMap->size >= TS_PACKET_SIZE ) {
	...
	while (pDemuxIn->pCurr+188 <= pDemuxIn->pEnd) {
		...
		if (targetCB < MAX_PID_FILTER_CB_NUM ) {
			...
			if (pCh->activePESTarget[targetCB].pid >= 0) {
				if(DEMUX_PID_TSPACKET == pCh->pidFilterListCB[targetCB].pidType)
					ParseTsPacketCB(pCh, targetCB, pDemuxIn);
				else
		...
		if (target == DEMUX_TARGET_PSI) {
			...
			if (target == DEMUX_TARGET_VIDEO) {
				chkScrambleCount++;
				if ((chkScrambleCount % 100) == 0) {
					dmx_mask_print(DEMUX_SCRAMBLE_DEBUG, "[CheckScramble] Video TS is %s.\n", HAS_SCRAMBLE(pDemuxIn->pCurr)? ("scramble") : ("clear"));
				}
			}
#endif
			if (pDemuxOut->numBuffers == NUM_OF_NAV_BUFFERS) {
...
void DEMUX_Read(demux_channel *pCh)
{
	...
	while (1) {
		...
		if(RHAL_Delivery_GetMode(pCh->tunerHandle) != TP_DELIVERY_MODE_TS){
			/*Do nothing, FPP will set RCD*/
#if 0/*def CONFIG_RTK_KDRV_JP4K*/
