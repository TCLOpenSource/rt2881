...
static const struct rtk_old_serial_port rtd2885p_old_serial_port[] = {
    RTD2885P_SERIAL_PORT_DFNS /* defined in asm/serial.h */
};
...
static void rtk_serial8250_isa_init_ports(void)
{
    ...
    int i, irqflag = 0;
    int uart_count = 0;
    const struct rtk_old_serial_port *init_old_serial_port;
    ...
    if(get_mach_type()==MACH_ARCH_RTK2885PP){
        uart_count = ARRAY_SIZE(rtd2885p_old_serial_port);
	init_old_serial_port = rtd2885p_old_serial_port;
    }else{
        uart_count = ARRAY_SIZE(old_serial_port);
	init_old_serial_port = old_serial_port;
    }
    for (i = 0, rtk_up = rtk_serial8250_ports;
         i < uart_count && i < nr_uarts;
         i++, rtk_up++) {
        struct uart_8250_port *up = &rtk_up->up;
        struct uart_port *port = &up->port;

        port->iobase   = init_old_serial_port[i].port;
        port->irq      = irq_canonicalize(init_old_serial_port[i].irq);
        port->irqflags = init_old_serial_port[i].irqflags;
        port->uartclk  = rtk_serial8250_get_clock(get_rbus_mapped_addr((unsigned long)init_old_serial_port[i].iomem_base));
        port->flags    = init_old_serial_port[i].flags;
        port->hub6     = init_old_serial_port[i].hub6;
        port->membase  = (unsigned char __iomem *)get_rbus_mapped_addr((unsigned long)init_old_serial_port[i].iomem_base);
        port->iotype   = init_old_serial_port[i].io_type;
        port->regshift = init_old_serial_port[i].iomem_reg_shift;
        rtk_serial8250_set_defaults(up);

        port->irqflags |= irqflag;
#ifdef CONFIG_RTK_KDRV_SERIAL_8250_UART_DMA
        rtk_up->rtk_dma_config.dma_membase = (unsigned char __iomem *)get_rbus_mapped_addr((unsigned long)init_old_serial_port[i].dma_iomem_base);
        rtk_up->rtk_dma_config.dma_membase_ofst = (init_old_serial_port[i].dma_iomem_base - init_old_serial_port[i].iomem_base) >> init_old_serial_port[i].iomem_reg_shift;
        rtk_up->rtk_dma_config.dma_ring_buf_len = init_old_serial_port[i].dma_ring_buf_len;
        rtk_up->rtk_dma_config.dma_tx_thd = init_old_serial_port[i].dma_tx_thd;
        rtk_up->rtk_dma_config.dma_tx_empty_thd = init_old_serial_port[i].dma_tx_empty_thd;
        rtk_up->rtk_dma = NULL;
        /*
	 ...
	 struct rtk_uart_port *rtk_up = &rtk_serial8250_ports[i];
        ...
        if (up->port.type != PORT_UNKNOWN && up->port.dev == &dev->dev) {
            ...
            if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {
                ...
                rtd_outl(ISO_MISC_OFF_INT_CTRL_reg, ((rtd_inl(ISO_MISC_OFF_INT_CTRL_reg) | (ISO_MISC_OFF_INT_CTRL_uart0_int_to_emcu_en_mask)) & (~ISO_MISC_OFF_INT_CTRL_uart0_int_to_scpu_en_mask))  ); //enable uart int to emcu & disable uart in scpu
            }
	 ...
	 struct rtk_uart_port *rtk_up = &rtk_serial8250_ports[i];
        ...
        if (up->port.type != PORT_UNKNOWN && up->port.dev == &dev->dev) {
            ...
            if (up->port.membase == (unsigned char *)GET_MAPPED_RBUS_ADDR(ISO_MISC_OFF_UART_U0RBR_THR_DLL_reg)) {
                ...
                rtd_outl(ISO_MISC_OFF_INT_CTRL_reg, ((rtd_inl(ISO_MISC_OFF_INT_CTRL_reg) & (~ISO_MISC_OFF_INT_CTRL_uart0_int_to_emcu_en_mask)) | ISO_MISC_OFF_INT_CTRL_uart0_int_to_scpu_en_mask) ); //disable uart int to emcu & enable uart in scpu
            }
