...
const char *write_poll_dev_v3 = "/dev/rpc12";
const char *write_intr_dev_v3 = "/dev/rpc13";
...
int			openStubRPC()
{
  ...
  done_open = 0;
	...
	if ((wfdp_v3 = xopen(write_poll_dev_v3, O_WRONLY)) < 0) {
		printf("ERROR; return code from open() is %d\n", wfdp_v2);
		perror("rpcstubapi::openRPC");
		exit(-1);
	}

	if ((wfdi_v3 = xopen(write_intr_dev_v3, O_WRONLY)) < 0) {
		printf("ERROR; return code from open() is %d\n", wfdi_v2);
		perror("rpcstubapi::openRPC");
		exit(-1);
	}
