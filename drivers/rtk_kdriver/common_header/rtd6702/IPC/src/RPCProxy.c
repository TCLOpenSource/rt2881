...
struct THREAD_STRUCT *g_pStruct_v3 = NULL;
...
pthread_t g_p_threads_v3[NUM_HANDLER_THREADS];
...
void RPCProxy_loop(struct THREAD_STRUCT * pData){
    ...
    else if(pData->flag == SEND_VIDEO_CPU3) {
        pli_setThreadName("RPC_thread_V3");
    }
...
int initRPCProxy(struct REG_STRUCT *reg) {
    ...
    static pthread_mutex_t request_mutex_v3 = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;
    ...
    if (!g_pStruct_v2)
        exit(1);
    g_pStruct_v3= (THREAD_STRUCT *)malloc(sizeof(THREAD_STRUCT));
    if (!g_pStruct_v3)
        exit(1);
    ...
    g_pStruct_v3->pReg = reg;
    g_pStruct_v3->pMutex = &request_mutex_v3;
    g_pStruct_v3->flag = SEND_VIDEO_CPU3;
    for (i=0; i<NUM_HANDLER_THREADS; i++)
        pthread_create(&g_p_threads_v3[i], NULL, (void *)RPCProxy_loop, g_pStruct_v3);
...
int DeInitRPCProxyEx(unsigned char wait_thread_exit) {
	...
	if (wait_thread_exit){
	    ...
	    for (i = 0; i<NUM_HANDLER_THREADS;i++)
          ...
          for (i = 0; i<NUM_HANDLER_THREADS;i++)
	        pthread_join(g_p_threads_v3[i], 0);
