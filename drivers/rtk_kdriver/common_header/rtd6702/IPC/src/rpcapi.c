...
const char *read_poll_dev_v3 = "/dev/rpc14";
const char *read_intr_dev_v3 = "/dev/rpc15";
#ifdef ENABLE_KCPU_COMMON
...
int			openRPC()
{
	...
	if ((rfdp_v3 = xopen(read_poll_dev_v3, O_RDONLY)) < 0) {
		printf("ERROR; return code from open() is %d\n", rfdp_v2);
		perror("rpcapi::openRPC");
		exit(-1);
	}

	if ((rfdi_v3 = xopen(read_intr_dev_v3, O_RDONLY)) < 0) {
		printf("ERROR; return code from open() is %d\n", rfdi_v2);
		perror("rpcapi::openRPC");
		exit(-1);
	}
...
int registerRPC(unsigned long programID)
{
    ...
    if(ret == -1) goto err;
    ret = ioctl(rfdi_v3, RPC_IOCTHANDLER, programID);
    if(ret == -1) goto err;
#ifdef ENABLE_KCPU_COMMON
    ...
    if(ret == -1) goto err;
    ret = ioctl(rfdp_v3, RPC_IOCTHANDLER, programID);
    if(ret == -1) goto err;
