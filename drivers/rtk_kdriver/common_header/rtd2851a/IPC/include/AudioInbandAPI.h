...
typedef struct
{
    AUDIO_INBAND_CMD_PKT_HEADER header;
    rpc_u32_t               wPtr ;
    rpc_u32_t               OffsetPTSH;
    rpc_u32_t               OffsetPTSL;
}   AUDIO_DEC_PTS_SKIP;
...

{
    ...
    AUDIO_INBAND_CMD_PKT_HEADER header;
    rpc_u32_t   PTSDurationH;           // Duration in PTS
    rpc_u32_t   PTSDuration;            // Duration in PTS
    rpc_s32_t    pinID[4];               // for at most 4 inputs, 1 = valid, 0 = invalid

    rpc_s32_t    alphaA[4];              // alpha for 4 inputs to begin with
    rpc_s32_t    alphaB[4];              // alpha for 4 inputs to stop with
    rpc_s32_t    channelValid[4];        // LSB 8 bits stand for 0~7 channels
    rpc_s32_t    frameSize;              // internal frame size, for DVMixer editing, default = 256.
...
/* ============================ for DAP API ==================================================*/
#define SWAP_UINT32(x) (rpc_u32_t)((((x) >> 24)&0x000000FF) | (((x) & 0x00FF0000) >> 8) | (((x) & 0x0000FF00) << 8) | ((x) << 24))
