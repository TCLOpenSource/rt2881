...
typedef struct
{
  ...
  uint32_t nPicCmprsCPitch;
  uint32_t nSampleWidth;  /* compressed width */
  uint32_t nSampleHeight; /* compressed height */
  uint32_t nPicYCmprsGpuAddrHigh;
  uint32_t nPicYCmprsGpuAddrLow;
  uint32_t nPicCCmprsGpuAddrHigh;
  uint32_t nPicCCmprsGpuAddrLow;
  uint32_t nPicHeightGPU;
  uint32_t nPackingUnit;
  uint32_t reserved_ext2[2];
