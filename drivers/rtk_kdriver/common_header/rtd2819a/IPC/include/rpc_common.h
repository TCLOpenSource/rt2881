...
struct _tagRefClock {
  ...
  long long      AORenderedSampleCnt; // include drop frames to avoid abnormal amount
  int           videoPid;         // App pid get from video
  int           audioSpeed;       // audioSpeed for videoFW
  int           reserved[4];
};
...
struct _tagRefClock {
  ...
  unsigned long long      audio_accum_outsamples;// report audio accumulated AO output samples
  long long      AORenderedSampleCnt; // include drop frames to avoid abnormal amount
  long           videoPid;         // App pid get from video
  long           audioSpeed;       // audioSpeed for videoFW
  long           reserved[4];
};
typedef struct _tagRefClock  REFCLOCK;
