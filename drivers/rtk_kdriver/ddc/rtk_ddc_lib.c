...
/*-------------------------------------------------------
 * Func : rtk_ddc_lib_is_crt_on
 *
 * Desc : check crt status
 *
 * Para : id       : DDC port id
 *
 * Retn : 0 : success, others : failed
...
{
    if(rtk_ddc_drv_is_crt_on(ch))
        return 1;

    return 0;
}

EXPORT_SYMBOL(rtk_ddc_lib_is_crt_on);
...
//    if (len > 256)
    rtk_ddc_drv_set_segen(id, 1);        //default enable ddc support max 256 edid;
//    else
...
/*-------------------------------------------------------
 * Func : rtk_ddc_lib_aux_switch_port
 *
 * Desc : Enable i2c over dprx aux.
 *
 * Para : id       : DDC port id
 *        aux_id   : 0: aux1 enable. 1 aux2 enable
 *
 * Retn : 0 : success, others : failed
 -------------------------------------------------------*/
int rtk_ddc_lib_aux_switch_port(
    unsigned char       id, 
    int                 aux_id)
{
    int ret  = 0;

    ret = rtk_ddc_drv_switch_aux_port(id, aux_id);

    return ret;
}
EXPORT_SYMBOL(rtk_ddc_lib_aux_switch_port);
...
{
    // Disable DDC wake up
    if (rtk_ddc_drv_enable_ddc_wakeup(id, 0) < 0)
        return -1;
    // Clear all wakeup addresses and disable them 
    if (rtk_ddc_drv_reset_ddc_wakeup_addr(id) < 0)
        return -1;
    // Clear hitted status
    if (rtk_ddc_drv_clr_ddc_hitted_wakeup_addr(id) < 0)
        return -1;
    // disable interrupt.
    if (rtk_ddc_drv_set_ddc_wakeup_irq(id, 0) < 0)
        return -1;
    return 0;
}
