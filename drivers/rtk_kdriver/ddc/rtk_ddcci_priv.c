...
#include "rtk_ddcci_config-rtk2819a.h"
#endif

#include <linux/kthread.h>

#ifndef BUILD_QUICK_SHOW
#define DDCCI_LOCK(a,b)   spin_lock_irqsave(a,b)
#define DDCCI_UNLOCK(a,b) spin_unlock_irqrestore(a,b)
#else
#define DDCCI_LOCK(a,b)   spin_lock(a)
#define DDCCI_UNLOCK(a,b) spin_unlock(a)
#endif
#define DDCCI_ARRAY_SIZE(x)           (sizeof(x)/sizeof(x[0]))
...
unsigned char g_ddcci_connect_flag = 0xff;
...
extern int rtk_ddc_drv_set_enable(unsigned char id, int enable);
extern int rtk_ddc_drv_is_enable(unsigned char id);

void rtk_ddcci_lib_init(unsigned char id)
{
    const srtk_ddcci_reg_map *preg_map = rtk_ddcci_chip[id].reg_remap;
    srtk_ddc_reg_map *ddc_preg_map = NULL;//rtk_ddc_chip[id].reg_remap;
    unsigned long flags;

    ddc_preg_map = rtk_ddc_get_ddc_reg_map(id);

    RTK_DDC_INFO("[rtk_ddcci_lib_init] id=%d",id);
    if(id >= DDCCI_ARRAY_SIZE(rtk_ddcci_chip) || rtk_ddcci_chip[id].reg_remap == NULL || ddc_preg_map == NULL) {
    ...
    DDCCI_LOCK(&ddc_preg_map->g_ddc_spinlock,flags);
    ...
    DDCCI_UNLOCK(&ddc_preg_map->g_ddc_spinlock,flags);

}

void rtk_ddcci_lib_uninit(void)
{

    int i = 0;
    struct srtk_ddcci_chip_t* pchip = NULL;
    srtk_ddc_reg_map *ddc_preg_map = NULL;//rtk_ddc_chip[id].reg_remap;
    unsigned long flags;

    for(i=0; i<sizeof(rtk_ddcci_chip)/sizeof(rtk_ddcci_chip[0]); i++)
    {
        ddc_preg_map = rtk_ddc_get_ddc_reg_map(i);

        if(rtk_ddcci_chip[i].reg_remap == NULL || ddc_preg_map == NULL)
            continue;

        pchip = &rtk_ddcci_chip[i];

        DDCCI_LOCK(&ddc_preg_map->g_ddc_spinlock,flags);
        //DDC1_DDC_SFCR: [3]Special mode [2:1]Buf Reset [0]Enable
        rtd_maskl(pchip->reg_remap->DDC_DDC_SFCR, ~(DDC_DDC_SFCR_CIID_EN_MASK), DDC_DDC_SFCR_CIID_EN(0));
        DDCCI_UNLOCK(&ddc_preg_map->g_ddc_spinlock,flags);

    }
    g_ddcci_connect_flag = 0xff;
...
void rtk_ddcci_isr_tx_fifo_t(void)
{
    ...
    ddc_preg_map = rtk_ddc_get_ddc_reg_map(id);
    if( id >= DDCCI_ARRAY_SIZE(rtk_ddcci_chip) || ddc_preg_map == NULL)
    {
       return;
    }
...
bool rtk_ddcci_pooling_t(void)
{
    ...
    srtk_ddc_reg_map *ddc_preg_map = NULL;//rtk_ddc_chip[id].reg_remap;
    unsigned int ddcci_enable = 0;
    unsigned int ddcci_reg_val = 0;
    unsigned long flags;
    ...
    DDCCI_LOCK(&ddc_preg_map->g_ddc_spinlock,flags);
    ddcci_enable = (rtd_inl(preg_map->DDC_DDC_SFCR) & 0x1);
    DDCCI_UNLOCK(&ddc_preg_map->g_ddc_spinlock,flags);

    if(ddcci_enable == 0)
    {
        rtk_ddcci_lib_init(id);
        RTK_DDC_ERR("rtk_ddcci_lib_reinit\n");
        return false;
    }

    DDCCI_LOCK(&ddc_preg_map->g_ddc_spinlock,flags);
    ddcci_reg_val = rtd_inl(preg_map->DDC_DDC_SFBSR);

    first_type = DDC_DDC_SFBSR_GET_CIRWS_1ST(ddcci_reg_val);
    second_type = DDC_DDC_SFBSR_GET_CIRWS_2ND(ddcci_reg_val);
    first_start_st = DDC_DDC_SFBSR_GET_CISPT_1ST(ddcci_reg_val);
    second_start_st = DDC_DDC_SFBSR_GET_CISPT_2ND(ddcci_reg_val);
    first_stop_st = DDC_DDC_SFBSR_GET_CIPPT_1ST(ddcci_reg_val);
    second_stop_st = DDC_DDC_SFBSR_GET_CIPPT_2ND(ddcci_reg_val);
