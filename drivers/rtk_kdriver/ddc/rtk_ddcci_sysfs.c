...
static ssize_t ddcci_disable_id_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int val;
    ...
    if (sscanf(buf, "%d", &val)!=1 || (val<0) || (val>3))
    {
        RTK_DDC_ERR("invalid disenable value (%s), disable_ddcci_id should be 0, 1, 2 or 3\n", buf);
        return -EINVAL;
    }
    RTK_DDC_ERR("[ddcci] disable ddcci id from(%d) => to (%d)\n", val);
    if(val ==1){
        rtk_ddcci_lib_uninit();
    }
    return count;
}
static DEVICE_ATTR(disable_ddcci_id, S_IRUGO | S_IWUSR, NULL, ddcci_disable_id_store);
...
static struct attribute *dev_attrs[] = {
    ...
    &dev_attr_disable_ddcci_id.attr,
