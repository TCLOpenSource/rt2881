...
srtk_ddc_reg_map *rtk_ddc_get_ddc_reg_map(unsigned char id)
{
    if(id >= ARRAY_SIZE(rtk_ddc_chip) || rtk_ddc_chip[id].reg_remap == NULL )
    {
        RTK_DDC_ERR("[rtk_ddc_get_ddc_reg_map], Id = %d doesn't exist or not creating.\n", id);
        return NULL;
    }
    return rtk_ddc_chip[id].reg_remap;
}
...
#if defined (CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2851C) || defined(CONFIG_ARCH_RTK2851F) || defined(CONFIG_ARCH_RTK2875Q)
...
extern int rtk_ddc_drv_is_enable(unsigned char id);
int rtk_ddc_get_edid_state(unsigned char id, unsigned char* start_sts, unsigned char* stop_sts)
{
    ...
    if(rtk_ddc_drv_is_enable(id) == 0)
    {
        rtk_ddc_drv_set_enable(id, 1);
        RTK_DDC_ERR("%s : DDC is not enable can't get edid state\n", __func__);
        return -EFAULT;
    }
...
int rtk_ddc_drv_is_crt_on(unsigned char ch)
{
    switch (ch)
    {
        case 0:
            return DDC_ST_CLKEN_GET_DDC0(rtd_inl(STB_ST_CLKEN1_REG));
        break;
        case 1:
            return DDC_ST_CLKEN_GET_DDC1(rtd_inl(STB_ST_CLKEN1_REG));
        break;
        case 2:
            return DDC_ST_CLKEN_GET_DDC2(rtd_inl(STB_ST_CLKEN1_REG));
        break;
        case 3:
            return DDC_ST_CLKEN_GET_DDC3(rtd_inl(STB_ST_CLKEN1_REG));
        break;
#ifndef CONFIG_ARCH_RTK2851A
        case 4:
            return DDC_ST_CLKEN_GET_DDC4(rtd_inl(STB_ST_CLKEN1_REG));
        break;
#endif
        default:
            return 0;
...
#ifdef RTK_DDC_HW_SEGMENT_AUTO_CLEAR
...
#ifdef RTK_DDC_HW_SEGMENT_AUTO_CLEAR
    rtk_ddc_drv_set_isr(id, 0);       // disable ddc irq
#else
    rtk_ddc_drv_set_isr(id, enable);  // enable ISR to clear segment pointer on IRQ
#endif
    return ret;
}
...
int rtk_ddc_drv_read(unsigned char id, srtk_edid* pedid)
{
    ...
    {
        rtk_ddc_drv_set_enable(id, 1);
        RTK_DDC_ERR("%s : DDC is not enable can't read edid\n", __func__);
        return -EFAULT;
    }
...
int rtk_ddc_drv_set_aux_rx_enable(unsigned char id, int enable)
{
...
#ifdef ISO_MISC_DDC_CTRL_auxrx_ddc_en_mask
    ...
    if(get_mach_type()==MACH_ARCH_RTK2885PP){
        ...
        {
            case 1:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_auxrx_hddc0_en_mask, M8P_ISO_MISC_DDC_CTRL_auxrx_hddc0_en(enable));
                break;
            case 2:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_auxrx_hddc1_en_mask, M8P_ISO_MISC_DDC_CTRL_auxrx_hddc1_en(enable));
                break;
            case 3:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_auxrx_hddc2_en_mask, M8P_ISO_MISC_DDC_CTRL_auxrx_hddc2_en(enable));
                break;
            case 4:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_auxrx_hddc3_en_mask, M8P_ISO_MISC_DDC_CTRL_auxrx_hddc3_en(enable));
                break;
            default:
                break;
        }
...
#elif defined(ISO_MISC_DDC_CTRL_auxrx_hddc0_en_mask)
    switch(id)
    {
        case 1:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_auxrx_hddc0_en_mask, ISO_MISC_DDC_CTRL_auxrx_hddc0_en(enable));
            break;
        case 2:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_auxrx_hddc1_en_mask, ISO_MISC_DDC_CTRL_auxrx_hddc1_en(enable));
            break;
        case 3:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_auxrx_hddc2_en_mask, ISO_MISC_DDC_CTRL_auxrx_hddc2_en(enable));
            break;
        case 4:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_auxrx_hddc3_en_mask, ISO_MISC_DDC_CTRL_auxrx_hddc3_en(enable));
            break;
        default:
            break;
    }
#endif
    RTK_DDC_INFO("[%s] segen set %d\n", __func__ , enable);
    return ret;
}

int rtk_ddc_drv_switch_aux_port(unsigned char id, int aux_id)
{
    int ret = 1;
    if(rtk_ddc_chip[id].reg_remap == NULL || id >= ARRAY_SIZE(rtk_ddc_chip))
    {
    ...
    if ((aux_id<0) || (aux_id>1))
    {
        RTK_DDC_ERR("invalid aux_id value (%d), aux_id need be 0 or 1\n", aux_id);
        return -EINVAL;
    }

#ifdef ISO_MISC_DDC_CTRL_aux_sel_for_hddc0_mask
    switch(id)
    {
        case 1:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_aux_sel_for_hddc0_mask, ISO_MISC_DDC_CTRL_aux_sel_for_hddc0(aux_id));
            break;
        case 2:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_aux_sel_for_hddc1_mask, ISO_MISC_DDC_CTRL_aux_sel_for_hddc1(aux_id));
            break;
        case 3:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_aux_sel_for_hddc2_mask, ISO_MISC_DDC_CTRL_aux_sel_for_hddc2(aux_id));
            break;
        case 4:
            rtd_maskl(DDC_CTRL_REG, ~ISO_MISC_DDC_CTRL_aux_sel_for_hddc3_mask, ISO_MISC_DDC_CTRL_aux_sel_for_hddc3(aux_id));
            break;
        default:
            break;
    }
    ...
    if(get_mach_type()==MACH_ARCH_RTK2885PP){
        ...
        {
            case 1:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc0_mask, M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc0(aux_id));
                break;
            case 2:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc1_mask, M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc1(aux_id));
                break;
            case 3:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc2_mask, M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc2(aux_id));
                break;
            case 4:
                rtd_maskl(DDC_CTRL_REG, ~M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc3_mask, M8P_ISO_MISC_DDC_CTRL_aux_sel_for_hddc3(aux_id));
               break;
            default:
                break;
...
int rtk_ddc_drv_enable_ddc_wakeup(unsigned char id, unsigned char enable)
{
    ...
    if(rtk_ddc_chip[id].reg_remap == NULL || id >= ARRAY_SIZE(rtk_ddc_chip) \
        || !rtk_ddc_chip[id].reg_remap->WAKEUP_HDCP)
    {
        RTK_DDC_ERR("[%s] Id = %d doesn't exist or not creating.\n", __func__, id);
        return -EINVAL;
    }
...
int rtk_ddc_drv_set_ddc_wakeup_addr(unsigned char id, unsigned char addr, unsigned char enable)
{
    ...
    if(rtk_ddc_chip[id].reg_remap == NULL || id >= ARRAY_SIZE(rtk_ddc_chip) \
        || !rtk_ddc_chip[id].reg_remap->WAKEUP_ADDR_1)
    {
        RTK_DDC_ERR("[%s] Id = %d doesn't exist or not creating.\n", __func__, id);
        return -EINVAL;
    }
...
static irqreturn_t rtk_ddc_isr(int this_irq, void *dev_id)
{
    ...
    int SP_STS = 0;
#ifndef RTK_DDC_HW_SEGMENT_AUTO_CLEAR
    int SP_SEG_PT = 0;
    int LATCHED_STS = 0;
#endif
    unsigned long flags;
    ...
    RTK_DDC_INFO("[rtk_ddc_isr] DDC interrupt : SP_STS = %d, LATCHED_STS = %d, SP_SEG_PT = %d, SP_STS = %d\n", SP_STS, LATCHED_STS, SP_SEG_PT, SP_STS);

    spin_unlock_irqrestore(&p_this->reg_remap->g_ddc_spinlock,flags);
#endif  // RTK_DDC_HW_SEGMENT_AUTO_CLEAR
#ifdef CONFIG_RTK_KDRV_DDCCI	
#ifndef CONFIG_RTK_KDRV_DDCCI_USING_POLLING
    rtk_ddcci_pooling_t();
#endif
