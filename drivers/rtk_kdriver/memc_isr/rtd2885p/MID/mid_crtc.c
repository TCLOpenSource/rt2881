								...
								OUTPUT_FORMAT enOutputFormat)
{
	...
	switch(enInputFrameRate)
	{
		case INPUT_FRAMERATE_24HZ:
		{
			switch(enOutputFrameRate)
			{
				...
				case OUTPUT_FRAMERATE_150HZ:
				{
					return FALSE;
				}
				case OUTPUT_FRAMERATE_165HZ:
				{
					return FALSE;
				}
	...
	if((u32M % 2 != 0) && (enOutputFrameRate > OUTPUT_FRAMERATE_60HZ) && !(enInputFrameRate == INPUT_FRAMERATE_24HZ && enOutputFrameRate == OUTPUT_FRAMERATE_120HZ && get_scaler_qms_mode_flag() == TRUE) && (u32N != u32M))
	{
		bDouble = TRUE;
	}
...
VOID Mid_CRTC_Set_LBMC_neg_vtrig_dly(unsigned char u1_enable)
{
#if (IC_K24)
	HAL_CRTC_Set_LBMC_neg_vtrig_dly(u1_enable);
#endif
}

unsigned int Mid_CRTC_Get_LBMC_neg_vtrig_dly(VOID)
{
#if (IC_K24)
	unsigned int lbmc_neg_vtrig_dly = 0;
	lbmc_neg_vtrig_dly = HAL_CRTC_Get_LBMC_neg_vtrig_dly();
	return lbmc_neg_vtrig_dly;
#endif
}

VOID Mid_CRTC_Set_DMA_Raw_PCMode_en(unsigned char u1_enable)
{
#if (IC_K24)
	HAL_CRTC_Set_DMA_Raw_PCMode_en(u1_enable);
#endif
}
