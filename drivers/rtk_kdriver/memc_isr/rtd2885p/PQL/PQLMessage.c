...
bool g_PQL_init = false;
extern BOOL ReadRegister(unsigned int uiAddr, int ibitStart, int ibitEnd, unsigned int *puiVal);
...
VOID PQL_CMD_crtc_N(const     unsigned char *pCommand)
{
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_19_reg, 31, 31, &log_en);
...
#if 1//RTK_MEMC_Performance_tunging_from_tv001
		if(pExtInfo == NULL || s_pContext_temp == NULL || pInfo == NULL){
			return;
		}		
#endif
