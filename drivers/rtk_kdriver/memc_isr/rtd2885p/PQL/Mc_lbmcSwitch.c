...
//////////////////////////////////////////////////////////////////////////
static VOID LbmcMode_Detect_onlyCnt_RTK2885p(const _PARAM_MC_LBMC_SWITCH *pParam, _OUTPUT_MC_LBMC_SWITCH *pOutput, unsigned char band_Type)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	_PQLPARAMETER *Me_Scene_Param = GetPQLParameter();
...
if(0)
{
	...
	if(Me_Scene_Param->_param_me_sceneAnalysis.u1_detect_Rotate_WindMill_en && s_pContext->_output_me_sceneAnalysis.u1_Rotate_Windmill_en){
		u8_lbmc_mode = s_pContext->_output_me_sceneAnalysis.u2_Rotate_Windmill_direction;
	}
...
VOID Mc_lbmcSwitch_Init_RTK2885p( _OUTPUT_MC_LBMC_SWITCH *pOutput)
{
	pOutput->u8_lf_lbmcMode = _MC_NORMAL_;
	pOutput->u8_hf_lbmcMode = _MC_NORMAL_;
}

VOID Mc_lbmcSwitch_Proc_RTK2885p(const _PARAM_MC_LBMC_SWITCH *pParam, _OUTPUT_MC_LBMC_SWITCH *pOutput)
{
/*	const _PQLCONTEXT *s_pContext = GetPQLContext();*/

	if (pParam->u8_lbmcSwitch_bypass == 0)
	{
		LbmcMode_Detect_onlyCnt_RTK2885p(pParam, pOutput, _LBMC_LF_);
		LbmcMode_Detect_onlyCnt_RTK2885p(pParam, pOutput, _LBMC_HF_);
	}
	else
	{
