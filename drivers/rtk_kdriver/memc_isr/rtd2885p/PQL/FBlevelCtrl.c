...
VOID SceneChage_Analysis_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FB_static2move_swSC_det_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
void SC_Signal_Proc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);

VOID FBTCCurveCalc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_FastMotion_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_MVChaos_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_SC_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_ME_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_GMV_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBGood_Scene_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBPan_Scene_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBCross_Scene_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBLevelCalc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID FBTCCurve_TH_specialScene_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);

VOID  fbTC_dhClose_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID  fbTC_deBlur_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
VOID  fbTC_deJudder_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);

unsigned int AutoTh_CurveMapping_RTK2885p(unsigned int iVal, unsigned int autoThL, unsigned int autoThH, unsigned char outPrecision);
unsigned int Multiply_2Val_protect(unsigned int multiA, unsigned int multiB, unsigned char divideBit_A);
VOID FBSystemCtrl_Proc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput);
...
//////////////////////////////////////////////////////////////////////////
VOID FBLevelCtrl_Init_RTK2885p(_OUTPUT_FBLevelCtrl *pOutput)
{
...
VOID FBLevelCtrl_Proc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
//	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	{
		SC_Signal_Proc_RTK2885p(pParam, pOutput);

		FB_static2move_swSC_det_RTK2885p(pParam, pOutput);
		SceneChage_Analysis_RTK2885p(pParam, pOutput);
		...
		FBTCCurveCalc_RTK2885p(pParam, pOutput);

		FBLevelCalc_RTK2885p(pParam, pOutput);
	}
	else
	{
	...
	//For system fallback
	FBSystemCtrl_Proc_RTK2885p(pParam, pOutput);
...
VOID FBTCCurveCalc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	FBTCCurve_SC_RTK2885p(pParam, pOutput);

	if (pOutput->u8_SCHoldCnt == 0 && pOutput->u8_SCNormCnt == pParam->u8_SC_NormCnt)
	{
		if(((u32_RB_val >>  19) & 0x01) ==0)
		FBTCCurve_TH_specialScene_RTK2885p(pParam, pOutput);

		if(((u32_RB_val >>  20) & 0x01) ==0)
		FBPan_Scene_RTK2885p(pParam, pOutput);

		FBTCCurve_FastMotion_RTK2885p(pParam,pOutput);
		FBTCCurve_MVChaos_RTK2885p(pParam,pOutput);

		//bad region control
		if(((u32_RB_val >>  21) & 0x01) ==0){
			if(s_pContext->_output_frc_sceneAnalysis.u8_sad_overshoot_flag!=1)
			FBTCCurve_ME_RTK2885p(pParam, pOutput);
		}
		//GMV control
		if(((u32_RB_val >>  22) & 0x01) ==0)
		FBTCCurve_GMV_RTK2885p(pParam, pOutput);
		if(((u32_RB_val >>  23) & 0x01) ==0)
		FBCross_Scene_RTK2885p(pParam, pOutput);
		if(((u32_RB_val >>  24) & 0x01) ==0)
		FBGood_Scene_RTK2885p(pParam, pOutput);
...
VOID FBTCCurve_MVChaos_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	if(log_en && pOutput->u8_MVChaos_cnt>0){ //d930[31]
		rtd_printk(KERN_EMERG, TAG_NAME_MEMC, "u8_Gain,%d u8_smooth_chg_cnt,%d, %u\n", u8_Gain, u8_smooth_chg_cnt, pOutput->u27_TempConsis_Th_h);
	}
	#endif
}
...
VOID FBTCCurve_SC_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	pOutput->u27_TempConsis_Th_h = pOutput->u27_SC_TC_Th_h>>MEMC_PARAM_TABLE.FB_Param[output_Resolution].u8_TempConsis_shift;
	pOutput->u27_TempConsis_Th_l = pOutput->u27_SC_TC_Th_l>>MEMC_PARAM_TABLE.FB_Param[output_Resolution].u8_TempConsis_shift;
...
VOID FBTCCurve_TH_specialScene_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	pOutput->u27_TempConsis_Th_h >>= MEMC_PARAM_TABLE.FB_Param[output_Resolution].u8_TempConsis_shift;
	pOutput->u27_TempConsis_Th_l >>= MEMC_PARAM_TABLE.FB_Param[output_Resolution].u8_TempConsis_shift;
...
VOID FBLevelCalc_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	// temp consistency mapping u8_FBResult
	{
		...
		u27_mv_tempconsis_limit >>= MEMC_PARAM_TABLE.FB_Param[output_Resolution].u8_TempConsis_shift;
		...
		pOutput->u8_FBResult      = AutoTh_CurveMapping_RTK2885p(pOutput->u27_FBTempConsis, pOutput->u27_TempConsis_Th_l, pOutput->u27_TempConsis_Th_h, 8);
  ...
  //      rtd_pr_memc_debug("u1_panx_scene=%d u1_pany_scene=%d\n",pOutput->u1_panx_scene,pOutput->u1_pany_scene);
   ...
   //     rtd_pr_memc_debug("u8_FBResult_IIR=%d\n",pOutput->u8_FBResult);
	...
	fbTC_dhClose_ctrl_RTK2885p(pParam, pOutput);
	...
	#endif
	fbTC_deBlur_ctrl_RTK2885p(pParam, pOutput);
	...
	#endif
	fbTC_deJudder_ctrl_RTK2885p(pParam, pOutput);
...
VOID fbTC_dhClose_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	u8_Dh_FbLvl_cur            = _MIN_(0xFF, AutoTh_CurveMapping_RTK2885p(pOutput->u27_FBTempConsis, u27_tc_thL, u27_tc_thH, 8));
...
VOID fbTC_deBlur_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	u16_blur_FBlvl_cur        = _MIN_(0x80, AutoTh_CurveMapping_RTK2885p(pOutput->u27_FBTempConsis, u27_tc_thL, u27_tc_thH, _PHASE_FLBK_LVL_PRECISION));
...
VOID fbTC_deJudder_ctrl_RTK2885p(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	u16_judder_FBlvl_cur     = _MIN_(0x80, AutoTh_CurveMapping_RTK2885p(pOutput->u27_FBTempConsis, u27_tc_thL, u27_tc_thH, _PHASE_FLBK_LVL_PRECISION));
...
VOID MC_RMV_blending_RTK2885p(const _PARAM_bRMV_rFB_CTRL *pParam, _OUTPUT_bRMV_rFB_CTRL *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
#if 1
VOID MC_ReginaolFB_RTK2885p(const _PARAM_bRMV_rFB_CTRL *pParam, _OUTPUT_bRMV_rFB_CTRL *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	int i = 0;
