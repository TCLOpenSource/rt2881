...
///////////////////////////////////////////////////////////////////////////////////////////
VOID Dh_close_action_RTK2885p(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
...
VOID Dh_closeSignal_Detect_RTK2885p(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	unsigned int reg_08_dh_off2on_rb_en = 0;
       ...
       unsigned int  special_pattern_227_flag = 0, special_pattern_49_flag = 0,special_pattern_421_flag = 0,special_blackapl=0,special_pattern_224_flag = 0,special_pattern_185_flag = 0;
...
#endif
	int skyworth_dh_bypass_flag = 0;

       unsigned int  special_8006 = 0;
	...
	if(pParam->u1_fblvl_en == 1)
	{
        ...
        //   rtd_pr_memc_debug("u8_dh_fbLvl=%d u8_fbLvl=%d u8_fbLvl_Th0=%d u8_fbLvl_Th1=%d\n",u8_dh_fbLvl,u8_fbLvl, pParam->u8_fbLvl_Th0,pParam->u8_fbLvl_Th1);
              ...
              ReadRegister(SOFTWARE3_SOFTWARE3_27_reg, 19, 19, &special_pattern_224_flag);//B809d968
		...
		#else
		if((pOutput->u1_fblvl_1frm_close == 1) && (!special_pattern_flag) && (!special_pattern_227_flag) && (!special_pattern_49_flag) && (!special_pattern_224_flag))
		#endif
		{
			u1_fblvl_dhClose          = 1;
	...
	{
	    Dh_close_sml_rgnAction_RTK2885p(pParam, pOutput);
	}
	else
	{
	...
	if (pParam->u1_meAllDtl_en == 1)
	{
		...
		unsigned char  u1_dh_on2off_en = u25_meAllDtl <= (pParam->u25_meAllDtl_Th0)? 1 : 0;
		unsigned char  u1_dh_off2on_en = u25_meAllDtl >= (pParam->u25_meAllDtl_Th1)? 1 : 0;
...
#endif
		u1_meAllDtl_dhClose = special_8006?0:u1_meAllDtl_dhClose;
	...
	if (pParam->u1_panning_en == 1)
	{
		...
		ReadRegister(KME_ME1_BG1_ME_BG_DUMMY_0_reg, 20, 20, &special_blackapl);
	       ReadRegister(HARDWARE_HARDWARE_25_reg, 10,10 , &special_pattern_185_flag); 
		...
		if((special_blackapl==1)||(special_pattern_185_flag==1))
              {
                  u1_panning_close = 0;
              }
	}
	...
	skyworth_dh_bypass_flag =  s_pContext->_output_frc_sceneAnalysis.u8_dh_condition_cat_dhbypass_flag == 1||//maoyaozhuan	
					  s_pContext->_output_frc_sceneAnalysis.u8_dh_condition_new_dhbypass_flag == 1||//sky_worth condition
					  s_pContext->_output_frc_sceneAnalysis.u8_dh_condition_C005plane_status  == 1|| //plane
					  s_pContext->_output_frc_sceneAnalysis.u8_dh_condition_insect_status == 1||//insect
					  s_pContext->_output_frc_sceneAnalysis.u8_dh_condition_rotate_mv_broken == 1;//list248
...
VOID Dh_close_Init_RTK2885p(_OUTPUT_DH_CLOSE *pOutput)
{
	pOutput->u8_dhOff_holdFrm         = 0;
	...
	pOutput->u1_IPR_dh_close  = 0;
}

VOID Dh_close_sml_rgnAction_RTK2885p(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	int i = 0;
	...
	//unsigned int u16_cnt_totalNum = u16_cnt_totalNum1 == 0 ? 1 : u16_cnt_totalNum1>>5;
	...
	if( (pOutput->u5_dh_good_rgn_num >= pParam->u5_dh_sml_rgnAction_goodrgn_cnt_thr) &&
		...
		(pParam->u8_dh_sml_rgnAction_apl_thr)))
	{
		pOutput->u1_dh_sml_rgnAction_close = 1;
...
VOID Dh_close_Proc_RTK2885p(const _PARAM_DH_CLOSE *pParam, _OUTPUT_DH_CLOSE *pOutput)
{
	// dh close signal detect
	if (pParam->u2_dhClose_bypass == 0)
	{
		Dh_closeSignal_Detect_RTK2885p(pParam, pOutput);
	}
	else
	{
	...
	// dh close action
	Dh_close_action_RTK2885p(pParam, pOutput);
	return;
}
