...
extern unsigned int g_ME1_VBUF_GAIN;
extern short GetMedian_SS(short *s16_Array, unsigned short u16_ArraySize);

#if CONFIG_REPEAT_NEW_ALOG
const PeriodicStatus gMEMC_PeriodicParamTable_Default[MEMC_PERIODIC_LEVEL_NUM] =
{
    /* DISABLE */
    {
...
VOID Periodic_Init_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
	unsigned char u8_i;
	...
	memcpy(&gMEMC_PeriodicParamTable, &gMEMC_PeriodicParamTable_Default, sizeof(gMEMC_PeriodicParamTable_Default));
	for(u8_i=0; u8_i<MEMC_PERIODIC_LEVEL_NUM; u8_i++){
		gMEMC_PeriodicParamTable[u8_i].me1_freq_avgbv_mv_cnt_th = (gMEMC_PeriodicParamTable_Default[u8_i].me1_freq_avgbv_mv_cnt_th<<g_ME1_VBUF_SHIFT)/g_ME1_VBUF_GAIN;
	}

	rtd_pr_memc_notice("[%s] me1_freq_avgbv_mv_cnt_th,%d,%d,%d,%d, calib param,%d,%d\n", __FUNCTION__,
		gMEMC_PeriodicParamTable[MEMC_PERIODIC_LEVEL_DISABLE].me1_freq_avgbv_mv_cnt_th, 
		gMEMC_PeriodicParamTable[MEMC_PERIODIC_LEVEL_LOW].me1_freq_avgbv_mv_cnt_th, 
		gMEMC_PeriodicParamTable[MEMC_PERIODIC_LEVEL_MID].me1_freq_avgbv_mv_cnt_th, 
		gMEMC_PeriodicParamTable[MEMC_PERIODIC_LEVEL_HIGH].me1_freq_avgbv_mv_cnt_th, 
		g_ME1_VBUF_GAIN, g_ME1_VBUF_SHIFT);

    Periodic_StatusInit_RTK2885p(pOutput);
#endif
}

VOID Periodic_Proc_RTK2885p(const _PARAM_PERIODIC_MANAGE *pParam, _OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
    /* update regr value */
	...
	Periodic_Total_Enable_RTK2885p();
    Periodic_UpdateInfo_RTK2885p(pOutput);
    #if 1
    Periodic_StatusUpdate_RTK2885p(pParam, pOutput);

    if ((kme_algo_me2_ref_vbuf_reg.dummy1809ed1c_27_13 & 0x1) == 0){ // use 13 bits to control firmware auto, 1=bypass
        //Periodic_Apply_RTK2885p(pOutput);
		Periodic_Apply_Global_AvgBv_RTK2885p(pOutput);
		Periodic_Apply_Region_AvgBv_RTK2885p(pOutput);
		Periodic_Apply_NeighborhoodMV_RTK2885p(pOutput);
    }
    #endif
#endif
}

VOID Periodic_Detect_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
    // check mv stablilty
	...
	lt_zmv_cnt = s_pContext->_output_read_comreg.me1_bv_stats_rg_cnt[0]+s_pContext->_output_read_comreg.me1_bv_stats_rg_cnt[1];//kme_me1_top11_me1_bv_stats_rg_idx_0_1_2_cnt_reg.me1_bv_stats_rg_idx0_cnt+kme_me1_top11_me1_bv_stats_rg_idx_0_1_2_cnt_reg.me1_bv_stats_rg_idx1_cnt;
	rt_zmv_cnt = s_pContext->_output_read_comreg.me1_bv_stats_rg_cnt[6]+s_pContext->_output_read_comreg.me1_bv_stats_rg_cnt[7];//kme_me1_top11_me1_bv_stats_rg_idx_6_7_8_cnt_reg.me1_bv_stats_rg_idx6_cnt+kme_me1_top11_me1_bv_stats_rg_idx_6_7_8_cnt_reg.me1_bv_stats_rg_idx7_cnt;
    ...
    {
        Periodic_ChangeLevel_RTK2885p(pOutput, MEMC_PERIODIC_LEVEL_DISABLE);
    }
#if 0
	else if (cnt3_count == 0)//beauty and the beast
	{
        Periodic_ChangeLevel_RTK2885p(pOutput, MEMC_PERIODIC_LEVEL_LOW);
	}
#endif
    else
    {
        Periodic_ChangeLevel_RTK2885p(pOutput, MEMC_PERIODIC_LEVEL_MID);
    }
#else
	Periodic_ChangeLevel_RTK2885p(pOutput, MEMC_PERIODIC_LEVEL_MID);
#endif
...
VOID Periodic_Detect_SC_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID Periodic_Detect_ZMV_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID Periodic_Detect_Cnt3_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID Periodic_Detect_Unstable_Rmv_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	...
	if(log_unstable_rgn){//d9d0[28]
		rtd_pr_memc_emerg("[%s] stable,%d, unstable,%d\n", __FUNCTION__, pOutput->u5_stable_rgn_num, pOutput->u5_unstable_repeat_reg_num);
	}
}

VOID Periodic_StatusInit_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
...
VOID Periodic_StatusUpdate_RTK2885p(const _PARAM_PERIODIC_MANAGE *pParam, _OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
    ...
    // function here
    Periodic_Detect_RTK2885p(pOutput);
	Periodic_Detect_SC_RTK2885p(pOutput);
	Periodic_Detect_Cnt3_RTK2885p(pOutput);
	Periodic_Detect_Unstable_Rmv_RTK2885p(pOutput);
	Periodic_Detect_SC_RTK2885p(pOutput);
	Periodic_Detect_ZMV_RTK2885p(pOutput);
#endif
}

VOID Periodic_Apply_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
...
VOID Periodic_Apply_Global_AvgBv_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	kme_algo_me1_freq_ctrl_RBUS kme_algo_me1_freq_ctrl_reg;
...
VOID Periodic_Apply_Region_AvgBv_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
...
void Periodic_Set_Replace_MV_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput, unsigned char u8_x, unsigned char u8_y)
{
	...
	if(mv_select==0){
		...
		if(log_en && u8_x==2 && u8_y==1){
			rtd_pr_memc_emerg("[%s]	mv,%d,%d  mv,%d,%d,%d,%d, %d,%d,%d,%d,\n", __FUNCTION__, s11_repeat_mvx, s10_repeat_mvy,
...
VOID Periodic_Apply_NeighborhoodMV_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	...
	if(log_en){//d858[30]
		rtd_pr_memc_emerg("[%s]	[%d] u8_cnt,%d, u5_unstable_repeat_reg_num,%d, sc,%d, other,%d,%d\n", __FUNCTION__, 
		...
		for(u8_y=0; u8_y<4; u8_y++){
			for(u8_x=0; u8_x<8; u8_x++){
				if(pOutput->u1_unstable_repeat_rgn[u8_y][u8_x]==true){
					Periodic_Set_Replace_MV_RTK2885p(pOutput, u8_x, u8_y);
				}
			}
		}
...
VOID Periodic_Total_Enable_RTK2885p(VOID)
{
#if 0
...
VOID Periodic_UpdateInfo_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput)
{
	...
	if(log_en && u8_cnt>10){ //d9d8[29]
		...
		rtd_pr_memc_emerg("[IIR] ,%d,%d,%d,%d,%d,%d,%d,%d	,%d,%d,%d,%d,%d,%d,%d,%d	,%d,%d,%d,%d,%d,%d,%d,%d	,%d,%d,%d,%d,%d,%d,%d,%d\n",
			pOutput->u10_freq_statistic_cnt3_IIR[0][0],pOutput->u10_freq_statistic_cnt3_IIR[0][1],
			pOutput->u10_freq_statistic_cnt3_IIR[0][2],pOutput->u10_freq_statistic_cnt3_IIR[0][3],
			pOutput->u10_freq_statistic_cnt3_IIR[0][4],pOutput->u10_freq_statistic_cnt3_IIR[0][5],
			pOutput->u10_freq_statistic_cnt3_IIR[0][6],pOutput->u10_freq_statistic_cnt3_IIR[0][7],
			pOutput->u10_freq_statistic_cnt3_IIR[1][0],pOutput->u10_freq_statistic_cnt3_IIR[1][1],
			pOutput->u10_freq_statistic_cnt3_IIR[1][2],pOutput->u10_freq_statistic_cnt3_IIR[1][3],
			pOutput->u10_freq_statistic_cnt3_IIR[1][4],pOutput->u10_freq_statistic_cnt3_IIR[1][5],
			pOutput->u10_freq_statistic_cnt3_IIR[1][6],pOutput->u10_freq_statistic_cnt3_IIR[1][7],
			pOutput->u10_freq_statistic_cnt3_IIR[2][0],pOutput->u10_freq_statistic_cnt3_IIR[2][1],
			pOutput->u10_freq_statistic_cnt3_IIR[2][2],pOutput->u10_freq_statistic_cnt3_IIR[2][3],
			pOutput->u10_freq_statistic_cnt3_IIR[2][4],pOutput->u10_freq_statistic_cnt3_IIR[2][5],
			pOutput->u10_freq_statistic_cnt3_IIR[2][6],pOutput->u10_freq_statistic_cnt3_IIR[2][7],
			pOutput->u10_freq_statistic_cnt3_IIR[3][0],pOutput->u10_freq_statistic_cnt3_IIR[3][1],
			pOutput->u10_freq_statistic_cnt3_IIR[3][2],pOutput->u10_freq_statistic_cnt3_IIR[3][3],
			pOutput->u10_freq_statistic_cnt3_IIR[3][4],pOutput->u10_freq_statistic_cnt3_IIR[3][5],
			pOutput->u10_freq_statistic_cnt3_IIR[3][6],pOutput->u10_freq_statistic_cnt3_IIR[3][7]);
...
VOID Periodic_ChangeLevel_RTK2885p(_OUTPUT_PERIODIC_MANAGE *pOutput, int level)
{
#if CONFIG_REPEAT_NEW_ALOG
...
VOID PRD_Init_RTK2885p(_OUTPUT_PRD_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
	pOutput->prd_level = MEMC_PRD_OFF;
	PRD_Apply_RTK2885p(pOutput);
#endif
}

VOID PRD_Proc_RTK2885p(const _PARAM_PRD_MANAGE *pParam, _OUTPUT_PRD_MANAGE *pOutput, _OUTPUT_PERIODIC_MANAGE *pOutput2)
{
#if CONFIG_REPEAT_NEW_ALOG
	...
	{
		PRD_Detect_RTK2885p(pParam, pOutput, pOutput2);
		PRD_Apply_RTK2885p(pOutput);		
	}
#endif
}

VOID PRD_Detect_RTK2885p(const _PARAM_PRD_MANAGE *pParam, _OUTPUT_PRD_MANAGE *pOutput, _OUTPUT_PERIODIC_MANAGE *pOutput2)
{
#if CONFIG_REPEAT_NEW_ALOG
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID PRD_Apply_RTK2885p(_OUTPUT_PRD_MANAGE *pOutput)
{
#if CONFIG_REPEAT_NEW_ALOG
