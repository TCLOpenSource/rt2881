...
unsigned short g_MAX_logo_dh_window_w = 0;
unsigned short g_MAX_logo_dh_window_h = 0;
...
VOID RimCtrl_OutResolution_Parse_RTK2885p(_OUTPUT_RimCtrl *pOutput)
{
	...
	if (outResolution_cur != (PQL_OUTPUT_RESOLUTION)pOutput->u4_outResolution_pre)
	{
		...
		g_MAX_logo_dh_window_w = pOutput->u12_out_resolution[_RIM_RHT]>>3;
		g_MAX_logo_dh_window_h = pOutput->u12_out_resolution[_RIM_BOT]>>3;
...
unsigned int SetSearchLine_RTK2885p(unsigned int H_line_search_position, unsigned short MinVal, unsigned short MaxVal)
{
...
VOID RimDet_by_KMC_YUV_Meter_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
#if RTK_MEMC_Performance_tunging_from_tv001
	//const _PQLCONTEXT *s_pContext = GetPQLContext();
#else
	...
	unsigned int u32_rb_val = 0, sum_apl = 0, avg_pos = 0, log_en = 0, u1_Algo_en = 0, yuv_Hist_useage = 0;
	...
	if(!u1_Algo_en && yuv_Hist_useage!=YUV_HIST_RIM) { //u1_Algo_en:d5ac[15]
		g_RightRim_16px = 0;
		line_pos[0]=0;
		return;
	}
	...
	if(status_machine == 0){//init
		...
		SetSearchLine_RTK2885p(H_line_search_position, 0, 0x1FFF);
		status_machine = 1;
	}
	else if(hold_cnt>0){
		hold_cnt--;
		...
		if(sum_apl == 0){
			...
			if(H_line_search_position!=H_line_search_position_pre){
				status_machine = 1;
				debugID = 2;
				H_line_search_position = SetSearchLine_RTK2885p(H_line_search_position, h_line_total-17, h_line_total-1);			
			}else{
				if(case_bright){ 
					status_machine = 2;
					debugID = 3;
					H_line_search_position = SetSearchLine_RTK2885p(H_line_search_position+1, h_line_total-17, h_line_total-1);
					hold_cnt=1;
 				}else if(case_dark){ 
	 				status_machine = 3;
					debugID = 4;
		...
		H_line_search_position = (H_line_search_position==h_line_total-1) ? 
									SetSearchLine_RTK2885p(finetune_pos-1, h_line_total-17, h_line_total-1) : 
									SetSearchLine_RTK2885p(finetune_pos+1, h_line_total-17, h_line_total-1);
		hold_cnt=3;
		debugID = 77;
	}
		...
		if(case_bright){ 
			...
			H_line_search_position = SetSearchLine_RTK2885p(finetune_pos+1, h_line_total-17, h_line_total-1);
			hold_cnt=1;
			debugID = 51;
		}
			...
			status_machine = 4;
			debugID = 63;
			H_line_search_position = SetSearchLine_RTK2885p(finetune_pos, h_line_total-17, h_line_total-1);	
		}else{
			status_machine = 6;
			H_line_search_position = SetSearchLine_RTK2885p(finetune_pos-1, h_line_total-17, h_line_total-1);
			hold_cnt=3;
...
VOID RimCtrl_Get_Rim_Info_RTK2885p(unsigned short *u12_Rim_Info, unsigned short *u12_Rim_Data)
{
...
VOID RimCtrl_Get_HW_rim_RTK2885p(_OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned short u12_BBD_fineRim = 0, u12_BBD_roughRim = 0, u12_out_resolution = 0, u12_HW_rim = 0;
	unsigned char u1_BBD_fineValid = 0, u1_BBD_roughValid = 0, rimID = 0;
	unsigned int log_en = 0;

	ReadRegister(HARDWARE_HARDWARE_43_reg, 10, 10, &log_en);

	for(rimID=0; rimID<_RIM_NUM; rimID++){
		u12_BBD_fineRim   = s_pContext->_output_read_comreg.u12_BBD_fineRim_rb[rimID];
		u12_BBD_roughRim  = s_pContext->_output_read_comreg.u12_BBD_roughRim_rb[rimID];
		u1_BBD_fineValid  = s_pContext->_output_read_comreg.u1_BBD_fineValid_rb[rimID];
		u1_BBD_roughValid = s_pContext->_output_read_comreg.u1_BBD_roughValid_rb[rimID];
		u12_out_resolution = pOutput->u12_out_resolution[rimID];
		...
		pOutput->u12_Rim_HW[rimID] = u12_HW_rim;
	}
...
unsigned short RimCtrl_Get_Rim_Bound_RTK2885p(unsigned char u2_Rim_id)
{
	u2_Rim_id = _CLIP_(u2_Rim_id, 0, _RIM_NUM-1);
	return g_rim_bound[u2_Rim_id];
}

VOID RimCtrl_Boundary_Protect_RTK2885p(unsigned short *u12_Rim_source, unsigned short *u12_Rim_Result, _OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	for (u8_k = 0; u8_k < _RIM_NUM; u8_k++){
		u12_rim_bound[u8_k] = RimCtrl_Get_Rim_Bound_RTK2885p(u8_k);
	}
...
unsigned short RimCtrl_Get_Rim_Speed_RTK2885p(unsigned rim_id)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned short u11_speed_avg = 0;

	if(rim_id == _RIM_TOP){
		u11_speed_avg = (_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[0])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[1])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[2])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[3])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[4])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[5])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[6])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[7]))>>3;
	}
	else if(rim_id == _RIM_BOT){
		u11_speed_avg = (_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[24])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[25])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[26])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[27])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[28])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[29])+
						 _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[30])+_ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[31]))>>3;;
	}
	else if(rim_id == _RIM_RHT){
		u11_speed_avg = (_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[7])+_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[15])+
						 _ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[23])+_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[31]))>>2;
	}
	else if(rim_id == _RIM_LFT){
		u11_speed_avg = (_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[0])+_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[8])+
						 _ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[16])+_ABS_(s_pContext->_output_read_comreg.s11_me_rMV_1st_vx_rb[24]))>>2;
	}

	return u11_speed_avg;
}

VOID RimCtrl_Shrink_By_Pan_RTK2885p(unsigned short *u12_det_Rim)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	const unsigned char u8_MAX_shrink_val = 4;
	unsigned char u8_shrink_case = 0;
	unsigned short u12_shrink_value = 0;
	unsigned int u1_pan_protect = 0, u8_unconf_th = 0, speed_th = 0, force_bot, shrink_1, shrink_2, log_en, show_cross;
	...
	unsigned short u12_unconf = s_pContext->_output_read_comreg.u12_me_GMV_1st_unconf_rb;

	ReadRegister(HARDWARE_HARDWARE_43_reg, 14, 14, &u1_pan_protect);
	ReadRegister(SOFTWARE2_SOFTWARE2_56_reg,16,19, &u8_unconf_th);
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg, 8,15, &speed_th);
	
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg,31,31, &force_bot);
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg,30,30, &log_en);
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg,29,29, &show_cross);
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg,16,23, &shrink_1);
	ReadRegister(SOFTWARE2_SOFTWARE2_55_reg,24,28, &shrink_2);

	// adjust rim for panning
	if(force_bot){
		u12_shrink_value = _MIN_((_ABS_(s10_gmv_mvy)/shrink_1), u8_MAX_shrink_val);
		...
		g_MC_Shrink_V = shrink_2;
		g_MC_Shrink_H = 0;

	}
	else if(u1_pan_protect && u2_Panning_flg!=Pan_Dir_None && u12_unconf<u8_unconf_th) {
		u8_shrink_case = 99;
		if(u2_Panning_flg==Pan_Dir_X) {
			u8_shrink_case = 10;
			u12_shrink_value = _MIN_((_ABS_(s11_gmv_mvx)/shrink_1), u8_MAX_shrink_val);//(g_resolution_width+512)>>10; //3840:4, 1920:2
			if(s11_gmv_mvx>0 && RimCtrl_Get_Rim_Speed_RTK2885p(_RIM_RHT)>speed_th) {
				u8_shrink_case = 1;
			...
			else if(s11_gmv_mvx<0 && RimCtrl_Get_Rim_Speed_RTK2885p(_RIM_LFT)>speed_th) {
				u8_shrink_case = 2;
			...
			g_MC_Shrink_V = 0;
			g_MC_Shrink_H = shrink_2;
		}
		else {//panning y direction
			u8_shrink_case = 11;
			u12_shrink_value = _MIN_((_ABS_(s10_gmv_mvy)/shrink_1), u8_MAX_shrink_val);//(g_resolution_height+512)>>10; //2160:2, 1080:1
			if(s10_gmv_mvy>0 && RimCtrl_Get_Rim_Speed_RTK2885p(_RIM_BOT)>speed_th) {
				u8_shrink_case = 3;
				//u12_det_Rim[_RIM_TOP] = (u12_det_Rim[_RIM_TOP] < 1) ? 0 : u12_det_Rim[_RIM_TOP]-1;
				u12_det_Rim[_RIM_BOT] = u12_det_Rim[_RIM_BOT] - u12_shrink_value;
			}
			else if(s10_gmv_mvy<0 && RimCtrl_Get_Rim_Speed_RTK2885p(_RIM_TOP)>speed_th) {
				u8_shrink_case = 4;
	...
	if(show_cross){
		if(u8_shrink_case>=1 && u8_shrink_case<=4){
	...
	if(log_en){//d8c4[30]
		rtd_printk(KERN_EMERG, TAG_NAME_MEMC,"shrink,%d,%d, bot,%d, case,%d, u2_Panning_flg,%d, MAD,%d,%d, u12_shrink_value,%d, gmv,%d,%d unconf,%d\n", g_MC_Shrink_V, g_MC_Shrink_H, u12_det_Rim[_RIM_BOT],
		u8_shrink_case, u2_Panning_flg, s_pContext->_output_me_sceneAnalysis.u8_MAD_x, s_pContext->_output_me_sceneAnalysis.u8_MAD_y, 
		u12_shrink_value, s11_gmv_mvx, s10_gmv_mvy, u12_unconf);
	}
}

VOID RimCtrl_HW_extract_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	unsigned int log_en2 = 0, log_en3 = 0, log_en4 = 0, log_en5 = 0, debug_en = 0, debug_val[4];
	...
	for (u8_k = 0; u8_k < _RIM_NUM; u8_k ++){
		u12_rim_bound[u8_k] = RimCtrl_Get_Rim_Bound_RTK2885p(u8_k);
		u12_det_Rim[u8_k] = pOutput->u12_Rim_HW[u8_k];
#if RTK_MEMC_Performance_tunging_from_tv001
	...
	RimCtrl_Shrink_By_Pan_RTK2885p(u12_det_Rim);
	...
	*/
	...
	// bound protect.
#if 1
		RimCtrl_Boundary_Protect_RTK2885p(u12_det_Rim, pOutput->u12_Rim_Cur, pOutput);
#else
...
VOID RimCtrl_FastOut_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
/*	const _PQLCONTEXT *s_pContext = GetPQLContext();*/
	...
	unsigned char u8_RimBiggerTh = pParam->u8_RimBiggerTh;
...
VOID RimCtrl_SlowIn_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
//	const _PQLCONTEXT *s_pContext = GetPQLContext();
	int i = 0;
	...
	unsigned int log_en = 0, log_en2 = 0, test_val_2 = 0, u1_limit_min_speed = 0, u1_speedup_pan = 0;
	ReadRegister(SOFTWARE2_SOFTWARE2_61_reg, 30, 30, &log_en);
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg, 28, 28, &u1_speedup_pan);
...
//		if (s_pContext->_external_info.u1_Hist_SC_flg)
		...
		else if(u2_Panning_flg!=Pan_Dir_None && u1_speedup_pan){
			if((u2_Panning_flg==Pan_Dir_X && (i==_RIM_LFT||i==_RIM_RHT)) || (u2_Panning_flg==Pan_Dir_Y && (i==_RIM_TOP||i==_RIM_BOT))) {
				u8_dyn_thr = pParam->u8_RimSmallerSCTh * u8_thr_coef;
			}
			else {
		...
		if(log_en && i == _RIM_RHT){
			rtd_pr_memc_notice("[Rim_check_protect_MC][%d][,%d,%d,][,%d,%d,][,%d,][,%d,%d,]\n\r", __LINE__, sc_cnt, k23755_flash_flg, g_HisDiff, g_HueDiff, flash_cnt, pOutput->s8_SmallerCnt[i], u8_dyn_thr);
		}
...
VOID RimCtrl_ME1_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
...
VOID RimCtrl_ME2_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	#if 0
	rtd_pr_memc_debug("===== RimCtrl_ME2_Rim_Convert_RTK2885p =========\n");	
...
VOID RimCtrl_MC_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
/*	const _PQLCONTEXT *s_pContext = GetPQLContext();*/
	...
	if(output_Resolution==_PQL_OUT_3840x1080){//4k1k //tv010 upper boundary with airplane
		...
		if(pOutput->u12_Rim_Result[_RIM_BOT]>=2){
		...
		if(pOutput->u12_Rim_Result[_RIM_BOT]>pParam->u8_mc_ptRim_shrink && (pOutput->u12_mc_rim0[_RIM_BOT]==pOutput->u12_mc_rim1[_RIM_BOT]))
...
VOID RimCtrl_Logo_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
/*	const _PQLCONTEXT *s_pContext = GetPQLContext();*/
	...
	#if 0
	rtd_pr_memc_debug("===== RimCtrl_Logo_Rim_Convert_RTK2885p =========\n");	
...
VOID RimCtrl_Film5RgnPos_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
/*		const _PQLCONTEXT *s_pContext = GetPQLContext();*/
...
VOID RimCtrl_Film_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	unsigned char  u4_Res_H_Divide = g_H_Divide;
	unsigned char  u4_Res_V_Divide = g_V_Divide;
	...
	RimCtrl_Film5RgnPos_Convert_RTK2885p(pParam, pOutput);
}

VOID RimCtrl_Dh_Rim_Convert_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	pOutput->u9_mc_logo_dh_window_rim[_RIM_LFT] = _CLIP_((pOutput->u12_Rim_Result[_RIM_LFT]>>3)+1, 0, g_MAX_logo_dh_window_w);
	pOutput->u9_mc_logo_dh_window_rim[_RIM_RHT] = ((pOutput->u12_Rim_Result[_RIM_RHT]>>3) < 1) ? 0 : (pOutput->u12_Rim_Result[_RIM_RHT]>>3)-1;
	pOutput->u9_mc_logo_dh_window_rim[_RIM_RHT] = _CLIP_(pOutput->u9_mc_logo_dh_window_rim[_RIM_RHT], 0, g_MAX_logo_dh_window_w);
	pOutput->u9_mc_logo_dh_window_rim[_RIM_TOP] = _CLIP_((pOutput->u12_Rim_Result[_RIM_TOP]>>3)+1, 0, g_MAX_logo_dh_window_h);
	pOutput->u9_mc_logo_dh_window_rim[_RIM_BOT] = ((pOutput->u12_Rim_Result[_RIM_BOT]>>3) < 1) ? 0 : (pOutput->u12_Rim_Result[_RIM_BOT]>>3)-1;
	pOutput->u9_mc_logo_dh_window_rim[_RIM_BOT] = _CLIP_(pOutput->u9_mc_logo_dh_window_rim[_RIM_BOT], 0, g_MAX_logo_dh_window_h);
}

VOID RimCtrl_Output_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned char u8_k = 0;

	// K-28129, protect rim_result by resolution
	for (u8_k = 0; u8_k < _RIM_NUM; u8_k ++)
	{
		if(u8_k == _RIM_BOT || u8_k == _RIM_RHT)
			pOutput->u12_Rim_Result[u8_k] = _MIN_(pOutput->u12_Rim_Result[u8_k], pOutput->u12_out_resolution[u8_k]);
	}
...
#endif
		RimCtrl_ME1_Rim_Convert_RTK2885p(pParam, pOutput);
		RimCtrl_ME2_Rim_Convert_RTK2885p(pParam, pOutput);	
		RimCtrl_Logo_Rim_Convert_RTK2885p(pParam, pOutput);
		RimCtrl_MC_Rim_Convert_RTK2885p(pParam, pOutput);

		RimCtrl_Film_Rim_Convert_RTK2885p(pParam, pOutput);
		RimCtrl_Dh_Rim_Convert_RTK2885p(pParam, pOutput);
	}
}

VOID Rim_check_protect_ME1_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
...
VOID Rim_check_protect_ME2_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
	unsigned int Res_H = g_resolution_width;
	unsigned int Res_V = g_resolution_height;	
...
VOID Rim_check_protect_Logo_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
	unsigned int Res_H = g_resolution_width;
	unsigned int Res_V = g_resolution_height;	
...
VOID Rim_check_protect_MC_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
	unsigned int Res_H = g_resolution_width;
	unsigned int Res_V = g_resolution_height;	
...
VOID Rim_check_protect_Film_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
	unsigned int Res_H = g_resolution_width;
	unsigned int Res_V = g_resolution_height;	
	...
	unsigned char  Res_V_Divide = g_V_Divide;  // Fot RimCtrl_Film_Rim_Convert_RTK2885p
...
VOID Rim_check_protect_Dh_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	int i=0;
	unsigned int Res_H = g_resolution_width;
	unsigned int Res_V = g_resolution_height;	
...
VOID Rim_check_protect_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
		
	//rtd_pr_memc_debug("[Rim_check_protect_RTK2885p] u1_RimChange  = %d\n",pOutput->u1_RimChange);
	Rim_check_protect_ME1_RTK2885p(pParam,pOutput);
	Rim_check_protect_ME2_RTK2885p(pParam,pOutput);
	Rim_check_protect_Logo_RTK2885p(pParam,pOutput);
	Rim_check_protect_MC_RTK2885p(pParam,pOutput);
	Rim_check_protect_Film_RTK2885p(pParam,pOutput);
	Rim_check_protect_Dh_RTK2885p(pParam,pOutput);
}

VOID RimCtrl_Stable_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
#if 0
    int i = 0; 
    for (i = 0; i < _RIM_NUM; i ++)
...
VOID RimCtrl_Panning_Detect_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	//const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID RimCtrl_DynamicBlackTh_RTK2885p(VOID)
{
#if 1
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	
	unsigned char u8_black_th = 0;	
...
VOID RimCtrl_Rim_Comp_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
*/
	...
	unsigned char debug_id = 0;
	unsigned int algo_mv_en, log_mv_en, u13_rmvy_th, u13_abs_top_rmvy_sum = 0, u13_abs_bot_rmvy_sum = 0;
	...
	ReadRegister(SOFTWARE2_SOFTWARE2_43_reg, 15, 15, &algo_mv_en);
	ReadRegister(SOFTWARE2_SOFTWARE2_43_reg, 14, 14, &log_mv_en);
	ReadRegister(SOFTWARE2_SOFTWARE2_43_reg,  0, 12, &u13_rmvy_th);
#endif
	...
	if(RHT_ratio > LFT_ratio+1){ // for #35 & K-23755  // R>L+1 for 30010_I_Robot
		if(Comp_R_cnt < 9)
			Comp_R_cnt++;
	}else{
		if(Comp_R_cnt > 0)
	...
	// the algo will check boundary rmv when system want to shift boundary
	// if boundary mvy is large, system won't shift boundary to prevent boundary garbage
	if(algo_mv_en){//d8ac[15]
		u13_abs_bot_rmvy_sum = _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[24]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[25]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[26]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[27]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[28]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[29]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[30]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[31]);
		u13_abs_top_rmvy_sum = _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[0]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[1]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[2]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[3]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[4]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[5]) + 
							   _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[6]) + _ABS_(s_pContext->_output_read_comreg.s10_me_rMV_1st_vy_rb[7]);

		if(pOutput->u1_RimCompflag == 1 || pOutput->u1_RimCompflag == 3){//for #453
			if(u13_abs_bot_rmvy_sum > u13_rmvy_th){
				debug_id = 1;
				pOutput->u1_RimCompflag = 0;
			}
		}
		else if(pOutput->u1_RimCompflag == 2){
			if(u13_abs_top_rmvy_sum > u13_rmvy_th){
				debug_id = 2;
				pOutput->u1_RimCompflag = 0;
			}
		}

		if(log_mv_en){//d8ac[14]
			rtd_pr_memc_emerg( "[COMP_WITH_MV] debug_id,%d rmvy_sum,%d,%d,\n", debug_id, u13_abs_top_rmvy_sum, u13_abs_bot_rmvy_sum);
		}
	}
	...
	if(log_en){ //d5ac[6]
		...
		rtd_pr_memc_err("result_H_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_Rim_Result[_RIM_RHT] , pOutput->u12_Rim_Result[_RIM_LFT]),pOutput->u12_Rim_Result[_RIM_RHT],pOutput->u12_Rim_Result[_RIM_LFT]);
		rtd_pr_memc_err("===== final check =========\n");		
		rtd_pr_memc_err(KERN_EMERG "final_comp_rim, id,%d,%d flag,%d,%d, ratio,%d,%d,%d, balace,%d\n", u8_info_id, debug_id, pOutput->u1_RimCompflag, Comp_flag_cnt, ratio_diff, TOP_ratio, BOT_ratio, pOutput->u1_RimTwoSide_balancing);
		rtd_pr_memc_err("===== value write for register =========\n");
		rtd_pr_memc_err("mc_rim1_V_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_mc_rim1[_RIM_BOT] , pOutput->u12_mc_rim1[_RIM_TOP]),pOutput->u12_mc_rim1[_RIM_BOT],pOutput->u12_mc_rim1[_RIM_TOP]);
		rtd_pr_memc_err("mc_rim1_H_size >> %d,%d,%d\n",_ABS_DIFF_(pOutput->u12_mc_rim1[_RIM_RHT] , pOutput->u12_mc_rim1[_RIM_LFT]),pOutput->u12_mc_rim1[_RIM_RHT],pOutput->u12_mc_rim1[_RIM_LFT]);
#endif
...
VOID RimCtrl_SC_rim_RTK2885p(_OUTPUT_RimCtrl *pOutput)
{
	unsigned char u8_k;
	...
	if(u8_SC_cnt>=2){
		...
		for (u8_k = 0; u8_k < _RIM_NUM; u8_k ++) {
			u12_Rim_HW_check[u8_k] = pOutput->u12_Rim_HW[u8_k];	
		}
	}
	...
	if(u8_check_rim_cnt>0){
		u8_check_rim_cnt++;
		for (u8_k = 0; u8_k < _RIM_NUM; u8_k ++) {
			u12_Rim_HW[u8_k] = pOutput->u12_Rim_HW[u8_k];		
		}
		...
		if(u8_check_rim_cnt<5){
			...
			u8_check_rim_cnt=0;
			RimCtrl_Boundary_Protect_RTK2885p(u12_Rim_HW, pOutput->u12_Rim_Result, pOutput);
...
VOID RimCtrl_Proc_RTK2885p(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	RimCtrl_OutResolution_Parse_RTK2885p(pOutput);
	//RimCtrl_Panning_Detect_RTK2885p(pParam, pOutput);
	if (pParam->u1_RimCtrl_en == 1)
	{
		...
		unsigned int u1_enable_HW_rim = 0;

		ReadRegister(HARDWARE_HARDWARE_44_reg, 31, 31, &u1_enable_HW_rim);/*0xB809D5B0*/	
		...
		u2_Panning_flg = s_pContext->_output_me_sceneAnalysis.u2_uniform_panning_direction;
		RimCtrl_DynamicBlackTh_RTK2885p();
		pOutput->u1_RimChange = 0;
		pOutput->u1_RimCompflag = 0;
		RimDet_by_KMC_YUV_Meter_RTK2885p(pParam, pOutput);
		
		RimCtrl_Get_HW_rim_RTK2885p(pOutput);

		//directly get hw rim to set boundary
		if(u1_enable_HW_rim){//d5b0[31]
			for (i = 0; i < _RIM_NUM; i ++)	{
				pOutput->u12_Rim_Result[i] = pOutput->u12_Rim_HW[i];
			}
		}else{
			RimCtrl_HW_extract_RTK2885p(pParam, pOutput);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Result[0], pOutput->u12_Rim_Result);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Target[0], pOutput->u12_Rim_Target);

			RimCtrl_Rim_Comp_RTK2885p(pParam, pOutput);//add from K5lp for boundary compensation! Terrence
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Result[1], pOutput->u12_Rim_Result);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Target[1], pOutput->u12_Rim_Target);

			RimCtrl_FastOut_RTK2885p(pParam, pOutput);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Result[2], pOutput->u12_Rim_Result);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Target[2], pOutput->u12_Rim_Target);

			RimCtrl_SlowIn_RTK2885p(pParam, pOutput);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Result[3], pOutput->u12_Rim_Result);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Target[3], pOutput->u12_Rim_Target);

			RimCtrl_SC_rim_RTK2885p(pOutput);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Result[4], pOutput->u12_Rim_Result);
			RimCtrl_Get_Rim_Info_RTK2885p(Rim_info_Target[4], pOutput->u12_Rim_Target);
		}

		RimCtrl_Output_RTK2885p(pParam, pOutput); // adjust features base on result rim
		Rim_check_protect_RTK2885p(pParam,pOutput); // protect function modify u1_RimChange which be used in RimCtrl_wrtAction
		//RimCtrl_Stable_RTK2885p(pParam, pOutput);

		if(log_Target_en[_RIM_TOP]){//d5ac[20]
			rtd_pr_memc_crit("[%s][TOP][Target]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_TOP], Rim_info_Target[0][_RIM_TOP],
		...
		if(log_Target_en[_RIM_BOT]){//d5ac[21]
			rtd_pr_memc_crit("[%s][BOT][Target]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_BOT], Rim_info_Target[0][_RIM_BOT],
		...
		if(log_Target_en[_RIM_LFT]){//d5ac[22]
			rtd_pr_memc_crit("[%s][LFT][Target]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_LFT], Rim_info_Target[0][_RIM_LFT],
		...
		if(log_Target_en[_RIM_RHT]){//d5ac[23]
			rtd_pr_memc_crit("[%s][RHT][Target]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_RHT], Rim_info_Target[0][_RIM_RHT],
		...
		if(log_Result_en[_RIM_TOP]){//d5ac[24]
			rtd_pr_memc_crit("[%s][TOP][Result]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_TOP], Rim_info_Result[0][_RIM_TOP],
		...
		if(log_Result_en[_RIM_BOT]){//d5ac[25]
			rtd_pr_memc_crit("[%s][BOT][Result]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_BOT], Rim_info_Result[0][_RIM_BOT],
		...
		if(log_Result_en[_RIM_LFT]){//d5ac[26]
			rtd_pr_memc_crit("[%s][LFT][Result]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_LFT], Rim_info_Result[0][_RIM_LFT],
		...
		if(log_Result_en[_RIM_RHT]){//d5ac[27]
			rtd_pr_memc_crit("[%s][RHT][Result]	HW,%d,%d,	Comp,%d,	FastOut,%d,	SlowIn,%d,	SC,%d,\n", __FUNCTION__, pOutput->u12_Rim_HW[_RIM_RHT], Rim_info_Result[0][_RIM_RHT],
	...
	else
	{
		...
		RimCtrl_Output_RTK2885p(pParam, pOutput);
	}
