...
extern unsigned int decide_vbe_disp_vtotal(unsigned int vbe_dvtotal_status, unsigned int vbe_input_framerate_source, unsigned int check_update, unsigned int vfreq_for_vtotal, unsigned char display);
...
VOID FRC_PhaseTable_Init_RTK2885p(_OUTPUT_FRC_PH_TABLE *pOutput)
{
...
VOID FRC_PhaseTable_Init_RTK2885pp(_OUTPUT_FRC_PH_TABLE *pOutput)
{
	unsigned int u32InINT_Clear = 0;

	rtd_pr_memc_notice( "FRC_PhaseTable_Init Called!![91b0,%x][9508,%x]\r\n", rtd_inl(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg), rtd_inl(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg));

	// reset wclr bit to sure interrupt can continue processing.
	ReadRegister(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg,20,23, &u32InINT_Clear);
	u32InINT_Clear = u32InINT_Clear & (~( 1 << (unsigned int)1)); // INT_TYPE_VSYNC
	WriteRegister(M8P_RTHW_IPPRE_TOP_IPPRE1_B0_reg,20,23, u32InINT_Clear);

	//rtd_pr_memc_notice("%s %d\n",__func__,__LINE__);	

	ReadRegister(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg,1,4, &u32InINT_Clear);
	u32InINT_Clear = u32InINT_Clear & (~( 1 << (unsigned int)1)); // INT_TYPE_VSYNC
	WriteRegister(M8P_RTHW_POST_TOP_KPOST_TOP_08_reg,1,4, u32InINT_Clear);
...
VOID FRC_PhaseTable_Init(_OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	if(RUN_MERLIN8P()){
		FRC_PhaseTable_Init_RTK2885pp(pOutput);
	}else if(RUN_MERLIN8()){
		FRC_PhaseTable_Init_RTK2885p(pOutput);
	}//RUN_MERLIN8P & RUN_MERLIN8
}

VOID FRC_phTable_Proc_RTK2885p(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
...
VOID FRC_phTable_Proc_RTK2885pp(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	unsigned int test_en = 0, test_en2 = 0, test_en3 = 0;
	test_en = (rtd_inl(SOFTWARE3_SOFTWARE3_09_reg)>>20)&0x1;
	test_en2 = (rtd_inl(SOFTWARE3_SOFTWARE3_09_reg)>>21)&0x1;
	test_en3 = (rtd_inl(SOFTWARE3_SOFTWARE3_09_reg)>>22)&0x1;
	if (pParam->u1_phT_autoGen_en == 1)
	{
		if(test_en){
			if(test_en2 == 0){
				...
				FRC_phTable_FPGA_Script(pParam, pOutput);
			}
		}
		FRC_phTable_Cadence_Change(pParam, pOutput);
		...
		if(test_en3 == 1){
		//	MEMC_Lib_debug_message_print_In_VS();//henry
		}
	}
}
...
{
	if(RUN_MERLIN8P()){
		FRC_phTable_Proc_RTK2885pp(pParam, pOutput);
	}else if(RUN_MERLIN8()){
		FRC_phTable_Proc_RTK2885p(pParam, pOutput);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
//////////////
	...
	u32_DBM_table[0][0]  =(u32_dejudder_0)&0x000000FF; 		//dejuddefr for tv006 control point 1;
	u32_DBM_table[0][1] = (u32_dejudder_1)&0x000000FF; 		//dejuddefr for tv006 control point 1
	u32_DBM_table[0][2] = (u32_dejudder_2)&0x000000FF; 		//dejuddefr for tv006 control point 2
	u32_DBM_table[0][3] = (u32_dejudder_3)&0x000000FF; 		//dejuddefr for tv006 control point 3
	u32_DBM_table[0][4] = (u32_dejudder_4)&0x000000FF; 		//dejuddefr for tv006 control point 4
	//u32_DBM_table[0][5] = u32_dejudder_Value_Max;
	u32_DBM_table[1][0]  =(u32_gmv_0)&0x0000FFFF;			//gmv for for tv006 control point 1
	u32_DBM_table[1][1] = (u32_gmv_1)&0x0000FFFF;			//gmv for for tv006 control point 1
	u32_DBM_table[1][2] = (u32_gmv_2)&0x0000FFFF;			//gmv for for tv006 control point 2
	u32_DBM_table[1][3] = (u32_gmv_3)&0x0000FFFF;			//gmv for for tv006 control point 3
	u32_DBM_table[1][4] = (u32_gmv_4)&0x0000FFFF;			//gmv for for tv006 control point 4
	//u32_DBM_table[1][5] = u32_GMV_Value_Max;

	u32_dejudder_0  =u32_DBM_table[0][0];		//dejuddefr for tv006 control point 1;
	u32_dejudder_1  =u32_DBM_table[0][1];		//dejuddefr for tv006 control point 1
	u32_dejudder_2  =u32_DBM_table[0][2];		//dejuddefr for tv006 control point 2
	u32_dejudder_3  =u32_DBM_table[0][3];		//dejuddefr for tv006 control point 3
	u32_dejudder_4  =u32_DBM_table[0][4];		//dejuddefr for tv006 control point 4
	//u32_DBM_table[0][5] = u32_dejudder_Value_Max;
	u32_gmv_0  =  u32_DBM_table[1][0];			//gmv for for tv006 control point 1
	u32_gmv_1  =  u32_DBM_table[1][1];			//gmv for for tv006 control point 1
	u32_gmv_2  =  u32_DBM_table[1][2];			//gmv for for tv006 control point 2
	u32_gmv_3  =  u32_DBM_table[1][3];			//gmv for for tv006 control point 3
	u32_gmv_4  =  u32_DBM_table[1][4];			//gmv for for tv006 control point 4
...
VOID FRC_phTable_set_LineModeCtrl(VOID)
{
	...
	if(log_en){
		#if (IC_K24)
		#else
		rtd_pr_memc_notice("[%s][u1_isLowDelayMode,%d,][u1_adaptivestream_flag,%d,][u1_callback_subscibe,%d,][u1_direct_media_flag,%d,][u1_vdec_direct_lowdelay_flag,%d,][MEMC_Lib_get_DisplayMode_Src,%d,][Vpq_LowDelay_to_MEMC,%d,]\n", 
			__FUNCTION__, u1_isLowDelayMode, u1_adaptivestream_flag, u1_callback_subscibe, u1_direct_media_flag, u1_vdec_direct_lowdelay_flag, MEMC_Lib_get_DisplayMode_Src(), MEMC_Lib_get_Vpq_LowDelay_to_MEMC_flag());
		#endif
	}
...
VOID FRC_phTable_set_NewLowDelay(unsigned char u1_enable)
{
	...
	if(u1_enable){
		if(status == 0){
			...
			if(RUN_MERLIN8P()){
				//CONFIG_NEED_TUNNING
				//python don't have pfv0 candidate
				//WriteRegister(RTME_DEHALO0_RTME_DEHALO0_10_reg, 0, 0, 0);//disable dehalo
			}else if(RUN_MERLIN8()){
		...
		if(status == 1){
			...
			if(RUN_MERLIN8P()){
				//CONFIG_NEED_TUNNING
				//WriteRegister(RTME_DEHALO0_RTME_DEHALO0_10_reg, 0, 0, 1);//enable dehalo
			}else if(RUN_MERLIN8()){
...
unsigned int FRC_phTable_QMS_FrmRateChgHoldCnt_Cal(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput, unsigned char N_pre, unsigned char M_pre, unsigned char N_cur, unsigned char M_cur)
{
	unsigned char ret = 0;
	if((N_pre == 1 && M_pre == 2) && (N_cur == 1 && M_cur == 5)){
		ret = 5;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_60_TO_24;
	}
	else if((N_pre == 1 && M_pre == 5) && (N_cur == 1 && M_cur == 2)){
		ret = 8;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_24_TO_60;
	}
	else if((N_pre == 1 && M_pre == 2) && (N_cur == 1 && M_cur == 4)){
		ret = 5;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_60_TO_30;
	}
	else if((N_pre == 1 && M_pre == 4) && (N_cur == 1 && M_cur == 2)){
		ret = 8;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_30_TO_60;
	}
	else if((N_pre == 1 && M_pre == 4) && (N_cur == 1 && M_cur == 5)){
		ret = 5;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_30_TO_24;
	}
	else if((N_pre == 1 && M_pre == 5) && (N_cur == 1 && M_cur == 4)){
		ret = 5;
		pOutput->QMS_FrmRateChg_case_ID = QMS_FRAME_RATE_CHG_CASE_24_TO_30;
	}
...
VOID FRC_phTable_StateMachine(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	unsigned char timing_status = 0;
	unsigned char timing_unstable_flag = 0; // (vbe_get_disp_timing_change_status() == pOutput->u1_timing_status_pre) ? 0 : 1;
	...
	unsigned char get_multiview_1p_input_in_2p_panel = 0, multiview_1p_input_in_2p_panel_chg = 0;
	...
	frc_vtrig_dly = HAL_CRTC_GetFRCVtrigDelay();
	...
	if(multiview_1p_input_in_2p_panel() == TRUE /*&& multiview_1p_input_decide_D1_timing(&temp_uzd_hactive, &temp_expand_uzudtg_htotal, &temp_shrink_uzudtg_vtotal) == TRUE
		&& jude_input_timing_is_hfr(SLR_MAIN_DISPLAY) == TRUE*/){
		get_multiview_1p_input_in_2p_panel = 1;
	}else{
		get_multiview_1p_input_in_2p_panel = 0;
	}

	if(get_multiview_1p_input_in_2p_panel != pOutput->multiview_1p_input_in_2p_panel_pre){
		multiview_1p_input_in_2p_panel_chg = 1;
	}
	else{
		multiview_1p_input_in_2p_panel_chg = 0;
	}

	if((get_product_type() == PRODUCT_TYPE_DIAS && (Scaler_MEMC_Get_SW_OPT_Flag() == 1)) && qms_flag){
		timing_status = 0; // 0:unstable, 1:stable
	}
	else{
		timing_status = vbe_get_disp_timing_change_status();
	}
	if(timing_status == pOutput->u1_timing_status_pre){
		timing_unstable_flag = 0;
	}
	else{
		timing_unstable_flag = 1;
	}
	...
	// synthesize to generate change signal, and merged into phase table control signal
	{
		...
		ReadRegister(SOFTWARE3_SOFTWARE3_09_reg, 28, 28, &u32_rb_val);
		u1_forcePhTable_Regen = u32_rb_val;
		...
		if(qms_chg == 1){
			...
			rtd_pr_memc_emerg("[%s][OutMode,%d,][QMS_flag_chg : %d -> %d]\n\r", __FUNCTION__, u1_outMode, pOutput->u1_qms_flag_pre, qms_flag);
		}
		
		if(u1_n2m_switch && qms_flag && u1_outMode == _PQL_OUT_VIDEO_MODE)
		{
			pOutput->QMS_FrmRateChg_hold_cnt = FRC_phTable_QMS_FrmRateChgHoldCnt_Cal(pParam, pOutput, pOutput->u8_sys_N_pre, pOutput->u8_sys_M_pre, u8_sys_N, u8_sys_M);
			pOutput->QMS_FrmRateChg_hold_cnt_max = pOutput->QMS_FrmRateChg_hold_cnt;
		}
		else if(in_fmRate == _PQL_IN_24HZ && out_fmRate == _PQL_OUT_120HZ && u8_sys_N == 1 && u8_sys_M == 5 && QMS_24hz_en && qms_flag && pOutput->QMS_FrmRateChg_hold_cnt == 0){
		...
		if((u1_outMode_chg == 1) || (u1_n2m_switch == 1) || (u1_in3dFormat_chg == 1) || (u1_out3dFormat_chg == 1)
			...
			|| (u1_vdec_direct_lowdelay_flag_chg) || (u1_direct_media_flag_chg == 1) || (N2M_memcdtg_dv_total_check==1 && u1_n2m_switch==0) || (timing_unstable_flag == 1)
			|| (QMS_24hz_state_chg) || (qms_chg) || (pOutput->QMS_FrmRateChg_hold_cnt>0) || (multiview_1p_input_in_2p_panel_chg)
			/*|| ((s_pContext->_external_info.u1_mMiraCast == 1)&&(u1_MEMCMode_chg))*//*backup for miracast*/)
		{
			pOutput->u1_generate_video_en = 1;
			if(N2M_memcdtg_dv_total_check==1){
				N2M_memcdtg_dv_total_check = 0;
			}
			if(DC_off_on_flg == 1){
				DC_off_on_flg = 0;
			}
			if(pOutput->QMS_FrmRateChg_hold_cnt > 0){
				pOutput->QMS_FrmRateChg_hold_cnt--;
			}
			rtd_pr_memc_emerg("[u1_generate_video_en](%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)\n\r",
				...
				timing_unstable_flag, QMS_24hz_state_chg, qms_chg, pOutput->QMS_FrmRateChg_hold_cnt,
				multiview_1p_input_in_2p_panel_chg);

			if(u1_in3dFormat_chg){
				if(u32_log_flag==1)
				{
					rtd_pr_memc_emerg("[in3dFormat_chg](pre_in, cur_in) = (%d, %d)\n\r", pOutput->in3d_format_pre, in_3dFormat);
				}
                      }
			if(u1_out3dFormat_chg){
				if(u32_log_flag==1)
				{
					rtd_pr_memc_emerg("[out3dFormat_chg](pre_out, cur_out) = (%d, %d)\n\r", pOutput->out3d_format_pre, out_3dFormat);
				}
			}
		}
		else{
		...
		if((u1_outMode_chg == 1) || (u1_n2m_switch == 1) || (u1_in3dFormat_chg == 1) || (u1_out3dFormat_chg == 1)
			...
			|| (u1_lowdelay_state_chg_flg) || (u1_direct_media_flag_chg == 1) || (NewLowDelay_chg == 1) || (u1_UI_GameMode_chg == 1)
			|| (u1_vdec_direct_lowdelay_flag_chg) || (u1_direct_media_flag_chg == 1) || (N2M_memcdtg_dv_total_check==1 && u1_n2m_switch==0) || (timing_unstable_flag == 1)
			|| (QMS_24hz_state_chg) || (qms_chg) || (pOutput->QMS_FrmRateChg_hold_cnt>0)
			/*|| ((s_pContext->_external_info.u1_mMiraCast == 1)&&(u1_MEMCMode_chg))*//*backup for miracast*/)
			{
			pOutput->u1_generate_video_en = 1;
			N2M_memcdtg_dv_total_check = 0;
		...
		// synthesize
		pOutput->u1_generate_PC_en    =  (u1_outMode_chg || u1_n2m_switch || pOutput->u8_system_start_lock == 0 || (u1_LowDelayMode_chg == 1) || (u1_UI_GameMode_chg == 1) || u1_PCMode_chg || (timing_unstable_flag == 1) || (multiview_1p_input_in_2p_panel_chg))? 1 : 0;
		...
		if (pParam->u1_force_gen_table_en == 1 || (RUN_MERLIN8P() && u1_forcePhTable_Regen == 1))
		{
			if(u32_log_flag==1)
			{
			...
			WriteRegister(SOFTWARE3_SOFTWARE3_09_reg, 28, 28, 0);
		}
		if(RUN_MERLIN8P() && pOutput->u1_generate_video_en == 1){
			if(log_en){
			rtd_pr_memc_emerg("[u1_generate_video_en](%d)(%d)(%d)(%d),(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d)\n\r",
				pOutput->u1_lowdelay_regen, u1_outMode_chg, u1_n2m_switch, u1_in3dFormat_chg,
				u1_out3dFormat_chg, pOutput->u8_system_start_lock, u1_LowDelayMode_chg,
				u8_LowDelay_cnt, in_fmRate, out_fmRate, u1_isLowDelayMode,
				LowDelayGen_cnt, u1_source_type_chg, VR360_en_chg, u1_adaptivestream_flag_chg,
				u1_lowdelay_state_chg_flg, u1_direct_media_flag_chg, u1_UI_GameMode_chg, u1_vdec_direct_lowdelay_flag_chg);
			}
		...
		if(RUN_MERLIN8P() && pOutput->u1_generate_film_en == 1)
		{
			if(log_en){
			rtd_pr_memc_emerg("$$$   u1_generate_film_en = (%d)(%d)(%d)(%d),(%d)(%d)(%d)(%d)   $$$\n\r",
				u1_outMode_chg, u1_n2m_switch, u1_in3dFormat_chg, u1_out3dFormat_chg,
	...
	// update states
	{
		...
		pOutput->u1_outframe_pre = out_fmRate;
		...
		pOutput->u1_timing_status_pre = timing_status; // vbe_get_disp_timing_change_status();
		pOutput->QMS_24hz_state_pre = QMS_24hz_state;
		pOutput->u1_qms_flag_pre = qms_flag;
		pOutput->multiview_1p_input_in_2p_panel_pre = get_multiview_1p_input_in_2p_panel;
	}
...
{
	const _PQLCONTEXT     *s_pContext = GetPQLContext();
	Ph_decGen_Param  phT_decGen_param = {0};
	static bool bflag=0;
	...
	if(pOutput->u1_generate_video_en == 1 || pOutput->u1_generate_film_en == 1)
	{
		...
		rtd_pr_memc_notice("memcdtgVTotal = [,%d,]\r\n", decide_vbe_disp_vtotal(VBE_DVTOTAL_BY_VFREQ,0,0,Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ),0));
		VTotal = decide_vbe_disp_vtotal(VBE_DVTOTAL_BY_VFREQ,0,0,Scaler_DispGetInputInfo(SLR_INPUT_DTG_MASTER_V_FREQ),0);
		...
		if(bflag==0){
			phT_decGen_param.u8_ph_sys_dly = 0;
			bflag=1;
		}
...
VOID FRC_phTable_Write(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	unsigned int log_en = 0;
	M8P_rthw_phase_kphase_0c_RBUS M8P_rthw_phase_kphase_0c_rbus;
	M8P_rthw_phase_kphase_10_RBUS M8P_rthw_phase_kphase_10_reg;
	unsigned char bsize = 0;
	static unsigned char bsize_pre = 0;

	ReadRegister(SOFTWARE3_SOFTWARE3_63_reg, 3, 3, &log_en);
	M8P_rthw_phase_kphase_0c_rbus.regValue = rtd_inl(M8P_RTHW_PHASE_kphase_0C_reg);
	...
	if(RUN_MERLIN8P() && M8P_rthw_phase_kphase_0c_rbus.kphase_sw_film_en == 1){
		if(M8P_rthw_phase_kphase_0c_rbus.kphase_auto_film_phase_en == 1){
			M8P_rthw_phase_kphase_0c_rbus.kphase_auto_film_phase_en = 0;
		}
	}
	...
	HAL_MC_SetKphaseSWFilmPhase(u8_filmPhase);

	//if(log_en){
	if(0){
		rtd_pr_memc_emerg("[%s][u8_filmPhase=%d][cadence_id_pre=%d][outMode,%d,]\n", __FUNCTION__, 
			u8_filmPhase, pOutput->u8_cadence_id_pre,
			pOutput->u1_outMode_pre);
	}
	...
	if(0)
	{
		...
		if(pOutput->u1_after_img == 1)
		{
			...
			u8_filmPhase_pre = HAL_MC_GetKphaseSWFilmPhase();
			u8_outPhase = HAL_MC_GetKphaseOutPhase();
			...
			if(after_img == 1)
			{
				...
				HAL_MC_SetKphaseSWFilmPhase(u8_filmPhase_det);
			}
			else
			{
				filmPhase_result = (u8_filmPhase_pre+ 1)% frc_cadTable[u8_cadence_id_pre].cad_outLen;
				HAL_MC_SetKphaseSWFilmPhase(filmPhase_result);
			}
			if(0)
			{
				...
				u32_wrt_film_mode = HAL_MC_GetKphaseFilmMode();
				cad_outLen = HAL_MC_GetKphaseFilmPhaseType();
				u8_filmPhase = HAL_MC_GetKphaseSWFilmPhase();
				u8_outPhase = HAL_MC_GetKphaseOutPhase();
	...
	if(RUN_MERLIN8P() && bsize != bsize_pre)
	{
		FRC_phTable_Init_1n2m_video_22_32_intable();
		bsize_pre = bsize;
	}
...
VOID   FRC_phTable_inDec_Wrt_RTK2885p(void)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID FRC_phTable_inDec_Wrt_RTK2885pp(void)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned char u8_cadence_id 		  = s_pContext->_output_frc_phtable.u8_cadence_id_pre;
	unsigned char u8_k = 0;
	...
	for (u8_k = Len_vid_22_32; u8_k < Len_vid_22_32_cadence; u8_k = u8_k + 4)
	{
		unsigned char pack_stIdx = u8_k - Len_vid_22_32;
		unsigned int u32_phT_wrtStep_pack = phTable_WrtStep_pack(pack_stIdx);
		rtd_outl(M8P_RTHW_PHASE_kphase_14_reg + u8_k, u32_phT_wrtStep_pack); // WriteRegister(KPHASE_kphase_14_reg + u8_k, 0, 31, u32_phT_wrtStep_pack);
	}
	for (u8_k = 0; u8_k < frc_cadTable[u8_cadence_id].cad_outLen; u8_k = u8_k + 8)
	{
		unsigned int u32_phT_wrtStep_pack = phTable_me2_WrtStep_pack(u8_k);
		rtd_outl(M8P_RTHW_PHASE_kphase_58_reg + me2_wrt_reg_stId, u32_phT_wrtStep_pack); // WriteRegister(KPHASE_kphase_58_reg + me2_wrt_reg_stId, 0, 31, u32_phT_wrtStep_pack);
		me2_wrt_reg_stId = me2_wrt_reg_stId + 4;
	}
	for (u8_k = 0; u8_k < 2; u8_k++)
	{
		unsigned int u32_phT_wrtStep_pack = phTable_mc_WrtStep_msb_pack(u8_k);
		rtd_outl(M8P_RTHW_PHASE2_kphase2_34_reg + u8_k*4, u32_phT_wrtStep_pack);
		//if(log_en){
		if(0){
		rtd_pr_memc_emerg("[%s][%d][addr,%x,][val,%x,]", __FUNCTION__, __LINE__, M8P_RTHW_PHASE2_kphase2_34_reg + u8_k*4, u32_phT_wrtStep_pack);
		}
	}
	for (u8_k = 0; u8_k < 2; u8_k++)
	{
		unsigned int u32_phT_wrtStep_pack = phTable_me1_WrtStep_msb_pack(u8_k);
		rtd_outl(M8P_RTHW_PHASE2_kphase2_3C_reg + u8_k*4, u32_phT_wrtStep_pack);
		//if(log_en){
		if(0){
		rtd_pr_memc_emerg("[%s][%d][addr,%x,][val,%x,]", __FUNCTION__, __LINE__, M8P_RTHW_PHASE2_kphase2_3C_reg + u8_k*4, u32_phT_wrtStep_pack);
		}
	}
}
...
{
	if(RUN_MERLIN8P()){
		FRC_phTable_inDec_Wrt_RTK2885pp();
	}else if(RUN_MERLIN8()){
		FRC_phTable_inDec_Wrt_RTK2885p();
	}//RUN_MERLIN8P & RUN_MERLIN8
}

VOID FRC_phTable_outDec_Wrt_RTK2885p(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	...
	{
		unsigned int u32_Tmp = HAL_MC_GetKphaseLutSel();
		u32_Tmp = ((u32_Tmp&0x01) == 1)? 0 : 1;
		HAL_MC_SetKphaseLutSel(u32_Tmp);
	}

}

VOID FRC_phTable_outDec_Wrt_RTK2885pp(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned short phT_stIdx;
	unsigned short phT_endIdx_p1		  = s_pContext->_output_frc_phtable.u16_phTable_wrt_endIdx_p1;
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_63_reg, 3, 3, &log_en);
	ReadRegister(SOFTWARE3_SOFTWARE3_63_reg, 7, 7, &test_en);
	//unsigned int ee_addr_base = 0x00000000;//0xC0000000
	//unsigned int ee_addr_base = KMC_TOP_kmc_top_00_reg;//0xC0000000
	...
	// 1st table
	FRC_kphase_sram_clear(clear_st, 50);

	rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000); // WriteRegister(ee_addr_base + 0x4F00, 0, 31, 0);	   //Prepare write EEPROM
	rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | phT_stIdx); // WriteRegister(ee_addr_base + 0x4F00, 0, 31, 0x80000000 | phT_stIdx);
	WriteRegister(M8P_RTHW_PHASE_TO_SRAM_phase_to_sram_F8_reg, 4, 4, 1);
	//LogPrintf(DBG_MSG, "%s::phT_stIdx=%d,phT_endIdx_pl=%d \n",__FUNCTION__,phT_stIdx,phT_endIdx_p1);
	if(log_en){
		rtd_pr_memc_emerg("[%s][%d][lut,%x,]", __FUNCTION__, __LINE__, rtd_inl(M8P_RTHW_PHASE_kphase_10_reg));
	}
	for (u16_k = phT_stIdx; u16_k < phT_endIdx_p1; u16_k ++)
	{
		...
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_04_reg, pack_LSB); // WriteRegister(ee_addr_base + 0x4F04, 0, 31, pack_LSB);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_0C_reg, pack_extra);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_FC_reg, pack_extra2);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_08_reg, pack_MSB); // WriteRegister(ee_addr_base + 0x4F08, 0, 31, pack_MSB);
		if(log_en){
		rtd_pr_memc_emerg("[%s][%d][%d][pack_LSB,%x,][pack_extra,%x,][pack_extra2,%x,][pack_MSB,%x,]", __FUNCTION__, __LINE__, u16_k, pack_LSB, pack_extra, pack_extra2, pack_MSB);
		}
	}
	if(log_en){
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | phT_stIdx);
		for (u16_k = phT_stIdx; u16_k < phT_endIdx_p1; u16_k ++)
		{
			rtd_pr_memc_emerg("[%s][%d][%d][pack_LSB,%x,][pack_extra,%x,][pack_extra2,%x,][pack_MSB,%x,]", __FUNCTION__, __LINE__, u16_k,
				rtd_inl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_04_reg), 
				rtd_inl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_0C_reg), 
				rtd_inl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_FC_reg), 
				rtd_inl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_08_reg)
				);
		}
	}
	...
	if(log_en){
	rtd_pr_memc_emerg("[%s][%d][phT_stIdx,%d,][phT_endIdx_p1,%d,][phT_endIdx_p1_new,%d,]", __FUNCTION__, __LINE__, phT_stIdx, phT_endIdx_p1, phT_endIdx_p1_new);
	}
#if 1//CONFIG_PYTHON_FPGA
#else
	...
	// change lut selection.
	{
		...
		M8P_rthw_phase_kphase_10_RBUS M8P_rthw_phase_kphase_10_rbus;
		M8P_rthw_phase_kphase_10_rbus.regValue = rtd_inl(M8P_RTHW_PHASE_kphase_10_reg);
		
		u32_Tmp = M8P_rthw_phase_kphase_10_rbus.kphase_lut_sel; // ReadRegister(KPHASE_kphase_10_reg,16,16, &u32_Tmp);
		u32_Tmp = ((u32_Tmp&0x01) == 1)? 0 : 1;
		M8P_rthw_phase_kphase_10_rbus.kphase_lut_sel = u32_Tmp;
		rtd_outl(M8P_RTHW_PHASE_kphase_10_reg, M8P_rthw_phase_kphase_10_rbus.regValue); // WriteRegister(KPHASE_kphase_10_reg,16,16, u32_Tmp);
	}
	if(log_en){
		rtd_pr_memc_emerg("[%s][%d][lut,%x,]", __FUNCTION__, __LINE__, rtd_inl(M8P_RTHW_PHASE_kphase_10_reg));
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000);
...
{
	if(RUN_MERLIN8P()){
		FRC_phTable_outDec_Wrt_RTK2885pp(pParam, pOutput);
	}else if(RUN_MERLIN8()){
		FRC_phTable_outDec_Wrt_RTK2885p(pParam, pOutput);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
VOID   FRC_phTable_Kphase_ctrl_Wrt(const _PARAM_FRC_PH_TABLE *pParam)
{
	...
	if (u1_outMode == _PQL_OUT_PC_MODE)
	{
		HAL_MC_SetKphaseSWFilmEnable(1);
		HAL_MC_SetKphaseSWFilmPhase(0);
		HAL_MC_SetKphaseFilmMode(0);
		HAL_MC_SetKphaseFilmPhaseType(1);
	}
	else
	{
		if(RUN_MERLIN8P() || pParam->u1_filmPh_sw_en == 1)//confirm with jerry
		{
			HAL_MC_SetKphaseSWFilmEnable(1);
			HAL_MC_SetKphaseFilmMode(u32_wrt_film_mode);
			HAL_MC_SetKphaseFilmPhaseType(frc_cadTable[u8_cadence_id].cad_outLen);
		}
		else
		{
			HAL_MC_SetKphaseSWFilmEnable(0);
			HAL_MC_SetKphaseFilmMode(0);
			HAL_MC_SetKphaseFilmPhaseType(frc_cadTable[u8_cadence_id].cad_outLen);
		}
	}
...
VOID   FRC_phTable_allInfo_Pack_Gen(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput, unsigned short phDecInfo_k, Ph_Dec_Pack_Info *pPackInfo)  // bai  need  change
{
	...
	unsigned char u1_ppfv_oft = 0;
	//unsigned char org_out_3d_lr = 0;
	unsigned int log_en = 0;
	unsigned short phTable_DecInfoGen_Size = (unsigned short)(sizeof(phTable_DecInfoGen) / sizeof(Ph_Dec_Info));

	ReadRegister(SOFTWARE3_SOFTWARE3_63_reg, 3, 3, &log_en);

	if(phDecInfo_k >= phTable_DecInfoGen_Size){
	...
	// firstPhase, lastPhase judge
	{
		...
		if(phDecInfo_k == u16_phT_stIdx){
			u16_phDecInfo_k_n1 = u16_phT_endIdx;
		}else if(phDecInfo_k==0){
			u16_phDecInfo_k_n1 = 0;
		}else{
			u16_phDecInfo_k_n1 = phDecInfo_k - 1;
		}
		if(phDecInfo_k == u16_phT_endIdx){
			u16_phDecInfo_k_p1 = u16_phT_stIdx;
		}else if(phDecInfo_k>=255){
			rtd_pr_memc_notice( "[%s]Invalid u16_phDecInfo_k_p1=%d.Resume!!\n",__FUNCTION__,phDecInfo_k + 1);
			u16_phDecInfo_k_p1 = 0;
		}else{
			u16_phDecInfo_k_p1 = phDecInfo_k + 1;
		}

		if(u16_phDecInfo_k_n1 >= phTable_DecInfoGen_Size){
			rtd_pr_memc_notice( "[%s]Invalid u16_phDecInfo_k_n1=%d.Resume!\n",__FUNCTION__,u16_phDecInfo_k_n1);
			u16_phDecInfo_k_n1 = 0;
		}
		if(u16_phDecInfo_k_p1 >= phTable_DecInfoGen_Size){
			rtd_pr_memc_notice( "[%s]Invalid u16_phDecInfo_k_p1=%d.Resume!\n",__FUNCTION__,u16_phDecInfo_k_p1);
			u16_phDecInfo_k_p1 = 0;
		}
                #if DEC_INFO_WRT_STIDX
		u16_phDecInfo_k_n1 = _CLIP_(u16_phDecInfo_k_n1, 0, 255);
                #endif
		if(RUN_MERLIN8P()){
			u1_is_firstPhase = (phTable_DecInfoGen[phDecInfo_k].me2_phase_long < phTable_DecInfoGen[u16_phDecInfo_k_n1].me2_phase_long) ? 1 : 0;
			u1_ppfv_oft = (phTable_DecInfoGen[u16_phDecInfo_k_p1].me2_phase_long <= phTable_DecInfoGen[phDecInfo_k].me2_phase_long) ? 0 : 1;
	
			//if(log_en){
			if(0){
			rtd_pr_memc_emerg("[%s][%d][k,%d,%d,][k_n1,%d,%d,][k_p1,%d,%d,][u1_is_firstPhase,%d,][u1_ppfv_oft,%d,]", __FUNCTION__, __LINE__,
				phDecInfo_k, phTable_DecInfoGen[phDecInfo_k].me2_phase_long, 
				u16_phDecInfo_k_n1, phTable_DecInfoGen[u16_phDecInfo_k_n1].me2_phase_long, 
				u16_phDecInfo_k_p1, phTable_DecInfoGen[u16_phDecInfo_k_p1].me2_phase_long, 
				u1_is_firstPhase, u1_ppfv_oft);
			}
		}else if(RUN_MERLIN8()){
	...
	if(RUN_MERLIN8P()){
		pPackInfo->u8_msb[MSB_ME2_PPFV_BUF_IDX]        = u1_ppfv_oft; // phTable_DecInfoGen[phDecInfo_k].ppfv_offset;
		...
		pPackInfo->u8_msb[MSB_ME2_PHASE]               = 0; // phTable_DecInfoGen[phDecInfo_k].me2_phase;
	}else if(RUN_MERLIN8()){
...
//////////////////////////////////////////////////////////////////////////
VOID FRC_kphase_sram_clear_RTK2885p(unsigned short stIdx, unsigned short endIdx)
{
...
VOID FRC_kphase_sram_clear_RTK2885pp(unsigned short stIdx, unsigned short endIdx)
{
	unsigned short u16_k;
	//unsigned int ee_addr_base = 0x00000000;//0xC0000000
	//unsigned int ee_addr_base = KMC_TOP_kmc_top_00_reg;
	...
	rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000); // WriteRegister(ee_addr_base + 0x4F00, 0, 31, 0);	   //Prepare write EEPROM
	rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | stIdx); // WriteRegister(ee_addr_base + 0x4F00, 0, 31, 0x80000000 | stIdx);
	for (u16_k = stIdx; u16_k <= endIdx; u16_k ++)
	{
		unsigned int pack_MSB = 0, pack_LSB = 0, pack_EXTRA = 0;

		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_04_reg, pack_LSB); // WriteRegister(ee_addr_base + 0x4F04, 0, 31, pack_LSB);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_0C_reg, pack_EXTRA); // WriteRegister(ee_addr_base + 0x4F04, 0, 31, pack_LSB);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_08_reg, pack_MSB); // WriteRegister(ee_addr_base + 0x4F08, 0, 31, pack_MSB);
	}
}
...
{
	if(RUN_MERLIN8P()){
		FRC_kphase_sram_clear_RTK2885pp(stIdx, endIdx);
	}else if(RUN_MERLIN8()){
		FRC_kphase_sram_clear_RTK2885p(stIdx, endIdx);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
VOID FRC_phTable_inDec_Wrt_init_RTK2885p(void)  // bai need change  need know 64 table
{
	//const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned char u8_k = 0;
	unsigned char Len_vid_22_32;
...
VOID FRC_phTable_inDec_Wrt_init_RTK2885pp(void)  // bai need change  need know 64 table
{
	//const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned char u8_k = 0;
	unsigned char Len_vid_22_32;
	...
	{
		unsigned int u32_phT_wrtStep_pack = phTable_me2_WrtStep_pack_vd_22_32(u8_k);
		rtd_outl(M8P_RTHW_PHASE_kphase_58_reg + u8_k, u32_phT_wrtStep_pack); // WriteRegister(KPHASE_kphase_58_reg + u8_k, 0, 31, u32_phT_wrtStep_pack);
	}
	for (u8_k = 0; u8_k < 32; u8_k ++)
	{
		if(u8_k < Len_vid_22_32){
			u8_rb_one  = (((phTable_WrtInfoGen_vd_22_32[u8_k].mc_in_idx)&0x8) >> 3);
		}
		else{
			u8_rb_one  = 0;
		}
	...
	rtd_outl(M8P_RTHW_PHASE2_kphase2_34_reg, u32_ret);
	//rtd_pr_memc_emerg("[%s][%d][addr,%x,][val,%x,]", __FUNCTION__, __LINE__, RTHW_PHASE2_kphase2_34_reg, u32_ret);
	...
	{
		if(u8_k < Len_vid_22_32){
			u8_rb_one  = (((phTable_WrtInfoGen_vd_22_32[u8_k].me1_in_idx)&0x8) >> 3);
		}
		else{
			u8_rb_one  = 0;
		}
	...
	rtd_outl(M8P_RTHW_PHASE2_kphase2_3C_reg, u32_ret);
	//rtd_pr_memc_emerg("[%s][%d][addr,%x,][val,%x,]", __FUNCTION__, __LINE__, RTHW_PHASE2_kphase2_3C_reg, u32_ret);

}


VOID FRC_phTable_inDec_Wrt_init(void)  // bai need change  need know 64 table
{
	if(RUN_MERLIN8P()){
		FRC_phTable_inDec_Wrt_init_RTK2885pp();
	}else if(RUN_MERLIN8()){
		FRC_phTable_inDec_Wrt_init_RTK2885p();
	}//RUN_MERLIN8P & RUN_MERLIN8
}

int g_memc_VR360_state = 0;
extern unsigned char g_cadence_change_flag;
VOID FRC_phTable_Cadence_Change_RTK2885p(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
 	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID FRC_phTable_Cadence_Change_RTK2885pp(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned int adapt_stream_flag = 0;
	unsigned int cur_cadence = 0;
	...
	unsigned int me2_phase = 0, mc_phase = 0;
	static unsigned int cadence_pre = 0;
	unsigned int log_en = 0, log_en2 = 0, gmv_rb = 0;
	PQL_OUTPUT_FRAME_RATE  out_fmRate = 0;
	PQL_INPUT_FRAME_RATE	in_fmRate = 0;
	unsigned int in_phase = 0, out_phase = 0;
	unsigned int in_film_phase = 0, out_film_phase = 0;
	unsigned int me1_last = 0, u8_k = 0;
	static unsigned char cadence_out[300] = {0};
	static unsigned char phase_id_out[300] = {0};
	static unsigned char mc_w_idx[300] = {0};
	static unsigned char mc_i_idx[300] = {0};
	static unsigned char mc_p_idx[300] = {0};
	static unsigned char mc_phase_array[300] = {0};
	static unsigned int motion_pool[300] = {0};
	...
	ReadRegister(M8P_SOFTWARE3_SOFTWARE3_63_reg, 1, 1, &log_en);
	ReadRegister(M8P_SOFTWARE3_SOFTWARE3_63_reg, 2, 2, &log_en2);
	gmv_rb = 0; // need fix it (signed short)s_pContext->_output_read_comreg.s11_me_GMV_1st_vx_rb;
	in_fmRate = s_pContext->_external_data._input_frameRate;
	out_fmRate = s_pContext->_external_data._output_frameRate;

	cadence_pre = cur_cadence;

	for(u8_k = 23; u8_k > 0; u8_k--){
		cadence_out[u8_k] = cadence_out[u8_k-1];
		phase_id_out[u8_k] = phase_id_out[u8_k-1];
		mc_w_idx[u8_k] = mc_w_idx[u8_k-1];
		mc_i_idx[u8_k] = mc_i_idx[u8_k-1];
		mc_p_idx[u8_k] = mc_p_idx[u8_k-1];
		mc_phase_array[u8_k] = mc_phase_array[u8_k-1];
		motion_pool[u8_k] = motion_pool[u8_k-1];
	}
	cadence_out[0] = s_pContext->_output_filmDetectctrl.u8_cur_cadence_out[_FILM_ALL];
	phase_id_out[0] = s_pContext->_output_filmDetectctrl.u8_phT_phase_Idx_out[_FILM_ALL];
	mc_w_idx[0] = mc_write;
	mc_i_idx[0] = mc_read_i;
	mc_p_idx[0] = mc_read_p;
	mc_phase_array[0] = mc_phase;
	motion_pool[0] = rtd_inl(M8P_RTME_IPHME_TOP0_IPHME_TOP_04_reg);

#if 1
	if(log_en){
		rtd_pr_memc_emerg("[phasetable][now_dma][,%d,%d,][,%d,%d,][ME1_idx,%d,%d,%d,,%d,][ME15,%d,%d,][ME2_idx,%d,%d,,%d,][MC_idx,%d,%d,%d,,%d,][Motion,%d,][Time,%d,]\n\r",
	...
	if(log_en2){
		rtd_pr_memc_emerg("[phasetable][in interrupt][,%d,%d,][,%d,][ME1_idx,%d,%d,%d,ME15,%d,%d,ME2_idx,%d,%d,][MC_idx,%d,%d,%d,][phase,%d,][Motion,%d,][Seq,%x,][FrameRate,%d,%d,][Time,%d,]\n\r",
			...
			mc_write, mc_read_i, mc_read_p, mc_phase,
			s_pContext->_output_read_comreg.u27_ipme_aMot_rb, 
			s_pContext->_output_filmDetectctrl.u32_Mot_sequence[_FILM_ALL], 
			in_fmRate, out_fmRate, rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[motion_pool][,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][0], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][1], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][2], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][3],
			s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][4], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][5], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][6], s_pContext->_output_filmDetectctrl.u27_ipme_motionPool[_FILM_ALL][7],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[motion_pool_register][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			motion_pool[0], motion_pool[1], motion_pool[2], motion_pool[3], motion_pool[4],
			motion_pool[5], motion_pool[6], motion_pool[7], motion_pool[8], motion_pool[9],
			motion_pool[10], motion_pool[11], motion_pool[12], motion_pool[13], motion_pool[14],
			motion_pool[15], motion_pool[16], motion_pool[17], motion_pool[18], motion_pool[19],
			motion_pool[20], motion_pool[21], motion_pool[22], motion_pool[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[cadence_out][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			cadence_out[0], cadence_out[1], cadence_out[2], cadence_out[3], cadence_out[4], 
			cadence_out[5], cadence_out[6], cadence_out[7], cadence_out[8], cadence_out[9], 
			cadence_out[10], cadence_out[11], cadence_out[12], cadence_out[13], cadence_out[14],
			cadence_out[15], cadence_out[16], cadence_out[17], cadence_out[18], cadence_out[19],
			cadence_out[20], cadence_out[21], cadence_out[22], cadence_out[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[phase_id_out][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			phase_id_out[0], phase_id_out[1], phase_id_out[2], phase_id_out[3], phase_id_out[4], 
			phase_id_out[5], phase_id_out[6], phase_id_out[7], phase_id_out[8], phase_id_out[9], 
			phase_id_out[10], phase_id_out[11], phase_id_out[12], phase_id_out[13], phase_id_out[14],
			phase_id_out[15], phase_id_out[16], phase_id_out[17], phase_id_out[18], phase_id_out[19],
			phase_id_out[20], phase_id_out[21], phase_id_out[22], phase_id_out[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[mc_w_idx][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			mc_w_idx[0], mc_w_idx[1], mc_w_idx[2], mc_w_idx[3], mc_w_idx[4], 
			mc_w_idx[5], mc_w_idx[6], mc_w_idx[7], mc_w_idx[8], mc_w_idx[9], 
			mc_w_idx[10], mc_w_idx[11], mc_w_idx[12], mc_w_idx[13], mc_w_idx[14],
			mc_w_idx[15], mc_w_idx[16], mc_w_idx[17], mc_w_idx[18], mc_w_idx[19],
			mc_w_idx[20], mc_w_idx[21], mc_w_idx[22], mc_w_idx[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[mc_i_idx][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			mc_i_idx[0], mc_i_idx[1], mc_i_idx[2], mc_i_idx[3], mc_i_idx[4], 
			mc_i_idx[5], mc_i_idx[6], mc_i_idx[7], mc_i_idx[8], mc_i_idx[9], 
			mc_i_idx[10], mc_i_idx[11], mc_i_idx[12], mc_i_idx[13], mc_i_idx[14],
			mc_i_idx[15], mc_i_idx[16], mc_i_idx[17], mc_i_idx[18], mc_i_idx[19],
			mc_i_idx[20], mc_i_idx[21], mc_i_idx[22], mc_i_idx[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[mc_p_idx][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			mc_p_idx[0], mc_p_idx[1], mc_p_idx[2], mc_p_idx[3], mc_p_idx[4], 
			mc_p_idx[5], mc_p_idx[6], mc_p_idx[7], mc_p_idx[8], mc_p_idx[9], 
			mc_p_idx[10], mc_p_idx[11], mc_p_idx[12], mc_p_idx[13], mc_p_idx[14],
			mc_p_idx[15], mc_p_idx[16], mc_p_idx[17], mc_p_idx[18], mc_p_idx[19],
			mc_p_idx[20], mc_p_idx[21], mc_p_idx[22], mc_p_idx[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		rtd_pr_memc_emerg("[mc_phase][,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,][Time,%d,]\n\r",
			mc_phase_array[0], mc_phase_array[1], mc_phase_array[2], mc_phase_array[3], mc_phase_array[4], 
			mc_phase_array[5], mc_phase_array[6], mc_phase_array[7], mc_phase_array[8], mc_phase_array[9], 
			mc_phase_array[10], mc_phase_array[11], mc_phase_array[12], mc_phase_array[13], mc_phase_array[14],
			mc_phase_array[15], mc_phase_array[16], mc_phase_array[17], mc_phase_array[18], mc_phase_array[19],
			mc_phase_array[20], mc_phase_array[21], mc_phase_array[22], mc_phase_array[23],
			rtd_inl(TIMER_SCPU_CLK90K_LO_reg));
		WriteRegister(M8P_SOFTWARE3_SOFTWARE3_63_reg, 2, 2, 0);
	}
#endif
}
...
{
	if(RUN_MERLIN8P()){
		FRC_phTable_Cadence_Change_RTK2885pp(pParam, pOutput);
	}else if(RUN_MERLIN8()){
		FRC_phTable_Cadence_Change_RTK2885p(pParam, pOutput);
	}//RUN_MERLIN8P & RUN_MERLIN8
}
...
void FRC_phTable_FPGA_Script(const _PARAM_FRC_PH_TABLE *pParam, _OUTPUT_FRC_PH_TABLE *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned int cur_cadence = 0;
	static unsigned int pre_cadence = 0;
	unsigned char u8_filmPhase = 0;
	//unsigned int log_en = 0;
	M8P_rthw_phase_kphase_0c_RBUS M8P_rthw_phase_kphase_0c_rbus;
	unsigned short clear_st = 0;
	unsigned int u32_wrt_film_mode = 0;
	unsigned char u8_filmPh = s_pContext->_output_filmDetectctrl.u8_phT_phase_Idx_out[_FILM_ALL];   // bai  need make sure

	cur_cadence = s_pContext->_output_filmDetectctrl.u8_cur_cadence_out[_FILM_ALL];
	u32_wrt_film_mode = cur_cadence == _CAD_VIDEO? 0 : (cur_cadence == _CAD_22? 1 : (cur_cadence == _CAD_32? 2 : 3));
	M8P_rthw_phase_kphase_0c_rbus.regValue = rtd_inl(M8P_RTHW_PHASE_kphase_0C_reg);

	M8P_rthw_phase_kphase_0c_rbus.kphase_sw_film_en = 1;
	M8P_rthw_phase_kphase_0c_rbus.kphase_film_mode = u32_wrt_film_mode;
	M8P_rthw_phase_kphase_0c_rbus.kphase_film_phase_type = frc_cadTable[cur_cadence].cad_outLen;

	u8_filmPhase = (pOutput->u1_outMode_pre == _PQL_OUT_PC_MODE)? 0 : (u8_filmPh + 1)% frc_cadTable[cur_cadence].cad_outLen;
	M8P_rthw_phase_kphase_0c_rbus.kphase_sw_film_phase = u8_filmPhase;

	rtd_outl(M8P_RTHW_PHASE_kphase_0C_reg, M8P_rthw_phase_kphase_0c_rbus.regValue);

	if(cur_cadence != pre_cadence){
		FRC_kphase_sram_clear(clear_st, 25);

		phase_table_script_RTK2885pp(cur_cadence);
...
void phase_table_script_RTK2885pp(FRC_CADENCE_ID cur_cadence)
{
	if(cur_cadence == _CAD_VIDEO){
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x00000000);
		rtd_outl(0xb809d420, 0x00000000);
		rtd_outl(0xb809d424, 0x00000000);
		rtd_outl(0xb809d428, 0x00000000);
		rtd_outl(0xb809d45C, 0x00000000);
		rtd_outl(0xb809d460, 0x00000000);
		rtd_outl(0xb809d334, 0x00000010);
		rtd_outl(0xb809d33c, 0x00000010);

		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | 0);

		rtd_outl(0xb809df04, 0x2C790002);
		rtd_outl(0xb809df0c, 0x0000C800);
		rtd_outl(0xb809df08, 0x00802204);
	}
	else if(cur_cadence == _CAD_22){
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x00000000);
		...
		rtd_outl(0xb809df04, 0x2FBB2002);
		rtd_outl(0xb809df0c, 0x21014C00);
		//rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x20803200);

		rtd_outl(0xb809df04, 0x40DA1014);
		rtd_outl(0xb809df0c, 0x20010A00);
		//rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x20824405);
	}
	else if(cur_cadence == _CAD_32){
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x00000000);
		...
		rtd_outl(0xb809df04, 0x20FB3982);
		rtd_outl(0xb809df0c, 0x00694E00);
		rtd_outl(0xb809df08, 0x40805604);

		rtd_outl(0xb809df04, 0x20FD6682);
		rtd_outl(0xb809df0c, 0x0135D600);
		rtd_outl(0xb809df08, 0x40825600);

		rtd_outl(0xb809df04, 0x2FDC5342);
		rtd_outl(0xb809df0c, 0x00019000);
		rtd_outl(0xb809df08, 0x40846804);

		rtd_outl(0xb809df04, 0x20FD6002);
		rtd_outl(0xb809df0c, 0x00CD4E00);
		rtd_outl(0xb809df08, 0x40865600);

		rtd_outl(0xb809df04, 0x2FDC4CC2);
		rtd_outl(0xb809df0c, 0x01999000);
		rtd_outl(0xb809df08, 0x40884400);
	}
	else if(cur_cadence == _CAD_32322){
		#if 1
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x00012200);
		rtd_outl(0xb809d420, 0x01220001);
		rtd_outl(0xb809d424, 0x01000100);
		rtd_outl(0xb809d428, 0x00000000);
		rtd_outl(0xb809d45C, 0x09000090);
		rtd_outl(0xb809d460, 0x00000000);
		rtd_outl(0xb809d334, 0x00004210);
		rtd_outl(0xb809d33c, 0x00004210);

		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | 0);

		rtd_outl(0xb809df04, 0x27BB2542);
		rtd_outl(0xb809df0c, 0x212D4C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60803200);

		rtd_outl(0xb809df04, 0x5FBB32D5);
		rtd_outl(0xb809df0c, 0x20014C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60825605);

		rtd_outl(0xb809df04, 0x20DC4002);
		rtd_outl(0xb809df0c, 0x20D50A00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60844400);

		rtd_outl(0xb809df04, 0x3FBB2D53);
		rtd_outl(0xb809df0c, 0x21AD4C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60863201);

		rtd_outl(0xb809df04, 0x60DA1AE6);
		rtd_outl(0xb809df0c, 0x20810A00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60884406);

		rtd_outl(0xb809df04, 0x27BB2802);
		rtd_outl(0xb809df0c, 0x21554C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x608A3208);

		rtd_outl(0xb809df04, 0x6FBB3566);
		rtd_outl(0xb809df0c, 0x202D4C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x608C5616);

		rtd_outl(0xb809df04, 0x20DC42C2);
		rtd_outl(0xb809df0c, 0x21010A00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x608E4418);

		rtd_outl(0xb809df04, 0x4FBB3014);
		rtd_outl(0xb809df0c, 0x21D54C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60903221);

		rtd_outl(0xb809df04, 0x60DA1D66);
		rtd_outl(0xb809df0c, 0x20AD0A00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x6092442E);

		rtd_outl(0xb809df04, 0x2FBB2AC2);
		rtd_outl(0xb809df0c, 0x21814C00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x60943230);

		rtd_outl(0xb809df04, 0x60DA1826);
		rtd_outl(0xb809df0c, 0x20550A00);
//		rtd_outl(0xb809dffc, 0x00000022);
		rtd_outl(0xb809df08, 0x6096443E);
		#else
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x00010022);
		rtd_outl(0xb809d420, 0x23000001);
		rtd_outl(0xb809d424, 0x01000100);
		rtd_outl(0xb809d428, 0x00000000);
		rtd_outl(0xb809d45C, 0x90000009);
		rtd_outl(0xb809d460, 0x00000000);
		rtd_outl(0xb809d334, 0x00008110);
		rtd_outl(0xb809d33c, 0x00008110);
		...
		rtd_outl(0xb809df04, 0x6FBB3566);
		rtd_outl(0xb809df0c, 0x002D4C00);
		rtd_outl(0xb809df08, 0x60805606);

		rtd_outl(0xb809df04, 0x20DC42C2);
		rtd_outl(0xb809df0c, 0x01010A00);
		rtd_outl(0xb809df08, 0x60824400);

		rtd_outl(0xb809df04, 0x4FBB3014);
		rtd_outl(0xb809df0c, 0x01D54C00);
		rtd_outl(0xb809df08, 0x60843201);

		rtd_outl(0xb809df04, 0x60DA1D66);
		rtd_outl(0xb809df0c, 0x00AD0A00);
		rtd_outl(0xb809df08, 0x60864406);

		rtd_outl(0xb809df04, 0x2FBB2AC2);
		rtd_outl(0xb809df0c, 0x01814C00);
		rtd_outl(0xb809df08, 0x60883200);

		rtd_outl(0xb809df04, 0x60DA1826);
		rtd_outl(0xb809df0c, 0x00550A00);
		rtd_outl(0xb809df08, 0x608A4406);

		rtd_outl(0xb809df04, 0x27BB2542);
		rtd_outl(0xb809df0c, 0x012D4C00);
		rtd_outl(0xb809df08, 0x608C3200);

		rtd_outl(0xb809df04, 0x5FBB32D5);
		rtd_outl(0xb809df0c, 0x00014C00);
		rtd_outl(0xb809df08, 0x608E5605);

		rtd_outl(0xb809df04, 0x20DC4002);
		rtd_outl(0xb809df0c, 0x00D50A00);
		rtd_outl(0xb809df08, 0x60904400);

		rtd_outl(0xb809df04, 0x3FBB2D53);
		rtd_outl(0xb809df0c, 0x01AD4C00);
		rtd_outl(0xb809df08, 0x60923201);

		rtd_outl(0xb809df04, 0x60DA1AE6);
		rtd_outl(0xb809df0c, 0x00810A00);
		rtd_outl(0xb809df08, 0x60944406);

		rtd_outl(0xb809df04, 0x27BB2802);
		rtd_outl(0xb809df0c, 0x01554C00);
		rtd_outl(0xb809df08, 0x60963200);
		#endif
	}
	else if(cur_cadence == _CAD_2224){
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x01000100);
		rtd_outl(0xb809d420, 0x22000100);
		rtd_outl(0xb809d424, 0x00000122);
		rtd_outl(0xb809d428, 0x00000000);
		rtd_outl(0xb809d45C, 0x90000000);
		rtd_outl(0xb809d460, 0x00000009);
		rtd_outl(0xb809d334, 0x00018010);
		rtd_outl(0xb809d33c, 0x00018010);
		...
		rtd_outl(0xb809df04, 0x60FB39A6);
		rtd_outl(0xb809df0c, 0x00694E00);
		rtd_outl(0xb809df08, 0x60805606);

		rtd_outl(0xb809df04, 0x27DC4682);
		rtd_outl(0xb809df0c, 0x01359000);
		rtd_outl(0xb809df08, 0x60824400);

		rtd_outl(0xb809df04, 0x5EBB3355);
		rtd_outl(0xb809df0c, 0x00014C00);
		rtd_outl(0xb809df08, 0x60845605);

		rtd_outl(0xb809df04, 0x27DC4002);
		rtd_outl(0xb809df0c, 0x00CD0800);
		rtd_outl(0xb809df08, 0x60864400);

		rtd_outl(0xb809df04, 0x3EBB2CD3);
		rtd_outl(0xb809df0c, 0x01994C00);
		rtd_outl(0xb809df08, 0x60883201);

		rtd_outl(0xb809df04, 0x67DA19A6);
		rtd_outl(0xb809df0c, 0x00690800);
		rtd_outl(0xb809df08, 0x608A4406);

		rtd_outl(0xb809df04, 0x26BB2682);
		rtd_outl(0xb809df0c, 0x01354C00);
		rtd_outl(0xb809df08, 0x608C3200);
		...
		rtd_outl(0xb809df04, 0x20FD6002);
		rtd_outl(0xb809df0c, 0x00CD4E00);
		rtd_outl(0xb809df08, 0x60905600);

		rtd_outl(0xb809df04, 0x3FDC4CD3);
		rtd_outl(0xb809df0c, 0x01999000);
		rtd_outl(0xb809df08, 0x60924401);
	}
	else if(cur_cadence == _CAD_22224){
		rtd_outl(0xb809d414, 0x00010001);
		rtd_outl(0xb809d418, 0x01000132);
		rtd_outl(0xb809d458, 0x00090000);
		rtd_outl(0xb809d41c, 0x01000100);
		rtd_outl(0xb809d420, 0x01000100);
		rtd_outl(0xb809d424, 0x01222200);
		rtd_outl(0xb809d428, 0x00000000);
		rtd_outl(0xb809d45C, 0x00000000);
		rtd_outl(0xb809d460, 0x00000990);
		rtd_outl(0xb809d334, 0x00060010);
		rtd_outl(0xb809d33c, 0x00060010);

		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x00000000);
		rtd_outl(M8P_RTHW_PHASE_TO_SRAM_kphase_to_sram_00_reg, 0x80000000 | 0);

		rtd_outl(0xb809df04, 0x20FB3D42);
		rtd_outl(0xb809df0c, 0x00AD4E00);
		rtd_outl(0xb809df08, 0x60805604);

		rtd_outl(0xb809df04, 0x2FDC4AC2);
		rtd_outl(0xb809df0c, 0x01819000);
		rtd_outl(0xb809df08, 0x60824400);

		rtd_outl(0xb809df04, 0x20FB3802);
		rtd_outl(0xb809df0c, 0x00554E00);
		rtd_outl(0xb809df08, 0x60845604);

		rtd_outl(0xb809df04, 0x27DC4542);
		rtd_outl(0xb809df0c, 0x012D9000);
		rtd_outl(0xb809df08, 0x60864400);

		rtd_outl(0xb809df04, 0x2EBB32C2);
		rtd_outl(0xb809df0c, 0x00014C00);
		rtd_outl(0xb809df08, 0x60885604);

		rtd_outl(0xb809df04, 0x27DC4002);
		rtd_outl(0xb809df0c, 0x00D50800);
		rtd_outl(0xb809df08, 0x608A4400);

		rtd_outl(0xb809df04, 0x2EBB2D42);
		rtd_outl(0xb809df0c, 0x01AD4C00);
		rtd_outl(0xb809df08, 0x608C3200);

		rtd_outl(0xb809df04, 0x27DA1AC2);
		rtd_outl(0xb809df0c, 0x00810800);
		rtd_outl(0xb809df08, 0x608E4404);

		rtd_outl(0xb809df04, 0x26BB2802);
		rtd_outl(0xb809df0c, 0x01554C00);
		rtd_outl(0xb809df08, 0x60903200);

		rtd_outl(0xb809df04, 0x2EBB3542);
		rtd_outl(0xb809df0c, 0x002D4C00);
		rtd_outl(0xb809df08, 0x60925604);

		rtd_outl(0xb809df04, 0x20FD62C2);
		rtd_outl(0xb809df0c, 0x01014E00);
		rtd_outl(0xb809df08, 0x60945600);

		rtd_outl(0xb809df04, 0x2FDC5002);
		rtd_outl(0xb809df0c, 0x01D59000);
		rtd_outl(0xb809df08, 0x60964400);
	}
}
