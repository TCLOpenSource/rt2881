...
VOID patch_HF_Detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID patch_HF_Action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID patch_RidingHorse_Detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID patch_RidingHorse_Action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	if(pParam->u1_RidingHorse_Detect_en)
	{
...
VOID  patch_BluceMic_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID  patch_BluceMic_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID  patch_casino_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID  patch_HighFreqPlayerOSD_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID  patch_golfBall_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID  patch_golfBall_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
#if 0/*need resolve:compiler error*/
...
VOID  patch_BallJump_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
#if 0
	//const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID  patch_BallJump_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID  patch_Soccer_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{

}

VOID  patch_Soccer_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	unsigned int uv_alpha;
	ReadRegister(SOFTWARE2_SOFTWARE2_40_reg, 16, 23, &uv_alpha);
	...
	if(pParam->u1_sportscene_en && s_pContext->_output_me_sceneAnalysis.u1_SportScene){
		...
		WriteRegister(IPPRE_IPPRE_04_reg, 16, 23, uv_alpha); //32
...
VOID  patch_tri_ball_Issue_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	unsigned int log_en, cad32322_en;
	ReadRegister(HARDWARE_HARDWARE_55_reg, 14, 14, &log_en);
	ReadRegister(HARDWARE_HARDWARE_55_reg, 13, 13, &cad32322_en);

#if 0//TV010_SMALL_OBJECT_ALGO
	if(s_pContext->_external_info.u1_CloseTriball==true) {
		pOutput->u1_tri_ball_Detected = 10;
		pOutput->u6_tri_ball_hold_cnt_tv010 = 0;
	}
	else if(s_pContext->_output_me_sceneAnalysis.u1_SportScene == true){
		if(g_new_sport_mode)
		{
			pOutput->u1_tri_ball_Detected = 12;
		}else{
			pOutput->u1_tri_ball_Detected = 11;
		}
		pOutput->u6_tri_ball_hold_cnt_tv010 = 8;
	}
	else if(pOutput->u6_tri_ball_hold_cnt_tv010>0){
		pOutput->u1_tri_ball_Detected = 11;
		pOutput->u6_tri_ball_hold_cnt_tv010--;
	}
	else if(u8_cadence_Id == _CAD_VIDEO && (in_fmRate == _PQL_IN_50HZ || in_fmRate == _PQL_IN_60HZ) && (out_fmRate == _PQL_OUT_100HZ || out_fmRate == _PQL_OUT_120HZ))
#else
	...
	{				
		...
		pOutput->u6_tri_ball_hold_cnt = 8;
	}else if(((u8_cadence_Id == _CAD_32322 && (in_fmRate == _PQL_IN_60HZ))) && ( _ABS_(u11_gmv_mvx)>=20 && _ABS_(u11_gmv_mvx)<=54 && _ABS_(u10_gmv_mvy)<=9) && (u32_gmv_cnt>=24000) && cad32322_en) //football2
	{
		pOutput->u1_tri_ball_Detected = 6;
		pOutput->u6_tri_ball_hold_cnt = 8;
	}
	...
	if(log_en) { //d5dc[14]
		rtd_pr_memc_emerg("[%s][%d] case,%d, frameRate,%d,%d, cad,%d, gmv,%d,%d,%u\n", __FUNCTION__, __LINE__, pOutput->u1_tri_ball_Detected, in_fmRate, out_fmRate, u8_cadence_Id, u11_gmv_mvx, u10_gmv_mvy, u32_gmv_cnt);
	}
}

VOID  patch_tri_ball_Issue_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
...
VOID patch_Forbes_action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
//	unsigned int test_val, test_val2, test_val3, test_val4;
...
#if 0//fix game to filmmaker black screen RTK_MEMC_Performance_tunging_from_tv001
	...
	if(memc_switch_en == 1 && (g_memc_switch_state == 1))
	{
...
#endif
			if(RUN_MERLIN8P()){
				rtd_pr_memc_notice("[PowerSaving_MEMC_bypass_switch][AA][%d,%d,%d,%x][%x]\n\r", memc_switch_frm_cnt, in_fmRate, rtd_inl(PPOVERLAY_uzudtg_DVS_cnt_reg), rtd_inl(TIMER_SCPU_CLK90K_LO_reg), rtd_inl(M8P_RTHW_PHASE_phase_A4_reg));
			}else if(RUN_MERLIN8()){
		...
		if(DTV_PowerSaving_stage == 1){
			...
			Mid_Mode_SetMEMCFrameRepeatEnable(1);
			if(RUN_MERLIN8P()){
				rtd_pr_memc_notice("[PowerSaving_MEMC_bypass_switch][ME_CLK_off][BB][,%x,%x,%x,x,%x,%x,%x,%x,]\n", rtd_inl(M8P_RTHW_MC_DMA_MC_RDMA_DB_CTRL_reg), rtd_inl(M8P_RTMC_LBMC_LBMC_CTRL_reg), rtd_inl(M8P_RTHW_LBMC_LBMC_DB_reg), HAL_MC_GetVarLPFEn(), HAL_MC_GetMCLogoVLpfEn_Enable(), rtd_inl(M8P_RTHW_MC_DMA_MC_LF_P_DMA_RD_Ctrl_reg), rtd_inl(M8P_RTHW_MC_DMA_MC_HF_P_DMA_RD_Ctrl_reg), HAL_MC_GetForceLFBEnable());
			}else if(RUN_MERLIN8()){
				rtd_pr_memc_notice("[PowerSaving_MEMC_bypass_switch][ME_CLK_off][BB][,%x,%x,%x,%x,%x,%x,%x,%x,]\n", rtd_inl(MC_DMA_MC_RDMA_DB_CTRL_reg), rtd_inl(MC_MC_B0_reg)&0x7, HAL_MC_GetVarLPFEn(), HAL_MC_GetMCLogoVLpfEn_Enable(), rtd_inl(MC_DMA_MC_LF_P_DMA_RD_Ctrl_reg), rtd_inl(MC_DMA_MC_HF_P_DMA_RD_Ctrl_reg), (rtd_inl(KME_ME1_TOP1_ME1_SCENE1_00_reg)>>9)&0x1, HAL_MC_GetForceLFBEnable());
			}//RUN_MERLIN8P & RUN_MERLIN8
	...
	if(memc_switch_en == 1){
		if(MEMC_Lib_HDMI_get_memc_bypass_to_enable_flag() == 1){
			...
			if((MEMC_LibGetMEMC_PCModeEnable()) || (u8_MEMCMode == 0) || (u8_MEMCMode == 4 && dejudder == 0)){
				...
				g_power_saving_flag = 1;
				if(RUN_MERLIN8P()){
					rtd_pr_memc_notice("[PowerSaving_MEMC_bypass_switch_HDMI][Repeat mode][,%d,%d,%d,%x,][,%x,][,%d,%d,%d,]\n\r", memc_switch_frm_cnt, in_fmRate, rtd_inl(PPOVERLAY_uzudtg_DVS_cnt_reg), rtd_inl(TIMER_SCPU_CLK90K_LO_reg), rtd_inl(M8P_RTHW_PHASE_phase_A4_reg), MEMC_LibGetMEMC_PCModeEnable(), u8_MEMCMode, dejudder);
				}else if(RUN_MERLIN8()){
...
////////////////////////////////////////////////////////////////////////
VOID PatchManage_Init_RTK2885p(_OUTPUT_PATCH_M *pOutput)
{
        pOutput->u1_HF1_Detected = 0;
	pOutput->u6_HF1_hold_cnt = 0;
	pOutput->u1_HF2_Detected = 0;
...
VOID Patch_CoringThr_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
#if 0/*need resolve:compiler error*/
...
VOID Patch_detect_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
        patch_HF_Detect_RTK2885p(pParam, pOutput);
	patch_RidingHorse_Detect_RTK2885p(pParam, pOutput);

	// golf ball
	if(pParam->u1_patch_golfBall_en == 1)
	{
		patch_golfBall_detect_RTK2885p(pParam, pOutput);
	}
	else
	{
		pOutput->u1_golfBall_HVdtl_FB_true = 0;
	...
	{
		patch_BallJump_detect_RTK2885p(pParam, pOutput);
	}
	else
	{
		pOutput->s8_BallJump_holdCnt = 0;
	...
	if(pParam->u1_BluceMic_En == 1)
	{
		patch_BluceMic_detect_RTK2885p(pParam, pOutput);
	}
	else
	{
		pOutput->u1_BluceMic_Detected = 0;
	...
	{
		patch_HighFreqPlayerOSD_detect_RTK2885p(pParam, pOutput);
	}
	else
	{
		pOutput->u1_patch_highFreq_zmv_true = 0;
	...
	{
		patch_tri_ball_Issue_detect_RTK2885p(pParam, pOutput);
	}
	else
	{
		pOutput->u1_tri_ball_Detected = 0;
...
VOID Patch_Action_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
        patch_HF_Action_RTK2885p(pParam, pOutput);
	patch_RidingHorse_Action_RTK2885p(pParam, pOutput);

	// const _PQLCONTEXT *s_pContext = GetPQLContext();

	patch_casino_action_RTK2885p(pParam, pOutput);     // casino action

	patch_golfBall_action_RTK2885p(pParam, pOutput);   // golf ball

	patch_BallJump_action_RTK2885p(pParam, pOutput);   // hisense ball jump

	patch_BluceMic_action_RTK2885p(pParam, pOutput);

	patch_Soccer_action_RTK2885p(pParam, pOutput);     // soccer

	patch_Forbes_action_RTK2885p(pParam, pOutput);

	patch_MEMC_bypass_switch(pParam, pOutput);

	patch_MEMC_bypass_switch_HDMI(pParam, pOutput);

	patch_tri_ball_Issue_action_RTK2885p(pParam, pOutput);
	//// double scan action
	{
...
VOID PatchManage_Proc_RTK2885p(const _PARAM_PATCH_M *pParam, _OUTPUT_PATCH_M *pOutput)
{
	if(pParam->u1_patch_M_en == 0)
		return;
		
	Patch_detect_RTK2885p(pParam, pOutput);
	Patch_Action_RTK2885p(pParam, pOutput);

	Patch_CoringThr_RTK2885p(pParam, pOutput);
