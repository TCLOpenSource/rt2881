...
VOID ReadComReg_Proc_inputInterrupt_Hsync_RTK2885p(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	mc_read_i = HAL_MC_GetKphaseMCRead_i_Index();
	mc_read_p = HAL_MC_GetKphaseMCRead_p_Index();
...
VOID ReadComReg_Proc_inputInterrupt_RTK2885p(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	if (pParam->u1_me_GMV_2nd_rb_en == 1)
	{
		...
		pOutput->u17_me_GMV_2nd_cnt_rb = ReadComReg_Calib_Data(u32_RB_val, CALIB_PARAM_GMV_CNT, 0, 32400, false);
	...
	if (pParam->u1_kphase_rb_en == 1)
	{
		...
		pOutput->u8_sys_N_rb = crtc1_crtc1_60.crtc1_n;
		pOutput->u8_sys_M_rb = crtc1_crtc1_60.crtc1_m;
	...
	if(pParam->u1_lg_blk_cnt_rb_en == 1)
	{
		...
		for(idx=0; idx < 11; idx++)
		{
			...
			if(idx < 10){
				ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg + idx * 4,
							0,
							9, &u32_RB_val);
				pOutput->u10_dh_blklogo_idx0_cnt[idx*3+0] = u32_RB_val;

				ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg + idx * 4,
							10,
							19, &u32_RB_val);
				pOutput->u10_dh_blklogo_idx0_cnt[idx*3+1] = u32_RB_val;

				ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg + idx * 4,
							20,
							29, &u32_RB_val);
				pOutput->u10_dh_blklogo_idx0_cnt[idx*3+2] = u32_RB_val;
			}else{	
				ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx30_31_win_en_reg,
							0,
							9, &u32_RB_val);
				pOutput->u10_dh_blklogo_idx0_cnt[30] = u32_RB_val;
				ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx30_31_win_en_reg,
							10,
							11, &u32_RB_val);
				pOutput->u10_dh_blklogo_idx0_cnt[31] = u32_RB_val;
			}
		}

		for(u8_i=0; u8_i<32; u8_i++){
			pOutput->u10_dh_blklogo_idx0_cnt[u8_i] = Calib_Data(pOutput->u10_dh_blklogo_idx0_cnt[u8_i], g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		}
...
VOID ReadComReg_Proc_outputInterrupt_RTK2885p(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
...
#if IC_K24 && CONFIG_REPEAT_NEW_ALOG
	...
	for (u32_i = 0; u32_i < 32; u32_i ++)
	{
		...
		if (((pParam->u32_me_rMV_1st_rb_en >> u32_i)&0x1) == 1)
		{
			...
			pOutput->u12_me_rMV_1st_cnt_rb[u32_i] = ReadComReg_Calib_Data((u32_RB_val & 0xfff), CALIB_PARAM_GMV_CNT, 0, 0xfff, true);
		...
		if (((pParam->u32_me_rMV_2nd_rb_en >> u32_i)&0x1) == 1)
		{
			...
			ReadRegister(KME_ME1_TOP5_KME_ME1_TOP5_00_reg + 4 * u32_i, 0, 31, &u32_RB_val);
			pOutput->u12_me_rMV_2nd_cnt_rb[u32_i] = ReadComReg_Calib_Data(((u32_RB_val >> 12) & 0xfff), CALIB_PARAM_GMV_CNT, 0, 0xfff, true);
			ReadRegister(KME_ME1_TOP5_KME_ME1_TOP5_80_reg + 4 * u32_i, 0, 31, &u32_RB_val);
