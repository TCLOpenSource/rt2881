...
VOID HAL_HDF_LimitCheck(unsigned int u32Page_Reg_Addr, unsigned int *u32Page_Reg_Val)
{
	...
	#if ENABLE_4K1K
		else if(Get_DISP_ACT_END_VPOS() <= 1088 && Get_DISPLAY_REFRESH_RATE() >= 120){
					*u32Page_Reg_Val = (memc_dma_address_info->MC00_start_addr + /*KMC_TOTAL_SIZE*/KMC_TOTAL_SIZE_8buf_8bit);
		}
	#endif
		else{
...
#if 1
VOID HAL_HDF_Load_RTK2885p(VOID)
{
#ifdef USE_LOAD_HDF
...
VOID HAL_HDF_Load_RTK2885pp(VOID)
{
	rtd_pr_memc_emerg("[%s] system won't load hdf\n", __FUNCTION__);
}

VOID HAL_HDF_Load(VOID)
{
	if(RUN_MERLIN8P()){
		HAL_HDF_Load_RTK2885pp();
	}else if(RUN_MERLIN8()){
		HAL_HDF_Load_RTK2885p();
...
VOID HAL_HDF_Load_Page_u8HDF(unsigned int page)
{
	...
	for(u32Page_Idx = 0; u32Page_Idx < u32PageNum; u32Page_Idx++)
	{
		...
		for(u32Page_Reg_Idx = 0; u32Page_Reg_Idx < u32PageSize / 4; u32Page_Reg_Idx++)
		{
			...
			#endif
			if(get_reset_MEMC_flg()==true && ((u32Page_Reg_Addr+MEMC_REG_BASE_OFFSET==CRTC1_CRTC1_20_reg)||(u32Page_Reg_Addr+MEMC_REG_BASE_OFFSET==CRTC1_CRTC1_98_reg)))
			{
				continue;
				rtd_pr_memc_emerg("HSR change,Do not write d198/d120=%d",u32Page_Reg_Addr+MEMC_REG_BASE_OFFSET);
			}
			WriteRegister(u32Page_Reg_Addr+MEMC_REG_BASE_OFFSET, 0, 31, u32Page_Reg_Val);
		}
		#if SHOW_LOG
