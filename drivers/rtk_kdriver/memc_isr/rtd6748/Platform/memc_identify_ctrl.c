...
#include "memc_isr/include/memc_identify_tv011.h"
#include "memc_isr/include/memc_identify_tv030.h"
#include "memc_isr/include/memc_identify_tv043.h"
...
{
	unsigned char Setting_Num = (MEMC_PERFORMANCE_SETTING_TABLE_MAX==0) ? 0 : 
								(MEMC_PERFORMANCE_SETTING_TABLE_MAX>255) ? 255 : MEMC_PERFORMANCE_SETTING_TABLE_MAX-1;
	return Setting_Num;
}
...
bool is_Prj_support_indentify(unsigned char u8_project_id)
{
	if(u8_project_id==VIP_Project_ID_TV001 || u8_project_id==VIP_Project_ID_TV002 || 
		...
		// use original flow: tv011, tv030, tv043
		return true;
	}
	return false;
}
...
bool is_indentify_Info_enable(unsigned char u8_project_id)
{
	...
	if(u8_project_id==VIP_Project_ID_TV030){
		//u8_project_id==VIP_Project_ID_TV011 || u8_project_id==VIP_Project_ID_TV043
		return false;
	}
...
{
	if(u8_project_id==VIP_Project_ID_TV001){//VIP_Project_ID_TV001
		return &DynamicOptimizeMEMC_Info_TV001;
	}
	if(u8_project_id==VIP_Project_ID_TV002){
		return &DynamicOptimizeMEMC_Info_TV002;
	}
	if(u8_project_id==VIP_Project_ID_TV006){
		return &DynamicOptimizeMEMC_Info_TV006;
	}
	if(u8_project_id==VIP_Project_ID_TV010){
		return &DynamicOptimizeMEMC_Info_TV010;
	}
	#if 0// use original flow
	//if(u8_project_id==VIP_Project_ID_TV011){
	//	return &DynamicOptimizeMEMC_Info_TV011;
	//}
	if(u8_project_id==VIP_Project_ID_TV030){
		return &DynamicOptimizeMEMC_Info_TV030;
	}
	//if(u8_project_id==VIP_Project_ID_TV043){
	//	return &DynamicOptimizeMEMC_Info_TV043;
	//}
	#endif
	return &DynamicOptimizeMEMC_Info_TV001;
}
...
bool Setting_Row_Is_Empty(unsigned char set_id, unsigned char row_id)
{
	if(MEMC_setting_Table[set_id][row_id][MEMC_PERFORMANCE_SETTING_reg]!=0){
		return false;
	}
	if(MEMC_setting_Table[set_id][row_id][MEMC_PERFORMANCE_SETTING_high_bit]!=0){
		return false;
	}
	if(MEMC_setting_Table[set_id][row_id][MEMC_PERFORMANCE_SETTING_low_bit]!=0){
		return false;
	}
	if(MEMC_setting_Table[set_id][row_id][MEMC_PERFORMANCE_SETTING_value]!=0){
		return false;
	}
	return true;
}
...
void memc_identify_ctrl_apply_setting(unsigned char set_id, unsigned char u8_option)
{
	...
	if(set_id >= MEMC_PERFORMANCE_SETTING_TABLE_MAX || u8_option >= MEMC_PERFORMANCE_SETTING_OPTION_NUM/*|| (setting_index > (sizeof(MEMC_setting_Table)/sizeof(MEMC_setting_Table[0]))-1)*/)
		return;
	...
	if(u8_option == MEMC_PERFORMANCE_SETTING_OPTION_SET){
		...
		if(!u1_backup_setting[set_id]){
			for(u8_i = 1; u8_i < sizeof(MEMC_setting_Table[set_id])>>4; u8_i++) // 4 (MEMC_PERFORMANCE_SETTING_MAX) * 4(unsigned int) = 16
			{
				...
				reg_addr = MEMC_setting_Table[set_id][u8_i][MEMC_PERFORMANCE_SETTING_reg];
				if(reg_addr_is_right(reg_addr)){
					//store the value when switching
					ReadRegister(reg_addr, MEMC_setting_Table[set_id][u8_i][1], MEMC_setting_Table[set_id][u8_i][2], &backup_reg_val[set_id][u8_i]);
				}else {
		...
		{
			//write table value
			if(set_id != 0 && MEMC_setting_Table[set_id][u8_i][0] != 0)
			{
				WriteRegister(MEMC_setting_Table[set_id][u8_i][0], MEMC_setting_Table[set_id][u8_i][1], MEMC_setting_Table[set_id][u8_i][2], MEMC_setting_Table[set_id][u8_i][3]);
			}
		}
	}
	// restore the setting
		...
		for(u8_i = 1; u8_i < sizeof(MEMC_setting_Table[set_id])>>4; u8_i++) // 4 (MEMC_PERFORMANCE_SETTING_MAX) * 4(unsigned int) = 16
		{
			if(MEMC_setting_Table[set_id][u8_i][0] != 0)
				WriteRegister(MEMC_setting_Table[set_id][u8_i][0], MEMC_setting_Table[set_id][u8_i][1], MEMC_setting_Table[set_id][u8_i][2], backup_reg_val[set_id][u8_i]);
		}
...
void MEMC_Performance_Checking_Database(void)
{
	...
	{
		for(i = 1; i < sizeof(MEMC_setting_Table[pre_check_index])>>4; i++) // 4 (MEMC_PERFORMANCE_SETTING_MAX) * 4(unsigned int) = 16
		{
	...
	for(i = 1; i < sizeof(MEMC_setting_Table[check_index])>>4; i++) // 4 (MEMC_PERFORMANCE_SETTING_MAX) * 4(unsigned int) = 16
	{
		//store the value when switching
...
bool Identify_Pattern_API(_IDENTIFY_SINGLE_PATTERN_INFO * idendify_info, unsigned char project_id, unsigned char pattern_id)
{
	...
	for(u8_i=0; u8_i<u8_array_len; u8_i++){
		...
		special_scene_id = (special_scene_id>=MEMC_SPECIAL_SCENE_TABLE_MAX) ? 0 : special_scene_id;
		if(y_ratio>=y_ratio_th && hue_ratio>=hue_ratio_th && DynamicOptimizeMEMC_SpecialScene[special_scene_id]==true){
			...
			set_id = (set_id>=MEMC_PERFORMANCE_SETTING_TABLE_MAX) ? 0 : set_id;
			status_id = idendify_info->Param.status_id;
			status_id = (status_id>=MEMC_STATUS_TABLE_MAX) ? 0 : status_id;
...
void Check_SettingTable(void)
{
	...
	rtd_pr_memc_notice("[%s][%s] setting_num,%d\n", __FUNCTION__, __MEMC_CHECK_SETTING_TABLE__, MEMC_PERFORMANCE_SETTING_TABLE_MAX);

	//check reg_addr, bit, reg_value isn't zero
	for(set_id=1; set_id<MEMC_PERFORMANCE_SETTING_TABLE_MAX; set_id++){
		set_enum_id = MEMC_setting_Table[set_id][0][MEMC_PERFORMANCE_SETTING_enum_id];
		...
		for(u8_i=1; u8_i<MEMC_PERFORMANCE_SETTING_ITEM_MAX; u8_i++){
			...
			reg_addr = MEMC_setting_Table[set_id][u8_i][MEMC_PERFORMANCE_SETTING_reg];
			high_bit = MEMC_setting_Table[set_id][u8_i][MEMC_PERFORMANCE_SETTING_high_bit];
			low_bit = MEMC_setting_Table[set_id][u8_i][MEMC_PERFORMANCE_SETTING_low_bit];
			reg_value = MEMC_setting_Table[set_id][u8_i][MEMC_PERFORMANCE_SETTING_value];
...
bool Check_Single_DynamicOptimizeArray(_IDENTIFY_SINGLE_PATTERN_INFO *Idendify_Info, unsigned char project_id, unsigned char pattern_id, bool show_log)
{
	...
	//check setting index
	if(Idendify_Info->Param.set_id>MEMC_PERFORMANCE_SETTING_TABLE_MAX){
		result = false;
		if(show_log){
			rtd_pr_memc_notice("[%s][%s] ERROR(setting index exceed max value(%d)): project_id,%d, pattern_id,%d, set_id,%d\n", __FUNCTION__, __MEMC_IDENTIFY_DATA__, MEMC_PERFORMANCE_SETTING_TABLE_MAX,
...
{
	unsigned char project_id, pattern_id, max_pattern_num;//u8_i, array_len, row_index;
	_IDENTIFY_INFO *Idendify_Info = NULL;
...
void Identify_ctrl_detect_pattern(void)
{
	...
	unsigned char project_id = 0, pattern_id = 0, max_pattern_num = 0, bin_pattern_num = 0;
...
void Identify_Restore_All_Setting(void)
{
	...
	for(u8_i=1; u8_i<MEMC_PERFORMANCE_SETTING_TABLE_MAX; u8_i++){
...
bool Identify_ctrl_get_specal_scene_status(unsigned char u8_index)
{
	...
	unsigned int u32_RB_Value = 0, u32_pattern298_zidoo_player_forbindden_en = 0, u32_RB_kmc_blend_y_alpha=0, vtest=0;
