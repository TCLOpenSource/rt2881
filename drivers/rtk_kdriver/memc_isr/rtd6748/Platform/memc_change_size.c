...
extern unsigned char OnlyRegenMe;
void MEMC_ChangeSize_main_Only_RegenMe(const unsigned char MEMC_SZ_SEL, unsigned int ME_base_addr, MEMC_INOUT_MODE emom, MEMC_OUTPUT_FRAMERATE Out_FrameRate,unsigned int MEMC_LoadSizeParam_Type)
{
	...
	rtd_pr_memc_emerg("[%s][%d]OnlyRegenMe done\n\r", __FUNCTION__, __LINE__);
...
void MEMC_ChangeSize_main(const unsigned char MEMC_SZ_SEL, unsigned int ME_base_addr, MEMC_INOUT_MODE emom, MEMC_OUTPUT_FRAMERATE Out_FrameRate,unsigned int MEMC_LoadSizeParam_Type)
{
	...
	if(OnlyRegenMe){
		MEMC_ChangeSize_main_Only_RegenMe(MEMC_SZ_SEL,ME_base_addr,emom,Out_FrameRate,MEMC_LoadSizeParam_Type);
		return;
	}
	...
	if((emom == MEMC_RGB_IN_VIDEO_OUT) || (emom == MEMC_YUV_IN_VIDEO_OUT) || (emom == MEMC_RGB_IN_VIDEO_FREERUN_OUT) || (emom == MEMC_RGB_IN_PC_FREERUN_OUT)){
		if((emom == MEMC_RGB_IN_VIDEO_FREERUN_OUT) || (emom == MEMC_RGB_IN_PC_FREERUN_OUT)){
			...
			if(GS_output_frame_rate > OUTPUT_60HZ){
				if(package_value == PACKAGE_BGA22_7_CN3){
					GS_pqc_selection = 7;
				}else{
					GS_pqc_selection = 5;	
				}
	...
	rtd_pr_memc_emerg("[%s][%d][%d,%d,%d][htotal,%d,][me_base_addr,%x,][clock,%x,]\n\r", __FUNCTION__, __LINE__, GS_MC_8_buffer, GS_MCBuffer_Size, GS_pqc_selection, Get_DISP_HORIZONTAL_TOTAL(), GS_me_base_addr, rtd_inl(SYS_REG_SYS_CLKEN3_reg));
...
void MEMC_ChangeSize_LBMC(void)
{
	...
	if(GS_image_hsize==2560 && GS_image_vsize==1440){
		prefetch_limit = 23;
		prefetch_limit_s = 22;
	}
...
void MEMC_ChangeSize_LOGO(void)
{
	...
	if(GS_bringup_logo){
		...
		kme_dehalo2_kme_dehalo2_98_reg.dh_logo_rgn_h1 = ( (GS_image_hsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_h1*/ 66/ 240; //0x42
		kme_dehalo2_kme_dehalo2_98_reg.dh_logo_rgn_h2 = ( (GS_image_hsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_h2*/ 119/ 240; //0x77
		kme_dehalo2_kme_dehalo2_98_reg.dh_logo_rgn_h3 = ( (GS_image_hsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_h3*/ 144/ 240; //0x90
		kme_dehalo2_kme_dehalo2_9c_reg.dh_logo_rgn_h4 = ( (GS_image_hsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_h4*/ 194/ 240; //0xC2
		...
		kme_dehalo2_kme_dehalo2_94_reg.dh_logo_rgn_v1 = ( (GS_image_vsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_v1*/ 22/ 135; // 0x16
		kme_dehalo2_kme_dehalo2_94_reg.dh_logo_rgn_v2 = ( (GS_image_vsize/GS_ipme_dn_ratio)/4 ) * /*Dehalo_Logo_Region_Suggest_Val.dh_logo_rgn_v2*/ 103/ 135; // 0x67
...
if(pqc_bypass == 0 /*&& comp_line_mode == 1*/)
	...
	rtd_pr_memc_emerg("[%s::%d][DMA check][DMA_en,%d,%d,%d,%d,%d,,%d,%d,%d,%d,%d,]\n", __func__, __LINE__, 
		...
		(rtd_inl(0xb809c26c)>>20)&0x1, (rtd_inl(0xb809c270)>>20)&0x1, (rtd_inl(0xb809c274)>>20)&0x1, (rtd_inl(0xb809c278)>>20)&0x1, (rtd_inl(0xb809c28c)>>20)&0x1);
			...
			mc_dma_mc_rdma_ctrl_reg.rdma_multi_boundary_limit = 1;
