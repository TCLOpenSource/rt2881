...
VOID FBTCCurve_FastMotion(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	#else
	if(log_en && u1_detect_fastmotion){//d920[23]
		rtd_pr_memc_emerg("[%s] MaxGmv,%d, gmv,%d,%d, 2nd_cnt,%d,\n",__FUNCTION__, 
...
VOID FBTCCurve_ME(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	unsigned char u8_TopRim_th = Convert_UHD_Param_To_Current_RES(16, PARAM_DIRECTION_Y);// 4k: u8_TopRim_th=16
	unsigned char u8_LftRim_th = Convert_UHD_Param_To_Current_RES(24, PARAM_DIRECTION_X);// 4k: u8_LftRim_th=24
	...
	ReadRegister(HARDWARE_HARDWARE_58_reg, 17, 17, &u32_RB_val);//bit 17
	ReadRegister(SOFTWARE3_SOFTWARE3_06_reg, 18, 18, &log_en);
	ReadRegister(SOFTWARE3_SOFTWARE3_57_reg, 0, 7, &u8_gain);
	ReadRegister(SOFTWARE3_SOFTWARE3_57_reg, 8,15, &u8_shift);
...
VOID FBLevelCalc(const _PARAM_FBLevelCtrl *pParam, _OUTPUT_FBLevelCtrl *pOutput)
{
	...
	#endif
	ReadRegister(SOFTWARE3_SOFTWARE3_06_reg, 16, 16, &log_en);
	ReadRegister(HARDWARE_HARDWARE_58_reg, 17, 17, &u32_RB_val);//bit 17
	...
	// temp consistency mapping u8_FBResult
	{
		...
		#if 0
		if(u32_ID_Log_en==1){
		rtd_pr_memc_info("[YE MEMC FBR] ,XXX, u8_FBResult=%d,u27_FBTempConsis=%d,u27_TempConsis_Th_l=%d, u27_TempConsis_Th_h=%d, mv_tempconsis=%d, mv_tempconsis_limit=%d, FBTempConsisIIR_up=%d,FBTempConsis_IIR_down=%d, u27_me_aTC_rb=%d, \n  ",
		pOutput->u8_FBResult ,pOutput->u27_FBTempConsis,pOutput->u27_TempConsis_Th_l,pOutput->u27_TempConsis_Th_h,
		u27_mv_tempconsis,u27_mv_tempconsis_limit,pParam->u8_FBTempConsisIIR_up,pParam->u8_FBTempConsis_IIR_down,s_pContext->_output_read_comreg.u27_me_aTC_rb);
		}
		#endif
