...
VOID RimDet_by_KMC_YUV_Meter(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	case_bright = (sum_rdbk_histo01to31_cnt>(((rdbk_histo00_cnt+sum_rdbk_histo01to31_cnt)*71)>>10));//51>>10 -> 5%
...
VOID RimCtrl_HW_extract(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
...
#if	1
	unsigned int log_en = 0, log_en2 = 0, log_en3 = 0, log_en4 = 0, log_en5 = 0, log_en6 = 0, debug_en = 0, debug_val[4], u1_pan_protect = 0;	
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg,28,28, &log_en6);
	...
	if(1){
		...
		if(log_en6){
			rtd_pr_memc_emerg("RIM_RHT=%d,RIM_LFT=%d,,avg_apli=%u,G_APL=%u, apl,%u,%u,%u,%u,%u,%u,%u,%u  %u,%u,%u,%u,%u,%u,%u,%u,  %u,%u,%u,%u,%u,%u,%u,%u,  %u,%u,%u,%u,%u,%u,%u,%u,\n",
			...
			rtd_pr_memc_emerg("%d,%d,%d,%d,%d,%d,%d",s_pContext->_output_me_sceneAnalysis.u2_panning_flag,flag0,flag3,pOutput->u12_out_resolution[_RIM_RHT],u12_det_Rim[_RIM_RHT],
...
VOID RimCtrl_ME2_Rim_Convert(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K){//if(Get_DISPLAY_PANEL_TYPE() == P_LVDS_2K1K){ //for K24905 #304 	
