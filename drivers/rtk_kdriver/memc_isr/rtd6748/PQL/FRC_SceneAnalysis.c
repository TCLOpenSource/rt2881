...
VOID MEMC_me1_bgmask_newalgo_Proc(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
    ...
    static int Dtl_array0_pre=0,Dtl_array14_pre=0,Dtl_array22_pre=0,Dtl_array31_pre=0;
    ...
    int gmv_fgcnt,cond_grey_slow_1,cond_grey_slow_2,cond_grey_slow_3,cond_grey_slow_4,toplow_dtlsum,apl_avrrgn,dtl_avrrgn,slow_bgstable_1,slow_bgstable_2,slow_bgstable_3;
    int flag_leftrmv,flag_rightrmv,flag_rmvdiff;
    static int holdframe_equalbgcnt=0,video_fastmvx_equalbgcnt=0,holdframe_greyslow=0,v_greyslow=0,holdframe_smallfg=0,flag_smallfg=0,hold_5vertical2=0,flag_5vertical2=0,hold_slow_bgstable=0,flag_slow_bgstable=0,hold_rotate=0,flag_rotate=0;
    static int hold_mvchaos=0,video_mvhchaos=0,hold_predver=0,video_predver,flag_bgunstable=0,flag_sadoff_veren=0,hold_sadoff_veren=0;
    static int hold_fastbgmotion=0,flag_fastbgmotion=0;
   ...
   //+++++++++++++++++++163 dog  vertical  moving fast+++++++//
    ...
    cond_013_rmv = ((rmvx[0]<=1 && rmvy[0]<=1)&& (rmvx[1]<=1 && rmvy[1]<=1)&& (rmvx[2]<=5 && rmvy[2]<=5)&& (rmvx[3]<=5 && rmvy[3]<=5)&& (rmvx[5]<=1 && rmvy[5]<=1)
        && (rmvx[6]<=1 && rmvy[6]<=1)&& (rmvx[7]<=1 && rmvy[7]<=1)); 
...
if(fg_apl_sum>0 && bg_apl_sum>0){
    ...
    //+++++++++++++++++++++++317 broken face+++++++++++++++++++//
    fgratio0 = 0;
    if((fg_apl_seg0_cnt + bg_apl_seg0_cnt)>0)
    {
...
//logo off small fg  228
	...
	for(rgnIdx=0;rgnIdx<16;rgnIdx++)
	{
		...
		slow_bgstable_2 = dtl_avrrgn >40000 && dtl_avrrgn<78000 && Dtl_array[0]>45000 && Dtl_array[0]<65000 && Dtl_array[24]>44000 && Dtl_array[24]<75000;
    ...
    (fg_apl_seg1_cnt+fg_apl_seg2_cnt)*100/fg_apl_sum>5 && (fg_apl_seg1_cnt+fg_apl_seg2_cnt)*100/fg_apl_sum<25  && Dtl_array[8]<10000
	...
	// fast motion
	if(gmv_bgcnt>27000 && gmv_bgx>170 && _ABS_(gmv_fgx)<15 && _ABS_(gmv_bgy)<6 &&  _ABS_(gmv_fgy)<6 && Dtl_array[26]<18000 && rgn_apli[31]>200000 && dtl_avrrgn<26000)
	{
	...
	//ver th
	if(ABS(gmv_fgy,u10_gmv_mvy)>20	&& ABS(gmv_fgx,u11_gmv_mvx)<20  && gmv_bgcnt>29000 && fg_apl_seg0_cnt>600)
	{
...
VOID MEMC_dehalo_newalgo_bgmaskBase_Proc(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
    ...
    int sum_dtl_top=0;
    static int avg_bg_diff_cond_top=0;  
    gmv_bgcnt=s_pContext->_output_read_comreg.me1_gmv_bgcnt;
...
VOID SAD_Peak_Skip_GFB(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_06_reg, 17, 17, &log_en);
...
#else
extern unsigned int Scaler_GetShareMemVirAddr(unsigned int a_ulCmd);
#endif
...
void FRC_is_not_natural_video(const _PARAM_FRC_SceneAnalysis *pParam, _OUTPUT_FRC_SceneAnalysis *pOutput)
{
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_45_reg,15, 15,&logtest);
	if(logtest)
		rtd_pr_memc_emerg("score=%d,dtl=%d,apli=%d,aplp=%d,gmvx=%d,gmvy=%d,u30_aSAD=%d",score,dtl,apli,aplp,gmvx,gmvy,u30_aSAD);
	...
	//MOKA-1417
	if(1>=_ABS_(gmvx) && 1>=_ABS_(gmvy)&&MEMC_Lib_get_Adaptive_Stream_Flag()==1&&450000>=u30_aSAD)
	{
		s_pContext->_output_wrt_comreg.u8_force_caddpnt=0;
		u5_blend_holdfrm_preProcess_a[4] = 60;
	...
	if(s_pContext->_output_wrt_comreg.u8_force_caddpnt==0){
		if(logtest){
		rtd_pr_memc_emerg("%d,%d,%d,%d,%d,%d"
		,s_pContext->_output_read_comreg.u20_me_rAPLi_rb[0],s_pContext->_output_read_comreg.u20_me_rAPLi_rb[31]
		,s_pContext->_output_me_sceneAnalysis.u1_Region_Periodic_detect_true
		,gmvx,gmvy,MEMC_Lib_get_Adaptive_Stream_Flag());
		}
	}
...
unsigned int FRC_Modify_DarkScene_scThresh(unsigned int u32_Thresh)
{
	...
	ReadRegister(SOFTWARE3_SOFTWARE3_54_reg,  0,  0, &algo_en);
	//ReadRegister(SOFTWARE3_SOFTWARE3_54_reg,  1,  1, &set_high_sc_th);
	ReadRegister(SOFTWARE3_SOFTWARE3_54_reg,  2,  2, &log_en);
	...
	if(u26_Apli > 500000) {
		if(log_en){
			rtd_pr_memc_emerg("[CHECK_INFO] case A\n");
		}
		return u32_Thresh;
	}
	...
	//	if(log_en){
	//		rtd_pr_memc_emerg("[CHECK_INFO] case B\n");
	//	}
	//	return u32_Thresh;
	//}
	...
	if(log_en){//d9d8[2]
		rtd_pr_memc_emerg("[CHECK_INFO] SAD,%u, thresh,%u,%u,	APL,%d, Cnt,%u, UnConf,%d, Vx,%d, Vy,%d\n", 
		u30_aSAD, u32_Thresh, (unsigned int)u64_thresh, u26_Apli, u17_gmv_cnt, u12_gmv_unconf, s11_gmv_mvx, s10_gmv_mvy);
	}
	
	//rtd_pr_memc_notice(""[Modify Thres] %d\n", thresh);
	if(algo_en){//d9d8[0]
