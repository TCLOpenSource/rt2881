...
VOID ReadComReg_Proc_outputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	for (u32_i = 0; u32_i < 32; u32_i ++)
	{
		...
		if (((pParam->u32_me_rSC_rb_en >> u32_i)&0x1) == 1)
		{
			...
			max_rSC = _MAX_(pOutput->u22_me_rSC_rb[u32_i], max_rSC);
		}
		else
		{
	...
	pOutput->u22_me_rSC_max = max_rSC;
	...
	for (u32_i = 0; u32_i < 4; u32_i ++)
	{
		if (pParam->u1_mc_HVdtl_rb_en == 1)
		{
			...
			pOutput->u10_mc_VHdtl_rb[u32_index] = Calib_Data((u32_RB_val & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u10_mc_VHdtl_rb[u32_index+1] = Calib_Data(((u32_RB_val>>10) & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u10_mc_VHdtl_rb[u32_index+2] = Calib_Data(((u32_RB_val>>20) & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			ReadRegister(KMC_TOP_kmc_top_C0_reg + 12 * u32_i + 4, 0, 31, &u32_RB_val);
			pOutput->u10_mc_VHdtl_rb[u32_index+3] = Calib_Data((u32_RB_val & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u10_mc_VHdtl_rb[u32_index+4] = Calib_Data(((u32_RB_val>>10) & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u10_mc_VHdtl_rb[u32_index+5] = Calib_Data(((u32_RB_val>>20) & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			ReadRegister(KMC_TOP_kmc_top_C0_reg + 12 * u32_i + 8, 0, 31, &u32_RB_val);
			pOutput->u10_mc_VHdtl_rb[u32_index+6] = Calib_Data((u32_RB_val & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u10_mc_VHdtl_rb[u32_index+7] = Calib_Data(((u32_RB_val>>10) & 0x3FF), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		}
		else
		{
			u32_index = 8*u32_i;
			pOutput->u10_mc_VHdtl_rb[u32_index] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+1] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+2] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+3] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+4] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+5] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+6] = 0;
			pOutput->u10_mc_VHdtl_rb[u32_index+7] = 0;
		}
	}
	...
	//RMV_blending & regional_fallback
	{
		...
		for(u32_i=0; u32_i<8; u32_i++)
		{
			...
			pOutput->u8_region_fb_max[u32_index] = Calib_Data((u32_RB_val & 0xff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u8_region_fb_max[u32_index+1] = Calib_Data(((u32_RB_val>>8) & 0xff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u8_region_fb_max[u32_index+2] = Calib_Data(((u32_RB_val>>16) & 0xff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u8_region_fb_max[u32_index+3] = Calib_Data(((u32_RB_val>>24) & 0xff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		}
		for(u32_i=0; u32_i<16; u32_i++)
		{
			u32_index = u32_i*2;
			ReadRegister(KMC_BI_bifb_cnt_00_reg + u32_i*4, 0, 31, &u32_RB_val);
			pOutput->u12_region_fb_cnt[u32_index] = Calib_Data((u32_RB_val & 0xfff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
			pOutput->u12_region_fb_cnt[u32_index+1] = Calib_Data(((u32_RB_val>>16) & 0xfff), g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		}
		for(u32_i=0; u32_i<32; u32_i++)
		{
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_BG_APL00_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	pOutput->dh_bg_apl_seg0_cnt=Calib_Data(RB_val, g_ME1_VBUF_GAIN, g_ME1_VBUF_SHIFT);
	RB_val = (u32_RB_val) & 0xffff;
	pOutput->dh_bg_apl_seg1_cnt=Calib_Data(RB_val, g_ME1_VBUF_GAIN, g_ME1_VBUF_SHIFT);

	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_BG_APL01_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_BG_APL02_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_BG_APL03_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_FG_APL00_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_FG_APL01_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_FG_APL02_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
	...
	ReadRegister(KME_ME1_BG1_ME_BG_INFO_RGN_FG_APL03_reg, 0,31, &u32_RB_val);
	RB_val = (u32_RB_val>>16) & 0xffff;
    ...
    for(rgnIdx =0 ;  rgnIdx< 10; rgnIdx++)
    {
        ...
        pOutput->s11_fg_rMV_x_rb[0+u32_index]=rmvx_fg;
        pOutput->s10_fg_rMV_y_rb[0+u32_index]=rmvy_fg;
        ...
        pOutput->s11_fg_rMV_x_rb[1+u32_index]=rmvx_fg;
        pOutput->s10_fg_rMV_y_rb[1+u32_index]=rmvy_fg; 
        ...
        pOutput->s11_fg_rMV_x_rb[2+u32_index]=rmvx_fg;
        pOutput->s10_fg_rMV_y_rb[2+u32_index]=rmvy_fg;  
	...
	for (u32_i = 0; u32_i < 16; u32_i ++) {
		...
		pOutput->u15_mv_diff_hist[0+u32_index] = Calib_MV(u32_RB_val);
		ReadRegister(KME_ME1_TOP11_KME_ME1_TOP11_GMV_BV_HIST_2_reg+4*u32_i , 16, 30, &u32_RB_val);
		pOutput->u15_mv_diff_hist[1+u32_index] = Calib_MV(u32_RB_val);
	}
