...
VOID RimCtrl_Get_HW_rim(_OUTPUT_RimCtrl *pOutput)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
unsigned short RimCtrl_Get_Rim_Speed(unsigned rim_id)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
...
VOID RimCtrl_Shrink_By_Pan(unsigned short *u12_det_Rim)
{
	const _PQLCONTEXT *s_pContext = GetPQLContext();
	...
	if(force_bot){
		...
		if(u2_Panning_flg==Pan_Dir_X) {
			...
			if(s11_gmv_mvx>0 && RimCtrl_Get_Rim_Speed(_RIM_RHT)>speed_th) {
				u8_shrink_case = 1;
			...
			else if(s11_gmv_mvx<0 && RimCtrl_Get_Rim_Speed(_RIM_LFT)>speed_th) {
				u8_shrink_case = 2;
			...
			if(s10_gmv_mvy>0 && RimCtrl_Get_Rim_Speed(_RIM_BOT)>speed_th) {
				u8_shrink_case = 3;
				//u12_det_Rim[_RIM_TOP] = (u12_det_Rim[_RIM_TOP] < 1) ? 0 : u12_det_Rim[_RIM_TOP]-1;
				u12_det_Rim[_RIM_BOT] = u12_det_Rim[_RIM_BOT] - u12_shrink_value;
			}
			else if(s10_gmv_mvy<0 && RimCtrl_Get_Rim_Speed(_RIM_TOP)>speed_th) {
				u8_shrink_case = 4;
...
VOID RimCtrl_HW_extract(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	unsigned int u1_two_side_balance_en,log_en2 = 0, log_en3 = 0, log_en4 = 0, log_en5 = 0, debug_en = 0, debug_val[4];
	...
	ReadRegister(HARDWARE_HARDWARE_43_reg, 31, 31, &u1_two_side_balance_en);
...
RimCtrl_Shrink_By_Pan(u12_det_Rim);
	...
	if(1){
	unsigned int total_apli=0,avg_apli=0,i=0;
	bool flag0=0,flag3=0;
	for(i=0;i<32;i++)
	{
		total_apli+=s_pContext->_output_read_comreg.u20_me_rAPLi_rb[i];
	}
	avg_apli=total_apli/32;

	//0,8,16,24
	if(s_pContext->_output_read_comreg.u20_me_rAPLi_rb[0]>avg_apli || s_pContext->_output_read_comreg.u20_me_rAPLi_rb[8]>avg_apli
	|| s_pContext->_output_read_comreg.u20_me_rAPLi_rb[16]>avg_apli || s_pContext->_output_read_comreg.u20_me_rAPLi_rb[24]>avg_apli){
		flag0=1;
	}

	//7,15,23,31
	if(s_pContext->_output_read_comreg.u20_me_rAPLi_rb[7]>avg_apli || s_pContext->_output_read_comreg.u20_me_rAPLi_rb[15]>avg_apli
	|| s_pContext->_output_read_comreg.u20_me_rAPLi_rb[23]>avg_apli || s_pContext->_output_read_comreg.u20_me_rAPLi_rb[31]>avg_apli){
		flag3=1;
	}

	if(0){
		rtd_pr_memc_err("RIM_RHT=%d,RIM_LFT=%d,,avg_apli=%u,G_APL=%u, apl,%u,%u,%u,%u,%u,%u,%u,%u	%u,%u,%u,%u,%u,%u,%u,%u,  %u,%u,%u,%u,%u,%u,%u,%u,	%u,%u,%u,%u,%u,%u,%u,%u,\n",
			u12_det_Rim[_RIM_RHT],u12_det_Rim[_RIM_LFT],avg_apli,s_pContext->_output_read_comreg.u26_me_aAPLi_rb,
		...
		rtd_pr_memc_err("%d,%d,%d,%d,%d,%d,%d",s_pContext->_output_me_sceneAnalysis.u2_panning_flag,flag0,flag3,pOutput->u12_out_resolution[_RIM_RHT],u12_det_Rim[_RIM_RHT],
		u12_det_Rim[_RIM_LFT],pParam->u12_RimDiffTh);
	}

	if(s_pContext->_output_me_sceneAnalysis.u2_panning_flag== 1 && (flag0||flag3)){
		if ((pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT]) > (u12_det_Rim[_RIM_LFT] + pParam->u12_RimDiffTh))//right> left
		{
			u12_det_Rim[_RIM_RHT] = (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_LFT]);
		}
		else if (u12_det_Rim[_RIM_LFT] > (pOutput->u12_out_resolution[_RIM_RHT] - u12_det_Rim[_RIM_RHT] + pParam->u12_RimDiffTh))//left> right
		{
...
// update the vaild region id
// valid region: rim area is less than one quarter of region area
VOID RimCtrl_Update_Vaild_Rgn_id(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	unsigned short top_rim = pOutput->u12_Rim_Result[_RIM_TOP];
	unsigned short bot_rim = pOutput->u12_Rim_Result[_RIM_BOT];
	unsigned short lft_rim = pOutput->u12_Rim_Result[_RIM_LFT];
	unsigned short rht_rim = pOutput->u12_Rim_Result[_RIM_RHT];
	unsigned short resolution_height = pOutput->u12_out_resolution[_RIM_BOT]+1;
	unsigned short resolution_width = pOutput->u12_out_resolution[_RIM_RHT]+1;
	unsigned char start_x_id = 0, end_x_id = 0, start_y_id = 0, end_y_id = 0;

	if(top_rim>((resolution_height*1)>>4)){ //top rim > image height/16
		start_y_id = 1;
	}else{
		start_y_id = 0;
	}

	if(bot_rim<((resolution_height*15)>>4)){
		end_y_id = 2;
	}else{
		end_y_id = 3;
	}

	if(lft_rim>((resolution_width*9)>>5)){
		start_x_id = 3;
	}else if(lft_rim>((resolution_width*5)>>5)){
		start_x_id = 2;
	}else if(lft_rim>((resolution_width*1)>>5)){
		start_x_id = 1;
	}else{
		start_x_id = 0;
	}

	if(rht_rim<((resolution_width*23)>>5)){
		end_x_id = 4;
	}else if(rht_rim<((resolution_width*27)>>5)){
		end_x_id = 5;
	}else if(rht_rim<((resolution_width*31)>>5)){
		end_x_id = 6;
	}else{
		end_x_id = 7;
	}

	pOutput->u3_ValidRgn_start_x_id = start_x_id;
	pOutput->u3_ValidRgn_end_x_id = end_x_id;
	pOutput->u2_ValidRgn_start_y_id = start_y_id;
	pOutput->u2_ValidRgn_end_y_id = end_y_id;
...
VOID RimCtrl_Output(const _PARAM_RimCtrl *pParam, _OUTPUT_RimCtrl *pOutput)
{
	...
	if (pOutput->u1_RimChange == 1)
	{
		...
		RimCtrl_Update_Vaild_Rgn_id(pParam, pOutput);
	}
}
...
*/
	...
	if(RHT_ratio > LFT_ratio+1){ // R>L for #35 & K-23755  // R>L+1 for 30010_I_Robot
		if(Comp_R_cnt < 9)
			Comp_R_cnt++;
	}else{
		if(Comp_R_cnt > 0)
