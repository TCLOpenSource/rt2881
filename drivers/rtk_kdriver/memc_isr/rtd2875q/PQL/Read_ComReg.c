...
VOID ReadComReg_Init(_OUTPUT_ReadComReg *pOutput)
{
	...
	u14_Motion_Region_H_Gain = (u32IPME1_Golden_HSize<<(g_WIDTH_SHIFT))/tmp_MEMC_buffer_size_table->IPME1_HSize;
	u14_Motion_Region_V_Gain = (u32IPME1_Golden_VSize<<(g_HEIGHT_SHIFT))/tmp_MEMC_buffer_size_table->IPME1_VSize;
...
VOID ReadComReg_Proc_inputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	unsigned int u30_ipme_Mot_sum = 0, u27_ipme_Mot_min = 0xffffffff, u27_ipme_Mot_max = 0;
	static unsigned int u27_Mot_list[10] = {0};
	...
	for(u8_i=MOT_DIFF_LIST_NUM-1; u8_i>=1; u8_i--){
		pOutput->u27_ipme_aMot_list[u8_i] = pOutput->u27_ipme_aMot_list[u8_i-1];
	}
	pOutput->u27_ipme_aMot_list[0] = pOutput->u27_ipme_aMot_rb;

	if(pOutput->u27_ipme_aMot_rb>=50){
		for(u8_i=9; u8_i>=1; u8_i--){
			u27_Mot_list[u8_i] = u27_Mot_list[u8_i-1];
		}
		u27_Mot_list[0] = pOutput->u27_ipme_aMot_rb;
		u27_ipme_Mot_min = pOutput->u27_ipme_aMot_rb;
		u27_ipme_Mot_max = pOutput->u27_ipme_aMot_rb;
		u30_ipme_Mot_sum = pOutput->u27_ipme_aMot_rb;

		for(u8_i=1; u8_i<10; u8_i++){
			u30_ipme_Mot_sum += u27_Mot_list[u8_i];
			if(u27_ipme_Mot_min>u27_Mot_list[u8_i]){
				u27_ipme_Mot_min = u27_Mot_list[u8_i];
			}else if(u27_ipme_Mot_max<u27_Mot_list[u8_i]){
				u27_ipme_Mot_max = u27_Mot_list[u8_i];
			}
		}
		pOutput->u27_ipme_Mot_MovingAvg = (u30_ipme_Mot_sum-u27_ipme_Mot_min-u27_ipme_Mot_max)>>3;
	}
	...
	if(pParam->u1_lg_blk_cnt_rb_en == 1)
	{
		...
		for (idx=0; idx < LOGO_RG_32; idx++)
		{
			...
			pOutput->u10_blklogo_rgcnt[idx] = ReadComReg_Calib_Data(pOutput->u10_blklogo_rgcnt[idx], CALIB_PARAM_LOGO, 0, 0x3ff, true);
		}
		...
		pOutput->u16_blklogo_dh_clrcnt[0] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt[1] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt[2] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt[3] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_bot[0] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_bot[1] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_bot[2] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_top[0] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_top[1] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		pOutput->u16_blklogo_dh_clrcnt_top[2] = ReadComReg_Calib_Data((u32_RB_val & 0xFF), CALIB_PARAM_LOGO, 0, 0xff, true);
		...
		for (idx=0; idx<16; idx++)
		{
			...
			pOutput->u14_pxllogo_rgcnt[idx*2+0] = ReadComReg_Calib_Data(u32_RB_val, CALIB_PARAM_LOGO, 0, 0x3fff, true);

			ReadRegister(KME_LOGO2_KME_LOGO2_40_reg + idx * 4,
						 14,
						 27, &u32_RB_val);
			///pOutput->u14_pxllogo_rgcnt[idx*2+1] = Calib_Data(u32_RB_val, g_PLOGO_VBUF_GAIN, g_PLOGO_VBUF_SHIFT);
			pOutput->u14_pxllogo_rgcnt[idx*2+1] = ReadComReg_Calib_Data(u32_RB_val, CALIB_PARAM_LOGO, 0, 0x3fff, true);
		}
		...
		// HS merlin8 logo
		// read 32 rgn dh logo cnt
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[0]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[1]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx0_1_2_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[2]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx3_4_5_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[3]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx3_4_5_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[4]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx3_4_5_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[5]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx6_7_8_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[6]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx6_7_8_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[7]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx6_7_8_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[8]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx9_10_11_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[9]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx9_10_11_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[10]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx9_10_11_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[11]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx12_13_14_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[12]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx12_13_14_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[13]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx12_13_14_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[14]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx15_16_17_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[15]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx15_16_17_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[16]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx15_16_17_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[17]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx18_19_20_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[18]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx18_19_20_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[19]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx18_19_20_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[20]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx21_22_23_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[21]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx21_22_23_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[22]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx21_22_23_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[23]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx24_25_26_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[24]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx24_25_26_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[25]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx24_25_26_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[26]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx27_28_29_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[27]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx27_28_29_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[28]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx27_28_29_reg, 20, 29, &pOutput->u10_dhlogo_rgcnt[29]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx30_31_win_en_reg, 0, 9, &pOutput->u10_dhlogo_rgcnt[30]);
		ReadRegister(KME_DEHALO6_dh_logo_blkrg_cnt_idx30_31_win_en_reg, 10, 19, &pOutput->u10_dhlogo_rgcnt[31]);
		for(u8_i=0; u8_i<32; u8_i++){
			pOutput->u10_dhlogo_rgcnt[u8_i] = Calib_Data(pOutput->u10_dhlogo_rgcnt[u8_i], g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		}
		// HS merlin7 logo
	}
	// MV_diff cnt
	{
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_0_1_2_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[0]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_0_1_2_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[1]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_0_1_2_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[2]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_3_4_5_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[3]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_3_4_5_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[4]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_3_4_5_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[5]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_6_7_8_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[6]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_6_7_8_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[7]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_6_7_8_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[8]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_9_10_11_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[9]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_9_10_11_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[10]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_9_10_11_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[11]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_12_13_14_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[12]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_12_13_14_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[13]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_12_13_14_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[14]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_15_16_17_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[15]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_15_16_17_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[16]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_15_16_17_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[17]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_18_19_20_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[18]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_18_19_20_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[19]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_18_19_20_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[20]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_21_22_23_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[21]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_21_22_23_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[22]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_21_22_23_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[23]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_24_25_26_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[24]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_24_25_26_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[25]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_24_25_26_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[26]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_27_28_29_cnt_reg, 20, 29, &pOutput->me1_bv_stats_rg_cnt[27]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_27_28_29_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[28]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_27_28_29_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[29]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_30_31_cnt_reg, 10, 19, &pOutput->me1_bv_stats_rg_cnt[30]);
		ReadRegister(KME_ME1_TOP11_me1_bv_stats_rg_idx_30_31_cnt_reg, 0, 9, &pOutput->me1_bv_stats_rg_cnt[31]);
		for(u8_i=0; u8_i<32; u8_i++){
			pOutput->me1_bv_stats_rg_cnt[u8_i] = Calib_Data(pOutput->me1_bv_stats_rg_cnt[u8_i], g_ME1_VBUF_GAIN, g_ME1_VBUF_SHIFT);
		}
	}
	
	if(pOutput->u27_ipme_aMot_list[0]==0 && pOutput->u27_ipme_aMot_list[1]==0 && pOutput->u27_ipme_aMot_list[2]==0 && pOutput->u27_ipme_aMot_list[3]==0){
		pOutput->u2_zMot_cnt = 4;
	}else if(pOutput->u2_zMot_cnt>0){
		pOutput->u2_zMot_cnt = (pOutput->u27_ipme_aMot_list[0]!=0) ? pOutput->u2_zMot_cnt-1 : pOutput->u2_zMot_cnt;
	}
...
VOID ReadComReg_Proc_outputInterrupt(const _PARAM_ReadComReg *pParam, _OUTPUT_ReadComReg *pOutput)
{
	...
	unsigned int y_ofst = 0, x_ofst = 0, regOffset = 0;
	#endif
	...
	unsigned short max_rMV_vx=0, max_rMV_vy=0, max_boundary_vx=0, max_boundary_vy=0;
	const unsigned char range_x = 8, range_y = 4;
	unsigned char u8_x, u8_y, u8_index;
	unsigned int u21_DTL = 0, u21_DTL_max = 0;
	...
	for (u32_i = 0; u32_i < 32; u32_i ++)
	{
		...
		if (((pParam->u32_me_rMV_1st_rb_en >> u32_i)&0x1) == 1)
		{
			...
			max_rMV_vx = _MAX_(max_rMV_vx, _ABS_(pOutput->s11_me_rMV_1st_vx_rb[u32_i]));
			max_rMV_vy = _MAX_(max_rMV_vy, _ABS_(pOutput->s10_me_rMV_1st_vy_rb[u32_i]));
			if((u32_i>=0&&u32_i<=7)||(u32_i>=24&&u32_i<=31) || u32_i==8 || u32_i==16 || u32_i==15 || u32_i==23){
				max_boundary_vx = _MAX_(max_boundary_vx, _ABS_(pOutput->s11_me_rMV_1st_vx_rb[u32_i]));
				max_boundary_vy = _MAX_(max_boundary_vy, _ABS_(pOutput->s10_me_rMV_1st_vy_rb[u32_i]));
			}
	...
	pOutput->u11_max_rMV_vx = max_rMV_vx;
	pOutput->u10_max_rMV_vy = max_rMV_vy;
	pOutput->u11_max_boundary_vx = max_boundary_vx;
	pOutput->u10_max_boundary_vy = max_boundary_vy;

	// get dtl of adjacent blk ​​in the horizontal direction
	for(u8_x=0; u8_x<range_x-1; u8_x++){
		for(u8_y=0; u8_y<range_y; u8_y++){
			u8_index = u8_x + u8_y*8;
			u21_DTL = pOutput->u20_me_rDTL_rb[u8_index]+pOutput->u20_me_rDTL_rb[u8_index+1];
			u21_DTL_max = _MAX_(u21_DTL, u21_DTL_max);
		}
	}
	// get dtl of adjacent blk ​​in the vertical direction
	for(u8_x=0; u8_x<range_x; u8_x++){
		for(u8_y=0; u8_y<range_y-1; u8_y++){
			u8_index = u8_x + u8_y*8;
			u21_DTL = pOutput->u20_me_rDTL_rb[u8_index]+pOutput->u20_me_rDTL_rb[u8_index+8];
			u21_DTL_max = _MAX_(u21_DTL, u21_DTL_max);
		}
	}
	pOutput->u21_Adjacent_Blk_DTL_max = u21_DTL_max;
...
*/
    ...
    for (u32_i = 0; u32_i < 16; u32_i++)
	{
		...
		pOutput->u32_region_local_flicker_sum[_CLIP_(u32_i * 2, 0, 31)] = Calib_Data(u32_local_flicker_val1, g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
		pOutput->u32_region_local_flicker_sum[_CLIP_((u32_i * 2 + 1), 0, 31)] = Calib_Data(u32_local_flicker_val2, g_ME2_VBUF_GAIN, g_ME2_VBUF_SHIFT);
    }    
#endif
	...
	pOutput->u32_dh_bypass_cov_sum = Calib_Data(u32_dh_bypass_cov_sum, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
	pOutput->u32_dh_bypass_ucov_sum = Calib_Data(u32_dh_bypass_ucov_sum, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);

    for(rgnIdx =0 ; rgnIdx< 10; rgnIdx++)
    {
        ReadRegister(KME_ME1_BG2_ME_CLR_FG_RGN_CNT0_reg+4*rgnIdx ,0,9,&u32_RB_val);   
		pOutput->u32_me1_fg_clo_rgncnt[0+3*rgnIdx] = Calib_Data(u32_RB_val, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
        ReadRegister(KME_ME1_BG2_ME_CLR_FG_RGN_CNT0_reg+4*rgnIdx ,10,19,&u32_RB_val);   
		pOutput->u32_me1_fg_clo_rgncnt[1+3*rgnIdx] = Calib_Data(u32_RB_val, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
        ReadRegister(KME_ME1_BG2_ME_CLR_FG_RGN_CNT0_reg+4*rgnIdx ,20,29,&u32_RB_val);   
		pOutput->u32_me1_fg_clo_rgncnt[2+3*rgnIdx] = Calib_Data(u32_RB_val, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
    }
        ReadRegister(KME_ME1_BG2_ME_CLR_FG_RGN_CNT10_reg ,0,9,&u32_RB_val);   
		pOutput->u32_me1_fg_clo_rgncnt[30] = Calib_Data(u32_RB_val, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
        ReadRegister(KME_ME1_BG2_ME_CLR_FG_RGN_CNT10_reg ,10,19,&u32_RB_val);   
		pOutput->u32_me1_fg_clo_rgncnt[31] = Calib_Data(u32_RB_val, g_ME2_PFV_GAIN, g_ME2_PFV_SHIFT);
		// mv hist by diff of mv and gmv
		for (u32_i = 0; u32_i < 16; u32_i ++) {
			ReadRegister(KME_ME1_TOP11_KME_ME1_TOP11_GMV_BV_HIST_2_reg+4*u32_i ,  0, 14, &u32_RB_val);
			pOutput->u15_mv_diff_hist[0+2*u32_i] = Calib_MV(u32_RB_val);
			ReadRegister(KME_ME1_TOP11_KME_ME1_TOP11_GMV_BV_HIST_2_reg+4*u32_i , 16, 30, &u32_RB_val);
			pOutput->u15_mv_diff_hist[1+2*u32_i] = Calib_MV(u32_RB_val);
		}

#if IC_K24
	s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt1 = 0;
	s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt2 = 0;
	s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt3 = 0;

	s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt1 = 0;
	s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt2 = 0;
	s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt3 = 0;

	for (y_ofst = 0; y_ofst <= 3; y_ofst += 1){
		for (x_ofst = 0; x_ofst <= 7; x_ofst += 1){
			regOffset = 4 * (y_ofst * 8 + x_ofst);
			ReadRegister(KME_ALGO_me1_freq_statistic_00_reg + regOffset, 0, 29, &u32_RB_val);
			ReadRegister(KME_ALGO_me1_freq_statistic_00_reg + regOffset, 0, 29, &u32_RB_val);
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1[y_ofst][x_ofst] = ReadComReg_Calib_Data(((u32_RB_val>>0)&0x3FF), CALIB_PARAM_PERIODIC_CNT, 0, 0x3FF, true);
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2[y_ofst][x_ofst] = ReadComReg_Calib_Data(((u32_RB_val>>10)&0x3FF), CALIB_PARAM_PERIODIC_CNT, 0, 0x3FF, true);
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3[y_ofst][x_ofst] = ReadComReg_Calib_Data(((u32_RB_val>>20)&0x3FF), CALIB_PARAM_PERIODIC_CNT, 0, 0x3FF, true);

			s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt1 = _MAX_(s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt1, s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1[y_ofst][x_ofst]);
			s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt2 = _MAX_(s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt2, s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2[y_ofst][x_ofst]);
			s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt3 = _MAX_(s_pContext_m->_output_periodic_manage.u15_max_freq_statistic_cnt3, s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3[y_ofst][x_ofst]);

			s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt1 += s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1[y_ofst][x_ofst];
			s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt2 += s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2[y_ofst][x_ofst];
			s_pContext_m->_output_periodic_manage.u15_sum_freq_statistic_cnt3 += s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3[y_ofst][x_ofst];

			// IIR
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1_IIR[y_ofst][x_ofst] = (3 * s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1_IIR[y_ofst][x_ofst] + s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt1[y_ofst][x_ofst] + 2)>>2;
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2_IIR[y_ofst][x_ofst] = (3 * s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2_IIR[y_ofst][x_ofst] + s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt2[y_ofst][x_ofst] + 2)>>2;
			s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3_IIR[y_ofst][x_ofst] = (3 * s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3_IIR[y_ofst][x_ofst] + s_pContext_m->_output_periodic_manage.u10_freq_statistic_cnt3[y_ofst][x_ofst] + 2)>>2;
			
			for(u32_i=1; u32_i<PRD_STATIC_RMV_NUM; u32_i++){
				s_pContext_m->_output_periodic_manage.s11_freg_statistic_avgbv_mvx[PRD_STATIC_RMV_NUM-u32_i][y_ofst][x_ofst] = s_pContext_m->_output_periodic_manage.s11_freg_statistic_avgbv_mvx[PRD_STATIC_RMV_NUM-u32_i-1][y_ofst][x_ofst];
				s_pContext_m->_output_periodic_manage.s10_freg_statistic_avgbv_mvy[PRD_STATIC_RMV_NUM-u32_i][y_ofst][x_ofst] = s_pContext_m->_output_periodic_manage.s10_freg_statistic_avgbv_mvy[PRD_STATIC_RMV_NUM-u32_i-1][y_ofst][x_ofst];
			}
			ReadRegister(KME_ALGO3_me1_freq_statistic_avgbv_00_reg + regOffset, 0, 20, &u32_RB_val);			
			s_pContext_m->_output_periodic_manage.s11_freg_statistic_avgbv_mvx[0][y_ofst][x_ofst] = (((u32_RB_val>>10) & 1)==0) ? (u32_RB_val & 0x7ff) : ((u32_RB_val & 0x7ff) - (1<<11));
			s_pContext_m->_output_periodic_manage.s10_freg_statistic_avgbv_mvy[0][y_ofst][x_ofst] = (((u32_RB_val>>20) & 1)==0)? ((u32_RB_val>>11) & 0x3ff) : (((u32_RB_val>>11) & 0x3ff)	- (1<<10));
			s_pContext_m->_output_periodic_manage.s11_freg_statistic_avgbv_mvx[0][y_ofst][x_ofst] = Calib_MV(s_pContext_m->_output_periodic_manage.s11_freg_statistic_avgbv_mvx[0][y_ofst][x_ofst]);
			s_pContext_m->_output_periodic_manage.s10_freg_statistic_avgbv_mvy[0][y_ofst][x_ofst] = Calib_MV(s_pContext_m->_output_periodic_manage.s10_freg_statistic_avgbv_mvy[0][y_ofst][x_ofst]);
		}
	}
#endif
