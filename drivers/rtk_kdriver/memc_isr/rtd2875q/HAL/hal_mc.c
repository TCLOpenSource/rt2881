...
{
#if (IC_K6LP || IC_K8LP || IC_K24 || IC_K25) // merlin4_blend_mode
	if(u32Mode == 1){
 ...
 * @retval VOID
*/
extern VOID MEMC_Lib_Set_VarLpf_Tap(unsigned char u2_Tap);
/*VOID HAL_MC_SetVar17TapEn(BOOL bEnable)
{
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K){
		...
		MEMC_Lib_Set_VarLpf_Tap(0); // 9tap
	}
		...
		if(bEnable){
			MEMC_Lib_Set_VarLpf_Tap(2); // 33tap
		}else{
			MEMC_Lib_Set_VarLpf_Tap(1); // 17tap
		}
	}
#else
		//reg_mc_var17tap_en
...
VOID HAL_MC_Set_VarLPFTapSel(unsigned char VarLPFTapSel)//0:9tap;1:17tap;2:33tap
{
	...
	if(Scaler_MEMC_GetPanelSizeByDisp() == MEMC_SIZE_2K){
		...
		//WriteRegister(MC2_MC2_68_reg,19,19,(unsigned int)bEnable);
		 if(new_dehalo_dynamic ==0){
			MEMC_Lib_Set_VarLpf_Tap(VarLPFTapSel);


		}else{
			MEMC_Lib_Set_VarLpf_Tap(2);
