...
#ifndef CONFIG_RTK_EVENTLOG_NO_PRINTF
...
unsigned int kernel_event_save(unsigned int fw_type, unsigned int event_type, \
...
EXPORT_SYMBOL(kernel_event_save);
	...
	const char *buf, size_t count)
{
	...
	if(EVENT_ERROR == kernel_event_save(param[0], param[1], param[2], param[3], param[4], param[5]))
	{
		ret = -EINVAL;
