#include <osal.h>
#include "general/debug_abnormal.h"
...
{
    //unsigned int time = (unsigned int) rtd_inl(TIMER_SCPU_CLK90K_LO_reg) * 11;//not accurate
    unsigned int time = (unsigned int) rtd_inl(TIMER_SCPU_CLK90K_LO_reg) ;
    return time;
}
                            ...
                            unsigned int module_reserved)
{
    ...
    if (fw_type >= FW_MAX || module >= MODULE_MAX || event >= EVENT_MAX || event_type >= EVENT_TYPE_MAX) {
        AUDIO_VitalPrint("(fw_type module event event_type) > Specified range\n");
        return EVENT_ERROR;
    }

    /* clean event_format */
    AP_memset(&event_format, 0, sizeof(event_format));
    ...
    if(g_event_log_print_enable)
    {
        AUDIO_VitalPrint("%08x-%08x-%08x-%08x\n", ((event_format.syncbyte) << 24) | ((event_format.fw_type) << 20) \
                         | ((event_format.event_type) << 16) | (event_format.module << 8) \
