...
static struct task_struct *rtkvdec_dump_user_tsk = NULL;
static unsigned int rtkvdec_dump_es_user_ctx = 0;
static unsigned int rtkvdec_crc_export_user_ctx = 0;
static unsigned int rtkvdec_crc_import_user_ctx = 0;
static unsigned int rtkvdec_mmap_user_ctx = 0;
...
#endif
		phy_addr = pli_malloc(para1 & 0x7ffffff, GFP_DCU1_LIMIT, DVR_OWNER_VE_FLUSH);
...
unsigned long rtkvdec_remote_malloc_specific(unsigned long para1, unsigned long para2)
{
	...
	if (para1 == RTKVDEC_ENCODER_ALLOC) {
		phy_addr = pli_malloc(para2, GFP_DCU1_LIMIT, DVR_OWNER_VE_FLUSH);
		if (phy_addr == INVALID_VAL) {
			rtd_pr_vdec_warn("[%s %d]alloc DCU1_LIMIT for Encoder memory fail\n", __func__, __LINE__);
			return 0;
		}
		DCU = RTKVDEC_DCU1_LIMIT ;
	}
	else if (para1 == RTKVDEC_DCU1) {
		phy_addr = pli_malloc(para2, GFP_DCU1, DVR_OWNER_VE_FLUSH);
		if (phy_addr == INVALID_VAL) {
		...
		phy_addr = pli_malloc(para2, GFP_DCU1_FIRST, DVR_OWNER_VE_FLUSH);
		if (phy_addr == INVALID_VAL) {
	...
	else if (para1 == RTKVDEC_DCU2_FIRST_FLUSH) {

#ifdef CONFIG_HIGHMEM_BW_CMA_REGION
		phy_addr = pli_malloc(para2, GFP_HIGHMEM_BW_FIRST);
#else
		phy_addr = pli_malloc(para2, GFP_DCU2_FIRST, DVR_OWNER_VE_FLUSH);
#endif

		if (phy_addr == INVALID_VAL) {
			rtd_pr_vdec_warn("[%s %d]alloc DCU2_FIRST_FLUSH memory fail\n", __func__, __LINE__);
			return 0;
		}
		DCU = RTKVDEC_DCU2_FIRST ;
	}
...
bool rtkvdec_send_signal_to_task(struct task_struct *sig_tsk, int sig_num, int sig_type)
{
	bool isDone = true;
	int ret;
	...
	info.si_value.sival_int = sig_type;
	if((ret = send_sig_info(sig_num, &info, sig_tsk)) < 0)
	{
		isDone = false;
	    rtd_pr_vdec_err("Unable to send signal %d %x!!!!!!!!!!!!!!!\n", ret, (unsigned int)(unsigned long)sig_tsk);
	}

	return isDone;
}

void rtkvdec_dumpes_disable_gki(void)
{
	unsigned long return_value = 0 ;
	unsigned long vir_addr ;
	DEBUG_BUFFER_HEADER *header;

	if (!rtkvdec_dump_user_tsk)
	{
		rtd_pr_vdec_err("rtkvdec: es dump has no user task!\n");
		return;
	}
	...
	header = (DEBUG_BUFFER_HEADER*) g_pDumpES.Memory;
	header->magic = htonl(0xdeadcafe) ;
	header->size = htonl(sizeof(DEBUG_BUFFER_HEADER)) ;
	header->rd = htonl(g_pDumpES.PhyAddr + sizeof(DEBUG_BUFFER_HEADER)) ;
	header->wr = htonl(g_pDumpES.PhyAddr + sizeof(DEBUG_BUFFER_HEADER)) ;
	*(unsigned long *)g_pDumpES_Send.Memory = htonl(g_pDumpES.PhyAddr) ;
	...
	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_DUMP_ES, VDEC_DUMP_SIG_FILE_CLOSE))
	{
		rtd_pr_vdec_debug("rtkvdec: dump signal %d fail\n", VDEC_DUMP_SIG_FILE_CLOSE);
		return;
	}
...
int rtkvdec_dumpes_enable_gki(const char *pattern, int length)
{
	unsigned long return_value = 0 ;
	DEBUG_BUFFER_HEADER *header;
	unsigned long vir_addr ;
	...
	{
		rtd_pr_vdec_debug("rtkvdec: RPC_VCPU_SET_DUMPES_MEMORY fail %ld\n", return_value);
		rtkvdec_dumpes_disable_gki();
		return 0;
	}

	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_DUMP_ES, VDEC_DUMP_SIG_FILE_OPEN))
	{
...
void rtkvdec_exportcrc_disable_gki(void)
{
	unsigned long return_value = 0 ;
	unsigned long vir_addr ;
	DEBUG_BUFFER_HEADER *header;
	...
	*(unsigned long *)g_pDumpES_Send.Memory = htonl(g_pDumpES.PhyAddr) ;
	...
	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_CRC_EXPORT, VDEC_DUMP_SIG_FILE_CLOSE))
	{
		rtd_pr_vdec_debug("rtkvdec: export signal %d fail\n", VDEC_DUMP_SIG_FILE_CLOSE);
		return;
	}
...
int rtkvdec_exportcrc_enable_gki(const char *pattern, int length)
{
	unsigned long return_value = 0 ;
	DEBUG_BUFFER_HEADER *header;
	unsigned long vir_addr ;
	...
	{
		rtd_pr_vdec_debug("rtkvdec: RPC_VCPU_SET_EXPORTCRC_MEMORY fail %ld\n", return_value);
		rtkvdec_exportcrc_disable_gki();
		return 0;
	}

	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_CRC_EXPORT, VDEC_DUMP_SIG_FILE_OPEN))
	{
		rtd_pr_vdec_debug("rtkvdec: export signal %d fail\n", VDEC_DUMP_SIG_FILE_OPEN);
		rtkvdec_exportcrc_disable_gki();
	}
...
int rtkvdec_importcrc_enable(const char *pattern, int length)
{
	...
	if ((file_DumpES != 0)) {
		...
		header->magic = htonl(0xdeadcafe) ;
		header->size = htonl(fileSize + headerSize) ;
...
void rtkvdec_importcrc_disable_gki(void)
{
	unsigned long return_value = 0 ;
	unsigned long vir_addr ;
	DEBUG_BUFFER_HEADER *header;
	...
	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_CRC_IMPORT, VDEC_DUMP_SIG_FILE_CLOSE))
	{
		rtd_pr_vdec_debug("rtkvdec: import signal %d fail\n", VDEC_DUMP_SIG_FILE_CLOSE);
		return;
	}
...
int rtkvdec_importcrc_rpc_gki(void)
{
	unsigned long return_value = 0 ;
	...
	if (!isImporting)
	{
		rtd_pr_vdec_notice("rtkvdec: importing not start!\n");
		return 0 ;
	}

	if (!DumpES_Config.enable) {
		rtd_pr_vdec_notice("rtkvdec: import not enable!\n");
		return 0 ;
	}
...
int rtkvdec_importcrc_load_gki(unsigned int fileSize)
{
	DEBUG_BUFFER_HEADER *header;
	unsigned long vir_addr ;
	int headerSize = sizeof(DEBUG_BUFFER_HEADER);
	int maxFileSize = DUMP_ES_SIZE - headerSize;
	...
	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_CRC_IMPORT, VDEC_DUMP_SIG_FILE_LOAD))
	{
...
int rtkvdec_importcrc_enable_gki(const char *pattern, int length)
{
	if (length > FILE_NAME_SIZE - 1) {
		rtd_pr_vdec_warn("rtkvdec: file name is too long(<%d).\n", FILE_NAME_SIZE - 1);
		return 0;
	...
	if (isImporting)
	{
		rtd_pr_vdec_notice("rtkvdec: still importing!\n");
		return 0 ;
	}

	isImporting = true;

	if (!rtkvdec_send_signal_to_task(rtkvdec_dump_user_tsk, RTKVDEC_SIG_NUM_CRC_IMPORT, VDEC_DUMP_SIG_FILE_OPEN))
	{
		rtd_pr_vdec_debug("rtkvdec: import signal %d fail\n", VDEC_DUMP_SIG_FILE_OPEN);
		rtkvdec_importcrc_disable_gki();
	}
...
void rtkvdec_dump_ctrl(VDEC_DUMP *pDump)
{
	void *buf = NULL;
	...
	buf = (void *)kmalloc(pDump->dataSize, GFP_KERNEL);

	if (buf)
	{
		unsigned long dataAddr = (unsigned long)pDump->dataAddr;
		if (copy_from_user((void *)buf, (const void __user *)dataAddr, pDump->dataSize))
			rtd_pr_vdec_err("rtkvdec dump cmd VDEC_DUMP_LOG failed!!!!!!!!!!!!!!!\n");
	}
	...
	switch(pDump->cmd)
	{
		...
		case VDEC_DUMP_SET_USER_TASK:
		{
			rtkvdec_dump_user_tsk = get_current();
			break;
		}
		case VDEC_DUMP_RESET_USER_TASK:
		{
			rtkvdec_dump_user_tsk = NULL;
			break;
		}

		case VDEC_DUMP_SET_MMAP_USER_CTX:
		{
			rtkvdec_mmap_user_ctx = pDump->dataAddr;
			break;
		}
		case VDEC_DUMP_RESET_MMAP_USER_CTX:
		{
			rtkvdec_mmap_user_ctx = 0;
			break;
		}

		case VDEC_DUMP_FILE_PATH:
		{
			unsigned int userCtx = *((unsigned int *)buf);
			unsigned char *fileName = NULL;

			if (
				userCtx == rtkvdec_dump_es_user_ctx
				|| userCtx == rtkvdec_crc_export_user_ctx
				|| userCtx == rtkvdec_crc_import_user_ctx
			)
			{
				fileName = DumpES_Config.file_name;
			}

			if (fileName)
			{
				if (copy_to_user((void __user *)(unsigned long)pDump->dataAddr, (void *)fileName, FILE_NAME_SIZE))
					rtd_pr_vdec_err("rtkvdec dump cmd VDEC_DUMP_OPEN failed!!!!!!!!!!!!!!!\n");
			}
			break;
		}
		case VDEC_DUMP_IMPORT_FILE_SIZE:
		{
			unsigned int fileSize = *((unsigned int *)buf);
			rtkvdec_importcrc_load_gki(fileSize);
			break;
		}
		case VDEC_DUMP_IMPORT_FILE_LOAD:
		{
			rtkvdec_importcrc_rpc_gki();
			break;
		}
		case VDEC_DUMP_DBG_BUF_HDR:
		{
			unsigned int userCtx = *((unsigned int *)buf);
			...
			if (
				...
				phyDbgBufHdrAddr = (unsigned int)g_pDumpES.PhyAddr;
			else if (userCtx == rtkvdec_crc_import_user_ctx)
				phyDbgBufHdrAddr = (unsigned int)g_pDumpES_Send.PhyAddr;

			if (phyDbgBufHdrAddr)
			{
				if (copy_to_user((void __user *)(unsigned long)pDump->dataAddr, (void *)(unsigned long)&phyDbgBufHdrAddr, sizeof(phyDbgBufHdrAddr)))
					rtd_pr_vdec_err("rtkvdec dump cmd VDEC_DUMP_OPEN failed!!!!!!!!!!!!!!!\n");
			}
			break;
		}
		case VDEC_DUMP_ES_SIG_START:
		{
			rtkvdec_dump_es_user_ctx = pDump->dataAddr;
			break;
		}
		case VDEC_DUMP_ES_SIG_STOP:
		{
			rtkvdec_dump_es_user_ctx = 0;
			break;
		}
		case VDEC_DUMP_CRC_EXPORT_SIG_START:
		{
			rtkvdec_crc_export_user_ctx = pDump->dataAddr;
			break;
		}
		case VDEC_DUMP_CRC_EXPORT_SIG_STOP:
		{
			rtkvdec_crc_export_user_ctx = 0;
			break;
		}
		case VDEC_DUMP_CRC_IMPORT_SIG_START:
		{
			rtkvdec_crc_import_user_ctx = pDump->dataAddr;
			break;
		}
		case VDEC_DUMP_CRC_IMPORT_SIG_STOP:
		{
			rtkvdec_crc_import_user_ctx = 0;
			break;
		}
		default:
		{
			rtd_pr_vdec_debug("rtkvdec: unknown dump cmd(0x%08x)\n", pDump->cmd);
			break;
		}
	}
...
//	unsigned long return_value ;
	...
	switch (cmd) {
		...
		case VDEC_IOC_SVP_DMABUF_PRE_RELEASE:
		case VDEC_IOC_SVP_DMABUF_SET_CONFIG:
		{
		...
		{
			VDEC_DUMP vdecDump;
			if (copy_from_user((void *)&vdecDump, (const void __user *)arg, sizeof(VDEC_DUMP)))
			{
				ret = -EFAULT;
				rtd_pr_vdec_err("rtkvdec ioctl code=VDEC_IOC_DUMP failed!!!!!!!!!!!!!!!\n");
				goto out;
			}
			else
				rtkvdec_dump_ctrl(&vdecDump);
			break;
		}
		default:
		{
...
#if 0
static int rtkvdec_mmap(struct file *filp, struct vm_area_struct *vma)
{
    int ret = 0;
    struct page *page = NULL;
    unsigned long size = (unsigned long)(vma->vm_end - vma->vm_start);
    char *pMem = NULL;
    unsigned int *test = (unsigned int *)g_pDumpES.Memory;
    rtd_pr_vdec_err("test %x %x!!!!!!!!!!!!!!!!!!!!!!!!\n", test[0], test[1]);

	if (rtkvdec_mmap_user_ctx)
	{
		if (rtkvdec_mmap_user_ctx == rtkvdec_dump_es_user_ctx)
			pMem = (char *)g_pDumpES.Memory;
	}

    if (pMem == NULL/* || size > (PAGE_SIZE << 1)*/)
    {
        ret = -EINVAL;
        goto out;
    }

    page = virt_to_page((unsigned long)pMem + (vma->vm_pgoff << PAGE_SHIFT));
...
static int rtkvdec_mmap(struct file *file, struct vm_area_struct *vma)
{
    unsigned long offset = vma->vm_pgoff << PAGE_SHIFT;
    unsigned long size = vma->vm_end - vma->vm_start;
    unsigned long pfn_start;
    unsigned long virt_start;
    unsigned long virAddr = 0L;
    unsigned long phyAddr = 0L;
    int ret = 0;

	if (rtkvdec_mmap_user_ctx)
	{
		if (
			rtkvdec_mmap_user_ctx == rtkvdec_dump_es_user_ctx
			|| rtkvdec_mmap_user_ctx == rtkvdec_crc_export_user_ctx
		)
		{
			virAddr = g_pDumpES.Memory;
			phyAddr = g_pDumpES.PhyAddr;
		}
		else if (rtkvdec_mmap_user_ctx == rtkvdec_crc_import_user_ctx)
		{
			virAddr = g_pDumpES_Send.Memory;
			phyAddr = g_pDumpES_Send.PhyAddr;
		}
	}
	...
	pfn_start = (phyAddr >> PAGE_SHIFT) + vma->vm_pgoff;
	virt_start = virAddr + offset;

    rtd_pr_vdec_debug("%s: phy: 0x%lx, offset: 0x%lx, size: 0x%lx\n", __FUNCTION__, pfn_start << PAGE_SHIFT, offset, size);

    vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);
    ret = remap_pfn_range(vma, vma->vm_start, pfn_start, size, vma->vm_page_prot);
    if (ret)
        rtd_pr_vdec_err("%s: remap_pfn_range failed at [0x%lx  0x%lx]\n",
            __FUNCTION__, vma->vm_start, vma->vm_end);
    else
        rtd_pr_vdec_notice("%s: map 0x%lx to 0x%lx, size: 0x%lx\n", __FUNCTION__, virt_start,
            vma->vm_start, size);
...
ssize_t rtkvdec_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if ((cmd_buf[0] == 'f') && (count == 2)) {
		...
		if ((cmd_buf[4] == 'e') && (cmd_buf[5] == 'n')) {
			...
			rtkvdec_dumpes_enable_gki(&cmd_buf[7], (count-7)-1);
			#else
		...
		if ((cmd_buf[7] == 'e') && (cmd_buf[8] == 'n')) {
			...
			#ifdef CONFIG_RTK_KDRV_VDEC_MODULE
			rtkvdec_exportcrc_enable_gki(&cmd_buf[10], (count-10)-1);
			#else
		...
		if ((cmd_buf[7] == 'e') && (cmd_buf[8] == 'n')) {
			...
			#ifdef CONFIG_RTK_KDRV_VDEC_MODULE
			rtkvdec_importcrc_enable_gki(&cmd_buf[10], (count-10)-1);
			#else
