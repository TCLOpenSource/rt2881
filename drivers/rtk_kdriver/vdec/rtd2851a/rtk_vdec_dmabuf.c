...
int vdec_dmabuf_secureRunning = 0;
module_param(vdec_dmabuf_secureRunning, int, S_IWUSR|S_IRUGO);
EXPORT_SYMBOL(vdec_dmabuf_secureRunning);
		...
		if (privdata && (privdata == buf_array[i].priv)) {
			if (!(privdata->flags & FLAG_IOCTL)) {
				...
				if (buf_array[i].counter == 0) {
					if (vdec_dmabuf_secureRunning > 0) {
						vdec_dmabuf_secureRunning--;
					}
