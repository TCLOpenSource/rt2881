...
#else
	if ( get_platform_model() == PLATFORM_MODEL_4K || get_platform_model() == PLATFORM_MODEL_2K ) {
		...
		case VDEC_INBAND_BUF:
		{
			...
			for ( i = 0; i < RTKVDEC_MAX_IBBUF_NUM; i++ ) {
				if ( IBBuf[i].used == 0 ) {
					...
					phy_addr = pli_malloc(IBBuf[i].size, GFP_DCU1_LIMIT);
		...
		case VDEC_INBAND_BUF:
		{
			...
			for (i = 0; i < RTKVDEC_MAX_IBBUF_NUM; i++) {
				if (IBBuf[i].addr == phy_addr) {
					pli_free((unsigned long)IBBuf[i].addr);
					IBBuf[i].addr = 0 ;
					IBBuf[i].used = 0 ;
					IBBuf[i].pid = 0 ;
			...
			if ( 0 != (addr = ntohl(frm_mm_ptr->memFreeLuAddr[i])) ) {
				...
				list_for_each_entry_safe(frmbuf_info, tmp_frmbuf_info, &rtkvdec_frame_buf_head, frmbuf_list)
				{
					if ( frmbuf_info->phy_addr == (unsigned long)addr ) {
						if ( frmbuf_info->frmbuf_ptr )
							dvr_dmabuf_free(frmbuf_info->frmbuf_ptr);
						else // only 2K from CMA
							pli_free(frmbuf_info->phy_addr);
...
int rtkvdec_vbm_alloc_by_frame(int NumofUse)
{
	...
	unsigned long cmabuf_LuArray[FRAME_ARRAY_SIZE] = {0};
	...
	if ( type == VBM_REQ_TYPE_4K ) {
		...
		if ( IS_ERR_OR_NULL(dmabuf_LuPtr) ) {
			...
			for ( i = 0; i < cnt; i++ ) {
				cmabuf_LuArray[i] = pli_malloc(LuSize + ChSize, GFP_DCU2_FIRST);
				if ( cmabuf_LuArray[i] == INVALID_VAL ) {
					rtd_pr_vdec_err("[%s %d] allocate LuHdrPA[%d] fail\n", __func__, __LINE__, i);
					goto ERR_CASE_LUMA_FAIL;
				}
			}
			...
			for ( i = 0; i < cnt; i++ ) {
				...
				frmbuf_info->phy_addr     = cmabuf_LuArray[i];
				frmbuf_info->hdr_phy_addr = LuHdrPA[i];
				list_add(&frmbuf_info->frmbuf_list, &rtkvdec_frame_buf_head);
			...
			for ( i = 0; i < cnt; i++ ) {
				...
				frmbuf_info->frmbuf_ptr   = NULL;
				frmbuf_info->phy_addr     = cmabuf_LuArray[i] + LuSize;
				frmbuf_info->hdr_phy_addr = ChHdrPA[i];
				list_add(&frmbuf_info->frmbuf_list, &rtkvdec_frame_buf_head);
...
ERR_CASE_LUMA_FAIL :
	for ( i = 0; i < cnt; i++ ) {
		if ( cmabuf_LuArray[i] != 0 && cmabuf_LuArray[i] != INVALID_VAL ) { pli_free(cmabuf_LuArray[i]); cmabuf_LuArray[i] = 0; }
	}
