...
static uint32_t sVersion = 0;
static bool sDisableProtect = false;
...
static int validate_dmabuf_data(struct rtk_vdec_dmabuf_priv_data *privdata) {
	...
	for (i = 0; i < MAX_DMABUF_COUNT; ++i) {
		if (privdata == buf_array[i].priv) {
			break;
		}
	}
	if (i == MAX_DMABUF_COUNT) {
		rtd_pr_vdec_err(
			"[rtk_vdec_dmabuf:%d] cannot find correspond privdata\n",
			__LINE__);
		return -1;
	}
...
static long rtk_vdec_dmabuf_ioctl_pre_realease_buf(struct file *filp,
						   unsigned long arg)
{
	int32_t secureIndex = -1;
	long ret = -EFAULT;
	(void)filp;

	if (copy_from_user(&secureIndex, (const void __user *)arg, sizeof(int32_t)))
		goto err;
	...
	down(&DmabufLock);
	if (buf_array[secureIndex].priv != NULL) {
		rtd_pr_vdec_info(
			"[rtk_vdec_dmabuf:%d] prelease buf with id=%d\n",
			__LINE__, secureIndex);
		release_privdata(buf_array[secureIndex].priv);
		buf_array[secureIndex].priv = NULL;
		buf_array[secureIndex].size = 0;
		buf_array[secureIndex].counter = 0;
	}
	up(&DmabufLock);
...
static long rtk_vdec_dmabuf_ioctl_set_config(struct file *filp,
					     unsigned long arg)
{
	struct rtk_vdec_dmabuf_config data;
	long ret = -EFAULT;

	if (copy_from_user(&data, (const void __user *)arg, sizeof(data))) {
		rtd_pr_vdec_err(
			"[rtk_vdec_dmabuf:%d] copy from user failed",
			__LINE__);
		up(&DmabufLock);
		goto err;
	}
	...
	down(&DmabufLock);
	rtd_pr_vdec_info(
			"[rtk_vdec_dmabuf:%d] setting version=%d->%d\n",
			__LINE__, sVersion, data.version);
	sVersion = data.version;
	sDisableProtect = data.disableProtect;

#if 0
	if (ret != 0) {
		rtd_pr_vdec_err(
			"[rtk_vdec_dmabuf:%d] set config failed",
			__LINE__);
		up(&DmabufLock);
		goto err;
	}
			   ...
			   unsigned long arg)
{
	...
	case VDEC_IOC_SVP_DMABUF_PRE_RELEASE:
		ret = rtk_vdec_dmabuf_ioctl_pre_realease_buf(filp, arg);
		break;
	case VDEC_IOC_SVP_DMABUF_SET_CONFIG:
		ret = rtk_vdec_dmabuf_ioctl_set_config(filp, arg);
		break;
	default:
		break;
	}
						  ...
						  unsigned long heap_flags)
{
	...
	DEFINE_DMA_BUF_EXPORT_INFO(exp_info);
	struct dma_buf *dmabuf = NULL;
	int ret = -EFAULT, idx = -1;

	uint32_t total_size = 0, i = 0, index = 0;
	(void)rtk_vdec_dma_heap;
	if (sVersion == 0) {
		data.len = len * 2;
	} else {
		index = (len & 0XF000) >> 12;
		data.len = (len & 0XFFFF0000) * 2;
	}
	...
	rtd_pr_vdec_info(
			"[rtk_vdec_dmabuf:%d] version=%d, request id=%d,"
			"size=%d x 2 -> allocate size=%d\n",
			__LINE__, sVersion, index, (len & 0XFFFF0000), data.len);
	...
	for (i = 0; i < MAX_DMABUF_COUNT; i++) {
		...
		} else if (buf_array[i].priv == NULL && idx == -1) {
			idx = i;
		}
	}
	if (sVersion == 0) {
