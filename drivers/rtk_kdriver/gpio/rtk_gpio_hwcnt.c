...
{
     gpio_iowrite32(ISO_GPIO_GPDEB_HWCNT, ((0x8 | val) << ISO_GPIO_GPDEB_clk_SHIFT_HWCNT));
}
/**************************************************
...
int rtk_hwcnt_get_counter(void){
    ...
    if(gpio_count==0) {
        if(rtk_gpio_input(panel_error_detect_pin))
            gpio_count++;
    }
...
unsigned int tcon_v22_panel_flag = 0;

void rtk_hwcnt_set_panel_tvon_v22_enable(unsigned int enable)
{
    tcon_v22_panel_flag = enable;
}

unsigned int rtk_hwcnt_get_panel_flag(void)
{
    return tcon_v22_panel_flag;
}
