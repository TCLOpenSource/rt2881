...
#define GET_PIN_PINMUX_UART1VALUE(x)       ((unsigned char) (((x) >> 10) & 0xF))
#define GET_PIN_PINMUX_DDCVALUE(x)       ((unsigned char) (((x) >> 14) & 0xF))
#define GET_PIN_PINMUX_UART0VALUE(x)        ((unsigned char) (((x) >> 18) & 0xF))
#define GET_PIN_PINMUX_REG_ENDBIT(x)      ((unsigned char) (((x) >> 22) & 0x1F))
#define GET_PIN_PINMUX_REG_STARTBIT(x)    ((unsigned char) (((x) >> 27) & 0x1F))
#define GET_PIN_PINMUX_ADDR(x)            ((unsigned int)  (((x) >> 32) & 0xFFFFFFFF))
...
unsigned char pinmux_switch_val = 0;
static unsigned char spdif_enable = 1;
                            ...
                            struct device_attribute *attr, char *buf)
{
    int ret = -1;
    char *buf_info = NULL;
    unsigned long long pinmux_uart_ddc_scl = 0;

    buf_info = kmalloc(PAGE_SIZE, GFP_KERNEL);
    if(buf_info == NULL)
        return 0;
    buf_info[0] = 0;
    if(strncmp(attr->attr.name, "pinmux_ddc_uart_switch", 23) == 0)
    {
        if(pcb_mgr_get_enum_info_byname("UART_DDC_SCL_SWITCH", &pinmux_uart_ddc_scl) == 0){
            ret = snprintf(buf, PAGE_SIZE, "echo 0/1/2 > pinmux_ddc_uart_switch \n0: ddc 1: uart0 2:uart1 \ncurrent:%s \n",pinmux_switch_val?(pinmux_switch_val==1?"UART0":"UART1"):"DDC");
        }else{
            ret = snprintf(buf, PAGE_SIZE, "failed, boot not define the ddc_uart pcbname\n");
        }
    }else if(strncmp(attr->attr.name, "spdif_pinmux", 12) == 0)
    {
        if(pcb_mgr_get_enum_info_byname("SPDIF_CFG_PINMUX", &pinmux_uart_ddc_scl) == 0){
            ret = snprintf(buf, PAGE_SIZE, "echo 0/1/2 > spdif_pinmux \n0: GPIO 1: SPDIF_OUT  \ncurrent:%s \n",spdif_enable?"SPDIF_CFG":"GPIO");
        }else{
            ret = snprintf(buf, PAGE_SIZE, "failed, boot not define the SPDIF_CFG_PINMUX pcbname\n");
        }
    }
    ...
    else if (strncmp(attr->attr.name, "hwcnt_detect_mode", 17) == 0)
    {
        ret = snprintf(buf, PAGE_SIZE, "current:%d \n0:as H/L 100ms delay level count \n1:as H/L immediately count\n",rtk_hwcnt_get_panel_flag());
    }
#endif
...
static void rtk_rtd_part_outl(unsigned int reg_addr, unsigned int endBit, unsigned int startBit, unsigned int value)
{
                           ...
                           const char *buf, size_t count)
{
    ...
    int ret = 0;
    unsigned long long pinmux_uart_ddc_scl = 0;
    unsigned long long pinmux_uart_ddc_sda = 0;
    ...
    {
        sscanf(buf, "%d\n", &val);
        pinmux_switch_val = val;
        ret = pcb_mgr_get_enum_info_byname("UART_DDC_SDA_SWITCH", &pinmux_uart_ddc_sda);
        ret |= pcb_mgr_get_enum_info_byname("UART_DDC_SDA_SWITCH", &pinmux_uart_ddc_scl);
        if(ret == 0)
        {
              if(val == 1){
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_UART0VALUE(pinmux_uart_ddc_sda));
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_UART0VALUE(pinmux_uart_ddc_scl));
            }else if(val == 2){
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_UART1VALUE(pinmux_uart_ddc_sda));
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_UART1VALUE(pinmux_uart_ddc_scl));
            }else if(val == 0){
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_DDCVALUE(pinmux_uart_ddc_sda));
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_REG_STARTBIT(pinmux_uart_ddc_scl),
                                                   GET_PIN_PINMUX_DDCVALUE(pinmux_uart_ddc_scl));
            }
        }else{
            RTK_GPIO_WARNING("DDC_SWITCH_UART the func not support\n");
            return -1;
        }
    }else if(strncmp(attr->attr.name, "spdif_pinmux", 12) == 0){
        sscanf(buf, "%d\n", &val);
        spdif_enable = val;
        ret = pcb_mgr_get_enum_info_byname("SPDIF_CFG_PINMUX", &pinmux_uart_ddc_sda);
        if(ret == 0){
            if(val == 1){
                rtk_rtd_part_outl(GET_PIN_PINMUX_ADDR(pinmux_uart_ddc_sda),
                                                   GET_PIN_PINMUX_REG_ENDBIT(pinmux_uart_ddc_sda),
                ...
                RTK_GPIO_WARNING("SPDIF_CFG_PINMUX the func not support\n");
                return -1;
            }
        }
    ...
    else  if (strncmp(attr->attr.name, "type", 4) == 0)
    {
        ...
        else
        {
            ...
            ret = pcb_mgr_get_enum_info_byname(cmd, &pinmux_uart_ddc_scl);
            if(ret == 0)
            {
                ret = IO_Direct_Set(cmd,val);
                if(ret)
                    RTK_GPIO_WARNING("%s %d set failed",cmd,val);
            }else{
                RTK_GPIO_WARNING("GPIO Command is invalid\n");
                RTK_GPIO_WARNING("GPIO Command Format: echo <pcb_name> <value>\n");
            }
        }
    }
    else  if (strncmp(attr->attr.name, "gpio", 6) == 0)
    ...
    {
        sscanf(buf, "%d\n", &val);
        rtk_hwcnt_set_panel_tvon_v22_enable(val);
    }
    else  if (strncmp(attr->attr.name, "hwcnt_count", 11) == 0)
    {
        sscanf(buf, "%s\n", cmd);
...
DEVICE_ATTR(hwcnt_detect_mode, S_IWUSR | S_IWGRP | S_IRUGO, rtk_gpio_show_param, rtk_gpio_store_param);
...
DEVICE_ATTR(pinmux_ddc_uart_switch, S_IWUSR | S_IWGRP | S_IRUGO, rtk_gpio_show_param, rtk_gpio_store_param);
DEVICE_ATTR(spdif_pinmux, S_IWUSR | S_IWGRP | S_IRUGO, rtk_gpio_show_param, rtk_gpio_store_param);
...
int rtk_gpio_create_attr(struct device *device)
{
    ...
    device_create_file(device, &dev_attr_hwcnt_detect_mode);
    device_create_file(device, &dev_attr_pinmux_ddc_uart_switch);
    device_create_file(device, &dev_attr_spdif_pinmux);
...
void rtk_gpio_remove_attr(struct device *device)
{
    ...
    device_remove_file(device, &dev_attr_hwcnt_detect_mode);
    device_remove_file(device, &dev_attr_pinmux_ddc_uart_switch);
    device_remove_file(device, &dev_attr_spdif_pinmux);
