...
static RTK_GPIO_IRQ *rtk_gpio_irq = NULL;
...
int gpio_num = MIS_GPIO_CNT + ISO_GPIO_CNT + GPI_ISO_CNT + MIO_GPIO_CNT + ISO_GPIO_MIO_CNT;
int gpi_mis_cnt = 0;

static DEFINE_SPINLOCK(rtk_gpio_irq_lock);

#define STB2_SYNC_REG   (0xb801A680)
#define SCPU_SYNC_MAGIC (0x2379)
void STB2Ctrl_semaphore_lock(void)
{
    /* HW sema for MIO control*/
    UINT32 timeout = 1000;
    if((rtd_inl(0xb8060050) & 0x01) == 0) {
        while(timeout--) {
            if((rtd_inl(STB2_SYNC_REG) & 0x1) == 1) {
                rtd_outl(STB2_SYNC_REG, SCPU_SYNC_MAGIC);
                break;
            }
            msleep(1);
        }
...
void STB2Ctrl_semaphore_unlock(void)
{
    if((rtd_inl(0xb8060050) & 0x01) == 0) {
        rtd_outl(STB2_SYNC_REG, 0);
    }
}
...
int rtk_gpio_get_dir(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        value = ioread_reg_bit(ISO_GPIO_MIO_GPDIR_reg + offset,bit);
        STB2Ctrl_semaphore_unlock();
        return value;
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
...
int rtk_gpio_input(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        value = ioread_reg_bit(ISO_GPIO_MIO_GPDATI_reg + offset, bit);
        STB2Ctrl_semaphore_unlock();
        return value;
...
int rtk_gpio_output_get(RTK_GPIO_ID gid )
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        value = ioread_reg_bit(ISO_GPIO_MIO_GPDATO_reg + offset,bit);
        STB2Ctrl_semaphore_unlock();
        break;
...
{
    int i;
    int gpi_mis_reg_sets = 0;
    if(get_mach_type()==MACH_ARCH_RTK2885P){
        gpi_mis_reg_sets = (Merlin8_GPI_MIS_CNT+31)>>5;
    }
    else{
        gpi_mis_reg_sets = (Merlin8P_GPI_MIS_CNT+31)>>5;;
    }
...
int rtk_gpio_chk_irq_enable(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        value = ioread_reg_bit(ISO_GPIO_MIO_GPIE_reg + offset,bit);
        STB2Ctrl_semaphore_unlock();
        return value;
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
...
int rtk_gpio_set_dis_irq_enable(RTK_GPIO_ID gid, unsigned char On)
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        iowrite_reg_bit(ISO_GPIO_MIO_GPINT_SCPU_reg + offset,bit, 1);
        iowrite_reg_bit(ISO_GPIO_MIO_GPIE_DA_reg + offset,bit, On);
        STB2Ctrl_semaphore_unlock();
        return 0;
...
int rtk_gpio_chk_dis_irq_enable(RTK_GPIO_ID gid)
{
    ...
    case ISO_GPIO_MIO:
        STB2Ctrl_semaphore_lock();
        value = ioread_reg_bit(ISO_GPIO_MIO_GPIE_DA_reg + offset,bit);
        STB2Ctrl_semaphore_unlock();
        return value;
#ifdef CONFIG_RTK_KDRV_MIO
    case MIO_GPIO:
...
int __init rtk_gpio_init(void)
{
    ...
    if(get_mach_type()==MACH_ARCH_RTK2885P){
        gpi_mis_cnt = Merlin8_GPI_MIS_CNT;
    }
    else{
        gpi_mis_cnt = Merlin8P_GPI_MIS_CNT;
    }
    gpio_num = gpio_num + gpi_mis_cnt;
    rtk_gpio_irq = (RTK_GPIO_IRQ*)kmalloc(gpio_num*sizeof(RTK_GPIO_IRQ), GFP_KERNEL);
    if (rtk_gpio_irq == NULL) {
        RTK_GPIO_DBG("Failed to allocate memory for rtk_gpio_irq!\n");
        kfree(rtk_gpio_irq);
    }
    else{
