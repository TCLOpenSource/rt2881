...
static int set_dram_refresh_rate_2851A(int temp);
#elif defined(CONFIG_ARCH_RTK2875Q)
static int set_dram_refresh_rate_2875Q(int temp);
#elif defined(CONFIG_ARCH_RTK2819A)
static int set_dram_refresh_rate_2819A(int temp);
#elif defined(CONFIG_ARCH_RTK6748)
static int update_ddr_parameters(int temp);
#else
...
static bool theraml_suspend_enter = 0;
static void rtk_report_trigger(void)
{
	...
	if(rtk_monitor_debug < 2 && (theraml_suspend_enter == 0) )
...
#elif defined(CONFIG_ARCH_RTK2851A)
	set_dram_refresh_rate_2851A(temp);
#elif defined(CONFIG_ARCH_RTK2875Q)
	set_dram_refresh_rate_2875Q(temp);
#elif defined(CONFIG_ARCH_RTK2819A)
	set_dram_refresh_rate_2819A(temp);
#elif defined(CONFIG_ARCH_RTK6748)
	update_ddr_parameters(temp);
#else
...
#if !(defined(CONFIG_ARCH_RTK2851C) || defined(CONFIG_ARCH_RTK2851F) || defined(CONFIG_ARCH_RTK2885P) || defined(CONFIG_ARCH_RTK2851A) || defined(CONFIG_ARCH_RTK2875Q))
...
#define TEMP_TO_DDR(t)      (t - 15)
#define DDR_L1_DEFAULT            82
#define DDR_L2_DEFAULT            92

#define DPI_F_CODE_T_reg_MC1 0xB80C9024
#define DPI_N_CODE_T_reg_MC1 0xB80C9028
#define DPI_F_CODE_T_reg_MC3 0xB80CA024
#define DPI_N_CODE_T_reg_MC3 0xB80CA028

#define FREQ_XTAL_MHZ   27
#define VAL_CONSTANT_1950	1950
#define VAL_CONSTANT_1000	1000
	...
	unsigned int dpi_f_code_t;
	unsigned int dpi_n_code_t;
	unsigned int ddr_freq;
	static unsigned int cal_setting_MC1 = 0, cal_setting_MC3 = 0;
	int l1_setting_MC1, l2_setting_MC1, l3_setting_MC1;
	int l1_setting_MC3, l2_setting_MC3, l3_setting_MC3;
	dc_mc_mc_dpar_ref_0_RBUS cur_val_MC;
	dc_mc3_mc_dpar_ref_0_RBUS cur_val_MC3;

	//for MC1 init
	if((cal_setting_MC1 == 0) && (is_enable_dynamic_refresh_set1() == 1))
	{
		dpi_f_code_t = rtd_inl(DPI_F_CODE_T_reg_MC1);
		dpi_f_code_t = (dpi_f_code_t>>16) & 0x7ff;	//dpi_f_code_t = 0x180c9024[26:16]
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg_MC1);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180c9028[15:8]
		ddr_freq = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11) + 1;
		cal_setting_MC1 = ((VAL_CONSTANT_1950 * ddr_freq)/VAL_CONSTANT_1000)+1;
		rtd_pr_thermal_info("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, ddr_freq:%d, cal_setting_MC1:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,ddr_freq, cal_setting_MC1);
	}
	l1_setting_MC1 = (cal_setting_MC1 << 2);
	l2_setting_MC1 = (cal_setting_MC1 << 1);
	l3_setting_MC1 = cal_setting_MC1;

	//for MC3 init
	if((cal_setting_MC3 == 0) && (is_enable_dynamic_refresh_set2() == 1))
	{
		dpi_f_code_t = rtd_inl(DPI_F_CODE_T_reg_MC3);
		dpi_f_code_t = (dpi_f_code_t>>16) & 0x7ff;	//dpi_f_code_t = 0x180cA024[26:16]
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg_MC3);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180cA028[15:8]
		ddr_freq = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11) + 1;
		cal_setting_MC3 = ((VAL_CONSTANT_1950 * ddr_freq)/VAL_CONSTANT_1000)+1;
		rtd_pr_thermal_info("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, ddr_freq:%d, cal_setting_MC3:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,ddr_freq, cal_setting_MC3);
	}
	l1_setting_MC3 = (cal_setting_MC3 << 2);
	l2_setting_MC3 = (cal_setting_MC3 << 1);
	l3_setting_MC3 = cal_setting_MC3;
	...
	if(is_enable_dynamic_refresh_set1() == 1)
	{
		tj = TEMP_TO_KGD(temp);
		change = false;
		l1_setting = l1_setting_MC1;
		l2_setting = l2_setting_MC1;
		l3_setting = l3_setting_MC1;
		cur_val_MC.regValue = rtd_inl(DC_MC_MC_DPAR_REF_0_reg);

	    if (tj <= KGD_L1_UP) {
	        if (cur_val_MC.t_refi != l1_setting) {
			cur_val_MC.t_refi = l1_setting;
			rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);
			KGD_L1_UP = KGD_L1_DEFAULT;
			KGD_L2_UP = KGD_L2_DEFAULT;
			change = true;
	        }
	    } else if (tj <= KGD_L2_UP) {
	        if (cur_val_MC.t_refi != l2_setting) {
			cur_val_MC.t_refi = l2_setting;
			rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);
	        ...
	        if (cur_val_MC.t_refi != l3_setting) {
			cur_val_MC.t_refi = l3_setting;
			rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);
			KGD_L1_UP = KGD_L1_DEFAULT;
	    ...
	    if (change)
	        rtd_pr_thermal_info("%s: [tmu] temp:%d, MC1:%d. 0x%x=0x%08x\n", __func__,
	...
	if(is_enable_dynamic_refresh_set2() == 1)
	{
		tj = TEMP_TO_DDR(temp);
		change = false;
		l1_setting = l1_setting_MC3;
		l2_setting = l2_setting_MC3;
		l3_setting = l3_setting_MC3;
		cur_val_MC3.regValue = rtd_inl(DC_MC3_MC_DPAR_REF_0_reg);

	    if (tj <= DDR_L1_UP) {
	        if (cur_val_MC3.t_refi != l1_setting) {
			cur_val_MC3.t_refi = l1_setting;
			rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC3.regValue);
			DDR_L1_UP = DDR_L1_DEFAULT;
			DDR_L2_UP = DDR_L2_DEFAULT;
			change = true;
	        }
	    } else if (tj <= DDR_L2_UP) {
	        if (cur_val_MC3.t_refi != l2_setting) {
			cur_val_MC3.t_refi = l2_setting;
			rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC3.regValue);
	        ...
	        if (cur_val_MC3.t_refi != l3_setting) {
			cur_val_MC3.t_refi = l3_setting;
			rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC3.regValue);
			DDR_L1_UP = DDR_L1_DEFAULT;
	    ...
	    if (change)
	        rtd_pr_thermal_info("%s: [tmu] temp:%d, MC3:%d. 0x%x=0x%08x\n", __func__,
...
#define L1_REF_SETTING_DDR4_1    0x282890A5
#define L2_REF_SETTING_DDR4_1    0x281440A5
#define L3_REF_SETTING_DDR4_1    0x280A20A5
#define L4_REF_SETTING_DDR4_1    0x280A20A5

#define L1_REF_SETTING_DDR4_2    0x282890E1
#define L2_REF_SETTING_DDR4_2    0x281440E1
#define L3_REF_SETTING_DDR4_2    0x280A20E1
#define L4_REF_SETTING_DDR4_2    0x280A20E1
...
{
	unsigned int dc1_mc_dummy_fw3_val;
	dc1_mc_dummy_fw3_val = rtd_inl(DC1_MC_DUMMY_FW3_reg);
	if (dc1_mc_dummy_fw3_val & 0x1)
		return 1;
	else
		return 0;
}
...
static int set_dram_refresh_rate_2851A(int temp)
{
    ...
    if (is_DDR4()) {
        if (rtd_inl(DC1_MC_DUMMY_FW3_reg) & 0x2) {
            l1_setting = L1_REF_SETTING_DDR4_2;
            l2_setting = L2_REF_SETTING_DDR4_2;
            l3_setting = L3_REF_SETTING_DDR4_2;
            l4_setting = L4_REF_SETTING_DDR4_2;
        } else {
            l1_setting = L1_REF_SETTING_DDR4_1;
            l2_setting = L2_REF_SETTING_DDR4_1;
            l3_setting = L3_REF_SETTING_DDR4_1;
            l4_setting = L4_REF_SETTING_DDR4_1;
        }
    } else {
...
#include <rbus/dc_mc_reg.h>
#include <rbus/dc_mc2_reg.h>
//dpi_f_code_t = 0x180c9024[26:16]
//dpi_n_code_t = 0x180c9028[15:8]
#define DPI_F_CODE_T_reg_MC12 0xB80C9024
#define DPI_N_CODE_T_reg_MC12 0xB80C9028
#define DPI_F_CODE_T_reg_MC34 0xB80CA024
#define DPI_N_CODE_T_reg_MC34 0xB80CA028

static bool is_enable_dynamic_refresh_MC12(void)
{
	unsigned int val_MC_DMY_FW15;
	val_MC_DMY_FW15 = rtd_inl(DC_MC_MC_DMY_FW15_reg);
	//check 180C278C[0]
	if (val_MC_DMY_FW15 & 0x1)
		return 1;
	else
		return 0;
}
static bool is_enable_dynamic_refresh_MC34(void)
{
	unsigned int val_MC_DMY_FW15;
	val_MC_DMY_FW15 = rtd_inl(DC_MC_MC_DMY_FW15_reg);
	//check 180C278C[1]
	val_MC_DMY_FW15 = val_MC_DMY_FW15>>1;
	if (val_MC_DMY_FW15 & 0x1)
		return 1;
	else
		return 0;
}
static int is_enable_MC2(void)
{
	dc_mc_mc_sys_2_RBUS dc_mc_mc_sys_2_val;
	dc_mc_mc_sys_2_val.regValue = rtd_inl(DC_MC_MC_SYS_2_reg);
	return dc_mc_mc_sys_2_val.mcx1_2cke_en?0:1;
}
static int is_enable_MC4(void)
{
	dc_mc3_mc_sys_2_RBUS dc_mc3_mc_sys_2_val;
	dc_mc3_mc_sys_2_val.regValue = rtd_inl(DC_MC3_MC_SYS_2_reg);
	return dc_mc3_mc_sys_2_val.mcx1_2cke_en?0:1;
}
static int set_dram_refresh_rate_2819A(int temp)
{
	static int KGD_L1_UP = KGD_L1_DEFAULT;
	static int KGD_L2_UP = KGD_L2_DEFAULT;
	bool is_enable_mc2, is_enable_mc4;
	int kgd_tj = TEMP_TO_KGD(temp);
	int l1_setting_MC12, l2_setting_MC12, l3_setting_MC12;
	int l1_setting_MC34, l2_setting_MC34, l3_setting_MC34;
	bool change = false;
	unsigned int dpi_f_code_t;
	unsigned int dpi_n_code_t;
	unsigned int ddr_freq;
	static unsigned int cal_setting_MC12 = 0, cal_setting_MC34 = 0;

	dc_mc_mc_dpar_ref_0_RBUS cur_val_MC;
	dc_mc2_mc_dpar_ref_0_RBUS cur_val_MC_2;
	dc_mc3_mc_dpar_ref_0_RBUS cur_val_MC_3;
	dc_mc4_mc_dpar_ref_0_RBUS cur_val_MC_4;

	if((is_enable_dynamic_refresh_MC12() == 0) && (is_enable_dynamic_refresh_MC34() == 0))
		return 0;

	is_enable_mc2 = is_enable_MC2();
	is_enable_mc4 = is_enable_MC4();

	//for MC1/2 init
	if((cal_setting_MC12 == 0) && (is_enable_dynamic_refresh_MC12() == 1))
	{
		dpi_f_code_t = rtd_inl(DPI_F_CODE_T_reg_MC12);
		dpi_f_code_t = (dpi_f_code_t>>16) & 0x7ff;	//dpi_f_code_t = 0x180c9024[26:16]
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg_MC12);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180c9028[15:8]
		ddr_freq = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11) + 1;
		cal_setting_MC12 = ((VAL_CONSTANT_1950 * ddr_freq)/VAL_CONSTANT_1000)+1;
		rtd_pr_thermal_info("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, ddr_freq:%d, cal_setting_MC12:%d, enable_mc2:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,ddr_freq, cal_setting_MC12, is_enable_mc2);
	}
	l1_setting_MC12 = (cal_setting_MC12 << 2);
	l2_setting_MC12 = (cal_setting_MC12 << 1);
	l3_setting_MC12 = cal_setting_MC12;

	//for MC3/4 init
	if((cal_setting_MC34 == 0) && (is_enable_dynamic_refresh_MC34() == 1))
	{
		dpi_f_code_t = rtd_inl(DPI_F_CODE_T_reg_MC34);
		dpi_f_code_t = (dpi_f_code_t>>16) & 0x7ff;	//dpi_f_code_t = 0x180cA024[26:16]
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg_MC34);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180cA028[15:8]
		ddr_freq = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11) + 1;
		cal_setting_MC34 = ((VAL_CONSTANT_1950 * ddr_freq)/VAL_CONSTANT_1000)+1;
		rtd_pr_thermal_info("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, ddr_freq:%d, cal_setting_MC34:%d, enable_mc4:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,ddr_freq, cal_setting_MC34, is_enable_mc4);
	}
	l1_setting_MC34 = (cal_setting_MC34 << 2);
	l2_setting_MC34 = (cal_setting_MC34 << 1);
	l3_setting_MC34 = cal_setting_MC34;

	//MC1/2 setting
	if(is_enable_dynamic_refresh_MC12() == 1)
	{
		cur_val_MC.regValue = rtd_inl(DC_MC_MC_DPAR_REF_0_reg);
		if(is_enable_mc2 == 1)
			cur_val_MC_2.regValue = rtd_inl(DC_MC2_MC_DPAR_REF_0_reg);

		if (kgd_tj <= KGD_L1_UP)
		{
			if (cur_val_MC.t_refi != l1_setting_MC12)
			{
				cur_val_MC.t_refi = l1_setting_MC12;
				rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);

				if(is_enable_mc2)
				{
					cur_val_MC_2.t_refi = l1_setting_MC12;
					rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
				}
		...
		{
			if (cur_val_MC.t_refi != l2_setting_MC12)
			{
				cur_val_MC.t_refi = l2_setting_MC12;
				rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);

				if(is_enable_mc2)
				{
					cur_val_MC_2.t_refi = l2_setting_MC12;
					rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
				}
		...
		{
			if (cur_val_MC.t_refi != l3_setting_MC12)
			{
				cur_val_MC.t_refi = l3_setting_MC12;
				rtd_outl(DC_MC_MC_DPAR_REF_0_reg, cur_val_MC.regValue);

				if(is_enable_mc2)
				{
					cur_val_MC_2.t_refi = l3_setting_MC12;
					rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
				}
	...
	//MC3/4 setting
	if(is_enable_dynamic_refresh_MC34() == 1)
	{
		cur_val_MC_3.regValue = rtd_inl(DC_MC3_MC_DPAR_REF_0_reg);
		if(is_enable_mc4 == 1)
			cur_val_MC_4.regValue = rtd_inl(DC_MC4_MC_DPAR_REF_0_reg);

		if (kgd_tj <= KGD_L1_UP)
		{
			if (cur_val_MC_3.t_refi != l1_setting_MC34)
			{
				cur_val_MC_3.t_refi = l1_setting_MC34;
				rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC_3.regValue);

				if(is_enable_mc4)
				{
					cur_val_MC_4.t_refi = l1_setting_MC34;
					rtd_outl(DC_MC4_MC_DPAR_REF_0_reg, cur_val_MC_4.regValue);
				}
		...
		{
			if (cur_val_MC_3.t_refi != l2_setting_MC34)
			{
				cur_val_MC_3.t_refi = l2_setting_MC34;
				rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC_3.regValue);

				if(is_enable_mc4)
				{
					cur_val_MC_4.t_refi = l2_setting_MC34;
					rtd_outl(DC_MC4_MC_DPAR_REF_0_reg, cur_val_MC_4.regValue);
				}
		...
		{
			if (cur_val_MC_3.t_refi != l3_setting_MC34)
			{
				cur_val_MC_3.t_refi = l3_setting_MC34;
				rtd_outl(DC_MC3_MC_DPAR_REF_0_reg, cur_val_MC_3.regValue);

				if(is_enable_mc4)
				{
					cur_val_MC_4.t_refi = l3_setting_MC34;
					rtd_outl(DC_MC4_MC_DPAR_REF_0_reg, cur_val_MC_4.regValue);
				}
	...
	if (change)
	{
		rtd_pr_thermal_info("%s:(%d) [tmu] temp:%d, kgd:%d. 0x%x=0x%08x\n", __func__, is_enable_mc2,
				temp, kgd_tj, DC_MC_MC_DPAR_REF_0, rtd_inl(DC_MC_MC_DPAR_REF_0_reg));
		rtd_pr_thermal_info("%s:(%d) [tmu] temp:%d, kgd:%d. 0x%x=0x%08x\n", __func__, is_enable_mc4,
				temp, kgd_tj, DC_MC3_MC_DPAR_REF_0, rtd_inl(DC_MC3_MC_DPAR_REF_0_reg));
	}
...
//dpi_f_code_t = 0x180c9024[26:16]
//dpi_n_code_t = 0x180c9028[15:8]
#define DPI_F_CODE_T_reg 0xB80C9024
#define DPI_N_CODE_T_reg 0xB80C9028
...
static int is_enable_MC2(void)
{
	dc_mc_mc_sys_2_RBUS dc_mc_mc_sys_2_val;
	dc_mc_mc_sys_2_val.regValue = rtd_inl(DC_MC_MC_SYS_2_reg);
	return dc_mc_mc_sys_2_val.mcx2_en?1:0;
}
static int set_dram_refresh_rate_2875Q(int temp)
{
	static int KGD_L1_UP = KGD_L1_DEFAULT;
	static int KGD_L2_UP = KGD_L2_DEFAULT;
	static bool is_enable_mc2;
	int kgd_tj = TEMP_TO_KGD(temp);
	...
	unsigned int dpi_n_code_t;
	unsigned int ddr_freq;
	static unsigned int cal_setting = 0;
	dc_mc_mc_dpar_ref_0_RBUS cur_val_MC;
	dc_mc2_mc_dpar_ref_0_RBUS cur_val_MC_2;
	...
	if(cal_setting == 0)
	{
		dpi_f_code_t = rtd_inl(DPI_F_CODE_T_reg);
		dpi_f_code_t = (dpi_f_code_t>>16) & 0x7ff;	//dpi_f_code_t = 0x180c9024[26:16]
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180c9028[15:8]
		ddr_freq = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11) + 1;
		cal_setting = (VAL_CONSTANT_1950 * ddr_freq)/VAL_CONSTANT_1000;
		rtd_pr_thermal_info("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, ddr_freq:%d, cal_setting:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,ddr_freq, cal_setting);
	}

	is_enable_mc2 = is_enable_MC2();
	l1_setting = (cal_setting << 2);
	l2_setting = (cal_setting << 1);
	l3_setting = cal_setting;
	...
	if (kgd_tj <= KGD_L1_UP)
	{
		if (cur_val_MC.t_refi != l1_setting)
		{
			...
			if(is_enable_mc2)
			{
				cur_val_MC_2.t_refi = l1_setting;
				rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
			}
	...
	else if (kgd_tj <= KGD_L2_UP)
	{
		if (cur_val_MC.t_refi != l2_setting)
		{
			...
			if(is_enable_mc2)
			{
				cur_val_MC_2.t_refi = l2_setting;
				rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
			}
	...
	else
	{
		if (cur_val_MC.t_refi != l3_setting)
		{
			...
			if(is_enable_mc2)
			{
				cur_val_MC_2.t_refi = l3_setting;
				rtd_outl(DC_MC2_MC_DPAR_REF_0_reg, cur_val_MC_2.regValue);
			}
...
#elif defined(CONFIG_ARCH_RTK6748)
#define LEVEL_1_DEFAULT            82
#define LEVEL_2_DEFAULT            92
#define CALCULATE_INDEX(v)         (v - 12)
#define DEBOUNCE_CONSTANT          3
#define FREQ_XTAL_MHZ              27
#define VAL_CONSTANT_1950          1950
#define VAL_CONSTANT_1000          1000
...
static bool is_enable_feature(void)
{
	unsigned int val_MC_DMY_FW15;
	val_MC_DMY_FW15 = rtd_inl(DC_MC_MC_DMY_FW15_reg);
	if (val_MC_DMY_FW15 & 0x1)
...
static int update_ddr_parameters(int temp)
{
	static int LEVEL_1_UP = LEVEL_1_DEFAULT;
	static int LEVEL_2_UP = LEVEL_2_DEFAULT;
	static bool is_enable_mc2;
	int kgd_tj = CALCULATE_INDEX(temp);
	int l1_setting, l2_setting, l3_setting;
	bool change = false;
	unsigned int dpi_f_code_t;
	unsigned int dpi_n_code_t;
	...
	{
		//for CCK debug can update ddr parameters
		cal_setting = 0;
		return 0;
	}
	...
	if(cal_setting == 0)
	{
		...
		dpi_n_code_t = rtd_inl(DPI_N_CODE_T_reg);
		dpi_n_code_t = (dpi_n_code_t>>8) & 0xff;	//dpi_n_code_t = 0x180c9028[15:8]
		freq_value = (((FREQ_XTAL_MHZ * ((dpi_n_code_t + 3) * 2048 + dpi_f_code_t))) >> 11);
		cal_setting = (VAL_CONSTANT_1950 * freq_value)/VAL_CONSTANT_1000;
		rtd_pr_thermal_err("%s: [tmu] dpi_f_code_t:%d, dpi_n_code_t:%d, freq_value:%d, cal_setting:%d\n",
							__func__,dpi_f_code_t,dpi_n_code_t,freq_value,cal_setting);
	}
	...
	if (kgd_tj <= LEVEL_1_UP)
	{
		if (cur_val_MC.t_refi != l1_setting)
		{
			...
			LEVEL_1_UP = LEVEL_1_DEFAULT;
			LEVEL_2_UP = LEVEL_2_DEFAULT;
			change = true;
		}

	}
	else if (kgd_tj <= LEVEL_2_UP)
	{
		if (cur_val_MC.t_refi != l2_setting)
		{
			cur_val_MC.t_refi = l2_setting;
			...
			LEVEL_1_UP = LEVEL_1_DEFAULT - DEBOUNCE_CONSTANT;
			LEVEL_2_UP = LEVEL_2_DEFAULT;
			change = true;
		}
	}
	else
	{
		if (cur_val_MC.t_refi != l3_setting)
		{
			...
			LEVEL_1_UP = LEVEL_1_DEFAULT;
			LEVEL_2_UP = LEVEL_2_DEFAULT - DEBOUNCE_CONSTANT;
			change = true;
		}
	}

	if (change)
		rtd_pr_thermal_err("%s:(%d) [tmu] temp:%d, kgd:%d. 0x%x=0x%08x\n", __func__, is_enable_mc2,
				temp, kgd_tj, DC_MC_MC_DPAR_REF_0, rtd_inl(DC_MC_MC_DPAR_REF_0_reg));
...
#if !(defined(CONFIG_ARCH_RTK2851C) || defined(CONFIG_ARCH_RTK2851F) || defined(CONFIG_ARCH_RTK2885P) || defined(CONFIG_ARCH_RTK2875Q))
...
static int rtk_thermal_pm_notifier_event(struct notifier_block *this,
                           unsigned long event, void *ptr)
{
        switch (event) {
        case PM_SUSPEND_PREPARE:
                theraml_suspend_enter = 1;
                break;
        case PM_POST_SUSPEND:
                theraml_suspend_enter = 0;
                break;
        default:
                break;
        }
...
static struct notifier_block rtk_thermal_pm_notifier = {
        .notifier_call = rtk_thermal_pm_notifier_event,
};
...
static int rtk_tmu_probe(struct platform_device *pdev)
{
	...
	rtd_pr_thermal_info("trip point(%d/%d/%d/%d)\n", value[0],value[1],value[2],value[3]);
...
#endif
	register_pm_notifier(&rtk_thermal_pm_notifier);
    return 0;
err_clk:
...
static inline void dccimvac(unsigned long *addr)
{
    __asm__ __volatile__ ("DC CIVAC, %0" : : "r" (addr));
    // __asm__ __volatile__ ("mcr p15, 0, %0, c7, c14, 1" : : "r" (addr));
}

//static unsigned long* __attribute__((optimize("-O3"),aligned(256))) dvlbench_asm(register unsigned long *addr, register unsigned int max_cnt)
static unsigned long* noinline dvlbench_asm(register unsigned long *addr, register unsigned int max_cnt)
{
    while(max_cnt>0){
...
#define DVL_TEST_SIZE 		(1024*1024)
#define DVL_STRIDE_SIZE		(16*1024)
#define DVL_LINE_SIZE		(64)
#define DVL_MAX_STRIDE_CNT      (DVL_TEST_SIZE/DVL_STRIDE_SIZE)
#define DVL_LINE_PER_STRIDE	(DVL_STRIDE_SIZE/DVL_LINE_SIZE)
#define DVL_MAX_ITERATION	(DVL_TEST_SIZE/DVL_LINE_SIZE)
#define DVL_PRERUN_TAG		(0xffff)

static unsigned long* refill_testzone(unsigned int nSeed, unsigned long**  pMAP, unsigned int nMaxCnt){
    unsigned int pos;
    unsigned int bias_idx[DVL_MAX_STRIDE_CNT];
    unsigned int i,x,y;
    unsigned int addr_ptr;
    unsigned int idx;
    unsigned long *pSA, *pNSA, *pRet;
    for(i=0;i<DVL_MAX_STRIDE_CNT; i++){
	bias_idx[i] = ((i+1)*(i+7)+nSeed) % DVL_LINE_PER_STRIDE;
    }
    y=0;
    x=(bias_idx[y] + DVL_LINE_PER_STRIDE - 1) % DVL_LINE_PER_STRIDE; // Last index
    pos = y * DVL_LINE_PER_STRIDE + x;
    addr_ptr = pos * DVL_LINE_SIZE + ((DVL_TEST_SIZE/DVL_LINE_SIZE-1)%4) * 0x10;
    idx = addr_ptr / sizeof(unsigned long*);
    pSA = (unsigned long*) (pMAP + idx);
    pRet= pSA;
    for(i=0;i<nMaxCnt;i++){
	y=(i+1) % DVL_MAX_STRIDE_CNT;
	x = bias_idx[y];

	pos = y * DVL_LINE_PER_STRIDE + x;
	addr_ptr = pos * DVL_LINE_SIZE + (i%4) * 0x10;
	idx = addr_ptr / sizeof(unsigned long*);
	pNSA = (unsigned long*) (pMAP+idx);
	*pSA = (unsigned long) pNSA;
	dccimvac(pSA);
	pSA = pNSA;
	bias_idx[y] = (x + 1) % DVL_LINE_PER_STRIDE;
    }
    return pRet;
}

static long long dvlbench_wrapper(unsigned int tag, unsigned long *pSA, unsigned int iteration)
{
    unsigned long long start, end;
    unsigned long long delay;
    unsigned long* loop_ptr;
    unsigned long* pStart;

    rtd_pr_thermal_crit("dvlbench [%d] dvlbench_wrapper refill\n", tag);
    pStart = refill_testzone(tag, (unsigned long**)pSA, DVL_MAX_ITERATION);
    rtd_pr_thermal_crit("dvlbench [%d] dvlbench_wrapper Begin @ %lx\n", tag, (unsigned long)pStart);
    // prefetch $I
    local_irq_disable();
    loop_ptr = (unsigned long*) (&loop_ptr);
    dvlbench_asm((unsigned long*) &loop_ptr, 0x10);
    DSB();
    start = ktime_get_ns();
    DSB();
    pStart=dvlbench_asm(pStart, iteration);
    DSB();
    end = ktime_get_ns();
    local_irq_enable();
    delay = end - start;
    rtd_pr_thermal_crit("dvlbench [%d] dvlbench_wrapper End @ %lx\n", tag, (unsigned long)pStart);
    if(tag != DVL_PRERUN_TAG) {
		rtd_pr_thermal_crit("dvlbench [%d] start time  %lld [ns]\n", tag, start);
		rtd_pr_thermal_crit("dvlbench [%d] end time  %lld [ns]\n", tag, end);
		rtd_pr_thermal_crit("dvlbench [%d] %d round : latency %llu [ns]\n", tag, iteration, delay / iteration);
    }
    return delay;
}

static int do_dvlbench_function(void)
{
    unsigned long k;
    unsigned long long total_delay = 0;
    unsigned long **pMAP = (unsigned long **) kmalloc(DVL_TEST_SIZE, GFP_KERNEL);

    if (!pMAP) {
		rtd_pr_thermal_crit("%s : Error - failed to malloc memory\n", __func__);
	return 0;
    }

	total_delay = 0;
	rtd_pr_thermal_crit("dvlbench Begin : %s Test\n", __func__);
	dvlbench_wrapper(DVL_PRERUN_TAG, (unsigned long *) (pMAP), DVL_MAX_ITERATION);
	for(k=0; k < 0x10; k++) {
		total_delay = total_delay + dvlbench_wrapper(k, (unsigned long *) (pMAP), DVL_MAX_ITERATION);
	}
	rtd_pr_thermal_crit("dvlbench DV Total: %llu [ns], AVG latency %llu [ns]\n", total_delay,  total_delay/(DVL_MAX_ITERATION*0x10));
	rtd_pr_thermal_crit("dvlbench Finish : %s Test\n", __func__);

    return 0;
}

#define COPY_SIZE (25*1024*1024)
int TEST_Memcpy(unsigned long* pSrc, unsigned long* pDest, int nTestCount)
{
	int nTest;
	for (nTest = 0; nTest < 10; nTest++)
		memcpy((void*)pDest, (void*)pSrc, nTestCount*sizeof(unsigned int));
	return nTest;
}
void do_checkmem(void)
{
	int nTestCount = 10;
	int nLoop  = 0;
	unsigned long *pBuf, *pAlignedBuf, *pSrc, *pDest;
	unsigned long	nAllocBytes;
	ktime_t start_time, count_time;
	int us_time;

	nAllocBytes = COPY_SIZE*2 + PAGE_SIZE;
	pBuf = (unsigned long *) dvr_malloc_specific(nAllocBytes, GFP_DCU2);

	if(pBuf != 0)
		rtd_pr_thermal_crit(" Allocated %d bytes at 0x%llx\n", nAllocBytes, (unsigned long) pBuf);
	else
	{
		rtd_pr_thermal_crit("%s: dvr_malloc GFP_DCU2 failed\n", __func__);
		return;
	}

	if ((unsigned long) pBuf % PAGE_SIZE)
	{
		pAlignedBuf = (unsigned long *)(((unsigned long) pBuf + PAGE_SIZE) & PAGE_MASK);
		rtd_pr_thermal_crit(" Aligned at 0x%llx\n", (unsigned long) pAlignedBuf);
	}
	else
	{
		pAlignedBuf = pBuf;
	...
	pSrc = (unsigned long*) pAlignedBuf;
	pDest = pSrc + COPY_SIZE/sizeof(unsigned long);

	rtd_pr_thermal_crit("\n kdriver Memory read/write test through memcpy() from 0x%llx to 0x%llx\n", (unsigned long)pSrc, (unsigned long)pDest);
	while (nLoop < 5)
	{
		nLoop++;
		start_time = ktime_get();
		local_irq_disable();
		nTestCount = TEST_Memcpy(pSrc, pDest,  COPY_SIZE/ sizeof(unsigned int));
		count_time = ktime_sub(ktime_get(), start_time);
		local_irq_enable();
		us_time = ktime_to_us(count_time);
		rtd_pr_thermal_crit("Read/Write %3d: %d times, %8d bytes, %4d usec => %6d KiB/sec\n",
				nLoop, nTestCount, COPY_SIZE, us_time,  (((COPY_SIZE * nTestCount) / 1024) * 1000) / (us_time/1000));
	}
...
{
	do_checkmem();
	do_dvlbench_function();
}
#endif  //RTK_THERMAL_BENCHMARK
