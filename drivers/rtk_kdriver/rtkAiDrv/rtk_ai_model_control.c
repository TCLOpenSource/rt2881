...
static struct workqueue_struct *mc_wq;
...
#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
	result = nn_ta_npu_usage_get();
#else
	{
	//check counter and store
	unsigned long total=IoReg_Read32(AI_TOTAL_CYCLE_REG);
	unsigned long idle=IoReg_Read32(AI_IDLE_CYCLE_REG);
...
static void mc_work_func(struct work_struct *work)
{
	mc_dumpNpuLoad();
}
static DECLARE_WORK(mc_work, mc_work_func);

void mc_checkNpuLoadTimer(struct timer_list *t){

	queue_work(mc_wq, &mc_work);
...
BOOL rtk_ai_mc_enableNpuLoadCheck(int enable){
	...
	if(isNpuLoadCheck!=enable){
		if(enable){
			...
			if (mc_wq == 0)
				mc_wq = create_singlethread_workqueue("ai-mc");

			ai_load_timer.expires = jiffies + AI_LOAD_TIMER_INTERVAL;
			add_timer(&ai_load_timer);

		#if IS_ENABLED(CONFIG_RTK_AI_OPTEE)
			nn_ta_npu_usage_est_start();
		#else
			//reset counter
