...
#if IS_ENABLED(CONFIG_RTK_KDRV_KILLER)
...
static int ai_refCnt[AI_ENABLE_STATUS_MAX];

#if IS_ENABLED(CONFIG_RTK_KDRV_KILLER)
extern void enable_butcher(enum BUTCHER_NAME name);
extern void disable_butcher(enum BUTCHER_NAME name);
#endif
...
int rtk_ai_set_enable_status(AI_ENABLE_STATUS *status)
{
	...
	switch(status->type){
...
#if IS_ENABLED(CONFIG_RTK_KDRV_KILLER)
			AI_INFO("%s:set butcher (%d)\n",__FUNCTION__,status->enable);
			if(status->enable)
				enable_butcher(KARAOKE);
			else
				disable_butcher(KARAOKE);
			AI_INFO("%s:set butcher done\n",__FUNCTION__);
#endif
		...
		case AI_ENABLE_STATUS_CAMERA:
			...
			break;
		case AI_ENABLE_STATUS_AUDIO_DENOISE:
			bitMask=BIT_AI_AUDIO_DENOISE;
			break;
		default:
			return -EINVAL;
	}
	...
	if(status->enable){
		...
		//upgrade NN BW limit as 4GB/s
		if(status->type==AI_ENABLE_STATUS_CAMERA)
...
int __init rtk_ai_dev_init(void)
{
    ...
    AI_INFO( "[%s] sw_func_nnsr =%d \n",__func__,nnsr_disable);
    ...
    if(rtk_is_NNIP_function_disable()){
        AI_ERROR( "[%s] not support at this chip\n",__func__);
        goto FAILED_REGISTER_PLATFORM_DRIVER;
    }
#endif
    ...
    if(register_ai_cdev() != 0)
		...
		memset((void *)ai_refCnt,0,sizeof(int)*(AI_ENABLE_STATUS_MAX));
