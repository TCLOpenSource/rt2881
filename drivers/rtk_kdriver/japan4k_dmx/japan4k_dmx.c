...
int japan4k_dmx_get_CIDMappingInfo(unsigned char ch, unsigned int *pcnt, JP4K_DELIVERY_MAPPING_INFO *pMapInfo)
{
    ...
    for(i = 0; i < CID_IP_MAPPING_MAX_NUM; i++){
        if(TLV_INVALID_CID_VALUE != g_jp4k_cid_ip_mappingInfo[ch][i].cid){
            if(copy_to_user((void __user *)to_user_ptr(pMapInfo), (const void __user *)&(g_jp4k_cid_ip_mappingInfo[ch][i]), sizeof(JP4K_DELIVERY_MAPPING_INFO))) {
...
EXPORT_SYMBOL(japan4k_dmx_flush);
 ...
 * Retn  : 0 : successed, oethers failed
 *======================================================*/
void japan4k_dmx_get_NTP(unsigned char tp_id, JP4K_DELIVERY_NTP_INFO *p_ntp_info)
{
    if(pTp_drv->tp_japan[tp_id].dmx != NULL){
        p_ntp_info->ntp = pTp_drv->tp_japan[tp_id].dmx->tlv_NTP;
        p_ntp_info->ntp_leap_indicator = pTp_drv->tp_japan[tp_id].dmx->ntp_leap_indicator;
    }
}

int japan4k_dmx_get_HW_ntp(unsigned int tp_id, unsigned int *ntp_high, unsigned int *ntp_low)
{
    struct japan4k_dmx* p_this = NULL;
