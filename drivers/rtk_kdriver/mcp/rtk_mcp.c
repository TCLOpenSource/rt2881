...
#ifdef CONFIG_ENABLE_CP_TP_RECOVERY_PROCESS
...
#define MCP_BACK_MEMORY_SIZE  1024*128//128k
...
#define DEBUG_REG1 CP_CP_TST1_reg
#define DEBUG_REG2 CP_CP_TST2_reg
#define DEBUG_REG3 CP_MCP_ST_reg
#define DEBUG_REG4 CP_MCP_ST1_reg
...
void _mcp_dump_hw_internal_FSM(void)
{
    ...
    rtd_outl(DEBUG_REG1, 0x0000018d);
    udelay(10);
    ...
    rtd_outl(DEBUG_REG1, 0x00000297);
    udelay(10);
    debug_reg1_val = rtd_inl(DEBUG_REG1);
    debug_reg2_val = rtd_inl(DEBUG_REG2);
    debug_reg3_val = rtd_inl(DEBUG_REG3);
    ...
    rtd_outl(DEBUG_REG1, 0x0000028f);
    udelay(10);
    debug_reg1_val = rtd_inl(DEBUG_REG1);
    debug_reg2_val = rtd_inl(DEBUG_REG2);
    debug_reg3_val = rtd_inl(DEBUG_REG3);
    ...
    rtd_outl(DEBUG_REG1, 0x0000020f);
    udelay(10);
    debug_reg1_val = rtd_inl(DEBUG_REG1);
    debug_reg2_val = rtd_inl(DEBUG_REG2);
    debug_reg3_val = rtd_inl(DEBUG_REG3);
...
/*------------------------------------------------------------------
* Func : _mcp_dump_tvsb3_rbus_status
*
* Desc : dump tvsb3 rbus status
*------------------------------------------------------------------*/
void _mcp_dump_tvsb3_rbus_status(void)
{
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb8007030) = %08x", rtd_inl(0xb8007030));
    mcp_warning("rtd_inl(0xb8007480) = %08x", rtd_inl(0xb8007480));
    mcp_warning("rtd_inl(0xb80072d4) = %08x", rtd_inl(0xb80072d4));
    mcp_warning("rtd_inl(0xb80074c0) = %08x", rtd_inl(0xb80074c0));
    rtd_outl(0xb8007110, (rtd_inl(0xb8007110) & 0xFFFFFF87) | 0x30);
    udelay(10);
    mcp_warning("rtd_inl(0xb8007170) = %08x", rtd_inl(0xb8007170));
    mcp_warning("rtd_inl(0xb8007174) = %08x", rtd_inl(0xb8007174));
    mcp_warning("rtd_inl(0xb801c5a0) = %08x", rtd_inl(0xb801c5a0));
    mcp_warning("rtd_inl(0xb801c5fc) = %08x", rtd_inl(0xb801c5fc));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
    mcp_warning("rtd_inl(0xb801c5c8) = %08x", rtd_inl(0xb801c5c8));
...
#ifdef CONFIG_ENABLE_CP_TP_RECOVERY_PROCESS
static void do_mcp_tp_recovery(void)
{
	unsigned int i = 0;
	if(!((rtd_inl(SYS_REG_SYS_SRST1_reg) & SYS_REG_SYS_SRST1_rstn_tp_mask)
		&& (rtd_inl(SYS_REG_SYS_CLKEN1_reg) & SYS_REG_SYS_CLKEN1_clken_tp_mask)))
		return ;
	rtd_outl(TP_TP_TF0_CNTL_reg, 0x3);
	rtd_outl(TP_TP_TF1_CNTL_reg, 0x3);
	rtd_outl(TP_TP_TF2_CNTL_reg, 0x3);
	SET_MCP_CTRL(MCP_CLEAR | 1); 
	while(i++  < 0x800000) {
		if(!(rtd_inl(MCP_CTRL) & 0x2))
			break;
	}
	rtd_outl(TP_TP_TF0_CNTL_reg, 0x2);
	rtd_outl(TP_TP_TF1_CNTL_reg, 0x2);
	rtd_outl(TP_TP_TF2_CNTL_reg, 0x2);
}
#endif
...
int _mcp_start_xfer(void)
{
    ...
    if (!complete) {
        ...
        // Reset FSM
        _mcp_sw_reset();
        mcp_warning("command timeout, after reset (Ctrl=%08lx->%08x, Status=%08lx->%08x) (Spend time %d ms)\n",
                        ctrl, GET_MCP_CTRL(), status, GET_MCP_STATUS(),  jiffies_to_msecs(jiffies - stime));
        ret = -1;
    }
...
void *g_tmp_buf_virt = NULL;
unsigned long g_tmp_buf_phys = 0;
void *g_tmp_buf_noncached = NULL;
 ...
 *------------------------------------------------------------------*/
static int _do_mcp_command(mcp_desc* p_desc, int n_desc, int is_user_pointer)
{
    int ret = 0;
    if (n_desc <= 0)
        return 0;
...
#if defined(CONFIG_ENABLE_CP_TP_RECOVERY_PROCESS)
        bool use_tmp_buf = false;
        void *tmp_dst_buf = NULL;
        mcp_desc* cur_desc = NULL;
#endif
...
#if defined(CONFIG_ENABLE_CP_TP_RECOVERY_PROCESS)
        cur_desc = desc_list;
        if(cur_desc->length != 0 && (cur_desc->data_in == cur_desc->data_out)
            && ((cur_desc->flags & 0x1f) == 0x5 || (cur_desc->flags & 0x1f) == 0x10 || (cur_desc->flags & 0x1f) == 0x11
            || (cur_desc->flags & 0x1f) == 0x0 || (cur_desc->flags & 0x1f) == 0x1)) {
            tmp_dst_buf = phys_to_virt(cur_desc->data_out);
            if(tmp_dst_buf) {
                if(g_tmp_buf_virt && cur_desc->length <= MCP_BACK_MEMORY_SIZE) {
                    use_tmp_buf = true;
                    cur_desc->data_out = g_tmp_buf_phys;
                    dmac_inv_range(tmp_dst_buf, tmp_dst_buf + cur_desc->length);
                }
            }   
        }
#endif
...
#if defined(CONFIG_ENABLE_CP_TP_RECOVERY_PROCESS)
        if(ret == 0) {
            if(use_tmp_buf && g_tmp_buf_noncached && tmp_dst_buf) {
                memcpy(tmp_dst_buf, g_tmp_buf_noncached, cur_desc->length);
                dmac_flush_range(tmp_dst_buf, tmp_dst_buf + cur_desc->length);
            }
        }
#endif
...
static int do_mcp_command(mcp_desc* p_desc, int n_desc, int is_user_pointer)
{
    ...
    for(i = 0; i < n_desc; i++) {
        int retry_cnt = 0;
        while(retry_cnt < 5) {
            ret = _do_mcp_command(p_desc + i, 1, is_user_pointer);
            if(ret == 0) {
                if(retry_cnt != 0)
                    mcp_warning("mcp recovery success\n");
                break;
            }
            else if(ret != -2) {
                return ret;
            ...
            retry_cnt++;
            mcp_warning("do the %d mcp recovery\n", retry_cnt);
        }
        if(ret == -2)
            mcp_warning("mcp recovery failed\n");
    }
    
    return ret;
#else
    return _do_mcp_command(p_desc, n_desc, is_user_pointer);
#endif
...
)
{
	...
	iv[3] = p_desc->key[3];
	iv[4] = p_desc->key[4];
	ret = sw_sha1_ce(false, iv, p_in, len, p_out);
...
int __init mcp_core_init(struct device *dev_device)
{
    ...
    g_tmp_buf_virt = dvr_malloc_uncached(MCP_BACK_MEMORY_SIZE, &g_tmp_buf_noncached);
    if(g_tmp_buf_virt) 
    {
        g_tmp_buf_phys = dvr_to_phys(g_tmp_buf_noncached);
    }
