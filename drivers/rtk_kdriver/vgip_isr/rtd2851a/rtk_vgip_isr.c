...
unsigned char Get_Val_hdmi_finish_seamless(void)
{
	unsigned char result = FALSE;

	spin_lock(get_i3ddma_change_spinlock());//lock i3ddma change spinlock
	result = hdmi_finish_seamless;
	spin_unlock(get_i3ddma_change_spinlock());//unlock i3ddma change  spinlock
...
__maybe_unused static unsigned char last_bg_status = 0;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if(handle_h3ddma_Cap_last_wr_flag)
	{
		if(get_i3ddma_change_format_flag())
		{
			...
			if((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_HDMI))
			{
				...
				{
					spin_lock(get_i3ddma_change_spinlock());//lock i3ddma change spinlock
					if((seamless_info_sharememory->hdmi_not_ready == 0) && (get_hdr_semaless_active()))
					{
	...
	if(handle_ch1_end_flag && Get_Val_hdmi_finish_seamless())
	{
		spin_lock(get_i3ddma_change_spinlock());//lock i3ddma change spinlock
    ...
    if(handle_ch1_end_flag)
    {
        ...
        {
            if ((Get_DisplayMode_Src(SLR_MAIN_DISPLAY) == VSC_INPUTSRC_VDEC) && Get_fake_scaler_connect_impl())
            {//run hdr change
                if(get_hdr_semaless_active())
                {
               		if (RPC_SysInfo) {
