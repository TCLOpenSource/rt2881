...
void scalerVIP_colorHistISR(unsigned char channel)
{
	...
	// check M-domain freeze, if freeze enable, don't update background histogram
	{
		...
		*/
		...
		if(nonlibdma_get_cap_instatus_reg_freeze_enable_2nd(0))
			return;
	}
...
//===== DCC SC avoid flash process  ===========
	...
	//merlin8/8p mdomain one-bin
	if(get_mach_type()==MACH_ARCH_RTK2885P) {
		//if((MDOMAIN_DISP_DDR_MainCtrl_get_main_source_sel(IoReg_Read32(MDOMAIN_DISP_DDR_MainCtrl_reg)) == 3) && (MainDCC_SC == 1)){
		if((nonlibdma_get_disp_ddr_ctrl_reg_source_sel(0) == 3) && (MainDCC_SC == 1)){	// frame sync
			Scene_change_DCC_apply_delay_flag = 1;
		}
	}
	//do
