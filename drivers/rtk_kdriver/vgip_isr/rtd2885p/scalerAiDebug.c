...
static const char* ai_cmd_str[] = {
	...
	"defocus_mode=",   /*AI_CMD_DEFOCUS_MODE*/
};
...
void execute_ai_cmd(int cmd_id, char* value_ptr)
{
	...
	case AI_CMD_AI_MODE:
	{
		...
		if(len >= 13)
		{
			ai_mode.defocus_mode = value_ptr[12] - '0';
			if(ai_mode.defocus_mode > 9 || ai_mode.defocus_mode < 0)
			{
				rtd_pr_ai_dbg_emerg("[%s %d] value_ptr[12] = %c\n", __func__, __LINE__, value_ptr[12]);
				break;
			}
		}
	...
	case AI_CMD_DEFOCUS_MODE:
	{
		RTK_AI_PQ_mode ai_mode = aipq_mode;
		if(kstrtoint(value_ptr, 0, &ai_mode.defocus_mode) != 0)
			break;

		ai_mode.ap_mode = (ai_mode.defocus_mode != 0);
		scalerAI_pq_mode_ctrl(ai_mode, 0);
		scalerAI_config(ai_mode.ap_mode);
		break;
	}
