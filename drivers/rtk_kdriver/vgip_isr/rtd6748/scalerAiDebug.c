...
extern int vpq_enable_ai_dbgosd(NN_DBGOSD_T *demo);
extern void NN_Meomory_dump(void);
...
enum {
	...
	AI_CMD_AUDIO_DEBUGOSD = 14,
	AI_CMD_AIPQ_DEBUGOSD = 15,
	AI_CMD_AI_STATUS = 16,
	AI_CMD_AIPQ_CMA_DUMP = 17,
	AI_CMD_AIAQ_MODE = 18,
...
static const char* ai_cmd_str[] = {
	...
	"audio_debugosd=",      /*AI_CMD_AUDIO_DEBUGOSD*/
	"aipq_debugosd=",      /*AI_CMD_AIPQ_DEBUGOSD*/
	"ai_status",      /*AI_CMD_AI_STATUS*/
	"aipq_cmadump",       /*AI_CMD_AIPQ_CMA_DUMP*/
	"aiaq_mode=",       /*AI_AIAQ_MODE*/
...
void execute_ai_cmd(int cmd_id, char* value_ptr)
{
	...
	case AI_CMD_FACE_MODE:
	{
		...
		scalerAI_pq_mode_ctrl(ai_mode, 0);
		scalerAI_config(ai_mode.ap_mode);
		break;
	}
	...
	case AI_CMD_AUDIO_MODE:
	{
		...
		aiAudioState.enable = (unsigned int)mode;
		rtd_pr_ai_dbg_emerg("[%s %d]mode = %d\n", __func__, __LINE__, aiAudioState.enable);
	...
	case AI_CMD_AUDIO_DEBUGOSD:
	{
		int debugOsd;
		if(kstrtoint(value_ptr, 0, &debugOsd) != 0)
			break;

		aiAudioState.dbgOsdOn = (unsigned int)debugOsd;
		rtd_pr_ai_dbg_emerg("[%s %d]dbgOsdOn = %d\n", __func__, __LINE__,aiAudioState.dbgOsdOn);

		break;
	}
	case AI_CMD_AIPQ_DEBUGOSD:
	{
		int debugOsd;
		int mode;
		NN_DBGOSD_T demo;
		...
		mode=value_ptr[0] - '0';
		debugOsd=value_ptr[1] - '0';

		switch(mode){
			case 0:
				demo.model=AI_SQM_DETECT;
				break;
			case 1:
				demo.model=AI_FACE_DETECT;
				break;
			case 2:
				demo.model=AI_GENRE_DETECT;
				break;
			default:
				return;
				break;
		}
		demo.enDbgOsd=(unsigned int)debugOsd;
		vpq_enable_ai_dbgosd(&demo);
		rtd_pr_ai_dbg_emerg("[%s %d]mode=%d,dbgOsdOn = %d\n", __func__, __LINE__,mode,debugOsd);

		break;
	}
	case AI_CMD_AI_STATUS:
	{
		int status;
		status=rtk_ai_get_enable_status(AI_PQ);
		rtd_pr_ai_dbg_emerg("[%s %d]mode=3,PQ status = %d\n", __func__, __LINE__,status);
		status=rtk_ai_get_karaoke_state();
		rtd_pr_ai_dbg_emerg("[%s %d]mode=3,Audio status = %d\n", __func__, __LINE__,status);
		status=rtk_ai_get_enable_status(AI_CAMERA);
		rtd_pr_ai_dbg_emerg("[%s %d]mode=3,camera status = %d\n", __func__, __LINE__,status);
		break;
	}
	case AI_CMD_AIPQ_CMA_DUMP:
	{
		NN_Meomory_dump();
		break;
	}
	case AI_CMD_AIAQ_MODE:
	{
		int value;
		rtd_pr_ai_dbg_emerg("[%s %d]AI_CMD_AIAQ_MODE\n", __func__, __LINE__);
		if(kstrtoint(value_ptr, 0, &value) != 0)
			break;

		ai_aq_mode = value;
		rtd_pr_ai_dbg_emerg("[%s %d]ai_aq_mode = %d\n", __func__, __LINE__,ai_aq_mode);
		if(ai_aq_mode)
			scalerAI_config(AI_MODE_ON);
		else{
			if(aipq_mode.ap_mode==0)
				scalerAI_config(AI_MODE_OFF);
		}
		break;
	}
