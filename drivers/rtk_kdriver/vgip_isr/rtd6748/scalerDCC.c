...
void scalerVIP_colorHistISR(unsigned char channel)
{
	...
	unsigned long long total_sum, tmp;
...
void scalerVIP_RK_MrKs_White_EX_byLevel(int *Gz, unsigned char MrKs_EX_LV_W,  _system_setting_info *_system_info)
{	
	int bin_range = RK_MrKs_W_B_EX_BY_LV_bin_Range;
	int internal_gain = RK_MrKs_internal_gain;
	int offset_tmp = 0;
	unsigned int i;
	unsigned char White_EX_Lv;
	...
	if (Gz == NULL | _system_info == NULL) {
		ROSPrintf_DCCLv7("scalerVIP_White_EX_byLevel, table null, Gz=%p, _system_info=%p,\n", Gz, _system_info);
		return;
	}
...
void scalerVIP_RK_MrKs_Black_EX_byLevel(int *Gz, unsigned char MrKs_EX_LV_B,  _system_setting_info *_system_info)
{	
	int bin_range = RK_MrKs_W_B_EX_BY_LV_bin_Range;
	int internal_gain = RK_MrKs_internal_gain;
	int offset_tmp = 0;
	unsigned int i;
	unsigned char Black_EX_Lv;
	unsigned int max_offset;
	unsigned int half_bin_range;
	...
	if (Gz == NULL | _system_info == NULL) {
		ROSPrintf_DCCLv7("scalerVIP_Black_EX_byLevel, table null, Gz=%p, _system_info=%p,\n", Gz, _system_info);
		return;
	}
...
//		scalerVIP_BlackCorrector_Cal_byLevel(Gz);
	...
	if ((Scaler_APDEM_Arg_Access(DEM_ARG_Black_Extension_level, 0, 0) != 0xFF) || (Scaler_APDEM_Arg_Access(DEM_ARG_White_Extension_level, 0, 0) != 0xFF)) {
		...
		scalerVIP_RK_MrKs_Black_EX_byLevel(Gz, APDEM_Black_Extension, system_info_structure_table);
		scalerVIP_RK_MrKs_White_EX_byLevel(Gz, APDEM_White_Extension, system_info_structure_table);
	}
