...
static AD82120_REG AD82120m_InitTbl[] = {
        ...
        {AD82120_C6VOL, 0x18, "Channel_6_volume_control"},	//##Channel_6_volume_control
        {AD82120_SCTL4, 0x90, "State_Control_4"},			//##State_Control_4
        {AD82120_SCTL5, 0x68, "State_Control_5"},           //##State_Control_5
        {AD82120_SCTL6, 0x80, "State_Control_6"},			//##State_Control_6
        {AD82120_SCTL8, 0x06, "State_Control_8"},                   //##State_Control_8
        {AD82120_I2S_OUT, 0x05, "I2S_data_output_sel_register"},	//##I2S_data_output_section_register
        {AD82120_MKHB,	  0x30, "Mono_Key_High_Byte"},				//##Mono_Key_High_Byte
        {AD82120_MKLB,	  0x06, "Mono_Key_Low_Byte"},				//##Mono_Key_Low_Byte
...
void ad82120_amp_phase_reset(int amp_i2c_id)
{
    ...
    unsigned short addr;
    int i =0 ;
    AMP_ERR("Ad82120_amp_phase_reset\n");
    ...
    for(i =0;i<2;i++){
        ...
        data[0] = 0x1a;
        data[1] = 0x00;
        if (i2c_master_send_ex_flag(amp_i2c_id, addr, data, 2, I2C_M_FAST_SPEED) < 0)  //--reset amp
...
void ad82120_func(int amp_i2c_id,unsigned short addr)
{
    ...
    if(reset_flag==1){
        AMP_ERR("AD82120 have already phase reset\n");
    }else{
        // software reset amp
        data[0] = 0x1a;
        data[1] = 0x00;
...
void ad82120m_func(int amp_i2c_id,unsigned short addr)
{
    ...
    for (index = 0; index <= retry_cnt; index++) {
        if (i2c_master_send_ex_flag(amp_i2c_id, addr, data, data_len+1, I2C_M_FAST_SPEED) < 0) { //--reset amp
            AMP_ERR("%s line: %d, program AD82120m failed (retry=%d)\n", __func__, __LINE__, index);
            mdelay(5);
            if(index == retry_cnt) return;
        }
        else {
    ...
    if (i2c_master_send_ex_flag(amp_i2c_id, addr, data, data_len+1, I2C_M_FAST_SPEED) < 0) //--Normal operation, MONO
        AMP_ERR("%s line: %d, program AD82120m failed\n", __func__, __LINE__);
    mdelay(20);
    ...
    for(index = 0; index < (sizeof(AD82120m_InitTbl)/sizeof(AD82120_REG)); index ++)
    {
        data[0] = AD82120m_InitTbl[index].bAddr;
        data_len = 1;
        data[1] = AD82120m_InitTbl[index].bArray;
        if(data[0] == 0x02)
            continue;
        if(data[0] >= 0x71 && data[0]<= 0x7C)
            continue;
		//set mono
		if(data[0] == 0x1A){
			data[1] =((AD82120m_InitTbl[index].bArray)| 0x40);
			if (i2c_master_send_ex_flag(amp_i2c_id, addr, data, data_len+1, I2C_M_FAST_SPEED) < 0){ //--mute amp
        	AMP_ERR("%s line: %d, program AD82120m failed\n", __func__, __LINE__);
			}
			continue;
        ...
        if (i2c_master_send_ex_flag(amp_i2c_id, addr , data, data_len+1 ,I2C_M_FAST_SPEED) < 0)
            AMP_ERR("program AD82120m failed\n");
    }
