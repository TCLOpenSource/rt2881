...
static WA156819_REG WA156819BLP_InitTbl[] = 
{
 {0x0B, 0x01},//Soft Reset contorl
 {0x02, 0x00},//MCLK ( 48k = 0x00, 96k = 0x01, 36k=0x02 )
 {0x5D, 0x01},//SHDN High
...
void WA156819LP_func(int amp_i2c_id, unsigned short slave_addr)
{
    unsigned char data[2];
    int u8Index=0;
    int timer = 0;
	AMP_ERR("\nWA156819_LP Initial Start\n");
    for(u8Index = 0; ; u8Index ++)
    {
        data[0] = WA156819BLP_InitTbl[u8Index].bAddr;
        data[1] = WA156819BLP_InitTbl[u8Index].bArray;
