...
int IOCTL_Delivery_Close(DELIVERY_DELIVERY_CHANNEL_T *pChannel)
{
    ...
    RHAL_Delivery_SetMode(tp_id, TP_DELIVERY_MODE_TS);
    deliveryMode[pChannel->ch] = TP_DELIVERY_MODE_TS;
...
int IOCTL_Delivery_SetTPInputConfig(DELIVERY_TP_SOURCE_CONFIG_T *pInfo)
{
    ...
    delivery_info(pInfo->ch,"[%s L:%d]ch:%d, exit ret:%d", __FUNCTION__,__LINE__, pInfo->ch, ret);
...
int IOCTL_Delivery_RequestData(DELIVERY_DELIVERY_CHANNEL_T *channel)
{
    ...
    if( deliveryMode[channel->ch] == TP_DELIVERY_MODE_JAPAN4K || deliveryMode[channel->ch] == TP_DELIVERY_MODE_TS ){
        ...
        if( srcType == TPK_DELI_SRC_TYPE_TLV_SW0 || srcType == TPK_DELI_SRC_TYPE_TS ){
            ...
            if( srcType == TPK_DELI_SRC_TYPE_TLV_SW0 ){
                //RHAL_TPSetRingBufferWritePolicy(TP_RING_WRITE_UPDATE_FOR_WDONE);
        ...
        delivery_info(channel->ch,"[%s L:%d]ch:%d, exit srcType:%d", __FUNCTION__,__LINE__, channel->ch , srcType);
    }
...
{
    TPK_TP_ENGINE_T tp_id;
    JP4K_DELIVERY_NTP_INFO ntp_info;
    ntp_info.ntp = 0;
    ntp_info.ntp_leap_indicator = 0;

    tp_id = DELIVERY_GET_DELIVERY_ID(pNTP->ch);
    japan4k_dmx_get_NTP(tp_id, &ntp_info);

    pNTP->ntp = ntp_info.ntp;
    pNTP->ntp_leap_indecator = ntp_info.ntp_leap_indicator;
    return 0;
}
int IOCTL_Delivery_GetHWNTP(DELIVERY_DELIVERY_TLV_NTP_T *channel)
{
    TPK_TP_ENGINE_T tp_id;
    DELIVERY_CHECK_CH(channel->ch);
    tp_id = DELIVERY_GET_DELIVERY_ID(channel->ch);

    return japan4k_dmx_get_HW_ntp(tp_id, &channel->ntp_high, &channel->ntp_low);
}
