...
long DELIVERY_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DELIVERY_IOC_DELIVERY_GET_TLV_HWNTP:
	{
		...
		if(copy_from_user((void *)&data, (const void __user *)arg, sizeof(DELIVERY_DELIVERY_TLV_NTP_T)))
		{
			...
			ret = IOCTL_Delivery_GetHWNTP(&data);
	...
	{
#ifdef CONFIG_COMPAT
		DELIVERY_DELIVERY_MAPPINT_INFO_COMPAT_T compat_param;
		if(copy_from_user((void *)&compat_param, (const void __user *)arg, sizeof(DELIVERY_DELIVERY_TLV_FILTER_INFO_COMPAT_T))){
			delivery_err(compat_param.ch,"RTKDELIVERY[%d]: ioctl get mapping info failed !!!!!!!!!!!!!!!\n", __LINE__);
			ret = -EFAULT;
		}else{
			DELIVERY_DELIVERY_MAPPINT_INFO_T mapping_info;
			mapping_info.ch = compat_param.ch;
			mapping_info.count = compat_param.count;
			mapping_info.pMapping_info = compat_ptr(compat_param.pMapping_info);
			ret = IOCTL_Delivery_GetCIDMappingInfo(&mapping_info);
    		if ((ret != 0) || copy_to_user((void __user *)arg, (void *)&mapping_info, sizeof(DELIVERY_DELIVERY_TLV_FILTER_INFO_COMPAT_T)))
    		{
