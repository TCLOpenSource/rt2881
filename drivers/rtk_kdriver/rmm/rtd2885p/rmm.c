...
RTK_DC_COMMON_RET_t rtk_dc_get_dram_bitwidth(RTK_DC_COMMON_BITWIDTH_t *p_dram_bitwidth)
{
        ...
        if(dc_mc1_mc_sys.ddr4_en==1 && dc_sys_dummy0.page_size==0) //dc1 ddr type
                ddr_bitwidth += 16;
        else
                ddr_bitwidth += 32;

        if(dc_mc3_mc_sys.ddr4_en==1 && dc_sys_dummy0.page_size_dc2==0) //dc2 ddr type
                ddr_bitwidth += 16;
        else
                ddr_bitwidth += 32;
...
unsigned int carvedout_buf_get_layout_idx(void)
{
	...
	if (get_product_type() == PRODUCT_TYPE_DIAS) {
		if (DRAM_size == (0x100000 - 0x4000))
			layout_idx = MM_LAYOUT_4GB;
		else 
			layout_idx = MM_LAYOUT_DIAS_2GB;
	} else { //TV
		if (DRAM_size == (0x100000 - 0x4000)) {
			layout_idx = MM_LAYOUT_4GB_TV;
			goto layout_out;
                }    
