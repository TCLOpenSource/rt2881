...
//	{NC_AUDIO_RINGBUFFER_START,  NC_AUDIO_RINGBUFFER_SIZE}, // CARVEDOUT_AUDIO_RINGBUF
	...
	{0x17900000, 9*_MB_},				// CARVEDOUT_SCALER_OD
...
//	{DDR_1G5_AUDIO_RINGBUFFER_START,  DDR_1G5_AUDIO_RINGBUFFER_SIZE}, // CARVEDOUT_AUDIO_RINGBUF
  ...
  // MM_LAYOUT_TV1G5_NC    for TV 4K 1.5GB 
  {
	{0x00000000,  1.5*_MB_},							// CARVEDOUT_BOOTCODE
	{DEMOD_CARVED_OUT,  0*_MB_},					// CARVEDOUT_DEMOD
	{0x10000000,  0*_MB_},							// CARVEDOUT_AV_DMEM
...
#define MAGIC_CAVEDOUT (0xdecade10)
    return rtd_inl(STB_SW_DATA13_reg) == MAGIC_CAVEDOUT ? 0 : 1;
#else
    return true;
#endif
}
...
unsigned int carvedout_buf_get_layout_idx(void)
{
	...
	if (get_product_type() == PRODUCT_TYPE_DIAS) {
		if (DRAM_size >= 0x60000) { //(get_memory_size_total())
		...
		if (get_platform_model() == PLATFORM_MODEL_2K) {
			...
			else if (carvedout_enable() == false && DRAM_size == 0x60000)
				layout_idx = MM_LAYOUT_TV1G5_NC;
			else
