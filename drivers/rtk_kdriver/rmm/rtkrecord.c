...
#define MAX_NUMBER_OF_MAPPING_STRING 6
#define MAX_MAPPING_STRING_LENGTH 32

enum CMAINFO_MODULE_ID {
	CMAINFO_MODULE_VDEC,
	CMAINFO_MODULE_AUDIO,
	CMAINFO_MODULE_SCALER,
	CMAINFO_MODULE_GRAPHICS,
	CMAINFO_MODULE_SDEC,
	NR_CMAINFO_MODULES
};

struct cma_info {
	int limit_size;		/* Value defined in memory map */
	char mapping_string[MAX_NUMBER_OF_MAPPING_STRING][MAX_MAPPING_STRING_LENGTH];
};

struct cma_info cma_module_info[NR_CMAINFO_MODULES] = {
	/* CMAINFO_MODULE_VDEC */
	[CMAINFO_MODULE_VDEC].limit_size = 241*_MB_,
	[CMAINFO_MODULE_VDEC].mapping_string[0] = "rtkvdec",
	[CMAINFO_MODULE_VDEC].mapping_string[1] = "vfifo",
	[CMAINFO_MODULE_VDEC].mapping_string[2] = "memory_vdec",
	/* CMAINFO_MODULE_AUDIO */
	[CMAINFO_MODULE_AUDIO].limit_size = 32*_MB_,
	[CMAINFO_MODULE_AUDIO].mapping_string[0] = "rtkaudio",
	/* CMAINFO_MODULE_SCALER */
	[CMAINFO_MODULE_SCALER].limit_size = 30*_MB_,
	[CMAINFO_MODULE_SCALER].mapping_string[0] = "Scaler",
	[CMAINFO_MODULE_SCALER].mapping_string[1] = "drvif",
	[CMAINFO_MODULE_SCALER].mapping_string[2] = "I3DDMA",
	[CMAINFO_MODULE_SCALER].mapping_string[3] = "h3ddma",
	/* CMAINFO_MODULE_GRAPHICS */
	[CMAINFO_MODULE_GRAPHICS].limit_size = 112*_MB_,
	[CMAINFO_MODULE_GRAPHICS].mapping_string[0] = "ION_DRV",
	[CMAINFO_MODULE_GRAPHICS].mapping_string[0] = "DMABUF",
	/* CMAINFO_MODULE_SDEC */
	[CMAINFO_MODULE_SDEC].limit_size = 22*_MB_,
	[CMAINFO_MODULE_SDEC].mapping_string[0] = "dmx_buf",
	[CMAINFO_MODULE_SDEC].mapping_string[1] = "TPP",
	[CMAINFO_MODULE_SDEC].mapping_string[2] = "create_tp",
	[CMAINFO_MODULE_SDEC].mapping_string[3] = "TPVerify",
};

#define RTK_RECORD_ADDR_MASK 0x3ffffffff
...
static unsigned long gpu_fast_current_page_total = 0;
unsigned long gpu_fast_peak_value = 0;
...
int rtk_record_insert(unsigned long addr, unsigned long type, unsigned long priv, const void *caller, unsigned long owner)
{
	...
	if (error == 0) {
		...
		if (owner == DVR_OWNER_GPU) {
			gpu_fast_current_page_total += (type & 0x00ffffff);
			if (gpu_fast_peak_value < gpu_fast_current_page_total)
				gpu_fast_peak_value = gpu_fast_current_page_total;
		}
...
EXPORT_SYMBOL(rtk_record_insert);
...
EXPORT_SYMBOL(rtk_record_lookup_ex);
...
EXPORT_SYMBOL(rtk_record_delete);
...
int cma_monitor_check_mem(unsigned long type, const void *caller, bool isHighMem, char *module_keyword)
{
	...
	if ( ((type & ID_MASK) == BUDDY_MESG_ID) || ((type & ID_MASK) == DRIVER_MESG_ID)) {
		...
		snprintf(caller_name, sizeof(caller_name), "%pS", caller);
	}
...
int filter_rtk_memory_allocation(filter_rtk_mem_t filter_func,int (*cmp_func)(const void *, const void *),void *data)
{
	rtk_record **pRecords = NULL;
	...
	ret = filter_func(pRecords, local_tree_node_count,data);
...
int list_all_rtk_memory_allocation_sort(list_rtk_mem_t list_func,int (*cmp_func)(const void *, const void *),void *data)
{
	...
	rtd_pr_rmm_info("gpu_fast alloc size : %8lx \n", gpu_fast_current_page_total << PAGE_SHIFT);
	rtd_pr_rmm_info("gpu_fast allocated peak size : %8lx \n", gpu_fast_peak_value << PAGE_SHIFT);
...
int list_func_mem(rtk_record **pRecords,unsigned long node_count,void *data)
{
	...
	if(data){
		...
		for(cnt = 0;cnt < node_count; cnt++){
			snprintf(caller,sizeof(caller) - 1,"%pS",pRecords[cnt]->caller);
		...
		for(cnt = 0;cnt < node_count; cnt++){
			...
			func_alloc += ret;
			total_alloc += ret;
			snprintf(name,sizeof(name) - 1,"%pS",pRecords[cnt]->caller);
		}
		if(cnt == node_count){
			rtd_pr_rmm_info("func(%s) total allocated: %8x \n\n",name,func_alloc);
		}
...
int filter_func_mem(rtk_record **pRecords,unsigned long node_count,void *data)
{
	int cnt,ret;
	int total_alloc = 0;
	int func_alloc = 0;
	...
	if(data){
		...
		for(cnt = 0;cnt < node_count; cnt++){
			if (((pRecords[cnt]->type & ID_MASK) == BUDDY_MESG_ID) || 
				((pRecords[cnt]->type & ID_MASK) == DRIVER_MESG_ID)) {
				snprintf(caller,sizeof(caller) - 1,"%s", (const char *)pRecords[cnt]->caller);
			} else {
				snprintf(caller,sizeof(caller) - 1,"%pS",pRecords[cnt]->caller);
			}
			if(strstr(caller,name)){
		...
		if(total_alloc)
			rtd_pr_rmm_info("function has %s total allocated: %8lx \n", name,total_alloc);
		else
			rtd_pr_rmm_info("function has %s , %s not allocated cma memory\n", name, caller);
	}
	else{
		for(cnt = 0;cnt < node_count; cnt++){
			if (((pRecords[cnt]->type & ID_MASK) == BUDDY_MESG_ID) || 
			...
			if((strcmp(caller,name)) && (name[0] != 0)){
				rtd_pr_rmm_info("func(%s) total allocated: %8lx \n\n",name,func_alloc);
				func_alloc = 0;
			}
			ret = remap_list_mem(pRecords[cnt]);
			func_alloc += ret;
			...
			if (((pRecords[cnt]->type & ID_MASK) == BUDDY_MESG_ID) || 
				((pRecords[cnt]->type & ID_MASK) == DRIVER_MESG_ID)) {
				snprintf(name,sizeof(name) - 1,"%s", (const char *)pRecords[cnt]->caller);
			} else {
				snprintf(name,sizeof(name) - 1,"%pS",pRecords[cnt]->caller);
			}
		}
		...
		rtd_pr_rmm_info("total allocated: %8lx \n", total_alloc);
	}

	return total_alloc;
}

static int __maybe_unused cma_get_module_alloc(int module_id)
{
	int i;
	int module_cma_alloc = 0;

	for (i = 0; i < MAX_NUMBER_OF_MAPPING_STRING; i++) {
		if (cma_module_info[module_id].mapping_string[i][0] == '\0')
			break;
		else
			module_cma_alloc += filter_rtk_memory_allocation(filter_func_mem, cmp_caller_ascending_order,
										(void *)(cma_module_info[module_id].mapping_string[i]));
	}

	return module_cma_alloc;
}

int cma_get_gpu_fast_alloc(void)
{
	unsigned long ret;

	spin_lock_irq(&rtk_tree_lock);
	ret = gpu_fast_current_page_total << PAGE_SHIFT;
	spin_unlock_irq(&rtk_tree_lock);
	
	return ret;
}

int cma_get_graphic_alloc(void)
{
	return cma_get_module_alloc(CMAINFO_MODULE_GRAPHICS);
}

int cma_get_video_alloc(void)
{
	return cma_get_module_alloc(CMAINFO_MODULE_VDEC);
}
