...
#if IS_ENABLED(CONFIG_MMC_RTKEMMC_PLUS)
extern unsigned int ext_tmout_ms;
#endif
...
void cma_unset_bp(int dev_idx)
{
	rtkcma_list[dev_idx].bp = NULL;
}
EXPORT_SYMBOL(cma_unset_bp);
...
unsigned long get_memory_size_total(void)
{
	return get_memory_size(GFP_DCU1) + get_memory_size(GFP_DCU2);
}
EXPORT_SYMBOL(get_memory_size_total);
...
unsigned long get_cma_avail_size_specific(int flags, size_t size, unsigned long *total)
{
    int dev_idx = 0;
	struct device *dev = ERR_PTR(INVALID_VAL);
	struct cma *cma = NULL;
	struct rtkcma *rtkcma = NULL;
	...
	rtd_pr_rmm_err("%s: dev_idx=%d, exist=%d\n", __func__, dev_idx, rtkcma_list[dev_idx].exist);

	if (dev_idx > DEV_CMA2 || !rtkcma_list[dev_idx].exist)
		return 0;

	dev = rtkcma_list[dev_idx].dev;
	cma = dev_get_cma_area(dev);
	rtkcma = &rtkcma_list[dev_idx];
	size_page = PAGE_ALIGN(size);

	rtd_pr_rmm_err("%s: dev=%lx, cma=%lx, rtkcma=%lx, bp=%lx\n", __func__,
				   (unsigned long)dev, (unsigned long)cma, (unsigned long)rtkcma, (unsigned long)rtkcma->bp);

	if (size == 0)
		free_size = cma_get_avail_size(rtkcma) << PAGE_SHIFT;
	else
		free_size = show_rtkbp_by_order(rtkcma->bp, get_order(size_page)) << PAGE_SHIFT;

	if (total)
		*total = cma->count << PAGE_SHIFT;

	rtd_pr_rmm_err("%s: free(%lx), size(%lx), total(%lx)\n", __func__, free_size, 
				   (unsigned long)size, (unsigned long)cma->count << PAGE_SHIFT);

	//debug
	show_rtkbp(rtkcma->bp);

	return free_size;
}
EXPORT_SYMBOL(get_cma_avail_size_specific);

extern int cma_get_gpu_fast_alloc(void);
int is_gpu_fast_in_cma_too_much(unsigned long free_limit, unsigned long gpu_limit)
{
	unsigned long space1, space2;
	unsigned long free1, free2;
	
	unsigned long total, free;
	unsigned long gpu_fast_used = cma_get_gpu_fast_alloc();
	unsigned long pa_free, pa_gpu;
	
	int ret = 0;
#define _K(x) ((x) >> 10)
#define PERCENT(a, b) ((_K(a) * 100) / _K(b))

	free2 = get_cma_avail_size_specific(GFP_DCU2, 0, &space2);
	free1 = get_cma_avail_size_specific(GFP_DCU1, 0, &space1);

	total = space2 + space1;
	free = free2 + free1;

	if (total != 0) {
		pa_free = PERCENT(free, total);
		pa_gpu = PERCENT(gpu_fast_used, total);
		
		if ((pa_free < free_limit) && (pa_gpu > gpu_limit)) {
			ret = 1;
			rtd_pr_rmm_info("free(%lx/%lx), total(%lx/%lx), pa(%ld/%ld), too much\n", free1, free2,
							space1, space2, pa_free, pa_gpu);
		} else {
			rtd_pr_rmm_info("free(%lx/%lx), total(%lx/%lx), pa(%ld/%ld)\n", free1, free2,
							space1, space2, pa_free, pa_gpu);
		}
	}
...
EXPORT_SYMBOL(is_gpu_fast_in_cma_too_much);
...
//	}
	...
	if (!page) {
        ...
        dmac_flush_range(phys_to_virt(addr), phys_to_virt(addr)+size);
...
//	void *ret = 0;
	...
	} else if ((flags & GFP_DCU3) && !rtkcma_list[DEV_CMA3].exist) {
		flags = GFP_DCU2_FIRST;
		rtd_pr_rmm_info("remap: %s not exist, turn to %s...\n", rtkcma_list[DEV_CMA3].name, rtkcma_list[DEV_CMA2].name);
	}
	...
	if (!flags || (flags & GFP_DCU1_FIRST)) {
		...
		if (rtkcma_list[DEV_CMA3].bp == NULL) // bitmap allocator
			page = dvr_bitmap_alloc_from_contiguous(rtkcma_list[DEV_CMA3].dev, count, 0, true);
		else {
			page = dvr_alloc_from_contiguous(rtkcma_list[DEV_CMA3].dev, count, 0, true);
			if (!page) {
				page = dvr_alloc_from_contiguous(auth_dev, count, 0, true);
			}
		}
...
#endif
    if (rtkcma_list[DEV_CMA3].exist) {
		if (rtkcma_list[DEV_CMA3].bp == NULL) // bitmap allocator
			ret |= dvr_bitmap_release_from_contiguous(rtkcma_list[DEV_CMA3].dev, page, count);
		else
			ret |= dvr_release_from_contiguous(rtkcma_list[DEV_CMA3].dev, page, count);
	}
...
/*! There are some cleint modules activate monitors at DC_MT init, they leverage rmm's dmac_flush_range,
  To remove module cycle dependency, dcmt register callback for dvr to use
*/
//extern char* DCMT_module_str(unsigned char id);

static char* (*__get_dcmt_module_str)(unsigned char) = NULL;

static char* __dcmt_module(unsigned long id)
{
	if ((__get_dcmt_module_str) && (id <= 0xFF)) { //DCMT MODULE_ID is now 1-byte width
		return __get_dcmt_module_str((unsigned char)id);
	}
	else { /* DVR_OWNER extension from dcmt id */
		switch(id) {
		case DVR_OWNER_NOID:
			return ("@NA"); // no owner specified for legacy
		case DVR_OWNER_MULTI:
			return ("@Multi"); // if more than one module to write
#if 1 //TODO deprecated if dc_mt id can be identified in source code level.
		case DVR_OWNER_GAL:
			return ("@Gal");
		case DVR_OWNER_AUTH:
			return ("@Auth");
		...
		case DVR_OWNER_VBM:
			return ("@Vbm");
		case DVR_OWNER_VE_FLUSH:
			return ("@Ve_");
#endif
		default:
			return ("@Unknown"); // unidentified id name
		}
	}
}

void dvr_get_dcmt_module_str_register(char *(*fp)(unsigned char))
{
	if (fp != NULL) {
		__get_dcmt_module_str = fp;
	}
}
EXPORT_SYMBOL(dvr_get_dcmt_module_str_register);

#define VA_ARGS_OWNER(LAST) \
	va_list args; \
	unsigned long owner; /* DCMT MODULE_INFO.id */ \
	va_start(args, LAST); \
	owner = va_arg(args, unsigned long); \
	va_end(args); \
	pr_debug("%s[%d] %s\n", __func__, __LINE__, __dcmt_module(owner))


//! for VA_ARGS wrapper, ordinary function should be inline to va_args wrapper function
//  to make the caller function stack could be referred in the same depth

//unsigned long pli_malloc(size_t size, int flags, unsigned long owner)
__always_inline unsigned long __pli_malloc(size_t size, int flags, unsigned long owner)
{
	int count = 0;
	...
	if (rtkcma_list[DEV_CMA3].exist && (rtkcma_list[DEV_CMA3].bp == NULL) && (flags & GFP_DCU3)) { // restore
		size = size_order = size_page;
		count = count_order = count_page;
	}
	...
	if (rtk_record_insert(phys_addr, RTK_SIGNATURE | BUDDY_ID | count_page, 0, __builtin_return_address(0), owner)) {
		free_contiguous_memory(page, count);

		return INVALID_VAL;
	}

	if (owner == DVR_OWNER_VE_FLUSH) {
		if (PageHighMem(page)) {
			void *va = dvr_remap_cached_memory(phys_addr, size, __builtin_return_address(0));
			if (va) {
				dmac_flush_range(va, va + size);
				outer_flush_range(phys_addr, phys_addr + size);
				dvr_unmap_memory(va, size);
				rtd_pr_rmm_err("%s: remap cache va OK, count=%d, vmalloc_total=%d, used=%d\n", __func__,
					count, (unsigned long)VMALLOC_TOTAL >> PAGE_SHIFT, vmalloc_nr_pages());
			} else {
				rtd_pr_rmm_err("%s: remap cache va fail, count=%d, vmalloc_total=%d, used=%d\n", __func__,
					count, (unsigned long)VMALLOC_TOTAL >> PAGE_SHIFT, vmalloc_nr_pages());
				BUG();
			}
		} else {
...
__maybe_unused unsigned long va_pli_malloc(size_t size, int flags, ...) // wrap for legacy api without ownerid
{
	VA_ARGS_OWNER(flags);
	return __pli_malloc(size, flags, owner);
}
EXPORT_SYMBOL(va_pli_malloc);
...
unsigned long pli_malloc_mesg(size_t size, int flags, char *mesg)
{
   ...
   if (PageHighMem(page)) {
	   if (size > 0x00e00000) {
		   /* Skip the larger size remapping, 
			  this cache flushing is for vdec ve memory, 
			  the maximum would be 16-2MB from vdec team measurment. 
		   */
		   rtd_pr_rmm_info("%s: count=%d, vmalloc_total=%d, used=%d\n", __func__, 
						  count, (unsigned long)VMALLOC_TOTAL >> PAGE_SHIFT, vmalloc_nr_pages());
	   } else {
		   void *va = dvr_remap_cached_memory(phys_addr, size, __builtin_return_address(0));
		   if (va) {
			   ...
			   rtd_pr_rmm_err("%s: remap cache va fail\n", __func__);
			   BUG();
		   }
	   }
   } else {
...
void *dvr_malloc_specific_addr(size_t size, int flags, unsigned long addr)
{
	...
	for (;;) {
		int err;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 108))
		err = alloc_contig_range(pfn, pfn + count, MIGRATE_CMA, GFP_KERNEL, &dummy);
#else
		err = alloc_contig_range(pfn, pfn + count, MIGRATE_CMA, GFP_KERNEL);
#endif
		if (err == 0) {
...
struct page *dvr_malloc_page_mesg(size_t size, int flags, char *mesg)
{
	...
	unsigned long owner = DVR_OWNER_NOID;
	...
	} else if (strcmp(ptr, "gpu_fast") == 0) {
		owner = DVR_OWNER_GPU;
	}

	if (rtk_record_insert(phys_addr, RTK_SIGNATURE | DRIVER_MESG_ID | count_page, 0, ptr, owner)) {
		free_contiguous_memory(page, count);
		page = 0;
		goto out;
	}
...
unsigned long (*__dvr_heap_to_phys) (struct dma_buf *) = NULL;
...
unsigned long (*__vbm_heap_to_phys) (struct dma_buf *) = NULL;
#if IS_ENABLED(CONFIG_DMABUF_HEAPS_VBM)
...
unsigned long (*__carveout_heap_to_phys) (struct dma_buf *) = NULL;
#if IS_ENABLED(CONFIG_DMABUF_CARVEOUT_HEAP)
...
int (*__vbm_heap_to_dmabuf_list) (struct dma_buf *, unsigned long *, int ) = NULL;
#if IS_ENABLED(CONFIG_DMABUF_HEAPS_VBM)
...
int (*__carveout_heap_to_dmabuf_list) (struct dma_buf *, unsigned long *, int ) = NULL;
#if IS_ENABLED(CONFIG_DMABUF_CARVEOUT_HEAP)
...
#if IS_ENABLED(CONFIG_DMABUF_HEAPS_VBM)
void (*__vbm_heap_dump_chunk_list_fp) (void *) = NULL;
void vbm_heap_dump_chunk_list_register(void (*fp)(void *))
{
	if (fp != NULL)
		__vbm_heap_dump_chunk_list_fp = fp;
}
EXPORT_SYMBOL(vbm_heap_dump_chunk_list_register);

inline void vbm_heap_dump_chunk_list(void)
{
	if (!vbm_heap) {
		vbm_heap = dma_heap_find("vbm");
		rtd_pr_rmm_info("%s, vbm(%lx)\n", __func__, vbm_heap);
	...
	if (__vbm_heap_dump_chunk_list_fp && vbm_heap)
		__vbm_heap_dump_chunk_list_fp(dma_heap_get_drvdata(vbm_heap));
}
#endif
...
void dvr_dmabuf_free(struct dma_buf *dmabuf)
{
	...
	{
	extern int vbm_heap_dmabuf_array_frame_deferfree(struct dma_buf *dmabuf);
	if (vbm_heap_dmabuf_array_frame_deferfree(dmabuf))
		return;
	}
#endif
...
{
	unsigned long addr;
	addr = pli_malloc(size, GFP_DCU1, DVR_OWNER_RPC);
	if (addr == INVALID_VAL) {
		return 0;
	} else {
...
int dvr_deferfree_check(unsigned long phys_addr)
{
	...
	if (phys_addr >= get_memory_size_total())  // fake memory
		return 0;
...
int dvr_guardband_check(unsigned long phys_addr, int value)
{
	...
	if (rtkcma_list[DEV_CMA3].exist && (rtkcma_list[DEV_CMA3].bp == NULL) && in_cma_range(rtkcma_list[DEV_CMA3].dev, phys_addr >> PAGE_SHIFT))
		return count_page; // return original page size
...
int remap_list_mem(rtk_record *records)
{
	...
	if ((type & ID_MASK) == DRIVER_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by driver) %pS[%s] (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == NOSAVE_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by nosave) %pS[%s] (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == CACHE_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli cache) %pS[%s] (%ld.%03ld sec)\n", defer, addr, type & 0x00ffffff, pid, tid, caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = type & 0x00ffffff;
	} else if ((type & ID_MASK) == BUDDY_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli buddy) %pS[%s] (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == BUDDY_MESG_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by pli buddy for user layer) %s[%s] (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, (const char *)caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	} else if ((type & ID_MASK) == DRIVER_MESG_ID) {
		rtd_pr_rmm_notice(" allocation address(%d): %08lx  size: %8lx (%6ld/%6ld)(by driver) %s[%s] (%ld.%03ld sec)\n", defer, addr, (type & 0x00ffffff) << PAGE_SHIFT, pid, tid, (const char *)caller, __dcmt_module(records->owner), msec / 1000, msec % 1000);
		ret = (type & 0x00ffffff) << PAGE_SHIFT;
	}
	else {
...
//dvr_cma.c cma fs sync helper
extern struct work_struct cma_sync_work_q;
extern void cma_sync_work(struct work_struct *work);

int __init remap_init(void)
{
#if 0 //CONFIG_DMABUF_HEAPS
	dvr_heap = dma_heap_find("dvr_heap");
	carveout_heap = dma_heap_find("carveout_heap");
	vbm_heap = dma_heap_find("vbm_heap");
#endif
