...
__always_inline unsigned int rtktlb_uaccess_enable(void)
{
	unsigned int flags = 0;
#ifdef CONFIG_ARM64
	uaccess_ttbr0_enable();
#else
	flags = uaccess_save_and_enable();
#endif
	return flags;
}

__always_inline void rtktlb_uaccess_disable(unsigned int flags)
{
#ifdef CONFIG_ARM64
...
#ifdef MODULE 
struct vm_area_struct * find_vma_ex (struct mm_struct * mm, unsigned long addr)
{
...
int remap_pfn_range_ex(struct vm_area_struct *vma, unsigned long addr,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0))
    down_write(& vm_mm->mmap_lock);
#else 
    down_write(& vm_mm->mmap_sem);
#endif
 
    err=remap_pfn_range(vma,addr,pfn,size,prot);
 
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0))
    up_write(& vm_mm->mmap_lock);
#else 
...
#define  remap_pfn_range_ex  remap_pfn_range
#endif
...
#endif
	BUG_ON(remap_pfn_range_ex(pli_area, uaddr, phys_addr >> PAGE_SHIFT, size, pli_area->vm_page_prot));
...
unsigned long pli_unmap_memory(struct file *filp, unsigned long virt_addr)
{
	...
	pli_area = find_vma_ex(current->mm, virt_addr);
	BUG_ON(pli_area == NULL);
...
unsigned long rtktlb_mmap(unsigned long addr)
{
	...
	rtk_area = find_vma_ex(current->mm, addr + DEF_MAP_SIZE);
	BUG_ON(!rtk_area);
...
#endif
	if (remap_pfn_range_ex(rtk_area, addr + DEF_MAP_SIZE, RBUS_BASE_PHYS >> 12, RBUS_USER_MAP_SIZE, rtk_area->vm_page_prot))
