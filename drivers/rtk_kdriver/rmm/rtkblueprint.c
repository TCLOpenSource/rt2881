...
void init_rtkbp(struct mem_bp *bp, unsigned long base_pfn, int count)
{
	...
	while ((count -= 256) >= 0) {
		...
		if (unlikely(is_banned(base_pfn+count, BAN_ALL_TYPE)))
			continue;
#if !defined(CONFIG_REALTEK_MEMORY_MANAGEMENT_MODULE) && !defined(CONFIG_FAKEMEMORY_GRAPHICS)
...
unsigned long show_rtkbp_by_order(struct mem_bp *bp, int order)
{
	unsigned long count = 0;
	int idx;

	if (bp == NULL || order >= RTK_BP_MAX_ORDER)
		return count;

    spin_lock_irq(&rtkbp_lock);
	for (idx = order; idx < RTK_BP_MAX_ORDER; idx++) {
		count += (int)bp->rtk_free_area[idx].nr_free * (1 << idx);
	}
    spin_unlock_irq(&rtkbp_lock);
