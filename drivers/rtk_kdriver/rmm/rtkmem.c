...
#define RTKMEM_IOC_QUERY_PHY_DMAHEAP_V1   _IOR('k', 0x32, rtkmem_query_address_V1)

//same as DVRDMA_CHUNKS_NUM
#define RTK_MEM_MAX_CMA_MULTI_SIZE (3)
...
extern int  dmaheap_get_multi_phys(int fd, rtkmem_query_address_V1* rec);
...
//					rtd_pr_rmm_info("*** remove1: %lx \n", rec->addr);
	...
	case RTKMEM_IOC_QUERY_PHY:
			{
				struct rtkmem_query_address query_info = {0};
				phys_addr_t addr = 0;
				...
				addr =	ion_to_phys( (int)query_info.fd);
	...
	case RTKMEM_IOC_QUERY_PHY_DMAHEAP:
	{
		...
		addr = 	dmaheap_to_phys( (int)query_info.fd);
	...
	case RTKMEM_IOC_QUERY_PHY_DMAHEAP_V1:
	{

		rtkmem_query_address_V1 query_info = {0};
		int ret = -1;
		
		if (copy_from_user(&query_info, (rtkmem_query_address_V1 *)arg, sizeof(query_info))) {
			rtd_pr_rmm_err("rtkmem query V1 copy_from_user fail   \n" );
			return -EFAULT;
		}

		ret = dmaheap_get_multi_phys((int)query_info.fd, &query_info);

		
		if( ret != 0 )
		{
			rtd_pr_rmm_err("rtkmem query V1 fail %d \n", query_info.fd );

			return -EINVAL;
		}

		if (copy_to_user((void *)arg, &(query_info), sizeof(query_info)) != 0) {
			rtd_pr_rmm_err("rtkmem queryV1 copy_to_user fail %d addr %pa si:%d\n", query_info.fd, &query_info.address,
				query_info.address_size );
	...
	default:
		rtd_pr_rmm_err("rtkmem invalid cmd :%d  others:%d %d\n", cmd, RTKMEM_IOC_QUERY_PHY_DMAHEAP_V1, RTKMEM_IOC_QUERY_PHY_DMAHEAP);
		return -EINVAL;
