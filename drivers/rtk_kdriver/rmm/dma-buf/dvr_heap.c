...
unsigned long dvr_heap_alloc_ve_addr(unsigned int size)
{
...
int  dmaheap_get_multi_phys(int fd, rtkmem_query_address_V1* rec)
{
	...
	if ( IS_ERR(dmabuf) ) {
		rtd_pr_rmm_err("%s() fail buf_get fd, fd(%d)\n", __func__, fd);
		goto err_out;
	}
	...
	if( rec->version != RTKMEM_QUERY_ADDR_V1_VERSION ) {
		rtd_pr_rmm_err("get_multi_phys fail ver:0x%X (0x%x), %d\n", rec->version, RTKMEM_QUERY_ADDR_V1_VERSION, fd);
		
		//goto err_out;  //  need reEnable 
	} 
	
    //rec->fd;
    rec->stamp_id = buffer->stamp_id;
	rec->multi_seg_count = 1;

	rec->address = buffer->phys_addr;
    rec->address_size = buffer->buf_0_size;

	if ( buffer->chunk_count != 0 )
	{
		if( buffer->chunk_pages[0] != NULL) {
		 rec->addr_2 = page_to_phys(buffer->chunk_pages[0]);
		 rec->addr_size_2 = (unsigned int) buffer->chunk_size[0];
		}
		 
		 if( buffer->chunk_pages[1] != NULL) {
		 	rec->addr_3 = page_to_phys(buffer->chunk_pages[1]);
		 	rec->addr_size_3 = (unsigned int) buffer->chunk_size[1];
		 }

		rec->multi_seg_count += buffer->chunk_count;

	}

	DVR_HEAP_DEBUG_PRINT(2, "kkk2 dvrMulti query phy. 0x%lx:%lu, 0x%lx:%lu :%d %d\n", rec->address, rec->address_size,
			rec->addr_2, rec->addr_size_2, rec->multi_seg_count, buffer->chunk_count );  
...
EXPORT_SYMBOL( dmaheap_get_multi_phys );
...
//	pgoff_t pg;
	...
	if (heap_flags & DMA_HEAP_FLAG_FOR_VE ) {
		snprintf(flags_str, FLAG_STR_SIZE, "DMABUF_VE:%lx_%llu", heap_flags, gDVR_HEAP_STAMP);
	}
	else if ( heap_flags & DMA_HEAP_FLAG_FOR_FM ) {
		snprintf(flags_str, FLAG_STR_SIZE, "DMABUF_FM:%lx_%llu", heap_flags, gDVR_HEAP_STAMP);
	}
	else if ( heap_flags & DMA_HEAP_FLAG_FOR_MULTI )
		snprintf(flags_str, FLAG_STR_SIZE, "GraMul:%lx_%llu", heap_flags, gDVR_HEAP_STAMP);
	else {
		snprintf(flags_str, FLAG_STR_SIZE, "DMABUF_%lx_%llu", heap_flags, gDVR_HEAP_STAMP);
	}
  ...
  for_each_sg( table->sgl, sg, table->nents, i) 
  {
	...
	if ( !(buffer->flags & DMA_HEAP_FLAG_FOR_FM ) )
	{
		if (PageHighMem( page )) {
			...
			//memset( vaddr, 0, PAGE_ALIGN(sg_len) );
			memset( vaddr, 0, sg_len );
			dmac_flush_range(vaddr, vaddr + sg_len);
		}
