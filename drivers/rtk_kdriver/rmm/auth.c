...
struct rtkcma rtkcma_list[] = {
	...
	[DEV_CMA3]			= { "cma3",		NULL, NULL, NULL, &dvr_bp[3],       0 },	// gal
...
void unset_rtkcma_list_bp(rtkcma_dev_t dev)
{
	rtkcma_list[dev].bp = NULL;
}
...
long auth_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		{
			mem_alloc_struct alloc = {0};
			ret = copy_from_user(&alloc, (void *)arg, sizeof(mem_alloc_struct));
			if (ret) {
				rtd_pr_rmm_err("auth: AUTH_IOCQ_ALLOC copy_from_user fail, ret=%d\n", ret);
				break;
			}
			ret = pli_malloc_mesg(alloc.size, (int)alloc.flag, alloc.str);
			if (ret == INVALID_VAL)
				ret = 0;
			else
		...
		case AUTH_IOCQ_ALLOCAV:
		{
			...
			ret = copy_from_user(&alloc, (void *)arg, sizeof(mem_alloc_struct));
			if (ret) {
				rtd_pr_rmm_err("auth: AUTH_IOCQ_ALLOCAV copy_from_user fail, ret=%d\n", ret);
				break;
			}
			ret = pli_malloc_mesg(alloc.size, (int)alloc.flag, alloc.str);
			if (ret == INVALID_VAL) {
				...
				flags = rtktlb_uaccess_enable();
				dmac_flush_range((void *)ptr, (void *)(ptr+alloc.size));
				rtktlb_uaccess_disable(flags);
		...
		case AUTH_IOCS_FLUSHVIRT:
		{
			...
			if (!ret) {
				vm_area = find_vma_ex(current->mm, region.addr);
...
//				}
		...
		case AUTH_IOCSINITLOGBUF:
		{
			...
			if (!ret) {
				vm_area = find_vma_ex(current->mm, log_struct.addr);
				if(vm_area) {
...
long auth_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_AUTH_IOCQ_ALLOC:
	{
		...
		compat_mem_alloc_struct alloc = {0};
		ret = copy_from_user(&alloc, user_req, sizeof(compat_mem_alloc_struct));
		if (ret) {
			rtd_pr_rmm_err("auth: COMPAT_AUTH_IOCQ_ALLOC copy_from_user fail, ret=%d\n", ret);
			break;
		}
		ret = pli_malloc_mesg(alloc.size, (int)alloc.flag, alloc.str);
		if (ret == INVALID_VAL)
	...
	case COMPAT_AUTH_IOCQ_ALLOCAV:
	{
		...
		ret = copy_from_user(&alloc, user_req, sizeof(compat_mem_alloc_struct));
		if (ret) {
			rtd_pr_rmm_err("auth: COMPAT_AUTH_IOCQ_ALLOCAV copy_from_user fail, ret=%d\n", ret);
			break;
		}
		ret = pli_malloc_mesg(alloc.size, (int)alloc.flag, alloc.str);
		if (ret == INVALID_VAL) {
...
static int __init init_gpu_mem_region(void)
{
	...
	unsigned long size = FAKEMEMORY_SIZE;
#else
	unsigned long size = FAKEMEMORY_SIZE - FAKEMEMORY_GAP_SIZE;
#endif
	unsigned long totalddr = get_memory_size_total();

	#ifdef CONFIG_ARCH_RTK2819A
		// > 8G
	  unsigned long paddr = 0x200000000 + FAKEMEMORY_GAP_SIZE;
	#else
	  unsigned long paddr = totalddr + FAKEMEMORY_GAP_SIZE;
	#endif//


	
	unsigned long base_pfn, count;
//	void *kva = NULL;
	...
	if (totalddr > 0x10000000) {
		...
		rtd_pr_rmm_info("%s, total(%x/%x)\n", __func__, memblock_end_of_DRAM(), totalddr);
		dump_page(pfn_to_page(PFN_DOWN(totalddr)), NULL);
		dump_page(pfn_to_page(PFN_DOWN(totalddr + FAKEMEMORY_GAP_SIZE)), NULL);
#else
#error "!no support memory model for fake memory"
#endif
		if (ret) {
...
unsigned long get_gpu_mem_region(unsigned long *addr)
{
	int count;
	struct mem_bp *bp = &gpu_mem_bp;
	
	if (bp->rtk_mem_map == 0) {
		rtd_pr_rmm_err("auth: there is no gpu region in this platform %d...\n", __LINE__);
		if (addr)
			*addr = (unsigned long)INVALID_VAL;

		return 0;
	}

	if (addr)
		*addr = (unsigned long)bp->base_pfn << PAGE_SHIFT;

	count = bp->max_mem_map - bp->rtk_mem_map;
	
	return (unsigned long)count << PAGE_SHIFT;
}
EXPORT_SYMBOL(get_gpu_mem_region);
...
int __init auth_init(void)
{
	...
	for (minor = DEV_CMA1_DEFAULT; minor < ARRAY_SIZE(rtkcma_list); minor++) {
		...
		if (strncmp("cma3", rtkcma_list[minor].name, 4) == 0) {
...
#if defined(CONFIG_SYSFS) && defined(CONFIG_REALTEK_MEMORY_MANAGEMENT)
int cma_get_video_alloc(void);
static ssize_t
video_show(struct kobject *kobj, struct kobj_attribute *attr,
		    char *buf)
{
	int size = cma_get_video_alloc();

	return sprintf(buf, "%llu\n", (u64)size);
}

extern int cma_get_graphic_alloc(void);

unsigned int (*get_graphic_gpu_alloc) (void) = NULL;
void get_total_GPU_memory_register (uint32_t (*fp)(void))
{
	if (fp != NULL)
		get_graphic_gpu_alloc = fp;
}
EXPORT_SYMBOL(get_total_GPU_memory_register);

static ssize_t
graphic_show(struct kobject *kobj, struct kobj_attribute *attr,
		    char *buf)
{
	unsigned int size = (unsigned int)cma_get_graphic_alloc();
	rtd_pr_rmm_notice("gpu render target=%ld\n", size);
	if (get_graphic_gpu_alloc != NULL) {
		size += (unsigned int)get_graphic_gpu_alloc();
		rtd_pr_rmm_notice("gpu texture=%ld\n", (unsigned int)get_graphic_gpu_alloc());
	}

	return sprintf(buf, "%llu\n", (u64)size);
}

static struct kobj_attribute video_attr =
	__ATTR_RO(video);

static struct kobj_attribute graphic_attr =
	__ATTR_RO(graphic);
...
static struct attribute_group rtk_attr_group = {
	.attrs = rtk_attrs,
};

static int __init mm_rtk_sysfs(void)
{
	int err;
	struct kobject *rtk_kobj;

	rtk_kobj = kobject_create_and_add("rtk", mm_kobj);
	if (!rtk_kobj) {
		rtd_pr_rmm_err("failed to create rtk kobject\n");
		return -ENOMEM;
	}
	err = sysfs_create_group(rtk_kobj, &rtk_attr_group);
	if (err) {
		rtd_pr_rmm_err("failed to register rtk group\n");
		goto delete_obj;
	}
	return 0;

delete_obj:
	kobject_put(rtk_kobj);
	return err;
}
device_initcall(mm_rtk_sysfs);
#endif