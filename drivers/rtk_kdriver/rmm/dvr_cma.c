...
retry:
		...
		if (sync_fs_once_when_ebusy) {
			...
			rmm_vh_cma_alloc_busy_info();
		}
#else
		rmm_vh_cma_alloc_busy_info(&info);
#endif
	}
	while (pn_idx > 0) {
		pn_idx--;
...
EXPORT_SYMBOL(pQShowVBM_1); // for scaler dual path, 2nd zone
...
int __weak reclaim_logo_memory(void)
{
    ...
    unsigned long logo_addr, logo_size;
    u64 tmp64[2];
...
void (*drvif_memory_DisableMCap_callback)(unsigned char) = NULL;
void (*disable_i3ddma_cap_callback)(void) = NULL;
EXPORT_SYMBOL(disable_i3ddma_cap_callback);
EXPORT_SYMBOL(drvif_memory_DisableMCap_callback);
...
//	unsigned int *unlzma = (unsigned int *)phys_to_virt(UNLZMA_SYNC_ADDR);
  ...
  #endif
#endif
	rtd_pr_rmm_err("\033[1;44m [%s] 123 cmd=%d...\033[0m\n", __FUNCTION__, cmd);
	/*
	 * Reclaim pAnimation
...
//		#endif
		...
		#endif
		rtd_pr_rmm_info("%s: comm[%s] call cmd=%d\n", __func__, current->comm, cmd);
		reclaim_logo_memory();
	}
...
#ifdef CONFIG_DMABUF_HEAPS_VBM
			vbm_heap_dump_chunk_list(); // dvr_dmabuf_free is asynchrous, here just dump info before free
#endif

			if (drvif_memory_DisableMCap_callback){
				drvif_memory_DisableMCap_callback(0);
			}

			if (disable_i3ddma_cap_callback){
				disable_i3ddma_cap_callback();
			}
            //drvif_memory_DisableMCap(0);
            //disable_i3ddma_cap();
			rtd_pr_rmm_info("%s release qshow vbm(%p) 0x%lx@0x%lx\n", __func__, pQShowVBM,
				pQShowVBM->size + ((pQShowVBM_1)?(pQShowVBM_1->size):0), dvr_dmabuf_to_phys(pQShowVBM));

			dvr_dmabuf_free(pQShowVBM);
			pQShowVBM = NULL;

			if (pQShowVBM_1) {
				dvr_dmabuf_free(pQShowVBM_1);
				rtd_pr_rmm_debug("%s release qshow 2nd vbm 0x%lx@0x%lx\n", __func__, pQShowVBM_1->size, dvr_dmabuf_to_phys(pQShowVBM_1));
				pQShowVBM_1 = NULL;
			}

		}
		else {
			rtd_pr_rmm_info("%s qshow vbm not available\n", __func__);
#ifdef CONFIG_DMABUF_HEAPS_VBM
