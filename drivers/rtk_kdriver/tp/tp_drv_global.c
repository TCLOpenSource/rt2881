...
INT32 tp_allocate_section_info_buffer(void)
{
    TP_DRV_T* p_this = pTp_drv;
    ...
    if (p_this) {
        ...
        if(p_this->info_pack_buf_cached == NULL) {
        ...
        if(p_this->section_data_buf_of_pid_filter == NULL) {
...
EXPORT_SYMBOL(tp_allocate_section_info_buffer);

void tp_release_section_info_buffer(void)
{
    if (pTp_drv) {
...
EXPORT_SYMBOL(tp_release_section_info_buffer);
...
EXPORT_SYMBOL(release_tp_controller);
