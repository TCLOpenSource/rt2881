...
INT32 Tp_SetTpFrmReg(TPK_TP_ENGINE_T tp_id, UINT32 data){
        REGADDR tp_tf_frm_reg;
        ...
        WRITE_REG32(tp_tf_frm_reg,data);
...
//disable GCK,gck function will cause overflow,when frm_en->0
#ifdef TP_TP_GCK
...
#ifdef CONFIG_RTK_KDRV_JP4K
        Tp_Get_Buffer_Full_status(buf_idx, &full_status);
        if(full_status){
            Tp_ENG_FrameEnable(i, 0);
            pTp_drv->tp[i].IsMassBufferFull = 1;
            Tp_Set_Buffer_Full_status(buf_idx, 0);
            TP_WARNING("##### TP isr. TP%d index:%d ring buffer full\n", i, buf_idx);
        }
#endif
            }
#ifdef TP_TP_ATSC_STATUS
    regValue = READ_REG32(TP_TP_ATSC_STATUS_reg);
    if(TP_TP_ATSC_STATUS_get_dring_ov_int(regValue)) {
        tp_tp_atsc_status_RBUS atsc_status;
        atsc_status.regValue = READ_REG32(TP_TP_ATSC_STATUS_reg);
        atsc_status.regValue |= TP_TP_ATSC_STATUS_dring_ov_int_mask;
        WRITE_REG32(TP_TP_ATSC_STATUS_reg, atsc_status.regValue);
        TP_WARNING("##### atsc3 overflow\n");
    }
#endif
#ifdef CONFIG_RTK_KDRV_JP4K
    //TP_WARNING("##### TP isr\n");
...
INT32 Tp_init(TPK_TP_ENGINE_T tp_id)
{
        ...
        tp_frm.lockno         = 1;
        tp_frm.serial         = 1;
        tp_frm.syncmode       = 0;//0x1F for external demod
...
#ifdef TP_USE_IRQ
        tp0_int_en.regValue   = TP_TP_TF0_INT_sync_mask | TP_TP_TF0_INT_drop_mask | TP_TP_TF0_INT_overflow_mask | TP_TP_TF0_INT_write_data_mask; // Enable adf_pcr_in_en
#else
...
#ifdef CONFIG_RTK_TP_WITH_MIO
        tp_out_ctrl.tp_frame_rate        = (188+5)*4;//188 + 32;
#else
...
INT32 Tp_TPO_UpdateTPFrameRate(void){
        tp_tp_out_ctrl_RBUS tp_out_ctrl;
        UINT32 tp_frame_rate = 0;
        UINT32 cur_pre_div = 0, cur_post_div = 0, cur_div16 = 0;

        if(pTp_drv->isFixedClkCam){
                /* re-calculate tp frame rate for fixed cam card
                tp_frame_rate = ((div_o/div_f) x 188) x 4
                */
                Tp_TPO_GetClkDiv(&cur_pre_div,&cur_post_div,&cur_div16);
                tp_frame_rate = (((((pTp_drv->tpo_pre_div+pTp_drv->tpo_post_div) + 2)*100)/((cur_pre_div+cur_post_div) + 2))*188) * 4;
                tp_frame_rate = tp_frame_rate / 100;
                        if(tp_frame_rate > 8191)
                                tp_frame_rate = 8191;
	}else{
//#ifdef CONFIG_RTK_TP_WITH_MIO
#if 0
                        tp_frame_rate	 = (188+5)*4;//188 + 32;
#else
                        tp_frame_rate	 = 188 + 5; // Default: 188 + 12
#endif
	}

	tp_out_ctrl.regValue		= READ_REG32(TP_TP_OUT_CTRL_reg);
	if(tp_out_ctrl.tp_frame_rate != tp_frame_rate){
                tp_out_ctrl.tp_frame_rate	= tp_frame_rate;
                WRITE_REG32(TP_TP_OUT_CTRL_reg, tp_out_ctrl.regValue);
	}
	return TPK_SUCCESS;
}
...
#ifdef CONFIG_RTK_TP_WITH_MIO
void Tp_TPO_SetTPOClkForFixedCam(UINT8 tp_id){
        UINT32 pre_div = 0,post_div = 0,div_16 = 0,init_div = 0,cur_div = 0,cnt=0;
        UINT32 timecnt = READ_REG32(TP_TP_PCRA_CNT_LOW_reg);

        pTp_drv->isTPOClkAdjusting = true;

        Tp_TPO_GetClkDiv(&pre_div,&post_div,&div_16);
        init_div = pre_div + post_div;
        cur_div = init_div;
        ...
        while(cur_div > 2){
                ...
                post_div 	= cur_div-pre_div;

                _tp_lock();
                Tp_TPO_SetClkDiv(pre_div, post_div, 0);
                Tp_TPO_UpdateTPFrameRate();
                _tp_unlock();
                ...
                if((READ_REG32(ISO_SPI_SYNCHRONIZER_TP_DBG_reg) & 0x1) == 0 ){

                        msleep(50);//delay 50ms for double check
                        if((READ_REG32(ISO_SPI_SYNCHRONIZER_TP_DBG_reg) & 0x1) == 0 ){
                                break;
                        }
                }
        }
        pTp_drv->isTPOClkAdjusting = false;
        TP_WARNING("fixed take time=%dms from %dKHz to %dKHz, change cnt=%d\n",(READ_REG32(TP_TP_PCRA_CNT_LOW_reg)-timecnt)/90,((199*1000)/(init_div+2)),((199*1000)/(cur_div+2)),cnt);
}
#endif
#endif
...
EXPORT_SYMBOL(Tp_Buf_Vir_to_Phy);
...
EXPORT_SYMBOL(Tp_Buf_Phy_to_Vir);
                  ...
                  UINT32 Threshold, UINT8 buffer_flag)
{
        ...
        if((pBuf->inited == 0) ||
            (pTp_drv->tp[tp_id].tp_stream_status != TP_STREAM_START)){
                if(pTp_drv->deliveryMode[tp_id] == TP_DELIVERY_MODE_TS)
...
INT32 Tp_GetPidParam(UINT16 pid_idx, TPK_PID_REG_PARAM_T* pParam)
{
        ...
        pParam->pid_descramble_disable = pid_data2.pid_des_disable;
#ifdef CONFIG_RTK_KDRV_SUPPORT_NAGNA
		/* Nagxx param */
...
INT32 Tp_Remove_PID(TPK_TP_ENGINE_T tp_id, UINT16 pid_idx)
{
        ...
        memset(&pid_reg, 0, sizeof(TPK_PID_REG_PARAM_T));
...
INT32 Tp_GetPID_info(TPK_TP_ENGINE_T tp_id, UINT8 pid_idx_reg, UINT16 *PID, UINT8 *pPid_idx)
{
        ...
        if(*pPid_idx >= pid_filter_count || *pPid_idx >= MAX_PID_PER_TP) {
                TP_WARNING("unsupported pid_idx_reg=%u ,tp_id=%d\n", pid_idx_reg, tp_id);
                return TPK_BAD_PARAMETER;
        }

        if(*pPid_idx >= 0 && *pPid_idx < MAX_PID_PER_TP && pTp_drv->tp_pid[tp_id].pid_api_setting[*pPid_idx].valid == 0) {
                TP_WARNING("TP%u, pid_idx=%u not init yet\n", tp_id, *pPid_idx);
                return TPK_NOT_INIT;
        }

        if(*pPid_idx >= 0 && *pPid_idx < MAX_PID_PER_TP)
...
INT32 Tp_ParseInfoPack(TPK_TP_ENGINE_T tp_id, UINT8* pData, SEC_INFO_T* pInfo) /* pData is virtual memory */
{
        ...
        if ((pData[0]& SEC_INFO_PACK_SEL) == SEC_INFO_PACK) {
                ...
                if(result != TPK_SUCCESS) {
                    TP_WARNING("TP%d L:%d call Tp_IsValidPhyAddr return error (%d)\n", tp_id, __LINE__, result);
                    return result;
                }
...
static INT32 section_assemble_thread(void* arg)
{
        ...
        Tp_get_curtime(&saved_time);
        Tp_get_curtime(&saved_time_check_tpp_buffer);
        ...
        while(!kthread_should_stop()) {
                Tp_get_curtime(&new_time);
                saved_time_check_tpp_buffer.tv_usec = 0;
                saved_time_check_tpp_buffer.tv_sec = 0;
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
#ifdef CONFIG_RTK_KDRV_JP4K
                        if(pTp_drv->deliveryMode[i] != TP_DELIVERY_MODE_TS)
                            continue;
#endif
                        /* update PCR tracking status */
                        _tp_lock();
...
static INT32 tp_dbg_thread(void* arg)
{
        ...
        TP_BUFFER_PARAM_T ring_buf, info_buf, startcode_buf = {0};
        UINT32 ring_buf_cnt, ring_buf_sz, info_buf_cnt, info_buf_sz, startcode_buf_cnt, startcode_buf_sz;
        wait_queue_head_t timeout_wq;
        UINT32 time_cnt_dbg, time_cnt_status;

        REGADDR tp_cntl_reg =0;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
        {
                struct timespec64 t64;
                ktime_get_real_ts64(&t64);
                new_time.tv_sec = t64.tv_sec;
                new_time.tv_usec = t64.tv_nsec / 1000;
        }
#else
                Tp_get_curtime(&new_time);
#endif
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
#ifdef CONFIG_RTK_KDRV_JP4K
                        if(tp_isr_cnt[i]){
                                Tp_IsrDumpTPRegister();
                                if(time_cnt_status >= status_log_interval_ms)
                                        TP_DBG_SIMPLE("[TP%d] overflow happened!!!, overflow cnt:%d\n", i, tp_isr_cnt[i]);
                        }
#endif
                        if( tp_log_onoff && (time_cnt_status >= status_log_interval_ms)) {
                                ...
                                if(pTp_drv->tp_japan[i].isJAPAN4K == 1) {
                                        ...
                                        TP_WARNING("[TP%d]flowrate=%d full=%u stc(<250ms)=%d stc(>250ms)=%d stc_max=%d\n", i, pTp_drv->tp_japan[i].for_debug_tlv_total_size / time_cnt_status, pTp_drv->tp_japan[i].for_debug_ring_buffer_full_cnt, pTp_drv->tp_japan[i].timestamp_range1, pTp_drv->tp_japan[i].timestamp_range2, pTp_drv->tp_japan[i].timestamp_max);
                ...
                for( i = 0; i < MAX_TP_COUNT; i++) {
                        ...
                        TP_TRACE("tp_cntl_reg = %d\n", READ_REG32(tp_cntl_reg));

                        //check if fix clk cam

#ifdef CONFIG_RTK_TP_WITH_MIO
                        {
                                tp_tp_tf0_cntl_RBUS tp_cntl;

                                //condition: TP is cam src && mio tp is overflow && pTp_drv->isFixedCam
                                tp_cntl.regValue = 0;
                                tp_cntl.regValue = READ_REG32(tp_cntl_reg);

                                if((tp_cntl.src_sel_mio==1) && (READ_REG32(ISO_SPI_SYNCHRONIZER_TP_DBG_reg) & 0x1)){
                                        TP_DBG("cam out clk is fixed, overflow happen,need enlarge tpo clk\n");
                                        pTp_drv->isFixedClkCam = true;
                                        if(!pTp_drv->isTPOClkAdjusting)
                                                   Tp_TPO_SetTPOClkForFixedCam(i);
                                }
                        }
#endif
                        if(pTp_drv->tp[i].tp_stream_status == TP_STREAM_STOP )
                                continue;
                        ...
                        if( tp_log_onoff && (time_cnt_dbg >= debug_log_interval_ms)) {
                                ...
                                if ( startcode_buf_sz && pTp_drv->tp[i].Shared_RWPtr.inited ) {
                                        ...
                                        TP_DBG_SIMPLE("TP%u: pattern=%u/%u ("PT_FMT_UADDR"(0x%p)/"PT_FMT_UADDR"(0x%p)/"PT_FMT_UADDR"/"PT_FMT_UADDR") pvr=0x%08X/0x%08X\n",
                                                      i ,
                                                      startcode_buf_cnt, startcode_buf_sz,
...
INT32 Tp_PCRTrackingEnable(TPK_TP_ENGINE_T tp_id , UINT8 On, UINT16 PID, TPK_PCR_CLK_SRC_T clk_src)
{
        ...
        memset(&pParam, 0, sizeof(TPK_PID_REG_PARAM_T));
...
INT32 Tp_SetPatternSearchInfoBuffer( TPK_TP_ENGINE_T tp_id, UINT8* pPhyAddr, UINT8* pVirAddr, UINT32 size, UINT8* pSharedRP, UINT8* pSharedWP)
{
        ...
        pTp_drv->tp[tp_id].Shared_RWPtr_start_code_search.pRP    = (UINT32*)pSharedRP;
        pTp_drv->tp[tp_id].Shared_RWPtr_start_code_search.pWP    = (UINT32*)pSharedWP;
...
INT32 Tp_FRC_GetEnable( TPK_TP_ENGINE_T tp_id)
{
        ...
        switch(tp_id) {
                case TP_TP0:
                        enable = fr_ctrl.tp0_en;
                        break;
                case TP_TP1:
                        enable = fr_ctrl.tp1_en;
                        break;
                case TP_TP2:
                        enable = fr_ctrl.tp2_en;
                        break;
#ifdef TP_SUPPORT_TP3
                case TP_TP3:
                        enable = fr_ctrl.tp3_en;
                        break;
#endif
                default:
                        TP_WARNING("bad parameter - tp_id:%d\n", tp_id);
...
INT32 Tp_ATSC30_Setting(TPK_TP_ENGINE_T tp_id)
{
    ...
    WRITE_REG32(TP_TP_ATSC_CONFIG1_reg,     0x47040110);
    WRITE_REG32(TP_TP_ATSC_CONFIG2_reg,     0x5a5a5a5a);
    WRITE_REG32(TP_TP_ATSC_CONFIG3_reg,     0x00000011);

#ifdef TP_TP_RING_OPTION_tpp_write_policy_mask
    {
        /*TPP write policy always wlast for atsc3*/
        tp_tp_ring_option_RBUS tp_ring_option;
        tp_ring_option.regValue = READ_REG32(TP_TP_RING_OPTION_reg);
        tp_ring_option.regValue |= TP_TP_RING_OPTION_tpp_write_policy_mask;
        WRITE_REG32(TP_TP_RING_OPTION_reg, tp_ring_option.regValue);
    }
#endif
#ifdef TP_TP_ATSC_STATUS
    {
        /*enable atsc3 overflow interrupt*/
        tp_tp_atsc_status_RBUS atsc_status;
        atsc_status.regValue = READ_REG32(TP_TP_ATSC_STATUS_reg);
        atsc_status.regValue |= TP_TP_ATSC_STATUS_dring_ov_int_en_mask;
        WRITE_REG32(TP_TP_ATSC_STATUS_reg, atsc_status.regValue);
    }
#endif
...
#ifdef CONFIG_RTK_KDRV_JP4K
        {"TP_TF3_CNTL           ", TP_TP_TF3_CNTL_reg},
        {"TP_TF3_STRM_ID_0      ", TP_TP_TF3_STRM_ID_0_reg},
        {"TP_TF3_STRM_ID_1      ", TP_TP_TF3_STRM_ID_1_reg},
        {"TP_TF3_STRM_ID_2      ", TP_TP_TF3_STRM_ID_2_reg},
        {"TP_TF3_STRM_ID_3      ", TP_TP_TF3_STRM_ID_3_reg},
        {"TP_TF3_STRM_VAL       ", TP_TP_TF3_STRM_VAL_reg},
        {"TP_TF3_CNT            ", TP_TP_TF3_CNT_reg},
        {"TP_TF3_DRP_CNT        ", TP_TP_TF3_DRP_CNT_reg},
        {"TP_TF3_ERR_CNT        ", TP_TP_TF3_ERR_CNT_reg},
        {"TP_TF3_FRMCFG         ", TP_TP_TF3_FRMCFG_reg},
        {"TP_TF3_INT            ", TP_TP_TF3_INT_reg},
        {"TP_TF3_INT_EN         ", TP_TP_TF3_INT_EN_reg},
#endif
#ifdef TP_SUPPORT_CI
...
#ifdef CONFIG_RTK_KDRV_JP4K
        {"TP3_M2M_RING_LIMIT    ", TP_TP3_M2M_RING_LIMIT_reg},
        {"TP3_M2M_RING_BASE     ", TP_TP3_M2M_RING_BASE_reg},
        {"TP3_M2M_RING_RP       ", TP_TP3_M2M_RING_RP_reg},
        {"TP3_M2M_RING_WP       ", TP_TP3_M2M_RING_WP_reg},
        {"TP3_M2M_RING_CTRL     ", TP_TP3_M2M_RING_CTRL_reg},
#endif
#ifdef TP_SUPPORT_CI
...
INT32 Tp_IsrDumpTPRegister(void)
{
    ...
    for(i = 0; i < MAX_PID_FILTER_COUNT && tp_isr_cnt[i]; i++)
        TP_WARNING("[TP Debug][TP%d] overflow happened!!!, overflow cnt:%d\n", i, tp_isr_cnt[i]);

    TP_WARNING("[TP Debug]0xB8014FA8 = 0x%08X, 0xB8014FAC = 0x%08X\n", READ_REG32(0xB8014FA8), READ_REG32(0xB8014FAC));
    for(j = 0; j < MAX_PID_FILTER_COUNT; j++) {
        ...
        for(i = 1; i < sizeof(tp_register_pid_filter) / sizeof(TP_REGISTER); i++) {
            TP_WARNING("[TP Debug][reg dump][PID%02d]%s = %08X\n", j, tp_register_pid_filter[i].name, READ_REG32(tp_register_pid_filter[i].address));
        }
    }
#if 0
    for(j = 0; j < MAX_SEC_COUNT; j++) {
        ...
        for(i = 1; i < sizeof(tp_register_sec_filter) / sizeof(TP_REGISTER); i++) {
            TP_WARNING("[TP Debug][reg dump][SEC%03d]%s = %08X\n", j, tp_register_sec_filter[i].name, READ_REG32(tp_register_sec_filter[i].address));
        }
    }
#endif
    for(j = 0; j < MAX_TP_BUFFER_COUNT; j++) {
        ...
        for(i = 1; i < sizeof(tp_register_tp_ring) / sizeof(TP_REGISTER); i++) {
            TP_WARNING("[TP Debug][reg dump][Ring%02d]%s = %08X\n", j, tp_register_tp_ring[i].name, READ_REG32(tp_register_tp_ring[i].address));
        }
    }

    for(i = 0; i < sizeof(tp_register_list) / sizeof(TP_REGISTER); i++) {
        TP_WARNING("[TP Debug][reg dump]%s = %08X\n", tp_register_list[i].name, READ_REG32(tp_register_list[i].address));
    }
...
INT32 Tp_TPP_Init(TPK_TPP_ENGINE_T tpp_id)
{
        ...
        tp_p_cntl.ci_mode       = 1;
        tp_p_cntl.null_en       = 1; //[KTASKWBS-8389] picture broken issue for others_02 cam card in GN
        tp_p_cntl.sync_en       = 1;
...
INT32 TP_TLV_Config(TPK_TP_ENGINE_T tp_id, RTK_TLV_REG_T *ptlv_regs)
{
    ...
    switch(pTp_drv->tp_japan[tp_id].inputType) {
        case TPK_INPUT_SERIAL:
            tlv_ctrl_reg.data_mode = 1;
            break;
        case TPK_INPUT_TWO_BIT:
            tlv_ctrl_reg.data_reverse = 1;
            tlv_ctrl_reg.data_location = 0x6;
            tlv_ctrl_reg.data_mode = 2;
            break;
        case TPK_INPUT_PARALLEL:
        default:
            tlv_ctrl_reg.data_mode = 0;
    ...
    TP_WARNING("deli config tlv inputType:%d input_port=%d\n", pTp_drv->tp_japan[tp_id].inputType, pTp_drv->tp_japan[tp_id].input_port);
    tlv_ctrl_reg.enable = 1;
    WRITE_REG32( ptlv_regs->TLV_CTRL, tlv_ctrl_reg.regValue );
    return 0;
}
...
void TP_japan4k_parse_NTP(UINT8 *p_data, struct japan4k_dmx* p_this)
{
    ...
    if(p_data[42] == 0x01 && p_data[43] == 0x01 && p_data[47] == 0x7b && len == 100) {
        ...
        ntp_first_byte = p_data[52];

        p_this->ntp_leap_indicator = (ntp_first_byte >> 6) & 0x3;
        ...
        if(p_this->avsync_NTP < p_this->LastNTP || abs(p_this->avsync_NTP - p_this->LastNTP) > 1000*90 ){
            TP_WARNING("resync wallclock offset=%lld lastntp=%lld ntp=%lld stc=%lld modify_offset=%lld, ntpdiff:%lld, ntpfirstbyte:%d, tlv_NTP:%lld\n",
                pTp_drv->tp_japan[p_this->tp_id].wallclock_stc_offset, p_this->LastNTP, p_this->avsync_NTP, STC_Local,
                p_this->avsync_NTP - STC_Local, p_this->avsync_NTP - p_this->LastNTP, ntp_first_byte,  p_this->tlv_NTP);
            p_this->RCD = p_this->avsync_NTP - STC_Local - g_jp4k_ntpoffset;
...
EXPORT_SYMBOL(TP_japan4k_parse_NTP);
                                 ...
                                 UINT32 Threshold, UINT8 buffer_flag)
{
        ...
        if( (buffer_flag != TP_BUFFER_INFO_PACK) &&
            (pTp_drv->tp[tp_id].IsMassBufferFull) ) {
                ...
                if(buf_cnt_tmp < ( GET_BUFF_SIZE(buf_param) / 10)) {
#ifdef CONFIG_RTK_KDRV_JP4K
                        TP_WARNING("TP%u mass buffer backto normal.restart streaming(buf_flag=%u, buf_idx=%u, threshold=%u, rp="PT_FMT_UADDR"("PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"), wp="PT_FMT_UADDR", base="PT_FMT_UADDR", limit="PT_FMT_UADDR", real_sz=%ld, rt_sz=%u)\n",
                                tp_id, buffer_flag, buf_idx,
                                READ_REG32(TP_TP_THRESHOLD_reg),
                                buf_param.RP,
                                pBuf->RP_Connect[0], pBuf->RP_Connect[1], pBuf->RP_Connect[2], pBuf->RP_Connect[3],
        ...
        *ppReadPhyPtr = (UINT8*)pTp_drv->tp[tp_id].mass_buffer.now_rp;
        ...
        if(full_stataus) {
                //Tp_Get_Buffer(buf_idx, &buf_param);
#ifdef CONFIG_RTK_KDRV_JP4K
                TP_WARNING("TP%u mass data buffer(buffer_flag=%d, buf_idx=%d) full detected, Start stop streaming\n", tp_id, buffer_flag, buf_idx);
#endif
                if(pTp_drv->tp[tp_id].tp_src != MTP) {
                        if (!TP_HAS_FLAG(logStatusFlag[tp_id], TP_STATUS_LOG_FLAG_MASSBUFFFULL)) {
                                TP_WARNING("TP%u buffer full detect (buf_flag=%u, buf_idx=%u, threshold=%u, rp="PT_FMT_UADDR"("PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"), wp="PT_FMT_UADDR", base="PT_FMT_UADDR", limit="PT_FMT_UADDR", real_sz=%ld, rt_sz=%u)\n",
                ...
                if(buffer_flag == TP_BUFFER_INFO_PACK) {
                        ...
                        if (pTp_drv->tp[tp_id].tp_src != MTP) {//if playback, do not set frame enable to zero and reset framer
                            if (!pTp_drv->tp[tp_id].IsMassBufferFull) {
#ifdef CONFIG_RTK_KDRV_JP4K
                                TP_WARNING("TP%u  start stop streaming (buf_flag=%u, buf_idx=%u, threshold=%u, rp="PT_FMT_UADDR"("PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"/"PT_FMT_UADDR"), wp="PT_FMT_UADDR", base="PT_FMT_UADDR", limit="PT_FMT_UADDR", real_sz=%ld, rt_sz=%u)\n",
                                           tp_id, buffer_flag, buf_idx,
                                           READ_REG32(TP_TP_THRESHOLD_reg),
                                           buf_param.RP,
                                           pBuf->RP_Connect[0], pBuf->RP_Connect[1], pBuf->RP_Connect[2], pBuf->RP_Connect[3],
...
EXPORT_SYMBOL(TP_ReadData_NoNeedAligment);
...
EXPORT_SYMBOL(TP_ReleaseData_NoNeedAligment);
...
EXPORT_SYMBOL(TP_Get_TLVBufferInformation);

#ifdef CONFIG_RTK_TP_WITH_MIO
/* MIO TP init*/
void Tp_MIO_Reset(void)
{
        iso_spi_synchronizer_sd_tp_ctrl_RBUS mio_tp_cntl;
        /* change tp val pinmux to gpio before mio tp reset */
        unsigned int tp_val = READ_REG32(PINMUX_MIO_TP_VAL_CFG+0x100);//read mio pinmux directly
        rtd_outl(PINMUX_MIO_TP_VAL_CFG, (tp_val & ~(0xF << 12)) | (0xF << 12));

        mio_tp_cntl.regValue = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_CTRL_reg+0x60);//read mio directly
        ...
        /*restore tp val pinmux */
        udelay(100);
        rtd_outl(PINMUX_MIO_TP_VAL_CFG, tp_val);
}
...
INT32 TP_MIO_Set_T2PMode(TPK_MIO_T2P_PARAM_T Param)
{
	...
	mio_t2p_cfg.regValue            = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CFG_reg+0x60);//read mio directly
	...
	/* set ts out sync_dur */
	mio_t2p_ctrl.regValue           = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_T2P_CTRL_reg+0x60);//read mio directly
...
INT32 TP_MIO_Set_P2TMode_(TPK_MIO_P2T_PARAM_T Param)
{
	...
	/* set t2p in parameter */
	mio_p2t_cfg.regValue            = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CFG_reg+0x60);//read mio directly
	...
	/* set ts out sync_dur */
	mio_p2t_ctrl.regValue           = READ_REG32(ISO_SPI_SYNCHRONIZER_SD_TP_P2T_CTRL_reg+0x60);//read mio directly
