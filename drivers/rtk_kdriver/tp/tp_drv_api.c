                    ...
                    if (result != TPK_SUCCESS) {
                        ...
                        while (pTp_drv->tp[i].old_wp != buf_param.WP) {
                            ...
                            if (pTp_drv->tp[i].show_startcode_info_cnt < 10) {
                                ...
                                TP_DBG_SIMPLE("[%d]stco_p="PT_FMT_UADDR" stco_v=%p mass_p=%p sRP=0x%08X sWP=0x%08X WP="PT_FMT_UADDR"\n",
                            ...
                            if (buf_param_tpbuf.WP >= *pTp_drv->tp[i].Shared_RWPtr.pRP) {
                                if (!((mass_temp_phy_temp >= *pTp_drv->tp[i].Shared_RWPtr.pRP) && (mass_temp_phy_temp <= buf_param_tpbuf.WP))) {
                                    TP_ERROR("ERROR!! mass_temp_phy="PT_FMT_UADDR" is not in the range RP=0x%08X WP="PT_FMT_UADDR"\n", mass_temp_phy_temp, *pTp_drv->tp[i].Shared_RWPtr.pRP, buf_param_tpbuf.WP);
                                    wp_temp_virtual[0] = 0;
                                    wp_temp_virtual[1] = 0;
                                    wp_temp_virtual[2] = 0;
                                    wp_temp_virtual[3] = 0;
                            ...
                            IN UINT8* pSharedWP)
{
        ...
        if (enable) {
                ...
                pTp_drv->tp[tp_id].Shared_RWPtr.pRP = (unsigned int *)pSharedRP;
                pTp_drv->tp[tp_id].Shared_RWPtr.pWP = (unsigned int *)pSharedWP;
...
#ifdef CONFIG_RTK_TP_WITH_MIO
        //if it's cam src we need restore demod ts clk first
        if(tp_cntl.src_sel_mio == 1){
                pTp_drv->isFixedClkCam = false;
                if(pTp_drv->tpo_pre_div > 0){
                        Tp_TPO_SetClkDiv(pTp_drv->tpo_pre_div, pTp_drv->tpo_post_div, 0);
                }
                Tp_TPO_UpdateTPFrameRate();
                //Tp_MIO_Reset();//to avoid tpo clk always keep high problem
        }
#endif
        /* set syncmode for external demod */
        if(!bHaveCAM && (tp_src == TS_SRC_0 || tp_src == TS_SRC_1 || tp_src == TS_SRC_2)) {
                Tp_GetTpFrmReg(tp_id,&tp_frm.regValue);
                tp_frm.syncmode       = 0x1F;
                Tp_SetTpFrmReg(tp_id,tp_frm.regValue);
        }
...
INT32 RHAL_TPOUT_SetTSClock( UINT32 TsClkKHz )
{
#ifdef TP_SUPPORT_CI
    UINT32                  pre_div;
    UINT32                  post_div;
    UINT32                  div16 = 0;
    unsigned int rbus_pll;
    unsigned int div_result;

    CHECK_TP_STRUCTURE();
    _tp_lock();

    //rbus_pll = pll_info_bus()/2; /* Improve accuracy like: 3.7 -> 4 */

    {
        unsigned long long param3 = 0;
        unsigned int autosharepin_en = 0;
        if (pcb_mgr_get_enum_info_byname("PCMCIA_CFG3", &param3) == 0) {
            autosharepin_en = param3 & 0x1;
            if(autosharepin_en) {
                TP_WARNING("Increase TPO clock if autopinshare en, demod set TsClkKHz:%u \n", TsClkKHz);
                TsClkKHz = TsClkKHz + 1500;
                if(TsClkKHz < 5000)
                   TsClkKHz = 5000;
            }
        }
    ...
    rbus_pll = TP_VDEC_SPEED_MHZ;
#else
    rbus_pll = TP_SPEED_MHZ;
#endif

    /* rbus pll mhz to khz x1000 */
#ifdef CONFIG_RTK_TP_WITH_MIO
    div_result = (rbus_pll * 1000)/(4*TsClkKHz); /* 8bit to 2bit,so increase tpo clk*/
    if(div_result < 4)
        div_result = 4;
    div_result = div_result - 2;
#else
    div_result = (rbus_pll * 1000)/TsClkKHz;
    if(div_result < 2)
        div_result = 2;
    div_result = div_result - 2;
#endif
    ...
    pTp_drv->tpo_pre_div 	= pre_div;
    pTp_drv->tpo_post_div 	= post_div;

    if(pTp_drv->isFixedClkCam){
                /* fixed cam card can not set TPO CLK by demod,but we need adjust the tp frame rate by ts clk*/
                if(TsClkKHz > MaxTsClkKHz){
                	MaxTsClkKHz = TsClkKHz;
                	Tp_TPO_UpdateTPFrameRate();
                }
    }else{
    ...
    TP_WARNING("Set TPO clock, TPOUT%u pre_div=%u post_div=%u div16=%u, pll_clk*10=%u, TsClkKHz:%u \n", 0, pre_div, post_div, div16, rbus_pll, TsClkKHz);
...
EXPORT_SYMBOL(RHAL_TPOUT_SetTSClock);
...
)
{
...

        {
            ...
            if (pcb_mgr_get_enum_info_byname("PCMCIA_CFG3", &param3) == 0) {
                autosharepin_en = param3 & 0x1;
                TP_WARNING("TPO clk before set. pcmcia_autosharepin:0x%x pre_div=%u post_div=%u div16=%u\n",
                        autosharepin_en, pre_div, post_div, div16);

                if(autosharepin_en) {
                    unsigned int div_tmp = 0;
                    unsigned int tmp_tpo_clk_khz = 0;

                    tmp_tpo_clk_khz = (TP_SPEED_MHZ * 1000)/(pre_div + post_div + 2);
                    tmp_tpo_clk_khz = tmp_tpo_clk_khz + 1500;
                    div_tmp = (TP_SPEED_MHZ * 1000)/tmp_tpo_clk_khz;
                    if(div_tmp < 2)
                        div_tmp = 2;
                    div_tmp = div_tmp - 2;
                    ...
                    post_div = div_tmp - pre_div;
                    if (post_div > 511)
                        post_div = 511;

                    if((pre_div+post_div) > 48){
                        TP_WARNING("TPO speed = %dHz!! less than 5M, set the minimum speed 5.0M\n",((TP_SPEED_MHZ*1000)/(pre_div+post_div+2)));
                        pre_div = 24;
                        post_div = 24;
                    }
...
EXPORT_SYMBOL(RHAL_Delivery_TPP_SetInputConfig);
/*------------------------------------------------------------------
...
#ifdef MULTI_MTP_HAS_FR_CNT_PROBLEM
        REGADDR fr_cnt_reg = TP_FR_CNT_TP0_reg;
        unsigned char* pucReadPtr1;
        int rp_timestamp;
        int timestamp_diff;
#endif
        if(ContinueWrite == 0){
                TP_DBG_SIMPLE("%s MTP%d: ContinueWrite==0\n", __func__, mtp_id);
                return TPK_BAD_PARAMETER;
        }
        switch (mtp_id) {
                case TP_TP0_MTP:
...
#ifdef MULTI_MTP_HAS_FR_CNT_PROBLEM
        Tp_Get_MTP_Buffer(mtp_id, &ring_buf);
        pucReadPtr1 = (unsigned char*)(pBuf_param->BaseVir + ((UADDRESS)ring_buf.RP - pBuf_param->BasePhy));
        rp_timestamp = pucReadPtr1[0]<<24 | pucReadPtr1[1]<<16 | pucReadPtr1[2]<<8 | pucReadPtr1[3];
        timestamp_diff = rp_timestamp - rtd_inl(fr_cnt_reg);
        // if timestamp_diff large than 1s we need re-trigger the initial fr_cnt, stop all mtp in_go first
        if((timestamp_diff > 90000) && Tp_FRC_GetEnable((TPK_TP_ENGINE_T)mtp_id)){
                //TP_WARNING("%s 111	mtp[%d]  fr_cnt_reg=%08x, rp_timestamp:%08x,frc_en=%d\n",__func__, mtp_id,rtd_inl(fr_cnt_reg),rp_timestamp,Tp_FRC_GetEnable((TPK_TP_ENGINE_T)mtp_id));
                for (i = 0; i < MAX_TP_COUNT; i++) {
                        if(pTp_drv->mtp[i].mtp_stream_status == MTP_STREAM_START){
                                TP_MTPStreamControl(i,MTP_STREAM_STOP);
                                pTp_drv->mtp[i].mtp_stream_status=MTP_STREAM_START;
                                TP_WARNING("%s MTP%d: stream ctrl change from 1->0 temporary when rp_timestamp-fr_cnt>1s \n",__func__, i);
                        }
                }
        }
#endif
...
#ifdef MULTI_MTP_HAS_FR_CNT_PROBLEM
        //after write mtp data, set current mtp in_go to carpute new timestamp for fr cnt, then restore other MTP in_go
        if((timestamp_diff > 90000) && Tp_FRC_GetEnable((TPK_TP_ENGINE_T)mtp_id)){
                TP_MTPStreamControl(mtp_id,MTP_STREAM_START);
                msleep(1);
                //fr cnt reg should be reset to current ddr packet timestamp
                //TP_WARNING("%s 222 mtp[%d]	fr_cnt_reg=%08x, rp_timestamp:%08x,frc_en=%d\n",__func__, mtp_id,rtd_inl(fr_cnt_reg),rp_timestamp,Tp_FRC_GetEnable((TPK_TP_ENGINE_T)mtp_id));
                for (i = 0; i < MAX_TP_COUNT; i++) {
                        if((i != (int)mtp_id) && (pTp_drv->mtp[i].mtp_stream_status == MTP_STREAM_START)){
                                TP_MTPStreamControl(i,MTP_STREAM_START);
                                TP_WARNING("%s MTP%d: restore stream ctrl from 0->1\n",__func__, i);
                        }
                }
        }
#endif
...
INT32 RHAL_GetPCRAdaptionField(TPK_TP_ENGINE_T tp_id,UINT16 PID,UINT8 *pData,UINT8 *pDataSize)
{
        ...
        *pDataSize = pTp_drv->tp_pid[tp_id].pcr.adaption_field[0];
        ...
        if(pTp_drv->tp_pid[tp_id].pcr.PID == PID && *pDataSize>0 && *pDataSize <= 183) {
           if (pTp_drv->tp_pid[tp_id].pcr.adaption_field[1] & 0x10)
            {
             *pDataSize =5;
              memcpy(pData,&pTp_drv->tp_pid[tp_id].pcr.adaption_field[2],*pDataSize);
            }
           else
...
INT32 RHAL_Delivery_RequestData(TPK_TP_ENGINE_T tp_id)
{
        ...
        if ( pTp_drv->tp_japan[tp_id].src_type == TPK_DELI_SRC_TYPE_TLV_SW0
            ...
            UINT8 buf_idx = 0;
            tp_buffer = dvr_malloc_specific( TLV_RINGBUFFER_SIZE, GFP_DCU2_FIRST);
            if(NULL == tp_buffer){
                _tp_unlock();
            ...
            if(NULL == pTp_drv->tp_japan[tp_id].dmx->ptlv_regs){
                TP_ERROR("[%s %d] please check input source config. \n", __func__ , __LINE__);
                return TPK_RESOURCE_CONFLICT;
            }
            ...
            for(i=0;i<TLV_DESCRIPTOR_CNT;i++){
               ...
               if(NULL == tlv_buffer){
                    UINT32 j;
                    for(j = 0; j < i; j++){
                        if(pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].BaseVir != 0x0) {
                            dvr_free( (UINT8*)(pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].BaseVir));
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].BaseVir = 0x0;
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].BasePhy = 0x0;
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].LimitPhy = 0;
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].LimitVir = 0;
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].BufSize = 0;
                            pTp_drv->tp_japan[tp_id].tlv_descripor_buffer[j].OriBufSize = 0;
                        }
                    }
                    TP_ERROR("tp_id:%d, id =%d malloc failed,not enough L:%d!!", tp_id, i, __LINE__);
                    _tp_unlock();
                    return TPK_NOT_ENOUGH_RESOURCE;
               }
        ...
        TP_DBG("%s tp_id=%d srctype:%d\n", __func__, tp_id, pTp_drv->tp_japan[tp_id].src_type);
        tp_dbg_thread_start_stop(1);
        _tp_unlock();
        return TPK_SUCCESS;
}
...
INT32 RHAL_Delivery_CancelData(TPK_TP_ENGINE_T tp_id)
{
    ...
    if ( pTp_drv->tp_japan[tp_id].src_type == TPK_DELI_SRC_TYPE_TLV_SW0
        ...
        if (pTp_drv->tp[tp_id].mass_buffer.BaseVir != 0x00) {
            TP_DBG("tp_id:%d, RHAL_Delivery_CancelData. free:"PT_FMT_UADDR", L:%d!!", tp_id, pTp_drv->tp[tp_id].mass_buffer.BaseVir, __LINE__);
...
INT32 RHAL_Delivery_GetData(TPK_TP_ENGINE_T tp_id, char *buffer, UINT32 *buffer_size) {
    ...
    if (pTp_drv->deliveryMode[tp_id] == TP_DELIVERY_MODE_CI20) {
    ...
    } else if (pTp_drv->deliveryMode[tp_id] == TP_DELIVERY_MODE_TS && pTp_drv->tp_japan[tp_id].isJAPAN4K) {
        RHAL_Delivery_GetTsData(tp_id, buffer, buffer_size);
    } else if (pTp_drv->deliveryMode[tp_id] == TP_DELIVERY_MODE_JAPAN4K && pTp_drv->tp_japan[tp_id].isJAPAN4K) {
        ...
        *buffer_size = pTp_drv->tp_japan[tp_id].dmx->rhal_tlv_buffer_size;
        ...
        TP_DBG("RHAL_Delivery_GetData not requested tp_id=%d mode:%d buffer_size=%d isJAPAN:%d\n", tp_id, pTp_drv->deliveryMode[tp_id], *buffer_size, pTp_drv->tp_japan[tp_id].isJAPAN4K);
        ret = -1;
    }
...
INT32 RHAL_Delivery_JAPAN4K_SetInputConfig(TPK_TP_ENGINE_T tp_id, TPK_INPUT_PORT_T  inputPort, TPK_INPUT_TYPE_T  inputType, TPK_DELIVERY_SRC_TYPE_T srcType)
{
    ...
    if ( pTp_drv->tp_japan[tp_id].src_type == TPK_DELI_SRC_TYPE_TLV_SW0 ) {
        ...
        if(inputType == TPK_INPUT_SERIAL){
            tpStatus.tp_param.serial = 1;
        }else if(inputType == TPK_INPUT_TWO_BIT) {
            tpStatus.tp_param.serial = 0;
    ...
    TP_DBG("RHAL_Delivery_JAPAN4K_SetInputConfig tp_id=%d inputPort=%d srcType:%d\n", tp_id, inputPort, srcType);
    return ret;
}
...
{
    unsigned long length_temp, i, descriptor_idx;
    //unsigned long tlv_packet_cnt;
