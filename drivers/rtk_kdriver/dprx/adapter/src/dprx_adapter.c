 ...
 *-----------------------------------------------*/
int dprx_adp_get_typec_pd_ready_status(dprx_dev_t* p_dev)
{
    if (p_dev && p_dev->ops.get_typec_pd_ready_status)
        return p_dev->ops.get_typec_pd_ready_status(&p_dev->adp);
    ...
    )
{
    ...
    memset(p_pps, 0, sizeof(DP_PPS_DATA_T));

    if (p_dev && p_dev->ops.get_pps_data)
        return p_dev->ops.get_pps_data(&p_dev->adp, p_pps);
 ...
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
int dprx_adp_get_stream_bypass_timing(dprx_dev_t* p_dev, DPRX_TIMING_INFO_T *p_timing_info)
{
    if (p_timing_info==NULL)
        return -1;
    ...
    if (p_dev && p_dev->ops.get_stream_bypass_timing)
        return p_dev->ops.get_stream_bypass_timing(&p_dev->adp, p_timing_info);
