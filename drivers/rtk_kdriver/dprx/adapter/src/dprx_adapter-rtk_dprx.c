...
#include <dprx_adapter-rtk_dprx_ext_feat.h>
...
#endif
dprx_priv_ops_t g_rtk_priv_ex_ops;
extern dprx_priv_ops_t g_rtk_priv2_ops;
extern dprx_priv_ops_t g_rtk_priv1_ops;
...
//----------------------------------------------------------------------------------------
#include <dprx_vfe.h>
unsigned char g_multi_port_enable = 0;
dprx_adp_t* g_p_adp_dprx_current_port[MAX_DPRX_PORT]={NULL};

#ifdef DPRX_ASYNCHRONOUS_DETECT_TASK
static struct task_struct *p_adapter_dprx_timing_detect_tsk = NULL;


void port_detect_tsk(dprx_adp_t* p_adp)
{
    if(g_multi_port_enable)
    {
        rtk_dprx_port_event_task_handler(GET_RTK_DPRX_PORT(p_adp));
    }else
    {
        rtk_dprx_event_task_handler();
    }

    if(p_adp != NULL)
    {
        if((rtk_dprx_get_connect_status(GET_RTK_DPRX_PORT(p_adp))) == 0)
        {
            p_adp->check_mode_status = -1;
        }
        else
        {
            p_adp->check_mode_status = rtk_dprx_source_check(GET_RTK_DPRX_PORT(p_adp));
        }

#ifdef CONFIG_DPRX_EXT_FEATURE_ENABLE
        dprx_ext_feat_updata_hdcp_status(GET_RTK_DPRX_PORT(p_adp));
#endif
    }
}
...
static int adapter_dprx_timing_detect_tsk(void *p)//This task run HDMI source timing detecting after source connect
{
    ...
    while (1)
    {
        ...
        {
            port_detect_tsk(g_p_adp_dprx_current_port[i]);
        }
        if (freezing(current))
        {
            try_to_freeze();
...
/*------------------------------------------------
 * Func : _set_rhal_version
 *
 * Desc : setting rhal version
 *
 * Para : DPRX_PORT_CONFIG  : store port config
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
void _set_rhal_version(DPRX_PORT_CONFIG* port_cfg)
{

#ifdef CONFIG_DPRX_ENABLE_MULTIPORT
    g_multi_port_enable=1;
#else
    g_multi_port_enable=0;
#endif

#ifdef DPRX_DRV_SUPPORT_DYNAMIC_RBUS
    switch(get_mach_type())
    {
        case MACH_ARCH_RTK2885P:
            DPRX_ADP_INFO("================Now SoC is Merlin8===========");
            port_cfg->hal_version = DPRX_HAL_RTD2885P;
            break;
        case MACH_ARCH_RTK2885P2:
        case MACH_ARCH_RTK2885PP:
            DPRX_ADP_INFO("================Now SoC is Merlin8P===========");
            g_multi_port_enable=1;
            port_cfg->hal_version = DPRX_HAL_RTD2885P2;
            break;
        default:
            port_cfg->hal_version = DPRX_HAL_NONE;
            break;
    }
#endif
}
...
static unsigned char _get_port_type(void)
{
    return g_multi_port_enable;
}
...
{
    if(_get_port_type())
        memcpy(&g_rtk_priv_ex_ops, &g_rtk_priv2_ops, sizeof(dprx_priv_ops_t));
    else
        memcpy(&g_rtk_priv_ex_ops, &g_rtk_priv1_ops, sizeof(dprx_priv_ops_t));
...
{
    int res;
    res = rtk_dprx_connect(GET_RTK_DPRX_PORT(p_adp));
#ifdef CONFIG_DPRX_EXT_FEATURE_ENABLE
    dprx_ext_feat_clear_hdcp_status();
#endif
    g_p_adp_dprx_current_port[GET_RTK_DPRX_PORT(p_adp)->mac_idx] = p_adp;
    return res;
}
...
{
    int res;
    res = rtk_dprx_disconnect(GET_RTK_DPRX_PORT(p_adp));
    g_p_adp_dprx_current_port[GET_RTK_DPRX_PORT(p_adp)->mac_idx] = NULL;
#ifdef CONFIG_DPRX_EXT_FEATURE_ENABLE
    dprx_ext_feat_clear_hdcp_status();
#endif
    return res;
...
/*------------------------------------------------
 * Func : _ops_dprx_get_typec_pd_ready_status
 *
 ...
 *-----------------------------------------------*/
static int _ops_dprx_get_typec_pd_ready_status(dprx_adp_t* p_adp)
{
    return rtk_dprx_get_typec_pd_ready_status(GET_RTK_DPRX_PORT(p_adp));
}
...
{
    return rtk_dprx_get_avi_info(GET_RTK_DPRX_PORT(p_adp), p_avi);
}
...
/*------------------------------------------------
 * Func : _ops_dprx_get_pps_data
 *
 * Desc : get pps data from a given DPRX Port
 *
 * Para : p_adp : handle of DPRX adapter
 *        p_pps : pps data output
 *
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
static int _ops_dprx_get_pps_data(
    dprx_adp_t*         p_adp,
    DP_PPS_DATA_T*      p_pps
    )
{
    return rtk_dprx_get_pps_data(GET_RTK_DPRX_PORT(p_adp), p_pps);
}
    ...
    )
{
    ...
    return p_adp->check_mode_status;
#else
...
/*------------------------------------------------
 * Func : _ops_dprx_get_stream_bypass_timing
 *
 ...
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
static int _ops_dprx_get_stream_bypass_timing(
    dprx_adp_t*         p_adp,
    DPRX_TIMING_INFO_T* p_timing
    )
{
#ifdef DPRX_ASYNCHRONOUS_DETECT_TASK
    rtk_dprx_get_stream_bypasss_timing(GET_RTK_DPRX_PORT(p_adp), p_timing);
    return p_adp->check_mode_status;
#else
    return rtk_dprx_get_stream_bypasss_timing(GET_RTK_DPRX_PORT(p_adp), p_timing);
#endif
...
{
#ifdef DPRX_ASYNCHRONOUS_DETECT_TASK
    return p_adp->check_mode_status;                            //Check input source with asynchronous source
#else
    ...
    )
{
    ...
    case DPRX_ADP_EVENT_ONLINE_MEASURE_FAIL:
        #ifdef DPRX_ASYNCHRONOUS_DETECT_TASK
        DPRX_ADP_WARN("source check g_dprx_check_mode_status = %d\n", p_adp->check_mode_status);
        #endif
...
static dprx_adp_ops_t g_rtk_dprx_ops =
{
    ...
    .get_typec_pd_ready_status = _ops_dprx_get_typec_pd_ready_status,
    ...
    .get_pps_data = _ops_dprx_get_pps_data,
    ...
    .get_stream_bypass_timing = _ops_dprx_get_stream_bypass_timing,
...
/*------------------------------------------------
 * Func : _dprx_adp_init_dprx_default_edid
 *
 * Desc : init dprx default edid
 *
 * Para : p_adp : handle of DPRX adapter
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
int _dprx_adp_init_dprx_default_edid(
    unsigned char   ddc_ch,
    unsigned char*  p_edid,
    unsigned short  edid_len)
{
    edid_dev_t* p_ddc = create_rtk_ddc_edid_device(ddc_ch);  // in QS mode
#ifdef DEBUG_DEFAULT_EDID
    unsigned short i;
#endif

    DPRX_ADP_INFO("_dprx_adp_init_dprx_default_edid, ddc_ch=%d!!!!\n", ddc_ch);

    if (p_ddc==NULL)
    {
        DPRX_ADP_WARN("_dprx_adp_init_dprx_default_edid failed, open ddc_ch%d failed!!!!\n", ddc_ch);
        return -1;
    }
    ...
    {
        DPRX_ADP_WARN("_dprx_adp_init_dprx_default_edid failed, invalid edid!!!!\n");
    ...
    )
{
    ...
    switch(p_cfg->type)
    {
        case DP_TYPE_USB_TYPE_C:
            if(_dprx_adp_init_dprx_default_edid(p_cfg->config.typec_dp.edid.ddc_i2c_idx, p_dprx->sink_edid, sizeof(p_dprx->sink_edid))<0)
                memcpy(p_dprx->sink_edid, g_dprx_default_edid, sizeof(g_dprx_default_edid));
            break;
        default :
            if(_dprx_adp_init_dprx_default_edid(p_cfg->config.dp_mdp.edid.ddc_i2c_idx, p_dprx->sink_edid, sizeof(p_dprx->sink_edid))<0)
                memcpy(p_dprx->sink_edid, g_dprx_default_edid, sizeof(g_dprx_default_edid));
            break;
    }
    ...
    case DP_TYPE_DP:
    case DP_TYPE_mDP:
        p_dprx->ddc_idx = p_cfg->config.dp_mdp.edid.ddc_i2c_idx;
        p_dprx->mac_idx = p_cfg->config.dp_mdp.mac_idx;
        _update_adp_port_config(&p_dprx->port_cfg, &p_cfg->config.dp_mdp.phy, &p_cfg->config.dp_mdp.aux);
        break;
    case DP_TYPE_USB_TYPE_C:
        p_dprx->ddc_idx = p_cfg->config.typec_dp.edid.ddc_i2c_idx;
        p_dprx->mac_idx = p_cfg->config.typec_dp.mac_idx;
    ...
    //----  flow config capability ----
    p_dprx->ignore_dsc_check_en = p_flow_cfg->ignore_dsc_check_en;
    ...
    _set_rhal_version(&p_dprx->port_cfg);

    DPRX_ADP_WARN("now enable multi port :%d\n",g_multi_port_enable);

    if(get_mach_type()!=MACH_ARCH_RTK2885P)
        p_dprx->p_vc = create_rtk_tv_dprx_video_common_device(p_cfg->ch);
...
/*------------------------------------------------
 * Func : set_dolby_vision_check_mode_function
 *
 * Desc : set dolby vision check mode function pointer
 *
 * Para : dolby vision check mode function pointer
 *
 * Retn : void
 *-----------------------------------------------*/
int set_dolby_vision_check_mode_function(
    long (*fp)(unsigned char)
    )
{
    return rtk_dprx_set_dolby_vision_check_mode_function(fp);
}
#endif
