...
static int  rtk_dprx_ddc_init(void);
static int  rtk_dprx_ddc_uninit(void);
...
static void rtk_dprx_event_task_handler(void);
#else
#define rtk_dprx_event_task_handler()
#endif

static void rtk_dprx_set_aux_int_crtl_rounting_enable(UINT8 enable);
...
static UINT32 g_reconnect_error_handler_time_ms = 0;
static UINT32 g_dp_reconnect_error_handler_time_interval = DEFAULT_DP_RECONNECT_TIME_INTERVAL_PERIOD_MS;
static UINT32 g_typec_reconnect_error_handler_time_interval = DEFAULT_TYPEC_RECONNECT_TIME_INTERVAL_PERIOD_MS;
#endif
    ...
    )
{
    ...
    DPRX_DRV_HDCP2x_EVENT_HEADLER_ENABLE(0, 1);
...
/*------------------------------------------------
 * Func : _dprx_port_stop
 *
 * Desc : stop dprx port to dprx low level driver
 *
 * Para : p_dprx : handle of DPRX adapter
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
void _dprx_port_stop(RTK_DPRX_ADP* p_dprx)
{
    DPRX_ADP_INFO("rtk_dprx_port_swap : stop current port\n");

    g_p_current_dprx_port->dprx_port_readly = 0;

    g_p_current_dprx_port->connected = 0;   // disconnect current port
#ifdef CONFIG_DPRX_ENABLE_TASK_EVENT_HANDLER
    g_p_current_dprx_port->reconnect_handler_event = 0;
#endif
...
/*------------------------------------------------
 * Func : _dprx_port_start
 *
 * Desc : start dprx port to dprx low level driver
 *
 * Para : p_dprx : handle of DPRX adapter
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
void _dprx_port_start(RTK_DPRX_ADP* p_dprx)
{
        DPRX_ADP_INFO("_dprx_port_start : start new DPRX port\n");
        ...
        if (g_force_max_lane_count && g_force_max_lane_count != g_dprx_sink_capability.max_lane_count)
            g_dprx_sink_capability.max_lane_count = g_force_max_lane_count;
        ...
        if (g_force_fake_tps2_lt)
            g_dprx_sink_capability.link_train_ctrl |= DPRX_SINK_CAP_LINK_TRAIN_CTRL_FAKE_TP2_LT;  // allow to change fake tp2_lt
        else
            g_dprx_sink_capability.link_train_ctrl = 0;  // over
#endif
        ...
        DPRX_DRV_EXT_IGNORE_DSC_ERR_CHECK(0, p_dprx->ignore_dsc_check_en);

        switch(p_dprx->type)
        {
            case DP_TYPE_USB_TYPE_C:
                DPRX_DRV_EXT_AUX_Z0_ENABLE(0, (p_dprx->cable_type)? 1:0);           //enable aux z0 if usbc enter alt mode
                break;
            default :
                DPRX_DRV_EXT_AUX_Z0_ENABLE(0, 1);                                   //force enable aux z0 on DP port
        }
...
#ifdef CONFIG_DPRX_ENABLE_TASK_EVENT_HANDLER
        // 10. set reconnect handler if it's no signal.
        g_reconnect_error_handler_time_ms = dprx_odal_get_system_time_ms();
        p_dprx->reconnect_handler_event = 1;
#endif
    ...
    )
{
    ...
    {
        DPRX_ADP_INFO("rtk_dprx_port_swap : stop new DPRX port\n");
        _dprx_port_stop(g_p_current_dprx_port);
    }
...
{
    UINT8 connected;
    DPRX_ADP_INFO("rtk_dprx_port_reset : dprx port reset\n");

    if(p_dprx == NULL)
        return -1;

    connected = p_dprx->connected;
    _dprx_port_stop(p_dprx);
    _dprx_port_start(p_dprx);
    ...
    )
{
    ...
    if (p_dprx->qs_init_ready)
    {
        ...
        if (p_dprx->connected)
        {
            ...
            {
                if(newbase_dprx_dpcd_get_msa_timing_ignored(0))
                {
                    p_dprx->sink_cap.freesync_support = 1;
                    p_dprx->sink_cap.adaptive_sync_support = 1;
                    DPRX_ADP_WARN("qs init: newbase_dprx_dpcd_get_msa_timing_ignored : %d", 1);
                }else
                {
                    p_dprx->sink_cap.freesync_support = 0;
                    p_dprx->sink_cap.adaptive_sync_support = 0;
                    DPRX_ADP_WARN("qs init : newbase_dprx_dpcd_get_msa_timing_ignored : %d", 0);
                }
                // QS init pass
...
#ifdef DPRX_SUPPORT_SET_LOW_POWER_MODE
            if (g_p_current_dprx_port == p_dprx && (ret==0 || p_dprx->lpm_enable) && p_dprx->resume_connect==0)
            {
...
#else
            if (g_p_current_dprx_port == p_dprx && ret==0 && p_dprx->resume_connect==0)
            {
                unsigned char connected = p_dprx->connected;
                DPRX_ADP_INFO("cable disconnected !!!! do port reset\n");
                rtk_dprx_port_reset(p_dprx);
    ...
    )
{
    ...
    if (g_p_current_dprx_port==p_dprx)
    {
        ...
        dprx_drv_get_link_status(0, &link_status);
...
static void _check_info_frame_expiration(UINT32 frame_rate_hz)
{
    UINT32 frame_preoid;
    ...
    if (_check_sdp_buff_expiration(&g_dprx_sdp_handler.vsi, frame_preoid * 30))    // for non Dolby VSI
    {
    ...
    if (_check_sdp_buff_expiration(&g_dprx_sdp_handler.hdr, frame_preoid * 30))
    {
        DPRX_ADP_WARN("HDR INFO expired, force clear HDR SDP\n");
    }

    if (_check_sdp_buff_expiration(&g_dprx_sdp_handler.vsc, frame_preoid * 30))
    {
        DPRX_ADP_WARN("VSC INFO expired, force clear VSC\n");
    }

    if (_check_sdp_buff_expiration(&g_dprx_sdp_handler.spd, frame_preoid * 30))
    {
...
#ifdef CONFIG_DPRX_CHECK_TIMING_VALID_WITH_SCALER_API

extern int vbe_check_input_support_otp_clock(int htotal, int vtotal, int vFreq);

/*------------------------------------------------
 * Func : _check_disp_timing_constrain
 *
 * Desc : check DISP constrain
 *
 * Para : p_adp  : handle of DPRX adapter
 *        p_timing : pointer of timing info
 *
 * Retn : 0 : failed, >0 : successed
 *-----------------------------------------------*/
int _check_disp_timing_constrain(
    DPRX_TIMING_INFO*   p_timing_info
    )
{
    UINT32 disp_htt = p_timing_info->htotal;
    UINT32 disp_vtt = p_timing_info->vtotal;
    UINT32 disp_vfreq = (p_timing_info->vfreq_hz_x100 + 99) / 100;
    int ret = 0;

#ifdef DPRX_DRV_TIMING_V_DROP_ENABLE
    if ((p_timing_info->ext_flags & DPRX_DRV_TIMING_V_DROP_ENABLE))
        disp_vtt >>= 1;
#endif

#ifdef DPRX_DRV_TIMING_H_DROP_ENABLE
    if ((p_timing_info->ext_flags & DPRX_DRV_TIMING_H_DROP_ENABLE))
        disp_htt >>= 1;
#endif

    ret = vbe_check_input_support_otp_clock(disp_htt, disp_vtt, disp_vfreq);

    DPRX_ADP_WARN("_check_disp_timing_constrain, disp_htt=%d, disp_vtt=%d, disp_vfreq=%d (0.01Hz), ret=%d\n",
            disp_htt, disp_vtt, disp_vfreq, ret);
...
#define _check_disp_timing_constrain(p_timing_info)                 (1)
    ...
    )
{
    ...
    if (dprx_drv_get_timing_info(0, &dprx_timing_info)==_TRUE)
    {
...
#ifdef CONFIG_SUPPORT_DOLBY_VISION_DP
        p_timing->is_dolby_hdr  = get_dolby_vision_dp_mode(0) ? 1 : 0;
#else
        ...
        if (_check_disp_timing_constrain(&dprx_timing_info)==0)
    ...
    )
{
    ...
    if (p_avi==NULL || p_msa ==NULL || dprx_drv_get_pixel_encoding_info(0, &dprx_pixel_format)!=_TRUE)
        return -1;
    ...
    )
{
    ...
    if (rtk_dprx_get_msa_data(p_dprx, &msa)==0)
    {
        ...
        if (rtk_dprx_get_sdp_data(p_dprx, DP_SDP_TYPE_VSC, &sdp)==0)
            return _gen_avi_by_msa_vsc(p_avi, &msa, &sdp);
        else
            return _gen_avi_by_msa_alone(p_avi, &msa);
    }
#endif
...
{
    if (p_pps_sdp==NULL || g_p_current_dprx_port!=p_dprx || p_dprx->connected==0)
        return -1;
...
static void _dprx_status_monitor(RTK_DPRX_ADP* p_dprx)
{
    ...
    if (vfsm==VFSM_9_VIDEO_OUT_ENABLE || vfsm==VFSM_16_DSC_OUT_ENABLE)
    {
        ...
        DPRX_ADP_WARN("Status : vfsm=%d(%s), crc=(%04x/%04x/%04x), vfreq=%d.%d (Hz), DRR mode=%s, HDCP=%s, pixel_mode=%d\n",
            vfsm, _get_dprx_vfsm_str(vfsm),
            dprx_mac_get_video_crc(0, 0),
            dprx_mac_get_video_crc(0, 1),
            dprx_mac_get_video_crc(0, 2),
...
#if 0
    //re-setup dprx setting and hpd toggle to recovery
    if(p_dprx->vfsm_status <= VFSM_5_SET_DISP_FORMAT)
    {
        UINT8 res = 0;
        res = _get_source_check_intenal(dprx_odal_get_system_time_ms(),
                                        p_dprx->pre_source_check_no_signal_time,
                                        DEFAULT_SOURCE_CHECK_NO_LT_INTERVAL_PERIOD_MS);
        if(res == 0)
        {
            DPRX_ADP_WARN("Set Source Check with in no_LT interval(%d ms), To Recovery mode\n",
                    DEFAULT_SOURCE_CHECK_NO_LT_INTERVAL_PERIOD_MS);
            rtk_dprx_port_reset(p_dprx);
            p_dprx->connected = 1;
        }
    ...
    )
{
    ...
    dprx_drv_set_ext_config(0, DPRX_EXT_RESET_SIG_DETECT_FLOW, 0);
#else
    ...
    )
{
    ...
    if ((dprx_drv_read_vbid(0) & VBID_6_COMPRESSED_STREAM_FLAG))
    {    // force reset video state if DSC
        dprx_drv_do_avmute(0);
    ...
    )
{
    ...
    case ADP_EXCTL_VIDEO_COMMON_SEL:
        ...
        case DP_VIDEO_COMMON_SEL_NONE:
            dprx_drv_set_ext_config(0, DPRX_EXT_SET_VIDEO_COMMON, DPRX_EXT_VIDEO_COMMON_NONE);
            break;
        case DP_VIDEO_COMMON_SEL_1:
        case DP_VIDEO_COMMON_SEL_2:
        case DP_VIDEO_COMMON_SEL_3:
        case DP_VIDEO_COMMON_SEL_4:
            dprx_drv_set_ext_config(0, DPRX_EXT_SET_VIDEO_COMMON, DPRX_EXT_VIDEO_COMMON_1);
            break;
        default:
            break;
        }
    ...
    case ADP_EXCTL_WAKEUP_MODE:
        ...
        break;
    case ADP_EXCTL_WAKUEP_MODE_SEL:
        switch(val)
        {
            case 0:
                DPRX_ADP_INFO("set mode wakeup mode sel : DPRX_WAKUEP_MODE_SEL_ONLY_DP_PORT_WAKEUP\n");
                p_dprx->dprx_wakeup_mode_sel = DPRX_WAKUEP_MODE_SEL_ONLY_DP_PORT_WAKEUP;
                break;
            case 1:
                DPRX_ADP_INFO("set mode wakeup mode sel : DPRX_WAKUEP_MODE_SEL_CURRENT_PORT_WAKEUP\n");
                p_dprx->dprx_wakeup_mode_sel = DPRX_WAKUEP_MODE_SEL_CURRENT_PORT_WAKEUP;
                break;
            default:
                p_dprx->dprx_wakeup_mode_sel = DPRX_WAKUEP_MODE_SEL_ONLY_DP_PORT_WAKEUP;
                break;
    ...
    case ADP_EXCTL_HDCP1x_KEY_INIT:
        DPRX_DRV_EXT_HDCP1x_KEY_INIT(0);
        break;
    case ADP_EXCTL_HDCP2x_KEY_INIT:
        DPRX_DRV_EXT_HDCP2x_KEY_INIT(0);
        break;
    // Stream Bypass
    case ADP_EXCTL_STREAM_BYPASS_MODE:
        ...
        case DPRX_STREAM_BYPASS_MODE_TO_HDMITX:
            dprx_drv_set_ext_config(0, DPRX_EXT_STREAM_BYPASS_MODE, DPRX_EXT_STREAM_BYPASS_TO_HDMITX);
            break;
        default:
        case DPRX_STREAM_BYPASS_MODE_OFF:
            dprx_drv_set_ext_config(0, DPRX_EXT_STREAM_BYPASS_MODE, DPRX_EXT_STREAM_BYPASS_OFF);
            break;
        }
    ...
    case ADP_EXCTL_STREAM_BYPASS_MODE:
        {
            ...
            dprx_drv_get_ext_config(0, DPRX_EXT_STREAM_BYPASS_MODE, &mode);
    ...
    )
{
    ...
    dprx_drv_set_ext_config(0, DPRX_EXT_WAKEUP_BY_DATA_ENABLE, 1); // enable wakeup by data
    #endif
    if (p_dprx->suspend==0)
    {
        ...
        switch(p_dprx->dprx_wakeup_mode_sel)
        {
            case DPRX_WAKUEP_MODE_SEL_CURRENT_PORT_WAKEUP:
            ...
            case DPRX_WAKUEP_MODE_SEL_ONLY_DP_PORT_WAKEUP:
                if(g_p_current_dprx_port!=NULL && g_p_current_dprx_port->type == DP_TYPE_USB_TYPE_C)
                {
                    rtk_dprx_port_swap(NULL);     // deactiavte typec port
                    p_dprx->resume_connect = 0;   // auto connect after resume
                }

                // 2. power off DPRX if it is DP_TYPE_DP
                if (p_dprx->type == DP_TYPE_DP)
                {
                    rtk_dprx_port_swap(p_dprx);     // actiavte dprx port
                    if ((mode & WAKEUP_BY_AUX))
                        newbase_dprx_dpcd_set_set_power_state(0, 0x5);  // change set the power state to D3
                    else
                        rtk_dprx_port_swap(NULL);     // deactiavte currect port
 ...
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
static void _update_sdp_debug_flag(void)
{
    ...
    )
{
    ...
    if (sdp_type==SDP_TYPE_INFOFRAME_SPD)
    {
        ...
        dprx_drv_get_amd_freesync_status(0, &status);

        if (g_dprx_sdp_handler.freesync_enable != status.freesync_en||
            g_dprx_sdp_handler.freesync_activated != status.freesync_act ||
            g_dprx_sdp_handler.freesync_support != status.freesync_support)
        {
            g_dprx_sdp_handler.freesync_enable    = status.freesync_en;
            g_dprx_sdp_handler.freesync_activated = status.freesync_act;
            g_dprx_sdp_handler.freesync_support   = status.freesync_support;
...
static void _dprx_hdcp2x_handler(RTK_DPRX_ADP* p_dprx)
{
    ...
    {
        DPRX_ADP_INFO("[_dprx_hdcp2x_handler]\n");
        DPRX_DRV_HDCP2x_HEADLER(0, p_dprx->hdcp2x_event_msg_id[p_dprx->hdcp2x_event_rp++]);
        if(g_p_current_dprx_port->hdcp2x_event_rp >= DPRX_MAX_HDCP_EVENT_QUEUE)
 ...
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
void _dprx_resume_handler(RTK_DPRX_ADP* p_dprx)
{
    UINT8 res = 0;
...
/*------------------------------------------------
 * Func : _dprx_reconnect_error_handler
 *
 * Desc : To reconnect error handler if it's no signal
 *
 * Para : p_adp : handle of DPRX adapter
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
static void _dprx_reconnect_error_handler(RTK_DPRX_ADP* p_dprx)
{
    UINT8 res = 0;
    UINT32 guard_time_interval = 0;

    if (p_dprx->connected==0)
        return;

    if(p_dprx->reconnect_handler_event)
    {
        switch(p_dprx->type)
        {
        case DP_TYPE_USB_TYPE_C:
            guard_time_interval = g_typec_reconnect_error_handler_time_interval;
            break;
        default :
            guard_time_interval = g_dp_reconnect_error_handler_time_interval;
        }
        res = _get_source_check_intenal(dprx_odal_get_system_time_ms(),
                                        g_reconnect_error_handler_time_ms,
                                        guard_time_interval);
        if(dprx_hpd_get_connect_status(p_dprx->p_hpd) == 0)
        {
            DPRX_ADP_INFO("[RECOVERY] Cable. RECOVERY Cable Status = 0\n");
            p_dprx->reconnect_handler_event = 0;
        }
        if(dprx_drv_source_check(0)==_TRUE)
        {
            DPRX_ADP_INFO("[RECOVERY] Video Timing Stable. RECOVERY Mode disable.\n");
            p_dprx->reconnect_handler_event = 0;
        }
        if(res == 0)
        {
            DPRX_ADP_INFO("[RECOVERY] Current port, Do port swap to reconnect!!!!\n");
            rtk_dprx_port_reset(p_dprx);
            p_dprx->connected = 1;
            p_dprx->reconnect_handler_event = 0;
        }
...
static void rtk_dprx_event_task_handler(void)
{
    ...
    _dprx_resume_handler(g_p_current_dprx_port);
    _dprx_reconnect_error_handler(g_p_current_dprx_port);
    _dprx_hdcp2x_handler(g_p_current_dprx_port);
}
#endif
...
dprx_priv_ops_t g_rtk_priv1_ops =
{
    // .reset                              = rtk_dprx_port_reset,
    .port_swap                          = rtk_dprx_port_swap,
    .init                               = rtk_dprx_init,
    ...
    //.port_set_aux_int_crtl_rounting_enable  = rtk_dprx_port_set_aux_int_crtl_rounting_enable,//v1 will not use
    .set_aux_int_crtl_rounting_enable  = rtk_dprx_set_aux_int_crtl_rounting_enable, 
...
#ifdef CONFIG_DPRX_ENABLE_TASK_EVENT_HANDLER
    //.port_event_task_handler              = rtk_dprx_port_event_task_handler,// v1 will not use
    .event_task_handler                     = rtk_dprx_event_task_handler, // v2 will not use
#endif
#ifdef CONFIG_SUPPORT_DOLBY_VISION_DP
