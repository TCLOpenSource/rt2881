...
irqreturn_t rtk_dprx_irq_handler(int irq, void *dev_id)
{
    ...
    ret |= ((dprx_drv_aux_interrupt_handler_exint0()) ? IRQ_HANDLED : IRQ_NONE);
    ret |= ((dprx_drv_mac_interrupt_handler_exint0()) ? IRQ_HANDLED : IRQ_NONE);
...
int rtk_dprx_plat_interrupt_enable(void)
{
    ...
    rtk_dprx_set_aux_int_crtl_rounting_enable(1);
...
static const struct dev_pm_ops rtk_dprx_pm_ops = {
    ...
    .resume_early  = rtk_dprx_pm_resume,
