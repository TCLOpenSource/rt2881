...
typedef enum
{
    ...
    // Video Common Sel
    ADP_EXCTL_VIDEO_COMMON_SEL,

    // DPRX VRR(Adatpvie Sync/Free Sync) Enable
    ADP_EXCTL_VRR_EN,

    ADP_EXCTL_WAKUEP_MODE_SEL, // set wakuep mode sel(DPRX_CURRENT_PORT_WAKEUP/DPRX_ONLY_DP_PORT_WAKEUP)
    ADP_EXCTL_WAKEUP_MODE,

    ADP_EXCTL_HDCP1x_KEY_INIT,
    ADP_EXCTL_HDCP2x_KEY_INIT,

    // API for Stream Bypass
    ADP_EXCTL_STREAM_BYPASS_ENABLE,
    ADP_EXCTL_STREAM_BYPASS_MODE,
...
typedef enum
{
    DPRX_STREAM_BYPASS_MODE_OFF = 0,         // non stream bypass
    DPRX_STREAM_BYPASS_MODE_TO_HDMITX = 1,   // stream bypass to HDMI TX
    DPRX_STREAM_BYPASS_MODE_MAX,
}DPRX_STREAM_BYPASS_MODE_MODE;

typedef enum
{
    DPRX_WAKUEP_MODE_SEL_ONLY_DP_PORT_WAKEUP = 0,
    DPRX_WAKUEP_MODE_SEL_CURRENT_PORT_WAKEUP = 1,
    DPRX_WAKUEP_MODE_SEL_MAX,
}DPRX_WAKUEP_MODE_SEL_MODE;
...
typedef struct
{
    ...
    int (*get_typec_pd_ready_status)(dprx_adp_t* p_adp);    // get typec pd ready status (0: pd ready, 1: pd busy)
    ...
    int (*get_pps_data)(dprx_adp_t* p_adp, DP_PPS_DATA_T* p_pps);
    ...
    int (*get_stream_bypass_timing)(dprx_adp_t* p_adp, DPRX_TIMING_INFO_T* p_timing);
...
/*------------------------------------------------
 * Func : dprx_adp_get_typec_pd_ready_status
 *
 ...
 *-----------------------------------------------*/
extern int dprx_adp_get_typec_pd_ready_status(dprx_dev_t* p_dev);
...
/*------------------------------------------------
 * Func : dprx_adp_get_pps_data
 *
 * Desc : get pps data from a given DPRX Port
 *
 * Para : p_adp : handle of DPRX adapter
 *        p_dev : PPS data output
 *
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
extern int dprx_adp_get_pps_data(dprx_dev_t* p_dev, DP_PPS_DATA_T* p_pps);
...
/*------------------------------------------------
 * Func : dprx_adp_get_stream_bypass_timing
 *
 * Desc : get timing info for stream bypass mode 
 *
 * Para : p_adp : handle of DPRX adapter
 *        p_timing_info : timing info
 *
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
extern int dprx_adp_get_stream_bypass_timing(dprx_dev_t* p_dev, DPRX_TIMING_INFO_T *p_timing_info);
