...
static unsigned char select_mode = 0;
#define GET_PORT_SELECT_MODE                select_mode
#define SET_PORT_SELECT_MODE(mode)          (select_mode = mode)

#define PORT_SELECT_MODE_CURRENT            0
#define PORT_SELECT_MODE_ALL                1
#define PORT_SELECT_MODE_ONLY_PORT_0        2
#define PORT_SELECT_MODE_ONLY_PORT_1        3
...
const char* _get_dprx_stream_bypass_mode_str(
    unsigned char   mode
    )
{
    switch(mode)
    {
    case DPRX_STREAM_BYPASS_MODE_OFF:       return "Off";
    case DPRX_STREAM_BYPASS_MODE_TO_HDMITX: return "TO HDMI TX";
    default:                                return "Unknown";
    }
}
...
OFFMS_CTRL_FALGS _gen_offms_flag(DPRX_TIMING_INFO_T* p_timing)
{
    ...
    if (p_timing->ptg_mode == DP_PTG_REGEN_NO_VSYNC_MODE)
        offms_flag |= OFFMS_FLAGS_DEMODE(1);
    ...
    )
{
    ...
    n = scnprintf(ptr, count, "VIDEO : h_total=%d, v_total=%d, h_act=%d, v_act=%d, h_freq=%d, v_freq=%d.%d /(actual_vfreq=%d.%d), dsc=%d, interlace=%d, drr_mode=%d, de_mode=%d, dolby_hdr=%d, ext_flags=%x\n",
    ...
    if (timing.hact && timing.vact)
    {
        ...
        if (timing.is_dsc)
            rtk_offms_measure_video(OFFMS_SOURCE_DSCD, &measre_timing, _gen_offms_flag(&timing));
        else
            rtk_offms_measure_video(OFFMS_SOURCE_DPRX, &measre_timing, _gen_offms_flag(&timing));

        n = scnprintf(ptr, count, "OFFMS : h_total=%d, v_total=%d, h_act=%d, v_act=%d, h_freq=%d, v_freq=%d.%d, h_proch=%d, v_proch=%d, h_sta=%d, v_sta=%d, h_front=%d, v_front=%d, polarity=%x (b1:vsp=%s, b0:hsp=%s). pixel_mode=%d\n",
    ...
    memset(&hdcp_status, 0, sizeof(hdcp_status));
    dprx_adp_get_hdcp_status(p_dprx, &hdcp_status);
    n = scnprintf(ptr, count, "Stream Bypass : %d (%s)\n",
        dprx_adp_get_ext_ctrl(p_dprx, ADP_EXCTL_STREAM_BYPASS_MODE),
        _get_dprx_stream_bypass_mode_str(dprx_adp_get_ext_ctrl(p_dprx, ADP_EXCTL_STREAM_BYPASS_MODE)));
    ptr+=n; count-=n;

    if (dprx_adp_get_ext_ctrl(p_dprx, ADP_EXCTL_STREAM_BYPASS_MODE)!=DPRX_STREAM_BYPASS_MODE_OFF)
    {
#if 0
        memset(&timing, 0, sizeof(timing));

        dprx_adp_get_stream_bypass_timing(p_dprx, &timing);
        n = scnprintf(ptr, count, "Bypass Timing : h_total=%d, v_total=%d, h_act=%d, v_act=%d, h_freq=%d, v_freq=%d.%d /(actual_vfreq=%d.%d), dsc=%d, interlace=%d, drr_mode=%d, de_mode=%d, dolby_hdr=%d\n",
            timing.htotal, timing.vtotal,
            timing.hact, timing.vact,
            timing.hfreq_hz, timing.vfreq_hz_x100/100, timing.vfreq_hz_x100%100,
            timing.curr_vfreq_hz_x100/100, timing.curr_vfreq_hz_x100%100,
        ...
        drvif_Dprx_GetPortRawTimingInfoForHdmiTX(ch, &measre_timing);

        n = scnprintf(ptr, count, "Bypass Timing : h_total=%d, v_total=%d, h_act=%d, v_act=%d, h_freq=%d, v_freq=%d.%d, h_proch=%d, v_proch=%d, h_sta=%d, v_sta=%d, h_front=%d, v_front=%d, polarity=%x (b1:vsp=%s, b0:hsp=%s). pixel_mode=%d\n",
            measre_timing.h_total,
            measre_timing.v_total,
            measre_timing.h_act_len,
            measre_timing.v_act_len,
...
static void _set_wakeup_by_aux(unsigned char enable)
{
    ...
    if (enable)
    {
        for(i = 0; i<MAX_DPRX_PORT;i++)
            g_dprx_vfe_suspend_mode[i] |= WAKEUP_BY_AUX;
    }
    else
    {
        for(i = 0; i<MAX_DPRX_PORT;i++)
...
static void _set_wakeup_by_signal(unsigned char enable)
{
    ...
    {
        for(i = 0; i<MAX_DPRX_PORT;i++)
            g_dprx_vfe_suspend_mode[i] |= WAKEUP_BY_SIGNAL;
    }
    else
    {
        for(i = 0; i<MAX_DPRX_PORT;i++)
...
int _dump_pps_data(unsigned char ch, char* p_buf, unsigned int count)
{
    DP_PPS_DATA_T pps_data;
    int i, n;
    char *ptr = p_buf;
    ...
    if (g_connected_dprx)
        dprx_adp_get_pps_data(g_connected_dprx, &pps_data);

    // print banner
    n = scnprintf(ptr, count, "\n----------------------------------------------------\n"); ptr+=n; count-=n;
    n = scnprintf(ptr, count, "- PPS\n"); ptr+=n; count-=n;
    n = scnprintf(ptr, count, "----------------------------------------------------\n"); ptr+=n; count-=n;

    for (i=0; i<128; i+=16)
    {
        unsigned char* p_pps = &pps_data.pps[i];

        n = scnprintf(ptr, count, "PPS[%3d]  | %02x %02x %02x %02x, %02x %02x %02x %02x, %02x %02x %02x %02x, %02x %02x %02x %02x \n",
                i,
                p_pps[0],p_pps[1],p_pps[2], p_pps[3],  p_pps[4], p_pps[5], p_pps[6], p_pps[7],
                p_pps[8],p_pps[9],p_pps[10],p_pps[11], p_pps[12],p_pps[13],p_pps[14],p_pps[15]);
        ptr+=n; count-=n;
    }
...
int _dump_msa_data(unsigned char ch, char* p_buf, unsigned int count)
{
    ...
    n = snprintf(ptr, count, "vs_pol=%d,  vs_width=%d, v_start=%d\n", msa.vsync_polarity, msa.vsync_width, msa.vstart); ptr+=n; count-=n;
    n = snprintf(ptr, count, "hs_pol=%d,  hs_width=%d, h_start=%d\n", msa.hsync_polarity, msa.hsync_width, msa.hstart); ptr+=n; count-=n;
...
const char* _get_avi_color_space_str(unsigned char color_space)
{
    switch(color_space)
    {
    case DP_AVI_CSC_RGB:    return "RGB";
    case DP_AVI_CSC_YCBCR422:  return "YUV422";
    case DP_AVI_CSC_YCBCR444:  return "YUV444";
    case DP_AVI_CSC_YCBCR420:  return "YUV420";
    default:                   return "Unknown";
    }
}

const char* _get_avi_colorimetry_str(unsigned char colorimetry)
{
    switch(colorimetry)
    {
    case DP_AVI_COLORIMETRY_NODATA:  return "No Data";
    case DP_AVI_COLORIMETRY_SMPTE170:return "SMPTE170";
    case DP_AVI_COLORIMETRY_ITU709:  return "ITU709";
    case DP_AVI_COLORIMETRY_FUTURE:  return "Future";
    default:                         return "Unknown";
    }
}

const char* _get_avi_extension_colorimetry_str(unsigned char ext_colorimetry)
{
    switch(ext_colorimetry)
    {
    case DP_AVI_EXT_COLORIMETRY_XVYCC601:   return "XVYCC601";
    case DP_AVI_EXT_COLORIMETRY_XVYCC709:   return "XVYCC709";
    case DP_AVI_EXT_COLORIMETRY_SYCC601:     return "SYCC601";
    case DP_AVI_EXT_COLORIMETRY_ADOBEYCC601: return "ADOBEYCC601";
    case DP_AVI_EXT_COLORIMETRY_ADOBERGB:        return "ADOBERGB";
    case DP_AVI_EXT_COLORIMETRY_BT2020_YCCBCCRC:  return "BT2020_YCCBCCRC";
    case DP_AVI_EXT_COLORIMETRY_BT2020_RGBORYCBCR:  return "BT2020_RGBORYCBCR";
    case DP_AVI_EXT_COLORIMETRY_XVRESERED:  return "Addition Colorimetry";
    default:  return "Unknown";
    }
}

const char* _get_avi_picture_aspect_ratio_str(unsigned char apr)
{
    switch(apr)
    {
    case DP_AVI_PICTURE_ARC_NODATA: return "No Data";
    case DP_AVI_PICTURE_ARC_4_3:    return "4:3";
    case DP_AVI_PICTURE_ARC_16_9:   return "16:9";
    case DP_AVI_PICTURE_ARC_FUTURE:
    default:  return "Reserved";
    }
}

const char* _get_avi_rgb_quantization_range_str(unsigned char quantization_range)
{
    switch(quantization_range)
    {
    case DP_AVI_RGB_QUANTIZATION_RANGE_DEFAULT:      return "Default";
    case DP_AVI_RGB_QUANTIZATION_RANGE_LIMITEDRANGE: return "Limited";
    case DP_AVI_RGB_QUANTIZATION_RANGE_FULLRANGE:    return "Full Range";
    default:  return "Unknown";
    }
}

const char* _get_avi_ycc_quantization_range_str(unsigned char quantization_range)
{
    switch(quantization_range)
    {
    case DP_AVI_YCC_QUANTIZATION_RANGE_LIMITEDRANGE:    return "Limited";
    case DP_AVI_YCC_QUANTIZATION_RANGE_FULLRANGE:       return "Full";
    default:  return "Unknown";
    }
}

const char* _get_avi_scan_info_str(unsigned char scan_info)
{
    switch(scan_info)
    {
    case DP_AVI_SCAN_INFO_NODATA:       return "No Data";
    case DP_AVI_SCAN_INFO_OVERSCANNED:  return "Over Scanned";
    case DP_AVI_SCAN_INFO_UNDERSCANNED: return "Under Sacnned";
    default:  return "Reserved";
    }
}

const char* _get_avi_content_type_str(unsigned char content_type)
{
    switch(content_type)
    {
    case DP_AVI_CONTENT_TYPE_GRAPHICS: return "Graphic";
    case DP_AVI_CONTENT_TYPE_PHOTO:    return "Photo";
    case DP_AVI_CONTENT_TYPE_CINEMA:   return "Cinema";
    case DP_AVI_CONTENT_TYPE_GAME:     return "Game";
    default:  return "Unknown";
    }
}

int _dump_avi_info(unsigned char ch, char* p_buf, unsigned int count)
{
    DPRX_AVI_INFO_T avi_info;
    int n;
    char *ptr = p_buf;
    ...
    if (vfe_dprx_drv_get_port_avi_info(ch, &avi_info)==DPRX_DRV_NO_ERR && avi_info.packetStatus==DP_PACKET_STATUS_UPDATED)
    {
        n = scnprintf(ptr, count, "\n----------------------------------------------------\n"); ptr+=n; count-=n;
        n = scnprintf(ptr, count, " AVI : Packet : Type=%d, Vesion=%d len=%d \n", avi_info.packet.type, avi_info.packet.version, avi_info.packet.length);  ptr+=n; count-=n;
        n = scnprintf(ptr, count, "----------------------------------------------------\n");   ptr+=n; count-=n;
        // print data
        n = snprintf(ptr, count, "Y[2:0] :   PixelEncoding=%d (%s)\n",   avi_info.ePixelEncoding, _get_avi_color_space_str(avi_info.ePixelEncoding)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "A0     :   ActiveFormatInfo=%d\n",     avi_info.eActiveInfo); ptr+=n; count-=n;
        n = snprintf(ptr, count, "B[1:0] :   BarInfo=%d\n",              avi_info.eBarInfo); ptr+=n; count-=n;
        n = snprintf(ptr, count, "S[1:0] :   ScanInfo=%d (%s)\n",        avi_info.eScanInfo, _get_avi_scan_info_str(avi_info.eScanInfo)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "C[10]  :   Colorimetry=%d (%s)\n",     avi_info.eColorimetry, _get_avi_colorimetry_str(avi_info.eColorimetry)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "M[1:0] :   PictureAspectRatio=%d (%s)\n",   avi_info.ePictureAspectRatio, _get_avi_picture_aspect_ratio_str(avi_info.ePictureAspectRatio)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "R[3:0] :   ActiveFormatAspectRatio=%d\n", avi_info.eActiveFormatAspectRatio); ptr+=n; count-=n;
        n = snprintf(ptr, count, "ITC    :   ITContent=%d\n",            avi_info.eITContent); ptr+=n; count-=n;
        n = snprintf(ptr, count, "EC[2:0]:   ExtendedColorimetry=%d (%s)\n",  avi_info.eExtendedColorimetry, _get_avi_extension_colorimetry_str(avi_info.eExtendedColorimetry)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "Q[1:0] :   RGBQuantizationRange=%d (%s)\n", avi_info.eRGBQuantizationRange, _get_avi_rgb_quantization_range_str(avi_info.eRGBQuantizationRange)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "SC[1:0]:   Scaling=%d\n",              avi_info.eScaling); ptr+=n; count-=n;
        n = snprintf(ptr, count, "VIC[7:0]:  VIC=%d\n",                  avi_info.VideoIdCode); ptr+=n; count-=n;
        n = snprintf(ptr, count, "YQ[1:0]:   YCCQuantizationRange=%d (%s)\n", avi_info.eYCCQuantizationRange, _get_avi_ycc_quantization_range_str(avi_info.eYCCQuantizationRange)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "CN[1:0]:   ContentType=%d (%s)\n",     avi_info.eContentType, _get_avi_content_type_str(avi_info.eContentType)); ptr+=n; count-=n;
        n = snprintf(ptr, count, "PR[3:0]:   PixelRepeat=%d\n",          avi_info.PixelRepeat); ptr+=n; count-=n;
        n = snprintf(ptr, count, "ETB[15:0]: TopBarEndLineNumber=%d\n",  avi_info.TopBarEndLineNumber); ptr+=n; count-=n;
        n = snprintf(ptr, count, "SBB[15:0]: BottomBarStartLineNumber=%d\n",  avi_info.BottomBarStartLineNumber); ptr+=n; count-=n;
        n = snprintf(ptr, count, "ELB[15:0]: LeftBarEndPixelNumber=%d\n",   avi_info.LeftBarEndPixelNumber); ptr+=n; count-=n;
        n = snprintf(ptr, count, "SRB[15:0]: RightBarEndPixelNumber=%d\n",  avi_info.RightBarEndPixelNumber); ptr+=n; count-=n;
    }
    else
    {
        n = scnprintf(ptr, count, "\n----------------------------------------------------\n"); ptr+=n; count-=n;
        ...
        n = scnprintf(ptr, count, "get avi info failed, No AVI info \n");   ptr+=n; count-=n;
    }
    ...
    )
{
    ...
    if (vfe_dprx_drv_get_connected_channel(&ch) == DPRX_DRV_NO_ERR)
    {
        ...
        // PPS data
        n = _dump_pps_data(ch, ptr, count);
        ptr +=n; count-=n;
    }

    n = _dump_avi_info(ch, ptr, count);
    ptr +=n; count-=n;
    ...
    )
{
    ...
    //---------------------------------------------------
    // Sysfs Port Select Mode (default use current port)
    //---------------------------------------------------
    n = scnprintf(ptr, count, "port_select_mode=%d (0: Current port, 1: All port, 2: Only port 0, 3: Only port 1)\n", GET_PORT_SELECT_MODE);
    ptr+=n; count-=n;

    //---------------------------------------------------
    // Cable Detect
    //---------------------------------------------------
    n = scnprintf(ptr, count, "force_cable_connect=%d (0: off, 1: connect, 2: disconnect)\n", g_dprx_force_cable_connect);
    ptr+=n; count-=n;
    ...
    // Wakeup by Aux
    //---------------------------------------------------
    n = scnprintf(ptr, count, "wakeup_by_aux[0]=%d, wakeup_by_aux[1]=%d\n", ((g_dprx_vfe_suspend_mode[0] & WAKEUP_BY_AUX) ? 1 : 0), ((g_dprx_vfe_suspend_mode[1] & WAKEUP_BY_AUX) ? 1 : 0));
    ptr+=n; count-=n;

    //---------------------------------------------------
    // Wakeup by Signal
    //---------------------------------------------------
    n = scnprintf(ptr, count, "wakeup_by_signal[0]=%d, wakeup_by_signal[1]=%d\n", ((g_dprx_vfe_suspend_mode[0] & WAKEUP_BY_SIGNAL) ? 1 : 0), ((g_dprx_vfe_suspend_mode[1] & WAKEUP_BY_SIGNAL) ? 1 : 0));
    ptr+=n; count-=n;
                   ...
                   const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "port_select_mode=%5d", &val)==1)
    {
        SET_PORT_SELECT_MODE(val);
    }
    ...
    else if (sscanf(buf, "force_cable_connect=%5d", &val)==1)
    {
        g_dprx_force_cable_connect = (val <=FORCE_CABLE_DISCONNECT) ? val: FORCE_CABLE_CONNECT_OFF ;
    }
    ...
    {
        switch(GET_PORT_SELECT_MODE)
        {
            case PORT_SELECT_MODE_CURRENT:
                val = dprx_adp_attr_store(g_connected_dprx, ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
                break;
            case PORT_SELECT_MODE_ALL:
                val = dprx_adp_attr_store(_vfe_dprx_drv_get_dprx_adapter(0), ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
                val = dprx_adp_attr_store(_vfe_dprx_drv_get_dprx_adapter(1), ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
                break;
            case PORT_SELECT_MODE_ONLY_PORT_0:
                val = dprx_adp_attr_store(_vfe_dprx_drv_get_dprx_adapter(0), ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
                break;
            case PORT_SELECT_MODE_ONLY_PORT_1:
                val = dprx_adp_attr_store(_vfe_dprx_drv_get_dprx_adapter(1), ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
                break;
            default:
                val = dprx_adp_attr_store(g_connected_dprx, ADP_ATTR_DEBUG_CTRL, (unsigned char *) buf, count);
...
//===========================================================================
// Func : dprx_show_ver_info
//===========================================================================
static ssize_t dprx_show_ver_info(
    struct device*              dev,
    struct device_attribute*    attr,
    char*                       buf
    )
{
    dprx_dev_t* p_dprx = _vfe_dprx_drv_get_dprx_adapter(0);
    int  n;
    int  count = PAGE_SIZE;
    char *ptr = buf;

    n = dprx_adp_attr_show(p_dprx, ADP_ATTR_VER_INFO, ptr, count); ptr+=n; count-=n;

    return ptr - buf;
}

static DEVICE_ATTR(ver_info, 0444, dprx_show_ver_info, NULL);
...
int dprx_sysfs_init(void)
{
    ...
    device_create_file(dprx_miscdev.this_device, &dev_attr_ver_info);
    return 0;
}
