...
unsigned char g_dprx_vfe_suspend_mode[MAX_DPRX_PORT] = {WAKEUP_BY_AUX};
#else
unsigned char g_dprx_vfe_suspend_mode[MAX_DPRX_PORT] = {0};
#endif

dprx_dev_t* _vfe_dprx_drv_get_dprx_adapter(unsigned char ch)
{
    if(ch == CURRENT_CHANNEL)
        return g_connected_dprx;
...
int vfe_dprx_drv_init(void)
{
    ...
    for(ch = 0; ch < MAX_DPRX_PORT; ch++)
    {
        ...
        if (dprx_platform_get_flow_config(port, &flow_cfg)<0)
        {
            DP_VFE_ERR("DP[%d] get hw flow config %d failed\n", ch, port);
            continue;
        }
        ...
        g_p_dprx[ch] = dprx_vfe_create_dprx_adapter_by_config(&port_cfg, &flow_cfg);
...
EXPORT_SYMBOL(vfe_dprx_drv_get_connected_channel);
...
int vfe_dprx_drv_disconnect(unsigned char ch)
{
    ...
    if(g_multi_port_enable)
    {
        if(p_adp)
        {
            ...
            dprx_adp_disconnect(p_adp); // if app call vfe_dprx_drv_disconnect twice, it will disable watchdog twice.
            return DPRX_DRV_NO_ERR;
        }else
        {
 ...
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_drv_set_wakeup_mode(unsigned char ch, DP_WAKEUP_MODE_E mode)
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
    unsigned char set_mode;
    DP_VFE_INFO("vfe_dprx_drv_set_wakeup_mode[%d] = %d\n", ch, mode);
    switch(mode)
    {
    ...
    dprx_adp_set_ext_ctrl(p_adp, ADP_EXCTL_WAKEUP_MODE, set_mode); // Set Wakeup Mode
    g_dprx_vfe_suspend_mode[ch] = set_mode;
    return DPRX_DRV_NO_ERR;
}
 ...
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_drv_get_wakeup_mode(unsigned char ch, DP_WAKEUP_MODE_E* p_mode)
{
    if (p_mode==NULL)
        return DPRX_DRV_ARG_ERR;

    if ((g_dprx_vfe_suspend_mode[ch] & (WAKEUP_BY_AUX|WAKEUP_BY_SIGNAL))==(WAKEUP_BY_AUX|WAKEUP_BY_SIGNAL))
        *p_mode = DP_WAKEUP_MODE_BY_AUX_OR_SIGNAL;
    else if ((g_dprx_vfe_suspend_mode[ch] & WAKEUP_BY_AUX))
        *p_mode = DP_WAKEUP_MODE_BY_AUX;
    else if ((g_dprx_vfe_suspend_mode[ch] & WAKEUP_BY_SIGNAL))
...
EXPORT_SYMBOL(vfe_dprx_drv_get_hpd);

/*------------------------------------------------
 * Func : vfe_dprx_drv_get_pd_ready_status
 *
 * Desc : get pd ready status
 *
 * Para : ch : DPRX channel
 *        p_pd_ready_status : pd ready state output
 *           DPRX_TYPEC_PD_READY : typec pd is ready,
 *           DPRX_TYPEC_PD_BUSY  : typec pd is busy
 *
 * Retn : DPRX_DRV_NO_ERR / DPRX_DRV_STATE_ERR
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_drv_get_pd_ready_status(
    unsigned char   ch,
    unsigned char*  p_pd_ready_status
    )
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
    ...
    if (p_adp==NULL || p_pd_ready_status==NULL)
        return DPRX_DRV_ARG_ERR;

    *p_pd_ready_status = dprx_adp_get_typec_pd_ready_status(p_adp);

    return DPRX_DRV_NO_ERR;
}

EXPORT_SYMBOL(vfe_dprx_drv_get_pd_ready_status);
...
EXPORT_SYMBOL(vfe_dprx_drv_read_dpcd);
...
EXPORT_SYMBOL(vfe_dprx_drv_write_dpcd);
...
EXPORT_SYMBOL(vfe_dprx_drv_get_port_vbid_info);
...
EXPORT_SYMBOL(vfe_dprx_drv_get_port_msa_info);
...
EXPORT_SYMBOL(vfe_dprx_drv_get_port_sdp_data);
 ...
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_drv_ch_handle_online_measure_error(unsigned char ch, unsigned char flags)
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
    ...
    if (p_adp)
    {
        dprx_adp_set_ext_ctrl(p_adp, ADP_EXCTL_WATCH_DOG_ENABLE, 0); // Disable Video Watch Dog
        dprx_adp_event_handler(p_adp, DPRX_ADP_EVENT_ONLINE_MEASURE_FAIL); // send event to current dprx port
    }

    return DPRX_DRV_NO_ERR;
}

EXPORT_SYMBOL(vfe_dprx_drv_ch_handle_online_measure_error);
 ...
 *
 * Para : ch       : DP channel
 *        p_avmute : AV mute status
 *
 * Retn : DPRX_DRV_NO_ERR / DPRX_DRV_STATE_ERR
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_drv_ch_get_avmute_status(unsigned char ch, unsigned char* p_avmute)
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
    ...
    *p_avmute = 0;
    ...
    if (p_adp)
        *p_avmute = dprx_adp_get_avmute(p_adp);

    return DPRX_DRV_NO_ERR;
}

EXPORT_SYMBOL(vfe_dprx_drv_ch_get_avmute_status);

/*------------------------------------------------------
 * Func : vfe_dprx_drv_ch_get_pixel_encoding_format
 *
 * Desc : Get pixel encoding format of currenct DPRX port
 *
 * Parm : ch             : DP channel
 *        pPixelEnFormat : pointer of pixel encoding format
 *
 * Retn : DPRX_DRV_NO_ERR / DPRX_DRV_STATE_ERR
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *------------------------------------------------------*/
int vfe_dprx_drv_ch_get_pixel_encoding_format(unsigned char ch, DP_PIXEL_ENCODING_FORMAT_T *pPixelEnFormat)
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
...
EXPORT_SYMBOL(vfe_dprx_drv_ch_get_pixel_encoding_format);
...
/*------------------------------------------------
 * Func : vfe_dprx_drv_ch_get_audio_coding_type
 *
 * Desc : get audio coding status of current DPRX Port
 *
 * Para : ch             : DP channel
 *        p_audio_status : audio status
 *
 * Retn : DPRX_DRV_NO_ERR / DPRX_DRV_STATE_ERR
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
extern int vfe_dprx_drv_ch_get_audio_status(unsigned char ch, DP_AUDIO_STATUS_T *p_audio_status)
{
    dprx_dev_t* p_adp = _vfe_dprx_drv_get_dprx_adapter(ch);
    ...
    if (p_adp)
    {
        dprx_adp_get_audio_status(p_adp, p_audio_status);
    }

    return DPRX_DRV_NO_ERR;
}

EXPORT_SYMBOL(vfe_dprx_drv_ch_get_audio_status);
...
int vfe_dprx_drv_suspend(void)
{
    ...
    for(ch = 0; ch < MAX_DPRX_PORT; ch++)
    {
        if(g_dprx_vfe_suspend_mode[ch])
        {
            suspend_mode = g_dprx_vfe_suspend_mode[ch];
            break;
        }
    }
    ...
    {
        if (g_p_dprx[ch])
            dprx_adp_suspend(g_p_dprx[ch], suspend_mode);
    }
...
/*------------------------------------------------
 * Func : vfe_dprx_set_check_dolby_vision_mode_function
 *
 * Desc : set check dolby vision mode function pointer
 *
 * Para : function pointer of check dolby vision mode
 *
 * Retn : DPRX_DRV_NO_ERR / DPRX_DRV_STATE_ERR
 *        DPRX_DRV_HW_ERR / DPRX_DRV_ARG_ERR
 *-----------------------------------------------*/
int vfe_dprx_set_check_dolby_vision_mode_function(long (*fp)(unsigned char))
{
    dprx_vfe_set_dolby_vision_check_mode_function(fp);

    return DPRX_DRV_NO_ERR;
}
EXPORT_SYMBOL(vfe_dprx_set_check_dolby_vision_mode_function);
#endif
