...
static DP_FLOW_CFG   g_dprx_flow_cfg;
...
static int _get_pcb_rx_port_info(unsigned char port, DP_PORT_CFG* p_cfg)
{
    ...
    case DP_PORT_TYPE_mDP:
        ...
        p_cfg->config.dp_mdp.mac_idx = port;
#else
        p_cfg->config.dp_mdp.mac_idx = 0;
#endif
        ...
        case DP_DET_GPIO_TYPE_ISO:
            ...
            p_cfg->config.dp_mdp.hpd_det.det_gpio         = GET_DP_DET_GPIO_ID(val);;
            break;
        case DP_DET_GPIO_TYPE_MASK:
            p_cfg->config.dp_mdp.hpd_det.det_gpio_valid   = 1;
            p_cfg->config.dp_mdp.hpd_det.det_gpio_type    = 2;	// iso-mio
            p_cfg->config.dp_mdp.hpd_det.det_gpio         = GET_DP_DET_GPIO_ID(val);;
            break;
        default:
        ...
        case DP_HPD_GPIO_TYPE_ISO:
            ...
            p_cfg->config.dp_mdp.hpd_det.hpd_gpio         = GET_DP_HPD_GPIO_ID(val);;
            break;
        case DP_HPD_GPIO_TYPE_MASK:
            p_cfg->config.dp_mdp.hpd_det.hpd_gpio_valid   = 1;
            p_cfg->config.dp_mdp.hpd_det.hpd_gpio_type    = 2;  // iso-mio
            p_cfg->config.dp_mdp.hpd_det.hpd_gpio         = GET_DP_HPD_GPIO_ID(val);;
            break;
        default:
        ...
        case DP_AUX_P_GPIO_TYPE_ISO:
            ...
            p_cfg->config.dp_mdp.hpd_det.aux_p_gpio         = GET_DP_AUX_P_GPIO_ID(val);;
            break;
        case DP_AUX_P_GPIO_TYPE_MASK:
            p_cfg->config.dp_mdp.hpd_det.aux_p_gpio_valid   = 1;
            p_cfg->config.dp_mdp.hpd_det.aux_p_gpio_type    = 2;  // iso-mio
            p_cfg->config.dp_mdp.hpd_det.aux_p_gpio         = GET_DP_AUX_P_GPIO_ID(val);;
            break;
        default:
        ...
        case DP_AUX_N_GPIO_TYPE_ISO:
            ...
            p_cfg->config.dp_mdp.hpd_det.aux_n_gpio         = GET_DP_AUX_N_GPIO_ID(val);;
            break;
        case DP_AUX_N_GPIO_TYPE_MASK:
            p_cfg->config.dp_mdp.hpd_det.aux_n_gpio_valid   = 1;
            p_cfg->config.dp_mdp.hpd_det.aux_n_gpio_type    = 2;  // iso-mio
            p_cfg->config.dp_mdp.hpd_det.aux_n_gpio         = GET_DP_AUX_N_GPIO_ID(val);;
            break;
        default:
...
#ifdef CONFIG_DPRX_ENABLE_MULTIPORT
        p_cfg->config.typec_dp.mac_idx = port;
#else
        p_cfg->config.typec_dp.mac_idx = 0;
#endif
...
/*------------------------------------------------
 * Func : _get_pcb_flow_cfg
 *
 * Desc : get pcb configurations from PCB Mgr
 *
 * Para : port  : dp port
 *        p_flow_cfg : port configirations
 *
 * Retn : 0 : successed, others : failed
 *-----------------------------------------------*/
static int _get_pcb_flow_cfg(DP_FLOW_CFG* p_flow_cfg)
{
    unsigned long long val;
    ...
    memset(p_flow_cfg, 0, sizeof(DP_FLOW_CFG));

    if (pcb_mgr_get_enum_info_byname("DP_FLOW_CFG", &val) < 0)
    {
        DP_PLAT_WARN("Get PCB Param - DP_FLOW_CFG failed \n");
        return -1;
    }
    else
    {
        DP_PLAT_WARN("Get PCB Param - DP_FLOW_CFG successed \n");
    }

    p_flow_cfg->ignore_dsc_check_en = GET_DP_IGNORE_DSC_ERR(val);
...
static void _load_dprx_cfg(void)
{
    ...
    // get flow config
    _get_pcb_flow_cfg(&g_dprx_flow_cfg);
}
...
/*------------------------------------------------
 * Func : dprx_platform_get_flow_config
 *
 * Desc : get config of a given DP port
 *
 * Para : idx :
 ...
 * Retn : 0 : success, -1 failed
 *-----------------------------------------------*/
int dprx_platform_get_flow_config(unsigned char port, DP_FLOW_CFG* p_flow_cfg)
{
    if (p_flow_cfg == NULL || port >= PLAT_DPRX_PORT_NUM)
        return -1;

    memcpy(p_flow_cfg, &g_dprx_flow_cfg, sizeof(DP_FLOW_CFG));
