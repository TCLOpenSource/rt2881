...
typedef struct
{
    ...
    unsigned char  hpd_gpio_type    : 3;   // 0 : misc / 1 : iso / 2 : iso-mio
    unsigned char  det_gpio_valid   : 1;
    unsigned char  det_gpio_type    : 3;   // 0 : misc / 1 : iso / 2 : iso-mio
    unsigned char  aux_n_gpio_valid : 1;
    unsigned char  aux_n_gpio_type  : 3;   // 0 : misc / 1 : iso / 2 : iso-mio
    unsigned char  aux_p_gpio_valid : 1;
    unsigned char  aux_p_gpio_type  : 3;   // 0 : misc / 1 : iso / 2 : iso-mio
...
typedef struct
{
    ...
    unsigned char   ch;                    //vfe channel port
    ...
    union
    {
        struct
        {
            ...
            unsigned char   mac_idx;       // port mac id
        }dp_mdp;
...
typedef struct
{
    unsigned char   ignore_dsc_check_en;
 ...
 *
 * Desc : get config of a given DP flow
 *
 * Para : idx :
 *
 * Retn : 0 : success, -1 failed
 *-----------------------------------------------*/
extern int dprx_platform_get_flow_config(unsigned char idx, DP_FLOW_CFG* p_flow_cfg);
