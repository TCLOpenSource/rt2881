...
extern unsigned char g_dprx_force_cable_connect;
#define FORCE_CABLE_CONNECT_OFF 0
#define FORCE_CABLE_CONNECT     1
#define FORCE_CABLE_DISCONNECT  2
...
typedef struct
{
    ...
    int (*set_hpd_guard_time)(dev_ctx_t* p_adp, unsigned short ms_delay);
    int (*get_hpd_guard_time)(dev_ctx_t* p_adp);
#endif
    ...
    int (*get_typec_pd_ready_status)(dev_ctx_t* p_adp);    // get typec pd ready status (0: pd ready, 1: pd busy)
...
#ifdef CONFIG_DPRX_VFE_ENABLE_HPD_GUARD_TIME
extern int dprx_hpd_set_guard_time(hpd_dev_t* p_dev, unsigned short ms_delay);
extern int dprx_hpd_get_guard_time(hpd_dev_t* p_dev);
#endif
...
extern int dprx_hpd_get_typec_pd_ready_status(hpd_dev_t* p_dev);
...
#define  DEFAULT_DP_HPD_LOW_PERIOD_MS          900  // MCE1 Slim Dock takes more than 800 ms to detect long hpd toggle
