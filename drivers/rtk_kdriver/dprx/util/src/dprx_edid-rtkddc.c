...
static int _ops_init(dev_ctx_t* p_ctx)
{
    ...
    if (RTK_DDC_FLAG_GET_ENABLE_QS_INIT(p_dev->flags)) {  // skip ddc initial if qs enabled
...
static int _ops_uninit(dev_ctx_t* p_ctx)
{
    ...
    if(rtk_ddc_lib_is_crt_on(p_dev->idx) == 0)
    {
        DPRX_EDID_ERR("RTK DDC CRT[%d] = OFF\n", p_dev->idx);
        return -1;
    }
...
static int _ops_enable(dev_ctx_t* p_ctx, unsigned char enable)
{
    ...
    if (enable==0)
    {
        rtk_ddc_lib_sync_reset(p_dev->idx);
        DPRX_EDID_INFO("RTK DDC disabled, force reset. (ddc_ch=%d)\n", p_dev->idx);
    }
    ...
    rtk_ddc_lib_aux_switch_port(p_dev->idx, p_dev->aux_idx);
#endif
    ...
    )
{
    ...
    if (p_dev)
    {
        ...
        p_dev->idx = ddc_idx;
        p_dev->flags = flags;
        p_dev->aux_idx = RTK_DDC_FLAG_GET_AUX_ID(flags);
