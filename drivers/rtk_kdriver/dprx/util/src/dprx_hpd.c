...
unsigned char g_dprx_force_cable_connect = FORCE_CABLE_CONNECT_OFF;
...
{
    switch (g_dprx_force_cable_connect)
    {
    case FORCE_CABLE_CONNECT:    return 1;
    case FORCE_CABLE_DISCONNECT: return 0;
    default: break;
    }
 ...
 * Retn : 0 : successed
 *-----------------------------------------------*/
int dprx_hpd_set_guard_time(hpd_dev_t* p_dev, unsigned short delay_ms)
{
    if (p_dev && p_dev->ops.set_hpd_guard_time)
        return p_dev->ops.set_hpd_guard_time(&p_dev->ctx, delay_ms);

    return 0;
}

/*------------------------------------------------
 * Func : dprx_hpd_get_guard_time
 *
 ...
 * Retn : guard time
 *-----------------------------------------------*/
int dprx_hpd_get_guard_time(hpd_dev_t* p_dev)
{
    if (p_dev && p_dev->ops.get_hpd_guard_time)
        return p_dev->ops.get_hpd_guard_time(&p_dev->ctx);
...
/*------------------------------------------------
 * Func : dprx_hpd_get_typec_pd_ready_status
 *
 ...
 *-----------------------------------------------*/
int dprx_hpd_get_typec_pd_ready_status(hpd_dev_t* p_dev)
{
    if (p_dev && p_dev->ops.get_typec_pd_ready_status)
        return p_dev->ops.get_typec_pd_ready_status(&p_dev->ctx);
    ...
    )
{
    ...
    if (guard_time==0 || pre_hpd_low_time_stamp==0)
        return 0;

    if (curr_time >= pre_hpd_low_time_stamp)
        time_diff = curr_time - pre_hpd_low_time_stamp;
    else
        time_diff = (UNSIGNED_LONG_MAX - pre_hpd_low_time_stamp) + curr_time;

    DPRX_HPD_INFO("_get_hpd_low_guard_time curr_time = %lu, pre_hpd_low_time_stamp = %lu\n", curr_time, pre_hpd_low_time_stamp);
