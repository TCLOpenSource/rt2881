...
static int _ops_init(dev_ctx_t* p_ctx)
{
    ...
    if (p_dev->cfg.hpd.valid)
    {
        ...
        {
            DPRX_HPD_INFO("[DP]Normal HPD init, force hpd low\n");
            rtk_gpio_output((RTK_GPIO_ID)p_dev->cfg.hpd.gid, 0);     // pull low hpd
        }
        else
        {
            DPRX_HPD_INFO("[DP]QS HPD init, do not change HPD state\n");
        }
...
static int _ops_get_connect_status(dev_ctx_t* p_ctx)
{
    ...
    if (p_dev->cfg.det.valid)
        return (rtk_gpio_input((RTK_GPIO_ID)p_dev->cfg.det.gid) ? 0 : 1) | _ops_get_aux_status(p_ctx, NULL);
...
static int _ops_set_hpd(dev_ctx_t* p_ctx, unsigned char on)
{
    ...
    if (p_dev->cfg.hpd.valid)
    {
        if (on)
        {
            ...
            if (guard_time)
            {
                DPRX_HPD_INFO("[DP]Set HPD on with in guard interval(%d ms), delay HPD high for %d ms\n",
 ...
 * Retn : 0 : successed, 0< : failed
 *-----------------------------------------------*/
static int _ops_set_hpd_guard_time(dev_ctx_t* p_ctx, unsigned short delay_ms)
{
    rtk_hpd_dev_t* p_dev = GET_RTK_HPD_DEV(p_ctx);
    ...
    p_dev->cfg.hpd_low_peroid_ms = delay_ms;
    DPRX_HPD_INFO("[DP]HPD new guard time = %d\n", p_dev->cfg.hpd_low_peroid_ms);

    return 0;
}

/*------------------------------------------------
 * Func : _ops_get_hpd_guard_time
 *
 ...
 *-----------------------------------------------*/
static int _ops_get_hpd_guard_time(dev_ctx_t* p_ctx)
{
    rtk_hpd_dev_t* p_dev = GET_RTK_HPD_DEV(p_ctx);
    ...
    return p_dev->cfg.hpd_low_peroid_ms;
}
#endif
...
hpd_dev_t* create_rtk_tv_dprx_hpd_device(rtk_hpd_cfg_t* p_cfg)
{
    ...
    if (p_cfg==NULL ||
        ...
        p_cfg->hpd.valid==0)
    {
        DPRX_HPD_ERR("[DP]create_rtk_tv_dprx_hpd_device failed, invalid configuration, please check your configiration\n");
        return NULL;
    }
...
#endif
#ifdef CONFIG_DPRX_VFE_ENABLE_HPD_GUARD_TIME
        p_dev->hpd_dev.ops.set_hpd_guard_time = _ops_set_hpd_guard_time;
        p_dev->hpd_dev.ops.get_hpd_guard_time = _ops_get_hpd_guard_time;
#endif
...
RTK_GPIO_ID GET_GPIO_ID(unsigned int type, unsigned char gpio_id) {
    ...
    return rtk_gpio_id(temp, gpio_id);
}
