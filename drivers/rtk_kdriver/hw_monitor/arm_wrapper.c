...
#ifdef SCPU_WRAPPER_mem_trash_dbg_start_0_reg
...
#define ARM_WRAPPER_MONITOR_BIT32    32
#define ARM_WRAPPER_MONITOR_BIT33    33

#define DBG_START_ADDR_BIT32_CTRL_BIT    30
#define DBG_START_ADDR_BIT33_CTRL_BIT    31

#define DBG_END_ADDR_BIT32_CTRL_BIT    28
#define DBG_END_ADDR_BIT33_CTRL_BIT    29
#endif
...
int set_arm_wrapper_arange (unsigned long start,unsigned long end, int set_id,WRAPPER_RW rw)
{
    ...
    if (((start>>ARM_WRAPPER_MONITOR_BIT32)&0x1) || ((start>>ARM_WRAPPER_MONITOR_BIT33)&0x1))
    {
        arm_wrapper_enable = arm_wrapper_enable | (((start>>ARM_WRAPPER_MONITOR_BIT32)&0x1) << DBG_START_ADDR_BIT32_CTRL_BIT);
        arm_wrapper_enable = arm_wrapper_enable | (((start>>ARM_WRAPPER_MONITOR_BIT33)&0x1) << DBG_START_ADDR_BIT33_CTRL_BIT);
        //rtd_pr_hw_monitor_err("%s %d  0x%016llx  %llx  %llx\n",__func__,__LINE__,start,(start>>32)&0x1,(start>>33)&0x1);
    }

    if (((end>>ARM_WRAPPER_MONITOR_BIT32)&0x1) || ((end>>ARM_WRAPPER_MONITOR_BIT33)&0x1))
    {
        arm_wrapper_enable = arm_wrapper_enable | (((end>>ARM_WRAPPER_MONITOR_BIT32)&0x1) << DBG_END_ADDR_BIT32_CTRL_BIT);
        arm_wrapper_enable = arm_wrapper_enable | (((end>>ARM_WRAPPER_MONITOR_BIT33)&0x1) << DBG_END_ADDR_BIT33_CTRL_BIT);
        //rtd_pr_hw_monitor_err("%s %d  0x%016llx  %llx %llx\n",__func__,__LINE__,end,(end>>32)&0x1,(end>>33)&0x1);
    }
#endif
...
#elif (defined CONFIG_ARCH_RTK2819A)
            len += show_msg(buf,len,"set%d: 0x%02x%08x-0x%02x%08x, 0x%08x[ctrl]. \e[1;31m %s monitor enabled!\e[0m\n",
                            i+1,
                            (rtd_inl(SCPU_MEM_TRASH_DBG_CTRL_0+4*i)>>DBG_START_ADDR_BIT32_CTRL_BIT)&0x3,
                            rtd_inl(SCPU_MEM_TRASH_DBG_START_0+4*i),
                            (rtd_inl(SCPU_MEM_TRASH_DBG_CTRL_0+4*i)>>DBG_END_ADDR_BIT32_CTRL_BIT)&0x3,
...
#elif (defined CONFIG_ARCH_RTK2819A)
            len += show_msg(buf,len,"set%d: 0x%02x%08x-0x%02x%08x, 0x%08x[ctrl].\n",
                            i+1,
                            (rtd_inl(SCPU_MEM_TRASH_DBG_CTRL_0+4*i)>>DBG_START_ADDR_BIT32_CTRL_BIT)&0x3,
                            rtd_inl(SCPU_MEM_TRASH_DBG_START_0+4*i),
                            (rtd_inl(SCPU_MEM_TRASH_DBG_CTRL_0+4*i)>>DBG_END_ADDR_BIT32_CTRL_BIT)&0x3,
...
#if !defined(CONFIG_ARCH_RTK2851C) && !defined(CONFIG_ARCH_RTK2851F) && !defined(CONFIG_ARCH_RTK6702) && !defined(CONFIG_ARCH_RTK6748) && !defined(CONFIG_ARCH_RTK2819A)
...
{
    unsigned long total_ddr_size = get_memory_size_total();
    unsigned long overrange_end = 0xffffffff;
...
#define PCIE_RANGE_START 0xC0000000
    overrange_end = PCIE_RANGE_START-1;
#elif (defined CONFIG_ARCH_RTK2875Q)
// Kernel will remap SPI(0x1820_0000 ~ 0x1a20_0000) to (0x8020_0000 ~ 0x8220_0000) as FBUS range
// arm-wrapper need allow read/write 0x8020_0000 ~ 0x8220_0000
#define RTK_SPI_ADDR    0x80200000
#define RTK_SPI_SIZE    0x02000000
    if (total_ddr_size < (RTK_SPI_ADDR + RTK_SPI_SIZE))
    {
        total_ddr_size = RTK_SPI_ADDR + RTK_SPI_SIZE;
    }
#else
    overrange_end = 0xffffffff;
#endif

    if (total_ddr_size < overrange_end)
    {
        set_arm_wrapper_arange(total_ddr_size, overrange_end, WRAPPER_DFT_ENTRY_OVERRANGE, WRAPPER_WRTIE);
    }
}
...
#else
    unsigned long long trap_address=0,trap_address_high=0;
#endif
...
#ifdef CONFIG_ARCH_RTK2819A
    trap_address_high=(rtd_inl(SCPU_WRAPPER_mem_trash_dbg_rw_reg)&0x30)>>SCPU_WRAPPER_mem_trash_dbg_rw_dbg_addr_m0_bit3332_shift;
