...
#endif
#include "rtk_sb2_module_define.h"
...
void sb2_emcu_timeout_intr_clear(void)
{
...
#ifdef SYS_REG_EFUSE_CTRL0_mt_isln_en_shift
    if (((rtd_inl(SYS_REG_EFUSE_CTRL0_reg) >> SYS_REG_EFUSE_CTRL0_mt_isln_en_shift) & 0x1) == 0x1)
    {
        return ;
    }
#endif

    // RTK6748 will stuck when standby register rbus timeout happen.
    // clear 0x18060400 bit4 fist can let RTK6748 standby register rbus timeout not stuck.
    rtd_maskl(SCPU_WRAPPER_wrapper_cfg1_reg, ~(SCPU_WRAPPER_wrapper_cfg1_rbus_slverr_disable_mask), SCPU_WRAPPER_wrapper_cfg1_rbus_slverr_disable(1));
    cancel_monitor_emcu_wrapper();
    EMCU_wrapper_8051_val = rtd_inl(EMCU_wrapper_8051_reg);
    if (EMCU_wrapper_8051_val & EMCU_wrapper_8051_rbus_timeout_int_mask)
    {
    ...
    rtd_maskl(SCPU_WRAPPER_wrapper_cfg1_reg, ~(SCPU_WRAPPER_wrapper_cfg1_rbus_slverr_disable_mask), SCPU_WRAPPER_wrapper_cfg1_rbus_slverr_disable(0));
#endif
}
...
void dump_sb2dbg_regval(char *buf, int ofst)
{
    ...
    for (i = 8; i < 16; i++)
    {
        ...
        len += show_msg(buf,ofst+len,"[set%02d]%08x=%08x SB2_DBG_END2\n", i + 1, SB2_DBG_END2_0_reg + (i - 8) * 4, rtd_inl(SB2_DBG_END2_0_reg + (i - 8) * 4));
...
void sb2_rbus_timeout_emcu_info(char *buf, int ofst)
{
    int len=0;
#if ((defined EMCU_RBUS_TIMEOUT_IP_ARRAY_NUM) || (defined EMCU_wrapper_8051_rbus_timeout_master_shift))
    int value = 0;
#endif
#ifdef EMCU_wrapper_8051_rbus_timeout_master_shift
    char *rbus_timeout_master[] = 
    {
        "standby uart2rbus",
        "sb2",
        "8051",
    ...
    if (((rtd_inl(SYS_REG_EFUSE_CTRL0_reg) >> SYS_REG_EFUSE_CTRL0_mt_isln_en_shift) & 0x1) == 0x1)
    {
        len += show_msg(buf,ofst+len,"\n===mt_isln_en is 1, no need display standby info===\n");
        return ;
    }
#endif

#ifdef EMCU_RBUS_TIMEOUT_IP_ARRAY_NUM
    value = ((EMCU_wrapper_8051_val&EMCU_wrapper_8051_rbus_timeout_ip_mask) >> EMCU_wrapper_8051_rbus_timeout_ip_shift);
    if ((value > 0) && (value < EMCU_RBUS_TIMEOUT_IP_ARRAY_NUM))
    {
        len += show_msg(buf,ofst+len,"\n EMCU rbus_timeout_ip : [%s] [0x%x] \n", emcu_rbus_timeout_ip[value], value);
    }
#endif
#ifdef EMCU_wrapper_8051_rbus_timeout_master_shift
    value = ((EMCU_wrapper_8051_val&EMCU_wrapper_8051_rbus_timeout_master_mask) >> EMCU_wrapper_8051_rbus_timeout_master_shift);
    if ((value > 0) && ((value-1) < (sizeof(rbus_timeout_master)/sizeof(rbus_timeout_master[0]))))
    {
        len += show_msg(buf,ofst+len,"\n EMCU rbus_timeout_master : [%s] [0x%x] \n", rbus_timeout_master[value-1], value);
    }
#endif

#ifdef EMCU_MCU_timeout_reg_reg
    len += show_msg(buf,ofst+len,"\n EMCU timeout reg 0x%08x:0x%08x \n", EMCU_MCU_timeout_reg_reg, rtd_inl(EMCU_MCU_timeout_reg_reg));
#endif
    
    // Main Top Isolation Enable status
    len += show_msg(buf,ofst+len,"\n MT_ISLN_EN[Bit0] 0x%08x:0x%08x \n", STB_ST_CTRL0_reg, rtd_inl(STB_ST_CTRL0_reg));
}
...
bus_err:
    ...
    len += show_msg(NULL,ofst+len,"\n EMCU status 0x%08x:0x%08x \n", EMCU_wrapper_8051_reg, EMCU_wrapper_8051_val);

    // display rbus timeout emcu info
    sb2_rbus_timeout_emcu_info(NULL,0);
#ifdef SYS_REG_EFUSE_CTRL0_mt_isln_en_shift
    // shadow of Main Top Isolation Enable status
    len += show_msg(NULL,ofst+len,"\n reg_mt_isln_en(18060010[0]) shadow register 0x%08x[Bit1]:0x%08x \n", SYS_REG_EFUSE_CTRL0_reg, rtd_inl(SYS_REG_EFUSE_CTRL0_reg));
#endif
...
int sb2_dbg_clear (sb2_dbg_param *pParam)
{
    ...
    unsigned int sb2dbg_final_clear = (BIT(SB2_DBG_CTRL_0_write_en7_shift)
                                       ...
                                       | BIT(SB2_DBG_CTRL_0_write_en1_shift));
...
EXPORT_SYMBOL(sb2_dbg_get_info);
...
{
    sb2_dbg_param param;
    unsigned long total_size = get_memory_size_total();
    memset(&param, 0, sizeof(param));
