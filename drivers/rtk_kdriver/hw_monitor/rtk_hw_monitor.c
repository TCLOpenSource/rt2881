...
#if defined(MODULE_SB2_VCPU2) && defined(CONFIG_RTK_KDRV_SUPPORT_VCPU2)
...
#else
        hook_fault_code(8, hw_monitor_check_handler, SIGBUS, 0, "external abort on bus error");
        hook_fault_code(0x16, hw_monitor_check_handler, SIGBUS, 0, "external async abort on bus error");
#endif
#endif
#endif
...
static void hw_monitor_module_exit(void)
{
        atomic_notifier_chain_unregister(&panic_notifier_list, &hw_monitor_write_panic);
        unregister_die_notifier(&hw_monitor_read_panic);
        return ;
