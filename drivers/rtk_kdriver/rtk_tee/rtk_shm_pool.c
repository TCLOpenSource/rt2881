...
#else
	struct tee_shm *shm = (struct tee_shm *)(regs->uregs[1]);
#endif
...
#else
	struct tee_shm_pool_mgr *pool_mgr = (struct tee_shm_pool_mgr *)(regs->uregs[0]);
	struct tee_shm *shm = (struct tee_shm *)(regs->uregs[1]);
	size_t size = regs->uregs[2];
#endif
