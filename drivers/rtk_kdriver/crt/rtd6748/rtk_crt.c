...
#define MAX_SWSEM_BITS 32
unsigned long rtd_swsem_user[MAX_SWSEM_BITS]={0};
int rtd_swsem_debug;  //sysctl used

int rtd_swsem_debug_func(void)
{
    int i;
    for(i=0; i<MAX_SWSEM_BITS; i++) {
        rtd_pr_crt_emerg( "###[%s(%d) user: 0x%x \n", __func__, __LINE__, rtd_swsem_user[i]);
    }
...
void rtd_crt_swsem_lock(int bit)
{
    ...
    rtd_pr_crt_info("###[%s(%d) Attempting to lock swsem bit: %d, index: %d\n", __func__, __LINE__, bit, index);

    if (index >= MAX_SWSEM_BITS) {
        rtd_pr_crt_err("###[%s(%d) Index out of bounds: %d\n", __func__, __LINE__, index);
        return;
    }
...
#if !defined (RTD_SWSEM_DEBUG_DISABLE)
        if (retry++ == 0x100000) {
            rtd_pr_crt_info("###[%s(%d) Retrying to lock swsem bit: %d\n", __func__, __LINE__, bit);
            rtd_swsem_debug_func();
        }
#endif //#if !defined (RTD_SWSEM_DEBUG_DISABLE)
    }

    rtd_pr_crt_info("###[%s(%d) Setting rtd_swsem_user\n", __func__, __LINE__);
    rtd_swsem_user[index] = (unsigned long)__builtin_return_address(0);
    rtd_pr_crt_info("###[%s(%d) Locked swsem bit: %d, index: %d\n", __func__, __LINE__, bit, index);
}
...
void rtd_crt_swsem_unlock(int bit)
{
    ...
    rtd_pr_crt_info("###[%s(%d) Unlocking swsem bit: %d, index: %d\n", __func__, __LINE__, bit, index);

    if (index >= MAX_SWSEM_BITS) {
        rtd_pr_crt_err("Index out of bounds: %d\n", index);
        return;
    }
...
#if !defined (RTD_SWSEM_DEBUG_DISABLE)
    rtd_swsem_user[index] = 0;
#endif //#if !defined (RTD_SWSEM_DEBUG_DISABLE)
    rtd_hwsem_1_unlock();
    rtd_pr_crt_info("###[%s(%d) Unlocked swsem bit: %d, index: %d\n", __func__, __LINE__, bit, index);
}
