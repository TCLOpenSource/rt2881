...
static void USB3_5G_P0_CLK_ON(void)
{		
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));
			//xhci u3 5G U2 P0
			rtd_outl(SYS_REG_USB_CTRL_reg, (rtd_inl(SYS_REG_USB_CTRL_reg) & (~0xFC000000)) | 0x6C000000);//pinmux

			rtd_outl(SYS_REG_SYS_CLKEN6_reg,
                    SYS_REG_SYS_CLKEN6_clken_usb3_hst_dwc_mask |
	        ...
	        rtd_outl(SYS_REG_SYS_SRST6_reg,
	                    SYS_REG_SYS_SRST6_rstn_usb3_hst_dwc_mask |
			...
			rtd_outl(SYS_REG_SYS_SRST3_reg,
						SYS_REG_SYS_SRST3_rstn_usb3_gen1_phy_p0_mask |
						SYS_REG_SYS_SRST3_rstn_usb3_gen1_mdio_p0_mask |
						SYS_REG_SYS_SRST3_write_data(0));	
	        udelay(USB_PHY_DELAY);
...
static void USB3_5G_P0_CLK_OFF(void)
{
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));
...
static void USB3_5G_P1_CLK_ON(void)
{		
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));
			
			rtd_outl(SYS_REG_USB_CTRL_reg, rtd_inl(SYS_REG_USB_CTRL_reg) & (~(0x3F << SYS_REG_USB_CTRL_p1_u2phy_mac_sel_shift)));
...
static void USB3_5G_P1_CLK_OFF(void)
{
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));
...
static void USB3_10G_P0_CLK_ON(void)
{
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));

			//switch P0 and U3 10G
			rtd_outl(SYS_REG_USB_CTRL_reg, (rtd_inl(SYS_REG_USB_CTRL_reg) & (~0xFC000000)) | 0xFC000000);
			...
			rtd_outl(SYS_REG_SYS_SRST6_reg,
						SYS_REG_SYS_SRST6_rstn_usb3_gen2_phy_p0_mask |
						SYS_REG_SYS_SRST6_rstn_usb3_gen2_mdio_p0_mask |
						SYS_REG_SYS_SRST6_write_data(0));	
			udelay(USB_PHY_DELAY);

			rtd_outl(SYS_REG_SYS_CLKEN6_reg,SYS_REG_SYS_CLKEN6_clken_usb3_dev_dwc_mask |
											SYS_REG_SYS_CLKEN6_clken_usb3_dev_wrapper_mask | 
			...
			rtd_outl(USB3_GEN2_PHY_CTRL_PHY_CTR0_reg,0x00000011);  // [4] phy mode: 1= 5G or 10G, [0] POW_PCIX (need set immediately after usb3 gen2 phy rst)
			rtd_outl(USB3_GEN2_PHY_CTRL_MDIO_CTR1_reg,0x00000000);  // [31] mdio_enable = 0	
			mdelay(1);
	        ...
	        rtd_outl(USB3_GEN2_PHY_CTRL_MDIO_CTR1_reg,0x80000000);  // [31] mdio_enable = 1
...
static void USB3_10G_P0_CLK_OFF(void)
{
			rtd_outl(SYS_REG_INT_CTRL_SCPU_reg,
							SYS_REG_INT_CTRL_SCPU_usb3_int_scpu_routing_en_mask |
							SYS_REG_INT_CTRL_SCPU_write_data(0));
...
int GDMA_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON) {
		...
		if (src_type == 0) {	/* enable/disable clken_disp_gdma */
			...
			rtd_outl(SYS_REG_SYS_SRST5_reg, SYS_REG_SYS_SRST5_rstn_tvsb8_mask | SYS_REG_SYS_SRST5_rstn_gdma_mask | SYS_REG_SYS_SRST5_write_data_mask);	 /* HW Reset*/
			rtd_outl(SYS_REG_SYS_CLKEN5_reg, SYS_REG_SYS_CLKEN5_clken_tvsb8_mask | SYS_REG_SYS_CLKEN5_clken_gdma_rbus_mask 	| SYS_REG_SYS_CLKEN5_clken_gdma_mask | SYS_REG_SYS_CLKEN5_write_data_mask);
		...
		if (src_type == 0) {
			...
			rtd_outl(SYS_REG_SYS_CLKEN5_reg, SYS_REG_SYS_CLKEN5_clken_gdma_rbus_mask | SYS_REG_SYS_CLKEN5_clken_gdma_mask);
...
int ETH_CriticalRegister(enum CRT_CLK clken, void *private_data)
{
	...
	if (clken == CLK_ON && wol_mode == 0) {
		...
		rtd_outl(PLLETN_GETN_RST_reg, PLLETN_GETN_RST_rstn_getn_sys_mask | PLLETN_GETN_RST_write_data_mask);  //RSTN_GETN_PHY bit[3] = 1, RSTN_GETN_MAC bit[2] = 1, RSTN_GETN_SYS[1] = 1
		rtd_outl(PLLETN_GETN_CLKEN_reg,  PLLETN_GETN_CLKEN_clken_getn_sys_mask | PLLETN_GETN_CLKEN_write_data_mask);  //CLKEN_GETN_MAC bit[2] = 1, CLKEN_GETN_SYS[1] = 1
		rtd_outl(PLLETN_GETN_RST_reg, PLLETN_GETN_RST_rstn_getn_phy_mask | PLLETN_GETN_RST_write_data_mask);  //RSTN_GETN_PHY bit[3] = 1, RSTN_GETN_MAC bit[2] = 1, RSTN_GETN_SYS[1] = 1
		
		rtd_outl(PLLETN_SYS_PLL_ETN1_reg, 0x01400000);// pllent_tielalf[24]=1, plletn_pow[23]=0,plletn_powldo[22]=1
    	mdelay(11);
    	rtd_outl(PLLETN_SYS_PLL_ETN1_reg, 0x01c00000); // pllent_tielalf[24]=1, plletn_pow[23]=1,plletn_powldo[22]=1

		rtd_outl(PLLETN_WOW_CLK_reg,  0xd8000000);

		rtd_outl(STB_ST_CTRL0_reg,rtd_inl(STB_ST_CTRL0_reg) | STB_ST_CTRL0_pad_power_down_rgmii_n_mask);

		rtd_outl(PLLETN_mdi_phyad_reg, 0x00000100);//PHY address=0x1

		//PAD
		rtd_outl(PLLETN_GPIO_RGMII_CFG_0_reg,0x01010101);    //MDC/MDIO/TXD3/TXD2
		rtd_outl(PLLETN_GPIO_RGMII_CFG_1_reg,0x01010101);    //TXD1/TXD0/TX_CTL/TXC
		rtd_outl(PLLETN_GPIO_RGMII_CFG_2_reg,0x01010101);    //RXD3/RXD2/RXD1/RXD0
		rtd_outl(PLLETN_GPIO_RGMII_CFG_3_reg,0x01010000);    //RX_CTL/RXC/
		rtd_outl(PLLETN_RGMII_Ctrl_1_reg,0x8001);    //Vdsel=0x10

		rtd_outl(PLLETN_GETN_RST_reg, PLLETN_GETN_RST_rstn_getn_mac_mask | PLLETN_GETN_RST_write_data_mask);  //RSTN_GETN_PHY bit[3] = 1, RSTN_GETN_MAC bit[2] = 1, RSTN_GETN_SYS[1] = 1
		rtd_outl(PLLETN_GETN_CLKEN_reg,  PLLETN_GETN_CLKEN_clken_getn_mac_mask | PLLETN_GETN_CLKEN_write_data_mask);  //CLKEN_GETN_MAC bit[2] = 1, CLKEN_GETN_SYS[1] = 1
...
int TV_All_CriticalRegister(enum CRT_CLK clken)
{
	...
	*/
    ...
    if (clken == CLK_OFF) {
    	...
    	rtd_maskl(DEMOD_SSB_DEMOD_SSB_DCU1_ARB_CR1_reg, ~(0x00000400 | 0x00000200), 0x0);//TVSB5 don't process command from cliend(demod/demoddbg)
	mdelay(1);
