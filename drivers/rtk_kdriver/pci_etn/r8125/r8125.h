...
#define R8125_NAPI_WEIGHT   96 //64
...
#define NUM_TX_DESC     256 // 1024    /* Number of Tx descriptor registers */
#define NUM_RX_DESC     256 //1024    /* Number of Rx descriptor registers */
...
#ifdef CONFIG_PLATFORM_ARM_RTD299X
#define RTL8125_TRACE()                    pr_err("[RTL8125][T] %s:%d \n", __FUNCTION__, __LINE__);
#define RTL8125_ERR(fmt, args...)          pr_err("[RTL8125][E] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_WARN(fmt, args...)         pr_err("[RTL8125][W] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_INFO(fmt, args...)         pr_err("[RTL8125][I] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_DBG(fmt, args...)          //pr_err("[RTL8125][D] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);

#define RTL8125_TX_ERR(fmt, args...)       pr_err("[RTL8125][E] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TX_WARN(fmt, args...)      pr_err("[RTL8125][W] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TX_INFO(fmt, args...)      pr_err("[RTL8125][I] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TX_DBG(fmt, args...)       //pr_err("[RTL8125][D] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);

#define RTL8125_RX_ERR(fmt, args...)       pr_err("[RTL8125][RX][E] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_RX_WARN(fmt, args...)      pr_err("[RTL8125][RX][W] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_RX_INFO(fmt, args...)      pr_err("[RTL8125][RX][I] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_RX_DBG(fmt, args...)       //pr_err("[RTL8125][RX][D] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);

#define RTL8125_TIMER_ERR(fmt, args...)    pr_err("[RTL8125][E] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TIMER_WARN(fmt, args...)   pr_err("[RTL8125][W] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TIMER_INFO(fmt, args...)   pr_err("[RTL8125][I] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#define RTL8125_TIMER_DBG(fmt, args...)    //pr_err("[RTL8125][D] %s:%d " fmt, __FUNCTION__, __LINE__, ## args);
#else
#define RTL8125_TRACE()
#define RTL8125_ERR(fmt, args...)
#define RTL8125_WARN(fmt, args...)
#define RTL8125_INFO(fmt, args...)
#define RTL8125_DBG(fmt, args...)

#define RTL8125_TX_ERR(fmt, args...)
#define RTL8125_TX_WARN(fmt, args...)
#define RTL8125_TX_INFO(fmt, args...)
#define RTL8125_TX_DBG(fmt, args...)

#define RTL8125_TIMER_ERR(fmt, args...)
#define RTL8125_TIMER_WARN(fmt, args...)
#define RTL8125_TIMER_INFO(fmt, args...)
#define RTL8125_TIMER_DBG(fmt, args...)
#endif

#ifdef CONFIG_PLATFORM_ARM_RTD299X
//#define CONFIG_TX_SECURITY_MEM
//#define SECURITY_MEM_ADDR   0x78800000
//#define SECURITY_MEM_ADDR   0x23ffc000
#define SECURITY_MEM_ADDR   0x24000000
#define SECURITY_MEM_SIZE   0x03800000

//#define SECURITY_MEM_ADDR   0x5f800000
//#define SECURITY_MEM_ADDR   0x5FEFC000		//Tx desc 前/Rx desc 前/Tx buff 前/Rx buff 後 OK
//#define SECURITY_MEM_ADDR   0x5FDFC000	//Tx desc 前/Rx desc 前/Tx buff 前/Rx buff 前 OK
//#define SECURITY_MEM_ADDR   0x5fffC000	//Tx desc 前/Rx desc 前/Tx buff 後/Rx buff 後 NG
//#define SECURITY_MEM_SIZE   0x3800000

extern int rtk_pcie_send_mem_rd(u32 addr, int size, u32 *data);
extern int rtk_pcie_send_mem_wr(u32 addr, int size, u32 data);
#endif
extern void rtk_pcie_dump_status(void);

//#define RTK_TV_DEBUG

#if 0   //CONFIG_PLATFORM_ARM_RTD299X
/* write/read MMIO register */
static u8 pci_readb(const volatile void __iomem *addr)
{
	u8 data;

	data = readb(addr);
#ifdef RTK_TV_DEBUG
	RTL8125_WARN("pci_readb(%08lx)=%02x\n", (u32) addr, (u8) data);
	rtk_pcie_dump_status();
#endif
	return (u8)data;
}

static u16	pci_readw(const volatile void __iomem *addr)
{
	u16 data;

	data = readw(addr);
#ifdef RTK_TV_DEBUG
	RTL8125_WARN("pci_readw(%08lx)=%04x\n", (u32) addr, (u16) data);
	rtk_pcie_dump_status();
#endif
	return (u16)data;
}

static u32	pci_readl(const volatile void __iomem *addr)
{
	u32 data;

	data = readl(addr);
#ifdef RTK_TV_DEBUG
	RTL8125_WARN("pci_readl(%08lx)=%08x\n", (u32) addr, (u32) data);
	rtk_pcie_dump_status();
#endif
	return (u32)data;
}

static void pci_writeb(u8 val, volatile void __iomem *addr)
{
	writeb(val, addr);
#ifdef RTK_TV_DEBUG
    RTL8125_WARN("pci_writeb(%08lx, %02x)\n", (u32) addr, (u8) val);
    rtk_pcie_dump_status();
#endif
}

static void pci_writew(u16 val, volatile void __iomem *addr)
{
	writew(val, addr);
#ifdef RTK_TV_DEBUG
	RTL8125_WARN("pci_writew(%08lx, %04x)\n", (u32) addr, (u16) val);
	rtk_pcie_dump_status();
#endif
}

static void pci_writel(u32 val, volatile void __iomem *addr)
{
	writel(val, addr);
#ifdef RTK_TV_DEBUG
	RTL8125_WARN("pci_writel(%08lx, %08x)\n", (u32) addr, (u32) val);
	rtk_pcie_dump_status();
#endif
}

#define RTL_W8(tp, reg, val8)	pci_writeb((val8),  tp->mmio_addr + reg)
#define RTL_W16(tp, reg, val16)	pci_writew((val16), tp->mmio_addr + reg)
#define RTL_W32(tp, reg, val32)	pci_writel((val32), tp->mmio_addr + reg)
#define RTL_R8(tp, reg)			pci_readb(tp->mmio_addr + reg)
#define RTL_R16(tp, reg)		pci_readw(tp->mmio_addr + reg)
#define RTL_R32(tp, reg)		pci_readl(tp->mmio_addr + reg)
...
#endif  //&& CONFIG_PLATFORM_ARM_RTD299X
...
#ifdef CONFIG_SECURITY_MEM
        struct secure_buffer Tx_databuff[NUM_TX_DESC];
#endif
        u32 NextHwDesCloPtr;
...
#ifdef CONFIG_SECURITY_MEM
        struct secure_buffer Rx_databuff[NUM_RX_DESC];
#endif
