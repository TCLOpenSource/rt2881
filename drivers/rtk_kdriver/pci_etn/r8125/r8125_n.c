...
#endif
//#define CONFIG_ENABLE_MSIX_INTERRUPT
...
static int proc_dump_desc(struct seq_file *m, void *v)
{
#if 0
        struct net_device *dev = m->private;
        struct rtl8125_private *tp = netdev_priv(dev);
    ...
    int i = 0;
    int pp= 0;
    pp=tp->tx_ring[0].cur_tx%NUM_TX_DESC;
    seq_printf(m, "cur_tx=%u\n",pp);
    pp=tp->rx_ring[0].cur_rx%NUM_RX_DESC;
    seq_printf(m, "cur_rx=%u\n",pp);
    seq_printf(m, "tp->tx_ring[0].TxDescArray = 0x%x\n", tp->tx_ring[0].TxDescArray);

    for(i=0;i<NUM_TX_DESC;i++)
    {
        txd = tp->tx_ring[0].TxDescArray+i;
        if(txd)
        seq_printf(m, "tx desc[%d] addr=0x%llx opt1=0x%x opt2=0x%x\n"
                ,i
                ,txd->addr
    ...
    {
        rxd = tp->rx_ring[0].RxDescArray+i;
        if(rxd)
        seq_printf(m, "rx desc[%d] addr=0x%llx opt1=0x%x opt2=0x%x\n"
                ,i
                ,rxd->addr
	...
	seq_printf(m,"StartAddr Low=%x High=%x\n", RTL_R32(tp, TxDescStartAddrLow),RTL_R32(tp, TxDescStartAddrHigh));
	seq_printf(m,"IntrStatus=%x xConfig=%x \n", RTL_R16(tp, IntrStatus),RTL_R16(tp, RxConfig));
...
static void rtl8125_get_mac_version(struct rtl8125_private *tp)
{
        ...
        u32 ICVerID;
        struct pci_dev *pdev = tp->pci_dev;
        pr_alert("[Alexkh]%s(%d)TxConfig=0x%08x\n",__func__,__LINE__,TxConfig);
        val32 = RTL_R32(tp, TxConfig);
        pr_alert("[Alexkh]%s(%d)val32=0x%08x\n",__func__,__LINE__,val32);
                   ...
                   void __iomem **ioaddr_out)
{
        ...
        pr_alert("%s(%d)[Alexkh] 2022/11/07 15:55\n",__func__,__LINE__);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
                if (netif_msg_probe(tp))
                        dev_err(&pdev->dev, "region #1 not an MMIO resource, aborting(Alexkh)\n");
#endif //LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
                rc = -ENODEV;
                pr_alert("%s(%d)err_out_mwi\n",__func__,__LINE__);
                goto err_out_mwi;
        }
        ...
        /* ioremap MMIO region */
        pr_alert("[Alexkh]%s(%d)pci_resource_start=0x%08x pci_resource_len=0x%08x\n",
            __func__,__LINE__,pci_resource_start(pdev, 2),pci_resource_len(pdev, 2));
#if 0   //def CONFIG_PLATFORM_ARM_RTD299X
        ioaddr = (void __iomem *)pci_resource_start(pdev, 2);
        pr_alert("[Alexkh]%s(%d)ioaddr=0x%08x\n",__func__,__LINE__,ioaddr);
#else
...
{
#ifndef CONFIG_ENABLE_MSIX_INTERRUPT
    pr_alert("[Alexkh]%s(%d)for msix error !!!\n",__func__,__LINE__);
    return -1;
#else
        int i, nvecs = 0;
...
static int rtl8125_try_msi(struct rtl8125_private *tp)
{
        ...
        if ((nvecs = rtl8125_enable_msix(tp)) > 0) {
            pr_alert("[Alexkh]%s(%d)MSIX enable\n",__func__,__LINE__);
                msi |= RTL_FEATURE_MSIX;
        } else if (!pci_enable_msi(pdev)) {
            pr_alert("[Alexkh]%s(%d)MSI enable\n",__func__,__LINE__);
                msi |= RTL_FEATURE_MSI;
        ...
        tp->features |= msi;
        pr_alert("[Alexkh]%s(%d)nvecs=%d\n",__func__,__LINE__,nvecs);
        return nvecs;
}
...
#elif LINUX_VERSION_CODE > KERNEL_VERSION(2,6,13)
#ifdef CONFIG_ENABLE_MSIX_INTERRUPT
        if (tp->features & (RTL_FEATURE_MSIX))
                 ...
                 const struct pci_device_id *ent)
{
    ...
    */
    if (0 == secure_memaddr) {
        RTL8125_WARN("secure_memaddr is not assigned.\n");
        WARN_ON(1);
        goto NO_DECLARE;
    }
    RTL8125_WARN("[Alexkh]!!!secure mode enable\n");
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
    if (dma_ret) {
        RTL8125_WARN("Failed to declare coherent memory for wifi device: ret = 0x%x, secure_memaddr = 0x%lx, size = %ld\n",
                        dma_ret, secure_memaddr, secure_size);
        return -ENOMEM;
    } else {
        RTL8125_WARN("Declare coherent memory ok for wifi device, ret = 0x%x, secure_memaddr = 0x%lx, secure_size = %ld, (&pdev->dev=%llx)\n",
                        dma_ret, secure_memaddr, secure_size, &pdev->dev);
    }
#else
    if (!dma_ret) {
        RTL8125_WARN("Failed to declare coherent memory for wifi device: ret = 0x%x, secure_memaddr = 0x%lx, secure_size = %ld\n",
                        dma_ret, secure_memaddr, secure_size);
        return -ENOMEM;
    } else {
        status = _SUCCESS;
        RTL8125_WARN("Declare coherent memory ok for wifi device, ret = 0x%x, secure_memaddr = 0x%lx\n, secure_size = %ld\n",
                        dma_ret, secure_memaddr, secure_size);
    }
#endif
NO_DECLARE:
                    ...
                    const u32 cur_rx)
{
        ...
        mapping = ring->Rx_databuff[cur_rx].phys_addr;       // force dma address to security buffer
#endif
...
#ifdef CONFIG_SECURITY_MEM
        mapping = ring->Rx_databuff[cur_rx].phys_addr;
#else
...
#ifdef CONFIG_SECURITY_MEM
                if (ring->Rx_databuff[i].virt_addr) {
                    dma_free_coherent(&pdev->dev,
                        PAGE_SIZE,
                        ring->Rx_databuff[i].virt_addr,
                        ring->Rx_databuff[i].phys_addr);
                    memset(&ring->Rx_databuff[i], 0, sizeof(ring->Rx_databuff[i]));
                 }
#endif
        }
}
...
#ifdef CONFIG_TX_SECURITY_MEM
static int rtl8125_tx_alloc_secure_buffer(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring)
{
        struct device *d = tp_to_dev(tp);
        unsigned int i;

        memset(ring->Tx_databuff, 0, sizeof(ring->Tx_databuff));

        for (i = 0; i < NUM_TX_DESC; i++) {
            ring->Tx_databuff[i].virt_addr = dma_alloc_coherent(d, PAGE_SIZE, &ring->Tx_databuff[i].phys_addr, GFP_KERNEL);
        }

        return 0;
}

static int rtl8125_rx_alloc_secure_buffer(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring)
{
        struct device *d = tp_to_dev(tp);
        unsigned int i;
        ...
        for (i = 0; i < NUM_RX_DESC; i++) {
            ring->Rx_databuff[i].virt_addr = dma_alloc_coherent(d, PAGE_SIZE, &ring->Rx_databuff[i].phys_addr, GFP_KERNEL);
        }
...
#ifdef CONFIG_TX_SECURITY_MEM
                rtl8125_tx_alloc_secure_buffer(tp, ring);
#endif
        }
...
#ifdef CONFIG_SECURITY_MEM
                rtl8125_rx_alloc_secure_buffer(tp, ring);
#endif
...
rtl8125_tx_clear(struct rtl8125_private *tp)
{
        ...
        for (i = 0; i < tp->num_tx_rings; i++) {
                ...
                {
                    if (ring->Tx_databuff[j].virt_addr == NULL)
                        continue;

                    dma_free_coherent(&pdev->dev,
                        PAGE_SIZE,
                        ring->Tx_databuff[j].virt_addr,
                        ring->Tx_databuff[j].phys_addr);
                }
        ...
        for (cur_frag = 0; cur_frag < nr_frags; cur_frag++) {
                ...
                if (tp->RequireLSOPatch  &&
                    ...
                    len > 1) {
                        ...
                        if ((entry >= NUM_TX_DESC) || (len > PAGE_SIZE) || ring->Tx_databuff[entry].phys_addr==NULL)
                        {
                            RTL8125_WARN("copy data to txsecury buffer[%d] failed virt=%08lx, phys=%08lx, len=%d\n",
                                entry,
                                ring->Tx_databuff[entry].virt_addr,
                                ring->Tx_databuff[entry].phys_addr,
                                len);
                            goto err_out;
                        }

                        memcpy(ring->Tx_databuff[entry].virt_addr, addr, len);
                        mapping = ring->Tx_databuff[entry].phys_addr;
#else
                   ...
                   struct net_device *dev)
{
        ...
        memcpy(ring->Tx_databuff[entry].virt_addr, skb->data, len);
        mapping = ring->Tx_databuff[entry].phys_addr;
#else
                    ...
                    int rx_buf_sz)
{
        ...
        if (pkt_size < rx_copybreak) {
                ...
                if (skb) {
                        ...
                        data = sk_buff[0]->data;
                        RTL8125_RX_DBG("rtl8125_try_rx_copy. data=%08x, sz=%d\n", (u32) data, pkt_size);
...
retry:
                ...
                RTL8125_RX_DBG("desc(%08lx)=(opts1=%08x, opts2=%08x, addr=%llx)\n",
                    desc, desc->opts1, desc->opts2, desc->addr);
                ...
                if (unlikely(rtl8125_check_rx_desc_error(dev, tp, status) < 0)) {
                        ...
                        RTL8125_RX_WARN("Rx ERROR. status = %08x (err_cnt=%d)\n", status, RTLDEV->stats.rx_errors);
...
process_pkt:
                        ...
                        if (unlikely(rtl8125_fragmented_frame(tp, status))) {
                                RTL8125_WARN("rtl8125_fragmented_frame, drop it\n");
                        ...
                        RTL8125_RX_DBG("desc->addr=%08lx/%08lx/%08lx, sz=%d  (entry=%d)\n",
                            (u32) le64_to_cpu(desc->addr),
                            rx_buf_phy_addr,
                            ring->Rx_databuff[entry].phys_addr,
                            pkt_size, entry);

                        memcpy(skb->data, ring->Rx_databuff[entry].virt_addr, tp->rx_buf_sz);   // copy data to skb data buffer
#else
                        rx_buf_phy_addr = ring->RxDescPhyAddr[entry];
