...
static bool is_target_meeting(void)
{
	unsigned int low_val = rtd_inl(0xb80380A4);
	unsigned int high_val = rtd_inl(0xb80380A8);
	if(((low_val & 0xFFFF0000) == 0x414d0000)
		&& ((high_val == 0x30323547)
		|| (high_val == 0x32373442))) {
		return  true;
	}
	else if(((low_val & 0xFFFF0000) == 0x344d0000)
		&& ((high_val == 0x32363748))) {
		return  true;
	}
	return false;
...
{
	if(portnum == 3 && page == 0x00
		&& reg == 0xE4 && otp_value == 0x00
		&& is_target_meeting())
		return false;
	else
		return true;
