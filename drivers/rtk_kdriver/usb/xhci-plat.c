...
static struct xhci_hcd *g_xhci_hcd = NULL;
...
__setup("usbdrd", usb_otg_switch_handler);
#else
static int parse_otg_switch_handler(void)
{
    char strings[16] = {0};

    if(rtk_parse_commandline_equal("usbdrd", strings,sizeof(strings)) == 0)
    {
...
static void xhci_check_disable_parkmode(struct xhci_plat_priv *priv)
{
	if(priv->disable_parkmode) {
		rtd_outl(priv->wrapper_regs.XHCI_USB3_TOP_GUCTL1_REG, 
			rtd_inl(priv->wrapper_regs.XHCI_USB3_TOP_GUCTL1_REG) | (0x7 << 15));
	} else {
		rtd_outl(priv->wrapper_regs.XHCI_USB3_TOP_GUCTL1_REG, 
			rtd_inl(priv->wrapper_regs.XHCI_USB3_TOP_GUCTL1_REG) & (~(0x7 << 15)));
	}
...
static int xhci_plat_probe(struct platform_device *pdev)
{
	 ...
	 if (device_property_read_bool(&pdev->dev, "disable_parkmode"))
                hcd_to_xhci_priv(hcd)->disable_parkmode = true;
...
#endif
        xhci_check_disable_parkmode(hcd_to_xhci_priv(hcd));
        g_xhci_hcd = xhci;
        return 0;
...
void xhci_plat_shutdown(struct platform_device *pdev)
{
...
/***********************************just for panic dump****************************************************/
void xhci_dump_event_ring_info(struct xhci_hcd *xhci)
{
	int i = 0;
	xhci_plat_err("USBSTS:cmd:%08x, status:%08x\n", 
		readl(&xhci->op_regs->status),
		readl(&xhci->op_regs->command));	
	xhci_plat_err("deq_seg:%lx,dequeue:%lx,enq_seg:%lx,enqueue:%lx\n", 
		(unsigned long)xhci->event_ring->deq_seg,
		(unsigned long)xhci->event_ring->dequeue, 
		(unsigned long)xhci->event_ring->enq_seg,
		(unsigned long)xhci->event_ring->enqueue);
	xhci_plat_err("irq_pending:%x,irq_control:%x,erst_size:%x, erst_base:%llx, erst_dequeue:%llx\n", 
		readl(&xhci->ir_set->irq_pending),
		readl(&xhci->ir_set->irq_control), 
		readl(&xhci->ir_set->erst_size),
		le64_to_cpu(xhci_read_64(xhci, &xhci->ir_set->erst_base)),
		le64_to_cpu(xhci_read_64(xhci, &xhci->ir_set->erst_dequeue)));

	for(i = 0; i < 256; i++) {
		xhci_plat_err("event_ring_trb[%d]: %08x,%08x, %08x,%08x\n", i,
			le32_to_cpu(xhci->event_ring->deq_seg->trbs[i].generic.field[0]),
			le32_to_cpu(xhci->event_ring->deq_seg->trbs[i].generic.field[1]),
			le32_to_cpu(xhci->event_ring->deq_seg->trbs[i].generic.field[2]),
			le32_to_cpu(xhci->event_ring->deq_seg->trbs[i].generic.field[3]));
	}
}

static void xhci_dump_ep_ring(struct xhci_hcd *xhci, struct xhci_ring *ep_ring)
{
	struct xhci_segment	*xhci_seg = NULL;
	struct xhci_segment	*first_seg = NULL;
	...
	while(xhci_seg) {
		int i = 0;
		xhci_plat_err("seg_dma = %lx\n", xhci_seg->dma);
		for(i = 0; i < 256; i++)
			xhci_plat_err("trb[%d:%llx]: %08x, %08x,%08x, %08x\n", i,
			xhci_seg->dma + i * 16,
			le32_to_cpu(xhci_seg->trbs[i].generic.field[0]),
			le32_to_cpu(xhci_seg->trbs[i].generic.field[1]),
			le32_to_cpu(xhci_seg->trbs[i].generic.field[2]),
			le32_to_cpu(xhci_seg->trbs[i].generic.field[3]));
		xhci_seg = xhci_seg->next;
		if(xhci_seg == first_seg)
			break;
	}
}

static struct xhci_ring *xhci_dma_to_transfer_ring_ex(
...
struct xhci_ep_ctx *xhci_get_ep_ctx_ex(struct xhci_hcd *xhci,
...
void xhci_dump_all_ep_rings(struct xhci_hcd *xhci,  int slot_id)
{
	int n = 0;
	...
	udev = xdev->udev;
	if(udev)
		xhci_plat_err("[slot's usb device]:vid=%x,pid=%x,speed=%d\n", udev->descriptor.idVendor, 
			udev->descriptor.idProduct, udev->speed);
	for(n = 0; n < EP_CTX_PER_DEV; n++) {
		struct xhci_ring *ep_ring = NULL;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0))
		struct xhci_ep_ctx *ep_ctx = xhci_get_ep_ctx_ex(xhci, xdev->out_ctx, n);
#else
		struct xhci_ep_ctx *ep_ctx = xhci_get_ep_ctx(xhci, xdev->out_ctx, n);
#endif
		ep_ring = xhci_dma_to_transfer_ring_ex(&xdev->eps[n], le64_to_cpu(ep_ctx->deq));
		if(ep_ring) {
			xhci_plat_err("******************dump ep_ring %d start*********************\n", n);
			xhci_plat_err("ep[%d] ep_info:%08x, ep_info2:%08x, deq:%llx, tx_info:%08x,cyc_state= %d\n", n,
			le32_to_cpu(ep_ctx->ep_info),
			le32_to_cpu(ep_ctx->ep_info2),
			le64_to_cpu(ep_ctx->deq),
			le32_to_cpu(ep_ctx->tx_info),
			ep_ring->cycle_state);
			xhci_dump_ep_ring(xhci, ep_ring);
			xhci_plat_err("******************dump ep_ring %d end*********************\n", n);
		}
	}
}


static void _rtk_xhci_dump_xhci_hcd_info(struct xhci_hcd *xhci)
{
	unsigned long flags;
	int i = 0;
	xhci_plat_err("***********dump xhci hcd information start***********\n");
	local_irq_save(flags);
	xhci_dump_event_ring_info(xhci);
	for (i = 0; i < MAX_HC_SLOTS; i++) {
		if (!xhci->devs[i])
			continue;
		xhci_plat_err("******************dump slot %d start*********************\n", i);
		xhci_dump_all_ep_rings(xhci, i);
		xhci_plat_err("******************dump slot %d end*********************\n", i);
	}
	local_irq_restore(flags);
	xhci_plat_err("***********dump ehci hcd information stop***********\n");
}

unsigned char g_enable_xhci_panic_dump = 0;
void rtk_xhci_debug_dump_xhci_information(void)
{
	if(g_enable_xhci_panic_dump) {
		if(g_xhci_hcd)
			_rtk_xhci_dump_xhci_hcd_info(g_xhci_hcd);
	}
}
EXPORT_SYMBOL(rtk_xhci_debug_dump_xhci_information);