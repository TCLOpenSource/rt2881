...
struct EHCI_PORT_Z0_TUNING_INFO 
{
	int id;
	unsigned int port;
	unsigned int z0_value;
	
} ehci_port_z0_tuning_info = {
	-1,
	0,
	0xffffffff
};
void ehci_usb2_save_z0_tuning_value(struct usb_hcd *hcd, unsigned int  port, unsigned int z0_value)
{
	...
	ehci_port_z0_tuning_info.id = priv->id;
	ehci_port_z0_tuning_info.port = port;
	ehci_port_z0_tuning_info.z0_value = z0_value;
}
EXPORT_SYMBOL(ehci_usb2_save_z0_tuning_value);
...
static DEFINE_MUTEX(g_ehci_phy_set_lock);
        ...
        unsigned char       val)
{
		...
		mutex_lock(&g_ehci_phy_set_lock);
        ret = _set_ehci_usb2_phy(ehci, port1, 0xF4, (page<<5) | 0x9b);  // bit [6:5] : page select = page
        if (ret)
			goto EXIT;

        ret =  _set_ehci_usb2_phy(ehci, port1, addr, val);
EXIT:	
		mutex_unlock(&g_ehci_phy_set_lock);	
		return ret;
}
EXPORT_SYMBOL(set_ehci_usb2_phy_reg);
        ...
        unsigned char       val)
{
	...
	return set_ehci_usb2_phy_reg(ehci, port1, page, addr, val);
}
EXPORT_SYMBOL(ehci_hcd_set_phy_reg);
...
EXPORT_SYMBOL(get_ehci_usb2_phy_reg);
...
{
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	return get_ehci_usb2_phy_reg(ehci, port1, page, addr);
}
EXPORT_SYMBOL(ehci_hcd_get_phy_reg);
        ...
        for(i  = 0; i < otp_val_array_size; i++)
        {
...

                {
                        ...
                        for (portnum = 0; portnum < priv->usb2_port_cnt; portnum++)
                        {
                                ...
                                if(!ehci_phy_otp_apply_check(pdev, portnum + 1, page, reg, tmp_otp))
                                    continue;
                                /*
...
static void ehci_specail_z0_set_flow(struct usb_hcd  *hcd_priv, int port_num, unsigned char z0_value)
{
	if(port_num == 1) {
		unsigned int mask = ((1 << EHCI_PORT_Z0_BIT_NUM) - 1);
		unsigned int val = ehci_hcd_get_phy_reg(hcd_priv,  
								port_num, EHCI_PORT_Z0_PAGE, EHCI_PORT_Z0_REG);
		val = (val & ~(mask << EHCI_PORT_Z0_BIT_OFFSET)) 
				  			| ((z0_value & mask) << EHCI_PORT_Z0_BIT_OFFSET);
		ehci_hcd_set_phy_reg(hcd_priv,  
								port_num, EHCI_PORT_Z0_PAGE, EHCI_PORT_Z0_REG, val);
	} else if(port_num == 2) {
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE3, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE3) & (~(1 << 6))) | ((z0_value & 0x1) << 6));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE1, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE1) & (~(1 << 3))) | ((z0_value & 0x1) << 3));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE3, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE3) & (~(1 << 7))) | ((z0_value & 0x1) << 7));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE2, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE2) & (~(1 << 0))) | ((z0_value & 0x1) << 0));
	} else{
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE2, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE2) & (~(1 << 1))) | ((z0_value & 0x1) << 1));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE1, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE1) & (~(1 << 4))) | ((z0_value & 0x1) << 4));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE2, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE2) & (~(1 << 2))) | ((z0_value & 0x1) << 2));
		z0_value = z0_value >> 1;
		ehci_hcd_set_phy_reg(hcd_priv, 2, 2,  0xE2, 
			(ehci_hcd_get_phy_reg(hcd_priv,  2, 2, 0xE2) & (~(1 << 3))) | ((z0_value & 0x1) << 3));		
	}
}
#endif
...
int ehci_usb2_phy_init(struct platform_device *pdev)
{
	 ...
	 if(priv->id == ehci_port_z0_tuning_info.id 
	 	&& ehci_port_z0_tuning_info.port != 0
	 	&& ehci_port_z0_tuning_info.z0_value != 0xFFFFFFFF) {
#ifdef EHCI_PORT_Z0_SPECAIL_SETTING
		ehci_specail_z0_set_flow(hcd, ehci_port_z0_tuning_info.port, ehci_port_z0_tuning_info.z0_value);
#else	 	
		unsigned int mask = ((1 << EHCI_PORT_Z0_BIT_NUM) - 1);
		unsigned int val = ehci_hcd_get_phy_reg(hcd,  
								ehci_port_z0_tuning_info.port, EHCI_PORT_Z0_PAGE, EHCI_PORT_Z0_REG);
		val = (val & ~(mask << EHCI_PORT_Z0_BIT_OFFSET)) 
				  			| ((ehci_port_z0_tuning_info.z0_value & mask) << EHCI_PORT_Z0_BIT_OFFSET);
		ehci_hcd_set_phy_reg(hcd, ehci_port_z0_tuning_info.port, EHCI_PORT_Z0_PAGE, EHCI_PORT_Z0_REG, val);
#ifdef EHCI_PORT_Z0_MASTER_PORT
		ehci_hcd_set_phy_reg(hcd, EHCI_PORT_Z0_MASTER_PORT, EHCI_PORT_Z0_PAGE, EHCI_PORT_Z0_REG, val);
#endif
#endif
		ehci_plat_info("[WIFI] port[%u] init tuning  z0 to %x\n", ehci_port_z0_tuning_info.port, ehci_port_z0_tuning_info.z0_value);
