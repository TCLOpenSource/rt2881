...
struct usb_hcd * ehci_find_ehci_hcd_by_reg_start(unsigned int reg_start, unsigned int reg_mask)
{
        ...
        list_for_each_entry_safe(priv, next, &ehci_list_head, ehci_list)
        {
                if((priv->reg_start & reg_mask) == (reg_start & reg_mask)) {
			if(priv->ehci)
				return ehci_to_hcd(priv->ehci);
                }
        }	
        return NULL;
}
EXPORT_SYMBOL(ehci_find_ehci_hcd_by_reg_start);


void ehci_hcd_switch_port_owner(struct usb_hcd *hcd, unsigned int port_num, bool switch_to_ehci)
{
	struct ehci_hcd *ehci = NULL;
	struct ehci_platform_priv *priv = NULL;
	unsigned int val = 0;
	if(!hcd)
		return ;
	ehci = hcd_to_ehci(hcd);
	priv = ehci_to_ehci_priv(ehci);
	if(port_num == 0 || port_num > priv->usb2_port_cnt)
		return;
	val = ehci_readl(ehci, &ehci->regs->port_status[port_num -1]);
	if(switch_to_ehci)
		val = val & (~PORT_OWNER);
	else
		val = val | (PORT_OWNER);
	ehci_writel(ehci, val, &ehci->regs->port_status[port_num -1]);
}
EXPORT_SYMBOL(ehci_hcd_switch_port_owner);
...
static int ehci_platform_probe(struct platform_device *dev)
{
	...
	if(platform_id == 0)
        ...
        */
        ...
        err = ehci_register_usb_bus_nb(dev);
        if(err) {
                goto err_register_usb_bus_nb;
        }

        err = ehci_register_thermal_callback(dev);
        if(err) {
                goto err_register_thermal_callback;
        }

	 err = ehci_post_init_callback(dev);
	 if(err) {
                goto err_post_init;
        }
        ...
        return err;
err_post_init:
	ehci_unregister_thermal_callback(dev);
err_register_thermal_callback:
        ehci_unregister_usb_bus_nb(dev);
err_register_usb_bus_nb:
err_power:
        if (pdata->power_off)
                pdata->power_off(dev);
err_reset:
...
static int ehci_platform_suspend(struct device *dev)
{
        ...
        int ret;
	 ehci_pre_suspend_callback(pdev);
...
static int ehci_platform_resume(struct device *dev)
{
        ...
        ehci_post_resume_callback(pdev);
        return 0;
...
void ehci_plat_shutdown(struct platform_device *pdev)
{
	struct usb_hcd  *hcd = dev_get_drvdata(&pdev->dev);
	//struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	ehci_plat_info("%s start\n", __FUNCTION__);
	ehci_pre_exit_callback(pdev);
...
static void dump_qh_hw( struct ehci_hcd *ehci,struct ehci_qh *qh){
	struct ehci_qh_hw   *hw = qh->hw;
	struct usb_device *udev = NULL;
	struct usb_host_endpoint *ep = NULL;
	if(!hw)
		return;
	ehci_plat_err("[qh(0x%p)] qh_dma=0x%08x\n"
			 "    -- next=0x%08x info1=0x%08x info2=0x%08x\n"
			 "    -- current=0x%08x q_next=0x%08x a_next=0x%08x token=0x%08x\n"
			 "    -- buf  : 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n"
			 "    -- buf_h: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n",
		qh,qh->qh_dma,
		hc32_to_cpu(ehci, hw->hw_next),
		hc32_to_cpu(ehci, hw->hw_info1),
		hc32_to_cpu(ehci, hw->hw_info2),
		hc32_to_cpu(ehci, hw->hw_current),
		hc32_to_cpu(ehci, hw->hw_qtd_next),
		hc32_to_cpu(ehci, hw->hw_alt_next),
		hc32_to_cpu(ehci, hw->hw_token),
		hc32_to_cpu(ehci, hw->hw_buf[0]),hc32_to_cpu(ehci, hw->hw_buf[1]),
		hc32_to_cpu(ehci, hw->hw_buf[2]),hc32_to_cpu(ehci, hw->hw_buf[3]),hc32_to_cpu(ehci, hw->hw_buf[4]),
		hc32_to_cpu(ehci, hw->hw_buf_hi[0]),hc32_to_cpu(ehci, hw->hw_buf_hi[1]),
		hc32_to_cpu(ehci, hw->hw_buf_hi[2]),hc32_to_cpu(ehci, hw->hw_buf_hi[3]),hc32_to_cpu(ehci, hw->hw_buf_hi[4]));

	udev = qh->ps.udev;
	if(udev) {
		ehci_plat_err("[qh's usb device]:vid=%x,pid=%x,speed=%d\n", udev->descriptor.idVendor, 
			udev->descriptor.idProduct, udev->speed);
	}
		
	ep = qh->ps.ep;
	if(ep) {
		ehci_plat_err("[qh's usb ep]:ep_addr=%x,attrs=%x\n", ep->desc.bEndpointAddress, ep->desc.bmAttributes);
	}
	

}

static void dump_qtd_hw( struct ehci_hcd *ehci,struct ehci_qtd	*qtd){

	ehci_plat_err("[qtd(0x%p)]dma=0x%08x len=%d\n"
			 "    -- next=0x%08x a-next=0x%08x token=0x%08x\n"
			 "    -- buf  : 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n"
			 "    -- buf_h: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n",
		qtd,
		(unsigned int)qtd->qtd_dma,
		(unsigned int)qtd->length,
		hc32_to_cpu(ehci, qtd->hw_next),
		hc32_to_cpu(ehci, qtd->hw_alt_next),
		hc32_to_cpu(ehci, qtd->hw_token),
		hc32_to_cpu(ehci, qtd->hw_buf[0]),hc32_to_cpu(ehci, qtd->hw_buf[1]),
		hc32_to_cpu(ehci, qtd->hw_buf[2]),hc32_to_cpu(ehci, qtd->hw_buf[3]),hc32_to_cpu(ehci, qtd->hw_buf[4]),
		hc32_to_cpu(ehci, qtd->hw_buf_hi[0]),hc32_to_cpu(ehci, qtd->hw_buf_hi[1]),
		hc32_to_cpu(ehci, qtd->hw_buf_hi[2]),hc32_to_cpu(ehci, qtd->hw_buf_hi[3]),hc32_to_cpu(ehci, qtd->hw_buf_hi[4]));
}



static void _rtk_ehci_dump_ehci_hcd_info(struct ehci_hcd *ehci)
{
	unsigned long flags;
	struct ehci_qh *qh_scan_next;
	struct ehci_qh *qh;
	ehci_plat_err("***********dump ehci hcd information start***********\n");
	...
	while (qh_scan_next) {
		...
		if (!list_empty(&qh->qtd_list)) {
			...
			list_for_each_safe (entry, tmp, &qh->qtd_list) {
				...
				dump_qtd_hw(ehci, qtd);
			}
		}
	}
EXIT:	
	local_irq_restore(flags);
	ehci_plat_err("***********dump ehci hcd information stop***********\n");
}

unsigned char g_enable_ehci_panic_dump = 0;
void rtk_ehci_debug_dump_ehci_information(void)
{
	if(g_enable_ehci_panic_dump) {
		struct ehci_platform_priv *priv, *next;

		list_for_each_entry_safe(priv, next, &ehci_list_head, ehci_list) {
			if(priv->ehci)
				_rtk_ehci_dump_ehci_hcd_info(priv->ehci);
		}
	}
}
EXPORT_SYMBOL(rtk_ehci_debug_dump_ehci_information);


static void _rtk_ehci_trigger_ehci_hcd_panic(struct ehci_hcd *ehci)
{
	unsigned long flags;
	struct ehci_qh *qh_scan_next;
	struct ehci_qh *qh;
	ehci_plat_err("***********trigger ehci hcd information start***********\n");
	local_irq_save(flags);
	if(!ehci->async)
		goto EXIT;
	qh_scan_next = ehci->async->qh_next.qh;
	if (qh_scan_next) {
		...
		qh_scan_next = qh->qh_next.qh;
		if(qh->hw)
			qh->hw->hw_next = 0xAAAAAAA0;
	}
EXIT:	
	local_irq_restore(flags);
	ehci_plat_err("***********trigger ehci hcd information stop***********\n");
}


void rtk_ehci_trigger_ehci_panic(void)
{
	struct ehci_platform_priv *priv, *next;

	list_for_each_entry_safe(priv, next, &ehci_list_head, ehci_list) {
			if(priv->ehci)
				_rtk_ehci_trigger_ehci_hcd_panic(priv->ehci);
	}
