...
extern unsigned char g_enable_ehci_panic_dump;
...
static ssize_t show_debug_control(struct device *dev, struct device_attribute *attr, char *buf)
{
        ...
        if ((count -= n) < 0)
                goto overflow;
        n = scnprintf(ptr, count, "panic_dump=%u\n", g_enable_ehci_panic_dump);
        ptr += n;
        if ((count -= n) < 0)
                goto overflow;		
...
static ssize_t store_debug_control(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	 ...
	 else if (sscanf(buf, "panic_dump=%x", &val) == 1)
        {
                g_enable_ehci_panic_dump = !!val;
        }
	 else if (sscanf(buf, "panic_test=%x", &val) == 1)
        {
                if(val)
			rtk_ehci_trigger_ehci_panic();		
        }
	 else if (sscanf(buf, "ehci_dump=%x", &val) == 1)
        {
        	   g_enable_ehci_panic_dump = 1;
