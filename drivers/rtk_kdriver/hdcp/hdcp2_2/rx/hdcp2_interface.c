...
extern unsigned char customer_hdcp2p2;
...
H2status hdcp2_Rx_SetEKpubKm(const H2uint8 *EKpubKm)
{
	...
	H2uint8 *KPriv_tmp = NULL;//[KPRIVRX_SIZE];
	H2status rc;
	...
	KPriv_tmp = (H2uint8 *)kmalloc(KPRIVRX_SIZE, GFP_KERNEL);
	if(!KPriv_tmp)
		return H2_ERROR;

  	memset(KPriv_tmp, 0, KPRIVRX_SIZE);
	    /* Decrypt Km */
	...
	RCP_HDCP2_GenKd(Km_tmp, rtx, rrx, rn, Kd_tmp, 1);
...
H2status hdcp2_Rx_SetEKhKm(H2uint8 *EKhKm, H2uint8 *m)
{
	...
	hdcp2_Rx_GetrTx(rtx, RTX_SIZE);
	hdcp2_Rx_GetrRx(rrx, RRX_SIZE);
	RCP_HDCP2_GenKd(Km_temp, rtx, rrx, rn, Kd_temp, 1);
...
H2status hdcp2_Rx_SetEdKeyKs(H2uint8 *EdKeyKs)
{
    ...
    /** Compute dKey2 */
	RCP_HDCP2_GenDKey(km, rtx, rrx, rn, 2, dkey2, 1);
...
//	spu_print(dkey2, 16);
    ...
    /** Compute Session Key */
	RCP_HDCP2_GenKs(dkey2, EdKeyKs, rrx, ks);
...
//	spu_print(ks, 16);
	...
	//RCP_HDCP2_GenKsXorLc128(lc128, ks, ks);
	RCP_HDCP2_GenKsXorLc128(lc128, ks, ks, LC128_SIZE);
	spu_SetKsXorLc128(ks);
	    /*
...
H2status hdcp2_Rx_Compute_EdKeyKs(H2uint8 *rtx, H2uint8 *rrx, H2uint8 *rn, H2uint8 *EdKeyKs, int modeHDCP22)
{
	...
	unsigned char km[KM_SIZE];
	spu_GetKM(km);
	RCP_HDCP2_GenDKey(km, rtx, rrx, rn, 2, dkey2, modeHDCP22);
	//RTK_RCP_GET_CW_HDCP2(SRAM_DK2_ENTRY , dkey2, DKEY_SIZE);
	spu_SetDKey2(dkey2);
...
H2status hdcp2_load_from_flash(void)
{
	...
	else if(customer_hdcp2p2) //hdmi hdcp2 key from ap
	{
		hdcp2_load_from_ap();
	}
