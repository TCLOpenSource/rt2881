...
#define DBG_PRINT(s, args...) rtd_pr_hdcp_debug(s, ## args)
...
int rtk_hal_hdcp2_WriteKeySet(unsigned char *pKeySet, unsigned char *dst_file_path, int keyLength)
{
	...
	{
		rtd_pr_hdcp_debug("hdcp2:rtk_hal_hdcp2_WriteKeySet input paramter is error!\n");
		return NOT_OK;
	}
	if(keyLength != KEYSET_SIZE)
	{
		rtd_pr_hdcp_debug("hdcp2:rtk_hal_hdcp2_WriteKeySet keylength is error!\n");
		return NOT_OK;
	}
...
int rtk_hal_hdcp2_GetEncHdcp2Key(unsigned char *pEncKey, unsigned char *src_file_path, unsigned char *keyLen)
{
	...
	{
		rtd_pr_hdcp_debug("hdcp2:rtk_hal_hdcp2_GetEncHdcp2Key input paramter is error!\n");
		return NOT_OK;
	}
...
int rtk_hal_hdcp2_Kd_Key_Derivation(unsigned char *pRtx, unsigned char *pRrx, unsigned int version)
{
	...
	pr_debug("version = %d\n", modeHDCP22);
#endif
	RCP_HDCP2_GenKd(km, pRtx, pRrx, rn, kd, 1);
...
int rtk_hal_hdcp2_SHA1(unsigned char * pOutputBuf, unsigned char *pInputBuf, unsigned int length)
{
#ifndef CONFIG_OPTEE_HDCP2
	H2status rc;
...
int rtk_hal_hdcp2_SHA256(unsigned char * pOutputBuf, unsigned char *pInputBuf, unsigned int length)
{
#ifndef CONFIG_OPTEE_HDCP2
	H2status rc;
...
int rtk_hal_hdcp2_HMAC_SHA256(unsigned char * pOutputBuf, unsigned char *pInputBuf, unsigned int length, unsigned char *pKey, unsigned int keylength)
{
#ifndef CONFIG_OPTEE_HDCP2
...
int rtk_hal_hdcp2_DRM_HDMI_Encrypto_key(unsigned char *pdata, unsigned char *pEnckey, unsigned int length)
{
	...
	{
		rtd_pr_hdcp_emerg("hdcp2:rtk_hal_hdcp2_DRM_HDMI_Encrypto_key input paramter is error!\n");
		return NOT_OK;
	}
...
int rtk_hal_hdcp2_DRM_HDMI_Decrypto_key(unsigned char *pEnckey, unsigned char *pDeckey, unsigned int length)
{
	...
	{
		rtd_pr_hdcp_emerg("hdcp2:rtk_hal_hdcp2_DRM_HDMI_Decrypto_key input paramter is error!\n");
		return NOT_OK;
	}
...
int rtk_hal_hdcp2_VFE_HDMI_WriteHDCP22(unsigned char *pdata, unsigned int size)
{
	...
	{
		rtd_pr_hdcp_emerg("hdcp2:rtk_hal_hdcp2_VFE_HDMI_WriteHDCP22 input paramter is error!\n");
		return NOT_OK;
	}
	if(size % 16)
	{
...
void rtk_hdcp2_gen_ksxorlc128(unsigned char *data)
{
	...
	if(data == NULL)
	{
		rtd_pr_hdcp_emerg("rtk_hdcp2_gen_ksxorlc128 fail\n");
		return;
	}
...
void rtk_hdcp2_get_ksxorlc128(unsigned char *data, unsigned int size)
{
	...
	{
		rtd_pr_hdcp_emerg("rtk_hdcp2_get_ksxorlc128 fail\n");
		return;
	}
...
long hdcp2_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case HDCP2_IOC_DECRYPT_RSAES_OAEP:
		{
			...
			{
				retval = -EFAULT;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_DECRYPT_RSAES_OAEP copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				...
				{
					retval = -EFAULT;
					rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_DECRYPT_RSAES_OAEP pEkpubkm error!!!\n");
				}
			}
			break;
		}
		...
		case HDCP2_IOC_HDCP_CertificateKey:
		{
			HDCP_CERT_KEY_PARA_T hdcp_cert_key;
			UINT8 *dataBuf=NULL;
			/*
				kdriver_fail = 0
				kdriver_ok = 1
				optee_ERR_KEYTYPE = -1,
				optee_STORAGE_ERR_TEE = -2,
				optee_STORAGE_ERR_GENERAL = -3,
				optee_STORAGE_ERR_NO_KEY = -4,
				optee_STORAGE_ERR_CERTIFICATE_FAIL = -5,
			*/
			if(copy_from_user((void *)&hdcp_cert_key, (const void __user *)arg, sizeof(HDCP_CERT_KEY_PARA_T)))
			{
				retval = HDCP_OPTEE_CERT_NO_DEF;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_CERT_HDCPKEY copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				if(hdcp_cert_key.dataLen>0){
					dataBuf = kmalloc(hdcp_cert_key.dataLen, GFP_KERNEL);
					if(dataBuf == NULL) {
						rtd_pr_hdcp_emerg("hdcp2 cert dataBuf kmalloc failed!!!!!!!!!!!!!!!\n");
						retval = HDCP_OPTEE_CERT_NO_DEF;
						break;
					}
					if(copy_from_user((void *)dataBuf, (const void __user *)to_user_ptr(hdcp_cert_key.dataBuf), hdcp_cert_key.dataLen))
					{
						retval = HDCP_OPTEE_CERT_NO_DEF;
						rtd_pr_hdcp_emerg("hdcp2 cert ioctl code=dataBuf copy_from_user failed!!!!!!!!!!!!!!!\n");
						if(dataBuf)
							kfree(dataBuf);
						dataBuf = NULL;
					}
				...
				if(hdcp_cert_key.keyType == 2){
					retval = HDCP_OPTEE_CERT_ERR_TEE; //KADP_STORAGE_ERR_TEE = -2,RHAL_TRUSTZONE_KEY_CERTIFICATE_NOT_SUPPORT
				}else{
#ifdef CONFIG_OPTEE_HDCP14
					retval = optee_hdcp14_cert_key(dataBuf,hdcp_cert_key.dataLen, NULL, 0);
#else
					retval = HDCP_OPTEE_CERT_ERR_TEE; //KADP_STORAGE_ERR_TEE = -2,RHAL_TRUSTZONE_KEY_CERTIFICATE_NOT_SUPPORT
#endif
				}
	...
	switch (cmd)
	{
		...
		case HDCP2_IOC_DRM_HDMI_SetSecureDataforHDCP22:
		{
			...
			else
			{
				...
				rtd_pr_hdcp_emerg("encropty before:[%x,%x,%x,%x]\n",pData[0],pData[1],pData[2],pData[3]);
				rtk_hal_hdcp2_DRM_HDMI_Encrypto_key(pData, pEncKey, DRM_HDMI_SetSecureDataforHDCP22.Datalength);	
				rtd_pr_hdcp_emerg("encropty after:[%x,%x,%x,%x]\n",pEncKey[0],pEncKey[1],pEncKey[2],pEncKey[3]);
				if(copy_to_user((void __user *)to_user_ptr(DRM_HDMI_SetSecureDataforHDCP22.pEncKey), (void *) pEncKey, DRM_HDMI_SetSecureDataforHDCP22.Datalength))
    			{
       				retval = -EFAULT;
        			DBG_PRINT("scaler hdcp2 ioctl code=pEncKey copy_from_user failed!!!!!!!!!!!!!!!\n");
		...
		case HDCP2_IOC_DRM_VerifyHDCP22forHDMI:
		{
			...
			else
			{
				...
				rtd_pr_hdcp_emerg("decropty before:[%x,%x,%x,%x]\n",pEncKey[0],pEncKey[1],pEncKey[2],pEncKey[3]);
				rtk_hal_hdcp2_DRM_HDMI_Decrypto_key(pEncKey, pDecKey, DRM_VerifyHDCP22forHDMI.length);	
				rtd_pr_hdcp_emerg("decropty after:[%x,%x,%x,%x]\n",pDecKey[0],pDecKey[1],pDecKey[2],pDecKey[3]);
				if(copy_to_user((void __user *)to_user_ptr(DRM_VerifyHDCP22forHDMI.pDecKey), (void *) pDecKey, DRM_VerifyHDCP22forHDMI.length))
    			{
       				retval = -EFAULT;
        			DBG_PRINT("scaler hdcp2 ioctl code=pDecKey copy_from_user failed!!!!!!!!!!!!!!!\n");
		...
		case HDCP2_IOC_HDCP_CertificateKey:
		{
			...
			*/
			...
			{
				retval = HDCP_OPTEE_CERT_NO_DEF;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_CERT_HDCPTXKEY copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				if(hdcp_cert_key.dataLen>0){
				...
				if(hdcp_cert_key.keyType == 2){
					retval = optee_hdcp2_cert_key(dataBuf, hdcp_cert_key.dataLen, NULL, 0);
				}else{
#ifdef CONFIG_OPTEE_HDCP14
					retval = optee_hdcp14_cert_key(dataBuf,hdcp_cert_key.dataLen, NULL, 0);
#else
		...
		case HDCP2_IOC_HDCP_CertificateTxKey:
		{
			HDCP_CERT_KEY_PARA_T hdcp_cert_key;
			UINT8 *dataBuf=NULL;
			/*
			...
			*/
			...
			{
				retval = HDCP_OPTEE_CERT_NO_DEF;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_HDCP_CertificateTxKey copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				if(hdcp_cert_key.dataLen>0){
				...
				if(hdcp_cert_key.keyType == 2){
					retval = optee_hdcp2_cert_tx_key(dataBuf, hdcp_cert_key.dataLen, NULL, 0);
				}else{
#ifdef CONFIG_OPTEE_HDCP14
					retval = optee_hdcp14_cert_tx_key(dataBuf,hdcp_cert_key.dataLen, NULL, 0);
#else
					retval = HDCP_OPTEE_CERT_ERR_TEE; //KADP_STORAGE_ERR_TEE = -2,RHAL_TRUSTZONE_KEY_CERTIFICATE_NOT_SUPPORT
#endif
				}
		...
		case HDCP2_IOC_HDCP_DeltTxKey:
		{
			UINT32 hdcp_tx_keytype;
			if(copy_from_user((void *)&hdcp_tx_keytype, (const void __user *)arg, sizeof(UINT32)))
			{
				retval = -EFAULT;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_HDCP_DeltTxKey copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				if(hdcp_tx_keytype == 2){
					retval = optee_hdcp2_del_tx_key();
				}else{
#ifdef CONFIG_OPTEE_HDCP14
					retval = optee_hdcp14_del_tx_key();
#else
					retval = -EFAULT; //KADP_STORAGE_ERR_TEE = -2,RHAL_TRUSTZONE_KEY_CERTIFICATE_NOT_SUPPORT
#endif
				}
			}
			break;
		}
		case HDCP2_IOC_HDCP_ApplyTxKey:
		{
			UINT32 hdcp_tx_keytype;
			if(copy_from_user((void *)&hdcp_tx_keytype, (const void __user *)arg, sizeof(UINT32)))
			{
				retval = -EFAULT;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_HDCP_ApplyTxKey copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
#if IS_ENABLED(CONFIG_HDCPTX)
		...
		case HDCP2_IOC_HDCP_SetTxKey:
		{
			HDCP_TX_KEY_PARA_T hdcp_tx_key;
			UINT8 *dataBuf=NULL;

			if(copy_from_user((void *)&hdcp_tx_key, (const void __user *)arg, sizeof(HDCP_TX_KEY_PARA_T)))
			{
				retval = -EFAULT;
				rtd_pr_hdcp_emerg("scaler hdcp2 ioctl code=HDCP2_IOC_HDCP_SetTxKey copy_from_user failed!!!!!!!!!!!!!!!\n");
			}
			else
			{
				if(hdcp_tx_key.dataLen>0){
					dataBuf = kmalloc(hdcp_tx_key.dataLen, GFP_KERNEL);
					if(dataBuf == NULL) {
						rtd_pr_hdcp_emerg("hdcp2 set key dataBuf kmalloc failed!!!!!!!!!!!!!!!\n");
						retval = HDCP_OPTEE_CERT_NO_DEF;
						break;
					}
					if(copy_from_user((void *)dataBuf, (const void __user *)to_user_ptr(hdcp_tx_key.dataBuf), hdcp_tx_key.dataLen))
					{
						retval = -EFAULT;
						rtd_pr_hdcp_emerg("hdcp2 cert ioctl code=dataBuf copy_from_user failed!!!!!!!!!!!!!!!\n");
						if(dataBuf)
				...
				if(hdcp_tx_key.keyType == 2){
					retval = optee_hdcp2_burn_tx_key(dataBuf, hdcp_tx_key.dataLen);
				}else{
					retval = optee_hdcp14_burn_tx_key(dataBuf,hdcp_tx_key.dataLen);
				}
				if(dataBuf)
					kfree(dataBuf);
				dataBuf = NULL;
