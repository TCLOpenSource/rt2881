...
{
    while(len-- && (buff_len + 3 < PAGE_SIZE-1))
    {
...
static int i2c_rtk_xfer(void *dev_id, struct i2c_msg *msgs, int num)
{
    ...
    if(p_this->disable_switch_port == 0)
    {
        ...
        //add not go set_port func switch pinmux
        {
    ...
    if (p_adp->port >= 0)
    {
        //p_this->unset_port(p_this, p_adp->port);    //modify not go unset_port func switch pinmux as gpio,pin use as i2c
    }
...
err_occur:
    ...
    if (p_adp->port >= 0)
    {
        //p_this->unset_port(p_this, p_adp->port);     //modify not go unset_port func switch pinmux as gpio,pin as as i2c
    }
...
static int i2c_rtk_register_adapter(struct platform_device *pdev, rtk_i2c * p_phy, int port)
{
    ...
    if (p_adp)
    {
        ...
        p_adp->adap.dev.of_node = pdev->dev.of_node;
...
static int rtk_i2c_pm_resume(struct device *dev)
{
    ...
    if(m_I2CPowerCnt == 0)
    {
        int i2c_phy = BIT(1)|BIT(2)|BIT(3)|BIT(4);
        CRT_CLK_OnOff(I2C, CLK_ON, &i2c_phy);
    }
    m_I2CPowerCnt ++;
