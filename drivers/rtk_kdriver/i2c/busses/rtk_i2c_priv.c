...
int rtk_i2c_set_port(rtk_i2c * p_this, unsigned char port_id)
{
    ...
    if ((rd_reg(port->pin_mux[0].addr) & port->pin_mux[0].mask) !=port->pin_mux[0].i2c_val)
    {
    ...
    if ((rd_reg(port->pin_mux[1].addr) & port->pin_mux[1].mask) !=port->pin_mux[1].i2c_val)
    {
...
int rtk_i2c_set_spd(rtk_i2c * p_this, int KHz)
{
    ...
    */
    if (p_this->ph_en && !MISC_I2C_I2C1_PH_CR_get_ph_en(GET_IC_PH_CR(p_this))) {
    ...
    if (KHz >= 1000)
    {
        ...
        SET_IC_FS_SCL_LCNT(p_this, 0x11);
                               ...
                               unsigned int irq)
{
    ...
    if (hHandle != NULL)
    {
        ...
        sprintf(name, "I2C_PORT_%d", logical_id);
        if (pcb_mgr_get_enum_info_byname(name, &i2c_config) == 0)
                    hHandle->ph_en = (i2c_config >> 8) & 0x1;

        I2C_INFO("[ch_%d][phy_%d] create_rtk_i2c_handle: rtk_i2c_flags = %02x irq=%d ph_en = %d\n",
                    logical_id, phy_id, (uint32_t) rtk_i2c_flags, irq, hHandle->ph_en);
    }
