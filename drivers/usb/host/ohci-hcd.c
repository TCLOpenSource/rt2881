...
static int ohci_urb_enqueue (
	...
	if (ed->state == ED_IDLE) {
		...
		if (unlikely(tick_before(frame, next))) {
			/*
			*For issue https://jira.realtek.com/browse/ANEAP-1931?filter=-1
			*when the ed is unlinked from list and driver deliery a urb which has expired,
			*the urb will not be dealed and complete callback nevery called. 
			*/
			if(frame + length < next) {
				retval = -ETIMEDOUT;
				usb_hcd_unlink_urb_from_ep(hcd, urb);
				goto fail;
			}
