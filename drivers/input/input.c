...
static bool g_input_event_filtered = false;
static int is_input_event_filtered(void)
{
        if(unlikely(g_input_event_filtered))
                return 1;
        return 0;
}
static ssize_t input_event_filter_show(struct class *class,
                                struct class_attribute *attr, char *buf)
{
        snprintf(buf, PAGE_SIZE, "%s\n", g_input_event_filtered ?  "enable" : "disable");
        return strlen(buf);

}
static ssize_t input_event_filter_store(struct class *class,
                                struct class_attribute *attr, const char *buf, size_t count)
{
	char action_str[32] = {0};
	if (sscanf(buf, "%31s", action_str) == 1) {
		if(strcmp(action_str, "enable") == 0)
			g_input_event_filtered = true;
		else if(strcmp(action_str, "disable") == 0)
			g_input_event_filtered = false;
		else
			return -EINVAL;
...
static CLASS_ATTR_RW(input_event_filter);
...
static int __init input_init(void)
{
	...
	err = class_create_file(&input_class, &class_attr_input_event_filter);
	if(err) {
...
{
	class_remove_file(&input_class, &class_attr_input_event_filter);
	input_proc_exit();
