...
// if not CONFIG_REALTEK_MEMORY_MANAGEMENT, maybe_unused
static bool __maybe_unused cma_in_zone(gfp_t gfp)
{
	unsigned long size;
	phys_addr_t end;
	struct cma *cma;
			      ...
			      gfp_t gfp)
{
	...
	size = size_page = PAGE_ALIGN(pool_size);
	...
	if (rtk_record_insert(page_to_pfn(page) << PAGE_SHIFT,
		...
		(const void *)atomic_pool_expand, DVR_OWNER_NOID)) {
		goto record_delete;
	}
