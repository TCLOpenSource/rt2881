...
#if 0
#define UPPER_OOM_SCORE 600
#define LOWER_OOM_SCORE 200
static int rtk_mem_cgroup(struct kernfs_open_file *of, struct kernfs_node *parent, char *buf)
{
#if 0
	pid_t pid;
	int ret = 0;
	struct task_struct *task = NULL;
	struct kernfs_node *temp_node = NULL;

	/* Check if it is vaild pid */
	...
	if (task->signal->oom_score_adj < LOWER_OOM_SCORE) {
		pr_emerg("%s : %s in mem_bg, oom_score_adj %d\n", __func__,
		        task->comm, task->signal->oom_score_adj);
	    goto EXIT;
	}

	if (task->signal->oom_score_adj > UPPER_OOM_SCORE) {
		temp_node = kernfs_find_and_get_ns(parent, "upper", NULL);
		if (!temp_node) {
			ret = -ENODEV;
			goto EXIT;
		}

		temp_node = kernfs_find_and_get_ns(temp_node, "cgroup.procs", NULL);
		if (!temp_node) {
			ret = -ENODEV;
			goto EXIT;
		}
		...
		pr_emerg("%s : %s in mem_bg/upper, oom_score_adj %d\n", __func__,
		        task->comm, task->signal->oom_score_adj);
	} else {
		temp_node = kernfs_find_and_get_ns(parent, "lower", NULL);
		if (!temp_node) {
			ret = -ENODEV;
			goto EXIT;
		}
		...
		pr_emerg("%s : %s in mem_bg/lower, oom_score_adj %d\n", __func__,
		        task->comm, task->signal->oom_score_adj);
	}
...
EXIT:
	...
	if (task->signal->oom_score_adj <= LOWER_OOM_SCORE) {
		pr_emerg("%s : %s avoids to into mem_bg, oom_score_adj %d\n", __func__,
		        task->comm, task->signal->oom_score_adj);
		ret = -EPERM;
		goto EXIT;
	}

	if (strstr(task->comm, "launcherx")) {
		pr_emerg("%s : %s avoids to into mem_bg, oom_score_adj %d\n", __func__,
		        task->comm, task->signal->oom_score_adj);
		ret = -EPERM;
		goto EXIT;
				     ...
				     bool threadgroup)
{
	...
	if (strstr(parent->name, "mem_bg")) {
		ret = rtk_mem_cgroup(of, parent, buf);
		if (ret < 0)
			return ret;
	}
#endif
